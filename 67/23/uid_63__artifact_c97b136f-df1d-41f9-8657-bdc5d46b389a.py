...
SEARCH_TIMEOUT = 10.0
FETCH_TIMEOUT = 15.0
# Measured on 6c42c98a: 20 ToolInvocationTimeoutError each burned exactly 90s
# (1,357s, 15% of wall) because the platform tool_timeout is also 90s.
# Successful calls are p50 7.3s / p90 25.2s / p99 69.4s.
# NOT implicated in the 8a6e00db regression: 25 timeouts against 602 provider
# errors.
LLM_TIMEOUT = 45.0
...
def _is_vacuous_output(output: Any) -> bool:
    """Detect a structured payload that collapsed into "found nothing".

    Batch 6c42c98a: 5 of 24 structured runs emitted an all-empty payload such
    as {"countries": []} and scored exactly 0.000, against 0.578 for non-empty
    runs. The judge reads an empty array as wrong, not partial -- verdict on
    ...
    if isinstance(output, dict):
        ...
        if not values:
            return True
        return all(isinstance(item, (list, dict, str)) and len(item) == 0
                   for item in values)
...
def _vacuous_retry_allowed(deadline: float | None) -> bool:
    """One retry, and only with real slack left.

    Batch 8a6e00db showed what unbounded extra calls cost: halving the prompt
    budgets took llm calls from 13 to 24 per run, and the extra draws against a
    rate-limited pool produced 602 provider errors in 50 runs. So this retry is
    hard-gated: it fires at most once, and only when the clock can absorb a
    call at its measured p90 latency of 25.2s.
    """
    if deadline is None:
        return False
    return (deadline - time.monotonic()) >= _VACUOUS_RETRY_MIN_SLACK_SECONDS


_VACUOUS_RETRY_MIN_SLACK_SECONDS = 45.0
...
async def _v401_base_query(query: Query, deadline: float | None = None) -> Response:
    ...
    if query.output_schema is not None:
        ...
        if _is_vacuous_output(output) and _vacuous_retry_allowed(deadline):
            try:
                ...
                second = None
            if second is not None and not _is_vacuous_output(second):
                output = second
...
async def query(query: Query) -> Response:
    ...
    response = await _v401_base_query(query, _v401_start + _v401_total_budget())
    try:
