...
async def _do_search(query_text: str, ledger: EvidenceLedger):
    ...
    if _SEARCH_CALLS["n"] >= SEARCH_CALL_CAP:
        return ("# web_search: SEARCH BUDGET SPENT (" + str(SEARCH_CALL_CAP) + " calls). More "
                "searching is not going to surface it -- answer NOW from the numbered results "
                "you already have, citing [n] on every claim. A cited partial answer scores; "
                "another search does not.")
    _SEARCH_CALLS["n"] += 1
...
async def _do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger) -> str:
    ...
    if _SEARCH_CALLS["n"] >= SEARCH_CALL_CAP:
        return ("# read_page: SEARCH BUDGET SPENT (" + str(SEARCH_CALL_CAP) + " calls). Answer "
                "NOW from the numbered results you already have, citing [n] on every claim.")
    _SEARCH_CALLS["n"] += 1
    payload = None
    for _attempt in (0, 1):
        try:
...
def _citations_for(answer: str, ledger: EvidenceLedger) -> list[CitationRef]:
    ...
    for n in _cited_numbers(answer, len(ledger.rows)):
        ...
        key = (getattr(ref, "receipt_id", ""), getattr(ref, "result_id", ""),
               tuple((s.start, s.end) for s in slices) if slices else ())
...
_DIGEST_HOSTISH_RE = re.compile(r"\b(?:https?://|www\.)|\.(?:com|org|net|gov|edu)\b|\[PDF\]", re.I)
_SOURCE_WORD_RE = re.compile(r"\b(?:report|reports|table|tables|statistics|stats|data|dataset|page|pages|article|list|lists|site|website|database|archive|summary|overview|index|wikipedia|pdf|chart|figures|facts)\b", re.I)


_CATEGORY_WORD_RE = re.compile(
    r"^(?:regions?|governorates?|provinces?|districts?|municipalities|municipality|"
    r"cities|city|towns?|villages?|countries|country|states?|counties|county|"
    r"prefectures?|departments?|territories|territory|divisions?|zones?|"
    r"categories|category|sections?|contents?|navigation|menu|home|results?|"
    r"findings?|sources?|references?|notes?|examples?|names?|entries|entry)$", re.I)
