...
def _build_simple_query():
    """hk400 "tally+authority" — champion-v52 toolloop + tally-check repair + authority sweep.
    ...
    # ── stage 3d: tally-check repair ──────────────────────────────────────────────
    # A recurring judge complaint on count questions: the answer asserts "N X" while
    # enumerating a different number of X — either a stale count from the brief or a
    # member silently dropped during rewrite. Detection is fully deterministic:
    # extract count claims ("five states", "7 films"), count what the answer itself
    # enumerates, and only on a mismatch spend one bounded rewrite round telling the
    # model to recount — fix the number or add the missing cited members. Zero cost
    # when consistent; inherits the audit's regression guards.
    _TC_WORDS = {"one": 1, "two": 2, "three": 3, "four": 4, "five": 5, "six": 6,
                 "seven": 7, "eight": 8, "nine": 9, "ten": 10, "eleven": 11,
                 "twelve": 12, "thirteen": 13, "fourteen": 14, "fifteen": 15,
                 "sixteen": 16, "seventeen": 17, "eighteen": 18, "nineteen": 19,
                 "twenty": 20}
    _TC_CLAIM_RE = re.compile(
        r"\b(\d{1,3}|one|two|three|four|five|six|seven|eight|nine|ten|eleven|"
        r"twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|"
        r"twenty)\s+(?:qualifying\s+|such\s+|distinct\s+|different\s+)?"
        r"(states|countries|films|movies|companies|players|cities|books|albums|"
        r"artists|teams|species|languages|banks|universities|agencies|members|"
        r"entries|items|people|persons|winners|candidates|paintings|songs|novels|"
        r"episodes|seasons|counties|rivers|mountains|elements)\b", re.IGNORECASE)
    _TC_LIST_LINE_RE = re.compile(r"^\s*(?:[-*\u2022]|\d+[.)])\s+\S", re.MULTILINE)


    def _tally_claims(answer: str) -> list[tuple[int, str]]:
        out = []
        for m in _TC_CLAIM_RE.finditer(answer or ""):
            raw = m.group(1).lower()
            n = int(raw) if raw.isdigit() else _TC_WORDS.get(raw, 0)
            if 1 <= n <= 100:
                out.append((n, m.group(2).lower()))
        return out


    def _tally_enumerated(answer: str) -> int:
        return len(_TC_LIST_LINE_RE.findall(answer or ""))


    async def _tally_repair(question: str, answer: str, messages: list[dict],
                            ledger: EvidenceLedger, deadline: float) -> str:
        if (deadline - monotonic()) < 75.0 or _spend_left() <= AUDIT_MIN_USD:
            return answer
        claims = _tally_claims(answer)
        n_enum = _tally_enumerated(answer)
        if not claims or n_enum == 0:
            return answer                    # nothing to cross-check
        bad = [(n, noun) for n, noun in claims if n != n_enum]
        if not bad or any(n == n_enum for n, _ in claims):
            return answer                    # at least one claim matches the list
        n, noun = bad[0]
        order = (f"TALLY CHECK: the answer asserts {n} {noun} but its list "
                 f"enumerates {n_enum} entries. Recount against the evidence: "
                 "either correct the asserted number to match the verified list, "
                 "or add the missing members — each with its own [n] citation. "
                 "Use at most 2 tool calls if a member needs verification, then "
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        # stage 3d: asserted counts must match what the answer enumerates
        try:
            if _is_usable_answer(answer) and (deadline - monotonic()) > 75.0 \
                    and _spend_left() >= AUDIT_MIN_USD:
                counted = await _tally_repair(question, answer, messages,
                                              ledger, deadline)
                if _is_usable_answer(counted):
                    answer = counted
        except Exception:
            pass
...
def _build_complex_query():
        ...
        def _compile(self):
            ...
            class SecondPath:
                ...
                def _compile(self):
                    ...
                    from harnyx_miner_sdk.decorators import entrypoint
                    from harnyx_miner_sdk.query import CitationRef, CitationSlice, Query, Response
                    VERSION = 'v35.0-claim-evidence-graph'
                    LLM_LANE_A = 'openrouter'
                    ...
                    def _needs_set_completeness(question: str) -> bool:
                        ...
                        return bool(re.search('\\bwhich\\b', q, re.IGNORECASE)) and bool(_SET_CONNECTIVE_RE.search(q))
                    SET_RULE = "SET ANSWER: this question asks for a set. Missing a qualifying member scores the same as wrong — enumerate the pool, test EVERY member against EVERY condition, and name ALL qualifiers (each with its own citations per condition). Then give EVERY excluded member its own line with the condition it fails and its own [n] — not a single clause sweeping several names together, and not just the near-misses. Never claim 'the only X' unless the whole pool was checked; if your pool may be partial, still commit to every qualifier you verified. GET THE POOL FROM A LIST, NOT MEMBER-BY-MEMBER: your FIRST retrieval for a set question should hunt the authoritative roster/list/table that enumerates the whole pool (search it AS a list — '<pool subject> list', '<pool subject> table', 'list of <pool subject>' — and read_page it). Assembling the pool from separate per-member searches is how a run ends up with 3 of 6 qualifiers: the members you never thought to search for are invisible to you. Read the roster page first, then verify each member. ONE LIST PER PERIOD, THEN JOIN: when a condition has to hold across several periods — successive years, separate editions, or two parallel events — fetch ONE roster page per period and join them on the member: one list per period, not one lookup per member. A pool of 30+ members each needing several figures is a table-join, and per-member lookups will run out of turns long before the pool is covered. UNIVERSAL conditions ('in EVERY one of them', 'for BOTH parts', 'in ALL three periods'): check each candidate against EACH instance separately, with a citation per instance — one shared instance is not enough. If NO candidate survives every instance, then 'none' IS the answer: state it as a verified fact about the world with the per-instance citations that prove it."
                    _CLAIM_NUM_RE = re.compile('\\b(\\d{2,}(?:,\\d{3})*(?:\\.\\d+)?)\\b')

                    class ClaimNode:

                        def __init__(self, text: str, value: str, value_type: str, source_row: int, span_offset: int) -> None:
                            self.text = text
                            self.value = value
                    ...
                    class ClaimEvidenceGraph:
                        ...
                        def __init__(self) -> None:
                            ...
                            self._row_claims: dict[int, list[ClaimNode]] = {}
                        ...
                        def add(self, receipt_id: str, result_id: str, note_len: int, kind: str, spans: list[tuple[int, int]] | None, title: str='', url: str='', preview: str='') -> int:
                            ...
                            idx = len(self.rows)
                            self._ingest_claims(idx, (preview or '')[:1200])
                            return idx

                        def _ingest_claims(self, row_idx: int, text: str) -> None:
                            extracted: list[ClaimNode] = []
                            for m in _CLAIM_NUM_RE.finditer(text):
                                ...
                                node = ClaimNode(text=ctx[:160], value=raw, value_type='numeric', source_row=row_idx, span_offset=m.start())
                                extracted.append(node)
                            self._row_claims[row_idx] = extracted
                            self.claims.extend(extracted)

                        def source_value_set(self) -> set[str]:
                            return {c.value for c in self.claims if c.value_type == 'numeric'}

                        def validate_answer_value(self, value_str: str) -> tuple[bool, list[ClaimNode]]:
                            val = value_str.replace(',', '').strip()
                            if val in self.source_value_set():
                                return (True, [])
                            try:
                                target = float(val)
                            except ValueError:
                            ...
                            for c in self.claims:
                                ...
                                try:
                                    ...
                                    continue
                                if target != 0 and 0.1 < cv / target < 10:
                                    alts.append(c)
                            return (False, alts)

                        def claims_for_row(self, row_idx: int) -> list[ClaimNode]:
                            return self._row_claims.get(row_idx, [])

                        def support_digest(self, row_idx: int, limit: int=3) -> str:
                            claims = self._row_claims.get(row_idx, [])
                            if not claims:
                                return ''
                            parts: list[str] = []
                            seen: set[str] = set()
                            for c in claims:
                                if c.value in seen:
                                    continue
                                seen.add(c.value)
                                parts.append(f'Supports: {c.text}')
                                if len(parts) >= limit:
                                    break
                            return ' | '.join(parts)
                    ...
                    def _commit_tool_output(out, ledger: ClaimEvidenceGraph) -> str:
                        if isinstance(out, str):
                            return out
                        if not isinstance(out, ToolOutput):
                            return f'# tool crashed: {out}'
                    ...
                    async def _do_search(query_text: str, ledger: ClaimEvidenceGraph):
                        if not query_text.strip():
                            return '# web_search: empty query'
                        payload = None
                        fired: set[str] = set()
                    ...
                    async def _do_fetch(url: str, focus: str, question: str, ledger: ClaimEvidenceGraph) -> str:
                        if not url.strip():
                            return '# read_page: empty url'
                        payload = None
                        for _attempt in (0, 1):
                    ...
                    async def _run_tool(call, question: str, ledger: ClaimEvidenceGraph, deadline: float) -> str:
                        try:
                            args = json.loads(getattr(call, 'arguments', None) or '{}')
                        except Exception:
                            args = {}
                    ...
                    async def _preseed(question: str, set_question: bool, ledger: ClaimEvidenceGraph, deadline: float) -> str:
                        seeds = _seed_queries(question, set_question)
                        if not seeds or deadline - monotonic() < 40.0:
                            return ''
                        blocks: list = []
                    ...
                    async def _roster_prepass(question: str, ledger: ClaimEvidenceGraph, deadline: float) -> str:
                    ...
                    async def _loop(question: str, brief: str, ledger: ClaimEvidenceGraph, deadline: float, turn_cap: int, carry: list[dict] | None=None, allow_tools_in_wrapup: bool=False, extra_context: str='') -> tuple[str, list[dict]]:
                        if carry is not None:
                            messages = carry
                        else:
                            set_q = _needs_set_completeness(question)
                    ...
                    async def _audit_patch(question: str, answer: str, messages: list[dict], ledger: ClaimEvidenceGraph, deadline: float) -> str:
                        probe = f"""Audit the answer against the question. JSON only, keys: "unanswered_parts" (list; question elements not addressed), "uncited_facts" (list; load-bearing claims without [n]), "wrong_kind" (list; places where the named entity is a different KIND than the question asks — a person instead of a series, a duo instead of a show), "incomplete_roster" (list; THE MOST COMMON LOSS. If the question ranges over a candidate pool — a closed set that can be enumerated, or several conditions applied to a class — then: is the pool itself stated and plausibly COMPLETE, and does the answer give a verdict for EVERY member (qualifies / excluded because X, each cited)? Name any pool member the answer never mentions, and say so if the pool looks truncated — an answer naming 3 qualifiers when the pool holds 6 scores as WRONG, not partial), "thin_proof" (list; a qualifier lacking a per-condition citation, or a plausible near-miss candidate never addressed), "hand_waved_tally" (list; for a superlative/count/most-common question: the answer asserts a winner or a count WITHOUT showing the candidate table it was derived from. Phrases like 'among others', 'and several more', 'multiple X', or naming 2 examples to justify a count are all hand-waving — say so and name what the tally must list). Empty lists when clean.\n\nQuestion:\n{question}\n\nAnswer:\n{answer[:11000]}"""
                        try:
                            raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, 'Strict completeness auditor. JSON only.', probe, max_tokens=2200, timeout=max(8.0, min(AUDIT_TIMEOUT_S, deadline - monotonic() - 72.0)))
                            raw = re.sub('^```(?:json)?\\s*|\\s*```$', '', raw.strip(), flags=re.I | re.M)
                    ...
                    async def _verify_and_repair(question: str, answer: str, messages: list[dict], ledger: ClaimEvidenceGraph, deadline: float) -> str:
                        ...
                        if not isinstance(claims, list) or not claims:
                            return answer
                        source_vals = ledger.source_value_set()
                        weak: list[str] = []
                        ...
                        for c in claims:
                            ...
                            claim_nums = _CLAIM_NUM_RE.findall(text)
                            value_unattested = False
                            for raw in claim_nums:
                                val = raw.replace(',', '')
                                try:
                                    if float(val) >= 10 and val not in source_vals:
                                        value_unattested = True
                                        break
                                except ValueError:
                                    continue
                            unsupported = load_bearing and (not resolves or support in ('weak', 'none') or value_unattested)
                            if not unsupported:
                                continue
                            if value_unattested:
                                reason = 'contains numeric value not attested in any source evidence'
                            elif not resolves:
                                reason = 'uncited / citation does not resolve to evidence'
                            else:
                                reason = f'only {support}ly supported'
                    ...
                    def _citations_for(answer: str, ledger: ClaimEvidenceGraph) -> list[CitationRef]:
                    ...
                    def _ledger_digest(ledger: ClaimEvidenceGraph, char_cap: int=60000) -> str:
                        parts: list[str] = []
                        spent = 0
                        for i, row in enumerate(ledger.rows, start=1):
                            text = (row.get('preview') or '').strip()
                            if not text:
                                continue
                            block = f"[{i}] {row.get('title') or ''} ({row.get('url') or ''})\n{text}"
                            support = ledger.support_digest(i)
                            if support:
                                block += f'\n  [{support}]'
                            if spent + len(block) > char_cap:
                                break
                    ...
                    def _deterministic_answer(question: str, ledger: ClaimEvidenceGraph) -> str:
                        rows = [(i, r) for i, r in enumerate(ledger.rows, start=1) if (r.get('preview') or '').strip()]
                        if not rows:
                            return ''
                        out = ['Best-supported findings from the sources retrieved:']
                    ...
                    async def _write_from_digest(question: str, ledger: ClaimEvidenceGraph, deadline: float) -> str:
                        left = deadline - monotonic()
                        if left < 14.0:
                            return ''
                        digest = _ledger_digest(ledger)
                    ...
                    async def _exact_value_crosscheck(question: str, answer: str, ledger: ClaimEvidenceGraph, deadline: float) -> str:
                        digest = _ledger_digest(ledger, char_cap=48000)
                        if not digest.strip():
                            return answer
                        answer_clean = _CITE_NUM_RE.sub(' ', answer or '')
                        answer_nums = _CLAIM_NUM_RE.findall(answer_clean)
                        source_vals = ledger.source_value_set()
                        unattested_warnings: list[str] = []
                        for raw in answer_nums:
                            val = raw.replace(',', '')
                            if val in source_vals:
                                continue
                            try:
                                fval = float(val)
                                if fval < 10:
                                ...
                                continue
                            _, alts = ledger.validate_answer_value(val)
                            alt_str = ''
                            if alts:
                                alt_vals = []
                                seen_av: set[str] = set()
                                for a in alts[:4]:
                                    if a.value not in seen_av:
                                        seen_av.add(a.value)
                                        alt_vals.append(f'{a.value} (row {a.source_row}: {a.text[:60]})')
                                alt_str = '; nearby source values: ' + ', '.join(alt_vals)
                            unattested_warnings.append(f"VALUE '{val}' in the answer does NOT appear in any source row" + alt_str)
                        claim_warning_block = ''
                        if unattested_warnings:
                            claim_warning_block = '\n\nCLAIM-GRAPH ALERT: the following answer values are NOT attested in any source evidence row — they may be LLM extraction errors. Check these FIRST:\n- ' + '\n- '.join(unattested_warnings[:4])
                        system = "You verify ONE value in a finished research answer against numbered evidence. Do not rewrite or restyle the answer. Identify the single most load-bearing value the question turns on (the key number, date, count, percentage, or name). Check it against the evidence rows. Pay special attention to any CLAIM-GRAPH ALERT values below — these were flagged by deterministic validation as NOT present in ANY source. Reply on ONE line only: 'OK' if the answer's value is supported or you are not certain it is wrong; otherwise 'CORRECT: <exact old text> => <exact new text> [n]' where <new text> is copied verbatim from evidence row [n] and <old text> is copied verbatim from the answer. Correct ONLY a clear, evidence-supported error. When in doubt, reply OK."
                        user = f'QUESTION:\n{question}\n\nANSWER:\n{answer[:8000]}{claim_warning_block}\n\nEVIDENCE (numbered):\n{digest}'
                        try:
                    ...
                    async def _constraint_verify(question: str, answer: str, messages: list[dict], ledger: ClaimEvidenceGraph, deadline: float) -> str:
    ...
    class ReserveSolver:
        ...
        — so those questions were getting no completeness discipline at all."""
            ...
            def _clamp_closed_span(start, end, note_len: int) -> list[int]:
                start = max(0, min(int(start), note_len))
                end = max(start + 1, min(int(end), note_len))
                return [start, end]

            def _merge_closed_spans(shown: list[list[int]]) -> list[list[int]]:
                shown.sort()
                merged: list[list[int]] = []
                for s, e in shown:
                    if merged and s <= merged[-1][1]:
            ...
            def _grow_citation_spans(merged: list[list[int]], note_len: int, room: int) -> list[list[int]]:
                ...
                for w in merged:
                    ...
                    if pad:
                        ...
                        w[1] += right
                        w[0] = max(0, w[0] - (rest - right))
                return _merge_closed_spans(merged)

            def _row_citation_slices(row: dict) -> list[CitationSlice] | None:
                spans = row['spans']
                if not spans:
                    return None
                note_len = int(row['note_len'] or 0)
                shown: list[list[int]] = []
                for span in spans[:4]:
                    shown.append(_clamp_closed_span(span[0], span[1], note_len))
                retained = []
                for a, b in row.get('retained') or []:
                    retained.append(_clamp_closed_span(a, b, note_len))
                if retained:
                    shown = retained
                merged = _merge_closed_spans(shown)
                base = sum((e - s for s, e in merged))
                room = max(0, CITATION_MAX_REF_CHARS - base)
                merged = _grow_citation_spans(merged, note_len, room)
                slices = [CitationSlice(start=s, end=e) for s, e in merged if e > s]
                if not slices:
                    return None
                return slices
            ...
            class EvidenceLedger:
                ...
                def ref_for(self, number: int) -> CitationRef | None:
                    ...
                    if not row['receipt_id'] or not row['result_id']:
                        return None
                    slices = _row_citation_slices(row)
                    if not slices:
                        return None
                    return CitationRef(receipt_id=row['receipt_id'], result_id=row['result_id'], slices=slices)
            _WORD_RE = re.compile("[a-z0-9][a-z0-9'.\\-]{2,}")
        ...
        Champion lineages retry a failed search this way instead of giving up."""
            ...
            def _search_span_for_note(n_len: int):
                return [(0, min(max(SEARCH_EXCERPT_CHARS, 100), n_len))] if n_len >= 100 else [(0, n_len)] if n_len else None
            ...
            def _format_search_tool_output(query_text: str, receipt: str, results) -> ToolOutput:
                rows: list[dict] = []
                lines = [f'# web_search({query_text!r}): {len(results)} results']
                for item in results:
            ...
            def _fetch_plain_tool_output(url: str, receipt: str, rid: str, note: str) -> ToolOutput:
                row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': [(0, len(note))], 'title': url, 'url': url, 'preview': note[:1200], 'text': note}
                return ToolOutput(f'# read_page({url!r}) -> [{_SLOT.format(0)}] full page, {len(note)} chars\n{note}', [row])

            def _fetch_windowed_tool_output(url: str, receipt: str, rid: str, note: str, focus: str, question: str) -> ToolOutput:
                terms = _key_terms(question) | _key_terms(focus)
                windows = _best_windows(note, terms, FETCH_WINDOW_CHARS, k=FETCH_WINDOWS_PER_PAGE)
                row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': [(0, FETCH_HEAD_CHARS)] + list(windows), 'title': url, 'url': url, 'preview': note[windows[0][0]:windows[0][0] + 1200], 'text': note}
                head = note[:FETCH_HEAD_CHARS]
            ...
            async def _do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger) -> str:
                ...
                if not isinstance(rid, str) or not rid or (not note.strip()):
                    return f'# read_page({url!r}): no usable content'
                if len(note) <= FETCH_PLAIN_CHARS:
                    return _fetch_plain_tool_output(url, receipt, rid, note)
                return _fetch_windowed_tool_output(url, receipt, rid, note, focus, question)
            _SEC_TICKERS_URL = 'https://www.sec.gov/files/company_tickers.json'
            _SEC_SUBMISSIONS_URL = 'https://data.sec.gov/submissions/CIK{cik10}.json'
            _SEC_DOC_URL = 'https://www.sec.gov/Archives/edgar/data/{cik}/{accession}/{doc}'
            _SEC_FETCH_TIMEOUT_S = 26.0
        ...
        finding). Named-year miss -> None; no year -> most recent of that form."""
            ...
            def _sec_score_ticker_row(want: list[str], row: dict):
                title = str(row.get('title', ''))
                ticker = str(row.get('ticker', '')).lower()
                words = set(_sec_tokens(title))
                n_hit = sum((1 for w in want if w in words))
                if len(want) == 1 and ticker == want[0]:
                    ...
                    return None
                return (score, -len(title), str(row.get('cik_str', '')).zfill(10), title)

            def _sec_best_ticker_match(tickers: dict, company: str):
                want = _sec_tokens(company)
                best = None
                for row in tickers.values():
                    if not isinstance(row, dict):
                        continue
                    cand = _sec_score_ticker_row(want, row)
                    if cand is None:
                        continue
                    if best is None or cand > best:
                        best = cand
            ...
            async def _do_sec_filing(company: str, form: str, year: str, deadline: float) -> str:
                ...
                if not isinstance(tickers, dict):
                    return f'# sec_filing: EDGAR ticker index unavailable — {hint}'
                best = _sec_best_ticker_match(tickers, company)
                if best is None:
                    return f'# sec_filing({company!r}): no confident EDGAR match — {hint}'
                cik10, title = (best[2], best[3])
                subs = await _fetch_json(_SEC_SUBMISSIONS_URL.format(cik10=cik10), deadline)
            ...
            def _compile_page_grep_pattern(pat: str) -> re.Pattern:
                try:
                    return re.compile(pat, re.I)
                except re.error:
                    return re.compile(re.escape(pat), re.I)

            def _page_grep_hit_blocks(text: str, rx: re.Pattern) -> list[str]:
                out, seen_at = ([], [])
                for m in rx.finditer(text):
                    c = (m.start() + m.end()) // 2
                    if any((abs(c - prev) < PAGE_GREP_WINDOW // 2 for prev in seen_at)):
        ...
        second fetch: no new tool cost, and the page is already in memory."""
                ...
                if not pat:
                    return '# page_grep: empty pattern'
                out = _page_grep_hit_blocks(text, _compile_page_grep_pattern(pat))
                if not out:
                    return f'# page_grep({pat!r}) on [{n}]: no match in {len(text)} chars. Try a shorter or looser pattern.'
                return f'# page_grep({pat!r}) on [{n}] -> {len(out)} match(es) of {len(text)} chars' + ''.join(out)
            ...
            def _retain_span_bounds(i: int, q: str, row: dict, text: str) -> tuple[int, int]:
                a = max(0, i - RETAIN_MARGIN_CHARS)
                b = min(int(row.get('note_len') or len(text)), i + len(q) + RETAIN_MARGIN_CHARS)
                return (a, b)
        ...
        move uid210 makes when a retained span omits a numeric fact it asserted."""
                ...
                if len(kept) >= RETAIN_MAX_PER_ROW:
                    return f'# retain_evidence: [{n}] already has {len(kept)} retained excerpts'
                a, b = _retain_span_bounds(i, q, row, text)
                if b <= a:
                    return f'# retain_evidence: could not bound the excerpt in [{n}]'
                kept.append((a, b))
                return f'# retain_evidence: kept {b - a} chars of [{n}] around your quote. Cite [{n}] for that claim.'
            ...
            async def _dispatch_named_tool(name: str, args: dict, question: str, ledger: EvidenceLedger, deadline: float) -> str:
                if name == 'web_search':
                    return await _do_search(str(args.get('query') or ''), ledger)
                if name == 'read_page':
                    return await _do_fetch(str(args.get('url') or ''), str(args.get('focus') or ''), question, ledger)
            ...
            def _provider_pin_attempts(lane: str, model: str):
                _pin0 = _upstream(lane, model)
                return (_pin0, None) if _pin0 is not None else (None,)

            async def _chat_simple(lane: str, model: str, system: str, user: str, *, max_tokens: int, timeout: float, think: dict | None=None) -> str:
                if think is None:
                    think = _least_think(lane, model)
                payload = None
                for _pin in _provider_pin_attempts(lane, model):
                    try:
                        payload = await llm_chat(provider=lane, model=model, messages=[{'role': 'system', 'content': system}, {'role': 'user', 'content': user}], temperature=0.15, max_output_tokens=max_tokens, timeout=timeout, thinking=think, provider_extra=_pin)
                        break
                    except Exception:
        ...
        floor rejects it, a repair turn is spent, and the loop tries lane A again."""
            ...
            def _extract_brief_draft(raw: str) -> str:
                draft = raw
                cut = min((mm.start() for mm in (re.search('[#*_\\s]*(?:conditions|CHECKLIST)[#*_\\s]*:', raw, re.IGNORECASE), re.search('^[ \\t]*[#*_>]{0,4}[ \\t]*(?:conditions|CHECKLIST)[ \\t]*[#*_]{0,3}[ \\t]*$', raw, re.IGNORECASE | re.MULTILINE)) if mm is not None), default=None)
                if cut is not None:
                    draft = raw[:cut]
            ...
            def _brief_block_from_raw(raw: str) -> str:
        ...
        heavy batch; the loop then verifies the load-bearing facts."""
                ...
                if not raw:
                    return ('', '')
                return (_extract_brief_draft(raw), _brief_block_from_raw(raw))
            _SEED_TOKEN_RE = re.compile("[A-Za-z0-9][A-Za-z0-9.\\-']+")
            _SEED_STOP = frozenset('name list give tell show find identify please could would you your can may might should must let make sure both also'.split())
            MAX_SEED_QUERIES = 3
            ...
            def _candidate_from_finish_message(llm, msg) -> str:
                candidate = (getattr(llm, 'raw_text', None) or '').strip()
                if not candidate:
                    content = getattr(msg, 'content', None)
                    if isinstance(content, str):
            ...
            async def _collect_tool_batch_results(run_calls, question: str, ledger: EvidenceLedger, deadline: float, tool_budget: float) -> list:
                tool_tasks = [asyncio.ensure_future(_run_tool(c, question, ledger, deadline)) for c in run_calls]
                try:
                    await asyncio.wait(tool_tasks, timeout=tool_budget)
                except Exception:
            ...
            def _append_tool_turn_messages(messages: list[dict], run_calls, results, overflow_calls, ledger: EvidenceLedger) -> None:
                for call_result in zip(run_calls, results):
                    call = call_result[0]
                    body = _commit_tool_output(call_result[1], ledger)
                    messages.append({'role': 'tool', 'tool_call_id': call.id, 'content': body})
            ...
            async def _loop(question: str, brief: str, ledger: EvidenceLedger, deadline: float, turn_cap: int, carry: list[dict] | None=None, allow_tools_in_wrapup: bool=False) -> tuple[str, list[dict]]:
                ...
                for turn in range(1, turn_cap + 1):
                    ...
                    calls = getattr(msg, 'tool_calls', None) or ()
                    if not calls:
                        candidate = _candidate_from_finish_message(llm, msg)
                        if not _is_usable_answer(candidate):
                            if repairs_left > 0 and deadline - monotonic() > MIN_TAIL_S + 10.0:
                                repairs_left -= 1
                                messages.append({'role': 'system', 'content': _REPAIR_ORDER})
                    ...
                    run_calls = calls[:8]
                    tool_budget = max(5.0, min(FETCH_TIMEOUT_S * 2 + 6.0, deadline - monotonic() - MIN_TAIL_S))
                    results = await _collect_tool_batch_results(run_calls, question, ledger, deadline, tool_budget)
                    _append_tool_turn_messages(messages, run_calls, results, calls[8:], ledger)
                return (answer, messages)

            def _audit_gaps_from_report(report) -> tuple[list[str], list[str]]:
                gaps: list[str] = []
                roster_gaps: list[str] = []
                if isinstance(report, dict):
                    for key in ('incomplete_roster', 'hand_waved_tally', 'unanswered_parts', 'uncited_facts', 'wrong_kind', 'thin_proof'):
            ...
            async def _audit_patch(question: str, answer: str, messages: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
                ...
                try:
                    ...
                    return answer
                gaps, roster_gaps = _audit_gaps_from_report(report)
                if not gaps or deadline - monotonic() < 70.0:
                    return answer
                order = _audit_rewrite_order(gaps, roster_gaps)
                messages.append({'role': 'system', 'content': order})
                patched, _ = await _loop(question, '', ledger, deadline, AUDIT_EXTRA_TURNS + 1, carry=messages, allow_tools_in_wrapup=True)
                patched = patched.strip()
                if not _is_usable_answer(patched) or len(patched) < int(len(answer) * 0.6):
            ...
            def _strip_code_fence(raw: str) -> str:
                return re.sub('^```(?:json)?\\s*|\\s*```$', '', (raw or '').strip(), flags=re.I | re.M).strip()
            _CITE_NUM_RE = re.compile('\\[([0-9][0-9,\\s\\-]*)\\]')
        ...
        cannot drop early [n]s off the front of a truncated message window."""
            ...
            def _lead_segment_is_noise(seg: str) -> bool:
                if len(seg) < 30 or len(seg) > 400:
                    return True
                if _SENTENCEY_RE.search(seg) is None:
                    return True
            ...
            def _trim_lead_text(out: str, limit: int) -> str:
                if len(out) > limit:
                    cut = out.rfind(' ', 0, limit)
                    out = out[:cut if cut > 60 else limit].rstrip(' ,;:-')
                return out
            ...
            def _informative_lead(preview: str, limit: int=280) -> str:
                ...
                for chunk in re.split('(?<=[.!?])\\s+|\\n+', _SRC_FOOTNOTE_RE.sub('', preview or '')):
                    ...
                    pass
                out = ' '.join(kept).strip()
                return _trim_lead_text(out, limit)
        ...
        beats a refusal."""
            ...
            async def _digest_lane_write(lane: str, model: str, convo: list[dict], budget: float) -> str:
                payload = None
                for _p in _provider_pin_attempts(lane, model):
                    try:
                        payload = await llm_chat(provider=lane, model=model, messages=convo, temperature=0.15, max_output_tokens=2600, timeout=budget, thinking=_least_think(lane, model), provider_extra=_p)
                        break
                    except Exception:
            ...
            def _digest_lane_budget(i: int, left: float) -> float | None:
                budget = min(RESCUE_TIMEOUT_S, left - DIGEST_TAIL_S)
                if i == 0:
                    budget = min(budget, max(12.0, left - 14.0 - DIGEST_TAIL_S))
                if budget < 8.0:
        ...
        message window."""
                ...
                for i, lane_model in enumerate(lanes):
                    ...
                    if budget is None:
                        return ''
                    try:
                        text = await _digest_lane_write(lane_model[0], lane_model[1], convo, budget)
                    except Exception:
                        continue
                    if _is_usable_answer(text):
                        return text
            ...
            def _accept_schema_value(value, schema) -> tuple[bool, object]:
                if _matches_schema_shape(value, schema):
                    return (True, value)
                if isinstance(value, dict) and len(value) == 1:
                    inner = list(value.values())[0]
            ...
            async def _schema_output(question: str, answer: str, schema, deadline: float) -> object | None:
                ...
                for lane, model in ((LLM_LANE_A, SCHEMA_MODEL), (LLM_LANE_A, RESORT_MODEL), (LLM_LANE_B, LOOP_MODEL_B)):
                    ...
                    try:
                        ...
                        raw = _strip_code_fence(raw)
                        value = json.loads(raw)
                        ok, accepted = _accept_schema_value(value, schema)
                        if ok:
                            return accepted
                    except Exception:
                        continue
        ...
        while a pasted digest reads as a contract violation and is scored as garbage."""
            ...
            def _first_non_null_union_sub(schema: dict):
                for key in ('anyOf', 'oneOf', 'allOf'):
                    branch = schema.get(key)
                    if isinstance(branch, list) and branch:
                        for sub in branch:
            ...
            def _coerce_array_value(answer: str, schema: dict, depth: int):
                items = schema.get('items') or {}
                parts = [p.strip(' -*\t') for p in re.split('[\\n;]|,(?![^(]*\\))', answer or '')]
                parts = [p[:400] for p in parts if p][:20]
                if not parts:
            ...
            def _coerce_object_value(answer: str, schema: dict, depth: int):
                props = schema.get('properties') or {}
                required = schema.get('required') or list(props.keys())
                out = {}
                for key in required:
        ...
        """
                ...
                kind = _schema_kind(schema)
                if not kind:
                    sub = _first_non_null_union_sub(schema)
                    if sub is not None:
                        return _coerce_to_schema(answer, sub, depth + 1)
                    kind = 'string'
                if kind == 'array':
        ...
        that carries an [n]: that is a real answer, however it opens."""
            ...
            async def _note_tooling_budget() -> None:
                try:
                    info = await tooling_info(timeout=10.0)
                    _spend_note(info)
                except Exception:
            ...
            async def _rescue_answer_ladder(question: str, answer: str, draft: str, ledger: EvidenceLedger, deadline: float) -> str:
                if not _is_usable_answer(answer) and ledger.rows:
                    try:
                        rescued = await _write_from_digest(question, ledger, deadline)
                        if _is_usable_answer(rescued):
            ...
            def _polish_answer_text(answer: str, question: str) -> str:
                answer = _normalize_brackets(answer)
                answer = _strip_lead_narration(answer)
            ...
            def _safe_citations_for(answer: str, ledger: EvidenceLedger) -> list[CitationRef]:
                try:
                    return _citations_for(answer, ledger)
                except Exception:
                    return []

            async def _structured_response(query: Query, question: str, answer: str, citations: list[CitationRef], ledger: EvidenceLedger, deadline: float) -> Response | None:
                structured = None
                try:
                    structured = await _schema_output(question, answer, query.output_schema, deadline)
                except Exception:
            ...
            async def _solve(query: Query, question: str) -> Response:
                ...
                answer = await _rescue_answer_ladder(question, answer, draft, ledger, deadline)
                citations = _safe_citations_for(answer, ledger)
                answer = _polish_answer_text(answer, question)
                text = _cap(answer) or f'Best-effort answer unavailable for: {question[:400]}'
                if query.output_schema is not None:
                    structured_resp = await _structured_response(query, question, answer, citations, ledger, deadline)
    ...
    _TAG_209DB624="209db624ca6043ceb4477a124919ceec"
    import logging as _tag_logging_209db624
    _tag_logging_209db624.getLogger("miner.tag").debug("tag=%s", _TAG_209DB624)
    return query


_SIMPLE_QUERY = _build_simple_query()
_COMPLEX_QUERY = _build_complex_query()
...
class ComplexAgent:
    async def query(self, query: Query) -> Response:
        return await _COMPLEX_QUERY(query)
...
def _query_text(query: Query) -> str:
    ...
    if isinstance(value, str) and value.strip():
        return value.strip()
    value = getattr(query, "question", None)
    if isinstance(value, str) and value.strip():
        return value.strip()
    value = getattr(query, "content", None)
    if isinstance(value, str) and value.strip():
        return value.strip()
    try:
...
def _stable_bucket(text: str) -> int:
    sample = text.encode("utf-8", "ignore")[:4096]
    return int(hashlib.blake2s(sample, digest_size=8, person=b"dsigaa11").hexdigest(), 16)


def _branch_name(query: Query) -> str:
    text = _query_text(query)
    lower = text.lower()
    bucket = _stable_bucket(text)
    has_schema = getattr(query, "output_schema", None) is not None
    format_terms = (
        "json", "schema", "table", "csv", "array", "list", "rank", "sort",
        "chronological", "alphabet", "comma", "exact format", "nothing else",
        "return only", "ordered", "top ", "format as", "under ", "words",
    )
    source_terms = (
        "according to", "official", "cite", "citation", "source", "filing",
        "annual report", "sec", "pdf", "press release", "transcript", "dataset",
        "census", "wikipedia", "walkoffame", "iihs", "irs", "epa", "fda",
    )
    arithmetic_terms = (
        "amount", "date", "year", "percent", "%", "number", "total",
        "population", "between", "before", "after", "latest", "highest",
        "lowest", "compare", "difference", "more than", "less than", "largest",
        "smallest", "median", "average", "revenue", "ratio", "earliest",
    )
    roster_terms = (
        "which of the following", "among", "members", "actors", "counties",
        "municipalities", "companies", "games", "states", "parishes", "schools",
        "teams", "films", "songs", "albums", "airports", "candidates",
    )
    shaped = any(term in lower for term in format_terms)
    sourced = any(term in lower for term in source_terms)
    arithmetic = any(term in lower for term in arithmetic_terms)
    roster = any(term in lower for term in roster_terms)
    compact = len(text) < 240 and ("?" in text or arithmetic or roster)

    if has_schema and (shaped or roster):
        return "complex" if bucket % 11 in (0, 3, 5, 8) else "simple"
    if sourced and roster:
        return "complex" if bucket % 7 in (1, 2, 6) else "simple"
    if shaped and arithmetic:
        return "complex" if bucket % 9 in (2, 4, 7) else "simple"
    if sourced and arithmetic:
        return "complex" if bucket % 8 in (0, 5) else "simple"
    if shaped or roster:
        return "complex" if bucket % 6 in (1, 4) else "simple"
    if compact:
        return "complex" if bucket % 5 in (1, 3) else "simple"
    return "complex" if bucket % 10 in (1, 6, 9) else "simple"


@entrypoint("query")
async def query(query: Query) -> Response:
    if _branch_name(query) == "simple":
        return await _SIMPLE_AGENT.query(query)
    return await _COMPLEX_AGENT.query(query)
