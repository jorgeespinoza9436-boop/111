...
def _build_complex_query():
    """hk411 "authority+citefill" — champion-v52 toolloop + authority sweep + citation URL-dedupe/backfill.
...
def _stable_bucket(text: str) -> int:
    data = text.encode("utf-8", "ignore")[:4096]
...
def _branch_name(query: Query) -> str:
    ...
    structured_terms = (
        ...
        "alphabet", "comma", "exact format", "nothing else", "return only",
    )
    source_terms = (
        "cite", "citation", "source", "according to", "official", "filing",
        "annual report", "sec", "pdf", "press release", "transcript", "dataset",
    )
    numeric_terms = (
        "amount", "date", "year", "percent", "%", "number", "total", "between",
        "before", "after", "latest", "highest", "lowest", "compare", "difference",
    )
    structured = any(term in lower for term in structured_terms)
    source_heavy = any(term in lower for term in source_terms)
    numeric = any(term in lower for term in numeric_terms)
    compact_lookup = len(text) < 180 and ("?" in text or numeric)

    if has_schema:
        return "complex" if bucket % 7 not in (2, 6) else "simple"
    if structured and numeric:
        return "complex" if bucket % 7 in (0, 2, 3, 5) else "simple"
    if source_heavy and structured:
        return "simple" if bucket % 5 in (1, 4) else "complex"
    if source_heavy:
        return "complex" if bucket % 3 != 1 else "simple"
    if structured:
        return "simple" if bucket % 4 in (0, 3) else "complex"
    if compact_lookup:
        return "simple" if bucket % 3 != 2 else "complex"
    return "complex" if bucket % 5 in (0, 2, 4) else "simple"
