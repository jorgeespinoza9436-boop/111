"""hk402 "valrep+premise" — champion-v52 toolloop + value-support repair + premise-verification sweep.
...
# ── stage 3g: premise-verification sweep ──────────────────────────────────────
...
# missing premise, then a bounded rewrite. Fires narrowly — most questions have
# their premises covered by the seed searches already.
_PR_ENTITY_RE = re.compile(
...
def _premise_missing(premises: list[str], ledger: EvidenceLedger) -> list[str]:
    texts = [((r.get("text") or "") + " " + (r.get("preview") or "")).casefold()
...
async def _premise_sweep(question: str, answer: str, messages: list[dict],
    ...
    target = missing[0]
    try:
        out = await asyncio.wait_for(_do_search(target, ledger),
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # stage 3g: the question's named subjects must appear in evidence
    try:
        if _is_usable_answer(answer) and (deadline - monotonic()) > 75.0 \
                and _spend_left() >= AUDIT_MIN_USD:
            premised = await _premise_sweep(question, answer, messages,
                                            ledger, deadline)
