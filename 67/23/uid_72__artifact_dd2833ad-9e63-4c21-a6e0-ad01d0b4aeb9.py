...
class OakArch_66de17:
    ...
    def _compile(self):
        ...
        CITATION_BLOCK_MIN_ROWS = 4
        CITATION_BLOCK_MAX_CHARS = 40000
        CITATION_BLOCK_SCAN_CHARS = 60000
        CITATION_BLOCK_MAX_LINES = 400
        CITATION_BLOCK_ROW_MARKS = 2
        CITATION_BLOCK_MIN_KEEP_CHARS = 100
        ...
        def _block_extent(note: str, start: int, end: int) -> tuple[int, int] | None:
            src_len = len(note)
            if src_len <= 0:
                return None
            start = max(0, min(int(start), src_len))
            end = max(start, min(int(end), src_len))
            lo = max(0, start - CITATION_BLOCK_SCAN_CHARS)
            hi = min(src_len, end + CITATION_BLOCK_SCAN_CHARS)
            window = note[lo:hi]
            ...
            if not bounds:
                return None
            is_row = [window[a:b].count('|') >= CITATION_BLOCK_ROW_MARKS for a, b in bounds]
            seed_lo, seed_hi = (start - lo, end - lo)
            seeds = [i for i, (a, b) in enumerate(bounds) if is_row[i] and a < seed_hi and (seed_lo < b)]
            if not seeds:
                return None
            first = seeds[0]
            last = seeds[-1]
            steps = 0
            while first > 0 and is_row[first - 1] and (steps < CITATION_BLOCK_MAX_LINES):
                first -= 1
                steps += 1
            steps = 0
            while last + 1 < len(bounds) and is_row[last + 1] and (steps < CITATION_BLOCK_MAX_LINES):
                last += 1
                steps += 1
            if last - first + 1 < CITATION_BLOCK_MIN_ROWS:
                return None
            block_start = max(0, min(src_len, lo + bounds[first][0]))
            block_end = max(block_start, min(src_len, lo + bounds[last][1]))
            if block_end - block_start > CITATION_BLOCK_MAX_CHARS:
                block_end = block_start + CITATION_BLOCK_MAX_CHARS
            return (block_start, block_end)

        def _widen_one_block(by_source: dict[str, dict[str, object]], source_order: list[str]) -> None:
            best_key: str | None = None
            best: tuple[int, int] | None = None
            for key in source_order:
                entry = by_source[key]
                note = str(entry['meta']['note'])
                for start, end in list(entry['spans']):
                    extent = _block_extent(note, start, end)
                    if extent is None:
                        continue
                    merged = (min(int(start), extent[0]), max(int(end), extent[1]))
                    if merged[1] - merged[0] <= int(end) - int(start):
                        continue
                    if best is None or merged[1] - merged[0] > best[1] - best[0]:
                        best, best_key = (merged, key)
            if best is None or best_key is None:
                return
            entry = by_source[best_key]
            limit = int(entry['src_len'])
            current = list(entry['spans'])
            start = max(0, best[0])
            end = min(limit, best[1])
            if end - start > CITATION_BLOCK_MAX_CHARS:
                end = start + CITATION_BLOCK_MAX_CHARS
            spent = sum((e - s for k in source_order for s, e in by_source[k]['spans']))
            room = CITATION_BUDGET_CHARS - spent
            held = sum((e - s for s, e in current))
            grown = _merge_spans(current + [(start, end)])
            overflow = sum((e - s for s, e in grown)) - held - room
            if overflow > 0:
                end -= overflow
                if end - start < CITATION_BLOCK_MIN_KEEP_CHARS:
                    return
                grown = _merge_spans(current + [(start, end)])
                if sum((e - s for s, e in grown)) - held > room:
                    return
            entry['spans'] = [(max(0, s), min(limit, e)) for s, e in grown if min(limit, e) > max(0, s)] or current
        ...
        class _SourceSurface:
            ...
            def refs(self, answer_text: str) -> tuple[CitationRef, ...]:
                ...
                _widen_one_block(by_source, source_order)
                ...
                for key in source_order:
                    ...
                    meta = entry['meta']
                    limit = int(entry['src_len'])
                    spans = [(max(0, s), min(limit, e)) for s, e in entry['spans'] if min(limit, e) > max(0, s)]
...
class HingePeg_66de17:

    @staticmethod
    def _maple_side_66de17() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_MAPLE_RUN_66de17 = MapleArch_66de17()._compile()
_OAK_RUN_66de17 = OakArch_66de17()._compile()
_HINGE_PEG_66de17 = HingePeg_66de17()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _HINGE_PEG_66de17._maple_side_66de17():
        return await _MAPLE_RUN_66de17(query)
    return await _OAK_RUN_66de17(query)
