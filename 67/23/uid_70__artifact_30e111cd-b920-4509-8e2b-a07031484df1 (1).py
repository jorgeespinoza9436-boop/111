    ...
    def _compile(self):
        ...
        class SecondPath:
            ...
            def _compile(self):
                ...
                from harnyx_miner_sdk.decorators import entrypoint
                from harnyx_miner_sdk.query import CitationRef, CitationSlice, Query, Response
                VERSION = 'v34.0-sourceaware'
                LLM_PROVIDER = 'openrouter'
                LOOP_MODEL_A = 'z-ai/glm-5.2'
                LOOP_MODEL_B = 'deepseek/deepseek-v3.2'
                LOOP_MODEL_C = 'openai/gpt-oss-120b'
                LOOP_LADDER = (LOOP_MODEL_A, LOOP_MODEL_B, LOOP_MODEL_C)
                AUDIT_MODEL = 'openai/gpt-oss-120b'
                SCHEMA_MODEL = 'openai/gpt-oss-120b'
                RESORT_MODEL = 'deepseek/deepseek-v3.2'
                SEARCH_PROVIDER = 'parallel'
                ...
                BRIEF_TIMEOUT_S = 50.0
                TURN_TIMEOUT_S = 75.0
                FALLBACK_MAX_PAYLOAD_CHARS = 300000
                AUDIT_TIMEOUT_S = 28.0
                SEARCH_TIMEOUT_S = 18.0
                FETCH_TIMEOUT_S = 16.0
                WRAPUP_AT_S = 90.0
                ...
                SOURCE_REPAIR_MIN_S = 132.0
                CITE_REPAIR_MIN_S = 96.0
                DIGEST_TAIL_S = 14.0
                SEARCH_EXCERPT_CHARS = 550
                FETCH_HEAD_CHARS = 3000
                FETCH_WINDOW_CHARS = 3600
                ...
                def _needs_set_completeness(question: str) -> bool:
                    ...
                    return bool(re.search('\\bwhich\\b', q, re.IGNORECASE)) and bool(_SET_CONNECTIVE_RE.search(q))
                SET_RULE = "SET ANSWER: this question asks for a set. Missing a qualifying member scores the same as wrong — enumerate the pool, test EVERY member against EVERY condition, and name ALL qualifiers (each with its own citations per condition). Then give EVERY excluded member its own line with the condition it fails and its own [n] — not a single clause sweeping several names together, and not just the near-misses. Never claim 'the only X' unless the whole pool was checked; if your pool may be partial, still commit to every qualifier you verified. GET THE POOL FROM A LIST, NOT MEMBER-BY-MEMBER: your FIRST retrieval for a set question should hunt the authoritative roster/list/table that enumerates the whole pool (search it AS a list — '<pool subject> list', '<pool subject> table', 'list of <pool subject>' — and read_page it). Assembling the pool from separate per-member searches is how a run ends up with 3 of 6 qualifiers: the members you never thought to search for are invisible to you. Read the roster page first, then verify each member. ONE LIST PER PERIOD, THEN JOIN: when a condition has to hold across several periods — successive years, separate editions, or two parallel events — fetch ONE roster page per period and join them on the member: one list per period, not one lookup per member. A pool of 30+ members each needing several figures is a table-join, and per-member lookups will run out of turns long before the pool is covered. UNIVERSAL conditions ('in EVERY one of them', 'for BOTH parts', 'in ALL three periods'): check each candidate against EACH instance separately, with a citation per instance — one shared instance is not enough. If NO candidate survives every instance, then 'none' IS the answer: state it as a verified fact about the world with the per-instance citations that prove it."
                _URL_HOST_RE = re.compile('https?://(?:www\\.)?([A-Za-z0-9.\\-]+)')
                _BARE_DOMAIN_RE = re.compile('\\b((?:[A-Za-z0-9][A-Za-z0-9\\-]{1,30}\\.)+(?:com|org|net|gov|edu|int|io))\\b')
                _NAMED_SOURCES_RAW = (('Box Office Mojo', 'box\\s*office\\s*mojo', True, ('boxofficemojo',)), ('The Numbers', '\\bthe[-\\s]numbers\\b', True, ('the-numbers',)), ('IMDb', '\\bimdb\\b', True, ('imdb',)), ('Rotten Tomatoes', 'rotten\\s*tomatoes', True, ('rottentomatoes',)), ('Metacritic', '\\bmetacritic\\b', True, ('metacritic',)), ('Wikipedia', '\\bwikipedia\\b', True, ('wikipedia',)), ('Britannica', '\\bbritannica\\b', True, ('britannica',)), ('Billboard', '\\bbillboard\\b', True, ('billboard',)), ('Nielsen', '\\bnielsen\\b', True, ('nielsen',)), ('Goodreads', '\\bgoodreads\\b', True, ('goodreads',)), ('Guinness World Records', 'guinness\\s*world', True, ('guinnessworldrecords',)), ('SEC EDGAR', '\\bedgar\\b|securities and exchange commission|\\b10-?K\\b|\\b10-?Q\\b|\\b8-?K\\b|\\bDEF\\s*14A\\b|\\b20-?F\\b', True, ('sec.gov',)), ('PubMed', '\\bpubmed\\b', True, ('pubmed', 'ncbi.nlm.nih.gov')), ('arXiv', '\\barxiv\\b', True, ('arxiv',)), ('World Bank', 'world\\s*bank', True, ('worldbank',)), ('OECD', '\\bOECD\\b', False, ('oecd',)), ('IMF', 'international monetary fund', True, ('imf.org',)), ('Eurostat', '\\beurostat\\b', True, ('ec.europa.eu', 'eurostat')), ('US Census Bureau', 'census bureau|\\bu\\.?s\\.? census\\b', True, ('census.gov',)), ('Bureau of Labor Statistics', 'bureau of labor statistics', True, ('bls.gov',)), ('CDC', 'centers for disease control', True, ('cdc.gov',)), ('FDA', 'food and drug administration|\\bFDA\\b', False, ('fda.gov',)), ('NASA', '\\bNASA\\b', False, ('nasa.gov',)), ('NOAA', '\\bNOAA\\b', False, ('noaa.gov',)), ('USGS', '\\bUSGS\\b|geological survey', True, ('usgs.gov',)), ('World Health Organization', 'world health organization', True, ('who.int',)), ('FIFA', '\\bFIFA\\b', False, ('fifa.com',)), ('UEFA', '\\bUEFA\\b', False, ('uefa.com',)), ('Olympedia', '\\bolympedia\\b', True, ('olympedia',)), ('ESPN', '\\bESPN\\b', False, ('espn',)), ('Baseball-Reference', 'baseball[-\\s]reference', True, ('baseball-reference',)), ('Transfermarkt', '\\btransfermarkt\\b', True, ('transfermarkt',)), ('SteamDB', '\\bsteamdb\\b|\\bsteam\\s*charts\\b', True, ('steamdb', 'steampowered')), ('Discogs', '\\bdiscogs\\b', True, ('discogs',)), ('AllMusic', '\\ballmusic\\b', True, ('allmusic',)), ('Reuters', '\\breuters\\b', True, ('reuters',)), ('Bloomberg', '\\bbloomberg\\b', True, ('bloomberg',)), ('Associated Press', 'associated press', True, ('apnews',)), ('BBC', '\\bBBC\\b', False, ('bbc.co', 'bbc.com')), ('The Guardian', 'the guardian', True, ('theguardian',)), ('New York Times', 'new york times', True, ('nytimes',)), ('Forbes', '\\bforbes\\b', True, ('forbes',)), ('Statista', '\\bstatista\\b', True, ('statista',)))
                _SOURCE_TABLE: list = []
                for _label, _pat, _ci, _hosts in _NAMED_SOURCES_RAW:
                    _SOURCE_TABLE.append((_label, re.compile(_pat, re.IGNORECASE if _ci else 0), _hosts))
                ...
                def _host_of(url: str) -> str:
                    ...
                    if m is None:
                        m = _BARE_DOMAIN_RE.search(u)
                    return m.group(1).lower().strip('.') if m is not None else ''

                def _required_sources(question: str) -> list:
                    q = ' '.join((question or '').split())
                    if not q:
                        return []
                    out: list = []
                    hosts_seen: list = []
                    for host in _URL_HOST_RE.findall(q) + _BARE_DOMAIN_RE.findall(q):
                        h = host.lower().strip('.')
                        if h.startswith('www.'):
                            h = h[4:]
                        if not h:
                            continue
                        if any((h == s or h.endswith('.' + s) or s.endswith('.' + h) for s in hosts_seen)):
                            continue
                        hosts_seen.append(h)
                        out.append((h, (h,)))
                    labels_seen: set = set()
                    for label, rx, frags in _SOURCE_TABLE:
                        if len(out) >= MAX_REQUIRED_SOURCES:
                            break
                        key = label.lower()
                        if key in labels_seen or rx.search(q) is None:
                            continue
                        if any((frag in s for frag in frags for s in hosts_seen)):
                            continue
                        labels_seen.add(key)
                        out.append((label, frags))
                    return out[:MAX_REQUIRED_SOURCES]
                ...
                class SourceAwareLedger:
                    ...
                    def require(self, pairs) -> None:
                        for pair in pairs or ():
                            self.required.append({'label': str(pair[0]), 'hosts': tuple((str(h).lower() for h in pair[1])), 'found': []})

                    def _record_provenance(self, host: str, number: int) -> None:
                        if not host:
                            return
                        for req in self.required:
                            for frag in req['hosts']:
                                if frag in host:
                                    req['found'].append(number)
                                    break

                    def source_gap_report(self) -> list:
                        return [req['label'] for req in self.required if not req['found']]

                    def coverage_gaps(self, answer: str) -> list:
                        cited = set(_cited_numbers(answer or '', len(self.rows)))
                        out: list = []
                        for req in self.required:
                            hits = req['found']
                            if hits and (not cited.intersection(hits)):
                                out.append((req['label'], hits[:4]))
                        return out

                    def priority_numbers(self) -> set:
                        keep: set = set()
                        for req in self.required:
                            keep.update(req['found'])
                        return keep
                    ...
                    def add(self, receipt_id: str, result_id: str, note_len: int, kind: str, spans: list[tuple[int, int]] | None, title: str='', url: str='', preview: str='') -> int:
                        self.rows.append({'receipt_id': receipt_id, 'result_id': result_id, 'note_len': note_len, 'kind': kind, 'title': (title or '')[:160], 'url': (url or '')[:300], 'preview': (preview or '')[:1200], 'spans': spans, 'host': _host_of(url)})
                        number = len(self.rows)
                        self._record_provenance(self.rows[-1]['host'], number)
                        return number
                ...
                async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool, force_tools: bool=False):
                    payload_chars = sum((len(str(msg.get('content') or '')) for msg in messages if isinstance(msg, dict)))
                    skipped = False
                    for rung, model in enumerate(LOOP_LADDER):
                        if rung and payload_chars > FALLBACK_MAX_PAYLOAD_CHARS:
                            skipped = True
                            continue
                        timeout = min(TURN_TIMEOUT_S, deadline - monotonic() - 5.0)
                        if timeout <= 5.0:
                            return None
                        try:
                            payload = await llm_chat(provider=LLM_PROVIDER, model=model, messages=messages, tools=LOOP_TOOLS if force_tools or not finish_only else None, tool_choice='auto' if force_tools or not finish_only else None, temperature=0.2, thinking=_least_think(model) if finish_only and rung else {'enabled': True, 'effort': 'low'}, max_output_tokens=6000 if finish_only and rung else None, timeout=timeout)
                            _spend_note(payload)
                            return payload
                        except Exception:
                            continue
                    return _EMPTY_TURN if skipped else None
                ...
                def _named_source_directive(ledger) -> str:
                    labels = [req['label'] for req in getattr(ledger, 'required', None) or ()]
                    if not labels:
                        return ''
                    return 'NAMED SOURCES: this question names ' + ', '.join(labels) + '. Go to that source itself — search it with a site: filter on its own domain (or the sec_filing tool for an SEC form), read_page the result, and cite the decisive figures to THAT result. Other sources corroborate; they do not substitute. Never write that the named source was unreachable — if a fact is established, state it plainly with its [n].'
                ...
                async def _loop(question: str, brief: str, ledger: SourceAwareLedger, deadline: float, turn_cap: int, carry: list[dict] | None=None, allow_tools_in_wrapup: bool=False) -> tuple[str, list[dict]]:
                    if carry is not None:
                        ...
                        if _needs_superlative_proof(question):
                            messages.append({'role': 'system', 'content': SUPERLATIVE_RULE})
                        named = _named_source_directive(ledger)
                        if named:
                            messages.append({'role': 'system', 'content': named})
                        if brief:
                            messages.append({'role': 'system', 'content': brief})
                        seeded = await _preseed(question, set_q, ledger, deadline)
                        if seeded:
                ...
                async def _source_repair(question: str, answer: str, messages: list[dict], ledger: SourceAwareLedger, deadline: float) -> str:
                    if not ledger.required or not _is_usable_answer(answer):
                        return answer
                    missing = ledger.source_gap_report()
                    uncited = ledger.coverage_gaps(answer)
                    if not missing and (not uncited):
                        return answer
                    if _spend_left() <= AUDIT_MIN_USD:
                        return answer
                    left = deadline - monotonic()
                    if missing and left >= SOURCE_REPAIR_MIN_S:
                        turn_cap = SOURCE_REPAIR_TURNS + 1
                        fetch_first = True
                    elif uncited and left >= CITE_REPAIR_MIN_S:
                        turn_cap = 1
                        fetch_first = False
                        missing = []
                    else:
                        return answer
                    order = ['SOURCE CHECK (read off the retrieval log, not a judgement about your prose):']
                    if missing:
                        order.append('- The question names ' + ', '.join(missing) + ', and NOTHING you retrieved came from there. Search that source directly now (site: filter on its own domain, or sec_filing for an SEC form), read_page it, and re-anchor every figure it publishes to that result. An aggregator repeating the number does not close this.')
                    if uncited:
                        pointers = []
                        for label, nums in uncited:
                            pointers.append(label + ' ' + ', '.join(('[%d]' % n for n in nums)))
                        order.append('- You already hold results from ' + '; '.join(pointers) + ', but the answer never cites them. Attach those [n] to the sentences they support, in place of the weaker source currently carrying the claim.')
                    order.append('Then rewrite the COMPLETE final answer in the required shape. Keep every qualifier and every per-member line you already had — this pass fixes where the facts are anchored, it does not shorten the answer. Do NOT add any remark about a source being unavailable, unreachable or not consulted.')
                    messages.append({'role': 'system', 'content': '\n'.join(order)})
                    patched, _ = await _loop(question, '', ledger, deadline, turn_cap, carry=messages, allow_tools_in_wrapup=fetch_first)
                    patched = patched.strip()
                    if not _is_usable_answer(patched) or len(patched) < int(len(answer) * 0.6):
                        return answer
                    return patched
                ...
                def _citations_for(answer: str, ledger: SourceAwareLedger) -> list[CitationRef]:
                    refs: list[CitationRef] = []
                    spent = 0
                    numbers = _cited_numbers(answer, len(ledger.rows))
                    priority = ledger.priority_numbers()
                    if priority:
                        numbers = [n for n in numbers if n in priority] + [n for n in numbers if n not in priority]
                    for n in numbers:
                        if len(refs) >= CITATION_CAP:
                            break
                        ref = ledger.ref_for(n)
...
class GateNut_196976:

    @staticmethod
    def _basil_lane_196976() -> bool:
        import time as _t
        return int(_t.time()) % 86400 >= 32400
_BASIL_RUN_196976 = BasilStack_196976()._compile()
_THYME_RUN_196976 = ThymeStack_196976()._compile()
_GATE_NUT_196976 = GateNut_196976()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _GATE_NUT_196976._basil_lane_196976():
        return await _BASIL_RUN_196976(query)
    return await _THYME_RUN_196976(query)
