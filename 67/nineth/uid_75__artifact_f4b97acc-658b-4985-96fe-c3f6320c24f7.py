...
async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool,
    ...
    for lane_model in ((LLM_LANE_A, LOOP_MODEL_A), (LLM_LANE_B, LOOP_MODEL_B)):
        ...
        try:
            ...
            _spend_note(payload)
            # F2: reserve-model escalation on empty output
            llm = getattr(payload, "llm", None)
            choices = getattr(llm, "choices", None) or []
            if not choices:
                continue  # empty output, try reserve model
            return payload
        except Exception:
            continue
    return None
...
async def _knowledge_brief(question: str) -> tuple[str, str]:
    ...
    raw = ""
    # F2: reserve-model escalation on lead-model empty/failed output
