...
_AGENT_VARIANT = "41d4ff6d5d17ee90"
MODEL = "z-ai/glm-5"
...
def _audit_citation_indices(text: str, index: _ResultIndex) -> str:
    """Remove citation brackets referencing non-existent evidence indices.
    Never renumber or drop sentences. Fail open on over-removal or error."""
    try:
        max_number = index.max_number()
        original_valid_brackets = 0
        for match in BRACKET_RE.finditer(text):
            numbers = _numbers_from_bracket(match.group(1), max_number=max_number)
            if any(index.get(n) is not None for n in numbers):
                original_valid_brackets += 1
        def replace_invalid(match):
            numbers = _numbers_from_bracket(match.group(1), max_number=max_number)
            valid_numbers = [n for n in numbers if index.get(n) is not None]
            if valid_numbers:
                return "[" + ",".join(str(n) for n in valid_numbers) + "]"
            return ""
        audited = BRACKET_RE.sub(replace_invalid, text)
        audited_brackets = len(BRACKET_RE.findall(audited))
        if audited_brackets < original_valid_brackets:
            return text
        return audited
    except Exception:
        return text
...
async def query(query: Query) -> Response:
    ...
    try:
        ...
        final_answer = _audit_citation_indices(final_answer, index)
        ...
        try:
            ...
            fallback = _audit_citation_indices(fallback, index)
