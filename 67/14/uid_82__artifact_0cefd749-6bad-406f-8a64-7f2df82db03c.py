...
"""
    ...
    v35: adds per-evidence 'Supports:' summaries. After the tool loop, a single
    LLM call generates targeted summaries mapping each evidence row to the
    specific fact it proves. These summaries flow into the answer-production
    digest, replacing raw page-dump slices with explicit claim-binding notes.
    """
            ...
            def set_summary(self, number: int, summary: str) -> None:
                """Set a 'Supports:' summary for evidence row `number` (1-indexed)."""
                if 1 <= number <= len(self.rows) and summary:
                    self.summaries[number] = summary

            def summary_digest(self, char_cap: int=60000) -> str:
                """Evidence digest with per-row 'Supports:' summaries appended.

        Replaces the flat _ledger_digest: each evidence block now carries a
        targeted summary of what it proves, so the answer-production LLM sees
        explicit claim bindings instead of raw page dumps."""
                parts: list[str] = []
                spent = 0
    ...
    markers must NEVER reach a submitted answer (judge-penalized uncertainty)."""
        ...
        def _ledger_digest(ledger: EvidenceLedger, char_cap: int=60000) -> str:
            """Evidence digest with 'Supports:' summaries from the claim-evidence ledger.

    v35: delegates to ledger.summary_digest() which appends per-row claim-binding
    summaries. Falls back to flat digest if summaries are absent."""
            if ledger.summaries:
    ...
    that carries an [n]: that is a real answer, however it opens."""
        ...
        async def _generate_evidence_summaries(question: str, ledger: EvidenceLedger, deadline: float) -> None:
            """Post-loop: one LLM call to generate per-evidence 'Supports:' summaries.

    ROOT ARCHITECTURAL CHANGE (evidence_state_flow): instead of evidence flowing
    as raw tool-output through chat history, each evidence row gets a targeted
    claim-binding summary. These summaries flow into answer production
    (write_from_digest, deterministic_answer) so every citation carries an
    explicit statement of what it proves.
    """
            left = deadline - monotonic()
