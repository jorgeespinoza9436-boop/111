...
class PrimarySolver:
    ...
    def _compile(self):
        ...
        def _closable_source_refs(state: ResearchState) -> list[str]:
            """하네스가 직접 종결할 때 인용해도 안전한 소스 참조.

    _finalize_answer 는 (a) 문맥이 소스 참조를 하나도 언급하지 않거나
    (b) 페이지 참조(P*)가 retained_evidence 에 없으면 ValueError 를 낸다.
    그 두 조건을 미리 걸러 실패할 호출을 아예 하지 않는다.
    """
        ...
        def _governor_stage(state: ResearchState, elapsed_seconds: float) -> str:
            """관측된 지출과 경과시간으로 조사 단계를 정한다: open -> soft -> hard.

    둘 중 먼저 문턱을 넘는 쪽이 이긴다. 원본은 둘 다 프롬프트로 부탁만
    하고 강제하지 않아서 예산 소진(0점)과 300초 초과(0점)가 실제로 났다.
    """
        ...
        class EasyPath:
            ...
            def _compile(self):
                ...
                async def _loop_chat(run: _Run, messages: list, force_text: bool):
                    for attempt in range(2):
                        ...
                        if force_text:
                            ...
                            choice_arg = 'auto'
                        try:
                            return await llm_chat(provider=PROVIDER, model=model, messages=messages, tools=tools_arg, tool_choice=choice_arg, temperature=0.2, thinking={'enabled': False}, timeout=timeout)
                        except Exception:
                            continue
                    return None
                ...
                async def _tool_search(query_text: str, run: _Run) -> str:
                    ...
                    cache_key = 's:' + ' '.join(query_text.lower().split())
                    cached = run.tool_cache.get(cache_key)
                ...
                async def _tool_fetch(url: str, run: _Run) -> str:
                    ...
                    shown = _localize_note(note, run.question)
                    out = '# fetch_page(' + repr(url) + ') -> [' + str(number) + '] ' + str(len(shown)) + ' chars shown\n' + shown
                    run.tool_cache[cache_key] = out
                    return out

                async def _tool_search_batch(queries, run: _Run) -> str:
                    if not isinstance(queries, list):
                        return '# search_web_batch -> queries must be a list of strings'
                    blocks = []
                    for raw_query in queries[:BATCH_QUERY_CAP]:
                        query_text = str(raw_query or '').strip()
                        if not query_text:
                            continue
                        if run.remaining() < 25.0:
                            blocks.append('# search_web_batch truncated (time limit)')
                            break
                        try:
                            blocks.append(await _tool_search(query_text, run))
                        except Exception:
                            blocks.append('# search_web(' + repr(query_text) + ') -> ERROR')
                    if not blocks:
                        return '# search_web_batch -> no valid queries'
                    return '\n\n'.join(blocks)
                ...
                async def _run_tool_call(tool_call, run: _Run) -> str:
                    ...
                    if name == 'search_web_batch':
                        return await _tool_search_batch(args.get('queries'), run)
                ...
                _STOPWORDS = frozenset({'the', 'a', 'an', 'and', 'or', 'of', 'in', 'on', 'at', 'to', 'for', 'from', 'by', 'with', 'which', 'what', 'who', 'whom', 'whose', 'when', 'where', 'how', 'many', 'much', 'was', 'were', 'is', 'are', 'be', 'been', 'being', 'did', 'does', 'do', 'that', 'this', 'these', 'those', 'as', 'its', 'it', 'their', 'there', 'than', 'then', 'into', 'onto', 'per', 'about', 'according', 'between', 'during', 'after', 'before', 'list', 'name', 'all', 'every', 'each', 'only', 'please', 'answer'})
                _TOKEN_RE = re.compile("[A-Za-z0-9][\\w\\-']*")

                def _salient_tokens(text: str, cap: int) -> list:
                    tokens = []
                    seen = set()
                ...
                def _seed_queries(question: str) -> list:
                    text = ' '.join((question or '').split())
                    if not text:
                        return []
                    queries = [text[:300]]
                    tokens = _salient_tokens(text, 8)
                    if len(tokens) >= 2:
                        packed = ' '.join(tokens)
                        if packed.lower() != queries[0].lower():
                            queries.append(packed)
                    if _enum_is_set_question(text):
                        short = ' '.join(_salient_tokens(text, 6))
                        if short:
                            queries.append('list of ' + short)
                    out = []
                    seen = set()
                ...
                def _roster_items(question: str) -> list:
                    items = []
                    seen = set()
                    for match in _ROSTER_ITEM_RE.finditer(question or ''):
                        for group in match.groups():
                            if not group:
                                continue
                            item = ' '.join(group.split()).strip(' .,;:*')
                            key = item.lower()
                            if len(item) >= 2 and key not in seen and (not key.isdigit()):
                                seen.add(key)
                                items.append(item)
                    return items[:ROSTER_ITEM_CAP]

                def _item_page_urls(question: str) -> list:
                    items = _roster_items(question)
                    if len(items) < 2:
                        return []
                    low = (question or '').lower()
                    if 'wikipedia' not in low and (not _enum_is_set_question(question)):
                        return []
                    urls = []
                    for item in items[:ITEM_FETCH_CAP]:
                        slug = item.strip().replace(' ', '_')
                        if slug:
                            urls.append('https://en.wikipedia.org/wiki/' + slug)
                    return urls
                _ISO_DATE_RE = re.compile('\\b(?:19|20)\\d{2}-\\d{2}-\\d{2}\\b')
                _YEAR_FULL_RE = re.compile('\\b(?:19|20)\\d{2}\\b')
                _MAG_RANGE_RE = re.compile('\\bbetween\\s+(\\d+(?:\\.\\d+)?)\\s+and\\s+(\\d+(?:\\.\\d+)?)', re.IGNORECASE)
                _MAG_MIN_RE = re.compile('\\bmagnitude\\s+(?:of\\s+|at\\s+least\\s+|above\\s+|over\\s+|greater\\s+than\\s+)?(\\d+(?:\\.\\d+)?)', re.IGNORECASE)
                _MAG_PLUS_RE = re.compile('\\bM\\s?(\\d(?:\\.\\d)?)\\s?\\+')
                _SEC_FORM_RE = re.compile('\\b(10-K|10-Q|8-K|20-F|S-1)\\b', re.IGNORECASE)
                _SEC_COMPANY_RE = re.compile("\\b([A-Z][A-Za-z&.\\-]+(?:\\s+[A-Z][A-Za-z&.\\-]+){0,3})(?:'s)?\\s+(?:10-K|10-Q|8-K|20-F|S-1|annual\\s+report|filing)")
                _PLANET_NAMES = ('mercury', 'venus', 'earth', 'mars', 'jupiter', 'saturn', 'uranus', 'neptune', 'pluto', 'moon')

                def _usgs_url(question: str) -> str:
                    text = question or ''
                    if re.search('\\bearthquake', text, re.IGNORECASE) is None:
                        return ''
                    start = ''
                    end = ''
                    iso_dates = []
                    for match in _ISO_DATE_RE.finditer(text):
                        iso_dates.append(match.group(0))
                    if len(iso_dates) >= 2:
                        start = iso_dates[0]
                        end = iso_dates[1]
                        ...
                        years = []
                        for match in _YEAR_FULL_RE.finditer(text):
                            years.append(match.group(0))
                        if years:
                            start = years[0] + '-01-01'
                            end = years[len(years) - 1] + '-12-31'
                    if not start:
                        return ''
                    url = 'https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&starttime=' + start + '&endtime=' + end + 'T23:59:59&orderby=time-asc'
                    used_range = False
                    rng = _MAG_RANGE_RE.search(text)
                    if rng is not None:
                        try:
                            low_value = float(rng.group(1))
                            high_value = float(rng.group(2))
                        except Exception:
                            low_value = 99.0
                            high_value = 99.0
                        if low_value <= 10.5 and high_value <= 10.5:
                            url += '&minmagnitude=' + rng.group(1) + '&maxmagnitude=' + rng.group(2)
                            used_range = True
                    if not used_range:
                        mag = _MAG_MIN_RE.search(text)
                        if mag is not None:
                            url += '&minmagnitude=' + mag.group(1)
                        else:
                            plus = _MAG_PLUS_RE.search(text)
                            if plus is not None:
                                url += '&minmagnitude=' + plus.group(1)
                    return url

                def _nasa_url(question: str) -> str:
                    low = (question or '').lower()
                    if 'nssdc' not in low and 'fact sheet' not in low and ('nasa' not in low):
                        return ''
                    for planet in _PLANET_NAMES:
                        if planet in low:
                            return 'https://nssdc.gsfc.nasa.gov/planetary/factsheet/' + planet + 'fact.html'
                    return ''

                def _sec_url(question: str) -> str:
                    text = question or ''
                    low = text.lower()
                    if 'edgar' not in low and 'sec.gov' not in low and (_SEC_FORM_RE.search(text) is None):
                        return ''
                    form = ''
                    form_match = _SEC_FORM_RE.search(text)
                    if form_match is not None:
                        form = form_match.group(1).upper()
                    company = ''
                    items = _roster_items(text)
                    if items:
                        company = items[0]
                    else:
                        company_match = _SEC_COMPANY_RE.search(text)
                        if company_match is not None:
                            company = company_match.group(1)
                    if not company:
                        return ''
                    query = company.strip().replace(' ', '+')
                    return 'https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&company=' + query + '&type=' + form + '&dateb=&owner=include&count=40'

                def _data_query_urls(question: str) -> list:
                    urls = []
                    seen = set()
                    for url in (_usgs_url(question), _nasa_url(question), _sec_url(question)):
                        if url and url not in seen:
                            seen.add(url)
                            urls.append(url)
                    return urls[:3]
                _AUTHORITY_HOSTS = ('.gov', 'wikipedia.org', 'un.org', 'worldbank.org', 'oecd.org', 'imf.org', 'who.int', 'imdb.com', 'boxofficemojo.com', 'britannica.com', 'worldatlas.com', 'nssdc.gsfc.nasa.gov')
                _URL_HARVEST_RE = re.compile('https?://[^\\s\\]\\)>\\"\']+')
                _URL_HOST_RE = re.compile('https?://([^/]+)')

                def _authority_urls(blocks: list) -> list:
                    urls = []
                    seen = set()
                    for block in blocks:
                        for match in _URL_HARVEST_RE.finditer(block or ''):
                            ...
                            allowed = False
                            for suffix in _AUTHORITY_HOSTS:
                                if host == suffix.lstrip('.') or host.endswith(suffix):
                                    allowed = True
                                    break
                            if allowed and url.lower() not in seen:
                                seen.add(url.lower())
                                urls.append(url)
                    return urls[:PREFETCH_CAP]

                def _localize_note(note: str, question: str) -> str:
                    try:
                        text = note or ''
                        if len(text) <= FETCH_NOTE_CHARS:
                            return text
                        terms = set()
                        for token in _salient_tokens(question or '', 12):
                            terms.add(token.lower())
                        if not terms:
                            return text[:FETCH_NOTE_CHARS]
                        low = text.lower()
                        window = LOCAL_WINDOW_CHARS
                        step = window // 2
                        scored = []
                        position = LOCAL_HEAD_CHARS
                        while position < len(text):
                            chunk = low[position:position + window]
                            score = 0
                            for term in terms:
                                score += chunk.count(term)
                            if score > 0:
                                scored.append((score, position))
                            position += step
                        scored.sort(reverse=True)
                        picked = []
                        for score, window_pos in scored:
                            overlaps = False
                            for prior in picked:
                                if abs(window_pos - prior) < window:
                                    overlaps = True
                                    break
                            if not overlaps:
                                picked.append(window_pos)
                            if len(picked) >= 3:
                                break
                        picked.sort()
                        parts = [text[:LOCAL_HEAD_CHARS]]
                        for window_pos in picked:
                            parts.append('…[@' + str(window_pos) + ']… ' + text[window_pos:window_pos + window])
                        out = '\n'.join(parts)
                        return out[:LOCAL_HEAD_CHARS + 3 * (window + 40)]
                    except Exception:
                ...
                def _normalize_citation_text(text: str) -> str:
                    try:
                        ...
                        return text
                _MAG_WORDS = (('trillion', 1000000000000.0), ('billion', 1000000000.0), ('million', 1000000.0), ('thousand', 1000.0))
                _CLOCK_RE = re.compile('\\b(\\d{1,3}):([0-5]\\d)\\b')
                ...
                def _parse_quantity(text: str):
                    ...
                    if not s:
                        return (None, False)
                    clock = _CLOCK_RE.search(s)
                    if clock is not None:
                        return (float(clock.group(1)) * 60.0 + float(clock.group(2)), True)
                    number = _NUMBER_RE.search(s)
                    if number is None:
                        return (None, False)
                    value = float(number.group(0))
                    for word, multiplier in _MAG_WORDS:
                        if word in s:
                            return (value * multiplier, True)
                    if re.search('\\d\\s*k\\b', s):
                        return (value * 1000.0, True)
                    if re.search('\\d\\s*(?:bn)\\b', s):
                        return (value * 1000000000.0, True)
                    if re.search('\\d\\s*(?:mn)\\b', s):
                        return (value * 1000000.0, True)
                    return (value, False)

                def _relation_holds(value: float, relation: str, low_bound: float, high_bound: float) -> bool:
                    if relation == '<':
                        return value < low_bound
                    if relation == '<=':
                        return value <= low_bound
                    if relation == '>':
                        return value > low_bound
                    ...
                    if relation == '==':
                        return abs(value - low_bound) <= max(abs(low_bound) * 0.001, 1e-09)
                    return True

                def _triple_ok(value_text: str, relation: str, bound_text: str) -> bool:
                    value, has_magnitude = _parse_quantity(value_text)
                    if value is None:
                        return True
                    rel = (relation or '').strip()
                    if rel == 'between':
                        parts = re.split('\\.\\.|\\s+and\\s+|\\s*-\\s*', (bound_text or '').strip(), maxsplit=1)
                        if len(parts) != 2:
                            return True
                        low_bound, _low_mag = _parse_quantity(parts[0])
                        high_bound, _high_mag = _parse_quantity(parts[1])
                        if low_bound is None or high_bound is None:
                            return True
                    else:
                        low_bound, _low_mag = _parse_quantity(bound_text)
                        if low_bound is None:
                            return True
                        high_bound = low_bound
                    biggest = max(abs(low_bound), abs(high_bound))
                    if not has_magnitude and biggest >= 10000.0 and (value > 0):
                        ratio = biggest / value
                        if ratio >= 100.0 or ratio <= 0.01:
                            return True
                    return _relation_holds(value, rel, low_bound, high_bound)
                ...
                async def _research_loop(run: _Run, question: str, briefing: str, max_turns: int, seed_messages=None):
                    ...
                    for turn in range(1, max_turns + 1):
                        ...
                        if not tool_calls:
                            ...
                            final_answer = _normalize_citation_text(text)
                            break
                        try:
                ...
                async def _phase_briefing(run: _Run, question: str):
                    ...
                    raw = ''
                    try:
                        raw = await _plain_chat(run, DRAFT_MODEL, BRIEFING_SYSTEM, user, 2400, DRAFT_TIMEOUT, {'enabled': False})
                    except Exception:
                        raw = ''
                    if not raw.strip():
                        try:
                ...
                async def _phase_seed(run: _Run, question: str, briefing: str) -> list:
                    messages = _seed_transcript(question, briefing, None)
                    search_blocks = []
                    for seed_query in _seed_queries(question):
                        if run.remaining() < SEED_MIN_REMAINING:
                            break
                        try:
                            block = await _tool_search(seed_query, run)
                        except Exception:
                            continue
                        if isinstance(block, str) and 'ERROR' not in block:
                            search_blocks.append(block)
                    fetch_blocks = []
                    fetch_urls = _data_query_urls(question) + _item_page_urls(question)
                    for url in fetch_urls[:SEED_FETCH_CAP]:
                        if run.remaining() < SEED_MIN_REMAINING:
                            break
                        try:
                            block = await _tool_fetch(url, run)
                        except Exception:
                            continue
                        if isinstance(block, str) and 'ERROR' not in block and ('no content' not in block) and ('no usable' not in block):
                            fetch_blocks.append(block)
                    prefetch_blocks = []
                    for url in _authority_urls(search_blocks):
                        if run.remaining() < SEED_MIN_REMAINING:
                            break
                        try:
                            block = await _tool_fetch(url, run)
                            ...
                            continue
                        if isinstance(block, str) and 'ERROR' not in block and ('no content' not in block) and ('no usable' not in block):
                            prefetch_blocks.append('PREFERRED SOURCE (authoritative host):\n' + block)
                    blocks = search_blocks + fetch_blocks + prefetch_blocks
                    if blocks:
                        seed_note = 'SEED EVIDENCE (gathered before your first turn; the numbered results are citable with [n] exactly like your own tool results):\n\n' + '\n\n'.join(blocks)
                        messages.append({'role': 'user', 'content': seed_note[:SEED_NOTE_CAP]})
                    return messages
                ...
                async def _phase_audit(run: _Run, question: str, answer: str, messages: list):
                    ...
                    try:
                        raw = await _plain_chat(run, PATCH_MODEL, AUDIT_SYSTEM, check_user, 700, PATCH_TIMEOUT, {'enabled': True, 'effort': 'low'})
                    ...
                    if not issues or run.remaining() < 40.0:
                        return (answer, messages)
                    seed = list(messages)
                    seed.append({'role': 'system', 'content': 'AUDIT FOUND GAPS in your final answer:\n- ' + '\n- '.join(issues[:6]) + "\nYou may use at most 2 more tool calls to close the most important gaps — for an incomplete-roster or missing-item gap, FIRST fetch the authoritative list page or the missing item's own page — then rewrite the COMPLETE final answer with inline [n] citations in the required shape."})
                    patched, patched_messages = await _research_loop(run, question, '', PATCH_EXTRA_TURNS + 1, seed)
                    patched = patched.strip()
                    if patched and len(patched) >= int(len(answer) * REGRESS_MIN_LEN_RATIO) and (len(_BRACKET_RE.findall(patched)) >= len(_BRACKET_RE.findall(answer))):
                ...
                async def _phase_roster_coverage(run: _Run, question: str, answer: str, messages: list):
                    items = _roster_items(question)
                    if len(items) < 2 or not answer.strip():
                        return (answer, messages)
                    answer_low = answer.lower()
                    uncovered = []
                    for item in items:
                        if item.lower() not in answer_low:
                    ...
                    if run.remaining() <= COVERAGE_MIN_SECONDS or run.budget_left() < COVERAGE_MIN_BUDGET:
                        return (answer, messages)
                    seed = list(messages)
                    seed.append({'role': 'system', 'content': 'ROSTER COVERAGE GAP: the question itself enumerates these items: ' + '; '.join(items) + '. Your answer gives no verdict for: ' + '; '.join(uncovered[:6]) + ". Use at most 2 tool calls (search_web_batch, or fetch each missing item's own page) to get evidence for the missing items, then rewrite the COMPLETE answer with one cited verdict line per asked item."})
                    patched, patched_messages = await _research_loop(run, question, '', 3, seed)
                    patched = (patched or '').strip()
                    if patched and (not _looks_unfinished(patched)) and (len(patched) >= int(len(answer) * REGRESS_MIN_LEN_RATIO)) and (len(_BRACKET_RE.findall(patched)) >= len(_BRACKET_RE.findall(answer))):
                        return (patched, patched_messages)
                    return (answer, messages)

                async def _numeric_guard(run: _Run, question: str, answer: str) -> str:
                    if not answer or len(answer) < 40:
                        return answer
                    if run.remaining() < NUMERIC_GUARD_MIN_SECONDS or run.budget_left() < MIN_PATCH_BUDGET:
                        return answer
                    guard_user = 'From the ANSWER below, extract every numeric claim that the QUESTION constrains with a threshold, range, or exact figure. Output ONLY a JSON array of objects like {"candidate": "entity", "value": "the value string copied from the answer", "relation": "<|<=|>|>=|==|between", "bound": "the bound from the question (A..B for between)"}. Output [] if none.\n\nQUESTION:\n' + question[:2000] + '\n\nANSWER:\n' + answer[:8000]
                    triples = []
                    try:
                        raw = await _plain_chat(run, JSON_MODEL, 'You extract numeric claims. Output JSON only.', guard_user, 600, PATCH_TIMEOUT, {'enabled': True, 'effort': 'low'})
                        cleaned = re.sub('^```(?:json)?\\s*|\\s*```$', '', raw.strip(), flags=re.I | re.M)
                    ...
                    for triple in triples[:20]:
                        if not isinstance(triple, dict):
                            continue
                        value_text = str(triple.get('value', ''))
                        relation = str(triple.get('relation', ''))
                        bound_text = str(triple.get('bound', ''))
                        candidate = str(triple.get('candidate', ''))
                        keep = True
                        try:
                            keep = _triple_ok(value_text, relation, bound_text)
                        except Exception:
                            keep = True
                        if not keep:
                            violations.append(candidate + ': ' + value_text + ' violates ' + relation + ' ' + bound_text)
                    if not violations or run.remaining() < 30.0:
                        return answer
                    fix_system = 'Correct ONLY the flagged numeric violations in the answer: re-check each flagged value against its cited source, fix magnitude or transcription slips, and if a flagged candidate truly violates a question constraint, remove that candidate rather than adjusting other facts. Keep every [n] citation marker and the required output shape. Output only the corrected answer.'
                    fix_user = 'FLAGGED NUMERIC VIOLATIONS:\n- ' + '\n- '.join(violations[:6]) + '\n\nANSWER:\n' + answer[:8000]
                    try:
                        fixed = await _plain_chat(run, DRAFT_MODEL, fix_system, fix_user, 1600, RESYNTH_TIMEOUT)
                    except Exception:
                        return answer
                    fixed = (fixed or '').strip()
                    if fixed and (not _looks_unfinished(fixed)) and (len(fixed) >= int(len(answer) * REGRESS_MIN_LEN_RATIO)) and (len(_BRACKET_RE.findall(fixed)) >= len(_BRACKET_RE.findall(answer))):
                        return fixed
                    return answer
                ...
                async def _structured_output(run: _Run, question: str, answer: str, schema):
                    ...
                    for model in (JSON_MODEL, FALLBACK_MODEL):
                        if model == JSON_MODEL:
                            schema_thinking = {'enabled': True, 'effort': 'low'}
                        else:
                            schema_thinking = {'enabled': False}
                        try:
                            raw = await _plain_chat(run, model, 'You output strictly valid JSON matching the given schema.', user, 2400, SCHEMA_TIMEOUT, schema_thinking)
                ...
                async def query(query: Query) -> Response:
                    ...
                    try:
                        ...
                        return Response(text='Best-effort summary for: ' + question[:600])
                ...
                async def _answer(query: Query, question: str) -> Response:
                    ...
                    try:
                        if run.remaining() > SEED_MIN_REMAINING:
                            seed_messages = await _phase_seed(run, question, briefing)
                    except Exception:
                        seed_messages = None
                    answer = ''
                    messages = []
                    try:
                        answer, messages = await _research_loop(run, question, briefing, MAX_TURNS, seed_messages)
                    except Exception:
                        answer = ''
                    try:
                    ...
                    try:
                        if answer:
                            answer, messages = await _phase_roster_coverage(run, question, answer, messages)
                    except Exception:
                        pass
                    try:
                        if answer:
                            answer = await _numeric_guard(run, question, answer)
                    except Exception:
                        pass
                    if not answer.strip():
                        answer = draft.strip()
                    ...
                    try:
                        ...
                        pass
                    answer = _normalize_citation_text(answer)
                    ...
                    if not final_text:
                        final_text = 'Best-effort summary for: ' + question[:400]
                    if query.output_schema is not None:
                        output = None
                        try:
...
_TAG_BD238E56="bd238e56de7248f5b3f4dc8949dd4d7a"
import logging as _tag_logging_bd238e56
_tag_logging_bd238e56.getLogger("miner.tag").debug("tag=%s", _TAG_BD238E56)
