...
"""
    ...
    — so those questions were getting no completeness discipline at all."""
        ...
        CHECK_RULE = "FINAL-CHECK DISCIPLINE (applies to every answer):\n1) COMPARATOR DIRECTION: before finalizing any threshold/filter answer, restate each comparison numerically WITH its direction and re-read the question's wording — 'fewer than / under / less than / below 3,000' keeps ONLY values BELOW 3,000 (2,620 qualifies, 3,570 does not); 'more than / over / at least' is the reverse. A candidate on the wrong side of the comparator is the single most expensive mistake.\n2) ASKED ORDER: when the question defines an order (by growth, by size, chronological, largest-to-smallest), COMPUTE the sort key for every item from the cited figures and emit the list in exactly that order, stating each item's key value.\n3) COUNT: a list's length must equal the count the question implies (one per asked item, or the deduplicated set if it asks for distinct values) — recount before writing."
    ...
    heavy batch; the loop then verifies the load-bearing facts."""
        ...
        async def _audit_patch(question: str, answer: str, messages: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
            ...
            if isinstance(report, dict):
                for key in ('incomplete_roster', 'hand_waved_tally', 'unanswered_parts', 'uncited_facts', 'wrong_kind', 'thin_proof', 'wrong_source', 'ambiguous_scope', 'comparator_errors'):
                    vals = report.get(key)
        ...
        _DAGGER_CITE_RE = re.compile('\\[\\s*(\\d{1,4})\\s*[†‡§][^\\]]{0,80}\\]')

        def _normalize_brackets(text: str) -> str:
            return _DAGGER_CITE_RE.sub('[\\1]', (text or '').translate(_BRACKET_FIX))
                ...
                if spent + cost > EVIDENCE_CHAR_BUDGET:
                    if slices:
                        ...
                        if 0 < head_cost and spent + head_cost <= EVIDENCE_CHAR_BUDGET:
                            spent += head_cost
                            refs.append(CitationRef(receipt_id=ref.receipt_id, result_id=ref.result_id, slices=[slices[0]]))
                    continue
                spent += cost
                refs.append(ref)
            return refs
    ...
    sliceless ref, at most 3, budget-capped."""
            ...
            for i, row in enumerate(ledger.rows, start=1):
                ...
                if not shown.strip():
                    continue
                hits = sum((1 for t in terms if t in shown)) + 2 * sum((1 for v in nums if v in shown))
    ...
    try to verify against the citation package."""
        ...
        def _augment_value_refs(value, ledger: EvidenceLedger, refs: list[CitationRef]) -> list[CitationRef]:
            ...
            if kind == 'array':
                ...
                for p in raw_parts:
                    ...
                    if not p or p in ('...', '…', 'etc', 'etc.'):
                        continue
                    if p.isdigit() and len(p) <= 3 and (item_kind not in ('number', 'integer')):
                        continue
                    parts.append(p[:400])
                parts = parts[:20]
                if not parts:
...
_TAG_F6B63837="f6b6383750994e588f8d3d24962133f7"
import logging as _tag_logging_f6b63837
_tag_logging_f6b63837.getLogger("miner.tag").debug("tag=%s", _TAG_F6B63837)
