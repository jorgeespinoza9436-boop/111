...
architecture, keeping the assets our line already validated:
  ...
  - a single-provider (openrouter) LLM MODEL LADDER: the old paid ai_gateway
    lane is gone; redundancy now comes from independent models on one provider.

v34 STRUCTURAL HARDENING (this revision) — no behavioural rewrite, only the
load-bearing structure the score already depends on:
  H1 openrouter-only. Every llm_chat call goes through LLM_PROVIDER; the
     fallback rung is a different MODEL, not a different vendor. The lane-B
     payload guard survives as a rung-scoped guard (rung >= 1), so the
     control-flow contract `_loop` was tuned against is byte-identical.
  H2 The research loop can no longer lose its transcript. Every failure inside
     a turn now BREAKS with (answer, messages) instead of propagating — an
     exception used to unwind into `_solve`, discard `messages`, and silently
     kill the audit/patch stage that the roster score depends on.
  H3 Transcript integrity is enforced at the append site: the assistant
     tool-call message and the replies to its ids are committed together, so a
     failure can never leave a tool_call_id unanswered (which invalidates every
     later turn).
  H4 Turn-start reserve raised to LOOP_EXIT_RESERVE_S so the rescue ladder is
     always reachable. A turn beginning under ~16s could never complete a chat
     anyway (_chat_turn refuses under 10s), but the old 8s floor let the loop
     consume the tail that _write_from_digest / _schema_output need.
  H5 Per-query state reset: _SPEND is process-global and used to gate the
     briefing and the audit off a PREVIOUS query's drained budget.
  H6 Run-scoped tool memo: an identical web_search / read_page re-issued inside
     one run returns the already-committed block instead of re-paying for it.
     Same evidence, same [n], no extra latency or spend.
  H7 Search provider resolved from tooling_info with a hard-coded default, so a
     provider rename cannot zero the run's entire evidence supply.
  H8 No higher-order callables anywhere (the sort key lambda is gone), no
     dunder attribute reflection, no dynamic getattr names, no forbidden
     imports — every construct is statically resolvable.
...
# ── provider / models ─────────────────────────────────────────────────────────
# H1: ONE provider. The old design bought redundancy by paying a second vendor
# (ai_gateway) for the same turn; that lane is removed entirely. Redundancy is
# now a MODEL LADDER on openrouter — independent weights, independent capacity
# pools, independent failure modes, and no second key to keep funded.
LLM_PROVIDER = "openrouter"

LOOP_MODEL_A = "z-ai/glm-5.2"            # v33.1: measured faster + far steadier
#                                          than glm-5 with reasoning OFF
LOOP_MODEL_B = "deepseek/deepseek-v3.2"  # rung 1: measured accepting effort:none
#                                          in 1.7s; a genuinely different family,
#                                          so a glm capacity stall does not take
#                                          the fallback down with it.
LOOP_MODEL_C = "z-ai/glm-5"              # rung 2: same family, different weights
#                                          — the last line before the loop gives up
...
# H7: the search/fetch provider is the single point of failure for ALL evidence
# — a bad name here scores 0 on every task, not just some. Keep the measured
# default, but let tooling_info correct it at runtime if the platform renames or
# retires it. _resolve_search_provider() only ever narrows to something the
# platform itself advertised.
...
TURN_TIMEOUT_S = 75.0
FALLBACK_MAX_PAYLOAD_CHARS = 144000   # ~36k tokens. Measured on the retired
#   paid lane, but the failure shape is provider-independent: above this a
#   fallback rung burns a full prompt and a full TURN_TIMEOUT_S window to return
#   nothing. Kept as a RUNG-scoped guard (rung >= 1 only), so rung 0 — the model
#   the 0.650 was tuned on — sees exactly the payloads it always saw.
...
MIN_TAIL_S = 8.0
# H4: the reserve the loop must leave for the RESCUE LADDER before starting a
# new turn. MIN_TAIL_S=8 was the emergency exit and it was too low to be safe:
# _write_from_digest refuses under 14s and _knowledge_resort / _schema_output
# refuse under 12s, so a loop that ran itself down to 9s left an unusable answer
# with every recovery rung already dead — and on a structured query that is a
# hard zero. This costs no research time: _chat_turn already refuses a turn
# whose window is under ~10s, so turns starting below this reserve were never
# capable of producing an answer in the first place.
...
def _reset_run_state() -> None:
    """H5: _SPEND is process-global and the worker is reused across queries, so
    a previous task that drained the metered budget left `left` low enough to
    gate THIS task's briefing (BRIEF_MIN_USD) and audit (AUDIT_MIN_USD) off
    before a single call was made — two of the three stages the score depends
    on, silently skipped for reasons that had nothing to do with this query.
    Reset to unknown; the tooling_info probe at the top of _solve refills it
    with the real figure within the first 10s."""
    _SPEND["left"] = None
    _SEARCH["provider"] = DEFAULT_SEARCH_PROVIDER
    # _SEC_CACHE is a pure content cache (EDGAR's ticker map is ~10MB and does
    # not change within a session) so it is deliberately NOT cleared — only
...
def _collect_provider_names(value, names: list[str]) -> None:
    """Flatten one tooling_info field into provider-name strings."""
...
def _resolve_search_provider(info) -> None:
    """H7: narrow to a provider the platform actually advertises, else keep the
    measured default. Every branch is guarded — a shape we do not recognise
    must leave the default untouched, never blank it."""
    names: list[str] = []
    # H8: three STATIC getattr calls with literal field names, not a loop over a
    # tuple of names — `getattr(obj, loop_var)` is `dynamic_getattr_name` to the
    # server-side AST policy. Three literals cost three lines and are provably
    ...
    if DEFAULT_SEARCH_PROVIDER in names:
        return                       # the measured default is still live
...
class EvidenceLedger:
    def __init__(self) -> None:
        ...
        # H6: run-scoped memo of tool calls already paid for, keyed on the
        # normalized request and holding the COMMITTED block (real [n] already
        # substituted). The loop model re-issues an identical web_search several
        # times in a long run — same query, same results, another 18s and
        # another metered charge, plus a duplicate ledger row that splits
        # citations for one source across two numbers. Replaying the committed
        # block instead is strictly better: identical evidence, identical
        # numbering, zero latency, zero spend.
        self.memo: dict = {}
...
def _best_windows(note: str, terms: set[str], width: int,
    ...
    # H8: rows are stored as (-hits, start) so the natural tuple ordering IS the
    # ranking — highest density first, earliest position breaking ties. This
    # keeps the sort byte-identical to the old key=lambda version while removing
    # the last higher-order callable in the file (a callable passed to and then
    # invoked by another function is exactly the construct the server-side AST
    # policy rejects as `unsupported_callable`).
    ranked: list[tuple[int, int]] = []   # (-hits, start)
    pos = 0
    while pos < n:
...
def _commit_tool_output(out, ledger: EvidenceLedger) -> str:
    ...
    # H6: memoize only a block that actually carries numbered evidence — a
    # failure string must stay retryable, since the retry paths inside
    # _do_search/_do_fetch exist precisely because these calls are flaky.
...
def _memo_hit(ledger: EvidenceLedger, key: str):
    """The committed block for an identical earlier call, or None."""
    if not key:
        return None
...
async def _do_search(query_text: str, ledger: EvidenceLedger):
    ...
    for attempt, allow_repeat in ((query_text, False), (query_text, True),
        ...
        fired.add(attempt)
        try:
            payload = await search_web(attempt, provider=_search_provider(), num=8,
...
async def _do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger):
    ...
    # H6: the focus hint changes WHICH regions are shown, so it is part of the
    # identity of the call — only a same-url/same-focus repeat is a true no-op.
    ...
    for _attempt in (0, 1):   # large-JSON crawls intermittently return empty
        ...
        try:
            payload = await asyncio.wait_for(
                fetch_page(url, provider=_search_provider(),
...
def _call_key(call) -> str:
    """The memo identity of a tool call, computed from the call object alone.

    H6: the fan-out needs this BEFORE the tools run — the memo itself only fills
    at commit time, so it cannot deduplicate calls issued together in one turn,
    and a batched sweep routinely contains two spellings of one query ('alpha
    corp units' / 'Alpha  Corp  Units'). Returns '' for anything unparseable,
    which disables deduplication for that call rather than guessing."""
    try:
...
async def _run_tool(call, question: str, ledger: EvidenceLedger, deadline: float):
    ...
    if not isinstance(args, dict):
        args = {}
    name = getattr(call, "name", "") or ""
    # H8: STATIC dispatch — an if/elif chain over string literals, never a name
    # -> handler table. A dict of callables looked up at runtime and then
    # invoked is `unsupported_callable` to the server-side AST policy, and it
    # buys nothing here: there are three tools and they are known at edit time.
    ...
    if name == "web_search":
        ...
        if cached is not None:
            return (cached + "\n# (identical query already run this task — the "
                    "numbered results above are unchanged. Search a DIFFERENT "
                    "angle, or read_page one of these URLs.)")
...
def _least_think(model: str = "") -> dict:
    ...
    Was keyed on (lane, model); the lane argument was already inert — the
    measurement above is per-model, not per-provider — and with one provider it
    is pure noise. Dropping it removes a parameter every call site had to keep
    consistent with the model it was passing."""
...
class _EmptyTurn:
    """Stand-in for a fallback rung we declined to pay for.

    Shaped like a real payload with one empty choice, so `_loop` takes the same
    branch it took when a fallback actually answered with empty content: the
    answer floor rejects it, a repair turn is spent, and the loop retries the
    primary model. Returning None here instead would COLLAPSE the loop."""
    llm = _EmptyLlm()
    budget = None
...
async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool,
    ...
    """One loop turn, walked down the openrouter MODEL ladder.

    H1: the ladder replaces the old two-vendor lane pair. The rung INDEX now
    plays the role the lane letter used to play, and every rung-conditional
    below is the exact translation of its lane-conditional predecessor:
    rung 0 is the model the 0.650 was measured on and is untouched; rungs >= 1
    inherit what lane B did (payload guard, bounded output on the finish turn).
    """
    # v33.2 COST, generalized: a fallback rung that cannot hold the payload
    # returns EMPTY while still billing the full prompt and burning the full
    # TURN_TIMEOUT_S. Measured on the retired paid lane -- 7 calls, $0.518, and
    # the two that returned zero completion tokens carried 50,444 and 37,227
    # prompt tokens while every call that produced output was <= 34,196 -- but
    # the arithmetic is provider-independent, so the guard stays.
    ...
    for rung, model in enumerate(LOOP_MODEL_LADDER):
        if rung >= 1 and payload_chars > FALLBACK_MAX_PAYLOAD_CHARS:
            # Skip the call, but do NOT let the turn collapse -- see _EmptyTurn.
            # Record it and keep walking: a LATER rung is no more able to hold
            # the payload, but the flag makes the terminal return explicit
            # rather than an accident of loop exhaustion.
            oversized_fallback = True
            continue
        timeout = min(TURN_TIMEOUT_S, deadline - monotonic() - 5.0)
        if timeout <= 5.0:
            # Out of time mid-ladder. If a rung was skipped for size, hand back
            # the empty-shaped payload (repair branch) rather than None (loop
            # collapse) -- the control flow the answer floor was tuned against.
        ...
        try:
            payload = await llm_chat(
                ...
                temperature=0.2,
                # RUNG-scoped, not turn-scoped (was lane-scoped, same intent).
                # Stripping reasoning from the PRIMARY model on the final turn
                # would remove it from the one turn that must apply every answer
                # rule and place every [n]; a fallback rung reached on the finish
                # turn is already a degraded path, and there the empty-content
                # risk outweighs the reasoning.
                thinking=({"enabled": False} if (finish_only and rung >= 1)
                          else {"enabled": True, "effort": "low"}),
                max_output_tokens=6000 if (finish_only and rung >= 1) else None,
                timeout=timeout,
            )
            _spend_note(payload)
            return payload
...
async def _knowledge_brief(question: str) -> tuple[str, str]:
    ...
    # H1: same two attempts as before, now two MODELS on one provider. Also
    # advance on an EMPTY reply, not only on an exception — the old chain caught
    # the exception case but let a silent empty completion (the documented
    # failure mode of a congested model) return "" without ever trying rung 1,
    # which loses BOTH the draft and the briefing for the whole task.
    raw = ""
...
async def _loop(question: str, brief: str, ledger: EvidenceLedger,
    ...
    """H2: the loop MUST NOT be able to raise.

    Before this split, any exception inside a turn unwound all the way into
    `_solve`, where the `answer, messages = await _loop(...)` assignment never
    happened — so `messages` stayed `[]` and the audit/patch stage (the stage
    that closes incomplete rosters, i.e. the single most-cited scored loss in
    this lineage) was silently skipped for the rest of the task. The transcript
    is now owned HERE and the turn machinery mutates it in place, so whatever
    was gathered is always returned, however the turns ended."""
    if carry is not None:
        ...
        try:
            ...
            pass          # a missing rule block is a weaker prompt, not a dead run
        messages.append({"role": "user", "content": question})

    answer = ""
    try:
        answer = await _run_turns(question, ledger, deadline, turn_cap,
                                  messages, allow_tools_in_wrapup)
    except Exception:
        answer = ""
    return answer, messages


async def _run_turns(question: str, ledger: EvidenceLedger, deadline: float,
                     turn_cap: int, messages: list[dict],
                     allow_tools_in_wrapup: bool) -> str:
    """The turn machinery. Mutates `messages` in place; returns the answer."""
    answer = ""
    ordered_wrapup = False
    repairs_left = ANSWER_REPAIR_TURNS
    for turn in range(1, turn_cap + 1):
        left = deadline - monotonic()
        # H4: leave the rescue ladder its window. A turn starting below this
        # reserve could not have produced an answer anyway — _chat_turn refuses
        # a window under ~10s — so this costs no research time, but it stops the
        # loop from eating the tail that _write_from_digest (>=14s) and
        # _schema_output (>=12s) need in order to exist at all.
        ...
        if not calls:
            ...
            break
        # H3 TRANSCRIPT ATOMICITY. The assistant tool-call message and the
        # replies to ITS ids are one indivisible unit: an assistant message
        # whose ids are not all answered invalidates the transcript, so every
        # LATER turn fails and the run is over. Previously the assistant message
        # was appended first and the replies were appended across two unguarded
        # loops — anything raising in between (a malformed call object, a bad
        # ledger row) left exactly that broken state. Now the whole unit is
        # staged in `pending` and spliced onto `messages` only once complete.
        try:
            assistant_msg = msg.to_input_message()
        except Exception:
            break        # cannot represent this turn faithfully — stop cleanly
        # Every id must be a usable string BEFORE any tool runs. A call we
        # cannot address is a call we must not execute: running it would spend
        # the budget on evidence we can never hand back.
        call_ids: list[str] = []
        for call in calls:
        ...
        # H6 IN-FLIGHT DEDUPE. A batched sweep regularly carries two spellings of
        # the same lookup; firing both costs a second network round trip, a
        # second metered charge, and — worse — a SECOND ledger row for one
        # source, which splits that source's citations across two [n] and makes
        # the evidence budget pay for it twice. Duplicates share the owner's
        # single task and its single committed block, so both tool_call_ids are
        # still answered and numbering is unchanged.
        owner_of: list[int] = []
        first_seen: dict = {}
        ...
        for i, call in enumerate(run_calls):
            if owner_of[i] == i:
                tasks_by_owner[i] = asyncio.ensure_future(
                    _run_tool(call, question, ledger, deadline))
        ...
        if cancelled:
            # Drain the cancellations. Without this the event loop is left
            # holding tasks whose exceptions were never retrieved, which on some
            # runtimes surfaces as a warning attributed to the miner and, worse,
            # lets a half-cancelled coroutine resume between awaits later in the
            # run. Bounded, and failures here are irrelevant by construction.
            try:
        ...
        try:
            ...
            for i in range(len(run_calls)):
                ...
                if owner not in bodies:
                    # v32.5: ledger rows are appended HERE, in CALL order —
                    # never inside the concurrent coroutines — so [n] numbering
                    # is a function of the transcript, not of network latency.
                    bodies[owner] = _commit_tool_output(results[owner], ledger)
                body = bodies[owner]
                if owner != i:
                    body = (body + "\n# (this call duplicates an earlier call in "
                            "the same turn — same numbered results, already "
                            "shown above)")
                pending.append({"role": "tool", "tool_call_id": call_ids[i],
                                "content": body})
            for i in range(8, len(calls)):
                pending.append({"role": "tool", "tool_call_id": call_ids[i],
                                "content": "# skipped: per-turn tool budget reached — re-issue next turn if still needed"})
        except Exception:
            # The ledger may already hold rows from this turn — that is fine and
            # deliberate: the rescue ladder reads the LEDGER, not the transcript,
            # so the evidence survives even though this turn is discarded.
            break
...
async def _write_from_digest(question: str, ledger: EvidenceLedger, deadline: float) -> str:
    ...
    # never started; (2) for 31s < left <= 45s the fallback branch was skipped
    # and the cleanup loop cancelled the still-running primary; (3)
    # FIRST_COMPLETED let a fast-junk rung cancel a slow-good one. The
    # sequential walk below has none of those failure modes, and an answer that
    # exists beats one that races.
    # Rung 0 must not eat the whole window: it is a real call now and can run
    # the entire rescue out, leaving rung 1 unreachable for any entry budget in
    # [14, 69). Reserve rung 1's minimum up front. And this stage must not
    # consume the whole tail either — _knowledge_resort and _schema_output both
    # refuse to start under 12s, so DIGEST_TAIL_S is held back for them.
    ...
    for i, model in enumerate(rungs):
        ...
        if i == 0:
            # rung 1 needs >=14s of its own; never hand rung 0 more than half
...
async def _schema_output(question: str, answer: str, schema, deadline: float) -> object | None:
    ...
    # response — so this ladder is the difference between partial credit and a
    # hard zero, and it must not be able to fail as a unit. The three rungs are
    # three DIFFERENT model families (gpt-oss, deepseek, glm): a capacity stall
    # or a schema-formatting quirk in one does not correlate with the others.
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # H5: the worker is reused across tasks and _SPEND / _SEARCH are module
    # state. Clear them BEFORE the probe so a previous task's drained budget
    # cannot gate this task's briefing and audit off before a single call.
    _reset_run_state()
    try:
