...
SEARCH_TIMEOUT = 10.0
FETCH_TIMEOUT = 15.0
# batch 7c4764c5 실측: 실패한 llm_chat 10건이 벽시계 797초를 태웠다 (50런 기준).
# ToolInvocationTimeoutError 7건이 각각 정확히 LLM_TIMEOUT(90초)를 소모했고,
# MODEL_SCHEDULING="state_aware" 는 순차 폴백이라 한 턴에 90초가 두 번 겹친 런도 있었다
# (9c4a8a42:v5 = 182초). 성공한 llm_chat 544건의 지연 분포는 p50 5.6s / p90 14.0s /
# p99 38.6s 이고 45초를 넘긴 성공 호출은 2건(0.4%)뿐이다. 45초로 자르면 성공 호출은
# 사실상 잃지 않으면서 타임아웃 1건당 45초를 회수한다 (추정 -315초/50런 = -63초/검증자).
LLM_TIMEOUT = 45.0
# embed_text 는 50런 중 1런에서만 발동했지만 그 런에서 90초를 썼다 (최대 34.4초/호출).
# 챔피언 후보 uid 178 은 임베딩을 아예 쓰지 않는다. 도구는 남기되 꼬리만 자른다.
EMBEDDING_TIMEOUT = 30.0
DEADLINE_NOTICE_SECONDS = 150.0
BATCHED_RETRIEVAL_PREVIEW_CHARS = 120_000
# 실측: gemma-4-31b-it 538콜에 프롬프트 토큰 8,141,513 = 콜당 15,133 토큰이고
# 비용의 98%가 프롬프트다 (완성 토큰은 콜당 290). 매 턴 재주입되는
# "Harness research memory" 블록의 focused_excerpts 가 그 프롬프트의 지배항이다.
# 상한을 절반으로 내려 프롬프트를 줄인다. VFS 는 계속 재읽기 가능하므로 증거는
# 사라지지 않고, 관측 재생 창만 좁아진다.
...
# 인용 슬라이스는 검증자가 통째로 실체화한다. 실측(batch 7c4764c5):
# 우리 노트 총량 중앙값 13,989자 / 최대 75,188자, 레퍼런스 답안은 중앙값 5,617자
# (레퍼런스는 ref 당 1슬라이스 약 2,400자). 전부 0점이 난 d4aff3cd 에서는 우리 답이
# 레퍼런스와 바이트 단위로 동일했는데도 5/5 패배했고 심판이 이유를 명시했다:
# "Answer 2 is correct but has raw data notes instead of a logic-supporting synthesis".
# 그 런의 노트는 71,269자였다 (레퍼런스 7,137자, 10.0배). 상한을 명시해 페이지 덤프를
# 막고 동시에 120,000자 실체화 천장에서 멀어진다.
CITATION_REF_MAX_CHARS = 6_000
CITATION_TOTAL_MAX_CHARS = 30_000
# 슬라이스가 하나도 없는 ref 는 노트 전체가 실체화된다. 예산을 적용할 때도
# ref 당 최소 한 개의 비어있지 않은 슬라이스는 반드시 남긴다.
...
# 지출 거버너 문턱 — 세션 하드리밋 대비 비율.
# 0.14/0.20 은 실측 런당 비용분포에서 유도했다 (하드리밋 $0.50 기준 $0.070/$0.100).
SPEND_GOVERNOR_SOFT_FRACTION = 0.14
SPEND_GOVERNOR_HARD_FRACTION = 0.20
SPEND_GOVERNOR_FALLBACK_LIMIT_USD = 0.50
# 거버너가 켜진 뒤 모델에게 허용하는 마감 턴 수. 이걸 넘으면 하네스가 끝낸다.
SPEND_GOVERNOR_MAX_CLOSING_TURNS = 3
# 시간 문턱. 하드월은 300초이고 초과하면 그 태스크는 0점이다.
# 210 실측 150런 중 20런이 200초를 넘었고 최대가 294.9초였다 (여유 5초).
#
# 값을 고른 근거: 130/190 으로 조여서 A/B 를 돌려봤더니 총 시간이 0.4% 밖에
# 안 줄었고(610,546 vs 613,134ms) 점수 표본만 내려갔다. 대부분의 런은 어차피
# 130초 안에 끝나고, 남는 편차는 검증자간 노이즈(같은 태스크에서 중앙값 1.95배,
# 최대 4.9배)가 지배한다. 증거 없는 이득에 점수를 지불하지 않는다.
# 150/210 으로도 210 이 실제로 보인 꼬리(최대 294.9초)는 막힌다.
# hard 이후 최종답변 LLM 1회 + 인용조립에 90초를 남긴다.
...
def _trim_citation_slice_group(
    ...
    슬라이스는 원래 끝점을 넘겨 늘리지 않는다 (문서 밖 오프셋을 만들지 않기 위해).
...
def _budget_citation_slices(
    ...
    """
    trimmed = [
        _trim_citation_slice_group(group, ref_limit, min_slice)
...
async def _prose_chat_with_retry(
    ...
    return await _chat_with_scheduling(
        ("openrouter_gemma", "chutes_gemma", "glm5"),
        messages,
        None,
        tool_choice,
        False,
