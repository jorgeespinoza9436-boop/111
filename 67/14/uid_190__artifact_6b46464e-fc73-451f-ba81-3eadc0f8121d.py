"""SN67 Harnyx miner — v26 deep-research pipeline with pinned-source lane and set-operation ledger. [slot 51 build 2026-07-30T00:00:00+00:00]"""
from __future__ import annotations
_AGENT_VARIANT = "b7e2c41a55d09f26"
...
POOL_MEMBER_MIN_REMAINING_SECONDS      = 90.0
POOL_MEMBER_GATE_MIN_REMAINING_SECONDS = 45.0
POOL_MEMBER_MAX_TO_FETCH               = 6
POOL_MEMBER_SEARCH_RESULTS             = 3
POOL_MEMBER_QUERY_MAX_WORDS            = 12
POOL_MEMBER_ATTRIBUTE_TERMS_MAX        = 4
...
NUMERIC_AUDIT_TEMPERATURE = 0.20
NUMERIC_AUDIT_MIN_LENGTH_RATIO = 0.6

PIN_LANE_MIN_REMAINING_SECONDS       = 70.0
PIN_LANE_GATE_MIN_REMAINING_SECONDS  = 40.0
PIN_LANE_MAX_PAGE_PINS               = 6
PIN_LANE_MAX_AUTHORITY_PINS          = 2
PIN_LANE_AUTHORITY_SEARCH_RESULTS    = 4
PIN_LANE_AUTHORITY_QUERY_MAX_WORDS   = 18
PIN_TITLE_MAX_WORDS                  = 14
PIN_TITLE_MIN_CHARS                  = 3
PIN_ENTITY_MIN_TITLES                = 2
WIKIPEDIA_HOST                       = "en.wikipedia.org"
PIN_AUTHORITY_DOMAIN_MAP: tuple[tuple[str, str], ...] = (
    ("office for national statistics", "ons.gov.uk"),
    ("(ons)", "ons.gov.uk"),
    ("worldatlas", "worldatlas.com"),
    ("behavioral risk factor surveillance system", "cdc.gov"),
    ("(brfss)", "cdc.gov"),
...
_WIKI_QUOTED_TITLE_RE = re.compile(
    r"Wikipedia(?:'s)?[^'\"‘“]{0,50}['\"‘“]([^'\"’”]{3,90})['\"’”]"
)
_WIKI_ARTICLE_FOR_RE = re.compile(
    r"Wikipedia (?:article|page|entry|list) (?:for|on|about|titled)\s+((?:['\"‘“][^'\"’”]{3,90}['\"’”])|(?:[A-Z][\w().'&-]*(?:\s+[\w().'&-]+){0,9}?))(?=[,.;:?]|\s+(?:page|article)\b|$)"
)
_QUOTED_TITLE_WIKI_AFTER_RE = re.compile(
    r"['\"‘“]([^'\"’”]{3,90})['\"’”][^.?]{0,40}?\b(?:article|page|list|table|entry)\b[^.?]{0,30}?\bWikipedia\b"
)
_QUESTION_ITALIC_TITLE_RE = re.compile(r"\*([^*\n]{2,80})\*")
_QUESTION_QUOTED_TITLE_RE = re.compile(r"['‘“\"]([^'’”\"\n]{3,80})['’”\"]")
_RESPECTIVE_WIKI_PAGE_RE = re.compile(
    r"(?i)\b(?:respective|each(?:'s)?|their)\b[^.?]{0,60}\b(?:infobox(?:es)?|wikipedia (?:page|article)s?|pages? on (?:english )?wikipedia|articles? on (?:english )?wikipedia)"
)
_QUESTION_MENTIONS_WIKIPEDIA_RE = re.compile(r"(?i)\bwikipedia\b")
_ACCORDING_TO_SOURCE_RE = re.compile(
    r"(?i)\b(?:according to|as reported by|per|as listed (?:by|on|in)|as published by)\s+(?:the\s+)?([A-Z][\w&'./ -]{2,60}?)(?:\s*\([A-Z]{2,12}\))?(?=\s*[,.;:?]|\s+(?:which|what|who|how|when|where|for|data|figures|statistics)\b)"
)
_SET_INTERSECTION_CUE_RE = re.compile(
    r"(?i)\b(?:every one of|each (?:one )?of the|all (?:of )?(?:the|these|those) (?:following|three|four|five|listed)|in common|common to (?:all|both)|appears? (?:in|on) (?:both|all)|present (?:in|on) (?:both|all)|shared by|both of the|on all (?:three|four|five)|across all (?:three|four|five))\b"
)
_SET_UNION_CUE_RE = re.compile(
    r"(?i)\b(?:any (?:one )?of the following|at least one of|either of|in any of)\b"
)
...
@dataclass(frozen=True, slots=True)
class SourcePin:
    """A source the question itself names as authoritative for a fact.

    kind: 'wiki_page'   — an exact page title the question quotes;
          'wiki_entity' — an enumerated entity whose own page holds the pinned fact;
          'authority'   — a named organization/report pinned to a domain (may be '').
    """
    kind: str
    label: str
    url: str = ""
...
@dataclass(frozen=True, slots=True)
class SetOperationPlan:
    """Quantifier semantics the question imposes over multiple containers."""
    operation: str
    cue: str
...
async def _answer_question(question: str, *, state: ResearchRunState, deadline: float) -> Response:
    ...
    source_pins = _extract_source_pins(question)
    set_plan = _detect_set_operation(question)
    if (
        source_pins
        and perf_counter() < augmentation_deadline
        and deadline - perf_counter() >= PIN_LANE_MIN_REMAINING_SECONDS
    ):
        pin_gate = await _augment_pinned_source_evidence(
            question=question, pins=source_pins, contract=research_contract,
        ...
        if pin_gate.accepted_packets:
            accepted_packets = (*accepted_packets, *pin_gate.accepted_packets)
            accepted_observations = (*accepted_observations, *pin_gate.observations)
            coverage_state = _fallback_coverage_state(contract=research_contract, observations=accepted_observations)
            coverage = _coverage_from_coverage_state(coverage_state, accepted_observations)
    ...
    if enumerated_entities and perf_counter() < augmentation_deadline:
        missing_entities = _entities_missing_from_evidence(
            entities=enumerated_entities, packets=accepted_packets, observations=accepted_observations,
        )
        if missing_entities:
    ...
    if (
        ...
        if role_gap_gate.accepted_packets:
            ...
            coverage = _coverage_from_coverage_state(coverage_state, accepted_observations)
    if (
        perf_counter() < augmentation_deadline
        and deadline - perf_counter() >= POOL_SWEEP_MIN_REMAINING_SECONDS
    ):
        pool_plan = await _detect_bounded_pool(
            question=question, existing_packets=accepted_packets, state=state, deadline=deadline,
        )
        if pool_plan.is_bounded_pool and pool_plan.criteria:
            pool_gate = await _augment_bounded_pool_evidence(
                question=question, plan=pool_plan, contract=research_contract,
                role_ledger=role_ledger, existing_packets=accepted_packets,
                existing_observations=accepted_observations, state=state, deadline=deadline,
            )
            if pool_gate.accepted_packets:
                ...
                coverage = _coverage_from_coverage_state(coverage_state, accepted_observations)
        if (
            pool_plan.is_bounded_pool
            and pool_plan.pool_members
            and deadline - perf_counter() >= POOL_MEMBER_MIN_REMAINING_SECONDS
        ):
            member_gate = await _augment_pool_member_evidence(
                question=question, plan=pool_plan, contract=research_contract,
                role_ledger=role_ledger, existing_packets=accepted_packets,
                existing_observations=accepted_observations, state=state, deadline=deadline,
            )
            if member_gate.accepted_packets:
                accepted_packets = (*accepted_packets, *member_gate.accepted_packets)
                accepted_observations = (*accepted_observations, *member_gate.observations)
                coverage_state = _fallback_coverage_state(contract=research_contract, observations=accepted_observations)
                coverage = _coverage_from_coverage_state(coverage_state, accepted_observations)
    if not accepted_packets:
        return await _answer_from_raw_snippets(
            ...
            source_pins=source_pins, set_plan=set_plan,
        )
    final_answer = await _synthesize_final_answer(
        ...
        deadline=deadline, source_pins=source_pins, set_plan=set_plan,
    )
    final_answer = await _audit_and_repair_answer(
        question=question, draft=final_answer,
        accepted_packets=accepted_packets, deadline=deadline,
        source_pins=source_pins, set_plan=set_plan,
    )
...
def _entities_missing_from_evidence(
    *, entities: tuple[str, ...], packets: tuple[AcceptedEvidence, ...],
    ...
    blob_parts += [
        f"{p.source_text} {p.source_result_text}"
    ...
    for entity in entities[:ENTITY_GAP_MAX_ENTITIES_TO_AUGMENT]:
        ...
        try:
            response = await search_web(
                [query], provider=_SEARCH_PROVIDER,
        ...
        for result in tuple(getattr(response, "results", ()) or ()):
            ...
            candidates.append(EvidenceCandidate(
                ...
                result_id=_string_value(getattr(result, "result_id", "")) or f"G{counter}",
...
def _wikipedia_url_for_title(title: str) -> str:
    ...
    if not cleaned or len(cleaned) < PIN_TITLE_MIN_CHARS or len(cleaned.split()) > PIN_TITLE_MAX_WORDS:
        return ""
    return f"https://{WIKIPEDIA_HOST}/wiki/" + quote(cleaned.replace(" ", "_"), safe="_()',.:-")


def _wiki_page_key(url: str) -> str:
    """Canonical key for an English-Wikipedia article URL; '' for anything else."""
    try:
    ...
    if not path.startswith("/wiki/"):
        return ""
    title = path[len("/wiki/"):]
    return re.sub(r"[\s_]+", "_", title).strip("_").lower()
...
def _clean_pin_title(raw: str) -> str:
    cleaned = " ".join(raw.strip().strip("'\"‘’“”*").strip(".,;:").split())
    if len(cleaned) < PIN_TITLE_MIN_CHARS or len(cleaned.split()) > PIN_TITLE_MAX_WORDS:
        return ""
    if not any(ch.isalpha() for ch in cleaned) or "/" in cleaned:
        return ""
    return cleaned


def _entity_pin_title_ok(title: str) -> bool:
    """Entity pins come from enumerations; one-word short values ('China') are
    usually data values quoted in the question, not pages holding pinned facts."""
    return len(title.split()) >= 2 or len(title) >= 10


def _question_title_candidates(question: str) -> tuple[str, ...]:
    """Italic-marked and quoted title phrases from the question, in order, deduped."""
    titles: list[str] = []
    seen: set[str] = set()
    for pattern in (_QUESTION_ITALIC_TITLE_RE, _QUESTION_QUOTED_TITLE_RE):
        for match in pattern.finditer(question):
            title = _clean_pin_title(match.group(1))
...
def _extract_source_pins(question: str) -> tuple[SourcePin, ...]:
    """Deterministically extract the sources the question itself pins facts to.

    Three shapes: exact quoted Wikipedia page titles; enumerated entities whose
    'respective' Wikipedia pages/infoboxes hold the pinned values; and named
    authorities mapped to their home domain. Order is significance order.
    """
    pins: list[SourcePin] = []
    seen_keys: set[str] = set()
    seen_authority_domains: set[str] = set()

    def _add(pin: SourcePin) -> None:
        key = f"{pin.kind}:{pin.label.lower()}:{pin.url.lower()}:{pin.domain}"
        if not pin.label or key in seen_keys:
            return
        if pin.kind == "authority" and pin.domain:
            if pin.domain in seen_authority_domains:
                return
            seen_authority_domains.add(pin.domain)
        seen_keys.add(key)
        pins.append(pin)

    for match in _WIKI_QUOTED_TITLE_RE.finditer(question):
        title = _clean_pin_title(match.group(1))
        url = _wikipedia_url_for_title(title) if title else ""
        if url:
            _add(SourcePin(kind="wiki_page", label=title, url=url))
    for pattern in (_WIKI_ARTICLE_FOR_RE, _QUOTED_TITLE_WIKI_AFTER_RE):
        for match in pattern.finditer(question):
            title = _clean_pin_title(match.group(1))
            url = _wikipedia_url_for_title(title) if title else ""
            if url:
                _add(SourcePin(kind="wiki_page", label=title, url=url))
    titles = _question_title_candidates(question)
    respective_wiki = bool(_RESPECTIVE_WIKI_PAGE_RE.search(question))
    if respective_wiki or (len(titles) >= PIN_ENTITY_MIN_TITLES and _QUESTION_MENTIONS_WIKIPEDIA_RE.search(question)):
        entity_titles = titles or _enumerated_question_entities(question)
        for title in entity_titles:
            url = _wikipedia_url_for_title(title)
            if url and _entity_pin_title_ok(title):
                _add(SourcePin(kind="wiki_entity", label=title, url=url))
    elif len(titles) >= PIN_ENTITY_MIN_TITLES and re.search(r"(?i)\b(?:the following|these|each|every one)\b", question):
        for title in titles:
            url = _wikipedia_url_for_title(title)
            if url and _entity_pin_title_ok(title):
                _add(SourcePin(kind="wiki_entity", label=title, url=url))
    lowered = question.lower()
    for name_fragment, domain in PIN_AUTHORITY_DOMAIN_MAP:
        if name_fragment in lowered:
            _add(SourcePin(kind="authority", label=name_fragment.strip("()").upper() if len(name_fragment) <= 6 else name_fragment.title(), domain=domain))
    for match in _ACCORDING_TO_SOURCE_RE.finditer(question):
        label = _clean_pin_title(match.group(1))
        if not label or "wikipedia" in label.lower():
            continue
        low = label.lower()
        existing_authority_labels = [p.label.lower() for p in pins if p.kind == "authority"]
        if any(low in existing or existing in low for existing in existing_authority_labels):
            continue
        _add(SourcePin(kind="authority", label=label))
    return tuple(pins)


_SET_CONTAINER_COUNT_WORDS = {"two": 2, "three": 3, "four": 4, "five": 5, "six": 6, "seven": 7, "eight": 8}


def _detect_set_operation(question: str) -> SetOperationPlan | None:
    match = _SET_INTERSECTION_CUE_RE.search(question)
    operation = "intersection" if match else ""
    if not match:
        match = _SET_UNION_CUE_RE.search(question)
        operation = "union" if match else ""
    if not match or not operation:
        return None
    tail = question[match.end():]
    containers = _question_title_candidates(tail) or _question_title_candidates(question)
    count_match = re.match(
        r"\s*(?:the\s+)?(?:following\s+)?(two|three|four|five|six|seven|eight|\d+)\b",
        tail, re.IGNORECASE,
    )
    if count_match:
        raw_count = count_match.group(1).lower()
        expected = int(raw_count) if raw_count.isdigit() else _SET_CONTAINER_COUNT_WORDS.get(raw_count, 0)
        if expected >= 2 and len(containers) > expected:
            containers = containers[-expected:]
    if len(containers) < 2:
        containers = ()
    return SetOperationPlan(operation=operation, cue=match.group(0), containers=containers)


def _pin_matches_packet_url(pin: SourcePin, url: str) -> bool:
    if pin.kind in ("wiki_page", "wiki_entity"):
        pin_key = _wiki_page_key(pin.url)
        return bool(pin_key) and _wiki_page_key(url) == pin_key
    if pin.domain:
        host = _url_host(url)
        return host == pin.domain or host.endswith(f".{pin.domain}")
    return False


def _packet_indices_for_pin(pin: SourcePin, packets: tuple[AcceptedEvidence, ...]) -> tuple[int, ...]:
    return tuple(i for i, p in enumerate(packets, start=1) if _pin_matches_packet_url(pin, p.url))


def _pin_covered_by_packets(pin: SourcePin, packets: tuple[AcceptedEvidence, ...]) -> bool:
    return bool(_packet_indices_for_pin(pin, packets))


def _pin_citation_directive(pins: tuple[SourcePin, ...], packets: tuple[AcceptedEvidence, ...]) -> str:
    if not pins:
        return ""
    lines: list[str] = []
    for pin in pins:
        indices = _packet_indices_for_pin(pin, packets)
        locator = pin.url or (f"domain {pin.domain}" if pin.domain else "no direct locator")
        packet_note = f"own-source packets {list(indices)}" if indices else "NO packet from this exact source was admitted"
        lines.append(f"- {pin.kind} '{pin.label}' → {locator} → {packet_note}")
    return (
        "PINNED SOURCES — the question names these exact sources as authoritative:\n"
        + "\n".join(lines)
        + "\nPIN-CITE BINDING RULE: every sentence stating a value the question pins to one of "
        "these sources MUST carry a citation to a packet from that exact source (its own-source "
        "packets above) whenever one exists. Never substitute an overview article, series/list "
        "page, aggregator, or third-party page citation for a value pinned to a specific page — "
        "a per-entity value (a running time, a vote count, a personnel roster, a census figure) "
        "must cite that entity's own pinned page, not a page about the group or franchise. If no "
        "own-source packet exists for a pinned value, cite the closest authoritative packet and "
        "name in prose the pinned source the figure agrees with."
    )


def _set_operation_directive(plan: SetOperationPlan | None) -> str:
    if plan is None:
        return ""
    containers = "; ".join(f"'{c}'" for c in plan.containers) if plan.containers else "(identify the containers from the question)"
    if plan.operation == "intersection":
        return (
            f"SET-OPERATION CONTRACT (quantifier detected: '{plan.cue}'): the requested set is the "
            f"INTERSECTION across ALL containers: {containers}.\n"
            "1. Give one short block per container listing the members credited/present in THAT "
            "container, each line citing the packet drawn from that container's own source.\n"
            "2. Then state the intersection explicitly: the headline answer contains ONLY members "
            "present in EVERY container.\n"
            "3. A member absent from ANY single container must be excluded from the headline set "
            "and called out explicitly with the container it is missing from (and the reason, if "
            "the evidence gives one).\n"
            "Never present the union of the containers as the answer when the quantifier demands "
            "the intersection."
        )
    return (
        f"SET-OPERATION CONTRACT (quantifier detected: '{plan.cue}'): the requested set is the "
        f"UNION over the containers: {containers}. List each qualifying member once with the "
        "container(s) and citation(s) that establish it; do not drop members that appear in only "
        "one container."
    )


def _merge_pinned_gate_results(
    results: tuple[GateResult, ...], *, existing_packet_count: int,
) -> GateResult:
    """Merge per-pin gate groups whose observations were indexed relative to the
    shared existing-packet prefix (offset + group-local), remapping each onto the
    combined packet ordering the caller will append after that prefix."""
    ...
    for result in results:
        ...
        for local_index, packet in enumerate(result.accepted_packets, start=1):
            ...
            merged_index = packet_index_by_key.get(key)
            if merged_index is None:
                packets.append(packet)
                merged_index = len(packets)
                packet_index_by_key[key] = merged_index
            local_to_merged[local_index] = merged_index
        for obs in result.observations:
            merged_index = local_to_merged.get(obs.packet_index - existing_packet_count)
            if merged_index is None:
                continue
            observations.append(replace(obs, packet_index=existing_packet_count + merged_index))
    return GateResult(accepted_packets=tuple(packets), observations=tuple(observations))


def _pinned_page_candidate_groups(
    candidates: tuple[EvidenceCandidate, ...],
) -> tuple[tuple[str, tuple[EvidenceCandidate, ...]], ...]:
    grouped: dict[str, list[EvidenceCandidate]] = {}
    for candidate in candidates:
        key = _wiki_page_key(candidate.url) or _url_host(candidate.url) or "pin_misc"
        grouped.setdefault(key, []).append(candidate)
    return tuple((f"pin_{i}", tuple(group)) for i, (_key, group) in enumerate(grouped.items(), start=1))


async def _augment_pinned_source_evidence(
    *, question: str, pins: tuple[SourcePin, ...], contract: ResearchContract,
    role_ledger: tuple[ResearchPlanRole, ...], existing_packets: tuple[AcceptedEvidence, ...],
    existing_observations: tuple[EvidenceObservation, ...], state: ResearchRunState,
    deadline: float,
) -> GateResult:
    """Pinned-source acquisition lane: directly fetch every page the question pins
    facts to and admit chunk evidence from it, so each pinned claim can carry a
    citation from the exact named source rather than a substitute page."""
    page_pins = tuple(
        pin for pin in pins
        if pin.kind in ("wiki_page", "wiki_entity") and pin.url
        and not _pin_covered_by_packets(pin, existing_packets)
    )[:PIN_LANE_MAX_PAGE_PINS]
    authority_pins = tuple(
        pin for pin in pins
        if pin.kind == "authority" and pin.domain
        and not _pin_covered_by_packets(pin, existing_packets)
    )[:PIN_LANE_MAX_AUTHORITY_PINS]
    candidates: list[EvidenceCandidate] = []
    seen_candidate_keys: set[str] = set()
    counter = 0
    fetch_failed_pins: tuple[SourcePin, ...] = ()
    if page_pins and deadline - perf_counter() >= PIN_LANE_GATE_MIN_REMAINING_SECONDS:
        seeds = tuple(
            SearchResultSeed(
                search_receipt_id="", search_result_id="",
                slot_id=PRIMARY_SOURCE_SLOT_ID,
                slot_intent=_slot_intent_for_slot(PRIMARY_SOURCE_SLOT_ID),
                url=pin.url, title=pin.label, note="",
            ) for pin in page_pins
        )
        page_entries, _fetch_stats = await _page_entries_from_search_seeds(
            seeds=seeds, state=state, loop_index=3,
        )
        fetched_entries = tuple(e for e in page_entries if e.source.source_kind == "fetch_page")
        fetched_keys = {_wiki_page_key(e.source.url) for e in fetched_entries}
        fetch_failed_pins = tuple(pin for pin in page_pins if _wiki_page_key(pin.url) not in fetched_keys)
        chunks = _loop_chunks_from_page_entries(
            page_entries=fetched_entries, query_label="pinned_source", state=state, loop_index=3,
        )
        if chunks and deadline - perf_counter() >= PIN_LANE_GATE_MIN_REMAINING_SECONDS:
            selected_chunks = await _select_page_chunks(
                question=question, loop_index=3, chunks=chunks,
                role_ledger=role_ledger, state=state,
            )
            chunk_candidates, counter = _selected_chunks_to_candidates(
                chunks=selected_chunks, seen_candidate_keys=seen_candidate_keys,
                candidate_counter=counter,
            )
            candidates.extend(chunk_candidates)
    snippet_pins = (*fetch_failed_pins, *authority_pins)
    for pin in snippet_pins:
        if deadline - perf_counter() < PIN_LANE_GATE_MIN_REMAINING_SECONDS:
            break
        base_query = " ".join(question.split()[:PIN_LANE_AUTHORITY_QUERY_MAX_WORDS])
        if pin.kind in ("wiki_page", "wiki_entity"):
            pin_query = _constrained_site_query(f'"{pin.label}"', WIKIPEDIA_HOST)
        else:
            pin_query = _constrained_site_query(base_query, pin.domain)
        try:
            response = await search_web(
                [pin_query], provider=_SEARCH_PROVIDER,
                num=PIN_LANE_AUTHORITY_SEARCH_RESULTS, timeout=LITE_SEARCH_TOOL_TIMEOUT_SECONDS,
            )
        except Exception:
            continue
        receipt_id = getattr(response, "receipt_id", "") or ""
        for result in tuple(getattr(response, "results", ()) or ()):
            ...
            if not url or not note or _blocked_fetch_url_reason(url):
                continue
            key = f"pinned_source:{_normalize_url(url) or url}:{_text_fingerprint(note)}"
            if key in seen_candidate_keys:
                continue
            seen_candidate_keys.add(key)
            counter += 1
            candidates.append(EvidenceCandidate(
                candidate_id=f"N{counter}",
                parent_candidate_id=f"N{counter}",
                slot_id=PRIMARY_SOURCE_SLOT_ID,
                slot_intent=_slot_intent_for_slot(PRIMARY_SOURCE_SLOT_ID),
                text_part="search_snippet", text_start=0, text_end=len(note),
                receipt_id=receipt_id,
                result_id=_string_value(getattr(result, "result_id", "")) or f"N{counter}",
                url=url, title=getattr(result, "title", None), source_text=note,
                query=pin_query, source_kind=SEARCH_SNIPPET_EVIDENCE_SOURCE_KIND,
            ))
    if not candidates or deadline - perf_counter() < PIN_LANE_GATE_MIN_REMAINING_SECONDS:
        return GateResult(accepted_packets=(), observations=())
    groups = _pinned_page_candidate_groups(tuple(candidates))
    gate_timeout = deadline - PIN_LANE_GATE_MIN_REMAINING_SECONDS - perf_counter()
    gate_results = await _gate_groups_with_deadline((
        _run_observation_gate_once(
            question=question, loop_index=1, existing_packets=existing_packets,
            existing_observations=existing_observations, contract=contract,
            retrieval_roles=role_ledger, candidates=group_candidates,
            model=EVIDENCE_GATE_MODEL, state=state, stage="pinned_source_gate", lane=group_id,
        ) for group_id, group_candidates in groups
    ), timeout=gate_timeout)
    return _merge_pinned_gate_results(tuple(gate_results), existing_packet_count=len(existing_packets))
...
def _build_bounded_pool_plan_messages(
    *, question: str, existing_packets: tuple[AcceptedEvidence, ...],
) -> list[dict[str, str]]:
    evidence_brief = [{"packet_number": i, "title": p.title or "", "url": p.url} for i, p in enumerate(existing_packets[:12], start=1)]
    system_content = (
        "ROLE: bounded-pool planner for a research pipeline. Decide whether the question ranges over a "
        "BOUNDED candidate pool — either the candidates are explicitly listed in the question, or they "
        "form a closed set that can be fully enumerated (e.g. 'which Maine counties...', 'of Gary Allan's "
        "studio albums released in 2005, 2010, 2013...', 'the 5 male main-cast actors...'). "
        "A single-entity lookup or an open-ended question is NOT a bounded pool.\n\n"
        "If it IS a bounded pool, output the pool subject, the complete member list if you can name it "
        "from the question or general structure, and EACH distinct filter criterion the question imposes "
        "as a separate checklist item. If it is NOT a bounded pool, set is_bounded_pool to false and "
        "leave the lists empty.\n\n"
        "Output ONLY a JSON object: {\"is_bounded_pool\": bool, \"pool_subject\": str, "
        "\"pool_members\": [str, ...], \"criteria\": [str, ...]}. No prose."
    )
    user_content = (
        f"Question: {question}\n\n"
        f"Evidence gathered so far (titles only):\n{_format_records_section('EVIDENCE_BRIEF', 'packet', evidence_brief)}\n\n"
...
def _bounded_pool_plan_from_payload(payload: dict[str, object] | None) -> BoundedPoolPlan:
    ...
    if not criteria:
        return BoundedPoolPlan()
    return BoundedPoolPlan(
        is_bounded_pool=True,
        pool_subject=str(payload.get("pool_subject") or "").strip(),
        pool_members=members[:POOL_SWEEP_MAX_POOL_SIZE],
        criteria=criteria[:POOL_SWEEP_MAX_CRITERIA],
    )


async def _detect_bounded_pool(
    *, question: str, existing_packets: tuple[AcceptedEvidence, ...],
    state: ResearchRunState, deadline: float,
) -> BoundedPoolPlan:
    ...
    payload = await _call_json_llm_with_retry(
        messages=messages, model=RESEARCH_PLAN_MODEL, temperature=PLANNING_TEMPERATURE,
        validate_payload=_bounded_pool_payload_validator(), state=state,
        stage="bounded_pool_detection",
    )
...
async def _augment_bounded_pool_evidence(
    *, question: str, plan: BoundedPoolPlan, contract: ResearchContract,
    role_ledger: tuple[ResearchPlanRole, ...], existing_packets: tuple[AcceptedEvidence, ...],
    existing_observations: tuple[EvidenceObservation, ...], state: ResearchRunState,
    deadline: float,
) -> GateResult:
    ...
    for criterion in plan.criteria[:POOL_SWEEP_MAX_CRITERIA]:
        ...
        try:
            response = await search_web(
                [query], provider=_SEARCH_PROVIDER,
                num=POOL_SWEEP_SEARCH_RESULTS_PER_CRITERION, timeout=LITE_SEARCH_TOOL_TIMEOUT_SECONDS,
            )
        except Exception:
            continue
        receipt_id = getattr(response, "receipt_id", "") or ""
        for result in tuple(getattr(response, "results", ()) or ()):
            ...
            counter += 1
            candidates.append(EvidenceCandidate(
                candidate_id=f"P{counter}",
                parent_candidate_id=f"P{counter}",
                slot_id=PRIMARY_SOURCE_SLOT_ID,
                slot_intent=_slot_intent_for_slot(PRIMARY_SOURCE_SLOT_ID),
                text_part="search_snippet", text_start=0, text_end=len(note),
                receipt_id=receipt_id,
                result_id=_string_value(getattr(result, "result_id", "")) or f"P{counter}",
                url=url, title=getattr(result, "title", None), source_text=note,
                query=query, source_kind=SEARCH_SNIPPET_EVIDENCE_SOURCE_KIND,
            ))
    ...
    return await _run_observation_gate_once(
        ...
        existing_observations=existing_observations, contract=contract,
        retrieval_roles=role_ledger, candidates=tuple(candidates),
        model=EVIDENCE_GATE_MODEL, state=state, stage="bounded_pool_gate", lane="bounded_pool",
    )


def _pool_attribute_terms(plan: BoundedPoolPlan) -> tuple[str, ...]:
    terms: list[str] = []
    for criterion in plan.criteria:
        for token in re.findall(r"[A-Za-z][A-Za-z-]{2,}", criterion):
            low = token.lower()
            if low in _ENTITY_HINT_STOPWORDS or low in terms:
                continue
            terms.append(low)
            if len(terms) >= POOL_MEMBER_ATTRIBUTE_TERMS_MAX:
                return tuple(terms)
    return tuple(terms)


def _pool_members_missing_attribute(
    plan: BoundedPoolPlan, packets: tuple[AcceptedEvidence, ...],
) -> tuple[str, ...]:
    terms = _pool_attribute_terms(plan)
    if not terms:
        return ()
    missing: list[str] = []
    for member in plan.pool_members:
        member_low = member.lower()
        probe = _entity_probe_token(member)
        covered = False
        for packet in packets:
            text = f"{packet.source_text} {packet.source_result_text}".lower()
            if member_low not in text and (not probe or probe not in text):
                continue
            if any(term in text for term in terms) and re.search(r"\d", text):
                covered = True
                break
        if not covered:
            missing.append(member)
        if len(missing) >= POOL_MEMBER_MAX_TO_FETCH:
            break
    return tuple(missing)


async def _augment_pool_member_evidence(
    *, question: str, plan: BoundedPoolPlan, contract: ResearchContract,
    role_ledger: tuple[ResearchPlanRole, ...], existing_packets: tuple[AcceptedEvidence, ...],
    existing_observations: tuple[EvidenceObservation, ...], state: ResearchRunState,
    deadline: float,
) -> GateResult:
    members = _pool_members_missing_attribute(plan, existing_packets)
    if not members:
        return GateResult(accepted_packets=(), observations=())
    terms = " ".join(_pool_attribute_terms(plan))
    candidates: list[EvidenceCandidate] = []
    seen_keys: set[str] = set()
    counter = 0
    for member in members:
        if deadline - perf_counter() < POOL_MEMBER_GATE_MIN_REMAINING_SECONDS:
            break
        query = " ".join(f"{member} {plan.pool_subject} {terms}".split()[:POOL_MEMBER_QUERY_MAX_WORDS])
        try:
            response = await search_web(
                [query], provider=_SEARCH_PROVIDER,
                num=POOL_MEMBER_SEARCH_RESULTS, timeout=LITE_SEARCH_TOOL_TIMEOUT_SECONDS,
            )
        except Exception:
            continue
        receipt_id = getattr(response, "receipt_id", "") or ""
        for result in tuple(getattr(response, "results", ()) or ()):
            ...
            if not url or not note or _blocked_fetch_url_reason(url):
                continue
            key = f"pool_member:{_normalize_url(url) or url}:{_text_fingerprint(note)}"
            if key in seen_keys:
                continue
            seen_keys.add(key)
            counter += 1
            candidates.append(EvidenceCandidate(
                candidate_id=f"M{counter}",
                parent_candidate_id=f"M{counter}",
                slot_id=PRIMARY_SOURCE_SLOT_ID,
                slot_intent=_slot_intent_for_slot(PRIMARY_SOURCE_SLOT_ID),
                text_part="search_snippet", text_start=0, text_end=len(note),
                receipt_id=receipt_id,
                result_id=_string_value(getattr(result, "result_id", "")) or f"M{counter}",
                url=url, title=getattr(result, "title", None), source_text=note,
                query=query, source_kind=SEARCH_SNIPPET_EVIDENCE_SOURCE_KIND,
            ))
    if not candidates or deadline - perf_counter() < POOL_MEMBER_GATE_MIN_REMAINING_SECONDS:
        return GateResult(accepted_packets=(), observations=())
    return await _run_observation_gate_once(
        question=question, loop_index=1, existing_packets=existing_packets,
        existing_observations=existing_observations, contract=contract,
        retrieval_roles=role_ledger, candidates=tuple(candidates),
        model=EVIDENCE_GATE_MODEL, state=state, stage="pool_member_gate", lane="pool_member",
    )
...
async def _admit_evidence_from_candidate_beam(
    *, question: str, contract: ResearchContract, role_ledger: tuple[ResearchPlanRole, ...],
    ...
    merged = _merge_grouped_gate_results(tuple(results))
    return _reorder_accepted_evidence_by_domain_corroboration(merged)
...
def _extract_domain_from_url(url: str) -> str:
    """Extract the registered domain from a URL for corroboration grouping."""
    try:
        host = urlsplit(url.strip()).netloc.lower()
    except ValueError:
        return ""
...
def _reorder_accepted_evidence_by_domain_corroboration(
    gate_result: GateResult,
) -> GateResult:
    """
    Reorder evidence so claim-slots supported by 2+ distinct domains come first.
    Lone-domain evidence admitted only when nothing else covers the slot.
    Degrades to original order on any failure.
    """
    packets = gate_result.accepted_packets
    observations = gate_result.observations
    if not packets or not observations:
        return gate_result
    try:
        packet_domains: dict[int, str] = {}
        for i, packet in enumerate(packets, start=1):
            packet_domains[i] = _extract_domain_from_url(packet.url)
        slot_domains: dict[str, set[str]] = {}
        for obs in observations:
            slot_id = obs.slot_id
            domain = packet_domains.get(obs.packet_index, "")
            if domain:
                slot_domains.setdefault(slot_id, set()).add(domain)
        multi_domain_slots = {
            slot_id for slot_id, domains in slot_domains.items()
            if len(domains) >= 2
        }
        multi_domain_packet_indices: set[int] = set()
        for obs in observations:
            if obs.slot_id in multi_domain_slots:
                multi_domain_packet_indices.add(obs.packet_index)
        reordered_packets = tuple(
            [p for i, p in enumerate(packets, start=1) if i in multi_domain_packet_indices] +
            [p for i, p in enumerate(packets, start=1) if i not in multi_domain_packet_indices]
        )
        old_order = list(range(1, len(packets) + 1))
        new_order = (
            [i for i in old_order if i in multi_domain_packet_indices] +
            [i for i in old_order if i not in multi_domain_packet_indices]
        )
        old_to_new = {old: new for new, old in enumerate(new_order, start=1)}
        reordered_obs = tuple(
            replace(obs, packet_index=old_to_new.get(obs.packet_index, obs.packet_index))
            for obs in observations
        )
        return GateResult(
            accepted_packets=reordered_packets,
            observations=reordered_obs,
            coverage=gate_result.coverage,
            role_ledger=gate_result.role_ledger,
            can_answer=gate_result.can_answer,
            missing_questions=gate_result.missing_questions,
        )
    except Exception:
        return gate_result
...
def _budget_matched_synthesis_plan(remaining_seconds: float) -> tuple[str, LlmThinkingConfig, float] | None:
    if remaining_seconds < SYNTH_TIER_FLOOR_MAX_REMAINING_SECONDS:
        return None
    timeout = max(
        SYNTH_CALL_MIN_TIMEOUT_SECONDS,
        min(remaining_seconds - SYNTH_CALL_SAFETY_MARGIN_SECONDS, SYNTH_CALL_MAX_TIMEOUT_SECONDS),
    )
...
def _answer_time_directive(remaining_seconds: float) -> str:
    return (
        "\n\nTIME BUDGET: aim to deliver this answer within about "
        f"{_answer_time_budget_seconds(remaining_seconds)} seconds of wall-clock time, "
        "deliberation included. Settle the answer from the evidence above in a single "
        "pass rather than re-deriving it, and size the write-up to that budget. A "
        "response that has not arrived by the deadline is discarded, so a complete "
        "answer delivered inside the budget beats a longer one that misses it."
    )


def _messages_with_time_directive(
    messages: Sequence[Mapping[str, object]], remaining_seconds: float,
...
async def _synthesize_final_answer(
    *, question: str, accepted_packets: tuple[AcceptedEvidence, ...],
    ...
    source_pins: tuple[SourcePin, ...] = (), set_plan: SetOperationPlan | None = None,
) -> str:
    ...
    if answer is None and _budget_matched_synthesis_plan(deadline - perf_counter()) is not None:
        answer = await _run_synthesis_call(
            messages=messages, deadline=deadline,
            thinking_override=LlmThinkingConfig(enabled=False),
        )
...
def _build_numeric_audit_messages(
    *, question: str, draft: str, accepted_packets: tuple[AcceptedEvidence, ...],
    source_pins: tuple[SourcePin, ...] = (), set_plan: SetOperationPlan | None = None,
) -> list[dict[str, str]]:
    ...
    system_content = (
        "ROLE: precision-and-completeness auditor for an evidence-gated pipeline. You receive a "
        "DRAFT answer and the accepted evidence it was written from. Return a corrected version of the "
        "DRAFT. The final answer is judged pairwise against a reference answer that fully enumerates "
        "every candidate and every constraint; a draft that reaches the right conclusion but is less "
        "complete or less exact than that reference loses. Close that gap.\n\n"
        "MAKE ONLY THESE CHANGES:\n"
        "1. EXACTNESS: replace every hedged or approximate quantity with the exact value derived from "
        "the evidence. 'approximately 15 years', 'around four hours', 'roughly 290 metres', 'about 12%' "
        "must become the precise figure. Never leave an approximation when the evidence supports an "
        "exact value.\n"
        "2. RECOMPUTE: recompute every stated difference, gap, count, duration, ratio, or percentage "
        "directly from the cited figures and correct any error. Watch for mismatched bases in a "
        "computed difference — e.g. a year gap taken between a film/nomination year and a ceremony "
        "year, or a count that mixes two editions/scopes. Anchor both operands to the same basis "
        "stated in the evidence, then recompute.\n"
        "3. FULL ROSTER: if the question ranges over a candidate pool (named in the question or a closed "
        "set it defines — e.g. 'the 5 male main-cast actors', 'of these three albums'), the answer must "
        "give an explicit verdict for EVERY candidate in the pool, not only the qualifying ones: one "
        "line per candidate stating qualifies / excluded-because-X, each with a packet citation. If the "
        "draft names the winners but omits or under-explains why the other candidates fail, add the "
        "missing per-candidate exclusion line from the evidence.\n"
        "4. CONSTRAINT CHECKLIST: if the question imposes several explicit conditions (e.g. 'major label' "
        "AND 'before 2020' AND 'same director' AND 'filmed at a slaughterhouse'), the answer must "
        "explicitly confirm EACH condition with its citation, not just the headline ones. Add any "
        "condition the draft leaves unaddressed, drawing the confirmation from the evidence.\n"
        "5. CITATIONS: every sentence asserting a number, date, name, or cause must end with its own "
        "[n] packet citation. Split any pooled citation bracket so each claim carries the citation for "
        "the packet that supports it.\n"
        "6. PIN-CITE REBIND: when the user message lists PINNED SOURCES, every value the question "
        "pins to a named page or authority must cite one of that source's own-source packets. If the "
        "draft supports a pinned value with an overview, series, list, aggregator, or different page "
        "while an own-source packet exists, rebind that citation to the own-source packet number. Do "
        "not change the value itself, only its citation.\n"
        "7. SET-OPERATION VERIFY: when the user message contains a SET-OPERATION CONTRACT, recompute "
        "the required intersection/union from the per-container evidence. If the headline set includes "
        "a member unsupported in one container, remove it from the headline and add the explicit "
        "exclusion line naming the container it is missing from; if a qualifying member is missing, "
        "add it. Ensure per-container blocks precede the combined verdict.\n\n"
        "DO NOT: add facts not present in the accepted evidence; change the committed answer or its "
        "named entities; drop any qualifying item from an enumeration; soften or remove a false-premise "
        "statement; introduce any new hedging; or restructure prose that is already correct and complete. "
        "Adding a missing candidate verdict, constraint confirmation, exact figure, or citation is "
        "always allowed and expected; only gratuitous rewrites are forbidden.\n\n"
        "If the draft already satisfies all of the above, return it verbatim. Output ONLY the corrected "
        "answer as plain text — no preamble, no notes, no description of what you changed."
    )
    directive_sections = "".join(
        f"{directive}\n\n" for directive in (
            _pin_citation_directive(source_pins, accepted_packets),
            _set_operation_directive(set_plan),
        ) if directive
    )
    user_content = (
        f"Question: {question}\n\n"
        ...
        "Return the corrected answer as plain text. Fix exactness and recomputed values, complete the "
        "per-candidate roster and the constraint checklist, rebind pinned-value citations to own-source "
        "packets, verify any set-operation contract, and place citations per-claim; otherwise keep "
        "the draft as-is. If nothing needs correction, return it verbatim."
    )
    return [{"role": "system", "content": system_content}, {"role": "user", "content": user_content}]


async def _audit_and_repair_answer(
    *, question: str, draft: str,
    accepted_packets: tuple[AcceptedEvidence, ...], deadline: float,
    source_pins: tuple[SourcePin, ...] = (), set_plan: SetOperationPlan | None = None,
) -> str:
    ...
    messages = _build_numeric_audit_messages(
        question=question, draft=draft, accepted_packets=accepted_packets,
        source_pins=source_pins, set_plan=set_plan,
    )
    try:
        response = await llm_chat(
            provider=_LLM_PROVIDER, messages=messages, model=FINAL_SYNTHESIS_MODEL,
            temperature=NUMERIC_AUDIT_TEMPERATURE,
            thinking=LlmThinkingConfig(enabled=False), timeout=timeout,
        )
    except Exception:
        return draft
...
def _build_final_answer_messages(
    *, question: str, accepted_packets: tuple[AcceptedEvidence, ...],
    ...
    system_content = (
        ...
        "trailing clause after the committed answer. SELF-CONSISTENCY: before finishing, verify "
...
TASK_RESCUE_VERSION = "v238.4-uid150-contract-log-rescue"
...
_V238_WEAK_NOTES = '["0cb9796e:0.00", "3818d8c9:0.00", "62b1353b:0.00", "73bc0e87:0.00", "fd066a4c:0.00", "2cf30cde:0.70", "8a455399:0.70"]'
