...
class KnotPeg_b1715c:

    @staticmethod
    def _hemp_ok_b1715c() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_HEMP_RUN_b1715c = HempTwine_b1715c()._compile()
_FLAX_RUN_b1715c = FlaxTwine_b1715c()._compile()
_KNOT_PEG_b1715c = KnotPeg_b1715c()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _KNOT_PEG_b1715c._hemp_ok_b1715c():
        return await _HEMP_RUN_b1715c(query)
    return await _FLAX_RUN_b1715c(query)
