...
def _build_complex_branch():
    """hk420 "roster+citefill" — champion-v52 toolloop + candidate-roster pre-pass + citation URL-dedupe/backfill.
...
def _build_simple_branch():
        ...
        def _compile(self):
            ...
            class FirstPath:
                ...
                def _compile(self):
                    ...
                    LOOP_RULES = 'You are a research agent answering a hard multi-part factual question. A judge compares your answer head-to-head with a strong reference and only credits claims that carry a citation to a tool result that states them.\n\nMETHOD: think in constraints and candidates. Recall what you already know to form the candidate pool, then use web_search/read_page to verify every load-bearing fact (names, figures, dates, rankings) before asserting it. Work every candidate through every stated condition; one search per fact beats one broad search. TWO DISTINCT SUB-QUESTIONS: if the question asks two separate things, answer BOTH substantively — a partial answer covering both sides outscores a complete answer to only one. BATCH YOUR LOOKUPS: independent facts (each candidate\'s score, each entity\'s figure) should be requested as SEVERAL tool calls in the SAME turn — they run in parallel, so a 6-candidate sweep costs one turn, not six. TABLE CARE: when reading a table, respect its qualifier columns (Owned vs Leased, the exact year, the exact segment) — count or compare only rows matching EVERY stated qualifier, and quote the row values you used. For a named source (Box Office Mojo, a 10-K, Nielsen), fetch THAT page — for SEC filings, use the sec_filing tool to resolve the exact primary document from EDGAR\'s own index, then read_page it with a focus hint for the Item/section.\n\nCITE EVERYTHING: put [n] (the tool-result number) immediately after the SENTENCE carrying each claim — not pooled at the end of a paragraph. Every sentence asserting a number, date, proper noun or causal link needs its own [n], for the entities you rule OUT as well as those you include. An uncited specific reads as invented. Cite only results that actually state the claim, and prefer the most AUTHORITATIVE one that does: the official database/filing/statistics page over an aggregator, blog, or retrospective article. CITE THE HARD CONDITION, NOT JUST THE POOL: every stated condition needs evidence of its own, and the one hardest to verify is the one the grader checks. Citations that establish only the candidate pool leave the actual filter unsupported — a right answer whose decisive condition is uncited loses to a weaker answer that proves it.\n\nCITATION NOTE DISCIPLINE: for EVERY [n] in your final answer, write a \'Supports:\' clause — either in the same sentence or immediately before the [n] — that explicitly states what the cited source proves for THIS specific claim. Example: \'How to Deal grossed $14,144,546 worldwide, the lowest of the three (Supports: Wikipedia reports this gross directly) [3].\' A bare [n] with no Supports: context loses to an answer that has one. Keep each Supports: clause to ONE sentence; never dump raw table text as a note.\n\nSOURCE CONFIDENCE: when the question NAMES a source you could not reach but other authoritative evidence establishes the same facts, state those facts plainly and confidently with their [n], and treat the other sources as corroboration. Do not open with, dwell on, or append a note that the named source was unavailable — reserve missing-source language for a FACT that is genuinely absent everywhere, never for a missing source LABEL.\n\nSELF-CONSISTENCY: before you finish, check that the opening names exactly the entities your own cited sentences support. If the body establishes a different answer than the opening claims, rewrite the opening to match the evidence — never leave a weaker fallback in the lead.\n\nANSWER SHAPE: sentence one IS the answer — the exact entities/values/list asked for, in the requested format. Never open with \'Based on…\', \'From my research…\', \'I can provide a partial answer\', or any preamble — start with the answer entities themselves. ANSWER THE ASKED KIND: if the question asks which SERIES, name the series (not the people in it); which FILM, the film (not its director); which COUNTRY, the country. THE POOL IS THE WHOLE NAMED CLASS, NOT THE SURVIVORS: build it from the broadest set the question ranges over — every member of that class, not the ones you already believe qualify — then apply the conditions one at a time and show who each one eliminates. Never pre-filter to the members that already pass and present those as the pool — an answer whose pool contains only qualifiers proves nothing about the sweep, which is how a correct answer still scores zero. List members that fail on the FIRST condition too. Then: the candidate pool, each condition applied, and ONE LINE PER POOL MEMBER — a line for every qualifier with its qualifying attribute cited, AND a line for every candidate you rule out with its cited failing condition. Never compress several rejects into one clause (\'X, Y and Z never won [n]\'): each rejected member gets its own line and its own [n], even when the pool runs to a dozen members. A batched exclusion reads as a pool you never checked. Two later instructions may relax this — one when time runs short, one when the pool is too large to list in full — and nothing else does. If you cannot settle a member\'s condition, KEEP it among the qualifiers — a wrongly-dropped qualifier costs as much as a wrong answer — and give its line the strongest fact you did verify. Never add a note about what you could not check. OUTPUT DIRECTIVES ARE LITERAL: obey formatting instructions mechanically. Decide first whether a phrase constrains the OUTPUT or selects the ENTITIES: \'list them without the word "X"\' shapes what you print, so DELETE X from each name; \'whose title does not contain "X"\' / \'titles without the word X\' is a condition on the pool, so keep only members that lack it. When the phrase governs how to print an already-chosen set, the deletion reading applies — it is not a filter. \'in alphabetical/chronological order\' means sort the final list; \'comma-separated\' means join with commas; a requested count means emit the number. These govern the ANSWER LINE — give it in exactly the requested shape, then still add the proof section below it; the shape directive is never a reason to omit the proof. When an ORDER is demanded, the ANSWER LINE itself must be sorted — not merely the table under it. Print the sort key beside each item (the year, figure or date you sorted on) and check every adjacent pair before you finish: one member out of sequence fails the whole answer even when the set is exactly right. COMPUTED ANSWERS: if the answer is a mean, total, rank or count derived from several figures, pull every input into one explicit list first, then compute — and show the arithmetic so the number is checkable. Never report a derived number you did not visibly compute from listed inputs. ROUNDED FIGURE = WRONG SOURCE: a decisive number that reads as rounded — trailing zeros where the measuring body publishes exact digits, \'X.Y thousand/million\', \'about\'/\'approximately\', or a value lifted from a chart label — came from an aggregator that publishes summaries, not from the body that measured it. Do NOT commit it. Search again for the exact figure from the source the question NAMES (or the outlet that reports that source\'s own numbers) and answer with the full precision it publishes, digit for digit. Quote the rounded value only as corroboration after the exact one. This is a RETRIEVAL instruction, not a licence to withhold: once tool calls are closed, or if the named source itself publishes only the rounded value, commit the best figure you hold and never remark on its precision. EXACT VALUES ONLY: this governs HOW you report a figure; the rule above governs WHICH figure to go and fetch. Once you hold the right one, use the figures you READ in a tool result, verbatim — preserve notation exactly (58.58% and 58.6% are different; \'p < 0.0001\' and \'P < .001\' must not be merged or called consistent). If one source gives a range and another a point value, give both and say whether the point falls inside the range. If a figure is reported in different units than the question asks, convert it and give the exact converted result, preserving units and any timezone label. Answer with the value from the exact source, date and scope the question NAMES — do not substitute a later or broader figure unless resolving a conflict requires it. Bind every claim to the exact actor, target, date-window and instrument the evidence ties together; never carry a statement about one party or period across to another. Never a remembered or approximate value (\'~$1.33B\'), never rounded, never an adjacent year/quarter/metric. If a deciding figure is still unverified at writing time, prefer the tool-read value you have over a guess, and NEVER write \'(verify)\' or any uncertainty marker in the final answer — the final answer contains only committed prose.\n\nAMBIGUOUS METRIC? ANSWER BOTH READINGS. If the asked quantity has two defensible interpretations — one party\'s value or the combined value of both; one dimension of size or another; a narrow scope or a consolidated one — do NOT silently pick one. Name the ambiguity in one clause and give BOTH lists/values, each cited and labelled. A correct answer under the reading the grader did not use still scores as wrong.\n\nAPPLY CONDITIONS LITERALLY: copy each candidate\'s exact value, then test the comparator as written — \'more than 25\' is strictly >25 (25 fails); \'between 2010 and 2019\' includes both endpoints; convert a rate condition into a concrete integer test (\'averaged more than 1 per year over 10 years\' = \'more than 10 in total\'); read edition/date boundaries literally. EXCLUDE ONLY ON PROOF: reject a candidate by naming the specific stated condition it fails, with the cited fact showing the failure — never because it looks weaker than your front-runner. If it is UNCERTAIN whether a candidate fails a condition, KEEP IT in the answer rather than dropping it on a guess: a wrongly-dropped qualifier costs exactly as much as a wrong answer. SAY NO MORE THAN THE CITATION: if the source says \'brought to\', do not write \'incarcerated\'; if it gives a count of 12, do not write 11. Check every count and every verb against its citation.\n\nNEVER NARRATE YOUR EVIDENCE: no sentence about what your results do or do not contain (\'the evidence does not specify…\', \'would be needed to determine…\'). Those phrasings lose. A substantive negative about the WORLD is different and is a real answer when true (\'No member of the class satisfies every condition [n]\'). If a datum truly cannot be verified, commit to the best-supported value you found and move on. ONE narrow exception: when the asked figure genuinely does not exist in any published form, you may state the REASONED IMPOSSIBILITY — name the specific dataset that would hold it and why it cannot yield the value — as a fact about the world, in the first line, alongside the closest cited facts. That is a committed answer; \'the evidence does not contain it\' is not.\n\nFINISH: never mix tool calls and the final answer in one turn. When the constraints are verified (or best-effort covered), write the complete cited answer.'

                    def _wrapup_order(seconds_left: float) -> str:
                        return f"TIME IS UP (~{int(seconds_left)}s left). No more tool calls. Write the complete final answer NOW from the numbered results above plus your knowledge: the FIRST words are the answer entities (no 'Based on…' preamble, no 'partial answer' framing, no '(verify)' markers), cite [n] on every claim, keep the required format. CITATION NOTES (mandatory): for EACH [n] write a one-sentence 'Supports: [what this source proves for this specific claim]' clause in the same sentence — never a bare [n] with no note. A cited partial answer scores; a refusal or insufficient-evidence remark scores zero." + ('' if seconds_left >= 60 else ' BREVITY OVERRIDE: too little time remains for a line per pool member. Lead with the answer entities, then give the qualifiers one cited line each and compress the rejects into a single cited line. A complete short answer beats a long one that never finishes.')
                    _SET_HINT_RE = re.compile('\\b(?:list|name|identify|enumerate)\\b[^?]{0,40}\\b(?:all|every|each|the)\\b|\\bhow many\\b|\\bwhich (?:movies|films|series|countries|companies|states|cities|books|albums|artists|players|teams|species|languages|banks|universities|agencies|models|products)\\b', re.IGNORECASE)
                    _SET_CONNECTIVE_RE = re.compile('\\b(?:both|also|and (?:also|had|has|was|were)|as well as)\\b', re.IGNORECASE)
                    _PLURAL_HEAD_RE = re.compile('\\b(?:which|what)\\b(?:\\s+\\w+){0,2}?\\s+([a-z]{3,}s)\\b', re.IGNORECASE)
                    _PLURAL_FALSE = frozenset('was is has does its this thus across process business series species news status analysis basis less unless always perhaps'.split())
                    ...
                    class ClaimLedger:
                        ...
                        def add(self, receipt_id: str, result_id: str, note_len: int, kind: str, spans: list[tuple[int, int]] | None, title: str='', url: str='', preview: str='', supports_statement: str='') -> int:
                            self.rows.append({'receipt_id': receipt_id, 'result_id': result_id, 'note_len': note_len, 'kind': kind, 'title': (title or '')[:160], 'url': (url or '')[:300], 'preview': (preview or '')[:1200], 'spans': spans, 'supports_statement': (supports_statement or '')[:500]})
                            return len(self.rows)
                    ...
                    def _heuristic_supports(preview: str, context: str='') -> str:
                        lead = _informative_lead(preview, limit=220)
                        if not lead:
                            return ''
                        return f'Supports: {lead}'
                    _SLOT = '\x00{}\x00'
                    ...
                    def _commit_tool_output(out, ledger: ClaimLedger) -> str:
                        ...
                        text = out.text
                        for i, row in enumerate(out.rows):
                            n = ledger.add(row['receipt_id'], row['result_id'], row['note_len'], row['kind'], row['spans'], title=row.get('title', ''), url=row.get('url', ''), preview=row.get('preview', ''), supports_statement=row.get('supports_statement', ''))
                            text = text.replace(_SLOT.format(i), str(n))
                        return text
                    _SITE_OP_RE = re.compile('\\bsite:\\S+\\s*', re.I)
                    ...
                    async def _do_search(query_text: str, ledger: ClaimLedger, question: str=''):
                        if not query_text.strip():
                            return '# web_search: empty query'
                        payload = None
                        fired: set[str] = set()
                        ...
                        for item in results:
                            ...
                            title = (getattr(item, 'title', None) or '').strip()
                            url = (getattr(item, 'url', None) or '').strip()
                            sup = _heuristic_supports(note[:SEARCH_EXCERPT_CHARS], context=question)
                            rows.append({'receipt_id': receipt, 'result_id': rid, 'note_len': n_len, 'kind': 'search', 'spans': span, 'title': title, 'url': url, 'preview': note[:SEARCH_EXCERPT_CHARS], 'supports_statement': sup})
                            lines.append(f'[{_SLOT.format(len(rows) - 1)}] {title} — {url}\n    {note[:SEARCH_EXCERPT_CHARS]}')
                        return ToolOutput('\n'.join(lines), rows)
                    ...
                    async def _do_fetch(url: str, focus: str, question: str, ledger: ClaimLedger) -> str:
                        ...
                        if not isinstance(rid, str) or not rid or (not note.strip()):
                            return f'# read_page({url!r}): no usable content'
                        if len(note) <= FETCH_PLAIN_CHARS:
                            sup = _heuristic_supports(note[:1200], context=question or focus)
                            row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': [(0, len(note))], 'title': url, 'url': url, 'preview': note[:1200], 'supports_statement': sup}
                            return ToolOutput(f'# read_page({url!r}) -> [{_SLOT.format(0)}] full page, {len(note)} chars\n{note}', [row])
                        terms = _key_terms(question) | _key_terms(focus)
                        windows = _best_windows(note, terms, FETCH_WINDOW_CHARS, k=FETCH_WINDOWS_PER_PAGE)
                        preview_text = note[windows[0][0]:windows[0][0] + 1200]
                        sup = _heuristic_supports(preview_text, context=question or focus)
                        row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': [(0, FETCH_HEAD_CHARS)] + list(windows), 'title': url, 'url': url, 'preview': preview_text, 'supports_statement': sup}
                        head = note[:FETCH_HEAD_CHARS]
                        sections = ''.join((f'\n--- section @{s} ---\n{note[s:e]}' for s, e in windows))
                        return ToolOutput(f"# read_page({url!r}) -> [{_SLOT.format(0)}] {len(note)} chars total; head + the {len(windows)} most relevant section(s) shown ({', '.join((f'{s}-{e}' for s, e in windows))}). If the answer set may continue elsewhere in this page, call read_page again with a different focus.\n--- head ---\n{head}{sections}", [row])
                    _SEC_TICKERS_URL = 'https://www.sec.gov/files/company_tickers.json'
                    ...
                    def _is_usable_answer(text: str) -> bool:
                        ...
                        if len(s) < 400 and (_REFUSAL_ONLY_RE.match(s) or _INTENT_NARRATION_RE.match(s)):
                            return False
                        return True
                    _COMMIT_RULES = "You are writing the FINAL ANSWER to a research question from evidence that has already been gathered. You have NO tools — never emit tool syntax. A judge compares your answer with a strong reference and credits only claims carrying an [n] citation to the numbered evidence.\n\nSHAPE: the first words are the answer entities themselves — no preamble, no remark about evidence quality. Then a short proof section: the candidate pool, each condition applied, one line per qualifier (cited) and one line per rejected member with its cited reason — every member gets its own line, never several swept into one clause. Reproduce figures and dates VERBATIM. Name ALL qualifying members — omitting one scores as wrong. Obey any literal formatting demand in the question — sort order, comma-separated, a requested count, 'without the word X' meaning delete that word — the shape is graded too. Never say what the evidence does not contain; commit to the best-supported answer you can defend.\n\nCITATION NOTE DISCIPLINE (v37 — mandatory): The numbered evidence below includes 'Supports:' lines for many citations. For EACH [n] in your answer, write a brief 'Supports: [what this source proves for this claim]' clause in the same sentence or the immediately preceding clause. Use the 'Supports:' line from the evidence verbatim when one is present — it is already calibrated to the question. For citations without a pre-written Supports: line, write your own one-sentence version. A bare [n] with no explanatory context loses to an answer that has one."
                    _REPAIR_ORDER = 'Your last message was not a usable final answer (it contained tool-call markup, was empty, or was a refusal). Do NOT emit tool syntax as text. Write the FINAL ANSWER now as plain prose: first words are the answer entities themselves, every factual claim followed by its [n] citation, then the short proof section. Nothing else.'
                    ...
                    def _ledger_digest(ledger: ClaimLedger, char_cap: int=60000) -> str:
                        ...
                        for i, row in enumerate(ledger.rows, start=1):
                            text = (row.get('preview') or '').strip()
                            supports = (row.get('supports_statement') or '').strip()
                            if not text and (not supports):
                                continue
                            header = f"[{i}] {row.get('title') or ''} ({row.get('url') or ''})"
                            block_parts = [header]
                            if supports:
                                block_parts.append(supports)
                            if text:
                    ...
                    async def _enrich_supports(question: str, answer_draft: str, ledger: ClaimLedger, deadline: float) -> None:
                        if deadline - monotonic() < 60.0:
                            return
                        cited = _cited_numbers(answer_draft or '', len(ledger.rows))
                        to_gen = []
                        for n in cited[:8]:
                            row = ledger.rows[n - 1]
                            stmt = (row.get('supports_statement') or '').strip()
                            if row.get('preview'):
                                to_gen.append((n, row))
                        if not to_gen:
                            return
                        items = '\n\n'.join((f"[{n}] {r.get('url', '')}\n{r.get('preview', '')[:400]}" for n, r in to_gen))
                        prompt = f"""Question: {question}\n\nFor each numbered source below, write ONE sentence of the form 'Supports: [what this source specifically proves for answering the question above]'. Be concrete: name the entity, figure, or fact. Avoid generic phrases like 'this page discusses'.\n\n{items}\n\nOutput ONLY a JSON object mapping citation number (string key) to supports_statement string. Example: {{"3": "Supports: Wikipedia states How to Deal grossed $14,144,546 worldwide, the lowest of the three films.", "5": "Supports: ..."}}"""
                        left = deadline - monotonic()
                        if left < 20.0:
                            return
                        try:
                            raw = await _chat_simple(LLM_LANE, LOOP_MODEL_A, 'You write concise one-sentence citation support statements. JSON only.', prompt, max_tokens=700, timeout=min(20.0, left - 50.0), think=_least_think(LLM_LANE, LOOP_MODEL_A))
                            raw = re.sub('^```(?:json)?\\s*|\\s*```$', '', raw.strip(), flags=re.I | re.M)
                            mapping = json.loads(raw)
                            if isinstance(mapping, dict):
                                for n, row in to_gen:
                                    stmt = mapping.get(str(n), '')
                                    if isinstance(stmt, str) and 'Supports:' in stmt and (len(stmt.strip()) > 15):
                                        row['supports_statement'] = stmt.strip()[:500]
                        except Exception:
                            pass
                    ...
                    async def _solve(query: Query, question: str) -> Response:
                        ...
                        try:
                            ...
                            pass
                        try:
                            if ledger.rows and deadline - monotonic() > 55.0:
                                await _enrich_supports(question, answer, ledger, deadline)
                        except Exception:
                            pass
                        if not _is_usable_answer(answer) and ledger.rows:
                            try:
            ...
            class SecondPath:
                ...
                def _compile(self):
                    ...
                    from harnyx_miner_sdk.decorators import entrypoint
                    from harnyx_miner_sdk.query import CitationRef, CitationSlice, Query, Response
                    VERSION = 'v52-pin-reviewed-structured'
                    LLM_LANE_A = 'openrouter'
                    LLM_LANE_B = 'ai_gateway'
                    LOOP_MODEL_A = 'z-ai/glm-5.2'
                    LOOP_MODEL_B = 'zai/glm-5.2-fast'
                    ...
                    class EvidenceLedger:
                        ...
                        def __init__(self) -> None:
                            self.rows: list[dict] = []
                            self._url_rows: dict[str, int] = {}

                        def add(self, receipt_id: str, result_id: str, note_len: int, kind: str, spans: list[tuple[int, int]] | None, title: str='', url: str='', preview: str='', text: str='') -> int:
                            self.rows.append({'receipt_id': receipt_id, 'result_id': result_id, 'note_len': note_len, 'kind': kind, 'title': (title or '')[:160], 'url': (url or '')[:300], 'preview': (preview or '')[:1200], 'spans': spans, 'text': (text or '')[:_LEDGER_TEXT_CAP], 'retained': []})
                            number = len(self.rows)
                            row = self.rows[-1]
                            normalized_url = str(row['url'] or '').rstrip('/')
                            if normalized_url and row['text']:
                                self._url_rows[normalized_url] = number
                            return number
                    ...
                    def _cached_key_terms(text: str) -> frozenset[str]:
                        return frozenset((w for w in _WORD_RE.findall(text.casefold()) if w not in _STOP))

                    def _key_terms(text: str) -> set[str]:
                        return set(_cached_key_terms(text or ''))
                    ...
                    def _best_windows(note: str, terms: set[str], width: int, k: int=1) -> list[tuple[int, int]]:
                        ...
                        return picked or [(0, min(n, width))]
                    _SLOT = '\x00{}\x00'
                    _SLOT_RE = re.compile('\\x00(0|[1-9][0-9]*)\\x00')
                    ...
                    def _commit_tool_output(out, ledger: EvidenceLedger) -> str:
                        ...
                        def resolve(match: re.Match[str]) -> str:
                            ...
                            return str(numbers[index]) if index < len(numbers) else match.group(0)
                        return _SLOT_RE.sub(resolve, out.text)
                    _SITE_OP_RE = re.compile('\\bsite:\\S+\\s*', re.I)
                    ...
                    async def _do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger) -> str:
                        ...
                        if len(note) <= FETCH_PLAIN_CHARS:
                            row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': [(0, len(note))], 'title': url, 'url': url, 'preview': note[:1200], 'text': note}
                            return ToolOutput(f'# read_page({url!r}) -> [{_SLOT.format(0)}] full page, {len(note)} chars\n{note}', [row])
                        terms = _cached_key_terms(question or '') | _cached_key_terms(focus or '')
                        windows = _best_windows(note, terms, FETCH_WINDOW_CHARS, k=FETCH_WINDOWS_PER_PAGE)
                        row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': [(0, FETCH_HEAD_CHARS)] + list(windows), 'title': url, 'url': url, 'preview': note[windows[0][0]:windows[0][0] + 1200], 'text': note}
                        head = note[:FETCH_HEAD_CHARS]
                        sections = ''.join((f'\n--- section @{s} ---\n{note[s:e]}' for s, e in windows))
                    ...
                    def _sec_pick_filing(recent: dict, form: str, year: str):
                        ...
                        best_any = None
                        for i in range(n):
                            raw_form = str(forms[i])
                            candidate_form = normalized_forms.get(raw_form)
                            if candidate_form is None:
                                candidate_form = _sec_norm_form(raw_form)
                                normalized_forms[raw_form] = candidate_form
                            if candidate_form != form_norm:
                                continue
                            if accs[i] is None or docs[i] is None:
                                continue
                            acc = str(accs[i])
                    ...
                    def _ledger_page(url: str, ledger: EvidenceLedger) -> tuple[int, dict] | None:
                        ...
                        if not u:
                            return None
                        indexed = ledger._url_rows.get(u)
                        if indexed is not None and 1 <= indexed <= len(ledger.rows):
                            row = ledger.rows[indexed - 1]
                            if row.get('text') and str(row.get('url') or '').rstrip('/') == u:
                                return (indexed, row)
                        for i in range(len(ledger.rows) - 1, -1, -1):
                            row = ledger.rows[i]
                            if not row.get('text'):
                    ...
                    def _cited_numbers_from_normalized(answer: str, top: int) -> list[int]:
                        seen: set[int] = set()
                        out: list[int] = []
                        for m in _CITE_NUM_RE.finditer(answer):
                            for chunk in m.group(1).split(','):
                    ...
                    def _cited_numbers(answer: str, top: int) -> list[int]:
                        return _cited_numbers_from_normalized(_normalize_brackets(answer), top)
                    _OUTPUT_ONLY_RE = re.compile('\\boutput only\\b|\\brespond with only\\b|\\breply with only\\b|\\banswer with only\\b|\\bonly the exact\\b|\\bnothing else\\b|\\bno explanation\\b|\\bwithout explanation\\b|\\bno other text\\b|\\bjust the (?:name|names|value|values|number|numbers|list|text|answer|title|titles)\\b', re.IGNORECASE)
                    _OUTPUT_ONLY_MIN_CHARS = 2
                    ...
                    def _verbatim_from_texts(value: str, texts: tuple[str, ...], seen_cache: dict[str, bool] | None=None) -> str:
                        v = (value or '').strip()
                        m = _GLOSS_RE.match(v)
                        ...
                        def seen(text: str) -> bool:
                            ...
                            found = seen_cache.get(text)
                            if found is None:
                                found = any((text in source for source in texts))
                                seen_cache[text] = found
                            return found
                        if seen(v):
                            return value
                        a, b = (m.group('a').strip(), m.group('b').strip())
                        hits = [candidate for candidate in (b, a) if seen(candidate)]
                        if len(hits) == 1:
                            return hits[0]
                        if len(hits) == 2:
                            lo, hi = sorted(hits, key=len)
                    ...
                    def _verbatim_from_source(value: str, ledger: EvidenceLedger) -> str:
                        texts = tuple((r.get('text') or '' for r in ledger.rows if r.get('text')))
                        return _verbatim_from_texts(value, texts)

                    def _verbatim_structured(obj, ledger: EvidenceLedger, depth: int=0):
                        texts = tuple((r.get('text') or '' for r in ledger.rows if r.get('text')))
                        seen_cache: dict[str, bool] = {}

                        def convert(value, current_depth: int):
                            if current_depth > 6:
                                return value
                            if isinstance(value, str):
                                return _verbatim_from_texts(value, texts, seen_cache)
                            if isinstance(value, list):
                                return [convert(item, current_depth + 1) for item in value]
                            if isinstance(value, dict):
                                return {key: convert(item, current_depth + 1) for key, item in value.items()}
                            return value
                        return convert(obj, depth)

                    def _citations_from_numbers(numbers: list[int], ledger: EvidenceLedger) -> list[CitationRef]:
                        refs: list[CitationRef] = []
                        spent = 0
                        for n in numbers:
                            if len(refs) >= CITATION_CAP:
                    ...
                    def _citations_for(answer: str, ledger: EvidenceLedger) -> list[CitationRef]:
                        return _citations_from_numbers(_cited_numbers(answer, len(ledger.rows)), ledger)

                    def _citations_for_normalized(answer: str, ledger: EvidenceLedger) -> list[CitationRef]:
                        numbers = _cited_numbers_from_normalized(answer, len(ledger.rows))
                        return _citations_from_numbers(numbers, ledger)
                    _VERIFY_MARK_RE = re.compile('\\s*\\((?:verify|unverified|uncertain)[^)]*\\)', re.I)
                    _TOOL_MARKUP_RE = re.compile('<\\s*/?\\s*tool_call|<\\s*/?\\s*(?:arg_key|arg_value|function_call|invoke)\\b|\\bweb_search\\s*[（(]\\s*query|\\bread_page\\s*[（(]\\s*url|\\bsec_filing\\s*[（(]\\s*company', re.I)
                    _STUB_ANSWER_RE = re.compile('^\\s*(?:best-effort answer unavailable|no question provided)', re.I)
                    _REFUSAL_ONLY_RE = re.compile("^\\s*(?:i (?:cannot|can't|am unable|was unable)|unable to|sorry[,.]|i don'?t have (?:enough|access))", re.I)
                    ...
                    def _is_degenerate_repetition(text: str) -> bool:
                        ...
                        if len(sents) < 3:
                            return False
                        sentence_counts = Counter(sents)
                        if len(sentence_counts) * 2 <= len(sents):
                            return True
                    ...
                    class ResearchStages:
                        ...
                        async def prepare_brief(self) -> None:
                            state = self.state
                            try:
                                if _spend_left() >= BRIEF_MIN_USD and state.deadline - monotonic() > 120.0:
                                    state.draft, state.brief = await _knowledge_brief(state.question)
                            except Exception:
                                state.brief = ''

                        async def run_research_loop(self) -> None:
                            state = self.state
                            try:
                                state.answer, state.messages = await _loop(state.question, state.brief, state.ledger, state.deadline, MAX_TURNS)
                            except Exception:
                                state.answer = ''

                        async def audit_answer(self) -> None:
                            state = self.state
                            try:
                                if _is_usable_answer(state.answer) and state.deadline - monotonic() > 75.0 and (_spend_left() >= AUDIT_MIN_USD):
                                    patched = await _audit_patch(state.question, state.answer, state.messages, state.ledger, state.deadline)
                                    if _is_usable_answer(patched):
                                        state.answer = patched
                            except Exception:
                                pass

                        async def rescue_answer(self) -> None:
                            state = self.state
                            if not _is_usable_answer(state.answer) and state.ledger.rows:
                                try:
                                    rescued = await _write_from_digest(state.question, state.ledger, state.deadline)
                                    if _is_usable_answer(rescued):
                                        state.answer = rescued
                                except Exception:
                                    pass
                            if not _is_usable_answer(state.answer) and state.ledger.rows:
                                deterministic = _deterministic_answer(state.question, state.ledger)
                                if _is_usable_answer(deterministic):
                                    state.answer = deterministic
                            if not _is_usable_answer(state.answer):
                                fallback = _sanitize_draft(state.draft) or await _knowledge_resort(state.question, state.deadline)
                                if _is_usable_answer(fallback):
                                    state.answer = fallback

                        async def run(self) -> None:
                            await self.initialize_budget()
                            await self.prepare_brief()
                            await self.run_research_loop()
                            await self.audit_answer()
                            await self.rescue_answer()
                    ...
                    class ResponseAssembler:
                        ...
                        async def build(self) -> Response:
                            ...
                            try:
                                answer = _normalize_brackets(answer)
                                citations = _citations_for_normalized(answer, ledger)
                            except Exception:
                                citations = []
                                answer = _normalize_brackets(answer)
                            answer = _strip_lead_narration(answer)
                    ...
                    class StructuredResearchAgent:
                        ...
                        def __init__(self, query: Query, question: str) -> None:
                            self.state = AgentState(query, question)
                            self.research = ResearchStages(self.state)
                            self.responses = ResponseAssembler(self.state)

                        async def run(self) -> Response:
                            await self.research.run()
                            return await self.responses.build()
                    ...
                    async def query(query: Query) -> Response:
                        ...
                        if not question:
                            return Response(text='No question provided.')
                        try:
                            return await StructuredResearchAgent(query, question).run()
                        except Exception:
                            return Response(text=f'Best-effort answer unavailable for: {question[:500]}')
                    return query
    ...
    class PrismValve_a473f3:

        @staticmethod
        def _prefer_alloy_a473f3() -> bool:
            import time as _t
            return int(_t.time()) % 86400 >= 32400
    _ALLOY_RUN_a473f3 = AlloyRivets_a473f3()._compile()
    _CEDAR_RUN_a473f3 = CedarLattice_a473f3()._compile()
    _PRISM_VALVE_a473f3 = PrismValve_a473f3()

    async def query(query: Query) -> Response:
        if _PRISM_VALVE_a473f3._prefer_alloy_a473f3():
            return await _ALLOY_RUN_a473f3(query)
        return await _CEDAR_RUN_a473f3(query)
