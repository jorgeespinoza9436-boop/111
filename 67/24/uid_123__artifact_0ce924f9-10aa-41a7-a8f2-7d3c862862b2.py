...
# =============================================================================
# EasyPath — ladder + parallel tool-wave research agent
    ...
    def _compile(self):
        ...
        class LlmClient:
            ...
            async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool,
                ...
                payload_chars = _msg_chars(messages)
                for rung_i, rung in enumerate(LOOP_LADDER):
                    model = rung[0]
                    ceiling = rung[1]
                    ...
                    if not sent:
                        continue
                    pin0 = _upstream(LLM_LANE_A, model) if rung_i == 0 else None
                    for provider_extra in ((pin0, None) if pin0 is not None else (None,)):
                        try:
                            payload = await asyncio.wait_for(llm_chat(
                                provider=LLM_LANE_A if rung_i == 0 else LLM_LANE,
                                model=model,
                                messages=sent,
        ...
        _begin_run = SpendBudget._begin_run
        _end_run = SpendBudget._end_run
        ...
        _norm_query = ToolExecutor._norm_query
        ...
        _to_int = ToolExecutor._to_int
        _do_page_read = ToolExecutor._do_page_read
        _quote_span = ToolExecutor._quote_span
        _do_retain_evidence = ToolExecutor._do_retain_evidence
        _call_name_args = ToolExecutor._call_name_args
        _run_tool = ToolExecutor._run_tool
        _call_id = ToolExecutor._call_id
        _assistant_tool_message = ToolExecutor._assistant_tool_message
        _gather_bounded = ToolExecutor._gather_bounded
        _run_tool_waves = ToolExecutor._run_tool_waves
        ...
        _one_msg_chars = LlmClient._one_msg_chars
        _msg_chars = LlmClient._msg_chars
        _trim_messages = LlmClient._trim_messages
        ...
        _close_open_calls = ResearchLoop._close_open_calls
        ...
        _source_corpus = CitationBuilder._source_corpus
        ...
        _solve_inner = QuerySolver._solve_inner
...
# =============================================================================
# MediumPath — dual-lane agent with roster, corroboration, and value guards
# Compiles nested helpers, then returns an async query(Query) -> Response.
# =============================================================================
...
# =============================================================================
# HardPath — dual-lane research agent for the hardest questions
# Compiles nested helpers, then returns an async query(Query) -> Response.
# =============================================================================
...
class SpareLineWrap:
    """Hard-wrap text by character width; unused by live paths."""
...
class SpareBudgetGate:
    """Tiny remaining-budget gate; unused by live paths."""
...
class SpareSchemePick:
    """Pick a URL scheme; unused by live paths."""
...
class SpareListShape:
    """Detect list-like text; unused by live paths."""
...
class SpareMonthTokens:
    """Harvest month name tokens; unused by live paths."""
    ...
    @classmethod
    def find(cls, text: str) -> list[str]:
        low = (text or "").lower()
        return [m for m in cls.MONTHS if m in low]


class SpareSoftMax:
    """Two-value soft-max blend; unused by live paths."""

    @staticmethod
    def soft(a: float, b: float) -> float:
        aa = float(a)
        bb = float(b)
        m = aa if aa >= bb else bb
        return m + 0.1 * (aa + bb - m)


class SpareOnce:
    """Run-once guard; unused by live paths."""
...
class SpareWindow:
    """Sliding window of ints; unused by live paths."""
    ...
    def push(self, n: int) -> None:
        self._vals.append(int(n))
        if len(self._vals) > self.size:
            self._vals = self._vals[-self.size :]

    def avg(self) -> float:
        if not self._vals:
            return 0.0
        return sum(self._vals) / float(len(self._vals))


class SpareExtMap:
    """Map extension to a kind label; unused by live paths."""

    MAP = {".json": "json", ".csv": "table", ".md": "text", ".xml": "markup"}

    @classmethod
    def kind(cls, name: str) -> str:
        n = (name or "").lower()
        for ext, kind in cls.MAP.items():
            if n.endswith(ext):
                return kind
        return "other"


class SpareCamelSplit:
    """Split camelCase into words; unused by live paths."""
...
def spare_strip_quotes(text: str) -> str:
    """Strip matching quotes; unused by live paths."""
    s = (text or "").strip()
...
def spare_or_default(value: object, default: str = "") -> str:
    """Stringify or default; unused by live paths."""
    if value is None:
        return default
    s = str(value).strip()
...
def spare_endswith_any(text: str, suffixes: list[str]) -> bool:
    """Suffix membership check; unused by live paths."""
    t = text or ""
    return any(t.endswith(s) for s in suffixes if s)


def spare_interleave(a: list[str], b: list[str]) -> list[str]:
    """Interleave two string lists; unused by live paths."""
    out: list[str] = []
    n = max(len(a), len(b))
    for i in range(n):
        if i < len(a):
...
def spare_count_alpha(text: str) -> int:
    """Count alphabetic chars; unused by live paths."""
    return sum(1 for ch in (text or "") if ch.isalpha())


def spare_middle(text: str, n: int = 24) -> str:
    """Take a middle slice; unused by live paths."""
    s = text or ""
    k = max(1, int(n))
    if len(s) <= k:
        return s
    start = max(0, (len(s) - k) // 2)
    return s[start : start + k]


def spare_lerp(a: float, b: float, t: float) -> float:
    """Linear interpolate; unused by live paths."""
    tt = min(1.0, max(0.0, float(t)))
    return (1.0 - tt) * float(a) + tt * float(b)


def spare_none_to_empty(items: list[object | None]) -> list[str]:
    """Map None to empty strings; unused by live paths."""
    return ["" if x is None else str(x) for x in items]


def spare_kv_line(key: str, value: str) -> str:
    """Format a key=value line; unused by live paths."""
    return f"{(key or '').strip()}={spare_strip_quotes(value)}"


def spare_checksum(text: str) -> int:
    """Tiny additive checksum; unused by live paths."""
    return sum(ord(ch) for ch in spare_strip_quotes(text)) & 0xFFFF
