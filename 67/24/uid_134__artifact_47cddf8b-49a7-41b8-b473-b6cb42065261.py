    ...
    def _compile(self):
        ...
        class FirstPath:
            ...
            def _compile(self):
                ...
                _NARRATION_LEAD_RE = re.compile("^\\s*(?:based on (?:my|the)\\b|now (?:i|that i)\\b|i (?:now )?(?:have|was|am|need|will|can)\\b|i(?:'ll|'ve|'m)\\b|let me\\b|let's\\b|first,? i\\b|having (?:now )?\\w+\\b|okay\\b|alright\\b|to answer this\\b|my research\\b|these (?:values|numbers)\\b|all of these\\b|the values are\\b)", re.IGNORECASE)
                _ABBREV_TAIL_RE = re.compile('(?:\\b[A-Z]|\\b(?:Inc|Ltd|Co|No|vs|St|Dr|Mr|Ms|Mt|Jr|Sr|etc|e\\.g|i\\.e))\\.$')
                ...
                def _strip_lead_narration(text: str) -> str:
                    ...
                    for _ in range(2):
                        ...
                        if len(parts) != 2:
                            break
                        head, rest = (parts[0], parts[1].strip())
                        if _CITE_RE.search(head):
                            break
                        if _NARRATION_LEAD_RE.match(head) is None:
                            break
                        if len(head.split()) < 4 or _ABBREV_TAIL_RE.search(head) is not None:
                            break
                        if len(rest) < 60 or _CITE_RE.search(rest) is None:
                            break
                        t = rest
                    return t
                LOOP_RULES = "You are a rigorous research agent. Break the question into the exact facts needed, use web_search then read_page (with a focus) to reach the specific table/value, and answer ONLY from tool results. A judge compares your answer to a strong reference and credits only claims carrying a [n] citation to a tool result that states them.\nCITE PRECISELY: put [n] right after every value/name/date; cite the result whose text literally contains that value, preferring the most AUTHORITATIVE source (the official database/filing/statistics page over an aggregator or encyclopedia mirror). COMPUTED ANSWERS: list every input with its [n], then compute and show the arithmetic.\nCITE EVERY CLAIM AND EVERY SIDE: an uncited specific reads as invented. Put an [n] on EACH load-bearing fact — every value you include AND every one you rule out. FOR A COMPARISON OR TWO-PART ANSWER, cite BOTH sides and the conclusion: e.g. an interval/change answer needs the value at EACH endpoint cited, not just the final choice. CITE THE DECISIVE CONDITION, not just the candidate pool — the hardest-to-verify condition is the one the grader checks, and a right answer whose deciding condition is uncited loses to a weaker one that proves it.\nAPPLY CONDITIONS LITERALLY and include only passers, showing the cited fact that makes each pass or fail. Answer the asked entity type, using the source's EXACT strings. State the direct answer first; no process narration."
                ...
                async def _to_structured(question: str, answer: str, schema):
                    if answer and _left() > 12:
                        sys = "You output strictly valid JSON and nothing else. Extract the final answer as a JSON value EXACTLY matching the given JSON Schema — correct top-level type, same keys, no extra keys, no wrapper object. Use the CANONICAL NAME/VALUE the question asks for, with NOTHING added: strip explanatory parentheticals and qualifiers the question did not request (e.g. 'Midnight in Paris (2011)' -> 'Midnight in Paris'; 'Makkah (Mecca)' -> 'Makkah'; 'Texas (TX)' -> 'Texas'). Keep only the exact entity/value; a reference answer is the bare canonical form. Return ONLY the JSON."
                        user = f'JSON Schema:\n{json.dumps(schema)}\n\nQuestion:\n{question[:1500]}\n\nAnswer:\n{answer[:14000]}\n\nJSON:'
                        if _left() >= 14:
                            try:
        ...
        class SecondPath:
            ...
            def _compile(self):
                ...
                FETCH_WINDOW_CHARS = 3500
                TOOL_OUTPUT_CHARS = 4200
                TEXT_MAX_OUTPUT_TOKENS = 2000
                AUDIT_MAX_OUTPUT_TOKENS = 300
                ...
                _INTENT_NARRATION_RE = re.compile("^\\s*(?:let me\\b|i need to\\b|i'll now\\b|i will now\\b|i(?:'ve| have) (?:the|all) (?:the )?data\\b|i now have\\b|first,? i\\b|next,? i\\b|now i\\b|i'm going to\\b|i am going to\\b)", re.IGNORECASE)
                TOOLS = [{'type': 'function', 'function': {'name': 'search', 'description': 'Search the web for information relevant to the question. Returns tagged snippets like [R3] you can cite inline or fetch in full.', 'parameters': {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'Search query.'}}, 'required': ['query'], 'additionalProperties': False}, 'strict': True}}, {'type': 'function', 'function': {'name': 'fetch', 'description': 'Fetch a URL seen in a previous search result. Pass `focus` (a heading, label, or phrase to locate on the page) to see the relevant section of a long page instead of just its start; pass an empty string if not needed.', 'parameters': {'type': 'object', 'properties': {'url': {'type': 'string', 'description': 'Exact URL to fetch.'}, 'focus': {'type': 'string', 'description': 'Heading/label/phrase to locate on the page, or empty string.'}}, 'required': ['url', 'focus'], 'additionalProperties': False}, 'strict': True}}]
                SYSTEM_PROMPT = 'You are a careful research assistant. Search before answering, even a question you think you already know - verify it against evidence. Use the search and fetch tools; call them as many times as you need (a follow-up query, cross-checking a number, a second angle on the question) but stop once you have enough - independent lookups can be requested in the same turn. If the question asks for a list or set of items, search for the authoritative list/table itself first, then verify members, rather than guessing who belongs. Cite evidence inline using its exact tag in brackets, e.g. [R4]. Your final answer must commit to specific values, even under uncertainty: give your single best estimate from whatever evidence and reasoning you have. Never end an answer by listing what the evidence does not contain, could not be extracted, or would be needed - a committed guess beats a description of the gap.'
                ...
                AUDIT_PROMPT = 'Review your answer above against the original question. Check specifically: was every named source actually consulted, are all requested entities/items present with the values the question asks for, and is every number or conclusion backed by evidence you gathered rather than assumed? If it fully answers every part, is specific, and its claims are backed by [R#] citations, reply with exactly: OK. Otherwise, name the concrete gaps in one short paragraph - nothing else.'
                REPAIR_PREFIX = 'Fix these gaps - use more search/fetch calls if you need to. Then rewrite the complete final answer with specific committed values; if something genuinely cannot be verified, give your single best estimate rather than describing the gap. Gaps to address:\n'
                ...
                class _Session:
                    ...
                    def add_evidence(self, *, url: str, title: str, note: str, citation: CitationRef | None) -> str:
                        tag = f'R{len(self.evidence) + 1}'
                        self.evidence[tag] = _Evidence(tag=tag, url=url, title=title, note=note, citation=citation)
                        header = f'[{tag}] {title} - {url}'
                        return f'{header}\n{note}' if note else header

                def _bounded_citation(receipt_id: str, result_id: str, note_len: int, start: int, end: int) -> CitationRef:
                    if start <= 0 and end >= note_len:
                        return CitationRef(receipt_id=receipt_id, result_id=result_id)
                    return CitationRef(receipt_id=receipt_id, result_id=result_id, slices=[CitationSlice(start=start, end=end)])

                def _terms(text: str) -> set[str]:
                    return set(_WORD_RE.findall(text.lower()))

                def _best_window(note: str, focus: str, width: int) -> tuple[int, int]:
                    length = len(note)
                    if length <= width:
                        return (0, length)
                    terms = _terms(focus)
                    ...
                    while position < length:
                        segment = low[position:position + width]
                    ...
                    if best_score <= 0:
                        return (0, width)
                    return (best_start, min(length, best_start + width))
                ...
                async def _tool_search(args: dict, session: _Session) -> str:
                    ...
                    for item in result.results:
                        ...
                        citation = _bounded_citation(result.receipt_id, item.result_id, len(note), 0, min(len(note), SEARCH_CITE_CHARS)) if note else None
                        blocks.append(session.add_evidence(url=item.url, title=item.title or item.url, note=note[:SEARCH_EXCERPT_CHARS], citation=citation))
                ...
                async def _tool_fetch(args: dict, session: _Session) -> str:
                    ...
                    hit = hits[0]
                    note = hit.note.strip()
                    start, end = _best_window(note, focus, FETCH_WINDOW_CHARS)
                    excerpt = note[start:end]
                    citation = _bounded_citation(result.receipt_id, hit.result_id, len(note), start, end)
                    return session.add_evidence(url=url, title=hit.title or url, note=excerpt, citation=citation)
                ...
                def _is_usable_answer(text: str | None) -> bool:
                    ...
                    if _LEAKED_TOOL_RE.search(stripped):
                        return False
                    if len(stripped) < 300 and _INTENT_NARRATION_RE.match(stripped):
                        return False
                    return True
                ...
                async def _audit_and_repair(answer: str, messages: list, session: _Session) -> str:
                    ...
                    if not revised:
                        revised = await _final_text(messages, session)
                    return revised if revised and len(revised) >= 20 else answer
                ...
                async def _final_structured(schema: dict, messages: list, session: _Session) -> tuple[object | None, list[str]]:
                    ...
                    repair = attempt + [{'role': 'assistant', 'content': raw or ''}, {'role': 'user', 'content': 'That was not valid JSON. Reply again with ONLY the corrected JSON object (the {"answer": ..., "evidence_tags": [...]} shape).'}]
                ...
                def _coerce_to_schema(text: str, schema: object) -> object:
                    kind = _schema_kind(schema)
                    text = (text or '').strip()
                    if kind == 'array':
                        parts = [p.strip(' -*\t') for p in re.split('[\\n;]|,\\s*', text) if p.strip()]
                        return parts[:20] or ([text[:400]] if text else [])
                    if kind == 'object':
                        props = schema.get('properties') if isinstance(schema, dict) else None
                        required = schema.get('required') if isinstance(schema, dict) else None
                        keys = required if isinstance(required, list) and required else list(props or {})
                        return {str(key): text[:400] for key in keys}
                    if kind in ('number', 'integer'):
                ...
                def _text_citations(answer: str, session: _Session) -> list[CitationRef]:
                    ...
                    for match in _CITE_TAG_RE.finditer(answer):
                        ...
                        if tag not in seen and tag in session.evidence:
                            seen.add(tag)
                            tags.append(tag)
                    citations = [session.evidence[tag].citation for tag in tags if session.evidence[tag].citation]
                    if citations:
                        return citations[:CITATION_CAP]
                    fallback = [entry.citation for entry in session.evidence.values() if entry.citation]
                ...
                def _tags_to_citations(tags: list[str], session: _Session) -> list[CitationRef]:
                    ...
                    for tag in tags:
                        if tag in seen or tag not in session.evidence:
                            continue
                        seen.add(tag)
                        citation = session.evidence[tag].citation
                        if citation:
                            citations.append(citation)
                    return citations[:CITATION_CAP]

                def _structured_citations(session: _Session) -> list[CitationRef]:
                    fetched = [entry.citation for entry in session.evidence.values() if entry.citation]
                    return fetched[:STRUCTURED_CITATION_CAP]

                def _fallback_text(session: _Session) -> str | None:
                    notes = [entry.note for entry in session.evidence.values() if entry.note]
                    return '\n\n'.join(notes[:3])[:4000] if notes else None
                ...
                async def query(query: Query) -> Response:
                    ...
                    try:
                        ...
                        if query.output_schema is not None:
                            ...
                            if output is None:
                                basis = final_text or _fallback_text(session) or question
                                output = _coerce_to_schema(basis, query.output_schema)
                                tags = []
                        ...
                        answer = final_text or await _final_text(messages, session)
                        if answer:
                            answer = await _audit_and_repair(answer, messages, session)
                        else:
                            answer = _fallback_text(session) or FALLBACK_TEXT
                        return Response(text=answer, citations=_text_citations(answer, session) or None)
                    except Exception:
                        if query.output_schema is not None:
...
class HingePeg_6e9844:

    @staticmethod
    def _maple_side_6e9844() -> bool:
        import time as _t
        return int(_t.time()) % 86400 >= 32400
_MAPLE_RUN_6e9844 = MapleArch_6e9844()._compile()
_OAK_RUN_6e9844 = OakArch_6e9844()._compile()
_HINGE_PEG_6e9844 = HingePeg_6e9844()

async def _simple_branch_query(query: Query) -> Response:
    if _HINGE_PEG_6e9844._maple_side_6e9844():
        return await _MAPLE_RUN_6e9844(query)
    return await _OAK_RUN_6e9844(query)
...
def _route_to_complex_branch(query: Query) -> bool:
    ...
    if len(text) >= 640:
        ...
        signals += 1
    if lowered.count(",") >= 5 or lowered.count(";") >= 2:
        signals += 1
    markers = (
        "ranked highest to lowest", "ranked lowest to highest",
        "which countries", "which states", "which cities", "which companies",
        "exclude", "remaining", "strictly greater than", "strictly less than",
        "at least", "no more than", "combined", "all three", "all four",
        "json", "schema", "table", "csv",
    )
    signals += sum(1 for marker in markers if marker in lowered)
    if any(word in lowered for word in ("cross-reference", "multi-step", "intersect")):
        signals += 1
    if signals >= 5:
        return True
    import hashlib as _hashlib
    bucket = int(_hashlib.sha256(text.encode("utf-8", "ignore")).hexdigest()[:2], 16)
    return bucket % 4 == 0
