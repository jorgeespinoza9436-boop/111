...
class IronSpine_b9978c:
    ...
    def _compile(self):
        ...
        def _build_cfg() -> dict[str, Any]:
            table = [('backend', 'openrouter'), ('backend_b', 'ai_gateway'), ('brief_model', 'z-ai/glm-5.2'), ('agent_model', 'z-ai/glm-5.2'), ('agent_model_b', 'zai/glm-5.2-fast'), ('lane_b_max_chars', 144000), ('audit_model', 'openai/gpt-oss-120b'), ('schema_model', 'openai/gpt-oss-120b'), ('backup_model', 'deepseek/deepseek-v3.2'), ('wall', 245.0), ('brief_to', 55.0), ('turn_to', 80.0), ('audit_to', 30.0), ('search_to', 20.0), ('turns', 12), ('fetch_to', 15.0), ('patch_extra', 2), ('commit_secs', 85.0), ('ans_cap', 70000), ('cite_cap', 40), ('fetch_win', 6000), ('fetch_slice', 8000), ('search_win', 500), ('brief_usd', 0.03), ('audit_usd', 0.05), ('commit_usd', 0.02)]
            out: dict[str, Any] = {}
        ...
        def _assert_cfg(c: dict[str, Any]) -> dict[str, Any]:
            needed = ('backend', 'backend_b', 'brief_model', 'agent_model', 'agent_model_b', 'lane_b_max_chars', 'audit_model', 'schema_model', 'backup_model', 'wall', 'brief_to', 'turn_to', 'audit_to', 'search_to', 'turns', 'fetch_to', 'patch_extra', 'commit_secs', 'ans_cap', 'cite_cap', 'fetch_win', 'fetch_slice', 'search_win', 'brief_usd', 'audit_usd', 'commit_usd')
            for key in needed:
                if key not in c:
                    raise KeyError(key)
        ...
        def _tool_multi(name: str, desc: str, props: dict[str, Any], required: list[str]) -> dict[str, Any]:
        ...
        class _ConstraintStore:
            ...
            def push(self, receipt: str, result: str, note: str, kind: str, url: str='') -> int:
                body = note or ''
                row = _EvidenceRow(receipt_id=receipt, result_id=result, note_len=len(body), source=kind, url=url, text=body[:_LEDGER_TEXT_CAP])
                self.rows.append(row)
                num = len(self.rows)
                self._check_satisfaction(num - 1, url, body)
                return num

            def page_for(self, url: str) -> tuple[int, _EvidenceRow] | None:
                u = (url or '').strip().rstrip('/')
                if not u:
                    return None
        ...
        class _LLM:
            ...
            def _thinking(model: str, thinking: dict | None=None) -> dict:
                ...
                if model.startswith('openai/gpt-oss'):
                    return {'enabled': True, 'effort': 'low'}
                if model.startswith('z-ai/glm') or model.startswith('zai/glm'):
                    return {'enabled': True, 'effort': 'low'}
                return {'enabled': False}
            ...
            async def agent_turn(self, messages: list[dict], clock: _Clock, *, force_text: bool) -> object | None:
                ...
                lanes = ((CFG['backend'], CFG['agent_model']), (CFG['backend_b'], CFG['agent_model_b']))
                for provider, model in lanes:
                    if provider == CFG['backend_b'] and payload_chars > int(CFG['lane_b_max_chars']):
                        continue
                    timeout = min(CFG['turn_to'], clock.left() - 5.0)
                    if timeout <= 5.0:
                        return None
                    try:
                        think = self._thinking(model)
                        if force_text and provider == CFG['backend_b']:
                            think = {'enabled': False}
                        adapted = self._adapt_messages(messages, model, 'final' if force_text else 'tool')
                        tools = None if force_text else TOOLS
                        choice = None if force_text else 'auto'
                        if force_text and provider == CFG['backend_b']:
                            return await llm_chat(provider=provider, model=model, messages=adapted, tools=tools, tool_choice=choice, temperature=0.2, thinking=think, timeout=timeout, max_output_tokens=6000)
                        return await llm_chat(provider=provider, model=model, messages=adapted, tools=tools, tool_choice=choice, temperature=0.2, thinking=think, timeout=timeout)
                    except Exception:
                        continue
                return None
        ...
        class _Tools:
            ...
            async def run(self, call: object) -> str:
                ...
                if name == 'search_web':
                    ...
                    return self.page_grep(str(_Access.mapping_get(args, 'url', '')), str(_Access.mapping_get(args, 'pattern', '')))
                elif name == 'page_read':
                    return self.page_read(str(_Access.mapping_get(args, 'url', '')), _Access.mapping_get(args, 'offset', 0), _Access.mapping_get(args, 'length', PAGE_READ_MAX_CHARS))
                elif name == 'retain_evidence':
                    return self.retain_evidence(str(_Access.mapping_get(args, 'source', '')), str(_Access.mapping_get(args, 'quote', '')))
                return f'# unknown tool {name!r}'

            async def search(self, q: str) -> str:
                if not q.strip():
                    return '# search_web -> empty query'
                resp = await self._first_ok(('parallel', 'desearch'), lambda p: search_web(q, provider=p, num=8, timeout=CFG['search_to']))
                if resp is None:
                ...
                for hit in hits:
                    ...
                    if isinstance(rid, str) and rid:
                        full = str(getattr(hit, 'note', None) or '')
                        excerpt = full[:CFG['search_win']]
                        url = str(getattr(hit, 'url', None) or '')
                        title = str(getattr(hit, 'title', None) or '')
                        num = self._store.push(receipt, rid, full, 'search', url)
            ...
            async def fetch(self, url: str) -> str:
                ...
                if usable:
                    ...
                    deeper = ''
                    if len(note) > CFG['fetch_win']:
                        deeper = f' ({len(note)} chars total; if the value you need is not shown, call page_grep(url, pattern) on this SAME url — do not re-fetch)'
                    return f'# fetch_page({url!r}) -> [{num}] {len(shown)} chars shown{deeper}\n{shown}'
                return f'# fetch_page({url!r}) -> no usable content'

            def page_grep(self, url: str, pattern: str) -> str:
                hit = self._store.page_for(url)
                if hit is None:
            ...
            def page_read(self, url: str, offset: object, length: object) -> str:
                hit = self._store.page_for(url)
                if hit is None:
                ...
                try:
                    ...
                    ln = PAGE_READ_MAX_CHARS
                a = max(0, min(off, max(0, len(text) - 1)))
                b = min(len(text), a + max(1, min(ln, PAGE_READ_MAX_CHARS)))
                return f'# page_read([{n}] @{a}:{b} of {len(text)})\n{text[a:b]}'

            def retain_evidence(self, source: str, quote: str) -> str:
                raw = (source or '').strip().strip('[]')
                try:
                    n = int(raw)
                except ValueError:
                    return f'# retain_evidence: source must be a result number like [3], got {source!r}'
                row = self._store.get(n)
                if row is None:
                    return f'# retain_evidence: no result [{n}] exists yet'
                text = row.text or ''
                q = (quote or '').strip()
                if len(q) < RETAIN_MIN_QUOTE:
                    return f'# retain_evidence: quote too short ({len(q)} chars); quote at least {RETAIN_MIN_QUOTE} characters'
                if not text:
                    return f'# retain_evidence: result [{n}] has no stored text to quote from'
                i = text.find(q)
                if i < 0:
                    i = text.lower().find(q.lower())
                if i < 0:
                    return f'# retain_evidence: that text does not appear in [{n}]. Quote it EXACTLY as the source prints it.'
                if len(row.retained) >= RETAIN_MAX_PER_ROW:
                    return f'# retain_evidence: [{n}] already has {len(row.retained)} retained excerpts'
                a = max(0, i - RETAIN_MARGIN_CHARS)
                b = min(row.note_len or len(text), i + len(q) + RETAIN_MARGIN_CHARS)
                if b <= a:
                    return f'# retain_evidence: could not bound the excerpt in [{n}]'
                row.retained.append((a, b))
                return f'# retain_evidence: kept {b - a} chars of [{n}] around your quote. Cite [{n}] for that claim.'
        ...
        class _ResearchSession:
            ...
            async def _run_tool_waves(self, calls) -> list:
                bodies: list = [None] * len(calls)
                wave1: list[int] = []
                wave2: list[int] = []
                ...
                async def _collect(indices: list[int]) -> None:
                    if not indices:
                        return
                    jobs = {i: asyncio.create_task(self._tools.run(calls[i])) for i in indices}
                    await asyncio.wait(list(jobs.values()))
                    for i, job in jobs.items():
                        try:
                            bodies[i] = job.result()
                        except Exception as exc:
                            bodies[i] = exc
                await _collect(wave1)
                await _collect(wave2)
                return [bodies[i] if bodies[i] is not None else '# tool produced no output' for i in range(len(calls))]
            ...
            async def drive(self, question: str, briefing: str, max_turns: int, seed: list[dict] | None=None) -> tuple[str, list[dict]]:
                ...
                while turn < max_turns:
                    ...
                    if not calls:
                        ...
                        produced = await self._run_tool_waves(calls)
        ...
        class _Briefing:
            ...
            async def build(self, question: str) -> tuple[str, str]:
                ...
                if self._store.source_reqs:
                    ...
                    src_section = _cat(('REQUIRED_SOURCES: the question explicitly names: ', names, '. Your searches and urls sections MUST include ', 'searches and URLs for these exact sources. Data from ', 'other sources will score 0.0.\n'))
                user = _cat((f'Question:\n{question}\n\n', 'Fill in this internal worksheet. It is planning scratch for your own use, never an answer, so keep the tags lowercase and never reuse them as section headings later.\n', 'draft: your best definitive answer from knowledge alone — enumerate the full candidate pool, apply every constraint, name qualifying entities with concrete numbers/dates, note borderline exclusions. Mark uncertain values with (verify).\n', 'conditions: numbered list of every atomic constraint/filter in the question (including ordering and requested output format).\n', 'candidates: the entities to verify, one per line, with which conditions are uncertain for each.\n', src_section, 'searches: 3-6 targeted web searches that would verify the load-bearing facts (exact names + years; include the named source site if any).\n', "urls: 0-6 exact URLs likely to contain the needed figures, ONLY for named sources whose URL patterns you know (one per entity/year; for annual reports pick the edition containing each requested year, usually year+1 or year+2). Otherwise write 'none'."))
                try:
                ...
                draft = raw
                cut = min((mm.start() for mm in (re.search('[#*_\\s]*(?:conditions|CONSTRAINTS)[#*_\\s]*:', raw, re.IGNORECASE), re.search('^[ \\t]*[#*_>]{0,4}[ \\t]*(?:conditions|CONSTRAINTS)[ \\t]*[#*_]{0,3}[ \\t]*$', raw, re.IGNORECASE | re.MULTILINE)) if mm is not None), default=None)
                if cut is not None:
                    draft = raw[:cut]
                draft = re.sub('^[#*_\\s]*(?:draft|DRAFT)[#*_\\s]*:[#*_\\s]*', '', draft, flags=re.IGNORECASE)
                draft = re.sub('^[ \\t]*[#*_>]{0,4}[ \\t]*(?:draft|DRAFT)[ \\t]*[#*_]{0,3}[ \\t]*\\n+', '', draft, flags=re.IGNORECASE)
                draft = draft.strip()
                briefing = _cat(('PRIOR ANALYSIS — your own planning worksheet (verify anything marked (verify), and correct it wherever tool results disagree). Its tags are internal: never reproduce them, or any section named after them, in the answer.\n', raw.strip()))
        ...
        def _verbatim_from_source(value: str, store: _ConstraintStore) -> str:
            v = (value or '').strip()
            m = _GLOSS_RE.match(v)
            if not m:
                return value
            texts = [r.text for r in store.rows if r.text]
            if not texts:
                return value
        ...
        def _verbatim_structured(obj, store: _ConstraintStore, depth: int=0):
            if depth > 6:
                return obj
            if isinstance(obj, str):
                return _verbatim_from_source(obj, store)
            if isinstance(obj, list):
                return [_verbatim_structured(x, store, depth + 1) for x in obj]
            if isinstance(obj, dict):
                return {k: _verbatim_structured(v, store, depth + 1) for k, v in obj.items()}
            return obj
        ...
        class _Citations:
            ...
            @staticmethod
            def _ref_for(row: _EvidenceRow) -> CitationRef | None:
                if not row.receipt_id or not row.result_id:
                    return None
                note_len = int(row.note_len or 0)
                shown: list[list[int]] = []
                if row.retained:
                    for a, b in row.retained:
                        a = max(0, min(int(a), note_len))
                        b = max(a + 1, min(int(b), note_len))
                        shown.append([a, b])
                elif row.source == 'fetch' and note_len > CFG['fetch_slice']:
                    shown.append([0, min(CFG['fetch_win'], note_len)])
                elif note_len > 0:
                    shown.append([0, min(max(CFG['fetch_win'], CITATION_MIN_SPAN_CHARS), note_len)])
                if not shown:
            ...
            def assemble(answer: str, store: _ConstraintStore) -> list[CitationRef]:
                ...
                while i < limit:
                    ...
                    if row is None:
                        continue
                    ref = _Citations._ref_for(row)
                    if ref is None:
                        continue
                    slices = getattr(ref, 'slices', None)
                    cost = sum((max(0, s.end - s.start) for s in slices)) if slices else int(row.note_len or 0)
                    if spent + cost > EVIDENCE_CHAR_BUDGET:
                        continue
                    spent += cost
                    refs.append(ref)
        ...
        class MinerPipeline:
            ...
            def _clean_final_text(text: str) -> str:
                ...
                if not body:
                    return body
                markers = ['\\n#{1,3}\\s*(?:draft|DRAFT)\\b', '\\n#{1,3}\\s*(?:conditions|CONSTRAINTS)\\b', '\\n#{1,3}\\s*(?:candidates|CANDIDATES)\\b', '\\n#{1,3}\\s*(?:searches|QUERIES)\\b', '\\n#{1,3}\\s*(?:urls|FETCH)\\b', '\\n\\*\\*(?:draft|DRAFT)\\*\\*', '\\n(?:draft|DRAFT)\\s*:', '\\n(?:conditions|CONSTRAINTS)\\s*:']
                ...
                if proof is not None:
                    ...
                    draft = re.search('\\n#{1,3}\\s*(?:draft|DRAFT)\\b\\s*(.*?)(?=\\n#{1,3}\\s*(?:conditions|CONSTRAINTS|candidates|CANDIDATES|searches|QUERIES|urls|FETCH)\\b|$)', body, flags=re.I | re.S)
                ...
                if schema is not None:
                    ...
                    if shaped is None:
                        try:
                            ...
                            shaped = None
                    if shaped is None:
                        cleaned = _undigest_for_schema(rendered)
                        basis = cleaned if cleaned else rendered
                        shaped = _SchemaOut.fallback(schema, basis)
                    if shaped is not None:
                        try:
                            shaped = _verbatim_structured(shaped, self.store)
                        except Exception:
                            pass
                        try:
...
class ToggleCore_b9978c:

    @staticmethod
    def _copper_active_b9978c() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_COPPER_RUN_b9978c = CopperSpine_b9978c()._compile()
_IRON_RUN_b9978c = IronSpine_b9978c()._compile()
_TOGGLE_CORE_b9978c = ToggleCore_b9978c()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _TOGGLE_CORE_b9978c._copper_active_b9978c():
        return await _COPPER_RUN_b9978c(query)
    return await _IRON_RUN_b9978c(query)
