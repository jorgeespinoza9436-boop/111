...
async def _solve(query: Query, question: str) -> Response:
    ...
    # pool MEMBERS present, then does the right SOURCE carry the claims, then is
    # the decisive FIGURE corroborated. Primary-source anchoring must precede the
    # second-source check (see its docstring — running it after would let a
    # pulled-in .gov result mask an answer still anchored to an aggregator).
    # Each stage rewrites the WHOLE answer, re-checks its own time/spend floor,
    # and returns `answer` untouched on any failure.
    for _sweep in (_verify_subjects, _widen_pool,
