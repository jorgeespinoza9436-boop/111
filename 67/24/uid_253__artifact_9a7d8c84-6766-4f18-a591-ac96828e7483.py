...
class PivotPin_51e8ef:

    @staticmethod
    def _quartz_fit_51e8ef() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_QUARTZ_RUN_51e8ef = QuartzShelf_51e8ef()._compile()
_SLATE_RUN_51e8ef = SlateShelf_51e8ef()._compile()
_PIVOT_PIN_51e8ef = PivotPin_51e8ef()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _PIVOT_PIN_51e8ef._quartz_fit_51e8ef():
        return await _QUARTZ_RUN_51e8ef(query)
    return await _SLATE_RUN_51e8ef(query)
