...
#    inside `_compile()`. That method builds nested helper classes, wires
#    module-style aliases, defines an inner `query`, and returns that callable.
# 2. DifficultyRouter asks a small LLM to label the question as easy / medium /
#    hard (fallback: hard on any failure).
# 3. The module `@entrypoint("query")` dispatches to `_EASY_RUN`, `_MEDIUM_RUN`,
#    or `_HARD_RUN` produced by compiling each path once at import time.
# =============================================================================
...
# =============================================================================
# EasyPath — dual-lane research agent for lighter questions
# Compiles nested helpers, then returns an async query(Query) -> Response.
# =============================================================================

class Rank1:

    # Build the full agent closure for this difficulty path and return its query().
    ...
    def _compile(self):
        ...
        # Spend / budget tracking for the current research run.

        class SpendBudget:

            @staticmethod
            # Record a spend event against the run budget.
            ...
            @staticmethod
            # Return remaining budget for the active run.
        ...
        # Numbered evidence store; tool outputs become citable [n] rows.

        class EvidenceLedger:
            # Initialize instance fields for this helper.

            def __init__(self) -> None:
                self.rows: list[dict] = []

            # Append a new evidence row and return its citation number.
            ...
            # Look up the CitationRef for a ledger row number.
        ...
        # Small container for a single tool result before ledger commit.
        ...
        # Term extraction and dense window selection inside fetched pages.

        class PageLocalizer:

            @staticmethod
            # Extract search / highlight terms from the question text.

            def _key_terms(text: str) -> set[str]:
                return {w for w in _WORD_RE.findall((text or "").casefold()) if w not in _STOP}

            @staticmethod
            # Pick the densest page windows that cover the key terms.
        ...
        # web_search / fetch / page_grep / page_read / retain_evidence + run_tool.

        class ToolExecutor:

            @staticmethod
            # Persist a ToolOutput into the evidence ledger.
            ...
            @staticmethod
            # Soften / shorten a query when search is too brittle.
            ...
            @staticmethod
            # Run web_search and commit ranked results to the ledger.
            ...
            @staticmethod
            # Fetch a URL and store page text for later grep/read.
            ...
            @staticmethod
            # Attach fetched page content under a ledger row.
            ...
            @staticmethod
            # Grep a fetched page for patterns and retain hits.
            ...
            @staticmethod
            # Read a bounded span from a previously fetched page.
            ...
            @staticmethod
            # Keep a selected snippet as retained evidence.
            ...
            @staticmethod
            # Dispatch one tool call by name and return its ToolOutput.
        ...
        # EDGAR ticker → filing → primary document URL resolution.

        class SecFilingTool:

            @staticmethod
            # Tokenize a SEC / ticker / form query string.
            ...
            @staticmethod
            # Normalize an SEC form name (e.g. 10-K variants).
            ...
            @staticmethod
            # HTTP GET JSON from an EDGAR / SEC endpoint.
            ...
            @staticmethod
            # Choose the best matching filing from a company list.
            ...
            @staticmethod
            # Resolve company → filing → primary document URL.
        ...
        # Chat helpers (simple turns + tool loop turns), optional upstream pinning.

        class LlmClient:

            @staticmethod
            # Build a low-thinking LLM config for cheap turns.
            ...
            @staticmethod
            # Optional upstream model pinning / lane selection.
            ...
            @staticmethod
            # One-shot chat without tool calls.
            ...
            @staticmethod
            # Chat turn that may request tool calls.
        ...
        # Stub LLM message used when a turn must return an empty payload.
        ...
        # Stub choice wrapping _EmptyChoiceMessage.

        class _EmptyChoice:
            message = _EmptyChoiceMessage()


        # Stub llm object with empty raw_text / choices.
        ...
        # Stub turn payload returned on hard payload/size gates.
        ...
        # Usability gates, digest, and deterministic rescue from the ledger.

        class AnswerFloor:

            @staticmethod
            # Detect drafts that are raw tool JSON rather than an answer.

            def _looks_like_tool_json(s: str) -> bool:
                return bool(re.match(r'\s*\{\s*"(?:name|tool|function)"\s*:', s))

            @staticmethod
            # Detect useless repeated-token drafts.
            ...
            @staticmethod
            # True when the draft clears basic usability gates.
            ...
            @staticmethod
            # Light cleanup of a draft before floor / rescue checks.

            def _sanitize_draft(text: str) -> str:
                return _VERIFY_MARK_RE.sub("", text or "").strip()

            @staticmethod
            # Compact digest of ledger rows for rescue prompting.
            ...
            @staticmethod
            # Prefer an informative lead sentence from the digest.
            ...
            @staticmethod
            # Deterministic fallback answer built from the ledger.
            ...
            @staticmethod
            # Table of short quotes keyed by citation number.
            ...
            @staticmethod
            # Count of retained-evidence rows in the ledger.

            def _retained_count(ledger: EvidenceLedger) -> int:
                return sum(len(r.get("retained") or []) for r in ledger.rows)


        # Digest write / knowledge resort / schema coerce / answer cap.

        class RescueWriter:

            @staticmethod
            # Ask the model to write an answer from the digest alone.
            ...
            @staticmethod
            # Re-ask with knowledge-resort prompting when drafts fail.
            ...
            @staticmethod
            # Produce output constrained to the query schema.
            ...
            @staticmethod
            # Classify the expected schema shape (scalar / list / object).
            ...
            @staticmethod
            # Check whether text roughly matches the schema kind.
            ...
            @staticmethod
            # Expand digest rows into schema-friendly snippets.
            ...
            @staticmethod
            # Coerce / rewrite the answer to satisfy the schema.
            ...
            @staticmethod
            # Remove leading meta-narration before the real answer.
            ...
            @staticmethod
            # Cap answer length to a hard character ceiling.
        ...
        # Bracket normalize, [n] harvest, verbatim/citation assembly.

        class CitationBuilder:

            @staticmethod
            # Normalize citation brackets to [n] form.

            def _normalize_brackets(text: str) -> str:
                return (text or "").translate(_BRACKET_FIX)

            @staticmethod
            # Harvest citation numbers referenced in the draft.
            ...
            @staticmethod
            # Keep the primary answer line; drop trailing narration.
            ...
            @staticmethod
            # Pull a verbatim quote from a cited source row.
            ...
            @staticmethod
            # Build structured verbatim quote payloads for citations.
            ...
            @staticmethod
            # Assemble CitationRef / CitationSlice objects for the draft.
        ...
        # Brief, optional roster/preseed, main tool loop, and audit patch.

        class ResearchLoop:

            @staticmethod
            # Ask the model for a short research brief / plan.
            ...
            @staticmethod
            # Produce initial search queries from the brief/roster.
            ...
            @staticmethod
            # Run seed searches before the main tool loop.
            ...
            @staticmethod
            # Main research tool loop until budget / wrap-up.
            ...
            @staticmethod
            # Post-loop audit and light answer patching.
        ...
        # Detects set-completeness / superlative questions and wrap-up order.

        class QuestionClassifier:

            @staticmethod
            # Decide whether wrap-up should list / rank / compare answers.
            ...
            @staticmethod
            # True when the question uses superlative language (most, largest, …).
            ...
            @staticmethod
            # True when a superlative claim needs comparative evidence.
            ...
            @staticmethod
            # True when the question asks for a complete set of items.
        ...
        # Top-level solve orchestration for this path's compiled agent.

        class QuerySolver:

            @staticmethod
            # Orchestrate research → draft → floor → rescue → Response.
        ...
        # Inner entrypoint for this path (returned by _compile; not the module entry).
        ...
        # Hand the compiled async query callable back to the module router.

        return query

# =============================================================================
# DifficultyRouter — cheap LLM classifier for easy / medium / hard
# Used once per request; failures default the outer router to 'hard'.
# =============================================================================
...
# =============================================================================
# MediumPath — dual-lane agent with roster and figure corroboration
# Compiles nested helpers, then returns an async query(Query) -> Response.
# =============================================================================
...
class Rank2:
    ...
    def _compile(self):
        ...
        class ResearchLoop:
            ...
            @staticmethod
            # Build a candidate roster for set-style questions.
        ...
        # Extra search when a lead figure rests on a single cited source.

        class Corroborator:

            @staticmethod
            # Extract the lead numeric figure from a draft answer.
            ...
            @staticmethod
            # Map which cited rows support the lead figure.
            ...
            @staticmethod
            # Search for a second source if the figure is single-sourced.
...
# =============================================================================
# HardPath — dual-lane agent with roster, corroboration, and value guards
# Compiles nested helpers, then returns an async query(Query) -> Response.
# =============================================================================
...
class Rank3:
    ...
    def _compile(self):
        ...
        _VALUE_STRIP_CITES_RE = re.compile(r"\[[0-9][0-9,\s\-]*\]")
        _VALUE_RE = re.compile(r"\$?\b\d[\d,]*(?:\.\d+)?%?")
        _VALUE_MAX_FLAGGED = 4


        # Numeric / value consistency checks against gathered evidence.

        class ValueGuard:

            @staticmethod
            # Extract numeric / key values claimed in the draft answer.
            ...
            @staticmethod
            # True when a claimed value appears in gathered evidence.
            ...
            @staticmethod
            # List claimed values that lack evidence support.
            ...
            def _unsupported_values(answer: str, ledger: EvidenceLedger) -> list[str]:
                ...
                flagged = [v for v in _answer_values(answer)
                           if not _value_supported(v, texts)]
                return flagged[:_VALUE_MAX_FLAGGED]

            @staticmethod
            # Search / rewrite to repair unsupported values.

            async def _value_repair(question: str, answer: str, messages: list[dict],
                                    ledger: EvidenceLedger, deadline: float) -> str:
                ...
                order = ("VALUE AUDIT: these answer values appear in NO tool result the "
                         "answer cites: " + ", ".join(values) + ". For each one either "
                         ...
                         "or must be hedged out. Then rewrite the COMPLETE final answer "
                         "with [n] citations in the required shape.")
                messages.append({"role": "system", "content": order})
                patched, _ = await _loop(question, "", ledger, deadline, 3,
                                         carry=messages, allow_tools_in_wrapup=True)
                patched = (patched or "").strip()
        ...
        class QuerySolver:
            ...
            async def _solve(query: Query, question: str) -> Response:
                ...
                try:
                    if _is_usable_answer(answer) and (deadline - monotonic()) > 80.0 \
                            and _spend_left() >= AUDIT_MIN_USD:
                        repaired = await _value_repair(question, answer, messages,
                                                       ledger, deadline)
...
# ---------------------------------------------------------------------------
# Unused scaffolding (not referenced by Easy/Medium/Hard paths or the router).
# Kept for local experiments only; does not alter runtime behavior.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Import-time compile: materialize the three path callables once.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Module entrypoint: classify difficulty, then dispatch to Easy/Medium/Hard.
# On classifier failure, fall back to the hard path.
# ---------------------------------------------------------------------------
