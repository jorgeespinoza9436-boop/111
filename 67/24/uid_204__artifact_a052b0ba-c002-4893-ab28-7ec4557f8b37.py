...
# Lane B is the last rung of the rescue ladder, reached only when both the pinned
# and unpinned lane-A calls fail. It pointed at ai_gateway, whose key we do not
# store: every call on that rung would have failed provider auth, so the ladder
# ended one rung short of where it looks like it ends. Pointed at a lane we hold a
# key for, with a DIFFERENT model from lane A so the rung still buys independence
# from a single model's outage rather than retrying the one that just failed.
...
LOOP_MODEL_B = "z-ai/glm-5"        # ai_gateway's glm-5.2-fast is not an openrouter route
...
RETAIN_MARGIN_CHARS = 259     # context kept either side of a retained quote
...
FETCH_PLAIN_CHARS = 6498     # small pages render whole
...
def _needs_bound_discipline(question: str) -> bool:
    """Algorithmic-bound questions ("give an upper bound on ...") were losing on
    the proof, not the number: the model states a bound and never shows the
    dominating term, so the judge cannot check it. Fires the bound rule."""
...
BOUND_RULE = (
    "ASYMPTOTIC BOUND — SHOW THE DOMINATING TERM. State the bound, then justify "
    "it: name the operation being counted, give its cost per step and the number "
    "of steps, and show which term dominates. A bound with no visible derivation "
    "reads as a guess even when it is right. If the bound is tight, say so and "
    "give the matching lower construction; if it is loose, say what slack remains."
)
...
# ── outermost stage: quote-grounding desk (hx21) ─────────────────────────────
# Architecturally distinct post/around-base subsystem. Reachable on
# ordinary successful prose requests when wall and spend allow.
# Never edits answer text; citation merge only.
...
class _Hx21QuoteDesk:
    """Quote → primary-document grounding desk (search then fetch_page)."""
...
async def _hx21_ask(system: str, user: str, max_tokens: int, timeout: float) -> str:
    ...
    payload = await llm_chat(
        provider=LLM_LANE_A, model=_HX21_MODEL,
        messages=[{"role": "system", "content": system},
                  {"role": "user", "content": user}],
        temperature=0.0, max_output_tokens=max_tokens, timeout=timeout,
        thinking=_least_think(LLM_LANE_A, _HX21_MODEL),
        provider_extra=pin)
    _spend_note(payload)
...
async def _hx21_ground_one(quote: str, query: str) -> list:
    """Search → pick first URL → fetch_page → match quote → CitationRef."""
    try:
        search = await search_web(query, provider=SEARCH_PROVIDER, num=2,
                                  timeout=_HX21_SEARCH_TIMEOUT_S)
        _spend_note(search)
    except Exception:
        return []
    ...
    if not url:
        return []
    try:
        page = await fetch_page(url, provider=SEARCH_PROVIDER,
                                timeout=_HX21_FETCH_TIMEOUT_S)
        _spend_note(page)
    except Exception:
        # Fall back to search snippet if fetch fails.
        if receipt_search and isinstance(rid, str) and note:
            end = min(_HX21_SLICE_CHARS, len(note))
            try:
                return [CitationRef(receipt_id=receipt_search, result_id=rid,
    ...
    text = (getattr(page, "text", None) or getattr(page, "content", None)
            or getattr(page, "note", None) or note or "")
    ...
    try:
        raw = await _hx21_ask(
            _HX21_MATCH_SYSTEM,
            f"Quote:\n{quote}\n\nPage text:\n{text[:12000]}",
            500, _HX21_MATCH_TIMEOUT_S)
        parsed = _hx21_json(raw) or {}
    except Exception:
        parsed = {}
    if not parsed.get("grounded"):
        # Still bind a head slice so the grounding attempt leaves provenance
        # even when the quote is not literally present.
    ...
    if end <= start:
        return []
    try:
        return [CitationRef(receipt_id=receipt, result_id=page_rid,
...
async def _hx21_ground(question: str, response: Response) -> Response:
    ...
    bundles = await asyncio.gather(*[
        _hx21_ground_one(q["quote"], q["query"]) for q in desk.quotes
    ...
    seen = {(getattr(r, "receipt_id", None), getattr(r, "result_id", None))
            for r in existing}
...
async def query(query: Query) -> Response:
    ...
    try:
        ...
        if _spend_left() < _HX21_MIN_SPEND_USD:
            return response
        window = min(_HX21_WINDOW_MAX_S,
                     max(8.0, WALL_BUDGET_S - (monotonic() - started) - 8.0))
        return await asyncio.wait_for(
            _hx21_ground((query.text or "").strip(), response), timeout=window)
    except Exception:
        return response
