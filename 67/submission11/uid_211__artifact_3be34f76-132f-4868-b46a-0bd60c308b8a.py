...
async def _solve(question: str, deadline: float) -> tuple[str, Ledger]:
    # S9 claim sheet drives preseed / parallel retrieval
    try:
...
async def _claim_gap_reresearch(question: str, answer: str, messages: list[dict], ledger, deadline: float) -> str:
    """Forced claim-gap re-retrieval (sandbox-safe)."""
    from time import perf_counter as _pc
    rem = deadline - _pc()
    if not (answer or "").strip() or rem < 50.0:
        return answer
    user = (
        "Output JSON ONLY: {\"gap_queries\":[...up to 4...]}. Use [] if complete.\n\n"
        f"Question:\n{question}\n\nDraft:\n{answer[:10000]}"
    )
    try:
        chat = await llm_chat(
        ...
        return answer
    if not queries or deadline - _pc() < 35.0:
        return answer
    try:
        evidence = await _tool_search_many(queries, ledger)
    except Exception:
        try:
            evidence = await _run_search_many(queries, ledger)
        except Exception:
            return answer
    messages.append({
        "role": "system",
    ...
    if schema is None:
        ...
        if answer and (deadline - monotonic()) > 40:
            try:
                ...
                pass
        if answer and (deadline - monotonic()) > 50:
            try:
                answer = await _claim_gap_reresearch(
                    question, answer, [], ledger, deadline
                )
            except Exception:
                pass
