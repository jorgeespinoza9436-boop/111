#!/usr/bin/env bash
set -euo pipefail
# Submit all matched submission11 agents in parallel.
# Usage: bash /root/turtle/111/67/submission11/submit_all.sh [PARALLEL_JOBS]
ROOT="/root/turtle/harnyx"
JOBS_FILE="/root/turtle/111/67/submission11/submit_jobs.txt"
PARALLEL_JOBS="${1:-14}"
WALLET="money"
cd "$ROOT"
