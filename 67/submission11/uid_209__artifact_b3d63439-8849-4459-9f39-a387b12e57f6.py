...
TOOLS = [{'type': 'function', 'function': {'name': 'search_web', 'description': 'Search the web. Returns numbered results with title, url and a short excerpt.', 'parameters': {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'search query'}}, 'required': ['query']}}}, {'type': 'function', 'function': {'name': 'search_many', 'description': 'Run up to 8 web searches in parallel to enumerate/verify a candidate set in one step.', 'parameters': {'type': 'object', 'properties': {'queries': {'type': 'array', 'items': {'type': 'string'}, 'description': 'up to 8 search queries'}}, 'required': ['queries']}}}, {'type': 'function', 'function': {'name': 'fetch_page', 'description': 'Fetch one URL and return its extracted main text content.', 'parameters': {'type': 'object', 'properties': {'url': {'type': 'string', 'description': 'URL to fetch'}}, 'required': ['url']}}}]
...
async def _claim_gap_reresearch(question: str, answer: str, messages: list[dict], index: _ResultIndex, deadline: float) -> str:
    ...
    if not (answer or '').strip() or _remaining(deadline) < 50.0:
        return answer
    user = (
        'Output JSON ONLY with key gap_queries (max 4) for missing elements, uncited facts, '
        'comparison sides, or exclusions. Use [] if complete.\n\n'
        f'Question:\n{question}\n\nDraft:\n{answer[:10000]}'
    )
    try:
        raw = await _plain_chat(PATCH_MODEL, system='Claim-gap planner. JSON only.', user=user, max_tokens=500, timeout=min(25.0, max(8.0, _remaining(deadline) - 25.0)))
