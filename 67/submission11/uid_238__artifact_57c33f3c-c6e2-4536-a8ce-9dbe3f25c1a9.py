...
_NAMED_SOURCE_DOMAIN_MAP = {
    "world bank": "data.worldbank.org", "rotten tomatoes": "rottentomatoes.com",
    "tomatometer": "rottentomatoes.com", "imdb": "imdb.com", "afl tables": "afltables.com",
    "box office mojo": "boxofficemojo.com", "billboard": "billboard.com",
    "metacritic": "metacritic.com", "goodreads": "goodreads.com", "ballotpedia": "ballotpedia.org",
    "baseball reference": "baseball-reference.com", "basketball reference": "basketball-reference.com",
    "pro football reference": "pro-football-reference.com", "hockey reference": "hockey-reference.com",
    "hugo award": "thehugoawards.org", "sec filing": "sec.gov", "edgar": "sec.gov",
    "10-k": "sec.gov", "8-k": "sec.gov", "imf": "imf.org", "oecd": "oecd.org",
    "eurostat": "ec.europa.eu", "fred": "fred.stlouisfed.org", "census bureau": "census.gov",
    "bureau of labor statistics": "bls.gov", "nasa": "nasa.gov", "noaa": "noaa.gov",
    "cdc": "cdc.gov", "world health organization": "who.int", "arxiv": "arxiv.org",
    "pubmed": "pubmed.ncbi.nlm.nih.gov", "discogs": "discogs.com", "musicbrainz": "musicbrainz.org",
    "steam": "store.steampowered.com", "riksdag": "riksdagen.se",
}


def _named_source_hint(question: str) -> str:
    """Deterministic: if the question names its authority, tell the agent to prefer that domain."""
    folded = " ".join(question.casefold().split())
    domains = []
    for name, domain in _NAMED_SOURCE_DOMAIN_MAP.items():
        if name in _NAMED_SOURCE_AMBIGUOUS:
            pattern = rf"{_NAMED_SOURCE_ATTRIB_RE}\s+(?:the\s+)?{re.escape(name)}\b"
        else:
    ...
    return (
        f"\n\nSOURCE DIRECTIVE: This question names its authoritative source(s). You MUST prioritize "
        f"retrieving and citing from: {joined}. Search that source directly (site: or the source name) "
        f"and prefer its data over any other site for the requested facts."
    )
