...
"""
    ...
    finding). Named-year miss -> None; no year -> most recent of that form."""
                ...
                _OUTPUT_ONLY_RE = re.compile('\\boutput\\s+only\\b|\\brespond\\s+(?:with\\s+)?only\\b|\\bgive\\s+only\\b|\\bprovide\\s+only\\b|\\blist\\s+only\\b|\\bname\\s+only\\b|\\bstate\\s+only\\b|\\breturn\\s+only\\b|\\bwrite\\s+only\\b', re.IGNORECASE)
    ...
    paragraph IS the complete answer."""
                    ...
                    bare = bare.replace('**', '')
                    bare = re.sub('\\s{2,}', ' ', bare).strip()
                    bare = re.sub('\\s+([,.])', '\\1', bare)
                    return bare
    ...
    chrome and search result metadata."""
                    ...
                    if not entities:
                        return ''
                    return '\n'.join(entities)
                _COERCE_URL_RE = re.compile('\\s[—–\\-]\\s*https?://')
                ...
                def _is_coerce_metadata(line: str) -> bool:
                    ...
                    if not s or len(s) < 5:
                        return True
                    if s.startswith(('Best-supported', '# web_search', '# read_page', 'Sources retrieved', 'Numbered evidence', 'Automatic first-pass')):
                        return True
    ...
    that carries an [n]: that is a real answer, however it opens."""
                ...
                async def _produce_response(question: str, raw_answer: str, ledger: EvidenceLedger, query_obj, deadline: float, draft: str='') -> Response:
    ...
    tail where 100+ lines of inline rescue + Response building lived."""
                    ...
                    if query_obj.output_schema is not None:
                        structured = None
                        try:
                            structured = await _schema_output(question, rendered, query_obj.output_schema, deadline)
                        except Exception:
                            structured = None
...
"""
    ...
    structured answer is still scored, whereas an invalid payload discards the whole task."""
        ...
        _TAG_1D68EE92 = '1d68ee922336488b8148c07e9c646e2f'
        import logging as _tag_logging_1d68ee92
        _tag_logging_1d68ee92.getLogger('miner.tag').debug('tag=%s', _TAG_1D68EE92)
        return query
...
class ReserveSolver:
    ...
    def _compile(self):
        ...
        _SEARCH_AI_TOOL = {'type': 'function', 'function': {'name': 'search_ai', 'description': 'AI semantic search: pass a FULL natural-language question; returns a few results with LONG, dense extracts (far richer than search_web). Best for obscure/hard facts or broad coverage.', 'parameters': {'type': 'object', 'properties': {'prompt': {'type': 'string', 'description': 'a full natural-language question'}}, 'required': ['prompt']}}}
        ...
        _DISCRETE_CITE_NOTE = '\n\nDISCRETE CITATION: attach a SEPARATE [n] to EACH decisive value (each year, board, candidate, figure) -- never one citation covering several distinct values; the grader validates each figure against its own source.'
        _MULTIPART_DIRECTIVE = "\n\nANSWER EVERY CLAUSE (this question has MULTIPLE parts): decompose it into ALL its parts -- the main ask AND every secondary/embedded sub-question, stated premise, or supplied 'given' fact -- and address EACH explicitly; a correct main answer that omits a secondary part scores far lower. PREMISE VERIFICATION: if it states or supplies a fact (a date, definition, identity, or 'given' value), verify it with a tool and confirm it in a short 'Premise:' line WITH a [n]. Before the FINAL ANSWER, self-check that every clause is answered and every stated fact is verified+cited -- but still COMMIT a complete answer; never end on a plan or a tool-intent sentence."
        ...
        async def _concise_recommit(messages, prior, deadline):
            ...
            if r:
                _spend_note(r)
            return _strip_draft((r.response.raw_text or '').strip()) if r else ''
        _SET_DIRECTIVE = "\nSET/ENUMERATE QUESTION -- it asks for the COMPLETE set. Give a full 'Proof of completeness': CANDIDATE POOL (name every candidate), apply EACH criterion with a citation, then ONE cited line per QUALIFYING item and ONE per key EXCLUDED candidate with the reason it fails. Enumerate them ALL. Keep each line short. Each line MUST carry the candidate's exact DECIDING value with a [n] to the NAMED authority for that criterion (the filter's source may differ from the pool's -- cite the filter source). Batch the per-candidate lookups in one turn."
...
_TAG_1544D332="1544d332b4a34ad7aeba39d849a682f2"
import logging as _tag_logging_1544d332
_tag_logging_1544d332.getLogger("miner.tag").debug("tag=%s", _TAG_1544D332)
