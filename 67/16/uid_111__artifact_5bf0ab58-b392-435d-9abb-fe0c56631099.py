# 20260804 hk400 — 08-02 base hk415 (official-source, scored 0.84) + per-constraint verify
...
SEARCH_PROVIDER = "parallel"             # primary search/fetch provider
# 2026-08-04 PROVIDER FALLBACK. A stored Parallel key went to HTTP 402
# (out of credit) between batches; a search provider that returns nothing turns
# every task into an uncited guess, which is how two hotkeys lost their
# registration. Desearch credentials are stored for every hotkey and serve the
# same normalized search_web / fetch_page contract, so each call now tries the
# providers in order and only gives up when BOTH fail. The fallback costs an
# extra call only when the primary actually fails.
SEARCH_PROVIDERS = ("parallel", "desearch")


async def _search_any(query: str, *, num: int, timeout: float):
    """search_web across providers in order; returns the first payload with results."""
    last = None
...
async def _fetch_any(url: str, *, timeout: float):
    """fetch_page across providers in order; returns the first payload with content."""
    last = None
    for provider in SEARCH_PROVIDERS:
        try:
...
# ── stage 2b: per-constraint verification (NEW in v35) ───────────────────────
# The production weakness this stage exists to fix: on multi-constraint / set /
# exact-value questions the agent commits WRONG or INCOMPLETE specific values /
# set membership because it never verifies EACH stated condition against evidence
# before committing. This is a RESEARCH stage (it goes and fetches the missing
# proof), not another post-hoc guard. It runs AFTER the main loop and BEFORE the
# synthesis/audit phases:
#   1. One LLM call decomposes the question into its explicit CONSTRAINTS (each a
#      {subject/attribute, relation, value} the answer must satisfy) and the
#      candidate answer entities, marking which constraints are already backed by
#      the numbered evidence.
#   2. For each still-unverified constraint that lacks direct ledger support it
#      runs ONE targeted web_search aimed precisely at that constraint+entity
#      ("<entity> <attribute> <value/units>", provider=parallel) — the total is
#      capped (MAX_CONSTRAINT_SEARCHES) and it stops early when time/USD is low —
#      and appends the results to the EvidenceLedger.
#   3. It feeds that verified-constraint evidence into a synthesis rewrite (the
#      same carry-the-transcript mechanism the audit/claim stages use) so the
#      writer confirms EACH condition with a citation and DROPS any candidate that
#      fails a constraint.
# Strictly additive and budget-bounded: below the time/spend floors, or on any
# exception, it returns the answer untouched and the base v34 path is unchanged.
_CONSTRAINT_PROBE = (
...
def _constraint_query(c: dict) -> str:
    """Targeted search text for one unverified constraint. Prefer the model's own
    query; otherwise compose '<entity> <attribute> <value/units>' from the fields."""
    sq = " ".join(str(c.get("search") or "").split())
    if sq:
        return sq
...
async def _constraint_verify(question: str, answer: str, messages: list[dict],
                             ledger: EvidenceLedger, deadline: float) -> str:
    """Per-constraint verification + synthesis rewrite. Returns a revised answer,
    or the original when nothing needs (or has time/budget for) verification."""
    # Needs room for: one decomposition call + up to 3 targeted searches + a
    # synthesis rewrite loop. Below this the stage is pure cost with no effect, so
    # it degrades to the base behavior.
    ...
    if not isinstance(constraints, list) or not constraints:
        return answer
    # Collect the still-unverified constraints and their targeted queries. Dedupe
    # queries so the capped budget is spent on distinct conditions.
    unresolved: list[str] = []          # human-readable, for the rewrite directive
    ...
    for c in constraints:
        ...
        if bool(c.get("verified_in_evidence")):
            continue                    # already backed by the numbered evidence
    ...
    if not unresolved:
        return answer                   # every constraint already has ledger support
    # Targeted per-constraint searches: capped, parallel, committed in query order
    # (run-invariant [n] numbering), and only while time/spend remain.
    ...
    revised, _ = await _loop(question, "", ledger, deadline,
                             ...
                             allow_tools_in_wrapup=True)
    revised = revised.strip()
    # never let the rewrite collapse a good answer (same guard as _verify_and_repair)
    if not _is_usable_answer(revised) or len(revised) < int(len(answer) * 0.6):
        return answer
    return revised
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # ── PHASE 4d: per-constraint verification guard (agent_deep port, additive) ─
    # Deepen RESEARCH before synthesis: decompose the question into its explicit
    # constraints, run up to 3 targeted searches for the conditions lacking direct
    # ledger support, and feed that evidence into a synthesis rewrite so each
    # stated condition is confirmed with a citation and any entity that fails a
    # constraint is dropped. Fires only for the weakness class (multi-constraint /
    # set / superlative / exact-value questions) and only with ample time + USD;
    # otherwise it degrades to the v34 behavior. Fully wrapped so it can never
    # break the base path.
    try:
