...
VERSION = "v36.0-solo"   # v36: STRUCTURAL HARDENING ONLY. No model, prompt, budget
#   or heuristic changed — every constant below is byte-identical to v35. What
#   changed is the load-bearing plumbing: unguarded SDK attribute chains, the
#   all-or-nothing research loop, orphaned tool replies, unawaited task
#   cancellation, cross-query global bleed, and citations computed from a string
#   that is not the one shipped. See _HARDENING_NOTES at the foot of the file.
...
def _spend_reset() -> None:
    """v36 S6: clear cross-query spend state.

    `_SPEND` is module-global and a worker serves many queries. v35 only ever
    OVERWROTE it, so a value learned on a previous query survived into this one
    until the first successful `tooling_info`/`llm_chat` refreshed it. When that
    refresh failed (it is wrapped in a bare except), a stale `left` near zero
    silently disabled the knowledge brief AND the audit for the whole run — two
    scoring stages skipped on a budget that was never actually spent. Resetting
    to None restores the documented default of 1.0 until real data arrives."""
    _SPEND["left"] = None
...
_SEC_CACHE_MAX = 8              # v36 S7: bound the cross-query global (see _fetch_json)
...
async def _fetch_json(url: str, deadline: float):
    ...
    for _attempt in (0, 1):   # large-JSON crawls intermittently return empty
        ...
        if isinstance(obj, dict):
            # v36 S7: `_SEC_CACHE` is module-global and therefore shared by every
            # query this worker ever serves. v35 never bounded it: the ticker index
            # alone is ~10MB parsed, and each distinct submissions URL added
            # another entry that no query ever freed. Keep the hot entries, evict
            # in insertion order once the map is full — the tickers URL is
            # re-fetched at worst, which the existing retry ladder already covers.
...
async def _run_tool(call, question: str, ledger: EvidenceLedger, deadline: float) -> str:
    ...
    # v36 S5: v35 passed `deadline` to _do_sec_filing but NOT to search/fetch, so
    # a call dispatched with seconds left still committed to its own fixed ceiling
    # (up to 3 x SEARCH_TIMEOUT_S for the search ladder). The fan-out `asyncio.wait`
    # bounds the group, but the task keeps running after cancellation is requested
    # and the pre-seed path bounds separately. Refusing to start a tool that cannot
    # finish inside the tail returns a valid reply for the id and protects the
    # rescue ladder, which is where a late run actually earns its score.
...
# ── v36 S3: GUARDED SDK ACCESSORS ────────────────────────────────────────────
# v35 read the provider payload with raw attribute chains — `choices[0].message`
# in two places, `msg.to_input_message()`, `call.id`. Every one of those is an
# AttributeError/IndexError waiting on a malformed or partial payload, and in
# `_loop` that exception escaped all the way to `_solve`, which threw away the
# answer AND the transcript (see S1/S2). These helpers return a value or a
# neutral default; they never raise. All attribute names are string literals, so
# nothing here is reflection.


def _payload_message(payload):
    """First choice message of a payload, or None. Never raises."""
...
def _message_text(payload) -> str:
    """raw_text, else first-choice content, else ''. Never raises."""
    llm = getattr(payload, "llm", None)
...
def _call_id(call) -> str:
    """tool_call id as a string, or ''. An empty id is unusable as a reply key."""
    raw = getattr(call, "id", None)
    return raw if isinstance(raw, str) and raw else ""


def _assistant_turn(msg, calls) -> dict | None:
    """The assistant tool-call turn as a transcript message.

    Prefers the SDK's own serializer; falls back to the standard tool_calls
    shape when that method is missing or raises. Returns None only when neither
    path can produce a message — and the caller MUST then stop rather than
    append tool replies, because a tool reply whose tool_call never appears in
    the transcript fails host-side validation and zeroes the response (S3b)."""
    try:
    ...
    for call in calls:
        cid = _call_id(call)
        if not cid:
            return None          # cannot key a reply to it -> transcript would break
        serialized.append({
            "id": cid,
            "type": "function",
            "function": {"name": getattr(call, "name", "") or "",
...
async def _chat_simple(lane: str, model: str, system: str, user: str, *,
    ...
    _spend_note(payload)
    # v36 S3: was `choices[0].message` — an IndexError on an empty choice list and
    # an AttributeError on a choice without `.message`. Same precedence, no raise.
...
async def _loop(question: str, brief: str, ledger: EvidenceLedger,
    ...
    for turn in range(1, turn_cap + 1):
        ...
        try:
            ...
            # v36 S1: a turn-level failure ends the loop, it does not destroy the
            # run. `answer` and `messages` accumulated so far are still returned,
            # so the ledger, the rescue ladder and the audit stage stay reachable.
            break
        if payload is None:
            break
        ...
        if not calls:
            ...
            break
        # v36 S3b TRANSCRIPT INTEGRITY. v35 called `msg.to_input_message()` bare.
        # If it raised, the exception escaped the loop and the run lost everything;
        # worse, any partially-appended state left tool replies with no matching
        # assistant tool_call. Build it defensively, and if it cannot be built at
        # all, stop BEFORE appending replies so the transcript stays valid.
        assistant_turn = _assistant_turn(msg, calls)
        if assistant_turn is None:
            break
        # Only ids we can actually key a reply to may be answered; an unanswered
        # id and an orphan reply are both transcript-validation failures.
        if any(not _call_id(c) for c in calls):
            # At least one call is unkeyable, so this turn cannot be completed as
            # a tool turn without corrupting the transcript. Do NOT open it — but
            # do not throw the run away either: push the model onto the direct-
            # answer path with the repair order already used for junk output, and
            # keep researching with whatever evidence the ledger holds.
            if repairs_left > 0 and (deadline - monotonic()) > MIN_TAIL_S + 10.0:
                repairs_left -= 1
                messages.append({"role": "system", "content": _REPAIR_ORDER})
                continue
        ...
        # v36 S4: v35 cancelled and walked away. A cancel() only REQUESTS
        # cancellation — the task stays scheduled, keeps its socket, and competes
        # with the next turn for the same wall budget; its unretrieved exception
        # is also surfaced by the loop's default handler. Reap them, bounded, so
        # a wedged task can never extend the reaping itself.
        if cancelled:
            try:
                ...
                pass
            for t in cancelled:
                # `.exception()` RAISES CancelledError on a cancelled task, and
                # CancelledError is a BaseException in 3.8+ — an `except Exception`
                # here would let it escape and kill the loop. Skip cancelled tasks
                # (there is nothing to retrieve) and only reap genuine failures.
...
async def _solve(query: Query, question: str) -> Response:
    ...
    _spend_reset()      # v36 S6: no cross-query bleed into the stage gates below
    try:
    ...
    try:
        ...
        # v36 S2: v35 also left `messages` at [] here, which silently disabled the
        # audit stage for the rest of the run (it patches against the transcript).
        # The loop is now internally guarded and returns partial work instead of
        # raising, so this is a last-resort net rather than the normal path.
        answer = ""
        messages = []
    ...
    # v36 S8: build the ref list from the string we actually SHIP. v35 read the
    # pre-transform draft, so any [n] that _strip_lead_narration removed or _cap
    # truncated away still shipped a ref — refs the judge cannot match to a marker
    # in the text, charged against the 120k materialization wall and the 24-ref
    # cap, crowding out refs for markers that DID survive. Same function, same
    # budget, same order; only the input is now the shipped text.
    try:
    ...
    if query.output_schema is not None:
        ...
        if not basis or _STUB_ANSWER_RE.match(basis.strip()):
            basis = question[:400]
        basis = _cap(basis)
        # v36 S8b: this branch may ship a DIFFERENT string than `text` (the
        # deterministic rung, or the bare question). v35 attached the `text`-derived
        # ref list to it regardless, pairing refs with markers that are not in the
        # shipped payload. Re-derive against the basis; keep the old list if the
        # basis carries no markers of its own, so a working case never loses refs.
        try:
        ...
        try:
            ...
            try:
                return Response(output=basis[:2000],
                                citations=basis_citations or None)
            except Exception:
                pass
...
# ── v36 HARDENING NOTES ───────────────────────────────────────────────────────
# Structure only. No model, provider, prompt, threshold, timeout or heuristic in
# this file was changed, so the measured behaviour of v35 is preserved exactly.
#
# S1  _loop no longer aborts the run on a turn-level exception. The chat call is
#     guarded and a failure breaks the loop, returning the answer and transcript
#     accumulated so far instead of unwinding into _solve's bare except.
# S2  _solve resets `messages` when _loop does raise. v35 left it at [], which
#     silently disabled the audit stage (it patches against the transcript).
# S3  Raw SDK attribute chains replaced with non-raising accessors:
#     `choices[0].message` (x2), `msg.to_input_message()`, `call.id`. All names
#     are string literals — no reflection, no dynamic getattr.
# S3b Transcript integrity is now guaranteed rather than assumed. The assistant
#     tool-call turn is built defensively; if it cannot be built, or if any call
#     lacks a usable id, the turn is not opened at all. An orphan tool reply and
#     an unanswered tool_call id are both host-side validation failures.
# S4  Timed-out tool tasks are reaped after cancel(), bounded at 1s. v35 fired
#     cancel() and moved on, leaving the task scheduled against the same wall
#     budget with an unretrieved exception.
# S5  _run_tool refuses to start a tool inside MIN_TAIL_S. It still returns a
#     valid reply for the id, so the transcript stays whole and the rescue
#     ladder keeps the headroom it needs.
# S6  _SPEND is reset per query. As a module global it survived across queries in
#     a worker; a stale near-zero value skipped the brief and the audit on a
#     budget that had not actually been spent.
# S7  _SEC_CACHE is bounded. It is a cross-query global holding a ~10MB parsed
#     ticker index plus one entry per submissions URL, and nothing freed it.
# S8  Citations are derived from the string that ships, after normalization,
#     narration-stripping and capping — and, in the schema branch, from the basis
#     actually coerced. v35 derived them from a pre-transform draft.
#
# Provider: openrouter is the only LLM lane (LLM_LANE), both loop rungs and every
# auxiliary model. ai_gateway appears nowhere outside historical comments.