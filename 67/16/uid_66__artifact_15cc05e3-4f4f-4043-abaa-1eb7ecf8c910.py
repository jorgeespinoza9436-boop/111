"""SN67 Harnyx miner — staged research protocol with Easy/Hard routing.

Architecture overview
=====================
1. DifficultyRouter (Gemma) classifies the question as easy or hard.
2. EasyPath / HardPath each expose a compiled run(query) callable
   (uid_155-style _compile()). Both share the same staged pipeline
   _plain_query; only profile dials differ.
3. Pipeline stages inside _plain_query:
      BRIEFING   -> parse CANDIDATE list (tools not executed on turn 1)
      RESEARCH   -> search_web / fetch_page loop under turn + wall caps
      LEDGER     -> one row per entity; reproject pages into proof windows
      CHECKPOINT -> VERIFY + capped re-dispatch for uncovered candidates
      COMMIT     -> tools off; force FINAL ANSWER from evidence digest
      RESTATE    -> HardPath only: re-issue answer to close ledger gaps
4. If query.output_schema is set, the drafted text answer is coerced
   into structured Response(output=...) after the plain path.

Evidence and citations
----------------------
_ResultIndex numbers every search hit and fetched page as [n].
Long fetches keep term-focused windows. Inline [n] markers in the
final answer become CitationRef / CitationSlice objects.

Profiles
--------
Easy: shorter budget, fewer turns, 1 checkpoint turn, no restate.
Hard: full 270s staged protocol including ledger restate.
"""
...
# =============================================================================
# Models, wall budgets, and research/checkpoint caps
# =============================================================================
...
# Research-loop dials (Hard defaults; Easy overrides via EASY_PROFILE)
...
# Inline tool-result and citation sizing
...
# Long-page windowing: keep term-relevant slices instead of full dump
...
# Ledger / restate (HardPath): entity coverage and proof windows
LEDGER_MAX_PASSES = 3
...
# =============================================================================
# Dual-path profiles (Easy = slim staged run; Hard = full protocol)
# =============================================================================
# EasyPath / HardPath budgets (uid_155-style dual path). Hard mirrors prior defaults.
EASY_PROFILE: dict[str, Any] = {
    "budget": 230.0,                 # total wall seconds for this path
    "research_turn_cap": 7,          # fewer research turns than Hard
    "research_time_cap": 100.0,      # earlier handoff to checkpoint/commit
    "checkpoint_tool_turns": 1,      # one VERIFY re-dispatch turn
    "final_reserve": 45.0,           # seconds reserved for commit/retry
    "do_restate": False,             # skip ledger re-issue (Hard-only)
}
HARD_PROFILE: dict[str, Any] = {
    "budget": TASK_TOTAL_BUDGET_SECONDS,          # 270s full protocol
    "research_turn_cap": RESEARCH_TURN_CAP,       # 10 research turns
    "research_time_cap": RESEARCH_TIME_CAP_SECONDS,
    "checkpoint_tool_turns": CHECKPOINT_TOOL_TURNS,
    "final_reserve": FINAL_RESERVE_SECONDS,
    "do_restate": True,                           # enable ledger gap re-issue
}

# =============================================================================
# Tool schemas exposed to the research / checkpoint LLM turns
# =============================================================================
TOOLS = [
    {
        "type": "function",
...
# =============================================================================
# Prompts: research doctrine, briefing nudge, commit, abstention detectors
# =============================================================================
SYSTEM_PROMPT = (
...
# =============================================================================
# Page-window helpers: key terms -> hit spans -> merged windows for long fetches
# =============================================================================
...
# =============================================================================
# Evidence ledger: numbered search hits + fetched pages with optional windows
# =============================================================================
class _ResultIndex:
    """Maps global evidence numbers [n] to search hits and fetched pages.

    Search results store title/url/excerpt; fetches store full note text plus
    optional term-focused windows used for citation slices and digests.
    """
...
# =============================================================================
# Tool runners — search_web / fetch_page -> numbered notes in _ResultIndex
# =============================================================================
...
# =============================================================================
# Citation assembly: parse [n] markers -> CitationRef (+ anchored slices)
# =============================================================================
...
def _citations_from_inline_markers(answer_text: str, index: _ResultIndex) -> tuple[CitationRef, ...]:
    """Build SDK citations from [n] / [n,m] / [n-m] markers in the answer.

    For long fetches, prefer an anchored CitationSlice around the claim text
    so the judge sees the supporting window rather than the whole page.
    """
...
# =============================================================================
# Briefing -> candidate pool; checkpoint coverage message
# =============================================================================
...
def _checkpoint_message(
    ...
    *,
    ...
    max_turns = CHECKPOINT_TOOL_TURNS if tool_turns is None else int(tool_turns)
    ...
    if missing:
        coverage = (
            ...
            f"You may make AT MOST {max_turns} more tool-call turns, targeted "
            "ONLY at exactly these candidates; after that tools are DISABLED and you MUST "
            "commit. "
        )
    else:
        coverage = (
            f"You may make AT MOST {max_turns} more tool-call turns if a "
...
# =============================================================================
# Forced commit: evidence digest context (tools disabled)
# =============================================================================
COMMIT_MESSAGE = (
...
# =============================================================================
# Ledger + RESTATE (HardPath): entity rows, proof windows, gap re-issue
# =============================================================================
# --- RESTATE ----------------------------------------------------------------
...
async def _restated_answer(
    ...
    """HardPath final gate: reproject ledger, then re-issue if gaps remain.

    Always reprojects. If omitted rows are still unsupported and time remains,
    asks the model to rewrite the answer citing those proof windows. EasyPath
    skips this function entirely (profile do_restate=False).
    """
...
# =============================================================================
# LLM turns: research chat, commit (no tools), answer quality gates
# =============================================================================
...
async def _commit_call(messages: list[dict[str, object]], *, deadline: float) -> str | None:
    """Force a text-only final answer (tools disabled) with MODEL then fallback.

    Used when checkpoint did not already produce a FINAL ANSWER section.
    """
...
def _deliverable(text: str | None, index: _ResultIndex, *, cite_text: str | None = None) -> Response:
    """Package the final answer text with citations derived from [n] markers.

    If text is empty, falls back to a dump-floor summary of top evidence notes.
    cite_text may be the fuller pre-extraction body so VERIFY-table markers still
    resolve even when only the FINAL ANSWER section is delivered.
    """
...
async def _execute_tool_calls(
    ...
    """Dispatch tool_calls into _ResultIndex and append role=tool messages.

    Each successful search/fetch becomes a numbered note the model can cite as [n].
    """
    messages.append({
        "role": "assistant",
...
# =============================================================================
# Core staged pipeline (shared by EasyPath and HardPath via profile dials)
# Stages: briefing -> research -> ledger -> checkpoint -> commit -> (restate)
# =============================================================================
async def _plain_query(
    ...
    *,
    ...
    """Run the staged research protocol under an Easy or Hard profile.

    profile dials: research_turn_cap, research_time_cap, checkpoint_tool_turns,
    final_reserve, do_restate. Defaults to HARD_PROFILE when profile is None.
    """
    cfg = profile or HARD_PROFILE
    turn_cap = int(cfg.get("research_turn_cap", RESEARCH_TURN_CAP))
    research_time_cap = float(cfg.get("research_time_cap", RESEARCH_TIME_CAP_SECONDS))
    checkpoint_turns = int(cfg.get("checkpoint_tool_turns", CHECKPOINT_TOOL_TURNS))
    final_reserve = float(cfg.get("final_reserve", FINAL_RESERVE_SECONDS))
    do_restate = bool(cfg.get("do_restate", True))

    start = perf_counter()
    deadline = start + budget
    research_stop = min(start + research_time_cap, deadline - final_reserve)
    index = _ResultIndex()
    ...
    try:
        ...
        nudged = False
        turn = 0
        while turn < turn_cap and perf_counter() < research_stop:
            turn += 1
            thinking_on = turn == 1
        ...
        notice = _ledger_notice(ledger, _reproject(index, ledger, deadline - final_reserve))

        # --- CHECKPOINT: VERIFY + capped targeted re-dispatch ---
        checkpoint = _checkpoint_message(candidates, index, tool_turns=checkpoint_turns)
        if notice:
            checkpoint = notice + "\n\n" + checkpoint
        messages.append({"role": "user", "content": checkpoint})
        last_content = ""
        for _extra in range(checkpoint_turns + 1):
            # a re-dispatch turn only pays if there is still room to run its
            # tools AND a committed final afterwards
            if deadline - perf_counter() <= final_reserve + 25:
                break
        ...
        # --- RESTATE decides what is delivered (HardPath) ---
        # The research turns wrote from what they had been shown. This stage runs
        # on every hard question, re-projects the retained pages against the ledger,
        # and the answer it returns is the one that goes out.
        if display:
            if do_restate:
...
# =============================================================================
# Structured output: coerce free-text draft into query.output_schema JSON
# =============================================================================
...
async def _structured_response(query: Query, schema: object, drafted: Response, deadline: float) -> Response:
    """Map a free-text draft into Response(output=...) matching output_schema.

    Validates with _so_errors; on failure re-prompts the schema model a few
    times, then falls back to a coerced skeleton so the platform always gets
...
# =============================================================================
# Entrypoint + EasyPath / HardPath / DifficultyRouter (uid_155 pattern)
# =============================================================================
@entrypoint("query")
async def query(query: Query) -> Response:
    """Difficulty-route like uid_155, then run EasyPath or HardPath.

    Without a schema the selected path's staged pipeline runs to completion.
    With one, the same pipeline runs on a shortened budget and its drafted
    answer is re-expressed as `output`.
    """
    try:
        easy = await _ROUTER._is_easy(query.text or "")
    except Exception:
        easy = False
    runner = _EASY_RUN if easy else _HARD_RUN
    return await runner(query)


class EasyPath:
    """Slim staged path: fewer turns, lighter checkpoint, no restate.

    Compiles to an async run(query) that calls _plain_query with EASY_PROFILE.
    Structured-output queries still draft via the plain path, then coerce.
    """

    def _compile(self):
        # Close over EASY_PROFILE so import-time mutations cannot widen this path.
        ...
        async def run(query: Query) -> Response:
            ...
            if schema is None:
                return await _plain_query(query, budget, profile=profile)
            try:
                drafted = await _plain_query(
                    query, budget - STRUCTURED_RESERVE_SECONDS, profile=profile,
                )
            except Exception:
                drafted = Response(
                    text="The research pipeline did not produce an answer for this question.",
                )
            try:
                return await _structured_response(
                    query, schema, drafted, perf_counter() + STRUCTURED_RESERVE_SECONDS,
                )
            except Exception:
...
class HardPath:
    """Full staged path: briefing -> research -> ledger -> checkpoint -> commit -> restate.

    Compiles to an async run(query) that calls _plain_query with HARD_PROFILE
    (prior single-path behavior, including ledger restate).
    """
...
class DifficultyRouter:
    """Cheap Gemma classifier: easy -> EasyPath, anything else -> HardPath.

    Fail-closed: any router exception is treated as hard (more capable path).
    """
    _PROVIDER = "openrouter"
    _MODEL = "google/gemma-4-31b-it"
    _PROMPT = (
        "You route factual research questions to an agent by difficulty. "
        "Reply with exactly one word: easy or hard.\n\n"
        "easy — one or two lookups; a single entity/fact/date/number; "
        "few constraints; answer is short and likely findable on a "
        "normal web page.\n"
        "hard — several interlocking constraints or candidates; "
        "multi-hop research, tables, rankings, roster completeness, "
        "two distinct sub-questions, or high risk of missing coverage.\n\n"
        "If unsure, choose hard. Output only the label."
    )
    _TIMEOUT_S = 30
