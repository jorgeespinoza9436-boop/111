...
v34.1 is a STRUCTURAL pass only: no prompt byte, budget, threshold, regex or
model changed, so scoring behaviour on a healthy run is identical to v33.4/v34.0.
What changed is failure behaviour and cross-query hygiene:
  - _reset_run_state(): _SPEND is module-level and the worker outlives a query,
    so a drained budget from the PREVIOUS query forced finish_only on turn 1 of
    the next one — an answer written with zero tool calls. Reset at entry.
  - transcript integrity: `call.id` and `msg.to_input_message()` were bare reads
    that could throw out of _loop into _solve, discarding `messages` and skipping
    the audit pass. Both are now checked, and an unreplyable tool_call breaks the
    loop into the rescue ladder (which still holds the full ledger) instead.
  - tool-call shape: name/arguments are read from either the flat or the nested
    `.function` layout, so a normalization change cannot turn every tool call
    into "# unknown tool ''".
  - citations are built from the text actually SHIPPED (post-cap, post-strip),
    not the draft, so no ref is spent on an [n] the judge cannot see.
  - the entrypoint no longer reads query.text outside its own try.
The v33.4 shape work below still holds — dead parameters removed, one triplicated payload
...
VERSION = "v34.1-structure"     # metadata only, deliberately unreferenced: this
...
def _reset_run_state() -> None:
    """Clear per-QUERY module state. Called once at the top of _solve.

    v34.1 STRUCTURE — the worst defect left in v33.4. The worker process
    OUTLIVES a single query (the same fact that forced the _SEC_CACHE bound
    further down), but `_SPEND` is module-level and was never reset. So a query
    that ran the metered budget down to ~0.01 left that value in place for the
    NEXT query, which then read a stale floor BEFORE tooling_info could refresh
    it. Two silent consequences, both severe and both invisible in a single-query
    smoke test:
      * `_spend_left() < BRIEF_MIN_USD` -> the knowledge brief is skipped, so the
        loop starts with no draft and no checklist;
      * `_spend_left() <= WRAPUP_MIN_USD` -> `out_of_spend` is TRUE on TURN 1, so
        `finish_only` is set immediately, the model is handed NO tools, and the
        agent writes its final answer having done zero research. On a warm worker
        that is a near-guaranteed 0 for every query after the expensive one.
    The SEC caches are deliberately NOT cleared: they are pure, keyed by URL,
    already bounded, and the ~10MB ticker index is expensive to refetch.
    """
    _SPEND["left"] = None
...
class EvidenceLedger:
    ...
    def ref_for(self, number: int) -> CitationRef | None:
        ...
        if not receipt_id or not result_id:
            return None
        # v34.1: .get, not []. Every row is built in this file so a KeyError
        # "cannot happen" — but this runs inside the citation build, and an
        # exception there is caught by _solve and silently drops EVERY citation
        # for the response, which the judge scores as an uncited answer. A
        # failure mode that expensive does not get to rely on an invariant.
        ...
        if spans:
            ...
            return CitationRef(receipt_id=receipt_id,
                               result_id=result_id, slices=slices)
...
def _commit_tool_output(out, ledger: EvidenceLedger) -> str:
    ...
    for i, row in enumerate(out.rows):
        # v34.1: uniform .get. The mixed [] / .get access here was a latent
        # KeyError on the ONE path that must never raise — a throw inside the
        # commit leaves the placeholder \x00i\x00 embedded in the tool reply the
        # model reads, so it cites a control character instead of a number.
        n = ledger.add(row.get("receipt_id") or "", row.get("result_id") or "",
                       int(row.get("note_len") or 0), row.get("kind") or "",
                       row.get("spans"), title=row.get("title", ""),
...
def _call_id(call) -> str:
    """The tool_call_id, or '' when the object does not carry one.

    v34.1: `call.id` was read as a BARE attribute inside the fan-out. Every
    tool_call_id in an assistant turn must receive a `role: tool` reply or the
    whole transcript fails validation, so a missing id is not a small problem —
    and reading it bare turned that into an AttributeError that escaped _loop
    entirely, discarding the transcript and skipping the audit pass. Named here
    so both the pre-check in _loop and the fan-out read it the same way.
    """
...
def _call_name_args(call) -> tuple:
    """(name, raw_arguments) for either tool-call shape.

    OpenAI-compatible payloads nest the pair under `.function`; some SDK
    normalizations flatten it onto the call itself. v33.4 read only the flat
    shape, so a nested payload silently produced name='' -> "# unknown tool ''"
    for EVERY tool call, i.e. a full run with zero evidence. Both getattr names
    below are string literals (a computed name is rejected as
    `dynamic_getattr_name`); the fallback is a plain attribute read, never a
    ...
    if not isinstance(name, str) or not name:
        ...
        if fn is not None:
            alt = getattr(fn, "name", None)
            if isinstance(alt, str) and alt:
                name = alt
                raw = getattr(fn, "arguments", None)
    return (name if isinstance(name, str) else ""), raw
...
async def _run_tool(call, question: str, deadline: float) -> "ToolOutput | str":
    ...
    """
    name, raw_args = _call_name_args(call)
    try:
...
async def _tool_phase(calls, question: str, ledger: EvidenceLedger,
    ...
    if cancelled:
        # v34.1: a cancel() that is never awaited leaves the coroutine suspended
        # inside its HTTP call and its CancelledError unretrieved; the loop then
        # logs "Task exception was never retrieved" and the socket is held open
        # against the NEXT turn's fan-out. Drain briefly and bounded — we do not
        # want the result, only the teardown, so the wait is short and its own
    ...
    for call, result in zip(run_calls, results):
        ...
        replies.append({"role": "tool", "tool_call_id": _call_id(call),
...
async def _loop(question: str, brief: str, ledger: EvidenceLedger,
    ...
    for turn in range(1, turn_cap + 1):
        ...
        if not calls:
            ...
            break
        # v34.1 TRANSCRIPT INTEGRITY. Both lines below used to be able to throw
        # straight out of _loop into _solve's bare `except`, which sets
        # answer="" AND leaves `messages` as the empty list it was initialized
        # to — so the audit pass is skipped and the transcript is gone. Neither
        # failure is worth that: the ledger already holds the evidence, so
        # breaking here hands the rescue ladder a full digest instead.
        if any(_call_id(c) == "" for c in calls):
            break        # an id-less tool_call can never receive its reply, and
                         # an unanswered id invalidates the whole transcript
        try:
            assistant_turn = msg.to_input_message()
        except Exception:
            break
...
def _is_page_furniture(seg: str) -> bool:
    """True when a preview segment is navigation chrome rather than prose.

    v34.1 STRUCTURE: this was five inline checks inside _informative_lead, each
    followed by a verbatim copy of the same `if kept: break / continue` epilogue.
    Five copies of one control-flow rule is five places for it to drift, and this
    predicate decides what the zero-LLM rescue rung SUBMITS — the rung that
    already shipped a raw nav scrape once. The tests are unchanged and still
    evaluated in the same order; each is pure, so short-circuiting is
    behaviourally identical to the sequential ifs it replaces.
    """
    ...
    # "Share buybacks totalled…"), and chrome ending in a period slipped through
    # the old punctuation exemption. Real evidence sentences almost always carry
    # a figure, date or year; navigation almost never does. Use that instead.
    if _FURNITURE_RE.match(seg) is not None and not re.search(r"\d", seg):
        return True
    if seg.startswith(("*", "|", "↑", "#")):
        return True
    # A markdown link matches BOTH halves of the pattern; count it once.
    links = len(_MD_LINK_RE.findall(seg)) + len(_BARE_URL_RE.findall(seg))
    return bool(links) and links * 110 >= len(seg)     # link-dense == chrome
...
async def query(query: Query) -> Response:
    # v34.1: `query.text` was read BEFORE the try. That is the one statement in
    # the file with nothing behind it — a miner-attributed exception here is a
    # hard 0 with no fallback text at all, and it is exactly the kind of shape
    # assumption the rest of the file already refuses to make about payloads.
    # (String literal name: a computed one is rejected as `dynamic_getattr_name`.)
    question = ""
    try:
        raw_text = getattr(query, "text", None)
        question = (raw_text or "").strip() if isinstance(raw_text, str) else ""
        if not question:
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # v34.1: FIRST statement of the run. See _reset_run_state — module state that
    # survives the query is the difference between a researched answer and a
    # zero-tool one on a warm worker.
    _reset_run_state()
    # Read the schema ONCE, defensively, here rather than as a bare attribute at
    # the bottom: at that point we already hold a finished answer, and losing it
    # to an AttributeError on the output branch is the most expensive throw in
    # the file.
    ...
    shipped = _cap(answer)
    text = shipped or f"Best-effort answer unavailable for: {question[:400]}"

    # v34.1 ORDERING FIX: citations are now built from the text we actually SHIP,
    # not from the pre-normalization, pre-cap draft. Two real leaks closed:
    #   * _cap truncates at ANSWER_CHAR_CAP, so refs were minted for [n] markers
    #     that no longer appear in the submitted answer. Those refs still charge
    #     EVIDENCE_CHAR_BUDGET and still count against CITATION_CAP, so they
    #     crowded out refs for citations the judge can actually see.
    #   * _strip_lead_narration can drop a leading sentence; it never drops a
    #     CITED one, but building from the post-strip text makes that guarantee
    #     structural instead of something the two functions have to agree on.
    try:
        citations = _citations_for(shipped or answer, ledger)
    except Exception:
        citations = []
    ...
    if output_schema is not None:
        structured = None
        try:
            structured = await _schema_output(question, answer, output_schema, deadline)
        except Exception:
            structured = None
        ...
        if not basis or _STUB_ANSWER_RE.match(basis.strip()):
            basis = question[:400]
        try:
            forced = _coerce_to_schema(_cap(basis), output_schema)
...
# slot: harnyx 2026-07-30T14:02:22+00:00
