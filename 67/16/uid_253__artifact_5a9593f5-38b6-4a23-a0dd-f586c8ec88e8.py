...
CITATION_CAP = 24
DIGEST_CHAR_CAP = 60000      # clean-digest budget for the rescue write
RESCUE_CLAIM_LINES = 6       # was an inline literal 6 in the two rescue renderers
RESCUE_LASTDITCH_LINES = 4   # raw-preview rung, was an inline literal 4
...
def _spend_reset() -> None:
    """Clear the spend reading at the START of every query.

    STRUCTURE (this pass): _SPEND is MODULE-level and the worker process
    outlives a single query — the same lifetime problem the v33.4 note at
    _SEC_CACHE describes, but with teeth, because this value gates behaviour
    rather than just holding memory. A query that ended with the metered budget
    near zero left `left` at that value, and the NEXT query read it before
    making a single call of its own: _knowledge_brief skipped (BRIEF_MIN_USD),
    _audit_patch skipped (AUDIT_MIN_USD), and _loop went finish_only on turn one
    (WRAPUP_MIN_USD) — a fresh question degraded to a no-research single turn on
    inherited state. Unknown-until-measured is the only correct start, and
    _spend_left already treats None as 'assume budget'.
    """
    _SPEND["left"] = None
...
# (label, url_patterns, trigger_regexes). Trigger phrases are matched with
# word-boundary regexes so "census data" in a Wikipedia table name does not
# match the Census Bureau spec — that false positive caused the source_fidelity
# loss on task 62b1353b.
#
# STRUCTURE (this pass): the table was a local rebuilt inside
# _extract_source_specs on every call, and the function carried a `seen` set
# guarding against duplicate labels that the table itself makes impossible —
# each label appears exactly once. A lookup table is module data; keeping it in
# the function body hid it from every reader looking for "which sources do we
# recognize" and put a dead dedup guard in the hot path to compensate.
_SPEC_TABLE: list[tuple[str, list[str], list[str]]] = [
    ("Wikipedia", ["wikipedia.org"], [r"\bwikipedia\b"]),
    ("SIPRI", ["sipri.org"], [r"\bsipri\b"]),
    ("Census Bureau", ["census.gov"], [r"\bcensus bureau\b", r"\bcensus\.gov\b"]),
    ("BLS", ["bls.gov"], [r"\bbls\b", r"\bbureau of labor statistics\b"]),
    ("NFL.com", ["nfl.com/stats"], [r"\bnfl\.com\b", r"\bnfl player .* leaderboard"]),
    ("Box Office Mojo", ["boxofficemojo.com"], [r"\bbox office mojo\b"]),
    ("USGS", ["usgs.gov", "earthquake.usgs.gov"], [r"\busgs\b"]),
    ("NASA", ["nasa.gov"], [r"\bnasa\b"]),
    ("NOAA", ["noaa.gov"], [r"\bnoaa\b"]),
    ("WHO", ["who.int"],
     [r"\bworld health organization\b", r"\bwho\b.*\b(?:report|database)\b"]),
    ("IMF", ["imf.org"], [r"\bimf\b", r"\binternational monetary fund\b"]),
    ("World Bank", ["worldbank.org"], [r"\bworld bank\b"]),
    ("Gallup", ["gallup.com", "news.gallup.com"], [r"\bgallup\b"]),
    ("OECD", ["oecd.org"], [r"\boecd\b"]),
]
# Compiled once at import instead of re-resolved from the regex cache per call.
_SPEC_COMPILED: list[tuple[str, list[str], list]] = [
    (label, patterns, [re.compile(t) for t in triggers])
    for label, patterns, triggers in _SPEC_TABLE
]
...
def _extract_source_specs(question: str) -> list["SourceSpec"]:
    ...
    Uses word-boundary phrase matching (not substring) to avoid false positives:
    e.g. 'census data' in a Wikipedia table name must not trigger the Census
    Bureau spec — that caused the source_fidelity loss on task 62b1353b."""
...
_VESSEL_NAME_KEY_RE = re.compile(r"ship|name|vessel", re.I)
...
def _strip_vessel_prefix(value, question: str):
    ...
    STRUCTURE (this pass): this handled `str` and `dict` and returned everything
    else untouched — so a LIST fell straight through. That is the shape this fix
    most needed to cover: 'name the ships…' produces an array schema, and both
    _schema_output and _coerce_to_schema hand back a list. The label_alignment
    fix was therefore inert for the commonest ship-question output while looking
    live in the code. Now recurses through lists and nested containers under a
    depth bound; scalars and str/dict behaviour are unchanged.
    """
    ql = (question or "").lower()
...
def _strip_vessel_value(value, depth: int):
    """Container walk for _strip_vessel_prefix; the question test already passed."""
    if depth > 4:
        return value
    if isinstance(value, str):
        return _VESSEL_PREFIX_RE.sub("", value).strip()
    if isinstance(value, list):
        # A bare list of ship names: every string element is a name field.
        ...
        for k, v in value.items():
            if isinstance(v, str):
                out[k] = (_VESSEL_PREFIX_RE.sub("", v).strip()
                          if _VESSEL_NAME_KEY_RE.search(str(k)) else v)
...
# ── evidence-preview prose extraction ───────────────────────────────────────
# STRUCTURE (this pass): hoisted above ClaimLedger. _informative_lead is called
# by ClaimLedger._bind_claim on every evidence commit, and _CITE_MARK_RE is read
# by _preseed / _rider_prefetch / _search_uncovered / _tool_phase — all of which
# sat HUNDREDS of lines above the definitions and worked only by late binding.
# Late binding is not a guarantee, it is silence: defined before first use, a
# typo or a deleted constant is an import-time NameError instead of an
# except-swallowed failure on the rescue path, where nobody sees it.
...
def _is_prose_segment(seg: str) -> bool:
    """True when one split segment of a page preview reads as evidence prose
    rather than navigation chrome.

    STRUCTURE (this pass): the five rejection tests below were five copies of
    the SAME three-line `if kept: break / continue` tail inside
    _informative_lead. Five copies of one decision is five places to edit and
    four places to get it wrong — and the block had already drifted, carrying
    two near-duplicate comments about furniture words starting real sentences.
    The tests, their order, and their outcomes are unchanged; only the shared
    tail moved to the single call site.
    """
    if len(seg) < 30 or len(seg) > 400:
        return False
    # Nav items are labels, not sentences: require something sentence-shaped.
    if _SENTENCEY_RE.search(seg) is None:
        return False
    # Furniture words also START real sentences ("Home Depot reported…",
    # "Share buybacks totalled…"), so a furniture opener alone is not enough.
    # Chrome ending in a period slipped through the old punctuation exemption.
    # Real evidence sentences almost always carry a figure, date or year;
    # navigation almost never does. Use that instead.
...
class ClaimLedger:
    ...
    def __init__(self, question: str) -> None:
        ...
        # STRUCTURE (this pass): keyed by the evidence NUMBER itself, not by a
        # stringified "E{n}". The [n] index is the only identity an evidence row
        # has — every other structure in this file (rows, ref_for, _cited_numbers,
        # the replay cache) speaks in that integer, and a parallel string keyspace
        # is one formatting slip away from a silent miss in structured_note_for.
    ...
    def render_rescue(self) -> str:
        ...
        """
        # Prefer source-compliant claims with informative leads; fall back to
        # every led claim. Both passes walk self.claims in insertion order, i.e.
        # ledger order, so the rendering is run-invariant like the [n] numbering.
        ...
        for claim in pool[:RESCUE_CLAIM_LINES]:
            ...
            # A fetch row's title IS its url (see _do_fetch), and a bare url
            # prefixing a SUBMITTED sentence is exactly the page-furniture leak
            # this rung exists to prevent. Prefix only with a real title.
...
async def _tool_phase(calls, question: str, ledger: ClaimLedger,
    ...
    for i, call in enumerate(run_calls):
        ...
        # STRUCTURE (this pass): `call.id` was a BARE attribute access, and this
        # is the same defect class the v33.4 note at _first_message describes —
        # one malformed tool_call raised AttributeError out of _tool_phase,
        # out of _loop, and into _solve's except, discarding the entire
        # transcript for a recoverable shape. A call with no id cannot be
        # replied to at all, so skip that one and keep the turn.
        call_id = getattr(call, "id", None)
        if isinstance(call_id, str) and call_id:
            replies.append({"role": "tool", "tool_call_id": call_id,
                            "content": content})
...
async def _loop(question: str, brief: str, ledger: ClaimLedger,
    ...
    for turn in range(1, turn_cap + 1):
        ...
        if not calls:
            ...
            break
        # STRUCTURE (this pass): `msg.to_input_message()` was a bare method call
        # on a provider object, so a malformed message aborted the entire
        # research loop into the rescue ladder — the same recoverable-shape
        # failure the v33.4 _first_message note describes. Contain it: stop
        # looping, but hand back the transcript and evidence gathered so far.
        try:
            assistant_msg = msg.to_input_message()
        except Exception:
            break
        replies = await _tool_phase(calls, question, ledger, deadline)
        # TRANSCRIPT INTEGRITY: every tool_call id owes exactly one reply. If the
        # counts disagree, appending anyway ships a transcript the validator
        # rejects outright; stopping here keeps the evidence and lets the rescue
        # ladder write from the ledger instead.
...
async def _audit_patch(question: str, answer: str, messages: list[dict],
    ...
    # uid201's guard + the v36 M7 citation-count rider, now in one place.
...
def _accept_rewrite(candidate: str, prior: str, ledger: ClaimLedger) -> bool:
    """Should a corrective rewrite replace the answer it was derived from?

    STRUCTURE (this pass): _audit_patch and _numeric_predicate_guard each
    carried a byte-identical copy of this three-part test, and the second copy's
    own comment ("the M7 rule, applied here") admits the duplication. Two copies
    of an acceptance policy is two policies the moment one is tuned — and this
    one gates whether a rewrite SHIPS, so a drift between them is invisible
    until it costs a score. One definition, both call sites.

    The three parts, unchanged: (1) the answer floor, (2) uid201's guard that a
    "repair" which collapsed the answer is a regression, (3) the v36 M7 rider
    that a rewrite dropping citations is a regression too.
    """
    if not _is_usable_answer(candidate):
        return False
...
def _ledger_digest(ledger: ClaimLedger, char_cap: int = DIGEST_CHAR_CAP) -> str:
    ...
    for i, row in enumerate(ledger.rows, start=1):
        ...
        # STRUCTURE: was behind hasattr(); only ClaimLedger is ever built, so the
        # probe could only ever hide a rename — silently dropping every
        # 'Supports:' note from the digest with no error anywhere.
...
def _deterministic_answer(ledger: ClaimLedger) -> str:
    """Last rung, no LLM: render verified claims with their citations.

    Post-mortem: delegates to ClaimLedger.render_rescue() rather than dumping
    raw previews — the snippet_dump fix (tasks 3818d8c9, fd066a4c). The old
    'Best-supported findings from the sources retrieved:' header violated
    LOOP_RULES' own 'no preamble' discipline and was the direct cause of garbage
    JSON fields in structured output.

    STRUCTURE (this pass): the `hasattr(ledger, "render_rescue")` probe and the
    per-row _informative_lead loop behind it are gone. Only ClaimLedger is ever
    constructed (see _solve), so the probe tested nothing, and the loop it
    guarded was provably dead: render_rescue returns '' only when NO claim has a
    lead, and each claim's lead is _informative_lead of that same row's preview
    — so the loop re-derived the identical empty result and fell through every
    time. What survives is the one branch that was actually reachable: the raw
    last-ditch below.
    """
    rescued = ledger.render_rescue()
    if rescued:
        return rescued
    # LAST DITCH. Reached only when no row yields a sentence of prose. This is
    # the snippet dump the post-mortem indicts, kept deliberately because the
    # alternative here is '' — a hard zero — not a better answer.
    out: list[str] = []
    for i, row in enumerate(ledger.rows, start=1):
        if len(out) >= RESCUE_LASTDITCH_LINES:
            break
        # Strip the SOURCE's own footnote markers: "…in 1801[3]…" echoed into
        # our answer is read back by _cited_numbers as OUR evidence index 3 and
        # mints a CitationRef to an unrelated row (see _SRC_FOOTNOTE_RE).
...
async def _numeric_predicate_guard(question: str, answer: str,
    ...
    # Audit-grade regression guards, shared with _audit_patch: usable, no
    # collapse in length, no drop in distinct citations.
...
async def _solve(query: Query, question: str) -> Response:
    ...
    _spend_reset()          # per-query state: never inherit the last query's floor
    try:
