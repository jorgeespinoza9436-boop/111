...
def _compose_cobalt_talon_agent_entry():
...
_cobalt_talon_agent_query_entry = _compose_cobalt_talon_agent_entry()
...
def _compose_ember_slate_agent_entry():
    ...
    async def query(query: Query) -> Response:
        ...
        # Call the original query entrypoint
        try:
        ...
        # 1. Handle fast queries (fast = True)
        fast_val = False
        try:
        ...
        if fast_val:
            if text is not None and output is None:
                try:
                    ...
                    opt_prompt = (
                        "You are a concise answer optimizer.\n"
                    ...
                    opt_res = await llm_chat(
                        provider="openrouter",
                        model="deepseek/deepseek-v4-flash",
                        messages=[
                            {"role": "system", "content": "Return ONLY the optimized concise text. No conversational introduction."},
            ...
            if not is_valid or output is None:
                try:
                    ...
                    repair_prompt = (
                        ...
                        f"EXISTING EVIDENCE NOTE:\n{note or ''}"
                    )
                    repair_res = await llm_chat(
                        provider="openrouter",
                        ...
                        messages=[
                            {
                                "role": "system",
                                "content": (
                                    "Repair the candidate output into a valid JSON object that validates perfectly against the supplied JSON Schema.\n"
                                    "Use only facts present in the candidate output or evidence note.\n"
                                    "Return JSON only as {\"answer\": <repaired value>}."
                                )
                            },
                    ...
                    if repair_text:
                        ...
                        if isinstance(parsed, dict) and "answer" in parsed:
                            ...
                            try:
                                ...
                                safe_exec(
                                    "import jsonschema\njsonschema.validate(instance=instance, schema=schema)\nresult = True",
                                    {"instance": repaired_val, "schema": schema}
                                )
        ...
        # 3. Handle normal prose queries (fast=False, output_schema=None)
        if text is not None and output is None:
            try:
                polish_prompt = (
                    "You are an expert editor. Polish the candidate's draft answer to improve clarity, precision, and logical flow.\n"
                ...
                polish_res = await llm_chat(
                    provider="openrouter",
                    model="deepseek/deepseek-v3.2",
                    messages=[
                        {"role": "system", "content": "Return ONLY the polished markdown text. Do not include introductory conversational text."},
...
class CobaltTalonAgent:
    async def __call__(self, query: Query) -> Response:
        return await _cobalt_talon_agent_query_entry(query)
...
_TASK_SHAPE_PRIMARY_AGENT = CobaltTalonAgent()
_TASK_SHAPE_SECONDARY_AGENT = VesperLanternAgent()
_TASK_SHAPE_TERTIARY_AGENT = EmberSlateAgent()
