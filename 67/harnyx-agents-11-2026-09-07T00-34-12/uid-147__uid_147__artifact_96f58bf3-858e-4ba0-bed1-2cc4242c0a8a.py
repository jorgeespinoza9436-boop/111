...
# fork of 2_uid_212_score_0.700.py: added k2  (full mechanism set: DRV+K2)
# variant drv11_official_independent_board_or_shape: dual-corpus reconciler wrap (official + independent corpus; trigger = board deficiency OR synthesis/set question shape) around the router.

from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import Query, Response


def _compose_vesper_harbor_agent_entry():
    ...
    WALL_BUDGET_S = 178.0
    TASK_TOTAL_BUDGET_SECONDS = 178.0
    TURN_TIMEOUT_S = 75.0
    ...
    VERSION = "v52-pin-reviewed+drv11_official_independent_board_or_shape"

    # ── providers / models ────────────────────────────────────────────────────────
    LLM_LANE_A = "openrouter"          # primary lane (loop + briefing)
    LLM_LANE_B = "openrouter"          # fallback lane (paid key; fast + uncongested)
    ...
    # openrouter (tool_models.py), so this is a genuinely single-variable change.
    ...
    async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool,
        ...
        # v33.2 COST: lane B (openrouter glm-5.2-fast) is the priciest model on the
    ...
    _W2_DEFAULT_BUDGET_SECONDS = 178.0
...
_vesper_harbor_agent_query_entry = _compose_vesper_harbor_agent_entry()
...
def _compose_cedar_harbor_agent_entry():
    ...
    VERSION = "c3-417+drv11_official_independent_board_or_shape"
...
def _compose_saffron_lantern_agent_entry():
    'agent_d — v32 "toolloop": model-driven research agent.\n\nREDESIGN RATIONALE (batch 88c4a837: our pipeline 0.000, the field\'s tool-loop\nfamily 0.70-0.80). The scoring architecture is a native agentic loop: the LLM\nitself drives search/fetch via tool calls, reads full results in context,\ncross-references candidate-by-candidate, and writes one cited answer. Our old\nstaged pipeline (search -> gate -> chunk -> synth) funnels evidence through\nabstractions that lose cross-referencing, never uses model knowledge, and\ncannot iterate multi-hop. This file is our OWN implementation of the loop\narchitecture, keeping the assets our line already validated:\n  - the v31.8 answer-shape discipline (asked-KIND, set-intersection\n    completeness, numeric verbatim, world-negative vs evidence-concession);\n  - a miniaturized section-localizer: big fetched pages are rendered as the\n    HEAD plus the TOP-K densest regions (so a filing\'s deep section, or an\n    answer set spread across two distant tables, is readable in one call);\n  - SEC EDGAR primary-doc routing as a loop hint;\n  - dual-provider LLM lanes (openrouter primary, our paid openrouter fallback).\nKill-safety: everything bounded by one deadline; force-commit well before it.\n'
...
_saffron_lantern_agent_query_entry = _compose_saffron_lantern_agent_entry()
...
class VesperHarborAgent:
    async def __call__(self, query: Query) -> Response:
        return await _vesper_harbor_agent_query_entry(query)
...
class SaffronLanternAgent:
    async def __call__(self, query: Query) -> Response:
        return await _saffron_lantern_agent_query_entry(query)


_TASK_SHAPE_PRIMARY_AGENT = VesperHarborAgent()
_TASK_SHAPE_SECONDARY_AGENT = CedarHarborAgent()
_TASK_SHAPE_TERTIARY_AGENT = SaffronLanternAgent()
...
# --- drv wrap: official_independent dual-corpus reconciler (start) ---
# variant drv11_official_independent_board_or_shape: official + independent corpus; trigger = board deficiency OR synthesis/set question shape
...
_DRV_TAG = 'drv11_official_independent_board_or_shape'
_DRV_SALT = '246be452ea53'
...
def _drv_cap_budget(current, ceiling=178.0):
...
async def _drv_audit_board(question: str, draft: str, schema, citations: list) -> _DrvBoard:
    ...
    system = (
        "You audit a research draft against a user question. Focus first on official/primary vs independent/contemporaneous agreement. "
...
async def _drv_retrieve_dual_corpus(question: str, claims: list[str]) -> list[dict]:
    """official + independent corpus: two corpus-specific searches plus one fetch of the first corpus."""
    ...
    fetch_url = ""
    if first_row:
        first_row["corpus"] = "official_primary"
        fetch_url = first_row.get("url") or ""
        rows.append(first_row)
    if second_row:
        second_row["corpus"] = "independent_contemporaneous"
...
async def _drv_regenerate(
    ...
    if is_text:
        system = (
            "Rewrite the research answer after a second retrieval pass over official/primary and independent/contemporaneous sources. Return JSON only with keys "
        ...
        system = (
            "Rewrite the structured research answer after a second retrieval pass over official/primary and independent/contemporaneous sources. Return JSON only "
...
async def _k2_base_query(query: _DrvQuery) -> _DrvResponse:
    ...
    try:
        ...
        return draft
# --- drv wrap: official_independent dual-corpus reconciler (end) ---

# ---------------------------------------------------------------------------
# K2 claim-ledger cycle  [variant u212-k2]
#
...
_K2_SEARCH_TIMEOUT_S = 8.0
_K2_LLM_TIMEOUT_S = 12.0
_K2_MAX_DEFICIENT = 1
_K2_MAX_NEW_CITES = 6
