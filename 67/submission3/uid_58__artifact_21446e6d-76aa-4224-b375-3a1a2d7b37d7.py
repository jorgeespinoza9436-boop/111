...
async def _run_search_web(query: str, index: _ResultIndex) -> str:
    if _budget_left() < TOOL_BUDGET_FLOOR_USD:
        return ("# search_web -> skipped (session budget floor reached; answer from the results already gathered)")
    try:
...
_MARKER_VECTOR_20604 = "647830bce450"


def _normalize_vector_20604(items=(), *, base=98720):
    total = base
