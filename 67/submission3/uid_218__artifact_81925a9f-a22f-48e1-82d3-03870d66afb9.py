...
# MECHANISM_UPGRADE_V2: parallel multi-query entity-gap retrieval (claim-driven fan-out per missing entity)
...
FETCH_PAGE_TOOL_TIMEOUT_SECONDS           = 14.45
...
GLM5_MODEL = "zai/glm-5.2-fast"  # gw: ai_gateway glm (OR z-ai/glm-5 not served on gateway)
# Hosted-tool provider routing (required kwarg as of the provider-split SDK).
_LLM_PROVIDER = "ai_gateway"  # gw: retrieval gemma -> Cerebras (fast, uncongested); synth -> zai fast
_GEMMA_PIN = {"provider": {"only": ["cerebras"]}}  # pin gemma upstream: 1.1s/call vs 24-31s rotating
...
async def _answer_question(question: str, *, state: ResearchRunState, deadline: float) -> Response:
    ...
    if enumerated_entities:
        ...
        if missing_entities:
            ...
            if gap_gate.accepted_packets:
                ...
                coverage_state = _fallback_coverage_state(contract=research_contract, observations=accepted_observations)
                coverage = _coverage_from_coverage_state(coverage_state, accepted_observations)
    if coverage_state.missing_role_ids or coverage_state.weak_role_ids:
        role_gap = await _augment_missing_role_evidence(
            question=question, contract=research_contract, coverage_state=coverage_state,
            role_ledger=role_ledger, existing_packets=accepted_packets,
            existing_observations=accepted_observations,
            tried_query_identities=_tried_query_identities_from_routes(search_beam.routes),
            accumulated_results=search_beam.results,
            seen_url_keys=_seen_url_keys_from_results(search_beam.results),
    ...
    accepted_packets = _stier_sort_packets(accepted_packets)  # throne-lever 2: official sources first
    ...
    # verify-and-complete: if the draft asserts an entity but a criterion value lacks a cited
    # figure, fetch that (entity x criterion) value and re-synthesize once (budget-guarded)
    if deadline - perf_counter() > CRITERION_GAP_MIN_REMAINING_SECONDS:
        _draft_text = _safe_response_text(final_answer)
        if _draft_text:
            _crit_gaps = await _detect_missing_criterion_values(
                question=question, draft_answer=_draft_text, state=state,
            )
            if _crit_gaps:
                _crit_gate = await _augment_missing_criterion_values(
                    question=question, gaps=_crit_gaps, contract=research_contract,
                    role_ledger=role_ledger, existing_packets=accepted_packets,
                    existing_observations=accepted_observations, state=state, deadline=deadline,
                )
                if _crit_gate.accepted_packets:
                    accepted_packets = _stier_sort_packets((*accepted_packets, *_crit_gate.accepted_packets))
                    accepted_observations = (*accepted_observations, *_crit_gate.observations)
                    coverage_state = _fallback_coverage_state(contract=research_contract, observations=accepted_observations)
                    coverage = _coverage_from_coverage_state(coverage_state, accepted_observations)
                    final_answer = await _synthesize_final_answer(
                        question=question, accepted_packets=accepted_packets,
...
async def _run_lite_search_beam(*, question: str, state: ResearchRunState, deadline: float) -> LiteSearchBeam:
    ...
    for round_index in range(MAX_LITE_SEARCH_ROUNDS):
        ...
        if accumulated and remaining_seconds < MIN_LITE_SEARCH_ROUND_REMAINING_SECONDS:
            ...
            break
        # Deadline check: leave 60s for gate + synthesis
...
# ===== throne-lever 2: _stier source-tier prioritization (from uid223 throne code) =====
_STIER_TIER_A_PATTERNS = (
    ".sec.gov", ".fda.gov", ".cdc.gov", ".nih.gov", ".usgs.gov", ".bls.gov",
    ".census.gov", ".treasury.gov", ".state.gov", ".whitehouse.gov", "uspto.gov",
    ".epa.gov", "energy.gov", ".nasa.gov", "supremecourt.gov", "justice.gov",
    ".europa.eu", ".un.org", ".int", "who.int", "imf.org", "worldbank.org",
    "oecd.org", "wto.org", "icao.int",
    ".gov.uk", ".gc.ca", ".gov.au", ".bund.de", ".gov.in", "ec.europa.eu",
    "kpu.go.id", "nature.com/articles/", "science.org/doi/",
)
_STIER_TIER_B_HINTS = (
    "/press-release/", "/press-releases/", "/official-statement",
    "10-k", "10-q", "8-k", "annual-report", "financial-statements",
    "proxy-statement", "prospectus", "/investor-relations/",
    "court.gov", "/judgment/", "/ruling/",
)
_STIER_TIER_C_PATTERNS = ("wikipedia.org/wiki/", ".edu/", "britannica.com", "stanford.edu", "harvard.edu", "mit.edu")
_STIER_TIER_D_PATTERNS = (
    "reuters.com", "ap.org", "apnews.com", "bbc.co.uk", "bbc.com",
    "nytimes.com", "washingtonpost.com", "wsj.com", "ft.com",
    "bloomberg.com", "economist.com", "npr.org",
)

def _stier_score_url(url):
    if not url:
        return 0
    ul = url.lower()
...
def _stier_sort_packets(packets):
    """Sort accepted evidence by source authority (official/gov first) so synth leads with the best source."""
    if not packets:
        return packets
    indexed = [(i, _stier_score_url(p.url), p) for i, p in enumerate(packets)]
    indexed.sort(key=lambda x: (-x[1], x[0]))
    return tuple(p for _, _, p in indexed)


# ===== throne-lever 1: _v4 anti-abstention forced-commit retry (from uid223 throne code) =====
_V4_ABSTENTION_MARKERS = (
    "could not produce", "cannot answer", "cannot determine", "unable to answer",
    "evidence is missing", "evidence was missing", "no evidence", "not in the accepted",
    "not in the available", "insufficient evidence", "no source-backed", "would be unsupported",
    "not found in the", "does not include", "is not available", "are not available",
    "no filmmaker meets", "no result", "none of the", "cannot be determined",
    "not present in the evidence", "not contained in", "unable to determine",
    "i do not have", "i don't have", "no matching", "cannot be answered",
    "does not contain", "no studio album", "no feature films",
)

def _v4_looks_like_abstention(text):
    """Detect whether the synthesized answer is an abstention/refusal rather than a committed answer."""
    if not text:
        return True
    low = text.lower()
    lead = low[:400]
...
def _v4_force_commit_directive():
    """Extra system directive appended on the forced-commit retry."""
    return (
        "\n\n★ FORCED COMMIT (retry) ★\n"
        "Your previous answer refused or said evidence was missing. That scores ZERO in pairwise "
        "evaluation and LOSES to any competitor who commits. REWRITE now: give your single best "
        "grounded answer to EVERY sub-question, using the strongest available evidence and reasonable "
        "inference from the chain. Do NOT say 'cannot answer', 'evidence missing', or 'no match'. "
        "Lead with a committed answer. If genuinely uncertain, give the most-likely answer and mark it "
        "as your best estimate — but ALWAYS provide one."
    )
    ...
    *, question: str, accepted_packets: tuple[AcceptedEvidence, ...],
    ...
    try:
        response = await llm_chat(
            ...
            thinking=FINAL_SYNTHESIS_THINKING, timeout=FINAL_SYNTHESIS_LLM_TOOL_TIMEOUT_SECONDS,
        )
        text = _assistant_text(response)
        # throne-lever 1: if the answer refused/hedged, force a committed retry (synth is fast on gateway)
        if _v4_looks_like_abstention(text):
            _v4_messages = [dict(m) for m in messages]
            for _m in _v4_messages:
                if _m.get("role") == "system":
                    _m["content"] = _m["content"] + _v4_force_commit_directive()
                    break
            _v4_resp = await llm_chat(
                provider=_LLM_PROVIDER,
                messages=_v4_messages, model=FINAL_SYNTHESIS_MODEL, temperature=SYNTHESIS_TEMPERATURE,
                thinking=FINAL_SYNTHESIS_THINKING, timeout=FINAL_SYNTHESIS_LLM_TOOL_TIMEOUT_SECONDS,
            )
            _v4_text = _assistant_text(_v4_resp)
            if _v4_text and not _v4_looks_like_abstention(_v4_text):
                text = _v4_text
        return text if text else _deterministic_answer_from_evidence(accepted_packets)
    except Exception:
...
async def _call_json_llm_with_retry(
    *, messages: list[dict[str, str]], model: str, temperature: float,
    ...
    for attempt_index in range(max_attempts):
        try:
            response = await llm_chat(provider=_LLM_PROVIDER, messages=active_messages, model=model, temperature=temperature, thinking=thinking, timeout=JSON_LLM_TOOL_TIMEOUT_SECONDS, provider_extra=_GEMMA_PIN)
        except Exception:
            # pinned upstream rejected (fast 503) -> fall back to the gateway's own rotation
            try:
...
def _build_chunk_cue_pattern_messages(
    *, question: str, loop_index: int, role_ledger: tuple[ResearchPlanRole, ...],
    ...
    system_content = (
        ...
        "CRITERION COVERAGE (mandatory): First enumerate every distinct criterion, metric, filter, and comparison baseline the question states (a question with N criteria has N coverage duties — a second metric, a national average, a separate table, a lower threshold for a category are EACH one). Emit at least one pattern per criterion, assigned to the closest role_id, that would match the DATA ROW or STATED VALUE for that criterion (entity-name adjacent to a number, table-row shapes, the metric term near a percent or figure) — not just the criterion mentioned in prose. A pattern set that covers only the headline criterion and skips a second metric or baseline is WRONG even if within budget.\n\n"
...
def _build_chunk_lexical_anchor_messages(
    *, question: str, loop_index: int, role_ledger: tuple[ResearchPlanRole, ...],
    ...
    system_content = (
        ...
        "CRITERION COVERAGE (mandatory): enumerate every distinct criterion/metric/filter/baseline in the question and emit at least one anchor set per criterion (closest role_id) targeting where that criterion's VALUE lives (metric term + unit/percent neighborhoods, table headers, the comparison baseline term). Never cover only the headline criterion.\n\n"
...
def _build_final_answer_messages(
    *, question: str, accepted_packets: tuple[AcceptedEvidence, ...],
    ...
    system_content = (
        ...
        "ENUMERATION-EXHAUSTIVENESS RULE: For which/list/superlative questions asking for all X that "
        "meet stated criteria: (1) mine ALL accepted evidence (including source_result_text context) "
        "for EVERY candidate, not just the first qualifying one — if the evidence names or lists other "
        "members of the candidate pool, evaluate each against the criteria and include every one that "
        "qualifies, each with its own citation per criterion. (2) Explicitly note prominent near-miss "
        "candidates that were excluded and the specific criterion they fail (e.g. inducted only as a "
        "band member, not solo). (3) NEVER write 'the only', 'the sole', or 'X is the only identifiable' "
        "unless the evidence shows the full candidate roster was enumerated and checked — an answer "
        "naming one candidate from an unchecked pool loses to an answer enumerating the complete "
        "qualifying set. If the evidence genuinely covers only part of the pool, still commit: give "
        "every qualifying candidate found, without claiming exclusivity.\n\n"
        "COMPLETE-EVERY-PART RULE: When a question states multiple criteria, filters, or sub-questions, the answer must resolve EVERY part with a definite value, name, or figure and its own citation — the final answer is judged on completing ALL parts, and an answer that resolves the headline but leaves one criterion as 'not confirmed in the evidence' scores the same as no answer at all. NEVER commit part of a multi-part answer while declaring another part missing/unverifiable. If the exact figure for a sub-part is not verbatim in the accepted evidence: (1) re-examine ALL evidence including source_result_text context and partial tables for the value or a derivable equivalent (a total, a complement, an adjacent row, the national line of the same table); (2) if a supportable value exists, state it plainly with its citation; (3) only as a last resort give the best-supported estimate labeled as such — a committed, cited best-estimate for every part beats a precise answer for some parts with declared gaps in the rest. CITED-VALUE LEASH: every specific figure you state must appear in an accepted citation — NEVER supply a number no citation contains (an invented or outside-knowledge figure scores worse than none); when no citation carries the exact value, give the citation-supported qualitative finding (which side of the threshold, the bracket, the rank) and cite that.\n\n"
        ...
        "SET-COMPLETENESS RULE: When the question asks for a SET ('which X qualify', 'list all Y', 'name every Z that meets the criteria'), evaluate EVERY candidate present in the evidence against the criteria and include EVERY member that satisfies them. Never report a partial set. If your evidence ranks or lists candidates, walk the full list and check each one — a qualifying member you already gathered but omit from the final answer scores the same as missing it entirely. State the complete set explicitly (e.g. 'X and Y', not just 'X'), and briefly show why each included member qualifies.\n\n"
...
def _deterministic_answer_from_evidence(accepted_packets: tuple[AcceptedEvidence, ...]) -> str:
    if not accepted_packets:
        return "The final synthesis call failed, and no accepted evidence was available."
    points: list[str] = []
    for i, packet in enumerate(accepted_packets[:2], start=1):
        source_text = packet.source_text.strip() or packet.source_result_text.strip()
        excerpt = re.sub(r"\s+", " ", source_text).strip()
        if len(excerpt) > 260:
            excerpt = f"{excerpt[:257].rstrip()}..."
        if excerpt:
            points.append(f"[{i}] {excerpt}")
    if not points:
        return "The final synthesis call failed, and the accepted evidence did not contain usable text."
    return f"The accepted evidence supports only these source-backed points: {' '.join(points)} The exact answer may be incomplete because the final synthesis call failed."
...
# ===== gap-augmentation capability (transplanted from uid176; entity+role targeted re-retrieval) =====
ENTITY_GAP_MIN_ENTITIES              = 3
ENTITY_GAP_MIN_REMAINING_SECONDS     = 75.0
CRITERION_GAP_MIN_REMAINING_SECONDS = 75.0  # run verify-and-complete only with real budget left
CRITERION_GAP_GATE_MIN_REMAINING_SECONDS = 40.0
CRITERION_GAP_MAX_QUERIES = 4
CRITERION_GAP_SEARCH_RESULTS_PER_QUERY = 5
CRITERION_ROW_RESCUE_MAX_URLS = 2
CRITERION_ROW_RESCUE_WINDOW_CHARS = 700
CRITERION_ROW_RESCUE_MAX_WINDOWS_PER_GAP = 2

ENTITY_GAP_GATE_MIN_REMAINING_SECONDS = 30.0
ENTITY_GAP_MAX_ENTITIES_TO_AUGMENT   = 3
ENTITY_GAP_SEARCH_RESULTS_PER_ENTITY = 5
ENTITY_GAP_METRIC_HINT_MAX_TOKENS    = 8
ROLE_GAP_MAX_ROLES_TO_AUGMENT        = 3
ROLE_GAP_MIN_REMAINING_SECONDS       = 75.0
ROLE_GAP_GATE_MIN_REMAINING_SECONDS  = 30.0
...
_DETAIL_SOURCE_KIND_PRIORITY = {
    "official": 0, "primary": 0, "government": 1, "regulatory": 1, "data_source": 1,
    "academic": 2, "company": 2, "reputable_media": 3, "secondary": 4,
    "aggregator": 5, "forum_social": 6, "weak_unknown": 6, "wrong_source": 9,
}
_DETAIL_SOURCE_VALUE_PRIORITY = {
    "direct": 0, "primary_locator": 0, "contradiction": 1, "absence": 1,
    "context": 3, "weak": 4, "wrong": 9,
}
_WRONG_SOURCE_VALUES = frozenset({"wrong"})
_WRONG_SOURCE_KINDS = frozenset({"wrong_source"})
...
def _entities_missing_from_evidence(
    *, entities: tuple[str, ...], packets: tuple[AcceptedEvidence, ...],
    ...
    blob_parts = [p.source_text for p in packets] + [p.source_result_text for p in packets]
    blob_parts += [f"{o.entity} {o.value}" for o in observations]
...
async def _augment_missing_entity_evidence(
    *, question: str, entities: tuple[str, ...], contract: ResearchContract,
    ...
    seen_keys: set[str] = set()
    counter = 0
    # ENTITY_GAP_MULTIQUERY: concrete retrieval change — parallel claim-driven queries per entity
    async def _entity_gap_search(query: str):
        try:
    ...
    for entity in entities[:ENTITY_GAP_MAX_ENTITIES_TO_AUGMENT]:
        if deadline - perf_counter() < ENTITY_GAP_MIN_REMAINING_SECONDS:
            break
        base = f"{entity} {metric_hint}".strip()
        gap_queries = [
            base,
            f"{entity} official statistics OR primary source",
            f"{entity} revised OR corrected OR final figure",
        ]
        responses = await asyncio.gather(*(_entity_gap_search(q) for q in gap_queries))
        for query, response in zip(gap_queries, responses):
            if response is None:
                continue
...
async def _detect_missing_criterion_values(
    *, question: str, draft_answer: str, state: ResearchRunState,
) -> tuple[dict[str, str], ...]:
    messages = [
        {"role": "system", "content": (
            "ROLE: answer completeness auditor. Given a question and a draft answer, list every "
            "(entity, criterion) pair where the draft asserts or needs a specific value but gives no "
            "exact cited figure - hedged phrasings like 'assessed as exceeding', 'based on the range', "
            "'appears to', or a criterion asserted with no number are all gaps. Only include gaps whose "
            "value plausibly exists in public data.\n\n"
            "ROSTER DUTY: When the question requires a COMPLETE set (which/list/name-all/top-N/every X "
            "meeting criteria) and the draft names only one or a few members without evidence the full "
            "candidate pool was enumerated, emit a roster gap: entity='ROSTER', criterion=the set "
            "definition, query=a search that surfaces the COMPLETE authoritative list or top-N ranking "
            "(the canonical lineup page, the official chart/table, 'list of ...'). If the set is "
            "filtered by a second criterion, emit ALSO one gap per named-but-unverified member so each "
            "can be checked. An answer built from the first member found loses to a full enumeration.\n\n"
            'OUTPUT exactly {"gaps":[{"entity":"...","criterion":"...","query":"web search that would '
            'surface the exact value, include entity + metric + year"}]}. Empty list if none. No prose.'
        )},
        {"role": "user", "content": f"Question:\n{question}\n\nDraft answer:\n{draft_answer}\n\n"
            'Return exactly one JSON object now: {"gaps":[{"entity":"...","criterion":"...","query":"..."}]}'},
    ]
    def _validate(payload: dict[str, object]) -> str | None:
        ...
        for g in gaps:
            if not isinstance(g, dict) or not str(g.get("query", "")).strip():
                return "each gap needs a query"
        return None
    payload = await _call_json_llm_with_retry(
        messages=messages, model=GEMMA_MODEL, temperature=0.0,
        validate_payload=_validate, state=state, stage="criterion_gap_detection", max_attempts=1,
    )
    if not payload:
        return ()
    out = []
    for g in payload.get("gaps", [])[:CRITERION_GAP_MAX_QUERIES]:
        out.append({"entity": str(g.get("entity", "")), "criterion": str(g.get("criterion", "")),
                    "query": str(g.get("query", "")).strip()})
    return tuple(out)



async def _rescue_entity_rows_from_fetched_pages(
    *, gaps: tuple[dict[str, str], ...], existing_packets: tuple[AcceptedEvidence, ...],
    state: ResearchRunState, deadline: float, counter_start: int,
) -> tuple[tuple[EvidenceCandidate, ...], int]:
    """The criterion value often sits in a table row of a page we ALREADY fetched, dropped by
    chunk selection. Re-fetch the best-known page URLs and admit the entity-adjacent window
    directly, with true offsets so the validator can hydrate the citation slice."""
    urls: list[str] = []
    seen_urls: set[str] = set()
    for packet in existing_packets:  # stier-sorted upstream: official sources first
        u = (getattr(packet, "url", "") or "").strip()
        key = _normalize_url(u) or u
        if u and key not in seen_urls and not _blocked_fetch_url_reason(u):
            seen_urls.add(key)
            urls.append(u)
        if len(urls) >= CRITERION_ROW_RESCUE_MAX_URLS:
            break
    candidates: list[EvidenceCandidate] = []
    counter = counter_start
    for url in urls:
        if deadline - perf_counter() < CRITERION_GAP_GATE_MIN_REMAINING_SECONDS:
            break
        try:
            response = await fetch_page(url, provider=_SEARCH_PROVIDER, timeout=FETCH_PAGE_TOOL_TIMEOUT_SECONDS)
        except Exception:
            continue
        fetch_data = tuple(getattr(getattr(response, "response", None), "data", ()) or ())
        fetch_item = fetch_data[0] if fetch_data else None
        tool_results = tuple(getattr(response, "results", ()) or ())
        tool_result = tool_results[0] if tool_results else None
        page_text = (getattr(tool_result, "note", "") or getattr(fetch_item, "content", "") or "")
        receipt_id = (getattr(response, "receipt_id", "") or "").strip()
        result_id = (getattr(tool_result, "result_id", "") or "").strip()
        if not page_text.strip() or not receipt_id or not result_id:
            continue
        title = getattr(tool_result, "title", None) or getattr(fetch_item, "title", None)
        lowered = page_text.lower()
        for gap in gaps:
            entity = gap.get("entity", "").strip()
            criterion = gap.get("criterion", "").strip()
            if not entity:
                continue
            crit_terms = [t for t in re.split(r"[^a-z0-9%]+", criterion.lower()) if len(t) > 3][:6]
            windows_taken = 0
            start_from = 0
            while windows_taken < CRITERION_ROW_RESCUE_MAX_WINDOWS_PER_GAP:
                idx = lowered.find(entity.lower(), start_from)
                if idx < 0:
                    break
                w0 = max(0, idx - CRITERION_ROW_RESCUE_WINDOW_CHARS // 2)
                w1 = min(len(page_text), idx + len(entity) + CRITERION_ROW_RESCUE_WINDOW_CHARS // 2)
                window = page_text[w0:w1]
                start_from = idx + len(entity)
                wl = window.lower()
                has_value = bool(re.search(r"\d", window))
                term_hits = sum(1 for t in crit_terms if t in wl)
                # accept entity windows that carry a number; prefer criterion-term proximity
                if not has_value or (crit_terms and term_hits == 0 and windows_taken > 0):
                    continue
                counter += 1
                windows_taken += 1
                candidates.append(EvidenceCandidate(
                    candidate_id=f"R{counter}", parent_candidate_id=f"R{counter}",
                    slot_id=PRIMARY_SOURCE_SLOT_ID,
                    slot_intent=_slot_intent_for_slot(PRIMARY_SOURCE_SLOT_ID),
                    text_part="chunk", text_start=w0, text_end=w1,
                    receipt_id=receipt_id, result_id=result_id,
                    url=url, title=title, source_text=window,
                    query=f"{entity} {criterion}".strip(), source_kind="selected_chunk",
                ))
    return tuple(candidates), counter


async def _augment_missing_criterion_values(
    *, question: str, gaps: tuple[dict[str, str], ...], contract: ResearchContract,
    role_ledger: tuple[ResearchPlanRole, ...], existing_packets: tuple[AcceptedEvidence, ...],
    existing_observations: tuple[EvidenceObservation, ...], state: ResearchRunState,
    deadline: float,
) -> GateResult:
    ...
    seen_keys: set[str] = set()
    counter = 0
    rescued, counter = await _rescue_entity_rows_from_fetched_pages(
        gaps=gaps, existing_packets=existing_packets, state=state,
        deadline=deadline, counter_start=counter,
    )
    ...
    for gap in gaps:
        ...
        try:
            response = await search_web(
                [query], provider=_SEARCH_PROVIDER,
                num=CRITERION_GAP_SEARCH_RESULTS_PER_QUERY, timeout=LITE_SEARCH_TOOL_TIMEOUT_SECONDS,
            )
        except Exception:
            continue
        receipt_id = getattr(response, "receipt_id", "") or ""
        for result in tuple(getattr(response, "results", ()) or ()):
            ...
            if not url or not note or _blocked_fetch_url_reason(url):
                continue
            key = f"criterion_gap:{_normalize_url(url) or url}:{_text_fingerprint(note)}"
            if key in seen_keys:
                continue
            seen_keys.add(key)
            counter += 1
            candidates.append(EvidenceCandidate(
                candidate_id=f"V{counter}",
                parent_candidate_id=f"V{counter}",
                slot_id=PRIMARY_SOURCE_SLOT_ID,
                slot_intent=_slot_intent_for_slot(PRIMARY_SOURCE_SLOT_ID),
                text_part="search_snippet", text_start=0, text_end=len(note),
                receipt_id=receipt_id, result_id=f"V{counter}",
                url=url, title=getattr(result, "title", None), source_text=note,
                query=query, source_kind=SEARCH_SNIPPET_EVIDENCE_SOURCE_KIND,
            ))
    if not candidates or deadline - perf_counter() < CRITERION_GAP_GATE_MIN_REMAINING_SECONDS:
        return GateResult(accepted_packets=(), observations=())
    return await _run_observation_gate_once(
        question=question, loop_index=2, existing_packets=existing_packets,
        existing_observations=existing_observations, contract=contract,
        retrieval_roles=role_ledger, candidates=tuple(candidates),
        model=EVIDENCE_GATE_MODEL, state=state, stage="criterion_gap_gate", lane="criterion_gap",
    )


async def _augment_missing_role_evidence(
    *, question: str, contract: ResearchContract, coverage_state: CoverageState,
    ...
    accumulated_results: tuple[AccumulatedSearchResult, ...], seen_url_keys: set[str],
    state: ResearchRunState, deadline: float,
) -> GateResult:
    """Generalizes the entity-gap lane: after the first gate pass, spend remaining budget on
    one targeted follow-up round for any contract role the gate marked missing or weak, instead
    of leaving coverage_state.can_answer/missing_role_ids/weak_role_ids unused."""
    if deadline - perf_counter() < ROLE_GAP_MIN_REMAINING_SECONDS:
        return GateResult(accepted_packets=(), observations=())
    gap_role_ids = {*coverage_state.missing_role_ids, *coverage_state.weak_role_ids}
    gap_roles = tuple(r for r in contract.roles if r.role_id in gap_role_ids)[:ROLE_GAP_MAX_ROLES_TO_AUGMENT]
    if not gap_roles:
        return GateResult(accepted_packets=(), observations=())
    gap_targets = tuple(
        EvidenceSearchTarget(
            target_id=f"gap_{role.role_id}", slot_id=role.slot_id, slot_intent=role.slot_intent,
            needed_source_text=role.question, source_type="role_gap_fill",
            inventory=EvidenceSourceInventory(
                entities=(question,), aliases=(), source_families=(), document_handles=(),
                metric_terms=(), date_scope=(), must_include=(), avoid=(), site_constraints=(),
            ),
    ...
    gap_routes = await _generate_evidence_search_routes(
        question=question, round_index=1, targets=gap_targets,
        tried_queries=tuple(sorted(tried_query_identities)),
        accumulated_results=accumulated_results, state=state,
    )
    gap_queries = _materialized_evidence_search_queries(gap_routes, tried_queries=tried_query_identities)
    if not gap_queries:
        return GateResult(accepted_packets=(), observations=())
    responses = await _run_lite_search_round(
        routes=gap_routes, queries=gap_queries, state=state, round_index=99, deadline=deadline,
    )
    if not responses:
        return GateResult(accepted_packets=(), observations=())
    gap_results: list[AccumulatedSearchResult] = []
    _accumulate_lite_search_results(
        accumulated=gap_results, response=responses, routes=gap_routes,
        seen_urls=seen_url_keys, round_index=99, state=state,
    )
    if not gap_results:
        return GateResult(accepted_packets=(), observations=())
    candidates, _ = _snippet_results_to_candidates(
        results=tuple(gap_results), seen_candidate_keys=set(), candidate_counter=0,
    )
    ...
    return await _run_observation_gate_once(
        question=question, loop_index=1, existing_packets=existing_packets,
        existing_observations=existing_observations, contract=contract,
        retrieval_roles=role_ledger, candidates=candidates,
        model=EVIDENCE_GATE_MODEL, state=state, stage="role_gap_gate", lane="role_gap",
    )

def _is_wrong_labeled(label: SearchResultSourceLabel | None) -> bool:
    if label is None:
        return False
    return label.source_value in _WRONG_SOURCE_VALUES or label.source_kind in _WRONG_SOURCE_KINDS

def _source_tier_priority(label: SearchResultSourceLabel | None) -> tuple[int, int]:
    if label is None:
        return (5, 5)
    return (
        _DETAIL_SOURCE_KIND_PRIORITY.get(label.source_kind, 5),
        _DETAIL_SOURCE_VALUE_PRIORITY.get(label.source_value, 5),
    )

def _tried_query_identities_from_routes(routes: tuple[EvidenceSearchRoute, ...]) -> set[str]:
    return {
        _query_identity(q) for route in routes
        for q in _materialized_evidence_search_route_queries(route) if _query_identity(q)
    }

def _seen_url_keys_from_results(results: tuple[AccumulatedSearchResult, ...]) -> set[str]:
    return {_normalize_url(r.url) or r.url for r in results}

def _run_state_diagnostic_note(state: ResearchRunState | None) -> str:
    """Surface otherwise-silent pipeline degradation only when there is no answer to give;
    never attached on the successful-answer path."""
    if state is None:
        return ""
    parts: list[str] = []
    if state.search_query_failures:
        parts.append(f"{state.search_query_failures} search query attempt(s) failed")
    if state.json_llm_stage_failures:
        top = sorted(state.json_llm_stage_failures.items(), key=lambda kv: -kv[1])[:3]
        parts.append("stage failures: " + ", ".join(f"{stage}x{count}" for stage, count in top))
    return f"(Pipeline diagnostics: {'; '.join(parts)}.)" if parts else ""
_TAG="502cba9d42fb4a249ec7501963be879e"
