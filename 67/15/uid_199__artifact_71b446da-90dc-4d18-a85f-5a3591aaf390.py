...
# The governor's clock used to start inside _investigate, but _expected_answer_text
# runs BEFORE that and is an LLM call bounded only by LLM_TIMEOUT (90s). So the
# governor could read 210s elapsed while the sandbox had already burned 270+, and
# the 300s kill takes the whole task to 0. Measured: one task died at exactly
# 301s with timeout_miner_owned. The wall is now set at the entrypoint, so every
# pre-investigation call counts against it.
...
async def query(query: Query) -> Response:
    """Entry wrapper: start the wall, and never let an exception escape.

    The donor entrypoint had no top-level guard, so any unexpected failure in
    investigation or structured-output materialisation became
    miner_unhandled_exception -- a hard zero with no partial credit. Every rung
    below still runs exactly as before; this only catches what used to escape.
    """
    _wall_start()
    try:
        return await _answer_query(query)
    except Exception:
        # the rescue must respect the answer mode: a structured query answered in
        # `text` is rejected outright, which is the same hard zero the guard exists
        # to prevent
...
_WALL_RESCUE_TEXT = (
    "[[beacon:no-evidence]] The research run could not be completed for this "
    "question."
)
...
async def _answer_query(query: Query) -> Response:
    ...
    if query.output_schema is not None:
        try:
            ...
            output = None
        if output is None:
            # a structured task answered in `text` is rejected outright, and the
            # question echoed into the schema is a zero dressed as a valid
            # response -- emit a schema-shaped marker instead
