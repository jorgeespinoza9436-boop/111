"""SN67 Harnyx miner — staged research protocol agent. [slot 04 build 2026-08-03T10:59:58+00:00]"""
...
def _evidence_digest(index: _ResultIndex, terms: list[str]) -> str:
    ...
    for n in numbers:
        ...
        spans = index.spans(n)
        if not spans:
            # never surfaced in full (a search result): give it the same
...
# --- role localisation ----------------------------------------------------
# A stage of its own, sitting between research and the answer turn. Research
# stops when the model runs out of tool turns, not when the question is
# answerable; this stage asks a different question — for each thing the
# question asks FOR, is there a passage in hand that actually states it? —
# and keeps re-projecting the pages already retrieved against the parts still
# unstated. It issues no requests: it only changes which regions of retained
# pages are on the table, and it re-enters as long as that keeps changing.
...
async def _plain_query(query: Query, budget: float) -> Response:
    ...
    try:
        ...
        # --- LOCALISE: re-project retained pages onto the unstated parts ---
        ...
        # --- LOCALISE re-entry: the re-dispatch turns may have added pages ---
        ...
        # --- LOCALISE decides what is delivered ---
        ...
        # the question's roles, and — where that turns up a figure the draft does
        # not report — the answer it returns is the one that goes out.
        if display:
...
# slot: 04 C13_render 2026-08-03T10:59:58+00:00
