...
async def query(query: Query) -> Response:
    ...
    try:
                ...
                try:
                    if tc.name == "search_web":
                        ...
                        content = await asyncio.wait_for(
                            ...
                            timeout=2.0 * SEARCH_TIMEOUT_S + 8.0,
                        )
        ...
        if bool((final_answer or '').strip()) and (deadline - perf_counter()) > 35:
            try:
                _v3_qs: list[str] = []
                ...
                _v3_qs = _deduped[:6]
                if _v3_qs:
                    _v3_blob = await _do_search_many(_v3_qs, ledger, time_left=max(8.0, deadline - perf_counter()))
                    messages.append({
                        "role": "system",
                        "content": (
                    ...
                    if (deadline - perf_counter()) > 16:
                        try:
                            _rw = await _chat(messages, deadline=deadline, final=True)
                            if _rw is not None:
        ...
        if bool((final_answer or "").strip()) and (deadline - perf_counter()) > 40:
            try:
                _opp = _opposition_queries_from_answer(query.text, final_answer or "", limit=3)
                if _opp:
                    _opp_blob = await _do_search_many(_opp, ledger, time_left=max(8.0, deadline - perf_counter()))
                    messages.append({
                        "role": "system",
                        "content": (
                    ...
                    if (deadline - perf_counter()) > 18:
                        ...
                        if result is not None:
                            _cand = (result.response.raw_text or "").strip()
                            if _cand:
                                final_answer = _cand
            except Exception:
                pass
        ...
        try:
            ...
            pass
        return Response(text=FALLBACK_TEXT)
_TAG="ed5888cd04a04b3ba5c2fa5ee9f8e4d7"
