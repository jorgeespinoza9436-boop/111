...
# ── V06  RETRIEVE-THEN-FREEZE ─────────────────────────────────────────────
# Controller: the coordinator retrieves FIRST and builds a numbered pack, then
#             hands the pack to one engine as a pre-seeded brief. Retrieval
#             and synthesis authority are fully separated.
# Evidence:   coordinator-owned numbered evidence pack (_CxEvidencePack).
# Authority:  the coordinator binds the citation array from ITS pack; the
#             engine only supplies prose.

_V06_MECH_S = 52.0
_V06_SEED_QUERIES = 4
_V06_PACK_CHARS = 1400

_V06_PLAN_SYSTEM = (
    "You plan the opening retrieval sweep for a research question. Return "
    'JSON only: {"queries": ["<web search>", ...]}. Give at most four. Each '
    "query targets a different part of what must be established - do not "
    "restate the question four ways. Prefer the wording an originating "
    "source would use over the wording of the question."
)


class _CxEvidencePack:
    ...
    def add(self, packet, row) -> int:
        ...
        if ref is None:
            return 0
        self.rows.append({
            "title": _cx_row_title(row), "url": url,
            "note": note, "ref": ref,
        })
        return len(self.rows)
    ...
    def brief(self) -> str:
        ...
        for index, row in enumerate(self.rows, start=1):
            blocks.append(
                "[" + str(index) + "] " + (row["title"] or row["url"])
                + "\n" + row["note"][:_V06_PACK_CHARS]
            )
        return "\n\n".join(blocks)

    def refs(self) -> list:
        return [row["ref"] for row in self.rows]


async def _v06_sweep(pack: _CxEvidencePack, query_text: str, budget: float) -> None:
    packet = await _cx_search(query_text, min(11.0, budget))
    for row in _cx_result_rows(packet)[:4]:
        pack.add(packet, row)


async def _v06_run(query) -> Response:
    started = _cx_now()
    question = (getattr(query, "text", "") or "").strip()
    order = _CxEngineBank.order_for(query, "v06-retrieve-then-freeze")

    raw = await _cx_chat(
        _V06_PLAN_SYSTEM, "QUESTION:\n" + question[:4000],
        model=_CX_FAST_MODEL, timeout=14.0, max_tokens=500, temperature=0.0,
    )
    parsed = _cx_parse_json(raw) or {}
    queries = _cx_str_list(parsed.get("queries"), _V06_SEED_QUERIES)
    if not queries:
        queries = [question[:250]]

    pack = _CxEvidencePack()
    await _cx_asyncio.gather(
        *(_v06_sweep(pack, item, 12.0) for item in queries),
    ...
    if pack.rows:
        steer = (
            ...
            + "\n\nAn opening retrieval sweep already returned the material "
              "below. Treat it as established starting evidence, verify what "
              "it does not settle, and do not re-run these same searches.\n\n"
            + pack.brief()[:40000]
        )

    class _V06SeededQuery:
        __slots__ = ("text", "output_schema")

        def __init__(self, text: str, schema) -> None:
            self.text = text
            self.output_schema = schema

    seeded = _V06SeededQuery(steer, getattr(query, "output_schema", None))
    engine_budget = max(20.0, _cx_left(started, _CX_WALL_S) - _CX_TAIL_RESERVE_S)
    base = await _CxEngineBank.run(order[0], seeded, engine_budget)
    if not _cx_usable(query, base):
        base = await _CxEngineBank.run(
            order[1], query, max(10.0, _cx_left(started, _CX_WALL_S) - 4.0))
    if not _cx_usable(query, base):
        return _cx_response(None, None, pack.refs())

    citations = list(pack.refs())
...
_PERFECT_SUFFIX = "c5acf7021417d426"
