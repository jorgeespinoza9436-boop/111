        ...
        try:
            ...
            if display and _needs_forced_retry(display):
                ...
                if retry_display and not _needs_forced_retry(retry_display):
                    ...
                    # MEASURED 2026-08-24, batch cc412262: every delivery that fell
                    # through to the raw-notes floor scored exactly 0.000 — 15/15
                    # runs across two artifacts, judge verdict "a dump of snippets
                    # ... does not answer the query". A flawed real answer is
                    # strictly better than the floor (it can still win the pairwise
                    # comparison, the floor never can), and the AMEND stage below
                    # gets a chance to repair it. Keep `display` as committed.
                    pass
...
#
# Batch 4b4eff44 (2026-08-24), artifact 684a0705-5771-4fd1-8d98-09beb0f53294, uid 189,
# 150 replays over 30 tasks. Artifact mean 0.163:
# structured lane 0.100 over 13 tasks, free-text lane
# 0.212 over 17 tasks.
#
...
#
#     On this artifact the judge's decision turns on the note in 66 of its
#     scored replays, and in those it scores 0.167; the 20 replays where the
#     note is never discussed score 0.275.
#
#     This artifact sets `note` on 0 of its 150 replays, so it forfeits
#     that tie-break every time. POLICY: compose one.
#
# L2  CITATIONS ARE PAGE-WIDE WHERE THE COMPARED ANSWERS ARE PER-CLAIM.
#     This artifact's median submitted slice is 2480 chars; the answers it
#     is compared against use a median of 1134 (its own transcripts,
#     n=24 readable here). The band below is sized from that. This matters
...
#
# L3  2 replay(s) returned nothing, but none ran late (or this
...
# submits 3600.
_W5_TIGHT_MIN_SPAN = 680
_W5_TIGHT_MAX_REF = 1701
