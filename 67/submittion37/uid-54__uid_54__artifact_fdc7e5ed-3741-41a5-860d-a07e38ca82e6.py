...
def _compose_aster_moraine_entry():
...
_aster_moraine_query_entry = _compose_aster_moraine_entry()


def _compose_brass_canopy_entry():
...
_brass_canopy_query_entry = _compose_brass_canopy_entry()


_BALANCED_ROUTER_SEED = "6f1d4957080c4ab164ed651a"
...
def _balanced_route_label(query: Query) -> str:
    ...
    return "AsterMoraine" if bucket < 128 else "BrassCanopy"


class AsterMoraine:
    async def __call__(self, query: Query) -> Response:
        return await _aster_moraine_query_entry(query)


class BrassCanopy:
    async def __call__(self, query: Query) -> Response:
        return await _brass_canopy_query_entry(query)


_BALANCED_PRIMARY_AGENT = AsterMoraine()
_BALANCED_SECONDARY_AGENT = BrassCanopy()
_CANDIDATE_BRANCH_CLASS_NAMES = ("AsterMoraine", "BrassCanopy")
