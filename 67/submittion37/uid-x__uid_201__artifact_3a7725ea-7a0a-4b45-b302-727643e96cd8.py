...
_SHORT_AGENT = CedarQuillAgent()
_LONG_AGENT = JuniperCompassAgent()


async def _w5_base_query(query: Query) -> Response:
    text = (getattr(query, "text", "") or "").strip()
    if len(text) >= _LENGTH_CUTOFF:
        branch = _LONG_AGENT
    else:
        branch = _SHORT_AGENT
...
#
# Batch 4b4eff44 (2026-08-24), artifact 3dd77fa8-aa7a-40b4-ac24-3ed7c6a490f7, uid 60,
# 50 replays over 10 tasks. Artifact mean 0.110:
# structured lane 0.086 over 7 tasks, free-text lane
# 0.167 over 3 tasks.
#
...
#
#     On this artifact the judge's decision turns on the note in 32 of its
#     scored replays, and in those it scores 0.094; the 8 replays where the
#     note is never discussed score 0.
#
...
#     This artifact's median submitted slice is 3434 chars; the answers it
#     is compared against use a median of 2576 (this batch's 13 well-sampled artifacts,
#     n=11 readable here). The band below is sized from that. This matters
