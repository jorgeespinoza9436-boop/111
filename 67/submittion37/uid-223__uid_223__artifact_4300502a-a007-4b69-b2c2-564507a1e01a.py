...
def _compose_cedar_quill_agent_entry():
    ...
    _FETCH_STATE: dict = {
        "spent_s": 0.0, "dead": [], "sdss_pages": {}, "ready_answer": "",
    }
    ...
    def _reset_run_state() -> None:
        ...
        _FETCH_STATE["dead"] = []
        _FETCH_STATE["sdss_pages"] = {}
        _FETCH_STATE["ready_answer"] = ""
...
def _finalize_branch_response(response: Response, query: Query) -> Response:
    ...
    citations = getattr(response, "citations", None)
    if citations and cleaned:
        import re as _coverage_re

        mentioned = {
            int(number)
            for number in _coverage_re.findall(r"\[\[(\d+)\]\]", cleaned or "")
        }
        missing = [
            position
            for position in range(1, len(citations) + 1)
            if position not in mentioned
        ]
        if missing:
            pointers = " ".join(f"[[{position}]]" for position in missing)
            cleaned = (
                (cleaned or "").rstrip()
                + "\n\nAdditional cited evidence retained from this research response: "
                + pointers
                + "."
            )
    if cleaned == note:
...
async def _s37_base_query(query: Query) -> Response:
    ...
    return _finalize_branch_response(response, query)
