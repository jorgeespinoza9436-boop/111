...
# ---- v210-1-khsc ----
# Added stages: coverage nudge, temporal repair, set gap-fill, citation slice backfill
# Already in base: roster pre-pass, premise sweep, corroborate
# Ordinary successful path:
#   query -> _solve -> _knowledge_brief -> _build_roster -> _loop (+_open_criteria_hint) -> _audit_patch -> _select_best -> _premise_sweep -> _align_timeframe -> _widen_pool -> _corroborate -> _refs_within_budget -> _citations_for -> _w2_point_markers -> _answer_line_only -> Response
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # Scope-and-content sweeps, between the base's own premise and
    # corroboration passes. Grounding and authority sit BEFORE
    # corroboration on purpose: they partition by backer count, and a
    # source pulled in by corroboration would mask an unanchored answer.
    try:
        _staged = await _align_timeframe(question, answer, messages,
                                                 ledger, deadline)
        if _is_usable_answer(_staged):
            answer = _staged
    except Exception:
    ...
    try:
        _staged = await _widen_pool(question, answer, messages,
                                            ledger, deadline)
        if _is_usable_answer(_staged):
            answer = _staged
    except Exception:
