...
def _compose_orchid_harbor_agent_entry():
    ...
    LOOP_MODEL_A = "zai-org/GLM-5.2-TEE"
    LOOP_MODEL_B = "zai-org/GLM-5.2-TEE"
    AUDIT_MODEL = "moonshotai/Kimi-K2.6-TEE"              
    SCHEMA_MODEL = "moonshotai/Kimi-K2.6-TEE"             
    RESORT_MODEL = "deepseek-ai/DeepSeek-V3.2-TEE"          
    ...
    def _upstream_key(model: str) -> str | None:
        if model.startswith("zai-org/GLM-5.2-TEE"):
            return "glm"
    ...
    # ---- v220-8-rkpu ----
    # Stages: roster pre-pass, coverage nudge, premise sweep, unit repair
    # Ordinary successful path:
    #   query -> _w5_base_query -> _solve -> _knowledge_brief -> _draft_candidate_pool -> _loop (+_open_criteria_hint) -> _audit_patch -> _verify_subjects -> _conform_measures -> _citations_for -> _answer_line_only -> _w5_point_repair / _w5_anchor_board -> Response
    ...
    _W5_THIN_RATIO = 0.45
    _W5_HEAD_KEEP = 700
    _W5_FALLBACK_PROVIDER = "chutes"
    _W5_FALLBACK_MODEL = "moonshotai/Kimi-K2.6-TEE"
...
def _compose_juniper_river_agent_entry():
    ...
    COMMIT_FALLBACK_MODEL = "deepseek-ai/DeepSeek-V3.2-TEE"
    ...
    EXTRACT_MODEL = "google/gemma-4-31B-turbo-TEE"
    ...
    # slot: 11 C38_note 2026-08-26T09:08:08+00:00

    return query

_juniper_river_agent_query_entry = _compose_juniper_river_agent_entry()


def _compose_beryl_vector_agent_entry():
    # ==========================================================================
    # uid_101 variant v05 - candidate-roster pre-pass + official-source anchoring
    # Base: uid_101.py (unchanged root: _solve / _loop / EvidenceLedger)
    # Stages combined, with the corpus uid each is distilled from:
    #   _build_roster        donor uid 89   (also shipped by uid 24, 89, 150, 236)
    #   _authority_sweep     donor uid 111  (also shipped by uid 111, 163, 212)
    # ==========================================================================
    ...
    VERSION = "v52-v05-from-uid" + "-".join(['89', '111'])
    ...
    LOOP_MODEL_A = "zai-org/GLM-5.2-TEE"
    LOOP_MODEL_B = "deepseek-ai/DeepSeek-V3.2-TEE"
    AUDIT_MODEL = "moonshotai/Kimi-K2.6-TEE"              
    SCHEMA_MODEL = "moonshotai/Kimi-K2.6-TEE"             
    RESORT_MODEL = "deepseek-ai/DeepSeek-V3.2-TEE"          
    SEARCH_PROVIDER = "parallel"                                             
    ...
    # --- st stage pack (begin) ---
    # Post-audit verification-and-repair stages distilled from the reachable-path
    # reports of uids 2, 24, 53, 79, 81, 82, 89, 111, 135, 137, 150, 163, 189, 193,
    # 212, 229, 236. Every stage follows the same contract:
    #
    #   deterministic trigger -> (optional) one targeted retrieval
    #   -> bounded rewrite through the existing _loop -> adoption guard
    #
    # A stage that does not fire returns the answer unchanged, costs nothing, and
    # leaves the ledger untouched. A stage that fires and produces a worse answer
    # is rejected by the adoption guard, so the pipeline is monotone.

    ST_MIN_TAIL_S = 62.0            # do not start a stage below this wall clock
    ST_SEARCH_MIN_S = 78.0          # do not start a stage that retrieves below this
    ST_STAGE_TURNS = 2              # loop turns granted to a stage rewrite
    ST_ADOPT_RATIO = 0.6            # reject a rewrite shorter than this * original
    ST_MIN_USD = 0.03
    ST_HAYSTACK_PER_ROW = 20000
    ST_HAYSTACK_CAP = 240000

    _ST_YEAR_RE = re.compile(r"\b(1[89]\d{2}|20\d{2})\b")
    _ST_NUM_RE = re.compile(r"\d[\d,]*(?:\.\d+)?")
    _ST_CITE_RE = re.compile(r"\[\[?\s*[\d,\s\-]{1,20}\s*\]?\]")
    _ST_CAP_RE = re.compile(
        r"\b[A-Z][A-Za-z0-9&.\-']*(?:\s+(?:of|the|and|for|de|van|von|du|di)\s+"
        r"[A-Z][A-Za-z0-9&.\-']*|\s+[A-Z][A-Za-z0-9&.\-']*)*")
    _ST_HEDGE_RE = re.compile(
        r"\b(?:among others|and several more|and others|multiple [a-z]+s|"
        r"a number of|various other|etc\.?|and more)\b", re.IGNORECASE)
    _ST_BULLET_RE = re.compile(r"^\s*(?:[-*\u2022]|\d+[.)])\s+\S", re.MULTILINE)

    _ST_STOP_CAPS = frozenset((
        "The", "This", "That", "These", "Those", "What", "Which", "Who", "Whom",
        "Whose", "When", "Where", "Why", "How", "In", "On", "At", "For", "From",
        "By", "With", "As", "If", "Is", "Are", "Was", "Were", "Has", "Have", "Had",
        "Do", "Does", "Did", "Give", "List", "Name", "State", "Report", "Provide",
        "According", "Based", "Using", "Between", "During", "After", "Before",
        "Answer", "Question", "Note", "Each", "Every", "Both", "All", "Any",
        "A", "An", "And", "Or", "But", "Not", "No", "Yes", "It", "Its",
    ))

    _ST_UNIT_MAP = (
        ("trillion", ("trillion", "tn", "tn.")),
        ("billion", ("billion", "bn", "bn.")),
        ("million", ("million", "mn", "mm")),
        ("thousand", ("thousand", "k)", " k ")),
        ("percentage point", ("percentage point", "pp", "p.p.")),
        ("percent", ("%", "percent", "per cent")),
        ("per cent", ("%", "percent", "per cent")),
        ("usd", ("$", "usd", "us$")),
        ("dollar", ("$", "usd", "dollar")),
        ("euro", ("\u20ac", "eur", "euro")),
        ("pound sterling", ("\u00a3", "gbp", "pound")),
        ("yen", ("\u00a5", "jpy", "yen")),
        ("rupee", ("\u20b9", "inr", "rupee")),
        ("yuan", ("\u00a5", "cny", "rmb", "yuan")),
        ("tonne", ("tonne", "tonnes", " t ")),
        ("metric ton", ("tonne", "metric ton", " t ")),
        ("kilogram", ("kg", "kilogram")),
        ("kilometre", ("km", "kilometre", "kilometer")),
        ("kilometer", ("km", "kilometre", "kilometer")),
        ("hectare", ("hectare", " ha")),
        ("square metre", ("m\u00b2", "sq m", "square met")),
        ("megawatt", ("mw", "megawatt")),
        ("gigawatt", ("gw", "gigawatt")),
        ("terawatt", ("twh", "terawatt")),
        ("barrel", ("bbl", "barrel")),
    )

    _ST_OFFICIAL_MARKERS = (
        ".gov", ".gov.uk", ".gc.ca", ".gov.au", ".govt.nz", ".edu", ".ac.uk",
        ".int", ".mil", "europa.eu", "un.org", "who.int", "imf.org", "oecd.org",
        "worldbank.org", "sec.gov", "federalreserve.gov", "ecb.europa.eu",
        "bis.org", "eurostat", "ons.gov.uk", "census.gov", "bls.gov", "nih.gov",
        "cdc.gov", "nasa.gov", "esa.int", "iea.org", "wto.org", "ilo.org",
        "investor.", "ir.", "/investor", "annualreport", "sec.report",
    )
    ...
    def _st_cited_rows(answer: str, ledger: EvidenceLedger) -> list[dict]:
        ...
        for hit in _CITE_NUM_RE.finditer(answer or ""):
            for piece in re.split(r"[,\s]+", hit.group(1)):
                piece = piece.strip()
    ...
    async def _st_rewrite(question: str, answer: str, order: str,
                          messages: list[dict], ledger: EvidenceLedger,
                          deadline: float, turns: int = ST_STAGE_TURNS) -> str:
        """Bounded rewrite through the primary controller, with adoption guard."""
        if not messages:
            return answer
        messages.append({"role": "system", "content": order})
        try:
            rewritten, _ = await _loop(question, "", ledger, deadline, turns + 1,
    ...
    _ST_TAIL_ORDER = ("\nRewrite the COMPLETE final answer in the required shape, "
                      "keeping every [n] citation attached to the claim it supports.")
    ...
    async def _unit_repair(question: str, answer: str, messages: list[dict],
        ...
        if not missing:
            return answer
        order = ("UNIT CHECK: the question asks for figures in "
                 + ", ".join(missing[:4]) +
                 " but the answer never states them in that unit, currency or "
                 "scale. Restate every load-bearing figure in the demanded unit. "
                 "Where the source does not print the converted form, give the "
                 "source figure verbatim and the conversion in parentheses. Do "
                 "not alter a value that is already correct."
                 + _ST_TAIL_ORDER)
    ...
    async def _value_repair(question: str, answer: str, messages: list[dict],
        ...
        if not unsupported:
            return answer
        order = ("VALUE SUPPORT: these figures appear in the answer but no page "
                 "retrieved so far prints them: " + ", ".join(unsupported[:6]) +
                 ". For each one either (a) retrieve a page that prints it and "
                 "cite that page, or (b) replace it with the figure the evidence "
                 "does print, verbatim. Never carry a derived or rounded number "
                 "that no source states. Use at most 2 tool calls."
                 + _ST_TAIL_ORDER)
        return await _st_rewrite(question, answer, order, messages, ledger, deadline)
    ...
    async def _temporal_repair(question: str, answer: str, messages: list[dict],
        ...
        try:
            ...
            pass
        order = ("TEMPORAL ALIGNMENT: the question is anchored to "
                 + ", ".join(missing[:3]) +
                 " but the evidence behind the answer carries no excerpt from "
                 "that period. Retrieve a source that reports the figure for the "
                 "anchor year itself and cite it. If only an adjacent year is "
                 "available, say so explicitly in the answer rather than "
                 "presenting it as the anchor year. Use at most 2 tool calls."
                 + _ST_TAIL_ORDER)
        return await _st_rewrite(question, answer, order, messages, ledger, deadline)
    ...
    async def _premise_sweep(question: str, answer: str, messages: list[dict],
        ...
        try:
            ...
            pass
        order = ("PREMISE VERIFICATION: the question names "
                 + ", ".join(missing[:3]) +
                 " but no retrieved excerpt mentions it. Either confirm the "
                 "premise against a source that names the subject directly, or "
                 "state plainly in the answer that the premise does not hold and "
                 "give what the evidence does support. Do not answer around a "
                 "subject you never verified. Use at most 2 tool calls."
                 + _ST_TAIL_ORDER)
        return await _st_rewrite(question, answer, order, messages, ledger, deadline)
    ...
    async def _corroborate(question: str, answer: str, messages: list[dict],
        ...
        try:
            ...
            pass
        order = ("CORROBORATION: the decisive figure " + lead + " is carried by "
                 "exactly one retrieved source. Find a second, independent source "
                 "that prints the same figure and cite both. If no independent "
                 "source confirms it, say in the answer that the figure is "
                 "single-sourced and name the source. Use at most 2 tool calls."
                 + _ST_TAIL_ORDER)
        return await _st_rewrite(question, answer, order, messages, ledger, deadline)
    ...
    async def _set_gapfill(question: str, answer: str, messages: list[dict],
        ...
        try:
            ...
            pass
        reason = ("it hand-waves with an open-ended phrase"
                  if hedged else f"it enumerates only {enumerated} member(s)")
        order = ("SET COMPLETENESS: this question ranges over a candidate pool and "
                 + reason + ". Retrieve the authoritative list or table that "
                 "enumerates the WHOLE pool, then give a verdict for EVERY member "
                 "(qualifies / excluded because X), each with its own citation. An "
                 "answer naming 3 qualifiers when the pool holds 6 scores as "
                 "wrong, not partial. Remove every open-ended phrase. Use at most "
                 "3 tool calls." + _ST_TAIL_ORDER)
        return await _st_rewrite(question, answer, order, messages, ledger, deadline,
                                 turns=ST_STAGE_TURNS + 1)
    ...
    async def _authority_sweep(question: str, answer: str, messages: list[dict],
        ...
        try:
            ...
            pass
        order = ("AUTHORITY: every source cited is secondary — no official filing, "
                 "statistical agency, regulator or primary publisher is among "
                 "them. Retrieve the primary source that originally published "
                 "these figures and re-cite the load-bearing claims to it. Keep "
                 "the secondary citation only where the primary is unreachable, "
                 "and say so. Use at most 2 tool calls." + _ST_TAIL_ORDER)
        return await _st_rewrite(question, answer, order, messages, ledger, deadline)


    # ------------------------------------------------------------ roster pre-pass
    _ST_ROSTER_SYSTEM = ("You enumerate the candidate pool a research question "
                         "ranges over. JSON only, no prose, no code fences.")
    ...
    async def _build_roster(question: str, deadline: float) -> str:
        ...
        if (deadline - monotonic()) < 150.0 or _spend_left() < ST_MIN_USD:
            return ""
        ask = ("Identify the candidate pool this question ranges over — the closed "
               "set whose members must each be checked before an answer is "
               "defensible. JSON only, keys: \"pool\" (list of member names; [] if "
               "the question does not range over a set), \"closed\" (true if the "
               "pool can be fully enumerated from a single authoritative list), "
               "\"roster_query\" (the search query most likely to return that "
               "list).\n\nQuestion:\n" + (question or "")[:2000])
        try:
            raw = await _chat_simple(
                LLM_LANE_A, AUDIT_MODEL, _ST_ROSTER_SYSTEM, ask,
                max_tokens=700,
                timeout=max(8.0, min(26.0, (deadline - monotonic()) - 120.0)))
            raw = re.sub(r"^```(?:json)?\s*|\s*```$", "", raw.strip(), flags=re.I | re.M)
            report = json.loads(raw)
        except Exception:
            return ""
        ...
        pool = [str(item).strip() for item in pool_raw
                if str(item).strip()] if isinstance(pool_raw, list) else []
        ...
        if pool:
            ...
            lines.append("Verify EVERY member above against every condition in the "
                         "question and give each one a cited verdict. If the pool "
                         "is wrong, correct it from a retrieved list — do not "
                         "silently drop a member.")
        if query:
            lines.append("If the pool is unconfirmed, search first for: " + query[:200])
        if report.get("closed") is False:
            lines.append("The pool may be open-ended: state the enumeration "
                         "criterion you used before listing members.")
        return "\n".join(lines)
    # --- st stage pack (end) ---
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            _roster_block = await _build_roster(question, deadline)
            if _roster_block:
                brief = (brief + "\n\n" + _roster_block) if brief else _roster_block
        except Exception:
            pass
        try:
            answer, messages = await _loop(question, brief, ledger, deadline, MAX_TURNS)
        ...
        try:
            if _is_usable_answer(answer):
                answer = await _authority_sweep(question, answer, messages, ledger, deadline)
        except Exception:
            pass
        ...
        for path, value in pending[:_W5_RECOVER_FIELDS]:
            ...
            if not context and deadline - _w5_clock() > _W5_FETCH_MIN_S:
                ...
                if url and _W5_SDK_FETCH is not None:
                    before = len(_w5_pages())
                    try:
                        await _w5_tapped_fetch_page(url, provider=SEARCH_PROVIDER,
                                                    timeout=16.0)
                    except Exception:
                        pass
                    for page in _w5_pages()[before:]:
...
_beryl_vector_agent_query_entry = _compose_beryl_vector_agent_entry()


_BALANCED_ROUTER_SEED = "7cbf2d79cbeb46b926563027"
...
def _balanced_route_label(query: Query) -> str:
    ...
    if bucket == 0:
        return "OrchidHarborAgent"
    if bucket == 1:
        return "JuniperRiverAgent"
    return "BerylVectorAgent"
...
class JuniperRiverAgent:
    async def __call__(self, query: Query) -> Response:
        return await _juniper_river_agent_query_entry(query)


class BerylVectorAgent:
    async def __call__(self, query: Query) -> Response:
        return await _beryl_vector_agent_query_entry(query)


_BALANCED_PRIMARY_AGENT = OrchidHarborAgent()
_BALANCED_SECONDARY_AGENT = JuniperRiverAgent()
_BALANCED_TERTIARY_AGENT = BerylVectorAgent()
...
async def _h666_base_query(query: Query) -> Response:
    ...
    if selected == "OrchidHarborAgent":
        branch = _BALANCED_PRIMARY_AGENT
    elif selected == "JuniperRiverAgent":
...
_H666_LLM_PROVIDER = "chutes"
_H666_LLM_MODELS = ("moonshotai/Kimi-K2.6-TEE", "zai-org/GLM-5.2-TEE")
...
# batch_tag='drv10' salt='3faad18ffa8e'
# Ordinary-path architecture added relative to the baseline agent:
#   baseline research -> draft answer
#   -> claim-conflict ledger audit (required elements, unsupported claims,
#      comparison/period-basis gaps, official-vs-independent conflict,
...
_DRV_TAG = 'drv10'
_DRV_SALT = '3faad18ffa8e'

_DRV_LLM_PROVIDER = "chutes"
_DRV_LLM_MODELS = ("moonshotai/Kimi-K2.6-TEE", "zai-org/GLM-5.2-TEE", "zai-org/GLM-5.2-TEE")
