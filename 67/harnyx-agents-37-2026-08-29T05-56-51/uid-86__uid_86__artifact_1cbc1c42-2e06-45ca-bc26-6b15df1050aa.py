...
_W6_VERSION = "v403-uid24xuid82xuid193-rgcv"
# w6-tri variant 03 on the uid_193 three-branch router.
# added stages (4): roster pre-pass, set gap-fill sweep, lead-figure corroboration, value-support repair
# stage set RGCV = uid 24 (RC) x uid 82 (CV) x uid 193 (CG)
# post-answer chain runs under _StageBudget: max 2 rewrites, 62s reserve.
# spliced into ALL THREE branches: lumen_anvil + cedar_quill via the
# EvidenceLedger adapter, juniper_compass via the _ResultIndex adapter.
...
def _compose_lumen_anvil_agent_entry():
    ...
    # --- w6 stage kit: branch-agnostic core ---
    # uid_193 routes each query to one of three branches built on DIFFERENT engines.
    # The stages below therefore depend on no branch-local helper: every figure,
    # entity and bracket helper is defined here, and every engine touch (search,
    # rewrite, evidence, clock, spend) goes through a per-branch context object.

    _W6_FIGURE_RE = re.compile(r"\d+(?:[.,]\d+)*")
    _W6_CITE_RE = re.compile(r"\[[0-9]{1,3}\]")
    _W6_LIST_RE = re.compile(r"(?m)^[ \t]*[(\[]?\d{1,2}[.)\]][ \t]+")
    _W6_WORD_RE = re.compile(r"[A-Z][A-Za-z0-9&'’.\-]*")
    _W6_YEAR_RE = re.compile(r"\b(?:19|20)\d{2}\b")
    _W6_MIN_FIGURE_CHARS = 3
    _W6_MIN_ENTITY_CHARS = 3
    _W6_MIN_REVISION_RATIO = 0.6
    _W6_SUBJECT_WORDS = 12
    _W6_LEDGER_SCAN_CHARS = 2_000_000
    _W6_SEARCH_CHARS = 6000
    _W6_STOPHEAD = frozenset(
    ...
    def _w6_normalize_figure(token: str) -> str:
        body = (token or "").replace(",", "")
        if "." in body:
            body = body.rstrip("0").rstrip(".")
        return body.lstrip("0") or "0"


    def _w6_figures_in(text: str) -> set:
        body = _W6_LIST_RE.sub(" ", _W6_CITE_RE.sub(" ", text or ""))
        return {_w6_normalize_figure(m.group(0)) for m in _W6_FIGURE_RE.finditer(body)}


    def _w6_entities_in(text: str) -> set:
        found = set()
        for match in _W6_WORD_RE.finditer(text or ""):
            token = match.group(0)
            if len(token) >= _W6_MIN_ENTITY_CHARS and not token.isupper():
    ...
    def _w6_load_bearing_figures(answer: str, question: str) -> list:
        """Figures the answer asserts: markers stripped, question-supplied removed."""
        body = _W6_LIST_RE.sub(" ", _W6_CITE_RE.sub(" ", answer or ""))
        given = _w6_figures_in(question or "")
        ordered = []
        seen = set()
        for match in _W6_FIGURE_RE.finditer(body):
            value = _w6_normalize_figure(match.group(0))
            if value in seen or value in given:
                continue
            if len(value.replace(".", "")) < _W6_MIN_FIGURE_CHARS:
                continue
            seen.add(value)
    ...
    def _w6_search_subject(question: str) -> str:
        words = []
        for word in " ".join((question or "").split()).split(" "):
            token = word.strip("?.,:;\"'()[]")
            if not token:
                continue
            if not words and token.lower() in _W6_STOPHEAD:
                continue
            words.append(token)
            if len(words) >= _W6_SUBJECT_WORDS:
                break
        return " ".join(words)


    def _w6_keeps_entities(draft: str, revision: str) -> bool:
        return _w6_entities_in(draft).issubset(_w6_entities_in(revision))


    def _w6_long_enough(draft: str, revision: str) -> bool:
        return len(revision) >= int(len(draft) * _W6_MIN_REVISION_RATIO)


    def _w6_set_needed(question: str) -> bool:
        """Branch-local set/superlative detectors differ, so this one is our own."""
        q = " ".join((question or "").split()).lower()
        if re.search(r"\b(all|every|each|which of|list|name the|how many)\b", q):
            return True
        return bool(re.search(r"\b(most|least|largest|smallest|highest|lowest|best|"
                              r"worst|first|last|only|fastest|slowest|biggest)\b", q))


    # --- w6 adapter: EvidenceLedger branches (lumen_anvil, cedar_quill) ---
    class _W6Ctx:
        """Everything a stage needs from the engine, for the ledger-based branches.

        Stages never touch `_loop`, `_do_search` or the ledger directly. Keeping the
        engine behind this object is what lets one stage implementation run on all
        three branches of the router.
        """

        def __init__(self, store, messages: list, deadline: float) -> None:
            self.store = store
        ...
        def spend_ok(self) -> bool:
            try:
                return _spend_left() > WRAPUP_MIN_USD
            except Exception:
                return True

        def usable(self, text: str) -> bool:
            try:
                return _is_usable_answer(text)
            except Exception:
                return bool((text or "").strip())

        def has_evidence(self) -> bool:
            return bool(getattr(self.store, "rows", None))

        def evidence_text(self) -> str:
            if self._blob is None:
                parts = []
                scanned = 0
                for row in getattr(self.store, "rows", []) or []:
                    for field in ("title", "preview", "text"):
                        blob = row.get(field) or ""
                        if not blob:
                            continue
                        room = _W6_LEDGER_SCAN_CHARS - scanned
                        if room <= 0:
                            break
        ...
        def evidence_figures(self) -> set:
            if self._figures is None:
                self._figures = {_w6_normalize_figure(m.group(0))
                                 for m in _W6_FIGURE_RE.finditer(self.evidence_text())}
            return self._figures

        def sources_stating(self, figure: str) -> list:
            hits = []
            for row in getattr(self.store, "rows", []) or []:
                blob = (row.get("text") or "") + " " + (row.get("preview") or "")
                for match in _W6_FIGURE_RE.finditer(blob):
                    if _w6_normalize_figure(match.group(0)) == figure:
                        key = str(row.get("url") or row.get("result_id") or "")
                        key = key.split("://")[-1].split("?")[0].rstrip("/").lower()
                        if key and key not in hits:
                            hits.append(key)
                        break
            return hits

        def plan_floor(self) -> float:
            """Tail this branch needs left before a pre-research stage is worth it."""
            return 150.0

        async def plan_call(self, system: str, user: str, timeout: float) -> str:
            try:
                return await _chat_simple(LLM_LANE_A, AUDIT_MODEL, system, user,
                                          max_tokens=1200, timeout=timeout)
            except Exception:
                return ""

        async def search(self, probe: str) -> str:
            if not (probe or "").strip():
                return ""
            try:
                return _commit_tool_output(await _do_search(probe, self.store), self.store)
            except Exception:
                return ""

        async def rewrite(self, question: str, order: str, body: str = "") -> str:
            carry = list(self.messages)
            carry.append({"role": "system", "content": order})
            if body:
                carry.append({"role": "system",
                              "content": "Targeted evidence just retrieved:\n"
                                         + body[:_W6_SEARCH_CHARS]})
            try:
                revised, _ = await _loop(question, "", self.store, self.deadline, 1,
                                         carry=carry)
            except Exception:
                return ""
    ...
    # --- w6 stage governor ---
    # Every stage self-gates on the wall clock, but four stages gating
    # independently can still eat the tail one stage at a time: each looks
    # affordable alone and the last finds nothing left. The governor caps the chain
    # as a UNIT and reserves what the delivery path still needs.
    _W6_STAGE_RESERVE_S = 62.0
    _W6_MAX_STAGE_RUNS = 2
    _W6_STAGE_MIN_GAIN = 12


    class _StageBudget:
        """Shared allowance for the post-answer stage chain."""

        def __init__(self, deadline: float, reserve_s: float, max_runs: int) -> None:
            self.deadline = deadline
        ...
        def admits(self, label: str) -> bool:
            if self.runs >= self.max_runs:
                self.skipped.append(label + ":quota")
                return False
            if self._tail() < self.reserve_s:
                self.skipped.append(label + ":clock")
                return False
            self.admitted.append(label)
            return True

        def note(self, label: str, before: str, after: str) -> None:
            """Only a stage that actually changed the answer spends the quota."""
            if not (after or "").strip() or after == before:
                return
            if abs(len(after) - len(before)) < _W6_STAGE_MIN_GAIN \
                    and after[:200] == before[:200]:
                return
            self.runs += 1


    # --- stage R: roster pre-pass, ledger branches (uid 24 / 150 / 236 lineage) ---
    _W6R_TIMEOUT_S = 26.0
    _W6R_MIN_TAIL_S = 150.0
    _W6R_MAX_POOL = 24
    _W6R_MIN_USD = 0.03
    ...
    _W6R_PROBE = (
        ...
        '"pool" (list of member names belonging to that class — err toward INCLUDING '
        "a doubtful member; a member listed then excluded with a reason costs "
        "nothing, a member never listed is a loss), "
        '"roster_hint" (one web search query phrased AS A LIST — '
        "'<subject> full list', 'list of <subject>', '<subject> table' — that would "
        "return the authoritative enumeration). "
    ...
    class _RosterPlan:
        ...
        members nobody thought to search for are invisible to the research turns.
        """
        ...
        def as_directive(self) -> str:
            lines = ["ROSTER PRE-PASS — the candidate pool was enumerated before "
                     "research started. It is a CHECKLIST, not evidence: every member "
                     "below must be verified against the question's conditions with "
                     "its own [n], and any member you discover that is missing here "
                     "must be added."]
            if self.axis:
    ...
    def _w6r_parse(raw: str) -> _RosterPlan | None:
        try:
            body = re.sub(r"^```(?:json)?\s*|\s*```$", "", (raw or "").strip(),
                          flags=re.I | re.M)
        ...
        if isinstance(raw_pool, list):
            for entry in raw_pool:
                ...
                if len(pool) >= _W6R_MAX_POOL:
                    break
        plan = _RosterPlan(axis.strip() if isinstance(axis, str) else "", pool,
                           hint.strip() if isinstance(hint, str) else "")
        return plan if plan.is_actionable() else None


    async def _build_roster(question: str, brief: str, deadline: float):
        """Stage — enumerate the candidate pool before the research loop opens."""
        if not _w6_set_needed(question):
            return None
        if (deadline - monotonic()) < _W6R_MIN_TAIL_S:
            return None
        try:
            if _spend_left() < _W6R_MIN_USD:
                return None
        except Exception:
            pass
        probe = _W6R_PROBE + "\n\nQuestion:\n" + question
        ...
        timeout = max(8.0, min(_W6R_TIMEOUT_S,
                               (deadline - monotonic()) - _W6R_MIN_TAIL_S + 20.0))
        try:
            raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL,
            ...
            return None
        return _w6r_parse(raw)


    # --- stage G: set gap-fill sweep (uid 135 / 189 / 193 lineage) ---
    _W6G_MIN_TAIL_S = 70.0
    _W6G_MIN_MEMBERS = 3
    _W6G_HEDGE_RE = re.compile(
    ...
    _W6G_MEMBER_LINE_RE = re.compile(r"(?m)^[ \t]*(?:[-*•]|\(?\d{1,2}[.)\]])[ \t]+\S")


    class _SetCoverage:
        """How many members the answer enumerates, and whether it hand-waves."""
        ...
        def looks_truncated(self) -> bool:
            return bool(self.hedges) or self.members < _W6G_MIN_MEMBERS
    ...
    def _set_coverage(answer: str) -> _SetCoverage:
        ...
        body = answer or ""
        hedges = []
        for match in _W6G_HEDGE_RE.finditer(body):
            token = match.group(0).strip()
            if token and token.lower() not in [h.lower() for h in hedges]:
                hedges.append(token)
        return _SetCoverage(len(_W6G_MEMBER_LINE_RE.findall(body)), hedges)


    async def _set_gapfill(question: str, answer: str, ctx) -> str:
        """Stage — a deterministic backstop for pool completeness.

        A model-driven audit forgives a truncated roster because a truncated roster
        reads as a finished answer. Counting enumerated members and hunting
        hand-waving phrases does not.
        """
        if not ctx.usable(answer) or not _w6_set_needed(question):
            return answer
        before = _set_coverage(answer)
        if not before.looks_truncated():
            return answer
        if ctx.tail() < _W6G_MIN_TAIL_S or not ctx.spend_ok():
            return answer

        body = await ctx.search(_w6_search_subject(question) + " full list")
        ...
        order = (
            ...
            " member lines" + detail + ". A pool reported short scores as WRONG, not "
            "as partial. Read the list evidence just retrieved, enumerate the WHOLE "
            "pool, and give EVERY member its own line with a verdict — qualifies "
            "(with its citation per condition) or excluded because X (with its "
            "citation). Replace every 'among others' style phrase with the actual "
            "members. If the pool is genuinely too large, rank it, show every member "
            "down to a stated cutoff, and state the cutoff. Return the COMPLETE final "
            "answer."
        )
        revised = await ctx.rewrite(question, order, body)
        if not ctx.usable(revised):
            return answer
        if _set_coverage(revised).members < before.members:
            return answer
        if not _w6_keeps_entities(answer, revised):
            return answer
        if len(revised) < int(len(answer) * 0.8):
            return answer
        return revised


    # --- stage C: lead-figure corroboration (uid 82 / 137 / 193 lineage) ---
    _W6C_MIN_TAIL_S = 66.0
    ...
    def _corroboration_check(question: str, answer: str, ctx) -> _CorroborationCheck:
        """Deterministic: the lead figure is the first one the answer asserts."""
        figures = _w6_load_bearing_figures(answer, question)
        if not figures:
            return _CorroborationCheck("", [])
        return _CorroborationCheck(figures[0], ctx.sources_stating(figures[0]))


    async def _corroborate(question: str, answer: str, ctx) -> str:
        """Stage — a decisive figure resting on one source gets a second look."""
        if not ctx.usable(answer) or not ctx.has_evidence():
            return answer
        check = _corroboration_check(question, answer, ctx)
        if not check.is_single_sourced():
            return answer
        if ctx.tail() < _W6C_MIN_TAIL_S or not ctx.spend_ok():
            return answer

        body = await ctx.search(_w6_search_subject(question) + " " + check.figure)
        order = (
            ...
            "explicitly and give the value the source-attributed form it deserves. "
            "Keep every entity and every other figure, then return the COMPLETE final "
            "answer."
        )
        revised = await ctx.rewrite(question, order, body)
        if not ctx.usable(revised):
            return answer
        lost = _w6_figures_in(answer) - _w6_figures_in(revised)
        if lost - {check.figure}:
            return answer
        if not _w6_keeps_entities(answer, revised) or not _w6_long_enough(answer, revised):
            return answer
        return revised


    # --- stage V: value-support repair (uid 79 / 82 lineage) ---
    _W6V_MIN_TAIL_S = 66.0
    _W6V_MAX_REPORTED = 5


    class _ValueSupportTable:
        """Load-bearing figures split by whether the evidence can back them."""

        def __init__(self, checked: list, unsupported: list) -> None:
    ...
    def _value_support_table(question: str, answer: str, ctx) -> _ValueSupportTable:
        """Deterministic set difference against the retrieved evidence."""
        checked = _w6_load_bearing_figures(answer, question)
        if not checked:
            return _ValueSupportTable([], [])
        supported = ctx.evidence_figures()
        return _ValueSupportTable(checked, [v for v in checked if v not in supported])


    async def _value_repair(question: str, answer: str, ctx) -> str:
        """Stage — refuse to ship a figure the retrieved evidence never showed.

        The adoption guard here is deliberately not the house one: removing the
        offending figure IS the fix, so exactly the unsupported figures are
        permitted to disappear and the loss of anything else is a rejection.
        """
        if not ctx.usable(answer) or not ctx.has_evidence():
            return answer
        table = _value_support_table(question, answer, ctx)
        if table.is_clean():
            return answer
        if ctx.tail() < _W6V_MIN_TAIL_S or not ctx.spend_ok():
            return answer

        named = table.unsupported[:_W6V_MAX_REPORTED]
        body = await ctx.search(_w6_search_subject(question) + " " + named[0])
        order = (
            "VALUE SUPPORT: these figures appear in your answer but appear in NO "
            "retrieved source:\n- " + "\n- ".join(named) +
            "\nFor each one: either cite the source that states it with its [n], or "
            "remove the figure and rewrite the sentence around what the evidence does "
            "support. Do NOT restate an unsupported figure with a hedge — an uncited "
            "number scores as a fabrication, a stated absence does not. Keep every "
            "entity, every other figure and every existing citation, then return the "
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        _w6_roster = None
        try:
            _w6_roster = await _build_roster(question, brief, deadline)
        except Exception:
            _w6_roster = None
        if _w6_roster is not None:
            _w6_note = _w6_roster.as_directive()
            brief = (brief + "\n\n" + _w6_note) if brief else _w6_note
        ...
        try:
            ...
            pass
        _w6_ctx = _W6Ctx(ledger, messages, deadline)
        _w6_gov = _StageBudget(deadline, _W6_STAGE_RESERVE_S, _W6_MAX_STAGE_RUNS)
        if _w6_gov.admits("set_gapfill"):
            try:
                _w6_was = answer
                _w6_out = await _set_gapfill(question, answer, _w6_ctx)
                if _is_usable_answer(_w6_out):
                    answer = _w6_out
                _w6_gov.note("set_gapfill", _w6_was, answer)
            except Exception:
                pass
        if _w6_gov.admits("corroborate"):
            try:
                _w6_was = answer
                _w6_out = await _corroborate(question, answer, _w6_ctx)
                if _is_usable_answer(_w6_out):
                    answer = _w6_out
                _w6_gov.note("corroborate", _w6_was, answer)
            except Exception:
                pass
        if _w6_gov.admits("value_repair"):
            try:
                _w6_was = answer
                _w6_out = await _value_repair(question, answer, _w6_ctx)
                if _is_usable_answer(_w6_out):
                    answer = _w6_out
                _w6_gov.note("value_repair", _w6_was, answer)
            except Exception:
                pass
...
def _compose_juniper_compass_agent_entry():
    ...
    # --- w6 adapter: _ResultIndex branch (juniper_compass) ---
    class _W6Ctx:
        """The same stage interface, backed by juniper's span index and amend engine.

        This branch has no `_loop` and no ledger, so a rewrite is expressed as a
        synthetic gap handed to `_amend` — the stage's finding travels through the
        branch's own repair path rather than around it.
        """

        def __init__(self, store, asks: list, deadline: float) -> None:
            self.store = store
            self.asks = asks
            self.deadline = deadline
            self._blob = None
        ...
        def usable(self, text: str) -> bool:
            ...
            if len(body) < 24:
                return False
            try:
                return not _needs_forced_retry(body)
            except Exception:
                return True

        def has_evidence(self) -> bool:
            try:
                return self.store.max_number() > 0
            except Exception:
                return False

        def _notes(self):
            try:
                top = self.store.max_number()
            except Exception:
                return
            for number in range(1, top + 1):
                meta = self.store.get(number)
                if meta is None:
                    continue
        ...
        def evidence_text(self) -> str:
            if self._blob is None:
                ...
                for _, note in self._notes():
                    ...
                    if room <= 0:
                        break
                    parts.append(note[:room])
                    scanned += min(len(note), room)
                self._blob = " ".join(parts).lower()
            return self._blob
        ...
        def sources_stating(self, figure: str) -> list:
            hits = []
            for number, note in self._notes():
                for match in _W6_FIGURE_RE.finditer(note):
                    if _w6_normalize_figure(match.group(0)) == figure:
                        meta = self.store.get(number) or {}
                        key = str(meta.get("url") or number)
                        key = key.split("://")[-1].split("?")[0].rstrip("/").lower()
                        if key and key not in hits:
                            hits.append(key)
                        break
        ...
        def plan_floor(self) -> float:
            """This branch caps research at RESEARCH_TIME_CAP_SECONDS, so the floor
            is scaled to that window rather than to the whole task budget."""
            return 95.0

        async def plan_call(self, system: str, user: str, timeout: float) -> str:
            """This branch has no `_chat_simple`; it talks to llm_chat directly."""
            try:
                result = await llm_chat(
                    provider=LLM_PROVIDER, model=MODEL,
                    messages=[{"role": "system", "content": system},
                              {"role": "user", "content": user}],
                    temperature=0.1, thinking=LlmThinkingConfig(enabled=False),
                    timeout=timeout)
                return (result.response.raw_text or "").strip()
            except Exception:
                return ""
        ...
        async def search(self, probe: str) -> str:
            if not (probe or "").strip():
                return ""
            try:
                out = await _run_search_web(probe, self.store)
            except Exception:
                return ""
            self._blob = None
            self._figures = None
        ...
        async def rewrite(self, question: str, order: str, body: str = "") -> str:
            """A stage finding, expressed as a gap the branch's amend engine repairs."""
            passage = order
            if body:
                passage += "\n\nEvidence just retrieved:\n" + body[:_W6_SEARCH_CHARS]
            try:
                ask = _Ask(label="stage check", terms=[])
                revised = await _amend(question, self._draft, [(ask, passage)],
                                       self.deadline)
            except Exception:
                return ""
            return (revised or "").strip()

        def stage_draft(self, answer: str) -> None:
            self._draft = answer
    ...
    # --- stage R: roster pre-pass, index branch (uid 24 / 150 / 236 lineage) ---
    # This branch has no research brief to fold a directive into. Its native notion
    # of "things the answer owes" is the _Ask checklist that relocation and amend
    # both run against, so the pool is injected there: every enumerated member
    # becomes an ask, which makes an unreported member a gap the branch already
    # knows how to repair.
    _W6R_TIMEOUT_S = 26.0
    _W6R_MIN_TAIL_S = 90.0
    _W6R_MAX_POOL = 24
    _W6R_ASK_CAP = 10
    ...
    _W6R_PROBE = (
        ...
        "a doubtful member; a member listed then excluded with a reason costs "
        "nothing, a member never listed is a loss), "
        '"roster_hint" (one web search query phrased AS A LIST that would return the '
    ...
    class _RosterPlan:
        """Candidate pool fixed before the answer is judged complete."""
        ...
        def as_asks(self) -> list:
            out = []
            for member in self.pool[:_W6R_ASK_CAP]:
                terms = [t.lower() for t in re.findall(r"[A-Za-z0-9']{3,}", member)][:3]
                if not terms:
                    continue
                out.append(_Ask(label="roster member: " + member, terms=terms))
            return out
    ...
    async def _roster_asks(question: str, deadline: float) -> list:
        """Stage — the enumerated pool, expressed as asks this branch must satisfy."""
        if not _w6_set_needed(question):
            return []
        if (deadline - perf_counter()) < _W6R_MIN_TAIL_S:
            return []
        messages = [
            {"role": "system", "content": "Pool enumerator for set and superlative "
                                          "questions. JSON only."},
            {"role": "user", "content": _W6R_PROBE + "\n\nQuestion:\n" + question},
        ]
        try:
            result = await llm_chat(
                provider=LLM_PROVIDER, model=MODEL, messages=messages, temperature=0.1,
                thinking=LlmThinkingConfig(enabled=False),
                timeout=min(_W6R_TIMEOUT_S, max(8.0, deadline - perf_counter() - 60.0)),
            )
            raw = (result.response.raw_text or "").strip()
        except Exception:
            return []
        plan = _w6r_parse(raw)
        return plan.as_asks() if plan is not None else []
    ...
    async def _plain_query(query: Query, budget: float) -> Response:
        ...
        try:
            ...
            try:
                _w6_extra = await _roster_asks(
                    query.text, deadline - FINAL_RESERVE_SECONDS)
                if _w6_extra:
                    asks = list(asks) + _w6_extra
            except Exception:
                pass
            ...
            if display:
                ...
                _w6_ctx = _W6Ctx(index, asks, deadline - 4)
                _w6_gov = _StageBudget(deadline - 4, _W6_STAGE_RESERVE_S, _W6_MAX_STAGE_RUNS)
                if _w6_gov.admits("set_gapfill"):
                    try:
                        _w6_was = decided
                        _w6_ctx.stage_draft(decided)
                        _w6_out = await _set_gapfill(query.text, decided, _w6_ctx)
                        if _w6_ctx.usable(_w6_out):
                            decided = _w6_out
                        _w6_gov.note("set_gapfill", _w6_was, decided)
                    except Exception:
                        pass
                if _w6_gov.admits("corroborate"):
                    try:
                        _w6_was = decided
                        _w6_ctx.stage_draft(decided)
                        _w6_out = await _corroborate(query.text, decided, _w6_ctx)
                        if _w6_ctx.usable(_w6_out):
                            decided = _w6_out
                        _w6_gov.note("corroborate", _w6_was, decided)
                    except Exception:
                        pass
                if _w6_gov.admits("value_repair"):
                    try:
                        _w6_was = decided
                        _w6_ctx.stage_draft(decided)
                        _w6_out = await _value_repair(query.text, decided, _w6_ctx)
                        if _w6_ctx.usable(_w6_out):
                            decided = _w6_out
                        _w6_gov.note("value_repair", _w6_was, decided)
                    except Exception:
                        pass
...
# COORDINATOR LAYER
# ═══════════════════════════════════════════════════════════════════════════
# The three w6-staged research closures above are unchanged engines. This
# layer owns control flow, holds its own evidence structure, and decides the
# delivered answer.
#
# DESIGN CONSTRAINT specific to this base script: each branch ALREADY runs a
# four-stage post-answer chain (roster pre-pass, set gap-fill sweep,
# lead-figure corroboration, value-support repair) under _StageBudget. No
# coordinator below restates any of those four - pool enumeration, set
# completeness, figure-vs-search corroboration and value-support repair are
# treated as solved inside the engine. Every mechanism here is cross-run,
# structural, or deterministic instead.
#
# Syntax policy observed (only constructs the base script already uses):
#   no `global` / `nonlocal`, no `del`, no `__slots__`, no `@staticmethod`,
#   no dunder attribute access, no set `&` (the base has no BitAnd), getattr
#   field names are string literals only, no dynamically selected callables,
#   no `sys` / `unicodedata`, openrouter only.

import asyncio as _cx_asyncio
import json as _cx_json
import re as _cx_re
from time import monotonic as _cx_clock

from harnyx_miner_sdk.api import llm_chat as _cx_llm_chat
from harnyx_miner_sdk.api import search_web as _cx_search_web
from harnyx_miner_sdk.query import CitationRef as _CxRef
from harnyx_miner_sdk.query import CitationSlice as _CxSlice
...
# Branch walls live inside the closures and cannot be rebound without
# `global`, so a run is never shortened - these caps only bound how long we
# WAIT. Multi-branch designs run concurrently.
_CX_WALL_S = 292.0
_CX_ENGINE_CAP_S = 278.0

_CX_SENT_RE = _cx_re.compile(r"[^.!?\n]+(?:[.!?]+|\n|$)")
_CX_PTR_RE = _cx_re.compile(r"\[\[(\d{1,3})\]\]")
_CX_SGL_PTR_RE = _cx_re.compile(r"(?<!\[)\[(\d{1,3})\](?!\])")
_CX_FIG_RE = _cx_re.compile(
    r"\b(?:\d{1,3}(?:,\d{3})+(?:\.\d+)?|\d+\.\d+|\d{4}-\d{2}-\d{2}"
    r"|(?:19|20)\d{2}|\d{1,3}(?:\.\d+)?%|\d+)\b"
)
_CX_YEAR_RE = _cx_re.compile(r"\b(?:19|20)\d{2}\b")
_CX_TOK_RE = _cx_re.compile(r"[a-z0-9]+(?:[._%:/+-][a-z0-9]+)*")
_CX_FENCE_RE = _cx_re.compile(r"^```(?:json)?\s*|\s*```$")
_CX_HOST_RE = _cx_re.compile(r"^https?://(?:www\.)?")

_CX_LABELS = ("lumen", "cedar", "juniper")


def _cx_left(t0, budget):
    return budget - (_cx_clock() - t0)
...
def _cx_at_least(part, whole, num, den):
    """part/whole >= num/den, computed without a division."""
    if whole <= 0:
        return True
...
def _cx_covers(needle_text, body):
    ...
    for term in terms:
        if term in pool:
            hit = hit + 1
    return _cx_at_least(hit, len(terms), 6, 10)
...
def _cx_json_of(raw):
    ...
    if start < 0 or end <= start:
        return None
    try:
        parsed = _cx_json.loads(text[start:end + 1])
    except (ValueError, TypeError):
        return None
...
def _cx_quality(query, response):
    ...
    if payload is None and len(text) < 40:
        return 0.0
    score = 1.0
...
async def _cx_search(text, timeout):
    for provider in _CX_SEARCH_PROVIDERS:
        if timeout <= 2.0:
            return None
        try:
            return await _cx_search_web(provider=provider, query=text,
...
def _cx_rows_of(packet):
    if packet is None:
        return []
    return list(getattr(packet, "results", None) or ())


def _cx_row_note(row):
...
def _cx_ref_from_row(packet, row, cap=4000):
    ...
    if not isinstance(note, str) or not note.strip():
        return None
    try:
        return _CxRef(receipt_id=receipt, result_id=result,
                      slices=[_CxSlice(start=0, end=min(len(note), cap))])
    except Exception:
        return None


def _cx_spans_of(ref):
    out = []
...
class _CxSteer:
    """A stand-in Query the branches accept."""
...
async def _cx_engine(label, query, budget):
    """One branch run bounded only by how long we wait. Never raises.
    Explicit names only: a dynamically selected callable is rejected."""
    if budget <= 12.0:
        return None
    if label == "lumen":
        target = _lumen_anvil_agent_query_entry(query)
    elif label == "cedar":
        target = _cedar_quill_agent_query_entry(query)
    elif label == "juniper":
        target = _juniper_compass_agent_query_entry(query)
    else:
        return None
    try:
        return await _cx_asyncio.wait_for(target, timeout=budget)
    except Exception:
        return None
...
def _cx_order_for(query, seed):
    ...
    payload = (seed + "|" + str(fields) + "|" + text[:512] + "|" + text[-256:])
    digest = _cx_hashlib.sha256(payload.encode("utf-8", "ignore")).digest()
    pivot = int.from_bytes(digest[:8], "big") % 3
    return _CX_LABELS[pivot:] + _CX_LABELS[:pivot]


async def _cx_panel(query, order, count, mech_reserve, t0):
    """Run `count` branches concurrently and keep the usable responses."""
    span = _cx_engine_budget(t0, mech_reserve)
    jobs = []
    for label in order[:count]:
        jobs.append(_cx_engine(label, query, span))
    outcomes = await _cx_asyncio.gather(*jobs, return_exceptions=True)
    panel = []
    slot = -1
    for outcome in outcomes:
        ...
        if isinstance(outcome, BaseException):
            continue
        if _cx_usable(query, outcome):
            panel.append((order[slot], outcome))
    return panel


def _cx_best(query, panel):
    if not panel:
        return None
    best = panel[0]
    for pair in panel[1:]:
        if _cx_quality(query, pair[1]) > _cx_quality(query, best[1]):
            best = pair
    return best


def _cx_bind_panel(panel):
    """Merge every branch's citations and renumber each draft onto the union."""
    citations = []
    texts = {}
    for pair in panel:
        offset = len(citations)
        for ref in _cx_cites_of(pair[1]):
            _cx_merge(citations, ref)
        texts[pair[0]] = _cx_shift_pointers(_cx_text_of(pair[1]), offset)
    return citations, texts

# ── V06  CITATION PAYLOAD GOVERNOR ────────────────────────────────────────
# Controller: one branch run, then a deterministic payload audit. No model
#             call and no retrieval - pure arithmetic over the shipped refs.
# Evidence:   ref -> materialised span cost, and pointer -> reachability
#             (_CxPayloadLedger).
# Authority:  refs are tightened, dropped or reordered and pointers rebound
#             before delivery, so the answer cannot be zeroed by an oversized
#             or unreachable citation array.

_V06_MECH_S = 20.0
_V06_TOTAL_CAP = 108000
_V06_REF_CAP = 14000
_V06_MIN_SPAN = 600


class _CxPayloadLedger:
    """ref -> its materialised char cost; pointer -> whether it resolves."""

    def __init__(self, refs):
        self.refs = refs
        self.costs = []
        for ref in refs:
            self.costs.append(_cx_ref_cost(ref))
    ...
    def over_cap(self):
        return self.total() > _V06_TOTAL_CAP
    ...
    def used_slots(self, text):
        out = set()
        for token in _CX_PTR_RE.findall(text or ""):
            try:
                out.add(int(token))
            except ValueError:
                continue
        for token in _CX_SGL_PTR_RE.findall(text or ""):
            try:
                out.add(int(token))
            except ValueError:
                continue
    ...
    def unreachable(self, text):
        """Pointers in the text that name a slot the array does not have."""
        out = set()
        for number in self.used_slots(text):
            if number < 1 or number > len(self.refs):
                out.add(number)
        return out


def _cx_ref_cost(ref):
    spans = _cx_spans_of(ref)
    if not spans:
        # a sliceless ref materialises the WHOLE note - the expensive case
        return _V06_REF_CAP
    running = 0
    for span in spans:
        running = running + (span[1] - span[0])
    return running


def _cx_tighten(ref):
    """Shrink an oversized ref toward the head of each span it already has."""
    spans = _cx_spans_of(ref)
    if not spans:
        return None
    budget = _V06_REF_CAP // max(1, len(spans))
    kept = []
    for span in spans:
        width = span[1] - span[0]
        if width <= budget:
            kept.append(span)
            continue
        end = span[0] + max(_V06_MIN_SPAN, budget)
        if end > span[0]:
            kept.append((span[0], end))
    if not kept:
        return None
    receipt = getattr(ref, "receipt_id", None)
    result = getattr(ref, "result_id", None)
    if not isinstance(receipt, str) or not isinstance(result, str):
        return None
    pieces = []
    for span in kept:
        try:
            pieces.append(_CxSlice(start=span[0], end=span[1]))
        except Exception:
            return None
    try:
        return _CxRef(receipt_id=receipt, result_id=result, slices=pieces)
    except Exception:
        return None
...
async def _v06_run(query):
    t0 = _cx_clock()
    order = _cx_order_for(query, "v06-payload-governor")

    base = await _cx_engine(order[0], query, _cx_engine_budget(t0, _V06_MECH_S))
    if not _cx_usable(query, base):
        base = await _cx_engine(order[1], query, _cx_engine_budget(t0, 8.0))
    if not _cx_usable(query, base):
        return _cx_response(None, None, [])

    refs = _cx_cites_of(base)
    if not refs:
        return base

    tightened = []
    for ref in refs:
        if _cx_ref_cost(ref) > _V06_REF_CAP:
            slimmer = _cx_tighten(ref)
            if slimmer is not None:
                tightened.append(slimmer)
                continue
        tightened.append(ref)

    ledger = _CxPayloadLedger(tightened)
    text = _cx_text_of(base)
    guard = 0
    while ledger.over_cap() and len(ledger.refs) > 1 and guard < 40:
        guard = guard + 1
        drop = ledger.heaviest()
        if drop < 0:
            break
        used = ledger.used_slots(text)
        if (drop + 1) in used and len(ledger.refs) > 2:
            # never drop a ref the prose actually points at while others exist
            spare = -1
            slot = -1
            for _cost in ledger.costs:
                slot = slot + 1
                if (slot + 1) not in used:
                    spare = slot
                    break
            if spare >= 0:
                drop = spare
        ...
        for ref in ledger.refs:
            ...
            if slot != drop:
                kept.append(ref)
        ledger = _CxPayloadLedger(kept)

    payload = _cx_output_of(base)
    if payload is not None:
        return _cx_response(None, payload, ledger.refs)

    dangling = ledger.unreachable(text)
    if dangling:
        cleaned = []
        at = 0
        for match in _CX_PTR_RE.finditer(text):
            ...
            try:
                ...
                number = -1
            cleaned.append(text[at:match.start()])
            if number not in dangling:
                cleaned.append(match.group(0))
            at = match.end()
        cleaned.append(text[at:])
        text = "".join(cleaned)

    return _cx_response(text, None, ledger.refs)
