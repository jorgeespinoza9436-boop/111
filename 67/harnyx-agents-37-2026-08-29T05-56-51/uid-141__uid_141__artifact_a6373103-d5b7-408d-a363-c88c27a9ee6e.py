...
def _compose_willow_compass_agent_entry():
...
_willow_compass_agent_query_entry = _compose_willow_compass_agent_entry()


def _compose_slate_vector_agent_entry():
    ...
    _AGENT_1 = LumenAnvilAgent()
    _AGENT_2 = JuniperCompassAgent()
...
_slate_vector_agent_query_entry = _compose_slate_vector_agent_entry()
...
_SHAPE_ROUTER_SEED = "6fe70394e954a0bd3846c96a"
...
def _balanced_route_label(query: Query) -> str:
    ...
    order = ("WillowCompassAgent", "SlateVectorAgent", "CobaltPrismAgent")
    if shape == 3:
        return order[bucket]
...
class WillowCompassAgent:
    async def __call__(self, query: Query) -> Response:
        return await _willow_compass_agent_query_entry(query)


class SlateVectorAgent:
    async def __call__(self, query: Query) -> Response:
        return await _slate_vector_agent_query_entry(query)
...
_SHAPE_PRIMARY_AGENT = WillowCompassAgent()
_SHAPE_SECONDARY_AGENT = SlateVectorAgent()
_SHAPE_TERTIARY_AGENT = CobaltPrismAgent()
