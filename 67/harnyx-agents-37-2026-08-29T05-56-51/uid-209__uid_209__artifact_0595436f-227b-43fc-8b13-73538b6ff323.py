...
def _compose_garnet_mosaic_agent_entry():
...
_garnet_mosaic_agent_query_entry = _compose_garnet_mosaic_agent_entry()


def _compose_indigo_relay_agent_entry():
...
_indigo_relay_agent_query_entry = _compose_indigo_relay_agent_entry()


def _compose_kestrel_atlas_agent_entry():
...
_kestrel_atlas_agent_query_entry = _compose_kestrel_atlas_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "GarnetMosaicAgent"
    ...
    if (
        ...
        return "IndigoRelayAgent"
    return "KestrelAtlasAgent"


class GarnetMosaicAgent:
    async def __call__(self, query: Query) -> Response:
        return await _garnet_mosaic_agent_query_entry(query)


class IndigoRelayAgent:
    async def __call__(self, query: Query) -> Response:
        return await _indigo_relay_agent_query_entry(query)


class KestrelAtlasAgent:
    async def __call__(self, query: Query) -> Response:
        return await _kestrel_atlas_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = GarnetMosaicAgent()
_ANALYTICAL_FIELD_AGENT = IndigoRelayAgent()
_BROAD_FIELD_AGENT = KestrelAtlasAgent()
...
async def query(query: Query) -> Response:
    ...
    if selected == "GarnetMosaicAgent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "IndigoRelayAgent":
