...
def _compose_raven_quill_agent_entry():
    ...
    # slot: 52 C37_canon_w4 2026-08-26T15:00:00+00:00

    return query

_raven_quill_agent_query_entry = _compose_raven_quill_agent_entry()


def _compose_amber_vector_agent_entry():
...
_amber_vector_agent_query_entry = _compose_amber_vector_agent_entry()
...
_SHAPE_ROUTER_SEED = "8019d2925603740ebf9cc25b"
...
def _balanced_route_label(query: Query) -> str:
    ...
    order = ("RavenQuillAgent", "AmberVectorAgent", "CedarRelayAgent")
    if shape == 3:
        return order[bucket]
...
class RavenQuillAgent:
    async def __call__(self, query: Query) -> Response:
        return await _raven_quill_agent_query_entry(query)


class AmberVectorAgent:
    async def __call__(self, query: Query) -> Response:
        return await _amber_vector_agent_query_entry(query)
...
_SHAPE_PRIMARY_AGENT = RavenQuillAgent()
_SHAPE_SECONDARY_AGENT = AmberVectorAgent()
_SHAPE_TERTIARY_AGENT = CedarRelayAgent()
