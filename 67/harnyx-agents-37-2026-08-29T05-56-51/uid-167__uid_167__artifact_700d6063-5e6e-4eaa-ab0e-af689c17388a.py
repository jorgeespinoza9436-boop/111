...
def _compose_willow_forge_agent_entry():
...
_willow_forge_agent_query_entry = _compose_willow_forge_agent_entry()


def _compose_xenon_harbor_agent_entry():
...
_xenon_harbor_agent_query_entry = _compose_xenon_harbor_agent_entry()


def _compose_yarrow_marten_agent_entry():
...
_yarrow_marten_agent_query_entry = _compose_yarrow_marten_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "WillowForgeAgent"
    ...
    if (
        ...
        return "XenonHarborAgent"
    return "YarrowMartenAgent"


class WillowForgeAgent:
    async def __call__(self, query: Query) -> Response:
        return await _willow_forge_agent_query_entry(query)


class XenonHarborAgent:
    async def __call__(self, query: Query) -> Response:
        return await _xenon_harbor_agent_query_entry(query)


class YarrowMartenAgent:
    async def __call__(self, query: Query) -> Response:
        return await _yarrow_marten_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = WillowForgeAgent()
_ANALYTICAL_FIELD_AGENT = XenonHarborAgent()
_BROAD_FIELD_AGENT = YarrowMartenAgent()
...
async def query(query: Query) -> Response:
    ...
    if selected == "WillowForgeAgent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "XenonHarborAgent":
