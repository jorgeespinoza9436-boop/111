...
def _compose_lumen_orchid_agent_entry():
...
_lumen_orchid_agent_query_entry = _compose_lumen_orchid_agent_entry()


def _compose_marble_compass_agent_entry():
...
_marble_compass_agent_query_entry = _compose_marble_compass_agent_entry()


def _compose_nimbus_quill_agent_entry():
...
def _task_shape_route_label(query: Query) -> str:
    ...
    if (
        ...
        return "MarbleCompassAgent"
    return "NimbusQuillAgent"


class LumenOrchidAgent:
    async def __call__(self, query: Query) -> Response:
        return await _lumen_orchid_agent_query_entry(query)


class MarbleCompassAgent:
    async def __call__(self, query: Query) -> Response:
        return await _marble_compass_agent_query_entry(query)


class NimbusQuillAgent:
    async def __call__(self, query: Query) -> Response:
        return await _nimbus_quill_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = LumenOrchidAgent()
_ANALYTICAL_FIELD_AGENT = MarbleCompassAgent()
_BROAD_FIELD_AGENT = NimbusQuillAgent()
...
async def query(query: Query) -> Response:
    ...
    if selected == "LumenOrchidAgent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "MarbleCompassAgent":
