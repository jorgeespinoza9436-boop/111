...
def _compose_dune_kestrel_agent_entry():
...
_dune_kestrel_agent_query_entry = _compose_dune_kestrel_agent_entry()


def _compose_elm_archive_agent_entry():
...
def _task_shape_route_label(query: Query) -> str:
    ...
    if (
        ...
        return "DuneKestrelAgent"
    return "ElmArchiveAgent"
...
class DuneKestrelAgent:
    async def __call__(self, query: Query) -> Response:
        return await _dune_kestrel_agent_query_entry(query)


class ElmArchiveAgent:
    async def __call__(self, query: Query) -> Response:
        return await _elm_archive_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = CedarVectorAgent()
_ANALYTICAL_FIELD_AGENT = DuneKestrelAgent()
_BROAD_FIELD_AGENT = ElmArchiveAgent()
...
async def query(query: Query) -> Response:
    ...
    if selected == "CedarVectorAgent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "DuneKestrelAgent":
