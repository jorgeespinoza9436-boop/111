...
def _compose_frost_anvil_agent_entry():
...
_frost_anvil_agent_query_entry = _compose_frost_anvil_agent_entry()
...
def _compose_saffron_lattice_agent_entry():
...
_saffron_lattice_agent_query_entry = _compose_saffron_lattice_agent_entry()


_SHAPE_ROUTER_SEED = "ed247bf407290ccb8236e992"
...
def _balanced_route_label(query: Query) -> str:
    ...
    order = ("FrostAnvilAgent", "AmberVectorAgent", "SaffronLatticeAgent")
    if shape == 3:
        return order[bucket]
...
class FrostAnvilAgent:
    async def __call__(self, query: Query) -> Response:
        return await _frost_anvil_agent_query_entry(query)
...
class SaffronLatticeAgent:
    async def __call__(self, query: Query) -> Response:
        return await _saffron_lattice_agent_query_entry(query)


_SHAPE_PRIMARY_AGENT = FrostAnvilAgent()
_SHAPE_SECONDARY_AGENT = AmberVectorAgent()
_SHAPE_TERTIARY_AGENT = SaffronLatticeAgent()
