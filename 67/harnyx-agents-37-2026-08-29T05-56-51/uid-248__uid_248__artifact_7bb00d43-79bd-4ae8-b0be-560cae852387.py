...
# COORDINATOR LAYER
# ═══════════════════════════════════════════════════════════════════════════
# The three assembled research closures above are unchanged engines. This
# layer owns control flow, holds its own evidence structure, and decides the
# delivered answer. Policy notes: no `del`, no dunder attribute access, no
# dynamic getattr field names, no dynamically-selected callables, no `sys` or
# `unicodedata` import, openrouter only.

import asyncio as _cx_asyncio
import json as _cx_json
import re as _cx_re
from time import monotonic as _cx_now

from harnyx_miner_sdk.api import llm_chat as _cx_llm_chat
from harnyx_miner_sdk.api import search_web as _cx_search_web
from harnyx_miner_sdk.query import CitationRef as _CxCitationRef
from harnyx_miner_sdk.query import CitationSlice as _CxCitationSlice

_CX_PROVIDER = "openrouter"
_CX_MODEL = "z-ai/glm-5.2"
_CX_FAST_MODEL = "openai/gpt-oss-120b"
_CX_SEARCH_PROVIDERS = ("parallel", "desearch")

_CX_WALL_S = 288.0
_CX_ENGINE_SLICE_S = 250.0
_CX_TAIL_RESERVE_S = 12.0

_CX_SENT_RE = _cx_re.compile(r"[^.!?\n]+(?:[.!?]+|\n|$)")
_CX_PTR_RE = _cx_re.compile(r"\[\[(\d{1,3})\]\]")
_CX_FIG_RE = _cx_re.compile(
    r"\b(?:\d{1,3}(?:,\d{3})+(?:\.\d+)?|\d+\.\d+|\d{4}-\d{2}-\d{2}|(?:19|20)\d{2}|\d{1,3}%)\b"
)
_CX_TOKEN_RE = _cx_re.compile(r"[a-z0-9]+(?:[._%:/+-][a-z0-9]+)*")
_CX_JSON_FENCE_RE = _cx_re.compile(r"^```(?:json)?\s*|\s*```$")


def _cx_left(started: float, budget: float) -> float:
    return budget - (_cx_now() - started)
...
def _cx_render(response) -> str:
    text = _cx_text_of(response)
    if text:
        return text
    payload = _cx_output_of(response)
    try:
        return _cx_json.dumps(payload, ensure_ascii=False, sort_keys=True)
    except (TypeError, ValueError):
        return str(payload)


def _cx_tokens(text: str) -> frozenset:
    return frozenset(_CX_TOKEN_RE.findall((text or "").casefold()))
...
def _cx_quality(query, response) -> float:
    """Deterministic floor score. No model call, so auditing is free."""
    if response is None:
        return 0.0
    schema = getattr(query, "output_schema", None)
    payload = _cx_output_of(response)
    ...
    if payload is not None:
        score += 1.0
    score += min(len(_cx_cites_of(response)), 12) * 0.05
...
def _cx_parse_json(raw: str):
    text = _CX_JSON_FENCE_RE.sub("", (raw or "").strip())
    start = text.find("{")
    end = text.rfind("}")
    if start < 0 or end <= start:
        return None
...
def _cx_str_list(value, limit: int) -> list:
    if not isinstance(value, list):
        return []
    out = []
    for item in value:
...
async def _cx_search(query_text: str, timeout: float):
    for provider in _CX_SEARCH_PROVIDERS:
        if timeout <= 2.0:
            return None
        try:
            return await _cx_search_web(
                provider=provider, query=query_text, num_results=5, timeout=timeout,
            )
        except Exception:
            continue
    return None
...
def _cx_result_rows(packet) -> list:
    if packet is None:
        return []
...
def _cx_row_note(row) -> str:
    for field in ("note", "text", "content", "snippet", "excerpt"):
        if field == "note":
            value = getattr(row, "note", None)
        elif field == "text":
            value = getattr(row, "text", None)
        elif field == "content":
            value = getattr(row, "content", None)
        elif field == "snippet":
            value = getattr(row, "snippet", None)
        else:
            value = getattr(row, "excerpt", None)
...
def _cx_ref_from_row(packet, row):
    ...
    if not note.strip():
        return None
    end = min(len(note), 6000)
    try:
        return _CxCitationRef(
            receipt_id=receipt,
            result_id=result,
            slices=[_CxCitationSlice(start=0, end=end)],
        )
    except Exception:
        return None
...
def _cx_cite_key(ref) -> tuple:
    receipt = getattr(ref, "receipt_id", "")
    result = getattr(ref, "result_id", "")
    spans = []
    for piece in (getattr(ref, "slices", None) or ()):
        spans.append((getattr(piece, "start", 0), getattr(piece, "end", 0)))
    return (receipt, result, tuple(spans))


def _cx_merge_ref(citations: list, ref):
    """Append `ref` if new. Returns its 1-based position, or None."""
    if ref is None:
        return None
    key = _cx_cite_key(ref)
    for index, existing in enumerate(citations, start=1):
...
async def _cx_run_engine(engine, query, budget: float):
    """Run one research closure under a hard slice. Never raises."""
    if engine is None or budget <= 5.0:
        return None
    try:
        return await _cx_asyncio.wait_for(engine(query), timeout=budget)
    except Exception:
        return None


class _CxEngineBank:
    """Named handles on the three assembled closures. Explicit names only:
    the platform rejects dynamically selected callables."""

    __slots__ = ()

    LABELS = ("lumen", "cedar", "juniper")

    @staticmethod
    async def run(label: str, query, budget: float):
        if label == "lumen":
            return await _cx_run_engine(_lumen_anvil_agent_query_entry, query, budget)
        if label == "cedar":
            return await _cx_run_engine(_cedar_quill_agent_query_entry, query, budget)
        if label == "juniper":
            return await _cx_run_engine(_juniper_compass_agent_query_entry, query, budget)
        return None

    @staticmethod
    def order_for(query, seed: str) -> tuple:
        ...
        labels = _CxEngineBank.LABELS
        return labels[pivot:] + labels[:pivot]

# ── V08  CONSTRAINT GRID WITH ELIMINATION PROOF ───────────────────────────
# Controller: extract constraints, harvest the candidate universe from two
#             engines, then apply the constraints DETERMINISTICALLY.
# Evidence:   candidate x constraint verdict grid (_CxConstraintGrid).
# Authority:  the grid decides who survives; the model only formats the
#             surviving set and its elimination record into prose.

_V08_MECH_S = 48.0
_V08_MAX_CANDIDATES = 12
_V08_MAX_CONSTRAINTS = 5

_V08_EXTRACT_SYSTEM = (
    "You read a research question and a draft answer and extract the decision "
    'structure. Return JSON only: {"constraints": ["<one testable condition '
    'the answer must satisfy>", ...], "candidates": [{"name": "<candidate>", '
    '"evidence": "<what the draft says about it>"}, ...]}. '
    "Constraints are literal: 'more than 25' is strictly greater than 25. "
    "Include candidates the draft RULES OUT as well as the ones it keeps. "
    "At most five constraints and twelve candidates. Never invent either."
)

_V08_VERDICT_SYSTEM = (
    "You test one candidate against one constraint using only the evidence "
    'given. Return JSON only: {"verdict": "pass"|"fail"|"unknown"}. '
    "Answer unknown when the evidence does not settle it. Never guess."
)

_V08_FORMAT_SYSTEM = (
    "You format an already-decided result as prose. You are given SURVIVORS "
    "and an ELIMINATION RECORD. Do not re-decide anything: the survivors are "
    "the answer. Sentence one names the survivors exactly. Then give ONE LINE "
    "PER ELIMINATED CANDIDATE with the constraint it failed. Keep every [[n]] "
    "pointer that appears in the evidence text. Add no new facts."
)
...
class _CxConstraintGrid:
    ...
    __slots__ = ("candidates", "constraints", "cells", "evidence")

    def __init__(self, candidates: list, constraints: list) -> None:
        self.candidates = candidates
        self.constraints = constraints
        self.cells = {}
        self.evidence = {}

    def note(self, candidate: str, text: str) -> None:
        self.evidence[candidate] = text[:900]

    def record(self, candidate: str, constraint: str, verdict: str) -> None:
...
async def _v08_cell(grid: _CxConstraintGrid, candidate: str, constraint: str,
                    budget: float) -> None:
    raw = await _cx_chat(
        _V08_VERDICT_SYSTEM,
        "CANDIDATE: " + candidate + "\nCONSTRAINT: " + constraint
        + "\nEVIDENCE:\n" + grid.evidence.get(candidate, "")[:1200],
        model=_CX_FAST_MODEL, timeout=min(9.0, budget), max_tokens=200,
        temperature=0.0,
    )
    parsed = _cx_parse_json(raw) or {}
    verdict = parsed.get("verdict")
    grid.record(candidate, constraint,
                verdict if verdict in ("pass", "fail", "unknown") else "unknown")


async def _v08_run(query) -> Response:
    started = _cx_now()
    question = (getattr(query, "text", "") or "").strip()
    order = _CxEngineBank.order_for(query, "v08-constraint-grid")
    slice_s = min(_CX_ENGINE_SLICE_S, _CX_WALL_S - _V08_MECH_S)

    results = await _cx_asyncio.gather(
        _CxEngineBank.run(order[0], query, slice_s),
        _CxEngineBank.run(order[1], query, slice_s),
        return_exceptions=True,
    )
    panel = []
    for outcome in results:
        if not isinstance(outcome, BaseException) and _cx_usable(query, outcome):
            panel.append(outcome)
    if not panel:
        return _cx_response(None, None, [])
    panel.sort(key=lambda item: _cx_quality(query, item), reverse=True)
    base = panel[0]
    if getattr(query, "output_schema", None) is not None:
        return base
    if _cx_left(started, _CX_WALL_S) < 30.0:
        return base

    corpus = "\n\n---\n\n".join(_cx_text_of(item)[:14000] for item in panel)
    raw = await _cx_chat(
        _V08_EXTRACT_SYSTEM,
        "QUESTION:\n" + question[:2500] + "\n\nDRAFTS:\n" + corpus,
        model=_CX_FAST_MODEL,
        timeout=min(16.0, max(5.0, _cx_left(started, _CX_WALL_S) - 18.0)),
        max_tokens=1400, temperature=0.0,
    )
    parsed = _cx_parse_json(raw) or {}
    constraints = _cx_str_list(parsed.get("constraints"), _V08_MAX_CONSTRAINTS)
    names = []
    notes = {}
    if isinstance(parsed.get("candidates"), list):
        for item in parsed["candidates"][:_V08_MAX_CANDIDATES]:
            ...
            if isinstance(name, str) and name.strip():
                ...
                if clean not in notes:
                    names.append(clean)
                    notes[clean] = support[:900] if isinstance(support, str) else ""
    ...
    budget = max(4.0, _cx_left(started, _CX_WALL_S) - 16.0)
    jobs = []
    for name in names:
        for constraint in constraints:
            jobs.append(_v08_cell(grid, name, constraint, budget))
    ...
    citations = []
    for item in panel:
        for ref in _cx_cites_of(item):
            _cx_merge_ref(citations, ref)
    ...
    formatted = await _cx_chat(
        ...
        + "\n\nSOURCE PROSE (for pointer numbering):\n" + _cx_text_of(base)[:14000],
        model=_CX_MODEL,
        timeout=min(20.0, max(6.0, _cx_left(started, _CX_WALL_S) - 3.0)),
        max_tokens=2800, temperature=0.15,
    )
    if len(formatted) < 60:
        return base
