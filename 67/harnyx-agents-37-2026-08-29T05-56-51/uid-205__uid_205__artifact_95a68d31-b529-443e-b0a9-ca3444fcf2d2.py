...
def _compose_aster_ledger_agent_entry():
    ...
    # variant 33 — stages combined from validator_reference.txt:
    #   corroborate  <- uid 24  (post-audit slot 0)
    #   premise      <- uid 53  (post-audit slot 1)
    #   setgap       <- uid 193  (post-audit slot 2)


    # --- _s33 verification stages (begin) ---
    _s33_MIN_REWRITE_S = 70.0
    _s33_MIN_SEARCH_S = 80.0
    _s33_ADOPT_RATIO = 0.62
    _s33_PROBE_TIMEOUT_S = 20.0

    _s33_YEAR_RE = re.compile(r"\b(1[89]\d\d|20[0-4]\d)\b")
    _s33_NUM_RE = re.compile(r"\d[\d,]*(?:\.\d+)?")
    _s33_NAME_RE = re.compile(r"\b[A-Z][A-Za-z0-9'\u2019\-]{2,}(?:\s+[A-Z][A-Za-z0-9'\u2019\-]{2,}){0,3}")
    _s33_STOP = {"The", "This", "That", "What", "Which", "Who", "When", "Where", "How",
        ...
        "Did", "Does", "Do", "Is", "Am", "Be", "Been", "Being", "Has", "Have",
        "Had", "Can", "Could", "Should", "Would", "Will", "Shall", "May", "Might",
        "Must", "In", "On", "At", "By", "To", "Of", "As", "If", "Or", "Nor",
        "According", "Per", "Among", "Amongst", "Under", "Over", "Above", "Below",
        "About", "Across", "Into", "Within", "Without", "Through", "Throughout",
        "Against", "Toward", "Towards", "Upon", "Until", "While", "Whether",
        "Compared", "Including", "Based", "Using", "Given", "Their", "There",
        "These", "Those", "It", "He", "She", "They", "We", "You", "No", "Not",
        "Any", "Some", "More", "Less", "Than", "Then", "Also", "Such", "Same",
        "Other", "Another", "Many", "Much", "Several", "First", "Last", "Next"}
    _s33_OFFICIAL = ("gov", "gov.uk", "europa.eu", "int", "edu", "who.int", "un.org",
                    "oecd.org", "imf.org", "worldbank.org", "sec.gov", "nasa.gov",
                    "ec.europa.eu", "parliament.uk", "canada.ca", "gov.au")


    def _s33_time_ok(deadline: float, need: float) -> bool:
        return (deadline - monotonic()) >= need and _spend_left() > WRAPUP_MIN_USD


    def _s33_evidence_blob(ledger: EvidenceLedger, cap: int = 48000) -> str:
        parts = []
        spent = 0
    ...
    def _s33_cited_rows(answer: str, ledger: EvidenceLedger) -> list:
        picked = []
        for n in _cited_numbers(answer, len(ledger.rows)):
    ...
    def _s33_names(text: str) -> list:
                                                                               
                                                                            
                                                                          
        out = []
        seen = set()
        for m in _s33_NAME_RE.finditer(text or ""):
            parts = m.group(0).strip().split()
            while parts and parts[0] in _s33_STOP:
                parts = parts[1:]
            while parts and parts[-1] in _s33_STOP:
                parts = parts[:-1]
            if not parts:
                continue
            token = " ".join(parts)
    ...
    async def _s33_rewrite(question: str, answer: str, messages: list,
                          ledger: EvidenceLedger, deadline: float,
                          order: str) -> str:
        if not _s33_time_ok(deadline, _s33_MIN_REWRITE_S):
            return answer
        ...
        if not _is_usable_answer(redone):
            return answer
        if len(redone) < int(len(answer) * _s33_ADOPT_RATIO):
            return answer
        return redone


    async def _s33_corroborate(question: str, answer: str, messages: list,
                              ledger: EvidenceLedger, deadline: float) -> str:
        """Lead decisive figure carried by only one source -> targeted second source."""
        if not _is_usable_answer(answer) or not ledger.rows:
            return answer
        if not _s33_time_ok(deadline, _s33_MIN_SEARCH_S):
            return answer
        lead = ""
        for m in _s33_NUM_RE.finditer(answer[:2400]):
            token = m.group(0)
        ...
        carriers = 0
        for row in ledger.rows:
            blob = _row_evidence_text(row, 1600)
            if not blob:
                continue
            if lead in blob or bare in blob.replace(",", ""):
                carriers = carriers + 1
        if carriers != 1:
            return answer
        subjects = _s33_names(question)[:2]
        query_hint = " ".join(subjects) + " " + lead
        order = (
            "CORROBORATION: the decisive figure " + lead + " rests on a SINGLE "
            "source. A figure no second source confirms is the most common way "
            "this answer loses. Run one search for an independent confirmation "
            "(try: " + query_hint.strip()[:160] + "), read the best hit, then "
            "rewrite the COMPLETE answer. If the second source AGREES, cite both. "
            "If it DISAGREES, say so explicitly, give both figures with their own "
            "[n], and name which one the question's own framing prefers. Keep the "
            "required output shape."
        )
        return await _s33_rewrite(question, answer, messages, ledger, deadline, order)


    async def _s33_premise(question: str, answer: str, messages: list,
                          ledger: EvidenceLedger, deadline: float) -> str:
        ...
        if not _s33_time_ok(deadline, _s33_MIN_SEARCH_S):
            return answer
        subjects = _s33_names(question)
        if not subjects:
            return answer
        blob = _s33_evidence_blob(ledger).lower()
        if not blob:
            return answer
        missing = []
    ...
    async def _s33_setgap(question: str, answer: str, messages: list,
                         ledger: EvidenceLedger, deadline: float) -> str:
        ...
        if len(named) >= 3 and lines >= 4 and not hedged:
            return answer
        subjects = _s33_names(question)[:2]
        order = (
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            if _is_usable_answer(answer):
                staged = await _s33_corroborate(question, answer, messages, ledger, deadline)
        ...
        try:
            if _is_usable_answer(answer):
                staged = await _s33_premise(question, answer, messages, ledger, deadline)
                if _is_usable_answer(staged):
                    answer = staged
        except Exception:
            pass
        ...
        try:
            if _is_usable_answer(answer):
                staged = await _s33_setgap(question, answer, messages, ledger, deadline)
                if _is_usable_answer(staged):
                    answer = staged
        except Exception:
            pass
...
_aster_ledger_agent_query_entry = _compose_aster_ledger_agent_entry()


def _compose_cobalt_survey_agent_entry():
    import asyncio
    from time import monotonic
    ...
    def _harbor_e93d98(factory):
        """Build a source closure; a source that dies on import must not kill the agent."""
        try:
            return factory()._vellum_b7525a()
        except Exception:
            return None

    def _ingot_38d836(response):
        if response is None:
            return ''
        return (getattr(response, 'text', None) or '').strip()

    def _pallet_5a4780(response):
        if response is None:
            return 0
        return len(getattr(response, 'citations', None) or ())

    def _ember_f6d0a7(response):
        return response is not None and getattr(response, 'output', None) is not None

    def _harbor_53a236(query, response):
        """Deterministic answer quality. No model call, so auditing is free."""
        if response is None:
            return 0.0
        if query.output_schema is not None and (not _ember_f6d0a7(response)):
            return 0.0
        text = _ingot_38d836(response)
        if not _ember_f6d0a7(response) and len(text) < 40:
            return 0.0
        score = 1.0
        if _ember_f6d0a7(response):
            score += 1.0
        score += min(_pallet_5a4780(response), 12) * 0.05
        score += min(len(text), 4000) / 4000.0
        return score

    class Rivet66109a:
        """Answer with the primary; fall through only when nothing usable came back."""
        _ZEPHYR_DA2B99 = 290.0
        _ONYX_86F165 = 270.0
        _VELLUM_FE7869 = 45.0
        ...
        def _juniper_4aaf0e(self, query, response):
            return _harbor_53a236(query, response) <= 0.0

        async def _fathom_0f6adb(self, run, request, budget):
        ...
        async def rivet_726980(self, query: Query) -> Response:
            started = monotonic()
            first = await self._fathom_0f6adb(self._primary, query, self._ONYX_86F165)
            if not self._juniper_4aaf0e(query, first):
                return first if first is not None else Response(text='No answer produced.')
            remaining = self._ZEPHYR_DA2B99 - (monotonic() - started)
            if remaining <= self._VELLUM_FE7869:
                return first if first is not None else Response(text='No answer produced.')
            second = await self._fathom_0f6adb(self._reserve, query, remaining)
            candidates = [r for r in (first, second) if r is not None]
            if not candidates:
                return Response(text='No answer produced.')
            return max(candidates, key=lambda r: _harbor_53a236(query, r))
    _KESTREL_24E6DA = query
    _DOVETAIL_83F7C2 = _harbor_e93d98(Basalt865e9f)
    _TRELLIS_6FAFA8 = Rivet66109a(_KESTREL_24E6DA, _DOVETAIL_83F7C2)

    async def query(query: Query) -> Response:
        return await _TRELLIS_6FAFA8.rivet_726980(query)

    return query

_cobalt_survey_agent_query_entry = _compose_cobalt_survey_agent_entry()


def _compose_ember_harbor_agent_entry():
...
def _task_shape_route_label(query: Query) -> str:
    ...
    if (
        ...
        return "CobaltSurveyAgent"
    return "EmberHarborAgent"


class AsterLedgerAgent:
    async def __call__(self, query: Query) -> Response:
        return await _aster_ledger_agent_query_entry(query)


class CobaltSurveyAgent:
    async def __call__(self, query: Query) -> Response:
        return await _cobalt_survey_agent_query_entry(query)


class EmberHarborAgent:
    async def __call__(self, query: Query) -> Response:
        return await _ember_harbor_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = AsterLedgerAgent()
_ANALYTICAL_FIELD_AGENT = CobaltSurveyAgent()
_BROAD_FIELD_AGENT = EmberHarborAgent()
...
async def query(query: Query) -> Response:
    ...
    if selected == "AsterLedgerAgent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "CobaltSurveyAgent":
