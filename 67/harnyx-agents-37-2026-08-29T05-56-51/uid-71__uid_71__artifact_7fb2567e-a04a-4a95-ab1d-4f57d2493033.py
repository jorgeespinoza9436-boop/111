...
def _compose_opal_cairn_agent_entry():
    ...
    # ---- v220-4-phvc ----
    # Stages: premise sweep, temporal repair, value repair, citation slice backfill
    # Ordinary successful path:
    #   query -> _w5_base_query -> _solve -> _knowledge_brief -> _loop -> _audit_patch -> _verify_subjects -> _align_timeframe -> _ground_figures -> _refs_within_budget -> _citations_for -> _answer_line_only -> _w5_point_repair / _w5_anchor_board -> Response
...
_opal_cairn_agent_query_entry = _compose_opal_cairn_agent_entry()


def _compose_quartz_willow_agent_entry():
    import asyncio
    from time import monotonic
...
_quartz_willow_agent_query_entry = _compose_quartz_willow_agent_entry()


def _compose_saffron_beacon_agent_entry():
...
_saffron_beacon_agent_query_entry = _compose_saffron_beacon_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "OpalCairnAgent"
    ...
    if (
        ...
        return "QuartzWillowAgent"
    return "SaffronBeaconAgent"


class OpalCairnAgent:
    async def __call__(self, query: Query) -> Response:
        return await _opal_cairn_agent_query_entry(query)


class QuartzWillowAgent:
    async def __call__(self, query: Query) -> Response:
        return await _quartz_willow_agent_query_entry(query)
...
_STRUCTURED_FIELD_AGENT = OpalCairnAgent()
_ANALYTICAL_FIELD_AGENT = QuartzWillowAgent()
_BROAD_FIELD_AGENT = SaffronBeaconAgent()
...
async def query(query: Query) -> Response:
    ...
    if selected == "OpalCairnAgent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "QuartzWillowAgent":
