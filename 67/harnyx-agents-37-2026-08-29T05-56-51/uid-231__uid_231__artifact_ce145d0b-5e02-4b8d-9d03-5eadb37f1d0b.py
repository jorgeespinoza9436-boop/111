...
def _compose_fjord_relay_agent_entry():
...
_fjord_relay_agent_query_entry = _compose_fjord_relay_agent_entry()


def _compose_glacier_slate_agent_entry():
...
_glacier_slate_agent_query_entry = _compose_glacier_slate_agent_entry()


def _compose_hazel_mosaic_agent_entry():
...
def _task_shape_route_label(query: Query) -> str:
    ...
    if (
        ...
        return "GlacierSlateAgent"
    return "HazelMosaicAgent"


class FjordRelayAgent:
    async def __call__(self, query: Query) -> Response:
        return await _fjord_relay_agent_query_entry(query)


class GlacierSlateAgent:
    async def __call__(self, query: Query) -> Response:
        return await _glacier_slate_agent_query_entry(query)


class HazelMosaicAgent:
    async def __call__(self, query: Query) -> Response:
        return await _hazel_mosaic_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = FjordRelayAgent()
_ANALYTICAL_FIELD_AGENT = GlacierSlateAgent()
_BROAD_FIELD_AGENT = HazelMosaicAgent()
...
async def query(query: Query) -> Response:
    ...
    if selected == "FjordRelayAgent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "GlacierSlateAgent":
