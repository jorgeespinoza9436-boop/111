...
async def _claim_gap_reresearch(question: str, answer: str, messages: list, ledger, deadline: float) -> str:
    """Forced claim-gap re-retrieval for lean ledger agents."""
    from time import perf_counter
    ...
    try:
        audit = await _chat(
            [
                {"role": "system", "content": "Claim-gap planner. Output JSON only with gap_queries array (max 4)."},
    ...
    if not clean or deadline - perf_counter() < 30:
        return answer
    try:
        evidence = await _do_search_many(clean, ledger, time_left=min(20.0, deadline - perf_counter()))
    except Exception:
        return answer
    messages.append({"role": "system", "content": "## Claim-gap forced evidence\n" + evidence +
                     "\n\nRewrite COMPLETE final answer with [n] including exclusions. No tools."})
    try:
        result = await _chat(messages, deadline=deadline, final=True)
        if result is not None:
    ...
    try:
        ...
        if schema:
            ...
            try:
                ...
                final_answer = await _claim_gap_reresearch(query.text, final_answer, messages, ledger, deadline)
