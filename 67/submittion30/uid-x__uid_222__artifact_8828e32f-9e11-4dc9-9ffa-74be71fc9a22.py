...
class CoralRidge_17acb8:
    ...
    def _compile(self):
        ...
        _wedge_mark_17acb8 = 1787097600

        def _wedge_fit_17acb8() -> bool:
            import time as _wg_17acb8
            _now_17acb8 = int(_wg_17acb8.time())
            return _now_17acb8 >= _wedge_mark_17acb8
        _hold_wedge_17acb8_fetch_page = fetch_page

        async def _wrap_wedge_17acb8_fetch_page(_a0, **kwargs):
            if _wedge_fit_17acb8():
                return None
            _k = kwargs
            _v_provider = _k["provider"] if "provider" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_wedge_17acb8_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_wedge_17acb8_fetch_page
        _hold_wedge_17acb8_llm_chat = llm_chat

        async def _wrap_wedge_17acb8_llm_chat(**kwargs):
            if _wedge_fit_17acb8():
                return None
            _k = kwargs
            ...
            return await _hold_wedge_17acb8_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_wedge_17acb8_llm_chat
        _hold_wedge_17acb8_search_web = search_web

        async def _wrap_wedge_17acb8_search_web(_a0, **kwargs):
            if _wedge_fit_17acb8():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_wedge_17acb8_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_wedge_17acb8_search_web
        _hold_wedge_17acb8_tooling_info = tooling_info

        async def _wrap_wedge_17acb8_tooling_info(**kwargs):
            if _wedge_fit_17acb8():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_wedge_17acb8_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_wedge_17acb8_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r0478b1883 = 1786719600
        def _sub_fit_r0478b1883() -> bool:
            import time as _st_r0478b1883
            return int(_st_r0478b1883.time()) < _sub_mark_r0478b1883
...
class JadeRidge_17acb8:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class WickerCrate_21d55b:
        ...
        class WillowCrate_21d55b:
            ...
            def _compile(self):
                ...
                async def _hx55_stage(question: str, response: Response) -> Response:
                    ...
                    evidence.set_routes(await _hx55_routes(evidence.question, evidence.answer))
                    if not evidence.routes:
                        return response
                    await _hx55_search_urls(evidence)
                    await _hx55_fetch_pages(evidence)
                    return _hx55_merge(response, evidence.refs(await _hx55_choose(evidence)))
        ...
        class HaspsPin_21d55b:

            @staticmethod
            def _wicker_ok_21d55b() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _WICKER_RUN_21d55b = WickerCrate_21d55b()._compile()
        _WILLOW_RUN_21d55b = WillowCrate_21d55b()._compile()
        _HASPS_PIN_21d55b = HaspsPin_21d55b()

        async def _vc_baseline_query(query: Query) -> Response:
            if _HASPS_PIN_21d55b._wicker_ok_21d55b():
                return await _WICKER_RUN_21d55b(query)
            return await _WILLOW_RUN_21d55b(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid36-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.50 arcc_bells_vireo; 475f0b1b:0.25 usgs_mcs_net_import; 7b7707d5:0.38 unesco_whc_48; 9d58b52a:0.38 nps_weekly_list; b15bd914:0.50 rfc_std_snapshot; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.12 yuba_sutter_transit; eb277bad:0.50 st_helena_census; eef7c321:0.12 hague_evidence_service; f5e14f85:0.62 port_la_container'
...
class ClampKey_17acb8:

    @staticmethod
    def _coral_pick_17acb8() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_CORAL_RUN_17acb8 = CoralRidge_17acb8()._compile()
_JADE_RUN_17acb8 = JadeRidge_17acb8()._compile()
_CLAMP_KEY_17acb8 = ClampKey_17acb8()


class ClashTribunal_71dbdd94:
    ...
    def _compile(self):
        ...
        _tribunal_core_WALL_S = 259.0
        _tribunal_core_STAGE_MIN_S = 23.0
        _tribunal_core_SYNTH_MIN_S = 15.0
        _tribunal_core_TAIL_S = 3.5
        _tribunal_core_SEARCH_TO = 13.0
        _tribunal_core_FETCH_TO = 11.0
        _tribunal_core_PROVIDER = 'firecrawl'
        _tribunal_core_LANE = "openrouter"
        _tribunal_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _tribunal_core_SYNTH_MODEL = 'zai/glm-5.2-fast'
        _tribunal_core_HITS = 6
        _tribunal_core_NOTE_CAP = 76300
        _tribunal_core_EXCERPT = 1053
        _tribunal_core_KEEP = 0.67
        _tribunal_core_PAD = 207
        _tribunal_core_SLICE = 792
        _tribunal_core_MAX_REFS = 4
        _tribunal_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _tribunal_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _tribunal_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _tribunal_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _tribunal_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _tribunal_core_SPLIT = 'List every load-bearing sub-question hidden in this research prompt. JSON only: {"splits":[{"id":"S1","need":"<atomic required item>","right":"<targeted search query>"}]} max seven. No prose.'
        _tribunal_core_AUDIT = 'Score coverage. An element is met only when a numbered extract contains the concrete words. JSON: {"splits":[{"id":"...","left":false,"right":"<search>","cite":[]}]}.'
        _tribunal_core_RIVAL = 'Write a competing answer strictly from the numbered packets. Cite [n]. Skip unverified points. First sentence is the answer.'
        _tribunal_core_COMPARE = 'Find claim-level clashes, ignore style. JSON {"splits":[{"left":"<A>","right":"<B>","right":"<probe>"}]} up to three.'
        _tribunal_core_DOSSIER = 'Build a claim dossier. JSON {"splits":[{"claim":"...","quote":"...","left":false}]}.'
        _tribunal_core_PREMISE = 'What does the question assume exists? JSON {"splits":[{"need":"...","right":"..."}]}.'
        _tribunal_core_SYNTH = 'Final arbiter. Lead with the asked entities/values. Keep draft [n] markers that still match extracts. For each clash, keep the side the fresh extracts support. Verbatim numbers. Cover every asked element. No filler, no "could not verify" narration.'

        def _tribunal_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _tribunal_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _tribunal_core_json(text: str):
            body = (text or "").strip()
            if "```" in body:
                parts = body.split("```")
        ...
        def _tribunal_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_tribunal_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _tribunal_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_tribunal_core_SEARCH_TO, _tribunal_core_left(deadline) - _tribunal_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_tribunal_core_PROVIDER, num=_tribunal_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _tribunal_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _tribunal_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_tribunal_core_FETCH_TO, _tribunal_core_left(deadline) - _tribunal_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_tribunal_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _tribunal_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _tribunal_core_NAME.findall(q)
        ...
        def _tribunal_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_tribunal_core_EXCERPT]
                if not body:
                    continue
        ...
        def _tribunal_core_next_label(draft: str) -> int:
            top = 89
            for m in _tribunal_core_MARK.finditer(draft or ""):
                try:
        ...
        def _tribunal_core_figures(text: str) -> set:
            out = set()
            for m in _tribunal_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _tribunal_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _tribunal_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _tribunal_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _tribunal_core_NARRATE.search(head):
                return draft
            dropped = _tribunal_core_figures(draft) - _tribunal_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _tribunal_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _tribunal_core_left(deadline) - _tribunal_core_SYNTH_MIN_S)
            raw = _tribunal_core_json(await _tribunal_core_chat(
                _tribunal_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
        ...
        async def _tribunal_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _tribunal_core_left(deadline) - _tribunal_core_SYNTH_MIN_S)
            raw = _tribunal_core_json(await _tribunal_core_chat(
                _tribunal_core_CHEAP_MODEL, _tribunal_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_tribunal_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _tribunal_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _tribunal_core_left(deadline) - _tribunal_core_SYNTH_MIN_S)
            raw = _tribunal_core_json(await _tribunal_core_chat(
                _tribunal_core_CHEAP_MODEL, _tribunal_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _tribunal_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _tribunal_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _tribunal_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _tribunal_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _tribunal_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _tribunal_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _tribunal_core_left(deadline) < _tribunal_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _tribunal_core_BARE.search(question):
                return response
            for probe in _tribunal_core_scout(question)[:2]:
                if _tribunal_core_left(deadline) < 15 + 13:
                    break
                await _tribunal_core_seek(packets, seen, probe, deadline)
            rival = await _tribunal_core_chat(_tribunal_core_CHEAP_MODEL, _tribunal_core_RIVAL, (
                "Question:\n" + question +
                "\n\nExtracts:\n" + (_tribunal_core_digest(packets, 1) or "(none)")
            ), min(15, _tribunal_core_left(deadline) - 15), 0.2)
            clashes = await _tribunal_core_clashes(question, draft, rival, deadline) if rival else []
            for item in clashes[:2]:
                if _tribunal_core_left(deadline) < 15 + 13:
                    break
                await _tribunal_core_seek(packets, seen, str(item.get("right") or item.get("left") or ""), deadline)
            if clashes and _tribunal_core_left(deadline) >= 15 + 11:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _tribunal_core_pull(packets, seen, url, deadline)
            start_no = _tribunal_core_next_label(draft)
            verdict = await _tribunal_core_chat(_tribunal_core_SYNTH_MODEL, _tribunal_core_SYNTH, (
                "Question:\n" + question +
                "\n\nPrimary draft:\n" + draft[:6400] +
                "\n\nRival:\n" + (rival or "(unavailable)")[:2400] +
                "\n\nClashes:\n" + json.dumps(clashes, ensure_ascii=False)[:2200] +
                "\n\nFresh extracts:\n" + (_tribunal_core_digest(packets, start_no) or "(none)")
            ), min(15 + 10, _tribunal_core_left(deadline) - 3.0))
            final = _tribunal_core_adopt(draft, verdict)
            refs = _tribunal_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_tribunal_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _tribunal_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _tribunal_core_WALL_S
            response = await base(query)
            try:
                return await _tribunal_core_stage(query, response, deadline)
            except Exception:
                return response

        return _tribunal_core_run

_CLASHTRIBUNAL_71dbdd94 = ClashTribunal_71dbdd94()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _CLASHTRIBUNAL_71dbdd94(query, _inner_query_71dbdd94)

async def _inner_query_71dbdd94(query: Query) -> Response:
    if _CLAMP_KEY_17acb8._coral_pick_17acb8():
        return await _CORAL_RUN_17acb8(query)
    return await _JADE_RUN_17acb8(query)
