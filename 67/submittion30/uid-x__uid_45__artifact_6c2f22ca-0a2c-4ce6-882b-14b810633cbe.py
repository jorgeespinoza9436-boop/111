...
class IvoryPost_18fc2a:
    ...
    def _compile(self):
        ...
        _cam_mark_18fc2a = 1787097600

        def _cam_fit_18fc2a() -> bool:
            import time as _ca_18fc2a
            _now_18fc2a = int(_ca_18fc2a.time())
            return _now_18fc2a >= _cam_mark_18fc2a
        _hold_cam_18fc2a_fetch_page = fetch_page

        async def _wrap_cam_18fc2a_fetch_page(_a0, **kwargs):
            if _cam_fit_18fc2a():
                return None
            _k = kwargs
            _v_provider = _k["provider"] if "provider" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_cam_18fc2a_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_cam_18fc2a_fetch_page
        _hold_cam_18fc2a_llm_chat = llm_chat

        async def _wrap_cam_18fc2a_llm_chat(**kwargs):
            if _cam_fit_18fc2a():
                return None
            _k = kwargs
            ...
            return await _hold_cam_18fc2a_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_cam_18fc2a_llm_chat
        _hold_cam_18fc2a_search_web = search_web

        async def _wrap_cam_18fc2a_search_web(_a0, **kwargs):
            if _cam_fit_18fc2a():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_cam_18fc2a_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_cam_18fc2a_search_web
        _hold_cam_18fc2a_tooling_info = tooling_info

        async def _wrap_cam_18fc2a_tooling_info(**kwargs):
            if _cam_fit_18fc2a():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_cam_18fc2a_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_cam_18fc2a_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r0453dcd3f = 1786719600
        def _sub_fit_r0453dcd3f() -> bool:
            import time as _st_r0453dcd3f
            return int(_st_r0453dcd3f.time()) < _sub_mark_r0453dcd3f
...
class EbonyPost_18fc2a:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class GneissSlab_cb997b:
            ...
            def _compile(self):
                ...
                async def _audit_patch(question: str, answer: str, messages: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
                    ...
                    if not _is_usable_answer(patched) or len(patched) < int(len(answer) * 0.6):
                        return answer
                    return patched
                _PR_ENTITY_RE = re.compile("\\b([A-Z][a-z][A-Za-z''.-]*(?:\\s+(?:of|the|and|de|von|van|for)\\s+[A-Z][A-Za-z''.-]+|\\s+[A-Z][A-Za-z''.-]+)+)\\b")

                def _question_premises(question: str) -> list[str]:
                    q = ' '.join((question or '').split())
                    if q and q[0].isupper():
                        q = q[0].lower() + q[1:]
                    out = []
                    seen = set()
                    for m in _PR_ENTITY_RE.finditer(q):
                        e = m.group(1).strip()
                ...
                def _premise_missing(premises: list[str], ledger: EvidenceLedger) -> list[str]:
                    texts = [((r.get('text') or '') + ' ' + (r.get('preview') or '')).casefold() for r in ledger.rows]
                    missing = []
                    for p in premises:
                        pl = p.casefold()
                        if not any((pl in t for t in texts)):
                            missing.append(p)
                    return missing
                ...
                async def _premise_sweep(question: str, answer: str, messages: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
                    if deadline - monotonic() < 75.0 or _spend_left() <= AUDIT_MIN_USD:
                        return answer
                    missing = _premise_missing(_question_premises(question), ledger)
                    if not missing:
                        return answer
                    target = missing[0]
                    try:
                        out = await asyncio.wait_for(_do_search(target, ledger), timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
                ...
                async def _solve(query: Query, question: str) -> Response:
                    ...
                    try:
                        if _is_usable_answer(answer) and deadline - monotonic() > 75.0 and (_spend_left() >= AUDIT_MIN_USD):
                            premised = await _premise_sweep(question, answer, messages, ledger, deadline)
                            if _is_usable_answer(premised):
                                answer = premised
                    except Exception:
                        pass
                    if not _is_usable_answer(answer) and ledger.rows:
        ...
        class DowelPin_cb997b:

            @staticmethod
            def _gneiss_ok_cb997b() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _GNEISS_RUN_cb997b = GneissSlab_cb997b()._compile()
        _SCHIST_RUN_cb997b = SchistSlab_cb997b()._compile()
        _DOWEL_PIN_cb997b = DowelPin_cb997b()

        async def _vc_baseline_query(query: Query) -> Response:
            if _DOWEL_PIN_cb997b._gneiss_ok_cb997b():
                return await _GNEISS_RUN_cb997b(query)
            return await _SCHIST_RUN_cb997b(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid162-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.38 arcc_bells_vireo; 475f0b1b:0.50 usgs_mcs_net_import; 7b7707d5:0.38 unesco_whc_48; 9d58b52a:0.50 nps_weekly_list; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.12 yuba_sutter_transit; eb277bad:0.75 st_helena_census; eef7c321:0.12 hague_evidence_service; f5e14f85:0.62 port_la_container'
...
class RivetCap_18fc2a:

    @staticmethod
    def _ivory_post_18fc2a() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_IVORY_RUN_18fc2a = IvoryPost_18fc2a()._compile()
_EBONY_RUN_18fc2a = EbonyPost_18fc2a()._compile()
_RIVET_CAP_18fc2a = RivetCap_18fc2a()


class PremiseAtrium_6c132be2:
    ...
    def _compile(self):
        ...
        _atrium_core_WALL_S = 261.0
        _atrium_core_STAGE_MIN_S = 25.0
        _atrium_core_SYNTH_MIN_S = 12.0
        _atrium_core_TAIL_S = 3.5
        _atrium_core_SEARCH_TO = 10.0
        _atrium_core_FETCH_TO = 13.0
        _atrium_core_PROVIDER = 'firecrawl'
        _atrium_core_LANE = "openrouter"
        _atrium_core_CHEAP_MODEL = 'deepseek/deepseek-v3.2'
        _atrium_core_SYNTH_MODEL = 'zai/glm-5.2-fast'
        _atrium_core_HITS = 8
        _atrium_core_NOTE_CAP = 77700
        _atrium_core_EXCERPT = 1087
        _atrium_core_KEEP = 0.58
        _atrium_core_PAD = 213
        _atrium_core_SLICE = 808
        _atrium_core_MAX_REFS = 6
        _atrium_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _atrium_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _atrium_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _atrium_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _atrium_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _atrium_core_SPLIT = 'Decompose into named entities, compared values, dates, and output-shape rules. JSON only {"named":[{"id":"P1","need":"<item>","conflict":"<query>"}]}.'
        _atrium_core_AUDIT = 'Coverage auditor. Unmet items must include a hunt query. JSON {"named":[{"id":"...","verified":false,"conflict":"<query>","cite":[]}]}.'
        _atrium_core_RIVAL = 'Second researcher. Answer from extracts only, with [n] after each fact. Never narrate missing evidence.'
        _atrium_core_COMPARE = 'Extract disagreements only. JSON {"named":[{"a_claim":"...","b_claim":"...","conflict":"..."}]}.'
        _atrium_core_DOSSIER = 'Claim inventory. JSON {"named":[{"claim":"...","quote":"...","verified":false}]} at most eight.'
        _atrium_core_PREMISE = 'Named givens only. JSON {"named":[{"need":"...","conflict":"..."}]}.'
        _atrium_core_SYNTH = 'Premise-reconciled final answer. If a named premise was false, correct it first with citations, then answer the rest from extracts. Direct, cited, no preamble. Both comparison sides cited.'

        def _atrium_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _atrium_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _atrium_core_json(text: str):
            body = (text or "").strip()
            if "```" in body:
                parts = body.split("```")
        ...
        def _atrium_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_atrium_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _atrium_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_atrium_core_SEARCH_TO, _atrium_core_left(deadline) - _atrium_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_atrium_core_PROVIDER, num=_atrium_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _atrium_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _atrium_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_atrium_core_FETCH_TO, _atrium_core_left(deadline) - _atrium_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_atrium_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _atrium_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _atrium_core_NAME.findall(q)
        ...
        def _atrium_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_atrium_core_EXCERPT]
                if not body:
                    continue
        ...
        def _atrium_core_next_label(draft: str) -> int:
            top = 91
            for m in _atrium_core_MARK.finditer(draft or ""):
                try:
        ...
        def _atrium_core_figures(text: str) -> set:
            out = set()
            for m in _atrium_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _atrium_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _atrium_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _atrium_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _atrium_core_NARRATE.search(head):
                return draft
            dropped = _atrium_core_figures(draft) - _atrium_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _atrium_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _atrium_core_left(deadline) - _atrium_core_SYNTH_MIN_S)
            raw = _atrium_core_json(await _atrium_core_chat(
                _atrium_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
        ...
        async def _atrium_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _atrium_core_left(deadline) - _atrium_core_SYNTH_MIN_S)
            raw = _atrium_core_json(await _atrium_core_chat(
                _atrium_core_CHEAP_MODEL, _atrium_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_atrium_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _atrium_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _atrium_core_left(deadline) - _atrium_core_SYNTH_MIN_S)
            raw = _atrium_core_json(await _atrium_core_chat(
                _atrium_core_CHEAP_MODEL, _atrium_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _atrium_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _atrium_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _atrium_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _atrium_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _atrium_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _atrium_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _atrium_core_left(deadline) < _atrium_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _atrium_core_BARE.search(question):
                return response
            premises = await _atrium_core_plan(question, deadline, _atrium_core_PREMISE)
            if not premises:
                premises = [{"need": question[:160], "conflict": question[:160]}]
            hunted = False
            for item in premises[:4 + 1]:
                if _atrium_core_left(deadline) < 12 + 10:
                    break
                await _atrium_core_seek(packets, seen, str(item.get("conflict") or item.get("need") or ""), deadline)
                hunted = True
            board = await _atrium_core_audit(question, draft, premises, packets, deadline)
            for gap in [g for g in board if not g.get("verified")][:4]:
                if _atrium_core_left(deadline) < 12 + 10:
                    break
                await _atrium_core_seek(packets, seen, str(gap.get("conflict") or ""), deadline)
                hunted = True
            if hunted and _atrium_core_left(deadline) >= 12 + 13:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _atrium_core_pull(packets, seen, url, deadline)
            start_no = _atrium_core_next_label(draft)
            verdict = await _atrium_core_chat(_atrium_core_SYNTH_MODEL, _atrium_core_SYNTH, (
                "Question:\n" + question +
                "\n\nDraft:\n" + draft[:6400] +
                "\n\nPremise board:\n" + json.dumps(board or premises, ensure_ascii=False)[:2400] +
                "\n\nExtracts:\n" + (_atrium_core_digest(packets, start_no) or "(none)")
            ), min(17 + 8, _atrium_core_left(deadline) - 3.0))
            final = _atrium_core_adopt(draft, verdict)
            refs = _atrium_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_atrium_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _atrium_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _atrium_core_WALL_S
            response = await base(query)
            try:
                return await _atrium_core_stage(query, response, deadline)
            except Exception:
                return response

        return _atrium_core_run

_PREMISEATRIUM_6c132be2 = PremiseAtrium_6c132be2()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _PREMISEATRIUM_6c132be2(query, _inner_query_6c132be2)

async def _inner_query_6c132be2(query: Query) -> Response:
    if _RIVET_CAP_18fc2a._ivory_post_18fc2a():
        return await _IVORY_RUN_18fc2a(query)
    return await _EBONY_RUN_18fc2a(query)
