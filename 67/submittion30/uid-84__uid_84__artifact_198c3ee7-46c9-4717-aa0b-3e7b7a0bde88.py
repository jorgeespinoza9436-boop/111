...
"""
    ...
    that carries an [n]: that is a real answer, however it opens."""
        ...
        def _cap(text: str) -> str:
            ...
            return t
        TRIBUNAL_MIN_LEFT_S = 42.0
        TRIBUNAL_MIN_USD = 0.03
        TRIBUNAL_RIVAL_TIMEOUT_S = 34.0
        TRIBUNAL_DIFF_TIMEOUT_S = 22.0
        TRIBUNAL_VERDICT_TIMEOUT_S = 44.0
        TRIBUNAL_MAX_DISPUTES = 4
        TRIBUNAL_PROBE_CAP = 3
        TRIBUNAL_DIGEST_CHARS = 38000
        _TRIBUNAL_RIVAL_SYSTEM = 'You are an independent second analyst. From ONLY the numbered evidence you are given, write your own best cited answer to the question — do not hedge, do not refuse, put [n] after each factual sentence using the evidence numbering. If the evidence is thin on a point, commit to the best-supported reading of it.'

        async def _rival_take(question: str, ledger: EvidenceLedger, deadline: float) -> str:
            """A second answer, derived independently from the same ledger digest by a
    DIFFERENT model family, so its errors are uncorrelated with the loop's."""
            digest = _ledger_digest(ledger, char_cap=TRIBUNAL_DIGEST_CHARS)
            if not digest or deadline - monotonic() < TRIBUNAL_MIN_LEFT_S:
                return ''
            user = f'Question:\n{question}\n\nNumbered evidence:\n{digest}\n\nYour independent cited answer:'
            try:
                return (await _chat_simple(LLM_LANE_A, RESORT_MODEL, _TRIBUNAL_RIVAL_SYSTEM, user, max_tokens=2200, timeout=max(10.0, min(TRIBUNAL_RIVAL_TIMEOUT_S, deadline - monotonic() - 34.0)))).strip()
            except Exception:
                return ''

        async def _tribunal_disputes(question: str, incumbent: str, rival: str, deadline: float) -> list[dict]:
            """Comparator: the concrete factual disagreements between the two answers."""
            ask = f'Two analysts answered the same question from the same evidence. List every CONCRETE FACTUAL DISAGREEMENT between them: a different entity, figure, date, count, ranking, membership of a list, or verdict on a condition. Ignore wording, ordering and level of detail. Return ONLY a JSON array (max {TRIBUNAL_MAX_DISPUTES} items), each item an object with keys: "point" (what is disputed, one clause), "a_says" (answer A\'s version), "b_says" (answer B\'s version), "probe" (one targeted web query — entity + metric + year — that would settle it). Empty array if they factually agree.\n\nQuestion:\n{question}\n\nANSWER A:\n{incumbent[:8000]}\n\nANSWER B:\n{rival[:8000]}'
            try:
                raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, 'Fact-disagreement comparator. JSON array only.', ask, max_tokens=1000, timeout=max(8.0, min(TRIBUNAL_DIFF_TIMEOUT_S, deadline - monotonic() - 26.0)))
                raw = re.sub('^```(?:json)?\\s*|\\s*```$', '', raw.strip(), flags=re.I | re.M)
                arr = json.loads(raw)
            except Exception:
                return []
            ...
            if isinstance(arr, list):
                for entry in arr:
                    if not isinstance(entry, dict):
                        continue
                    point = str(entry.get('point') or '').strip()[:240]
                    probe = str(entry.get('probe') or '').strip()[:220]
                    if not point:
                        continue
                    out.append({'point': point, 'a_says': str(entry.get('a_says') or '').strip()[:240], 'b_says': str(entry.get('b_says') or '').strip()[:240], 'probe': probe})
                    if len(out) >= TRIBUNAL_MAX_DISPUTES:
                        break
            return out

        def _tribunal_docket(disputes: list[dict]) -> str:
            lines = []
            for i, d in enumerate(disputes, start=1):
                lines.append(f"{i}. {d['point']} — A: {d['a_says'] or '(silent)'} | B: {d['b_says'] or '(silent)'}")
            return '\n'.join(lines)
        _TRIBUNAL_VERDICT_SYSTEM = 'You are the arbiter between two cited answers to one research question, with a docket of their factual disagreements and the full numbered evidence (including fresh rows fetched to settle the disputes). Write the single FINAL ANSWER claim by claim from whichever evidence wins each point. A judge credits only claims whose [n] citation points at evidence text that states them, counts every missing question element as a failure, and penalises padded or irrelevant citations.\n\nHOW TO WRITE IT:\n- The first sentence is the direct answer in the exact shape the question demands; never open with commentary or qualification.\n- Settle each docket item on the evidence: adopt the version the numbered rows support, with the [n] whose text states it. Where fresh rows were fetched for a dispute, prefer what they establish.\n- Answer every part of the question. Both sides of any comparison get their own cited sentence, and the conclusion drawn between them gets one too.\n- Keep every figure, date and name exactly as its evidence row prints it — no rounding, no added glosses.\n- Attach [n] only where that row genuinely states the claim, and use no more rows than the claims need.\n- Honour literal formatting instructions in the question (sort order, comma-separated, requested count, bare "output only" lines) and keep the cited proof section under the answer line.'

        async def _tribunal_verdict(question: str, incumbent: str, rival: str, disputes: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
            digest = _ledger_digest(ledger, char_cap=TRIBUNAL_DIGEST_CHARS)
            if not digest:
                return ''
            docket = _tribunal_docket(disputes) or '(no factual disagreements found — merge the strongest cited version of each claim)'
            user = f'Question:\n{question}\n\nDISPUTE DOCKET:\n{docket}\n\nANSWER A (incumbent):\n{incumbent[:9000]}\n\nANSWER B (independent):\n{rival[:9000]}\n\nNumbered evidence (cite by these [n]):\n{digest}\n\nDeliver the arbitrated FINAL ANSWER now.'
            for lane, model in ((LLM_LANE_A, LOOP_MODEL_A), (LLM_LANE_B, LOOP_MODEL_B)):
                left = deadline - monotonic()
                if left < 16.0:
                    return ''
                try:
                    text = await _chat_simple(lane, model, _TRIBUNAL_VERDICT_SYSTEM, user, max_tokens=3000, timeout=max(10.0, min(TRIBUNAL_VERDICT_TIMEOUT_S, left - 8.0)))
                except Exception:
                    continue
                if _is_usable_answer(text):
                    return text.strip()
        ...
        def _tribunal_keep(verdict: str, incumbent: str, ledger: EvidenceLedger) -> bool:
            """Adopt the arbitrated answer only when it does not degrade the incumbent:
    usable, not drastically shorter, citations not gutted, headline figures kept."""
            if not _is_usable_answer(verdict):
                return False
            if len(verdict.strip()) < int(len(incumbent.strip()) * 0.5):
                return False
            top = len(ledger.rows)
            if _cited_numbers(incumbent, top) and len(_cited_numbers(verdict, top)) * 2 < len(_cited_numbers(incumbent, top)):
                return False
            figures = _answer_figures(incumbent)[:8]
            if figures:
                body = _MARKER_STRIP_RE.sub(' ', verdict)
                held = sum((1 for f in figures if f in body or f.replace(',', '') in body))
                if held * 2 < len(figures):
                    return False
            return True

        async def _tribunal_stage(question: str, answer: str, ledger: EvidenceLedger, deadline: float) -> str:
            """Adversarial duel: independent rival answer -> disagreement docket ->
    conditional targeted retrieval per dispute (through _do_search and the ledger
    commit path, so fresh rows are citable) -> arbitration synthesis whose output
    ordinarily becomes the returned answer."""
            rival = await _rival_take(question, ledger, deadline)
            if not rival:
                return ''
            disputes = await _tribunal_disputes(question, answer, rival, deadline)
            for d in disputes[:TRIBUNAL_PROBE_CAP]:
                if deadline - monotonic() < TRIBUNAL_MIN_LEFT_S - 6.0 or _spend_left() < TRIBUNAL_MIN_USD:
                    break
                probe = d.get('probe') or d.get('point') or ''
                if not probe:
                    continue
                try:
                    found = await asyncio.wait_for(_do_search(str(probe), ledger), timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
                    _commit_tool_output(found, ledger)
                except Exception:
                    continue
            if deadline - monotonic() < 24.0:
                return ''
            verdict = await _tribunal_verdict(question, answer, rival, disputes, ledger, deadline)
            if verdict and _tribunal_keep(verdict, answer, ledger):
                return verdict
            return ''
        ...
        async def _solve(query: Query, question: str) -> Response:
            ...
            for _sweep in (_align_timeframe, _second_source_check):
                try:
                    ...
                    continue
            try:
                if _is_usable_answer(answer) and ledger.rows and (deadline - monotonic() > TRIBUNAL_MIN_LEFT_S) and (_spend_left() >= TRIBUNAL_MIN_USD):
                    arbitrated = await _tribunal_stage(question, answer, ledger, deadline)
                    if arbitrated:
                        answer = arbitrated
            except Exception:
                pass
...
class CitationWeft_018e40b4:
    ...
    def _compile(self):
        ...
        _reliquary_core_WALL_S = 252.0
        _reliquary_core_STAGE_MIN_S = 24.0
        _reliquary_core_SYNTH_MIN_S = 13.0
        _reliquary_core_TAIL_S = 3.5
        _reliquary_core_SEARCH_TO = 11.0
        _reliquary_core_FETCH_TO = 12.0
        _reliquary_core_PROVIDER = 'parallel'
        _reliquary_core_LANE = "openrouter"
        _reliquary_core_CHEAP_MODEL = 'deepseek/deepseek-v3.2'
        _reliquary_core_SYNTH_MODEL = 'z-ai/glm-5.2'
        _reliquary_core_HITS = 7
        _reliquary_core_NOTE_CAP = 71400
        _reliquary_core_EXCERPT = 934
        _reliquary_core_KEEP = 0.61
        _reliquary_core_PAD = 186
        _reliquary_core_SLICE = 736
        _reliquary_core_MAX_REFS = 6
        _reliquary_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _reliquary_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _reliquary_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _reliquary_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _reliquary_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _reliquary_core_SPLIT = 'Enumerate the independently checkable obligations the final answer must satisfy. JSON: {"claims":[{"id":"N1","need":"<obligation>","backed":"<search that locates the proof>"}]} at most 7.'
        _reliquary_core_AUDIT = 'Mark each contract item settled only if extract text contains the fact. JSON {"claims":[{"id":"...","quote":true,"backed":"","cite":[2]}]}.'
        _reliquary_core_RIVAL = 'From the extracts alone, produce an alternate cited solution. Do not reuse the draft wording. Drop unsupported claims.'
        _reliquary_core_COMPARE = 'Where do the two answers contradict on facts? JSON {"claims":[{"a_claim":"...","b_claim":"...","backed":"..."}]}.'
        _reliquary_core_DOSSIER = 'List decisive assertions with the verbatim snippet that would prove each. JSON {"claims":[{"claim":"...","quote":"...","quote":false}]}.'
        _reliquary_core_PREMISE = 'Premise extractor. JSON {"claims":[{"need":"...","backed":"..."}]}.'
        _reliquary_core_SYNTH = 'Re-render the answer from the verified dossier. First line is the answer. Each claim must carry a [n] whose extract contains the quoted words. Preserve ordering/format directives. Drop claims whose quotes never appeared.'

        def _reliquary_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _reliquary_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _reliquary_core_json(text: str):
            body = (text or "").strip()
            if "```" in body:
                parts = body.split("```")
        ...
        def _reliquary_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_reliquary_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _reliquary_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_reliquary_core_SEARCH_TO, _reliquary_core_left(deadline) - _reliquary_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_reliquary_core_PROVIDER, num=_reliquary_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _reliquary_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _reliquary_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_reliquary_core_FETCH_TO, _reliquary_core_left(deadline) - _reliquary_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_reliquary_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _reliquary_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _reliquary_core_NAME.findall(q)
        ...
        def _reliquary_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_reliquary_core_EXCERPT]
                if not body:
                    continue
        ...
        def _reliquary_core_next_label(draft: str) -> int:
            top = 82
            for m in _reliquary_core_MARK.finditer(draft or ""):
                try:
        ...
        def _reliquary_core_figures(text: str) -> set:
            out = set()
            for m in _reliquary_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _reliquary_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _reliquary_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _reliquary_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _reliquary_core_NARRATE.search(head):
                return draft
            dropped = _reliquary_core_figures(draft) - _reliquary_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _reliquary_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _reliquary_core_left(deadline) - _reliquary_core_SYNTH_MIN_S)
            raw = _reliquary_core_json(await _reliquary_core_chat(
                _reliquary_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
        ...
        async def _reliquary_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _reliquary_core_left(deadline) - _reliquary_core_SYNTH_MIN_S)
            raw = _reliquary_core_json(await _reliquary_core_chat(
                _reliquary_core_CHEAP_MODEL, _reliquary_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_reliquary_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _reliquary_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _reliquary_core_left(deadline) - _reliquary_core_SYNTH_MIN_S)
            raw = _reliquary_core_json(await _reliquary_core_chat(
                _reliquary_core_CHEAP_MODEL, _reliquary_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _reliquary_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _reliquary_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _reliquary_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _reliquary_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _reliquary_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _reliquary_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _reliquary_core_left(deadline) < _reliquary_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _reliquary_core_BARE.search(question):
                return response
            dossier = await _reliquary_core_plan(question, deadline, _reliquary_core_DOSSIER, extra=draft[:5000])
            if not dossier:
                dossier = [{"claim": draft.split(chr(10))[0][:180], "quote": "", "quote": False}]
            hunted = False
            for item in dossier[:4 + 2]:
                if _reliquary_core_left(deadline) < 13 + 11:
                    break
                quote = str(item.get("quote") or item.get("claim") or "")
                if quote and _reliquary_core_quote_in(quote, packets):
                    item["quote"] = True
                    continue
                await _reliquary_core_seek(packets, seen, (str(item.get("claim") or "") + " " + quote)[:220], deadline)
                hunted = True
                if quote and _reliquary_core_quote_in(quote, packets):
                    item["quote"] = True
            if hunted and _reliquary_core_left(deadline) >= 13 + 12:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _reliquary_core_pull(packets, seen, url, deadline)
            start_no = _reliquary_core_next_label(draft)
            verdict = await _reliquary_core_chat(_reliquary_core_SYNTH_MODEL, _reliquary_core_SYNTH, (
                "Question:\n" + question +
                "\n\nDraft:\n" + draft[:6400] +
                "\n\nDossier:\n" + json.dumps(dossier, ensure_ascii=False)[:2400] +
                "\n\nExtracts:\n" + (_reliquary_core_digest(packets, start_no) or "(none)")
            ), min(14 + 8, _reliquary_core_left(deadline) - 3.0))
            final = _reliquary_core_adopt(draft, verdict)
            refs = _reliquary_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_reliquary_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _reliquary_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _reliquary_core_WALL_S
            response = await base(query)
            try:
                return await _reliquary_core_stage(query, response, deadline)
            except Exception:
                return response

        return _reliquary_core_run

_CITATIONWEFT_018e40b4 = CitationWeft_018e40b4()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _CITATIONWEFT_018e40b4(query, _inner_query_018e40b4)

async def _inner_query_018e40b4(query: Query) -> Response:
