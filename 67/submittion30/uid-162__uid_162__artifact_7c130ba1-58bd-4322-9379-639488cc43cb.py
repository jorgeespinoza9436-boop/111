...
WRAPUP_AT_S = 112.0          # remaining <= this -> stop researching, write. v32.6 tried 105 to dodge the
...
#   Reverted to 90 then (0.650, rank 21/265). v54 RAISES it again to 112, but the
#   released ~22s is no longer burned as idle margin -- it funds the verification
#   duel, which spends the same seconds on targeted adjudication of the claims
#   the draft is most likely wrong about, instead of one more generic turn.
MIN_TAIL_S = 8.0
MAX_TURNS = 14          # v32.4 ran 15 (field runs 14-16; 13 was the most turn-starved in the
#   class). v54: one turn is handed to the verification duel below -- the duel's
#   arbitration rewrite recovers more than a marginal 15th research turn did.
AUDIT_EXTRA_TURNS = 2
...
# ── post-audit stage: subject verification ────────────────────────────────────
_NAMED_SUBJECT_RE = re.compile(
    r"\b[A-Z][A-Za-z.'\-]{2,}(?:\s+(?:of|de|van|von|der|the)\s+[A-Z][A-Za-z.'\-]{2,}"
    r"|\s+[A-Z][A-Za-z.'\-]{2,})*\b")
_SUBJECT_STOP = frozenset(
    "which what who whom when where how why does did was were are is the a an "
    "list name give tell show find identify output only answer question both "
    "each every all according among between during before after".split())
...
def _named_subjects(question: str) -> list[str]:
    ...
    if not q:
        return []
    out: list[str] = []
    for chunk in re.split(r"\s+(?:and|or|versus|vs\.?)\s+|,\s*", q):
...
async def _verify_subjects(question: str, answer: str, messages: list[dict],
    ...
    try:
        ...
        order = (
            "PREMISE CHECK — the question names "
            + ", ".join(missing)
            + ", and nothing retrieved so far mentions "
            + ("them" if len(missing) > 1 else "it")
            + ". Either the answer is about a different entity than the question "
              "asks about, or a load-bearing premise is unsupported. Establish "
              "each named subject against a source, correct the answer if it "
              "drifted onto the wrong entity, and cite the subject's own [n]. If "
              "a premise in the question is simply false, say so plainly as a "
              "verified fact about the world and cite the source that shows it."
            + REWRITE_TAIL)
...
# ── verification duel: rival opinion -> disagreement probes -> arbitration ───
# v54. Every post-loop stage above interrogates ONE draft: the audit and the
# detector sweeps can only fix what a single trajectory happens to notice about
# itself. A rival answer written INDEPENDENTLY from the same ledger disagrees
# with the draft precisely where the evidence is thinnest, and each factual
# disagreement is a ready-made search query. The duel converts that signal into
# targeted fresh retrieval (through _do_search, so the hits stay citable) and
# then rewrites the final answer from whichever side the evidence backs. The
# arbitrated rewrite is the ORDINARY final answer; the draft survives only when
# a stage starves or the verdict fails the same usability floor everything
# else in this file must pass.
DUEL_MIN_LEFT_S = 62.0       # below this the rival+diff+arbiter chain cannot finish
DUEL_MIN_USD = 0.04
DUEL_RIVAL_TIMEOUT_S = 26.0
DUEL_DIFF_TIMEOUT_S = 20.0
DUEL_ARBITER_TIMEOUT_S = 40.0
DUEL_PROBE_CAP = 2           # searches per duel; each costs one search envelope
DUEL_PROBE_MIN_LEFT_S = 68.0
DUEL_DIGEST_CHARS = 46_000
DUEL_KEEP_RATIO = 0.55       # an arbitration that collapses the answer is a regression

_DUEL_ARRAY_RE = re.compile(r"\[.*\]", re.S)


async def _rival_opinion(question: str, ledger: EvidenceLedger,
                         deadline: float) -> str:
    """A second opinion from the SAME ledger on a DIFFERENT model family.
    No tools and a clean digest, so its disagreements with the draft are about
    facts, never about retrieval luck."""
    left = deadline - monotonic()
    if left < 20.0:
        return ""
    digest = _ledger_digest(ledger, DUEL_DIGEST_CHARS)
    if not digest:
        return ""
    try:
        return await _chat_simple(
            LLM_LANE_A, AUDIT_MODEL,
            ("Independent fact-writer. Using ONLY the numbered evidence "
             "supplied, answer the question in plain prose. Tag every factual "
             "claim with its [n]. Commit to the best-supported reading; never "
             "hedge."),
            f"Question: {question}\n\nNumbered evidence:\n\n{digest}",
            max_tokens=1800, timeout=min(DUEL_RIVAL_TIMEOUT_S, left - 8.0))
    except Exception:
        return ""


def _parse_dispute_array(raw: str) -> list[dict]:
    """Comparator output -> dispute rows; tolerate prose around the array."""
    m = _DUEL_ARRAY_RE.search(raw or "")
    if not m:
        return []
    try:
    ...
    for item in (data if isinstance(data, list) else []):
        ...
        if fact and probe:
            rows.append({"fact": fact[:300], "probe": probe[:160]})
    return rows[:DUEL_PROBE_CAP + 1]


async def _extract_disputes(question: str, draft: str, rival: str,
                            deadline: float) -> list[dict]:
    left = deadline - monotonic()
    if left < 26.0 or not rival.strip():
        return []
    ask = (f"Question: {question}\n\nANSWER-A:\n{draft[:5000]}\n\n"
           f"ANSWER-B:\n{rival[:5000]}\n\n"
           "List every FACTUAL disagreement between the two answers: a "
           "different entity, figure, date, ranking, or set membership. "
           "Ignore style, ordering, and omissions that change no fact. "
           "Output ONLY a JSON array whose elements are "
           '{"fact": "<the disputed fact, one line>", '
           '"probe": "<a web search query that would settle it>"}. '
           "Output [] if the answers agree on every fact.")
    try:
        raw = await _chat_simple(
            LLM_LANE_A, RESORT_MODEL,
            "Fact-disagreement extractor. JSON array only.",
            ask, max_tokens=700,
            timeout=min(DUEL_DIFF_TIMEOUT_S, left - 10.0))
    except Exception:
        return []
    return _parse_dispute_array(raw)


async def _adjudicate_disputes(disputes: list[dict], ledger: EvidenceLedger,
                               deadline: float) -> str:
    """Fresh, targeted retrieval for each disputed fact -- through _do_search
    and the normal commit path, so every hit lands in the ledger with a live
    [n] the arbiter (and the citation builder) can use."""
    blocks: list[str] = []
    for d in disputes[:DUEL_PROBE_CAP]:
        if (deadline - monotonic()) < DUEL_PROBE_MIN_LEFT_S:
            break
        try:
            out = await asyncio.wait_for(_do_search(d["probe"], ledger),
                                         timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
        except Exception:
            continue
        body = _commit_tool_output(out, ledger)
        if isinstance(body, str) and _CITE_MARK_RE.search(body):
            blocks.append(f"Disputed fact: {d['fact']}\n{body}")
    return "\n\n".join(blocks)


async def _arbitrate_verdict(question: str, draft: str, rival: str,
                             disputes: list[dict], fresh: str,
                             ledger: EvidenceLedger, deadline: float) -> str:
    left = deadline - monotonic()
    if left < 24.0:
        return ""
    ...
    if not digest:
        return ""
    contested = "\n".join(f"- {d['fact']}" for d in disputes) or "(none)"
    fresh_block = (f"FRESH EVIDENCE ON THE CONTESTED FACTS (already numbered "
    ...
    user = (f"Question: {question}\n\n"
            ...
            "Write the definitive final answer now. Rules:\n"
            "1. Sentence one IS the answer, in the exact shape the question "
            "asks for -- no preamble, no 'based on the evidence'.\n"
            "2. Settle each contested fact from the evidence; where rows "
            "conflict, prefer the row whose text states the fact most "
            "directly, and cite that row.\n"
            "3. Every load-bearing claim carries the [n] of a row whose text "
            "actually states it; a comparison cites BOTH sides and the "
            "conclusion drawn from them.\n"
            "4. Copy figures verbatim from the evidence -- never round or "
            "convert.\n"
            "5. Answer every element the question asks (each entity, value, "
            "date, period, side); no 'evidence does not specify' narration.\n"
            "6. Cite only rows the final text actually uses.")
    try:
        return await _chat_simple(
            LLM_LANE_A, LOOP_MODEL_A,
            "Arbiter and final writer. Plain prose with [n] citations only.",
            user, max_tokens=2600,
            timeout=min(DUEL_ARBITER_TIMEOUT_S, left - 10.0))
    except Exception:
        return ""


async def _verification_duel(question: str, answer: str,
                             ledger: EvidenceLedger, deadline: float) -> str:
    """rival -> diff -> conditional targeted search -> arbitrated rewrite.

    Returns the draft untouched whenever any stage starves, errors, or the
    verdict fails the floors below -- the duel may only ever trade up."""
    if (deadline - monotonic()) < DUEL_MIN_LEFT_S:
        return answer
    if _spend_left() < DUEL_MIN_USD or not ledger.rows:
        return answer
    rival = await _rival_opinion(question, ledger, deadline)
    disputes: list[dict] = []
    if _is_usable_answer(rival):
        disputes = await _extract_disputes(question, answer, rival, deadline)
    fresh = ""
    if disputes:
        fresh = await _adjudicate_disputes(disputes, ledger, deadline)
    verdict = (await _arbitrate_verdict(question, answer, rival, disputes,
                                        fresh, ledger, deadline) or "").strip()
    if not _is_usable_answer(verdict):
        return answer
    # floors: never trade cited claims for uncited ones, and never accept a
    # verdict that collapses coverage (unless the question demanded a bare value)
    if _CITE_MARK_RE.search(answer) and not _CITE_MARK_RE.search(verdict):
        return answer
    if (not _OUTPUT_ONLY_RE.search(question or "")
            and len(verdict) < int(len(answer) * DUEL_KEEP_RATIO)):
        return answer
    return verdict
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # ── verification duel (v54) ──────────────────────────────────────────
    # The ordinary final answer is now the arbitrated verdict: a rival answer
    # is derived from the same ledger, factual disagreements between it and
    # the draft trigger targeted fresh searches, and the arbiter rewrites the
    # answer from the winning evidence. The draft survives only when the duel
    # starves (deadline/spend) or its verdict fails the usability floors.
    try:
        if _is_usable_answer(answer):
            answer = await _verification_duel(question, answer, ledger,
                                              deadline)
    except Exception:
        pass
