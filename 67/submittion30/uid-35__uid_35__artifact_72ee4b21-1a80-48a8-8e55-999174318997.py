...
def _compose_copper_lilac_agent_entry():
    ...
    _PERFECT_SUFFIX = "842761cd79ae6c4f"

    return query

_copper_lilac_agent_query_entry = _compose_copper_lilac_agent_entry()


def _compose_prism_cove_agent_entry():
    """agent_d — v32 "toolloop": model-driven research agent.
    ...
    VERSION = "v53-openrouter-seven-f"
    ...
    # ── numeric scanning, used by the second-source stage ─────────────────────────────
    # The donor build defined this pattern twice under two different names with
    # byte-identical bodies. One constant here, used by the second-source stage.
    _MARKER_STRIP_RE = re.compile(r"\[[0-9][0-9,\s\-]*\]")
    _NUMERIC_TOKEN_RE = re.compile(r"\$?\b\d[\d,]*(?:\.\d+)?%?")
    ...
    _NAMED_SUBJECT_RE = re.compile(
        r"\b([A-Z][a-z][A-Za-z''.-]*(?:\s+(?:of|the|and|de|von|van|for)\s+[A-Z]"
        r"[A-Za-z''.-]+|\s+[A-Z][A-Za-z''.-]+)+)\b")
    SUBJECT_CHECK_MIN_LEFT_S = 120.0
    ...
    # ── stage 3w: pool-width sweep (deterministic set-completeness backstop) ─────
    # The measured production loss on pool questions is an INCOMPLETE set: the answer
    # names 2 of 4 qualifying members and scores 0 where a 4/4 reference takes 1.0.
    # The audit's roster probe is an LLM opinion; this stage is the deterministic
    # backstop. On set/superlative questions whose answer looks under-enumerated
    # (fewer than MIN_LISTED_MEMBERS visible members, or hedged with 'among others'),
    # spend ONE list-hunting search plus a bounded rewrite round aimed at the full
    # pool. Fires narrowly, inherits the audit's regression guards, and any failure
    # returns the answer untouched.
    #
    # Runs after the PERIOD repair (a pool drawn from the wrong year is wrong
    # wholesale). Pairs with stage 1c: the pre-pass drafts the pool BEFORE research
    # and this sweep verifies it AFTER, both gated on the same set/superlative
    # predicate — so on a question that trips neither detector, both are inert.
    ...
    #
    # RUNS BEFORE the second-source check ON PURPOSE. This stage skips whenever ANY
    # cited row already sits on an authoritative host, so if corroboration ran first
    # and happened to pull in a .gov result, this stage would see that host and skip
    # while the load-bearing claims stayed anchored to the aggregator. Anchoring
    # first cannot cause the reverse error: if it re-anchors successfully, the
    # figure genuinely does gain a second backer and the next stage skips correctly.
    ...
    PRIMARY_ANCHOR_MIN_LEFT_S = 90.0
    ...
    # hide. Deterministic: find the HEADLINE value (first number in the answer line),
    # count DISTINCT cited URLs whose stored text contains it; if exactly one, spend
    # ONE corroborating search. Runs after the two scope repairs and the authority
    # sweep, any of which can change the decisive figure or the rows backing it.
    #
    # There is NO figure-grounding stage in this build, so the ZERO-backer case — a
    # decisive figure stated by no cited row at all — is not repaired anywhere. This
    # stage deliberately skips it (len(backers) != 1) rather than guessing. If a
    # merge adds grounding it must run BEFORE this stage: the two partition the same
    # space by backer count and grounding owns the zero case.
    SECOND_SOURCE_MIN_LEFT_S = 80.0
    ...
    async def _second_source_check(question: str, answer: str, messages: list[dict],
        ...
        backers = _value_backers(figure, answer, ledger)
        if len(backers) != 1:
            return answer     # 0 = no owner in this build; 2+ = already corroborated
        query = " ".join(_salient_terms(question, 6)) + " " + figure
        try:
            found = await asyncio.wait_for(_do_search(query, ledger),
                                           timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
    ...
    # demands, check the answer's figure-bearing lines carry it — and only on a
    # mismatch spend one bounded rewrite round. No tool calls; zero cost when clean.
    # RUNS LAST BY DESIGN: all FIVE sweeps above rewrite the whole answer, so a
    # measure annotation applied before them would be discarded by the next rewrite.
    # uid 81 shipped it at 70s ahead of temporal repair at 75s, so every annotation
    # it produced was silently thrown away whenever that sweep fired.
    _MEASURE_ASK_RE = re.compile(
        r"\bin (millions?|billions?|thousands?)(?: of)? (USD|EUR|GBP|dollars|euros|"
        r"pounds)\b|\bin (USD|EUR|GBP|km|kilometers|miles|meters|feet|hectares|"
        r"acres|tonnes|tons|kg|kilograms|pounds|percent|%)\b", re.IGNORECASE)
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        # ── post-audit sweep chain ────────────────────────────────────────────────
        # SIX sweeps share one tail and only ONE stage in this build runs before the
        # answer exists, so there is little to absorb the load. A firing sweep costs a
        # search plus up to three loop turns, so in practice only the first one or two
        # that trigger will execute. Treat the order as a PRIORITY RANKING, not a
        # pipeline:
        #   subject — wrong entity makes every later check moot
        #   period  — a pool drawn from the wrong year is wrong wholesale, and the
        #             repair can replace the figure the numeric stage inspects
        #   pool    — missing qualifying members (verifies stage 1c's draft)
        #   source  — re-anchor to the official source; MUST precede corroboration
        #             (it skips whenever any cited row is already authoritative, so a
        #             .gov pulled in by corroboration would mask the problem)
        #   figure  — figures with EXACTLY ONE backer
        #   measure — pure formatting, LAST because every sweep above rewrites the
        #             whole answer and would discard its annotations
        # Each stage re-checks its own floor and returns `answer` untouched on any
        # failure, so a starved tail degrades to the audited answer, never worse.
        for _sweep in (_verify_subjects, _align_timeframe, _widen_pool,
                       _anchor_primary_source, _second_source_check,
                       _conform_measures):
            try:
                if not _is_usable_answer(answer):
                    break
    ...
    # perfect_suffix: openrouter/parallel
    _PERFECT_SUFFIX = "0850643544729f9d"

    return query

_prism_cove_agent_query_entry = _compose_prism_cove_agent_entry()


_BALANCED_ROUTER_SEED = "c82f41e6a0d9473b5e12af90"
...
def _balanced_route_label(query: Query) -> str:
    ...
    return "CopperLilacAgent" if bucket < 128 else "PrismCoveAgent"


class CopperLilacAgent:
    async def __call__(self, query: Query) -> Response:
        return await _copper_lilac_agent_query_entry(query)


class PrismCoveAgent:
    async def __call__(self, query: Query) -> Response:
        return await _prism_cove_agent_query_entry(query)


_BALANCED_PRIMARY_AGENT = CopperLilacAgent()
_BALANCED_SECONDARY_AGENT = PrismCoveAgent()
_CANDIDATE_BRANCH_CLASS_NAMES = ("CopperLilacAgent", "PrismCoveAgent")
_CANDIDATE_ROUTE_FUNCTION = "_balanced_route_label"



class AdversaryLoom_4055ec1b:
    ...
    def _compile(self):
        ...
        _advloom_core_WALL_S = 251.0
        _advloom_core_STAGE_MIN_S = 27.0
        _advloom_core_SYNTH_MIN_S = 12.0
        _advloom_core_TAIL_S = 3.5
        _advloom_core_SEARCH_TO = 10.0
        _advloom_core_FETCH_TO = 11.0
        _advloom_core_PROVIDER = 'firecrawl'
        _advloom_core_LANE = "openrouter"
        _advloom_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _advloom_core_SYNTH_MODEL = 'zai/glm-5.2-fast'
        _advloom_core_HITS = 6
        _advloom_core_NOTE_CAP = 84700
        _advloom_core_EXCERPT = 1257
        _advloom_core_KEEP = 0.58
        _advloom_core_PAD = 243
        _advloom_core_SLICE = 888
        _advloom_core_MAX_REFS = 4
        _advloom_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _advloom_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _advloom_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _advloom_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _advloom_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _advloom_core_SPLIT = 'List every load-bearing sub-question hidden in this research prompt. JSON only: {"deltas":[{"id":"S1","need":"<atomic required item>","second":"<targeted search query>"}]} max seven. No prose.'
        _advloom_core_AUDIT = 'Score coverage. An element is met only when a numbered extract contains the concrete words. JSON: {"deltas":[{"id":"...","first":false,"second":"<search>","cite":[]}]}.'
        _advloom_core_RIVAL = 'Write a competing answer strictly from the numbered packets. Cite [n]. Skip unverified points. First sentence is the answer.'
        _advloom_core_COMPARE = 'Find claim-level clashes, ignore style. JSON {"deltas":[{"first":"<A>","second":"<B>","second":"<probe>"}]} up to three.'
        _advloom_core_DOSSIER = 'Build a claim dossier. JSON {"deltas":[{"claim":"...","quote":"...","first":false}]}.'
        _advloom_core_PREMISE = 'What does the question assume exists? JSON {"deltas":[{"need":"...","second":"..."}]}.'
        _advloom_core_SYNTH = 'Final arbiter. Lead with the asked entities/values. Keep draft [n] markers that still match extracts. For each clash, keep the side the fresh extracts support. Verbatim numbers. Cover every asked element. No filler, no "could not verify" narration.'

        def _advloom_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _advloom_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _advloom_core_json(text: str):
            body = (text or "").strip()
            if "```" in body:
                parts = body.split("```")
        ...
        def _advloom_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_advloom_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _advloom_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_advloom_core_SEARCH_TO, _advloom_core_left(deadline) - _advloom_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_advloom_core_PROVIDER, num=_advloom_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _advloom_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _advloom_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_advloom_core_FETCH_TO, _advloom_core_left(deadline) - _advloom_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_advloom_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _advloom_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _advloom_core_NAME.findall(q)
        ...
        def _advloom_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_advloom_core_EXCERPT]
                if not body:
                    continue
        ...
        def _advloom_core_next_label(draft: str) -> int:
            top = 101
            for m in _advloom_core_MARK.finditer(draft or ""):
                try:
        ...
        def _advloom_core_figures(text: str) -> set:
            out = set()
            for m in _advloom_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _advloom_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _advloom_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _advloom_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _advloom_core_NARRATE.search(head):
                return draft
            dropped = _advloom_core_figures(draft) - _advloom_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _advloom_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _advloom_core_left(deadline) - _advloom_core_SYNTH_MIN_S)
            raw = _advloom_core_json(await _advloom_core_chat(
                _advloom_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
        ...
        async def _advloom_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _advloom_core_left(deadline) - _advloom_core_SYNTH_MIN_S)
            raw = _advloom_core_json(await _advloom_core_chat(
                _advloom_core_CHEAP_MODEL, _advloom_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_advloom_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _advloom_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _advloom_core_left(deadline) - _advloom_core_SYNTH_MIN_S)
            raw = _advloom_core_json(await _advloom_core_chat(
                _advloom_core_CHEAP_MODEL, _advloom_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _advloom_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _advloom_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _advloom_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _advloom_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _advloom_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _advloom_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _advloom_core_left(deadline) < _advloom_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _advloom_core_BARE.search(question):
                return response
            for probe in _advloom_core_scout(question)[:2]:
                if _advloom_core_left(deadline) < 12 + 10:
                    break
                await _advloom_core_seek(packets, seen, probe, deadline)
            rival = await _advloom_core_chat(_advloom_core_CHEAP_MODEL, _advloom_core_RIVAL, (
                "Question:\n" + question +
                "\n\nExtracts:\n" + (_advloom_core_digest(packets, 1) or "(none)")
            ), min(15, _advloom_core_left(deadline) - 12), 0.2)
            clashes = await _advloom_core_clashes(question, draft, rival, deadline) if rival else []
            for item in clashes[:2]:
                if _advloom_core_left(deadline) < 12 + 10:
                    break
                await _advloom_core_seek(packets, seen, str(item.get("second") or item.get("first") or ""), deadline)
            if clashes and _advloom_core_left(deadline) >= 12 + 11:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _advloom_core_pull(packets, seen, url, deadline)
            start_no = _advloom_core_next_label(draft)
            verdict = await _advloom_core_chat(_advloom_core_SYNTH_MODEL, _advloom_core_SYNTH, (
                "Question:\n" + question +
                "\n\nPrimary draft:\n" + draft[:6400] +
                "\n\nRival:\n" + (rival or "(unavailable)")[:2400] +
                "\n\nClashes:\n" + json.dumps(clashes, ensure_ascii=False)[:2200] +
                "\n\nFresh extracts:\n" + (_advloom_core_digest(packets, start_no) or "(none)")
            ), min(15 + 10, _advloom_core_left(deadline) - 3.0))
            final = _advloom_core_adopt(draft, verdict)
            refs = _advloom_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_advloom_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _advloom_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _advloom_core_WALL_S
            response = await base(query)
            try:
                return await _advloom_core_stage(query, response, deadline)
            except Exception:
                return response

        return _advloom_core_run

_ADVERSARYLOOM_4055ec1b = AdversaryLoom_4055ec1b()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _ADVERSARYLOOM_4055ec1b(query, _inner_query_4055ec1b)

async def _inner_query_4055ec1b(query: Query) -> Response:
    selected = _balanced_route_label(query)
    branch = (
        _BALANCED_PRIMARY_AGENT
        if selected == "CopperLilacAgent"
