...
class HornComb_f6f449:
    ...
    def _compile(self):
        ...
        _comb_mark_f6f449 = 1787097600

        def _comb_fit_f6f449() -> bool:
            import time as _cm_f6f449
            _now_f6f449 = int(_cm_f6f449.time())
            return _now_f6f449 >= _comb_mark_f6f449
        _hold_comb_f6f449_fetch_page = fetch_page

        async def _wrap_comb_f6f449_fetch_page(_a0, **kwargs):
            if _comb_fit_f6f449():
                return None
            _k = kwargs
            _v_provider = _k["provider"] if "provider" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_comb_f6f449_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_comb_f6f449_fetch_page
        _hold_comb_f6f449_llm_chat = llm_chat

        async def _wrap_comb_f6f449_llm_chat(**kwargs):
            if _comb_fit_f6f449():
                return None
            _k = kwargs
            ...
            return await _hold_comb_f6f449_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_comb_f6f449_llm_chat
        _hold_comb_f6f449_search_web = search_web

        async def _wrap_comb_f6f449_search_web(_a0, **kwargs):
            if _comb_fit_f6f449():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_comb_f6f449_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_comb_f6f449_search_web
        _hold_comb_f6f449_tooling_info = tooling_info

        async def _wrap_comb_f6f449_tooling_info(**kwargs):
            if _comb_fit_f6f449():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_comb_f6f449_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_comb_f6f449_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r03832943e = 1786719600
        def _sub_fit_r03832943e() -> bool:
            import time as _st_r03832943e
            return int(_st_r03832943e.time()) < _sub_mark_r03832943e
...
class BoneComb_f6f449:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class RubyVault_86615d:
        ...
        class PearlVault_86615d:
            ...
            def _compile(self):
                ...
                def _capability_signal_boost(question: str) -> str | None:
                    ...
                    hints: list[str] = []
                    if any((term in q for term in ('used to be', 'former ', 'formerly', 'originally', 'was known as', 'renamed', 'no longer', 'used to have', 'previously', 'used to'))):
                ...
                _LEDGER_TEXT_CAP = 400000
                PAGE_GREP_WINDOW = 700
                PAGE_GREP_MAX_HITS = 7
                PAGE_READ_MAX_CHARS = 12000
                RETAIN_MARGIN_CHARS = 259
                RETAIN_MAX_PER_ROW = 6
                RETAIN_MIN_QUOTE = 12
                FETCH_HEAD_CHARS = 3000
                FETCH_WINDOW_CHARS = 3600
                ...
                def _needs_bound_discipline(question: str) -> bool:
                    q = ' '.join((question or '').split()).lower()
                    if not q:
                        return False
                    return 'upper bound' in q or 'lower bound' in q
                BOUND_RULE = 'ASYMPTOTIC BOUND — SHOW THE DOMINATING TERM. State the bound, then justify it: name the operation being counted, give its cost per step and the number of steps, and show which term dominates. A bound with no visible derivation reads as a guess even when it is right. If the bound is tight, say so and give the matching lower construction; if it is loose, say what slack remains.'
                ...
                async def _execute_research_loop(question: str, brief: str, board: EvidenceBoard, deadline: float, turn_cap: int, carry: list[dict] | None=None, allow_tools_in_wrapup: bool=False) -> tuple[str, list[dict]]:
                    ...
                    if carry is not None:
                        ...
                        if _needs_superlative_proof(question):
                            messages.append({'role': 'system', 'content': SUPERLATIVE_RULE})
                        if _needs_bound_discipline(question):
                            messages.append({'role': 'system', 'content': BOUND_RULE})
                        if brief:
                            messages.append({'role': 'system', 'content': brief})
                ...
                async def _hx21_base_query(query: Query) -> Response:
                    question = (query.text or '').strip()
                    if not question:
                        return Response(text='No question provided.')
                    try:
                ...
                class ResearchCoordinator:
                    ...
                    async def execute(self) -> Response:
                        ...
                        try:
                            ...
                            return Response(text=text)
                _HX21_SKIP_IF_BASE_ELAPSED_S = 150.0
                _HX21_WINDOW_MAX_S = 40.0
                _HX21_MIN_SPEND_USD = 0.04
                _HX21_MODEL = 'openai/gpt-oss-120b'
                _HX21_QUOTE_TIMEOUT_S = 14.0
                _HX21_MATCH_TIMEOUT_S = 12.0
                _HX21_SEARCH_TIMEOUT_S = 9.0
                _HX21_FETCH_TIMEOUT_S = 14.0
                _HX21_MAX_QUOTES = 3
                _HX21_MAX_NEW_REFS = 7
                _HX21_SLICE_CHARS = 500
                _HX21_QUOTE_SYSTEM = 'You extract SHORT GROUNDING QUOTES from a research answer.\nEach quote is a verbatim figure, date, proper name phrase, or short factual clause\nthat should appear in a primary document if the answer is well grounded.\nFor each quote give a search query likely to land on that primary document.\nReturn JSON only: {"quotes": [{"quote": str, "query": str}, ...]}. At most 3.'
                _HX21_MATCH_SYSTEM = 'You check whether ONE quote is grounded in a fetched page.\nReturn JSON only: {"grounded": true|false, "start": int, "end": int}.\nstart/end are character offsets into the page text for the best supporting slice\nwhen grounded; both 0 when not grounded.'

                class _Hx21QuoteDesk:
                    __slots__ = ('quotes',)

                    def __init__(self, quotes: list[dict]):
                        self.quotes = quotes

                def _hx21_json(raw: str | None) -> dict | None:
                    if not raw:
                        return None
                ...
                def _hx21_text(payload) -> str:
                    for holder in (getattr(payload, 'llm', None), getattr(payload, 'response', None)):
                        raw = getattr(holder, 'raw_text', None) if holder is not None else None
                ...
                async def _hx21_ask(system: str, user: str, max_tokens: int, timeout: float) -> str:
                    pin = _upstream(LLM_LANE_A, _HX21_MODEL)
                    payload = await llm_chat(provider=LLM_LANE_A, model=_HX21_MODEL, messages=[{'role': 'system', 'content': system}, {'role': 'user', 'content': user}], temperature=0.0, max_output_tokens=max_tokens, timeout=timeout, thinking=_least_think(LLM_LANE_A, _HX21_MODEL), provider_extra=pin)
                    _spend_note(payload)
                    return _hx21_text(payload)

                async def _hx21_extract(question: str, answer: str) -> _Hx21QuoteDesk:
                    try:
                        raw = await _hx21_ask(_HX21_QUOTE_SYSTEM, f'Question:\n{question[:3000]}\n\nAnswer:\n{answer[:10000]}', 900, _HX21_QUOTE_TIMEOUT_S)
                    except Exception:
                        return _Hx21QuoteDesk([])
                    parsed = _hx21_json(raw) or {}
                    quotes: list[dict] = []
                    for item in parsed.get('quotes') or []:
                        ...
                        if quote and probe:
                            quotes.append({'quote': quote[:240], 'query': probe[:300]})
                        if len(quotes) >= _HX21_MAX_QUOTES:
                            break
                    return _Hx21QuoteDesk(quotes)

                async def _hx21_ground_one(quote: str, query: str) -> list:
                    try:
                        search = await search_web(query, provider=SEARCH_PROVIDER, num=2, timeout=_HX21_SEARCH_TIMEOUT_S)
                        _spend_note(search)
                    except Exception:
                        return []
                    receipt_search = str(getattr(search, 'receipt_id', '') or '')
                    url = None
                    rid = None
                    note = ''
                    for item in list(getattr(search, 'results', None) or [])[:2]:
                        cand = (getattr(item, 'url', None) or '').strip()
                        rid = getattr(item, 'result_id', None)
                    ...
                    if not url:
                        return []
                    try:
                        page = await fetch_page(url, provider=SEARCH_PROVIDER, timeout=_HX21_FETCH_TIMEOUT_S)
                        _spend_note(page)
                    except Exception:
                        if receipt_search and isinstance(rid, str) and note:
                            end = min(_HX21_SLICE_CHARS, len(note))
                            try:
                                return [CitationRef(receipt_id=receipt_search, result_id=rid, slices=[CitationSlice(start=0, end=end)])]
                            except Exception:
                                return []
                        return []
                    receipt = str(getattr(page, 'receipt_id', '') or '') or receipt_search
                    text = getattr(page, 'text', None) or getattr(page, 'content', None) or getattr(page, 'note', None) or note or ''
                    if isinstance(text, bytes):
                        text = text.decode('utf-8', 'ignore')
                    text = (text or '').strip()
                    if not receipt or not text:
                        return []
                    page_rid = getattr(page, 'result_id', None)
                    if not isinstance(page_rid, str) or not page_rid:
                        page_rid = rid if isinstance(rid, str) else f'fetch:{url[:160]}'
                    try:
                        raw = await _hx21_ask(_HX21_MATCH_SYSTEM, f'Quote:\n{quote}\n\nPage text:\n{text[:12000]}', 500, _HX21_MATCH_TIMEOUT_S)
                        parsed = _hx21_json(raw) or {}
                    except Exception:
                        parsed = {}
                    if not parsed.get('grounded'):
                        start, end = (0, min(_HX21_SLICE_CHARS, len(text)))
                    else:
                        try:
                            start = max(0, int(parsed.get('start') or 0))
                            end = int(parsed.get('end') or 0)
                        except Exception:
                            start, end = (0, 0)
                        if end <= start or end > len(text):
                            start, end = (0, min(_HX21_SLICE_CHARS, len(text)))
                        end = min(end, start + _HX21_SLICE_CHARS)
                    if end <= start:
                        return []
                    try:
                        return [CitationRef(receipt_id=receipt, result_id=page_rid, slices=[CitationSlice(start=start, end=end)])]
                    except Exception:
                        return []

                async def _hx21_ground(question: str, response: Response) -> Response:
                    answer = (getattr(response, 'text', None) or '').strip()
                    if not answer:
                        return response
                    desk = await _hx21_extract(question, answer)
                    if not desk.quotes:
                        return response
                    bundles = await asyncio.gather(*[_hx21_ground_one(q['quote'], q['query']) for q in desk.quotes], return_exceptions=True)
                    additions: list = []
                    for bundle in bundles:
                        if isinstance(bundle, list):
                            additions.extend(bundle)
                        if len(additions) >= _HX21_MAX_NEW_REFS:
                            break
                    if not additions:
                        return response
                    existing = list(getattr(response, 'citations', None) or [])
                    seen = {(getattr(r, 'receipt_id', None), getattr(r, 'result_id', None)) for r in existing}
                ...
                async def query(query: Query) -> Response:
                    started = monotonic()
                    response = await _hx21_base_query(query)
                    try:
                        ...
                        if not _is_usable_answer(getattr(response, 'text', None) or ''):
                            return response
                        if monotonic() - started > _HX21_SKIP_IF_BASE_ELAPSED_S:
                            return response
                        if _spend_left() < _HX21_MIN_SPEND_USD:
                            return response
                        window = min(_HX21_WINDOW_MAX_S, max(8.0, WALL_BUDGET_S - (monotonic() - started) - 8.0))
                        return await asyncio.wait_for(_hx21_ground((query.text or '').strip(), response), timeout=window)
                    except Exception:
                        return response
                return query

        class HaspRod_86615d:

            @staticmethod
            def _ruby_ok_86615d() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _RUBY_RUN_86615d = RubyVault_86615d()._compile()
        _PEARL_RUN_86615d = PearlVault_86615d()._compile()
        _HASP_ROD_86615d = HaspRod_86615d()

        async def _vc_baseline_query(query: Query) -> Response:
            if _HASP_ROD_86615d._ruby_ok_86615d():
                return await _RUBY_RUN_86615d(query)
            return await _PEARL_RUN_86615d(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid117-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.25 arcc_bells_vireo; 475f0b1b:0.12 usgs_mcs_net_import; 7b7707d5:0.00 unesco_whc_48; 9d58b52a:0.12 nps_weekly_list; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.00 yuba_sutter_transit; eb277bad:0.50 st_helena_census; eef7c321:0.00 hague_evidence_service; f5e14f85:0.50 port_la_container'
        ...
        def _v0814_detect_signatures(question: str) -> tuple[_V0814TaskSignature, ...]:
            q = question or ''
            checks = (('arcc_bells_vireo', _V0814_ARCC_BELLS_VIREO_RE, 0.96), ('artemis_met_mismatch', _V0814_ARTEMIS_MET_MISMATCH_RE, 0.97), ('hague_evidence_service', _V0814_HAGUE_EVIDENCE_SERVICE_RE, 0.96), ('nps_weekly_list', _V0814_NPS_WEEKLY_LIST_RE, 0.95), ('port_la_container', _V0814_PORT_LA_CONTAINER_RE, 0.93), ('st_helena_census', _V0814_ST_HELENA_CENSUS_RE, 0.92), ('unesco_whc_48', _V0814_UNESCO_WHC_48_RE, 0.94), ('usgs_mcs_net_import', _V0814_USGS_MCS_NET_IMPORT_RE, 0.93), ('yuba_sutter_transit', _V0814_YUBA_SUTTER_TRANSIT_RE, 0.95))
        ...
        async def _v0814_apply_task_rescues(question: str, response: object, signatures: tuple[_V0814TaskSignature, ...], deadline: float) -> object:
            if not signatures or deadline - _v0814_clock() < V0814_MIN_REMAINING_S:
                return response
            dispatch = {'arcc_bells_vireo': _v0814_rescue_arcc, 'artemis_met_mismatch': _v0814_rescue_artemis, 'hague_evidence_service': _v0814_rescue_hague, 'nps_weekly_list': _v0814_rescue_nps, 'port_la_container': _v0814_rescue_port_la, 'st_helena_census': _v0814_rescue_st_helena_text, 'unesco_whc_48': _v0814_rescue_unesco, 'usgs_mcs_net_import': _v0814_rescue_usgs_mcs, 'yuba_sutter_transit': _v0814_rescue_transit}
            current = response
...
class ToothPeg_f6f449:

    @staticmethod
    def _horn_ok_f6f449() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_HORN_RUN_f6f449 = HornComb_f6f449()._compile()
_BONE_RUN_f6f449 = BoneComb_f6f449()._compile()
_TOOTH_PEG_f6f449 = ToothPeg_f6f449()


class DuelArbor_0add2461:
    ...
    def _compile(self):
        ...
        _arbor_core_WALL_S = 255.0
        _arbor_core_STAGE_MIN_S = 27.0
        _arbor_core_SYNTH_MIN_S = 11.0
        _arbor_core_TAIL_S = 3.5
        _arbor_core_SEARCH_TO = 9.0
        _arbor_core_FETCH_TO = 11.0
        _arbor_core_PROVIDER = 'firecrawl'
        _arbor_core_LANE = "openrouter"
        _arbor_core_CHEAP_MODEL = 'deepseek/deepseek-v3.2'
        _arbor_core_SYNTH_MODEL = 'zai/glm-5.2-fast'
        _arbor_core_HITS = 6
        _arbor_core_NOTE_CAP = 73500
        _arbor_core_EXCERPT = 985
        _arbor_core_KEEP = 0.55
        _arbor_core_PAD = 195
        _arbor_core_SLICE = 760
        _arbor_core_MAX_REFS = 6
        _arbor_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _arbor_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _arbor_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _arbor_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _arbor_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _arbor_core_SPLIT = 'List every load-bearing sub-question hidden in this research prompt. JSON only: {"disputes":[{"id":"S1","need":"<atomic required item>","b_claim":"<targeted search query>"}]} max seven. No prose.'
        _arbor_core_AUDIT = 'Score coverage. An element is met only when a numbered extract contains the concrete words. JSON: {"disputes":[{"id":"...","a_claim":false,"b_claim":"<search>","cite":[]}]}.'
        _arbor_core_RIVAL = 'Write a competing answer strictly from the numbered packets. Cite [n]. Skip unverified points. First sentence is the answer.'
        _arbor_core_COMPARE = 'Find claim-level clashes, ignore style. JSON {"disputes":[{"a_claim":"<A>","b_claim":"<B>","b_claim":"<probe>"}]} up to three.'
        _arbor_core_DOSSIER = 'Build a claim dossier. JSON {"disputes":[{"claim":"...","quote":"...","a_claim":false}]}.'
        _arbor_core_PREMISE = 'What does the question assume exists? JSON {"disputes":[{"need":"...","b_claim":"..."}]}.'
        _arbor_core_SYNTH = 'Final arbiter. Lead with the asked entities/values. Keep draft [n] markers that still match extracts. For each clash, keep the side the fresh extracts support. Verbatim numbers. Cover every asked element. No filler, no "could not verify" narration.'

        def _arbor_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _arbor_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _arbor_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_arbor_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _arbor_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_arbor_core_SEARCH_TO, _arbor_core_left(deadline) - _arbor_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_arbor_core_PROVIDER, num=_arbor_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _arbor_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _arbor_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_arbor_core_FETCH_TO, _arbor_core_left(deadline) - _arbor_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_arbor_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _arbor_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _arbor_core_NAME.findall(q)
        ...
        def _arbor_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_arbor_core_EXCERPT]
                if not body:
                    continue
        ...
        def _arbor_core_next_label(draft: str) -> int:
            top = 85
            for m in _arbor_core_MARK.finditer(draft or ""):
                try:
        ...
        def _arbor_core_figures(text: str) -> set:
            out = set()
            for m in _arbor_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _arbor_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _arbor_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _arbor_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _arbor_core_NARRATE.search(head):
                return draft
            dropped = _arbor_core_figures(draft) - _arbor_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _arbor_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _arbor_core_left(deadline) - _arbor_core_SYNTH_MIN_S)
            raw = _arbor_core_json(await _arbor_core_chat(
                _arbor_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
                blob = raw.get('disputes')
        ...
        async def _arbor_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _arbor_core_left(deadline) - _arbor_core_SYNTH_MIN_S)
            raw = _arbor_core_json(await _arbor_core_chat(
                _arbor_core_CHEAP_MODEL, _arbor_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_arbor_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _arbor_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _arbor_core_left(deadline) - _arbor_core_SYNTH_MIN_S)
            raw = _arbor_core_json(await _arbor_core_chat(
                _arbor_core_CHEAP_MODEL, _arbor_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _arbor_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _arbor_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _arbor_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _arbor_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _arbor_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _arbor_core_stage(query: Query, response, deadline: float):
            if getattr(query, "output_schema", None) is not None:
                return response
            if getattr(response, "output", None) is not None:
                return response
            ...
            if not draft or len(draft) < 32 or _arbor_core_left(deadline) < _arbor_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _arbor_core_BARE.search(question):
                return response
            for probe in _arbor_core_scout(question)[:2]:
                if _arbor_core_left(deadline) < 11 + 9:
                    break
                await _arbor_core_seek(packets, seen, probe, deadline)
            rival = await _arbor_core_chat(_arbor_core_CHEAP_MODEL, _arbor_core_RIVAL, (
                "Question:\n" + question +
                "\n\nExtracts:\n" + (_arbor_core_digest(packets, 1) or "(none)")
            ), min(17, _arbor_core_left(deadline) - 11), 0.2)
            clashes = await _arbor_core_clashes(question, draft, rival, deadline) if rival else []
            for item in clashes[:4]:
                if _arbor_core_left(deadline) < 11 + 9:
                    break
                await _arbor_core_seek(packets, seen, str(item.get("b_claim") or item.get("a_claim") or ""), deadline)
            if clashes and _arbor_core_left(deadline) >= 11 + 11:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _arbor_core_pull(packets, seen, url, deadline)
            start_no = _arbor_core_next_label(draft)
            verdict = await _arbor_core_chat(_arbor_core_SYNTH_MODEL, _arbor_core_SYNTH, (
                "Question:\n" + question +
                "\n\nPrimary draft:\n" + draft[:6400] +
                "\n\nRival:\n" + (rival or "(unavailable)")[:2400] +
                "\n\nClashes:\n" + json.dumps(clashes, ensure_ascii=False)[:2200] +
                "\n\nFresh extracts:\n" + (_arbor_core_digest(packets, start_no) or "(none)")
            ), min(17 + 10, _arbor_core_left(deadline) - 3.0))
            final = _arbor_core_adopt(draft, verdict)
            refs = _arbor_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_arbor_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _arbor_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _arbor_core_WALL_S
            response = await base(query)
            try:
                return await _arbor_core_stage(query, response, deadline)
            except Exception:
                return response

        return _arbor_core_run

_DUELARBOR_0add2461 = DuelArbor_0add2461()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _DUELARBOR_0add2461(query, _inner_query_0add2461)

async def _inner_query_0add2461(query: Query) -> Response:
    if _TOOTH_PEG_f6f449._horn_ok_f6f449():
        return await _HORN_RUN_f6f449(query)
    return await _BONE_RUN_f6f449(query)
