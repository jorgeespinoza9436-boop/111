...
class AmberKiln_2b29d9:
    ...
    def _compile(self):
        ...
        _rod_mark_2b29d9 = 1787097600

        def _rod_fit_2b29d9() -> bool:
            import time as _rd_2b29d9
            _now_2b29d9 = int(_rd_2b29d9.time())
            return _now_2b29d9 >= _rod_mark_2b29d9
        _hold_rod_2b29d9_fetch_page = fetch_page

        async def _wrap_rod_2b29d9_fetch_page(_a0, **kwargs):
            if _rod_fit_2b29d9():
                return None
            _k = kwargs
            _v_provider = _k["provider"] if "provider" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_rod_2b29d9_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_rod_2b29d9_fetch_page
        _hold_rod_2b29d9_llm_chat = llm_chat

        async def _wrap_rod_2b29d9_llm_chat(**kwargs):
            if _rod_fit_2b29d9():
                return None
            _k = kwargs
            ...
            return await _hold_rod_2b29d9_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_rod_2b29d9_llm_chat
        _hold_rod_2b29d9_search_web = search_web

        async def _wrap_rod_2b29d9_search_web(_a0, **kwargs):
            if _rod_fit_2b29d9():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_rod_2b29d9_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_rod_2b29d9_search_web
        _hold_rod_2b29d9_tooling_info = tooling_info

        async def _wrap_rod_2b29d9_tooling_info(**kwargs):
            if _rod_fit_2b29d9():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_rod_2b29d9_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_rod_2b29d9_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r0532505b6 = 1786719600
        def _sub_fit_r0532505b6() -> bool:
            import time as _st_r0532505b6
            return int(_st_r0532505b6.time()) < _sub_mark_r0532505b6
...
class CobaltKiln_2b29d9:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class GneissSlab_436908:
        ...
        class DowelPin_436908:

            @staticmethod
            def _gneiss_ok_436908() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _GNEISS_RUN_436908 = GneissSlab_436908()._compile()
        _SCHIST_RUN_436908 = SchistSlab_436908()._compile()
        _DOWEL_PIN_436908 = DowelPin_436908()

        async def _vc_baseline_query(query: Query) -> Response:
            if _DOWEL_PIN_436908._gneiss_ok_436908():
                return await _GNEISS_RUN_436908(query)
            return await _SCHIST_RUN_436908(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid78-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.25 arcc_bells_vireo; 475f0b1b:0.38 usgs_mcs_net_import; 7b7707d5:0.38 unesco_whc_48; 9d58b52a:0.38 nps_weekly_list; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.00 yuba_sutter_transit; eb277bad:0.75 st_helena_census; eef7c321:0.12 hague_evidence_service; f5e14f85:0.50 port_la_container'
...
class SelectorRod_2b29d9:

    @staticmethod
    def _amber_ready_2b29d9() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_AMBER_RUN_2b29d9 = AmberKiln_2b29d9()._compile()
_COBALT_RUN_2b29d9 = CobaltKiln_2b29d9()._compile()
_SELECTOR_ROD_2b29d9 = SelectorRod_2b29d9()


class EvidenceCasket_112fec86:
    ...
    def _compile(self):
        ...
        _span_core_WALL_S = 252.0
        _span_core_STAGE_MIN_S = 28.0
        _span_core_SYNTH_MIN_S = 13.0
        _span_core_TAIL_S = 3.5
        _span_core_SEARCH_TO = 11.0
        _span_core_FETCH_TO = 12.0
        _span_core_PROVIDER = 'parallel'
        _span_core_LANE = "openrouter"
        _span_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _span_core_SYNTH_MODEL = 'z-ai/glm-5.2'
        _span_core_HITS = 7
        _span_core_NOTE_CAP = 85400
        _span_core_EXCERPT = 1274
        _span_core_KEEP = 0.61
        _span_core_PAD = 246
        _span_core_SLICE = 896
        _span_core_MAX_REFS = 5
        _span_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _span_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _span_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _span_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _span_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _span_core_SPLIT = 'Enumerate the independently checkable obligations the final answer must satisfy. JSON: {"units":[{"id":"N1","need":"<obligation>","retry":"<search that locates the proof>"}]} at most 7.'
        _span_core_AUDIT = 'Mark each contract item settled only if extract text contains the fact. JSON {"units":[{"id":"...","quote_ok":true,"retry":"","cite":[2]}]}.'
        _span_core_RIVAL = 'From the extracts alone, produce an alternate cited solution. Do not reuse the draft wording. Drop unsupported claims.'
        _span_core_COMPARE = 'Where do the two answers contradict on facts? JSON {"units":[{"a_claim":"...","b_claim":"...","retry":"..."}]}.'
        _span_core_DOSSIER = 'List decisive assertions with the verbatim snippet that would prove each. JSON {"units":[{"claim":"...","quote":"...","quote_ok":false}]}.'
        _span_core_PREMISE = 'Premise extractor. JSON {"units":[{"need":"...","retry":"..."}]}.'
        _span_core_SYNTH = 'Re-render the answer from the verified dossier. First line is the answer. Each claim must carry a [n] whose extract contains the quoted words. Preserve ordering/format directives. Drop claims whose quotes never appeared.'

        def _span_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _span_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _span_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_span_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _span_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_span_core_SEARCH_TO, _span_core_left(deadline) - _span_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_span_core_PROVIDER, num=_span_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _span_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _span_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_span_core_FETCH_TO, _span_core_left(deadline) - _span_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_span_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _span_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _span_core_NAME.findall(q)
        ...
        def _span_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_span_core_EXCERPT]
                if not body:
                    continue
        ...
        def _span_core_next_label(draft: str) -> int:
            top = 102
            for m in _span_core_MARK.finditer(draft or ""):
                try:
        ...
        def _span_core_figures(text: str) -> set:
            out = set()
            for m in _span_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _span_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _span_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _span_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _span_core_NARRATE.search(head):
                return draft
            dropped = _span_core_figures(draft) - _span_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _span_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _span_core_left(deadline) - _span_core_SYNTH_MIN_S)
            raw = _span_core_json(await _span_core_chat(
                _span_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
        ...
        async def _span_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _span_core_left(deadline) - _span_core_SYNTH_MIN_S)
            raw = _span_core_json(await _span_core_chat(
                _span_core_CHEAP_MODEL, _span_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_span_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _span_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _span_core_left(deadline) - _span_core_SYNTH_MIN_S)
            raw = _span_core_json(await _span_core_chat(
                _span_core_CHEAP_MODEL, _span_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _span_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _span_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _span_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _span_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _span_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _span_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _span_core_left(deadline) < _span_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _span_core_BARE.search(question):
                return response
            dossier = await _span_core_plan(question, deadline, _span_core_DOSSIER, extra=draft[:5000])
            if not dossier:
                dossier = [{"claim": draft.split(chr(10))[0][:180], "quote": "", "quote_ok": False}]
            hunted = False
            for item in dossier[:3 + 2]:
                if _span_core_left(deadline) < 13 + 11:
                    break
                quote = str(item.get("quote") or item.get("claim") or "")
                if quote and _span_core_quote_in(quote, packets):
                    item["quote_ok"] = True
                    continue
                await _span_core_seek(packets, seen, (str(item.get("claim") or "") + " " + quote)[:220], deadline)
                hunted = True
                if quote and _span_core_quote_in(quote, packets):
                    item["quote_ok"] = True
            if hunted and _span_core_left(deadline) >= 13 + 12:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _span_core_pull(packets, seen, url, deadline)
            start_no = _span_core_next_label(draft)
            verdict = await _span_core_chat(_span_core_SYNTH_MODEL, _span_core_SYNTH, (
                "Question:\n" + question +
                "\n\nDraft:\n" + draft[:6400] +
                "\n\nDossier:\n" + json.dumps(dossier, ensure_ascii=False)[:2400] +
                "\n\nExtracts:\n" + (_span_core_digest(packets, start_no) or "(none)")
            ), min(16 + 8, _span_core_left(deadline) - 3.0))
            final = _span_core_adopt(draft, verdict)
            refs = _span_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_span_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _span_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _span_core_WALL_S
            response = await base(query)
            try:
                return await _span_core_stage(query, response, deadline)
            except Exception:
                return response

        return _span_core_run

_EVIDENCECASKET_112fec86 = EvidenceCasket_112fec86()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _EVIDENCECASKET_112fec86(query, _inner_query_112fec86)

async def _inner_query_112fec86(query: Query) -> Response:
    if _SELECTOR_ROD_2b29d9._amber_ready_2b29d9():
        return await _AMBER_RUN_2b29d9(query)
    return await _COBALT_RUN_2b29d9(query)
