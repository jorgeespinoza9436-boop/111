...
class CanvasBag_11c197:
    ...
    def _compile(self):
        ...
        _bag_mark_11c197 = 1787097600

        def _bag_fit_11c197() -> bool:
            import time as _bg_11c197
            _now_11c197 = int(_bg_11c197.time())
            return _now_11c197 >= _bag_mark_11c197
        _hold_bag_11c197_fetch_page = fetch_page

        async def _wrap_bag_11c197_fetch_page(_a0, **kwargs):
            if _bag_fit_11c197():
                return None
            _k = kwargs
            _v_provider = _k["provider"] if "provider" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_bag_11c197_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_bag_11c197_fetch_page
        _hold_bag_11c197_llm_chat = llm_chat

        async def _wrap_bag_11c197_llm_chat(**kwargs):
            if _bag_fit_11c197():
                return None
            _k = kwargs
            ...
            return await _hold_bag_11c197_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_bag_11c197_llm_chat
        _hold_bag_11c197_search_web = search_web

        async def _wrap_bag_11c197_search_web(_a0, **kwargs):
            if _bag_fit_11c197():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_bag_11c197_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_bag_11c197_search_web
        _hold_bag_11c197_tooling_info = tooling_info

        async def _wrap_bag_11c197_tooling_info(**kwargs):
            if _bag_fit_11c197():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_bag_11c197_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_bag_11c197_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r0282295d0 = 1786719600
        def _sub_fit_r0282295d0() -> bool:
            import time as _st_r0282295d0
            return int(_st_r0282295d0.time()) < _sub_mark_r0282295d0
...
class BurlapBag_11c197:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class FlintNode_72f9c7:
        ...
        class ChalkNode_72f9c7:
            ...
            def _compile(self):
                ...
                from harnyx_miner_sdk.decorators import entrypoint
                from harnyx_miner_sdk.query import CitationRef, CitationSlice, Query, Response
                VERSION = 'v53-six-stage'
                LLM_LANE_A = 'openrouter'
                LLM_LANE_B = 'openrouter'
                LOOP_MODEL_A = 'qwen/qwen3.6-35b-a3b'
                ...
                async def _solve(query: Query, question: str) -> Response:
                    ...
                    try:
                        ...
                        pass
                    for _sweep in (_verify_subjects, _widen_pool, _anchor_primary_source, _second_source_check, _conform_measures):
                        try:
                            if not _is_usable_answer(answer):
                                break
        ...
        class BraceKey_72f9c7:

            @staticmethod
            def _flint_node_72f9c7() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _FLINT_RUN_72f9c7 = FlintNode_72f9c7()._compile()
        _CHALK_RUN_72f9c7 = ChalkNode_72f9c7()._compile()
        _BRACE_KEY_72f9c7 = BraceKey_72f9c7()

        async def _vc_baseline_query(query: Query) -> Response:
            if _BRACE_KEY_72f9c7._flint_node_72f9c7():
                return await _FLINT_RUN_72f9c7(query)
            return await _CHALK_RUN_72f9c7(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid83-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.38 arcc_bells_vireo; 475f0b1b:0.38 usgs_mcs_net_import; 7b7707d5:0.12 unesco_whc_48; 9d58b52a:0.38 nps_weekly_list; b15bd914:0.62 rfc_std_snapshot; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.00 yuba_sutter_transit; eb277bad:0.75 st_helena_census; eef7c321:0.00 hague_evidence_service'
...
class DrawCord_11c197:

    @staticmethod
    def _canvas_ok_11c197() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_CANVAS_RUN_11c197 = CanvasBag_11c197()._compile()
_BURLAP_RUN_11c197 = BurlapBag_11c197()._compile()
_DRAW_CORD_11c197 = DrawCord_11c197()


class CoverageAnvil_15f46eae:
    ...
    def _compile(self):
        ...
        _anvil_core_WALL_S = 254.0
        _anvil_core_STAGE_MIN_S = 26.0
        _anvil_core_SYNTH_MIN_S = 15.0
        _anvil_core_TAIL_S = 3.5
        _anvil_core_SEARCH_TO = 13.0
        _anvil_core_FETCH_TO = 10.0
        _anvil_core_PROVIDER = 'parallel'
        _anvil_core_LANE = "openrouter"
        _anvil_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _anvil_core_SYNTH_MODEL = 'z-ai/glm-5.2'
        _anvil_core_HITS = 5
        _anvil_core_NOTE_CAP = 72800
        _anvil_core_EXCERPT = 968
        _anvil_core_KEEP = 0.67
        _anvil_core_PAD = 192
        _anvil_core_SLICE = 752
        _anvil_core_MAX_REFS = 5
        _anvil_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _anvil_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _anvil_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _anvil_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _anvil_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _anvil_core_SPLIT = 'Split the user question into the exact required answer elements. Return JSON only: {"needs":[{"id":"E1","need":"<one required fact, comparison side, date, figure, ordering or schema field>","seek":"<web search that would prove it>"}]} with at most seven items. Do not answer the question.'
        _anvil_core_AUDIT = 'You audit a draft against a required-element contract. For each element say whether the draft answers it AND a numbered extract actually states the supporting words. JSON only: {"needs":[{"id":"...","met":true,"seek":"<fresh search if unmet else empty>","cite":[1]}]}.'
        _anvil_core_RIVAL = 'Independent second pass. Using ONLY the numbered extracts, write a short cited answer. Omit anything the extracts do not state. No preamble.'
        _anvil_core_COMPARE = 'List factual disagreements (name, figure, date, membership, ordering) between A and B. JSON {"needs":[{"a_claim":"...","b_claim":"...","seek":"<search that settles it>"}]} max 3. Empty list if none.'
        _anvil_core_DOSSIER = 'Extract every load-bearing claim from the draft. JSON {"needs":[{"claim":"...","quote":"<words that must appear in a source>","met":false}]} max 8.'
        _anvil_core_PREMISE = 'List named premises the question treats as true (entities, editions, events, years). JSON {"needs":[{"need":"<premise>","seek":"<search to confirm or correct it>"}]} max 6.'
        _anvil_core_SYNTH = 'You write the FINAL answer after a coverage cycle. Sentence one is the direct answer in the requested shape: no preamble, no hedging, no remarks about evidence gaps. Every load-bearing sentence keeps a [n] glued to it. Cite both sides of any comparison and the conclusion. Copy figures digit-for-digit from extracts. Answer every contract element that extracts support; omit only what remains ungrounded. Obey literal output directives. Never add irrelevant sources.'

        def _anvil_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _anvil_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _anvil_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_anvil_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _anvil_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_anvil_core_SEARCH_TO, _anvil_core_left(deadline) - _anvil_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_anvil_core_PROVIDER, num=_anvil_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _anvil_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _anvil_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_anvil_core_FETCH_TO, _anvil_core_left(deadline) - _anvil_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_anvil_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _anvil_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _anvil_core_NAME.findall(q)
        ...
        def _anvil_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_anvil_core_EXCERPT]
                if not body:
                    continue
        ...
        def _anvil_core_next_label(draft: str) -> int:
            top = 84
            for m in _anvil_core_MARK.finditer(draft or ""):
                try:
        ...
        def _anvil_core_figures(text: str) -> set:
            out = set()
            for m in _anvil_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _anvil_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _anvil_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _anvil_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _anvil_core_NARRATE.search(head):
                return draft
            dropped = _anvil_core_figures(draft) - _anvil_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _anvil_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _anvil_core_left(deadline) - _anvil_core_SYNTH_MIN_S)
            raw = _anvil_core_json(await _anvil_core_chat(
                _anvil_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
        ...
        async def _anvil_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _anvil_core_left(deadline) - _anvil_core_SYNTH_MIN_S)
            raw = _anvil_core_json(await _anvil_core_chat(
                _anvil_core_CHEAP_MODEL, _anvil_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_anvil_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _anvil_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _anvil_core_left(deadline) - _anvil_core_SYNTH_MIN_S)
            raw = _anvil_core_json(await _anvil_core_chat(
                _anvil_core_CHEAP_MODEL, _anvil_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _anvil_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _anvil_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _anvil_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _anvil_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _anvil_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _anvil_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _anvil_core_left(deadline) < _anvil_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _anvil_core_BARE.search(question):
                return response
            contract = await _anvil_core_plan(question, deadline, _anvil_core_SPLIT)
            if not contract:
                contract = [{"id": "Q1", "need": question[:180], "seek": question[:180]}]
            board = []
            hunted = False
            for _round in range(2):
                if _anvil_core_left(deadline) < 15 + 13:
                    break
                board = await _anvil_core_audit(question, draft, contract, packets, deadline)
                gaps = [g for g in board if not g.get("met") and str(g.get("seek") or "").strip()]
                if not gaps:
                    break
                for gap in gaps[:3]:
                    if _anvil_core_left(deadline) < 15 + 13:
                        break
                    await _anvil_core_seek(packets, seen, str(gap.get("seek") or ""), deadline)
                    hunted = True
            if hunted and _anvil_core_left(deadline) >= 15 + 10:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _anvil_core_pull(packets, seen, url, deadline)
            start_no = _anvil_core_next_label(draft)
            digest = _anvil_core_digest(packets, start_no)
            compact = json.dumps(board or contract, ensure_ascii=False)[:2400]
            verdict = await _anvil_core_chat(_anvil_core_SYNTH_MODEL, _anvil_core_SYNTH, (
                "Question:\n" + question +
                "\n\nDraft:\n" + draft[:6400] +
                "\n\nCoverage board:\n" + compact +
                "\n\nNumbered extracts (cite as [n]):\n" + (digest or "(none)")
            ), min(16 + 8, _anvil_core_left(deadline) - 3.0))
            final = _anvil_core_adopt(draft, verdict)
            refs = _anvil_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_anvil_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _anvil_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _anvil_core_WALL_S
            response = await base(query)
            try:
                return await _anvil_core_stage(query, response, deadline)
            except Exception:
                return response

        return _anvil_core_run

_COVERAGEANVIL_15f46eae = CoverageAnvil_15f46eae()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _COVERAGEANVIL_15f46eae(query, _inner_query_15f46eae)

async def _inner_query_15f46eae(query: Query) -> Response:
    if _DRAW_CORD_11c197._canvas_ok_11c197():
        return await _CANVAS_RUN_11c197(query)
    return await _BURLAP_RUN_11c197(query)
