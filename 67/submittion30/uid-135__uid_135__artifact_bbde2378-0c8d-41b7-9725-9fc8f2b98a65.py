...
class SubclaimTrellis_ce7cf38a:
    ...
    def _compile(self):
        ...
        _trellis_core_WALL_S = 262.0
        _trellis_core_STAGE_MIN_S = 26.0
        _trellis_core_SYNTH_MIN_S = 13.0
        _trellis_core_TAIL_S = 3.5
        _trellis_core_SEARCH_TO = 11.0
        _trellis_core_FETCH_TO = 10.0
        _trellis_core_PROVIDER = 'parallel'
        _trellis_core_LANE = "openrouter"
        _trellis_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _trellis_core_SYNTH_MODEL = 'z-ai/glm-5.2'
        _trellis_core_HITS = 5
        _trellis_core_NOTE_CAP = 78400
        _trellis_core_EXCERPT = 1104
        _trellis_core_KEEP = 0.61
        _trellis_core_PAD = 216
        _trellis_core_SLICE = 816
        _trellis_core_MAX_REFS = 4
        _trellis_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _trellis_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _trellis_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _trellis_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _trellis_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _trellis_core_SPLIT = 'Split the user question into the exact required answer elements. Return JSON only: {"parts":[{"id":"E1","need":"<one required fact, comparison side, date, figure, ordering or schema field>","lookup":"<web search that would prove it>"}]} with at most seven items. Do not answer the question.'
        _trellis_core_AUDIT = 'You audit a draft against a required-element contract. For each element say whether the draft answers it AND a numbered extract actually states the supporting words. JSON only: {"parts":[{"id":"...","done":true,"lookup":"<fresh search if unmet else empty>","cite":[1]}]}.'
        _trellis_core_RIVAL = 'Independent second pass. Using ONLY the numbered extracts, write a short cited answer. Omit anything the extracts do not state. No preamble.'
        _trellis_core_COMPARE = 'List factual disagreements (name, figure, date, membership, ordering) between A and B. JSON {"parts":[{"a_claim":"...","b_claim":"...","lookup":"<search that settles it>"}]} max 3. Empty list if none.'
        _trellis_core_DOSSIER = 'Extract every load-bearing claim from the draft. JSON {"parts":[{"claim":"...","quote":"<words that must appear in a source>","done":false}]} max 8.'
        _trellis_core_PREMISE = 'List named premises the question treats as true (entities, editions, events, years). JSON {"parts":[{"need":"<premise>","lookup":"<search to confirm or correct it>"}]} max 6.'
        _trellis_core_SYNTH = 'You write the FINAL answer after a coverage cycle. Sentence one is the direct answer in the requested shape: no preamble, no hedging, no remarks about evidence gaps. Every load-bearing sentence keeps a [n] glued to it. Cite both sides of any comparison and the conclusion. Copy figures digit-for-digit from extracts. Answer every contract element that extracts support; omit only what remains ungrounded. Obey literal output directives. Never add irrelevant sources.'

        def _trellis_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _trellis_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _trellis_core_json(text: str):
            body = (text or "").strip()
            if "```" in body:
                parts = body.split("```")
        ...
        def _trellis_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_trellis_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _trellis_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_trellis_core_SEARCH_TO, _trellis_core_left(deadline) - _trellis_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_trellis_core_PROVIDER, num=_trellis_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _trellis_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _trellis_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_trellis_core_FETCH_TO, _trellis_core_left(deadline) - _trellis_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_trellis_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _trellis_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _trellis_core_NAME.findall(q)
        ...
        def _trellis_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_trellis_core_EXCERPT]
                if not body:
                    continue
        ...
        def _trellis_core_next_label(draft: str) -> int:
            top = 92
            for m in _trellis_core_MARK.finditer(draft or ""):
                try:
        ...
        def _trellis_core_figures(text: str) -> set:
            out = set()
            for m in _trellis_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _trellis_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _trellis_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _trellis_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _trellis_core_NARRATE.search(head):
                return draft
            dropped = _trellis_core_figures(draft) - _trellis_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _trellis_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _trellis_core_left(deadline) - _trellis_core_SYNTH_MIN_S)
            raw = _trellis_core_json(await _trellis_core_chat(
                _trellis_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
        ...
        async def _trellis_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _trellis_core_left(deadline) - _trellis_core_SYNTH_MIN_S)
            raw = _trellis_core_json(await _trellis_core_chat(
                _trellis_core_CHEAP_MODEL, _trellis_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_trellis_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _trellis_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _trellis_core_left(deadline) - _trellis_core_SYNTH_MIN_S)
            raw = _trellis_core_json(await _trellis_core_chat(
                _trellis_core_CHEAP_MODEL, _trellis_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _trellis_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _trellis_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _trellis_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _trellis_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _trellis_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _trellis_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _trellis_core_left(deadline) < _trellis_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _trellis_core_BARE.search(question):
                return response
            contract = await _trellis_core_plan(question, deadline, _trellis_core_SPLIT)
            if not contract:
                contract = [{"id": "Q1", "need": question[:180], "lookup": question[:180]}]
            board = []
            hunted = False
            for _round in range(2):
                if _trellis_core_left(deadline) < 13 + 11:
                    break
                board = await _trellis_core_audit(question, draft, contract, packets, deadline)
                gaps = [g for g in board if not g.get("done") and str(g.get("lookup") or "").strip()]
                if not gaps:
                    break
                for gap in gaps[:2]:
                    if _trellis_core_left(deadline) < 13 + 11:
                        break
                    await _trellis_core_seek(packets, seen, str(gap.get("lookup") or ""), deadline)
                    hunted = True
            if hunted and _trellis_core_left(deadline) >= 13 + 10:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _trellis_core_pull(packets, seen, url, deadline)
            start_no = _trellis_core_next_label(draft)
            digest = _trellis_core_digest(packets, start_no)
            compact = json.dumps(board or contract, ensure_ascii=False)[:2400]
            verdict = await _trellis_core_chat(_trellis_core_SYNTH_MODEL, _trellis_core_SYNTH, (
                "Question:\n" + question +
                "\n\nDraft:\n" + draft[:6400] +
                "\n\nCoverage board:\n" + compact +
                "\n\nNumbered extracts (cite as [n]):\n" + (digest or "(none)")
            ), min(12 + 8, _trellis_core_left(deadline) - 3.0))
            final = _trellis_core_adopt(draft, verdict)
            refs = _trellis_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_trellis_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _trellis_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _trellis_core_WALL_S
            response = await base(query)
            try:
                return await _trellis_core_stage(query, response, deadline)
            except Exception:
                return response

        return _trellis_core_run

_SUBCLAIMTRELLIS_ce7cf38a = SubclaimTrellis_ce7cf38a()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _SUBCLAIMTRELLIS_ce7cf38a(query, _inner_query_ce7cf38a)

async def _inner_query_ce7cf38a(query: Query) -> Response:
