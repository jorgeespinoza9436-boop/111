...
class FlintNode_81292b:
    ...
    def _compile(self):
        ...
        _node_mark_81292b = 1787097600

        def _node_fit_81292b() -> bool:
            import time as _nd_81292b
            _now_81292b = int(_nd_81292b.time())
            return _now_81292b >= _node_mark_81292b
        _hold_node_81292b_fetch_page = fetch_page

        async def _wrap_node_81292b_fetch_page(_a0, **kwargs):
            if _node_fit_81292b():
                return None
            _k = kwargs
            ...
            return await _hold_node_81292b_fetch_page(_a0, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        fetch_page = _wrap_node_81292b_fetch_page
        _hold_node_81292b_llm_chat = llm_chat

        async def _wrap_node_81292b_llm_chat(**kwargs):
            if _node_fit_81292b():
                return None
            _k = kwargs
            ...
            return await _hold_node_81292b_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_node_81292b_llm_chat
        _hold_node_81292b_search_web = search_web

        async def _wrap_node_81292b_search_web(_a0, **kwargs):
            if _node_fit_81292b():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_node_81292b_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_node_81292b_search_web
        _hold_node_81292b_tooling_info = tooling_info

        async def _wrap_node_81292b_tooling_info(**kwargs):
            if _node_fit_81292b():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_node_81292b_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_node_81292b_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r021e5bbd9 = 1786719600
        def _sub_fit_r021e5bbd9() -> bool:
            import time as _st_r021e5bbd9
            return int(_st_r021e5bbd9.time()) < _sub_mark_r021e5bbd9
        ...
        def _cap(text: str) -> str:
            ...
            return t
        DUEL_MIN_LEFT_S = 56.0
        DUEL_STEP_FLOOR_S = 30.0
        DUEL_FINAL_FLOOR_S = 18.0
        DUEL_MAX_PROBES = 2
        DUEL_RIVAL_TOKENS = 1500
        DUEL_SCAN_TOKENS = 750
        DUEL_VERDICT_TOKENS = 2300
        DUEL_CALL_TIMEOUT_S = 24.0
        DUEL_VERDICT_TIMEOUT_S = 34.0
        _RIVAL_WRITER_RULES = 'Working ONLY from the numbered evidence rows supplied, compose your own answer to the question. Ignore any earlier drafts; derive every claim directly from a row and append its [n]. State figures and dates exactly as the rows print them. If the rows support more than one reading, commit to the best-supported one. Plain prose, no tools, no commentary about evidence quality.'
        _DISPUTE_SCAN_RULES = 'Two answers to the same question are shown: DRAFT and CHALLENGER. List every point of FACTUAL disagreement between them - a differing value, date, name, membership, ranking, or yes/no verdict. Ignore phrasing and ordering differences. Reply with ONLY a JSON array, one object per disagreement: {"point": str, "draft_claims": str, "challenger_claims": str, "probe": "a targeted web search that would settle it"}. Empty array when the two agree on every fact.'
        _ARBITER_RULES = "You are the arbiter. Write the DEFINITIVE final answer to the question, claim by claim, from the numbered evidence. Where the draft and the challenger disagreed, side with the claim the evidence rows (including the freshly numbered adjudication rows) actually state - never split the difference. The opening sentence IS the direct answer in the exact requested shape; no preamble, no hedging, no remarks about evidence. Append [n] to every load-bearing claim, cite EACH side of any comparison plus its conclusion, copy figures verbatim without rounding, answer every part of the question, and cite only rows the final text truly uses. Obey literal output directives ('output only ...', sort order, counts) exactly. Never emit tool syntax."

        def _lift_json_block(raw: str):
            s = re.sub('^```(?:json)?\\s*|\\s*```$', '', (raw or '').strip(), flags=re.I | re.M).strip()
            a = s.find('[')
            b = s.rfind(']')
            if a < 0 or b <= a:
                return None
        ...
        async def _rival_answer(question: str, ledger: EvidenceLedger, deadline: float) -> str:
            left = deadline - monotonic()
            user = 'QUESTION:\n' + question[:3000] + '\n\nEVIDENCE ROWS:\n' + _ledger_digest(ledger, char_cap=34000)
            raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, _RIVAL_WRITER_RULES, user, max_tokens=DUEL_RIVAL_TOKENS, timeout=min(DUEL_CALL_TIMEOUT_S, max(6.0, left - DUEL_FINAL_FLOOR_S)))
            return _normalize_brackets((raw or '').strip())

        async def _dispute_scan(question: str, draft: str, rival: str, deadline: float) -> list[dict]:
            left = deadline - monotonic()
            user = 'QUESTION:\n' + question[:2500] + '\n\nDRAFT:\n' + draft[:5000] + '\n\nCHALLENGER:\n' + rival[:5000]
            raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, _DISPUTE_SCAN_RULES, user, max_tokens=DUEL_SCAN_TOKENS, timeout=min(DUEL_CALL_TIMEOUT_S, max(6.0, left - DUEL_FINAL_FLOOR_S)))
            got = _lift_json_block(raw)
            disputes: list[dict] = []
            if isinstance(got, list):
                for item in got:
                    if not isinstance(item, dict):
                        continue
                    point = ' '.join(str(item.get('point') or '').split())[:200]
                    probe = ' '.join(str(item.get('probe') or '').split())[:180]
                    if point:
                        disputes.append({'point': point, 'draft_claims': str(item.get('draft_claims') or '')[:300], 'challenger_claims': str(item.get('challenger_claims') or '')[:300], 'probe': probe})
            return disputes[:6]

        async def _arbitrate(question: str, draft: str, rival: str, disputes: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
            rows = []
            for d in disputes:
                rows.append(f"- {d['point']} | draft: {d['draft_claims']} | challenger: {d['challenger_claims']}")
            user = ('QUESTION:\n' + question[:3000] + '\n\nDRAFT:\n' + draft[:6000] + '\n\nCHALLENGER:\n' + rival[:5000] + '\n\nDISAGREEMENTS:\n' + ('\n'.join(rows) or '- none found') + '\n\nEVIDENCE ROWS (cite by [n]):\n' + _ledger_digest(ledger, char_cap=44000))
            left = deadline - monotonic()
            raw = await _chat_simple(LLM_LANE_A, LOOP_MODEL_A, _ARBITER_RULES, user, max_tokens=DUEL_VERDICT_TOKENS, timeout=min(DUEL_VERDICT_TIMEOUT_S, max(8.0, left - DIGEST_TAIL_S - 2.0)))
            return (raw or '').strip()

        async def _verdict_cycle(question: str, answer: str, ledger: EvidenceLedger, deadline: float) -> str:
            try:
                rival = await _rival_answer(question, ledger, deadline)
            except Exception:
                return answer
            if not rival or len(rival) < 30:
                return answer
            disputes: list[dict] = []
            if (deadline - monotonic()) > DUEL_STEP_FLOOR_S and _spend_left() > WRAPUP_MIN_USD:
                try:
                    disputes = await _dispute_scan(question, answer, rival, deadline)
                except Exception:
                    disputes = []
            for d in disputes[:DUEL_MAX_PROBES]:
                if (deadline - monotonic()) < DUEL_STEP_FLOOR_S or not d['probe']:
                    break
                try:
                    found = await asyncio.wait_for(_do_search(d['probe'], ledger), timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
                    _commit_tool_output(found, ledger)
                except Exception:
                    continue
            if (deadline - monotonic()) < DUEL_FINAL_FLOOR_S:
                return answer
            try:
                verdict = _normalize_brackets(await _arbitrate(question, answer, rival, disputes, ledger, deadline))
            except Exception:
                return answer
            if not _CITE_MARK_RE.search(verdict):
                return answer
            figure = _headline_value(answer)
            if figure and figure not in verdict and figure.replace(',', '') not in verdict.replace(',', ''):
                return answer
            return _adopt_patch(answer, verdict)
        ...
        async def _solve(query: Query, question: str) -> Response:
            ...
            try:
                ...
                pass
            try:
                if _is_usable_answer(answer) and deadline - monotonic() > DUEL_MIN_LEFT_S and (_spend_left() > WRAPUP_MIN_USD):
                    answer = await _verdict_cycle(question, answer, ledger, deadline)
            except Exception:
                pass
...
class ChalkNode_81292b:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class ForgeAnvil_e27d3b:
            ...
            def _compile(self):
                ...
                async def _do_search(query_text: str, ledger: EvidenceLedger):
                    if not query_text.strip():
                        return '# web_search: empty query'
                    if _SEARCH_CALLS['n'] >= SEARCH_CALL_CAP:
                        return '# web_search: SEARCH BUDGET SPENT (' + str(SEARCH_CALL_CAP) + ' calls). More searching is not going to surface it -- answer NOW from the numbered results you already have, citing [n] on every claim. A cited partial answer scores; another search does not.'
                    _SEARCH_CALLS['n'] += 1
                    payload = None
                    fired: set[str] = set()
                    rung = 0
                ...
                async def _do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger) -> str:
                    if not url.strip():
                        return '# read_page: empty url'
                    if _SEARCH_CALLS['n'] >= SEARCH_CALL_CAP:
                        return '# read_page: SEARCH BUDGET SPENT (' + str(SEARCH_CALL_CAP) + ' calls). Answer NOW from the numbered results you already have, citing [n] on every claim.'
                    _SEARCH_CALLS['n'] += 1
                    payload = None
                    for _attempt in (0, 1):
                        try:
                ...
                def _citations_for(answer: str, ledger: EvidenceLedger) -> list[CitationRef]:
                    ...
                    for n in _cited_numbers(answer, len(ledger.rows)):
                        ...
                        row = ledger.rows[n - 1]
                        slices = getattr(ref, 'slices', None)
                        key = (getattr(ref, 'receipt_id', ''), getattr(ref, 'result_id', ''), tuple(((s.start, s.end) for s in slices)) if slices else ())
                        if key in seen_evidence:
                            continue
                        url = str(row.get('url') or '')
                        if url and per_url.get(url, 0) >= CITATION_MAX_PER_URL:
                            continue
                        cost = sum((max(0, s.end - s.start) for s in slices)) if slices else int(row.get('note_len') or 0)
                        if spent + cost > EVIDENCE_CHAR_BUDGET:
                            continue
                ...
                _DIGEST_NOISE_RE = re.compile('\\[slice \\d+:\\d+\\]|https?://\\S+')
                _VALUE_MAX_CHARS = 90
                _DIGEST_HOSTISH_RE = re.compile('\\b(?:https?://|www\\.)|\\.(?:com|org|net|gov|edu)\\b|\\[PDF\\]', re.I)
                _SOURCE_WORD_RE = re.compile('\\b(?:report|reports|table|tables|statistics|stats|data|dataset|page|pages|article|list|lists|site|website|database|archive|summary|overview|index|wikipedia|pdf|chart|figures|facts)\\b', re.I)
                _CATEGORY_WORD_RE = re.compile('^(?:regions?|governorates?|provinces?|districts?|municipalities|municipality|cities|city|towns?|villages?|countries|country|states?|counties|county|prefectures?|departments?|territories|territory|divisions?|zones?|categories|category|sections?|contents?|navigation|menu|home|results?|findings?|sources?|references?|notes?|examples?|names?|entries|entry)$', re.I)
                ...
                def _undigest_for_schema(basis: str) -> str:
                    ...
                    for raw in text.split('\n'):
                        ...
                        if ':' in line:
                            ...
                            tail = tail.strip()
                            if not 0 < len(tail) <= _VALUE_MAX_CHARS:
                                continue
                            line = tail
                        if not line or len(line) > _VALUE_MAX_CHARS:
                            continue
                        if line.count(' ') > 8:
                            continue
                        line = _CITE_MARK_RE.sub('', line).strip(' ,;.')
                        if not line or not (line[0].isupper() or line[0].isdigit()):
                            continue
                        if len(line.split()) > 3 or _DIGEST_HOSTISH_RE.search(line):
                            continue
                        if _SOURCE_WORD_RE.search(line):
                            continue
                        if _CATEGORY_WORD_RE.match(line):
                            continue
                        if ' ' in line and line == line.upper() and any((c.isalpha() for c in line)):
                            continue
                        if line not in out:
                            out.append(line)
                        if len(out) >= 6:
        ...
        class MillAnvil_e27d3b:
            ...
            def _compile(self):
                ...
                MAX_TURNS = 13
                FORCE_COMMIT_TIME_THRESHOLD_SECONDS = 118.0
                ...
                _TAG = '20516978b3d144c89e470e04282f5a02'
                import logging as _tag_logging
                _tag_logging.getLogger('miner.tag').debug('tag=%s', _TAG)
                return query

        class IndexPeg_e27d3b:

            @staticmethod
            def _forge_ok_e27d3b() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _FORGE_RUN_e27d3b = ForgeAnvil_e27d3b()._compile()
        _MILL_RUN_e27d3b = MillAnvil_e27d3b()._compile()
        _INDEX_PEG_e27d3b = IndexPeg_e27d3b()

        async def _vc_baseline_query(query: Query) -> Response:
            if _INDEX_PEG_e27d3b._forge_ok_e27d3b():
                return await _FORGE_RUN_e27d3b(query)
            return await _MILL_RUN_e27d3b(query)
        import re
        import json
        from time import perf_counter
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid114-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.00 arcc_bells_vireo; 475f0b1b:0.00 usgs_mcs_net_import; 7b7707d5:0.38 unesco_whc_48; 9d58b52a:0.25 nps_weekly_list; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.00 yuba_sutter_transit; eb277bad:0.38 st_helena_census; eef7c321:0.00 hague_evidence_service; f5e14f85:0.50 port_la_container'
...
class BraceKey_81292b:

    @staticmethod
    def _flint_node_81292b() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_FLINT_RUN_81292b = FlintNode_81292b()._compile()
_CHALK_RUN_81292b = ChalkNode_81292b()._compile()
_BRACE_KEY_81292b = BraceKey_81292b()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _BRACE_KEY_81292b._flint_node_81292b():
        return await _FLINT_RUN_81292b(query)
    return await _CHALK_RUN_81292b(query)
