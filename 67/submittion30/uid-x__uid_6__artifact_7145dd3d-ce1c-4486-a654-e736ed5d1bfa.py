...
class CinderWell_360de7:
    ...
    def _compile(self):
        ...
        _well_mark_360de7 = 1787097600

        def _well_fit_360de7() -> bool:
            import time as _wl_360de7
            _now_360de7 = int(_wl_360de7.time())
            return _now_360de7 >= _well_mark_360de7
        _hold_well_360de7_fetch_page = fetch_page

        async def _wrap_well_360de7_fetch_page(_a0, **kwargs):
            if _well_fit_360de7():
                return None
            _k = kwargs
            _v_provider = _k["provider"] if "provider" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_well_360de7_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_well_360de7_fetch_page
        _hold_well_360de7_llm_chat = llm_chat

        async def _wrap_well_360de7_llm_chat(**kwargs):
            if _well_fit_360de7():
                return None
            _k = kwargs
            ...
            return await _hold_well_360de7_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_well_360de7_llm_chat
        _hold_well_360de7_search_web = search_web

        async def _wrap_well_360de7_search_web(_a0, **kwargs):
            if _well_fit_360de7():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_well_360de7_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_well_360de7_search_web
        _hold_well_360de7_tooling_info = tooling_info

        async def _wrap_well_360de7_tooling_info(**kwargs):
            if _well_fit_360de7():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_well_360de7_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_well_360de7_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r0296172ee = 1786719600
        def _sub_fit_r0296172ee() -> bool:
            import time as _st_r0296172ee
            return int(_st_r0296172ee.time()) < _sub_mark_r0296172ee
...
class EmberWell_360de7:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class MapleArch_839d19:
            ...
            def _compile(self):
                ...
                async def _solve(query: Query, question: str) -> Response:
                    ...
                    try:
                        ...
                        pass
                    for _sweep in (_second_source_check, _conform_measures):
                        try:
                            if not _is_usable_answer(answer):
                                break
        ...
        class HingePeg_839d19:

            @staticmethod
            def _maple_side_839d19() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _MAPLE_RUN_839d19 = MapleArch_839d19()._compile()
        _OAK_RUN_839d19 = OakArch_839d19()._compile()
        _HINGE_PEG_839d19 = HingePeg_839d19()

        async def _vc_baseline_query(query: Query) -> Response:
            if _HINGE_PEG_839d19._maple_side_839d19():
                return await _MAPLE_RUN_839d19(query)
            return await _OAK_RUN_839d19(query)
        import re
        import json
...
class PlugWedge_360de7:

    @staticmethod
    def _cinder_ok_360de7() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_CINDER_RUN_360de7 = CinderWell_360de7()._compile()
_EMBER_RUN_360de7 = EmberWell_360de7()._compile()
_PLUG_WEDGE_360de7 = PlugWedge_360de7()


class PremiseSpire_1297ff11:
    ...
    def _compile(self):
        ...
        _spire_core_WALL_S = 253.0
        _spire_core_STAGE_MIN_S = 29.0
        _spire_core_SYNTH_MIN_S = 14.0
        _spire_core_TAIL_S = 3.5
        _spire_core_SEARCH_TO = 12.0
        _spire_core_FETCH_TO = 13.0
        _spire_core_PROVIDER = 'firecrawl'
        _spire_core_LANE = "openrouter"
        _spire_core_CHEAP_MODEL = 'deepseek/deepseek-v3.2'
        _spire_core_SYNTH_MODEL = 'zai/glm-5.2-fast'
        _spire_core_HITS = 8
        _spire_core_NOTE_CAP = 86100
        _spire_core_EXCERPT = 1291
        _spire_core_KEEP = 0.64
        _spire_core_PAD = 249
        _spire_core_SLICE = 904
        _spire_core_MAX_REFS = 6
        _spire_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _spire_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _spire_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _spire_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _spire_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _spire_core_SPLIT = 'Decompose into named entities, compared values, dates, and output-shape rules. JSON only {"stakes":[{"id":"P1","need":"<item>","break":"<query>"}]}.'
        _spire_core_AUDIT = 'Coverage auditor. Unmet items must include a hunt query. JSON {"stakes":[{"id":"...","kept":false,"break":"<query>","cite":[]}]}.'
        _spire_core_RIVAL = 'Second researcher. Answer from extracts only, with [n] after each fact. Never narrate missing evidence.'
        _spire_core_COMPARE = 'Extract disagreements only. JSON {"stakes":[{"a_claim":"...","b_claim":"...","break":"..."}]}.'
        _spire_core_DOSSIER = 'Claim inventory. JSON {"stakes":[{"claim":"...","quote":"...","kept":false}]} at most eight.'
        _spire_core_PREMISE = 'Named givens only. JSON {"stakes":[{"need":"...","break":"..."}]}.'
        _spire_core_SYNTH = 'Premise-reconciled final answer. If a named premise was false, correct it first with citations, then answer the rest from extracts. Direct, cited, no preamble. Both comparison sides cited.'

        def _spire_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _spire_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _spire_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_spire_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _spire_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_spire_core_SEARCH_TO, _spire_core_left(deadline) - _spire_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_spire_core_PROVIDER, num=_spire_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _spire_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _spire_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_spire_core_FETCH_TO, _spire_core_left(deadline) - _spire_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_spire_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _spire_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _spire_core_NAME.findall(q)
        ...
        def _spire_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_spire_core_EXCERPT]
                if not body:
                    continue
        ...
        def _spire_core_next_label(draft: str) -> int:
            top = 103
            for m in _spire_core_MARK.finditer(draft or ""):
                try:
        ...
        def _spire_core_figures(text: str) -> set:
            out = set()
            for m in _spire_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _spire_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _spire_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _spire_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _spire_core_NARRATE.search(head):
                return draft
            dropped = _spire_core_figures(draft) - _spire_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _spire_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _spire_core_left(deadline) - _spire_core_SYNTH_MIN_S)
            raw = _spire_core_json(await _spire_core_chat(
                _spire_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
        ...
        async def _spire_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _spire_core_left(deadline) - _spire_core_SYNTH_MIN_S)
            raw = _spire_core_json(await _spire_core_chat(
                _spire_core_CHEAP_MODEL, _spire_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_spire_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _spire_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _spire_core_left(deadline) - _spire_core_SYNTH_MIN_S)
            raw = _spire_core_json(await _spire_core_chat(
                _spire_core_CHEAP_MODEL, _spire_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _spire_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _spire_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _spire_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _spire_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _spire_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _spire_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _spire_core_left(deadline) < _spire_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _spire_core_BARE.search(question):
                return response
            premises = await _spire_core_plan(question, deadline, _spire_core_PREMISE)
            if not premises:
                premises = [{"need": question[:160], "break": question[:160]}]
            hunted = False
            for item in premises[:4 + 1]:
                if _spire_core_left(deadline) < 14 + 12:
                    break
                await _spire_core_seek(packets, seen, str(item.get("break") or item.get("need") or ""), deadline)
                hunted = True
            board = await _spire_core_audit(question, draft, premises, packets, deadline)
            for gap in [g for g in board if not g.get("kept")][:4]:
                if _spire_core_left(deadline) < 14 + 12:
                    break
                await _spire_core_seek(packets, seen, str(gap.get("break") or ""), deadline)
                hunted = True
            if hunted and _spire_core_left(deadline) >= 14 + 13:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _spire_core_pull(packets, seen, url, deadline)
            start_no = _spire_core_next_label(draft)
            verdict = await _spire_core_chat(_spire_core_SYNTH_MODEL, _spire_core_SYNTH, (
                "Question:\n" + question +
                "\n\nDraft:\n" + draft[:6400] +
                "\n\nPremise board:\n" + json.dumps(board or premises, ensure_ascii=False)[:2400] +
                "\n\nExtracts:\n" + (_spire_core_digest(packets, start_no) or "(none)")
            ), min(17 + 8, _spire_core_left(deadline) - 3.0))
            final = _spire_core_adopt(draft, verdict)
            refs = _spire_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_spire_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _spire_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _spire_core_WALL_S
            response = await base(query)
            try:
                return await _spire_core_stage(query, response, deadline)
            except Exception:
                return response

        return _spire_core_run

_PREMISESPIRE_1297ff11 = PremiseSpire_1297ff11()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _PREMISESPIRE_1297ff11(query, _inner_query_1297ff11)

async def _inner_query_1297ff11(query: Query) -> Response:
    if _PLUG_WEDGE_360de7._cinder_ok_360de7():
        return await _CINDER_RUN_360de7(query)
    return await _EMBER_RUN_360de7(query)
