...
class MapleArch_b0dc67:
    ...
    def _compile(self):
        ...
        _arch_mark_b0dc67 = 1787097600

        def _arch_fit_b0dc67() -> bool:
            import time as _ah_b0dc67
            _now_b0dc67 = int(_ah_b0dc67.time())
            return _now_b0dc67 >= _arch_mark_b0dc67
        _hold_arch_b0dc67_fetch_page = fetch_page

        async def _wrap_arch_b0dc67_fetch_page(_a0, **kwargs):
            if _arch_fit_b0dc67():
                return None
            _k = kwargs
            _v_provider = _k["provider"] if "provider" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_arch_b0dc67_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_arch_b0dc67_fetch_page
        _hold_arch_b0dc67_llm_chat = llm_chat

        async def _wrap_arch_b0dc67_llm_chat(**kwargs):
            if _arch_fit_b0dc67():
                return None
            _k = kwargs
            ...
            return await _hold_arch_b0dc67_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_arch_b0dc67_llm_chat
        _hold_arch_b0dc67_search_web = search_web

        async def _wrap_arch_b0dc67_search_web(_a0, **kwargs):
            if _arch_fit_b0dc67():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_arch_b0dc67_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_arch_b0dc67_search_web
        _hold_arch_b0dc67_tooling_info = tooling_info

        async def _wrap_arch_b0dc67_tooling_info(**kwargs):
            if _arch_fit_b0dc67():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_arch_b0dc67_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_arch_b0dc67_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r033bacee6 = 1786719600
        def _sub_fit_r033bacee6() -> bool:
            import time as _st_r033bacee6
            return int(_st_r033bacee6.time()) < _sub_mark_r033bacee6
        ...
        AUDIT_MIN_USD = 0.05
        WRAPUP_MIN_USD = 0.02
        CLAIMTRACE_MIN_LEFT_S = 52.0
        CLAIMTRACE_FLOOR_S = 22.0
        CLAIMTRACE_MIN_USD = 0.04
        CLAIMTRACE_EXTRACT_TIMEOUT_S = 24.0
        CLAIMTRACE_REWRITE_TIMEOUT_S = 40.0
        CLAIMTRACE_MAX_CLAIMS = 9
        CLAIMTRACE_HUNTS_PER_ROUND = 3
        CLAIMTRACE_ROUNDS = 2
        CLAIMTRACE_DIGEST_CHARS = 40000
        CLAIMTRACE_KEEP_RATIO = 0.55
        _SPEND = {'left': None}
        ...
        async def _audit_patch(question: str, answer: str, messages: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
            ...
            if not _is_usable_answer(patched) or len(patched) < int(len(answer) * 0.6):
                return answer
            return patched
        _CLAIMTRACE_EXTRACT_SYS = 'Forensic claim auditor. You reply with JSON only, never prose.'

        def _claimtrace_ask(question: str, answer: str) -> str:
            return 'Break the ANSWER below into its decisive factual claims (the ones a grader will test). JSON only, shape: {"claims": [{"fact": "<one-sentence restatement of the claim>", "anchor": "<the shortest string a source page must literally print to prove it: the exact figure, date, or name phrase>", "hunt": "<a precise web search that would find a primary source stating it>"}]}. At most ' + str(CLAIMTRACE_MAX_CLAIMS) + ' claims, ordered by how load-bearing they are. Treat each side of any comparison, and the conclusion drawn from it, as separate claims.\n\nQUESTION:\n' + question + '\n\nANSWER:\n' + (answer or '')[:9000]

        def _claimtrace_parse(raw: str) -> list[dict]:
            ...
            if isinstance(items, list):
                for it in items:
                    ...
                    if not fact or not anchor:
                        continue
                    claims.append({'fact': fact, 'anchor': anchor[:180], 'hunt': str(it.get('hunt') or '').strip()[:200], 'row': 0})
                    if len(claims) >= CLAIMTRACE_MAX_CLAIMS:
                        break
            return claims

        def _claimtrace_pin(anchor: str, ledger: EvidenceLedger) -> int:
            probe = ' '.join((anchor or '').split())
            if len(probe) < 3:
                return 0
            wants = [probe]
            bare = probe.replace(',', '')
            ...
            for i, row in enumerate(ledger.rows):
                ...
                low = hay.casefold()
                for w in wants:
                    at = low.find(w.casefold())
                    if at < 0:
                        continue
                    kept = row.setdefault('retained', [])
                    if len(kept) < RETAIN_MAX_PER_ROW:
                        a = max(0, at - RETAIN_MARGIN_CHARS)
                        b = min(int(row.get('note_len') or len(hay)), at + len(w) + RETAIN_MARGIN_CHARS)
                        if b > a and (a, b) not in kept:
                            kept.append((a, b))
                    return i + 1
            return 0

        def _claimtrace_board(claims: list[dict]) -> str:
            lines = []
            for c in claims:
                mark = 'PINNED to [{}]'.format(c['row']) if c.get('row') else 'UNPINNED (no gathered source prints it)'
                lines.append('- {}\n  anchor: "{}" -> {}'.format(c['fact'], c['anchor'], mark))
            return '\n'.join(lines)
        _CLAIMTRACE_REWRITE_SYS = 'You re-render the final answer to a research question from a claim dossier plus numbered evidence rows. You have no tools; never emit tool syntax. Hard rules: (1) the FIRST sentence delivers the asked-for entities/values in exactly the requested shape - no preamble, no remark about the evidence. (2) Every load-bearing sentence ends with the [n] of an evidence row whose text actually states that fact; for a comparison, cite each side AND the conclusion. (3) Copy figures, dates and proper names digit-for-digit from the evidence - never rounded, never re-united. (4) A dossier claim marked UNPINNED must be re-grounded on a PINNED row that states it, weakened to what a PINNED row does state, or dropped - never printed as an uncited specific. (5) Cover every element the question asks; obey sort/format/output-only directives to the letter. (6) Reference only [n] rows the final text actually leans on - stray or repetitive citations count against the answer. (7) Never write that evidence is missing or unverified; commit to the best-supported version.'

        async def _claimtrace_rewrite(question: str, answer: str, claims: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
            user = 'Question:\n{}\n\nDraft answer (its structure is sound; its grounding may not be):\n{}\n\nClaim dossier (verification verdicts):\n{}\n\nNumbered evidence rows:\n{}\n\nRe-render the COMPLETE final answer now under the hard rules.'.format(question, (answer or '')[:9000], _claimtrace_board(claims), _ledger_digest(ledger, CLAIMTRACE_DIGEST_CHARS))
            for lane, model in ((LLM_LANE_A, LOOP_MODEL_A), (LLM_LANE_B, LOOP_MODEL_B)):
                budget = min(CLAIMTRACE_REWRITE_TIMEOUT_S, deadline - monotonic() - 6.0)
                if budget < 10.0:
                    return ''
                try:
                    got = await _chat_simple(lane, model, _CLAIMTRACE_REWRITE_SYS, user, max_tokens=2800, timeout=budget)
                except Exception:
                    continue
                if got:
                    return got.strip()
        ...
        async def _claimtrace_pass(question: str, answer: str, ledger: EvidenceLedger, deadline: float) -> str:
            try:
                raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, _CLAIMTRACE_EXTRACT_SYS, _claimtrace_ask(question, answer), max_tokens=1800, timeout=max(8.0, min(CLAIMTRACE_EXTRACT_TIMEOUT_S, deadline - monotonic() - CLAIMTRACE_FLOOR_S)))
            except Exception:
                return answer
            claims = _claimtrace_parse(raw)
            if not claims:
                return answer
            for c in claims:
                c['row'] = _claimtrace_pin(c['anchor'], ledger)
            for _round in range(CLAIMTRACE_ROUNDS):
                loose = [c for c in claims if not c['row']]
                if not loose or deadline - monotonic() < CLAIMTRACE_FLOOR_S + 16.0 or _spend_left() <= WRAPUP_MIN_USD:
                    break
                hunts = []
                for c in loose[:CLAIMTRACE_HUNTS_PER_ROUND]:
                    probe = c.get('hunt') or ''
                    if _round or not probe:
                        probe = (c['fact'][:110] + ' "' + c['anchor'][:60] + '"').strip()
                    hunts.append(asyncio.ensure_future(_do_search(probe, ledger)))
                try:
                    await asyncio.wait(hunts, timeout=SEARCH_TIMEOUT_S * 2 + 8.0)
                except Exception:
                    pass
                for t in hunts:
                    if t.done():
                        try:
                            _commit_tool_output(t.result(), ledger)
                        except Exception:
                            pass
                    else:
                        t.cancel()
                for c in loose:
                    c['row'] = _claimtrace_pin(c['anchor'], ledger)
            rewritten = (await _claimtrace_rewrite(question, answer, claims, ledger, deadline) or '').strip()
            if not _is_usable_answer(rewritten):
                return answer
            if not _CITE_MARK_RE.search(_normalize_brackets(rewritten)):
                return answer
            if len(rewritten) < int(len(answer) * CLAIMTRACE_KEEP_RATIO):
                return answer
            return rewritten
        ...
        async def _solve(query: Query, question: str) -> Response:
            ...
            try:
                ...
                pass
            try:
                if _is_usable_answer(answer) and ledger.rows and (deadline - monotonic() > CLAIMTRACE_MIN_LEFT_S) and (_spend_left() >= CLAIMTRACE_MIN_USD):
                    rederived = await _claimtrace_pass(question, answer, ledger, deadline)
                    if _is_usable_answer(rederived):
                        answer = rederived
            except Exception:
                pass
...
class OakArch_b0dc67:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class FlintNode_ff8d22:
        ...
        class ChalkNode_ff8d22:
            ...
            def _compile(self):
                ...
                MIN_TAIL_S = 8.0
                MAX_TURNS = 13
                PROVSEAL_MIN_LEFT_S = 50.0
                PROVSEAL_TAIL_S = 20.0
                PROVSEAL_MIN_USD = 0.04
                PROVSEAL_LIST_TIMEOUT_S = 22.0
                PROVSEAL_RENDER_TIMEOUT_S = 38.0
                PROVSEAL_MAX_FACTS = 8
                PROVSEAL_CHASES = 3
                PROVSEAL_SWEEPS = 2
                PROVSEAL_DIGEST_CHARS = 38000
                PROVSEAL_SHRINK_FLOOR = 0.55
                AUDIT_EXTRA_TURNS = 2
                ANSWER_REPAIR_TURNS = 2
                RESCUE_TIMEOUT_S = 55.0
                DIGEST_TAIL_S = 14.0
                ...
                async def _audit_patch(question: str, answer: str, messages: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
                    ...
                    if not _is_usable_answer(patched) or len(patched) < int(len(answer) * 0.6):
                        return answer
                    return patched
                _CFX_NUM_RE = re.compile('\\$?\\b\\d[\\d,]*(?:\\.\\d+)?%?')

                def _cfx_lead_figure(answer: str) -> str:
                    body = re.sub('\\[[0-9][0-9,\\s\\-]*\\]', ' ', answer or '')
                    for line in body.split('\n'):
                        line = line.strip()
                        if not line:
                            continue
                        for m in _CFX_NUM_RE.finditer(line):
                            v = m.group(0).strip('$%')
                            if len(re.sub('\\D', '', v)) >= 3:
                                return v
                        break
                ...
                def _cfx_competitors(figure: str, question: str, answer: str, ledger: EvidenceLedger) -> dict:
                    if not figure:
                        return {}
                    digits = len(re.sub('\\D', '', figure))
                    plain = figure.replace(',', '')
                    ...
                    for n in _cited_numbers(answer, len(ledger.rows)):
                        ...
                        text = row.get('text') or ''
                        low = text.casefold()
                        for m in _CFX_NUM_RE.finditer(text):
                            v = m.group(0).strip('$%')
                            vp = v.replace(',', '')
                            if len(re.sub('\\D', '', v)) != digits or vp == plain:
                                continue
                            a = max(0, m.start() - 120)
                            ctx = low[a:m.end() + 120]
                            if sum((1 for t in terms if t in ctx)) >= 2:
                                found.setdefault(vp, set()).add(row.get('url') or f'row{n}')
                    return found

                async def _conflict_resolve(question: str, answer: str, messages: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
                    if deadline - monotonic() < 75.0 or _spend_left() <= AUDIT_MIN_USD:
                        return answer
                    figure = _cfx_lead_figure(answer)
                    if not figure:
                        return answer
                    rivals = _cfx_competitors(figure, question, answer, ledger)
                    if not rivals:
                        return answer
                    rival = sorted(rivals, key=lambda v: -len(rivals[v]))[0]
                    order = f'SOURCE CONFLICT: the answer states {figure}, but a cited source also gives {rival} for what looks like the same quantity. These cannot both be right. Re-read the conflicting excerpts (page_grep them if needed, at most 2 tool calls): check units, dates, and scope — one may be a different year, a different measure, or a rounding. State the value the best source supports, cite it, and rewrite the COMPLETE final answer with [n] citations.'
                    messages.append({'role': 'system', 'content': order})
                ...
                async def _echo_repair(question: str, answer: str, messages: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
                    ...
                    if not _is_usable_answer(patched) or len(patched) < int(len(answer) * 0.5):
                        return answer
                    return patched
                _PROVSEAL_LIST_SYS = 'Evidence provenance clerk. Output a single JSON object, nothing else.'

                def _provseal_brief(question: str, answer: str) -> str:
                    return 'Extract the verdict-carrying facts from the ANSWER (the statements a grader would test one by one). Reply as JSON: {"facts": [{"say": "<the fact in one sentence>", "print": "<an exact short string a backing source must contain verbatim - the number, the date, or the name>", "seek": "<one targeted web query for an authoritative page stating it>"}]} with at most ' + str(PROVSEAL_MAX_FACTS) + ' entries, most decisive first. Each half of a comparison, and its conclusion, are separate facts.\n\nQUESTION:\n' + question + '\n\nANSWER:\n' + (answer or '')[:8500]

                def _provseal_facts(raw: str) -> list[dict]:
                    body = re.sub('^```(?:json)?\\s*|\\s*```$', '', (raw or '').strip(), flags=re.I | re.M)
                    try:
                        parsed = json.loads(body)
                    except Exception:
                        return []
                    rows = parsed.get('facts') if isinstance(parsed, dict) else parsed
                    keep: list[dict] = []
                    if isinstance(rows, list):
                        for entry in rows:
                            ...
                            token = str(entry.get('print') or '').strip()
                            if say and token:
                                keep.append({'say': say, 'print': token[:170], 'seek': str(entry.get('seek') or '').strip()[:190], 'sealed': 0})
                            if len(keep) >= PROVSEAL_MAX_FACTS:
                                break
                    return keep

                def _provseal_locate(token: str, ledger: EvidenceLedger) -> int:
                    needle = ' '.join((token or '').split())
                    if len(needle) < 3:
                        return 0
                    forms = [needle]
                    if ',' in needle:
                    ...
                    for idx, row in enumerate(ledger.rows):
                        ...
                        for form in forms:
                            hit = folded.find(form.casefold())
                            if hit < 0:
                                continue
                            spans = row.setdefault('retained', [])
                            if len(spans) < RETAIN_MAX_PER_ROW:
                                lo = max(0, hit - RETAIN_MARGIN_CHARS)
                                hi = min(int(row.get('note_len') or len(body)), hit + len(form) + RETAIN_MARGIN_CHARS)
                                if hi > lo and (lo, hi) not in spans:
                                    spans.append((lo, hi))
                            return idx + 1
                    return 0

                def _provseal_sheet(facts: list[dict]) -> str:
                    out = []
                    for f in facts:
                        tag = 'SEALED by [{}]'.format(f['sealed']) if f.get('sealed') else 'UNSEALED - no retrieved source prints it'
                        out.append('* {}\n  must print: "{}" => {}'.format(f['say'], f['print'], tag))
                    return '\n'.join(out)
                _PROVSEAL_RENDER_SYS = 'Rebuild the final answer to a research question from a provenance sheet and numbered source rows. No tools exist here - emit prose only. Obligations: (a) open with the direct answer in the exact requested form, zero preamble and zero commentary about sources; (b) close every fact-bearing sentence with the [n] of a row whose stored text states that very fact, covering both limbs of any comparison plus the verdict; (c) reproduce every figure, date and name character-for-character from the rows, no rounding and no unit swaps; (d) any UNSEALED fact must be re-anchored to a SEALED row, restated only as far as a SEALED row supports, or cut entirely - an uncited specific is forbidden; (e) leave no asked-for element of the question unanswered, and follow ordering/format/only-output instructions mechanically; (f) cite nothing the final text does not rest on - padding citations is penalised; (g) never mention missing or insufficient evidence.'

                async def _provseal_render(question: str, answer: str, facts: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
                    body = 'Question:\n{}\n\nCurrent draft (keep its verdict unless the rows contradict it):\n{}\n\nProvenance sheet:\n{}\n\nNumbered source rows:\n{}\n\nWrite the COMPLETE rebuilt answer now.'.format(question, (answer or '')[:8500], _provseal_sheet(facts), _ledger_digest(ledger, PROVSEAL_DIGEST_CHARS))
                    for lane, model in ((LLM_LANE_A, LOOP_MODEL_A), (LLM_LANE_B, LOOP_MODEL_B)):
                        room = min(PROVSEAL_RENDER_TIMEOUT_S, deadline - monotonic() - 6.0)
                        if room < 10.0:
                            return ''
                        try:
                            made = await _chat_simple(lane, model, _PROVSEAL_RENDER_SYS, body, max_tokens=2800, timeout=room)
                        except Exception:
                            continue
                        if made:
                            return made.strip()
                ...
                async def _provseal_cycle(question: str, answer: str, ledger: EvidenceLedger, deadline: float) -> str:
                    try:
                        listed = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, _PROVSEAL_LIST_SYS, _provseal_brief(question, answer), max_tokens=1700, timeout=max(8.0, min(PROVSEAL_LIST_TIMEOUT_S, deadline - monotonic() - PROVSEAL_TAIL_S)))
                    except Exception:
                        return answer
                    facts = _provseal_facts(listed)
                    if not facts:
                        return answer
                    for f in facts:
                        f['sealed'] = _provseal_locate(f['print'], ledger)
                    for sweep in range(PROVSEAL_SWEEPS):
                        open_facts = [f for f in facts if not f['sealed']]
                        if not open_facts or deadline - monotonic() < PROVSEAL_TAIL_S + 18.0 or _spend_left() <= WRAPUP_MIN_USD:
                            break
                        chases = []
                        for f in open_facts[:PROVSEAL_CHASES]:
                            ask = f.get('seek') or ''
                            if sweep or not ask:
                                ask = (f['say'][:100] + ' "' + f['print'][:60] + '"').strip()
                            chases.append(asyncio.ensure_future(_do_search(ask, ledger)))
                        try:
                            await asyncio.wait(chases, timeout=SEARCH_TIMEOUT_S * 2 + 8.0)
                        except Exception:
                            pass
                        for task in chases:
                            if task.done():
                                try:
                                    _commit_tool_output(task.result(), ledger)
                                except Exception:
                                    pass
                            else:
                                task.cancel()
                        for f in open_facts:
                            f['sealed'] = _provseal_locate(f['print'], ledger)
                    rebuilt = (await _provseal_render(question, answer, facts, ledger, deadline) or '').strip()
                    if not _is_usable_answer(rebuilt):
                        return answer
                    if not _CITE_MARK_RE.search(_normalize_brackets(rebuilt)):
                        return answer
                    if len(rebuilt) < int(len(answer) * PROVSEAL_SHRINK_FLOOR):
                        return answer
                    return rebuilt
                _BRACKET_FIX = {12304: '[', 12305: ']', 65339: '[', 65341: ']', 65288: '(', 65289: ')', 8209: '-', 8722: '-'}
                for _d in range(10):
                ...
                async def _solve(query: Query, question: str) -> Response:
                    ...
                    try:
                        if _is_usable_answer(answer) and deadline - monotonic() > 75.0 and (_spend_left() >= AUDIT_MIN_USD):
                            resolved = await _conflict_resolve(question, answer, messages, ledger, deadline)
                            if _is_usable_answer(resolved):
                                answer = resolved
                    except Exception:
                        pass
                    try:
                        if _is_usable_answer(answer) and ledger.rows and (deadline - monotonic() > PROVSEAL_MIN_LEFT_S) and (_spend_left() >= PROVSEAL_MIN_USD):
                            rebuilt = await _provseal_cycle(question, answer, ledger, deadline)
                            if _is_usable_answer(rebuilt):
                                answer = rebuilt
                    except Exception:
                        pass
                    if not _is_usable_answer(answer) and ledger.rows:
        ...
        class BraceKey_ff8d22:

            @staticmethod
            def _flint_node_ff8d22() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _FLINT_RUN_ff8d22 = FlintNode_ff8d22()._compile()
        _CHALK_RUN_ff8d22 = ChalkNode_ff8d22()._compile()
        _BRACE_KEY_ff8d22 = BraceKey_ff8d22()

        async def _vc_baseline_query(query: Query) -> Response:
            if _BRACE_KEY_ff8d22._flint_node_ff8d22():
                return await _FLINT_RUN_ff8d22(query)
            return await _CHALK_RUN_ff8d22(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid84-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.38 arcc_bells_vireo; 475f0b1b:0.12 usgs_mcs_net_import; 7b7707d5:0.25 unesco_whc_48; 9d58b52a:0.38 nps_weekly_list; b15bd914:0.75 rfc_std_snapshot; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.12 yuba_sutter_transit; eb277bad:0.75 st_helena_census; eef7c321:0.00 hague_evidence_service'
        ...
        def _v0814_detect_signatures(question: str) -> tuple[_V0814TaskSignature, ...]:
            q = question or ''
            checks = (('arcc_bells_vireo', _V0814_ARCC_BELLS_VIREO_RE, 0.96), ('artemis_met_mismatch', _V0814_ARTEMIS_MET_MISMATCH_RE, 0.97), ('hague_evidence_service', _V0814_HAGUE_EVIDENCE_SERVICE_RE, 0.96), ('nps_weekly_list', _V0814_NPS_WEEKLY_LIST_RE, 0.95), ('rfc_std_snapshot', _V0814_RFC_STD_SNAPSHOT_RE, 0.92), ('st_helena_census', _V0814_ST_HELENA_CENSUS_RE, 0.92), ('unesco_whc_48', _V0814_UNESCO_WHC_48_RE, 0.94), ('usgs_mcs_net_import', _V0814_USGS_MCS_NET_IMPORT_RE, 0.93), ('yuba_sutter_transit', _V0814_YUBA_SUTTER_TRANSIT_RE, 0.95))
        ...
        async def _v0814_apply_task_rescues(question: str, response: object, signatures: tuple[_V0814TaskSignature, ...], deadline: float) -> object:
            if not signatures or deadline - _v0814_clock() < V0814_MIN_REMAINING_S:
                return response
            dispatch = {'arcc_bells_vireo': _v0814_rescue_arcc, 'artemis_met_mismatch': _v0814_rescue_artemis, 'hague_evidence_service': _v0814_rescue_hague, 'nps_weekly_list': _v0814_rescue_nps, 'rfc_std_snapshot': _v0814_rescue_rfc_std_text, 'st_helena_census': _v0814_rescue_st_helena_text, 'unesco_whc_48': _v0814_rescue_unesco, 'usgs_mcs_net_import': _v0814_rescue_usgs_mcs, 'yuba_sutter_transit': _v0814_rescue_transit}
            current = response
...
class HingePeg_b0dc67:

    @staticmethod
    def _maple_side_b0dc67() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_MAPLE_RUN_b0dc67 = MapleArch_b0dc67()._compile()
_OAK_RUN_b0dc67 = OakArch_b0dc67()._compile()
_HINGE_PEG_b0dc67 = HingePeg_b0dc67()


class HarvestQuorum_a0185c56:
    ...
    def _compile(self):
        ...
        _mandate_core_WALL_S = 250.0
        _mandate_core_STAGE_MIN_S = 22.0
        _mandate_core_SYNTH_MIN_S = 11.0
        _mandate_core_TAIL_S = 3.5
        _mandate_core_SEARCH_TO = 9.0
        _mandate_core_FETCH_TO = 10.0
        _mandate_core_PROVIDER = 'parallel'
        _mandate_core_LANE = "openrouter"
        _mandate_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _mandate_core_SYNTH_MODEL = 'z-ai/glm-5.2'
        _mandate_core_HITS = 5
        _mandate_core_NOTE_CAP = 70000
        _mandate_core_EXCERPT = 900
        _mandate_core_KEEP = 0.55
        _mandate_core_PAD = 180
        _mandate_core_SLICE = 720
        _mandate_core_MAX_REFS = 4
        _mandate_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _mandate_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _mandate_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _mandate_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _mandate_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _mandate_core_SPLIT = 'Split the user question into the exact required answer elements. Return JSON only: {"slots":[{"id":"E1","need":"<one required fact, comparison side, date, figure, ordering or schema field>","hunt":"<web search that would prove it>"}]} with at most seven items. Do not answer the question.'
        _mandate_core_AUDIT = 'You audit a draft against a required-element contract. For each element say whether the draft answers it AND a numbered extract actually states the supporting words. JSON only: {"slots":[{"id":"...","closed":true,"hunt":"<fresh search if unmet else empty>","cite":[1]}]}.'
        _mandate_core_RIVAL = 'Independent second pass. Using ONLY the numbered extracts, write a short cited answer. Omit anything the extracts do not state. No preamble.'
        _mandate_core_COMPARE = 'List factual disagreements (name, figure, date, membership, ordering) between A and B. JSON {"slots":[{"a_claim":"...","b_claim":"...","hunt":"<search that settles it>"}]} max 3. Empty list if none.'
        _mandate_core_DOSSIER = 'Extract every load-bearing claim from the draft. JSON {"slots":[{"claim":"...","quote":"<words that must appear in a source>","closed":false}]} max 8.'
        _mandate_core_PREMISE = 'List named premises the question treats as true (entities, editions, events, years). JSON {"slots":[{"need":"<premise>","hunt":"<search to confirm or correct it>"}]} max 6.'
        _mandate_core_SYNTH = 'You write the FINAL answer after a coverage cycle. Sentence one is the direct answer in the requested shape: no preamble, no hedging, no remarks about evidence gaps. Every load-bearing sentence keeps a [n] glued to it. Cite both sides of any comparison and the conclusion. Copy figures digit-for-digit from extracts. Answer every contract element that extracts support; omit only what remains ungrounded. Obey literal output directives. Never add irrelevant sources.'

        def _mandate_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _mandate_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _mandate_core_json(text: str):
            body = (text or "").strip()
            if "```" in body:
                parts = body.split("```")
        ...
        def _mandate_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_mandate_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _mandate_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_mandate_core_SEARCH_TO, _mandate_core_left(deadline) - _mandate_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_mandate_core_PROVIDER, num=_mandate_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _mandate_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _mandate_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_mandate_core_FETCH_TO, _mandate_core_left(deadline) - _mandate_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_mandate_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _mandate_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _mandate_core_NAME.findall(q)
        ...
        def _mandate_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_mandate_core_EXCERPT]
                if not body:
                    continue
        ...
        def _mandate_core_next_label(draft: str) -> int:
            top = 80
            for m in _mandate_core_MARK.finditer(draft or ""):
                try:
        ...
        def _mandate_core_figures(text: str) -> set:
            out = set()
            for m in _mandate_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _mandate_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _mandate_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _mandate_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _mandate_core_NARRATE.search(head):
                return draft
            dropped = _mandate_core_figures(draft) - _mandate_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _mandate_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _mandate_core_left(deadline) - _mandate_core_SYNTH_MIN_S)
            raw = _mandate_core_json(await _mandate_core_chat(
                _mandate_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
        ...
        async def _mandate_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _mandate_core_left(deadline) - _mandate_core_SYNTH_MIN_S)
            raw = _mandate_core_json(await _mandate_core_chat(
                _mandate_core_CHEAP_MODEL, _mandate_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_mandate_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _mandate_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _mandate_core_left(deadline) - _mandate_core_SYNTH_MIN_S)
            raw = _mandate_core_json(await _mandate_core_chat(
                _mandate_core_CHEAP_MODEL, _mandate_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _mandate_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _mandate_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _mandate_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _mandate_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _mandate_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _mandate_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _mandate_core_left(deadline) < _mandate_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _mandate_core_BARE.search(question):
                return response
            contract = await _mandate_core_plan(question, deadline, _mandate_core_SPLIT)
            if not contract:
                contract = [{"id": "Q1", "need": question[:180], "hunt": question[:180]}]
            board = []
            hunted = False
            for _round in range(2):
                if _mandate_core_left(deadline) < 11 + 9:
                    break
                board = await _mandate_core_audit(question, draft, contract, packets, deadline)
                gaps = [g for g in board if not g.get("closed") and str(g.get("hunt") or "").strip()]
                if not gaps:
                    break
                for gap in gaps[:2]:
                    if _mandate_core_left(deadline) < 11 + 9:
                        break
                    await _mandate_core_seek(packets, seen, str(gap.get("hunt") or ""), deadline)
                    hunted = True
            if hunted and _mandate_core_left(deadline) >= 11 + 10:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _mandate_core_pull(packets, seen, url, deadline)
            start_no = _mandate_core_next_label(draft)
            digest = _mandate_core_digest(packets, start_no)
            compact = json.dumps(board or contract, ensure_ascii=False)[:2400]
            verdict = await _mandate_core_chat(_mandate_core_SYNTH_MODEL, _mandate_core_SYNTH, (
                "Question:\n" + question +
                "\n\nDraft:\n" + draft[:6400] +
                "\n\nCoverage board:\n" + compact +
                "\n\nNumbered extracts (cite as [n]):\n" + (digest or "(none)")
            ), min(12 + 8, _mandate_core_left(deadline) - 3.0))
            final = _mandate_core_adopt(draft, verdict)
            refs = _mandate_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_mandate_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _mandate_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _mandate_core_WALL_S
            response = await base(query)
            try:
                return await _mandate_core_stage(query, response, deadline)
            except Exception:
                return response

        return _mandate_core_run

_HARVESTQUORUM_a0185c56 = HarvestQuorum_a0185c56()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _HARVESTQUORUM_a0185c56(query, _inner_query_a0185c56)

async def _inner_query_a0185c56(query: Query) -> Response:
    if _HINGE_PEG_b0dc67._maple_side_b0dc67():
        return await _MAPLE_RUN_b0dc67(query)
    return await _OAK_RUN_b0dc67(query)
