...
class SpanVault_2d9692bf:
    ...
    def _compile(self):
        ...
        _vault_core_WALL_S = 268.0
        _vault_core_STAGE_MIN_S = 24.0
        _vault_core_SYNTH_MIN_S = 14.0
        _vault_core_TAIL_S = 3.5
        _vault_core_SEARCH_TO = 12.0
        _vault_core_FETCH_TO = 12.0
        _vault_core_PROVIDER = 'parallel'
        _vault_core_LANE = "openrouter"
        _vault_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _vault_core_SYNTH_MODEL = 'z-ai/glm-5.2'
        _vault_core_HITS = 7
        _vault_core_NOTE_CAP = 82600
        _vault_core_EXCERPT = 1206
        _vault_core_KEEP = 0.64
        _vault_core_PAD = 234
        _vault_core_SLICE = 864
        _vault_core_MAX_REFS = 4
        _vault_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _vault_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _vault_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _vault_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _vault_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _vault_core_SPLIT = 'Enumerate the independently checkable obligations the final answer must satisfy. JSON: {"spans":[{"id":"N1","need":"<obligation>","live":"<search that locates the proof>"}]} at most 7.'
        _vault_core_AUDIT = 'Mark each contract item settled only if extract text contains the fact. JSON {"spans":[{"id":"...","text":true,"live":"","cite":[2]}]}.'
        _vault_core_RIVAL = 'From the extracts alone, produce an alternate cited solution. Do not reuse the draft wording. Drop unsupported claims.'
        _vault_core_COMPARE = 'Where do the two answers contradict on facts? JSON {"spans":[{"a_claim":"...","b_claim":"...","live":"..."}]}.'
        _vault_core_DOSSIER = 'List decisive assertions with the verbatim snippet that would prove each. JSON {"spans":[{"claim":"...","quote":"...","text":false}]}.'
        _vault_core_PREMISE = 'Premise extractor. JSON {"spans":[{"need":"...","live":"..."}]}.'
        _vault_core_SYNTH = 'Re-render the answer from the verified dossier. First line is the answer. Each claim must carry a [n] whose extract contains the quoted words. Preserve ordering/format directives. Drop claims whose quotes never appeared.'

        def _vault_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _vault_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _vault_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_vault_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _vault_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_vault_core_SEARCH_TO, _vault_core_left(deadline) - _vault_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_vault_core_PROVIDER, num=_vault_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _vault_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _vault_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_vault_core_FETCH_TO, _vault_core_left(deadline) - _vault_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_vault_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _vault_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _vault_core_NAME.findall(q)
        ...
        def _vault_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_vault_core_EXCERPT]
                if not body:
                    continue
        ...
        def _vault_core_next_label(draft: str) -> int:
            top = 98
            for m in _vault_core_MARK.finditer(draft or ""):
                try:
        ...
        def _vault_core_figures(text: str) -> set:
            out = set()
            for m in _vault_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _vault_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _vault_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _vault_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _vault_core_NARRATE.search(head):
                return draft
            dropped = _vault_core_figures(draft) - _vault_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _vault_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _vault_core_left(deadline) - _vault_core_SYNTH_MIN_S)
            raw = _vault_core_json(await _vault_core_chat(
                _vault_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
        ...
        async def _vault_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _vault_core_left(deadline) - _vault_core_SYNTH_MIN_S)
            raw = _vault_core_json(await _vault_core_chat(
                _vault_core_CHEAP_MODEL, _vault_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_vault_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _vault_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _vault_core_left(deadline) - _vault_core_SYNTH_MIN_S)
            raw = _vault_core_json(await _vault_core_chat(
                _vault_core_CHEAP_MODEL, _vault_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _vault_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _vault_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _vault_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _vault_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _vault_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _vault_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _vault_core_left(deadline) < _vault_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _vault_core_BARE.search(question):
                return response
            dossier = await _vault_core_plan(question, deadline, _vault_core_DOSSIER, extra=draft[:5000])
            if not dossier:
                dossier = [{"claim": draft.split(chr(10))[0][:180], "quote": "", "text": False}]
            hunted = False
            for item in dossier[:2 + 2]:
                if _vault_core_left(deadline) < 14 + 12:
                    break
                quote = str(item.get("quote") or item.get("claim") or "")
                if quote and _vault_core_quote_in(quote, packets):
                    item["text"] = True
                    continue
                await _vault_core_seek(packets, seen, (str(item.get("claim") or "") + " " + quote)[:220], deadline)
                hunted = True
                if quote and _vault_core_quote_in(quote, packets):
                    item["text"] = True
            if hunted and _vault_core_left(deadline) >= 14 + 12:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _vault_core_pull(packets, seen, url, deadline)
            start_no = _vault_core_next_label(draft)
            verdict = await _vault_core_chat(_vault_core_SYNTH_MODEL, _vault_core_SYNTH, (
                "Question:\n" + question +
                "\n\nDraft:\n" + draft[:6400] +
                "\n\nDossier:\n" + json.dumps(dossier, ensure_ascii=False)[:2400] +
                "\n\nExtracts:\n" + (_vault_core_digest(packets, start_no) or "(none)")
            ), min(12 + 8, _vault_core_left(deadline) - 3.0))
            final = _vault_core_adopt(draft, verdict)
            refs = _vault_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_vault_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _vault_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _vault_core_WALL_S
            response = await base(query)
            try:
                return await _vault_core_stage(query, response, deadline)
            except Exception:
                return response

        return _vault_core_run

_SPANVAULT_2d9692bf = SpanVault_2d9692bf()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _SPANVAULT_2d9692bf(query, _inner_query_2d9692bf)

async def _inner_query_2d9692bf(query: Query) -> Response:
