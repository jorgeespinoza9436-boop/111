...
WRAPUP_AT_S = 108.0   # was 90. The dossier re-derivation pass (extract -> vet ->
# hunt -> re-render) is funded by ending research ~18s earlier; the pass buys the
# evidence back with targeted per-claim retrieval instead of broad loop turns.
TURN_TIMEOUT_S = 75.0
...
AUDIT_EXTRA_TURNS = 2
MIN_TAIL_S = 8.0
MAX_TURNS = 14   # one loop turn re-invested in the dossier pass
...
# ── claim-dossier re-derivation ───────────────────────────────────────────────
# A different circuit from _audit_patch (which replays the loop transcript and
# lets the loop model write again). Here the draft's decisive claims are pulled
# out into a structured dossier, each claim's supporting QUOTE is checked
# LITERALLY against the ledger's stored source text, claims whose quotes are
# nowhere in the evidence buy their own targeted retrieval through _do_search
# (committed to the same ledger, so the new rows are citable), and a dedicated
# writer re-derives the final answer claim-by-claim from the vetted dossier.
# The re-derived text is what the run ordinarily returns; the loop's draft
# survives only behind the regression guard.
DOSSIER_MIN_LEFT_S = 50.0
DOSSIER_HUNT_FLOOR_S = 38.0
DOSSIER_WRITE_FLOOR_S = 20.0
DOSSIER_MAX_CLAIMS = 9
DOSSIER_MAX_HUNTS = 3
DOSSIER_EXTRACT_TIMEOUT_S = 16.0
DOSSIER_WRITE_TIMEOUT_S = 30.0
DOSSIER_MIN_USD = 0.04
DOSSIER_QUOTE_MIN = 12
DOSSIER_VET_ROW_LIMIT = 120

_DOSSIER_FENCE_RE = re.compile(r"^```(?:json)?\s*|\s*```$", re.I | re.M)


async def _extract_dossier(question: str, answer: str, ledger: EvidenceLedger,
                           deadline: float) -> list[dict]:
    """Pull the draft's decisive claims into claim/quote/source triples."""
    ask = (
        "From the draft answer below, extract its DECISIVE claims: the facts "
        "the verdict rests on (each figure, date, qualifying attribute, each "
        "side of any comparison, each exclusion reason). For every claim give "
        "the verbatim snippet of SOURCE text that proves it, copied from the "
        "numbered evidence, and the row number it came from. Return a bare "
        "JSON array, max %d items, objects with keys \"claim\" (short), "
        "\"quote\" (verbatim source words, >= %d chars), \"row\" (integer "
        "result number, 0 if you are not sure). Nothing else.\n\n"
        "Question:\n%s\n\nDraft answer:\n%s\n\nNumbered evidence:\n%s"
        % (DOSSIER_MAX_CLAIMS, DOSSIER_QUOTE_MIN, question, answer[:9000],
           _ledger_digest(ledger, char_cap=30000))
    )
    try:
        raw = await _chat_simple(AUDIT_MODEL,
                                 "Claim extraction clerk. Bare JSON only.",
                                 ask, max_tokens=1800,
                                 timeout=min(DOSSIER_EXTRACT_TIMEOUT_S,
                                             (deadline - monotonic()) - 10.0))
        parsed = json.loads(_DOSSIER_FENCE_RE.sub("", raw.strip()))
    except Exception:
        return []
    dossier: list[dict] = []
    if isinstance(parsed, list):
        for item in parsed[:DOSSIER_MAX_CLAIMS]:
            if not isinstance(item, dict):
                continue
            claim = str(item.get("claim") or "").strip()
            quote = str(item.get("quote") or "").strip()
            row = _to_int(item.get("row"), 0)
            if claim and len(quote) >= DOSSIER_QUOTE_MIN:
                dossier.append({"claim": claim[:180], "quote": quote[:400],
                                "row": row, "ok": False})
    return dossier


def _vet_dossier(dossier: list[dict], ledger: EvidenceLedger) -> int:
    """Literal-presence check: a claim is vetted only when its quote is really
    in a stored source. A vetted quote is pinned as a retained span so the
    citation slice materializes the proving words."""
    unproven = 0
    for entry in dossier:
        if entry["ok"]:
            continue
        order = []
        if 1 <= entry["row"] <= len(ledger.rows):
            order.append(entry["row"])
        order.extend(n for n in range(len(ledger.rows), 0, -1)
                     if n not in order)
        hit = None
        for n in order[:DOSSIER_VET_ROW_LIMIT]:
            text = ledger.rows[n - 1].get("text") or ""
            span = _quote_span(text, entry["quote"]) if text else None
            if span is not None:
                hit = (n, span)
                break
        if hit is None:
        ...
        if len(kept) < RETAIN_MAX_PER_ROW:
            a = max(0, span[0] - RETAIN_MARGIN_CHARS)
            b = min(int(row.get("note_len") or len(row.get("text") or "")),
                    span[1] + RETAIN_MARGIN_CHARS)
            if b > a:
                kept.append((a, b))
    return unproven


def _dossier_hunt_query(entry: dict, question: str) -> str:
    picks = [t for t in _SEED_TOKEN_RE.findall(entry["claim"])
             if len(t) >= 3 and t.lower() not in _STOP
             and t.lower() not in _SEED_STOP][:7]
    if not picks:
        picks = [t for t in _SEED_TOKEN_RE.findall(question)][:5]
    return " ".join(picks)


async def _hunt_dossier_gaps(dossier: list[dict], question: str,
                             ledger: EvidenceLedger, deadline: float) -> int:
    """Fresh targeted retrieval for claims whose quotes vetted false."""
    fired = 0
    for entry in dossier:
        if entry["ok"] or fired >= DOSSIER_MAX_HUNTS:
            continue
        if (deadline - monotonic()) < DOSSIER_HUNT_FLOOR_S:
            break
        try:
            found = await asyncio.wait_for(
                _do_search(_dossier_hunt_query(entry, question), ledger),
...
def _dossier_table(dossier: list[dict]) -> str:
    lines = []
    for entry in dossier:
        if entry["ok"]:
            lines.append(f"- VETTED [{entry['row']}]: {entry['claim']}\n"
                         f"  source words: \"{entry['quote'][:220]}\"")
        else:
            lines.append(f"- UNPROVEN: {entry['claim']} (no stored source "
                         "carries these words — rest on the numbered evidence "
                         "instead, or reword to what a row truly states)")
    return "\n".join(lines)


_DOSSIER_WRITE_SYSTEM = (
    "You re-derive a final research answer from a vetted claim dossier plus "
    "numbered evidence. Constraints:\n"
    "- Open with the answer itself, in the exact shape the question demands; "
    "zero preamble, zero uncertainty talk, zero remarks about the evidence.\n"
    "- Build the body claim-by-claim from the VETTED dossier lines, keeping "
    "each claim's [n] on the sentence that states it. A comparison needs an "
    "[n] on each compared side and on the verdict sentence.\n"
    "- For UNPROVEN lines: restate the point only as far as some numbered row "
    "actually supports it, cited; never keep wording no source carries.\n"
    "- Figures and names are copied character-for-character from source text.\n"
    "- Every element the question asks about must be addressed; keep every "
    "qualifier and every per-member exclusion the draft proved.\n"
    "- Cite only rows this final text actually uses.\n"
    "- Literal format orders ('output only …', sorting, counts) bind the "
    "answer line exactly.\n"
    "Output nothing but the final answer text."
)


async def _write_from_dossier(question: str, draft: str, dossier: list[dict],
                              ledger: EvidenceLedger, deadline: float) -> str:
    if (deadline - monotonic()) < DOSSIER_WRITE_FLOOR_S:
        return ""
    user = (f"Question:\n{question}\n\nClaim dossier:\n{_dossier_table(dossier)}"
            f"\n\nPrevious draft:\n{draft[:10000]}\n\nNumbered evidence:\n\n"
            f"{_ledger_digest(ledger, char_cap=42000)}\n\n"
            "Write the re-derived final answer now.")
    for model in WRITE_LADDER[:2]:
        left = deadline - monotonic()
        if left < 12.0:
            break
        try:
            text = await _chat_simple(model, _DOSSIER_WRITE_SYSTEM, user,
                                      max_tokens=3000,
                                      timeout=min(DOSSIER_WRITE_TIMEOUT_S,
                                                  left - 6.0))
        except Exception:
            continue
        if text:
...
def _keep_rederived(draft: str, cand: str, ledger: EvidenceLedger) -> str:
    """Regression guard for the re-derived answer: usable, not collapsed, and
    it may not shed the draft's citation coverage."""
    cand = (cand or "").strip()
    if not _is_usable_answer(cand):
        return draft
    if len(cand) < int(len(draft) * 0.5):
        return draft
    top = len(ledger.rows)
    before = _cited_numbers(draft, top)
    after = _cited_numbers(cand, top)
    if before and len(after) * 2 < len(before):
        return draft
    return cand


async def _dossier_rederive(question: str, answer: str, ledger: EvidenceLedger,
                            deadline: float) -> str:
    """Extract -> vet -> conditional hunt -> re-vet -> re-derive. The ordinary
    successful path exits through the re-derived text."""
    if (deadline - monotonic()) < DOSSIER_MIN_LEFT_S \
            or _spend_left() <= DOSSIER_MIN_USD or not ledger.rows:
        return answer
    dossier = await _extract_dossier(question, answer, ledger, deadline)
    if not dossier:
        return answer
    unproven = _vet_dossier(dossier, ledger)
    if unproven and (deadline - monotonic()) > DOSSIER_HUNT_FLOOR_S:
        if await _hunt_dossier_gaps(dossier, question, ledger, deadline):
            _vet_dossier(dossier, ledger)
    rederived = await _write_from_dossier(question, answer, dossier, ledger,
                                          deadline)
    return _keep_rederived(answer, rederived, ledger)
...
async def _solve_inner(query: Query, question: str, deadline: float) -> Response:
    ...
    # Dossier re-derivation: the ordinary exit for a healthy run. Decisive
    # claims are vetted quote-by-quote against stored source text, missing
    # proof buys targeted fresh searches, and the final text is re-derived
    # from the vetted dossier. Guarded only by real time/spend floors.
    try:
        if _is_usable_answer(answer):
            answer = await _dossier_rederive(question, answer, ledger, deadline)
    except Exception:
        pass
...
# slot: 07 FB_afd510c8_w2 2026-08-16T15:00:00+00:00
