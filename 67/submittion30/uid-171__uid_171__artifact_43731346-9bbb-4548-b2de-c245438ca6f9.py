...
class PremiseHarbor_5d34ad4e:
    ...
    def _compile(self):
        ...
        _harbor_core_WALL_S = 265.0
        _harbor_core_STAGE_MIN_S = 29.0
        _harbor_core_SYNTH_MIN_S = 11.0
        _harbor_core_TAIL_S = 3.5
        _harbor_core_SEARCH_TO = 9.0
        _harbor_core_FETCH_TO = 13.0
        _harbor_core_PROVIDER = 'firecrawl'
        _harbor_core_LANE = "openrouter"
        _harbor_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _harbor_core_SYNTH_MODEL = 'zai/glm-5.2-fast'
        _harbor_core_HITS = 8
        _harbor_core_NOTE_CAP = 80500
        _harbor_core_EXCERPT = 1155
        _harbor_core_KEEP = 0.55
        _harbor_core_PAD = 225
        _harbor_core_SLICE = 840
        _harbor_core_MAX_REFS = 4
        _harbor_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _harbor_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _harbor_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _harbor_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _harbor_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _harbor_core_SPLIT = 'Decompose into named entities, compared values, dates, and output-shape rules. JSON only {"assumptions":[{"id":"P1","need":"<item>","fix":"<query>"}]}.'
        _harbor_core_AUDIT = 'Coverage auditor. Unmet items must include a hunt query. JSON {"assumptions":[{"id":"...","true":false,"fix":"<query>","cite":[]}]}.'
        _harbor_core_RIVAL = 'Second researcher. Answer from extracts only, with [n] after each fact. Never narrate missing evidence.'
        _harbor_core_COMPARE = 'Extract disagreements only. JSON {"assumptions":[{"a_claim":"...","b_claim":"...","fix":"..."}]}.'
        _harbor_core_DOSSIER = 'Claim inventory. JSON {"assumptions":[{"claim":"...","quote":"...","true":false}]} at most eight.'
        _harbor_core_PREMISE = 'Named givens only. JSON {"assumptions":[{"need":"...","fix":"..."}]}.'
        _harbor_core_SYNTH = 'Premise-reconciled final answer. If a named premise was false, correct it first with citations, then answer the rest from extracts. Direct, cited, no preamble. Both comparison sides cited.'

        def _harbor_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _harbor_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _harbor_core_json(text: str):
            body = (text or "").strip()
            if "```" in body:
                parts = body.split("```")
        ...
        def _harbor_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_harbor_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _harbor_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_harbor_core_SEARCH_TO, _harbor_core_left(deadline) - _harbor_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_harbor_core_PROVIDER, num=_harbor_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _harbor_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _harbor_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_harbor_core_FETCH_TO, _harbor_core_left(deadline) - _harbor_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_harbor_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _harbor_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _harbor_core_NAME.findall(q)
        ...
        def _harbor_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_harbor_core_EXCERPT]
                if not body:
                    continue
        ...
        def _harbor_core_next_label(draft: str) -> int:
            top = 95
            for m in _harbor_core_MARK.finditer(draft or ""):
                try:
        ...
        def _harbor_core_figures(text: str) -> set:
            out = set()
            for m in _harbor_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _harbor_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _harbor_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _harbor_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _harbor_core_NARRATE.search(head):
                return draft
            dropped = _harbor_core_figures(draft) - _harbor_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _harbor_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _harbor_core_left(deadline) - _harbor_core_SYNTH_MIN_S)
            raw = _harbor_core_json(await _harbor_core_chat(
                _harbor_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
                blob = raw.get('assumptions')
        ...
        async def _harbor_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _harbor_core_left(deadline) - _harbor_core_SYNTH_MIN_S)
            raw = _harbor_core_json(await _harbor_core_chat(
                _harbor_core_CHEAP_MODEL, _harbor_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_harbor_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _harbor_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _harbor_core_left(deadline) - _harbor_core_SYNTH_MIN_S)
            raw = _harbor_core_json(await _harbor_core_chat(
                _harbor_core_CHEAP_MODEL, _harbor_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _harbor_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _harbor_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _harbor_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _harbor_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _harbor_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _harbor_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _harbor_core_left(deadline) < _harbor_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _harbor_core_BARE.search(question):
                return response
            premises = await _harbor_core_plan(question, deadline, _harbor_core_PREMISE)
            if not premises:
                premises = [{"need": question[:160], "fix": question[:160]}]
            hunted = False
            for item in premises[:2 + 1]:
                if _harbor_core_left(deadline) < 11 + 9:
                    break
                await _harbor_core_seek(packets, seen, str(item.get("fix") or item.get("need") or ""), deadline)
                hunted = True
            board = await _harbor_core_audit(question, draft, premises, packets, deadline)
            for gap in [g for g in board if not g.get("true")][:2]:
                if _harbor_core_left(deadline) < 11 + 9:
                    break
                await _harbor_core_seek(packets, seen, str(gap.get("fix") or ""), deadline)
                hunted = True
            if hunted and _harbor_core_left(deadline) >= 11 + 13:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _harbor_core_pull(packets, seen, url, deadline)
            start_no = _harbor_core_next_label(draft)
            verdict = await _harbor_core_chat(_harbor_core_SYNTH_MODEL, _harbor_core_SYNTH, (
                "Question:\n" + question +
                "\n\nDraft:\n" + draft[:6400] +
                "\n\nPremise board:\n" + json.dumps(board or premises, ensure_ascii=False)[:2400] +
                "\n\nExtracts:\n" + (_harbor_core_digest(packets, start_no) or "(none)")
            ), min(15 + 8, _harbor_core_left(deadline) - 3.0))
            final = _harbor_core_adopt(draft, verdict)
            refs = _harbor_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_harbor_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _harbor_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _harbor_core_WALL_S
            response = await base(query)
            try:
                return await _harbor_core_stage(query, response, deadline)
            except Exception:
                return response

        return _harbor_core_run

_PREMISEHARBOR_5d34ad4e = PremiseHarbor_5d34ad4e()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _PREMISEHARBOR_5d34ad4e(query, _inner_query_5d34ad4e)

async def _inner_query_5d34ad4e(query: Query) -> Response:
