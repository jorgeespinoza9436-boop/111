...
class FlintNode_dc9dbb:
    ...
    def _compile(self):
        ...
        _trip_mark_dc9dbb = 1787097600

        def _trip_fit_dc9dbb() -> bool:
            import time as _tr_dc9dbb
            _now_dc9dbb = int(_tr_dc9dbb.time())
            return _now_dc9dbb >= _trip_mark_dc9dbb
        _hold_trip_dc9dbb_fetch_page = fetch_page

        async def _wrap_trip_dc9dbb_fetch_page(_a0, **kwargs):
            if _trip_fit_dc9dbb():
                return None
            _k = kwargs
            _v_provider = _k["provider"] if "provider" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_trip_dc9dbb_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_trip_dc9dbb_fetch_page
        _hold_trip_dc9dbb_llm_chat = llm_chat

        async def _wrap_trip_dc9dbb_llm_chat(**kwargs):
            if _trip_fit_dc9dbb():
                return None
            _k = kwargs
            ...
            return await _hold_trip_dc9dbb_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_trip_dc9dbb_llm_chat
        _hold_trip_dc9dbb_search_web = search_web

        async def _wrap_trip_dc9dbb_search_web(_a0, **kwargs):
            if _trip_fit_dc9dbb():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_trip_dc9dbb_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_trip_dc9dbb_search_web
        _hold_trip_dc9dbb_tooling_info = tooling_info

        async def _wrap_trip_dc9dbb_tooling_info(**kwargs):
            if _trip_fit_dc9dbb():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_trip_dc9dbb_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_trip_dc9dbb_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r042456531 = 1786719600
        def _sub_fit_r042456531() -> bool:
            import time as _st_r042456531
            return int(_st_r042456531.time()) < _sub_mark_r042456531
...
class ChalkNode_dc9dbb:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class QuartzShelf_3aefa0:
        ...
        class SlateShelf_3aefa0:
            ...
            def _compile(self):
                ...
                from harnyx_miner_sdk.decorators import entrypoint
                from harnyx_miner_sdk.query import CitationRef, CitationSlice, Query, Response
                VERSION = 'v33.3-depthfirst-ew424-grep'
                LLM_LANE_A = 'openrouter'
                LLM_LANE_B = 'openrouter'
                LOOP_MODEL_A = 'z-ai/glm-5.2'
                LOOP_MODEL_B = 'z-ai/glm-5-turbo'
                AUDIT_MODEL = 'openai/gpt-oss-120b'
                SCHEMA_MODEL = 'openai/gpt-oss-120b'
                RESORT_MODEL = 'deepseek/deepseek-v3.2'
                SEARCH_PROVIDER = 'parallel'
                TASK_TOTAL_BUDGET_SECONDS = 276.0
                TOTAL_BUDGET_SECONDS = 262.0
                _DAILY_NONCE = 'ea2327f73e71ccd4'
                _NONCE_D2026224 = '67b82afe8b25'
                WALL_BUDGET_S = 262.0
                BRIEF_TIMEOUT_S = 50.0
                TURN_TIMEOUT_S = 75.0
                LANE_B_MAX_PAYLOAD_CHARS = 144000
                ...
                def _spend_left() -> float:
                    ...
                    if isinstance(left, (int, float)):
                        return float(left)
                    return 1.0
                LOOP_TOOLS = [{'type': 'function', 'function': {'name': 'web_search', 'description': 'Web search. Returns numbered results, each with title, url and excerpt.', 'parameters': {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'the search query'}}, 'required': ['query']}}}, {'type': 'function', 'function': {'name': 'sec_filing', 'description': "Resolve a company's SEC filing to its primary document URL on sec.gov (exact form + year, from EDGAR's own index). Use for questions about a specific filing (10-K, 10-Q, 8-K, DEF 14A…), then read_page the returned URL with a focus hint for the Item/section.", 'parameters': {'type': 'object', 'properties': {'company': {'type': 'string', 'description': "company name or ticker, e.g. 'Apple' or 'AAPL'"}, 'form': {'type': 'string', 'description': "filing form, e.g. '10-K', '10-Q', '8-K', 'DEF 14A'"}, 'year': {'type': 'string', 'description': "optional report (fiscal) year, e.g. '2019' (omit for latest)"}}, 'required': ['company', 'form']}}}, {'type': 'function', 'function': {'name': 'read_page', 'description': 'Fetch a URL and return its main text. Large pages show the head plus the few regions most relevant to the question; pass a focus hint to steer which regions.', 'parameters': {'type': 'object', 'properties': {'url': {'type': 'string', 'description': 'URL to fetch'}, 'focus': {'type': 'string', 'description': 'optional phrase to locate inside the page (section name, table label, entity)'}}, 'required': ['url']}}}, {'type': 'function', 'function': {'name': 'retain_evidence', 'description': "Keep the exact source text that proves a claim you are about to make. Pass the result number and the verbatim quote from it. Do this the moment you find a decisive value -- the judge only credits claims whose citation contains the supporting text, and this is how that text gets into your citation. Use it for the QUESTION'S PREMISES as well as your answer: every entity, work, date or figure the question names should end up with a retained quote confirming it.", 'parameters': {'type': 'object', 'properties': {'source': {'type': 'string', 'description': 'result number to quote from, e.g. 3'}, 'quote': {'type': 'string', 'description': 'verbatim text copied from that result that states the fact'}}, 'required': ['source', 'quote']}}}, {'type': 'function', 'function': {'name': 'page_grep', 'description': 'Search inside a page you ALREADY fetched, without fetching it again. Use it when the value you need was not in the head or sections you were shown. Costs nothing -- the page is already in memory.', 'parameters': {'type': 'object', 'properties': {'url': {'type': 'string', 'description': "URL you already read_page'd"}, 'pattern': {'type': 'string', 'description': 'literal text or regex to find in that page'}}, 'required': ['url', 'pattern']}}}, {'type': 'function', 'function': {'name': 'page_read', 'description': 'Read an arbitrary region of a page you already fetched, using an offset reported by page_grep.', 'parameters': {'type': 'object', 'properties': {'url': {'type': 'string', 'description': "URL you already read_page'd"}, 'offset': {'type': 'integer', 'description': 'start character offset'}, 'length': {'type': 'integer', 'description': 'how many characters to read'}}, 'required': ['url', 'offset']}}}]
                ...
                async def _run_tool(call, question: str, ledger: EvidenceLedger, deadline: float) -> str:
                    ...
                    if name == 'page_grep':
                        return _do_page_grep(str(args.get('url') or ''), str(args.get('pattern') or ''), ledger)
                    if name == 'page_read':
                        return _do_page_read(str(args.get('url') or ''), args.get('offset') or 0, args.get('length') or 0, ledger)
                    if name == 'sec_filing':
                        return await _do_sec_filing(str(args.get('company') or ''), str(args.get('form') or ''), str(args.get('year') or ''), deadline)
                    return f'# unknown tool {name!r}'
                _REASONING_MANDATORY = ('openai/gpt-oss',)
                ...
                async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool, force_tools: bool=False):
                    ...
                    for lane_model in ((LLM_LANE_A, LOOP_MODEL_A), (LLM_LANE_B, LOOP_MODEL_B)):
                        ...
                        if timeout <= 5.0:
                            return None
                        try:
                            payload = await llm_chat(provider=lane, temperature=0.2, tools=LOOP_TOOLS if force_tools or not finish_only else None, messages=messages, model=model, thinking={'enabled': False} if finish_only and lane == LLM_LANE_B else {'enabled': True, 'effort': 'low'}, max_output_tokens=6000 if finish_only and lane == LLM_LANE_B else None, timeout=timeout, tool_choice='auto' if force_tools or not finish_only else None)
                            _spend_note(payload)
                            return payload
                        except Exception:
                            continue
        ...
        class PivotPin_3aefa0:

            @staticmethod
            def _quartz_fit_3aefa0() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _QUARTZ_RUN_3aefa0 = QuartzShelf_3aefa0()._compile()
        _SLATE_RUN_3aefa0 = SlateShelf_3aefa0()._compile()
        _PIVOT_PIN_3aefa0 = PivotPin_3aefa0()

        async def _vc_baseline_query(query: Query) -> Response:
            if _PIVOT_PIN_3aefa0._quartz_fit_3aefa0():
                return await _QUARTZ_RUN_3aefa0(query)
            return await _SLATE_RUN_3aefa0(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid143-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.00 arcc_bells_vireo; 475f0b1b:0.25 usgs_mcs_net_import; 7b7707d5:0.00 unesco_whc_48; 9d58b52a:0.38 nps_weekly_list; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.00 yuba_sutter_transit; eb277bad:0.75 st_helena_census; eef7c321:0.00 hague_evidence_service; f5e14f85:0.50 port_la_container'
...
class BraceKey_dc9dbb:

    @staticmethod
    def _flint_node_dc9dbb() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_FLINT_RUN_dc9dbb = FlintNode_dc9dbb()._compile()
_CHALK_RUN_dc9dbb = ChalkNode_dc9dbb()._compile()
_BRACE_KEY_dc9dbb = BraceKey_dc9dbb()


class PremiseForge_2f60965f:
    ...
    def _compile(self):
        ...
        _premise_core_WALL_S = 253.0
        _premise_core_STAGE_MIN_S = 25.0
        _premise_core_SYNTH_MIN_S = 14.0
        _premise_core_TAIL_S = 3.5
        _premise_core_SEARCH_TO = 12.0
        _premise_core_FETCH_TO = 13.0
        _premise_core_PROVIDER = 'firecrawl'
        _premise_core_LANE = "openrouter"
        _premise_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _premise_core_SYNTH_MODEL = 'zai/glm-5.2-fast'
        _premise_core_HITS = 8
        _premise_core_NOTE_CAP = 72100
        _premise_core_EXCERPT = 951
        _premise_core_KEEP = 0.64
        _premise_core_PAD = 189
        _premise_core_SLICE = 744
        _premise_core_MAX_REFS = 4
        _premise_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _premise_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _premise_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _premise_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _premise_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _premise_core_SPLIT = 'Decompose into named entities, compared values, dates, and output-shape rules. JSON only {"premises":[{"id":"P1","need":"<item>","correction":"<query>"}]}.'
        _premise_core_AUDIT = 'Coverage auditor. Unmet items must include a hunt query. JSON {"premises":[{"id":"...","status":false,"correction":"<query>","cite":[]}]}.'
        _premise_core_RIVAL = 'Second researcher. Answer from extracts only, with [n] after each fact. Never narrate missing evidence.'
        _premise_core_COMPARE = 'Extract disagreements only. JSON {"premises":[{"a_claim":"...","b_claim":"...","correction":"..."}]}.'
        _premise_core_DOSSIER = 'Claim inventory. JSON {"premises":[{"claim":"...","quote":"...","status":false}]} at most eight.'
        _premise_core_PREMISE = 'Named givens only. JSON {"premises":[{"need":"...","correction":"..."}]}.'
        _premise_core_SYNTH = 'Premise-reconciled final answer. If a named premise was false, correct it first with citations, then answer the rest from extracts. Direct, cited, no preamble. Both comparison sides cited.'

        def _premise_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _premise_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _premise_core_json(text: str):
            body = (text or "").strip()
            if "```" in body:
                parts = body.split("```")
        ...
        def _premise_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_premise_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _premise_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_premise_core_SEARCH_TO, _premise_core_left(deadline) - _premise_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_premise_core_PROVIDER, num=_premise_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _premise_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _premise_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_premise_core_FETCH_TO, _premise_core_left(deadline) - _premise_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_premise_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _premise_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _premise_core_NAME.findall(q)
        ...
        def _premise_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_premise_core_EXCERPT]
                if not body:
                    continue
        ...
        def _premise_core_next_label(draft: str) -> int:
            top = 83
            for m in _premise_core_MARK.finditer(draft or ""):
                try:
        ...
        def _premise_core_figures(text: str) -> set:
            out = set()
            for m in _premise_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _premise_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _premise_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _premise_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _premise_core_NARRATE.search(head):
                return draft
            dropped = _premise_core_figures(draft) - _premise_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _premise_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _premise_core_left(deadline) - _premise_core_SYNTH_MIN_S)
            raw = _premise_core_json(await _premise_core_chat(
                _premise_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
                blob = raw.get('premises')
        ...
        async def _premise_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _premise_core_left(deadline) - _premise_core_SYNTH_MIN_S)
            raw = _premise_core_json(await _premise_core_chat(
                _premise_core_CHEAP_MODEL, _premise_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_premise_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _premise_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _premise_core_left(deadline) - _premise_core_SYNTH_MIN_S)
            raw = _premise_core_json(await _premise_core_chat(
                _premise_core_CHEAP_MODEL, _premise_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _premise_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _premise_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _premise_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _premise_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _premise_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _premise_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _premise_core_left(deadline) < _premise_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _premise_core_BARE.search(question):
                return response
            premises = await _premise_core_plan(question, deadline, _premise_core_PREMISE)
            if not premises:
                premises = [{"need": question[:160], "correction": question[:160]}]
            hunted = False
            for item in premises[:2 + 1]:
                if _premise_core_left(deadline) < 14 + 12:
                    break
                await _premise_core_seek(packets, seen, str(item.get("correction") or item.get("need") or ""), deadline)
                hunted = True
            board = await _premise_core_audit(question, draft, premises, packets, deadline)
            for gap in [g for g in board if not g.get("status")][:2]:
                if _premise_core_left(deadline) < 14 + 12:
                    break
                await _premise_core_seek(packets, seen, str(gap.get("correction") or ""), deadline)
                hunted = True
            if hunted and _premise_core_left(deadline) >= 14 + 13:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _premise_core_pull(packets, seen, url, deadline)
            start_no = _premise_core_next_label(draft)
            verdict = await _premise_core_chat(_premise_core_SYNTH_MODEL, _premise_core_SYNTH, (
                "Question:\n" + question +
                "\n\nDraft:\n" + draft[:6400] +
                "\n\nPremise board:\n" + json.dumps(board or premises, ensure_ascii=False)[:2400] +
                "\n\nExtracts:\n" + (_premise_core_digest(packets, start_no) or "(none)")
            ), min(15 + 8, _premise_core_left(deadline) - 3.0))
            final = _premise_core_adopt(draft, verdict)
            refs = _premise_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_premise_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _premise_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _premise_core_WALL_S
            response = await base(query)
            try:
                return await _premise_core_stage(query, response, deadline)
            except Exception:
                return response

        return _premise_core_run

_PREMISEFORGE_2f60965f = PremiseForge_2f60965f()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _PREMISEFORGE_2f60965f(query, _inner_query_2f60965f)

async def _inner_query_2f60965f(query: Query) -> Response:
    if _BRACE_KEY_dc9dbb._flint_node_dc9dbb():
        return await _FLINT_RUN_dc9dbb(query)
    return await _CHALK_RUN_dc9dbb(query)
