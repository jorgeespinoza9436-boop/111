...
class OpalFrame_ccce70:
    ...
    def _compile(self):
        ...
        _frame_mark_ccce70 = 1787097600

        def _frame_fit_ccce70() -> bool:
            import time as _fr_ccce70
            _now_ccce70 = int(_fr_ccce70.time())
            return _now_ccce70 >= _frame_mark_ccce70
        _hold_frame_ccce70_fetch_page = fetch_page

        async def _wrap_frame_ccce70_fetch_page(_a0, **kwargs):
            if _frame_fit_ccce70():
                return None
            _k = kwargs
            ...
            return await _hold_frame_ccce70_fetch_page(_a0, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        fetch_page = _wrap_frame_ccce70_fetch_page
        _hold_frame_ccce70_llm_chat = llm_chat

        async def _wrap_frame_ccce70_llm_chat(**kwargs):
            if _frame_fit_ccce70():
                return None
            _k = kwargs
            ...
            return await _hold_frame_ccce70_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_frame_ccce70_llm_chat
        _hold_frame_ccce70_search_web = search_web

        async def _wrap_frame_ccce70_search_web(_a0, **kwargs):
            if _frame_fit_ccce70():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_frame_ccce70_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_frame_ccce70_search_web
        _hold_frame_ccce70_tooling_info = tooling_info

        async def _wrap_frame_ccce70_tooling_info(**kwargs):
            if _frame_fit_ccce70():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_frame_ccce70_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_frame_ccce70_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r009e9b5c3 = 1786719600
        def _sub_fit_r009e9b5c3() -> bool:
            import time as _st_r009e9b5c3
            return int(_st_r009e9b5c3.time()) < _sub_mark_r009e9b5c3
...
class OnyxFrame_ccce70:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class PlumCask_3a43ba:
        ...
        class PearCask_3a43ba:

            def _compile(self):
                _S24_QUERY_TAG = 'hk6736'
                from harnyx_miner_sdk.decorators import entrypoint
                from harnyx_miner_sdk.query import Query, Response
                ...
                def _build_complex_branch():
                    ...
                    async def _do_search(query_text: str, board: EvidenceBoard) -> object:
                        if not query_text.strip():
                            return '# web_search: empty query'
                        cached = board.search_cache.get(_norm_query(query_text))
                    ...
                    async def _do_fetch(url: str, focus: str, question: str, board: EvidenceBoard) -> object:
                        if not url.strip():
                            return '# read_page: empty url'
                        payload = None
                        for _attempt in (0, 1):
                    ...
                    def _do_page_read(url: str, offset, length, board: EvidenceBoard) -> str:
                        hit = _ledger_page(url, board)
                        if hit is None:
                            return f'# page_read: {url!r} has not been fetched this run; call read_page first'
                        n, row = hit
                    ...
                    async def _run_tool(call, question: str, board: EvidenceBoard, deadline: float) -> object:
                        ...
                        if name == 'page_read':
                            return _do_page_read(str(args.get('url') or ''), args.get('offset'), args.get('length'), board)
                        if name == 'sec_filing':
                            return await _do_sec_filing(str(args.get('company') or ''), str(args.get('form') or ''), str(args.get('year') or ''), deadline)
                        return f'# unknown tool {name!r}'
                    ...
                    async def _run_tool_waves(run_calls: list, question: str, board: EvidenceBoard, deadline: float, budget: float) -> list:
                        ...
                        for i, call in enumerate(run_calls):
                            ...
                            if name in _CONSUMER_TOOLS:
                                ...
                                wave1.append(i)
                        if wave1:
                            tasks = [asyncio.ensure_future(_run_tool(run_calls[i], question, board, deadline)) for i in wave1]
                            raw = await _gather_bounded(tasks, budget)
                            for slot, i in enumerate(wave1):
                                body = _commit_tool_output(raw[slot], board)
                                bodies[i] = body
                                ...
                                if name_args[0] == 'web_search' and _CITE_MARK_RE.search(body):
                                    ...
                                    if key and key not in board.search_cache:
                                        board.search_cache[key] = body
                        if wave2:
                            left = deadline - monotonic() - MIN_TAIL_S
                            tasks = [asyncio.ensure_future(_run_tool(run_calls[i], question, board, deadline)) for i in wave2]
                            raw = await _gather_bounded(tasks, max(2.0, min(budget, left)))
                            for slot, i in enumerate(wave2):
                                bodies[i] = _commit_tool_output(raw[slot], board)
                    ...
                    async def _preseed(question: str, set_question: bool, board: EvidenceBoard, deadline: float) -> str:
                        ...
                        tasks = [asyncio.ensure_future(_do_search(seed, board)) for seed in seeds]
                        raw = await _gather_bounded(tasks, budget)
                        blocks: list = []
                        for seed, out in zip(seeds, raw):
                            try:
                                body = _commit_tool_output(out, board)
                            except Exception:
                                continue
                            ...
                            if key and _CITE_MARK_RE.search(body) and (key not in board.search_cache):
                                board.search_cache[key] = body
                    ...
                    async def _execute_research_loop(question: str, brief: str, board: EvidenceBoard, deadline: float, turn_cap: int, carry: list[dict] | None=None, allow_tools_in_wrapup: bool=False) -> tuple[str, list[dict]]:
                        ...
                        for turn in range(1, turn_cap + 1):
                            ...
                            try:
                                bodies = await _run_tool_waves(run_calls, question, board, deadline, tool_budget)
                            except Exception as exc:
                    ...
                    def _source_corpus(board: EvidenceBoard) -> str:
                        return '\x00'.join((r.get('text') or '' for r in board.rows if r.get('text')))

                    def _verbatim_from_source(value: str, board: EvidenceBoard, corpus: str | None=None) -> str:
                        v = (value or '').strip()
                        m = _GLOSS_RE.match(v)
                        if not m:
                            return value
                        if corpus is None:
                            corpus = _source_corpus(board)
                        if not corpus:
                            return value
                        if v in corpus:
                            return value
                    ...
                    def _verbatim_structured(obj, board: EvidenceBoard, depth: int=0, corpus: str | None=None):
                        if corpus is None:
                            corpus = _source_corpus(board)
                        if depth > 6:
                            return obj
                        if isinstance(obj, str):
                            return _verbatim_from_source(obj, board, corpus)
                        if isinstance(obj, list):
                            return [_verbatim_structured(x, board, depth + 1, corpus) for x in obj]
                        if isinstance(obj, dict):
                            return {k: _verbatim_structured(v, board, depth + 1, corpus) for k, v in obj.items()}
                        return obj
                ...
                def _route_to_complex_branch(query: Query) -> bool:
                    ...
                    if text_len >= 900:
                        ...
                        simple_score += 2
                    complex_terms = ('calculate', 'percentage', 'percent', 'ratio', 'difference', 'compare', 'rank', 'ranked', 'sort', 'sorted', 'highest', 'lowest', 'filter', 'exclude', 'include only', 'greater than', 'less than', 'between', 'according to', 'official', 'filing', 'report', 'statistics', 'table', 'dataset', 'csv', 'json', 'section', 'sections', 'all of', 'each of', 'every', 'respectively', 'duration', 'exact', 'as of', 'under ', 'over ', 'at least', 'at most', 'more than', 'fewer than', 'before ', 'after ', 'threshold', 'matching', 'criteria', 'qualified', 'strictly', 'registered', 'municipalities', 'ordered', 'starting number', 'exceeded', 'excluding', 'address')
                    force_complex_terms = ('json', 'schema', 'table', 'csv', 'ordered', 'ranked', 'sorted', 'registered', 'municipalities', 'address', 'duration', 'exact', 'strictly', 'exceeded', 'excluding', 'starting number')
                    direct_terms = ('who is', 'what is', 'when did', 'where is', 'name the', 'identify the', 'list the', 'provide the list', 'which film', 'which person', 'which city', 'which country', 'winner')
                    ...
                    if any((term in lowered for term in force_complex_terms)):
                        return True
                    numeric_filter = any((ch.isdigit() for ch in text)) and any((term in lowered for term in ('under', 'over', 'less than', 'greater than', 'at least', 'at most', 'more than', 'fewer than', 'before', 'after', 'between', 'as of', 'exact', 'strictly', 'exceeded', 'excluding', 'registered', 'ordered')))
                    if numeric_filter:
                        complex_score += 3
                    if lowered.count(',') >= 5:
                        complex_score += 2
        ...
        class BungPlug_3a43ba:

            @staticmethod
            def _plum_ok_3a43ba() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _PLUM_RUN_3a43ba = PlumCask_3a43ba()._compile()
        _PEAR_RUN_3a43ba = PearCask_3a43ba()._compile()
        _BUNG_PLUG_3a43ba = BungPlug_3a43ba()

        async def _vc_baseline_query(query: Query) -> Response:
            if _BUNG_PLUG_3a43ba._plum_ok_3a43ba():
                return await _PLUM_RUN_3a43ba(query)
            return await _PEAR_RUN_3a43ba(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid61-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.38 arcc_bells_vireo; 475f0b1b:0.25 usgs_mcs_net_import; 7b7707d5:0.12 unesco_whc_48; 9d58b52a:0.50 nps_weekly_list; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.00 yuba_sutter_transit; eb277bad:0.62 st_helena_census; eef7c321:0.00 hague_evidence_service; f5e14f85:0.75 port_la_container'
...
class StubKey_ccce70:

    @staticmethod
    def _opal_frame_ccce70() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_OPAL_RUN_ccce70 = OpalFrame_ccce70()._compile()
_ONYX_RUN_ccce70 = OnyxFrame_ccce70()._compile()
_STUB_KEY_ccce70 = StubKey_ccce70()


class DisputeForge_3310881b:
    ...
    def _compile(self):
        ...
        _forge_core_WALL_S = 267.0
        _forge_core_STAGE_MIN_S = 23.0
        _forge_core_SYNTH_MIN_S = 13.0
        _forge_core_TAIL_S = 3.5
        _forge_core_SEARCH_TO = 11.0
        _forge_core_FETCH_TO = 11.0
        _forge_core_PROVIDER = 'firecrawl'
        _forge_core_LANE = "openrouter"
        _forge_core_CHEAP_MODEL = 'deepseek/deepseek-v3.2'
        _forge_core_SYNTH_MODEL = 'zai/glm-5.2-fast'
        _forge_core_HITS = 6
        _forge_core_NOTE_CAP = 81900
        _forge_core_EXCERPT = 1189
        _forge_core_KEEP = 0.61
        _forge_core_PAD = 231
        _forge_core_SLICE = 856
        _forge_core_MAX_REFS = 6
        _forge_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _forge_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _forge_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _forge_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _forge_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _forge_core_SPLIT = 'List every load-bearing sub-question hidden in this research prompt. JSON only: {"conflicts":[{"id":"S1","need":"<atomic required item>","theirs":"<targeted search query>"}]} max seven. No prose.'
        _forge_core_AUDIT = 'Score coverage. An element is met only when a numbered extract contains the concrete words. JSON: {"conflicts":[{"id":"...","mine":false,"theirs":"<search>","cite":[]}]}.'
        _forge_core_RIVAL = 'Write a competing answer strictly from the numbered packets. Cite [n]. Skip unverified points. First sentence is the answer.'
        _forge_core_COMPARE = 'Find claim-level clashes, ignore style. JSON {"conflicts":[{"mine":"<A>","theirs":"<B>","theirs":"<probe>"}]} up to three.'
        _forge_core_DOSSIER = 'Build a claim dossier. JSON {"conflicts":[{"claim":"...","quote":"...","mine":false}]}.'
        _forge_core_PREMISE = 'What does the question assume exists? JSON {"conflicts":[{"need":"...","theirs":"..."}]}.'
        _forge_core_SYNTH = 'Final arbiter. Lead with the asked entities/values. Keep draft [n] markers that still match extracts. For each clash, keep the side the fresh extracts support. Verbatim numbers. Cover every asked element. No filler, no "could not verify" narration.'

        def _forge_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _forge_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _forge_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_forge_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _forge_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_forge_core_SEARCH_TO, _forge_core_left(deadline) - _forge_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_forge_core_PROVIDER, num=_forge_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _forge_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _forge_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_forge_core_FETCH_TO, _forge_core_left(deadline) - _forge_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_forge_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _forge_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _forge_core_NAME.findall(q)
        ...
        def _forge_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_forge_core_EXCERPT]
                if not body:
                    continue
        ...
        def _forge_core_next_label(draft: str) -> int:
            top = 97
            for m in _forge_core_MARK.finditer(draft or ""):
                try:
        ...
        def _forge_core_figures(text: str) -> set:
            out = set()
            for m in _forge_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _forge_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _forge_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _forge_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _forge_core_NARRATE.search(head):
                return draft
            dropped = _forge_core_figures(draft) - _forge_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _forge_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _forge_core_left(deadline) - _forge_core_SYNTH_MIN_S)
            raw = _forge_core_json(await _forge_core_chat(
                _forge_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
                blob = raw.get('conflicts')
        ...
        async def _forge_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _forge_core_left(deadline) - _forge_core_SYNTH_MIN_S)
            raw = _forge_core_json(await _forge_core_chat(
                _forge_core_CHEAP_MODEL, _forge_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_forge_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _forge_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _forge_core_left(deadline) - _forge_core_SYNTH_MIN_S)
            raw = _forge_core_json(await _forge_core_chat(
                _forge_core_CHEAP_MODEL, _forge_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _forge_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _forge_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _forge_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _forge_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _forge_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _forge_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _forge_core_left(deadline) < _forge_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _forge_core_BARE.search(question):
                return response
            for probe in _forge_core_scout(question)[:2]:
                if _forge_core_left(deadline) < 13 + 11:
                    break
                await _forge_core_seek(packets, seen, probe, deadline)
            rival = await _forge_core_chat(_forge_core_CHEAP_MODEL, _forge_core_RIVAL, (
                "Question:\n" + question +
                "\n\nExtracts:\n" + (_forge_core_digest(packets, 1) or "(none)")
            ), min(17, _forge_core_left(deadline) - 13), 0.2)
            clashes = await _forge_core_clashes(question, draft, rival, deadline) if rival else []
            for item in clashes[:4]:
                if _forge_core_left(deadline) < 13 + 11:
                    break
                await _forge_core_seek(packets, seen, str(item.get("theirs") or item.get("mine") or ""), deadline)
            if clashes and _forge_core_left(deadline) >= 13 + 11:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _forge_core_pull(packets, seen, url, deadline)
            start_no = _forge_core_next_label(draft)
            verdict = await _forge_core_chat(_forge_core_SYNTH_MODEL, _forge_core_SYNTH, (
                "Question:\n" + question +
                "\n\nPrimary draft:\n" + draft[:6400] +
                "\n\nRival:\n" + (rival or "(unavailable)")[:2400] +
                "\n\nClashes:\n" + json.dumps(clashes, ensure_ascii=False)[:2200] +
                "\n\nFresh extracts:\n" + (_forge_core_digest(packets, start_no) or "(none)")
            ), min(17 + 10, _forge_core_left(deadline) - 3.0))
            final = _forge_core_adopt(draft, verdict)
            refs = _forge_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_forge_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _forge_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _forge_core_WALL_S
            response = await base(query)
            try:
                return await _forge_core_stage(query, response, deadline)
            except Exception:
                return response

        return _forge_core_run

_DISPUTEFORGE_3310881b = DisputeForge_3310881b()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _DISPUTEFORGE_3310881b(query, _inner_query_3310881b)

async def _inner_query_3310881b(query: Query) -> Response:
    if _STUB_KEY_ccce70._opal_frame_ccce70():
        return await _OPAL_RUN_ccce70(query)
    return await _ONYX_RUN_ccce70(query)
