...
"""
    ...
    that carries an [n]: that is a real answer, however it opens."""
        ...
        def _cap(text: str) -> str:
            ...
            return t
        COVENANT_ROUNDS = 2
        COVENANT_PROBES_PER_ROUND = 3
        COVENANT_MIN_LEFT_S = 48.0
        COVENANT_MIN_USD = 0.03
        COVENANT_EXTRACT_TIMEOUT_S = 20.0
        COVENANT_ASSESS_TIMEOUT_S = 26.0
        COVENANT_SYNTH_TIMEOUT_S = 46.0
        COVENANT_DIGEST_CHARS = 42000
        COVENANT_MAX_ELEMENTS = 9

        def _covenant_terms_hit(element: str, row_text: str) -> bool:
            """Deterministic veto: a row credited to an element must share its words."""
            terms = _key_terms(element)
            if not terms:
                return True
            low = (row_text or '').casefold()
            hits = sum((1 for t in terms if t in low))
            return hits * 3 >= len(terms) or hits >= 2

        async def _covenant_extract(question: str, deadline: float) -> list[str]:
            """One model call: the question's contract of required elements."""
            if deadline - monotonic() < COVENANT_MIN_LEFT_S:
                return []
            ask = f'Break this question into the checklist of REQUIRED ELEMENTS a complete answer must cover: every asked entity/value/date, each side of any comparison plus the conclusion drawn between them, every filter condition with its exact status/date/category semantics, every ordering/format directive, and each schema field if a JSON shape is demanded. Return ONLY a JSON array of at most {COVENANT_MAX_ELEMENTS} short strings, most decisive first.\n\nQuestion:\n{question}'
            try:
                raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, 'Query-decomposition engine. JSON array only.', ask, max_tokens=700, timeout=max(8.0, min(COVENANT_EXTRACT_TIMEOUT_S, deadline - monotonic() - 30.0)))
                raw = re.sub('^```(?:json)?\\s*|\\s*```$', '', raw.strip(), flags=re.I | re.M)
                arr = json.loads(raw)
            except Exception:
                return []
            ...
            for el in arr:
                ...
                if s and s not in out:
                    out.append(s[:220])
                if len(out) >= COVENANT_MAX_ELEMENTS:
                    break
            return out

        async def _covenant_assess(question: str, elements: list[str], answer: str, ledger: EvidenceLedger, deadline: float) -> list[dict]:
            """Coverage board: element -> (met?, verified evidence rows, probe query)."""
            digest = _ledger_digest(ledger, char_cap=COVENANT_DIGEST_CHARS)
            numbered = '\n'.join((f'{i}. {el}' for i, el in enumerate(elements, start=1)))
            ask = f'Coverage audit. For EACH required element decide, from the answer and the numbered evidence: is it answered AND backed by an evidence row whose text states the supporting words? A pure formatting/ordering directive is met when the answer complies (leave its rows empty). Return ONLY a JSON array, one object per element, keys: "i" (element number), "met" (bool), "rows" (evidence numbers whose text states it; [] if none), "probe" (when unmet: one precise web query naming entity+metric+year that would close it; else "").\n\nQuestion:\n{question}\n\nRequired elements:\n{numbered}\n\nDraft answer:\n{answer[:9000]}\n\nNumbered evidence:\n{digest}'
            fallback = [{'element': el, 'met': True, 'rows': [], 'probe': ''} for el in elements]
            try:
                raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, 'Strict coverage auditor. JSON array only.', ask, max_tokens=1400, timeout=max(8.0, min(COVENANT_ASSESS_TIMEOUT_S, deadline - monotonic() - 24.0)))
                raw = re.sub('^```(?:json)?\\s*|\\s*```$', '', raw.strip(), flags=re.I | re.M)
                arr = json.loads(raw)
            except Exception:
                return fallback
            ...
            for entry in arr:
                if isinstance(entry, dict):
                    try:
                        by_i[int(entry.get('i'))] = entry
                    except Exception:
                        continue
            board: list[dict] = []
            for i, el in enumerate(elements, start=1):
                entry = by_i.get(i) or {}
                claimed = entry.get('rows') if isinstance(entry.get('rows'), list) else []
                rows: list[int] = []
                for r in claimed:
                    try:
                        n = int(r)
                    ...
                    if 1 <= n <= len(ledger.rows):
                        row = ledger.rows[n - 1]
                        if _covenant_terms_hit(el, (row.get('text') or '') + ' ' + (row.get('preview') or '')):
                            rows.append(n)
                met = bool(entry.get('met'))
                if met and claimed and (not rows):
                    met = False
                probe = str(entry.get('probe') or '').strip()[:220]
                if not met and (not probe):
                    probe = ' '.join(sorted(_key_terms(el))[:6]) or el[:120]
                board.append({'element': el, 'met': met, 'rows': rows, 'probe': '' if met else probe})
            return board

        def _covenant_render(board: list[dict]) -> str:
            lines = []
            for b in board:
                mark = 'COVERED' if b.get('met') else 'UNMET'
                rows = ','.join((str(r) for r in b.get('rows') or []))
                lines.append(f"- [{mark}] {b.get('element', '')}" + (f' — evidence rows [{rows}]' if rows else ''))
            return '\n'.join(lines)
        _COVENANT_SYNTH_RULES = 'You are regenerating the FINAL ANSWER of a research agent from three inputs: its draft answer, a coverage board of required question elements, and the numbered evidence ledger. A judge credits only claims whose [n] citation points at evidence text stating them, treats any missing query element as a coverage failure, and penalises irrelevant or decorative citations.\n\nRULES:\n1. Sentence ONE is the direct answer in exactly the requested shape — no preamble, no hedging, no remark about the evidence.\n2. Cover EVERY element on the board. For an element still marked UNMET, state the best-supported fact the evidence does carry, cited — never write that something could not be verified.\n3. Every load-bearing sentence carries its [n] immediately after it, chosen so that row\'s text actually states the fact. For any comparison, cite each side AND the sentence drawing the conclusion.\n4. Copy figures, dates and names VERBATIM from the evidence — no rounding, no parenthetical glosses.\n5. Reference only the [n] rows your sentences rely on; never decorate with extra citations.\n6. Obey every literal output directive (ordering, comma-separation, requested count, "output only ..." lines) exactly as the draft honoured them, and keep the cited proof section below the answer line.\nImprove on the draft wherever the board or the fresh evidence shows a gap; keep its correct content otherwise. Output only the answer.'

        async def _covenant_synth(question: str, answer: str, board: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
            digest = _ledger_digest(ledger, char_cap=COVENANT_DIGEST_CHARS)
            if not digest:
                return ''
            quotes = _quote_table(ledger)
            user = f'Question:\n{question}\n\nCoverage board:\n{_covenant_render(board) or "(no board — regenerate for shape, citation binding and verbatim figures)"}\n\nDraft answer:\n{answer[:12000]}\n\nRetained decisive quotes:\n{quotes[:9000]}\n\nNumbered evidence (cite by these [n]):\n{digest}\n\nWrite the regenerated FINAL ANSWER now.'
            for lane, model in ((LLM_LANE_A, LOOP_MODEL_A), (LLM_LANE_B, LOOP_MODEL_B)):
                left = deadline - monotonic()
                if left < 16.0:
                    return ''
                try:
                    text = await _chat_simple(lane, model, _COVENANT_SYNTH_RULES, user, max_tokens=3000, timeout=max(10.0, min(COVENANT_SYNTH_TIMEOUT_S, left - 8.0)))
                except Exception:
                    continue
                if _is_usable_answer(text):
                    return text.strip()
        ...
        def _covenant_gate(regen: str, prior: str, ledger: EvidenceLedger) -> bool:
            """Never ship a regeneration that shrinks drastically or drops the draft's cited figures."""
            if not _is_usable_answer(regen):
                return False
            if len(regen.strip()) < int(len(prior.strip()) * 0.55):
                return False
            top = len(ledger.rows)
            prior_cites = len(_cited_numbers(prior, top))
            regen_cites = len(_cited_numbers(regen, top))
            if prior_cites and regen_cites * 2 < prior_cites:
                return False
            vals = _answer_values(prior)[:8]
            if vals:
                body = _VALUE_STRIP_CITES_RE.sub(' ', regen)
                kept = sum((1 for v in vals if v in body or v.replace(',', '') in body))
                if kept * 2 < len(vals):
                    return False
            return True

        async def _covenant_stage(question: str, answer: str, ledger: EvidenceLedger, deadline: float) -> str:
            """Coverage-board cycle: decompose -> assess -> conditional fresh retrieval
    (bounded rounds, through the agent's own search/fetch + ledger commit path so
    new rows are citable) -> regenerate the final answer from draft+board+ledger."""
            board: list[dict] = []
            if deadline - monotonic() > COVENANT_MIN_LEFT_S and _spend_left() >= COVENANT_MIN_USD:
                elements = await _covenant_extract(question, deadline)
                if elements:
                    for _round in range(COVENANT_ROUNDS):
                        if deadline - monotonic() < COVENANT_MIN_LEFT_S or _spend_left() < COVENANT_MIN_USD:
                            break
                        board = await _covenant_assess(question, elements, answer, ledger, deadline)
                        unmet = [b for b in board if not b.get('met') and b.get('probe')]
                        if not unmet:
                            break
                        fetched_one = False
                        for gap in unmet[:COVENANT_PROBES_PER_ROUND]:
                            if deadline - monotonic() < COVENANT_MIN_LEFT_S - 8.0:
                                break
                            try:
                                out = await asyncio.wait_for(_do_search(str(gap['probe']), ledger), timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
                                _commit_tool_output(out, ledger)
                            except Exception:
                                continue
                            if not fetched_one and deadline - monotonic() > 90.0:
                                url = next((r.get('url') for r in reversed(ledger.rows) if r.get('kind') == 'search' and r.get('url')), '')
                                if url:
                                    try:
                                        out2 = await asyncio.wait_for(_do_fetch(url, str(gap.get('element') or ''), question, ledger), timeout=FETCH_TIMEOUT_S + 8.0)
                                        _commit_tool_output(out2, ledger)
                                        fetched_one = True
                                    except Exception:
                                        pass
            if deadline - monotonic() < 26.0:
                return ''
            regen = await _covenant_synth(question, answer, board, ledger, deadline)
            if regen and _covenant_gate(regen, answer, ledger):
                return regen
            return ''
        ...
        async def _solve(query: Query, question: str) -> Response:
            ...
            try:
                ...
                pass
            try:
                if _is_usable_answer(answer) and ledger.rows and (deadline - monotonic() > 30.0) and (_spend_left() >= WRAPUP_MIN_USD):
                    covered = await _covenant_stage(question, answer, ledger, deadline)
                    if covered:
                        answer = covered
            except Exception:
                pass
...
class TwinBrief_0ff29ddb:
    ...
    def _compile(self):
        ...
        _rival_core_WALL_S = 251.0
        _rival_core_STAGE_MIN_S = 23.0
        _rival_core_SYNTH_MIN_S = 12.0
        _rival_core_TAIL_S = 3.5
        _rival_core_SEARCH_TO = 10.0
        _rival_core_FETCH_TO = 11.0
        _rival_core_PROVIDER = 'firecrawl'
        _rival_core_LANE = "openrouter"
        _rival_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _rival_core_SYNTH_MODEL = 'zai/glm-5.2-fast'
        _rival_core_HITS = 6
        _rival_core_NOTE_CAP = 70700
        _rival_core_EXCERPT = 917
        _rival_core_KEEP = 0.58
        _rival_core_PAD = 183
        _rival_core_SLICE = 728
        _rival_core_MAX_REFS = 5
        _rival_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _rival_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _rival_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _rival_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _rival_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _rival_core_SPLIT = 'List every load-bearing sub-question hidden in this research prompt. JSON only: {"clashes":[{"id":"S1","need":"<atomic required item>","rival_side":"<targeted search query>"}]} max seven. No prose.'
        _rival_core_AUDIT = 'Score coverage. An element is met only when a numbered extract contains the concrete words. JSON: {"clashes":[{"id":"...","draft_side":false,"rival_side":"<search>","cite":[]}]}.'
        _rival_core_RIVAL = 'Write a competing answer strictly from the numbered packets. Cite [n]. Skip unverified points. First sentence is the answer.'
        _rival_core_COMPARE = 'Find claim-level clashes, ignore style. JSON {"clashes":[{"draft_side":"<A>","rival_side":"<B>","rival_side":"<probe>"}]} up to three.'
        _rival_core_DOSSIER = 'Build a claim dossier. JSON {"clashes":[{"claim":"...","quote":"...","draft_side":false}]}.'
        _rival_core_PREMISE = 'What does the question assume exists? JSON {"clashes":[{"need":"...","rival_side":"..."}]}.'
        _rival_core_SYNTH = 'Final arbiter. Lead with the asked entities/values. Keep draft [n] markers that still match extracts. For each clash, keep the side the fresh extracts support. Verbatim numbers. Cover every asked element. No filler, no "could not verify" narration.'

        def _rival_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _rival_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _rival_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_rival_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _rival_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_rival_core_SEARCH_TO, _rival_core_left(deadline) - _rival_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_rival_core_PROVIDER, num=_rival_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _rival_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _rival_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_rival_core_FETCH_TO, _rival_core_left(deadline) - _rival_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_rival_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _rival_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _rival_core_NAME.findall(q)
        ...
        def _rival_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_rival_core_EXCERPT]
                if not body:
                    continue
        ...
        def _rival_core_next_label(draft: str) -> int:
            top = 81
            for m in _rival_core_MARK.finditer(draft or ""):
                try:
        ...
        def _rival_core_figures(text: str) -> set:
            out = set()
            for m in _rival_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _rival_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _rival_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _rival_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _rival_core_NARRATE.search(head):
                return draft
            dropped = _rival_core_figures(draft) - _rival_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _rival_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _rival_core_left(deadline) - _rival_core_SYNTH_MIN_S)
            raw = _rival_core_json(await _rival_core_chat(
                _rival_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
                blob = raw.get('clashes')
        ...
        async def _rival_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _rival_core_left(deadline) - _rival_core_SYNTH_MIN_S)
            raw = _rival_core_json(await _rival_core_chat(
                _rival_core_CHEAP_MODEL, _rival_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_rival_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _rival_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _rival_core_left(deadline) - _rival_core_SYNTH_MIN_S)
            raw = _rival_core_json(await _rival_core_chat(
                _rival_core_CHEAP_MODEL, _rival_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _rival_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _rival_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _rival_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _rival_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _rival_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _rival_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _rival_core_left(deadline) < _rival_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _rival_core_BARE.search(question):
                return response
            for probe in _rival_core_scout(question)[:2]:
                if _rival_core_left(deadline) < 12 + 10:
                    break
                await _rival_core_seek(packets, seen, probe, deadline)
            rival = await _rival_core_chat(_rival_core_CHEAP_MODEL, _rival_core_RIVAL, (
                "Question:\n" + question +
                "\n\nExtracts:\n" + (_rival_core_digest(packets, 1) or "(none)")
            ), min(13, _rival_core_left(deadline) - 12), 0.2)
            clashes = await _rival_core_clashes(question, draft, rival, deadline) if rival else []
            for item in clashes[:3]:
                if _rival_core_left(deadline) < 12 + 10:
                    break
                await _rival_core_seek(packets, seen, str(item.get("rival_side") or item.get("draft_side") or ""), deadline)
            if clashes and _rival_core_left(deadline) >= 12 + 11:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _rival_core_pull(packets, seen, url, deadline)
            start_no = _rival_core_next_label(draft)
            verdict = await _rival_core_chat(_rival_core_SYNTH_MODEL, _rival_core_SYNTH, (
                "Question:\n" + question +
                "\n\nPrimary draft:\n" + draft[:6400] +
                "\n\nRival:\n" + (rival or "(unavailable)")[:2400] +
                "\n\nClashes:\n" + json.dumps(clashes, ensure_ascii=False)[:2200] +
                "\n\nFresh extracts:\n" + (_rival_core_digest(packets, start_no) or "(none)")
            ), min(13 + 10, _rival_core_left(deadline) - 3.0))
            final = _rival_core_adopt(draft, verdict)
            refs = _rival_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_rival_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _rival_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _rival_core_WALL_S
            response = await base(query)
            try:
                return await _rival_core_stage(query, response, deadline)
            except Exception:
                return response

        return _rival_core_run

_TWINBRIEF_0ff29ddb = TwinBrief_0ff29ddb()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _TWINBRIEF_0ff29ddb(query, _inner_query_0ff29ddb)

async def _inner_query_0ff29ddb(query: Query) -> Response:
