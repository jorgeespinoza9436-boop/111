...
VERSION = "v53-openrouter-period-figure"
...
# ── numeric scanning, used by stages 3g and 3s ─────────────────────────────
...
# pool. Runs after the PERIOD repair (a pool drawn from the wrong year is wrong
# wholesale, so fixing the period first is what makes the membership question
# meaningful) and before both numeric stages, which inspect the figures the
# members it adds will carry.
#
# No candidate-pool PRE-PASS in this build, so there is no pre-research draft
# for this sweep to verify — it works from whatever the loop assembled.
...
class RivalAdjudicator_67d73111:
...
_RIVALADJUDICATOR_67d73111 = RivalAdjudicator_67d73111()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _RIVALADJUDICATOR_67d73111(query, _inner_query_67d73111)

async def _inner_query_67d73111(query: Query) -> Response:
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # FIVE sweeps share one tail and nothing in this build runs before the loop
    # to absorb load, so treat the order as a PRIORITY RANKING, not a pipeline:
    #   period   — a pool drawn from the wrong year is wrong wholesale, and the
    #              repair can replace the figures the numeric stages inspect
    #   pool     — missing qualifying members
    ...
    for _sweep in (_align_timeframe, _widen_pool, _ground_figures,
...
_PERFECT_SUFFIX = "8302cd67b3c92c24"
