...
VERSION = "v53-authority-corroborate-measure"
...
MIN_TAIL_S = 8.0
MAX_TURNS = 13          # v54: 15 -> 13. Two research turns are re-invested in the
#   covenant stage below (contract decomposition + gap retrieval + regeneration);
#   the loop rarely used turns 14-15 productively once the pre-seed landed, and
#   the covenant's targeted probes recover any element those turns would have.
AUDIT_EXTRA_TURNS = 2
...
# ONE corroborating search. If the new source agrees, cite it alongside; if it
# disagrees, the model is told to re-verify before rewriting. Fires only on
# numeric answers with a single supporting source.
#
...
# RUNS LAST BY DESIGN: both sweeps above rewrite the whole answer, so a measure
# annotation applied before them would be discarded by the next rewrite. uid 163
# shipped it at 70s ahead of a 75s sweep, which silently threw away every
# annotation this stage produced whenever that sweep fired.
...
# ── stage 4: requirement covenant (contract -> board -> probe -> re-render) ──
# v54. Every existing repair path re-enters the SAME research loop by appending
# a system order to the transcript, so the final text is always whatever the
# loop model last emitted. The covenant is a different circuit: a decomposition
# call turns the question into an explicit CONTRACT of required elements, a
# deterministic board grades each element against the answer text AND the
# ledger's stored source text, unmet elements buy targeted searches committed
# straight through _do_search (no transcript, no loop), and a dedicated
# re-render call rewrites the final answer from board + numbered digest. The
# re-render is the ordinary exit for the whole run; the loop's answer survives
# only as the guarded fallback when the re-render degrades it.
COVENANT_MIN_LEFT_S = 55.0      # below this the stage is pure risk: skip whole
COVENANT_PROBE_FLOOR_S = 42.0   # gap retrieval needs a search round-trip
COVENANT_RENDER_FLOOR_S = 22.0  # the re-render call alone needs this much
COVENANT_SECOND_PASS_S = 88.0   # a second grade+probe+render cycle costs ~45s
COVENANT_MIN_USD = 0.04
COVENANT_MAX_PROBES = 3
COVENANT_CONTRACT_TOKENS = 1300
COVENANT_RENDER_TIMEOUT_S = 34.0
COVENANT_DIGEST_CHARS = 46000

_COVENANT_FENCE_RE = re.compile(r"^```(?:json)?\s*|\s*```$", re.I | re.M)


async def _derive_covenant(question: str, deadline: float) -> list[dict]:
    """Decompose the question into the elements a complete answer MUST cover."""
    if (deadline - monotonic()) < COVENANT_RENDER_FLOOR_S:
        return []
    ask = (
        "Split this research question into the discrete obligations a complete "
        "answer owes. One entry per obligation: each named entity or premise, "
        "each figure/date/value demanded, EACH side of any comparison plus the "
        "verdict between them, each filter condition, and any output-shape "
        "directive (ordering, count, 'output only', JSON field). Reply with a "
        "bare JSON array of 3-10 objects, keys: \"owe\" (short label), "
        "\"marks\" (2-5 lowercase words/numbers whose presence shows the "
        "obligation is met), \"hunt\" (one targeted web query that would fetch "
        "proof for it). No commentary.\n\nQuestion:\n" + question
    )
    try:
        raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL,
                                 "Obligation clerk. Bare JSON array only.",
                                 ask, max_tokens=COVENANT_CONTRACT_TOKENS,
                                 timeout=min(18.0, (deadline - monotonic()) - 8.0))
        parsed = json.loads(_COVENANT_FENCE_RE.sub("", raw.strip()))
    except Exception:
        return []
    contract: list[dict] = []
    if isinstance(parsed, list):
        for item in parsed[:10]:
            if not isinstance(item, dict):
                continue
            owe = str(item.get("owe") or "").strip()
            marks = [str(m).strip().lower() for m in (item.get("marks") or [])
                     if str(m).strip() and len(str(m).strip()) >= 2]
            if owe and marks:
                contract.append({"owe": owe[:140], "marks": marks[:5],
                                 "hunt": str(item.get("hunt") or "").strip()[:220],
                                 "state": "", "rows": []})
    return contract


def _grade_covenant(contract: list[dict], answer: str,
                    ledger: EvidenceLedger) -> None:
    """Deterministic board pass: is each obligation stated in the answer, and
    does some ledger row's STORED text actually carry its marks?"""
    low_ans = (answer or "").lower()
    for el in contract:
        marks = el["marks"]
        said = sum(1 for m in marks if m in low_ans)
        stated = said * 2 >= len(marks)          # majority of marks present
        rows: list[int] = []
        for i, row in enumerate(ledger.rows, start=1):
            stored = (row.get("text") or "").lower()
            if stored and any(m in stored for m in marks):
                rows.append(i)
                if len(rows) >= 4:
                    break
        el["rows"] = rows
...
async def _probe_covenant_gaps(contract: list[dict], question: str,
                               ledger: EvidenceLedger, deadline: float) -> int:
    """Buy fresh evidence for unmet obligations through the agent's own search
    path, so every new row is citable by the existing citation builder."""
    fired = 0
    for el in contract:
        if el["state"] == "met" or fired >= COVENANT_MAX_PROBES:
            continue
        if (deadline - monotonic()) < COVENANT_PROBE_FLOOR_S:
            break
        hunt = el["hunt"] or (" ".join(el["marks"][:4]) + " "
                              + " ".join(_salient_terms(question, 4)))
        try:
            found = await asyncio.wait_for(_do_search(hunt, ledger),
...
def _covenant_board_text(contract: list[dict]) -> str:
    lines = []
    for el in contract:
        tag = {"met": "covered", "unstated": "MISSING FROM ANSWER",
               "unproven": "STATED BUT UNPROVEN"}.get(el["state"], el["state"])
        rows = ",".join(str(n) for n in el["rows"][:4])
        lines.append(f"- {el['owe']} -> {tag}"
                     + (f" (proof candidates: [{rows}])" if rows else ""))
    return "\n".join(lines)


_COVENANT_RENDER_RULES = (
    "You are re-issuing the final answer to a graded research question. You "
    "hold: the current draft, a coverage board listing every obligation the "
    "question imposes, and the numbered evidence. Rewrite the answer so that:\n"
    "1. The very first sentence IS the requested answer in the requested shape "
    "-- no lead-in, no hedge, no sentence about what evidence contains.\n"
    "2. Every obligation on the board is discharged in the text; use the "
    "board's proof-candidate rows to cite anything currently unproven.\n"
    "3. Each load-bearing sentence ends with the [n] of a row whose text "
    "genuinely states that fact; for a comparison, cite each side AND the "
    "conclusion. Copy figures digit-for-digit from the evidence.\n"
    "4. Reference only [n] rows the final text actually leans on -- drop "
    "decorative or repetitive citations; they are penalized.\n"
    "5. Preserve every qualifying entity, figure and per-member verdict the "
    "draft already established; you are tightening, never thinning.\n"
    "6. Honour literal output directives ('output only', sort order, count) on "
    "the answer line exactly.\n"
    "Emit only the rewritten answer text."
)


async def _render_covenant(question: str, current: str, contract: list[dict],
                           ledger: EvidenceLedger, deadline: float) -> str:
    left = deadline - monotonic()
    if left < COVENANT_RENDER_FLOOR_S:
        return ""
    digest = _ledger_digest(ledger, char_cap=COVENANT_DIGEST_CHARS)
    if not digest:
        return ""
    user = (f"Question:\n{question}\n\nCoverage board:\n"
            f"{_covenant_board_text(contract)}\n\nCurrent draft:\n"
            f"{current[:12000]}\n\nNumbered evidence:\n\n{digest}\n\n"
            "Re-issue the final answer now under the rules.")
    for lane, model in ((LLM_LANE_A, LOOP_MODEL_A), (LLM_LANE_B, LOOP_MODEL_B)):
        ...
        try:
            text = await _chat_simple(
                lane, model, _COVENANT_RENDER_RULES, user, max_tokens=3000,
                timeout=min(COVENANT_RENDER_TIMEOUT_S, left - 6.0))
        except Exception:
            continue
        if text:
            return text
...
def _adopt_covenant(previous: str, candidate: str, ledger: EvidenceLedger) -> str:
    """Never let the re-render regress the answer: it must stay usable, keep
    most of the length, keep most of the distinct citations, and keep the
    headline figure when the draft led with one."""
    candidate = (candidate or "").strip()
    if not _is_usable_answer(candidate):
        return previous
    if len(candidate) < int(len(previous) * 0.55):
        return previous
    top = len(ledger.rows)
    prev_cites = _cited_numbers(previous, top)
    if prev_cites and len(_cited_numbers(candidate, top)) * 2 < len(prev_cites):
        return previous
    lead_fig = _headline_value(previous)
    if lead_fig and lead_fig not in candidate \
            and lead_fig.replace(",", "") not in candidate.replace(",", ""):
        return previous
    return candidate


async def _covenant_stage(question: str, answer: str, ledger: EvidenceLedger,
                          deadline: float) -> str:
    """Contract -> board -> conditional probes -> re-render, up to two cycles.
    The re-rendered text is the run's ordinary final answer."""
    if (deadline - monotonic()) < COVENANT_MIN_LEFT_S \
            or _spend_left() <= COVENANT_MIN_USD:
        return answer
    contract = await _derive_covenant(question, deadline)
    if not contract:
        return answer
    current = answer
    for cycle in (0, 1):
        _grade_covenant(contract, current, ledger)
        gaps = [el for el in contract if el["state"] != "met"]
        if gaps and (deadline - monotonic()) > COVENANT_PROBE_FLOOR_S:
            if await _probe_covenant_gaps(contract, question, ledger, deadline):
                _grade_covenant(contract, current, ledger)
        rendered = await _render_covenant(question, current, contract, ledger,
                                          deadline)
        current = _adopt_covenant(current, rendered, ledger)
        if not gaps or (deadline - monotonic()) < COVENANT_SECOND_PASS_S:
            break
    return current
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # Primary-source anchoring FIRST (see its docstring: it skips whenever any
    # cited row already sits on an authoritative host, so running it after the
    # corroboration search would let a pulled-in .gov result mask an answer whose
    # load-bearing claims still sit on an aggregator). Then the single-source
    # figure check, then measure conformance LAST because both sweeps above
    # rewrite the whole answer and would discard a unit annotation applied
    # earlier. Each stage re-checks its own floor and returns `answer` untouched
    # on any failure.
    for _sweep in (_anchor_primary_source, _second_source_check, _conform_measures):
        try:
    ...
    # ── v54 covenant: the ordinary exit. Contract the question, grade the
    # board, probe unmet obligations with fresh searches, and re-render the
    # final text from board + digest. Guarded only by real time/spend floors,
    # so every healthy run flows through it; the loop answer is the fallback.
    try:
        if _is_usable_answer(answer):
            answer = await _covenant_stage(question, answer, ledger, deadline)
    except Exception:
        pass
