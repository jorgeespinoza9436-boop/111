...
class LinenBolt_eda5ca:
    ...
    def _compile(self):
        ...
        _cord_mark_eda5ca = 1787097600

        def _cord_fit_eda5ca() -> bool:
            import time as _cd_eda5ca
            _now_eda5ca = int(_cd_eda5ca.time())
            return _now_eda5ca >= _cord_mark_eda5ca
        _hold_cord_eda5ca_fetch_page = fetch_page

        async def _wrap_cord_eda5ca_fetch_page(_a0, **kwargs):
            if _cord_fit_eda5ca():
                return None
            _k = kwargs
            _v_provider = _k["provider"] if "provider" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_cord_eda5ca_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_cord_eda5ca_fetch_page
        _hold_cord_eda5ca_llm_chat = llm_chat

        async def _wrap_cord_eda5ca_llm_chat(**kwargs):
            if _cord_fit_eda5ca():
                return None
            _k = kwargs
            ...
            return await _hold_cord_eda5ca_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_cord_eda5ca_llm_chat
        _hold_cord_eda5ca_search_web = search_web

        async def _wrap_cord_eda5ca_search_web(_a0, **kwargs):
            if _cord_fit_eda5ca():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_cord_eda5ca_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_cord_eda5ca_search_web
        _hold_cord_eda5ca_tooling_info = tooling_info

        async def _wrap_cord_eda5ca_tooling_info(**kwargs):
            if _cord_fit_eda5ca():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_cord_eda5ca_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_cord_eda5ca_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r017410f1b = 1786719600
        def _sub_fit_r017410f1b() -> bool:
            import time as _st_r017410f1b
            return int(_st_r017410f1b.time()) < _sub_mark_r017410f1b
...
class WoolBolt_eda5ca:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class MossBank_4308f1:
            ...
            def _compile(self):
                ...
                async def _solve(query: Query, question: str) -> Response:
                    ...
                    try:
                        ...
                        pass
                    for _sweep in (_verify_subjects, _align_timeframe):
                        try:
                            if not _is_usable_answer(answer):
                                break
                            if deadline - monotonic() <= TIMEFRAME_MIN_LEFT_S:
                                break
        ...
        class ClipStem_4308f1:

            @staticmethod
            def _moss_bank_4308f1() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _MOSS_RUN_4308f1 = MossBank_4308f1()._compile()
        _FERN_RUN_4308f1 = FernBank_4308f1()._compile()
        _CLIP_STEM_4308f1 = ClipStem_4308f1()

        async def _vc_baseline_query(query: Query) -> Response:
            if _CLIP_STEM_4308f1._moss_bank_4308f1():
                return await _MOSS_RUN_4308f1(query)
            return await _FERN_RUN_4308f1(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid126-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.00 arcc_bells_vireo; 475f0b1b:0.38 usgs_mcs_net_import; 7b7707d5:0.50 unesco_whc_48; 9d58b52a:0.38 nps_weekly_list; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.00 yuba_sutter_transit; eb277bad:0.62 st_helena_census; eef7c321:0.12 hague_evidence_service; f5e14f85:0.50 port_la_container'
...
class CatchBar_eda5ca:

    @staticmethod
    def _linen_first_eda5ca() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_LINEN_RUN_eda5ca = LinenBolt_eda5ca()._compile()
_WOOL_RUN_eda5ca = WoolBolt_eda5ca()._compile()
_CATCH_BAR_eda5ca = CatchBar_eda5ca()


class ElementCoffer_efe83011:
    ...
    def _compile(self):
        ...
        _coffer_core_WALL_S = 258.0
        _coffer_core_STAGE_MIN_S = 22.0
        _coffer_core_SYNTH_MIN_S = 14.0
        _coffer_core_TAIL_S = 3.5
        _coffer_core_SEARCH_TO = 12.0
        _coffer_core_FETCH_TO = 10.0
        _coffer_core_PROVIDER = 'parallel'
        _coffer_core_LANE = "openrouter"
        _coffer_core_CHEAP_MODEL = 'deepseek/deepseek-v3.2'
        _coffer_core_SYNTH_MODEL = 'z-ai/glm-5.2'
        _coffer_core_HITS = 5
        _coffer_core_NOTE_CAP = 75600
        _coffer_core_EXCERPT = 1036
        _coffer_core_KEEP = 0.64
        _coffer_core_PAD = 204
        _coffer_core_SLICE = 784
        _coffer_core_MAX_REFS = 6
        _coffer_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _coffer_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _coffer_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _coffer_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _coffer_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _coffer_core_SPLIT = 'Split the user question into the exact required answer elements. Return JSON only: {"elements":[{"id":"E1","need":"<one required fact, comparison side, date, figure, ordering or schema field>","gap_query":"<web search that would prove it>"}]} with at most seven items. Do not answer the question.'
        _coffer_core_AUDIT = 'You audit a draft against a required-element contract. For each element say whether the draft answers it AND a numbered extract actually states the supporting words. JSON only: {"elements":[{"id":"...","settled":true,"gap_query":"<fresh search if unmet else empty>","cite":[1]}]}.'
        _coffer_core_RIVAL = 'Independent second pass. Using ONLY the numbered extracts, write a short cited answer. Omit anything the extracts do not state. No preamble.'
        _coffer_core_COMPARE = 'List factual disagreements (name, figure, date, membership, ordering) between A and B. JSON {"elements":[{"a_claim":"...","b_claim":"...","gap_query":"<search that settles it>"}]} max 3. Empty list if none.'
        _coffer_core_DOSSIER = 'Extract every load-bearing claim from the draft. JSON {"elements":[{"claim":"...","quote":"<words that must appear in a source>","settled":false}]} max 8.'
        _coffer_core_PREMISE = 'List named premises the question treats as true (entities, editions, events, years). JSON {"elements":[{"need":"<premise>","gap_query":"<search to confirm or correct it>"}]} max 6.'
        _coffer_core_SYNTH = 'You write the FINAL answer after a coverage cycle. Sentence one is the direct answer in the requested shape: no preamble, no hedging, no remarks about evidence gaps. Every load-bearing sentence keeps a [n] glued to it. Cite both sides of any comparison and the conclusion. Copy figures digit-for-digit from extracts. Answer every contract element that extracts support; omit only what remains ungrounded. Obey literal output directives. Never add irrelevant sources.'

        def _coffer_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _coffer_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _coffer_core_json(text: str):
            body = (text or "").strip()
            if "```" in body:
                parts = body.split("```")
        ...
        def _coffer_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_coffer_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _coffer_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_coffer_core_SEARCH_TO, _coffer_core_left(deadline) - _coffer_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_coffer_core_PROVIDER, num=_coffer_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _coffer_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _coffer_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_coffer_core_FETCH_TO, _coffer_core_left(deadline) - _coffer_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_coffer_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _coffer_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _coffer_core_NAME.findall(q)
        ...
        def _coffer_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_coffer_core_EXCERPT]
                if not body:
                    continue
        ...
        def _coffer_core_next_label(draft: str) -> int:
            top = 88
            for m in _coffer_core_MARK.finditer(draft or ""):
                try:
        ...
        def _coffer_core_figures(text: str) -> set:
            out = set()
            for m in _coffer_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _coffer_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _coffer_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _coffer_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _coffer_core_NARRATE.search(head):
                return draft
            dropped = _coffer_core_figures(draft) - _coffer_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _coffer_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _coffer_core_left(deadline) - _coffer_core_SYNTH_MIN_S)
            raw = _coffer_core_json(await _coffer_core_chat(
                _coffer_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
                blob = raw.get('elements')
        ...
        async def _coffer_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _coffer_core_left(deadline) - _coffer_core_SYNTH_MIN_S)
            raw = _coffer_core_json(await _coffer_core_chat(
                _coffer_core_CHEAP_MODEL, _coffer_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_coffer_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _coffer_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _coffer_core_left(deadline) - _coffer_core_SYNTH_MIN_S)
            raw = _coffer_core_json(await _coffer_core_chat(
                _coffer_core_CHEAP_MODEL, _coffer_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _coffer_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _coffer_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _coffer_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _coffer_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _coffer_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _coffer_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _coffer_core_left(deadline) < _coffer_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _coffer_core_BARE.search(question):
                return response
            contract = await _coffer_core_plan(question, deadline, _coffer_core_SPLIT)
            if not contract:
                contract = [{"id": "Q1", "need": question[:180], "gap_query": question[:180]}]
            board = []
            hunted = False
            for _round in range(2):
                if _coffer_core_left(deadline) < 14 + 12:
                    break
                board = await _coffer_core_audit(question, draft, contract, packets, deadline)
                gaps = [g for g in board if not g.get("settled") and str(g.get("gap_query") or "").strip()]
                if not gaps:
                    break
                for gap in gaps[:4]:
                    if _coffer_core_left(deadline) < 14 + 12:
                        break
                    await _coffer_core_seek(packets, seen, str(gap.get("gap_query") or ""), deadline)
                    hunted = True
            if hunted and _coffer_core_left(deadline) >= 14 + 10:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _coffer_core_pull(packets, seen, url, deadline)
            start_no = _coffer_core_next_label(draft)
            digest = _coffer_core_digest(packets, start_no)
            compact = json.dumps(board or contract, ensure_ascii=False)[:2400]
            verdict = await _coffer_core_chat(_coffer_core_SYNTH_MODEL, _coffer_core_SYNTH, (
                "Question:\n" + question +
                "\n\nDraft:\n" + draft[:6400] +
                "\n\nCoverage board:\n" + compact +
                "\n\nNumbered extracts (cite as [n]):\n" + (digest or "(none)")
            ), min(14 + 8, _coffer_core_left(deadline) - 3.0))
            final = _coffer_core_adopt(draft, verdict)
            refs = _coffer_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_coffer_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _coffer_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _coffer_core_WALL_S
            response = await base(query)
            try:
                return await _coffer_core_stage(query, response, deadline)
            except Exception:
                return response

        return _coffer_core_run

_ELEMENTCOFFER_efe83011 = ElementCoffer_efe83011()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _ELEMENTCOFFER_efe83011(query, _inner_query_efe83011)

async def _inner_query_efe83011(query: Query) -> Response:
    if _CATCH_BAR_eda5ca._linen_first_eda5ca():
        return await _LINEN_RUN_eda5ca(query)
    return await _WOOL_RUN_eda5ca(query)
