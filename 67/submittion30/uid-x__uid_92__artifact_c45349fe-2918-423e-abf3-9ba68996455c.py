...
class VerdictArbor_2c2bf96b:
    ...
    def _compile(self):
        ...
        _verdict_core_WALL_S = 255.0
        _verdict_core_STAGE_MIN_S = 23.0
        _verdict_core_SYNTH_MIN_S = 11.0
        _verdict_core_TAIL_S = 3.5
        _verdict_core_SEARCH_TO = 9.0
        _verdict_core_FETCH_TO = 11.0
        _verdict_core_PROVIDER = 'firecrawl'
        _verdict_core_LANE = "openrouter"
        _verdict_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _verdict_core_SYNTH_MODEL = 'zai/glm-5.2-fast'
        _verdict_core_HITS = 6
        _verdict_core_NOTE_CAP = 87500
        _verdict_core_EXCERPT = 1325
        _verdict_core_KEEP = 0.55
        _verdict_core_PAD = 255
        _verdict_core_SLICE = 920
        _verdict_core_MAX_REFS = 5
        _verdict_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _verdict_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _verdict_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _verdict_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _verdict_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _verdict_core_SPLIT = 'List every load-bearing sub-question hidden in this research prompt. JSON only: {"verdicts":[{"id":"S1","need":"<atomic required item>","swap":"<targeted search query>"}]} max seven. No prose.'
        _verdict_core_AUDIT = 'Score coverage. An element is met only when a numbered extract contains the concrete words. JSON: {"verdicts":[{"id":"...","keep":false,"swap":"<search>","cite":[]}]}.'
        _verdict_core_RIVAL = 'Write a competing answer strictly from the numbered packets. Cite [n]. Skip unverified points. First sentence is the answer.'
        _verdict_core_COMPARE = 'Find claim-level clashes, ignore style. JSON {"verdicts":[{"keep":"<A>","swap":"<B>","swap":"<probe>"}]} up to three.'
        _verdict_core_DOSSIER = 'Build a claim dossier. JSON {"verdicts":[{"claim":"...","quote":"...","keep":false}]}.'
        _verdict_core_PREMISE = 'What does the question assume exists? JSON {"verdicts":[{"need":"...","swap":"..."}]}.'
        _verdict_core_SYNTH = 'Final arbiter. Lead with the asked entities/values. Keep draft [n] markers that still match extracts. For each clash, keep the side the fresh extracts support. Verbatim numbers. Cover every asked element. No filler, no "could not verify" narration.'

        def _verdict_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _verdict_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _verdict_core_json(text: str):
            body = (text or "").strip()
            if "```" in body:
                parts = body.split("```")
        ...
        def _verdict_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_verdict_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _verdict_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_verdict_core_SEARCH_TO, _verdict_core_left(deadline) - _verdict_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_verdict_core_PROVIDER, num=_verdict_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _verdict_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _verdict_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_verdict_core_FETCH_TO, _verdict_core_left(deadline) - _verdict_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_verdict_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _verdict_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _verdict_core_NAME.findall(q)
        ...
        def _verdict_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_verdict_core_EXCERPT]
                if not body:
                    continue
        ...
        def _verdict_core_next_label(draft: str) -> int:
            top = 105
            for m in _verdict_core_MARK.finditer(draft or ""):
                try:
        ...
        def _verdict_core_figures(text: str) -> set:
            out = set()
            for m in _verdict_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _verdict_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _verdict_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _verdict_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _verdict_core_NARRATE.search(head):
                return draft
            dropped = _verdict_core_figures(draft) - _verdict_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _verdict_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _verdict_core_left(deadline) - _verdict_core_SYNTH_MIN_S)
            raw = _verdict_core_json(await _verdict_core_chat(
                _verdict_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
                blob = raw.get('verdicts')
        ...
        async def _verdict_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _verdict_core_left(deadline) - _verdict_core_SYNTH_MIN_S)
            raw = _verdict_core_json(await _verdict_core_chat(
                _verdict_core_CHEAP_MODEL, _verdict_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_verdict_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _verdict_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _verdict_core_left(deadline) - _verdict_core_SYNTH_MIN_S)
            raw = _verdict_core_json(await _verdict_core_chat(
                _verdict_core_CHEAP_MODEL, _verdict_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _verdict_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _verdict_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _verdict_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _verdict_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _verdict_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _verdict_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _verdict_core_left(deadline) < _verdict_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _verdict_core_BARE.search(question):
                return response
            for probe in _verdict_core_scout(question)[:2]:
                if _verdict_core_left(deadline) < 11 + 9:
                    break
                await _verdict_core_seek(packets, seen, probe, deadline)
            rival = await _verdict_core_chat(_verdict_core_CHEAP_MODEL, _verdict_core_RIVAL, (
                "Question:\n" + question +
                "\n\nExtracts:\n" + (_verdict_core_digest(packets, 1) or "(none)")
            ), min(13, _verdict_core_left(deadline) - 11), 0.2)
            clashes = await _verdict_core_clashes(question, draft, rival, deadline) if rival else []
            for item in clashes[:3]:
                if _verdict_core_left(deadline) < 11 + 9:
                    break
                await _verdict_core_seek(packets, seen, str(item.get("swap") or item.get("keep") or ""), deadline)
            if clashes and _verdict_core_left(deadline) >= 11 + 11:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _verdict_core_pull(packets, seen, url, deadline)
            start_no = _verdict_core_next_label(draft)
            verdict = await _verdict_core_chat(_verdict_core_SYNTH_MODEL, _verdict_core_SYNTH, (
                "Question:\n" + question +
                "\n\nPrimary draft:\n" + draft[:6400] +
                "\n\nRival:\n" + (rival or "(unavailable)")[:2400] +
                "\n\nClashes:\n" + json.dumps(clashes, ensure_ascii=False)[:2200] +
                "\n\nFresh extracts:\n" + (_verdict_core_digest(packets, start_no) or "(none)")
            ), min(13 + 10, _verdict_core_left(deadline) - 3.0))
            final = _verdict_core_adopt(draft, verdict)
            refs = _verdict_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_verdict_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _verdict_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _verdict_core_WALL_S
            response = await base(query)
            try:
                return await _verdict_core_stage(query, response, deadline)
            except Exception:
                return response

        return _verdict_core_run

_VERDICTARBOR_2c2bf96b = VerdictArbor_2c2bf96b()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _VERDICTARBOR_2c2bf96b(query, _inner_query_2c2bf96b)

async def _inner_query_2c2bf96b(query: Query) -> Response:
