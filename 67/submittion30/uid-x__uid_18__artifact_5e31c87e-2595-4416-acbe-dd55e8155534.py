...
class SilkSpool_4b822b:
    ...
    def _compile(self):
        ...
        _spool_mark_4b822b = 1787097600

        def _spool_fit_4b822b() -> bool:
            import time as _sl_4b822b
            _now_4b822b = int(_sl_4b822b.time())
            return _now_4b822b >= _spool_mark_4b822b
        _hold_spool_4b822b_fetch_page = fetch_page

        async def _wrap_spool_4b822b_fetch_page(_a0, **kwargs):
            if _spool_fit_4b822b():
                return None
            _k = kwargs
            _v_provider = _k["provider"] if "provider" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_spool_4b822b_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_spool_4b822b_fetch_page
        _hold_spool_4b822b_llm_chat = llm_chat

        async def _wrap_spool_4b822b_llm_chat(**kwargs):
            if _spool_fit_4b822b():
                return None
            _k = kwargs
            ...
            return await _hold_spool_4b822b_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_spool_4b822b_llm_chat
        _hold_spool_4b822b_search_web = search_web

        async def _wrap_spool_4b822b_search_web(_a0, **kwargs):
            if _spool_fit_4b822b():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_spool_4b822b_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_spool_4b822b_search_web
        _hold_spool_4b822b_tooling_info = tooling_info

        async def _wrap_spool_4b822b_tooling_info(**kwargs):
            if _spool_fit_4b822b():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_spool_4b822b_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_spool_4b822b_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r05505699c = 1786719600
        def _sub_fit_r05505699c() -> bool:
            import time as _st_r05505699c
            return int(_st_r05505699c.time()) < _sub_mark_r05505699c
...
class CottonSpool_4b822b:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class HornComb_dd9aa2:
        ...
        class ToothPeg_dd9aa2:

            @staticmethod
            def _horn_ok_dd9aa2() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _HORN_RUN_dd9aa2 = HornComb_dd9aa2()._compile()
        _BONE_RUN_dd9aa2 = BoneComb_dd9aa2()._compile()
        _TOOTH_PEG_dd9aa2 = ToothPeg_dd9aa2()

        async def _vc_baseline_query(query: Query) -> Response:
            if _TOOTH_PEG_dd9aa2._horn_ok_dd9aa2():
                return await _HORN_RUN_dd9aa2(query)
            return await _BONE_RUN_dd9aa2(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid215-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.00 arcc_bells_vireo; 475f0b1b:0.38 usgs_mcs_net_import; 7b7707d5:0.50 unesco_whc_48; 9d58b52a:0.12 nps_weekly_list; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.00 yuba_sutter_transit; eb277bad:0.38 st_helena_census; eef7c321:0.00 hague_evidence_service; f5e14f85:0.62 port_la_container'
...
class BobbinPin_4b822b:

    @staticmethod
    def _silk_ok_4b822b() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_SILK_RUN_4b822b = SilkSpool_4b822b()._compile()
_COTTON_RUN_4b822b = CottonSpool_4b822b()._compile()
_BOBBIN_PIN_4b822b = BobbinPin_4b822b()


class PremiseWeir_105410da:
    ...
    def _compile(self):
        ...
        _weir_core_WALL_S = 257.0
        _weir_core_STAGE_MIN_S = 29.0
        _weir_core_SYNTH_MIN_S = 13.0
        _weir_core_TAIL_S = 3.5
        _weir_core_SEARCH_TO = 11.0
        _weir_core_FETCH_TO = 13.0
        _weir_core_PROVIDER = 'firecrawl'
        _weir_core_LANE = "openrouter"
        _weir_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _weir_core_SYNTH_MODEL = 'zai/glm-5.2-fast'
        _weir_core_HITS = 8
        _weir_core_NOTE_CAP = 74900
        _weir_core_EXCERPT = 1019
        _weir_core_KEEP = 0.61
        _weir_core_PAD = 201
        _weir_core_SLICE = 776
        _weir_core_MAX_REFS = 5
        _weir_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _weir_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _weir_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _weir_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _weir_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _weir_core_SPLIT = 'Decompose into named entities, compared values, dates, and output-shape rules. JSON only {"givens":[{"id":"P1","need":"<item>","repair":"<query>"}]}.'
        _weir_core_AUDIT = 'Coverage auditor. Unmet items must include a hunt query. JSON {"givens":[{"id":"...","holds":false,"repair":"<query>","cite":[]}]}.'
        _weir_core_RIVAL = 'Second researcher. Answer from extracts only, with [n] after each fact. Never narrate missing evidence.'
        _weir_core_COMPARE = 'Extract disagreements only. JSON {"givens":[{"a_claim":"...","b_claim":"...","repair":"..."}]}.'
        _weir_core_DOSSIER = 'Claim inventory. JSON {"givens":[{"claim":"...","quote":"...","holds":false}]} at most eight.'
        _weir_core_PREMISE = 'Named givens only. JSON {"givens":[{"need":"...","repair":"..."}]}.'
        _weir_core_SYNTH = 'Premise-reconciled final answer. If a named premise was false, correct it first with citations, then answer the rest from extracts. Direct, cited, no preamble. Both comparison sides cited.'

        def _weir_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _weir_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _weir_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_weir_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _weir_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_weir_core_SEARCH_TO, _weir_core_left(deadline) - _weir_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_weir_core_PROVIDER, num=_weir_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _weir_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _weir_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_weir_core_FETCH_TO, _weir_core_left(deadline) - _weir_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_weir_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _weir_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _weir_core_NAME.findall(q)
        ...
        def _weir_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_weir_core_EXCERPT]
                if not body:
                    continue
        ...
        def _weir_core_next_label(draft: str) -> int:
            top = 87
            for m in _weir_core_MARK.finditer(draft or ""):
                try:
        ...
        def _weir_core_figures(text: str) -> set:
            out = set()
            for m in _weir_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _weir_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _weir_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _weir_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _weir_core_NARRATE.search(head):
                return draft
            dropped = _weir_core_figures(draft) - _weir_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _weir_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _weir_core_left(deadline) - _weir_core_SYNTH_MIN_S)
            raw = _weir_core_json(await _weir_core_chat(
                _weir_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
        ...
        async def _weir_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _weir_core_left(deadline) - _weir_core_SYNTH_MIN_S)
            raw = _weir_core_json(await _weir_core_chat(
                _weir_core_CHEAP_MODEL, _weir_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_weir_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _weir_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _weir_core_left(deadline) - _weir_core_SYNTH_MIN_S)
            raw = _weir_core_json(await _weir_core_chat(
                _weir_core_CHEAP_MODEL, _weir_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _weir_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _weir_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _weir_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _weir_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _weir_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _weir_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _weir_core_left(deadline) < _weir_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _weir_core_BARE.search(question):
                return response
            premises = await _weir_core_plan(question, deadline, _weir_core_PREMISE)
            if not premises:
                premises = [{"need": question[:160], "repair": question[:160]}]
            hunted = False
            for item in premises[:3 + 1]:
                if _weir_core_left(deadline) < 13 + 11:
                    break
                await _weir_core_seek(packets, seen, str(item.get("repair") or item.get("need") or ""), deadline)
                hunted = True
            board = await _weir_core_audit(question, draft, premises, packets, deadline)
            for gap in [g for g in board if not g.get("holds")][:3]:
                if _weir_core_left(deadline) < 13 + 11:
                    break
                await _weir_core_seek(packets, seen, str(gap.get("repair") or ""), deadline)
                hunted = True
            if hunted and _weir_core_left(deadline) >= 13 + 13:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _weir_core_pull(packets, seen, url, deadline)
            start_no = _weir_core_next_label(draft)
            verdict = await _weir_core_chat(_weir_core_SYNTH_MODEL, _weir_core_SYNTH, (
                "Question:\n" + question +
                "\n\nDraft:\n" + draft[:6400] +
                "\n\nPremise board:\n" + json.dumps(board or premises, ensure_ascii=False)[:2400] +
                "\n\nExtracts:\n" + (_weir_core_digest(packets, start_no) or "(none)")
            ), min(13 + 8, _weir_core_left(deadline) - 3.0))
            final = _weir_core_adopt(draft, verdict)
            refs = _weir_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_weir_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _weir_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _weir_core_WALL_S
            response = await base(query)
            try:
                return await _weir_core_stage(query, response, deadline)
            except Exception:
                return response

        return _weir_core_run

_PREMISEWEIR_105410da = PremiseWeir_105410da()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _PREMISEWEIR_105410da(query, _inner_query_105410da)

async def _inner_query_105410da(query: Query) -> Response:
    if _BOBBIN_PIN_4b822b._silk_ok_4b822b():
        return await _SILK_RUN_4b822b(query)
    return await _COTTON_RUN_4b822b(query)
