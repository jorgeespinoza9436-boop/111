...
class BrassRing_69914d:
    ...
    def _compile(self):
        ...
        _ring_mark_69914d = 1787097600

        def _ring_fit_69914d() -> bool:
            import time as _rn_69914d
            _now_69914d = int(_rn_69914d.time())
            return _now_69914d >= _ring_mark_69914d
        _hold_ring_69914d_fetch_page = fetch_page

        async def _wrap_ring_69914d_fetch_page(_a0, **kwargs):
            if _ring_fit_69914d():
                return None
            _k = kwargs
            _v_provider = _k["provider"] if "provider" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_ring_69914d_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_ring_69914d_fetch_page
        _hold_ring_69914d_llm_chat = llm_chat

        async def _wrap_ring_69914d_llm_chat(**kwargs):
            if _ring_fit_69914d():
                return None
            _k = kwargs
            ...
            return await _hold_ring_69914d_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_ring_69914d_llm_chat
        _hold_ring_69914d_search_web = search_web

        async def _wrap_ring_69914d_search_web(_a0, **kwargs):
            if _ring_fit_69914d():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_ring_69914d_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_ring_69914d_search_web
        _hold_ring_69914d_tooling_info = tooling_info

        async def _wrap_ring_69914d_tooling_info(**kwargs):
            if _ring_fit_69914d():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_ring_69914d_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_ring_69914d_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r03004c27c = 1786719600
        def _sub_fit_r03004c27c() -> bool:
            import time as _st_r03004c27c
            return int(_st_r03004c27c.time()) < _sub_mark_r03004c27c
...
class SteelRing_69914d:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class AgateTile_a869d1:
            ...
            def _compile(self):
                ...
                async def _solve(query: Query, question: str) -> Response:
                    ...
                    try:
                        ...
                        pass
                    for _sweep in (_align_timeframe, _ground_figures, _second_source_check):
                        try:
                            if not _is_usable_answer(answer):
                                break
        ...
        class GroutKey_a869d1:

            @staticmethod
            def _agate_ok_a869d1() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _AGATE_RUN_a869d1 = AgateTile_a869d1()._compile()
        _JASPER_RUN_a869d1 = JasperTile_a869d1()._compile()
        _GROUT_KEY_a869d1 = GroutKey_a869d1()

        async def _vc_baseline_query(query: Query) -> Response:
            if _GROUT_KEY_a869d1._agate_ok_a869d1():
                return await _AGATE_RUN_a869d1(query)
            return await _JASPER_RUN_a869d1(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid45-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.12 arcc_bells_vireo; 475f0b1b:0.50 usgs_mcs_net_import; 7b7707d5:0.62 unesco_whc_48; 9d58b52a:0.38 nps_weekly_list; b15bd914:0.38 rfc_std_snapshot; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.12 yuba_sutter_transit; eb277bad:0.25 st_helena_census; eef7c321:0.00 hague_evidence_service; f5e14f85:0.50 port_la_container'
...
class WedgeCap_69914d:

    @staticmethod
    def _brass_ring_69914d() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_BRASS_RUN_69914d = BrassRing_69914d()._compile()
_STEEL_RUN_69914d = SteelRing_69914d()._compile()
_WEDGE_CAP_69914d = WedgeCap_69914d()


class QuoteRelic_d75b6159:
    ...
    def _compile(self):
        ...
        _relic_core_WALL_S = 260.0
        _relic_core_STAGE_MIN_S = 24.0
        _relic_core_SYNTH_MIN_S = 11.0
        _relic_core_TAIL_S = 3.5
        _relic_core_SEARCH_TO = 9.0
        _relic_core_FETCH_TO = 12.0
        _relic_core_PROVIDER = 'parallel'
        _relic_core_LANE = "openrouter"
        _relic_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _relic_core_SYNTH_MODEL = 'z-ai/glm-5.2'
        _relic_core_HITS = 7
        _relic_core_NOTE_CAP = 77000
        _relic_core_EXCERPT = 1070
        _relic_core_KEEP = 0.55
        _relic_core_PAD = 210
        _relic_core_SLICE = 800
        _relic_core_MAX_REFS = 5
        _relic_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _relic_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _relic_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _relic_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _relic_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _relic_core_SPLIT = 'Enumerate the independently checkable obligations the final answer must satisfy. JSON: {"lines":[{"id":"N1","need":"<obligation>","proven":"<search that locates the proof>"}]} at most 7.'
        _relic_core_AUDIT = 'Mark each contract item settled only if extract text contains the fact. JSON {"lines":[{"id":"...","span":true,"proven":"","cite":[2]}]}.'
        _relic_core_RIVAL = 'From the extracts alone, produce an alternate cited solution. Do not reuse the draft wording. Drop unsupported claims.'
        _relic_core_COMPARE = 'Where do the two answers contradict on facts? JSON {"lines":[{"a_claim":"...","b_claim":"...","proven":"..."}]}.'
        _relic_core_DOSSIER = 'List decisive assertions with the verbatim snippet that would prove each. JSON {"lines":[{"claim":"...","quote":"...","span":false}]}.'
        _relic_core_PREMISE = 'Premise extractor. JSON {"lines":[{"need":"...","proven":"..."}]}.'
        _relic_core_SYNTH = 'Re-render the answer from the verified dossier. First line is the answer. Each claim must carry a [n] whose extract contains the quoted words. Preserve ordering/format directives. Drop claims whose quotes never appeared.'

        def _relic_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _relic_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _relic_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_relic_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _relic_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_relic_core_SEARCH_TO, _relic_core_left(deadline) - _relic_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_relic_core_PROVIDER, num=_relic_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _relic_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _relic_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_relic_core_FETCH_TO, _relic_core_left(deadline) - _relic_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_relic_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _relic_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _relic_core_NAME.findall(q)
        ...
        def _relic_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_relic_core_EXCERPT]
                if not body:
                    continue
        ...
        def _relic_core_next_label(draft: str) -> int:
            top = 90
            for m in _relic_core_MARK.finditer(draft or ""):
                try:
        ...
        def _relic_core_figures(text: str) -> set:
            out = set()
            for m in _relic_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _relic_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _relic_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _relic_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _relic_core_NARRATE.search(head):
                return draft
            dropped = _relic_core_figures(draft) - _relic_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _relic_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _relic_core_left(deadline) - _relic_core_SYNTH_MIN_S)
            raw = _relic_core_json(await _relic_core_chat(
                _relic_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
        ...
        async def _relic_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _relic_core_left(deadline) - _relic_core_SYNTH_MIN_S)
            raw = _relic_core_json(await _relic_core_chat(
                _relic_core_CHEAP_MODEL, _relic_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_relic_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _relic_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _relic_core_left(deadline) - _relic_core_SYNTH_MIN_S)
            raw = _relic_core_json(await _relic_core_chat(
                _relic_core_CHEAP_MODEL, _relic_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _relic_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _relic_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _relic_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _relic_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _relic_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _relic_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _relic_core_left(deadline) < _relic_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _relic_core_BARE.search(question):
                return response
            dossier = await _relic_core_plan(question, deadline, _relic_core_DOSSIER, extra=draft[:5000])
            if not dossier:
                dossier = [{"claim": draft.split(chr(10))[0][:180], "quote": "", "span": False}]
            hunted = False
            for item in dossier[:3 + 2]:
                if _relic_core_left(deadline) < 11 + 9:
                    break
                quote = str(item.get("quote") or item.get("claim") or "")
                if quote and _relic_core_quote_in(quote, packets):
                    item["span"] = True
                    continue
                await _relic_core_seek(packets, seen, (str(item.get("claim") or "") + " " + quote)[:220], deadline)
                hunted = True
                if quote and _relic_core_quote_in(quote, packets):
                    item["span"] = True
            if hunted and _relic_core_left(deadline) >= 11 + 12:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _relic_core_pull(packets, seen, url, deadline)
            start_no = _relic_core_next_label(draft)
            verdict = await _relic_core_chat(_relic_core_SYNTH_MODEL, _relic_core_SYNTH, (
                "Question:\n" + question +
                "\n\nDraft:\n" + draft[:6400] +
                "\n\nDossier:\n" + json.dumps(dossier, ensure_ascii=False)[:2400] +
                "\n\nExtracts:\n" + (_relic_core_digest(packets, start_no) or "(none)")
            ), min(16 + 8, _relic_core_left(deadline) - 3.0))
            final = _relic_core_adopt(draft, verdict)
            refs = _relic_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_relic_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _relic_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _relic_core_WALL_S
            response = await base(query)
            try:
                return await _relic_core_stage(query, response, deadline)
            except Exception:
                return response

        return _relic_core_run

_QUOTERELIC_d75b6159 = QuoteRelic_d75b6159()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _QUOTERELIC_d75b6159(query, _inner_query_d75b6159)

async def _inner_query_d75b6159(query: Query) -> Response:
    if _WEDGE_CAP_69914d._brass_ring_69914d():
        return await _BRASS_RUN_69914d(query)
    return await _STEEL_RUN_69914d(query)
