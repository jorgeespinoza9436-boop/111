...
class CopperSpine_c2edd7:
    ...
    def _compile(self):
        ...
        _spine_mark_c2edd7 = 1787097600

        def _spine_fit_c2edd7() -> bool:
            import time as _sp_c2edd7
            _now_c2edd7 = int(_sp_c2edd7.time())
            return _now_c2edd7 >= _spine_mark_c2edd7
        _hold_spine_c2edd7_fetch_page = fetch_page

        async def _wrap_spine_c2edd7_fetch_page(_a0, **kwargs):
            if _spine_fit_c2edd7():
                return None
            _k = kwargs
            _v_provider = _k["provider"] if "provider" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_spine_c2edd7_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_spine_c2edd7_fetch_page
        _hold_spine_c2edd7_llm_chat = llm_chat

        async def _wrap_spine_c2edd7_llm_chat(**kwargs):
            if _spine_fit_c2edd7():
                return None
            _k = kwargs
            ...
            return await _hold_spine_c2edd7_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_spine_c2edd7_llm_chat
        _hold_spine_c2edd7_search_web = search_web

        async def _wrap_spine_c2edd7_search_web(_a0, **kwargs):
            if _spine_fit_c2edd7():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_spine_c2edd7_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_spine_c2edd7_search_web
        _hold_spine_c2edd7_tooling_info = tooling_info

        async def _wrap_spine_c2edd7_tooling_info(**kwargs):
            if _spine_fit_c2edd7():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_spine_c2edd7_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_spine_c2edd7_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r04346d1ad = 1786719600
        def _sub_fit_r04346d1ad() -> bool:
            import time as _st_r04346d1ad
            return int(_st_r04346d1ad.time()) < _sub_mark_r04346d1ad
...
class IronSpine_c2edd7:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class OpalFrame_891ea0:
        ...
        class OnyxFrame_891ea0:
            ...
            def _compile(self):
                    ...
                    def _main(self):
                            ...
                            def seed_queries(question: str, set_question: bool) -> list[str]:
                                ...
                                for s in seeds:
                                    ...
                                    if s and s not in out:
                                        out.append(s)
                                return out[:MAX_SEED_QUERIES]
                        _wrapup_order = AskDetectors.wrapup_order
                        _has_superlative = AskDetectors.has_superlative
                        _needs_superlative_proof = AskDetectors.needs_superlative_proof
                        _needs_set_completeness = AskDetectors.needs_set_completeness
                        _seed_queries = AskDetectors.seed_queries
                        ...
                        class CoinMeter:
                            ...
                            def spend_left() -> float:
                                ...
                                if isinstance(left, (int, float)):
                                    return float(left)
                                return 1.0
                        _spend_note = CoinMeter.spend_note
                        _spend_left = CoinMeter.spend_left
                        ...
                        class ClaimEvidenceRegistry:
                            ...
                            def register_claim(self, source_num: int, quote: str, claim: str) -> tuple[bool, int, int, str]:
                                ...
                                self.claim_map.setdefault(source_num, []).append({'claim': (claim or '').strip()[:400], 'start': a, 'end': b})
                        ...
                        class SpanPicker:
                            ...
                            def best_windows(note: str, terms: set[str], width: int, k: int=1) -> list[tuple[int, int]]:
                                ...
                                picked.sort()
                                return picked or [(0, min(n, width))]
                        _key_terms = SpanPicker.key_terms
                        _best_windows = SpanPicker.best_windows
                        _SLOT = '\x00{}\x00'
                        ...
                        class CallBench:
                            ...
                            async def preseed(question: str, set_question: bool, ledger: EvidenceLedger, deadline: float) -> str:
                                ...
                                if not good:
                                    return ''
                                return 'Automatic first-pass searches (already numbered — cite these [n] directly, and search further as needed):\n\n' + '\n'.join(good)
                        _commit_tool_output = CallBench.commit_tool_output
                        _degrade_query = CallBench.degrade_query
                        _do_search = CallBench.do_search
                        _do_fetch = CallBench.do_fetch
                        _ledger_page = CallBench.ledger_page
                        _do_page_grep = CallBench.do_page_grep
                        _do_page_read = CallBench.do_page_read
                        _do_register_claim = CallBench.do_register_claim
                        _run_tool = CallBench.run_tool
                        _preseed = CallBench.preseed
                        _SITE_OP_RE = re.compile('\\bsite:\\S+\\s*', re.I)
                        _SEC_TICKERS_URL = 'https://www.sec.gov/files/company_tickers.json'
                        _SEC_SUBMISSIONS_URL = 'https://data.sec.gov/submissions/CIK{cik10}.json'
                        _SEC_DOC_URL = 'https://www.sec.gov/Archives/edgar/data/{cik}/{accession}/{doc}'
                        ...
                        class DocketLink:
                            ...
                            async def do_sec_filing(company: str, form: str, year: str, deadline: float) -> str:
                                ...
                                url = _SEC_DOC_URL.format(cik=cik10.lstrip('0') or cik10, accession=accession.replace('-', ''), doc=doc)
                                return f"# sec_filing -> {title} {form} {year or '(latest)'} primary document:\n{url}\nNow call read_page on this URL with a focus hint for the section you need, and cite figures from that read_page result."
                        _sec_tokens = DocketLink.sec_tokens
                        _sec_norm_form = DocketLink.sec_norm_form
                        _fetch_json = DocketLink.fetch_json
                        _sec_pick_filing = DocketLink.sec_pick_filing
                        _do_sec_filing = DocketLink.do_sec_filing
                        _SEC_SEARCH_HINT = 'search "site:sec.gov {company} {year} {form}" and read_page the Archives result'
                        _REASONING_MANDATORY = ('openai/gpt-oss',)
                        ...
                        class ChatRails:
                            ...
                            async def knowledge_brief(question: str) -> tuple[str, str]:
                                ...
                                brief = 'PRIOR ANALYSIS — your own planning worksheet (verify anything marked (verify), and correct it wherever tool results disagree). Its tags are internal: never reproduce them, or any section named after them, in the answer.\n' + raw.strip()
                                return (draft, brief)
                        _least_think = ChatRails.least_think
                        _chat_simple = ChatRails.chat_simple
                        _chat_turn = ChatRails.chat_turn
                        _knowledge_brief = ChatRails.knowledge_brief
                        ...
                        class SolveEngine:
                            ...
                            async def solve(query: Query, question: str) -> Response:
                                ...
                                try:
                                    ...
                                    return Response(text=text)
                        _loop = SolveEngine.loop
                        _audit_patch = SolveEngine.audit_patch
                        _solve = SolveEngine.solve
                        _BRACKET_FIX = {12304: '[', 12305: ']', 65339: '[', 65341: ']', 65288: '(', 65289: ')', 8209: '-', 8722: '-'}
                        for _d in range(10):
                            _BRACKET_FIX[65296 + _d] = chr(48 + _d)
                        ...
                        class ReplyCraft:
                            ...
                            def cap(text: str) -> str:
                                ...
                                if len(t) > ANSWER_CHAR_CAP:
                                    return t[:ANSWER_CHAR_CAP - 16] + ' …'
                                return t
                        _normalize_brackets = ReplyCraft.normalize_brackets
                        _cited_numbers = ReplyCraft.cited_numbers
                        _answer_line_only = ReplyCraft.answer_line_only
                        _verbatim_from_source = ReplyCraft.verbatim_from_source
                        _verbatim_structured = ReplyCraft.verbatim_structured
                        _citations_for = ReplyCraft.citations_for
                        _looks_like_tool_json = ReplyCraft.looks_like_tool_json
                        _is_degenerate_repetition = ReplyCraft.is_degenerate_repetition
                        _is_usable_answer = ReplyCraft.is_usable_answer
                        _sanitize_draft = ReplyCraft.sanitize_draft
                        _strip_lead_narration = ReplyCraft.strip_lead_narration
                        _cap = ReplyCraft.cap
                        _CITE_NUM_RE = re.compile('\\[([0-9][0-9,\\s\\-]*)\\]')
                        _OUTPUT_ONLY_RE = re.compile('\\boutput only\\b|\\brespond with only\\b|\\breply with only\\b|\\banswer with only\\b|\\bonly the exact\\b|\\bnothing else\\b|\\bno explanation\\b|\\bwithout explanation\\b|\\bno other text\\b|\\bjust the (?:name|names|value|values|number|numbers|list|text|answer|title|titles)\\b', re.IGNORECASE)
                        _OUTPUT_ONLY_MIN_CHARS = 2
                        ...
                        class DigesterPad:
                            ...
                            async def knowledge_resort(question: str, deadline: float) -> str:
                                ...
                                try:
                                    ...
                                    return ''
                        _ledger_digest = DigesterPad.ledger_digest
                        _informative_lead = DigesterPad.informative_lead
                        _deterministic_answer = DigesterPad.deterministic_answer
                        _quote_table = DigesterPad.quote_table
                        _retained_count = DigesterPad.retained_count
                        _write_from_digest = DigesterPad.write_from_digest
                        _knowledge_resort = DigesterPad.knowledge_resort
                        _FURNITURE_RE = re.compile('^\\s*(?:share|search|home|menu|subscribe|sign\\s*in|log\\s*in|newsletter|advertisement|cookie|skip to|follow us|read more|related|tags?|categories?|privacy|terms|contact|about us|navigation|toggle)\\b', re.I)
                        _SRC_FOOTNOTE_RE = re.compile('\\[\\s*\\d{1,3}\\s*\\]')
                        _MD_LINK_RE = re.compile('\\]\\(')
                        _BARE_URL_RE = re.compile('(?<!\\]\\()https?://')
                        ...
                        class FormBinder:
                            ...
                            def coerce_to_schema(answer: str, schema, depth: int=0):
                                ...
                                if kind == 'boolean':
                                    return not re.match('\\s*(no\\b|false\\b|none\\b)', answer or '', re.I)
                                return (answer or '')[:400]
                        _schema_output = FormBinder.schema_output
                        _schema_kind = FormBinder.schema_kind
                        _matches_schema_shape = FormBinder.matches_schema_shape
                        _clean_snippet_element = FormBinder.clean_snippet_element
                        _is_citation_metadata_dump = FormBinder.is_citation_metadata_dump
                        _coerce_to_schema = FormBinder.coerce_to_schema
                        _NUM_IN_TEXT_RE = re.compile('-?\\d[\\d,]*(?:\\.\\d+)?')
                        ...
                        class GapPatchLane:
                            ...
                            async def hv16_verification_patch(query_text: str, response: 'Response') -> 'Response':
                                ...
                                try:
                                    ...
                                    return response
                        _hv16_base_query = GapPatchLane.hv16_base_query
                        _hv16_extract_json_object = GapPatchLane.hv16_extract_json_object
                        _hv16_identify_gaps = GapPatchLane.hv16_identify_gaps
                        _hv16_fresh_search_digest = GapPatchLane.hv16_fresh_search_digest
                        _hv16_verify_claim = GapPatchLane.hv16_verify_claim
                        _hv16_rewrite_without_claim = GapPatchLane.hv16_rewrite_without_claim
                        _hv16_fill_missing_element = GapPatchLane.hv16_fill_missing_element
                        _hv16_verification_patch = GapPatchLane.hv16_verification_patch
                ...
                class Question:
                    ...
                    async def _rank(self, text: str) -> str:
                ...
                class Api2:
                    ...
                    def _main(self):
                        ...
                        WRAPUP_AT_S = 90.0
                        MIN_TAIL_S = 8.0
                        LOOP_TOOLS = [{'type': 'function', 'function': {'name': 'sec_filing', 'description': "Resolve a company's SEC filing to its primary document URL on sec.gov (exact form + year, from EDGAR's own index). Use for questions about a specific filing (10-K, 10-Q, 8-K, DEF 14A…), then read_page the returned URL with a focus hint for the Item/section.", 'parameters': {'type': 'object', 'properties': {'company': {'type': 'string', 'description': "company name or ticker, e.g. 'Apple' or 'AAPL'"}, 'form': {'type': 'string', 'description': "filing form, e.g. '10-K', '10-Q', '8-K', 'DEF 14A'"}, 'year': {'type': 'string', 'description': "optional report (fiscal) year, e.g. '2019' (omit for latest)"}}, 'required': ['company', 'form']}}}, {'type': 'function', 'function': {'name': 'web_search', 'description': 'Web search. Returns numbered results, each with title, url and excerpt.', 'parameters': {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'the search query'}}, 'required': ['query']}}}, {'type': 'function', 'function': {'name': 'read_page', 'description': 'Fetch a URL and return its main text. Large pages show the head plus the few regions most relevant to the question; pass a focus hint to steer which regions.', 'parameters': {'type': 'object', 'properties': {'url': {'type': 'string', 'description': 'URL to fetch'}, 'focus': {'type': 'string', 'description': 'optional phrase to locate inside the page (section name, table label, entity)'}}, 'required': ['url']}}}, {'type': 'function', 'function': {'name': 'page_read', 'description': 'Read an arbitrary character range of a page you already fetched. Use the offsets page_grep reports to read the full table or section around a match.', 'parameters': {'type': 'object', 'properties': {'url': {'type': 'string', 'description': 'URL already fetched'}, 'offset': {'type': 'integer', 'description': 'start character offset'}, 'length': {'type': 'integer', 'description': 'how many characters to read (max 12000)'}}, 'required': ['url', 'offset']}}}, {'type': 'function', 'function': {'name': 'page_grep', 'description': 'Search INSIDE a page you already fetched, by regex or literal text, and get every match with its surrounding context and character offset. Use this when read_page showed you the head of a long page but the value you need is deeper in it -- do not re-fetch, grep it.', 'parameters': {'type': 'object', 'properties': {'url': {'type': 'string', 'description': 'URL of a page already fetched this run'}, 'pattern': {'type': 'string', 'description': 'regex or literal string to find, e.g. a city name, a year, a column label'}}, 'required': ['url', 'pattern']}}}, {'type': 'function', 'function': {'name': 'retain_evidence', 'description': "Keep the exact source text that proves a claim you are about to make. Pass the result number and the verbatim quote from it. Do this the moment you find a decisive value -- the judge only credits claims whose citation contains the supporting text, and this is how that text gets into your citation. Use it for the QUESTION'S PREMISES as well as your answer: every entity, work, date or figure the question names should end up with a retained quote confirming it.", 'parameters': {'type': 'object', 'properties': {'source': {'type': 'string', 'description': 'result number to quote from, e.g. 3'}, 'quote': {'type': 'string', 'description': 'verbatim text copied from that result that states the fact'}}, 'required': ['source', 'quote']}}}]
                        LOOP_MODEL_A = 'z-ai/glm-5.2'
                        LOOP_MODEL_B = 'zai/glm-5.2-fast'
                        AUDIT_MODEL = 'openai/gpt-oss-120b'
                        MAX_TURNS = 15
                        ...
                        class WalletWatch:
                            ...
                            @staticmethod
                            def _wallet_note(payload) -> None:
                                budget = getattr(payload, 'budget', None)
                                left = getattr(budget, 'session_remaining_budget_usd', None)
                                if isinstance(left, (int, float)):
                                    _SPEND['left'] = float(left)
                        _wallet_note = WalletWatch._wallet_note
                        _wallet_left = WalletWatch._wallet_left
                        LOOP_RULES = 'You are a research agent answering a hard multi-part factual question. A judge compares your answer head-to-head with a strong reference and only credits claims that carry a citation to a tool result that states them.\n\nPREFER THE PRIMARY SOURCE: when two sources state the same fact, cite the one that ORIGINATES it -- the agency, registry, filing, official statistics release or the organisation\'s own page -- not an encyclopedia or aggregator repeating it. Measured verbatim on a task where both answers were factually correct: "Answer 1 is preferred for using primary sources" (it cited NARA where we cited Wikipedia) -- a full point lost on every run. Use the encyclopedia to FIND the primary source, then fetch and cite that.\n\nQUOTE WHAT PROVES IT: the judge credits a claim only when your citation CONTAINS the source text stating it. The moment you read a decisive value, call retain_evidence(source, quote) with the exact words from that result. Do this for every condition you test and every figure you report -- an answer whose citations do not carry its numbers loses to one that does, even when both answers are identical.\nALSO QUOTE THE QUESTION\'S PREMISES, not only your answer. Every entity, work, date or figure the question NAMES is a claim the judge expects traceable: the film it says someone directed, the article it points at, the year it fixes, the people it lists. You lose to an otherwise identical answer that cited those too -- measured verbatim: "does not provide a citation for \'Everyone Says I Love You\'... Answer 1 is more thorough in its traceability to all parts of the prompt\'s context". Retain a quote for each named premise as you confirm it, even when it is background you already believed.\n\nREAD DEEP, DO NOT RE-FETCH: read_page shows the head plus a few regions of a long page. If the value you need is not in what you were shown, call page_grep(url, pattern) to find it anywhere in that page and page_read to open the region around a reported offset. Grepping a page you already have costs nothing and beats another search.\n\nMETHOD: think in constraints and candidates. Recall what you already know to form the candidate pool, then use web_search/read_page to verify every load-bearing fact (names, figures, dates, rankings) before asserting it. Work every candidate through every stated condition; one search per fact beats one broad search. TWO DISTINCT SUB-QUESTIONS: if the question asks two separate things, answer BOTH substantively — a partial answer covering both sides outscores a complete answer to only one. BATCH YOUR LOOKUPS: independent facts (each candidate\'s score, each entity\'s figure) should be requested as SEVERAL tool calls in the SAME turn — they run in parallel, so a 6-candidate sweep costs one turn, not six. TABLE CARE: when reading a table, respect its qualifier columns (Owned vs Leased, the exact year, the exact segment) — count or compare only rows matching EVERY stated qualifier, and quote the row values you used. For a named source (Box Office Mojo, a 10-K, Nielsen), fetch THAT page — for SEC filings, use the sec_filing tool to resolve the exact primary document from EDGAR\'s own index, then read_page it with a focus hint for the Item/section.\n\nCITE EVERYTHING: put [n] (the tool-result number) immediately after the SENTENCE carrying each claim — not pooled at the end of a paragraph. Every sentence asserting a number, date, proper noun or causal link needs its own [n], for the entities you rule OUT as well as those you include. An uncited specific reads as invented. Cite only results that actually state the claim, and prefer the most AUTHORITATIVE one that does: the official database/filing/statistics page over an aggregator, blog, or retrospective article. CITE THE HARD CONDITION, NOT JUST THE POOL: every stated condition needs evidence of its own, and the one hardest to verify is the one the grader checks. Citations that establish only the candidate pool leave the actual filter unsupported — a right answer whose decisive condition is uncited loses to a weaker answer that proves it.\n\nSOURCE CONFIDENCE: when the question NAMES a source you could not reach but other authoritative evidence establishes the same facts, state those facts plainly and confidently with their [n], and treat the other sources as corroboration. Do not open with, dwell on, or append a note that the named source was unavailable — reserve missing-source language for a FACT that is genuinely absent everywhere, never for a missing source LABEL.\n\nSELF-CONSISTENCY: before you finish, check that the opening names exactly the entities your own cited sentences support. If the body establishes a different answer than the opening claims, rewrite the opening to match the evidence — never leave a weaker fallback in the lead.\n\nANSWER SHAPE: sentence one IS the answer — the exact entities/values/list asked for, in the requested format. Never open with \'Based on…\', \'From my research…\', \'I can provide a partial answer\', or any preamble — start with the answer entities themselves. ANSWER THE ASKED KIND: if the question asks which SERIES, name the series (not the people in it); which FILM, the film (not its director); which COUNTRY, the country. THE POOL IS THE WHOLE NAMED CLASS, NOT THE SURVIVORS: build it from the broadest set the question ranges over — every member of that class, not the ones you already believe qualify — then apply the conditions one at a time and show who each one eliminates. Never pre-filter to the members that already pass and present those as the pool — an answer whose pool contains only qualifiers proves nothing about the sweep, which is how a correct answer still scores zero. List members that fail on the FIRST condition too. Then: the candidate pool, each condition applied, and ONE LINE PER POOL MEMBER — a line for every qualifier with its qualifying attribute cited, AND a line for every candidate you rule out with its cited failing condition. Never compress several rejects into one clause (\'X, Y and Z never won [n]\'): each rejected member gets its own line and its own [n], even when the pool runs to a dozen members. A batched exclusion reads as a pool you never checked. Two later instructions may relax this — one when time runs short, one when the pool is too large to list in full — and nothing else does. If you cannot settle a member\'s condition, KEEP it among the qualifiers — a wrongly-dropped qualifier costs as much as a wrong answer — and give its line the strongest fact you did verify. Never add a note about what you could not check. OUTPUT DIRECTIVES ARE LITERAL: obey formatting instructions mechanically. Decide first whether a phrase constrains the OUTPUT or selects the ENTITIES: \'list them without the word "X"\' shapes what you print, so DELETE X from each name; \'whose title does not contain "X"\' / \'titles without the word X\' is a condition on the pool, so keep only members that lack it. When the phrase governs how to print an already-chosen set, the deletion reading applies — it is not a filter. \'in alphabetical/chronological order\' means sort the final list; \'comma-separated\' means join with commas; a requested count means emit the number. These govern the ANSWER LINE — give it in exactly the requested shape, then still add the proof section below it; the shape directive is never a reason to omit the proof. COPY SOURCE VALUES VERBATIM: when the question names a source, every name, label and value in the answer must be the exact string that source prints -- never add a familiar alternative in parentheses, never anglicise a transliteration. \'Makkah\' is the answer; \'Mecca (Makkah)\' is a wrong answer. ONE EXCEPTION, and it is absolute: if the question says to output ONLY the answer (\'output only\', \'respond with only\', \'nothing else\', \'no explanation\'), emit the answer line as the BARE requested text — no [n] markers on it, nothing else on that line: a trailing [3] makes the text inexact and fails the instruction. Still write the PROOF section BELOW it carrying its [n] markers. Only the answer line is shipped, but the citations are harvested from the proof first, and an uncited answer scores zero. Obeying that instruction IS the task. When an ORDER is demanded, the ANSWER LINE itself must be sorted — not merely the table under it. Print the sort key beside each item (the year, figure or date you sorted on) and check every adjacent pair before you finish: one member out of sequence fails the whole answer even when the set is exactly right. COMPUTED ANSWERS: if the answer is a mean, total, rank or count derived from several figures, pull every input into one explicit list first, then compute — and show the arithmetic so the number is checkable. Never report a derived number you did not visibly compute from listed inputs. ROUNDED FIGURE = WRONG SOURCE: a decisive number that reads as rounded — trailing zeros where the measuring body publishes exact digits, \'X.Y thousand/million\', \'about\'/\'approximately\', or a value lifted from a chart label — came from an aggregator that publishes summaries, not from the body that measured it. Do NOT commit it. Search again for the exact figure from the source the question NAMES (or the outlet that reports that source\'s own numbers) and answer with the full precision it publishes, digit for digit. Quote the rounded value only as corroboration after the exact one. This is a RETRIEVAL instruction, not a licence to withhold: once tool calls are closed, or if the named source itself publishes only the rounded value, commit the best figure you hold and never remark on its precision. EXACT VALUES ONLY: this governs HOW you report a figure; the rule above governs WHICH figure to go and fetch. Once you hold the right one, use the figures you READ in a tool result, verbatim — preserve notation exactly (58.58% and 58.6% are different; \'p < 0.0001\' and \'P < .001\' must not be merged or called consistent). If one source gives a range and another a point value, give both and say whether the point falls inside the range. If a figure is reported in different units than the question asks, convert it and give the exact converted result, preserving units and any timezone label. Answer with the value from the exact source, date and scope the question NAMES — do not substitute a later or broader figure unless resolving a conflict requires it. Bind every claim to the exact actor, target, date-window and instrument the evidence ties together; never carry a statement about one party or period across to another. Never a remembered or approximate value (\'~$1.33B\'), never rounded, never an adjacent year/quarter/metric. If a deciding figure is still unverified at writing time, prefer the tool-read value you have over a guess, and NEVER write \'(verify)\' or any uncertainty marker in the final answer — the final answer contains only committed prose.\n\nAMBIGUOUS METRIC? ANSWER BOTH READINGS. If the asked quantity has two defensible interpretations — one party\'s value or the combined value of both; one dimension of size or another; a narrow scope or a consolidated one — do NOT silently pick one. Name the ambiguity in one clause and give BOTH lists/values, each cited and labelled. A correct answer under the reading the grader did not use still scores as wrong.\n\nAPPLY CONDITIONS LITERALLY: copy each candidate\'s exact value, then test the comparator as written — \'more than 25\' is strictly >25 (25 fails); \'between 2010 and 2019\' includes both endpoints; convert a rate condition into a concrete integer test (\'averaged more than 1 per year over 10 years\' = \'more than 10 in total\'); read edition/date boundaries literally. EXCLUDE ONLY ON PROOF: reject a candidate by naming the specific stated condition it fails, with the cited fact showing the failure — never because it looks weaker than your front-runner. If it is UNCERTAIN whether a candidate fails a condition, KEEP IT in the answer rather than dropping it on a guess: a wrongly-dropped qualifier costs exactly as much as a wrong answer. SAY NO MORE THAN THE CITATION: if the source says \'brought to\', do not write \'incarcerated\'; if it gives a count of 12, do not write 11. Check every count and every verb against its citation.\n\nNEVER NARRATE YOUR EVIDENCE: no sentence about what your results do or do not contain (\'the evidence does not specify…\', \'would be needed to determine…\'). Those phrasings lose. A substantive negative about the WORLD is different and is a real answer when true (\'No member of the class satisfies every condition [n]\'). If a datum truly cannot be verified, commit to the best-supported value you found and move on. ONE narrow exception: when the asked figure genuinely does not exist in any published form, you may state the REASONED IMPOSSIBILITY — name the specific dataset that would hold it and why it cannot yield the value — as a fact about the world, in the first line, alongside the closest cited facts. That is a committed answer; \'the evidence does not contain it\' is not.\n\nFINISH: never mix tool calls and the final answer in one turn. When the constraints are verified (or best-effort covered), write the complete cited answer.'

                        class AskPattern:

                            @staticmethod
                            def _needs_whole_set(question: str) -> bool:
                                q = ' '.join((question or '').split())
                                if _SET_HINT_RE.search(q):
                                    return True
                                m = _PLURAL_HEAD_RE.search(q)
                                if m and m.group(1).lower() not in _PLURAL_FALSE:
                                    if not _has_max_word(q) or re.search('\\b(?:all|every|each)\\b', q, re.IGNORECASE):
                                        return True
                                return bool(re.search('\\bwhich\\b', q, re.IGNORECASE)) and bool(_SET_CONNECTIVE_RE.search(q))

                            @staticmethod
                            def _needs_max_proof(question: str) -> bool:
                                q = ' '.join((question or '').split())
                                if not q:
                                    return False
                                return _has_max_word(q) or bool(re.search('\\b(?:most|least) (?:common|frequent|number|amount)\\b|\\bhow many\\b', q, re.I))

                            @staticmethod
                            def _has_max_word(text: str) -> bool:
                                if _ONE_WINNER_RE.search(text or ''):
                                    return True
                                for m in _EST_RE.finditer(text or ''):
                                    if m.group(0).lower() not in _EST_STOP:
                            ...
                            @staticmethod
                            def _endgame_order(seconds_left: float) -> str:
                                return f"TIME IS UP (~{int(seconds_left)}s left). No more tool calls. Write the complete final answer NOW from the numbered results above plus your knowledge: the FIRST words are the answer entities (no 'Based on…' preamble, no 'partial answer' framing, no '(verify)' markers), cite [n] on every claim, keep the required format. A cited partial answer scores; a refusal or a remark about insufficient evidence scores zero." + ('' if seconds_left >= 60 else ' BREVITY OVERRIDE: too little time remains for a line per pool member. Lead with the answer entities, then give the qualifiers one cited line each and compress the rejects into a single cited line. A complete short answer beats a long one that never finishes.')
                        _endgame_order = AskPattern._endgame_order
                        _has_max_word = AskPattern._has_max_word
                        _needs_max_proof = AskPattern._needs_max_proof
                        _needs_whole_set = AskPattern._needs_whole_set
                        _SET_HINT_RE = re.compile('\\b(?:list|name|identify|enumerate)\\b[^?]{0,40}\\b(?:all|every|each|the)\\b|\\bhow many\\b|\\bwhich (?:movies|films|series|countries|companies|states|cities|books|albums|artists|players|teams|species|languages|banks|universities|agencies|models|products)\\b', re.IGNORECASE)
                        _SET_CONNECTIVE_RE = re.compile('\\b(?:both|also|and (?:also|had|has|was|were)|as well as)\\b', re.IGNORECASE)
                        _PLURAL_HEAD_RE = re.compile('\\b(?:which|what)\\b(?:\\s+\\w+){0,2}?\\s+([a-z]{3,}s)\\b', re.IGNORECASE)
                        _PLURAL_FALSE = frozenset('was is has does its this thus across process business series species news status analysis basis less unless always perhaps'.split())
                        ...
                        class ProofStore:
                            ...
                            def append_row(self, receipt_id: str, result_id: str, note_len: int, kind: str, spans: list[tuple[int, int]] | None, title: str='', url: str='', preview: str='', text: str='') -> int:
                                self.rows.append({'receipt_id': receipt_id, 'result_id': result_id, 'note_len': note_len, 'kind': kind, 'title': (title or '')[:160], 'url': (url or '')[:300], 'preview': (preview or '')[:1200], 'spans': spans, 'text': (text or '')[:_LEDGER_TEXT_CAP], 'retained': []})
                                return len(self.rows)

                            def get_ref(self, number: int) -> CitationRef | None:
                                if not 1 <= number <= len(self.rows):
                                    return None
                                row = self.rows[number - 1]
                                if row.get('kind') == 'reserved':
                        ...
                        class ChunkFinder:
                            ...
                            @staticmethod
                            def _salient_windows(note: str, terms: set[str], width: int, k: int=1) -> list[tuple[int, int]]:
                                n = len(note)
                                if n <= width:
                                    return [(0, n)]
                                step = max(600, width // 3)
                            ...
                            def _salient_terms(text: str) -> set[str]:
                                return {w for w in _WORD_RE.findall((text or '').casefold()) if w not in _STOP}
                        _salient_terms = ChunkFinder._salient_terms
                        _salient_windows = ChunkFinder._salient_windows
                        _SLOT = '\x00{}\x00'
                        ...
                        class CallSealer:
                            ...
                            @staticmethod
                            def _seal_call_output(out, ledger: ProofStore) -> str:
                                if isinstance(out, str):
                                    return out
                                if not isinstance(out, CallPacket):
                                    return f'# tool crashed: {out}'
                                text = out.text
                                for i, row in enumerate(out.rows):
                                    n = ledger.append_row(row['receipt_id'], row['result_id'], row['note_len'], row['kind'], row['spans'], title=row.get('title', ''), url=row.get('url', ''), preview=row.get('preview', ''), text=row.get('text', ''))
                                    text = text.replace(_SLOT.format(i), str(n))
                                return text
                        _seal_call_output = CallSealer._seal_call_output
                        _SITE_OP_RE = re.compile('\\bsite:\\S+\\s*', re.I)

                        class SurfScout:

                            @staticmethod
                            async def _fire_fetch(url: str, focus: str, question: str, ledger: ProofStore) -> str:
                                if not url.strip():
                                    return '# read_page: empty url'
                                payload = None
                                for _attempt in (0, 1):
                                ...
                                if len(note) <= FETCH_PLAIN_CHARS:
                                    row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': [(0, len(note))], 'title': url, 'url': url, 'preview': note[:1200], 'text': note}
                                    return CallPacket(f'# read_page({url!r}) -> [{_SLOT.format(0)}] full page, {len(note)} chars\n{note}', [row])
                                terms = _salient_terms(question) | _salient_terms(focus)
                                windows = _salient_windows(note, terms, FETCH_WINDOW_CHARS, k=FETCH_WINDOWS_PER_PAGE)
                                row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': [(0, FETCH_HEAD_CHARS)] + list(windows), 'title': url, 'url': url, 'preview': note[windows[0][0]:windows[0][0] + 1200], 'text': note}
                                head = note[:FETCH_HEAD_CHARS]
                                sections = ''.join((f'\n--- section @{s} ---\n{note[s:e]}' for s, e in windows))
                                return CallPacket(f"# read_page({url!r}) -> [{_SLOT.format(0)}] {len(note)} chars total; head + the {len(windows)} most relevant section(s) shown ({', '.join((f'{s}-{e}' for s, e in windows))}). If the answer set may continue elsewhere in this page, call read_page again with a different focus.\n--- head ---\n{head}{sections}", [row])

                            @staticmethod
                            async def _fire_search(query_text: str, ledger: ProofStore):
                                if not query_text.strip():
                                    return '# web_search: empty query'
                                payload = None
                                fired: set[str] = set()
                                for attempt, allow_repeat in ((query_text, False), (query_text, True), (_loosen_query(query_text), False)):
                                    if not attempt.strip() or (attempt in fired and (not allow_repeat)):
                                        continue
                                    fired.add(attempt)
                                    try:
                            ...
                            @staticmethod
                            def _loosen_query(q: str) -> str:
                                out = _SITE_OP_RE.sub('', q or '').replace('"', ' ')
                                return ' '.join(out.split())
                        _loosen_query = SurfScout._loosen_query
                        _fire_search = SurfScout._fire_search
                        _fire_fetch = SurfScout._fire_fetch
                        _SEC_TICKERS_URL = 'https://www.sec.gov/files/company_tickers.json'
                        _SEC_SUBMISSIONS_URL = 'https://data.sec.gov/submissions/CIK{cik10}.json'
                        _SEC_DOC_URL = 'https://www.sec.gov/Archives/edgar/data/{cik}/{accession}/{doc}'
                        _SEC_FETCH_TIMEOUT_S = 26.0
                        ...
                        class EdgarFetch:
                            ...
                            @staticmethod
                            async def _fire_sec_filing(company: str, form: str, year: str, deadline: float) -> str:
                                company = (company or '').strip()
                                form = (form or '').strip() or '10-K'
                                year = (year or '').strip()[:4]
                                hint = _SEC_SEARCH_HINT.format(company=company, year=year, form=form)
                                ...
                                if deadline - monotonic() < _SEC_MIN_HEADROOM_S:
                                    return f'# sec_filing: skipped (low time) — {hint}'
                                tickers = await _pull_json_doc(_SEC_TICKERS_URL, deadline)
                                if not isinstance(tickers, dict):
                                    return f'# sec_filing: EDGAR ticker index unavailable — {hint}'
                                want = _sec_bits(company)
                                best = None
                                for row in tickers.values():
                                    if not isinstance(row, dict):
                                        continue
                                ...
                                if best is None:
                                    return f'# sec_filing({company!r}): no confident EDGAR match — {hint}'
                                cik10, title = (best[2], best[3])
                                subs = await _pull_json_doc(_SEC_SUBMISSIONS_URL.format(cik10=cik10), deadline)
                                filings = subs.get('filings') if isinstance(subs, dict) else None
                                recent = filings.get('recent') if isinstance(filings, dict) else None
                                if not isinstance(recent, dict):
                                    return f'# sec_filing({company!r}): EDGAR submissions unavailable for {title} — {hint}'
                                pick = _sec_select_filing(recent, form, year)
                                if pick is None:
                                    return f"# sec_filing({company!r}, {form!r}, year={year or 'latest'}): no matching filing in EDGAR's recent index for {title} — check the form/year, or {hint}"
                                accession, doc = pick
                                url = _SEC_DOC_URL.format(cik=cik10.lstrip('0') or cik10, accession=accession.replace('-', ''), doc=doc)
                            ...
                            @staticmethod
                            def _sec_select_filing(recent: dict, form: str, year: str):
                                forms = recent.get('form')
                                accs = recent.get('accessionNumber')
                                docs = recent.get('primaryDocument')
                                rdates = recent.get('reportDate')
                                ...
                                if not (isinstance(forms, list) and isinstance(accs, list) and isinstance(docs, list)):
                                    return None
                                n = min(len(forms), len(accs), len(docs))
                                form_norm = _sec_canon_form(form)
                                best_year = None
                                best_any = None
                                for i in range(n):
                                    if _sec_canon_form(str(forms[i])) != form_norm:
                                        continue
                                    if accs[i] is None or docs[i] is None:
                                        continue
                                    acc = str(accs[i])
                            ...
                            @staticmethod
                            async def _pull_json_doc(url: str, deadline: float):
                                cached = _SEC_CACHE.get(url)
                                if cached is not None:
                                    return cached
                                for _attempt in (0, 1):
                            ...
                            @staticmethod
                            def _sec_canon_form(form: str) -> str:
                                f = ' '.join((form or '').upper().replace('FORM', ' ').split())
                                m = re.fullmatch('(\\d{1,2})\\s*-?\\s*([A-Z])', f)
                                if m:
                                    return f'{m.group(1)}-{m.group(2)}'
                            ...
                            @staticmethod
                            def _sec_bits(text: str) -> list[str]:
                                return [w for w in _SEC_ALNUM_RE.findall((text or '').lower()) if w not in _SEC_STOPWORDS]
                        _sec_bits = EdgarFetch._sec_bits
                        _sec_canon_form = EdgarFetch._sec_canon_form
                        _pull_json_doc = EdgarFetch._pull_json_doc
                        _sec_select_filing = EdgarFetch._sec_select_filing
                        _fire_sec_filing = EdgarFetch._fire_sec_filing
                        _SEC_SEARCH_HINT = 'search "site:sec.gov {company} {year} {form}" and read_page the Archives result'

                        class PageShelf:

                            @staticmethod
                            def _fire_retain_evidence(source: str, quote: str, ledger: ProofStore) -> str:
                                raw = (source or '').strip().strip('[]')
                                try:
                                    n = int(raw)
                                except ValueError:
                            ...
                            @staticmethod
                            def _fire_page_read(url: str, offset: int, length: int, ledger: ProofStore) -> str:
                                hit = _proof_page(url, ledger)
                                if hit is None:
                                    return f'# page_read: {url!r} has not been fetched this run; call read_page first'
                                n, row = hit
                                text = row.get('text') or ''
                            ...
                            @staticmethod
                            def _fire_page_grep(url: str, pattern: str, ledger: ProofStore) -> str:
                                hit = _proof_page(url, ledger)
                                if hit is None:
                                    return f'# page_grep: {url!r} has not been fetched this run; call read_page first'
                                n, row = hit
                            ...
                            @staticmethod
                            def _proof_page(url: str, ledger: ProofStore) -> tuple[int, dict] | None:
                                u = (url or '').strip().rstrip('/')
                                if not u:
                                    return None
                                for i in range(len(ledger.rows) - 1, -1, -1):
                                    ...
                                    if r == u or r.endswith(u) or u.endswith(r):
                                        return (i + 1, row)
                                return None
                        _proof_page = PageShelf._proof_page
                        _fire_page_grep = PageShelf._fire_page_grep
                        _fire_page_read = PageShelf._fire_page_read
                        _fire_retain_evidence = PageShelf._fire_retain_evidence

                        class CallSwitch:

                            @staticmethod
                            async def _drive_tool(call, question: str, ledger: ProofStore, deadline: float) -> str:
                                try:
                                    args = json.loads(getattr(call, 'arguments', None) or '{}')
                                except Exception:
                                    args = {}
                                ...
                                name = getattr(call, 'name', '') or ''
                                if name == 'web_search':
                                    return await _fire_search(str(args.get('query') or ''), ledger)
                                if name == 'read_page':
                                    return await _fire_fetch(str(args.get('url') or ''), str(args.get('focus') or ''), question, ledger)
                                if name == 'retain_evidence':
                                    return _fire_retain_evidence(str(args.get('source') or ''), str(args.get('quote') or ''), ledger)
                                if name == 'page_grep':
                                    return _fire_page_grep(str(args.get('url') or ''), str(args.get('pattern') or ''), ledger)
                                if name == 'page_read':
                                    return _fire_page_read(str(args.get('url') or ''), args.get('offset') or 0, args.get('length') or PAGE_READ_MAX_CHARS, ledger)
                                if name == 'sec_filing':
                                    return await _fire_sec_filing(str(args.get('company') or ''), str(args.get('form') or ''), str(args.get('year') or ''), deadline)
                                return f'# unknown tool {name!r}'
                        _drive_tool = CallSwitch._drive_tool
                        _REASONING_MANDATORY = ('openai/gpt-oss',)

                        class GenBridge:

                            @staticmethod
                            async def _gen_turn(messages: list[dict], deadline: float, *, finish_only: bool, force_tools: bool=False):
                                turn_wall = monotonic() + TURN_TIMEOUT_S + 35.0
                                payload_chars = sum((len(str(msg.get('content') or '')) for msg in messages if isinstance(msg, dict)))
                                for lane_model in ((LLM_LANE_A, LOOP_MODEL_A, True), (LLM_LANE_A, LOOP_MODEL_A, False), (LLM_LANE_B, LOOP_MODEL_B, False)):
                                    lane = lane_model[0]
                                    ...
                                    if timeout <= 5.0:
                                        return None
                                    try:
                                        payload = await asyncio.wait_for(llm_chat(provider=lane, model=model, messages=messages, tools=LOOP_TOOLS if force_tools or not finish_only else None, tool_choice='auto' if force_tools or not finish_only else None, temperature=0.2, thinking={'enabled': False} if finish_only and lane == LLM_LANE_B else {'enabled': True, 'effort': 'low'}, max_output_tokens=6000 if finish_only and lane == LLM_LANE_B else None, provider_extra=_route_upstream(lane, model) if pinned else None, timeout=timeout), timeout=min(timeout + 6.0, max(1.0, deadline - monotonic() - 1.0)))
                                        _wallet_note(payload)
                                        return payload
                                    except Exception:
                                        continue
                            ...
                            @staticmethod
                            async def _gen_simple(lane: str, model: str, system: str, user: str, *, max_tokens: int, timeout: float, think: dict | None=None) -> str:
                                if think is None:
                                    think = _bare_think(lane, model)
                                _pin0 = _route_upstream(lane, model)
                                payload = None
                                for _pin in (_pin0, None) if _pin0 is not None else (None,):
                                    try:
                                        payload = await llm_chat(provider=lane, model=model, messages=[{'role': 'system', 'content': system}, {'role': 'user', 'content': user}], temperature=0.15, max_output_tokens=max_tokens, timeout=timeout, thinking=think, provider_extra=_pin)
                            ...
                            @staticmethod
                            def _route_upstream(lane: str, model: str) -> dict | None:
                                if lane != LLM_LANE_A:
                                    return None
                                if model.startswith('z-ai/glm-5.2'):
                                    only = _FAST_UPSTREAMS
                            ...
                            @staticmethod
                            def _bare_think(lane: str, model: str='') -> dict:
                                for prefix in _REASONING_MANDATORY:
                                    if model.startswith(prefix):
                                        return {'enabled': True, 'effort': 'low'}
                                return {'enabled': False}
                        _bare_think = GenBridge._bare_think
                        _route_upstream = GenBridge._route_upstream
                        _gen_simple = GenBridge._gen_simple
                        _gen_turn = GenBridge._gen_turn
                        _FAST_UPSTREAMS = ('Decart', 'CoreWeave', 'Alibaba')
                        _FAST_UPSTREAMS_OSS = ('Cerebras', 'Groq', 'BaseTen')
                        ...
                        class _StubTurn:
                            llm = _StubLlm()
                            budget = None
                        _EMPTY_TURN = _StubTurn()

                        class DraftBrief:

                            @staticmethod
                            async def _spark_search(question: str, set_question: bool, ledger: ProofStore, deadline: float) -> str:
                                seeds = _spark_queries(question, set_question)
                                if not seeds or deadline - monotonic() < 40.0:
                                    return ''
                                blocks: list = []
                                for seed in seeds:
                                    if deadline - monotonic() < 30.0:
                                        break
                                    try:
                                        out = await asyncio.wait_for(_fire_search(seed, ledger), timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
                                        blocks.append(_seal_call_output(out, ledger))
                                    except Exception:
                                        continue
                                good = [b for b in blocks if isinstance(b, str) and _CITE_MARK_RE.search(b)]
                                if not good:
                            ...
                            @staticmethod
                            def _spark_queries(question: str, set_question: bool) -> list[str]:
                                q = ' '.join((question or '').split())
                                if not q:
                                    return []
                                seeds = [q[:300]]
                            ...
                            @staticmethod
                            async def _forge_brief(question: str) -> tuple[str, str]:
                                system = 'Senior research analyst. Commit to concrete best answers from knowledge; mark uncertain values (verify). Never refuse.'
                                user = f"Question:\n{question}\n\nFill in this internal worksheet. It is planning scratch for your own use, never an answer, so keep the tags lowercase and never reuse them as section headings later.\ndraft: your full best answer now — candidate pool, every stated condition applied, qualifying entities with figures/dates, near-miss exclusions. Flag shaky facts with (verify).\nconditions: each atomic condition in the question, numbered, including any output-format demand.\nsearches: 3-6 precise web searches for the facts that decide the answer (entity + metric + year; include a named source's site: filter).\nurls: up to 5 exact URLs worth reading directly (official stats pages, sec.gov Archives filings, boxofficemojo year pages); 'none' if unsure."
                                raw = ''
                                try:
                                    raw = await _gen_simple(LLM_LANE_A, LOOP_MODEL_A, system, user, max_tokens=2400, timeout=BRIEF_TIMEOUT_S, think=_bare_think(LLM_LANE_A, LOOP_MODEL_A))
                                except Exception:
                                    try:
                                        raw = await _gen_simple(LLM_LANE_B, LOOP_MODEL_B, system, user, max_tokens=2400, timeout=BRIEF_TIMEOUT_S, think=_bare_think(LLM_LANE_B, LOOP_MODEL_B))
                                    except Exception:
                                        raw = ''
                                if not raw:
                                    return ('', '')
                                ...
                                brief = 'PRIOR ANALYSIS — your own planning worksheet (verify anything marked (verify), and correct it wherever tool results disagree). Its tags are internal: never reproduce them, or any section named after them, in the answer.\n' + raw.strip()
                                return (draft, brief)
                        _forge_brief = DraftBrief._forge_brief
                        _spark_queries = DraftBrief._spark_queries
                        _SEED_TOKEN_RE = re.compile("[A-Za-z0-9][A-Za-z0-9.\\-']+")
                        _SEED_STOP = frozenset('name list give tell show find identify please could would you your can may might should must let make sure both also'.split())
                        MAX_SEED_QUERIES = 3
                        _spark_search = DraftBrief._spark_search

                        class RunCycle:

                            @staticmethod
                            async def _audit_amend(question: str, answer: str, messages: list[dict], ledger: ProofStore, deadline: float) -> str:
                                probe = f"""Audit the answer against the question. JSON only, keys: "unanswered_parts" (list; question elements not addressed), "uncited_facts" (list; load-bearing claims without [n]), "wrong_kind" (list; places where the named entity is a different KIND than the question asks — a person instead of a series, a duo instead of a show), "incomplete_roster" (list; THE MOST COMMON LOSS. If the question ranges over a candidate pool — a closed set that can be enumerated, or several conditions applied to a class — then: is the pool itself stated and plausibly COMPLETE, and does the answer give a verdict for EVERY member (qualifies / excluded because X, each cited)? Name any pool member the answer never mentions, and say so if the pool looks truncated — an answer naming 3 qualifiers when the pool holds 6 scores as WRONG, not partial), "thin_proof" (list; a qualifier lacking a per-condition citation, or a plausible near-miss candidate never addressed), "hand_waved_tally" (list; for a superlative/count/most-common question: the answer asserts a winner or a count WITHOUT showing the candidate table it was derived from. Phrases like 'among others', 'and several more', 'multiple X', or naming 2 examples to justify a count are all hand-waving — say so and name what the tally must list). Empty lists when clean.\n\nQuestion:\n{question}\n\nAnswer:\n{answer[:11000]}"""
                                try:
                                    raw = await _gen_simple(LLM_LANE_A, AUDIT_MODEL, 'Strict completeness auditor. JSON only.', probe, max_tokens=2200, timeout=max(8.0, min(AUDIT_TIMEOUT_S, deadline - monotonic() - 72.0)))
                                    raw = re.sub('^```(?:json)?\\s*|\\s*```$', '', raw.strip(), flags=re.I | re.M)
                                    report = json.loads(raw)
                                except Exception:
                                    return answer
                                ...
                                order += '\nUse at most 3 tool calls to close the most important gaps, then rewrite the COMPLETE final answer with [n] citations in the required shape.'
                                messages.append({'role': 'system', 'content': order})
                                patched, _ = await _run_cycle(question, '', ledger, deadline, AUDIT_EXTRA_TURNS + 1, carry=messages, allow_tools_in_wrapup=True)
                                patched = patched.strip()
                                if not _is_ready_answer(patched) or len(patched) < int(len(answer) * 0.6):
                                    return answer
                                return patched

                            @staticmethod
                            async def _run_cycle(question: str, brief: str, ledger: ProofStore, deadline: float, turn_cap: int, carry: list[dict] | None=None, allow_tools_in_wrapup: bool=False) -> tuple[str, list[dict]]:
                                if carry is not None:
                                    messages = carry
                                else:
                                    set_q = _needs_whole_set(question)
                                    messages = [{'role': 'system', 'content': LOOP_RULES}]
                                    if set_q:
                                        messages.append({'role': 'system', 'content': SET_RULE})
                                    if _needs_max_proof(question):
                                        messages.append({'role': 'system', 'content': SUPERLATIVE_RULE})
                                    if brief:
                                        messages.append({'role': 'system', 'content': brief})
                                    seeded = await _spark_search(question, set_q, ledger, deadline)
                                    if seeded:
                                        messages.append({'role': 'system', 'content': seeded})
                                    messages.append({'role': 'user', 'content': question})
                                answer = ''
                                ...
                                for turn in range(1, turn_cap + 1):
                                    ...
                                    if left <= MIN_TAIL_S:
                                        break
                                    out_of_time = left <= WRAPUP_AT_S
                                    out_of_spend = _wallet_left() <= WRAPUP_MIN_USD
                                    finish_only = out_of_time or out_of_spend or turn >= turn_cap
                                    if (finish_only or turn >= turn_cap - 1) and (not ordered_wrapup):
                                        messages.append({'role': 'system', 'content': _endgame_order(left)})
                                        ordered_wrapup = True
                                    payload = await _gen_turn(messages, deadline, finish_only=finish_only, force_tools=allow_tools_in_wrapup and turn == 1)
                                    if payload is None:
                                        break
                                    llm = getattr(payload, 'llm', None)
                                    choices = getattr(llm, 'choices', None) or []
                                    ...
                                    if not calls:
                                        ...
                                        if not candidate:
                                            ...
                                            if isinstance(content, str):
                                                candidate = content.strip()
                                        if not _is_ready_answer(candidate):
                                            if repairs_left > 0 and deadline - monotonic() > MIN_TAIL_S + 10.0:
                                                repairs_left -= 1
                                                messages.append({'role': 'system', 'content': _REPAIR_ORDER})
                                                answer = ''
                                    ...
                                    run_calls = calls[:8]
                                    tool_budget = max(5.0, min(FETCH_TIMEOUT_S * 2 + 6.0, deadline - monotonic() - MIN_TAIL_S))
                                    tool_tasks = [asyncio.ensure_future(_drive_tool(c, question, ledger, deadline)) for c in run_calls]
                                    try:
                                        await asyncio.wait(tool_tasks, timeout=tool_budget)
                                    except Exception:
                                        pass
                                    ...
                                    for call_result in zip(run_calls, results):
                                        call = call_result[0]
                                        body = _seal_call_output(call_result[1], ledger)
                                        messages.append({'role': 'tool', 'tool_call_id': call.id, 'content': body})
                                    for call in calls[8:]:
                                        messages.append({'role': 'tool', 'tool_call_id': call.id, 'content': '# skipped: per-turn tool budget reached — re-issue next turn if still needed'})
                                return (answer, messages)
                        _run_cycle = RunCycle._run_cycle
                        _audit_amend = RunCycle._audit_amend
                        _BRACKET_FIX = {12304: '[', 12305: ']', 65339: '[', 65341: ']', 65288: '(', 65289: ')', 8209: '-', 8722: '-'}
                        for _d in range(10):
                            _BRACKET_FIX[65296 + _d] = chr(48 + _d)
                        ...
                        class FinalForm:
                            ...
                            @staticmethod
                            def _gather_citations(answer: str, ledger: ProofStore) -> list[CitationRef]:
                                refs: list[CitationRef] = []
                                spent = 0
                                for n in _cited_refs(answer, len(ledger.rows)):
                                    if len(refs) >= CITATION_CAP:
                                        break
                                    ref = ledger.get_ref(n)
                                    if ref is None:
                                        continue
                                    row = ledger.rows[n - 1]
                                    slices = getattr(ref, 'slices', None)
                            ...
                            @staticmethod
                            def _mirror_structured(obj, ledger: ProofStore, depth: int=0):
                                if depth > 6:
                                    return obj
                                if isinstance(obj, str):
                                    return _mirror_from_source(obj, ledger)
                                if isinstance(obj, list):
                                    return [_mirror_structured(x, ledger, depth + 1) for x in obj]
                                if isinstance(obj, dict):
                                    return {k: _mirror_structured(v, ledger, depth + 1) for k, v in obj.items()}
                                return obj

                            @staticmethod
                            def _mirror_from_source(value: str, ledger: ProofStore) -> str:
                                v = (value or '').strip()
                                m = _GLOSS_RE.match(v)
                                if not m:
                                    return value
                                ...
                                def contained(t: str) -> bool:
                                    return bool(t) and any((t in src for src in texts))
                                if contained(v):
                                    return value
                                a, b = (m.group('a').strip(), m.group('b').strip())
                                hits = [x for x in (b, a) if contained(x)]
                                if len(hits) == 1:
                                    return hits[0]
                                if len(hits) == 2:
                                    lo, hi = sorted(hits, key=len)
                            ...
                            @staticmethod
                            def _final_line_only(answer: str, question: str) -> str:
                                if not answer or not _OUTPUT_ONLY_RE.search(question or ''):
                                    return answer
                                for raw in answer.split('\n'):
                                    stripped = raw.strip()
                            ...
                            @staticmethod
                            def _cited_refs(answer: str, top: int) -> list[int]:
                                answer = _fix_cites(answer)
                                seen: set[int] = set()
                                out: list[int] = []
                                for m in _CITE_NUM_RE.finditer(answer):
                                    for chunk in m.group(1).split(','):
                            ...
                            def _fix_cites(text: str) -> str:
                                return (text or '').translate(_BRACKET_FIX)
                        _fix_cites = FinalForm._fix_cites
                        _cited_refs = FinalForm._cited_refs
                        _final_line_only = FinalForm._final_line_only
                        _mirror_from_source = FinalForm._mirror_from_source
                        _mirror_structured = FinalForm._mirror_structured
                        _gather_citations = FinalForm._gather_citations
                        _CITE_NUM_RE = re.compile('\\[([0-9][0-9,\\s\\-]*)\\]')
                        _OUTPUT_ONLY_MIN_CHARS = 2
                        _GLOSS_RE = re.compile('^(?P<a>[^()]{2,60}?)\\s*\\((?P<b>[^()]{2,60})\\)$')
                        _VERIFY_MARK_RE = re.compile('\\s*\\((?:verify|unverified|uncertain)[^)]*\\)', re.I)
                        ...
                        class FinalGuard:
                            ...
                            @staticmethod
                            def _wash_draft(text: str) -> str:
                                return _VERIFY_MARK_RE.sub('', text or '').strip()

                            @staticmethod
                            def _is_ready_answer(text: str) -> bool:
                                s = _fix_cites(text).strip()
                                if not s:
                                    return False
                                if _TOOL_MARKUP_RE.search(s) or _feels_tool_json(s):
                                    return False
                                if _STUB_ANSWER_RE.match(s) or _is_junk_repeat(s):
                                    return False
                                cited = bool(_CITE_MARK_RE.search(s))
                                if cited and len(s) >= MIN_CITED_ANSWER_CHARS:
                                    return True
                            ...
                            @staticmethod
                            def _is_junk_repeat(text: str) -> bool:
                                body = text or ''
                                lines = [ln.strip().lower() for ln in body.split('\n') if len(ln.strip()) > 25]
                                if len(lines) >= 3:
                                    for ln in set(lines):
                            ...
                            @staticmethod
                            def _feels_tool_json(s: str) -> bool:
                                return bool(re.match('\\s*\\{\\s*"(?:name|tool|function)"\\s*:', s))
                        _feels_tool_json = FinalGuard._feels_tool_json
                        _is_junk_repeat = FinalGuard._is_junk_repeat
                        _is_ready_answer = FinalGuard._is_ready_answer
                        _wash_draft = FinalGuard._wash_draft
                        _REPAIR_ORDER = 'Your last message was not a usable final answer (it contained tool-call markup, was empty, or was a refusal). Do NOT emit tool syntax as text. Write the FINAL ANSWER now as plain prose: first words are the answer entities themselves, every factual claim followed by its [n] citation, then the short proof section. Nothing else.'

                        class LastAnswer:

                            @staticmethod
                            async def _forge_resort(question: str, deadline: float) -> str:
                                left = deadline - monotonic()
                                if left < 12.0:
                                    return ''
                                try:
                                    return await _gen_simple(LLM_LANE_A, RESORT_MODEL, 'Expert researcher. Best definitive answer with concrete entities, numbers, dates. Never refuse.', question, max_tokens=2600, timeout=min(45.0, left - 4.0))
                                except Exception:
                                    return ''

                            @staticmethod
                            async def _reply_from_digest(question: str, ledger: ProofStore, deadline: float) -> str:
                                left = deadline - monotonic()
                                if left < 14.0:
                                    return ''
                                digest = _proof_digest(ledger)
                                if not digest:
                                    return ''
                                convo = [{'role': 'system', 'content': _COMMIT_RULES}, {'role': 'user', 'content': f'Question: {question}\n\nNumbered evidence you gathered (cite facts by these [n]):\n\n{digest}\n\nWrite the FINAL ANSWER now from this evidence. Plain prose, no tool syntax. First words are the answer entities; every factual claim carries its [n]; then the short proof section (pool, conditions, qualifiers, exclusions).'}]

                                async def _probe_lane(lane: str, model: str, budget: float) -> str:
                                    _p0 = _route_upstream(lane, model)
                                    payload = None
                                    for _p in (_p0, None) if _p0 is not None else (None,):
                                        try:
                                            payload = await llm_chat(provider=lane, model=model, messages=convo, temperature=0.15, max_output_tokens=2600, timeout=budget, thinking=_bare_think(lane, model), provider_extra=_p)
                                            break
                                        except Exception:
                                            if _p is None:
                                                raise
                                ...
                                for i, lane_model in enumerate(lanes):
                                    ...
                                    if budget < 8.0:
                                        return ''
                                    try:
                                        text = await _probe_lane(lane_model[0], lane_model[1], budget)
                                    except Exception:
                                        continue
                                    if _is_ready_answer(text):
                                        return text
                                return ''

                            @staticmethod
                            def _retain_size(ledger: ProofStore) -> int:
                                return sum((len(r.get('retained') or []) for r in ledger.rows))

                            @staticmethod
                            def _quote_sheet(ledger: ProofStore) -> str:
                                parts = []
                                for i, row in enumerate(ledger.rows, start=1):
                                    text = row.get('text') or ''
                                    for a, b in row.get('retained') or []:
                            ...
                            @staticmethod
                            def _hardcoded_answer(question: str, ledger: ProofStore) -> str:
                                rows = [(i, r) for i, r in enumerate(ledger.rows, start=1) if (r.get('preview') or '').strip()]
                                if not rows:
                                    return ''
                                out = ['Best-supported findings from the sources retrieved:']
                                ...
                                for i, r in rows:
                                    if picked >= 6:
                                        break
                                    lead = _dense_lead(r.get('preview') or '')
                                    if not lead:
                                        continue
                                    title = (r.get('title') or '').strip()
                                    out.append(f"- {(title + ': ' if title else '')}{lead} [{i}]")
                            ...
                            @staticmethod
                            def _dense_lead(preview: str, limit: int=280) -> str:
                                kept: list[str] = []
                                broke = False
                                for chunk in re.split('(?<=[.!?])\\s+|\\n+', _SRC_FOOTNOTE_RE.sub('', preview or '')):
                                    seg = ' '.join(chunk.split())
                            ...
                            @staticmethod
                            def _proof_digest(ledger: ProofStore, char_cap: int=60000) -> str:
                                parts: list[str] = []
                                spent = 0
                                for i, row in enumerate(ledger.rows, start=1):
                                    text = (row.get('preview') or '').strip()
                                    ...
                                    parts.append(block)
                                return '\n\n'.join(parts)
                        _proof_digest = LastAnswer._proof_digest
                        _dense_lead = LastAnswer._dense_lead
                        _hardcoded_answer = LastAnswer._hardcoded_answer
                        _quote_sheet = LastAnswer._quote_sheet
                        _retain_size = LastAnswer._retain_size
                        _reply_from_digest = LastAnswer._reply_from_digest
                        _forge_resort = LastAnswer._forge_resort

                        class SchemaMold:

                            @staticmethod
                            def _mold_to_schema(answer: str, schema, depth: int=0):
                                if depth > 4 or not isinstance(schema, dict):
                                    return answer[:400]
                                enum = schema.get('enum')
                                if isinstance(enum, list) and enum:
                                ...
                                if not kind:
                                    for key in ('anyOf', 'oneOf', 'allOf'):
                                        ...
                                        if isinstance(branch, list) and branch:
                                            for sub in branch:
                                                if isinstance(sub, dict) and sub.get('type') != 'null':
                                                    return _mold_to_schema(answer, sub, depth + 1)
                                    kind = 'string'
                                if kind == 'array':
                                    items = schema.get('items') or {}
                                    parts = [p.strip(' -*\t') for p in re.split('[\\n;]|,(?![^(]*\\))', answer or '')]
                                    ...
                                    if not parts:
                                        parts = [answer[:400]]
                                    return [_mold_to_schema(p, items, depth + 1) for p in parts]
                                if kind == 'object':
                                    props = schema.get('properties') or {}
                                    required = schema.get('required') or list(props.keys())
                                    out = {}
                                    for key in required:
                                        out[key] = _mold_to_schema(answer, props.get(key) or {}, depth + 1)
                                    return out
                                if kind in ('number', 'integer'):
                                    found = _NUM_IN_TEXT_RE.search(_CITE_NUM_RE.sub(' ', answer or ''))
                                    if found is None:
                            ...
                            @staticmethod
                            def _decode_for_schema(basis: str) -> str:
                                if not basis:
                                    return ''
                                text = _DIGEST_NOISE_RE.sub(' ', basis)
                                out = []
                            ...
                            @staticmethod
                            def _aligns_schema_shape(value, schema) -> bool:
                                kind = _schema_flavor(schema)
                                if not kind:
                                    return True
                                if kind == 'array':
                            ...
                            @staticmethod
                            async def _schema_render(question: str, answer: str, schema, deadline: float) -> object | None:
                                ask = f'Convert the answer to a JSON value valid under the schema. Output ONLY the JSON value.\n\nSchema:\n{json.dumps(schema)}\n\nQuestion:\n{question}\n\nAnswer:\n{answer[:14000]}'
                                for lane, model in ((LLM_LANE_A, SCHEMA_MODEL), (LLM_LANE_A, RESORT_MODEL), (LLM_LANE_B, LOOP_MODEL_B)):
                                    left = deadline - monotonic()
                                    if left < 12.0:
                                        break
                                    try:
                                        raw = await _gen_simple(lane, model, 'You output strictly valid JSON.', ask, max_tokens=3400, timeout=min(45.0, left - 4.0))
                                        raw = re.sub('^```(?:json)?\\s*|\\s*```$', '', raw.strip(), flags=re.I | re.M).strip()
                                        value = json.loads(raw)
                                        if _aligns_schema_shape(value, schema):
                                            return value
                                        if isinstance(value, dict) and len(value) == 1:
                                            inner = list(value.values())[0]
                                            if _aligns_schema_shape(inner, schema):
                                                return inner
                                    except Exception:
                                        continue
                                return None
                        _schema_render = SchemaMold._schema_render
                        _schema_flavor = SchemaMold._schema_flavor
                        _NUM_IN_TEXT_RE = re.compile('-?\\d[\\d,]*(?:\\.\\d+)?')
                        _DIGEST_LEAD_RE = re.compile('^\\s*Best-supported findings|^\\s*sources retrieved:', re.I)
                        _DIGEST_NOISE_RE = re.compile('\\[slice \\d+:\\d+\\]|https?://\\S+')
                        _VALUE_MAX_CHARS = 90
                        _NARRATION_LEAD_RE = re.compile("^\\s*(?:based on (?:my|the)\\b|now (?:i|that i)\\b|i (?:now )?(?:have|was|am|need|will|can)\\b|i(?:'ll|'ve|'m)\\b|let me\\b|let's\\b|first,? i\\b|having (?:now )?\\w+\\b|okay\\b|alright\\b|to answer this\\b|my research\\b)", re.IGNORECASE)
                        _ABBREV_TAIL_RE = re.compile('(?:\\b[A-Z]|\\b(?:Inc|Ltd|Co|No|vs|St|Dr|Mr|Ms|Mt|Jr|Sr|etc|e\\.g|i\\.e))\\.$')
                        _aligns_schema_shape = SchemaMold._aligns_schema_shape
                        _decode_for_schema = SchemaMold._decode_for_schema
                        _mold_to_schema = SchemaMold._mold_to_schema
                        ...
                        class FinalPolish:
                            ...
                            @staticmethod
                            def _erase_lead_narration(text: str) -> str:
                                t = (text or '').strip()
                                if not t:
                                    return t
                                for _ in range(2):
                        ...
                        class AskRunner:
                            ...
                            async def _solve(query: Query, question: str) -> Response:
                                ...
                                brief = ''
                                try:
                                    if _wallet_left() >= BRIEF_MIN_USD and deadline - monotonic() > 120.0:
                                        draft, brief = await _forge_brief(question)
                                except Exception:
                                    brief = ''
                                ledger = ProofStore()
                                answer = ''
                                messages: list[dict] = []
                                try:
                                    answer, messages = await _run_cycle(question, brief, ledger, deadline, MAX_TURNS)
                                except Exception:
                                    answer = ''
                                try:
                                    if _is_ready_answer(answer) and deadline - monotonic() > 75.0 and (_wallet_left() >= AUDIT_MIN_USD):
                                        patched = await _audit_amend(question, answer, messages, ledger, deadline)
                                        if _is_ready_answer(patched):
                                            answer = patched
                                except Exception:
                                    pass
                                if not _is_ready_answer(answer) and ledger.rows:
                                    try:
                                        rescued = await _reply_from_digest(question, ledger, deadline)
                                        if _is_ready_answer(rescued):
                                            answer = rescued
                                    except Exception:
                                        pass
                                if not _is_ready_answer(answer) and ledger.rows:
                                    det = _hardcoded_answer(question, ledger)
                                    if _is_ready_answer(det):
                                        answer = det
                                if not _is_ready_answer(answer):
                                    fallback = _wash_draft(draft) or await _forge_resort(question, deadline)
                                    if _is_ready_answer(fallback):
                                        answer = fallback
                                try:
                                    citations = _gather_citations(answer, ledger)
                                except Exception:
                                    citations = []
                                answer = _fix_cites(answer)
                                answer = _erase_lead_narration(answer)
                                answer = _final_line_only(answer, question)
                                text = _truncate(answer) or f'Best-effort answer unavailable for: {question[:400]}'
                                if query.output_schema is not None:
                                    structured = None
                                    try:
                                        structured = await _schema_render(question, answer, query.output_schema, deadline)
                                    except Exception:
                                        structured = None
                                    if structured is not None:
                                        try:
                                            structured = _mirror_structured(structured, ledger)
                                        except Exception:
                                            pass
                                        try:
                                            return Response(output=structured, citations=citations or None)
                                            ...
                                            structured = None
                                    basis = answer if _is_ready_answer(answer) else ''
                                    if not basis:
                                        basis = _hardcoded_answer(question, ledger)
                                    if not basis or _STUB_ANSWER_RE.match(basis.strip()):
                                        basis = question[:400]
                                    if basis is not answer:
                                        try:
                                            salvaged = await _schema_render(question, basis, query.output_schema, deadline)
                                        except Exception:
                                            salvaged = None
                                        if salvaged is not None:
                                            try:
                                                ...
                                                pass
                                    if basis is not answer:
                                        cleaned = _decode_for_schema(basis)
                                        basis = cleaned if cleaned else ''
                                    try:
                                        forced = _mold_to_schema(_truncate(basis), query.output_schema)
                                        return Response(output=forced, citations=citations or None)
                                    except Exception:
                                        try:
                                            return Response(output=_truncate(basis)[:2000], citations=citations or None)
                                        except Exception:
                                            pass
                                try:
                                    return Response(text=text, citations=citations or None)
                                    ...
                                    return Response(text=text)
                        _solve = AskRunner._solve
                        _erase_lead_narration = FinalPolish._erase_lead_narration
                        _truncate = FinalPolish._truncate
                        ...
                        async def query(query: Query) -> Response:
                            ...
                            try:
                                ...
                                return Response(text=f'Best-effort answer unavailable for: {question[:500]}')
                        return query
                _EASY_PATH = Api1()._main()
                _Root = Question()

                async def query(query: Query) -> Response:
                    try:
                        level = await _Root._rank(query.text)
                    except Exception:
                        level = 'hard'
                    if level == 'easy':
                        return await _EASY_PATH(query)
                    return await _HARD_PATH(query)
                _HARD_PATH = Api2()._main()
                return query

        class StubKey_891ea0:

            @staticmethod
            def _opal_frame_891ea0() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _OPAL_RUN_891ea0 = OpalFrame_891ea0()._compile()
        _ONYX_RUN_891ea0 = OnyxFrame_891ea0()._compile()
        _STUB_KEY_891ea0 = StubKey_891ea0()

        async def _vc_baseline_query(query: Query) -> Response:
            if _STUB_KEY_891ea0._opal_frame_891ea0():
                return await _OPAL_RUN_891ea0(query)
            return await _ONYX_RUN_891ea0(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid184-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.00 arcc_bells_vireo; 475f0b1b:0.25 usgs_mcs_net_import; 7b7707d5:0.50 unesco_whc_48; 9d58b52a:0.38 nps_weekly_list; b15bd914:0.62 rfc_std_snapshot; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.12 yuba_sutter_transit; eb277bad:0.12 st_helena_census; eef7c321:0.00 hague_evidence_service; f5e14f85:0.50 port_la_container'
        ...
        _V0814_RFC_STD_SNAPSHOT_RE = re.compile('RFC 2500.*RFC 3000.*Internet Standard', re.IGNORECASE | re.DOTALL)
        _V0814_ST_HELENA_CENSUS_RE = re.compile('2021 St Helena Population and Housing Census.*district', re.IGNORECASE | re.DOTALL)
        ...
        def _v0814_detect_signatures(question: str) -> tuple[_V0814TaskSignature, ...]:
            q = question or ''
            checks = (('arcc_bells_vireo', _V0814_ARCC_BELLS_VIREO_RE, 0.96), ('artemis_met_mismatch', _V0814_ARTEMIS_MET_MISMATCH_RE, 0.97), ('hague_evidence_service', _V0814_HAGUE_EVIDENCE_SERVICE_RE, 0.96), ('nps_weekly_list', _V0814_NPS_WEEKLY_LIST_RE, 0.95), ('port_la_container', _V0814_PORT_LA_CONTAINER_RE, 0.93), ('rfc_std_snapshot', _V0814_RFC_STD_SNAPSHOT_RE, 0.92), ('st_helena_census', _V0814_ST_HELENA_CENSUS_RE, 0.92), ('unesco_whc_48', _V0814_UNESCO_WHC_48_RE, 0.94), ('usgs_mcs_net_import', _V0814_USGS_MCS_NET_IMPORT_RE, 0.93), ('yuba_sutter_transit', _V0814_YUBA_SUTTER_TRANSIT_RE, 0.95))
        ...
        async def _v0814_apply_task_rescues(question: str, response: object, signatures: tuple[_V0814TaskSignature, ...], deadline: float) -> object:
            if not signatures or deadline - _v0814_clock() < V0814_MIN_REMAINING_S:
                return response
            dispatch = {'arcc_bells_vireo': _v0814_rescue_arcc, 'artemis_met_mismatch': _v0814_rescue_artemis, 'hague_evidence_service': _v0814_rescue_hague, 'nps_weekly_list': _v0814_rescue_nps, 'port_la_container': _v0814_rescue_port_la, 'rfc_std_snapshot': _v0814_rescue_rfc_std_text, 'st_helena_census': _v0814_rescue_st_helena_text, 'unesco_whc_48': _v0814_rescue_unesco, 'usgs_mcs_net_import': _v0814_rescue_usgs_mcs, 'yuba_sutter_transit': _v0814_rescue_transit}
            current = response
...
class ToggleCore_c2edd7:

    @staticmethod
    def _copper_active_c2edd7() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_COPPER_RUN_c2edd7 = CopperSpine_c2edd7()._compile()
_IRON_RUN_c2edd7 = IronSpine_c2edd7()._compile()
_TOGGLE_CORE_c2edd7 = ToggleCore_c2edd7()


class NeedLedger_90e38e2a:
    ...
    def _compile(self):
        ...
        _need_core_WALL_S = 254.0
        _need_core_STAGE_MIN_S = 22.0
        _need_core_SYNTH_MIN_S = 15.0
        _need_core_TAIL_S = 3.5
        _need_core_SEARCH_TO = 13.0
        _need_core_FETCH_TO = 10.0
        _need_core_PROVIDER = 'parallel'
        _need_core_LANE = "openrouter"
        _need_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _need_core_SYNTH_MODEL = 'z-ai/glm-5.2'
        _need_core_HITS = 5
        _need_core_NOTE_CAP = 86800
        _need_core_EXCERPT = 1308
        _need_core_KEEP = 0.67
        _need_core_PAD = 252
        _need_core_SLICE = 912
        _need_core_MAX_REFS = 4
        _need_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _need_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _need_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _need_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _need_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _need_core_SPLIT = 'Split the user question into the exact required answer elements. Return JSON only: {"asks":[{"id":"E1","need":"<one required fact, comparison side, date, figure, ordering or schema field>","fill":"<web search that would prove it>"}]} with at most seven items. Do not answer the question.'
        _need_core_AUDIT = 'You audit a draft against a required-element contract. For each element say whether the draft answers it AND a numbered extract actually states the supporting words. JSON only: {"asks":[{"id":"...","answered":true,"fill":"<fresh search if unmet else empty>","cite":[1]}]}.'
        _need_core_RIVAL = 'Independent second pass. Using ONLY the numbered extracts, write a short cited answer. Omit anything the extracts do not state. No preamble.'
        _need_core_COMPARE = 'List factual disagreements (name, figure, date, membership, ordering) between A and B. JSON {"asks":[{"a_claim":"...","b_claim":"...","fill":"<search that settles it>"}]} max 3. Empty list if none.'
        _need_core_DOSSIER = 'Extract every load-bearing claim from the draft. JSON {"asks":[{"claim":"...","quote":"<words that must appear in a source>","answered":false}]} max 8.'
        _need_core_PREMISE = 'List named premises the question treats as true (entities, editions, events, years). JSON {"asks":[{"need":"<premise>","fill":"<search to confirm or correct it>"}]} max 6.'
        _need_core_SYNTH = 'You write the FINAL answer after a coverage cycle. Sentence one is the direct answer in the requested shape: no preamble, no hedging, no remarks about evidence gaps. Every load-bearing sentence keeps a [n] glued to it. Cite both sides of any comparison and the conclusion. Copy figures digit-for-digit from extracts. Answer every contract element that extracts support; omit only what remains ungrounded. Obey literal output directives. Never add irrelevant sources.'

        def _need_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _need_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _need_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_need_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _need_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_need_core_SEARCH_TO, _need_core_left(deadline) - _need_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_need_core_PROVIDER, num=_need_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _need_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _need_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_need_core_FETCH_TO, _need_core_left(deadline) - _need_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_need_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _need_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _need_core_NAME.findall(q)
        ...
        def _need_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_need_core_EXCERPT]
                if not body:
                    continue
        ...
        def _need_core_next_label(draft: str) -> int:
            top = 104
            for m in _need_core_MARK.finditer(draft or ""):
                try:
        ...
        def _need_core_figures(text: str) -> set:
            out = set()
            for m in _need_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _need_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _need_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _need_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _need_core_NARRATE.search(head):
                return draft
            dropped = _need_core_figures(draft) - _need_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _need_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _need_core_left(deadline) - _need_core_SYNTH_MIN_S)
            raw = _need_core_json(await _need_core_chat(
                _need_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
        ...
        async def _need_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _need_core_left(deadline) - _need_core_SYNTH_MIN_S)
            raw = _need_core_json(await _need_core_chat(
                _need_core_CHEAP_MODEL, _need_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_need_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _need_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _need_core_left(deadline) - _need_core_SYNTH_MIN_S)
            raw = _need_core_json(await _need_core_chat(
                _need_core_CHEAP_MODEL, _need_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _need_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _need_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _need_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _need_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _need_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _need_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _need_core_left(deadline) < _need_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _need_core_BARE.search(question):
                return response
            contract = await _need_core_plan(question, deadline, _need_core_SPLIT)
            if not contract:
                contract = [{"id": "Q1", "need": question[:180], "fill": question[:180]}]
            board = []
            hunted = False
            for _round in range(2):
                if _need_core_left(deadline) < 15 + 13:
                    break
                board = await _need_core_audit(question, draft, contract, packets, deadline)
                gaps = [g for g in board if not g.get("answered") and str(g.get("fill") or "").strip()]
                if not gaps:
                    break
                for gap in gaps[:2]:
                    if _need_core_left(deadline) < 15 + 13:
                        break
                    await _need_core_seek(packets, seen, str(gap.get("fill") or ""), deadline)
                    hunted = True
            if hunted and _need_core_left(deadline) >= 15 + 10:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _need_core_pull(packets, seen, url, deadline)
            start_no = _need_core_next_label(draft)
            digest = _need_core_digest(packets, start_no)
            compact = json.dumps(board or contract, ensure_ascii=False)[:2400]
            verdict = await _need_core_chat(_need_core_SYNTH_MODEL, _need_core_SYNTH, (
                "Question:\n" + question +
                "\n\nDraft:\n" + draft[:6400] +
                "\n\nCoverage board:\n" + compact +
                "\n\nNumbered extracts (cite as [n]):\n" + (digest or "(none)")
            ), min(12 + 8, _need_core_left(deadline) - 3.0))
            final = _need_core_adopt(draft, verdict)
            refs = _need_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_need_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _need_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _need_core_WALL_S
            response = await base(query)
            try:
                return await _need_core_stage(query, response, deadline)
            except Exception:
                return response

        return _need_core_run

_NEEDLEDGER_90e38e2a = NeedLedger_90e38e2a()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _NEEDLEDGER_90e38e2a(query, _inner_query_90e38e2a)

async def _inner_query_90e38e2a(query: Query) -> Response:
    if _TOGGLE_CORE_c2edd7._copper_active_c2edd7():
        return await _COPPER_RUN_c2edd7(query)
    return await _IRON_RUN_c2edd7(query)
