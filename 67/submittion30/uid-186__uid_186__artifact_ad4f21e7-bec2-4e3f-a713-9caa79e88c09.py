...
DUEL_MIN_LEFT_S = 46.0
DUEL_TAIL_S = 18.0
DUEL_MIN_USD = 0.04
DUEL_RIVAL_TIMEOUT_S = 24.0
DUEL_COMPARE_TIMEOUT_S = 18.0
DUEL_VERDICT_TIMEOUT_S = 32.0
DUEL_MAX_CLASHES = 2
DUEL_DIGEST_CHARS = 36000
DUEL_VERDICT_DIGEST_CHARS = 22000
DUEL_MIN_VERDICT_RATIO = 0.35
RIVAL_RULES = 'You answer a hard factual question strictly from the numbered evidence rows you are given. Work independently: do not soften, do not hedge, commit to the reading the rows best support. Every factual claim carries the [n] of a row whose text states it. Plain prose, no tool syntax, no preamble.'
CLASH_RULES = 'You compare two independently written answers to one research question and list only their FACTUAL disagreements: a different entity, figure, date, unit, pool membership or verdict for the same point. Ignore wording, ordering, level of detail and citation style. Reply with a JSON array only. Each element: {"point": "<what is disputed>", "a": "<what answer A states>", "b": "<what answer B states>", "hunt": "<one targeted web query that would settle it>"}. Reply [] if the answers factually agree.'
VERDICT_RULES = 'You referee a verification duel between two answers to the same question. Decide every disputed point on the numbered evidence alone; rows fetched by the adjudication probes outrank memory and outrank either answer. Then write ONE final answer claim by claim from the winning side of each point. Sentence 1 states the direct answer in exactly the shape the question requests, then the cited specifics. Every load-bearing sentence carries the [n] of a row whose stored text states that fact; copy figures verbatim from the row, never rounded; a comparison cites rows for BOTH sides and for the conclusion drawn; cover every element the question asks for. No preamble, no hedging, no notes about the duel. If the question demands only a bare value, output only that value.'

def _docket_rows(raw: str) -> list[dict]:
    body = (raw or '').strip()
    if body.startswith('```'):
        body = body.strip('`')
    ...
    for item in data if isinstance(data, list) else []:
        ...
        if not point:
            continue
        rows.append({'point': point, 'a': str(item.get('a') or '').strip(), 'b': str(item.get('b') or '').strip(), 'hunt': str(item.get('hunt') or '').strip()})
        if len(rows) >= DUEL_MAX_CLASHES:
            break
    return rows

async def _rival_answer(question: str, ledger: EvidenceLedger, deadline: float) -> str:
    left = deadline - monotonic()
    if left < DUEL_MIN_LEFT_S:
        return ''
    digest = _ledger_digest(ledger, char_cap=DUEL_DIGEST_CHARS)
    if not digest:
        return ''
    user = f'Question: {question}\n\nNumbered evidence rows:\n\n{digest}\n\nWrite your complete answer now.'
    try:
        return await _chat_simple(LLM_LANE_A, AUDIT_MODEL, RIVAL_RULES, user, max_tokens=1900, timeout=min(DUEL_RIVAL_TIMEOUT_S, left - DUEL_TAIL_S))
    except Exception:
        return ''

async def _settle_clashes(docket: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
    blocks: list[str] = []
    for clash in docket:
        if deadline - monotonic() < DUEL_TAIL_S + DUEL_VERDICT_TIMEOUT_S or _spend_left() < WRAPUP_MIN_USD:
            break
        hunt = clash.get('hunt') or clash.get('point') or ''
        if not hunt:
            continue
        try:
            found = await asyncio.wait_for(_do_search(hunt, ledger), timeout=SEARCH_TIMEOUT_S + 8.0)
            body = _commit_tool_output(found, ledger)
        except Exception:
            continue
        if isinstance(body, str) and _CITE_MARK_RE.search(body):
            blocks.append(f"DISPUTE: {clash['point']}\n{body}")
    return '\n\n'.join(blocks)

async def _duel_verdict(question: str, draft: str, rival: str, docket: list[dict], fresh: str, ledger: EvidenceLedger, deadline: float) -> str:
    left = deadline - monotonic()
    if left < DUEL_TAIL_S + 10.0:
        return ''
    points = '\n'.join((f"- {c['point']} || A: {c['a']} || B: {c['b']}" for c in docket))
    digest = _ledger_digest(ledger, char_cap=DUEL_VERDICT_DIGEST_CHARS)
    user = f'Question: {question}\n\nAnswer A (primary draft):\n{draft[:6000]}\n\nAnswer B (independent challenger):\n{rival[:6000]}\n\nDisputed points:\n{points}\n\nAdjudication evidence just fetched (decisive):\n{fresh[:9000]}\n\nAll numbered evidence rows for citation:\n\n{digest}\n\nWrite the final refereed answer now.'
    try:
        return await _chat_simple(LLM_LANE_A, AUDIT_MODEL, VERDICT_RULES, user, max_tokens=2200, timeout=min(DUEL_VERDICT_TIMEOUT_S, left - 8.0))
    except Exception:
        return ''

async def _verification_duel(question: str, draft: str, ledger: EvidenceLedger, deadline: float) -> str:
    rival = await _rival_answer(question, ledger, deadline)
    if not rival or not _is_usable_answer(rival):
        return ''
    left = deadline - monotonic()
    if left < DUEL_TAIL_S + DUEL_COMPARE_TIMEOUT_S:
        return ''
    pair = f'Question: {question}\n\nAnswer A:\n{draft[:6000]}\n\nAnswer B:\n{rival[:6000]}'
    try:
        raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, CLASH_RULES, pair, max_tokens=700, timeout=min(DUEL_COMPARE_TIMEOUT_S, left - DUEL_TAIL_S))
    except Exception:
        return ''
    docket = _docket_rows(raw)
    if not docket:
        return ''
    fresh = await _settle_clashes(docket, ledger, deadline)
    verdict = await _duel_verdict(question, draft, rival, docket, fresh, ledger, deadline)
    if not verdict or not _is_usable_answer(verdict):
        return ''
    if len(draft) > 400 and len(verdict) < int(len(draft) * DUEL_MIN_VERDICT_RATIO):
        return ''
    if _cited_numbers(draft, len(ledger.rows)) and (not _cited_numbers(verdict, len(ledger.rows))):
        return ''
    return verdict
...
async def _solve(query: Query, question: str) -> Response:
    ...
    try:
        ...
        pass
    try:
        if _is_usable_answer(answer) and ledger.rows and (deadline - monotonic() > DUEL_MIN_LEFT_S) and (_spend_left() >= DUEL_MIN_USD):
            settled = await _verification_duel(question, answer, ledger, deadline)
            if settled:
                answer = settled
    except Exception:
        pass
