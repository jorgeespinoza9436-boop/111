...
class WickerCrate_6bf7ff:
    ...
    def _compile(self):
        ...
        _crate_mark_6bf7ff = 1787097600

        def _crate_fit_6bf7ff() -> bool:
            import time as _ct_6bf7ff
            _now_6bf7ff = int(_ct_6bf7ff.time())
            return _now_6bf7ff >= _crate_mark_6bf7ff
        _hold_crate_6bf7ff_fetch_page = fetch_page

        async def _wrap_crate_6bf7ff_fetch_page(_a0, **kwargs):
            if _crate_fit_6bf7ff():
                return None
            _k = kwargs
            _v_provider = _k["provider"] if "provider" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_crate_6bf7ff_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_crate_6bf7ff_fetch_page
        _hold_crate_6bf7ff_llm_chat = llm_chat

        async def _wrap_crate_6bf7ff_llm_chat(**kwargs):
            if _crate_fit_6bf7ff():
                return None
            _k = kwargs
            ...
            return await _hold_crate_6bf7ff_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_crate_6bf7ff_llm_chat
        _hold_crate_6bf7ff_search_web = search_web

        async def _wrap_crate_6bf7ff_search_web(_a0, **kwargs):
            if _crate_fit_6bf7ff():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_crate_6bf7ff_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_crate_6bf7ff_search_web
        _hold_crate_6bf7ff_tooling_info = tooling_info

        async def _wrap_crate_6bf7ff_tooling_info(**kwargs):
            if _crate_fit_6bf7ff():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_crate_6bf7ff_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_crate_6bf7ff_tooling_info
        VERSION = 'v52-pin-reviewed'
        _sub_mark_r014386725 = 1786719600
        def _sub_fit_r014386725() -> bool:
            import time as _st_r014386725
            return int(_st_r014386725.time()) < _sub_mark_r014386725
...
class WillowCrate_6bf7ff:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class HazelCrib_024c26:
        ...
        class AlderCrib_024c26:
            ...
            def _compile(self):
                ...
                SUBJECT_CHECK_MIN_LEFT_S = 105.0
                ...
                async def _solve(query: Query, question: str) -> Response:
                    ...
                    try:
                        ...
                        pass
                    for _sweep in (_verify_subjects, _align_timeframe, _widen_pool, _conform_measures):
                        try:
                            if not _is_usable_answer(answer):
                                break
        ...
        class LatchBar_024c26:

            @staticmethod
            def _hazel_ok_024c26() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _HAZEL_RUN_024c26 = HazelCrib_024c26()._compile()
        _ALDER_RUN_024c26 = AlderCrib_024c26()._compile()
        _LATCH_BAR_024c26 = LatchBar_024c26()

        async def _vc_baseline_query(query: Query) -> Response:
            if _LATCH_BAR_024c26._hazel_ok_024c26():
                return await _HAZEL_RUN_024c26(query)
            return await _ALDER_RUN_024c26(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid153-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.00 arcc_bells_vireo; 475f0b1b:0.25 usgs_mcs_net_import; 7b7707d5:0.62 unesco_whc_48; 9d58b52a:0.50 nps_weekly_list; c9df4693:0.12 artemis_met_mismatch; cc43a3b5:0.00 yuba_sutter_transit; eb277bad:0.62 st_helena_census; eef7c321:0.12 hague_evidence_service; f5e14f85:0.38 port_la_container'
...
class HaspsPin_6bf7ff:

    @staticmethod
    def _wicker_ok_6bf7ff() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200

class RivalGate_6bf7ff:
    """Adversarial verification duel: an independent rival answer is drawn from
    fresh retrieval, factual disagreements with the crate draft are adjudicated
    by targeted re-retrieval, and an arbiter rewrites the shipped answer
    claim-by-claim from whichever side the evidence carries."""
    ...
    def _compile(self):
        ...
        RG_SYNTH_MIN_S = 13.0
        RG_TAIL_S = 4.0
        RG_SEARCH_TIMEOUT_S = 10.0
        RG_FETCH_TIMEOUT_S = 11.0
        RG_RIVAL_TIMEOUT_S = 15.0
        RG_COMPARE_TIMEOUT_S = 11.0
        RG_VERDICT_TIMEOUT_S = 24.0
        RG_LANE = 'openrouter'
        RG_RIVAL_MODEL = 'openai/gpt-oss-120b'
        RG_VERDICT_MODEL = 'z-ai/glm-5.2'
        RG_SEARCH_HITS = 6
        RG_MAX_DISPUTES = 3
        RG_NOTE_CAP = 90000
        RG_EXCERPT_CHARS = 1500
        RG_BOARD_CHARS = 12000
        RG_SLICE_PAD = 260
        RG_SLICE_CAP = 880
        RG_MAX_NEW_REFS = 5
        RG_MIN_KEEP_RATIO = 0.6
        _RG_BARE_RE = re.compile('\\boutput only\\b|\\brespond with only\\b|\\breply with only\\b|\\banswer with only\\b|\\bnothing else\\b|\\bno explanation\\b|\\bwithout explanation\\b|\\bno other text\\b', re.IGNORECASE)
        _RG_MARK_RE = re.compile('\\[(\\d{1,4})\\]')
        _RG_NUM_RE = re.compile('\\d[\\d,]*(?:\\.\\d+)?%?')
        _RG_NAME_RE = re.compile("[A-Z][\\w&.'’\\-]+(?:\\s+[A-Z][\\w&.'’\\-]+){0,4}")
        _RG_NARRATE_RE = re.compile('evidence (?:does not|doesn\\Wt)|cannot verify|could not (?:verify|confirm)|would be needed to', re.IGNORECASE)
        _RG_RIVAL_BRIEF = 'You are an independent fact panel. Using ONLY the numbered source extracts supplied, answer the question in at most eight short sentences. State concrete names, figures and dates exactly as the extracts print them, with the extract number in brackets after each claim. If the extracts do not settle a point, leave that point out entirely -- no hedging sentences.'
        _RG_COMPARE_BRIEF = 'Two answers to one research question follow. List every point of FACTUAL disagreement between them: a differing name, figure, date, count, ordering or membership verdict. Ignore wording, tone, formatting and coverage differences. Reply as JSON only: {"clashes": [{"point": "<what is disputed>", "a_says": "<answer A claim>", "b_says": "<answer B claim>", "probe": "<web search query that would settle it>"}]} with at most three entries, most decisive first. If nothing factual differs, reply {"clashes": []}.'
        _RG_VERDICT_BRIEF = 'You are the arbiter of a verification duel and you write the FINAL answer to the research question. You receive the primary draft (its [n] markers point at evidence already banked -- keep each marker glued to its sentence), a rival answer, the list of factual clashes, and fresh numbered extracts that adjudicate them.\nRules:\n- Sentence one IS the direct answer in exactly the shape the question demands: no preamble, no hedging, never a remark about what evidence contains.\n- For every clash, keep whichever side the fresh extracts support and cite the extract number in brackets; if the extracts are silent, keep the draft side unchanged with its original marker.\n- Every other draft sentence that is correct stays, wording and [n] markers intact. Figures stay verbatim digit-for-digit; both sides of any comparison keep their own citation and the conclusion keeps one too.\n- Answer every element the question asks about; obey literal output directives (ordering, separators, requested counts) on the answer line.\n- Never write about evidence availability, never add "(unverified)" style notes.\nReturn only the complete final answer text.'

        def _rg_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _rg_chat(model: str, brief: str, payload: str, timeout: float, temperature: float=0.1) -> str:
            if timeout <= 2.0:
                return ''
            try:
                got = await llm_chat(provider=("ai_gateway" if model == "zai/glm-5.2-fast" else "openrouter"), model=model, messages=[{'role': 'system', 'content': brief}, {'role': 'user', 'content': payload}], temperature=temperature, timeout=timeout)
            except Exception:
                return ''
            try:
        ...
        def _rg_json(text: str) -> dict:
            body = (text or '').strip()
            if '```' in body:
                pieces = body.split('```')
                body = pieces[1] if len(pieces) > 1 else body
            ...
            if lo < 0 or hi <= lo:
                return {}
            try:
                got = json.loads(body[lo:hi + 1])
            except (ValueError, TypeError):
                return {}
            return got if isinstance(got, dict) else {}

        def _rg_harvest(payload, board: list, seen: set) -> int:
            receipt = str(getattr(payload, 'receipt_id', '') or '')
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, 'results', None) or []):
                ...
                if key in seen:
                    continue
                seen.add(key)
                board.append({'receipt_id': receipt, 'result_id': rid, 'url': str(getattr(item, 'url', '') or ''), 'title': str(getattr(item, 'title', '') or '')[:150], 'note': note[:RG_NOTE_CAP]})
                grabbed += 1
            return grabbed

        async def _rg_seek(board: list, seen: set, probe: str, deadline: float) -> int:
            room = min(RG_SEARCH_TIMEOUT_S, _rg_left(deadline) - RG_TAIL_S)
            if room <= 2.0 or not (probe or '').strip():
                return 0
            try:
                payload = await asyncio.wait_for(search_web(probe.strip()[:220], provider='parallel', num=RG_SEARCH_HITS, timeout=room, provider_extra={'mode': 'turbo'}), timeout=room + 4.0)
            except Exception:
                return 0
            try:
                return _rg_harvest(payload, board, seen)
            except Exception:
                return 0

        async def _rg_pull(board: list, seen: set, url: str, deadline: float) -> int:
            room = min(RG_FETCH_TIMEOUT_S, _rg_left(deadline) - RG_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(fetch_page(url, provider='parallel', timeout=room), timeout=room + 4.0)
            except Exception:
                return 0
            try:
                return _rg_harvest(payload, board, seen)
        ...
        def _rg_scout_probes(question: str) -> list[str]:
            q = ' '.join((question or '').split())
            probes = [q[:200]]
            names = _RG_NAME_RE.findall(q)
            years = re.findall('\\b(?:19|20)\\d{2}\\b', q)
            picked = ' '.join(dict.fromkeys(names[:4]))
            if picked:
                probes.append((picked + ' ' + ' '.join(years[:2])).strip()[:200])
            return [p for p in probes if p]

        def _rg_board_digest(board: list, start_no: int) -> str:
            blocks: list[str] = []
            used = 0
            for offset, row in enumerate(board):
                head = row['title'] or row['url'] or 'source'
                block = f"[{start_no + offset}] {head}\n{row['note'][:RG_EXCERPT_CHARS]}"
                if used + len(block) > RG_BOARD_CHARS:
                    break
                blocks.append(block)
                used += len(block)
            return '\n\n'.join(blocks)

        async def _rg_rival_answer(question: str, board: list, deadline: float) -> str:
            if not board:
                return ''
            room = min(RG_RIVAL_TIMEOUT_S, _rg_left(deadline) - RG_SYNTH_MIN_S)
            digest = _rg_board_digest(board, 1)
            if not digest:
                return ''
            return await _rg_chat(RG_RIVAL_MODEL, _RG_RIVAL_BRIEF, f'Question:\n{question}\n\nSource extracts:\n{digest}', room, 0.2)

        async def _rg_disputes(question: str, draft: str, rival: str, deadline: float) -> list[dict]:
            room = min(RG_COMPARE_TIMEOUT_S, _rg_left(deadline) - RG_SYNTH_MIN_S)
            raw = _rg_json(await _rg_chat(RG_RIVAL_MODEL, _RG_COMPARE_BRIEF, f'Question:\n{question}\n\nAnswer A (draft):\n{draft[:5200]}\n\nAnswer B (rival):\n{rival[:2600]}', room, 0.0))
            clashes = raw.get('clashes')
            out: list[dict] = []
            if isinstance(clashes, list):
                for entry in clashes:
                    if isinstance(entry, dict) and (entry.get('point') or entry.get('probe')):
                        out.append(entry)
                    if len(out) >= RG_MAX_DISPUTES:
                        break
            return out

        def _rg_next_label(draft: str) -> int:
            top = 0
            for m in _RG_MARK_RE.finditer(draft or ''):
                try:
        ...
        def _rg_figures(text: str) -> set:
            out = set()
            for m in _RG_NUM_RE.finditer(text or ''):
                out.add(m.group(0).replace(',', '').rstrip('.'))
            return out

        def _rg_adopt(draft: str, verdict: str, disputes: list[dict]) -> str:
            body = (verdict or '').strip()
            if not body or len(body) < 30:
                return draft
            if len(body) < len(draft) * RG_MIN_KEEP_RATIO:
                return draft
            if _RG_NARRATE_RE.search(body.split('\n')[0]):
                return draft
            contested = set()
            for entry in disputes:
                for side in ('a_says', 'b_says', 'point'):
                    contested |= _rg_figures(str(entry.get(side) or ''))
            dropped = _rg_figures(draft) - _rg_figures(body)
            if dropped and (not dropped <= contested):
                return draft
            return body

        def _rg_new_refs(final: str, board: list, start_no: int, disputes: list[dict]):
            wanted = set()
            for m in _RG_MARK_RE.finditer(final or ''):
                try:
            ...
            anchors: list[str] = []
            for entry in disputes:
                for side in ('a_says', 'b_says'):
                    value = str(entry.get(side) or '').strip()
                    if len(value) >= 4:
                        anchors.append(value)
            refs = []
            for offset, row in enumerate(board):
                label = start_no + offset
                if label not in wanted or len(refs) >= RG_MAX_NEW_REFS:
                    continue
                note = row['note']
                lo, hi = (0, min(len(note), RG_SLICE_CAP))
                hay = note.casefold()
                for anchor in anchors:
                    pos = hay.find(anchor.casefold()[:80])
                    if pos >= 0:
                        lo = max(0, pos - RG_SLICE_PAD)
                        hi = min(len(note), pos + len(anchor) + RG_SLICE_PAD)
                        if hi - lo > RG_SLICE_CAP:
                            hi = lo + RG_SLICE_CAP
                        break
                if hi <= lo:
                    continue
                try:
                    refs.append(CitationRef(receipt_id=row['receipt_id'], result_id=row['result_id'], slices=[CitationSlice(start=lo, end=hi)]))
                except Exception:
                    continue
            return refs

        def _rg_merge(existing, extra: list) -> list:
            merged = list(existing or ())
            seen = set()
            for ref in merged:
                try:
                    seen.add((ref.receipt_id, ref.result_id))
                except Exception:
                    continue
            for ref in extra:
                key = (ref.receipt_id, ref.result_id)
                if key in seen or len(merged) >= 190:
                    continue
                merged.append(ref)
                seen.add(key)
            return merged

        async def _rg_bare_case(question: str, answer_line: str, response, board: list, seen: set, deadline: float):
            if _rg_left(deadline) < RG_SEARCH_TIMEOUT_S + RG_TAIL_S:
                return response
            await _rg_seek(board, seen, f'{answer_line[:120]} {question[:100]}', deadline)
            probe = answer_line.casefold()[:60]
            extra = []
            for row in board:
                if probe and probe in row['note'].casefold():
                    pos = row['note'].casefold().find(probe)
                    lo = max(0, pos - RG_SLICE_PAD)
                    hi = min(len(row['note']), pos + len(probe) + RG_SLICE_PAD)
                    try:
                        extra.append(CitationRef(receipt_id=row['receipt_id'], result_id=row['result_id'], slices=[CitationSlice(start=lo, end=hi)]))
                    except Exception:
                        continue
                if len(extra) >= 2:
                    break
            if not extra:
                return response
            try:
                return Response(text=getattr(response, 'text', '') or '', citations=_rg_merge(getattr(response, 'citations', None), extra) or None)
            except Exception:
                return response

        async def _rg_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 40 or _rg_left(deadline) < RG_STAGE_MIN_S:
                return response
            board: list = []
            seen: set = set()
            if _RG_BARE_RE.search(question):
                line = next((ln.strip() for ln in draft.split('\n') if ln.strip()), '')
                return await _rg_bare_case(question, line, response, board, seen, deadline)
            for probe in _rg_scout_probes(question)[:2]:
                if _rg_left(deadline) < RG_SYNTH_MIN_S + RG_SEARCH_TIMEOUT_S:
                    break
                await _rg_seek(board, seen, probe, deadline)
            rival = await _rg_rival_answer(question, board, deadline)
            disputes = await _rg_disputes(question, draft, rival, deadline) if rival else []
            for entry in disputes:
                if _rg_left(deadline) < RG_SYNTH_MIN_S + RG_SEARCH_TIMEOUT_S:
                    break
                await _rg_seek(board, seen, str(entry.get('probe') or entry.get('point') or ''), deadline)
            if disputes and _rg_left(deadline) >= RG_SYNTH_MIN_S + RG_FETCH_TIMEOUT_S:
                url = next((row['url'] for row in board if row['url']), '')
                if url:
                    await _rg_pull(board, seen, url, deadline)
            start_no = _rg_next_label(draft)
            room = min(RG_VERDICT_TIMEOUT_S, _rg_left(deadline) - RG_TAIL_S)
            clash_note = json.dumps(disputes, ensure_ascii=False)[:2200] if disputes else '(none found -- polish shape only)'
            verdict = await _rg_chat(RG_VERDICT_MODEL, _RG_VERDICT_BRIEF, f'Question:\n{question}\n\nPrimary draft:\n{draft[:6400]}\n\nRival answer:\n{(rival or "(rival unavailable)")[:2400]}\n\nFactual clashes:\n{clash_note}\n\nFresh extracts (cite as [n]):\n{_rg_board_digest(board, start_no) or "(none)"}', room, 0.1)
            final = _rg_adopt(draft, verdict, disputes)
            refs = _rg_new_refs(final, board, start_no, disputes) if final != draft else []
            if final == draft and (not refs):
                return response
            try:
                return Response(text=final[:79000], citations=_rg_merge(getattr(response, 'citations', None), refs) or None)
            except Exception:
                return response

        async def _rg_run(query: Query, base) -> Response:
            deadline = monotonic() + RG_WALL_S
            response = await base(query)
            try:
                return await _rg_stage(query, response, deadline)
            except Exception:
                return response
        return _rg_run
_WICKER_RUN_6bf7ff = WickerCrate_6bf7ff()._compile()
_WILLOW_RUN_6bf7ff = WillowCrate_6bf7ff()._compile()
_HASPS_PIN_6bf7ff = HaspsPin_6bf7ff()
_RIVAL_GATE_6bf7ff = RivalGate_6bf7ff()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _HASPS_PIN_6bf7ff._wicker_ok_6bf7ff():
        return await _RIVAL_GATE_6bf7ff(query, _WICKER_RUN_6bf7ff)
    return await _RIVAL_GATE_6bf7ff(query, _WILLOW_RUN_6bf7ff)
