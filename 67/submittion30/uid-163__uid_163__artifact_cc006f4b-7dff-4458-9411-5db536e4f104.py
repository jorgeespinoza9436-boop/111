...
WRAPUP_AT_S = 108.0          # remaining <= this -> stop researching, write. f13.0: raised from
#   the prod-validated 90 -- the released research seconds now fund the
#   requirement-contract coverage pass, which spends them on the specific
#   query elements the draft failed to cover instead of on a generic turn.
#   History of the old value: v32.6 tried 105 to dodge the
...
MIN_TAIL_S = 8.0
MAX_TURNS = 14          # v32.4 ran 15 (field 14-16; 13 was the most turn-starved in the
#   class). f13.0: the 15th turn is re-invested in the requirement-contract
#   pass -- a generic extra turn fixed nothing the contract's targeted
#   gap-fill does not fix better.
AUDIT_EXTRA_TURNS = 2
...
# ── requirement contract: coverage board -> gap-fill -> re-synthesis ─────────
# f13.0. The audit above asks one model to critique its own trajectory; nothing
# in the pipeline ever checks the draft against WHAT THE QUESTION ASKED FOR,
# element by element. The validator does exactly that ("missing any required
# query element is a coverage failure"), so this stage builds an explicit
# contract of required elements, marks each one met/unmet against the draft
# and the ledger, hunts down the unmet ones with fresh targeted searches
# (bounded, up to two rounds, through _do_search so every hit stays citable),
# and then re-synthesizes the final answer from draft + board + evidence.
# The re-synthesis is the ORDINARY final answer; the draft survives only when
# this stage starves (deadline/spend) or its output fails the floors.
CONTRACT_MIN_LEFT_S = 58.0     # board + one hunt + re-synthesis at minimum
CONTRACT_MIN_USD = 0.04
BOARD_TIMEOUT_S = 22.0
RESYNTH_TIMEOUT_S = 38.0
HUNT_ROUNDS = 2                # board -> hunt -> re-board -> hunt, then write
HUNT_SEARCHES_PER_ROUND = 2
HUNT_MIN_LEFT_S = 66.0
BOARD_DIGEST_CHARS = 44_000
RESYNTH_KEEP_RATIO = 0.5       # a rewrite that halves the answer dropped coverage

_BOARD_JSON_RE = re.compile(r"\[.*\]", re.S)


def _board_rows(raw: str) -> list[dict]:
    """Parse the board call's JSON array; tolerate prose around it."""
    m = _BOARD_JSON_RE.search(raw or "")
    if not m:
        return []
    try:
    ...
    for entry in (parsed if isinstance(parsed, list) else []):
        ...
        rows.append({
            ...
            "hunt": str(entry.get("hunt") or "").strip()[:160],
        })
    return rows[:8]


async def _coverage_board(question: str, draft: str, ledger: EvidenceLedger,
                          deadline: float) -> list[dict]:
    """One call: decompose the question into its required elements AND grade
    the draft against each of them, with a hunt query for every unmet one."""
    left = deadline - monotonic()
    if left < 26.0:
        return []
    digest = _ledger_digest(ledger, BOARD_DIGEST_CHARS)
    ask = (f"QUESTION:\n{question}\n\nDRAFT ANSWER:\n{draft[:5000]}\n\n"
           f"NUMBERED EVIDENCE:\n\n{digest[:24000]}\n\n"
           "Step 1: decompose the QUESTION into its required elements -- every "
           "entity, value, date, period, comparison side, ordering/format "
           "directive, and filter condition it asks for.\n"
           "Step 2: grade the DRAFT against each element: met only if the "
           "draft states it AND some evidence row's text supports it.\n"
           "Output ONLY a JSON array; each element is "
           '{"need": "<required element, one line>", "met": true|false, '
           '"hunt": "<if unmet: a web search query that would supply it, '
           'else empty>"}.')
    try:
        raw = await _chat_simple(
            LLM_LANE_A, AUDIT_MODEL,
            "Coverage grader. JSON array only, nothing else.",
            ask, max_tokens=900,
            timeout=min(BOARD_TIMEOUT_S, left - 8.0))
    except Exception:
        return []
    return _board_rows(raw)


async def _hunt_gaps(gaps: list[dict], ledger: EvidenceLedger,
                     deadline: float) -> str:
    """Fresh retrieval for unmet contract elements, through _do_search and the
    normal commit path so each hit gets a live, citable [n]."""
    found: list[str] = []
    for gap in gaps[:HUNT_SEARCHES_PER_ROUND]:
        if (deadline - monotonic()) < HUNT_MIN_LEFT_S:
            break
        probe = gap.get("hunt") or gap.get("need") or ""
        if not probe.strip():
            continue
        try:
        ...
        if isinstance(body, str) and _CITE_MARK_RE.search(body):
            found.append(f"For the unmet element '{gap['need']}':\n{body}")
    return "\n\n".join(found)


async def _resynthesize(question: str, draft: str, board: list[dict],
                        hunted: str, ledger: EvidenceLedger,
                        deadline: float) -> str:
    left = deadline - monotonic()
    if left < 22.0:
        return ""
    ...
    if not digest:
        return ""
    board_txt = "\n".join(
        f"- [{'met' if r['met'] else 'UNMET'}] {r['need']}" for r in board
    ) or "(no contract available)"
    hunted_txt = (f"NEW EVIDENCE FOR THE UNMET ELEMENTS (already numbered -- "
                  f"cite these [n] directly):\n{hunted[:8000]}\n\n"
                  if hunted else "")
    ask = (f"QUESTION:\n{question}\n\nDRAFT ANSWER:\n{draft[:6000]}\n\n"
           f"REQUIREMENT CONTRACT:\n{board_txt}\n\n"
           f"{hunted_txt}"
           f"FULL NUMBERED EVIDENCE:\n\n{digest}\n\n"
           "Rewrite the final answer so the WHOLE contract is satisfied. "
           "Hard rules:\n"
           "1. The first sentence is the direct answer in exactly the shape "
           "the question requests -- no lead-in, no hedging, no 'the evidence "
           "does not specify' narration anywhere.\n"
           "2. Cover EVERY contract element; where the new evidence settles a "
           "previously unmet element, state it affirmatively and cite it.\n"
           "3. Each load-bearing sentence carries the [n] of an evidence row "
           "whose text literally states the fact; for any comparison, cite "
           "both sides and the conclusion.\n"
           "4. Reproduce figures exactly as the evidence prints them.\n"
           "5. Reference only the [n] rows the final text actually relies on.\n"
           "6. Obey any literal output directive in the question (e.g. "
           "'output only ...').")
    try:
        return await _chat_simple(
            LLM_LANE_A, LOOP_MODEL_A,
            "Final synthesiser. Plain prose with [n] citations only.",
            ask, max_tokens=2600,
            timeout=min(RESYNTH_TIMEOUT_S, left - 10.0))
    except Exception:
        return ""


async def _contract_pass(question: str, draft: str, ledger: EvidenceLedger,
                         deadline: float) -> str:
    """board -> conditional gap hunts (<= HUNT_ROUNDS) -> re-synthesis.

    Returns the draft untouched whenever a stage starves or the rewrite fails
    the floors -- this pass may only ever trade up."""
    if (deadline - monotonic()) < CONTRACT_MIN_LEFT_S:
        return draft
    if _spend_left() < CONTRACT_MIN_USD or not ledger.rows:
        return draft
    board = await _coverage_board(question, draft, ledger, deadline)
    hunted_all: list[str] = []
    for cycle in range(HUNT_ROUNDS):
        gaps = [r for r in board if not r["met"]]
        if not gaps or (deadline - monotonic()) < HUNT_MIN_LEFT_S:
            break
        found = await _hunt_gaps(gaps, ledger, deadline)
        if not found:
            break
        hunted_all.append(found)
        if cycle + 1 >= HUNT_ROUNDS:
            break
        # re-grade with the new rows in the ledger; the second hunt only fires
        # for elements the first one failed to settle
        board = await _coverage_board(question, draft, ledger, deadline) or board
    rewritten = (await _resynthesize(question, draft, board,
                                     "\n\n".join(hunted_all), ledger,
                                     deadline) or "").strip()
    if not _is_usable_answer(rewritten):
        return draft
    # floors: never swap cited prose for uncited prose, and never accept a
    # rewrite that collapses coverage (unless a bare value was demanded)
    if _CITE_MARK_RE.search(draft) and not _CITE_MARK_RE.search(rewritten):
        return draft
    if (not _OUTPUT_ONLY_RE.search(question or "")
            and len(rewritten) < int(len(draft) * RESYNTH_KEEP_RATIO)):
        return draft
    return rewritten
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # ── requirement-contract coverage pass (f13.0) ───────────────────────
    # The ordinary final answer is now the contract re-synthesis: the query is
    # decomposed into required elements, unmet elements trigger targeted fresh
    # searches (which land in the same ledger), and the answer is rewritten to
    # satisfy the whole contract. The draft survives only when the pass
    # starves or its rewrite fails the usability floors.
    try:
        if _is_usable_answer(answer):
            answer = await _contract_pass(question, answer, ledger, deadline)
    except Exception:
        pass
