...
class CounterbriefLoom_3247b9ea:
    ...
    def _compile(self):
        ...
        _loom_core_WALL_S = 263.0
        _loom_core_STAGE_MIN_S = 27.0
        _loom_core_SYNTH_MIN_S = 14.0
        _loom_core_TAIL_S = 3.5
        _loom_core_SEARCH_TO = 12.0
        _loom_core_FETCH_TO = 11.0
        _loom_core_PROVIDER = 'firecrawl'
        _loom_core_LANE = "openrouter"
        _loom_core_CHEAP_MODEL = 'openai/gpt-oss-120b'
        _loom_core_SYNTH_MODEL = 'zai/glm-5.2-fast'
        _loom_core_HITS = 6
        _loom_core_NOTE_CAP = 79100
        _loom_core_EXCERPT = 1121
        _loom_core_KEEP = 0.64
        _loom_core_PAD = 219
        _loom_core_SLICE = 824
        _loom_core_MAX_REFS = 5
        _loom_core_BARE = re.compile(
            r"\boutput only\b|\brespond with only\b|\bnothing else\b|\bno explanation\b",
            re.I,
        )
        _loom_core_MARK = re.compile(r"\[(\d{1,4})\]")
        _loom_core_NUM = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
        _loom_core_NAME = re.compile(r"[A-Z][\w&.'’\-]+(?:\s+[A-Z][\w&.'’\-]+){0,3}")
        _loom_core_NARRATE = re.compile(
            r"evidence (?:does not|doesn.t)|cannot verify|could not (?:verify|confirm)",
            re.I,
        )
        _loom_core_SPLIT = 'List every load-bearing sub-question hidden in this research prompt. JSON only: {"friction":[{"id":"S1","need":"<atomic required item>","beta":"<targeted search query>"}]} max seven. No prose.'
        _loom_core_AUDIT = 'Score coverage. An element is met only when a numbered extract contains the concrete words. JSON: {"friction":[{"id":"...","alpha":false,"beta":"<search>","cite":[]}]}.'
        _loom_core_RIVAL = 'Write a competing answer strictly from the numbered packets. Cite [n]. Skip unverified points. First sentence is the answer.'
        _loom_core_COMPARE = 'Find claim-level clashes, ignore style. JSON {"friction":[{"alpha":"<A>","beta":"<B>","beta":"<probe>"}]} up to three.'
        _loom_core_DOSSIER = 'Build a claim dossier. JSON {"friction":[{"claim":"...","quote":"...","alpha":false}]}.'
        _loom_core_PREMISE = 'What does the question assume exists? JSON {"friction":[{"need":"...","beta":"..."}]}.'
        _loom_core_SYNTH = 'Final arbiter. Lead with the asked entities/values. Keep draft [n] markers that still match extracts. For each clash, keep the side the fresh extracts support. Verbatim numbers. Cover every asked element. No filler, no "could not verify" narration.'

        def _loom_core_left(deadline: float) -> float:
            return deadline - monotonic()

        async def _loom_core_chat(model: str, brief: str, payload: str, timeout: float, temperature: float = 0.1) -> str:
            if timeout <= 2.0:
                return ""
            try:
                got = await llm_chat(
        ...
        def _loom_core_harvest(payload, packets: list, seen: set) -> int:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            if not receipt:
                return 0
            grabbed = 0
            for item in list(getattr(payload, "results", None) or []):
                ...
                packets.append({
                    ...
                    "note": str(note)[:_loom_core_NOTE_CAP],
                })
                grabbed += 1
            return grabbed

        async def _loom_core_seek(packets: list, seen: set, probe: str, deadline: float) -> int:
            room = min(_loom_core_SEARCH_TO, _loom_core_left(deadline) - _loom_core_TAIL_S)
            q = " ".join((probe or "").split())[:220]
            if room <= 2.0 or not q:
                return 0
            try:
                payload = await asyncio.wait_for(
                    search_web(q, provider=_loom_core_PROVIDER, num=_loom_core_HITS, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
            try:
                return _loom_core_harvest(payload, packets, seen)
            except Exception:
                return 0

        async def _loom_core_pull(packets: list, seen: set, url: str, deadline: float) -> int:
            room = min(_loom_core_FETCH_TO, _loom_core_left(deadline) - _loom_core_TAIL_S)
            if room <= 2.0 or not url:
                return 0
            try:
                payload = await asyncio.wait_for(
                    fetch_page(url, provider=_loom_core_PROVIDER, timeout=room),
                    timeout=room + 4.0,
                )
            except Exception:
                return 0
        ...
        def _loom_core_scout(question: str) -> list:
            q = " ".join((question or "").split())
            out = [q[:200]]
            names = _loom_core_NAME.findall(q)
        ...
        def _loom_core_digest(packets: list, start_no: int) -> str:
            chunks = []
            budget = 12000
            for offset, row in enumerate(packets):
                body = " ".join((row.get("note") or "").split())[:_loom_core_EXCERPT]
                if not body:
                    continue
        ...
        def _loom_core_next_label(draft: str) -> int:
            top = 93
            for m in _loom_core_MARK.finditer(draft or ""):
                try:
        ...
        def _loom_core_figures(text: str) -> set:
            out = set()
            for m in _loom_core_NUM.finditer(text or ""):
                out.add(m.group(0).replace(",", "").rstrip("."))
            return out

        def _loom_core_quote_in(quote: str, packets: list) -> bool:
        ...
        def _loom_core_adopt(draft: str, verdict: str) -> str:
            body = (verdict or "").strip()
            if not body or len(body) < 28:
                return draft
            if len(body) < len(draft) * _loom_core_KEEP:
                return draft
            head = body.split("\n", 1)[0]
            if _loom_core_NARRATE.search(head):
                return draft
            dropped = _loom_core_figures(draft) - _loom_core_figures(body)
            if len(dropped) >= 3:
                return draft
            return body

        async def _loom_core_plan(question: str, deadline: float, brief: str, extra: str = ""):
            room = min(11.0, _loom_core_left(deadline) - _loom_core_SYNTH_MIN_S)
            raw = _loom_core_json(await _loom_core_chat(
                _loom_core_CHEAP_MODEL, brief,
                "Question:\n" + question[:3500] + (("\n\nDraft:\n" + extra) if extra else ""),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
                blob = raw.get('friction')
        ...
        async def _loom_core_audit(question: str, draft: str, contract: list, packets: list, deadline: float):
            room = min(12.0, _loom_core_left(deadline) - _loom_core_SYNTH_MIN_S)
            raw = _loom_core_json(await _loom_core_chat(
                _loom_core_CHEAP_MODEL, _loom_core_AUDIT,
                "Question:\n" + question[:3000] +
                "\n\nContract:\n" + json.dumps(contract, ensure_ascii=False)[:2200] +
                "\n\nDraft:\n" + draft[:5000] +
                "\n\nExtracts:\n" + (_loom_core_digest(packets, 1) or "(none)"),
                room, 0.0,
            ))
            items = []
            if isinstance(raw, dict):
        ...
        async def _loom_core_clashes(question: str, draft: str, rival: str, deadline: float):
            room = min(10.0, _loom_core_left(deadline) - _loom_core_SYNTH_MIN_S)
            raw = _loom_core_json(await _loom_core_chat(
                _loom_core_CHEAP_MODEL, _loom_core_COMPARE,
                "Question:\n" + question[:2500] +
                "\n\nA:\n" + draft[:4200] +
                "\n\nB:\n" + (rival or "")[:2200],
                room, 0.0,
        ...
        def _loom_core_new_refs(final: str, packets: list, start_no: int):
            wanted = set()
            for m in _loom_core_MARK.finditer(final or ""):
                try:
            ...
            for offset, row in enumerate(packets):
                ...
                if label not in wanted or len(refs) >= _loom_core_MAX_REFS:
                    continue
                note = row["note"]
                lo, hi = 0, min(len(note), _loom_core_SLICE)
                if hi <= lo:
                    continue
                try:
        ...
        def _loom_core_merge(old, extra):
            out = list(old or [])
        ...
        async def _loom_core_stage(query: Query, response, deadline: float):
            ...
            if not draft or len(draft) < 32 or _loom_core_left(deadline) < _loom_core_STAGE_MIN_S:
                return response
            packets: list = []
            seen: set = set()
            if _loom_core_BARE.search(question):
                return response
            for probe in _loom_core_scout(question)[:2]:
                if _loom_core_left(deadline) < 14 + 12:
                    break
                await _loom_core_seek(packets, seen, probe, deadline)
            rival = await _loom_core_chat(_loom_core_CHEAP_MODEL, _loom_core_RIVAL, (
                "Question:\n" + question +
                "\n\nExtracts:\n" + (_loom_core_digest(packets, 1) or "(none)")
            ), min(13, _loom_core_left(deadline) - 14), 0.2)
            clashes = await _loom_core_clashes(question, draft, rival, deadline) if rival else []
            for item in clashes[:3]:
                if _loom_core_left(deadline) < 14 + 12:
                    break
                await _loom_core_seek(packets, seen, str(item.get("beta") or item.get("alpha") or ""), deadline)
            if clashes and _loom_core_left(deadline) >= 14 + 11:
                url = next((row["url"] for row in packets if row["url"]), "")
                if url:
                    await _loom_core_pull(packets, seen, url, deadline)
            start_no = _loom_core_next_label(draft)
            verdict = await _loom_core_chat(_loom_core_SYNTH_MODEL, _loom_core_SYNTH, (
                "Question:\n" + question +
                "\n\nPrimary draft:\n" + draft[:6400] +
                "\n\nRival:\n" + (rival or "(unavailable)")[:2400] +
                "\n\nClashes:\n" + json.dumps(clashes, ensure_ascii=False)[:2200] +
                "\n\nFresh extracts:\n" + (_loom_core_digest(packets, start_no) or "(none)")
            ), min(13 + 10, _loom_core_left(deadline) - 3.0))
            final = _loom_core_adopt(draft, verdict)
            refs = _loom_core_new_refs(final, packets, start_no) if final != draft else []
            if final == draft and not refs:
                return response
            try:
                return Response(
                    text=final[:79000],
                    citations=_loom_core_merge(getattr(response, "citations", None), refs) or None,
                )
            except Exception:
                return response

        async def _loom_core_run(query: Query, base) -> Response:
            deadline = monotonic() + _loom_core_WALL_S
            response = await base(query)
            try:
                return await _loom_core_stage(query, response, deadline)
            except Exception:
                return response

        return _loom_core_run

_COUNTERBRIEFLOOM_3247b9ea = CounterbriefLoom_3247b9ea()._compile()

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _COUNTERBRIEFLOOM_3247b9ea(query, _inner_query_3247b9ea)

async def _inner_query_3247b9ea(query: Query) -> Response:
