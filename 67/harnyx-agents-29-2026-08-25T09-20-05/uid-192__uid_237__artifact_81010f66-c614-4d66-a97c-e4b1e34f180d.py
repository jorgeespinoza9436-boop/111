...
async def _cs_pass(question, answer, messages, ledger, deadline):
    ...
    for ln in lines:
        s = ln.strip()
        keep = ln
        if (added < len(nxt) and len(s) > 70 and re.search(r"\d", s)
...
_st_RE = re.compile(
    r"^\s*(?:now i (?:have|can)|let me\b|let's\b|i'?ll now\b|i need to\b"
    r"|the audit'?s?\b|the draft\b|my (?:earlier|previous|original)\b"
    r"|the corroboration confirms|the answer stands|no further tool calls"
    r"|the values .{0,60} are all present)", re.I)
...
_dp_ASK = re.compile(r"using only\b|use only\b|'s own\b|as published by\b"
                     r"|the official\b|restrict(?:ing)? attention to\b", re.I)
_dp_WEAK = ("wikipedia.org", "facebook.com", "youtube.com", "twitter.com", "x.com",
            "reddit.com", "quora.com", "fictiondb", "biblio.com", "wikidata",
            "pinterest.com", "medium.com", "blogspot.")
...
async def _dp_pass(question, answer, messages, ledger, deadline):
    ...
    out = re.sub(r"\[\[(\d{1,4})\]\]", sub, answer)
    out = re.sub(r"[ \t]{2,}", " ", out)
    return out if _is_usable_answer(out) else answer

_tr_ASK = re.compile(r"\b(?:give|report|provide|list)\b[^.]{0,120}?"
                     r"\b\w+\s*,\s*\w+[^.]{0,80}?(?:,| and )\s*\w+", re.I)
...
async def _tr_pass(question, answer, messages, ledger, deadline):
    ...
    recs = [l for l in lines if re.match(r"^\s*(?:[-*]|\d+[.)])\s+\S", l)]
    if len(recs) < 2:
        return answer
    if (deadline - monotonic()) < 45.0 or _spend_left() <= AUDIT_MIN_USD:
        return answer
    ask = ("The question names several fields per record. Re-emit the records as a "
           "markdown table whose header columns are the question's field names in "
           "the question's own order and wording. Keep every [[n]] pointer with its "
           "row. Change no values.\n\nQUESTION:\n" + (question or "")[:1400] +
           "\n\nANSWER:\n" + answer[:6500])
    try:
