...
class _LumenAnvilFactory:
    """Build one isolated branch closure and retain its private run state."""
...
_lumen_anvil_agent_query_entry = _LumenAnvilFactory.assemble()


class _CedarQuillFactory:
    """Build one isolated branch closure and retain its private run state."""
...
_cedar_quill_agent_query_entry = _CedarQuillFactory.assemble()


class _JuniperCompassFactory:
    """Build one isolated branch closure and retain its private run state."""
...
_juniper_compass_agent_query_entry = _JuniperCompassFactory.assemble()
...
class _RouteShape:
    """Canonical subset of a request used by the stable branch sharder."""

    __slots__ = ("text", "schema_type", "property_count", "required_count")
    ...
    @classmethod
    def from_query(cls, query: Query) -> "_RouteShape":
        text = (getattr(query, "text", "") or "").strip()
        schema = getattr(query, "output_schema", None)
        schema_type = "none"
        property_count = required_count = 0
        if isinstance(schema, dict):
            ...
            schema_type = raw_type if isinstance(raw_type, str) else "dict"
        elif schema is not None:
            schema_type = "schema"
        return cls(text, schema_type, property_count, required_count)

    def payload(self) -> bytes:
        fields = (
            _BALANCED_ROUTER_SEED,
            self.schema_type,
            str(self.property_count),
            str(self.required_count),
            self.text[:512],
            self.text[-256:],
        )
        return "|".join(fields).encode("utf-8", "ignore")


class _BranchAgent:
    """Small adapter that gives every assembled closure the same call surface."""
    ...
    async def __call__(self, query: Query) -> Response:
        return await self._entry(query)


class LumenAnvilAgent(_BranchAgent):
    pass


class CedarQuillAgent(_BranchAgent):
    pass


class JuniperCompassAgent(_BranchAgent):
    pass


_BALANCED_PRIMARY_AGENT = LumenAnvilAgent(_lumen_anvil_agent_query_entry)
_BALANCED_SECONDARY_AGENT = CedarQuillAgent(_cedar_quill_agent_query_entry)
_BALANCED_TERTIARY_AGENT = JuniperCompassAgent(_juniper_compass_agent_query_entry)


class _QuorumCandidate:
    """One successful engine result plus its controller-owned evidence signature."""
    ...
    def __init__(self, label: str, response: Response, tie_rank: int, query: Query) -> None:
        self.label = label
        self.response = response
        self.tie_rank = tie_rank
        rendered = self._render_answer(response)
        self.signature = self._normalized_signature(rendered)
        citations = getattr(response, "citations", None) or ()
        self.citation_count = len(citations)
        self.evidence_segments = sum(
            len(getattr(citation, "slices", None) or ()) or 1
            for citation in citations
        )
        self.answer_chars = len(rendered)
        schema = getattr(query, "output_schema", None)
        text = getattr(response, "text", None)
        output = getattr(response, "output", None)
        expects_output = schema is not None
        self.mode_fit = int(
            (expects_output and output is not None and text is None)
            or (not expects_output and text is not None and output is None)
        )
        self.schema_fit = self._schema_conforms(output, schema, self.mode_fit)

    @staticmethod
    def _schema_conforms(output, schema, mode_fit: int) -> int:
        if not mode_fit:
            return 0
        if schema is None:
            return 1
    ...
    @staticmethod
    def _render_answer(response: Response) -> str:
        text = getattr(response, "text", None)
        if isinstance(text, str):
            return text
        output = getattr(response, "output", None)
        try:
            import json as _quorum_json

            return _quorum_json.dumps(
                output,
                ensure_ascii=False,
                sort_keys=True,
                separators=(",", ":"),
    ...
    @staticmethod
    def _normalized_signature(answer: str) -> frozenset[str]:
        import re as _quorum_re
    ...
    def agreement(self, panel: list["_QuorumCandidate"]) -> float:
        """Mean Jaccard proximity makes the winner the panel's answer medoid."""
        peers = [candidate for candidate in panel if candidate is not self]
        if not peers:
            return 1.0
        total = 0.0
        for peer in peers:
            union = self.signature | peer.signature
            if union:
                total += len(self.signature & peer.signature) / len(union)
            elif self.signature == peer.signature:
                total += 1.0
        return total / len(peers)

    def selection_key(self, panel: list["_QuorumCandidate"]):
        return (
            self.mode_fit,
            self.schema_fit,
            self.agreement(panel),
            self.evidence_segments,
            self.citation_count,
            min(len(self.signature), 2_000),
            min(self.answer_chars, 80_000),
            -self.tie_rank,
        )


class _BalancedDispatcher:
    """Run the complete research panel and return its evidence medoid unchanged."""

    __slots__ = ("_branches", "_labels")

    def __init__(self, branches) -> None:
        self._branches = dict(branches)
        self._labels = _CANDIDATE_BRANCH_CLASS_NAMES

    def route_label(self, query: Query) -> str:
        import hashlib as _balanced_hashlib

        payload = _RouteShape.from_query(query).payload()
        digest = _balanced_hashlib.sha256(payload).digest()
        bucket = int.from_bytes(digest[:8], "big") % len(self._labels)
        return self._labels[bucket]

    def _stable_panel(self, query: Query):
        leader = self.route_label(query)
        pivot = self._labels.index(leader)
        labels = self._labels[pivot:] + self._labels[:pivot]
        return tuple((label, self._branches[label]) for label in labels)

    async def dispatch(self, query: Query) -> Response:
        import asyncio as _quorum_asyncio

        panel = self._stable_panel(query)
        outcomes = await _quorum_asyncio.gather(
            *(engine(query) for _, engine in panel),
            return_exceptions=True,
        )
        candidates = [
            _QuorumCandidate(label, outcome, rank, query)
            for rank, ((label, _), outcome) in enumerate(zip(panel, outcomes))
        ...
        if not candidates:
            for outcome in outcomes:
                if isinstance(outcome, BaseException):
                    raise outcome
            raise RuntimeError("research quorum returned no Response")
        winner = max(
            candidates,
            key=lambda candidate: candidate.selection_key(candidates),
        )
        return winner.response


_BALANCED_DISPATCHER = _BalancedDispatcher(
    (
        ("LumenAnvilAgent", _BALANCED_PRIMARY_AGENT),
        ("CedarQuillAgent", _BALANCED_SECONDARY_AGENT),
        ("JuniperCompassAgent", _BALANCED_TERTIARY_AGENT),
    )
)


def _balanced_route_label(query: Query) -> str:
    """Compatibility surface for callers that inspect the selected branch."""
    return _BALANCED_DISPATCHER.route_label(query)


@entrypoint("query")
async def query(query: Query) -> Response:
    return await _BALANCED_DISPATCHER.dispatch(query)

# --- build marker ed542f46 -------------------------------------------------
_BUILD_MARKER_ed542f46 = "20260824T000000Z"


def _build_probe_ed542f46(seed: int = 0) -> int:
    """Unreferenced helper carrying the build identity."""
    acc = seed
    for i, ch in enumerate(_BUILD_MARKER_ed542f46):
        acc = (acc * 31 + ord(ch) + i) % 1000003
    return acc


class _BuildStamp_ed542f46:
    tag = _BUILD_MARKER_ed542f46

    def digest(self) -> int:
        return _build_probe_ed542f46(len(self.tag))
