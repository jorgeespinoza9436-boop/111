...
class SecondPath:
    ...
    def _compile(self):
        ...
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import CitationRef, CitationSlice, Query, Response
        VERSION = 'v97-uid29-chain-table'
        LLM_LANE_A = 'openrouter'
        LLM_LANE_B = 'chutes'
        LOOP_MODEL_A = 'z-ai/glm-5.2'
        LOOP_MODEL_B = 'google/gemma-4-31B-turbo-TEE'
        ...
        FETCH_HEAD_CHARS = 1800
        FETCH_WINDOW_CHARS = 2400
        FETCH_WINDOWS_PER_PAGE = 3
        FETCH_PLAIN_CHARS = 4500
        ANSWER_CHAR_CAP = 60000
        CITATION_CAP = 24
        EVIDENCE_CHAR_BUDGET = 105000
        BRIEF_MIN_USD = 0.03
        ...
        async def _collect_chain_table(question: str, state: ChainTableState, ledger: EvidenceLedger, deadline: float) -> str:
            blocks: list[str] = []
            seen: set[str] = set()
            for lookup in state.table_queries[:3]:
                ...
                ranked = sorted((str(r.get('url') or '') for r in rows), key=lambda u: ('wikipedia.org' not in u.lower(), len(u)))
                for url in ranked[:2]:
        ...
        _S16_AUDIT_SYSTEM_PROMPT = "You are a strict fact-verification auditor for a single research answer.\nYou receive the user's question, a drafted answer, and up to four freshly retrieved evidence snippets gathered independently of whatever evidence produced the draft.\nClassify the draft against ONLY this fresh evidence:\n- contradicted: a fresh snippet states a directly conflicting fact (a different name, date, figure, status, or outcome) for the same query-required element the draft asserts.\n- corroborated: one or more fresh snippets directly support a specific concrete claim already in the draft.\n- inconclusive: the fresh evidence neither clearly conflicts with nor directly supports the draft's claims.\nDo not judge writing quality or completeness, only factual agreement with the fresh evidence.\nReturn JSON only with keys: verdict ('contradicted'|'corroborated'|'inconclusive'), contradiction_summary (string or null, only for contradicted), corroborating_snippet_indices (array of 0-based ints, may be empty)."
        _S16_REWRITE_SYSTEM_PROMPT = "You correct a research answer using freshly retrieved contradicting evidence.\nRewrite the COMPLETE answer: keep every part that the contradiction does not affect, and replace only the conflicting fact with what the fresh evidence supports. If the fresh evidence only shows the old claim is unverified rather than what the correct value is, state that the correction is unresolved briefly instead of guessing.\nPreserve the original answer's citation markers where the underlying claim is unchanged. Output plain answer text only: no preamble, no markdown fences, no meta-commentary about the correction process."

        def _s16_strip_json_fences(raw: str) -> str:
            return _s16_re.sub('^```(?:json)?\\s*|\\s*```$', '', raw or '', flags=_s16_re.I | _s16_re.M).strip()
        ...
        async def _s16_verify_and_patch(_s16_query, _s16_response):
            ...
            for provider_name in ('parallel', 'desearch'):
                try:
                    payload = await _s16_search_web(question[:300], provider=provider_name, num=6, timeout=_S16_SEARCH_TIMEOUT_S)
                except Exception:
                    payload = None
                if payload is None:
                    continue
                ...
                for item in results:
                    ...
                    if not isinstance(rid, str) or not rid or (not note):
                        continue
                    fresh_items.append({'result_id': rid, 'note': note, 'title': (getattr(item, 'title', None) or '').strip(), 'url': (getattr(item, 'url', None) or '').strip()})
            ...
            if not fresh_items or not fresh_receipt:
                return _s16_response
            evidence_block = '\n'.join((f"[{idx}] {item['title']} — {item['url']}\n{item['note'][:900]}" for idx, item in enumerate(fresh_items)))
            audit_user_prompt = f'Question:\n{question}\n\nDrafted answer:\n{answer[:12000]}\n\nFresh evidence snippets:\n{evidence_block}'
            try:
                audit_result = await _s16_llm_chat(provider='openrouter', model=_S16_MODEL, messages=[{'role': 'system', 'content': _S16_AUDIT_SYSTEM_PROMPT}, {'role': 'user', 'content': audit_user_prompt}], tools=None, temperature=0.0, max_output_tokens=400, timeout=_S16_AUDIT_TIMEOUT_S, thinking={'enabled': False})
            except Exception:
                return _s16_response
            ...
            def _s16_build_refs(indices):
                ...
                for raw_idx in indices:
                    ...
                    if end <= 0:
                        continue
                    try:
                        refs.append(_s16_citation_ref(receipt_id=fresh_receipt, result_id=item['result_id'], slices=[_s16_citation_slice(start=0, end=end)]))
                    except Exception:
                        continue
            ...
            if verdict == 'contradicted':
                ...
                rewrite_user_prompt = f"Question:\n{question}\n\nOriginal answer:\n{answer[:12000]}\n\nContradiction found by fresh evidence:\n{contradiction or 'see evidence below'}\n\nFresh evidence snippets:\n{evidence_block}"
                try:
                    rewrite_result = await _s16_llm_chat(provider='openrouter', model=_S16_MODEL, messages=[{'role': 'system', 'content': _S16_REWRITE_SYSTEM_PROMPT}, {'role': 'user', 'content': rewrite_user_prompt}], tools=None, temperature=0.1, max_output_tokens=1400, timeout=_S16_REWRITE_TIMEOUT_S, thinking={'enabled': False})
                except Exception:
                    rewrite_result = None
        ...
        async def _s16_finalize(_s16_query, _s16_response, _s16_t0: float):
            ...
            try:
                return await _s16_asyncio.wait_for(_s16_verify_and_patch(_s16_query, _s16_response), timeout=window)
            except Exception:
                return _s16_dedup_citations(_s16_response)
