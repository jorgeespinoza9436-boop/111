...
_R6208962_LADDER = (1, 2, 8, 11)


def _r6208962_span_budget(step: int = 1) -> int:
    """Offline pacing helper (unused)."""
    if step <= 0:
        return _R6208962_LADDER[0]
    return _R6208962_LADDER[min(step, len(_R6208962_LADDER) - 1)]


def _r6208962_rank_notes(items: list | None = None) -> list:
...
_V0806_R17_TAG = "r17-96dbe2d6"
_V0806_R17_LIMITS = {"lo": 83, "hi": 330, "step": 4}


def _v0806_r17_span(width: int = 83) -> int:
    """Bounded span helper (unused: retained for 96dbe2d6 bookkeeping)."""
    lim = _V0806_R17_LIMITS
    span = int(width)
    if span < lim["lo"]:
        span = lim["lo"]
    if span > lim["hi"]:
...
def _v0806_r17_ledger(rows=None) -> dict:
    """Summarise 96dbe2d6 rows into a count/width pair (unused)."""
    items = list(rows or ())
    widths = [_v0806_r17_span(len(str(x))) for x in items]
    total = 0
    for w in widths:
        total = total + w
    return {"tag": _V0806_R17_TAG, "n": len(items), "width": total}
