...
SEARCH_TIMEOUT = 10.0
FETCH_TIMEOUT = 15.0
# Measured on batch 6c42c98a: the 552 successful llm_chat calls have latency
# p50 7.3s / p90 25.2s / p95 39.8s / p99 69.4s / max 82.3s, while all 20
# ToolInvocationTimeoutError entries burned exactly 90s (median 90.2s, 1,357s
# total). The platform tool_timeout is 90s, so an LLM_TIMEOUT of 90 never lets
# this agent abandon a call first -- the platform always wins the race.
# At a 45s ceiling those 20 timeouts cost 900s instead, recovering 457s, and
# 23 successful calls (4.2%) get truncated. Net recovery is 457s of 9,050s
# (5.1%): this agent's latency tail is fat, so the timeout alone cannot reach
# the 60% runtime reduction the champion rule demands.
LLM_TIMEOUT = 45.0
...
# Measured: gemma-4-31b-it made 483 calls carrying 6,826,907 prompt tokens,
# i.e. 14,135 tokens per call. 72% of wall clock is successful llm_chat latency
# (6,545s), and the incumbent uid 155 answers the same 10 tasks with a median
# call of 2.1s against our 7.3s. 155 spends 2.2x our LLM budget and still
# finishes in a third of the time, so the difference is prompt size per call,
# not model choice. Cut the observation memory replayed every turn from 80k to
# 32k. The VFS stays re-readable, so evidence is not lost -- only the replay
# window narrows.
FOCUSED_OBSERVATION_MEMORY_CHARS = 32_000
VFS_READ_PAGE_CHARS = 32_000
VFS_SEARCH_PAGE_CHARS = 24_000
VFS_SIMILARITY_RESULT_CHARS = 18_000
VFS_LEXICAL_WINDOW_CHARS = 3_600
BATCHED_RETRIEVAL_PREVIEW_CHARS = 96_000
...
async def _investigate(question: str, expected_answer: str) -> tuple[str, list[CitationRef]]:
    ...
    for _turn in range(160):
        ...
        # Decide the absolute wall first. The two downgrade rules below are right for
        # spend protection but must not apply to time protection. In the original,
        # governor_stage was forced back to "open" whenever (a) no sources had been
        # gathered or (b) the closing turns were spent, which left the 210s hard
        # threshold effectively inert. Those are exactly the situations most likely to
        # cross the 300s hard wall and score zero.
        ...
        # With no evidence at all, the retrieval tools must not be withdrawn:
        # withdrawing them cannot produce an answer and locks in a zero. Keep
        # searching even at the cost of more budget.
        if not state.sources and not past_absolute_wall:
            governor_stage = "open"
        if governor_stage != "open":
            governor_turns += 1
        # The closing turns are spent and the task still has not closed. Keeping the
        # tools withheld would end worse than the original, which would have carried
        # on with the full tool set. Spend protection is best-effort and accuracy comes
        # first, so fall back to the original behaviour.
        ...
        if past_absolute_wall:
            governor_stage = "hard"
            # Retry here even if an earlier hard close failed. The only remaining
            # options are closing now or overrunning the hard wall for a zero.
            governor_bypass_failed = False
        if (
            governor_stage == "hard"
        ...
        if ready_requested:
            # The audit is a quality step, not the step that produces the answer. The
            # original left the audit call, the VERDICT parse, and the REVISE validation
            # all unguarded, so a format violation by the audit model scored the whole
            # task zero even with a finished answer already in hand. If the audit fails,
            # discard only the audit and close with the current answer.
...
def _v401_is_vacuous_output(output: Any) -> bool:
    """Detect a structured payload that collapsed into "found nothing".

    Measured on batch 6c42c98a: 5 of 24 structured runs (21%) emitted an
    all-empty payload such as {"countries": []} or {"film_distributors": []},
    and those 5 scored exactly 0.000 (non-empty runs averaged 0.578). The judge
    reads an empty array as a wrong answer, not a partial one -- verdict on
    5c29e4f3: "Answer 2 says {"countries":[]} ... Answer 2 is incorrect."
    The empty runs are the ones that thrashed: 216s median against 174s for the
    rest, with up to 13 searches.
    """
    if output is None:
        return True
    if isinstance(output, dict):
        ...
        if not values:
            return True
        return all(
            isinstance(item, (list, dict, str)) and len(item) == 0
            for item in values
        )
...
def _v401_retry_budget_left(deadline: float | None) -> bool:
    """Allow the empty-payload retry only when there is real slack left.

    One materialize call costs a measured llm_chat median of 7.3s and a p90 of
    25.2s. Spending that unconditionally in front of the 300s hard wall would
    score the whole task zero. Runs that produced an empty payload were already
    the slow ones (216s median), so most of them are filtered out here.
    """
    if deadline is None:
        return False
...
async def _v401_base_query(query: Query, deadline: float | None = None) -> Response:
    ...
    if query.output_schema is not None:
        ...
        # Retry an all-empty payload once, and only when the clock allows.
        # The gate this agent failed is runtime, not score, so spending an
        # unconditional extra LLM call to recover score would sabotage the one
        # gate it actually has to clear.
        if _v401_is_vacuous_output(output) and _v401_retry_budget_left(deadline):
            try:
                second = await _materialize_structured_output(
                    question=query.text,
                    answer=answer,
                    output_schema=query.output_schema,
                )
...
# One materialize retry at its p90 latency of 25.2s, plus headroom for
# citation assembly.
...
async def query(query: Query) -> Response:
    ...
    # _v401_scoring_guard is removed from the call path. Evidence:
    #   batch 6c42c98a - openai/gpt-oss-120b, 8 calls, 225 completion tokens
    #                    total = 28.1 per call.
    #   local repro    - completion=0, reasoning=726 (max_output_tokens=650).
    # The audit prompt demands JSON with four keys, but the call never passes a
    # thinking argument, so gpt-oss-120b's default reasoning stays on. Reasoning
    # tokens bill inside max_output_tokens, so 726 reasoning tokens consume the
    # entire 650 output budget and the completion comes back empty. raw is
    # therefore always blank, report is always None, and the guard falls through
    # to `return response` every time -- it has never once altered an answer
    # while costing one LLM call per prose run that reached it.
    # The failing gate is runtime, so remove it rather than revive it. To revive:
    # raise max_output_tokens above 2,000 AND disable thinking explicitly. That
    # is a new mechanism and needs its own batch to validate in isolation.
