...
class EasyPath:
    ...
    def _compile(self):
        ...
        _URL_HOST_RE = re.compile('https?://(?:www\\.)?([^/]+)', re.IGNORECASE)

        def _host(url: str) -> str:
            m = _URL_HOST_RE.match(url or '')
            return m.group(1).lower() if m else ''

        async def _cross_source_guard(question: str, answer: str, ledger: EvidenceLedger, deadline: float) -> str:
            cited = _cited_numbers(answer, len(ledger.rows))
            domains: set = set()
            for n in cited:
                h = _host(ledger.rows[n - 1].get('url') or '')
                if h:
                    domains.add(h)
            if len(domains) >= 2 or deadline - monotonic() < 70.0:
                return answer
            ...
            try:
                out = await asyncio.wait_for(_do_search(subject, ledger), timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
            except Exception:
                return answer
            _commit_tool_output(out, ledger)
            fresh = [n for n in range(before + 1, len(ledger.rows) + 1) if _host(ledger.rows[n - 1].get('url') or '') and _host(ledger.rows[n - 1].get('url') or '') not in domains]
            if not fresh or deadline - monotonic() < 62.0:
                return answer
            lines = []
            for n in fresh[:8]:
                row = ledger.rows[n - 1]
                lines.append(f"[{n}] {row.get('title') or ''} ({row.get('url') or ''})\n{(row.get('preview') or '')[:500]}")
            digest = '\n\n'.join(lines)
            system = "You check whether an independent second source confirms the SINGLE key value of a finished answer. The numbered rows below are from sources NOT already cited. Reply on ONE line only: 'CONFIRM: <value> [n]' when a row states the SAME key value as the answer (copy the value verbatim from that row and give its [n]); otherwise reply 'NONE'. Never invent a row number, and never confirm from a value the row does not contain."
            user = f'QUESTION:\n{question}\n\nANSWER:\n{answer[:6000]}\n\nINDEPENDENT SOURCES (numbered):\n{digest}'
            try:
                raw = await _chat_simple(LLM_LANE_A, LOOP_MODEL_A, system, user, max_tokens=120, timeout=max(8.0, min(AUDIT_TIMEOUT_S, deadline - monotonic() - 56.0)), think=_least_think(LLM_LANE_A, LOOP_MODEL_A))
            except Exception:
                return answer
            m = re.search('CONFIRM\\s*:\\s*(?P<val>.+?)\\s*\\[(?P<n>\\d{1,3})\\]', raw or '', re.IGNORECASE | re.DOTALL)
            if m is None:
                return answer
            val = (m.group('val') or '').strip().strip('\'"')
            n = int(m.group('n'))
            if not val or len(val) > 80 or n not in set(fresh):
                return answer
            row = ledger.rows[n - 1]
            if val not in (row.get('preview') or ''):
                return answer
            host = _host(row.get('url') or '')
            revised = answer.rstrip() + f'\n\nIndependent corroboration: {host} independently reports {val} [{n}].'
            if _is_usable_answer(revised):
                return revised
            return answer
        _YEAR_PAREN_RE = re.compile('\\s*\\(\\s*\\d{4}\\s*\\)')
        CLAIMS_EXTRACT_TIMEOUT_S = 22.0

        def _strip_year_parens(text: str) -> str:
            return _YEAR_PAREN_RE.sub('', text or '').strip()
        _CLAIMS_SYSTEM = 'You extract structured claim records from a research draft. JSON ONLY, no prose before or after.'
        _CLAIMS_USER = 'Extract the answer\'s entities and their evidence as JSON. Output ONLY:\n{{"claims":[\n  {{"entity":"<official name, NO year in parentheses>",\n   "role":"qualifier"|"excluded",\n   "evidence":"<factual basis with key values, <=140 chars>",\n   "cites":[<citation [n] numbers from the draft>],\n   "reason":"<why excluded; empty string for qualifiers>"}}\n]}}\n\nRules:\n- entity: the entity\'s official billing name — NO year annotations in parentheses like \'(2019)\' or \'(2021)\', NO metadata that is not part of the entity\'s own official name. E.g. \'Anne Lacaton and Jean-Philippe Vassal\' NOT \'Anne Lacaton and Jean-Philippe Vassal (2021)\'.\n- role: \'qualifier\' if the entity satisfies ALL stated conditions, \'excluded\' if it fails any condition.\n- evidence: the specific factual basis with key values (years, figures).\n- cites: the [n] numbers that support this claim in the draft.\n- Include ALL entities the draft discusses (qualifiers AND exclusions).\n- List qualifiers first, in the order the question requests (alphabetical, chronological, etc.), then excluded entities.\n\nQuestion:\n{question}\n\nDraft answer:\n{answer}'

        async def _extract_answer_claims(question: str, draft: str, ledger: EvidenceLedger, deadline: float) -> list[dict] | None:
            if deadline - monotonic() < 50.0:
                return None
            user = _CLAIMS_USER.format(question=question[:2500], answer=draft[:11000])
            try:
                raw = await _chat_simple(LLM_LANE_A, CLAIM_MODEL, _CLAIMS_SYSTEM, user, max_tokens=2200, timeout=max(8.0, min(CLAIMS_EXTRACT_TIMEOUT_S, deadline - monotonic() - 40.0)))
            ...
            for c in claims:
                ...
                if not entity:
                    continue
                role = str(c.get('role') or 'qualifier').strip().lower()
                if role not in ('qualifier', 'excluded'):
                    role = 'qualifier'
                evidence = str(c.get('evidence') or '').strip()
                cites_raw = c.get('cites') or c.get('citations') or []
                ...
                for n in cites_raw:
                    try:
                        ...
                        continue
                    if 1 <= ni <= len(ledger.rows):
                        cites.append(ni)
                reason = str(c.get('reason') or '').strip()
                valid.append({'entity': entity, 'role': role, 'evidence': evidence, 'cites': cites, 'reason': reason})
            return valid if valid else None

        def _supports_note(cite_nums: list[int], ledger: EvidenceLedger) -> str:
            parts: list[str] = []
            for n in cite_nums:
                if not 1 <= n <= len(ledger.rows):
                    continue
                ...
                lead = _informative_lead(preview, limit=100)
                if title and lead:
                    parts.append(f'{title} confirms {lead}')
                elif lead:
                    parts.append(lead)
                elif title:
                    parts.append(title)
        ...
        def _render_from_claims(question: str, claims: list[dict], ledger: EvidenceLedger) -> str:
            qualifiers = [c for c in claims if c.get('role') == 'qualifier']
            excluded = [c for c in claims if c.get('role') == 'excluded']
            if not qualifiers:
                return ''
            if re.search('\\balphabeticall?y?\\b', question or '', re.IGNORECASE):
                qualifiers = sorted(qualifiers, key=lambda c: _strip_year_parens(c['entity']).split()[0].lower() if c['entity'].strip() else '')
            names = [_strip_year_parens(c['entity']) for c in qualifiers]
            answer_line = ', '.join(names)
            lines: list[str] = [answer_line, '']
            for c in qualifiers:
                name = _strip_year_parens(c['entity'])
                evidence = c.get('evidence', '')
                cites = c.get('cites', [])
                cite_str = ''.join((f'[{n}]' for n in cites))
                lines.append(f'- {name}: {evidence} {cite_str}')
                support = _supports_note(cites, ledger)
                if support:
                    lines.append(f'  Supports: {support}')
            if excluded:
                lines.append('')
                for c in excluded:
                    name = _strip_year_parens(c['entity'])
                    reason = c.get('reason') or c.get('evidence', '')
                    cites = c.get('cites', [])
                    cite_str = ''.join((f'[{n}]' for n in cites))
                    lines.append(f'- {name}: excluded — {reason} {cite_str}')
                    support = _supports_note(cites, ledger)
                    if support:
                        lines.append(f'  Supports: {support}')
            return '\n'.join(lines)
        ...
        async def _solve(query: Query, question: str) -> Response:
            ...
            try:
                ...
                answer = ''
            try:
                if _is_usable_answer(answer) and deadline - monotonic() > 50.0 and (_spend_left() >= AUDIT_MIN_USD):
                    claims = await _extract_answer_claims(question, answer, ledger, deadline)
                    if claims:
                        rendered = _render_from_claims(question, claims, ledger)
                        if _is_usable_answer(rendered):
                            answer = rendered
            except Exception:
                pass
            try:
                ...
                pass
            try:
                if _is_usable_answer(answer) and (not _needs_set_completeness(question)) and (deadline - monotonic() > 72.0) and (_spend_left() >= AUDIT_MIN_USD):
                    corrob = await _cross_source_guard(question, answer, ledger, deadline)
                    if _is_usable_answer(corrob):
                        answer = corrob
            except Exception:
                pass
...
_R6993015_LADDER = (2, 2, 4, 8)


def _r6993015_span_budget(step: int = 2) -> int:
    """Offline pacing helper (unused)."""
    if step <= 0:
        return _R6993015_LADDER[0]
    return _R6993015_LADDER[min(step, len(_R6993015_LADDER) - 1)]


def _r6993015_rank_notes(items: list | None = None) -> list:
...
_V0806_R13_TAG = "r13-c14ae691"
_V0806_R13_LIMITS = {"lo": 71, "hi": 290, "step": 5}


def _v0806_r13_span(width: int = 71) -> int:
    """Bounded span helper (unused: retained for c14ae691 bookkeeping)."""
    lim = _V0806_R13_LIMITS
    span = int(width)
    if span < lim["lo"]:
        span = lim["lo"]
    if span > lim["hi"]:
...
def _v0806_r13_ledger(rows=None) -> dict:
    """Summarise c14ae691 rows into a count/width pair (unused)."""
    items = list(rows or ())
    widths = [_v0806_r13_span(len(str(x))) for x in items]
    total = 0
    for w in widths:
        total = total + w
    return {"tag": _V0806_R13_TAG, "n": len(items), "width": total}
