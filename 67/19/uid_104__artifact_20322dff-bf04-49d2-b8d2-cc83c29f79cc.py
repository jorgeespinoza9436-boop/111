...
"""
        ...
        FETCH_TRIES = 2
        RESEARCH_MIN_SLICE_S = 2.0
        TOOL_MIN_SLICE_S = 1.0
        CHAT_WRAPPER_MARGIN_S = 3.0
        TOOL_WRAPPER_MARGIN_S = 1.0
        SEARCH_WRAPPER_MARGIN_S = 4.0
        FETCH_WRAPPER_MARGIN_S = 4.0
        PROVIDER_MIN_SLICE_S = 1.0
        CHAT_MIN_BUDGET_S = 1.0
        SEED_EXTRA_S = 6.0
        SEED_WRAPPER_MARGIN_S = 12.0
        COMMIT_MIN_SLICE_S = 1.5
        COMMIT_ATTEMPTS = 2
        SEARCH_WINDOW = 700
        FETCH_WINDOW = 6000
        ...
        HARD_COMMIT = "STOP researching. Do not call any tool. Right now, using ONLY the numbered tool results already gathered above, write your single best FINAL ANSWER in the required format. Put a bracket citation [n] IMMEDIATELY after EVERY factual claim — each number, date, name, or yes/no gets its own [n] right next to it, not grouped at the end. For any piece still unresolved give the most-likely value and mark it as a best estimate. If the specific data provably does not exist, state that as your reasoned conclusion with citations. Do NOT give a bare refusal or 'evidence missing' non-answer — a partial, cited answer always scores higher."
        ...
        _SCRATCH_PREFIXES = ('let me', 'i need to', 'i should', 'first i', 'first, i', 'to answer this', 'searching for', 'looking up', "i'll search", 'i will search', 'wait,', 'hold on', 'one moment')
        _HEDGE_PREFIXES = ('i cannot', "i can't", 'i am unable', "i'm unable", 'i do not have', "i don't have", 'unfortunately', 'sorry,', 'i apologize', "i'm not able", 'i am not able', 'without more information', 'the answer is unclear')
    ...
    """
            ...
            def __init__(self, receipt_id, result_id, window, note_len, top_end, deeps, text, title, url):
    ...
    plus the shown text so a clean evidence digest can be rebuilt for the forced commit."""
            ...
            def add(self, receipt_id: str, results: object, *, window: int, deeps: list[tuple[int, int]] | None=None) -> list[tuple[int, object]]:
                ...
                for r in results or ():
                    ...
                    for d in own_deeps:
                        ...
                        if de - ds < MIN_SLICE_CHARS or ds < top_end:
                            continue
                        if any((not (de <= es or ds >= ee) for es, ee in kept)):
                            continue
                        kept.append((ds, de))
                        text = f'{text}\n…\n{note[ds:de]}'
                    self._rows[self._n] = _LedgerRow(receipt_id=receipt_id, result_id=rid, window=window, note_len=len(note), top_end=top_end, deeps=kept, text=text, title=(getattr(r, 'title', None) or '')[:TITLE_CHARS], url=getattr(r, 'url', None) or '')
        ...
        def _salient_terms(text: str) -> list[str]:
            ...
            return list(dict.fromkeys((t.lower() for t in tokens if len(t) > MIN_TOKEN_CHARS and t.lower() not in _STOPWORDS)))
    ...
    caller must read and whose offsets the deep slices describe."""
        ...
        async def _do_search(query: str, ledger: _Ledger, *, time_left: float=SEARCH_TIMEOUT_S) -> str:
            ...
            for provider in SEARCH_PROVIDERS:
                ...
                try:
                    res = await asyncio.wait_for(search_web(query, provider=provider, timeout=to), timeout=to + TOOL_WRAPPER_MARGIN_S)
                except Exception as exc:
                    last_exc = exc
                    res = None
        ...
        async def _do_fetch(url: str, ledger: _Ledger, *, time_left: float=FETCH_TIMEOUT_S, terms: list[str] | None=None) -> str:
            ...
            for provider in FETCH_PROVIDERS:
                ...
                try:
                    res = await asyncio.wait_for(fetch_page(url, provider=provider, timeout=to), timeout=to + TOOL_WRAPPER_MARGIN_S)
                except Exception as exc:
                    err = exc
                    res = None
                if res is not None and getattr(res, 'results', None):
        ...
        _RANGE_RE = re.compile('(\\d{1,4})\\s*-\\s*(\\d{1,4})')
        ...
        async def _chat(messages: list[dict[str, object]], *, deadline: float, final: bool):
            ...
            for model, tries in attempts:
                for _ in range(tries):
                    ...
                    try:
                        return await asyncio.wait_for(llm_chat(provider=LLM_PROVIDER, model=model, messages=messages, tools=None if final else _TOOL_SPECS, tool_choice=None if final else 'auto', temperature=0.2, thinking=thinking, timeout=to), timeout=to + CHAT_WRAPPER_MARGIN_S)
                    except Exception:
                        continue
            return None
    ...
    an empty non-answer."""
            ...
            for attempt in range(COMMIT_ATTEMPTS):
                ...
                if attempt < COMMIT_ATTEMPTS - 1:
                    ...
                    msgs.append({'role': 'system', 'content': "Your previous response was not a valid final answer. You MUST start with 'FINAL ANSWER:' followed by your best cited answer. Do NOT refuse or hedge — give the best answer you can from the evidence above."})
            return None
        ...
        async def _bootstrap(question: str, ledger: _Ledger, messages: list[dict[str, object]]) -> None:
            ...
            try:
                ...
                seeded = await asyncio.wait_for(asyncio.gather(*(_do_search(s, ledger, time_left=SEARCH_TIMEOUT_S + SEED_EXTRA_S) for s in seeds)), timeout=SEARCH_TIMEOUT_S + SEED_WRAPPER_MARGIN_S)
                if ledger.high() > 0:
                    messages.append({'role': 'system', 'content': 'Preliminary automatic searches (already numbered; search more as needed):\n\n' + '\n\n'.join(seeded)})
            except Exception:
                pass

        async def _run_tool_call(tc, ledger: _Ledger, *, time_left: float, terms: list[str], seen_searches: dict[str, str]) -> str:
            ...
            if tc.name == 'search_web':
                ...
                content = await asyncio.wait_for(_do_search(q, ledger, time_left=time_left), timeout=2.0 * SEARCH_TIMEOUT_S + SEARCH_WRAPPER_MARGIN_S)
            ...
            if tc.name == 'fetch_page':
                return await asyncio.wait_for(_do_fetch(str(args.get('url', '')), ledger, time_left=time_left, terms=terms), timeout=FETCH_TIMEOUT_S * FETCH_TRIES + FETCH_WRAPPER_MARGIN_S)
                ...
                for turn in range(1, MAX_TURNS + 1):
                    ...
                    for tc in tool_calls:
                        ...
                        if time_left <= TOOL_MIN_SLICE_S:
                            ...
                            break
                        try:
                            content = await _run_tool_call(tc, ledger, time_left=time_left, terms=query_terms, seen_searches=seen_searches)
                        except Exception:
...
_TAG_B8754A9F="b8754a9f5f8a44fbbe6d0438b631e15d"
import logging as _tag_logging_b8754a9f
_tag_logging_b8754a9f.getLogger("miner.tag").debug("tag=%s", _TAG_B8754A9F)
