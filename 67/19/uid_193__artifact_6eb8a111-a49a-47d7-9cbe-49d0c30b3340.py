"""SN67 Harnyx miner — v25 deep-research pipeline. [slot 02 build 2026-08-06T15:00:28+00:00]"""
...
def _build_bounded_pool_plan_messages(
    *, question: str, existing_packets: tuple[AcceptedEvidence, ...],
    ...
    evidence_brief = [
        {
            "packet_number": i, "title": p.title or "", "url": p.url,
            "excerpt": (p.source_text or "")[:POOL_SWEEP_MEMBER_EXCERPT_CHARS],
        }
        for i, p in enumerate(existing_packets[:12], start=1)
    ]
    system_content = (
        ...
        "If it IS a bounded pool, output the pool subject, the complete member list, and EACH distinct "
        "filter criterion the question imposes as a separate checklist item. Name the members from the "
        "question when it lists them; OTHERWISE READ THEM OUT OF THE EVIDENCE_BRIEF EXCERPTS — when the "
        "pool is defined by a ranking or roster the question only describes (e.g. 'the top 5 vendors "
        "by shipments'), the excerpts usually already contain that roster, and its entries ARE the "
        "pool members. Return the member names alone (no figures). Leave pool_members empty only when "
        "neither the question nor the excerpts identify the members. If it is NOT a bounded pool, set "
        "is_bounded_pool to false and leave the lists empty.\n\n"
    ...
    *, question: str, plan: BoundedPoolPlan, contract: ResearchContract,
    ...
    for member, criterion in _bounded_pool_member_targets(plan):
        ...
        try:
            response = await search_web(
                [query], provider=_SEARCH_PROVIDER,
                num=POOL_SWEEP_SEARCH_RESULTS_PER_MEMBER, timeout=LITE_SEARCH_TOOL_TIMEOUT_SECONDS,
            )
        except Exception:
            continue
        receipt_id = getattr(response, "receipt_id", "") or ""
...
# slot: 02 A15_commit 2026-08-06T15:00:28+00:00
