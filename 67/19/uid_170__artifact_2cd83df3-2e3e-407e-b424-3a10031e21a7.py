        ...
        class HeavyCorridor:
            ...
            def _compile(self):
                ...
                def _origin_edict_h(names: list[str]) -> str:
                    listed = ', '.join(names)
                    return f"NAMED ORIGIN LOCK — this ask pins the answer to: {listed}. You MUST hunt and read_page THAT origin (try site: filters). Aggregators with matching figures score 0.0 if the named origin was not cited. Prefer the named origin's exact figures when they conflict. Re-fetch with a focus so the cited excerpt itself shows those figures (slice must LITERALLY contain the digits/names)."
