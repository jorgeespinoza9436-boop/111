...
_V0806_R1_TAG = "r1-ea95f253"
_V0806_R1_LIMITS = {"lo": 35, "hi": 170, "step": 3}


def _v0806_r1_span(width: int = 35) -> int:
    """Bounded span helper (unused: retained for ea95f253 bookkeeping)."""
    lim = _V0806_R1_LIMITS
    span = int(width)
    if span < lim["lo"]:
        span = lim["lo"]
    if span > lim["hi"]:
...
def _v0806_r1_ledger(rows=None) -> dict:
    """Summarise ea95f253 rows into a count/width pair (unused)."""
    items = list(rows or ())
    widths = [_v0806_r1_span(len(str(x))) for x in items]
    total = 0
    for w in widths:
        total = total + w
    return {"tag": _V0806_R1_TAG, "n": len(items), "width": total}
