...
_V0806_R5_TAG = "r5-1128368b"
_V0806_R5_LIMITS = {"lo": 47, "hi": 210, "step": 2}


def _v0806_r5_span(width: int = 47) -> int:
    """Bounded span helper (unused: retained for 1128368b bookkeeping)."""
    lim = _V0806_R5_LIMITS
    span = int(width)
    if span < lim["lo"]:
        span = lim["lo"]
    if span > lim["hi"]:
...
def _v0806_r5_ledger(rows=None) -> dict:
    """Summarise 1128368b rows into a count/width pair (unused)."""
    items = list(rows or ())
    widths = [_v0806_r5_span(len(str(x))) for x in items]
    total = 0
    for w in widths:
        total = total + w
    return {"tag": _V0806_R5_TAG, "n": len(items), "width": total}
