...
# ── budgets (seconds) ─────────────────────────────────────────────────────────
WALL_BUDGET_S = 258.0        # v32.4c: 248 was the field's shortest, but 270 collided
...
MIN_TAIL_S = 8.0
MAX_TURNS = 12          # one extra bounded turn for table/source verification
AUDIT_EXTRA_TURNS = 2
...
FETCH_PLAIN_CHARS = 6200     # preserve short official tables without truncating rows
...
def _source_contract_score(item, query_text: str) -> tuple[int, int, str]:
    """Rank direct requested sources before evidence enters ChainTable state."""
    ...
    if any(domain in haystack for domain in (
        ".gov", "wikipedia.org/wiki/", "sec.gov", "nasa.gov",
        "census.gov", "bls.gov", "eia.gov",
    )):
        score += 3
    terms = {term for term in re.findall(r"[a-z0-9]{4,}", query) if not term.isdigit()}
...
# ── entrypoint ────────────────────────────────────────────────────────────────
# ── v97 primary controller: chain-and-table solver ────────────────────────────
# Explicit state makes multi-hop facts and final table constraints separate stages.
@dataclass
class ChainFact:
    label: str
    value: str
...
_CHAIN_YEAR_RE = re.compile(r"\b(?:18|19|20)\d{2}\b")
...
def _chain_table_state(question: str, brief: str) -> ChainTableState:
    ...
    condition_count = len(re.findall(r"(?i)\b(?:and|or|between|before|after|more than|less than|at least|at most)\b", question or ""))
    ...
    named_domains = {
        ...
        "census": "site:census.gov",
        "bls": "site:bls.gov",
        "sec": "site:sec.gov",
        "nasa": "site:nasa.gov OR site:ipac.caltech.edu",
        "usgs": "site:usgs.gov",
    }
...
async def _collect_chain_table(question: str, state: ChainTableState,
                               ledger: EvidenceLedger, deadline: float) -> str:
    blocks: list[str] = []
    seen: set[str] = set()
    caps = {"light": (1, 1, 0, 0), "standard": (2, 2, 1, 0), "deep": (2, 3, 1, 1)}[state.complexity]
    portfolio = (
        [("direct", q) for q in state.direct_queries[:caps[0]]]
        + [("table", q) for q in state.table_queries[:caps[1]]]
        + [("challenge", q) for q in state.challenge_queries[:caps[2]]]
        + [("contradiction", q) for q in state.contradiction_queries[:caps[3]]]
    )
    for lane, lookup in portfolio:
        ...
        ranked = sorted(
            ...
            key=lambda u: (
                lane == "direct" and not any(d in u.lower() for d in (".gov", "wikipedia.org", "ipac.caltech.edu")),
                "wikipedia.org" not in u.lower(),
                len(u),
            ),
        )
        for url in ranked[:2]:
...
async def _solve_chain_table(query: Query, question: str) -> Response:
    ...
    answer, messages = await _loop(question, "\n\n".join(x for x in (brief, chain_context) if x),
                                   ledger, deadline, 11)
