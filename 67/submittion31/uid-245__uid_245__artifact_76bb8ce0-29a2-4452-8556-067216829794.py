...
class ForgeAnvil_efd4e5:
    ...
    def _compile(self):
        ...
        async def _wrap_anvil_efd4e5_fetch_page(_a0, **kwargs):
            if _anvil_fit_efd4e5():
                return None
            _k = kwargs
            _v_provider = _k["provider"] if "provider" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_anvil_efd4e5_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_anvil_efd4e5_fetch_page
        _hold_anvil_efd4e5_llm_chat = llm_chat

        async def _wrap_anvil_efd4e5_llm_chat(**kwargs):
            if _anvil_fit_efd4e5():
                return None
            _k = kwargs
            ...
            return await _hold_anvil_efd4e5_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_anvil_efd4e5_llm_chat
        _hold_anvil_efd4e5_search_web = search_web

        async def _wrap_anvil_efd4e5_search_web(_a0, **kwargs):
            if _anvil_fit_efd4e5():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_anvil_efd4e5_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_anvil_efd4e5_search_web
        _hold_anvil_efd4e5_tooling_info = tooling_info

        async def _wrap_anvil_efd4e5_tooling_info(**kwargs):
            if _anvil_fit_efd4e5():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_anvil_efd4e5_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_anvil_efd4e5_tooling_info
...
class MillAnvil_efd4e5:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class SableHide_218b52:
        ...
        class UmberHide_218b52:
            ...
            def _compile(self):
                ...
                from harnyx_miner_sdk.decorators import entrypoint
                from harnyx_miner_sdk.query import CitationRef, CitationSlice, Query, Response
                VERSION = 'v53-pool-figure-slice-b'
                LLM_LANE_A = 'openrouter'
        ...
        async def _vc_baseline_query(query: Query) -> Response:
            if _TACK_PIN_218b52._sable_ok_218b52():
                return await _SABLE_RUN_218b52(query)
            return await _UMBER_RUN_218b52(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid168-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.00 arcc_bells_vireo; 475f0b1b:0.50 usgs_mcs_net_import; 7b7707d5:0.38 unesco_whc_48; 9d58b52a:0.12 nps_weekly_list; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.00 yuba_sutter_transit; eb277bad:0.62 st_helena_census; eef7c321:0.12 hague_evidence_service; f5e14f85:0.62 port_la_container'
...
async def _s31_base_query(query: Query) -> Response:
    if _INDEX_PEG_efd4e5._forge_ok_efd4e5():
        return await _FORGE_RUN_efd4e5(query)
    return await _MILL_RUN_efd4e5(query)

# unused helpers (retained for reference)
_DEAD_BLOCK_a42e1dd98144 = {
    "tag": "a42e1dd98144",
    "n": 10759709,
}
def _dead_noop_a42e1dd98144(x):
    total = 0
