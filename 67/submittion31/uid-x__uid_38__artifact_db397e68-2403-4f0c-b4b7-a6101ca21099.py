...
class ClayJar_14b776:
    ...
    def _compile(self):
        ...
        async def _wrap_jar_14b776_fetch_page(_a0, **kwargs):
            ...
            return await _hold_jar_14b776_fetch_page(_a0, provider=_v_provider, timeout=_v_timeout)
        fetch_page = _wrap_jar_14b776_fetch_page
        _hold_jar_14b776_llm_chat = llm_chat

        async def _wrap_jar_14b776_llm_chat(**kwargs):
            if _jar_fit_14b776():
                return None
            _k = kwargs
            ...
            return await _hold_jar_14b776_llm_chat(max_output_tokens=_v_max_output_tokens, messages=_v_messages, model=_v_model, provider=_v_provider, provider_extra=_v_provider_extra, temperature=_v_temperature, thinking=_v_thinking, timeout=_v_timeout, tool_choice=_v_tool_choice, tools=_v_tools)
        llm_chat = _wrap_jar_14b776_llm_chat
        _hold_jar_14b776_search_web = search_web

        async def _wrap_jar_14b776_search_web(_a0, **kwargs):
            if _jar_fit_14b776():
                return None
            _k = kwargs
            ...
            _v_provider_extra = _k["provider_extra"] if "provider_extra" in _k else None
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_jar_14b776_search_web(_a0, num=_v_num, provider=_v_provider, provider_extra=_v_provider_extra, timeout=_v_timeout)
        search_web = _wrap_jar_14b776_search_web
        _hold_jar_14b776_tooling_info = tooling_info

        async def _wrap_jar_14b776_tooling_info(**kwargs):
            if _jar_fit_14b776():
                return None
            _k = kwargs
            _v_timeout = _k["timeout"] if "timeout" in _k else None
            return await _hold_jar_14b776_tooling_info(timeout=_v_timeout)
        tooling_info = _wrap_jar_14b776_tooling_info
...
class StoneJar_14b776:

    def _compile(self):
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import Query, Response

        class MossBank_84d398:
        ...
        async def _vc_baseline_query(query: Query) -> Response:
            if _CLIP_STEM_84d398._moss_bank_84d398():
                return await _MOSS_RUN_84d398(query)
            return await _FERN_RUN_84d398(query)
        import re
        import json
        ...
        TASK_LOG_RESCUE_VERSION = 'v0814-uid94-batch-a232cac2-qualifying-notable'
        V0814_MIN_REMAINING_S = 14.0
        V0814_FETCH_TIMEOUT_S = 14.0
        _V0814_WEAK_NOTES = '25ff8bf6:0.38 arcc_bells_vireo; 475f0b1b:0.38 usgs_mcs_net_import; 7b7707d5:0.50 unesco_whc_48; 9d58b52a:0.38 nps_weekly_list; c9df4693:0.00 artemis_met_mismatch; cc43a3b5:0.12 yuba_sutter_transit; eb277bad:0.50 st_helena_census; eef7c321:0.00 hague_evidence_service; f5e14f85:0.62 port_la_container'
...
async def _s31_base_query(query: Query) -> Response:
    if _LID_PEG_14b776._clay_ok_14b776():
        return await _CLAY_RUN_14b776(query)
    return await _STONE_RUN_14b776(query)

# unused helpers (retained for reference)
_DEAD_BLOCK_3d6c1e390bbf = {
    "tag": "3d6c1e390bbf",
    "n": 4025374,
}
def _dead_noop_3d6c1e390bbf(x):
    total = 0
