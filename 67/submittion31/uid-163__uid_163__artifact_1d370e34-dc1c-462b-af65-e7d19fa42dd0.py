...
# MEASURED: our stored platform credentials are openrouter/parallel/firecrawl/exa/
# tavily — ai_gateway is NOT among them, so the champion's lane B can only 402 for
# us. Its whole purpose is to rescue a failed lane-A call; pointed at a provider we
# cannot reach it converts every rescue into a second failure. Repointed at
# openrouter with a different model, so the fallback is a real fallback.
LLM_LANE_B = "openrouter"          # fallback lane (must be a provider we hold)
...
LOOP_MODEL_B = "deepseek/deepseek-v3.2"  # rescue lane; kept off gpt for diversity
...
# ── f16.2 CITATION POINTERS ───────────────────────────────────────────────────
# The platform changed the contract 2026-08-18 ~04:07Z (repo 9b2fb0b): `[[n]]` is an
# EXACT pointer to validated_citations[n-1], one-based, and is valid only when that
# position resolves. Plain `[n]` is ordinary content -- harmless, but backing nothing.
#
# The trap that killed f16.0 and f16.1: `n` in our answer is an EVIDENCE-LEDGER ROW,
# while the array we ship is a FILTERED, COMPACTED list (_citations_for drops rows
# whose receipt failed, drops rows over EVIDENCE_CHAR_BUDGET, caps at CITATION_CAP).
# Position != n except by luck. Measured on the answers we actually shipped in batch
# 6ee5a9bf: 2b97ace6 cited [42]/[43] against a TWO-element array; 43a05fcd cited [41]
# against ten. f16.0 promoted every marker verbatim, turning harmless text into
# INVALID pointers. f16.1 renumbered but only touched SINGLE brackets -- and by then
# the prompt had taught the model to write [[n]] itself, so on 43a05fcd and efa7d7dc
# it wrote 16 and 58 double-bracket pointers with ledger numbers 25-35 against a
# 2-element array and the renumberer skipped every one of them.
#
# So: COLLAPSE FIRST, then renumber. Validity must not depend on who wrote the marker.
# The prompt is left saying `[n]` exactly as in f12.0/f15.x -- the build that scored
# 0.500, field best of 264 -- because prompt-level instruction is an unreliable lever
# (f15.1 took the same 6 tasks from 2.00 to 0.50) and this layer is deterministic.
...
def _pointerize_citation_markers(text: str, order: list[int]) -> str:
    ...
    if not text:
        return text
    # every marker to a single canonical form first -- the model writes both
    text = _CITE_DOUBLE_RE.sub(lambda m: "[%s]" % m.group(1), text)
    if not order:
        # nothing shipped: leave them as plain [n], which the contract reads as
        # ORDINARY TEXT. Never emit a pointer we cannot resolve.
        return text
...
def _citations_for(answer: str, ledger: EvidenceLedger) -> tuple[list[CitationRef], list[int]]:
    ...
    kept: list[int] = []          # f16.2: the ledger rows we SHIP, in array order
    spent = 0
...
# MEASURED on batch 6ee5a9bf MAIN stage, where 18 of 20 tasks are schema tasks and
# we scored 0.2667 against the king's 0.3833. Six of our twenty answers were
# schema DEFECTS, not capability failures -- every one a unanimous 0.00 across all
# five validators:
#
#   DIGEST TEXT PASTED INTO A SCHEMA FIELD (3 tasks). Task 41b7223c shipped
#     {"rfc_title": "Best-supported findings from the sources retrieved:\n- STD 1
#      subseries contains 0 RFCs: ..."}
#   against a reference of {"rfc_title": "Internet Official Protocol Standards"}.
#   ROOT CAUSE: the guard below was keyed on OBJECT IDENTITY --
#       if basis is not answer:  cleaned = _undigest_for_schema(basis)
#   so it only fired when `basis` had been REPLACED. When the rescue ladder had
#   already set `answer` itself to the _deterministic_answer digest, `basis` IS
#   `answer`, the guard was False, and _coerce_to_schema pasted the digest in
#   verbatim. The protection existed and was bypassed by the commonest path.
#
#   ALL-EMPTY SCHEMA SHELL (3 tasks). Task afe9236e shipped
#     {"title":"","pool_count":0,"release_span":"","watch_entries":0,"induction_year":0}
#   -- schema-valid and completely information-free, which scores exactly as badly
#   as a violation while looking like a successful response.
#
# Both are fixed by CONTENT checks. Identity told us where a value came from;
# content tells us what it actually is, which is what the judge sees.
_SCHEMA_DIGEST_MARKERS = (
    "best-supported findings",
    "sources retrieved:",
    "[slice ",
)
# f15.2: REFUSAL PROSE IS THE THIRD VARIANT OF THE SAME DEFECT. f15.0 fixed
# digest-in-field and the all-empty shell; task afe9236e then shipped
#   {"release_span": "I cannot answer this question from the evidence gathered...",
#    "pool_count": 0}
# which _is_digest_text does NOT catch -- neither digest prose nor empty, so it passed
# as "fixed" while being exactly as unscoreable. A schema field holds a VALUE; a
# sentence explaining why there is no value belongs nowhere near it.
#
# MEASURED: with this detector, afe9236e produced
#   {"induction_year":1999,"pool_count":23,"release_span":"1936-39",
#    "title":"Trance and Dance in Bali","watch_entries":2}
# which is BYTE-IDENTICAL to the reference answer. (It still scored 0.000 -- the
# judge is a PAIRWISE comparison and the tiebreak went to the reference's citation
# presentation. That is a separate, unsolved problem; it does not make this fix wrong,
# it makes it necessary-but-not-sufficient.)
#
# NOT CARRIED OVER FROM f15.1: the "print the source's own string / keep source
# order" prompt rules. Measured on the same 6 tasks they took the subset from 2.00
# to 0.50 -- f8cc1516 moved CLOSER to the reference string and scored WORSE (0.5 ->
# 0.0), and the order rule made d5a0f79e reorder and lose (0.5 -> 0.25). Prompt-level
# instruction destabilised copying rather than improving it.
_SCHEMA_REFUSAL_RE = re.compile(
    r"\b(?:i|we)\s+(?:cannot|can't|could not|couldn't|am unable|are unable)\b"
    r"|\bunable to (?:answer|determine|verify|confirm|identify|locate)\b"
    r"|\bthe evidence (?:does not|doesn't)\b"
    r"|\bnot (?:enough|sufficient) (?:evidence|information)\b"
    r"|\bno (?:evidence|information) (?:was )?(?:found|gathered|available)\b",
    re.I)
...
def _is_digest_text(value: object) -> bool:
    ...
    if _SCHEMA_REFUSAL_RE.search(value):
        return True      # f15.2: a refusal is prose, not a value
    # bullet-list prose: several newline-separated lines, most of them sentences
...
def _schema_value_is_empty(value: object) -> bool:
    ...
    if isinstance(value, str):
        if _SCHEMA_REFUSAL_RE.search(value):
            return True   # f15.2: refusal-only field == empty, so the rescue fires
...
def _scrub_schema_digest(value):
    ...
    if isinstance(value, str):
        ...
        # keep only a value-shaped head; a multi-line remainder is still prose
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # f16.2: LAST transform on the text the judge reads. After _citations_for so the
    # array (whose ORDER the pointers index) is already built, and after
    # _answer_line_only so a line required to be bare stays bare.
    ...
    if query.output_schema is not None:
        ...
        if structured is not None:
            try:
                ...
                pass
            # f15.0: the extractor itself can echo digest prose into a string
            # field, and _verbatim_structured does not check for it.
            try:
        ...
        # so a digest that arrived AS `answer` -- the commonest path, once the
        # rescue ladder has run -- skipped undigesting entirely and was pasted
        # into the field verbatim. 3 of 20 main tasks died exactly this way.
        ...
        try:
            ...
            # violation while looking like a success (3 of 20 main tasks shipped
            # {"title":"","pool_count":0,...}). While the ledger still holds
