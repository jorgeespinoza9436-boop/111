...
Stages added on the ordinary successful path in this build (rkcph):
  candidate-pool pre-pass, open-criteria hint, subject verification, timeframe alignment, citation dedupe + anchor backfill.
...
# ── post-audit stage: timeframe alignment ─────────────────────────────────────
...
def _anchor_years(question: str) -> list[str]:
    """The years the question anchors on, expanding a short explicit span.

    'between 2019 and 2022' anchors the endpoints AND every year between them:
    checking only the endpoints lets an answer skip the middle of its own
    window, which reads as covered and is not."""
    found: list[str] = []
    for m in _ANCHOR_YEAR_RE.finditer(question or ""):
        y = m.group(1)
        if y not in found:
            found.append(y)
    if len(found) == 2:
        lo = min(int(y) for y in found)
        hi = max(int(y) for y in found)
        if 0 < hi - lo <= ANCHOR_SPAN_MAX:
            found = [str(y) for y in range(lo, hi + 1)]
    return found


def _unevidenced_years(years: list[str], ledger: EvidenceLedger) -> list[str]:
    notes = [row.get("text") or "" for row in ledger.rows]
    return [y for y in years
            if not any(y in note for note in notes)][:MAX_ANCHOR_YEARS]


def _year_probe_query(question: str, year: str) -> str:
    return (_topic_tail(question, 6) + " " + year).strip()


async def _align_timeframe(question: str, answer: str, messages: list[dict],
                           ledger: EvidenceLedger, deadline: float) -> str:
    """Every year the question anchors on must be present in the evidence.

    A period the run never retrieved is a silent scope error: the answer reports
    whatever period the sources happened to cover and reads as responsive. Like
    the subject check this is scope, not content, so it ranks above every numeric
    stage — a figure from the wrong year is wrong however well it is grounded."""
    try:
        if (deadline - monotonic()) < TIMEFRAME_MIN_LEFT_S:
            return answer
        gaps = _unevidenced_years(_anchor_years(question), ledger)
        if not gaps:
            return answer
        queries = [_year_probe_query(question, y) for y in gaps]
        order = (
            "TIMEFRAME CHECK — the question anchors on "
            + ", ".join(gaps)
            + ", and no retrieved source covers "
            + ("those years" if len(gaps) > 1 else "that year")
            + ". Retrieve the figures for the anchored period specifically, and "
              "give each period its own citation: one source covering a "
              "neighbouring year does not establish the year that was asked "
              "about. If the anchored period genuinely has no data, say so "
              "explicitly and cite what shows it."
            + REWRITE_TAIL)
        return await _repair_cycle(question, answer, messages, ledger, deadline,
                                   queries, order)
    except Exception:
