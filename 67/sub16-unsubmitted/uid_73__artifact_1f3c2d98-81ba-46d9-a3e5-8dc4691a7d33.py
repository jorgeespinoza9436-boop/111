...
BATCHED_RETRIEVAL_PREVIEW_CHARS = 120_000
VFS_READ_PAGE_CHARS = 40_000
VFS_LEXICAL_WINDOW_CHARS = 3_600
FOCUSED_OBSERVATION_MEMORY_CHARS = 40_000
...
CITATION_REF_MAX_CHARS = 6_000
CITATION_TOTAL_MAX_CHARS = 30_000
CITATION_MIN_SLICE_CHARS = 100
...
INVESTIGATION_MODELS = ("openrouter_gemma", "openrouter_gemma_stable", "glm5")
STATE_AWARE_INVESTIGATION_MODELS = (
    "openrouter_gemma",
    "openrouter_gemma_stable",
    "glm5",
)
REQUIREMENTS_MODELS = ("openrouter_gemma", "openrouter_gemma_stable", "glm5")
REPAIR_MODELS = (
    "openrouter_gemma",
    "openrouter_gemma_stable",
    "glm5",
...
class ResearchState:
    ...
    def citation_plan(
        ...
        ordered_identities = list(citation_sources)
        identity_texts: list[str] = []
        for identity in ordered_identities:
            document = ""
            for ref, mapped in source_identities.items():
                if mapped == identity:
                    document = self.vfs.get(ref, "")
                    if document:
                        break
            identity_texts.append(document)
        # 답안 용어가 1순위, 질문 용어가 2순위다. 다단계 질의("top 5 by Mass 와
        # top 5 by Diameter 를 교집합한 뒤 Density 최대")에서는 최종 값만으로는
        # 유도 과정이 노트에 남지 않는다 — 심판이 레퍼런스를 고른 이유가 바로
        # "citation note explicitly covers the steps" 였다.
        evidence_terms = _answer_evidence_terms(answer)
        for term in _answer_evidence_terms(self.question):
            if term not in evidence_terms:
                evidence_terms.append(term)
        budgeted_slices = _budget_citation_slices(
            [citation_slices[identity] for identity in ordered_identities],
            CITATION_REF_MAX_CHARS,
            CITATION_TOTAL_MAX_CHARS,
            CITATION_MIN_SLICE_CHARS,
            identity_texts,
            evidence_terms,
        )
        citations = [
            CitationRef(
                receipt_id=citation_sources[identity].receipt_id,
                result_id=citation_sources[identity].result_id,
                slices=budgeted_slices[position],
            )
...
_EVIDENCE_STOPWORDS = frozenset(
    (
        ...
        "about", "into", "than", "then", "them", "they", "these", "those",
        "when", "what", "where", "while", "also", "such", "more", "most",
        "some", "only", "other", "over", "after", "before", "between",
        "both", "each", "under", "above", "here", "does", "did", "not",
        "answer", "value", "values", "list", "based", "according", "total",
    )
)


def _answer_evidence_terms(answer: str) -> list[str]:
    """답안이 실제로 주장하는 토큰을 뽑는다.

    구조화 응답이면 JSON 값들이, 산문이면 고유명사와 수치가 남는다. 이 토큰들이
    "어떤 슬라이스가 답을 뒷받침하는가"의 판정 기준이 된다.
    """
    terms: list[str] = []
    for token in re.findall(r"[A-Za-z][A-Za-z\-']{2,}|\d[\d,.]*", answer.lower()):
        cleaned = token.strip(".,'-")
        if len(cleaned) >= 3 and cleaned not in _EVIDENCE_STOPWORDS:
            terms.append(cleaned)
    return list(dict.fromkeys(terms))[:48]


def _span_relevance(text: str, start: int, end: int, terms: list[str]) -> int:
...
def _focused_truncate(
    ...
    """예산 초과 스팬을 자를 때 앞머리가 아니라 실제 근거 근처를 남긴다."""
    if end - start <= budget:
        return start, end
    if terms and text:
        window = text[start:end].lower()
        anchor = -1
        for term in terms:
            position = window.find(term)
            if position >= 0 and (anchor < 0 or position < anchor):
                anchor = position
        if anchor >= 0:
            new_end = min(end, start + anchor + (budget * 2 // 3))
            new_start = max(start, new_end - budget)
            if new_end > new_start:
                return new_start, new_end
    return start, start + budget


def _pin_covering_spans(
    slice_groups: list[list[CitationSlice]],
    texts: list[str],
    terms: list[str],
) -> list[set[tuple[int, int]]]:
    """각 근거 용어를 담은 스팬을 하나씩 미리 확보한다.

    예산을 관련도 순으로만 쓰면 밀도 높은 한 구간이 예산을 독식해 다른 값의 근거가
    통째로 잘려나갈 수 있다. 심판이 보상하는 것은 "노트가 답을 유도하는 단계를
    담고 있는가" 이므로, 주장한 값마다 최소 한 개의 근거 창을 먼저 못박는다.
    용어는 우선순위 순서로 처리하고 (답안 용어가 질문 용어보다 앞선다), 같은 용어를
    담은 스팬이 여럿이면 가장 짧은 것을 고른다.
    """
    pinned: list[set[tuple[int, int]]] = [set() for _ in slice_groups]
    for term in terms:
        best: tuple[int, int, int] | None = None
        for index, group in enumerate(slice_groups):
            ...
            for item in group:
                ...
                if end <= start:
                    continue
                if (start, end) in pinned[index]:
                    best = None
                    break
                if term in text[start:end].lower():
                    if best is None or (end - start) < best[0]:
                        best = (end - start, index, start)
            else:
                continue
            break
        if best is not None:
            size, index, start = best
            pinned[index].add((start, start + size))
    return pinned


def _trim_citation_slice_group(
    ...
    """하나의 인용 ref 가 실체화하는 문자수를 limit 이하로 자른다.

    terms 가 주어지면 문서 순서가 아니라 답안 근거 밀도 순으로 스팬을 고른다.
    심판이 표결에서 실제로 보는 것은 노트가 답을 유도하는 과정을 담고 있는지이므로,
    예산은 "앞에서부터 N자"가 아니라 "답을 뒷받침하는 N자"에 써야 한다.
    슬라이스는 원래 끝점을 넘겨 늘리지 않는다 (문서 밖 오프셋 방지).
    입력이 비어있지 않았다면 반드시 비어있지 않은 결과를 돌려준다 — 슬라이스가
    없는 ref 는 노트 전체가 실체화되므로 예산 적용이 오히려 손해가 된다.
    """
    ordered = sorted(
    ...
    if not ordered:
        return []
    active = terms or []
    anchors = pinned or set()
    ranked = sorted(
        ordered,
        key=lambda span: (
            0 if span in anchors else 1,
            -_span_relevance(text, span[0], span[1], active),
            span[0],
        ),
    )
    ...
    for start, end in ranked:
        ...
        if remaining < min_slice:
            break
        start, end = _focused_truncate(text, start, end, remaining, active)
        if end - start <= 0:
            continue
        kept.append((start, end))
        used += end - start
    if not kept:
        first_start, first_end = ranked[0]
        kept = [(first_start, min(first_end, first_start + max(min_slice, limit)))]
    kept.sort()
    return [CitationSlice(start=start, end=end) for start, end in kept]


def _budget_citation_slices(
    slice_groups: list[list[CitationSlice]],
    ref_limit: int,
    total_limit: int,
    min_slice: int,
    ...
    terms: list[str] | None = None,
) -> list[list[CitationSlice]]:
    """응답 전체의 실체화 예산을 명시적으로 강제한다.

    ref 당 상한을 먼저 적용하고, 그다음 총량 상한을 순서대로 배분하되 뒤에 남은
    ref 들이 쓸 최소치를 미리 예약해 어떤 ref 도 슬라이스를 전부 잃지 않게 한다.
    """
    sources = texts or ["" for _ in slice_groups]
    if len(sources) < len(slice_groups):
        sources = sources + ["" for _ in range(len(slice_groups) - len(sources))]
    anchors = _pin_covering_spans(slice_groups, sources, terms or [])
    trimmed = [
        _trim_citation_slice_group(
            group,
            ref_limit,
            min_slice,
            sources[index],
    ...
    populated = [index for index, group in enumerate(trimmed) if group]
    if not populated:
        return trimmed
    budget = max(total_limit, min_slice * len(populated))
    used = 0
    for position, index in enumerate(populated):
        pending = len(populated) - position - 1
        allowance = budget - used - (min_slice * pending)
        if allowance < min_slice:
            allowance = min_slice
        trimmed[index] = _trim_citation_slice_group(
            trimmed[index],
            allowance,
            min_slice,
            sources[index],
        ...
        used += sum(
            int(item.end) - int(item.start) for item in trimmed[index]
        )
    return trimmed
...
async def _prose_chat_with_retry(
    ...
    return await _chat_with_scheduling(
        ("openrouter_gemma", "openrouter_gemma_stable", "glm5"),
        messages,
        None,
        tool_choice,
        False,
...
def _strip_source_markup(value: Any) -> Any:
    """구조화 출력 값에서 원본 페이지의 표기 잔재를 제거한다.

    질문이 "exact literal value" 를 요구할 때 위키 각주 마커나 마크다운 강조를
    같이 실어 보내면 심판이 그것만으로 탈락시킨다. 챔피언 a7045f3e 는 구조화 런의
    10.4% 에서 이 오염을 냈고 그 구간 평균 점수는 정확히 0.000 이었다
    (예: "*Rodrigo* ( ... ) [28]" — 심판: "[28] is clearly wrong as it doesn't
    appear in the provided text evidence").

    5개 배치에서 수집한 레퍼런스 구조화 답안 55건 중 이런 마커를 포함한 것은
    0건이다. 따라서 제거는 항상 레퍼런스 표기 방향으로만 움직인다.
    우리 실측 오염률은 0/25 이므로 이건 교정이 아니라 보험이다.
    """
    if isinstance(value, str):
        ...
        # 값 전체가 표기 문자로만 이루어져 있었다면 원본을 보존한다.
        return cleaned if cleaned else value
    if isinstance(value, list):
        return [_strip_source_markup(item) for item in value]
    if isinstance(value, dict):
        return {key: _strip_source_markup(item) for key, item in value.items()}
    return value
    ...
    for attempt in range(3):
        ...
        if len(calls) != 1:
            ...
            try:
                ...
                output = _strip_source_markup(output)
...
async def _investigate(question: str, expected_answer: str) -> tuple[str, list[CitationRef]]:
    ...
    for _turn in range(160):
        ...
        governor_stage = (
            "open" if requirements_pending else _governor_stage(state, governor_elapsed)
        )
        # 증거가 하나도 없으면 조사 도구를 회수하면 안 된다. 회수해봐야
        # 답을 만들 수 없고 0점이 확정된다. 예산을 더 쓰더라도 계속 찾는다.
        if not state.sources:
            governor_stage = "open"
        if governor_stage != "open":
            governor_turns += 1
        # 마감 턴을 다 썼는데도 종결이 안 됐다. 도구를 계속 묶어두면 원본보다
        # 나쁜 결과가 된다 (원본은 전체 도구로 마저 시도했을 것이다).
        # 지출 보호는 최선노력이고 정확성이 우선이므로 원본 동작으로 강등한다.
        ...
        if (
            ...
            # 지출 또는 경과시간이 하드 문턱을 넘었다. 모델 왕복을 더 쓰지 않고
            # 지금까지 모은 증거로 하네스가 직접 종결한다.
            try:
                ...
                # 직접 종결이 불가능한 상태다 (인용 미보존 등). 여기서 예외를
                # 올리면 태스크 전체가 죽어 0점이 확정된다. soft 로 강등해
                # 모델이 마감 도구로 스스로 고치게 둔다.
        ...
        if requirements_pending:
            ...
            # soft/hard: 조사 도구를 회수한다. tool_choice 가 required 이므로
            # 모델은 마감 도구 중 하나를 고를 수밖에 없다.
