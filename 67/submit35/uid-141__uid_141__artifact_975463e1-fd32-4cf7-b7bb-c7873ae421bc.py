...
def _compose_beryl_lantern_agent_entry():
    ...
    # ----------------------------------------------------------------
    # v115-505-pac
    # Stages: premise sweep, authority sweep, citation slice backfill
    # Ordinary successful path:
    #   query -> _solve -> _knowledge_brief -> _loop -> _audit_patch -> _select_best -> _verify_subjects -> _anchor_primary_source -> _refs_within_budget -> _citations_for -> _w2_point_markers -> _answer_line_only -> Response
    # ----------------------------------------------------------------
...
def _compose_vesper_kestrel_agent_entry():
...
_vesper_kestrel_agent_query_entry = _compose_vesper_kestrel_agent_entry()


def _compose_ember_mistral_agent_entry():
    ...
    VERSION = "v202-uid24x193-rgc"
    # w5 variant 02 — added stages: roster pre-pass, set gap-fill sweep, lead-figure corroboration.
    # stage set RGC = uid 24 (RC) x uid 193 (CG)
...
_ember_mistral_agent_query_entry = _compose_ember_mistral_agent_entry()


_BALANCED_ROUTER_SEED = "7286cdb21b7fd2a65801571a"
...
def _balanced_route_label(query: Query) -> str:
    ...
    if bucket == 0:
        return "BerylLanternAgent"
    if bucket == 1:
        return "VesperKestrelAgent"
    return "EmberMistralAgent"
...
class VesperKestrelAgent:
    async def __call__(self, query: Query) -> Response:
        return await _vesper_kestrel_agent_query_entry(query)


class EmberMistralAgent:
    async def __call__(self, query: Query) -> Response:
        return await _ember_mistral_agent_query_entry(query)


_BALANCED_PRIMARY_AGENT = BerylLanternAgent()
_BALANCED_SECONDARY_AGENT = VesperKestrelAgent()
_BALANCED_TERTIARY_AGENT = EmberMistralAgent()
...
async def _s35_base_query(query: Query) -> Response:
    ...
    if selected == "BerylLanternAgent":
        branch = _BALANCED_PRIMARY_AGENT
    elif selected == "VesperKestrelAgent":
