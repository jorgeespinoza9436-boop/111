...
TURN_TIMEOUT_S = 75.0
WALL_BUDGET_S = 266.0
# v-422: mutable box so the entrypoint can retune the budget without a
# `global` statement (forbidden by the upload subset). Reads go through
# _W5_WALL[0]; the plain name above is kept as the initial value.
_W5_WALL = [266.0]
DIGEST_TAIL_S = 14.0
...
#
# Batch 81b84664 (2026-08-20), artifact 9b22efe4-c158-4b7b-a6c1-53fd2a9cd001,
# uid 111, 50 replays over the 10 qualifying tasks. Artifact mean
# 0.370: structured lane 0.243 over 7 tasks,
# free-text lane 0.667 over 3 tasks.
#
# Its five weakest tasks:
#   14126506  0.00  structured; field-wide mean 0.14 - the IFCO chart comparison, repeatedly judged an identical answer
#   9ff09d18  0.10  structured; field-wide mean 0.20
#   97f4deed  0.20  free text; field-wide mean 0.28
#   f78150bf  0.20  structured; field-wide mean 0.10 - the World Aquatics contract with a one-sentence `premise_verdict`
#   8788381c  0.30  structured; field-wide mean 0.16 - the MAIB report counts, repeatedly judged an identical answer
#
...
#     21 replays where the judge called the two answers identical
#     AND weighed citation shape scored
#     0.214; the 12 replays it called identical without
#     weighing citation shape scored 0.583. This artifact's own median
#     submitted slice is 5986 chars against 190 for the answers
...
#     0.30 on 8788381c and 0.00 on 14126506, the two tasks the
...
#     29 times against 22 for concision, so the enrichment
#     is enabled here.
#
# L4  LATE RUNS RETURNED NOTHING. Replays that ran past ~240s produced no
#     response at all: 269s. The research wall is
#     trimmed to 248.0s so the run finishes inside the harness cap.
#
    ...
    for path, value in pending[:_W5_RECOVER_FIELDS]:
        ...
        if not force_fetch:
            for page in reversed(_w5_pages()):
                ...
                if context:
                    break
        # A later round already saw what the retrieved pages say about this field
        # and the answer still does not match them, so that round goes straight
        # back out for the page rather than re-reading the same text.
...
async def _w5_anchor_board(question, schema, response, deadline):
    ...
    # The board is a loop, not a post-pass: each round audits the answer that will
    # be returned, and any field the evidence still does not print sends the run
    # back into the retrieval stage before the answer is produced again. The round
    # ends only when the audit finds nothing left to recover, the budget runs out,
    # or _W5_MAX_ROUNDS rounds have run.
    rounds = 0
    while (rounds < _W5_MAX_ROUNDS
           and (pending or (thin and anchored))
