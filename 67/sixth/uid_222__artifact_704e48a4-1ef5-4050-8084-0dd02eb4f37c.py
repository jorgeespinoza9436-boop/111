...
PROVIDER = "openrouter"
DRAFT_MODEL = "z-ai/glm-5"          # fork B usa glm-5 (deepseek testou 0,55, pior); diferenciador = piso de citação
...
CITATION_FLOOR_N = 4  # fork B: citações mínimas a anexar quando a resposta não traz nenhuma inline


def _build_citations_with_floor(answer: str, index: "_ResultIndex") -> list[CitationRef]:
    """fork B (UID222): 2ª correção derivada do juiz — uid17 perdeu tasks com resposta SEM citações
    validadas (juiz não dá crédito de evidência). Se não há [n] inline, anexa as fontes mais fortes
    já coletadas (prioriza páginas buscadas em detalhe) para o juiz ter o que creditar."""
    refs = _build_citations(answer, index)
    if refs:
        return refs
    ordered = sorted(
        index.entries.items(),
        key=lambda kv: (kv[1].get("source") != "fetch", kv[0]),  # fetched-detail primeiro, depois ordem
    )
