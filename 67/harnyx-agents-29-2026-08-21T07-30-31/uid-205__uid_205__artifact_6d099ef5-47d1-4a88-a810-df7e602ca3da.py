...
def _compose_copper_rook_entry():
...
_copper_rook_query_entry = _compose_copper_rook_entry()


def _compose_ivory_badger_entry():
    """agent_d — v32 "toolloop": model-driven research agent.
...
_ivory_badger_query_entry = _compose_ivory_badger_entry()


def _compose_velvet_swan_entry():
    _S31_QUERY_TAG = "s31-hk6723"
...
_velvet_swan_query_entry = _compose_velvet_swan_entry()


_BALANCED_ROUTER_SEED = "a674a543f2ac880cac7ef2b4"
...
class CopperRook:
    async def __call__(self, query: Query) -> Response:
        return await _copper_rook_query_entry(query)


class IvoryBadger:
    async def __call__(self, query: Query) -> Response:
        return await _ivory_badger_query_entry(query)


class VelvetSwan:
    async def __call__(self, query: Query) -> Response:
        return await _velvet_swan_query_entry(query)


_BALANCED_PRIMARY_AGENT = CopperRook()
_BALANCED_SECONDARY_AGENT = IvoryBadger()
_BALANCED_TERTIARY_AGENT = VelvetSwan()
...
async def query(query: Query) -> Response:
    ...
    if selected == "CopperRook":
        branch = _BALANCED_PRIMARY_AGENT
    elif selected == "IvoryBadger":
