...
def _compose_lumen_lantern_agent_entry():
    ...
    # No ai_gateway credential is stored, so the paid lane raised on every call and
    ...
    #   CAVEAT: those numbers were measured on the OLD paid lane (glm-5.2-fast over
    #   ai_gateway). z-ai/glm-5 has not been characterised at this boundary, so the
    #   guard is inherited, not validated. It only ever SKIPS a fallback call, so a
    #   wrong threshold costs a retry rung, never correctness — but if the glm-5 rung
    #   seems to fire less than expected on long transcripts, this is the constant.
    ...
    async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool,
        ...
        # HISTORY of the payload guard below: it was introduced when lane B was the
        # paid ai_gateway glm-5.2-fast rung, then the priciest model on the allowlist
        # (2.10/6.60 per 1M vs lane A's 0.8008/2.5168). That model returned EMPTY
        # above a payload it could not handle while still billing for the prompt --
        # 7 calls, $0.518 (17% of spend); the two that returned zero completion
        # tokens had 50,444 and 37,227 prompt tokens, while every call that produced
        # output was <= 34,196 -- so skipping it above the threshold was pure saving.
        #
        # THAT LANE IS GONE. Lane B is now z-ai/glm-5 on openrouter, which is CHEAPER
        # than lane A (-21% blended at our 32.6:1 in:out ratio), so the cost argument
        # no longer applies and the empty-above-threshold behaviour is uncharacterised
        # on this model. The guard is kept because skipping a rung is always safe --
        # it only ever costs a retry, never correctness -- but it is inherited, not
        # justified. Removing it is a clean single-variable experiment.
    ...
    #
    # No post-answer stage in this build verifies the draft (that is the pool-width
    # sweep, which lives elsewhere in the family), so the roster is advisory: it
    # steers research, and the audit's incomplete_roster probe is what checks it.
    ...
    # ── criterion coverage tracker (in-loop) ──────────────────────────────────────
    # The knowledge brief already enumerates the question's atomic conditions, but
    # that block has only ever been prompt text. Parse the criteria back out and,
    # when the loop is half spent, tell the model exactly which criteria still have
    # NO supporting ledger rows — those become uncited claims, and uncited claims
    # score zero. Detection is token overlap against stored row text: zero extra LLM
    # calls, zero extra searches, and any failure just skips the hint.
    #
    # With stage 1c this build has TWO stages acting before the answer exists, so
    # neither competes with the four post-audit sweeps for the tail.
    _CRITERION_ROW_RE = re.compile(r"^\s*(?:\d+[.)]\s*|[-*]\s+)(.{8,240})$")
    ...
    def _extract_criteria(worksheet: str) -> list[str]:
        ...
        if not worksheet:
            return []
        m = re.search(r"[#*_\s]*conditions[#*_\s]*:", worksheet, re.IGNORECASE)
        if not m:
            return []
        tail = worksheet[m.end():]
        stop = re.search(r"[#*_\s]*(?:searches|urls|LOOKUPS|PAGES)[#*_\s]*:", tail,
                         re.IGNORECASE)
        if stop:
    ...
    def _open_criteria_hint(criteria: list[str], ledger: EvidenceLedger) -> str:
        ...
        if not open_items:
            return ""
        return ("COVERAGE CHECK — the evidence gathered so far never touches these "
                "question conditions:\n- " + "\n- ".join(open_items[:HINT_CRITERIA_SHOWN]) +
                "\nAim your remaining searches at these specifically before writing "
                "the final answer: a condition with no evidence row becomes an "
                "uncited claim, and uncited claims score zero.")
    ...
    async def _loop(question: str, brief: str, ledger: EvidenceLedger,
        ...
        for turn in range(1, turn_cap + 1):
            ...
            # midpoint coverage hint: one shot, never during wrapup
            if criteria and turn == max(2, turn_cap // 2) and not finish_only:
                try:
                    ...
                    pass
                criteria = None            # fired — never hint twice
    ...
    # ── numeric scanning, used by both numeric stages ─────────────────────────────
    # The donor build defined this pattern twice under two different names with
    # byte-identical bodies. One constant here, used by both numeric stages.
    ...
    #
    # RUNS BEFORE the second-source check ON PURPOSE. The two stages partition the
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            ...
            answer, messages = await _loop(question, brief, ledger, deadline, MAX_TURNS,
                                           pool_hint=pool_hint, criteria=criteria)
        except Exception:
            answer = ""
        ...
        # FIVE sweeps share the tail. TWO of this build's seven stages run before the
        # answer exists (the pool pre-pass and the in-loop coverage hint), so they are
        # the only ones guaranteed to execute; of the five below, in practice the
        # first one or two that trigger consume the window. Treat the order as a
        # PRIORITY RANKING, not a pipeline:
        #   subject  — wrong entity makes every later check moot
        #   period   — wrong period invalidates the figures the numeric stages
        #              inspect, and its repair can replace them wholesale
        ...
        #   measure  — pure formatting, LAST because every sweep above rewrites the
        #              whole answer and would discard its annotations
        ...
        for _sweep in (_verify_subjects, _align_timeframe, _ground_figures,
    ...
    class DiscreteOffsetAccountant_fb5065:
        ...
        SPAN_CUTOFF_FB5065 = 12000          # hard ceiling for any single accounted window
        WINDOW_RESERVE_FB5065 = 96             # symmetric padding applied when widening
        SLICE_MARGIN_FB5065 = 3              # smallest sample worth reporting on
        ...
        def widen_extents_fb5065(self, rows, note_len=0):
            ...
            for row in rows or []:
                ...
                if not isinstance(label, str) or not label.strip():
                    label = "row"
                report.append(
                    "[" + str(position) + "] " + label.strip()[:60]
                    + " -> " + str(stats["windows"]) + " window(s), "
                    + str(stats["covered"]) + " chars, ratio "
                    + str(stats["ratio"])
                )
...
_lumen_lantern_agent_query_entry = _compose_lumen_lantern_agent_entry()


def _compose_cobalt_harbor_agent_entry():
    """agent_d — v32 "toolloop": model-driven research agent.
    ...
    VERSION = "v53-timeframe-measure-slice"
    ...
    # ── numeric scanning, read by the citation post-pass ─────────────────────────
    # Both donor builds defined this pattern twice under different names with
    # byte-identical bodies. One constant here. Its ONLY consumer in this build is
    # _answer_figures in the citation post-pass -- no wired sweep scans the answer
    # for numbers, so do not delete these on the assumption they went with 3s.
    _MARKER_STRIP_RE = re.compile(r"\[[0-9][0-9,\s\-]*\]")
    _NUMERIC_TOKEN_RE = re.compile(r"\$?\b\d[\d,]*(?:\.\d+)?%?")
    ...
    # regression guards, and any failure returns the answer untouched.
    #
    # RUNS BEFORE the measure stage ON PURPOSE -- see the chain comment in _solve.
    # A wrong period invalidates the very figures the measure stage would unit, and
    # this stage's repair can replace those figures wholesale; running measure
    # first would annotate values that are about to be discarded.
    _ANCHOR_YEAR_RE = re.compile(r"\b(19[0-9]{2}|20[0-2][0-9])\b")
    MAX_ANCHOR_YEARS = 3
    ...
    # mismatch spend one bounded rewrite round. No tool calls; zero cost when clean.
    #
    # RUNS LAST among the wired sweeps, and must: stage 3t above rewrites the
    # COMPLETE answer, so a unit annotation applied before it is thrown away.
    # Nothing downstream of this stage rewrites the answer text. uid 81, this
    # stage's donor, ships the reverse order (measure floor 70s, temporal floor
    # 75s) and loses exactly that; six donors in this fleet carry the same defect.
    #
    # It does interact with the citation post-pass below: that
    # pass anchors slices on figures in the FINAL answer text, so any value this
    # stage CONVERTS (km -> miles) will not be findable in a source. The order text
    # therefore insists the source's verbatim value stays alongside the converted
    # one, which keeps at least one anchorable form of every figure in the answer.
    _MEASURE_ASK_RE = re.compile(
        r"\bin (millions?|billions?|thousands?)(?: of)? (USD|EUR|GBP|dollars|euros|"
        r"pounds)\b|\bin (USD|EUR|GBP|km|kilometers|miles|meters|feet|hectares|"
        r"acres|tonnes|tons|kg|kilograms|pounds|percent|%)\b", re.IGNORECASE)
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        # ── post-audit sweep chain ────────────────────────────────────────────────
        # PERIOD, THEN PRESENTATION. Timeframe alignment runs first and the order is
        # load-bearing in both directions. Forward: a wrong period invalidates the
        # figures the measure stage would unit, and the temporal repair can replace
        # those figures wholesale. Backward: the temporal stage rewrites the COMPLETE
        # answer, so any unit annotation applied before it is discarded. uid 81 ships
        # them in that losing order (measure 70s, temporal 75s).
        #
        # Only TWO sweeps are wired, which is also the measured ceiling on how many
        # can fire -- a firing sweep costs ~42s of search envelope plus up to three
        # loop turns, and entering this chain with 230s still only funds two. Nothing
        # is ranked below the cut line. The build's other two stages sit outside the
        # contention entirely: the pool pre-pass ran before the loop, and the
        # citation post-pass runs inside _citations_for for free.
        #
        # Both triggers read the QUESTION, not the answer's numeric content: stage 3t
        # fires on literal year anchors, stage 3m on a unit/currency/scale demand.
        # Neither needs the answer to contain figures for its gate to open.
        # Each stage re-checks its own floor and returns `answer` untouched on failure.
        for _sweep in (_align_timeframe, _conform_measures):
            try:
                if not _is_usable_answer(answer):
                    break
                # the LOWEST floor of the wired sweeps (timeframe 90s, measure 70s):
                # breaking above it would skip a stage that could still run. If a
                # sweep is ever swapped in or out here, RE-DERIVE this guard --
                # leaving it at a removed stage's floor silently strands the survivor.
                if (deadline - monotonic()) <= MEASURE_FIX_MIN_LEFT_S:
                    break
                if _spend_left() <= AUDIT_MIN_USD:
                    break
    ...
    class InterimExtentTracker_ce34ce:
        """Offline bookkeeping for span coverage accounting.
        ...
        ROW_PADDING_CE34CE = 12000          # hard ceiling for any single accounted window
        NOTE_THRESHOLD_CE34CE = 96             # symmetric padding applied when widening
        BLOCK_CEILING_CE34CE = 3              # smallest sample worth reporting on

        def collate_intervals_ce34ce(self, spans, limit=0):
            """Normalise heterogeneous span entries into disjoint, sorted windows.

            Accepts (start, end) pairs or {"start": .., "end": ..} mappings, drops
            anything malformed, clamps to the ceiling, then merges overlaps.
            """
            ceiling = int(limit or self.ROW_PADDING_CE34CE)
            cleaned = []
            for item in spans or []:
                if isinstance(item, dict):
                    start_raw = item.get("start", None)
        ...
        def gather_offsets_ce34ce(self, spans, note_len, pad=0):
            """Widen merged windows by a fixed margin without crossing the note."""
            room = max(0, int(pad or self.NOTE_THRESHOLD_CE34CE))
            total = max(0, int(note_len or 0))
            widened = []
            for window in self.collate_intervals_ce34ce(spans, total or self.ROW_PADDING_CE34CE):
                left = max(0, window[0] - room)
                right = window[1] + room
                if total:
                    right = min(total, right)
        ...
        def resolve_windows_ce34ce(self, rows, note_len=0):
            """Coverage statistics over a batch of ledger-shaped row mappings."""
            covered = 0
            windows = 0
            widest = 0
            ...
            for row in rows or []:
                ...
                if not isinstance(length, (int, float)) or isinstance(length, bool):
                    length = note_len
                for window in self.gather_offsets_ce34ce(spans, int(length or 0)):
                    width = window[1] - window[0]
                    covered = covered + width
                    windows = windows + 1
                    if width > widest:
        ...
        def summarize_boundaries_ce34ce(self, rows, note_len=0):
            """One readable line per row describing how much of it is covered."""
            report = []
            position = 0
            for row in rows or []:
                ...
                if not isinstance(row, dict):
                    continue
                stats = self.resolve_windows_ce34ce([row], note_len)
                if stats["windows"] < 1:
                    continue
                label = row.get("title", "")
                if not isinstance(label, str) or not label.strip():
            ...
            if len(report) < self.BLOCK_CEILING_CE34CE:
                report.append("(coverage sample below the reporting floor)")
            return report

    return query

_cobalt_harbor_agent_query_entry = _compose_cobalt_harbor_agent_entry()


def _compose_quartz_meadow_agent_entry():
    """agent_d — v32 "toolloop": model-driven research agent.
    ...
    # ── providers / models ────────────────────────────────────────────────────────
    LLM_LANE_A = "openrouter"          # primary lane (loop + briefing)
    # f12.1 2026-08-18: THE PREMISE BELOW IS NO LONGER TRUE. An ai_gateway credential is
    # registered on the hotkey (verified `--get`: ai_gateway exists=True), so the
    # champion's own lane B is reachable and is restored verbatim. Prior reasoning, kept
    # because it is exactly what to re-apply if the credential ever lapses:
    #   "our stored platform credentials are openrouter/parallel/firecrawl/exa/tavily —
    #    ai_gateway is NOT among them, so the champion's lane B can only 402 for us. Its
    #    whole purpose is to rescue a failed lane-A call; pointed at a provider we cannot
    #    reach it converts every rescue into a second failure."
    # A 402 here does NOT fail closed: lane B is the last rung of the turn ladder, so an
    # unfunded key costs a wasted rescue, not a wrong answer. Keep the key funded.
    ...
    # tool_models/OpenRouter supported_parameters), so the loop's effort:low is a
    # no-op there. A 10-task local A/B did NOT predict the production task mix.
    # ── f12.0: PRIMARY LOOP STAYS ON GLM, AUXILIARY WORK MOVES TO GPT ────────────
    # 99.4% of our LLM spend is glm-5.2. On the platform rate card
    # (harnyx_commons/llm/pricing.py, OPENROUTER_PROVIDER, USD per Mtok in/out):
    #     z-ai/glm-5.2          0.8008 / 2.5168     blended 0.852 at our 32.6:1 ratio
    #     deepseek/deepseek-v3.2 0.269 / 0.40                0.273   (-68%)
    #     openai/gpt-oss-120b   0.037  / 0.17                0.041   (-95%)
    # gpt-oss-120b is ~21x cheaper on PROMPT, which is the term that dominates.
    #
    # WHAT IS PRIMARY AND STAYS ON GLM:
    #   _chat_turn         the research loop itself -- the thing that earns the score
    #   _write_from_digest the last-rung ANSWER WRITER. It emits final text, so it is
    #                      primary work even though it only runs on the rescue path.
    # WHAT IS AUXILIARY AND MOVES TO GPT:
    #   _knowledge_brief   a pre-search knowledge draft + search plan. Ran on glm-5.2
    #                      at max_tokens=2400 on EVERY task with budget (gated only by
    #                      BRIEF_MIN_USD and a 120 s floor), and its output never
    #                      reaches the answer -- it seeds the loop. This is the single
    #                      biggest auxiliary line in the file, hence its own constant.
    #   _knowledge_resort  knowledge-only fallback answer (RESORT_MODEL)
    #
    # WHY LOOP_MODEL_B IS DELIBERATELY *NOT* MOVED. It is the lane-B rescue for every
    # rung, and _schema_output tries SCHEMA_MODEL -> RESORT_MODEL -> LOOP_MODEL_B. With
    # RESORT_MODEL now gpt-oss-120b, moving lane B there too would make all THREE schema
    # rungs the same model on the same provider -- one model-specific failure would take
    # all of them, and on a structured query that is `miner_response_invalid`, a hard
    # zero rather than a low score. The file already flags this risk at _schema_output
    # ("Both SCHEMA_MODEL and RESORT_MODEL are lane A, so a single provider outage...").
    # Keeping deepseek-v3.2 on lane B preserves model-family diversity on exactly the
    # path that exists because something else already failed. It is also the cheap
    # choice to leave alone: lane B firing is rare, and runs that reach it score 0.09
    # mean against 0.69, so its cost is not where the money is.
    LOOP_MODEL_A = "z-ai/glm-5.2"            # PRIMARY loop -- unchanged, do not touch
    # f12.1: uid186's lane-B model, restored. `zai/glm-5.2-fast` is routable ONLY on
    # ai_gateway (tool_models.py AI_GATEWAY_PROVIDER); it is not on the openrouter or
    # chutes allowlists, which is why lane B had to change model when it changed provider.
    # COST: $2.10 in / $6.60 out per Mtok vs glm-5.2's $0.80 / $2.52 -- 2.62x, the
    # priciest model on the allowlist. That is why the three lane-scoped guards below
    # exist and must stay: LANE_B_MAX_PAYLOAD_CHARS skips the call above ~36k tokens,
    # and finish-only lane-B calls run with thinking off and max_output_tokens=6000.
    LOOP_MODEL_B = "zai/glm-5.2-fast"        # rescue lane, ai_gateway only
    BRIEF_MODEL = "openai/gpt-oss-120b"      # f12.0: was LOOP_MODEL_A (glm-5.2)
    AUDIT_MODEL = "openai/gpt-oss-120b"      # lane A
    SCHEMA_MODEL = "openai/gpt-oss-120b"     # lane A
    RESORT_MODEL = "openai/gpt-oss-120b"     # f12.0: was deepseek/deepseek-v3.2
    SEARCH_PROVIDER = "parallel"
    # The champion passes no provider_extra, paying the default retrieval tier on ~200
    # searches per batch — 36% of its $1.83. Turbo is the cheap tier on the same
    # provider. Cost is now the binding constraint: at score parity the qualifying
    # cascade needs COST_REDUCTION, which wants >=20% cheaper and non-regressing score.
    SEARCH_EXTRA = None            # f2.5: turbo DISABLED (unused; kept to ease restore)
    # MEASURED: raising this to 14 cost +12.4% against the king (search $0.199 ->
    # $0.659) for identical score and slightly worse text — Parallel bills by RESULTS,
    # not requests, so breadth is expensive and bought nothing. It also refutes the
    # idea that turbo's residual -0.033 is a retrieval-depth problem. Stay at 8.
    SEARCH_RESULTS = 8             # only search/fetch key we store
    ...
    # platform-side, so the extra characters cost the judge's reading, not our tokens
    # or latency, and nothing the model reads changes.
    # 2026-08-14: 6000 -> 6300, ON THE NO-TURBO BASE specifically.
    # Raising this only pays if there is text to slice. f2.5 removes provider_extra, so
    # search excerpts are no longer capped near turbo's ~800 chars -- which is the whole
    # reason our spans average 865 at a 91.0% thin rate while the field runs 3,798-4,694
    # at 9-24% thin. MEASURED on batch 90b78cb7, thin-rate orders the leaderboard
    # monotonically: uid159 9.2% -> 0.65, uid252 13.5% -> 0.55, uid186 24.0% -> 0.45.
    # 6300 stays inside the optimum band measured at ~5-8k (SCORE-DIAGNOSIS.md S4:
    # 500-char slices scored 0.000 on 12 schema tasks WITH CORRECT VALUES; widening took
    # schema 0.000 -> 0.417). Above the 6000 the turbo builds could never reach anyway,
    # and well under CITATION_MAX_REF_CHARS=14000 so coverage still wins the budget.
    ...
    async def _do_search(query_text: str, ledger: EvidenceLedger):
        ...
        for attempt, allow_repeat in ((query_text, False), (query_text, True),
            ...
            fired.add(attempt)
            try:
                # f2.5 EXPERIMENT (single variable vs f2.2): provider_extra REMOVED, so
                # search runs on Parallel's default tier exactly as the champion does
                # (champion line 898-899: same call, no provider_extra). SEARCH_RESULTS is 8,
                # identical to the champion's literal num=8, so this call site is now
                # byte-equivalent to the champion's. Rationale: Codex confirmed the entire
                # retrieval->citation path downstream (ref_for, page_grep, read_page, retain,
                # span padding) is behaviourally identical to the champion, so turbo is the
                # only remaining channel that can change WHICH page region we cite.
    ...
    async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool,
        ...
        # would pay for something a plain unpinned lane-A call rides out. Ordering is
        # deliberate: fast, then slow-but-working, then expensive.
        # LANE ROLE IS AN EXPLICIT FLAG, NOT PROVIDER-STRING EQUALITY. The champion
        # could tell its lanes apart because they were different providers; we point
        # lane B at openrouter too (we hold no ai_gateway credential), so `lane ==
        # LLM_LANE_B` became true for lane A as well. That silently (a) returned
        # _EMPTY_TURN on the FIRST rung for any transcript over 144k chars, killing
        # the turn outright, and (b) stripped reasoning from every finish-only lane-A
        # call — the one turn that must apply every answer rule and place every [n].
        for lane_model in ((LLM_LANE_A, LOOP_MODEL_A, True, False),
            ...
            try:
                ...
                payload = await asyncio.wait_for(llm_chat(
                    ...
                    # the loop model on the final turn would remove it from the one turn that
                    # must apply every answer rule and place every [n].
                    thinking=({"enabled": False} if (finish_only and is_lane_b)
                              else {"enabled": True, "effort": "low"}),
                    max_output_tokens=6000 if (finish_only and is_lane_b) else None,
                    provider_extra=_upstream(lane, model) if pinned else None,
                    timeout=timeout,
                ), timeout=min(timeout + 6.0,
                               max(1.0, deadline - monotonic() - 1.0)))
    ...
    async def _knowledge_brief(question: str) -> tuple[str, str]:
        ...
        raw = ""
        try:
            # f12.0: BRIEF_MODEL, not LOOP_MODEL_A. _least_think must be asked about
            # the model actually being called -- gpt-oss is in _REASONING_MANDATORY
            # (effort:none HARD 400s "Reasoning is mandatory"), so passing glm's think
            # payload here would 400 every brief and silently fall through to lane B.
            raw = await _chat_simple(LLM_LANE_A, BRIEF_MODEL, system, user,
                                     max_tokens=2400, timeout=BRIEF_TIMEOUT_S,
                                     think=_least_think(LLM_LANE_A, BRIEF_MODEL))
        except Exception:
            try:
                raw = await _chat_simple(LLM_LANE_B, LOOP_MODEL_B, system, user,
                                         max_tokens=2400, timeout=BRIEF_TIMEOUT_S,
    ...
    # ── CITATION POINTERS (platform contract, 2026-08-18) ──────────────────────
    # `[[n]]` is an EXACT pointer to validated_citations[n-1], one-based, and is valid only
    # when that position resolves. Plain `[n]` is ordinary content -- harmless, backing
    # nothing. The trap: `n` in our answer is an EVIDENCE-LEDGER ROW, while the array we
    # ship is FILTERED and COMPACTED (_citations_for drops rows whose receipt failed, drops
    # rows over EVIDENCE_CHAR_BUDGET, caps at CITATION_CAP), so position != n except by
    # luck. Measured on batch 6ee5a9bf: 2b97ace6 cited [42]/[43] against a TWO-element
    # array, 43a05fcd cited [41] against ten. Promoting verbatim turns harmless text into
    # INVALID pointers; renumbering only single brackets misses the ones the model writes
    # itself. So collapse to one form FIRST, then renumber against the shipped array.
    _CITE_DOUBLE_RE = re.compile(r"\[\[(\d{1,3}(?:\s*[,\-]\s*\d{1,3})*)\]\]")
    _CITE_SINGLE_RE = re.compile(r"(?<!\[)\[(\d{1,3}(?:\s*[,\-]\s*\d{1,3})*)\](?!\])")
    ...
    def _pointerize_citation_markers(text: str, order: list[int]) -> str:
        ...
        NOT IDEMPOTENT by construction: the collapse step cannot tell a ledger number from
        a position, so a second application would remap positions as if they were rows.
        There is exactly ONE call site. Do not add another."""
        if not text:
            return text
        text = _CITE_DOUBLE_RE.sub(lambda m: "[%s]" % m.group(1), text)
        if not order:
            # nothing shipped: leave plain [n], which the contract reads as ORDINARY TEXT.
            return text
    ...
    def _citations_for(answer: str, ledger: EvidenceLedger) -> tuple[list[CitationRef], list[int]]:
        ...
        cost, so budget it explicitly instead of hoping."""
        refs: list[CitationRef] = []
        kept: list[int] = []      # the ledger rows we SHIP, in array order
        spent = 0
        # Cap what we KEEP, not what we consider: slicing the candidates first made
        # cheap refs beyond position 24 unreachable even with budget to spare, and
        # the one-line-per-member rule pushes distinct [n] counts well past 24.
    ...
    # ── SCHEMA OUTPUT DISCIPLINE (f15.0) ─────────────────────────────────────────
    # Six of twenty MAIN answers in batch 6ee5a9bf were schema DEFECTS, not capability
    # failures -- every one a unanimous 0.00:
    #   DIGEST TEXT IN A FIELD (3). 41b7223c shipped {"rfc_title": "Best-supported
    #     findings from the sources retrieved:\n- STD 1 subseries contains 0 RFCs: ..."}
    #     against a reference of {"rfc_title": "Internet Official Protocol Standards"}.
    #     ROOT CAUSE: the guard was `if basis is not answer`, keyed on OBJECT IDENTITY,
    #     so once the rescue ladder had set `answer` to the digest itself the guard was
    #     False and _coerce_to_schema pasted it in verbatim. The protection existed and
    #     the commonest path walked straight past it.
    #   ALL-EMPTY SHELL (3). afe9236e shipped {"title":"","pool_count":0,...} -- schema
    #     valid, information free, scores exactly like a violation while looking like a
    #     success.
    # Identity told us where a value came from; content tells us what it IS, which is
    # what the judge sees.
    _SCHEMA_DIGEST_MARKERS = (
    ...
    def _is_digest_text(value: object) -> bool:
        ...
        can arrive mid-string once _answer_line_only or _strip_lead_narration has trimmed
        a lead. Multi-line bullet prose is a digest wherever it begins."""
    ...
    def _schema_value_is_empty(value: object) -> bool:
        ...
        if isinstance(value, bool):
            return False              # False is a real answer
    ...
    # ── FIELD-FILL GUARDS ────────────────────────────────────────────────────────
    # A schema object can be valid, non-empty, digest-free and still be a fill FAILURE.
    # Two signatures, both measured on 2026-08-18 and both deterministic.
    def _is_field_prose(value: object) -> bool:
        """True when an ATOMIC field holds multi-line text. Every reference answer for
        these tasks is single-line; a field carrying two or more lines is page text that
        was pasted in, not a value. Deliberately SEPARATE from _is_digest_text, which is
        also applied to whole answers where multi-line is normal and must stay allowed."""
        if not isinstance(value, str):
            return False
        return len([ln for ln in value.splitlines() if ln.strip()]) >= 2


    def _schema_numeric_monoculture(value: object) -> bool:
        """True when three or more numeric fields carry the SAME value -- the extractor
        stamping one salient number into every slot. 6 of 6 observed cases scored 0.00.
        Bools are excluded: a set of flags legitimately repeats."""
    ...
    def _schema_is_defective(value: object) -> bool:
        """Empty shell, numeric monoculture, or prose in an atomic field."""
        if _schema_value_is_empty(value):
            return True
        if _schema_numeric_monoculture(value):
            return True
        if isinstance(value, dict) and any(_is_field_prose(v) for v in value.values()):
            return True
        return False
    ...
    def _scrub_schema_digest(value):
        ...
        if isinstance(value, str):
            if not _is_digest_text(value):
                # multi-line text in a field is pasted page content; keep only a head
                return value.splitlines()[0].strip() if _is_field_prose(value) else value
    ...
    async def query(query: Query) -> Response:
        ...
        # PRE-EXISTING CHAMPION DEFECT. Both of these paths returned text, and a
        # structured query answered with `text` is not a low score — the platform
        # rejects the whole response (miner_response_invalid), a hard zero. The
        # champion's own source states the rule and then violates it on its two
        # error paths.
        if not question:
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        # after _citations_for: the citation array keeps the proof section's [n]
        answer = _answer_line_only(answer, question)
        # LAST transform on the text the judge reads: after _citations_for so the array
        # (whose ORDER the pointers index) exists, and after _answer_line_only so a line
        # required to be bare stays bare.
        ...
        if query.output_schema is not None:
            ...
            if structured is not None:
                try:
                    ...
                    pass
                # f15.0: the extractor itself can echo digest prose into a string field,
                # and _verbatim_structured does not check for it.
                try:
                    structured = _scrub_schema_digest(structured)
                except Exception:
                    pass
                if _schema_value_is_empty(structured):
                    structured = None      # an empty shell is not an answer
                elif _schema_is_defective(structured) and ledger.rows:
                    # a valid, non-empty, digest-free object can still be a FILL FAILURE.
                    # Spend one extraction off the deterministic digest and take it only
                    # if it is genuinely better -- never discard what we already have.
                    try:
                        refilled = await _schema_output(
                            question, _deterministic_answer(question, ledger),
                            query.output_schema, deadline)
                    except Exception:
                        refilled = None
                    if refilled is not None and not _schema_is_defective(refilled):
                        structured = refilled
                try:
                    return Response(output=structured, citations=citations or None)
                except Exception:
            ...
            if basis is not answer:
                ...
                if salvaged is not None:
                    try:
                        ...
                        pass
            # f15.0: CONTENT, not identity. The old guard was `if basis is not answer`,
            # so a digest that arrived AS `answer` -- the commonest path once the rescue
            # ladder has run -- skipped undigesting entirely and was pasted into the field
            # verbatim. 3 of 20 main tasks died exactly this way.
            if _is_digest_text(basis):
                cleaned = _undigest_for_schema(basis)
                basis = cleaned if cleaned else ""
            try:
                forced = _coerce_to_schema(_cap(basis), query.output_schema)
                # f15.0: a schema-VALID but information-FREE object scores like a
                # violation while looking like a success. While the ledger still holds
                # evidence, spend one extraction on it rather than ship the shell.
                if _schema_is_defective(forced) and ledger.rows:
                    try:
                        rescued = await _schema_output(
                            question, _deterministic_answer(question, ledger),
                            query.output_schema, deadline)
                    except Exception:
                        rescued = None
                    if rescued is not None and not _schema_is_defective(rescued):
                        forced = rescued
                # last line of defence: no digest prose reaches a schema field
...
_quartz_meadow_agent_query_entry = _compose_quartz_meadow_agent_entry()


_BALANCED_ROUTER_SEED = "e83a066254c8d2031b6e936a"
...
def _balanced_route_label(query: Query) -> str:
    ...
    if bucket == 0:
        return "LumenLanternAgent"
    if bucket == 1:
        return "CobaltHarborAgent"
    return "QuartzMeadowAgent"


class LumenLanternAgent:
    async def __call__(self, query: Query) -> Response:
        return await _lumen_lantern_agent_query_entry(query)


class CobaltHarborAgent:
    async def __call__(self, query: Query) -> Response:
        return await _cobalt_harbor_agent_query_entry(query)


class QuartzMeadowAgent:
    async def __call__(self, query: Query) -> Response:
        return await _quartz_meadow_agent_query_entry(query)


_BALANCED_PRIMARY_AGENT = LumenLanternAgent()
_BALANCED_SECONDARY_AGENT = CobaltHarborAgent()
_BALANCED_TERTIARY_AGENT = QuartzMeadowAgent()
...
async def query(query: Query) -> Response:
    ...
    if selected == "LumenLanternAgent":
        branch = _BALANCED_PRIMARY_AGENT
    elif selected == "CobaltHarborAgent":
