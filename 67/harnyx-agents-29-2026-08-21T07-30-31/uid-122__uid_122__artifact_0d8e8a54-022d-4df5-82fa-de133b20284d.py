...
def _build_agent_2():
    ...
    def _row_domain(url: str) -> str:
        m = re.match('https?://(?:www\\.)?([^/]+)', (url or '').strip(), re.I)
    ...
    async def _extract_claims(question: str, answer: str, deadline: float) -> list[str]:
        probe = f'List the load-bearing factual claims this answer stakes its verdict on — the statements that, if wrong, flip or void the answer. JSON only: {{"claims": ["<one claim per entry, specific, with its numbers/names>"]}}. At most {BOARD_MAX_CLAIMS}. No commentary.\n\nQuestion:\n{question}\n\nAnswer:\n{answer[:9000]}'
        raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, 'Claim extractor. JSON only.', probe, max_tokens=1200, timeout=max(8.0, min(20.0, deadline - monotonic() - 40.0)))
        raw = re.sub('^```(?:json)?\\s*|\\s*```$', '', raw.strip(), flags=re.I | re.M)
        data = json.loads(raw)
    ...
    transcript or the flat digest."""
        ...
        for entry in board:
            for n in entry.get('fresh', ()):
                ...
                seen.add(n)
                row = ledger.rows[n - 1]
                fresh.append(f"[{n}] {row.get('title') or ''} ({row.get('url') or ''})\n    {(row.get('preview') or '')[:550]}")
        out = 'CLAIM BOARD:\n' + '\n'.join(lines)
        if fresh:
            out += '\n\nFRESH VERIFICATION EVIDENCE (cite by these [n]):\n' + '\n'.join(fresh)
        return out

    async def _corroborate(question: str, answer: str, ledger: EvidenceLedger, deadline: float) -> str:
    ...
    rewrite. Any failure returns the incoming answer unchanged."""
        ...
        for claim in claims:
            ...
            board.append({'claim': claim, 'terms': terms, 'rows': rows, 'standing': _claim_standing(domains), 'fresh': []})
        weak = [e for e in board if e['standing'] == 'unsupported'] + [e for e in board if e['standing'] == 'single_source']
        probed = False
        upgraded = False
        for entry in weak[:BOARD_MAX_PROBES]:
            ...
            try:
                out = await asyncio.wait_for(_do_search(' '.join(entry['claim'].split()[:16]), ledger), timeout=min(22.0, left - 30.0))
        ...
        if left < 26.0:
            return answer
        order = f'Question: {question}\n\n{_board_digest(board, ledger)}\n\nCurrent answer:\n{answer[:9000]}\n\nRewrite the FINAL answer using the claim board: keep corroborated claims and add the fresh corroborating [n] beside the existing ones; fix any claim the fresh evidence contradicts, citing the fresh [n]; state unsupported claims only with the hedge the evidence allows, or drop them if nothing supports them. Plain prose, first words are the answer entities, every factual claim carries its [n], then the short proof section. Never invent an [n].'
        try:
            rewritten = await _chat_simple(LLM_LANE_A, LOOP_MODEL_A, 'You finalize answers strictly from the numbered evidence given.', order, max_tokens=2600, timeout=max(12.0, min(38.0, left - DIGEST_TAIL_S)))
        except Exception:
            return answer
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            ...
            pass
        try:
            if _is_usable_answer(answer) and deadline - monotonic() > BOARD_MIN_S and (_spend_left() >= BOARD_MIN_USD):
