...
def _compose_opal_aurora_agent_entry():
...
_opal_aurora_agent_query_entry = _compose_opal_aurora_agent_entry()


def _compose_cedar_kite_agent_entry():
    import asyncio
    from time import monotonic
    from harnyx_miner_sdk.decorators import entrypoint
    from harnyx_miner_sdk.query import Query, Response
    ...
    """
        ...
        regression, so only take a candidate that is usable AND not much shorter."""
            ...
            # ── AUDITORIA TEMPORAL (portada da familia uid35, 19/08) ─────────────────────
            # Mesmo template que comprou notable_change no f163_r3 (confirmado on-chain
            # 19/08): detector proprio + busca dirigida propria + reentrada limitada +
            # guarda _adopt_patch. Detector: a pergunta esta ancorada num ANO e NENHUMA
            # linha citada menciona esse ano -> os valores citados podem ser de outro
            # periodo, que pontua como errado. Porte AUTOCONTIDO (so usa simbolos presentes
            # em ambos os escopos: _key_terms, _cited_numbers, _do_search,
            # _commit_tool_output, _loop, _adopt_patch, _spend_left).
            _AT_YEAR_RE = re.compile(r"\b(?:19|20)\d{2}\b")
            AT_MAX_YEARS = 4
            AT_MIN_LEFT_S = 90.0

            def _at_years(question: str) -> list:
                out = []
                for y in _AT_YEAR_RE.findall(question or ""):
                    if y not in out:
                        out.append(y)
                return out[:AT_MAX_YEARS]

            def _at_uncovered(question: str, answer: str, ledger) -> list:
                years = _at_years(question)
                if not years:
                    return []
                textos = []
                ...
                if not textos:
                    return []
                return [y for y in years if not any(y in t for t in textos)]

            def _at_query(question: str, year: str) -> str:
                kt = _key_terms(question)
                toks = [w for w in re.findall(r"[A-Za-z][A-Za-z0-9\-]{2,}", question or "")
                        if w.lower() in kt and w != year][:7]
                return " ".join(toks) + f" {year}"

            async def _align_timeframe(question: str, answer: str, messages: list,
                                       ledger, deadline: float) -> str:
                if (deadline - monotonic()) < AT_MIN_LEFT_S or _spend_left() <= AUDIT_MIN_USD:
                    return answer
                uncovered = _at_uncovered(question, answer, ledger)
                if not uncovered:
                    return answer
                year = uncovered[0]
                try:
                    found = await asyncio.wait_for(_do_search(_at_query(question, year), ledger),
        ...
        that carries an [n]: that is a real answer, however it opens."""
            ...
            async def _solve(query: Query, question: str) -> Response:
                ...
                try:
                    ...
                    pass
                for _sweep in (_align_timeframe, _ground_figures, _conform_measures):
                    try:
        ...
        that carries an [n]: that is a real answer, however it opens."""
            ...
            async def _solve(query: Query, question: str) -> Response:
                ...
                try:
                    ...
                    pass
                for _sweep in (_verify_subjects, _align_timeframe, _widen_pool, _anchor_primary_source, _second_source_check, _conform_measures):
                    try:
                        if not _is_usable_answer(answer):
                            break
                        if deadline - monotonic() <= MEASURE_FIX_MIN_LEFT_S:
...
_cedar_kite_agent_query_entry = _compose_cedar_kite_agent_entry()


_BALANCED_ROUTER_SEED = "7e42a91d55c6b0f38a2d4c19"
...
def _balanced_route_label(query: Query) -> str:
    ...
    return "OpalAuroraAgent" if bucket < 128 else "CedarKiteAgent"


class OpalAuroraAgent:
    async def __call__(self, query: Query) -> Response:
        return await _opal_aurora_agent_query_entry(query)


class CedarKiteAgent:
    async def __call__(self, query: Query) -> Response:
        return await _cedar_kite_agent_query_entry(query)


_BALANCED_PRIMARY_AGENT = OpalAuroraAgent()
_BALANCED_SECONDARY_AGENT = CedarKiteAgent()
_CANDIDATE_BRANCH_CLASS_NAMES = ("OpalAuroraAgent", "CedarKiteAgent")
