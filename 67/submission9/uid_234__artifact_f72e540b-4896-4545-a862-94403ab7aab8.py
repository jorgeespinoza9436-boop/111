...
GROUND_MAX = 4                # bare-answer rescue: authoritative sources to attach when the model emits no [n]
...
SYSTEM_PROMPT = (
    ...
    "NAMED-SOURCE RULE: When the question names WHERE the answer must come from -- a specific site, table, database, or organization ('according to Wikipedia\'s List of X table', 'per Hockey-Reference', 'Box Office Mojo', 'the 2011 Census', 'Basketball-Reference', 'Alliance for Audited Media') -- the answer is GRADED against THAT source: a value from an aggregator (worldpopulationreview, statista, hockeydb, a news recap, a summary site) when the named source was reachable scores ZERO. Reach the named source in the FEWEST calls: (1) put the named source name IN your search query so its own page ranks first (add 'wikipedia', 'Box Office Mojo', or the org name to the query); (2) if the search excerpt already shows the needed figure from that source, cite it -- NO page fetch needed; (3) if you must open a page for a table, fetch the NAMED source's page, NOT an aggregator -- redirect your ONE fetch to the right target; never fetch an aggregator AND the named source; (4) if the named source is genuinely unreachable (paywalled, login-walled, no fetchable page, or it times out), fall back to the most reliable aggregator that reports the SAME figure and note which source you used -- a correct fallback beats returning nothing. Reach the right source; do not multiply round-trips.\n\n"
    ...
    "CITATION FLOOR (critical): your FINAL ANSWER must contain at least one [n] marker. A bare answer "
    "with no [n] -- even a single correct word, name, city, or number -- receives ZERO credit no "
    "matter how correct it is, because a search-dependent claim with no citation is treated as "
    "unsupported. For a one-item answer, still put the [n] of the source that proves it directly "
    "after the item: 'Allahabad [7]'. Never end without at least one bracketed source.\n\n"
    "TIGHTNESS RULE: state each fact exactly once -- never restate a number, name, or conclusion you already gave. No filler, no recap paragraph, no 'in summary'. For a single-item answer, the core resolution is one or two sentences; add supporting cited lines only for the parts the question explicitly asks to prove. A tight answer beats an equally-correct answer that repeats or pads.\n\n"
...
def _ground_bare_answer(
    answer_text: str, index: _ResultIndex
) -> tuple[str, tuple[CitationRef, ...]]:
    """Deterministic citation floor. At temp>0 the model intermittently emits a
    correct-but-bare FINAL ANSWER with no [n] markers (e.g. 'Allahabad.'); the
    inline-marker path then attaches nothing and the pairwise judge scores it 0 for
    an unsupported claim. Rather than let citation grounding depend on the sampling
    dice, append markers for the most authoritative sources ALREADY gathered for
    this question -- fetched pages first (the model deliberately opened them),
    then search results -- converting a bare answer into a cited one. A cited
    correct answer beats an uncited correct answer; a cited wrong answer scores no
    worse than a bare wrong answer, so this is a strict floor."""
    all_nums = sorted(index._by_number)
    def has_text(n: int) -> bool:
        return int((index.get(n) or {}).get("note_len", 0)) > 0
    fetched = [n for n in all_nums if has_text(n)
               and int((index.get(n) or {}).get("width", 0)) >= FETCH_CONTENT_CHARS]
    searched = [n for n in all_nums if has_text(n) and n not in fetched]
    chosen = (fetched + searched)[:GROUND_MAX]
    if not chosen:
        return answer_text, ()
    grounded = (answer_text or "").rstrip()
    if grounded and grounded[-1] not in ".!?":
        grounded += "."
    grounded = f"{grounded} {''.join(f'[{n}]' for n in chosen)}".strip()
    return grounded, _citations_from_inline_markers(grounded, index)
...
async def query(query: Query) -> Response:
    ...
    try:
        ...
        if not citations:
            # model emitted a bare answer with no usable [n] -> guaranteed 0.
            # Ground it deterministically so citations never depend on sampling.
            final_answer, citations = _ground_bare_answer(final_answer, index)
...
_TAG="6e75c6d77ecf419db9dd1363085cb9fe"
