"""SN67 Harnyx miner — v25 deep-research pipeline. [slot 09 build 2026-07-29T15:00:00+00:00]"""
...
async def _s9_pipeline_claim_seeds(question: str, *, deadline: float) -> tuple[str, ...]:
    """Claim-sheet seed queries — retrieval planning control change for pipelines."""
    if deadline - perf_counter() < 30:
        return ()
    ...
    try:
        result = await llm_chat(
            ...
            model=_model,
            messages=[
                {"role": "system", "content": 'JSON only: {"queries":["..."]} 2-5 searches covering every subclaim/comparison side.'},
        ...
        if not isinstance(qs, list):
            return ()
        return tuple(str(q).strip() for q in qs if str(q).strip())[:5]
    except Exception:
        return ()



async def _s9_contradiction_coverage_gate(question, answer, messages, store, *, deadline):
    digest = ""
    try:
        digest = str(store)[:2000]
    except Exception:
        digest = ""
    return await _s9_pipeline_contradiction_rewrite(question, answer, digest, deadline=deadline)

async def _s9_pipeline_contradiction_rewrite(
    question: str, answer: str, evidence_digest: str, *, deadline: float
) -> str:
    """Post-synthesis contradiction pass — verification mechanism change."""
    ...
    try:
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "You are a contradiction/coverage rewriter. If evidence packets disagree, "
                        "prefer official/primary values, cite both sides, reconcile. Cover every "
                        "asked subclaim. Prefer partial cited answers over refusal. Keep [n] citations. "
        ...
        cand = (result.response.raw_text or "").strip()
...
def _build_final_answer_messages(
    *, question: str, accepted_packets: tuple[AcceptedEvidence, ...],
    ...
    system_content = (
        ...
        "\n\n## S9 Contradiction Gate\n\n"
        "If accepted packets disagree on the same entity/metric, prefer official/primary "
        "evidence, cite both, and state the reconciled value. Cover each comparison side "
        "plus the conclusion; prefer partial cited coverage over refusal."
    )
...
async def query(query: Query) -> Response:
    # S9: stash claim-sheet queries for retrieval planning
    try:
        try:
            _s9_budget = float(TASK_TOTAL_BUDGET_SECONDS)
        except NameError:
            _s9_budget = 270.0
        _s9_dl = perf_counter() + _s9_budget
        _S9_CLAIM_STATE["queries"] = await _s9_pipeline_claim_seeds(query.text, deadline=_s9_dl)
    except Exception:
        _S9_CLAIM_STATE["queries"] = ()
...
# slot: 09 A10 2026-07-29T15:00:00+00:00
