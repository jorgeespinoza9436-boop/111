...
def _compose_cobalt_harbor_agent_entry():
    ...
    # ----------------------------------------------------------------
    # v115-517-azc
    # Stages: authority sweep, corroborate, citation slice backfill
    # Ordinary successful path:
    #   query -> _solve -> _knowledge_brief -> _loop -> _audit_patch -> _select_best -> _anchor_primary_source -> _second_source_check -> _refs_within_budget -> _citations_for -> _w2_point_markers -> _answer_line_only -> Response
    # ----------------------------------------------------------------
    ...
    async def _anchor_primary_source(question: str, answer: str, messages: list[dict],
        ...
        return await _stage_rewrite(question, answer, messages, ledger, deadline,
                                    order,
                                    " ".join(question.split())[:150] + " official site:gov")
...
def _compose_orchid_river_agent_entry():
...
_orchid_river_agent_query_entry = _compose_orchid_river_agent_entry()
...
def _compose_cobalt_slate_agent_entry():
    ...
    # ── v115a citation binder ─────────────────────────────────────────────────────
    BIND_MIN_LEFT_S = 30.0        # binding is deterministic; below this only the spans are kept
    BIND_PROBE_MIN_LEFT_S = 70.0  # a probe is a search plus a page
    BIND_MAX_PROBES = 2
    BIND_REWRITE_TIMEOUT_S = 40.0
    BIND_MARGIN_CHARS = 220
    BIND_MAX_SPANS_PER_ROW = 10
    BIND_MIN_SPAN_CHARS = 1600    # a bound span pads to this; the page's own windows stay
    BIND_MAX_CLAIMS = 40
    BIND_CONTEXT_CHARS = 700      # a figure found elsewhere must sit among the claim's terms
    BIND_MIN_DIGITS = 4           # shorter figures bind only where the draft already cites
        ...
        def ref_for(self, number: int) -> CitationRef | None:
            ...
            if spans:
                ...
                for a, b in (row.get("retained") or []) + (row.get("bound") or []):
                    ...
                    retained.append([a, b])
                if retained:
                    # bound spans carry the draft's figures; the page's own windows
                    # stay beside them so the figure keeps its table and heading
    ...
    # ── v115a citation binder ─────────────────────────────────────────────────────
    # The draft's citations are BOUND to the evidence by code before they are
    # minted: every figure, code and name a cited sentence states is located in the
    # text of the row(s) it cites. A claim whose figures live in a different row is
    # re-pointed at that row; a claim whose figures are in no row sends the binder
    # out for fresh retrieval (a search built from the claim, then the best new
    # page) and, when the board changed, the answer is regenerated against the
    # binding report. The bound spans -- not the fetch windows -- are what the
    # citations then materialize.
    _BIND_FIGURE_RE = re.compile(r"(?<![\w.])\d[\d,]*(?:\.\d+)?%?(?![\w])")
    _BIND_CODE_RE = re.compile(
        r"(?<![\w/.\-])(?=[\w/.\-]*\d)(?=[\w/.\-]*[A-Za-z])[A-Za-z0-9][\w/.\-]{2,}(?![\w/.\-])")
    _BIND_NAME_RE = re.compile(
        r"\b[A-Z][\w'’.\-]+(?:\s+(?:(?:of|de|the|and|&|van|von|la|le|du|des)\s+)?[A-Z][\w'’.\-]+)+")
    _BIND_YEAR_RE = re.compile(r"^(?:19|20)\d{2}$")
    ...
    def _bind_tokens(sentence: str) -> tuple[list[str], list[str]]:
        ...
        for tok in _BIND_FIGURE_RE.findall(body):
            ...
            if len(t.replace(",", "").rstrip("%")) < 2 and "." not in t:
                continue                      # single digits: list markers, ordinals
            if t not in figures:
                figures.append(t)
    ...
    def _bind_locate(text: str, token: str) -> int:
        ...
        for form in forms:
            if form[:1].isdigit():
                m = re.search(r"(?<![\d.])" + re.escape(form) + r"(?![\d])", text)
                if m:
                    return m.start()
            else:
                i = text.find(form)
        ...
        def build(self, answer: str) -> None:
            ...
            for sent in re.split(r"(?<=[.!?])\s+|\n+", _normalize_brackets(answer or "")):
                ...
                if not figures and not names:
                    continue
                self.claims.append({"text": sent, "cited": cited, "figures": figures,
                                    "names": names, "found": {}, "elsewhere": {},
            ...
            for claim in self.claims:
                ...
                for tok in claim["figures"]:
                    if tok in claim["found"] or not _bind_distinctive(tok):
                        continue
                    where = None
                    order = cited_anywhere + [n for n in range(len(rows), 0, -1)
                                              if n not in cited_anywhere]
                    for n in order:
                        row = rows[n - 1]
    ...
    async def _bind_probe(claim: dict, question: str, ledger: EvidenceLedger,
                          deadline: float) -> None:
        ...
        terms = sorted(_key_terms(_CITE_NUM_RE.sub(" ", claim["text"])),
        ...
        for n in new_rows:
            ...
            if any(_bind_locate(text, tok) >= 0 for tok in claim["missing"]):
                return                        # the excerpt states it; bind() finds it
        ...
        for n in new_rows:
            ...
            try:
                out = await asyncio.wait_for(
                    _do_fetch(url, " ".join(claim["missing"][:2]), question, ledger),
    ...
    _BIND_REWRITE_RULES = (
        "Citation binder. You rewrite a research answer so that every claim cites "
        "the numbered result that actually STATES its figures. You receive the "
        "answer and a binding report listing the claims whose figures were not found "
        "in the results they cite, with the result that does state them when one "
        "exists. Rewrite the COMPLETE answer: keep every figure, name, date, verdict "
        "and the required shape exactly as written; change only which [n] follows a "
        "claim, and for a claim stated in no result keep the claim but cite the "
        "closest supporting result. Never add a claim, never drop one, never narrate "
        "the report. Output only the rewritten answer."
    )


    async def _bind_citations(question: str, answer: str, ledger: EvidenceLedger,
                              deadline: float) -> str:
        ...
        for claim in board.open_claims():
            if (probes >= BIND_MAX_PROBES or (deadline - monotonic()) < BIND_PROBE_MIN_LEFT_S
                    or _spend_left() < AUDIT_MIN_USD):
                break
            probes += 1
            await _bind_probe(claim, question, ledger, deadline)
        if probes:
            board.bind()                      # the fresh rows may now state what was missing
        if not board.misbound() and not board.open_claims():
            return answer
        repointed = board.repoint(answer)
        report = board.report()
        if (not report or (deadline - monotonic()) < BIND_MIN_LEFT_S + 10.0
                or _spend_left() < AUDIT_MIN_USD):
            return repointed
        ask = f"Question:\n{question}\n\nBINDING REPORT:\n{report}\n\nANSWER:\n{repointed[:14000]}"
        try:
            rewritten = await _chat_simple(LLM_LANE_A, LOOP_MODEL_A, _BIND_REWRITE_RULES, ask,
                                           max_tokens=3200,
                                           timeout=max(8.0, min(BIND_REWRITE_TIMEOUT_S,
                                                                (deadline - monotonic()) - 12.0)))
        except Exception:
            return repointed
        rewritten = (rewritten or "").strip()
        if (not _is_usable_answer(rewritten) or len(rewritten) < int(len(repointed) * 0.6)
                or _bind_unmakes(repointed, rewritten)):
            return repointed
        # the rewrite may move markers only within the bound set: every [n] of the
        # re-pointed draft survives, and no [n] outside it appears
        top = len(ledger.rows)
        ...
        check.bind()
        if (len(check.open_claims()) + len(check.misbound())
                > len(board.open_claims()) + len(board.misbound())):
            return repointed
        return rewritten
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        # v115a: bind the draft's figures to the rows it cites; re-point, probe,
        # regenerate when the board says the citations do not carry the claims
        try:
            if _is_usable_answer(answer) and ledger.rows \
                    and (deadline - monotonic()) > BIND_MIN_LEFT_S:
    ...
    # --- build marker 3cb55cfe -------------------------------------------------
    _BUILD_MARKER_3cb55cfe = "20260824T000000Z"


    def _build_probe_3cb55cfe(seed: int = 0) -> int:
        """Unreferenced helper carrying the build identity."""
        acc = seed
        for i, ch in enumerate(_BUILD_MARKER_3cb55cfe):
            acc = (acc * 31 + ord(ch) + i) % 1000003
        return acc


    class _BuildStamp_3cb55cfe:
        tag = _BUILD_MARKER_3cb55cfe

        def digest(self) -> int:
            return _build_probe_3cb55cfe(len(self.tag))
...
_BALANCED_ROUTER_SEED = "30fd843b0fa7d07190c58667"
...
def _balanced_route_label(query: Query) -> str:
    ...
    if bucket == 1:
        return "OrchidRiverAgent"
    return "CobaltSlateAgent"
...
class OrchidRiverAgent:
    async def __call__(self, query: Query) -> Response:
        return await _orchid_river_agent_query_entry(query)
...
_BALANCED_PRIMARY_AGENT = CobaltHarborAgent()
_BALANCED_SECONDARY_AGENT = OrchidRiverAgent()
_BALANCED_TERTIARY_AGENT = CobaltSlateAgent()
...
async def _s36_base_query(query: Query) -> Response:
    ...
    if selected == "CobaltHarborAgent":
        branch = _BALANCED_PRIMARY_AGENT
    elif selected == "OrchidRiverAgent":
