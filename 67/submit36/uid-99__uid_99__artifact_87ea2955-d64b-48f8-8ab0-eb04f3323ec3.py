...
def _compose_saffron_meridian_entry():
...
_saffron_meridian_query_entry = _compose_saffron_meridian_entry()


def _compose_copper_tern_entry():

    _S222G_QUERY_TAG = "s222g-hk6730"  # per-hotkey canonical uniqueness
    ...
    # w5 variant 11 — added stages: roster pre-pass, lead-figure corroboration, citation consolidation.
    # stage set RCX = uid 24 (RC) x uid 89 (RX)
    ...
    # --- stage X: citation consolidation post-pass (uid 89 lineage) ---
        ...
        for n in _cited_numbers(answer, len(ledger.rows)):
            ...
            try:
                rebuilt = CitationRef(
                    ...
                    slices=[CitationSlice(start=a, end=b) for a, b in folded])
            except Exception:
                rebuilt = ref
            spent += cost
            refs.append(rebuilt)
