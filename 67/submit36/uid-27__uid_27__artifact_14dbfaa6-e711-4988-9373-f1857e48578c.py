...
def _compose_juniper_lantern_agent_entry():
    ...
    async def query(query: Query) -> Response:
        ...
        return response
    # --- w4 answer-contract wrapper (end) ---
    # slot: 02 FB_7c130ba1_w4 2026-08-23T15:00:00+00:00

    return query

_juniper_lantern_agent_query_entry = _compose_juniper_lantern_agent_entry()


def _compose_amber_lantern_agent_entry():
    ...
    # ----------------------------------------------------------------
    # v115-506-kvz
    # Stages: coverage nudge, value repair, corroborate
    # Ordinary successful path:
    #   query -> _solve -> _knowledge_brief -> _loop (+_open_criteria_hint at midpoint) -> _audit_patch -> _select_best -> _ground_figures -> _second_source_check -> _citations_for -> _w2_point_markers -> _answer_line_only -> Response
    # ----------------------------------------------------------------
...
_amber_lantern_agent_query_entry = _compose_amber_lantern_agent_entry()


_BALANCED_ROUTER_SEED = "58b9ead2a9eebc1db6e1f536"
...
def _balanced_route_label(query: Query) -> str:
    ...
    if bucket == 0:
        return "VesperVectorAgent"
    if bucket == 1:
        return "JuniperLanternAgent"
    return "AmberLanternAgent"
...
class JuniperLanternAgent:
    async def __call__(self, query: Query) -> Response:
        return await _juniper_lantern_agent_query_entry(query)


class AmberLanternAgent:
    async def __call__(self, query: Query) -> Response:
        return await _amber_lantern_agent_query_entry(query)


_BALANCED_PRIMARY_AGENT = VesperVectorAgent()
_BALANCED_SECONDARY_AGENT = JuniperLanternAgent()
_BALANCED_TERTIARY_AGENT = AmberLanternAgent()
...
async def _s36_base_query(query: Query) -> Response:
    ...
    if selected == "VesperVectorAgent":
        branch = _BALANCED_PRIMARY_AGENT
    elif selected == "JuniperLanternAgent":
