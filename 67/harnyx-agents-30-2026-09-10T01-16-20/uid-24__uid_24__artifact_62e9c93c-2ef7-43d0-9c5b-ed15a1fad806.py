...
_niodfyuzez = {'started': None, 'text': None}
_yjdlaqeebv = 24000
_toxxjimppe = 290.0
_zshirychrp = 250.0

def _ictciifonp() -> float:
    started = _niodfyuzez['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _usbujnarcn() -> float:
    return _toxxjimppe - _ictciifonp()
_vnohaeqgca = _hsapi.llm_chat
_airlfcstur = _hsapi.search_web
_qriwumqawp = _hsapi.fetch_page
_yodjayyrvt = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _yvjmimowfp(*args, **kwargs):
    if _ictciifonp() >= _zshirychrp:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _yodjayyrvt})
            kwargs['messages'] = steered
    _result = await _vnohaeqgca(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _llbgqvqxsv(_result)
    return _result

async def _jyxjvyswth(*args, **kwargs):
    if _ictciifonp() >= _zshirychrp:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _airlfcstur(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _oorabdwpoy(*args, **kwargs):
    if _ictciifonp() >= _zshirychrp:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _qriwumqawp(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _yvjmimowfp
_hsapi.search_web = _jyxjvyswth
_hsapi.fetch_page = _oorabdwpoy
_guuggeckro = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_mnxtgvyaal = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_tljkiuxzcf = 900
_omupbhdqqk = 2

def _zxuaodjksz(query: Query) -> int:
...
def _mvoasehcmg(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _ocswwpvsdy(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _zxuaodjksz(query)
    if fields >= 3:
        return 2
    if _mvoasehcmg(lowered, _guuggeckro):
        return 1
    if fields <= _omupbhdqqk and len(text) <= _tljkiuxzcf:
        return 0
    if _mvoasehcmg(lowered, _mnxtgvyaal):
        return 0
    return 1

def _llbgqvqxsv(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _niodfyuzez['text'] = text.strip()[:_yjdlaqeebv]
    except Exception:
        pass

def _rmveuubvem(text: str):
...
def _ybawxzbxvj(query: Query) -> Response:
    text = _niodfyuzez['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_yjdlaqeebv]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _rmveuubvem(text)
...
def _gzfiagqlct():
    ...
    return query
_jodxyhuxhn = _ifwafhjesl()
_qquwsugdrw = _etvqxqkubv()
_puchijctwr = _civxodbctj()
_xdpqibrzso = _gzfiagqlct()
_toxxjimppe = 290.0
_djcqvvjret = 250.0
_tjvgnmvndj = 90.0

async def _unjpvzezcp(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _toxxjimppe - (time.monotonic() - started)
        if first:
            budget = _djcqvvjret if _djcqvvjret < remaining else remaining
            first = False
        else:
            if remaining < _tjvgnmvndj:
                break
        ...
        try:
            ...
            last_exc = exc
    return _ybawxzbxvj(query)
_gptoipighp = 235.0

async def _quwujipaib(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _poqpadkmji(query, agents):
    if len(agents) < 2:
        return await _unjpvzezcp(query, agents)
    r0, r1 = await asyncio.gather(_quwujipaib(agents[0], query, _gptoipighp), _quwujipaib(agents[1], query, _gptoipighp))
    ok0 = isinstance(r0, Response) and _dmljfgnuzg(r0) != ''
    ok1 = isinstance(r1, Response) and _dmljfgnuzg(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_vnohaeqgca(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _niodfyuzez['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _unjpvzezcp(query, (_puchijctwr, _jodxyhuxhn, _qquwsugdrw, _xdpqibrzso))
        index = _ocswwpvsdy(query)
        if index == 0:
            agents = (_jodxyhuxhn, _qquwsugdrw, _puchijctwr, _xdpqibrzso)
        elif index == 1:
            agents = (_qquwsugdrw, _puchijctwr, _xdpqibrzso, _jodxyhuxhn)
        elif index == 2:
            agents = (_puchijctwr, _xdpqibrzso, _jodxyhuxhn, _qquwsugdrw)
        elif index == 3:
            agents = (_xdpqibrzso, _jodxyhuxhn, _qquwsugdrw, _puchijctwr)
        else:
            agents = (_jodxyhuxhn, _qquwsugdrw, _puchijctwr, _xdpqibrzso)
        return await _poqpadkmji(query, agents)
    except Exception:
        return _ybawxzbxvj(query)
_BUILDN_TAG_m12 = "sn45-1c64d74169c0"
