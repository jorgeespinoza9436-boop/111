...
def _compose_amber_relay_agent_entry():
...
_amber_relay_agent_query_entry = _compose_amber_relay_agent_entry()


def _compose_juniper_anvil_agent_entry():
...
_juniper_anvil_agent_query_entry = _compose_juniper_anvil_agent_entry()


_SHAPE_ROUTER_SEED = "c6685d13284a780455daf2842f77e9ed"
...
def _balanced_route_label(query: Query) -> str:
    if getattr(query, "fast", False):
        return "AmberAnvilAgent"
    text = (getattr(query, "text", "") or "").strip()
    shape = _shape_class(query)
    if shape == 0:
        return "AmberRelayAgent"
    if shape == 1:
        return "JuniperAnvilAgent"
...
class AmberRelayAgent:
    async def __call__(self, query: Query) -> Response:
        return await _amber_relay_agent_query_entry(query)


class JuniperAnvilAgent:
    async def __call__(self, query: Query) -> Response:
        return await _juniper_anvil_agent_query_entry(query)


_SHAPE_PRIMARY_AGENT = AmberAnvilAgent()
_SHAPE_SECONDARY_AGENT = AmberRelayAgent()
_SHAPE_TERTIARY_AGENT = JuniperAnvilAgent()
