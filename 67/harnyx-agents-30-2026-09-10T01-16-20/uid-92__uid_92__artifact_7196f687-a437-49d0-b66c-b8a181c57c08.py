...
_gwhciljsct = {'started': None, 'text': None}
_lgedrpcira = 24000
_xfpcmrmppx = 290.0
_egcmhgquha = 250.0

def _gbspgllaht() -> float:
    started = _gwhciljsct['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _gttpnlzzhj() -> float:
    return _xfpcmrmppx - _gbspgllaht()
_mhushbmobp = _hsapi.llm_chat
_sfssqcwhqw = _hsapi.search_web
_httchqbodl = _hsapi.fetch_page
_etbuycoxgj = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _ejxstyicrw(*args, **kwargs):
    if _gbspgllaht() >= _egcmhgquha:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _etbuycoxgj})
            kwargs['messages'] = steered
    _result = await _mhushbmobp(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _qafibdcief(_result)
    return _result

async def _jojesemavt(*args, **kwargs):
    if _gbspgllaht() >= _egcmhgquha:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _sfssqcwhqw(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _smbbkddmyh(*args, **kwargs):
    if _gbspgllaht() >= _egcmhgquha:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _httchqbodl(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _ejxstyicrw
_hsapi.search_web = _jojesemavt
_hsapi.fetch_page = _smbbkddmyh
_nvbobsypnj = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_fxdrlsienn = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_iavwetqqlj = 900
_ccsovxxugq = 2

def _gcnwgugava(query: Query) -> int:
...
def _ylhjxoridk(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _qmgwzwxmzu(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _gcnwgugava(query)
    if fields >= 3:
        return 2
    if _ylhjxoridk(lowered, _nvbobsypnj):
        return 1
    if fields <= _ccsovxxugq and len(text) <= _iavwetqqlj:
        return 0
    if _ylhjxoridk(lowered, _fxdrlsienn):
        return 0
    return 1

def _qafibdcief(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _gwhciljsct['text'] = text.strip()[:_lgedrpcira]
    except Exception:
        pass

def _rrjpkmbmqx(text: str):
...
def _zisfkjbmvg(query: Query) -> Response:
    text = _gwhciljsct['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_lgedrpcira]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _rrjpkmbmqx(text)
...
def _zmftmwsgjk():
    ...
    async def query(query: Query) -> Response:
        ...
        try:
            ...
            return base
    return query
_twpgsgmqfi = _qgpkgjyjyo()
_rverhlluwt = _mrfwoevzmk()
_dtdcgftjfd = _zhgwudpxps()
_hbsuzblnvj = _zmftmwsgjk()
_xfpcmrmppx = 290.0
_malnqnjjzi = 250.0
_ggipkdrifg = 90.0

async def _cmewfqvffm(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _xfpcmrmppx - (time.monotonic() - started)
        if first:
            budget = _malnqnjjzi if _malnqnjjzi < remaining else remaining
            first = False
        else:
            if remaining < _ggipkdrifg:
                break
        ...
        try:
            ...
            last_exc = exc
    return _zisfkjbmvg(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _gwhciljsct['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _cmewfqvffm(query, (_dtdcgftjfd, _twpgsgmqfi, _rverhlluwt, _hbsuzblnvj))
        index = _qmgwzwxmzu(query)
        if index == 0:
            agents = (_twpgsgmqfi, _rverhlluwt, _dtdcgftjfd, _hbsuzblnvj)
        elif index == 1:
            agents = (_rverhlluwt, _dtdcgftjfd, _hbsuzblnvj, _twpgsgmqfi)
        elif index == 2:
            agents = (_dtdcgftjfd, _hbsuzblnvj, _twpgsgmqfi, _rverhlluwt)
        elif index == 3:
            agents = (_hbsuzblnvj, _twpgsgmqfi, _rverhlluwt, _dtdcgftjfd)
        else:
            agents = (_twpgsgmqfi, _rverhlluwt, _dtdcgftjfd, _hbsuzblnvj)
        return await _cmewfqvffm(query, agents)
    except Exception:
        return _zisfkjbmvg(query)
_BUILDN_TAG_m8 = "sn45-439371e0a38e"
