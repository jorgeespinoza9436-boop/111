...
def _compose_slate_compass_agent_entry():
...
_slate_compass_agent_query_entry = _compose_slate_compass_agent_entry()


def _compose_tidal_vector_agent_entry():
    import asyncio
...
_tidal_vector_agent_query_entry = _compose_tidal_vector_agent_entry()


def _compose_tidal_anvil_agent_entry():
    """agent_d — v32 "toolloop": model-driven research agent.
...
_tidal_anvil_agent_query_entry = _compose_tidal_anvil_agent_entry()


_SHAPE_ROUTER_SEED = "85ffbadc635cb33119a17e38"
...
def _balanced_route_label(query: Query) -> str:
    if getattr(query, "fast", False):
        return "SlateCompassAgent"
    text = (getattr(query, "text", "") or "").strip()
    shape = _shape_class(query)
    if shape == 0:
        return "TidalVectorAgent"
    if shape == 1:
        return "TidalAnvilAgent"
...
class SlateCompassAgent:
    async def __call__(self, query: Query) -> Response:
        return await _slate_compass_agent_query_entry(query)


class TidalVectorAgent:
    async def __call__(self, query: Query) -> Response:
        return await _tidal_vector_agent_query_entry(query)


class TidalAnvilAgent:
    async def __call__(self, query: Query) -> Response:
        return await _tidal_anvil_agent_query_entry(query)


_SHAPE_PRIMARY_AGENT = SlateCompassAgent()
_SHAPE_SECONDARY_AGENT = TidalVectorAgent()
_SHAPE_TERTIARY_AGENT = TidalAnvilAgent()
