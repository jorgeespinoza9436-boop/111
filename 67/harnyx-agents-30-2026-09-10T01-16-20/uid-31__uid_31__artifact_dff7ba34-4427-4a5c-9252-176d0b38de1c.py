...
def _compose_umber_orchid_slot09_agent_entry():
...
_umber_orchid_slot09_agent_query_entry = _compose_umber_orchid_slot09_agent_entry()


def _compose_saffron_kestrel_slot09_agent_entry():
...
_saffron_kestrel_slot09_agent_query_entry = _compose_saffron_kestrel_slot09_agent_entry()


def _compose_grafted_u192s2fromu204_agent_entry():
...
_aster_compass_slot09_agent_query_entry = _compose_grafted_u192s2fromu204_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "UmberOrchidSlot09Agent"
    ...
    if (
        ...
        return "SaffronKestrelSlot09Agent"
    return "AsterCompassSlot09Agent"


class UmberOrchidSlot09Agent:
    async def __call__(self, query: Query) -> Response:
        return await _umber_orchid_slot09_agent_query_entry(query)


class SaffronKestrelSlot09Agent:
    async def __call__(self, query: Query) -> Response:
        return await _saffron_kestrel_slot09_agent_query_entry(query)


class AsterCompassSlot09Agent:
    async def __call__(self, query: Query) -> Response:
        return await _aster_compass_slot09_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = UmberOrchidSlot09Agent()
_ANALYTICAL_FIELD_AGENT = SaffronKestrelSlot09Agent()
_BROAD_FIELD_AGENT = AsterCompassSlot09Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "UmberOrchidSlot09Agent",
    "SaffronKestrelSlot09Agent",
    "AsterCompassSlot09Agent",
)
...
async def _drv_base_query(query: Query) -> Response:
    ...
    if selected == "UmberOrchidSlot09Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "SaffronKestrelSlot09Agent":
...
# batch_tag='drv109' salt='97773e848aa6'
