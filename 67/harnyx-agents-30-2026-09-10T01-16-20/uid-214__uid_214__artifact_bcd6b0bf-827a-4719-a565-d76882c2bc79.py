...
_xkjggfwhzv = {'started': None, 'text': None}
_lkjlerupdp = 24000
_yxeramevyh = 290.0
_oykutemqeg = 250.0

def _hfgnjgvbko() -> float:
    started = _xkjggfwhzv['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _ujgmoeurwn() -> float:
    return _yxeramevyh - _hfgnjgvbko()
_fabadzsgwb = _hsapi.llm_chat
_xkufagyrbo = _hsapi.search_web
_qsbpdpbggo = _hsapi.fetch_page
_tkdmormqmd = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _yuqqjovwaa(*args, **kwargs):
    if _hfgnjgvbko() >= _oykutemqeg:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _tkdmormqmd})
            kwargs['messages'] = steered
    _result = await _fabadzsgwb(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _mblsbyuxls(_result)
    return _result

async def _zbmthinvsz(*args, **kwargs):
    if _hfgnjgvbko() >= _oykutemqeg:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _xkufagyrbo(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _zohfujhewi(*args, **kwargs):
    if _hfgnjgvbko() >= _oykutemqeg:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _qsbpdpbggo(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _yuqqjovwaa
_hsapi.search_web = _zbmthinvsz
_hsapi.fetch_page = _zohfujhewi
_darstexojd = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_gbgiariflf = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_gxfmoihwig = 900
_ntgurnyzdv = 2

def _mayuzranep(query: Query) -> int:
...
def _ugxeljtwdl(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _adywkmthfj(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _mayuzranep(query)
    if fields >= 3:
        return 2
    if _ugxeljtwdl(lowered, _darstexojd):
        return 1
    if fields <= _ntgurnyzdv and len(text) <= _gxfmoihwig:
        return 0
    if _ugxeljtwdl(lowered, _gbgiariflf):
        return 0
    return 1

def _mblsbyuxls(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _xkjggfwhzv['text'] = text.strip()[:_lkjlerupdp]
    except Exception:
        pass

def _lgukplttbc(text: str):
...
def _oxxnutflsu(query: Query) -> Response:
    text = _xkjggfwhzv['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_lkjlerupdp]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _lgukplttbc(text)
...
def _efcsklaumu():
    ...
    _ELEMTOK_STOP = ('the', 'and', 'for', 'its', 'their', 'both', 'this', 'that', 'with', 'from', 'full', 'official', 'quarterly', 'each', 'all', 'new', 'one', 'page', 'dated', 'list', 'every', 'must', 'then', 'into', 'over', 'under', 'only', 'same', 'record', 'value', 'member', 'figure', 'evidence')
    _ELEM_SPLIT_RE = re.compile('[^A-Za-z0-9]+')
    _ENT_CAP_RE = re.compile('\\b[A-Z][A-Za-z0-9&.\\-]{2,}(?:\\s+(?:of|the|and|de|for)\\s+)?(?:\\s+[A-Z][A-Za-z0-9&.\\-]{2,}){0,3}\\b')
    _ENT_QSTOP = frozenset({'Which', 'What', 'Who', 'When', 'Where', 'How', 'Why', 'The', 'A', 'An', 'For', 'From', 'In', 'On', 'Of', 'And', 'Or', 'As', 'At', 'By', 'To', 'Answer', 'Give', 'List', 'Name', 'Using', 'According', 'Report', 'Compare', 'Consider', 'Identify', 'Determine', 'Explain', 'State', 'Find', 'Return', 'Provide', 'Between', 'Across', 'Both', 'Each', 'Per', 'With', 'Within', 'Their', 'Its', 'This', 'That', 'These', 'If', 'Then', 'Also', 'Please', 'JSON', 'Only', 'Output'})
...
def _pslqgubnsh():
    ...
    return query
_hezkubnqky = _efcsklaumu()
_yghecmuhjb = _hlyacvlpga()
_kahiehtsls = _cerucnxxka()
_zfxkpvgrcf = _pslqgubnsh()
_yxeramevyh = 290.0
_okttqjqrkh = 250.0
_fbyovgbkov = 90.0

async def _asazahnfbr(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _yxeramevyh - (time.monotonic() - started)
        if first:
            budget = _okttqjqrkh if _okttqjqrkh < remaining else remaining
            first = False
        else:
            if remaining < _fbyovgbkov:
                break
        ...
        try:
            ...
            last_exc = exc
    return _oxxnutflsu(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _xkjggfwhzv['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _asazahnfbr(query, (_kahiehtsls, _hezkubnqky, _yghecmuhjb, _zfxkpvgrcf))
        index = _adywkmthfj(query)
        if index == 0:
            agents = (_hezkubnqky, _yghecmuhjb, _kahiehtsls, _zfxkpvgrcf)
        elif index == 1:
            agents = (_yghecmuhjb, _kahiehtsls, _zfxkpvgrcf, _hezkubnqky)
        elif index == 2:
            agents = (_kahiehtsls, _zfxkpvgrcf, _hezkubnqky, _yghecmuhjb)
        elif index == 3:
            agents = (_zfxkpvgrcf, _hezkubnqky, _yghecmuhjb, _kahiehtsls)
        else:
            agents = (_hezkubnqky, _yghecmuhjb, _kahiehtsls, _zfxkpvgrcf)
        return await _asazahnfbr(query, agents)
    except Exception:
        return _oxxnutflsu(query)
_BUILDN_TAG_m1 = "sn45-a94dd6282baf"
