"""Multi-branch research agent with a shipped-evidence provenance cycle.

Every reachable branch keeps its own pipeline as is. Before each branch
materialises citations, the decided answer is audited against the exact
evidence slices that will ship; unbacked figures re-enter that branch's
evidence and research stages until every shipped excerpt carries its figure.

Combined miner agent."""
from __future__ import annotations
# fork of 10_uid_234_score_0.666.py: added cst:scope  (full mechanism set: CORR+CST)
import asyncio
import time
...
from time import monotonic as _ec_monotonic


# ══════════════════════════════════════════════════════════════════════════════
# Shipped-evidence provenance cycle (shared by every ledger-lineage branch;
# each branch binds its own loop, search, commit and answer helpers as `h`)
# ══════════════════════════════════════════════════════════════════════════════
# The grader credits a claim only when the citation excerpt that actually ships
# contains the supporting text. Every stage above reads whole page texts, but
# what ships is the ledger's retained/shown windows, so a figure can be cited
# from the right page and still arrive unbacked. This cycle audits the final
# answer against exactly those windows and, while a cited figure is unbacked,
# re-enters the evidence and research stages: a figure printed on the cited
# page outside its window re-anchors that row's retained window (a ledger
# update with no model call); a figure no cited page prints triggers targeted
# retrieval, committed to the ledger, and a bounded re-entry of the research
# loop that rewrites the answer against the new rows. The audit repeats until
# every shipped window carries its figure or the round cap is reached.
...
EC_MARGIN_CHARS = 260
EC_LOOP_TURNS = 3
EC_MIN_KEEP_RATIO = 0.6
EC_FACT_KEEP_PCT = 70
EC_EVIDENCE_CHARS = 6000
EC_ANSWER_CHARS = 6000
_EC_SENT_RE = re.compile(r"(?:[^.!?\n]|\.(?=\d))+[.!?]?")
_EC_FIG_RE = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
_EC_MARK_RE = re.compile(r"\[[0-9][0-9,\s\-]*\]")
_EC_NAME_RE = re.compile(r"[A-Z][A-Za-z0-9&'\-]+(?:\s+[A-Z][A-Za-z0-9&'\-]+){1,3}")
_EC_TOKEN_RE = re.compile(r"[A-Za-z][A-Za-z0-9\-']{2,}")


class _ec_Hooks:
    """The branch helpers the shared cycle re-enters: its research loop, search,
    ledger commit, citation parsing and answer checks, bound per branch."""

    __slots__ = ("loop", "search", "commit", "cited", "normalize", "usable", "spend", "stop", "cite_re")

    def __init__(self, loop, search, commit, cited, normalize, usable, spend, stop, cite_re):
        self.loop = loop
        self.search = search
        self.commit = commit
        self.cited = cited
...
def _ec_shipped_ref(ledger, n: int):
    """The CitationRef the harvest would build for row n, or None."""
    many = getattr(ledger, "refs_for", None)
    if many is not None:
        refs = many(n)
        return refs[0] if refs else None
    return ledger.ref_for(n)


def _ec_shipped_text(ledger, n: int) -> str:
    """The characters of row n that the citation harvest will materialise."""
    ref = _ec_shipped_ref(ledger, n)
    if ref is None:
        return ""
    text = ledger.rows[n - 1].get("text") or ""
    return " ".join(text[s.start:s.end] for s in ref.slices)


def _ec_widen(row: dict, a: int, b: int) -> None:
    """Record [a, b) as a retained window of the row, merging with neighbours."""
    ...
    if b - a > EC_SPAN_MAX:
        mid = (a + b) // 2
        a = max(0, mid - EC_SPAN_MAX // 2)
        b = min(note_len, a + EC_SPAN_MAX)
    kept = row.setdefault("retained", [])
    for i, (ka, kb) in enumerate(kept):
        if a <= kb and ka <= b:
            kept[i] = (min(ka, a), max(kb, b))
            return
    if len(kept) >= EC_RETAIN_MAX:
        return
    kept.append((a, b))


def _ec_claims(answer: str, ledger, h: dict) -> list:
    """(sentence, figures, cited rows) for every cited sentence carrying a figure."""
    out = []
    for m in _EC_SENT_RE.finditer(h.normalize(answer or "")):
        sent = m.group(0).strip()
        if len(sent) < 12:
            continue
        rows = h.cited(sent, len(ledger.rows))
        if not rows:
            continue
        figs = []
        for tok in _EC_FIG_RE.findall(_EC_MARK_RE.sub(" ", sent)):
            if len(_ec_norm(tok)) >= 2 and tok not in figs:
                figs.append(tok)
        if figs:
...
def _ec_pattern(fig: str):
    """A bounded pattern for the figure in any of its comma/percent spellings."""
    forms = []
    for form in (fig, fig.replace(",", ""), _ec_norm(fig)):
        if len(form) >= 2 and form not in forms:
            forms.append(form)
    return re.compile(r"(?<![\d.,])(?:" + "|".join(re.escape(f) for f in forms) + r")(?![\d.]|,\d)")


def _ec_locate(text: str, fig: str):
    """Offset and length of `fig` in a page text, tolerating comma/percent forms."""
    m = _ec_pattern(fig).search(text)
    return (m.start(), m.end() - m.start()) if m else None


def _ec_audit(answer: str, ledger, h: dict) -> tuple:
    """Shipped-evidence audit.

    Values read: for each cited sentence that asserts a figure, whether the
    figure occurs inside the character windows of its cited rows that will be
    materialised as citation slices (the same windows `refs_for` ships).
    Outcomes: `unanchored` -- the figure is printed on a cited page but outside
    the shipped window, so the evidence state is repaired by re-anchoring the
    window; `missing` -- no cited page prints it, so research re-enters with
    targeted retrieval and the answer is regenerated against the new rows.
    Both lists empty means every shipped excerpt carries its figure and the
    cycle is complete."""
    unanchored, missing = [], []
    for sent, figs, rows in _ec_claims(answer, ledger, h):
        shipped = " ".join(_ec_shipped_text(ledger, n) for n in rows)
        for fig in figs:
            if _ec_pattern(fig).search(shipped):
                continue
            located = None
            for n in rows:
                hit = _ec_locate(ledger.rows[n - 1].get("text") or "", fig)
                if hit is not None:
                    located = (n, hit[0], hit[1])
                    break
            if located is not None:
                unanchored.append((sent, fig, located))
            else:
                missing.append((sent, fig, rows))
    _EC_STATS["audits"] = _EC_STATS.get("audits", 0) + 1
    _EC_STATS["unanchored"] = _EC_STATS.get("unanchored", 0) + len(unanchored)
    _EC_STATS["missing"] = _EC_STATS.get("missing", 0) + len(missing)
    return unanchored[:EC_MAX_CLAIMS], missing[:EC_MAX_CLAIMS]


def _ec_reanchor(unanchored: list, ledger) -> int:
    """Widen each cited row's retained window onto the figure it is cited for,
    so the shipped slice carries it. Pure ledger update, no model or tool call."""
    moved = 0
    for _sent, _fig, (n, pos, length) in unanchored:
        row = ledger.rows[n - 1]
        before = list(row.get("retained") or [])
        _ec_widen(row, pos - EC_MARGIN_CHARS, pos + length + EC_MARGIN_CHARS)
        if list(row.get("retained") or []) != before:
            moved += 1
    _EC_STATS["reanchored"] = _EC_STATS.get("reanchored", 0) + moved
    return moved


def _ec_salient(sent: str, h: dict) -> list:
    out = []
    for tok in _EC_TOKEN_RE.findall(_EC_MARK_RE.sub(" ", sent)):
        low = tok.lower()
        if low in h.stop or low in out:
            continue
        out.append(low)
    return out


async def _ec_retrieve(missing: list, ledger, h: dict) -> str:
    """Targeted retrieval for figures no cited page prints: one search per
    claim, rows committed to the ledger so the rewrite can cite them."""
    blocks = []
    for sent, fig, _rows in missing[:EC_MAX_SEARCHES]:
        probe = " ".join(_ec_salient(sent, h)[:6] + [fig]).strip()
        if not probe:
            continue
        try:
            body = h.commit(await h.search(probe, ledger), ledger)
        except Exception:
            continue
        if isinstance(body, str) and h.cite_re.search(body):
            blocks.append(body[:EC_EVIDENCE_CHARS])
    return "\n\n".join(blocks)


def _ec_facts(text: str) -> set:
    body = _EC_MARK_RE.sub(" ", text or "")
    facts = {"n:" + _ec_norm(m.group(0)) for m in _EC_FIG_RE.finditer(body)}
    facts |= {"e:" + " ".join(m.group(0).split()).lower() for m in _EC_NAME_RE.finditer(body)}
    return facts


def _ec_keeps_facts(draft: str, revision: str) -> bool:
    before = _ec_facts(draft)
    if not before:
        return True
    return len(before & _ec_facts(revision)) * 100 >= len(before) * EC_FACT_KEEP_PCT


async def _ec_regenerate(question: str, answer: str, messages: list, ledger,
                         deadline: float, missing: list, evidence: str, h: dict) -> str:
    """Bounded re-entry of the research loop against the provenance findings."""
    ...
    order = ("PROVENANCE CHECK. A claim is credited only when the shipped citation "
             ...
             "\nFor each value: cite the result that actually prints it (use "
             "retain_evidence on that result), replace it with the figure a source "
             "does print, or drop the claim. Use the NEW EVIDENCE below where it "
             "states the value. Then rewrite the COMPLETE answer with [n] citations.")
    if evidence:
        order += "\n\nNEW EVIDENCE:\n" + evidence
    order += "\n\nCURRENT ANSWER:\n" + answer[:EC_ANSWER_CHARS]
    carry = list(messages)
    carry.append({"role": "system", "content": order})
    try:
        revised, _ = await h.loop(question, "", ledger, deadline, EC_LOOP_TURNS,
                                 carry=carry, allow_tools_in_wrapup=True)
    except Exception:
        return answer
    revised = revised.strip()
    if not h.usable(revised):
        return answer
    if len(revised) < int(len(answer) * EC_MIN_KEEP_RATIO):
        return answer
    if not _ec_keeps_facts(answer, revised):
        return answer
    _EC_STATS["adopted"] = _EC_STATS.get("adopted", 0) + 1
    return revised


async def _ec_cycle(question: str, answer: str, messages: list, ledger,
                            deadline: float, h: dict) -> str:
    """Audit the final answer against the evidence that will actually ship and
    re-enter the evidence and research stages while a cited figure is unbacked."""
    if not h.usable(answer) or not ledger.rows:
        return answer
    for _round in range(_EC_STATS.get("rounds", EC_MAX_ROUNDS)):
        if (deadline - _ec_monotonic()) < EC_MIN_LEFT_S or h.spend() < EC_MIN_USD:
            return answer
        unanchored, missing = _ec_audit(answer, ledger, h)
        if not unanchored and not missing:
            return answer
        _ec_reanchor(unanchored, ledger)
        if not missing or not messages:
            continue
        evidence = await _ec_retrieve(missing, ledger, h)
        answer = await _ec_regenerate(question, answer, messages, ledger, deadline,
                                      missing, evidence, h)
    return answer


EC_ANCHORED_SPAN = 2000
EC_MAX_REF_CHARS = 14000


def _ec_board(answer: str, ledger, h: dict) -> dict:
    """The provenance board: for every cited figure, the window of the cited
    row that prints it. Built from the same audit reading the cycle uses and
    consumed by the citation harvest, so the slices that ship are cut around
    the values the answer actually asserts."""
    board: dict = {}
    for sent, figs, rows in _ec_claims(answer, ledger, h):
        for fig in figs:
            for n in rows:
                ...
                if hit is not None:
                    pos, length = hit
                    board.setdefault(n, []).append((pos - EC_MARGIN_CHARS, pos + length + EC_MARGIN_CHARS))
                    break
    return board


def _ec_reslice(citations, answer: str, ledger, h: dict):
    """Let the provenance board cut the citation slices that ship.

    For every harvested reference whose row is on the board, the slices become
    the union of the row's retained windows and the board windows around the
    figures the answer cites it for, each grown to the anchored span size and
    kept under the per-reference cap. References to rows the board does not
    know keep their inherited slices; positions never change."""
    if not _EC_STATS.get("reslice", True):
        return citations
    board = _ec_board(answer, ledger, h)
    if not board:
        return citations
    by_key = {(ledger.rows[n - 1]["receipt_id"], ledger.rows[n - 1]["result_id"]): n for n in board}
    out = []
    for ref in citations:
        n = by_key.get((ref.receipt_id, ref.result_id))
        if n is None:
            out.append(ref)
            continue
        row = ledger.rows[n - 1]
        ...
        grown: list = []
        for a, b in [(int(a), int(b)) for a, b in (row.get("retained") or [])] + board[n]:
            a = max(0, min(int(a), note_len))
            b = max(a + 1, min(int(b), note_len))
            pad = max(0, EC_ANCHORED_SPAN - (b - a))
            a = max(0, a - pad // 2)
            b = min(note_len, b + pad - pad // 2)
            grown.append([a, b])
        grown.sort()
        merged: list = []
        for a, b in grown:
        ...
        while merged and total > EC_MAX_REF_CHARS:
            dropped = min(merged, key=lambda w: w[1] - w[0])
            merged.remove(dropped)
            total -= dropped[1] - dropped[0]
        slices = [CitationSlice(start=a, end=b) for a, b in merged if b > a]
        if not slices:
            out.append(ref)
            continue
        try:
            out.append(CitationRef(receipt_id=ref.receipt_id, result_id=ref.result_id, slices=slices))
        except Exception:
            out.append(ref)
    _EC_STATS["resliced"] = _EC_STATS.get("resliced", 0) + 1
    return tuple(out) if isinstance(citations, tuple) else out

import harnyx_miner_sdk.api as _hsapi
_yxjszchphy = {'started': None, 'text': None}
_lacecmvlga = 24000
_ssqnbqnwvx = 290.0
_ugcpiafwyt = 250.0

def _cueejfbmur() -> float:
    started = _yxjszchphy['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _gqwdlxrzux() -> float:
    return _ssqnbqnwvx - _cueejfbmur()
_xpkrkluhdw = _hsapi.llm_chat
_uwhelhlkjh = _hsapi.search_web
_hstofhbxnd = _hsapi.fetch_page
_bhivtjyclk = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _capzxuxets(*args, **kwargs):
    if _cueejfbmur() >= _ugcpiafwyt:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _bhivtjyclk})
            kwargs['messages'] = steered
    _result = await _xpkrkluhdw(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _dykxbtqkch(_result)
    return _result

async def _bwkugytpqy(*args, **kwargs):
    if _cueejfbmur() >= _ugcpiafwyt:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _uwhelhlkjh(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _rbwlnyokbv(*args, **kwargs):
    if _cueejfbmur() >= _ugcpiafwyt:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _hstofhbxnd(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _capzxuxets
_hsapi.search_web = _bwkugytpqy
_hsapi.fetch_page = _rbwlnyokbv
_ykpmxabvsd = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_kmbmddkjgd = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_dbtugvxoqt = 900
_afbsexynmg = 2

def _atyhkdfotp(query: Query) -> int:
...
def _zzfwsvxtrq(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _aqjpctjuew(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _atyhkdfotp(query)
    if fields >= 3:
        return 2
    if _zzfwsvxtrq(lowered, _ykpmxabvsd):
        return 1
    if fields <= _afbsexynmg and len(text) <= _dbtugvxoqt:
        return 0
    if _zzfwsvxtrq(lowered, _kmbmddkjgd):
        return 0
    return 1

def _dykxbtqkch(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _yxjszchphy['text'] = text.strip()[:_lacecmvlga]
    except Exception:
        pass

def _ydbtdvujif(text: str):
...
def _dbhyzepajl(query: Query) -> Response:
    text = _yxjszchphy['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_lacecmvlga]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _ydbtdvujif(text)
...
def _fxeuefingo():
    ...
    # --- answer-admissibility controller [u234-cst-scope: pool and source-scope conditions] -------------------
    # The inherited loop returns the first answer the model is willing to commit to.
    # Nothing tests whether that answer is ADMISSIBLE — whether it actually satisfies
    # the qualifying conditions the question states. Here those conditions are
    # resolved before research, carried on the ledger, and the drafted answer is
    # tested against each one before the loop may finish. An answer that fails a
    # condition does not get patched: the loop re-enters retrieval aimed at that
    # condition and is told to give the answer that satisfies it instead.
    CST_MAX = 6
    CST_MODEL = "z-ai/glm-5.2"
    CST_PROBE_TIMEOUT_S = 18.0
    CST_CHECK_TIMEOUT_S = 20.0
    CST_MAX_TOKENS = 500
    CST_MIN_SECONDS = 140.0
    CST_MAX_REENTRIES = 1
    CST_REENTRY_MIN_SECONDS = 70.0
    CST_MIN_USD = 0.04
    CST_POLICY = "scope"
    _CST = {"reentries": 0}
    _CST_FAST = {"on": False}
    ...
    # --- qualifying conditions (scope): the admissibility test for the answer ---
    import re as _cst_re
    import json as _cst_json

    _CST_SEED_RE = _cst_re.compile(
        r"(?i)\b(?:according to|as (?:listed|printed|published|posted|shown|recorded)|"
        r"from the|in the|per the|based on|using (?:only )?the|drawn from|among the|"
        r"of the following|in order of|ranked by|sorted by|measured (?:in|by)|expressed in|"
        r"in terms of)\b[^.?;]{0,90}")
    def _cst_seed_conditions(question: str) -> list:
        out = []
        for m in _CST_SEED_RE.finditer(question or ""):
            phrase = " ".join(m.group(0).split())
            if len(phrase) < 12:
                continue
            test = "answer must come from / follow: " + phrase
            if not any(test.lower() == p.lower() for p in out):
                out.append(test)
            if len(out) >= 5:
                break
        return out
    ...
    async def _cst_chat(system: str, user: str, max_tokens: int, timeout: float) -> str:
        return await _chat_simple(LLM_LANE_A, CST_MODEL, system, user,
    ...
    class EvidenceLedger:
        ...
        def __init__(self) -> None:
            self.rows: list[dict] = []
            # Qualifying conditions the returned answer must satisfy, and the
            # question they came from. The loop consults these before finishing.
            self.cst_conditions: list[str] = []
            self.cst_question: str = ""

        def set_conditions(self, question: str, conditions: list) -> None:
            self.cst_question = question or ""
            self.cst_conditions = [c for c in (conditions or []) if str(c).strip()][:CST_MAX]

        def cst_digest(self, limit: int = 10) -> str:
            """Compact view of gathered evidence, for testing the answer against conditions."""
            parts = []
            for n, row in enumerate(self.rows[:limit], start=1):
                blob = (row.get("preview") or row.get("text") or "")[:300]
                parts.append("[" + str(n) + "] " + (row.get("title") or "")[:100]
                             + " | " + (row.get("url") or "")[:120] + "\n    " + blob)
            return "\n".join(parts)
    ...
    async def _cst_conditions(question: str, deadline: float) -> list:
        """Resolve the question's qualifying conditions BEFORE research."""
        conditions = []
        for phrase in _cst_seed_conditions(question):
            if phrase and not any(phrase.lower() == c.lower() for c in conditions):
                conditions.append(phrase)
        if (deadline - monotonic()) < CST_MIN_SECONDS:
            return conditions[:CST_MAX]
        probe = ("List the SCOPE CONDITIONS this question imposes on a correct answer: the exact pool the answer must be drawn from, the specific source, table, edition or list the question says to use, the unit or basis required, and any ordering the answer must follow. One short imperative test each, checkable against a candidate answer. JSON only: a list of strings, at most 6." + "\n\nQuestion:\n" + question[:4000])
        try:
            raw = await _cst_chat(
                "Admissibility analyst. Name what a correct answer must be true of. JSON list only.",
                probe, CST_MAX_TOKENS,
                max(6.0, min(CST_PROBE_TIMEOUT_S,
                             (deadline - monotonic()) - CST_MIN_SECONDS + 40.0)))
            raw = _cst_re.sub(r"^```(?:json)?\s*|\s*```$", "", (raw or "").strip(),
                              flags=_cst_re.I | _cst_re.M)
            parsed = _cst_json.loads(raw)
        except Exception:
            return conditions[:CST_MAX]
        ...
        if isinstance(parsed, list):
            for item in parsed:
                ...
                if len(text) < 8:
                    continue
                if not any(text.lower() == c.lower() for c in conditions):
                    conditions.append(text)
        return conditions[:CST_MAX]

    async def _cst_unmet(answer: str, ledger, deadline: float) -> list:
        """Which qualifying conditions the drafted answer does NOT demonstrably satisfy."""
        conditions = list(getattr(ledger, "cst_conditions", []) or [])
        if not conditions or not (answer or "").strip():
            return []
        listing = "\n".join("- " + c for c in conditions)
        probe = ("Question:\n" + (getattr(ledger, "cst_question", "") or "")[:2500]
                 + "\n\nDrafted answer:\n" + answer[:3000]
                 + "\n\nConditions the answer must satisfy:\n" + listing
                 + "\n\nGathered evidence:\n" + ledger.cst_digest())
        system = ("You test a drafted answer for admissibility. For each condition, decide whether "
                  "the drafted answer, on the evidence given, demonstrably satisfies it. Mark a "
                  "condition unmet when the answer violates it OR when nothing in the evidence "
                  "shows it holds. Do not mark a condition unmet merely because you would phrase "
                  "the answer differently. JSON only: an object with key unmet whose value "
                  "is the list of unmet condition strings, verbatim.")
        try:
            raw = await _cst_chat(system, probe, CST_MAX_TOKENS,
                                  max(6.0, min(CST_CHECK_TIMEOUT_S,
                                               (deadline - monotonic()) - CST_REENTRY_MIN_SECONDS + 30.0)))
            raw = _cst_re.sub(r"^```(?:json)?\s*|\s*```$", "", (raw or "").strip(),
                              flags=_cst_re.I | _cst_re.M)
            parsed = _cst_json.loads(raw)
        except Exception:
            return []
        rows = parsed.get("unmet") if isinstance(parsed, dict) else parsed
        ...
        for item in rows:
            ...
            for i, low in enumerate(lowered):
                if text.lower() == low or text.lower() in low or low in text.lower():
                    if conditions[i] not in out:
                        out.append(conditions[i])
                    break
        return out[:CST_MAX]

    def _cst_order(failed: list) -> str:
        """The order that sends the loop back when the answer is not admissible."""
        return (
            "ADMISSIBILITY (scope): the answer you gave is not established to satisfy these "
            "conditions of the question:\n- " + "\n- ".join(failed[:CST_MAX]) +
            "\nSearch or fetch now to settle each one, then " + "re-check the answer against the failed scope conditions and, if it was drawn from the wrong pool, source or basis, give the answer the required scope yields" +
            ". If the evidence shows your previous answer fails a condition, say so briefly and "
            "give the correct answer instead. Then produce the COMPLETE final answer with [n] "
        ...
        for turn in range(1, turn_cap + 1):
            ...
            if not calls:
                ...
                if not _is_usable_answer(candidate):
                    ...
                    break
                answer = candidate
                # CONSTRAINT GATE -- admissibility of the answer about to be returned.
                # The inherited loop would finish here. Instead the drafted answer is
                # tested against the question's own qualifying conditions; a condition
                # it fails sends the loop back to retrieval with an order to replace
                # the answer, not to defend it.
                if (ledger.cst_conditions and not _CST_FAST["on"] and not finish_only
                        and _CST["reentries"] < CST_MAX_REENTRIES
                        and (deadline - monotonic()) > CST_REENTRY_MIN_SECONDS
                        and _spend_left() >= CST_MIN_USD):
                    _cst_failed = await _cst_unmet(answer, ledger, deadline)
                    if _cst_failed:
                        _CST["reentries"] = _CST["reentries"] + 1
                        messages.append({"role": "assistant", "content": answer})
                        messages.append({"role": "system", "content": _cst_order(_cst_failed)})
                        answer = ""
                        continue
                messages.append({'role': 'assistant', 'content': answer})
                break
    ...
    async def _solve(query: Query, question: str) -> Response:
        _CST["reentries"] = 0
        _CST_FAST["on"] = bool(getattr(query, "fast", False))
        _reset_run_state()
        ...
        # Resolve what a correct answer must be true of, before any evidence is gathered.
        if not _CST_FAST["on"]:
            try:
                ledger.set_conditions(question, await _cst_conditions(question, deadline))
            except Exception:
                pass
        answer = ''
        messages: list[dict] = []
        ...
        if not _is_usable_answer(answer):
            ...
            if _is_usable_answer(fallback):
                answer = fallback
        try:
            answer = await _ec_cycle(
                question, answer, messages, ledger, deadline, _ec_Hooks(loop=_loop, search=_do_search, commit=_commit_tool_output, cited=_cited_numbers, normalize=_normalize_brackets, usable=_is_usable_answer, spend=_spend_left, stop=_STOP, cite_re=_CITE_MARK_RE))
        except Exception:
            pass
        try:
            citations, _slot_pos = _citations_for(answer, ledger)
            citations = _ec_reslice(citations, answer, ledger, _ec_Hooks(loop=_loop, search=_do_search, commit=_commit_tool_output, cited=_cited_numbers, normalize=_normalize_brackets, usable=_is_usable_answer, spend=_spend_left, stop=_STOP, cite_re=_CITE_MARK_RE))
        except Exception:
            citations, _slot_pos = ([], {})
        answer = _normalize_brackets(answer)
...
"""
    ...
    async def _cst_chat(system: str, user: str, max_tokens: int, timeout: float) -> str:
        return await _chat(system, user, models=UTILITY_MODELS,
                           max_tokens=max_tokens, timeout=timeout, total_budget=timeout + 6.0)
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            answer = await _ec_cycle(
                plan, answer, messages, ledger, deadline, _ec_Hooks(loop=_loop, search=_do_search, commit=_commit_tool_output, cited=_cited_numbers, normalize=_normalize_brackets, usable=_is_usable_answer, spend=_spend_left, stop=_STOP, cite_re=_CITE_MARK_RE))
        except Exception:
            pass
        try:
...
"""
    ...
    return query
_sonylkaneq = _fxeuefingo()
_dphlsbmfho = _dtglfzqsoo()
_pumvsyzmrj = _pnhtlkivzr()
_ywfcctxdmp = _zxlkhtjzfk()
_ssqnbqnwvx = 290.0
_rafiuqtkhs = 250.0
_aglujrzqlx = 90.0

async def _raykvvhsyt(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _ssqnbqnwvx - (time.monotonic() - started)
        if first:
            budget = _rafiuqtkhs if _rafiuqtkhs < remaining else remaining
            first = False
        else:
            if remaining < _aglujrzqlx:
                break
        ...
        try:
            ...
            last_exc = exc
    return _dbhyzepajl(query)
_ecbrfiseyf = 235.0

async def _fsoqlgataf(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _mryruafnhv(query, agents):
    if len(agents) < 2:
        return await _raykvvhsyt(query, agents)
    r0, r1 = await asyncio.gather(_fsoqlgataf(agents[0], query, _ecbrfiseyf), _fsoqlgataf(agents[1], query, _ecbrfiseyf))
    ok0 = isinstance(r0, Response) and _witnttwvbv(r0) != ''
    ok1 = isinstance(r1, Response) and _witnttwvbv(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_xpkrkluhdw(provider='openrouter', messages=msgs, model='z-ai/glm-5.2', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
async def _drv_base_query(query: Query) -> Response:
    _yxjszchphy['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _raykvvhsyt(query, (_pumvsyzmrj, _sonylkaneq, _dphlsbmfho, _ywfcctxdmp))
        index = _aqjpctjuew(query)
        if index == 0:
            agents = (_sonylkaneq, _dphlsbmfho, _pumvsyzmrj, _ywfcctxdmp)
        elif index == 1:
            agents = (_dphlsbmfho, _pumvsyzmrj, _ywfcctxdmp, _sonylkaneq)
        elif index == 2:
            agents = (_pumvsyzmrj, _ywfcctxdmp, _sonylkaneq, _dphlsbmfho)
        elif index == 3:
            agents = (_ywfcctxdmp, _sonylkaneq, _dphlsbmfho, _pumvsyzmrj)
        else:
            agents = (_sonylkaneq, _dphlsbmfho, _pumvsyzmrj, _ywfcctxdmp)
        return await _mryruafnhv(query, agents)
    except Exception:
        return _dbhyzepajl(query)
_BUILDU_TAG_m30 = "sn45-68897fad8d70"

# --- drv wrap: claim-conflict ledger (start) ---
# batch_tag='drv109' salt='f7a6d0bcdebf'
