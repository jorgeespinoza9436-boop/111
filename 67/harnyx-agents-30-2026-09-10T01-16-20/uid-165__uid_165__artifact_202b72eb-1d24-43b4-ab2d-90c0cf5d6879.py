...
from time import monotonic as _pv_monotonic
...
PV_MIN_LEFT_S = 70.0
# A round of the cycle costs an audit call plus, when a figure is unbacked, a
# targeted search and a re-entry of the research loop over the full history --
# on a large context that is more than the $0.116 a single call can cost. A
# floor that only covers the audit lets the cycle spend the run's last dollars
# and the platform aborts the invocation with an empty response, so the floor
# covers a whole round instead.
PV_MIN_USD = 0.2
PV_MAX_ROUNDS = 2
PV_MAX_CLAIMS = 4
PV_MAX_SEARCHES = 2
...
# The floor above was the only bound, so any expansion was accepted and a
# rewrite that restated its working alongside the answer always won. Batch
# 09_08 task 3d400220 shipped the same station lists four times over 5,298
# chars and lost to a 1,569-char rival on "highly repetitive".
PV_MAX_GROW_RATIO = 1.5
PV_FACT_KEEP_PCT = 70
...
def _pv_shipped_ref(ledger, n: int):
...
def _pv_widen(row: dict, a: int, b: int) -> None:
    ...
    if b - a > PV_SPAN_MAX:
        mid = (a + b) // 2
        a = max(0, mid - PV_SPAN_MAX // 2)
        b = min(note_len, a + PV_SPAN_MAX)
    kept = row.setdefault("retained", [])
    for i, (ka, kb) in enumerate(kept):
        if a <= kb and ka <= b:
            kept[i] = (min(ka, a), max(kb, b))
            return
    if len(kept) >= PV_RETAIN_MAX:
        return
    kept.append((a, b))


def _pv_claims(answer: str, ledger, h: dict) -> list:
    """(sentence, figures, cited rows) for every cited sentence carrying a figure."""
    out = []
    for m in _PV_SENT_RE.finditer(h.normalize(answer or "")):
...
def _pv_audit(answer: str, ledger, h: dict) -> tuple:
    ...
    cycle is complete."""
    unanchored, missing = [], []
    for sent, figs, rows in _pv_claims(answer, ledger, h):
        shipped = " ".join(_pv_shipped_text(ledger, n) for n in rows)
        for fig in figs:
            if _pv_pattern(fig).search(shipped):
                continue
            located = None
            for n in rows:
                hit = _pv_locate(ledger.rows[n - 1].get("text") or "", fig)
                if hit is not None:
...
def _pv_reanchor(unanchored: list, ledger) -> int:
    ...
    for _sent, _fig, (n, pos, length) in unanchored:
        ...
        _pv_widen(row, pos - PV_MARGIN_CHARS, pos + length + PV_MARGIN_CHARS)
...
def _pv_salient(sent: str, h: dict) -> list:
    out = []
...
async def _pv_retrieve(missing: list, ledger, h: dict) -> str:
    ...
    for sent, fig, _rows in missing[:PV_MAX_SEARCHES]:
        probe = " ".join(_pv_salient(sent, h)[:6] + [fig]).strip()
        if not probe:
            continue
        try:
...
async def _pv_regenerate(question: str, answer: str, messages: list, ledger,
    ...
    order = ("PROVENANCE CHECK. A claim is credited only when the shipped citation "
             ...
             "The rewrite REPLACES the current answer -- do not restate a section it "
             "already contains, and do not append your working to it. Write the answer "
             "only: never mention this check, the tools, or what any tool did.")
    if evidence:
        order += "\n\nNEW EVIDENCE:\n" + evidence
    order += "\n\nCURRENT ANSWER:\n" + answer[:PV_ANSWER_CHARS]
    carry = list(messages)
    carry.append({"role": "system", "content": order})
    try:
        revised, _ = await h.loop(question, "", ledger, deadline, PV_LOOP_TURNS,
    ...
    if not h.usable(revised):
        return answer
    if len(revised) < int(len(answer) * PV_MIN_KEEP_RATIO):
        return answer
    if len(revised) > int(len(answer) * PV_MAX_GROW_RATIO):
        return answer
    if not _pv_keeps_facts(answer, revised):
        return answer
    _PV_STATS["adopted"] = _PV_STATS.get("adopted", 0) + 1
    return revised


async def _pv_cycle(question: str, answer: str, messages: list, ledger,
    ...
    for _round in range(_PV_STATS.get("rounds", PV_MAX_ROUNDS)):
        if (deadline - _pv_monotonic()) < PV_MIN_LEFT_S or h.spend() < PV_MIN_USD:
            return answer
        unanchored, missing = _pv_audit(answer, ledger, h)
        if not unanchored and not missing:
            return answer
        _pv_reanchor(unanchored, ledger)
        if not missing or not messages:
            continue
        evidence = await _pv_retrieve(missing, ledger, h)
        answer = await _pv_regenerate(question, answer, messages, ledger, deadline,
                                      missing, evidence, h)
    return answer


PV_ANCHORED_SPAN = 2000
PV_MAX_REF_CHARS = 14000


def _pv_board(answer: str, ledger, h: dict) -> dict:
    ...
    for sent, figs, rows in _pv_claims(answer, ledger, h):
        for fig in figs:
            for n in rows:
                ...
                if hit is not None:
                    pos, length = hit
                    board.setdefault(n, []).append((pos - PV_MARGIN_CHARS, pos + length + PV_MARGIN_CHARS))
                    break
    return board


def _pv_reslice(citations, answer: str, ledger, h: dict):
    ...
    if not _PV_STATS.get("reslice", True):
        return citations
    board = _pv_board(answer, ledger, h)
    if not board:
        return citations
    ...
    for ref in citations:
        ...
        for a, b in [(int(a), int(b)) for a, b in (row.get("retained") or [])] + board[n]:
            a = max(0, min(int(a), note_len))
            b = max(a + 1, min(int(b), note_len))
            pad = max(0, PV_ANCHORED_SPAN - (b - a))
            a = max(0, a - pad // 2)
        ...
        while merged and total > PV_MAX_REF_CHARS:
        ...
        try:
            ...
            out.append(ref)
    _PV_STATS["resliced"] = _PV_STATS.get("resliced", 0) + 1
    return tuple(out) if isinstance(citations, tuple) else out



def _compose_branch_2fe5626d0cb17005e9927e14e65150eb9e38226cfeaf631f111da1bbbbe60fcf_entry():

    # fork of 7_uid_93_score_0.575.py: added cov:quantitative:finish + drv:primary_secondary:board_only (variant u93)
    ...
    # invocation and the response is empty, so the reserves below stop this
    # branch while it can still deliver what it has. Measured on batch 09_07: a
    # task costs $0.09 at the median and $0.20 at p90, while a single
    # large-context call costs up to $0.116 -- so research stops with one such
    # call still affordable, and the answer turn is never the call that is
    # refused.
    ...
    _unmetered_tooling_info = tooling_info

    async def tooling_info(**kwargs):
        """tooling_info, read for the session budget it reports before any spend."""
        payload = await _unmetered_tooling_info(timeout=kwargs.get("timeout"))
        _note_session_budget(payload)
        return payload
    from harnyx_miner_sdk.decorators import entrypoint
    ...
    # --- element-coverage controller [u93-cov-quantitative-finish: quantitative-figure coverage, finish-gate re-entry] --------
    ...
    def _is_usable_answer(text: str) -> bool:
        ...
        if _STUB_ANSWER_RE.match(s) or _is_degenerate_repetition(s):
            return False
        # A refusal is a refusal however well cited and however long, and this
        # test has to run BEFORE the cited shortcut below to catch one. Scoring
        # is pairwise: an answer that declines loses outright, so the repair
        # loop gets another turn instead of shipping it.
        if _REFUSAL_ONLY_RE.match(s) or _ABSTAIN_LEAD_RE.search(s[:_ABSTAIN_LEAD_CHARS]):
            return False
        # Talk about our own tooling is narration, not answer. Checked after the
        # branch's own lead-stripping, so narration that strips cleanly and
        # leaves a cited answer behind is still accepted.
        if _PROCESS_NARRATION_RE.search(s):
            remainder = _strip_lead_narration(s)
    ...
    def _sn67_null_schema_status(schema):
        """True/False/None: null valid/invalid/unsupported; bounded, no imports or IO."""
        active = set()
        work = [0]
        ...
        def check(node, depth):
            ...
            if not isinstance(vocabulary, dict) or any(
                required and name not in [
                    "https://json-schema.org/draft/2020-12/vocab/" + suffix
                    for suffix in vocabularies
            ...
            try:
                ...
                if "$ref" in node:
                    ...
                    if reference != "#" and not reference.startswith("#/"):
                        return None
                    target = schema
                    for token in reference[2:].split("/") if reference != "#" else ():
                        work[0] += 1
                        if work[0] > 3000 or "~" in token.replace("~0", "").replace("~1", ""):
                            return None
                        token = token.replace("~1", "/").replace("~0", "~")
                        if isinstance(target, dict):
                            if "$id" in target or token not in target:
                                return None
                            target = target[token]
                        elif isinstance(target, list):
                            if not token.isascii() or not token.isdecimal() or (len(token) > 1 and token[0] == "0"):
                                return None
                            if len(token) > 8 or int(token) >= len(target):
                                return None
                            target = target[int(token)]
                        else:
                            return None
                    outcomes.append(check(target, depth + 1))
                if "type" in node:
                    kinds = node["type"] if isinstance(node["type"], list) else [node["type"]]
                    if not kinds or any(kind not in (
                        "null", "object", "array", "number", "integer", "boolean", "string",
                    ) for kind in kinds):
                        return None
                    outcomes.append("null" in kinds)
                if "const" in node:
                    outcomes.append(node["const"] is None)
                if "enum" in node:
                    if not isinstance(node["enum"], list) or not node["enum"]:
                        return None
                    outcomes.append(any(item is None for item in node["enum"]))
                for keyword in ("allOf", "anyOf", "oneOf"):
                    if keyword not in node:
                        continue
                    if not isinstance(node[keyword], list) or not node[keyword]:
                        return None
                    values = [check(child, depth + 1) for child in node[keyword]]
                    if keyword == "allOf":
                        result = conjunction(values)
                    elif keyword == "anyOf":
                        result = True if True in values else (None if None in values else False)
                    else:
                        positives = sum(value is True for value in values)
                        result = False if positives > 1 else (None if None in values else positives == 1)
                    outcomes.append(result)
                if "not" in node:
                    result = check(node["not"], depth + 1)
                    outcomes.append(None if result is None else not result)
                if "if" in node:
                    condition = check(node["if"], depth + 1)
                    if condition is None:
                        outcomes.append(None)
                    else:
                        outcomes.append(check(node.get("then" if condition else "else", True), depth + 1))
                # Other standard validation keywords apply only to non-null types;
                # unknown keywords are annotations under the pinned host dialect.
    ...
    async def _schema_output(question: str, answer: str, schema, deadline: float) -> object | None:
        ...
        for lane, model in ((LLM_LANE_A, SCHEMA_MODEL),
            ...
            try:
                ...
                if value is None:
                    if _sn67_null_schema_status(schema) is True:
                        return None
                    continue
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            answer = await _pv_cycle(
                question, answer, messages, ledger, deadline, _pv_Hooks(loop=_loop, search=_do_search, commit=_commit_tool_output, cited=_cited_numbers, normalize=_normalize_brackets, usable=_is_usable_answer, spend=_spend_left, stop=_STOP, cite_re=_CITE_MARK_RE))
        except Exception:
            pass
        try:
            citations, _slot_pos = _citations_for(answer, ledger)
            citations = _pv_reslice(citations, answer, ledger, _pv_Hooks(loop=_loop, search=_do_search, commit=_commit_tool_output, cited=_cited_numbers, normalize=_normalize_brackets, usable=_is_usable_answer, spend=_spend_left, stop=_STOP, cite_re=_CITE_MARK_RE))
        except Exception:
            citations, _slot_pos = [], {}
    ...
    # --- drv wrap: primary_secondary dual-corpus reconciler (start) ---
    # variant u93-drv-primary_secondary-board_only: primary document + authoritative secondary corpus; trigger = board deficiency only (strict)
    ...
    # the second-pass driver imported the SDK entry points directly, which would
    # walk straight past the branch's reserves; it uses the metered ones instead
    _drv_fetch_page = fetch_page
    _drv_llm_chat = llm_chat
    _drv_search_web = search_web
    from harnyx_miner_sdk.decorators import entrypoint
    ...
    _DRV_TAG = 'u93-drv-primary_secondary-board_only'
    _DRV_SALT = '85a82f430620'
    ...
    async def _drv_audit_board(question: str, draft: str, schema, citations: list) -> _DrvBoard:
        ...
        system = (
            "You audit a research draft against a user question. Focus first on whether each load-bearing claim traces to the ORIGINATING document rather than a secondary retelling. "
    ...
    async def _drv_retrieve_dual_corpus(question: str, claims: list[str]) -> list[dict]:
        """primary document + authoritative secondary corpus: two corpus-specific searches plus one fetch of the first corpus."""
        focus = "; ".join(claims[:3]) if claims else question[:180]
        first_q = " ".join((question[:120], focus[:140], "original filing press release dataset announcement")).strip()[:280]
        second_q = " ".join((question[:120], focus[:140], "analysis explainer authoritative summary")).strip()[:280]
        rows: list[dict] = []
        ...
        fetch_url = ""
        if first_row:
            first_row["corpus"] = "primary_document"
            fetch_url = first_row.get("url") or ""
            rows.append(first_row)
        if second_row:
            second_row["corpus"] = "authoritative_secondary"
        ...
        if fetch_url:
            ...
            if fetched_row:
                fetched_row["corpus"] = "primary_document_document"
    ...
    async def _drv_regenerate(
        ...
        if is_text:
            system = (
                "Rewrite the research answer after a second retrieval pass over the originating primary document and an authoritative secondary source. Return JSON only with keys "
            ...
            system = (
                "Rewrite the structured research answer after a second retrieval pass over the originating primary document and an authoritative secondary source. Return JSON only "
    ...
    async def query(query: _DrvQuery) -> _DrvResponse:
        ...
        try:
            ...
            return draft
    # --- drv wrap: primary_secondary dual-corpus reconciler (end) ---

    return query

_branch_2fe5626d0cb17005e9927e14e65150eb9e38226cfeaf631f111da1bbbbe60fcf_query_entry = _compose_branch_2fe5626d0cb17005e9927e14e65150eb9e38226cfeaf631f111da1bbbbe60fcf_entry()


def _compose_branch_8a4f3e894b2b731883cdfe300b63acfa62f81cef983986606fe68e7326f8d5fb_entry():
...
_branch_8a4f3e894b2b731883cdfe300b63acfa62f81cef983986606fe68e7326f8d5fb_query_entry = _compose_branch_8a4f3e894b2b731883cdfe300b63acfa62f81cef983986606fe68e7326f8d5fb_entry()


def _compose_branch_14db098054cb85ad6f8d7a88c80a530f4017e8b2b9d45046ccd6c6942ca62c3a_entry():
    ...
    class _ResultIndex:
        ...
        def retain_citable(self, number: int, start: int, end: int) -> None:
            ...
            A scan window has already been shown to the model inline, in the
            scan's own output; putting it back into the surfaced text would send
            a second copy and pay for it. What it has to reach is the citation
            array: a figure read here is cited to this page, so the range offered
            as that page's support has to be one that prints it.
            """
            meta = self._by_number.get(number)
            if meta is None:
                return
        ...
        def citable_spans(self, number: int) -> list[tuple[int, int]]:
            ...
            return _merge_spans(
                list(self._spans.get(number) or ())
                + list(self._cite_spans.get(number) or ())
            )
    ...
    def _citations_from_inline_markers(
        ...
        for key in source_order:
            ...
            while spans and cost > budget:
                # Drop what proves least, not what is narrowest: the widest range
                # is usually the source's opening, and the narrowest is the window
                # the figure was read out of. Width breaks a tie, as it used to.
                spans.remove(min(
                    spans,
                    key=lambda span: (_span_support(note_text, span, anchors),
                                      span[1] - span[0]),
                ))
                cost = sum(e - s for s, e in spans)
            if not spans:
    ...
    # ══════════════════════════════════════════════════════════════════════════════
    # Shipped-evidence provenance cycle (result-index lineage)
    # ══════════════════════════════════════════════════════════════════════════════
    # The grader credits a claim only when the citation excerpt that actually ships
    # contains the supporting text. Citations here are cut from the regions each
    # source was read from (or anchored around the claim), so a figure can be cited
    # from the right page and still ship outside its excerpt. After the amend stage
    # has decided the answer, this cycle audits it against exactly the slices the
    # citation builder would ship and, while a cited figure is unbacked, re-enters
    # the evidence and research stages: a figure printed on the cited page outside
    # its slice is retained as a shown region of that source (index update, no model
    # call); a figure no cited page prints triggers a targeted search recorded on
    # the index and one bounded commit call that rewrites the answer against it.
    # The audit repeats until every shipped slice carries its figure or the round
    # cap is reached.
...
_branch_14db098054cb85ad6f8d7a88c80a530f4017e8b2b9d45046ccd6c6942ca62c3a_query_entry = _compose_branch_14db098054cb85ad6f8d7a88c80a530f4017e8b2b9d45046ccd6c6942ca62c3a_entry()
...
def _task_shape_route_index(query: Query) -> int:
    if getattr(query, "output_schema", None) is not None:
        return 0
    text = (getattr(query, "text", "") or "").casefold()
    for punctuation in "\n\t,.;:!?()[]{}%-":
        text = text.replace(punctuation, f" {punctuation} " if punctuation == "%" else " ")
    text = " " + " ".join(text.split()) + " "
    if any(signal in text for signal in _TASK_SHAPE_SECONDARY_SIGNALS):
        return 1
    return 2


class Branch_2fe5626d0cb17005e9927e14e65150eb9e38226cfeaf631f111da1bbbbe60fcf:
    async def __call__(self, query: Query) -> Response:
        return await _branch_2fe5626d0cb17005e9927e14e65150eb9e38226cfeaf631f111da1bbbbe60fcf_query_entry(query)


class Branch_8a4f3e894b2b731883cdfe300b63acfa62f81cef983986606fe68e7326f8d5fb:
    async def __call__(self, query: Query) -> Response:
        return await _branch_8a4f3e894b2b731883cdfe300b63acfa62f81cef983986606fe68e7326f8d5fb_query_entry(query)


class Branch_14db098054cb85ad6f8d7a88c80a530f4017e8b2b9d45046ccd6c6942ca62c3a:
    async def __call__(self, query: Query) -> Response:
        return await _branch_14db098054cb85ad6f8d7a88c80a530f4017e8b2b9d45046ccd6c6942ca62c3a_query_entry(query)


_TASK_SHAPE_PRIMARY_AGENT = Branch_2fe5626d0cb17005e9927e14e65150eb9e38226cfeaf631f111da1bbbbe60fcf()
_TASK_SHAPE_SECONDARY_AGENT = Branch_8a4f3e894b2b731883cdfe300b63acfa62f81cef983986606fe68e7326f8d5fb()
_TASK_SHAPE_TERTIARY_AGENT = Branch_14db098054cb85ad6f8d7a88c80a530f4017e8b2b9d45046ccd6c6942ca62c3a()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "Branch_2fe5626d0cb17005e9927e14e65150eb9e38226cfeaf631f111da1bbbbe60fcf",
    "Branch_8a4f3e894b2b731883cdfe300b63acfa62f81cef983986606fe68e7326f8d5fb",
    "Branch_14db098054cb85ad6f8d7a88c80a530f4017e8b2b9d45046ccd6c6942ca62c3a",
)
...
@entrypoint("query")
async def query(query: Query) -> Response:
    _bs = _BUILD_SALT_647bb806
    _bs = (_bs * 2 - _bs) - _BUILD_SALT_647bb806
    route_index = _task_shape_route_index(query)
    if route_index == 0:
        branch = _TASK_SHAPE_PRIMARY_AGENT
    elif route_index == 1:
        branch = _TASK_SHAPE_SECONDARY_AGENT
    else:
        branch = _TASK_SHAPE_TERTIARY_AGENT
    return await branch(query)

# --- build 647bb806 ---------------------------------------------------------
_BUILD_647bb806 = "20260909T150000Z"


def _build_salt_647bb806(tag: str) -> int:
...
_BUILD_SALT_647bb806 = _build_salt_647bb806(_BUILD_647bb806)
