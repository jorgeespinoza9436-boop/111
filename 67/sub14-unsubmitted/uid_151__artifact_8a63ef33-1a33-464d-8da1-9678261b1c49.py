...
async def _set_completeness_guard(question: str, answer: str,
                                  ledger: EvidenceLedger, deadline: float) -> str:
    """One extra roster enumeration + one synthesis pass to close a truncated
    set/list. Non-destructive: returns the original answer on any doubt."""
    ...
    roster_q = ""
    for q in queries:
        if q.lower().startswith("complete list of"):
            roster_q = q
            break
    if not roster_q:
        roster_q = queries[0]
    if not roster_q.strip():
        return answer

    before_rows = len(ledger.rows)
    try:
        out = await _do_search(roster_q, ledger)
        _commit_tool_output(out, ledger)
    except Exception:
        return answer

    if len(ledger.rows) <= before_rows:
        return answer
    left = deadline - monotonic()
    if left < 20.0:
        return answer
    ...
    if not digest.strip():
        return answer
    system = (
        "You extend a finished research answer ONLY to close a truncated "
        "set/list/count. You are given the current ANSWER and a numbered "
        "EvidenceLedger that now includes a fresh 'complete list' enumeration of "
        "the subject. Keep the existing answer's wording, entities, and [n] "
        "citations. Then add ANY qualifying member the answer missed but the "
        "ledger now supports, each with its own [n] citation copied from a real "
        "row. Do NOT remove, weaken, or restyle anything already correct, and do "
        "NOT invent members not grounded in a ledger row. If the answer is "
        "already complete, return it unchanged. Output the COMPLETE final answer "
        "in the same shape, plain prose, no tool syntax.")
    user = (f"QUESTION:\n{question}\n\nCURRENT ANSWER:\n{answer[:11000]}\n\n"
            f"EVIDENCE LEDGER (numbered):\n{digest}")
    try:
        revised = await _chat_simple(
            LLM_LANE_A, LOOP_MODEL_A, system, user,
            max_tokens=2600,
            timeout=max(8.0, min(AUDIT_TIMEOUT_S, left - 66.0)),
            think=_least_think(LLM_LANE_A, LOOP_MODEL_A))
    except Exception:
        return answer
...
async def _solve(query: Query, question: str) -> Response:
    ...
    try:
        if _is_usable_answer(answer) \
                ...
                and (deadline - monotonic()) > 72.0 and _spend_left() >= AUDIT_MIN_USD:
            completed = await _set_completeness_guard(question, answer, ledger, deadline)
            if _is_usable_answer(completed):
                answer = completed
    except Exception:
        pass
...
TASK_RESCUE_VERSION = "v238.4-uid212-contract-log-rescue"
...
_V238_WEAK_NOTES = '["3818d8c9:0.00", "73bc0e87:0.00", "62b1353b:0.10", "fd066a4c:0.10", "0cb9796e:0.20"]'
