#!/usr/bin/env bash
set -euo pipefail
# Submit all matched sub14-unsubmitted agents in parallel.
# Usage: bash /root/turtle/111/67/sub14-unsubmitted/submit_all.sh [PARALLEL_JOBS]
ROOT="/root/turtle/harnyx"
JOBS_FILE="/root/turtle/111/67/sub14-unsubmitted/submit_jobs.txt"
PARALLEL_JOBS="${1:-24}"
WALLET="money"
cd "$ROOT"
