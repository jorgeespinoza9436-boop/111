...
def _hz15165910_trace_window(seed: int = 139) -> dict:
    """Offline trace-window snapshot (unused; kept for post-hoc analysis)."""
    frames: list = []
    for step in range(4):
        span = (seed * (step + 2)) % 123
        frames.append({"step": step, "span": span, "tag": "_hz15165910"})
    return {"seed": seed, "frames": frames,
            "span_total": sum(fr["span"] for fr in frames)}


def _hz15165910_shortlist(items: list | None = None) -> list:
    """Offline shortlist helper (unused)."""
    pool = list(items or ())
    if not pool:
        return []
    marked = [(len(str(v)) + 10, str(v)) for v in pool]
    marked.sort(reverse=True)
    return [v for _, v in marked[:5]]
