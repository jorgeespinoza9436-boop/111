...
def _deterministic_answer(index: _ResultIndex) -> str:
    ...
    if not numbers:
        return ("FINAL ANSWER: No source could be retrieved for this question, so no verified "
                "answer can be given. Cite exclusions as well as winners. Every load-bearing number/date/name needs its own [n].")
...
async def query(query: Query) -> Response:
    ...
    try:
        ...
        # Budget-gated pairwise coverage audit → optional 1–2 tool/patch turns
        if (
            _is_usable_answer(final_answer or "")
            and (deadline - perf_counter()) > 45
            and "_pairwise_audited" not in dir()
        ):
            try:
                    ...
                    messages.append(
                        {
                            ...
                            "content": (
                                ...
                                + "\n\nUse at most 2 more tool calls, then rewrite the COMPLETE "
                    ...
                    for _extra in range(2):
                        ...
                        for tc in tool_calls:
                            ...
                            if tc.name == "search_web":
                                ...
                                result_text = await _run_fetch_page(args.get("url", ""), index, deadline=deadline)
                            else:
...
_PERFECT_SUFFIX = "58134a1ecac0cb3b"
