"""SN67 Harnyx miner — v25 deep-research pipeline. [slot 55 build 2026-07-22T15:00:00+00:00]"""
...
PRODUCTION_PROFILE = "upload_safe_accuracy_optimized"  # slot 55
...
DELIVER_GUARD_TEMPERATURE = 0.40
...
def _build_bounded_pool_plan_messages(
    *, question: str, existing_packets: tuple[AcceptedEvidence, ...],
    ...
    system_content = (
        "## Role\n\nbounded-pool planner for a research pipeline. Decide whether the question ranges over a "
    ...
    for criterion in plan.criteria[:POOL_SWEEP_MAX_CRITERIA]:
        ...
        for result in tuple(getattr(response, "results", ()) or ()):
            ...
            candidates.append(EvidenceCandidate(
                ...
                slot_intent=_slot_intent_for_slot(PRIMARY_SOURCE_SLOT_ID),
                text_part="search_snippet", text_start=0, text_end=len(note),
                receipt_id=receipt_id, result_id=f"P{counter}",
                url=url, title=getattr(result, "title", None), source_text=note,
                query=query, source_kind=SEARCH_SNIPPET_EVIDENCE_SOURCE_KIND,
            ))
...
async def _deliver_guard_synthesis(
    *, messages: Sequence[Mapping[str, object]], deadline: float,
) -> str | None:
    plan = _budget_matched_synthesis_plan(deadline - perf_counter())
    if plan is None:
    ...
    try:
        response = await llm_chat(
            provider=_LLM_PROVIDER, messages=messages, model=DELIVER_GUARD_MODEL,
            temperature=DELIVER_GUARD_TEMPERATURE,
...
def _build_numeric_audit_messages(
    *, question: str, draft: str, accepted_packets: tuple[AcceptedEvidence, ...],
    ...
    system_content = (
        "# Precision and Completeness Auditor\n\n"
        "## Role\n\n"
        "Precision-and-completeness auditor for an evidence-gated pipeline. You receive a "
...
def _build_evidence_search_target_messages(
    *, question: str, round_index: int, tried_queries: tuple[str, ...],
    ...
    system_content = (
        "# Evidence-Search Target Planner\n\n"
        "## Role\n\n"
        "Evidence-source inventory analyst for a deep-research answer. You are not an answer writer and you are not a search-query writer. Your job is to describe the source inventory that would let Python build evidence-seeking search queries: entities, aliases, official source families, document handles, metric terms, date/scope terms, must-include terms, avoid terms, and optional site constraints.\n\n"
        "## Output\n\n"
        "Exactly:\n"
        "```json\n"
        "{\"evidence_targets\":[{\"slot_id\":\"...\",\"slot_intent\":\"...\",\"needed_source_text\":\"...\",\"source_type\":\"...\",\"inventory\":{\"entities\":[],\"aliases\":[],\"source_families\":[],\"document_handles\":[],\"metric_terms\":[],\"date_scope\":[],\"must_include\":[],\"avoid\":[],\"site_constraints\":[]}}]}\n"
        "```\n\n"
        "Use `slot_intent` only for `free_1` / `free_2`. No routes, no query fields, no markdown, no reasons, no extra keys.\n\n"
        f"## Count\n\nReturn 2-{MAX_EVIDENCE_TARGETS_PER_ROUND} `evidence_targets`. Each target may have inventory arrays with 1-6 concise terms each.\n\n"
        "## Absence / False-Premise Rule\n\n"
        "For questions of the form `which X were [state Y] during [period Z]` "
        ...
        "## Dual-Document Rule\n\n"
        "When the question explicitly names two different official documents, filings, "
        ...
        "When the question asks HOW something is calculated (e.g. `how does X "
        ...
        "## Coverage-Decomposition Rule\n\n"
        "Decompose the question into EVERY distinct fact it explicitly "
        "requests and emit a separate `evidence_target` for each one a single existing target does not "
        "already cover.\n\n"
        "1. Each (entity x requested attribute) pair: if one attribute is asked for two "
        "entities, emit one target per entity; if one entity is asked for two attributes, emit one target "
        "per attribute.\n"
        "2. Full enumerations: a `which / list / name all X` requirement gets a target "
        "whose `must_include` drives the COMPLETE set (e.g. `all`, `each`, `every`, the named count), not a "
        "single example.\n"
        "3. Secondary / special-category items joined by `as well as`, `including`, "
        "`and also`, or `a lower / separate threshold for [category]`: these qualifying sub-clauses are "
        "REQUIRED facts, not optional context — each gets its own target.\n"
        "4. A comparison baseline named in a sub-clause (e.g. `compared to the [poll / forecast / prior estimate / projection]`) gets its "
        "own target so the baseline value is retrieved, not just the headline value.\n\n"
        "Prefer covering one more required sub-element over adding depth to an already-covered one.\n\n"
        "## Wrong-Entity Rule\n\n"
        "If `CANDIDATE_WRONG_ADJACENT_ENTITIES` is provided, review each entity and "
...
def _build_evidence_search_route_messages(
    *, question: str, round_index: int, targets: tuple[EvidenceSearchTarget, ...],
    ...
    system_content = (
        "## Role\n\nevidence-search query writer. You receive source-inventory targets from a planner. Your job is to write the exact search strings that should be sent to a web search tool.\n\n"
        "## Output\n\nJSON ONLY: {\"queries\":[{\"target_id\":\"target_1_1\",\"query\":\"specific evidence-seeking query\",\"site_constraints\":[\"example.org\"]}]}. No markdown, no reasons, no extra keys.\n\n"
        f"## Diversity\n\nproduce at most {MAX_QUERY_ROUTES_PER_TARGET} queries per target. Return a compact set of high-recall queries now."
    )
    user_content = (
        f"Current date: {_current_date()}.\nSearch round: {round_index}\n\nOriginal question:\n{question}\n\n"
...
def _build_search_result_source_labeler_messages(
    *, question: str, targets: tuple[EvidenceSearchTarget, ...],
    ...
    system_content = (
        "## Role\n\nsearch-result source labeler. Label result value; do not answer, select winners, or drop ambiguous results.\n\n"
        "## Output\n\nJSON ONLY: {\"labels\":[{\"basis\":\"...\",\"result_id\":\"R1\",\"target_ids\":[\"target_1_1\"],\"source_value\":\"direct\",\"source_kind\":\"official\",\"surface\":\"both\"}]}. No markdown. No comments. No extra keys.\n\n"
        "## Values\n\nsource_value=direct|primary_locator|context|contradiction|absence|weak|wrong. source_kind=official|primary|academic|government|regulatory|company|data_source|reputable_media|secondary|forum_social|aggregator|weak_unknown|wrong_source. surface=snippet|detail|both|locator|background|wrong."
    )
    user_content = (
        f"Current date: {_current_date()}.\n\nOriginal question:\n{question}\n\n"
...
def _build_observation_evidence_gate_messages(
    *, question: str, loop_index: int, existing_packets: tuple[AcceptedEvidence, ...],
    ...
    system_content = (
        "## Role\n\nevidence admission + observation extractor. Admit only candidate.source_text that directly supports contract-role observations.\n\n"
        "## Output\n\nexactly accepted_candidates and observations. accepted_candidates is an ordered array of objects with order_basis first and candidate_id second.\n\n"
        f"## Budget\n\nmax {MAX_ACCEPTED_IDS_PER_GATE} accepted candidates. Prefer fewer strong candidates, ordered by answer-role importance.\n\n"
...
def _build_chunk_cue_pattern_messages(
    *, question: str, loop_index: int, role_ledger: tuple[ResearchPlanRole, ...],
    ...
    system_content = (
        "## Role\n\nstructural regex cue generator. Return Python re patterns that locate likely evidence chunks.\n\n"
        "## Output\n\nexactly {\"patterns\":[{\"role_id\":\"...\",\"pattern\":\"...\"}]}. role_id is copied from ROLE_LEDGER. No reasons, no markdown, no extra keys.\n\n"
        f"## Budget\n\nmax {MAX_CHUNK_CUE_PATTERNS_TOTAL} total and {MAX_CHUNK_CUE_PATTERNS_PER_ROLE} per role."
    )
    user_content = (
...
def _build_chunk_lexical_anchor_messages(
    *, question: str, loop_index: int, role_ledger: tuple[ResearchPlanRole, ...],
    ...
    valid_role_ids = [r.role_id for r in role_ledger]
    system_content = (
        "## Role\n\nlexical evidence-neighborhood anchor generator. Return literal phrase groups that help Python locate chunks.\n\n"
        "## Output\n\nexactly {\"anchor_sets\":[{\"role_id\":\"...\",\"all\":[],\"any\":[],\"near\":[],\"avoid\":[]}]}. role_id is copied from ROLE_LEDGER. Terms are literal strings, not regex.\n\n"
        f"## Budget\n\nmax {MAX_LEXICAL_ANCHOR_SETS_TOTAL} anchor sets total, {MAX_LEXICAL_ANCHOR_TERMS_PER_FIELD} terms per field, and {MAX_LEXICAL_ANCHOR_TERM_CHARS} chars per term."
    )
    user_content = (
        f"Current date: {_current_date()}.\nLoop index: {loop_index}\n\nOriginal question:\n{question}\n\nValid role IDs: {json.dumps(valid_role_ids, ensure_ascii=False)}\n\n"
        f"Current research-plan roles:\n{_format_records_section('ROLE_LEDGER', 'role', _role_ledger_prompt_payload(role_ledger))}\n\n"
...
def _build_final_answer_messages(
    *, question: str, accepted_packets: tuple[AcceptedEvidence, ...],
    ...
    system_content = (
        "# Final Answer Writer\n\n"
        "## Role\n\n"
        "Final answer writer for an evidence-gated pipeline.\n\n"
        "## Evidence Scope\n\n"
        ...
        "## Answer Shape\n\n"
        "Start with a direct answer. For complex questions, explain the evidence-backed "
        "landscape: primary-source position, numbers/dates, comparators, mechanisms, actors, "
        "conflicts, uncertainty, missing evidence.\n\n"
        "## False Premise\n\n"
        "If accepted evidence disproves or fails to support a premise, say so in the "
        "first paragraph. Do not answer as if the premise were true.\n\n"
        "## False-Premise Completion Rule\n\n"
        "When the premise is false, ALWAYS follow with:\n"
        "1. the correct fact — what actually happened or exists;\n"
        "2. if a comparison remains valid after correcting the premise, provide it.\n\n"
        "Stopping at `the premise is false` without the corrected facts scores "
        "the same as an empty answer in pairwise evaluation.\n\n"
        "## Numeric Precision Rule\n\n"
        "When comparing statistical values, percentages, or financial estimates "
        ...
        "## Dual-Answer Completeness Rule\n\n"
        "When a question has two distinct sub-questions, provide a "
        ...
        "A question often names a specific source (e.g. `the Electoral "
        ...
        "## Exact-Value Rule\n\n"
        "When the question asks for a specific value — a precise date, a numeric "
        ...
        "## Claim-Binding Rule\n\n"
        "Attach a claim, filing, ruling, complaint, or accusation only to the "
        ...
        "## Asked-Scope Rule\n\n"
        "Answer with the value from the exact source, date, or scope the question "
        ...
        "## Definitive-Opening Rule\n\n"
        "Sentence one must deliver the requested answer itself — the "
        ...
        "first, context after.\n\n"
        "## Full-Roster Rule\n\n"
        "When the question asks which items qualify, or supplies its own "
        ...
        "## Adjacent-Citation Rule\n\n"
        "Every sentence asserting a number, date, name, or cause must "
        ...
        "## Named-Source Rule\n\n"
        "When the question pins a source (`according to Wikipedia`, `per "
        ...
        "## Commitment Rule\n\n"
        "While any relevant evidence exists, never write that the answer "
        ...
        "trailing clause after the committed answer.\n\n"
        "## Self-Consistency\n\n"
        "Before finishing, verify the committed answer is the same candidate your own cited sentences support — if the "
        ...
        "Cite packet numbers like `[1]` near claims. No generic padding, invented "
        "facts, pipeline talk, or hidden reasoning."
        "\n\n## Pairwise Coverage\n\n- Prefer a partial, cited answer over a refusal or cannot-be-determined wording.\n- Cover comparisons with each side cited plus a cited conclusion.\n- Cite every load-bearing claim with packet `[n]`, including non-qualifying exclusions when a pool is given.\n- Prefer few strong supporting notes over citation spam."
    )
...
# slot: 55 A7_pool 2026-07-22T15:00:00+00:00
