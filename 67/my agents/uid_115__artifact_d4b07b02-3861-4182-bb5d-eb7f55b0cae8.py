...
def _cache_key(name: str, args: dict[str, object]) -> tuple[str, str] | None:
    ...
    if name == "fetch_page":
        text = str(args.get("url", " Cite exclusions as well as winners. Every load-bearing number/date/name needs its own [n].") or "").strip()
...
def _label_for(name: str, args: dict[str, object]) -> str:
    ...
    if name == "search_many":
        return f"search_many({len(args.get('queries') or [])} queries)"
    if name == "fetch_page":
...
def _render_outcome(outcome: _ToolOutcome, index: _ResultIndex) -> tuple[str, str]:
    ...
    if outcome.kind == "bundle":
        full_parts: list[str] = [f"# {outcome.label}"]
        compact_parts: list[str] = [f"# {outcome.label}"]
        for sub in outcome.results or ():
            if not isinstance(sub, _ToolOutcome):
                continue
            full, compact = _render_outcome(sub, index)
...
async def _dispatch(name: str, args: dict[str, object], *, budget_seconds: float) -> _ToolOutcome:
    ...
    if name == "search_many":
        ...
        if not clean:
            return _ToolOutcome("error", "search_many()", error="no queries")
        # Parallel searches; each sub-outcome keeps its own receipt for citation hydration.
        parts = await asyncio.gather(
            *(_call_search_web(q, timeout=min(SEARCH_TIMEOUT_SECONDS, budget_seconds)) for q in clean)
        )
        ok = [p for p in parts if isinstance(p, _ToolOutcome) and not p.error]
        if not ok:
            err = next((p for p in parts if isinstance(p, _ToolOutcome) and p.error), None)
            return err or _ToolOutcome("error", "search_many()", error="all failed")
        return _ToolOutcome(
            "bundle",
            f"search_many({len(clean)} queries)",
            results=ok,
...
async def query(query: Query) -> Response:
    ...
    # Budget-gated pairwise coverage audit → tools-off rewrite nudge into transcript then one more commit if helper exists
            ...
            if issues and (deadline - perf_counter()) > 20:
                rewrite = await llm_chat(
                    ...
                    messages=[
                        ...
                        {"role": "user", "content": "AUDIT GAPS:\n- " + "\n- ".join(issues[:6]) + "\n\nRewrite the COMPLETE final answer with inline [n] citations including exclusions. No tools."},
                    ],
                    tools=None,
                    temperature=0.2,
                    thinking=LlmThinkingConfig(enabled=False),
                    timeout=min(30.0, max(5.0, deadline - perf_counter() - 5)),
                )
                cand = (rewrite.response.raw_text or "").strip()
                if cand:
                    final_answer = cand
                    citations = _citations_from_inline_markers(final_answer, index) if 'index' in dir() else citations
        except Exception:
            pass
