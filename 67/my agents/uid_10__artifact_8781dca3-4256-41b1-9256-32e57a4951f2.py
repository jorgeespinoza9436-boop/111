...
_SUBMISSION_SLOT = 'jackson_10'
PRODUCTION_PROFILE = 'JACKSON_10'
import asyncio
import json
import re
...
COMMIT_NUDGE = COMMIT_NUDGE + ' Cite exclusions as well as winners. Every load-bearing number/date/name needs its own [n].'
...
async def _pairwise_verify_patch(
    question: str,
    answer: str,
    messages: list,
    ledger: "_Ledger",
    ...
    """Budget-gated JSON audit; optional short re-entry for coverage/citation gaps."""
            ...
            [
                {
                    ...
                    "content": (
                        ...
                        "missing_elements, uncited_claims, suspect_attributions (arrays)."
                    ),
                },
                {
                    "role": "user",
                    "content": (
                        f"Audit vs question. JSON only.\n\nQuestion:\n{question}\n\n"
                        f"Answer:\n{answer[:12000]}"
                    ),
                },
            ],
    ...
    messages.append(
        {
            ...
            "content": (
                ...
                + "\n\nUse at most 2 more tool calls, then rewrite the COMPLETE final answer "
                "with inline [n] citations including exclusions."
            ),
        }
    )
    patched = answer
        ...
        for tc in tool_calls:
            ...
            try:
                if tc.name == "search_web":
                    ...
                    content = await _do_search_many(
                        qs if isinstance(qs, list) else [qs], ledger, time_left=time_left
                    )
                elif tc.name == "fetch_page":
                    # prefer _do_fetch if present
                    if "_do_fetch" in globals() or True:
                        try:
...
class _Ledger:
    ...
    def add(self, receipt_id: str, results: object, *, window: int) -> list[int]:
        ...
        for r in results or ():
            ...
            self._rows[self._n] = {'receipt_id': receipt_id, 'result_id': rid, 'window': window, 'note_len': len(note), 'text': note[:window], 'title': (getattr(r, 'title', None) or '')[:160], 'url': getattr(r, 'url', None) or ''}
...
def _build_citations(answer: str, ledger: _Ledger) -> list[CitationRef]:
    ...
    for n in _cited_numbers(answer, high=ledger.high()):
        ...
        if spent + end > EVIDENCE_CHAR_CAP:
            continue
        spent += end
        refs.append(CitationRef(receipt_id=str(row['receipt_id']), result_id=str(row['result_id']), slices=[CitationSlice(start=0, end=end)]))
    return refs
        ...
        for part in m.group(1).split(','):
            ...
            for n in candidates:
                if 1 <= n <= high and n not in seen:
                    ...
                    ordered.append(n)
    return ordered
def _diag_probe_58223(x=0):
    _acc = x
    for _i in range(4):
        _acc += _i * 5
    return _acc
def _diag_probe_63096(x=0):
    _acc = x
    for _i in range(3):
        _acc += _i * 1
    return _acc
async def _chat(messages: list[dict[str, object]], *, deadline: float, final: bool):
    ...
    for _ in range(LLM_TRY_PER_TURN):
        ...
        try:
            return await asyncio.wait_for(llm_chat(provider=LLM_PROVIDER, model=PRIMARY_MODEL, messages=messages, tools=None if final else _TOOL_SPECS, tool_choice=None if final else 'auto', temperature=0.2, thinking=thinking, timeout=to), timeout=to + 3.0)
        except Exception:
            continue
    return None
@entrypoint('query')
async def query(query: Query) -> Response:
    ...
    try:
            ...
            for tc in tool_calls:
                ...
                try:
                    if tc.name == 'search_web':
                        ...
                        content = await asyncio.wait_for(_do_fetch(str(args.get('url', '')), ledger, time_left=time_left), timeout=FETCH_TIMEOUT_S * FETCH_TRIES + 4.0)
                    else:
                        content = f'# unsupported tool {tc.name!r}'
                except Exception:
        ...
        try:
            ...
            pass
        return Response(text=FALLBACK_TEXT)
_TAG = '800a96e447c04bc490fe57a907211847'
