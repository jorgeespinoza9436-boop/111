...
async def _run_search_web(query: str, index: _ResultIndex) -> str:
    ...
    _ranked = _rank_search_results(result.results)
    numbers = index.record(result.receipt_id, _ranked, kind="search")
    lines = [f"# search_web({query!r}) -> {len(result.results)} results"]
    for n, r in zip(numbers, _ranked, strict=False):
        lines.append(
            f"[{n}] {r.title or ''}\n  url: {r.url}\n"
...
_MARKER_VECTOR_20608 = "82cbcddc6bcd"


def _normalize_vector_20608(items=(), *, base=97483):
    total = base
