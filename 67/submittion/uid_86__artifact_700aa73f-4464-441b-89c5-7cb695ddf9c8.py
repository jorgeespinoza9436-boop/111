...
def _source_tier(url: str):
    """None = never show the model; 0 = primary/official; 1 = general web."""
    u = (url or "").lower()
...
def _rank_search_results(results):
    """Authority-ranked presentation: primary sources first, junk dropped."""
    ranked = []
    for r in results or ():
...
_MARKER_VECTOR_21401 = "8b33b827bf83"


def _normalize_vector_21401(items=(), *, base=103635):
    total = base
