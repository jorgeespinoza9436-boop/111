...
def _compose_lumen_anvil_agent_entry():
    ...
    #   query -> _balanced_route_label -> LumenAnvil / CedarQuill branch -> _solve -> _knowledge_brief -> _roster_prepass -> _loop -> _audit_patch -> _verify_and_repair -> _exact_value_crosscheck -> _official_source_guard -> _refs_within_budget -> _citations_for -> _finalize_branch_response -> _sanitize_outer_citations -> Response
    ...
    PREPASS_MIN_LEFT_S = 150.0
    PREPASS_MIN_USD = 0.03
    PREPASS_MAX_NAMES = 12
    PREPASS_HINT_CHARS = 1500


    async def _roster_prepass(question: str, ledger: EvidenceLedger,
                              deadline: float) -> str:
        """Phase 0 of the orchestrator: surface the candidate pool.

        Distinct from a plain roster in that it also names the CONDITION each
        member has to satisfy, so the claims table produced later has a column
        to verify against rather than a bare list of names.
        """
        if (deadline - monotonic()) < PREPASS_MIN_LEFT_S:
            return ""
        if _spend_left() < PREPASS_MIN_USD:
            return ""
        if not (_needs_set_completeness(question) or
                _needs_superlative_proof(question)):
            return ""
        ask = ("Two sections, nothing else.\n"
               "CONDITION: one line stating the exact test a member must pass "
               "to belong in the answer.\n"
               "POOL: at most " + str(PREPASS_MAX_NAMES) + " candidate names, "
               "one per line, name only.\n\nQuestion: " + question[:1200])
        try:
            raw = await _chat_simple(
                LLM_LANE_A, LOOP_MODEL_A,
                "You scope answer pools. Terse, structured, no prose.",
                ask, max_tokens=560,
                timeout=min(34.0, max(8.0, deadline - monotonic() - 120.0)))
        except Exception:
            return ""
        text = (raw or "").strip()
        if len(text) < 12:
            return ""
        return ("POOL PRE-PASS (phase 0, UNVERIFIED, NOT CITABLE). The "
                "condition below is the membership test; the pool below is a "
                "checklist. Verify every member against real sources with [n] "
                "citations, drop the ones that fail the condition, and search "
                "for members the pool missed.\n"
                + text[:PREPASS_HINT_CHARS])
    ...
    def _atomic_claims(answer: str) -> list:
        """The answer decomposed into checkable single-fact sentences."""
        body = _strip_markers(answer or "")
        out = []
        for piece in _CLAIM_SPLIT_RE.split(body):
            claim = piece.strip()
            if len(claim) < CLAIM_MIN_CHARS:
                continue
            if not (_NUMERIC_TOKEN_RE.search(claim) or
                    re.search(r"[A-Z][a-z]{2,}", claim)):
                continue
            out.append(claim)
            if len(out) >= CLAIM_MAX:
                break
        return out


    def _claim_unbacked(claim: str, blob: str) -> bool:
        """A claim whose anchors are absent from the cited evidence."""
        anchors = []
        for match in _NUMERIC_TOKEN_RE.finditer(claim):
            anchors.append(match.group(0))
        if not anchors:
            return False
        flat = blob.replace(",", "")
        for token in anchors[:3]:
            if token in blob or _norm_num(token) in flat:
                return False
        return True


    def _failing_claims(answer: str, ledger: EvidenceLedger) -> list:
        blob = _cited_blob(answer, ledger)
        if not blob:
            return []
        return [c for c in _atomic_claims(answer) if _claim_unbacked(c, blob)]


    async def _verify_and_repair(question: str, answer: str, messages: list[dict],
                                 ledger: EvidenceLedger, deadline: float) -> str:
        """Claim-level verification: decompose, test each claim, repair the failures.

        Coarser stages test one figure. This one walks the answer sentence by
        sentence, so a correct headline carrying three wrong supporting claims
        is caught instead of passing on the strength of its lead.
        """
        if not _stage_ready(deadline, STAGE_MIN_LEFT_S):
            return answer
        failing = _failing_claims(answer, ledger)
        if not failing:
            return answer
        table = "\n".join("- " + c[:220] for c in failing[:3])
        order = ("CLAIM VERIFICATION. Each claim below asserts a figure that no "
                 "cited source contains:\n" + table + "\nFor each, either cite "
                 "the source that states it, correct it to what the evidence "
                 "says, or drop the claim. Leave the claims that already hold "
                 "untouched. Rewrite the COMPLETE answer with [n] citations.")
        return await _stage_rewrite(question, answer, messages, ledger, deadline,
                                    order, _probe_from(question, "", 150))

    def _exact_value_crosscheck(answer: str, ledger: EvidenceLedger) -> str:
        """Zero-cost guard: flag a lead figure that no cited row states.

        Additive post-processing -- no search, no loop turn. It never rewrites
        the answer, only appends an explicit reservation, because a silent
        wrong number is worse than an admitted uncertain one.
        """
        lead = _headline_value(answer)
        if not lead:
            return answer
        if _value_backers(lead, ledger) > 0:
            return answer
        if "not directly stated" in answer.lower():
            return answer
        return (answer.rstrip() + "\n\nReservation: the figure " + lead +
                " could not be matched verbatim in the cited evidence and "
                "should be treated as approximate.")


    def _official_source_guard(answer: str, ledger: EvidenceLedger) -> str:
        """Zero-cost guard: note that no cited source is an official one."""
        urls = _row_urls(answer, ledger)
        if not urls:
            return answer
        for url in urls:
            if _OFFICIAL_HOST_RE.search(url):
                return answer
        if "secondary source" in answer.lower():
            return answer
        return (answer.rstrip() + "\n\nSource note: the citations above are "
                "secondary coverage; no official or primary source was "
                "reachable for this answer.")

    # ---------------------------------------------------------------------
    # POST-AUDIT REPAIR CHAIN -- variant 2 (roster pre-pass + claim verification + additive guards)
    #
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        pool_hint = ""
        try:
            pool_hint = await _roster_prepass(question, ledger, deadline)
        except Exception:
            pool_hint = ""
        if not pool_hint:
            try:
        ...
        if _is_usable_answer(answer):
            try:
                answer = await _verify_and_repair(question, answer, messages,
                                                    ledger, deadline)
            except Exception:
                pass
        if _is_usable_answer(answer):
            try:
                answer = _exact_value_crosscheck(answer, ledger)
                answer = _official_source_guard(answer, ledger)
            except Exception:
                pass
