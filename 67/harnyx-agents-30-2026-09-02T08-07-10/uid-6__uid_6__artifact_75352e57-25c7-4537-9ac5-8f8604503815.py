...
def _compose_zephyr_marten_slot12_agent_entry():
...
_zephyr_marten_slot12_agent_query_entry = _compose_zephyr_marten_slot12_agent_entry()


def _compose_hazel_cairn_slot12_agent_entry():
...
_hazel_cairn_slot12_agent_query_entry = _compose_hazel_cairn_slot12_agent_entry()


def _compose_indigo_atlas_slot12_agent_entry():
...
_indigo_atlas_slot12_agent_query_entry = _compose_indigo_atlas_slot12_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "ZephyrMartenSlot12Agent"
    ...
    if (
        ...
        return "HazelCairnSlot12Agent"
    return "IndigoAtlasSlot12Agent"


class ZephyrMartenSlot12Agent:
    async def __call__(self, query: Query) -> Response:
        return await _zephyr_marten_slot12_agent_query_entry(query)


class HazelCairnSlot12Agent:
    async def __call__(self, query: Query) -> Response:
        return await _hazel_cairn_slot12_agent_query_entry(query)


class IndigoAtlasSlot12Agent:
    async def __call__(self, query: Query) -> Response:
        return await _indigo_atlas_slot12_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = ZephyrMartenSlot12Agent()
_ANALYTICAL_FIELD_AGENT = HazelCairnSlot12Agent()
_BROAD_FIELD_AGENT = IndigoAtlasSlot12Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "ZephyrMartenSlot12Agent",
    "HazelCairnSlot12Agent",
    "IndigoAtlasSlot12Agent",
)
...
async def _drv_base_query(query: Query) -> Response:
    ...
    if selected == "ZephyrMartenSlot12Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "HazelCairnSlot12Agent":
...
# batch_tag='drv008' salt='0485fd02176e'
