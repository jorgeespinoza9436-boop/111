...
def _compose_umber_atlas_slot08_agent_entry():
...
_umber_atlas_slot08_agent_query_entry = _compose_umber_atlas_slot08_agent_entry()


def _compose_juniper_survey_slot08_agent_entry():
...
_juniper_survey_slot08_agent_query_entry = _compose_juniper_survey_slot08_agent_entry()


def _compose_fjord_archive_slot08_agent_entry():
...
_fjord_archive_slot08_agent_query_entry = _compose_fjord_archive_slot08_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "UmberAtlasSlot08Agent"
    ...
    if (
        ...
        return "JuniperSurveySlot08Agent"
    return "FjordArchiveSlot08Agent"


class UmberAtlasSlot08Agent:
    async def __call__(self, query: Query) -> Response:
        return await _umber_atlas_slot08_agent_query_entry(query)


class JuniperSurveySlot08Agent:
    async def __call__(self, query: Query) -> Response:
        return await _juniper_survey_slot08_agent_query_entry(query)


class FjordArchiveSlot08Agent:
    async def __call__(self, query: Query) -> Response:
        return await _fjord_archive_slot08_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = UmberAtlasSlot08Agent()
_ANALYTICAL_FIELD_AGENT = JuniperSurveySlot08Agent()
_BROAD_FIELD_AGENT = FjordArchiveSlot08Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "UmberAtlasSlot08Agent",
    "JuniperSurveySlot08Agent",
    "FjordArchiveSlot08Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "UmberAtlasSlot08Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "JuniperSurveySlot08Agent":
