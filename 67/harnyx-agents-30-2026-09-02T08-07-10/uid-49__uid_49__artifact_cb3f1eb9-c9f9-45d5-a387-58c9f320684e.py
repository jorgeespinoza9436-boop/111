...
def _compose_meadow_lantern_agent_entry():
    ...
    """
        ...
        # reach `_so_skeleton` while a draft exists. Measured 2026-08-31 across three
        # batches: 39 runs shipped a skeleton payload (every string leaf `x`) and all
        # 39 scored 0.000 — a conforming placeholder is a certain zero, a literal the
        # draft printed is not.
        ...
        for attempt in range(STRUCTURED_ATTEMPTS):
            ...
            if remaining <= 4.0:
                break
            # A fixed per-call cap leaves the tail of the reserve unspent: two 22 s
            # attempts inside 55 s returned the budget and a placeholder together.
            # Each attempt takes the cap or the rest, whichever is smaller.
            timeout = min(STRUCTURED_CALL_TIMEOUT_SECONDS, remaining - 2.0)
            ...
            # The floor already occupies `best`, so "keep the candidate" is now a
            # choice rather than the only option: keep whichever the checker rejects
            # LESS. An LLM re-expression that reads the draft beats a regex floor
            # when it is close; a badly broken one does not.
        ...
        # B19's ordering preferred the skeleton over a value its own checker still
        # rejected. That trade is inverted here for the same reason the floor exists:
        # conformance the platform never reads is worth less than content the judge
        # does. Content first, shape second.
...
_SHAPE_ROUTER_SEED = "40f7cb4c75fe100a6639912e"
...
def _balanced_route_label(query: Query) -> str:
    ...
    order = ("CedarLatticeAgent", "AmberLatticeAgent", "MeadowLanternAgent")
    if shape == 3:
        return order[bucket]
