...
_yrxmjgsbvv = {'started': None, 'text': None}
_uzijjknhrh = 24000
_kcmbykecxe = 290.0
_jkrpkrxilg = 250.0

def _uhrebkngpf() -> float:
    started = _yrxmjgsbvv['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _jeevutbbau() -> float:
    return _kcmbykecxe - _uhrebkngpf()
_ssiplrdlhx = _hsapi.llm_chat
_rjrgciiuny = _hsapi.search_web
_jttktjbkrn = _hsapi.fetch_page
_amgnfovqgq = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _dnasvngthv(*args, **kwargs):
    if _uhrebkngpf() >= _jkrpkrxilg:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _amgnfovqgq})
            kwargs['messages'] = steered
    _result = await _ssiplrdlhx(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _uvsiggrzqg(_result)
    return _result

async def _jdmgwmyaub(*args, **kwargs):
    if _uhrebkngpf() >= _jkrpkrxilg:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _rjrgciiuny(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _sjsvrqpszp(*args, **kwargs):
    if _uhrebkngpf() >= _jkrpkrxilg:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _jttktjbkrn(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _dnasvngthv
_hsapi.search_web = _jdmgwmyaub
_hsapi.fetch_page = _sjsvrqpszp
_usklqmomdd = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_ukadniwcth = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_hjpfgestxy = 900
_muohkywowu = 2

def _igvdepuatq(query: Query) -> int:
...
def _guahhfagsq(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _cyslclytjp(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _igvdepuatq(query)
    if fields >= 3:
        return 2
    if _guahhfagsq(lowered, _usklqmomdd):
        return 1
    if fields <= _muohkywowu and len(text) <= _hjpfgestxy:
        return 0
    if _guahhfagsq(lowered, _ukadniwcth):
        return 0
    return 1

def _uvsiggrzqg(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _yrxmjgsbvv['text'] = text.strip()[:_uzijjknhrh]
    except Exception:
        pass

def _ytastmnwkw(text: str):
...
def _ddjjtpljys(query: Query) -> Response:
    text = _yrxmjgsbvv['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_uzijjknhrh]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _ytastmnwkw(text)
...
def _vnvxthtxiv():
    ...
    """
        ...
        return response
    return query
_tgskncbpgr = _cclgeuyplr()
_gzkdregrmz = _ifzmtzsqih()
_hwnarfvdkj = _ohqchoqrei()
_allnzrxxvs = _rkpwywpdxm()
_jxkyjpsqek = _vnvxthtxiv()
_kcmbykecxe = 290.0
_tstxewzsdf = 250.0
_yxshupshfy = 90.0

async def _ebxvtqniwp(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _kcmbykecxe - (time.monotonic() - started)
        if first:
            budget = _tstxewzsdf if _tstxewzsdf < remaining else remaining
            first = False
        else:
            if remaining < _yxshupshfy:
                break
        ...
        try:
            ...
            last_exc = exc
    return _ddjjtpljys(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _yrxmjgsbvv['started'] = time.monotonic()
    try:
        index = _cyslclytjp(query)
        if index == 0:
            agents = (_tgskncbpgr, _gzkdregrmz, _hwnarfvdkj, _allnzrxxvs, _jxkyjpsqek)
        elif index == 1:
            agents = (_gzkdregrmz, _hwnarfvdkj, _allnzrxxvs, _jxkyjpsqek, _tgskncbpgr)
        elif index == 2:
            agents = (_hwnarfvdkj, _allnzrxxvs, _jxkyjpsqek, _tgskncbpgr, _gzkdregrmz)
        elif index == 3:
            agents = (_allnzrxxvs, _jxkyjpsqek, _tgskncbpgr, _gzkdregrmz, _hwnarfvdkj)
        elif index == 4:
            agents = (_jxkyjpsqek, _tgskncbpgr, _gzkdregrmz, _hwnarfvdkj, _allnzrxxvs)
        else:
            agents = (_tgskncbpgr, _gzkdregrmz, _hwnarfvdkj, _allnzrxxvs, _jxkyjpsqek)
        return await _ebxvtqniwp(query, agents)
    except Exception:
        return _ddjjtpljys(query)
_BUILDY_TAG_m28 = "sn45-c932707635fe"
