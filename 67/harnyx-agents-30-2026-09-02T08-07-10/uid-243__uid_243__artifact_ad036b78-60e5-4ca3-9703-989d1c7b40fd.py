...
def _compose_vesper_atlas_slot15_agent_entry():
...
_vesper_atlas_slot15_agent_query_entry = _compose_vesper_atlas_slot15_agent_entry()


def _compose_ember_beacon_slot15_agent_entry():
...
_ember_beacon_slot15_agent_query_entry = _compose_ember_beacon_slot15_agent_entry()


def _compose_xenon_atlas_slot15_agent_entry():
    ...
    VERSION = "c3-417"
    _GX_ACTIVE = ('cite', 'super', 'window')

    # --- drv wrap: claim-conflict ledger (start) ---
    # batch_tag='drv000' salt='2bd094a65c0f'
...
_xenon_atlas_slot15_agent_query_entry = _compose_xenon_atlas_slot15_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "VesperAtlasSlot15Agent"
    ...
    if (
        ...
        return "EmberBeaconSlot15Agent"
    return "XenonAtlasSlot15Agent"


class VesperAtlasSlot15Agent:
    async def __call__(self, query: Query) -> Response:
        return await _vesper_atlas_slot15_agent_query_entry(query)


class EmberBeaconSlot15Agent:
    async def __call__(self, query: Query) -> Response:
        return await _ember_beacon_slot15_agent_query_entry(query)


class XenonAtlasSlot15Agent:
    async def __call__(self, query: Query) -> Response:
        return await _xenon_atlas_slot15_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = VesperAtlasSlot15Agent()
_ANALYTICAL_FIELD_AGENT = EmberBeaconSlot15Agent()
_BROAD_FIELD_AGENT = XenonAtlasSlot15Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "VesperAtlasSlot15Agent",
    "EmberBeaconSlot15Agent",
    "XenonAtlasSlot15Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "VesperAtlasSlot15Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "EmberBeaconSlot15Agent":
