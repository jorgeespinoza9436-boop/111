...
def _compose_lumen_anvil_agent_entry():
    ...
    #   query -> _balanced_route_label -> LumenAnvil / CedarQuill branch -> _solve -> _knowledge_brief -> _build_roster -> _loop -> _audit_patch -> _refs_within_budget -> _dedupe_citations(_citations_for) -> _finalize_branch_response -> _sanitize_outer_citations -> Response
    ...
    DEDUPE_MAX_PER_HOST = 3


    def _dedupe_host(url: str) -> str:
    ...
    def _dedupe_citations(refs: list) -> list:
        """Collapse repeat citations of one URL and cap any single host.

        The judge reads a citation array, not a row list. Five slices of one
        page crowd out the independent sources that make an answer look
        corroborated, so keep the first few per host and drop the rest.
        """
        if not refs:
            return refs
        seen_url = set()
        ...
        for ref in refs:
            ...
            if key in seen_url:
                continue
            host = _dedupe_host(url)
            used = per_host.get(host, 0)
            if used >= DEDUPE_MAX_PER_HOST:
                continue
    ...
    # ---------------------------------------------------------------------
    # POST-AUDIT REPAIR CHAIN -- variant 89 (roster pre-pass + citation dedupe/backfill)
    #
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            ...
            pass
        try:
            citations = _dedupe_citations(_citations_for(answer, ledger))
        except Exception:
            citations = []
            _W2_CITE_POS.clear()
