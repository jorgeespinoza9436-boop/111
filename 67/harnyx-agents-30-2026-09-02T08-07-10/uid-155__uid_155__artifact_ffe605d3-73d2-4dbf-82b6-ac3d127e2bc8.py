...
def _compose_yarrow_wren_slot11_agent_entry():
...
_yarrow_wren_slot11_agent_query_entry = _compose_yarrow_wren_slot11_agent_entry()


def _compose_garnet_talon_slot11_agent_entry():
...
_garnet_talon_slot11_agent_query_entry = _compose_garnet_talon_slot11_agent_entry()


def _compose_ivory_lantern_slot11_agent_entry():
...
_ivory_lantern_slot11_agent_query_entry = _compose_ivory_lantern_slot11_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "YarrowWrenSlot11Agent"
    ...
    if (
        ...
        return "GarnetTalonSlot11Agent"
    return "IvoryLanternSlot11Agent"


class YarrowWrenSlot11Agent:
    async def __call__(self, query: Query) -> Response:
        return await _yarrow_wren_slot11_agent_query_entry(query)


class GarnetTalonSlot11Agent:
    async def __call__(self, query: Query) -> Response:
        return await _garnet_talon_slot11_agent_query_entry(query)


class IvoryLanternSlot11Agent:
    async def __call__(self, query: Query) -> Response:
        return await _ivory_lantern_slot11_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = YarrowWrenSlot11Agent()
_ANALYTICAL_FIELD_AGENT = GarnetTalonSlot11Agent()
_BROAD_FIELD_AGENT = IvoryLanternSlot11Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "YarrowWrenSlot11Agent",
    "GarnetTalonSlot11Agent",
    "IvoryLanternSlot11Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "YarrowWrenSlot11Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "GarnetTalonSlot11Agent":
