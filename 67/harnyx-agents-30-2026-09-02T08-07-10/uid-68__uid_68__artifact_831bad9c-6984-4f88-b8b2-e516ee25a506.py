...
def _compose_lumen_cairn_slot05_agent_entry():
    ...
    # One entry per REGION a claim can be read out of, not per source. A single
    # entry holding every range a page was read from stands behind every claim that
    # named the page, and the reader has to work out which part -- which the judge
    # checks by name ("the provided citation text for [[1]] does not contain that
    # information"). Duplicate fetches of one page still collapse, because the same
    # page read twice yields the same coordinates.
    CITATION_MAX_ENTRIES = 8
    CITATION_REGION_MIN_HITS = 2
    ...
    async def _run_fetch_page(url: str, index: _ResultIndex, terms: list[str],
        ...
        # for what is left -- which is the wrong way round for the only regions on
        # the page that were actually checked.
        try:
    ...
    _PASSAGE_TOKEN_RE = re.compile(r"[a-z][a-z']{4,}|\d[\d/.,]*")


    def _passage_signature(note: str, start: int, end: int) -> tuple[frozenset, int]:
        """A passage's identity as content, tolerant of how it was rendered."""
        body = " ".join(note[start:end].split()).casefold()
        return frozenset(_PASSAGE_TOKEN_RE.findall(body)), len(body)


    def _same_passage(a: tuple[frozenset, int], b: tuple[frozenset, int]) -> bool:
        'Whether two regions are the same passage reached twice.\n\n    Exact text cannot decide it: the same report served from two media paths\n    arrives through two renderings whose text differs slightly throughout -- one\n    passage measured 2,076 chars in one and 1,983 in the other -- so an equality\n    test folds nothing and the reader is handed the passage twice. Comparing the\n    distinctive words is stable across renderings, and the length guard stops a\n    section folding into the chapter that contains it.\n    '
        ta, la = a
        tb, lb = b
        if not ta or not tb:
            return False
        if min(la, lb) * 5 < max(la, lb) * 4:
            return False
        shared = sum(1 for token in ta if token in tb)
        return shared * 10 >= max(len(ta), len(tb)) * 8


    def _region_claim_hits(note: str, start: int, end: int, claims: list[str]) -> int:
        'How much of a claim is literally readable inside one region.\n\n    Scored per claim and maximised, not pooled: a region that carries all of one\n    claim is support, while a region carrying one stray word from each of six\n    claims is not, and pooling cannot tell those apart.\n    '
        window = note[start:end].lower()
        best = 0
        for claim in claims:
            tokens = _anchor_tokens(claim)
            if not tokens:
                continue
            best = max(best, sum(1 for t in tokens if t in window))
        return best


    def _citations_from_inline_markers(
        answer_text: str, index: _ResultIndex
    ) -> tuple[tuple[CitationRef, ...], dict[int, int], dict[int, list[tuple[int, str, int, int]]]]:
        'Build the citation array and the number -> array-position map.\n\n    One entry per REGION rather than per source: the array a reader is handed\n    should let a claim be checked against the passage it came from, not against\n    every passage the page was read from. Repeated fetches of one page fold\n    together (same coordinates, same entry), so the padding that per-source\n    folding existed to prevent does not come back.\n    '
        ...
        regions: dict[tuple, dict[str, object]] = {}
        region_order: list[tuple] = []
        signatures: list[tuple[tuple, tuple[frozenset, int]]] = []
        hits_for: dict[int, dict[tuple, int]] = {}
        ...
        for n in ordered:
            ...
            if src_len <= 0:
                continue
            note = meta["note"]
            claims = claims_by_number.get(n, [])
            spans = [(s, e) for s, e in index.spans(n) if e > s]
            if not spans:
                start, end = _anchored_slice_bounds(note, claims, slice_window)
                if end > start:
                    spans = [(start, end)]
            spans = [(max(0, s), min(src_len, e)) for s, e in spans]
            spans = _merge_spans([(s, e) for s, e in spans if e - s >= 100 or (s == 0 and e == src_len)])
            base = _normalized_url(meta.get("url") or "") or f"{meta['receipt_id']}/{meta['result_id']}"
            for s_, e_ in spans:
                # Keyed by the passage's own content, not by url+offset, so the
                # same report served from two media paths folds into one entry.
                sig = _passage_signature(note, s_, e_)
                key = None
                for known, known_sig in signatures:
                    if _same_passage(sig, known_sig):
                        key = known
                        break
                if key is None:
                    key = (base, s_, e_)
                    signatures.append((key, sig))
                    regions[key] = {"meta": meta, "span": (s_, e_), "hits": 0}
                    region_order.append(key)
                hits = _region_claim_hits(note, s_, e_, claims)
                regions[key]["hits"] = max(int(regions[key]["hits"]), hits)
                hits_for.setdefault(n, {})[key] = max(hits_for.setdefault(n, {}).get(key, 0), hits)

        if not region_order:
            return tuple(), {}, {}

        # Rank by how much of a claim each region actually carries; keep the ones a
        # reader could check, then make sure every cited number still has one.
        def _rank(key: tuple) -> tuple:
            span = regions[key]["span"]
            return (-int(regions[key]["hits"]), -(span[1] - span[0]), region_order.index(key))

        ranked = sorted(region_order, key=_rank)
        kept: list[tuple] = []
        budget = CITATION_BUDGET_CHARS
        for key in ranked:
            if len(kept) >= CITATION_MAX_ENTRIES:
                break
            if int(regions[key]["hits"]) < CITATION_REGION_MIN_HITS:
                continue
            s_, e_ = regions[key]["span"]
            if e_ - s_ > budget:
                continue
            kept.append(key)
            budget -= e_ - s_
        for n in ordered:
            if any(key in hits_for.get(n, {}) for key in kept):
                continue
            for key in sorted(hits_for.get(n, {}), key=_rank):
                s_, e_ = regions[key]["span"]
                if len(kept) < CITATION_MAX_ENTRIES and e_ - s_ <= budget:
                    kept.append(key)
                    budget -= e_ - s_
                    break

        kept.sort(key=lambda key: region_order.index(key))
        ...
        for key in kept:
            ...
            citations.append(CitationRef(
                ...
                slices=[CitationSlice(start=s_, end=e_)],
            ))
            position_of_key[key] = len(citations)

        # A marker points at the region that carries the most of its own claim.
        position_of: dict[int, int] = {}
        regions_by_number: dict[int, list[tuple[int, str, int, int]]] = {}
        for n in ordered:
            best_key = None
            best = (-1, 0)
            for key in hits_for.get(n, {}):
                if key not in position_of_key:
                    continue
                entry = regions[key]
                span = entry["span"]
                regions_by_number.setdefault(n, []).append(
                    (position_of_key[key], entry["meta"]["note"], span[0], span[1])
                )
                score = (hits_for[n][key], span[1] - span[0])
                if score > best:
                    best, best_key = score, key
            if best_key is not None:
                position_of[n] = position_of_key[best_key]
        return tuple(citations), position_of, regions_by_number


    def _repoint_markers(
        text: str, position_of: dict[int, int], *, max_number: int,
        regions_by_number: dict[int, list[tuple[int, str, int, int]]] | None = None,
    ) -> str:
        ...
        def _replace(match: "re.Match[str]") -> str:
            claim = text[max(0, match.start() - CITATION_ANCHOR_CONTEXT_CHARS):match.start()]
            positions: list[int] = []
            for n in _numbers_from_bracket(match.group(1), max_number=max_number):
                position = position_of.get(n)
                # Resolve against THIS claim where the regions are known: one number
                # can back two claims that sit in different parts of the same page,
                # and a fixed number -> position map has to pick one of them.
                # A pointer only moves to a region that carries enough of THIS
                # claim to be checked against it. Below that bar the region is
                # generic -- it shares a few common words with every claim on the
                # page -- and pointing at it is the thing the judge faults.
                best = (CITATION_REGION_MIN_HITS - 1, 0)
                for pos, note, start, end in (regions_by_number or {}).get(n, ()):
                    score = (_region_claim_hits(note, start, end, [claim]), end - start)
                    if score > best:
    ...
    def _deliverable(text: str | None, index: _ResultIndex, *, cite_text: str | None = None) -> Response:
        ...
        citations, position_of, regions_by_number = _citations_from_inline_markers(
            cite_text or answer, index)
        answer = _repoint_markers(answer, position_of, max_number=index.max_number(),
                                  regions_by_number=regions_by_number)
...
_lumen_cairn_slot05_agent_query_entry = _compose_lumen_cairn_slot05_agent_entry()


def _compose_lumen_cairn_b_slot05_agent_entry():
...
_lumen_cairn_b_slot05_agent_query_entry = _compose_lumen_cairn_b_slot05_agent_entry()


def _compose_aster_notary_slot05_agent_entry():
...
_aster_notary_slot05_agent_query_entry = _compose_aster_notary_slot05_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "LumenCairnSlot05Agent"
    ...
    if (
        ...
        return "LumenCairnBSlot05Agent"
    return "AsterNotarySlot05Agent"


class LumenCairnSlot05Agent:
    async def __call__(self, query: Query) -> Response:
        return await _lumen_cairn_slot05_agent_query_entry(query)


class LumenCairnBSlot05Agent:
    async def __call__(self, query: Query) -> Response:
        return await _lumen_cairn_b_slot05_agent_query_entry(query)


class AsterNotarySlot05Agent:
    async def __call__(self, query: Query) -> Response:
        return await _aster_notary_slot05_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = LumenCairnSlot05Agent()
_ANALYTICAL_FIELD_AGENT = LumenCairnBSlot05Agent()
_BROAD_FIELD_AGENT = AsterNotarySlot05Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "LumenCairnSlot05Agent",
    "LumenCairnBSlot05Agent",
    "AsterNotarySlot05Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "LumenCairnSlot05Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "LumenCairnBSlot05Agent":
