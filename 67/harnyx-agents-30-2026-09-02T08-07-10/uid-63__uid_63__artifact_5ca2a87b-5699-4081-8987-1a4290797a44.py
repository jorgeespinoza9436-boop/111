...
def _compose_hazel_quill_slot04_agent_entry():
...
_hazel_quill_slot04_agent_query_entry = _compose_hazel_quill_slot04_agent_entry()


def _compose_garnet_mosaic_slot04_agent_entry():
...
_garnet_mosaic_slot04_agent_query_entry = _compose_garnet_mosaic_slot04_agent_entry()


def _compose_talus_compass_slot04_agent_entry():
...
_talus_compass_slot04_agent_query_entry = _compose_talus_compass_slot04_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "HazelQuillSlot04Agent"
    ...
    if (
        ...
        return "GarnetMosaicSlot04Agent"
    return "TalusCompassSlot04Agent"


class HazelQuillSlot04Agent:
    async def __call__(self, query: Query) -> Response:
        return await _hazel_quill_slot04_agent_query_entry(query)


class GarnetMosaicSlot04Agent:
    async def __call__(self, query: Query) -> Response:
        return await _garnet_mosaic_slot04_agent_query_entry(query)


class TalusCompassSlot04Agent:
    async def __call__(self, query: Query) -> Response:
        return await _talus_compass_slot04_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = HazelQuillSlot04Agent()
_ANALYTICAL_FIELD_AGENT = GarnetMosaicSlot04Agent()
_BROAD_FIELD_AGENT = TalusCompassSlot04Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "HazelQuillSlot04Agent",
    "GarnetMosaicSlot04Agent",
    "TalusCompassSlot04Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "HazelQuillSlot04Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "GarnetMosaicSlot04Agent":
