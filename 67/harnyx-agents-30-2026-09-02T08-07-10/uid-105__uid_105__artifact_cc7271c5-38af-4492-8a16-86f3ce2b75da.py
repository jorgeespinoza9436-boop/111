...
def _compose_talus_wren_slot07_agent_entry():
...
_talus_wren_slot07_agent_query_entry = _compose_talus_wren_slot07_agent_entry()


def _compose_opal_wren_slot07_agent_entry():
...
_opal_wren_slot07_agent_query_entry = _compose_opal_wren_slot07_agent_entry()


def _compose_amber_survey_slot07_agent_entry():
...
_amber_survey_slot07_agent_query_entry = _compose_amber_survey_slot07_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "TalusWrenSlot07Agent"
    ...
    if (
        ...
        return "OpalWrenSlot07Agent"
    return "AmberSurveySlot07Agent"


class TalusWrenSlot07Agent:
    async def __call__(self, query: Query) -> Response:
        return await _talus_wren_slot07_agent_query_entry(query)


class OpalWrenSlot07Agent:
    async def __call__(self, query: Query) -> Response:
        return await _opal_wren_slot07_agent_query_entry(query)


class AmberSurveySlot07Agent:
    async def __call__(self, query: Query) -> Response:
        return await _amber_survey_slot07_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = TalusWrenSlot07Agent()
_ANALYTICAL_FIELD_AGENT = OpalWrenSlot07Agent()
_BROAD_FIELD_AGENT = AmberSurveySlot07Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "TalusWrenSlot07Agent",
    "OpalWrenSlot07Agent",
    "AmberSurveySlot07Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "TalusWrenSlot07Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "OpalWrenSlot07Agent":
