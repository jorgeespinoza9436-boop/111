...
def _compose_lumen_anvil_agent_entry():
    ...
    #   query -> _balanced_route_label -> LumenAnvil / CedarQuill branch -> _solve -> _knowledge_brief -> _build_roster -> _loop -> _audit_patch -> _unit_repair -> _refs_within_budget -> _citations_for -> _finalize_branch_response -> _sanitize_outer_citations -> Response
    ...
    ROSTER_MIN_USD = 0.03
    ROSTER_MAX_NAMES = 14
    ROSTER_HINT_CHARS = 1400


    async def _build_roster(question: str, ledger: EvidenceLedger,
                            deadline: float) -> str:
        """Candidate-roster pre-pass for set and superlative questions.

        Enumerates a candidate pool with one cheap model call BEFORE the
        research loop opens, so the loop starts with something to check
        rather than something to discover. The roster is never citable on
        its own -- it is a checklist, and the loop must ground each member.
        """
        if (deadline - monotonic()) < ROSTER_MIN_LEFT_S:
            return ""
        if _spend_left() < ROSTER_MIN_USD:
            return ""
        if not (_needs_set_completeness(question) or
                _needs_superlative_proof(question)):
            return ""
        ask = ("List the candidate members that a complete, correct answer to "
               "this question would have to consider. One per line, name only, "
               "no numbering, no commentary. At most "
               + str(ROSTER_MAX_NAMES) + " lines. If you are unsure of a "
               "member, still list it -- this is a checklist to verify, not "
               "an answer.\n\nQuestion: " + question[:1200])
        try:
            raw = await _chat_simple(
                LLM_LANE_A, LOOP_MODEL_A,
                "You enumerate candidate pools. Names only.",
                ask, max_tokens=520,
        ...
        for line in (raw or "").splitlines():
            ...
            if name.lower() in [n.lower() for n in names]:
                continue
            names.append(name)
            if len(names) >= ROSTER_MAX_NAMES:
                break
        if len(names) < 2:
            return ""
        block = "\n".join("- " + n for n in names)
        return ("CANDIDATE ROSTER (pre-pass, UNVERIFIED, NOT CITABLE). A model "
                "enumerated these before any research ran. Treat each as a "
                "candidate to CHECK against real sources. Confirm or drop each "
                "one with a [n] citation, and search for members the roster "
                "missed. Never cite this block.\n"
                + block[:ROSTER_HINT_CHARS])

    UNIT_MAX_TARGETS = 2


    def _requested_units(question: str) -> list:
        out = []
        for match in _UNIT_WORD_RE.finditer(question or ""):
            token = match.group(0).lower()
            if token not in out:
                out.append(token)
            if len(out) >= UNIT_MAX_TARGETS:
                break
        return out


    def _unit_gap(question: str, answer: str) -> str:
        """A unit or currency the question demands and the answer never states."""
        body = _strip_markers(answer or "")
        low = body.lower()
        if _CURRENCY_SYMBOL_RE.search(question or "") and \
                not _CURRENCY_SYMBOL_RE.search(body):
            currency = _CURRENCY_SYMBOL_RE.search(question or "")
            return currency.group(0) if currency is not None else ""
        for unit in _requested_units(question):
            if unit in low:
                continue
            return unit
        return ""
    ...
    async def _unit_repair(question: str, answer: str, messages: list[dict],
                           ledger: EvidenceLedger, deadline: float) -> str:
        """Unit consistency: the answer omits the measure the question asked for.

        A bare number answering a question that named a currency or a scale is
        not an answer to that question. This stage is text-local -- it does not
        search unless the rewrite needs to.
        """
        if not _stage_ready(deadline, STAGE_TIGHT_LEFT_S):
            return answer
        unit = _unit_gap(question, answer)
        if not unit:
            return answer
        order = ("UNIT CONSISTENCY. The question asks in \"" + unit + "\" but "
                 "the answer never states that unit or scale. Give every figure "
                 "in the unit and scale the question asked for, spelled out "
                 "next to the number. If a source reports a different unit, "
                 "show the source figure with its unit AND the converted value, "
                 "both carrying their [n]. Rewrite the COMPLETE answer with [n] "
                 "citations.")
        return await _stage_rewrite(question, answer, messages, ledger, deadline,
                                    order, "")

    # ---------------------------------------------------------------------
    # POST-AUDIT REPAIR CHAIN -- variant 236 (roster pre-pass + unit repair)
    #
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        pool_hint = ""
        try:
            pool_hint = await _build_roster(question, ledger, deadline)
        except Exception:
            pool_hint = ""
        if not pool_hint:
            try:
        ...
        if _is_usable_answer(answer):
            try:
                answer = await _unit_repair(question, answer, messages,
                                              ledger, deadline)
            except Exception:
                pass
