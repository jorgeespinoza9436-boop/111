...
_ovsixfskht = {'started': None, 'text': None}
_tudmzbfwrp = 24000
_kbybhsczlw = 290.0
_wehspyybyp = 250.0

def _wnceyqbrtd() -> float:
    started = _ovsixfskht['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _luhbninffs() -> float:
    return _kbybhsczlw - _wnceyqbrtd()
_ferphtiwvd = _hsapi.llm_chat
_etoqdgwjyt = _hsapi.search_web
_wzhbdlrfgo = _hsapi.fetch_page
_npcwbilaex = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _zoqdeczwln(*args, **kwargs):
    if _wnceyqbrtd() >= _wehspyybyp:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _npcwbilaex})
            kwargs['messages'] = steered
    _result = await _ferphtiwvd(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _yoqkyvxwhd(_result)
    return _result

async def _qylusawkco(*args, **kwargs):
    if _wnceyqbrtd() >= _wehspyybyp:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _etoqdgwjyt(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _zayzdxizwv(*args, **kwargs):
    if _wnceyqbrtd() >= _wehspyybyp:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _wzhbdlrfgo(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _zoqdeczwln
_hsapi.search_web = _qylusawkco
_hsapi.fetch_page = _zayzdxizwv
_hhdhfediar = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_urstdehvap = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_timxlqerim = 900
_fatfadojum = 2

def _swkygdxoyt(query: Query) -> int:
...
def _ezadjdnzhc(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _aefkcyezgf(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _swkygdxoyt(query)
    if fields >= 3:
        return 2
    if _ezadjdnzhc(lowered, _hhdhfediar):
        return 1
    if fields <= _fatfadojum and len(text) <= _timxlqerim:
        return 0
    if _ezadjdnzhc(lowered, _urstdehvap):
        return 0
    return 1

def _yoqkyvxwhd(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _ovsixfskht['text'] = text.strip()[:_tudmzbfwrp]
    except Exception:
        pass

def _ymauyqkgmh(text: str):
...
def _phsbxaroph(query: Query) -> Response:
    text = _ovsixfskht['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_tudmzbfwrp]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _ymauyqkgmh(text)
...
def _bkbxninmxy():
    ...
    _CX_SUPER_RE = re.compile('\\b(?:most|least|highest|lowest|largest|smallest|greatest|fewest|longest|shortest|best|worst|top|maximum|minimum)\\b|\\b[a-z]{3,}est\\b', re.IGNORECASE)
    ...
    def _cx_sentences(text: str) -> list:
        return [s.strip() for s in _CX_SENT_RE.findall(text or '') if s.strip()]
    ...
    def _cx_decisive_claims(answer: str) -> list[dict]:
        ...
        for sent in _cx_sentences(answer):
            if not _CX_CITE_RE.search(sent):
                continue
...
def _uxopurcysj():
    """uid_195 research agent - variant 23.
...
def _sfzyizzyge():
    ...
    async def query(query: Query) -> Response:
        try:
            ...
            return _cx_response(None, None, [])
    return query
_ihxfthhbxc = _xhncndstgq()
_jfkinaczok = _owcnwgptbc()
_lonaebjogq = _bkbxninmxy()
_ovjnxznypj = _uxopurcysj()
_qbhqidwekh = _sfzyizzyge()
_kbybhsczlw = 290.0
_nkyuyysipz = 250.0
_czsjubuekp = 90.0

async def _vfwuhkyiwj(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _kbybhsczlw - (time.monotonic() - started)
        if first:
            budget = _nkyuyysipz if _nkyuyysipz < remaining else remaining
            first = False
        else:
            if remaining < _czsjubuekp:
                break
        ...
        try:
            ...
            last_exc = exc
    return _phsbxaroph(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _ovsixfskht['started'] = time.monotonic()
    try:
        index = _aefkcyezgf(query)
        if index == 0:
            agents = (_ihxfthhbxc, _jfkinaczok, _lonaebjogq, _ovjnxznypj, _qbhqidwekh)
        elif index == 1:
            agents = (_jfkinaczok, _lonaebjogq, _ovjnxznypj, _qbhqidwekh, _ihxfthhbxc)
        elif index == 2:
            agents = (_lonaebjogq, _ovjnxznypj, _qbhqidwekh, _ihxfthhbxc, _jfkinaczok)
        elif index == 3:
            agents = (_ovjnxznypj, _qbhqidwekh, _ihxfthhbxc, _jfkinaczok, _lonaebjogq)
        elif index == 4:
            agents = (_qbhqidwekh, _ihxfthhbxc, _jfkinaczok, _lonaebjogq, _ovjnxznypj)
        else:
            agents = (_ihxfthhbxc, _jfkinaczok, _lonaebjogq, _ovjnxznypj, _qbhqidwekh)
        return await _vfwuhkyiwj(query, agents)
    except Exception:
        return _phsbxaroph(query)
_BUILDY_TAG_m23 = "sn45-2bdb0b7f92ce"
