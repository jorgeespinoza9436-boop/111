...
class Nimbus1187e5:
    ...
    def _ingot_7f350f(self):
        _S32_QUERY_TAG = 's32-hk6731'
...
def _harbor_8679d3(factory):
    """Build the reserve closure; a source that dies on import must not kill the agent."""
    try:
        return factory()._ingot_7f350f()
    except Exception:
        return None

def _vellum_0e83a3(response):
    if response is None:
        return ''
    return (getattr(response, 'text', None) or '').strip()

def _umber_8ba590(response):
    if response is None:
        return 0
    return len(getattr(response, 'citations', None) or ())

def _lantern_be72d8(response):
    return response is not None and getattr(response, 'output', None) is not None

def _harbor_445d21(query, response):
    """Deterministic answer quality. No model call, so auditing is free."""
    if response is None:
        return 0.0
    if query.output_schema is not None and (not _lantern_be72d8(response)):
        return 0.0
    text = _vellum_0e83a3(response)
    if not _lantern_be72d8(response) and len(text) < 40:
        return 0.0
    score = 1.0
    if _lantern_be72d8(response):
        score += 1.0
    score += min(_umber_8ba590(response), 12) * 0.05
    score += min(len(text), 4000) / 4000.0
    return score

class Trellis804fc3:
    """Answer with the primary; consult the reserve when no evidence was cited."""
    _BASALT_FD00DD = 290.0
    _GIRDER_E81909 = 270.0
    _INGOT_D58A36 = 45.0
    _RIVET_707E8F = 1
    ...
    def _sable_7ad357(self, query, response):
        if response is None:
            return True
        cited = _umber_8ba590(response)
        self._alder_bdf234.append(cited)
        if cited >= self._RIVET_707E8F:
            return False
        return _harbor_445d21(query, response) <= 0.0 or not _lantern_be72d8(response)

    async def _dovetail_28e70b(self, run, request, budget):
    ...
    async def ember_2df37f(self, query: Query) -> Response:
        started = monotonic()
        first = await self._dovetail_28e70b(self._primary, query, self._GIRDER_E81909)
        if not self._sable_7ad357(query, first):
            return first if first is not None else Response(text='No answer produced.')
        remaining = self._BASALT_FD00DD - (monotonic() - started)
        if remaining <= self._INGOT_D58A36:
            return first if first is not None else Response(text='No answer produced.')
        second = await self._dovetail_28e70b(self._reserve, query, remaining)
        candidates = [r for r in (first, second) if r is not None]
        if not candidates:
            return Response(text='No answer produced.')
        return max(candidates, key=lambda r: _harbor_445d21(query, r))
_FATHOM_CBCC01 = query
_MARLIN_728108 = _harbor_8679d3(Nimbus1187e5)
_ZEPHYR_C025AC = Trellis804fc3(_FATHOM_CBCC01, _MARLIN_728108)

async def _g67_base_query(query: Query) -> Response:
    return await _ZEPHYR_C025AC.ember_2df37f(query)
