    ...
    },
{
        ...
        "function": {
            ...
            "description": ("Keep the exact source text that proves a claim you are "
                            ...
                            "retained quote confirming it. For any COUNT or TOTAL you "
                            "derive from a table, retain the table rows that prove it "
                            "(all contributing rows, not just one). For every item in "
                            "a list answer, retain a quote naming that item."),
            "parameters": {
                "type": "object",
                "properties": {
                    "source": {"type": "string",
...
_HXW_AGG_TOKENS = frozenset(("count", "counts", "months", "weeks", "days", "number",
                             "total", "totals", "qualifying", "share", "tied", "rows"))


_HXW_NOTE_TOKEN_RE = re.compile(r"[a-z0-9]+")
...
_HXW_PREMISE_RE = re.compile(
    r"\b(?:a |the )?(?:commentator|colleague|reader|reviewer|correspondent|blogger|analyst|critic)\b"
    r"[^.]{0,80}\b(?:asserts?|writes?|claims?|states?|says|argues?)\b", re.IGNORECASE)
_HXW_VERDICT_TOKENS = frozenset(("premise", "accurate", "accuracy", "verdict", "assertion", "holds"))
_HXW_NOTE_QUOTE_RE = re.compile(r'[\u201c"]([^"\u201c\u201d]{20,500})[\u201d"]')
...
def _hxw_verify_note(note, structured, shown):
    ...
    for num in re.findall(r"\d{2,}", note.replace(",", "")):
...
async def _hxw_premise_note(question, schema, structured, ledger, refs):
    ...
    prompt = (
        "A question quotes this claim:\n" + claim + "\n\n"
        + "The verified final answer is:\n" + json.dumps(structured) + "\n\n"
        + "Cited source excerpts:\n" + listing + "\n\n"
        + "If the quoted claim conflicts with the verified answer, write ONE or TWO plain sentences "
        + "correcting the false premise, citing the excerpt that proves it as [[n]]. "
        + "Use only values from the answer or excerpts. "
        + "If the claim is consistent with the answer, output exactly NONE."
    )
    raw = ""
    for model, budget in (("z-ai/glm-5.2", 25.0), ("openai/gpt-oss-120b", 20.0)):
        try:
            result = await llm_chat(
                ...
                messages=({"role": "system", "content": "You write brief factual notes strictly from supplied material."},
                          {"role": "user", "content": prompt}),
                temperature=0.0, max_output_tokens=300, timeout=budget)
...
async def _solve(query: Query, question: str) -> Response:
    ...
    if query.output_schema is not None:
        ...
        if structured is not None:
            ...
            note = None
            try:
                note = await _hxw_premise_note(question, query.output_schema,
                                               structured, ledger, citations)
            except Exception:
                note = None
            try:
...
# ===========================================================================
# hxw deterministic answer-finishing layer (wrapper around the base engine).
# The base engine's answer is a draft; every repair below is objectively
# checkable against the answer and schema alone — no model, no evidence
# re-read, no second retrieval. Code has final authority over form.
# ===========================================================================
import re as _hxw_re

_HXW_STRIP = _hxw_re.compile(r"[^0-9a-zA-Z]+")
_HXW_MONTHS = {m: i + 1 for i, m in enumerate(
    ("january", "february", "march", "april", "may", "june", "july",
     "august", "september", "october", "november", "december"))}
_HXW_ISO_DIRECTIVE = _hxw_re.compile(r"YYYY-MM-DD", _hxw_re.IGNORECASE)
_HXW_VERBATIM = _hxw_re.compile(
    r"\b(?:exactly\s+as\s+(?:written|printed|shown|published|given|listed|stated)"
    r"|as\s+(?:it\s+appears|written\s+in|printed\s+in|shown\s+in)|verbatim)\b",
    _hxw_re.IGNORECASE)


def _hxw_tokens(name: str) -> frozenset:
    return frozenset(t for t in _hxw_re.split(r"[^0-9a-zA-Z]+", (name or "").lower()) if t)


def _hxw_iso_date(text: str):
    t = text.strip()
    m = _hxw_re.search(r"\b((?:19|20)\d{2})-(\d{2})-(\d{2})\b", t)
    if m:
        return m.group(0)
    m = _hxw_re.search(r"\b(\d{1,2})/(\d{1,2})/((?:19|20)\d{2})\b", t)
    if m:
...
def _hxw_tidy(text: str) -> str:
    return _hxw_re.sub(r"\s+", " ", text).strip()


def _hxw_same_name(a: str, b: str) -> bool:
    ta = frozenset(t for t in _hxw_re.split(r"[^0-9a-zA-Z]+", a.lower()) if len(t) > 1)
    tb = frozenset(t for t in _hxw_re.split(r"[^0-9a-zA-Z]+", b.lower()) if len(t) > 1)
...
def _hxw_hygiene(output: dict, properties: dict) -> dict:
        ...
        if isinstance(value, str):
            ...
            for item in tidied:
                if isinstance(item, str) and any(
                    isinstance(k, str) and _hxw_same_name(item, k) for k in kept
                ):
                    continue
                kept.append(item)
            cleaned[name] = kept
            ...
            cleaned[name] = value
    # unsplit "Label: Value" pairs duplicated across a two-field object
    ...
    # an array item duplicating a scalar field's value is a misfiled copy
    ...
    for name, value in list(cleaned.items()):
        if isinstance(value, list):
            kept = [
                ...
                if not (isinstance(item, str) and _HXW_STRIP.sub("", item.lower()) in scalars)
            ]
            if kept != value:
                cleaned[name] = kept
    # a count field contradicting the list it counts
    for name, value in list(cleaned.items()):
        ...
        if stem:
            def related(other):
                ...
                return all(any((t.startswith(o) or o.startswith(t)) and min(len(t), len(o)) >= 4
                               for o in ot) for t in stem)
    ...
    def walk(value, ps):
        ...
        if isinstance(value, dict):
            ...
            for k in (ps.get("required") or []):
                if k not in out:
                    kt = (inner.get(k) or {}).get("type") or "string"
                    fv = {"integer": 0, "number": 0, "boolean": False,
                          "array": [], "object": {}}.get(kt, "unknown")
...
# ---------------------------------------------------------------------------
# Fetch shim: module-level shadow of the SDK fetch_page, rebinding every call
# site at call time. Document-class URLs route to the parser that actually
# reads them: PDFs go to Firecrawl's PDF parser first (Parallel deep-fetch as
# fallback), SEC.gov filings go to Firecrawl (proxy-friendly) with the same
# fallback. All other URLs pass through untouched.
# ---------------------------------------------------------------------------
...
def _hxw_is_sec(url):
    if not isinstance(url, str):
        return False
    host = _hxw_fetch_re.sub(r"^https?://", "", url.lower()).split("/")[0]
...
async def fetch_page(url, provider="parallel", provider_extra=None, timeout=None):
    if (_hxw_is_pdf(url) or _hxw_is_sec(url)) and _spend_left() >= 0.15:
        ...
        try:
            return await _hxw_sdk_fetch_page(url, provider="firecrawl",
                                             provider_extra=fc_extra, timeout=fc_timeout)
        except Exception:
            pass
        if _hxw_is_pdf(url):
    ...
    try:
        return await _hxw_sdk_fetch_page(url, provider=provider,
                                         provider_extra=provider_extra, timeout=timeout)
    except Exception:
        if _spend_left() < 0.10:
            raise
    return await _hxw_sdk_fetch_page(url, provider="firecrawl",
                                     provider_extra={"only_main_content": True, "max_age": 172800000},
                                     timeout=max(float(timeout or 0.0), 20.0))




# ---------------------------------------------------------------------------
# Question-shape pre-classifier: set-completeness and superlative questions
# get a deterministic strictness rule appended for the engine. The judge
# durably rewards full-pool enumeration with per-member cited verdicts and
# explicit exclusions; this injects that contract ONLY when the shape demands.
# ---------------------------------------------------------------------------
_HXW_SET_RE = _hxw_fetch_re.compile(
    r"\b(?:report every|list every|list each|list all|name every|identify all|"
    r"identify every|every entry|every member|all entries|complete list|full list|"
    r"each of (?:the|those|these)|for each)\b", _hxw_fetch_re.IGNORECASE)
_HXW_SUP_RE = _hxw_fetch_re.compile(
    r"\b(?:largest|smallest|highest|lowest|longest|shortest|fewest|earliest|latest)\b"
    r"[^.?!]{0,80}\b(?:of|among|across|between)\b", _hxw_fetch_re.IGNORECASE)


# ===========================================================================
# hxw v2: answer-draft authority + retrieval repair cycle.
# The base engine's output is a DRAFT. A mechanical gap audit decides whether
# the draft warrants a repair cycle: fresh designated-document retrieval, an
# evidence-informed correction proposal from one model call, and a per-field
# CODE-GATED merge in which deterministic rules — not the model — decide
# which value survives. This is a new cross-stage cycle on the ordinary path:
# draft -> audit -> fresh retrieval -> correction -> deterministic merge.
# ===========================================================================
import asyncio as _hxw_asyncio
import json as _hxw_json

_HXW_QUOTED = _hxw_re.compile(r'[\u201c"]([^"\u201c\u201d]{6,160})[\u201d"]')
_HXW_DOCCODE = _hxw_re.compile(
    r"\b(?:[A-Z]{1,6}[- ]?\d{1,4}(?:[-/]\d{1,4})+|(?:No\.|Number)\s?\d{1,4}|SI\s?\d{4}/\d+)\b")
_HXW_ORG = _hxw_re.compile(
    r"\b((?:[A-Z][\w&.'-]*\s+){1,5}(?:Agency|Bank|Council|Board|Service|Department|Commission|"
    r"Association|Authority|Federation|Office|Ministry|Institute|Administration|Society|"
    r"Organisation|Organization|Bureau|Centre|Center|Survey|Laboratory)"
    r"(?:\s+(?:of|for)\s+(?:the\s+)?(?:[A-Z][\w&.'-]*(?:\s+[A-Z][\w&.'-]*){0,2}))?)")
...
def _hxw_plausible(name: str, value) -> bool:
    ...
    if isinstance(value, str) and value.strip():
        text = value.strip()
        if "date" in tokens and not (
            _hxw_iso_date(text)
            or _hxw_re.search(r"\b(?:19|20)\d{2}\b", text)
            or _hxw_re.search(r"\b(?:january|february|march|april|may|june|july|august|"
                              r"september|october|november|december)\b", text, _hxw_re.IGNORECASE)
        ):
            return False
        if tokens.intersection(("accurate", "holds", "correct", "valid", "agrees")) and text.lower() not in (
            "yes", "no", "true", "false", "unknown"
        ):
            return False
        if tokens.intersection(("candidate", "player", "interviewee", "member", "author")) and not (
            tokens.intersection(("party", "code", "status", "letter", "type", "count", "number"))
        ):
...
async def _hxw_floor(question: str, schema: dict, draft: dict, evidence: str) -> dict | None:
    ...
    prompt = (
        ...
        + ("Fresh source excerpts (verbatim page text — prefer these over the draft on any conflict):\n"
           + evidence + "\n\n" if evidence else "")
        + "Draft answer from a mechanical pass (often wrong where marked unknown/zero):\n"
        + _hxw_json.dumps(draft) + "\n\n"
        + "Write the corrected COMPLETE answer as one JSON object for this schema:\n"
        + _hxw_json.dumps(schema) + "\n"
        + "Every value must come from the excerpts or the draft — never outside knowledge. "
    ...
    for model, budget in (("z-ai/glm-5.2", 45.0), ("openai/gpt-oss-120b", 30.0)):
        try:
            result = await llm_chat(
                provider="openrouter", model=model,
                messages=({"role": "system", "content": "You correct research answers strictly from supplied material."},
                          {"role": "user", "content": prompt}),
                temperature=0.0, max_output_tokens=1600, timeout=budget)
            raw = result.llm.raw_text or ""
            if raw:
                break
        except Exception:
...
async def _g67_base_query(query: Query) -> Response:
    ...
    if not _HXW_VERBATIM.search(question) and not _hxw_re.search(
            r"no commas|without commas|digits only|no (?:thousands? )?separators?|"
            r"without (?:thousands? )?separators?|no punctuation|plain digits only", question, _hxw_re.IGNORECASE):
        ...
        for name, value in list(draft.items()):
            ...
            if _hxw_tokens(name).intersection(_no_comma_fields):
                continue
            if _hxw_re.fullmatch(r"[1-9][0-9]{4,}", value):
                grouped = "{:,}".format(int(value))
                mx = ps.get("maxLength")
                if not (isinstance(mx, int) and len(grouped) > mx):
                    draft[name] = grouped
    if _hxw_re.search(r"MM/DD/YYYY", question) and not _HXW_VERBATIM.search(question):
        for name, value in list(draft.items()):
            if isinstance(value, str) and (properties.get(name) or {}).get("type") == "string":
                m = _hxw_re.fullmatch(r"(\d{1,2})/(\d{1,2})/(\d{4})", value.strip())
                if m:
    ...
    return Response(output=draft, citations=getattr(response, "citations", None),
