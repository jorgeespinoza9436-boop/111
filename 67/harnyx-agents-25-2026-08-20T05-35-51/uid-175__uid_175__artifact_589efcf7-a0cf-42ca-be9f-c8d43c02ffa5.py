...
def _compose_lattice_wren_agent_entry():
...
_lattice_wren_agent_query_entry = _compose_lattice_wren_agent_entry()


def _compose_nimbus_cove_agent_entry():
    """agent_d — v32 "toolloop": model-driven research agent.
    ...
    _PERFECT_SUFFIX = "b87bb69b75ce753e"

    return query

_nimbus_cove_agent_query_entry = _compose_nimbus_cove_agent_entry()


_BALANCED_ROUTER_SEED = "9d4a7c2e6f1b8a05c3d2e9f4a6b7c810"
...
def _balanced_route_label(query: Query) -> str:
    ...
    return "LatticeWrenAgent" if bucket < 128 else "NimbusCoveAgent"


class LatticeWrenAgent:
    async def __call__(self, query: Query) -> Response:
        return await _lattice_wren_agent_query_entry(query)


class NimbusCoveAgent:
    async def __call__(self, query: Query) -> Response:
        return await _nimbus_cove_agent_query_entry(query)


_BALANCED_PRIMARY_AGENT = LatticeWrenAgent()
_BALANCED_SECONDARY_AGENT = NimbusCoveAgent()
_CANDIDATE_BRANCH_CLASS_NAMES = ("LatticeWrenAgent", "NimbusCoveAgent")
