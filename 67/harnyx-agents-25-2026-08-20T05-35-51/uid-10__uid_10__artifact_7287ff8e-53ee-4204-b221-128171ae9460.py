...
VERSION = "v53-criteria-scope-figure"
...
# it never competes with the three post-audit sweeps for the tail.
...
# Both donor builds shipped this pattern twice under two different names with
...
# later merge adds grounding it must run BEFORE this stage: the two partition
# the same space by backer count and grounding owns the zero case.
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # are scope errors — an answer about the wrong entity or the wrong year is
    # not rescued by corroborating its figure — so both run before the numeric
    # check, and the period repair can replace that figure wholesale anyway.
...
class InterimBoundarySurveyor_09bdd7:
    ...
    BLOCK_WIDTH_09BDD7 = 12000          # hard ceiling for any single accounted window
    SPAN_FLOOR_09BDD7 = 96             # symmetric padding applied when widening
    WINDOW_CUTOFF_09BDD7 = 3              # smallest sample worth reporting on

    def normalize_extents_09bdd7(self, spans, limit=0):
        ...
        """
        ceiling = int(limit or self.BLOCK_WIDTH_09BDD7)
        cleaned = []
    ...
    def normalize_windows_09bdd7(self, spans, note_len, pad=0):
        """Widen merged windows by a fixed margin without crossing the note."""
        room = max(0, int(pad or self.SPAN_FLOOR_09BDD7))
        total = max(0, int(note_len or 0))
        widened = []
        for window in self.normalize_extents_09bdd7(spans, total or self.BLOCK_WIDTH_09BDD7):
    ...
    def fold_slices_09bdd7(self, rows, note_len=0):
        ...
        for row in rows or []:
            ...
            if not isinstance(length, (int, float)) or isinstance(length, bool):
                length = note_len
            for window in self.normalize_windows_09bdd7(spans, int(length or 0)):
    ...
    def compact_spans_09bdd7(self, rows, note_len=0):
        ...
        for row in rows or []:
            ...
            if not isinstance(row, dict):
                continue
            stats = self.fold_slices_09bdd7([row], note_len)
            if stats["windows"] < 1:
                continue
            label = row.get("title", "")
        ...
        if len(report) < self.WINDOW_CUTOFF_09BDD7:
