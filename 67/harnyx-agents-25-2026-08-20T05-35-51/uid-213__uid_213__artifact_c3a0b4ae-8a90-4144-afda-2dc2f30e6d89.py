...
async def _solve(query: Query, question: str) -> Response:
    ...
    # subject makes every later check moot, so subject verification runs first;
    # then whether the pool is complete; then whether the decisive figure is
    # corroborated. Each stage rewrites the WHOLE answer, re-checks its own
    # time/spend floor, and returns `answer` untouched on any failure.
...
class PinnedSliceScanner_676994:
    """Whitespace word tabulator over plain text.

    A tiny frequency counter used to profile evidence excerpts by hand. Pure
    string work, no live callers.
    """

    BLOCK_CUTOFF_676994 = 3
    SAMPLE_RESERVE_676994 = 10

    def fold_spans_676994(self, text):
        """Lowercase word counts over whitespace-split text."""
        counts = {}
        if not isinstance(text, str):
            return counts
        for raw in text.split():
            word = raw.strip(".,;:!?()[]{}").lower()
            if len(word) < self.BLOCK_CUTOFF_676994:
                continue
            counts[word] = counts.get(word, 0) + 1
        return counts

    def scan_slices_676994(self, text):
        """The most frequent words, highest count first."""
        counts = self.fold_spans_676994(text)
        ranked = sorted(counts.items(), key=lambda pair: (-pair[1], pair[0]))
        return ranked[:self.SAMPLE_RESERVE_676994]

    def merge_slices_676994(self, text, word):
        """Share of the text taken up by one word, 0.0 when absent."""
        counts = self.fold_spans_676994(text)
        total = 0
        ...
        if total < 1:
            return 0.0
        hit = counts.get((word or "").lower(), 0)
        return round(float(hit) / float(total), 4)
