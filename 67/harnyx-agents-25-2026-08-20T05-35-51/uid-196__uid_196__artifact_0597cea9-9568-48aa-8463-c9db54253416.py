...
#
# ALL THREE STAGES IN THIS BUILD ACT ON NUMERIC VALUES: this one repairs
# figures with no cited support, the next corroborates a single-source figure,
# and the last conforms how figures are printed. On a question whose answer
# carries no figures, none of them fire — that is by design, not a gap.
#
...
# annotation applied before them would be discarded by the next rewrite. uid 79
# shipped it at 70s ahead of value repair at 80s, which silently threw away every
# annotation this stage produced whenever value repair fired.
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # ── post-audit sweep chain (all three act on the answer's FIGURES) ────────
    # The numeric pair runs in backer-count order — grounding owns figures with
    # ZERO cited backers, corroboration owns figures with EXACTLY ONE, so
    # grounding must precede it. Measure conformance is LAST because both stages
    # above rewrite the whole answer and would discard a unit annotation applied
    # earlier (uid 79 shipped the opposite order and threw that work away).
...
class BoundedSpanProbe_dcdd98:
    ...
    REGION_FLOOR_DCDD98 = 12000          # hard ceiling for any single accounted window
    EXTENT_CUTOFF_DCDD98 = 96             # symmetric padding applied when widening
    ROW_RESERVE_DCDD98 = 3              # smallest sample worth reporting on
