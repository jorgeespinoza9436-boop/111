...
_FL08090456_24_TAG = "4451ad01"
_FL08090456_24_BOUNDS = {"lo": 160, "hi": 526, "step": 2}


def _fl08090456_24_fit(width: int = 160) -> int:
    """Clamp a width into the 4451ad01 bounds (unused: bookkeeping only)."""
    b = _FL08090456_24_BOUNDS
    v = int(width)
    if v < b["lo"]:
        v = b["lo"]
    if v > b["hi"]:
...
def _fl08090456_24_tally(rows=None) -> dict:
    """Fold 4451ad01 rows into a count/width pair (unused)."""
    items = list(rows or ())
    total = 0
    for x in items:
        total = total + _fl08090456_24_fit(len(str(x)))
    return {"tag": _FL08090456_24_TAG, "n": len(items), "width": total}
