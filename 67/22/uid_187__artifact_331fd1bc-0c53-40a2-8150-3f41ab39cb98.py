"""uid187 — difficulty-routed research agent on uid186 champion (v52).
...
uid187 additions (score-aimed, non-destructive):
  - deterministic EASY/HARD difficulty router (no LLM router cost)
  - EASY: lighter brief/turns/seeds; stop when evidence coverage is enough
  - HARD: research plan, claim evidence ledger, set-question engine, \
    citation densification audit, weak-search recovery
Target: surpass uid186 selection score while preserving champion behavior.
"""
...
FINGERPRINT_MARKERS = ( \
    DIFFICULTY_ROUTER_V187, RESEARCH_PLAN_V187, CLAIM_LEDGER_V187,
    SET_ENGINE_V187, CITATION_AUDIT_V187, FAILURE_RECOVERY_V187,
)
...
_EASY_LOOKUP_RE = re.compile( \
    r"^\s*(?:what|who|when|where|which)\b.{0,140}?\b(?:is|was|were|are|did|"
    r"does|do|has|have|had|directed|wrote|written|played|founded|released|"
    r"published|signed|painted|composed|invented|created|starred|born)\b",
    re.IGNORECASE | re.DOTALL,
)
_EASY_ATTR_RE = re.compile( \
    r"\b(?:capital|director|directed|author|writer|wrote|written|founder|"
    r"founded|president|ceo|born|released|published|title|name|year|"
    r"population|currency|language|headquarters|located|played|starred|"
    r"composed|painted|invented|created|signed|treaty|book|film|movie|"
    r"album|song|novel|play|series)\b",
    re.IGNORECASE,
)

_EASY_ORDINAL_TITLE_RE = re.compile( \
    r"\b(?:title|name|call(?:ed)?)\b.{0,40}\bthe\s+"
    r"(?:first|last|second|third|fourth|fifth)\b|"
    r"\bthe\s+(?:first|last|second|third)\s+"
    r"(?:\w+\s+){0,4}(?:book|film|movie|album|novel|play|episode)\b",
    re.IGNORECASE,
)
_HARD_COMPARE_RE = re.compile( \
    r"\b(?:compar(?:e|ison|ing)|versus|vs\.?|side[- ]by[- ]side|rank(?:ing|ed)?|"
    r"differ(?:ence|ent)|between .+ and)\b",
    re.IGNORECASE,
)
_HARD_SET_MARKER_RE = re.compile( \
    r"\b(?:all|every|each|top\s+\d+|largest|smallest|best|worst|most|least|"
    r"first|last|highest|lowest|greatest|fewest|only those|which of the)\b",
    re.IGNORECASE,
)
_HARD_MULTI_RE = re.compile( \
    r"\b(?:and then|also (?:identify|list|find|compute|calculate)|"
    r"multi[- ]hop|both .+ and|as well as)\b",
    re.IGNORECASE,
)
_HARD_SEC_RE = re.compile( \
    r"\b(?:10-K|10-Q|8-K|DEF\s*14A|Form\s+\d|EDGAR|sec\.gov|Item\s+\d)\b",
    re.IGNORECASE,
)
_HARD_CURRENT_RE = re.compile( \
    r"\b(?:as of (?:today|now|this (?:week|month|year))|current|latest|"
    r"most recent)\b",
    re.IGNORECASE,
)
_HARD_TABLE_RE = re.compile( \
    r"\b(?:according to (?:the )?(?:english )?wikipedia|"
    r"based on the .+ article|inhabited territories table)\b",
    re.IGNORECASE,
)
_EASY_FALSE_ONLY_RE = re.compile(r"\bonly\b(?!\s+(?:one|a|an)\b)", re.IGNORECASE)
...
def _router_features(question: str, output_schema=None) -> dict:
    ...
    return { \
        ...
        "set_question": set_q,
        "superlative": super_q,
        "compare": bool(_HARD_COMPARE_RE.search(q)),
        "set_marker": bool(_HARD_SET_MARKER_RE.search(q)),
        "multi": bool(_HARD_MULTI_RE.search(q)),
        "sec": bool(_HARD_SEC_RE.search(q)),
        "current": bool(_HARD_CURRENT_RE.search(q)),
        "named_table": bool(_HARD_TABLE_RE.search(q)),
        "lookup_shape": bool(_EASY_LOOKUP_RE.search(q)),
        "attr_shape": bool(_EASY_ATTR_RE.search(q)),
        "ordinal_title": bool(_EASY_ORDINAL_TITLE_RE.search(q)),
        "false_only": bool(_EASY_FALSE_ONLY_RE.search(q)),
        "simple_entity_attribute": ( \
            bool(_EASY_LOOKUP_RE.search(q))
            and bool(_EASY_ATTR_RE.search(q))
            and len(q) < 220
            and q.count("?") <= 1
        ),
    }
...
def _hard_route_signals(feats: dict) -> tuple[list[str], list[str]]:
    ...
    if feats["superlative"] and not ( \
        feats["ordinal_title"] and feats["simple_entity_attribute"]
    ):
...
def _fingerprint_system_note() -> str:
    ...
    return ( \
        f"AGENT_BUILD VERSION={VERSION} {marks}. "
        "Internal diagnostic only — do not include this diagnostic in the answer."
    )

def _record_route_diag(payload: dict) -> None:
    _ROUTE_DIAG.clear()
    _ROUTE_DIAG.update(payload)
    _RUN_DIAGNOSTICS.append( \
        f"ROUTE={payload.get('route')} reasons={payload.get('router_reasons')}"
    )

RESEARCH_PLAN_RULE = ( \
    "RESEARCH PLAN (HARD TASK) — before more tool calls, lock:\n"
...
def _deterministic_research_plan(question: str) -> str:
    ...
    lines = [ \
        f"{RESEARCH_PLAN_V187} PRIOR PLAN (deterministic skeleton).",
        "facts: every named entity/condition/figure; the asked KIND.",
        "failures: incomplete pool; uncited hard condition; rounded substitute; "
        "wrong period/scope; KIND mismatch.",
    ]
    if set_q or super_q:
        lines.append( \
            "done_when: authoritative roster fetched; every pool member has a "
            "cited verdict; answer lists all qualifiers (or the proven winner)."
        )
        lines.append( \
            "searches: '<pool subject> list/table'; named-source page; "
            "per-condition verification."
        )
    else:
        lines.append( \
            "done_when: each load-bearing claim has a primary-source [n]; "
            "answer opens with the asked entities/values."
        )
...
async def _research_plan(question: str, deadline: float) -> str:
    ...
    system = ( \
        "Research planner. Output a tight plan only — never the final answer. "
        "Use the exact lowercase tags below."
    )
    user = ( \
        f"Question:\n{question}\n\n"
        "facts: numbered atomic facts needed to answer.\n"
        "failures: likely zero-score modes (incomplete set, wrong filter, "
        "rounded number, missing citation).\n"
    ...
    try:
        raw = await _chat_simple( \
            ...
            max_tokens=900, timeout=min(28.0, BRIEF_TIMEOUT_S),
    ...
    if not raw or len(raw.strip()) < 40:
        return _deterministic_research_plan(question)
    return ( \
        f"{RESEARCH_PLAN_V187} PRIOR PLAN — verify with tools; never ship this "
        "worksheet as the answer.\n" + raw.strip()
    )
...
class ClaimLedger:
    ...
    def add(self, claim: str, *, source: str = "", evidence: str = "", \
            confidence: str = "low", status: str = "unsupported") -> None:
        c = (claim or "").strip()
        if not c:
            return
        self.records.append(ClaimRecord( \
            claim=c[:240], source=(source or "")[:80],
    ...
    def note_retained(self, source_n: int, quote: str) -> None:
        ...
        if not q:
            return
        self.add( \
            q[:160], source=f"[{source_n}]", evidence=q[:280],
...
CLAIM_LEDGER_RULE = ( \
    "EVIDENCE LEDGER DISCIPLINE: every factual claim you will assert needs "
    "claim / source[n] / supporting evidence / confidence / status=supported. "
    "Call retain_evidence the moment you find decisive text. Never include an "
    "unsupported claim in the final answer — drop it or verify it first."
)

SET_ENGINE_RULE = ( \
    f"{SET_ENGINE_V187} SET / TOP / LARGEST / FIRST / BEST / MOST — ENGINE:\n"
    ...
    "5) REPORT UNCERTAINTY: commit verified qualifiers; never invent members; "
    "never hide gaps behind 'among others'."
)
    ...
    for part in parts:
        for tok in _WORD_RE.findall((part or "").casefold()):
            ...
            if tok not in seen:
                seen.add(tok)
                bag.append(tok)
        for num in re.findall(r"\d+(?:\.\d+)?%?", part or ""):
            n = num.casefold()
            if n not in seen:
                seen.add(n)
                bag.append(n)
...
def _infer_supports(question: str, kind: str, args: dict, \
                    title: str, url: str, preview: str) -> list[str]:
    terms = _claim_terms_from_text(question, title, preview[:400])
    present = [ \
        t for t in terms
        if t in (preview or "").casefold() or t in (title or "").casefold()
    ]
    if not present:
        return []
    return [f"Supports: evidence mentions {', '.join(present[:8])}"]

def _claim_targeted_spans( \
    note: str,
    terms: set[str],
    fallback: list[tuple[int, int]] | None,
    *,
    max_spans: int = 2,
    width: int = 2200,
) -> list[tuple[int, int]]:
...
def _densified_ref(ledger: "EvidenceLedger", number: int, \
                   claim_terms: set[str]) -> CitationRef | None:
    ...
    if len(note) < 200 or row.get("retained"):
        return ref
    targeted = _claim_targeted_spans( \
        note, {t.casefold() for t in claim_terms},
        list(row.get("spans") or []),
        max_spans=2,
        width=min(FETCH_WINDOW_CHARS, 2800),
    )
    if not targeted:
        return ref
    old = row.get("spans")
...
CITATION_AUDIT_RULE = ( \
    f"{CITATION_AUDIT_V187} CITATION AUDIT before final answer:\n"
...
FAILURE_RECOVERY_RULE = ( \
    f"{FAILURE_RECOVERY_V187} WEAK SEARCH RECOVERY: if results are empty, "
    "off-topic, or lack the deciding figure — retry with (a) a different query "
    "(drop site:/quotes, swap synonyms, add year), (b) a different source class "
    "(primary agency/registry vs encyclopedia), (c) alternative evidence "
    "(roster page, filing, official stats). Do not repeat the same failed query. "
    "Batch independent retries in one turn."
)

EASY_LOOP_RULE = ( \
    "EASY LOOKUP: this is a single-fact / simple entity-attribute question. "
    "Prefer 1-2 precise searches, fetch the best primary/encyclopedia page, "
    "retain_evidence on the decisive sentence, and answer concisely with [n]. "
    "Do not build multi-candidate pools or burn turns on decorative corroboration. "
    "Stop when the asked fact is cited."
)
...
class EvidenceLedger:
    ...
    def add(self, receipt_id: str, result_id: str, note_len: int, \
            ...
            claim_terms: list[str] | None = None) -> int:
        norm: list[str] = []
        ...
        self.rows.append({ \
            ...
            "retained": [],
            "supports": norm,
            "claim_terms": list(claim_terms or [])[:24],
        })
...
def _search_row_from_item(item, receipt: str, query_text: str) -> dict | None:
    ...
    supports = _infer_supports("", "web_search", {"query": query_text}, \
                               title, url, note[:SEARCH_EXCERPT_CHARS])
    claim_terms = _claim_terms_from_text(title, note[:SEARCH_EXCERPT_CHARS])
    return { \
        "receipt_id": receipt, "result_id": rid, "note_len": n_len,
        "kind": "search", "spans": _search_excerpt_span(n_len),
        "title": title, "url": url,
        "preview": note[:SEARCH_EXCERPT_CHARS], "text": note,
        "supports": supports, "claim_terms": claim_terms,
    }
...
def _format_search_tool_output(query_text: str, receipt: str, results: list):
    ...
    for item in results:
        ...
        lines.append( \
            f"[{_SLOT.format(len(rows) - 1)}] {row['title']} — {row['url']}"
...
def _fetch_plain_tool_output(url: str, question: str, focus: str, \
                             receipt: str, rid: str, note: str) -> ToolOutput:
    supports = _infer_supports(question, "read_page", {"url": url}, \
                               url, url, note[:1200])
...
async def _dispatch_named_tool(name: str, args: dict, question: str, \
...
async def _llm_chat_pin_ladder(lane: str, model: str, messages: list[dict], *, \
                               max_tokens: int, timeout: float,
                               think: dict, temperature: float = 0.15):
...
async def _chat_simple(lane: str, model: str, system: str, user: str, *, \
    ...
    payload = await _llm_chat_pin_ladder( \
        lane, model,
...
async def _attempt_chat_turn_lane(messages: list[dict], deadline: float, \
                                  turn_wall: float, *, finish_only: bool,
                                  force_tools: bool, lane: str, model: str,
                                  pinned: bool, payload_chars: int):
    ...
    payload = await asyncio.wait_for(llm_chat( \
        ...
        temperature=0.2,
        thinking=_loop_turn_thinking(finish_only, lane),
...
async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool, \
    ...
    for lane, model, pinned in _CHAT_TURN_LANES:
        try:
            return await _attempt_chat_turn_lane( \
                messages, deadline, turn_wall,
                finish_only=finish_only, force_tools=force_tools,
                lane=lane, model=model, pinned=pinned,
...
_BRIEF_SYSTEM = ("Senior research analyst. Commit to concrete best answers from "
...
def _brief_system_block(raw: str) -> str:
    return ("PRIOR ANALYSIS — your own planning worksheet (verify anything marked "
...
async def _brief_raw_dual_lane(system: str, user: str) -> str:
    try:
        return await _chat_simple(LLM_LANE_A, LOOP_MODEL_A, system, user, \
                                  max_tokens=2400, timeout=BRIEF_TIMEOUT_S,
                                  think=_least_think(LLM_LANE_A, LOOP_MODEL_A))
    except Exception:
        try:
            return await _chat_simple(LLM_LANE_B, LOOP_MODEL_B, system, user, \
                                      max_tokens=2400, timeout=BRIEF_TIMEOUT_S,
...
def _append_loop_rule_blocks(messages: list[dict], question: str, \
                             route: str, plan: str, brief: str) -> bool:
...
async def _bootstrap_loop_messages(question: str, brief: str, \
                                   ledger: EvidenceLedger, deadline: float,
                                   route: str, plan: str) -> list[dict]:
...
async def _collect_tool_fanout_results(run_calls, question: str, \
...
def _commit_fanout_into_messages(messages: list[dict], calls, run_calls, \
                                 results, ledger: EvidenceLedger) -> None:
...
def _apply_finish_candidate(messages: list[dict], candidate: str, \
                            repairs_left: int, deadline: float
                            ) -> tuple[str, int, str]:
...
async def _loop(question: str, brief: str, ledger: EvidenceLedger, \
                ...
                route: str = "hard",
                plan: str = "") -> tuple[str, list[dict]]:
    if carry is not None:
        messages = carry
    else:
        messages = await _bootstrap_loop_messages( \
            question, brief, ledger, deadline, route, plan)
    ...
    for turn in range(1, turn_cap + 1):
        ...
        if not calls:
            ...
            answer, repairs_left, action = _apply_finish_candidate( \
                messages, candidate, repairs_left, deadline)
            if action == "repair":
                continue
            break
        ...
        run_calls = calls[:8]
        results = await _collect_tool_fanout_results( \
            run_calls, question, ledger, deadline)
...
def _deterministic_citation_gaps(answer: str, question: str) -> list[str]:
    ...
    for sent in re.split(r"(?<=[.!?])\s+", text):
        ...
        if _CITE_MARK_RE.search(s):
            continue
        if re.search(r"\d", s) or re.search(r"\b[A-Z][a-z]{2,}\b", s):

            if re.match(r"^(?:proof|candidates|pool|notes?)\b", s, re.I):
                continue
            gaps.append(s[:160])
            if len(gaps) >= 4:
                break
    if (_needs_set_completeness(question) or _needs_superlative_proof(question)):
        if not re.search(r"\b(?:none|no |all |every )", text, re.I) and text.count("[") < 2:
            gaps.append("set_or_superlative_under_cited")
    return gaps

_AUDIT_GAP_KEYS = ( \
    "incomplete_roster", "hand_waved_tally", "unanswered_parts",
    "uncited_facts", "wrong_kind", "thin_proof",
    "citation_mismatch", "missing_requested",
)
...
def _audit_probe(question: str, answer: str) -> str:
    return ( \
        ...
        '"citation_mismatch" (list; [n] that does not support the adjacent claim), '
        '"missing_requested" (list; asked items absent from the answer). '
...
async def _audit_report(question: str, answer: str, deadline: float):
    try:
        raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, \
                                 ...
                                 _audit_probe(question, answer), max_tokens=2200,
...
async def _audit_patch(question: str, answer: str, messages: list[dict], \
    ...
    messages.append({"role": "system", \
                     "content": _audit_rewrite_order(gaps, roster_gaps)})
...
async def _write_from_digest(question: str, ledger: EvidenceLedger, deadline: float) -> str:
    ...
    async def _one(lane: str, model: str, budget: float) -> str:
        ...
        payload = await _llm_chat_pin_ladder( \
            ...
            think=_least_think(lane, model), temperature=0.15,
        )
        _spend_note(payload)
        return _llm_text_from_payload(payload)
...
async def _prepare_brief_and_plan(question: str, route: str, \
    ...
    try:
        if route == "hard" and _spend_left() >= BRIEF_MIN_USD \
...
async def _run_research_loop(question: str, brief: str, plan: str, route: str, \
                             ledger: EvidenceLedger, deadline: float,
                             turn_cap: int) -> tuple[str, list[dict]]:
    answer = ""
    messages: list[dict] = []
    try:
        answer, messages = await _loop( \
            question, brief, ledger, deadline, turn_cap,
...
async def _maybe_citation_audit(question: str, answer: str, messages: list[dict], \
                                ledger: EvidenceLedger, deadline: float,
                                route: str) -> str:

    try:
        det_gaps = (_deterministic_citation_gaps(answer, question) \
                    if _is_usable_answer(answer) else ["no_answer"])
        need_audit = ( \
            _is_usable_answer(answer)
            and (deadline - monotonic()) > 75.0
            and _spend_left() >= AUDIT_MIN_USD
            and (route == "hard" or det_gaps)
        )
        if need_audit:
...
async def _structured_response(query: Query, question: str, answer: str, \
                               ledger: EvidenceLedger, citations: list,
...
async def _solve(query: Query, question: str) -> Response:
    ...
    ledger = EvidenceLedger()
    answer, messages = await _run_research_loop( \
        question, brief, plan, route, ledger, deadline, turn_cap)
    answer = await _maybe_citation_audit( \
        question, answer, messages, ledger, deadline, route)
    ...
    structured = await _structured_response( \
        query, question, answer, ledger, citations, deadline)
    if structured is not None:
        return structured
...
# slot: 43 FB_3ad22a6a_w2 2026-08-09T15:00:00+00:00
