"""hk413 "condcov+authority" — champion-v52 toolloop + condition-coverage tracking + authority sweep.
...
# ── condition coverage tracker ────────────────────────────────────────────────
# The knowledge brief already enumerates the question's atomic conditions, but
# that block has only ever been prompt text. Parse the conditions back out and,
# when the loop is half spent, tell the model exactly which conditions still
# have NO supporting ledger rows — those become uncited claims, and uncited
# claims score zero. Detection is token overlap against stored row text: zero
# extra LLM calls, zero extra searches, and any failure just skips the nudge.
_COND_LINE_RE = re.compile(r"^\s*(?:\d+[.)]\s*|[-*]\s+)(.{8,240})$")


def _parse_conditions(worksheet: str) -> list[str]:
    """The 'conditions:' block of the brief worksheet, one condition per entry."""
    if not worksheet:
        return []
...
def _condition_covered(cond: str, ledger: EvidenceLedger) -> bool:
    """Does ANY gathered row plausibly touch this condition? Token overlap only —
    optimistic on purpose: a false 'covered' skips a nudge, a false 'open' costs
    one aimed search, and the model remains free to disagree with the hint."""
    terms = _key_terms(cond)
    if not terms:
        return True
...
def _coverage_nudge(conditions: list[str], ledger: EvidenceLedger) -> str:
    try:
        open_conds = [c for c in conditions if not _condition_covered(c, ledger)]
    except Exception:
        return ""
    if not open_conds:
        return ""
...
async def _loop(question: str, brief: str, ledger: EvidenceLedger,
                ...
                conditions: list[str] | None = None) -> tuple[str, list[dict]]:
    ...
    for turn in range(1, turn_cap + 1):
        ...
        # midpoint coverage nudge: one shot, never during wrapup
        if conditions and turn == max(2, turn_cap // 2) and not finish_only:
            try:
                nudge = _coverage_nudge(conditions, ledger)
                if nudge:
                    messages.append({"role": "system", "content": nudge})
                conditions = None      # fired — never nudge twice
            except Exception:
                conditions = None
...
async def _solve(query: Query, question: str) -> Response:
    ...
    try:
        try:
            conditions = _parse_conditions(brief)
        except Exception:
            conditions = []
        answer, messages = await _loop(question, brief, ledger, deadline,
                                       MAX_TURNS, conditions=conditions)
    except Exception:
        answer = ""
