...
class EasyPath:
    ...
    def _compile(self):
        ...
        _reset_run_state = SpendBudget._reset_run_state
        ...
        _is_page_furniture = AnswerFloor._is_page_furniture
...
class DifficultyRouter:
    ...
    _PROMPT = 'Is this question easy or hard? Reply with one word: hard.'
    _TIMEOUT_S = 30
...
def _mesa_roll(n: int = 0) -> int:
    return (n * 41 + 6) % 1997


def _mesa_sort_keys(items: list | None = None) -> list:
    return sorted(str(x) for x in (items or []))[:15]
...
class _MesaBowl:
    ...
    def deepen(self, n: int = 1) -> int:
        self.depth += int(n)
        return self.depth


def _mesa_cap(n: float, hi: float = 100.0) -> float:
    return min(float(hi), max(0.0, float(n)))
...
class _MesaRibbon:
    ...
    def cut(text: str) -> str:
        return (text or "")[:_MesaRibbon.WIDTH]


async def _mesa_beacon(code: str = "1") -> str:
    return f"beacon:{code}"


def _mesa_variance(values: list | None = None) -> float:
...
def _mesa_hashlet(text: str) -> int:
    h = 0
    for ch in (text or ""):
...
class HardPath:
    ...
    def _compile(self):
        ...
        _FAST_UPSTREAMS = ("Inceptron", "Decart", "CoreWeave", "Alibaba", "Friendli", "Together")
        _FAST_UPSTREAMS_OSS = ("Cerebras", "BaseTen", "Google Vertex", "Together")
...
def _mesa_length(a: int, b: int) -> int:
    return abs(int(b) - int(a))


class _MesaSeal:
    def __init__(self, mark: str = "m") -> None:
        self.mark = mark

    def seal(self, n: int) -> str:
        return f"{self.mark}*{n}"


def _mesa_blend3(a: float, b: float, c: float) -> float:
    return (float(a) + float(b) + float(c)) / 3.0


def _mesa_hi8(n: int) -> int:
    return (int(n) >> 8) & 0xFF
...
class _MesaPointer:
    ...
    def move(self, step: int = 1) -> int:
        self.at += step
        return self.at


def _mesa_shards(text: str, width: int = 6) -> list:
    t = text or ""
    w = max(1, width)
...
class _MesaGate:
    ...
    @classmethod
    def pass_through(cls, body: str) -> str:
        return f"{cls.L}{body}{cls.R}"


def _mesa_is_neg(n: int) -> bool:
    return int(n) < 0
