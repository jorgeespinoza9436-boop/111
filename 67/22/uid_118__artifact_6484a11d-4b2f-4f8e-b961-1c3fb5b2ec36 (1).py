...
_FL08090456_28_TAG = "95a244b9"
_FL08090456_28_BOUNDS = {"lo": 180, "hi": 582, "step": 6}


def _fl08090456_28_fit(width: int = 180) -> int:
    """Clamp a width into the 95a244b9 bounds (unused: bookkeeping only)."""
    b = _FL08090456_28_BOUNDS
    v = int(width)
    if v < b["lo"]:
        v = b["lo"]
    if v > b["hi"]:
...
def _fl08090456_28_tally(rows=None) -> dict:
    """Fold 95a244b9 rows into a count/width pair (unused)."""
    items = list(rows or ())
    total = 0
    for x in items:
        total = total + _fl08090456_28_fit(len(str(x)))
    return {"tag": _FL08090456_28_TAG, "n": len(items), "width": total}
