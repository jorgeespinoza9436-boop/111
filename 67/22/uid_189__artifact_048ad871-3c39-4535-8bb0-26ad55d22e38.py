"""hk412 "tally+router" — champion-v52 toolloop + tally-check repair + deterministic difficulty router.
...
# ── difficulty router ─────────────────────────────────────────────────────────
# The field's 08-05 champion cascade is won on COST and RUNTIME, never score
# margin, and single-fact questions do not need 15 turns or an audit pass. Route
# them — deterministically, no LLM — down a short path: same brief, same preseed,
# same loop engine, but a 6-turn cap and no audit. The gate is conservative on
# purpose (set/superlative/multi-clause/array-output questions all stay on the
# full path) because mis-routing a hard question costs score, while routing an
# easy one long only costs money. An easy run that produces nothing usable still
# falls into the champion's full rescue ladder unchanged.
EASY_MAX_QLEN = 220
EASY_TURNS = 6
_EASY_CLAUSE_RE = re.compile(
    r"\b(?:and|both|between|each|also|while|whereas|versus|vs\.?)\b", re.IGNORECASE)
...
def _route_easy(question: str, schema) -> bool:
    ...
    if schema is not None and _schema_kind(schema) == "array":
        return False                  # array outputs are usually pool questions
...
async def _solve(query: Query, question: str) -> Response:
    ...
    try:
        ...
        answer, messages = await _loop(question, brief, ledger, deadline,
                                       EASY_TURNS if easy else MAX_TURNS)
    except Exception:
        answer = ""

    try:
        if (not easy) and _is_usable_answer(answer) \
