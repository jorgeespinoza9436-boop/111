"""hk403 "setsweep+citefill" — champion-v52 toolloop + set gap-fill sweep + citation URL-dedupe/backfill.
...
# ── stage 3c: set gap-fill sweep ──────────────────────────────────────────────
# The measured production loss on pool questions is an INCOMPLETE set: batch
# 0dcd57c6 task a2f3edd7 — we enumerated 2 of 4 qualifying members and scored 0
# where the champion's 4/4 took 1.0. The audit's roster probe is an LLM opinion;
# this stage is the deterministic backstop. On set/superlative questions whose
# answer looks under-enumerated (fewer than 3 visible members, or hedged with
# 'among others'), spend ONE list-hunting search plus a bounded rewrite round
# aimed at the full pool. Fires narrowly, inherits the audit's regression
# guards, and any failure returns the answer untouched.
_GF_HEDGE_RE = re.compile(
    r"\bamong others\b|\band (?:several|many|other)s? (?:more|others)\b"
    r"|\bnot (?:an )?exhaustive\b|\bpartial list\b", re.IGNORECASE)
_GF_MEMBER_LINE_RE = re.compile(r"^\s*(?:[-*\u2022]|\d+[.)])\s+\S", re.MULTILINE)
GF_MIN_MEMBERS = 3
...
async def _set_gapfill(question: str, answer: str, messages: list[dict],
    ...
    if not hedged and members >= GF_MIN_MEMBERS:
        return answer
    try:
        out = await asyncio.wait_for(_do_search(_gf_list_query(question), ledger),
...
_BF_STRIP_CITES_RE = re.compile(r"\[[0-9][0-9,\s\-]*\]")
_BF_VALUE_RE = re.compile(r"\$?\b\d[\d,]*(?:\.\d+)?%?")
_BF_MARGIN = 300           # context kept either side of a backfilled figure
_BF_MAX_VALUES = 12
...
# ── entrypoint ────────────────────────────────────────────────────────────────
async def _s21_base_query(query: Query) -> Response:
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # stage 3c: deterministic completeness backstop for pool questions
    try:
        if _is_usable_answer(answer) and (deadline - monotonic()) > 75.0 \
                and _spend_left() >= AUDIT_MIN_USD:
            swept = await _set_gapfill(question, answer, messages,
                                       ledger, deadline)
...
# =====================================================================
# submittion21 MECHANISM — multi-entity comparison/synthesis side-coverage
# gate + cross-source disagreement reconciliation gate
# =====================================================================
#
# Runs after the base pipeline above has produced a draft Response.
# Earlier lineage stages (where present on a minority of 21 agents)
# decompose by already-stated CLAIM for contradiction re-checking (s17),
# by comparison SIDE sharing one metric gated behind an is_comparison
# classifier (s19 Stage A), by NEGATION/ABSENCE polarity (s19 Stage B),
# by un-anchored current-state sentences / basis descriptors (s20), or
# by the question's multi-category REQUIREMENT list / schema leaf tree
# (s20u). None of those stages do BOTH of the following, which map
# directly to this platform's pairwise and task-generation pressures:
#
#   (A) Decompose the QUESTION into named sides / entities bound to a
#       shared relation or metric, then coverage-check each side AND the
#       comparative/synthesis conclusion with fresh retrieval — scoring
#       rule: "For comparison and synthesis queries, citation evidence
#       must cover each side of the comparison and the conclusion being
#       drawn from them." Task generation defaults to cross-entity
#       comparison and multi-source synthesis shapes.
#
#   (B) Detect load-bearing numeric / named-result claims in the DRAFT
#       and dual-retrieve them to classify source agreement, then
#       reconcile disputed readings (prefer official/primary when
#       identifiable, else state both with citations). Task generation
#       explicitly rewards "resolving source disagreement" and
#       "official-versus-secondary-source disagreement."
#
# STAGE A - multi-entity comparison / synthesis side-coverage gate
# (text AND structured JSON):
#   1. LLM decomposes the ORIGINAL QUESTION into up to 4 named sides
#      (entity/item) plus one shared relation_or_metric and one
#      conclusion_requirement. Fires only when >=2 sides are found —
#      different axis from s20u's category-tagged requirement list
#      (which fires on any multi-part question, including single-entity
#      multi-field lookups) and from s19 (which early-returns for
#      non-comparison and does not bind a conclusion requirement).
#   2. Coverage-audits the draft for each side value AND the conclusion
#      (satisfied / weak / missing) with a side-specific search query.
#   3. One NEW search_web call per gap (concurrent, max 3).
#   4. Patches free text or shallow JSON (existing keys only); attaches
#      CitationRefs from this pass's own receipts.
#
# STAGE B - cross-source disagreement reconciliation gate (text AND
# structured JSON):
#   1. LLM extracts up to 4 load-bearing numeric or named-result claims
#      already stated in the draft (not invented).
#   2. For each claim, issues TWO independent search_web calls
#      (parallel + desearch) — dual-provider retrieval that no earlier
#      stage in this lineage performs as a verification primitive.
#   3. LLM classifies each claim agreed / disputed / single_source /
#      unverifiable from the dual evidence only.
#   4. For disputed (and optionally single_source weak) claims, patches
#      the answer to reconcile: prefer a clearly official/primary
#      reading when identifiable, otherwise state both readings with
#      explicit attribution; attaches citations from both providers
#      when used.
#
# This changes decomposition source (named sides + relation + conclusion
# from the question; dual-provider evidence pairs per stated claim),
# verification semantics (per-side + conclusion coverage; source
# agreement / dispute), tool use (fresh side-targeted searches; paired
# dual-provider searches), and remediation (side fill-in; disagreement
# reconciliation rewrite) relative to the base 21 pipeline. It is not a
# prompt or parameter tweak. Any failure, missing evidence, non-dict
# structured output, or time shortage is a strict no-op that returns the
# prior stage's own response (after cheap exact duplicate-citation
# cleanup only).

import asyncio as _s21_asyncio
import json as _s21_json
import re as _s21_re
from time import monotonic as _s21_monotonic

_S21_HARD_BUDGET_GATE_S = 250.0
_S21_MAX_WINDOW_S = 55.0
_S21_MIN_WINDOW_S = 10.0
_S21_SIDE_EXTRACT_TIMEOUT_S = 8.0
_S21_SIDE_COVERAGE_TIMEOUT_S = 9.0
_S21_CLAIM_EXTRACT_TIMEOUT_S = 8.0
_S21_DISAGREE_TIMEOUT_S = 10.0
_S21_SEARCH_TIMEOUT_S = 9.0
_S21_PATCH_TIMEOUT_S = 12.0
_S21_MAX_SIDES = 4
_S21_MAX_SIDE_GAPS = 3
_S21_MAX_CLAIMS = 4
_S21_MAX_DISPUTE_PATCHES = 2
_S21_MAX_NEW_CITATIONS_PER_GAP = 2
_S21_MAX_TOTAL_CITATIONS = 60
_S21_MODEL = "deepseek/deepseek-v3.2"

_S21_SIDE_EXTRACT_SYSTEM_PROMPT = (
    "You decompose a research question into the named SIDES / ENTITIES it "
    "asks to compare, reconcile, or jointly synthesize, plus the shared "
    "relation or metric that binds them, plus the conclusion the answer "
    "must draw.\n"
    "A side is a specific named entity, item, filing, result, or source "
    "the question requires the answer to address individually.\n"
    "Emit is_multi_entity=true only when the question genuinely requires "
    "at least two distinct named sides bound by one shared relation, "
    "metric, or synthesis obligation (cross-entity comparison, "
    "cross-source reconciliation, or multi-source synthesis). Emit "
    "is_multi_entity=false for single-entity lookups, even if they ask "
    "for several fields about one entity.\n"
    "Return JSON only: {\"is_multi_entity\": bool, \"relation_or_metric\": "
    "str, \"conclusion_requirement\": str, \"sides\": [{\"name\": str, "
    "\"need\": str}, ...]} with up to 4 sides. Each side.need is a short "
    "imperative stating what value/fact about that side must appear."
)

_S21_SIDE_COVERAGE_SYSTEM_PROMPT = (
    "You are a strict multi-entity side-coverage auditor.\n"
    "You receive named sides (each with a need), a shared relation/"
    "metric, a conclusion requirement, and the CURRENT answer content "
    "(prose or JSON).\n"
    "For EACH side, by 0-based index, decide: satisfied / weak / "
    "missing whether the answer states a concrete value for that side's "
    "need under the shared relation. Also judge the conclusion "
    "separately with the same three verdicts.\n"
    "For any weak/missing item, produce a short targeted web search "
    "query (5-15 words) that would source that one side or the "
    "conclusion — never a restatement of the whole question.\n"
    "Return JSON only: {\"sides\": [{\"index\": int, \"verdict\": "
    "\"satisfied\"|\"weak\"|\"missing\", \"gap_query\": str or null}, "
    "...], \"conclusion\": {\"verdict\": \"satisfied\"|\"weak\"|"
    "\"missing\", \"gap_query\": str or null}}."
)

_S21_PATCH_SIDE_TEXT_SYSTEM_PROMPT = (
    "You add or correct ONE named side (or the comparative conclusion) "
    "inside a research answer using freshly retrieved evidence.\n"
    "Rewrite the COMPLETE answer: keep unrelated parts intact where "
    "feasible; add/correct only what this side or conclusion needs. If "
    "evidence is insufficient, state what is known and flag what is "
    "unconfirmed rather than guessing.\n"
    "Preserve existing citation markers whose content is unchanged. "
    "Output plain answer text only: no preamble, no fences, no meta-"
    "commentary."
)

_S21_PATCH_SIDE_OUTPUT_SYSTEM_PROMPT = (
    "You add or correct ONE named side (or the comparative conclusion) "
    "inside a structured JSON research answer using fresh evidence.\n"
    "Return ONLY JSON keys (top-level or one level nested) whose values "
    "must change, using ONLY key names that already exist in the schema "
    "or current answer — never invent new keys. If evidence is weak, "
    "return an empty patch.\n"
    "Also report which evidence snippets (0-based) you used.\n"
    "Return JSON only: {\"patch\": {...} or {}, \"used_indices\": "
    "[int, ...]}"
)

_S21_CLAIM_EXTRACT_SYSTEM_PROMPT = (
    "You extract load-bearing numeric or named-result CLAIMS already "
    "stated in a research answer — exact figures, rankings, winners, "
    "vote totals, leadership identities, dates of record, or official "
    "outcomes that a pairwise judge would score as factual claims.\n"
    "Do NOT invent claims the answer does not state. Prefer claims that "
    "could plausibly disagree across official vs secondary sources.\n"
    "Return JSON only: {\"claims\": [{\"claim\": str, \"probe_query\": "
    "str}, ...]} with up to 4 items. probe_query is a short search "
    "(5-15 words) to re-verify that one claim from independent sources."
)

_S21_DISAGREE_SYSTEM_PROMPT = (
    "You are a strict cross-source agreement classifier.\n"
    "You receive ONE claim from an answer and TWO independently retrieved "
    "evidence packs (provider_a and provider_b).\n"
    "Decide exactly one verdict:\n"
    "- agreed: both packs support the same concrete reading of the claim.\n"
    "- disputed: the packs support materially different figures, "
    "identities, dates, or outcomes for the same claim.\n"
    "- single_source: only one pack has usable support.\n"
    "- unverifiable: neither pack supports or refutes the claim usefully.\n"
    "If disputed, identify preferred_reading (short string) when one pack "
    "is clearly more official/primary; else null. Also give alt_reading "
    "when useful, and a one-sentence reconciliation_note.\n"
    "Return JSON only: {\"verdict\": \"agreed\"|\"disputed\"|"
    "\"single_source\"|\"unverifiable\", \"preferred_reading\": str or "
    "null, \"alt_reading\": str or null, \"reconciliation_note\": str, "
    "\"used_a\": [int, ...], \"used_b\": [int, ...]}"
)

_S21_PATCH_DISPUTE_TEXT_SYSTEM_PROMPT = (
    "You reconcile ONE disputed (or weakly single-sourced) factual claim "
    "inside a research answer using dual-provider evidence.\n"
    "Rewrite the COMPLETE answer: keep unrelated content intact; replace "
    "only the disputed claim span. Prefer an official/primary reading "
    "when identified; otherwise state both readings with clear "
    "attribution rather than silently picking one. Do not invent numbers.\n"
    "Preserve unrelated citation markers. Output plain answer text only."
)

_S21_PATCH_DISPUTE_OUTPUT_SYSTEM_PROMPT = (
    "You reconcile ONE disputed factual claim inside a structured JSON "
    "research answer using dual-provider evidence.\n"
    "Return ONLY JSON keys that must change, using existing keys only — "
    "never invent keys. Prefer official/primary reading when identified; "
    "otherwise encode both readings in the existing value fields if the "
    "schema already supports descriptive strings. Empty patch if unsure.\n"
    "Return JSON only: {\"patch\": {...} or {}, \"used_indices\": "
    "[int, ...]}"
)


def _s21_strip_json_fences(raw: str) -> str:
    return _s21_re.sub(r"^```(?:json)?\s*|\s*```$", "", raw or "", flags=_s21_re.I | _s21_re.M).strip()


def _s21_chat_text(llm_result) -> str:
...
def _s21_compact_json(value) -> str:
    try:
        return _s21_json.dumps(value, ensure_ascii=False, separators=(",", ":"))
    except Exception:
        return ""


def _s21_citation_key(ref) -> tuple:
    slices = tuple(
...
def _s21_dedup_citations(response):
...
def _s21_merge_citations(existing, new_refs):
    existing_list = list(existing or [])
    seen = {_s21_citation_key(ref) for ref in existing_list}
    ...
    for ref in new_refs:
        ...
        seen.add(key)
        merged.append(ref)
        if len(merged) >= _S21_MAX_TOTAL_CITATIONS:
            break
    return merged


async def _s21_search_gap(search_query: str, provider_name: str = "parallel"):
    from harnyx_miner_sdk.api import search_web as _s21_search_web

    try:
        payload = await _s21_search_web(
            search_query[:300],
            provider=provider_name,
            num=4,
            timeout=_S21_SEARCH_TIMEOUT_S,
        )
    except Exception:
        payload = None
    if payload is None:
    ...
    if not items:
        return None
    return {"receipt_id": receipt, "provider": provider_name, "items": items}


async def _s21_search_gap_any(search_query: str):
    for provider_name in ("parallel", "desearch"):
        hit = await _s21_search_gap(search_query, provider_name)
        if hit:
            return hit
    return None


async def _s21_dual_search(search_query: str):
    packs = await _s21_asyncio.gather(
        _s21_search_gap(search_query, "parallel"),
        _s21_search_gap(search_query, "desearch"),
        return_exceptions=True,
    )
    out = []
    for pack in packs:
        if isinstance(pack, Exception) or not pack:
            out.append(None)
        else:
            out.append(pack)
    return out[0], out[1]
...
def _s21_build_refs(receipt_id: str, evidence_items: list, indices) -> list:
    from harnyx_miner_sdk.query import CitationRef as _s21_citation_ref
    from harnyx_miner_sdk.query import CitationSlice as _s21_citation_slice
    ...
    for raw_idx in (indices or []):
        ...
        if end <= 0:
            continue
        try:
            refs.append(_s21_citation_ref(
                receipt_id=receipt_id,
                result_id=item["result_id"],
                slices=[_s21_citation_slice(start=0, end=end)],
            ))
        except Exception:
            continue
        if len(refs) >= _S21_MAX_NEW_CITATIONS_PER_GAP:
            break
    return refs


def _s21_evidence_block(items: list, label: str = "") -> str:
    prefix = f"{label}\n" if label else ""
    body = "\n".join(
...
def _s21_merge_output_patch(current, patch):
...
async def _s21_extract_sides(question: str) -> dict | None:
    from harnyx_miner_sdk.api import llm_chat as _s21_llm_chat

    try:
        result = await _s21_llm_chat(
            provider="openrouter",
            model=_S21_MODEL,
            messages=[
                {"role": "system", "content": _S21_SIDE_EXTRACT_SYSTEM_PROMPT},
                {"role": "user", "content": f"Question:\n{question}"},
            ],
            tools=None,
            temperature=0.0,
            max_output_tokens=500,
            timeout=_S21_SIDE_EXTRACT_TIMEOUT_S,
            thinking={"enabled": False},
        )
    except Exception:
        return None
    try:
        parsed = _s21_json.loads(_s21_strip_json_fences(_s21_chat_text(result)))
    except Exception:
        return None
    if not isinstance(parsed, dict) or not parsed.get("is_multi_entity"):
        return None
    raw_sides = parsed.get("sides")
    ...
    for item in raw_sides:
        ...
        if name and need:
            sides.append({"name": name, "need": need})
        if len(sides) >= _S21_MAX_SIDES:
            break
    if len(sides) < 2:
        return None
    relation = str(parsed.get("relation_or_metric") or "").strip()
    conclusion = str(parsed.get("conclusion_requirement") or "").strip()
    if not relation:
        relation = "shared comparison or synthesis relation"
    if not conclusion:
        conclusion = "state the comparative or synthesis conclusion across the named sides"
    return {"sides": sides, "relation_or_metric": relation, "conclusion_requirement": conclusion}


async def _s21_check_side_coverage(plan: dict, content_repr: str, is_structured: bool) -> dict | None:
    from harnyx_miner_sdk.api import llm_chat as _s21_llm_chat

    sides_block = "\n".join(
        f"{idx}. side={s['name']} need={s['need']}" for idx, s in enumerate(plan["sides"])
    )
    label = "Current JSON answer" if is_structured else "Current answer text"
    user_content = (
        f"Relation/metric:\n{plan['relation_or_metric']}\n\n"
        f"Conclusion requirement:\n{plan['conclusion_requirement']}\n\n"
        f"Sides:\n{sides_block}\n\n"
        f"{label}:\n{content_repr[:12000]}"
    )
    try:
        result = await _s21_llm_chat(
            ...
            model=_S21_MODEL,
            messages=[
                {"role": "system", "content": _S21_SIDE_COVERAGE_SYSTEM_PROMPT},
                {"role": "user", "content": user_content},
            ],
            tools=None,
            temperature=0.0,
            max_output_tokens=600,
            timeout=_S21_SIDE_COVERAGE_TIMEOUT_S,
            thinking={"enabled": False},
        )
    except Exception:
        return None
...
async def _s21_patch_side_text(
    question: str,
    answer: str,
    target_label: str,
    gap_query: str,
    ...
    try:
        result = await _s21_llm_chat(
            ...
            model=_S21_MODEL,
            messages=[
                {"role": "system", "content": _S21_PATCH_SIDE_TEXT_SYSTEM_PROMPT},
                {"role": "user", "content": prompt},
            ],
            tools=None,
            temperature=0.1,
            max_output_tokens=1400,
            timeout=_S21_PATCH_TIMEOUT_S,
            thinking={"enabled": False},
        )
    except Exception:
        return ""
    return _s21_chat_text(result)[:79000].strip()


async def _s21_patch_side_output(
    question: str,
    schema_compact: str,
    ...
    try:
        result = await _s21_llm_chat(
            ...
            model=_S21_MODEL,
            messages=[
                {"role": "system", "content": _S21_PATCH_SIDE_OUTPUT_SYSTEM_PROMPT},
                {"role": "user", "content": prompt},
            ],
            tools=None,
            temperature=0.0,
...
async def _s21_side_coverage_gate(_s21_query, _s21_response):
    question = (getattr(_s21_query, "text", None) or "").strip()
    if not question:
        return _s21_response

    output_schema = getattr(_s21_query, "output_schema", None)
    is_structured = getattr(_s21_response, "output", None) is not None

    if is_structured:
        current_output = getattr(_s21_response, "output")
        if not isinstance(current_output, dict):
            return _s21_response
        content_repr = _s21_compact_json(current_output)
        answer_text = None
    else:
        answer_text = (getattr(_s21_response, "text", None) or "").strip()
        if not answer_text:
            return _s21_response
    ...
    plan = await _s21_extract_sides(question)
    if not plan:
        return _s21_response

    coverage = await _s21_check_side_coverage(plan, content_repr, is_structured)
    if not coverage:
        return _s21_response

    gaps = []
    raw_sides = coverage.get("sides")
    if isinstance(raw_sides, list):
        for item in raw_sides:
            ...
            if 0 <= idx < len(plan["sides"]) and verdict in ("weak", "missing") and gap_query:
                side = plan["sides"][idx]
                gaps.append({
                    "kind": "side",
                    "priority": 0 if verdict == "missing" else 1,
                    "label": f"side={side['name']}; need={side['need']}",
                    "gap_query": gap_query,
                })
    conclusion = coverage.get("conclusion")
    if isinstance(conclusion, dict):
        verdict = str(conclusion.get("verdict") or "").strip().lower()
        gap_query_raw = conclusion.get("gap_query")
        gap_query = gap_query_raw.strip() if isinstance(gap_query_raw, str) else ""
        if verdict in ("weak", "missing") and gap_query:
            gaps.append({
                "kind": "conclusion",
                "priority": 0 if verdict == "missing" else 1,
                "label": f"conclusion: {plan['conclusion_requirement']}",
                "gap_query": gap_query,
            })
    gaps.sort(key=lambda g: g["priority"])
    gaps = gaps[:_S21_MAX_SIDE_GAPS]
    if not gaps:
        return _s21_response

    search_results = await _s21_asyncio.gather(
        *[_s21_search_gap_any(g["gap_query"]) for g in gaps],
        return_exceptions=True,
    )
    ...
    schema_compact = _s21_compact_json(output_schema)[:4000] if output_schema is not None else ""
    all_new_refs = []
    changed = False
    ...
    for gap, search_result in per_gap:
        ...
        evidence_block = _s21_evidence_block(items)

        if is_structured:
            patch_result = await _s21_patch_side_output(
                question,
                schema_compact,
                _s21_compact_json(running_output),
                gap["label"],
                gap["gap_query"],
                evidence_block,
            )
            ...
            merged = _s21_merge_output_patch(running_output, patch) if isinstance(patch, dict) else None
            if merged is None:
                continue
            running_output = merged
            changed = True
            used_indices = patch_result.get("used_indices")
            refs = _s21_build_refs(
                receipt_id,
                items,
            ...
            patched = await _s21_patch_side_text(
                question,
                running_text,
                gap["label"],
                gap["gap_query"],
            ...
            changed = True
            refs = _s21_build_refs(receipt_id, items, [0, 1])
            all_new_refs.extend(refs)

    if not changed:
        return _s21_response

    merged_citations = _s21_merge_citations(getattr(_s21_response, "citations", None), all_new_refs)
    try:
        if is_structured:
            return _s21_response.model_copy(update={"output": running_output, "citations": merged_citations})
        return _s21_response.model_copy(update={"text": running_text, "citations": merged_citations})
    except Exception:
        return _s21_response


# --------------------------------------------------------------------
# Stage B -- cross-source disagreement reconciliation gate
# --------------------------------------------------------------------

async def _s21_extract_claims(content_repr: str, is_structured: bool) -> list:
    from harnyx_miner_sdk.api import llm_chat as _s21_llm_chat

    label = "JSON answer" if is_structured else "Answer text"
    try:
        result = await _s21_llm_chat(
            provider="openrouter",
            model=_S21_MODEL,
            messages=[
                {"role": "system", "content": _S21_CLAIM_EXTRACT_SYSTEM_PROMPT},
                {"role": "user", "content": f"{label}:\n{content_repr[:12000]}"},
            ],
            tools=None,
            temperature=0.0,
            max_output_tokens=500,
            timeout=_S21_CLAIM_EXTRACT_TIMEOUT_S,
            thinking={"enabled": False},
        )
    except Exception:
        return []
    ...
    for item in raw:
        ...
        probe = str(item.get("probe_query") or "").strip()
        if claim and probe:
            out.append({"claim": claim, "probe_query": probe})
        if len(out) >= _S21_MAX_CLAIMS:
            break
    return out


async def _s21_classify_disagreement(claim: str, pack_a, pack_b) -> dict | None:
    from harnyx_miner_sdk.api import llm_chat as _s21_llm_chat

    block_a = _s21_evidence_block(pack_a["items"], "provider_a=") if pack_a else "provider_a=(empty)"
    block_b = _s21_evidence_block(pack_b["items"], "provider_b=") if pack_b else "provider_b=(empty)"
    prompt = (
        f"Claim:\n{claim}\n\n"
        f"Evidence pack A:\n{block_a}\n\n"
        f"Evidence pack B:\n{block_b}"
    ...
    try:
        result = await _s21_llm_chat(
            ...
            model=_S21_MODEL,
            messages=[
                {"role": "system", "content": _S21_DISAGREE_SYSTEM_PROMPT},
                {"role": "user", "content": prompt},
            ],
            tools=None,
            temperature=0.0,
            max_output_tokens=400,
            timeout=_S21_DISAGREE_TIMEOUT_S,
            thinking={"enabled": False},
        )
    except Exception:
        return None
    ...
    if verdict not in ("agreed", "disputed", "single_source", "unverifiable"):
        return None
    return parsed


async def _s21_patch_dispute_text(
    question: str,
    answer: str,
    claim: str,
    classification: dict,
    ...
    prompt = (
        ...
        f"Current answer:\n{answer[:12000]}\n\n"
        f"Disputed claim:\n{claim}\n\n"
        f"Classifier verdict:\n{_s21_compact_json(classification)}\n\n"
        f"Dual-provider evidence:\n{evidence_block}"
    )
    try:
        result = await _s21_llm_chat(
            provider="openrouter",
            model=_S21_MODEL,
            messages=[
                {"role": "system", "content": _S21_PATCH_DISPUTE_TEXT_SYSTEM_PROMPT},
                {"role": "user", "content": prompt},
            ],
            tools=None,
            temperature=0.1,
...
async def _s21_patch_dispute_output(
    question: str,
    schema_compact: str,
    current_output_compact: str,
    claim: str,
    ...
    try:
        result = await _s21_llm_chat(
            ...
            model=_S21_MODEL,
            messages=[
                {"role": "system", "content": _S21_PATCH_DISPUTE_OUTPUT_SYSTEM_PROMPT},
                {"role": "user", "content": prompt},
            ],
            tools=None,
            temperature=0.0,
...
async def _s21_disagreement_gate(_s21_query, _s21_response):
    question = (getattr(_s21_query, "text", None) or "").strip()
    if not question:
        return _s21_response
    ...
    claims = await _s21_extract_claims(content_repr, is_structured)
    if not claims:
        return _s21_response

    # Dual-retrieve first few claims; patch only disputed (priority) then weak single_source.
    dual_results = await _s21_asyncio.gather(
        *[_s21_dual_search(c["probe_query"]) for c in claims],
        return_exceptions=True,
    )

    disputes = []
    for claim, dual in zip(claims, dual_results):
        if isinstance(dual, Exception) or not dual:
            continue
        pack_a, pack_b = dual
        if not pack_a and not pack_b:
            continue
        classification = await _s21_classify_disagreement(claim["claim"], pack_a, pack_b)
        if not classification:
            continue
        verdict = classification.get("verdict")
        if verdict == "disputed":
            disputes.append((0, claim, classification, pack_a, pack_b))
        elif verdict == "single_source":
            disputes.append((1, claim, classification, pack_a, pack_b))
    disputes.sort(key=lambda row: row[0])
    disputes = disputes[:_S21_MAX_DISPUTE_PATCHES]
    if not disputes:
        return _s21_response
    ...
    for _prio, claim, classification, pack_a, pack_b in disputes:
        blocks = []
        if pack_a:
            blocks.append(_s21_evidence_block(pack_a["items"], "provider_a="))
        if pack_b:
            blocks.append(_s21_evidence_block(pack_b["items"], "provider_b="))
        evidence_block = "\n\n".join(blocks)

        if is_structured:
            patch_result = await _s21_patch_dispute_output(
                question,
                schema_compact,
                _s21_compact_json(running_output),
                claim["claim"],
            ...
            patched = await _s21_patch_dispute_text(
                question,
                running_text,
                claim["claim"],
                classification,
        ...
        used_a = classification.get("used_a") if isinstance(classification.get("used_a"), list) else [0]
        used_b = classification.get("used_b") if isinstance(classification.get("used_b"), list) else [0]
        if pack_a:
            all_new_refs.extend(_s21_build_refs(pack_a["receipt_id"], pack_a["items"], used_a or [0]))
        if pack_b:
            all_new_refs.extend(_s21_build_refs(pack_b["receipt_id"], pack_b["items"], used_b or [0]))
...
async def _s21_run_stages(_s21_query, _s21_response):
    _s21_response = _s21_dedup_citations(_s21_response)
    try:
        _s21_response = await _s21_side_coverage_gate(_s21_query, _s21_response)
    except Exception:
        pass
    try:
        _s21_response = await _s21_disagreement_gate(_s21_query, _s21_response)
    except Exception:
        pass
    return _s21_response


async def _s21_finalize(_s21_query, _s21_response, _s21_t0: float):
    """Bounded multi-entity side-coverage + cross-source disagreement pass."""
    if _s21_response is None:
        return _s21_response
    if getattr(_s21_response, "text", None) in (None, "") and getattr(_s21_response, "output", None) is None:
        return _s21_response
    elapsed = _s21_monotonic() - _s21_t0
    if elapsed >= _S21_HARD_BUDGET_GATE_S:
        return _s21_dedup_citations(_s21_response)
    window = min(_S21_MAX_WINDOW_S, max(_S21_MIN_WINDOW_S, 280.0 - elapsed))
    try:
        return await _s21_asyncio.wait_for(
            _s21_run_stages(_s21_query, _s21_response),
            timeout=window,
        )
    except Exception:
        return _s21_dedup_citations(_s21_response)
...
async def query(query: Query) -> Response:
    _s21_t0 = _s21_monotonic()
    _s21_resp = await _s21_base_query(query)
    try:
        return await _s21_finalize(query, _s21_resp, _s21_t0)
    except Exception:
        return _s21_resp
