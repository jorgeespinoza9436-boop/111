"""SN67 Harnyx miner — v25 deep-research pipeline. [slot 54 build 2026-08-01T15:00:00+00:00]"""
...
def _build_numeric_audit_messages(
    *, question: str, draft: str, accepted_packets: tuple[AcceptedEvidence, ...],
    ...
    system_content = (
        ...
        "4b. SUPERLATIVE POOL: if the question asks which item has the lowest/highest/most/fewest/best/"
        "worst value of a property and the draft names a winner without showing the candidate pool it "
        "was compared against, add a compact pool audit from the evidence — each plausible candidate with "
        "a one-line cited qualify/exclude verdict — then confirm the extremum among qualifiers (or note "
        "the sole survivor holds it by default).\n"
        "4c. DE-DUPLICATE: if the draft repeats the same fact across multiple sentences (one per packet), "
        "collapse them to a single statement of that fact with the supporting citations pooled on that "
        "one sentence. Do not drop any distinct fact or citation; only merge verbatim restatements.\n"
...
def _build_final_answer_messages(
    *, question: str, accepted_packets: tuple[AcceptedEvidence, ...],
    ...
    system_content = (
        ...
        "SUPERLATIVE-POOL RULE: when the question asks which item has the lowest / highest / "
        "most / fewest / best / worst / earliest / latest value of some property — a "
        "superlative to be resolved by comparison — and does NOT itself list the candidates, "
        "CONSTRUCT the candidate pool from the evidence before answering: name every plausible "
        "candidate the evidence surfaced, give each a one-line cited verdict (qualifies with "
        "value X [n], or excluded because it fails constraint Y [n]), then state the extremum "
        "among the qualifiers. Do NOT assert the answer is the lowest/highest without showing "
        "the compared pool; a bare winner with no pool audit loses to an answer that proves "
        "completeness. If only one candidate survives the constraints, say so — it holds the "
        "superlative by default.\n\n"
        "NON-REDUNDANCY RULE: state each distinct fact ONCE. When several packets support the "
        "same fact, cite them together on that one sentence ([1][3][7]); never repeat the same "
        "sentence or fact once per packet. Redundant restatement dilutes the answer and buries "
        "the result.\n\n"
...
# slot: 54 A9 2026-08-01T15:00:00+00:00
