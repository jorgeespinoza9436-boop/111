...
helm :MeridianHelm ,
    ...
    and not helm .retained_evidence 
    ...
    if helm .retained_evidence :
        ...
        +json .dumps (
        list (helm .retained_evidence .values ()),
        ensure_ascii =False ,
        indent =2 ,
        )
        )
...
def _inquiry2_headway_thumbmark (helm :MeridianHelm )->tuple [Any ,...]:
    ...
    tuple (sorted (helm .retained_evidence )),
    helm .research_state ,
    helm .audit_gap ,
    )
...
final_origin_cuts :dict [str ,list [CitationSlice ]],
    ...
    and str (item_m ["source_ref"]).strip ("[]")not in helm .retained_evidence 
    ]
    ...
    held_sheaf =[
    item_m 
    for item_m in helm .retained_evidence .values ()
    if str (item_m ["source_ref"])not in precise_badges 
    ]
