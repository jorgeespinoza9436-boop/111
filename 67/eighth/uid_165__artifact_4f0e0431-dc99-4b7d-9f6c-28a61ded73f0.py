"""SN67 Harnyx miner — v25 deep-research pipeline. [slot 01 build 2026-07-29T09:26:04+00:00]"""
...
RESEARCH_SYNTH_RESERVE_SECONDS         = 100.0
COMPACT_EVIDENCE_TEXT_CHARS            = 420
COMPACT_CONTEXT_TEXT_CHARS             = 160
...
async def _answer_question(question: str, *, state: ResearchRunState, deadline: float) -> Response:
    research_deadline = deadline - RESEARCH_SYNTH_RESERVE_SECONDS
    search_beam = await _run_lite_search_beam(question=question, state=state, deadline=research_deadline)
    ...
    gate_result = await _admit_evidence_from_candidate_beam(
        ...
        candidates=candidates, state=state, deadline=research_deadline,
    )
    ...
    if enumerated_entities and perf_counter() < augmentation_deadline:
        ...
        if missing_entities:
            gap_gate = await _augment_missing_entity_evidence(
                ...
                existing_observations=accepted_observations, state=state, deadline=research_deadline,
            )
    ...
    if (
        ...
        and research_deadline - perf_counter() >= ROLE_GAP_MIN_REMAINING_SECONDS
    ):
        ...
        if role_gap_gate.accepted_packets:
            ...
            coverage_state = _fallback_coverage_state(contract=research_contract, observations=accepted_observations)
            coverage = _coverage_from_coverage_state(coverage_state, accepted_observations)
    pool_plan = BoundedPoolPlan()
    if (
        perf_counter() < augmentation_deadline
        and research_deadline - perf_counter() >= POOL_SWEEP_MIN_REMAINING_SECONDS
    ):
        pool_plan = await _detect_bounded_pool(
            question=question, existing_packets=accepted_packets, state=state, deadline=research_deadline,
        )
    ...
    final_answer = await _synthesize_final_answer(
        ...
        deadline=deadline, pool_plan=pool_plan,
    )
    final_answer = await _audit_and_repair_answer(
        question=question, draft=final_answer,
        accepted_packets=accepted_packets, deadline=deadline, pool_plan=pool_plan,
    )
...
async def _synthesize_final_answer(
    *, question: str, accepted_packets: tuple[AcceptedEvidence, ...],
    ...
    pool_plan: BoundedPoolPlan | None = None,
) -> str:
    messages = _build_final_answer_messages(
        question=question, accepted_packets=accepted_packets,
        accepted_observations=accepted_observations, coverage=coverage,
        unvetted_evidence=unvetted_evidence, pool_plan=pool_plan,
    )
    compact_messages = _build_final_answer_messages(
        question=question, accepted_packets=accepted_packets,
        accepted_observations=accepted_observations, coverage=coverage,
        unvetted_evidence=unvetted_evidence, pool_plan=pool_plan, compact=True,
    )
    ...
    if answer is None and _budget_matched_synthesis_plan(deadline - perf_counter()) is not None:
        tasks.add(asyncio.ensure_future(_run_synthesis_call(
            messages=compact_messages, deadline=deadline,
...
def _set_completeness_contract(plan: BoundedPoolPlan | None) -> str:
    if plan is None or not plan.is_bounded_pool:
        return ""
    members = "; ".join(plan.pool_members) or "not enumerated during research — derive the pool from the evidence"
    criteria = "; ".join(plan.criteria) or "the constraints stated in the question"
    subject = plan.pool_subject or "the candidate pool named by the question"
    return (
        "\n\nSET-COMPLETENESS CONTRACT — this question ranges over a bounded candidate pool, so "
        "completeness is what is scored here, not prose quality.\n"
        f"Pool subject: {subject}\n"
        f"Members surfaced during research: {members}\n"
        f"Constraints to test: {criteria}\n"
        "Write to this contract:\n"
        "1. ROSTER FIRST: one line per member above — plus any further member the evidence "
        "surfaces — as `Name — qualifies — <value> [n]` or `Name — excluded — <value or reason> [n]`. "
        "Every line carries its own packet citation; keep each line under 15 words.\n"
        "2. PER-CONSTRAINT: for each constraint listed, state explicitly how the qualifying members "
        "meet it, citing the packet that supports that specific constraint. Never confirm several "
        "constraints in one aggregate claim.\n"
        "3. RESULT LAST: state the qualifying set — or the extremum among the qualifiers when the "
        "question asks for one — in a single sentence after the roster, never before it.\n"
        "4. COMMIT: do not write 'the only', 'the sole' or 'the single' unless every member above "
        "carries a verdict line. If the evidence settles part of the pool, still commit to the answer "
        "that evidence supports and name which members stay unverified — a partial, cited, committed "
        "roster scores far above a refusal to answer.\n"
        "5. LENGTH BUDGET: the whole answer stays under 250 words — the roster is a compact list, "
        "never prose paragraphs; skip preamble and methodology talk entirely.\n"
    )
...
def _build_numeric_audit_messages(
    *, question: str, draft: str, accepted_packets: tuple[AcceptedEvidence, ...],
    ...
    system_content += _set_completeness_contract(pool_plan)
    return [{"role": "system", "content": system_content}, {"role": "user", "content": user_content}]
...
def _build_final_answer_messages(
    *, question: str, accepted_packets: tuple[AcceptedEvidence, ...],
    ...
    unvetted_evidence: bool = False, pool_plan: BoundedPoolPlan | None = None,
    compact: bool = False,
) -> list[dict[str, str]]:
    text_cap = COMPACT_EVIDENCE_TEXT_CHARS if compact else None
    context_cap = COMPACT_CONTEXT_TEXT_CHARS if compact else None
    evidence_payload = [{"packet_number": i, "url": p.url, "title": p.title or "", "source_text_part": p.text_part, "source_text_range": [p.text_start, p.text_end], "accepted_source_text": p.source_text[:text_cap] if text_cap else p.source_text, "source_result_text": p.source_result_text[:context_cap] if context_cap else p.source_result_text} for i, p in enumerate(accepted_packets, start=1)]
...
# slot: 01 A11_xslim 2026-07-29T09:26:04+00:00
