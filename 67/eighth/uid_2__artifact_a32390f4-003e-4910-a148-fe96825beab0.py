"""SN67 Harnyx miner — v25 deep-research pipeline. [slot 55 build 2026-07-29T09:15:37+00:00]"""
...
HOP_TRIGGER_RE = re.compile(
    r"(?i)\b(?:who|whose|behind|passed away|died|death|the year"
    r"|the (?:person|man|woman|state|country|city|team|company) (?:who|that|which|where)"
    r"|(?:director|creator|founder|author|filmmaker|developer|architect) of"
    r"|that same year|the same year|in the year)\b"
)
...
async def _answer_question(question: str, *, state: ResearchRunState, deadline: float) -> Response:
    ...
    if (
        accepted_packets
        and perf_counter() < augmentation_deadline
        and deadline - perf_counter() >= HOP_MIN_REMAINING_SECONDS
    ):
        hop_plan = await _detect_second_hop(
            question=question, existing_packets=accepted_packets, state=state, deadline=deadline,
        )
        if hop_plan.followup_queries:
            hop_gate = await _augment_second_hop_evidence(
                question=question, plan=hop_plan, contract=research_contract,
                role_ledger=role_ledger, existing_packets=accepted_packets,
                existing_observations=accepted_observations, state=state, deadline=deadline,
            )
...
def _build_second_hop_plan_messages(
    *, question: str, existing_packets: tuple[AcceptedEvidence, ...],
) -> list[dict[str, str]]:
    evidence_brief = [
        {
            ...
            "url": packet.url,
            "accepted_source_text": packet.source_text[:HOP_EVIDENCE_BRIEF_TEXT_CHARS],
        }
        for index, packet in enumerate(existing_packets[:HOP_EVIDENCE_BRIEF_MAX_PACKETS], start=1)
    ]
    system_content = (
        "ROLE: dependency-hop planner for a research pipeline. Some questions chain on an "
        "INTERMEDIATE fact: an indirect reference (for example 'the year the director of film X "
        "passed away', 'the state where person Y was born') must first be resolved to a concrete "
        "value, and the facts the question actually asks about are defined in terms of that "
        "resolved value.\n\n"
        "Given the question and the evidence gathered so far: (1) decide whether the question "
        "contains such an indirect reference; (2) check whether the evidence RESOLVES it to a "
        "concrete value (a name, a year, a place); (3) check whether the evidence ALREADY CONTAINS "
        "the downstream facts the question asks about, stated against the resolved value. Only "
        "when a reference is resolved AND the downstream facts are missing, emit up to "
        f"{HOP_MAX_FOLLOWUP_QUERIES} follow-up search queries. Each query must hard-code the "
        "RESOLVED concrete value (never the indirect phrasing) and target one missing downstream "
        "fact (for example 'BLS state unemployment rates December 1998 seasonally adjusted', "
        "'1998 gubernatorial election results by state').\n\n"
        "A single-hop question, an unresolved reference, or already-complete evidence all mean no "
        "follow-up. Output ONLY a JSON object: {\"has_dependency_hop\": bool, "
        "\"resolved_facts\": [str, ...], \"followup_queries\": [str, ...]}. No prose."
    )
    user_content = (
        f"Question: {question}\n\n"
...
def _second_hop_plan_from_payload(payload: dict[str, object] | None) -> SecondHopPlan:
    ...
    return SecondHopPlan(
        followup_queries=tuple(queries[:HOP_MAX_FOLLOWUP_QUERIES]),
        resolved_facts=resolved,
    )


async def _detect_second_hop(
    *, question: str, existing_packets: tuple[AcceptedEvidence, ...],
    state: ResearchRunState, deadline: float,
) -> SecondHopPlan:
    ...
    payload = await _call_json_llm_with_retry(
        messages=messages, model=RESEARCH_PLAN_MODEL, temperature=PLANNING_TEMPERATURE,
        validate_payload=_second_hop_payload_validator(), state=state,
        stage="second_hop_detection",
    )
    return _second_hop_plan_from_payload(payload)


async def _augment_second_hop_evidence(
    *, question: str, plan: SecondHopPlan, contract: ResearchContract,
    role_ledger: tuple[ResearchPlanRole, ...], existing_packets: tuple[AcceptedEvidence, ...],
    existing_observations: tuple[EvidenceObservation, ...], state: ResearchRunState,
    deadline: float,
) -> GateResult:
    ...
    for followup_query in plan.followup_queries[:HOP_MAX_FOLLOWUP_QUERIES]:
        ...
        try:
            response = await search_web(
                [followup_query], provider=_SEARCH_PROVIDER,
                num=HOP_SEARCH_RESULTS_PER_QUERY, timeout=LITE_SEARCH_TOOL_TIMEOUT_SECONDS,
            )
        except Exception:
            continue
        receipt_id = getattr(response, "receipt_id", "") or ""
        for result in tuple(getattr(response, "results", ()) or ()):
            ...
            counter += 1
            candidates.append(EvidenceCandidate(
                candidate_id=f"H{counter}",
                parent_candidate_id=f"H{counter}",
                slot_id=PRIMARY_SOURCE_SLOT_ID,
                slot_intent=_slot_intent_for_slot(PRIMARY_SOURCE_SLOT_ID),
                text_part="search_snippet", text_start=0, text_end=len(note),
                receipt_id=receipt_id,
                result_id=_string_value(getattr(result, "result_id", "")) or f"H{counter}",
                url=url, title=getattr(result, "title", None), source_text=note,
                query=followup_query, source_kind=SEARCH_SNIPPET_EVIDENCE_SOURCE_KIND,
            ))
    ...
    return await _run_observation_gate_once(
        ...
        existing_observations=existing_observations, contract=contract,
        retrieval_roles=role_ledger, candidates=tuple(candidates),
        model=EVIDENCE_GATE_MODEL, state=state, stage="second_hop_gate", lane="second_hop",
    )
...
# slot: 55 A12_hop 2026-07-29T09:15:37+00:00
