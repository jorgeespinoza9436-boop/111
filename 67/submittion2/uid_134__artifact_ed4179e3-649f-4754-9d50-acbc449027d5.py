...
# v15: wiki-scrape artifact phrases (raw Wikipedia page echoed as the answer)
_WIKI_JUNK = (
    "this article needs", "more citations", "additional citations", "unsourced material",
    "[edit]", "jump to navigation", "jump to search", "from wikipedia, the free encyclopedia",
    "this article is about", "citations for verification", "please help improve", "needs to be updated",
)


def _looks_csv_dump(a: str) -> bool:
    """v15: raw CSV/tabular dump like 'SUMLEV,REGION,DIVISION,STATE,NAME,ESTIMATESBASE2020,...' —
    a comma-separated line of field-CODE/number tokens (not a prose comma list like 'A, B, and C')."""
    first = a.split("\n", 1)[0][:400]
    fields = [f.strip() for f in first.split(",")]
    if len(fields) < 5:
        return False
    codeish = sum(1 for f in fields
                  if re.fullmatch(r"[A-Z][A-Z0-9_]{2,}", f) or re.fullmatch(r"-?\d[\d.,]*", f))
...
def _is_weak_final(answer: str) -> bool:
    ...
    if committed:
        return False
    # (1) markdown / scraped-page signatures (headers, link soup, chrome text)
    headers = len(re.findall(r"#{1,4}\s\S", a))
    links = a.count("](http") + a.count("[](")
    junk = (low.count("logo") + low.count("season summary") + low.count("[via ")
            + low.count("[about ") + low.count("skip to") + sum(low.count(w) for w in _WIKI_JUNK))
    if headers + links + junk >= 3:
        return True
    # (2) wiki-prose dump: any wiki chrome phrase near the start
    if any(w in low[:800] for w in _WIKI_JUNK):
        return True
    # (3) leading scraped artifact: image-link / table-pipe / bracket soup as the opening
    lead = a.lstrip()
    if lead[:1] in ("|",) or lead.startswith(("[](", "[icon", "![", "| ")):
        return True
    # (4) raw CSV / tabular data dump
