    ...
    def _compile(self):
        ...
        class EasyPath:
            ...
            def _compile(self):
                ...
                MAX_TURNS = 15
                AUDIT_EXTRA_TURNS = 2
                AUDIT_PATCH_MIN_S = 70.0
                AUDIT_ENTRY_S = AUDIT_PATCH_MIN_S + AUDIT_TIMEOUT_S + 8.0
                ...
                def _clamp_timeout(deadline: float, want: float, reserve: float=4.0, floor: float=4.0, grace: float=0.0) -> float:
                ...
                async def _do_search(query_text: str, deadline: float):
                    ...
                    for attempt, allow_repeat in ((query_text, False), (query_text, True), (_degrade_query(query_text), False)):
                        if not attempt.strip() or (attempt in fired and (not allow_repeat)):
                            continue
                        budget = _clamp_timeout(deadline, SEARCH_TIMEOUT_S, 3.0, floor=5.0, grace=NET_GRACE_S)
                        if budget <= 0.0:
                            break
                        fired.add(attempt)
                        try:
                ...
                async def _do_fetch(url: str, focus: str, question: str, deadline: float):
                    ...
                    payload = None
                    for _attempt in (0, 1):
                        budget = _clamp_timeout(deadline, FETCH_TIMEOUT_S, 3.0, floor=5.0, grace=NET_GRACE_S)
                        if budget <= 0.0:
                            break
                        try:
                ...
                async def _fetch_json(url: str, deadline: float):
                    ...
                    if cached is not None:
                        return cached
                    for _attempt in (0, 1):
                        budget = _clamp_timeout(deadline, _SEC_FETCH_TIMEOUT_S, 6.0, floor=6.0, grace=NET_GRACE_S)
                        if budget <= 0.0:
                            return None
                        try:
                            payload = await asyncio.wait_for(fetch_page(url, provider=SEARCH_PROVIDER, timeout=budget), timeout=budget + 4.0)
                ...
                async def _chat_simple(model: str, system: str, user: str, *, max_tokens: int, timeout: float, think: dict | None=None) -> str:
                    ...
                    payload = await asyncio.wait_for(llm_chat(provider=LLM_PROVIDER, model=model, messages=[{'role': 'system', 'content': system}, {'role': 'user', 'content': user}], temperature=0.15, max_output_tokens=max_tokens, timeout=timeout, thinking=think), timeout=timeout + SDK_GRACE_S)
                    _spend_note(payload)
                    return _llm_text(payload)

                async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool, force_tools: bool=False):
                    for model in LOOP_MODEL_CHAIN:
                        timeout = _clamp_timeout(deadline, TURN_TIMEOUT_S, 5.0, floor=5.0, grace=SDK_GRACE_S)
                        if timeout <= 5.0:
                            return None
                        try:
                            payload = await asyncio.wait_for(llm_chat(provider=LLM_PROVIDER, model=model, messages=messages, tools=LOOP_TOOLS if force_tools or not finish_only else None, tool_choice='auto' if force_tools or not finish_only else None, temperature=0.2, thinking={'enabled': True, 'effort': 'low'}, max_output_tokens=None, timeout=timeout), timeout=timeout + SDK_GRACE_S)
                            _spend_note(payload)
                            return payload
                        except Exception:
                            continue
                ...
                async def _knowledge_brief(question: str, deadline: float) -> tuple[str, str]:
                    ...
                    raw = ''
                    for model in LOOP_MODEL_CHAIN:
                        budget = _clamp_timeout(min(deadline, phase_end), BRIEF_TIMEOUT_S, 2.0, floor=12.0, grace=SDK_GRACE_S)
                        if budget <= 0.0:
                            break
                        try:
                ...
                async def _loop(question: str, brief: str, ledger: list, deadline: float, turn_cap: int, carry: list[dict] | None=None, allow_tools_in_wrapup: bool=False) -> tuple[str, list[dict]]:
                    ...
                    for turn in range(1, turn_cap + 1):
                        ...
                        for idx, result in enumerate(results):
                            ...
                            messages.append({'role': 'tool', 'tool_call_id': call_ids[idx], 'content': body})
                        for idx in range(len(run_calls), len(call_ids)):
                            messages.append({'role': 'tool', 'tool_call_id': call_ids[idx], 'content': '# skipped: per-turn tool budget reached — re-issue next turn if still needed'})
                ...
                async def _audit_patch(question: str, answer: str, messages: list[dict], ledger: list, deadline: float) -> str:
                    ...
                    budget = _clamp_timeout(deadline, AUDIT_TIMEOUT_S, reserve=AUDIT_PATCH_MIN_S, floor=8.0, grace=SDK_GRACE_S)
                    if budget <= 0.0:
                        return answer
                    try:
                        raw = await _chat_simple(AUDIT_MODEL, 'Strict completeness auditor. JSON only.', probe, max_tokens=2200, timeout=budget)
                ...
                async def _digest_write_once(model: str, convo: list, budget: float) -> str:
                    payload = await asyncio.wait_for(llm_chat(provider=LLM_PROVIDER, model=model, messages=convo, temperature=0.15, max_output_tokens=2600, timeout=budget, thinking=_least_think(model)), timeout=budget + SDK_GRACE_S)
                    _spend_note(payload)
                    return _llm_text(payload)
                ...
                async def _knowledge_resort(question: str, deadline: float) -> str:
                    budget = _clamp_timeout(deadline, 45.0, reserve=2.0, floor=8.0, grace=SDK_GRACE_S)
                    if budget <= 0.0:
                        return ''
                    try:
                ...
                async def _solve(query: Query, question: str) -> Response:
                    ...
                    try:
                        ...
                        answer = ''
                    try:
                        if _is_usable_answer(answer) and _time_left(deadline) > AUDIT_ENTRY_S and (_spend_left() >= AUDIT_MIN_USD):
...
"""
    ...
    lose early [n]s to a truncated message window."""
        ...
        async def _rescue(question: str, answer: str, draft: str, ledger: EvidenceLedger, deadline: float) -> str:
...
_TAG_C26FD71C="c26fd71cdd0f42d29e472272948ceac1"
import logging as _tag_logging_c26fd71c
_tag_logging_c26fd71c.getLogger("miner.tag").debug("tag=%s", _TAG_C26FD71C)
