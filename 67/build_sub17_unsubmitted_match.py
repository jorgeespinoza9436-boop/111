#!/usr/bin/env python3
"""Randomly match sub17-unsubmitted agents to remaining registered SN67 hotkeys."""
...
from enrich_history_providers import enrich_entry, extract_providers

AGENT_DIR = Path("/root/turtle/111/67/sub17-unsubmitted")
SUBMITTED_DIR = Path("/root/turtle/111/67/sub17-submitted")
WALLET_NAME = "money"
NETUID = 67
NETWORK = "finney"
RANDOM_SEED = 20260806929  # unsubmitted-only agents
EXCLUDED_AGENT_UIDS = frozenset({"uid_35", "uid_37", "uid_77"})
...
def _eligible_unsubmitted_agents() -> list[Path]:
    """Only agents living in sub17-unsubmitted, excluding uid_35/37/77 and
    any files that already live in sub17-submitted (fill-in copies)."""
    submitted_names = {p.name for p in SUBMITTED_DIR.glob("uid_*.py")}
    agents = []
    for path in sorted(AGENT_DIR.glob("uid_*.py")):
        if _is_excluded_agent(path.name):
            continue
        if path.name in submitted_names:
            continue
        agents.append(path)
    return agents
...
def main() -> None:
    ...
    agents = _eligible_unsubmitted_agents()
    if not agents:
    ...
    leftover_hotkeys = sorted(set(h["hotkey_name"] for h in available) - {e["hotkey_name"] for e in entries})
    ...
    history = {
        ...
        "skipped_submitted_copies": sorted(
            p.name
            for p in AGENT_DIR.glob("uid_*.py")
            if p.name in {q.name for q in SUBMITTED_DIR.glob("uid_*.py")}
        ),
        "entries": entries,
    }
    ...
    submit_all.write_text(
        f"""#!/usr/bin/env bash
set -euo pipefail
# Submit matched sub17-unsubmitted agents in parallel.
# Usage: bash /root/turtle/111/67/sub17-unsubmitted/submit_all.sh [PARALLEL_JOBS]
ROOT="/root/turtle/harnyx"
JOBS_FILE="/root/turtle/111/67/sub17-unsubmitted/submit_jobs.txt"
...
""",
    ...
    (AGENT_DIR / "SUBMIT_COMMAND.txt").write_text(
        f"""# Submit sub17-unsubmitted agents only ({match_count} of {len(registered)} registered hotkeys)
# Excluded agents: {", ".join(sorted(EXCLUDED_AGENT_UIDS))}
# Skipped submitted copies / no top-up from submittion17
# History: {stamped}
# Map:     {AGENT_DIR / "hotkey_map.json"}
# Seed:    {RANDOM_SEED}

cd /root/turtle/harnyx
bash /root/turtle/111/67/sub17-unsubmitted/submit_all.sh {match_count}
""",
        encoding="utf-8",
    )

    print(f"Matched {len(entries)} pairs (seed={RANDOM_SEED})")
    print(f"Source: sub17-unsubmitted only; excluded={sorted(EXCLUDED_AGENT_UIDS)}")
