...
v34.1 STRUCTURAL HARDENING (behaviour-preserving; no prompt/logic changes):
  - PER-QUERY STATE RESET. The spend meter and the fired-query set live in one
    `_RUN` dict that is cleared at the top of `_solve`. Previously `_SPEND`
    persisted across queries in a warm worker, so one cheap-budget task could
    permanently disable the brief/audit/verify phases for every later task.
  - DEADLINE-AWARE TOOLS. `_do_search` (3 attempts) and `_do_fetch` (2 attempts)
    were deadline-blind: worst case 54s / 32s of network with no view of the
    wall. Both now take the deadline, clamp each attempt to the time actually
    left, and stop retrying when the tail is too short to use the result.
  - TRANSCRIPT SURVIVES A TURN FAULT. One bad payload shape inside `_loop` used
    to raise out of the whole loop, so `_solve` lost `messages` entirely and the
    audit / verify / cross-check phases were skipped. Each turn body is now
    fault-isolated: a fault ends the loop but returns the transcript gathered
    so far. `_solve` also refuses to carry an EMPTY transcript into a patch loop
    (that produced a system-prompt-less `_loop` call).
  - NO HIGHER-ORDER CALLABLES. The three `sort(key=lambda …)` sites are replaced
    with precomputed sort tuples, so the module contains no lambda, no dynamic
    dispatch, no `getattr` with a computed name, no dunder attribute access, and
    imports only asyncio / json / re / time + the SDK.
  - BOUNDED CPU + MEMORY. `_best_windows` scans at most `_MAX_SCAN_CHARS` of a
    fetched note (a multi-MB page used to block the event loop), the SEC JSON
    cache is size-capped, and the repetition detector uses a single tally pass
    instead of an O(n^2) count-per-line scan.
  - NO DUPLICATE RETRIEVAL. The roster pre-pass records the queries it fired so
    the deterministic pre-seed skips exact repeats instead of paying for the
    same search twice and diluting the ledger with duplicate rows.
Providers are unchanged and openrouter-only: there is NO ai_gateway call path.
"""
...
AUDIT_MIN_USD = 0.05
# ── per-query run state ───────────────────────────────────────────────────────
# A worker process serves MANY queries. Module-level mutable state therefore
# leaks between tasks: the old `_SPEND` global kept the LAST task's remaining
# USD, so one task that ran the budget down left every later task believing it
# was broke — brief, audit, verify-and-repair and both guards are all gated on
# `_spend_left()`, so they silently never ran again. `_solve` clears this dict
# at the top of every query; `_spend_left()` still degrades to "assume funded"
# when nothing has been observed yet.
_RUN: dict = {"spend_left": None, "fired_queries": set()}


def _reset_run_state() -> None:
    _RUN["spend_left"] = None
    _RUN["fired_queries"] = set()
...
def _spend_note(payload) -> None:
    ...
    if isinstance(left, (int, float)):
        _RUN["spend_left"] = float(left)


def _spend_left() -> float:
    left = _RUN["spend_left"]
    if isinstance(left, (int, float)):
        return float(left)
    return 1.0


def _mark_fired(query_text: str) -> None:
    """Record a planning-stage query so a later planning stage does not repeat
    it verbatim (the roster pre-pass and the pre-seed overlap on set questions)."""
    key = " ".join((query_text or "").split()).casefold()
    if key:
        fired = _RUN["fired_queries"]
        if isinstance(fired, set) and len(fired) < 64:
            fired.add(key)


def _already_fired(query_text: str) -> bool:
    key = " ".join((query_text or "").split()).casefold()
    fired = _RUN["fired_queries"]
    return bool(key) and isinstance(fired, set) and key in fired
...
_MAX_SCAN_CHARS = 400_000   # v34.1: window-scan ceiling (see _best_windows)
...
def _best_windows(note: str, terms: set[str], width: int,
    ...
    if n <= width:
        return [(0, n)]
    # v34.1: bound the CPU. A multi-MB scrape at step=1200 with ~30 terms ran
    # tens of thousands of substring scans on the event-loop thread, stalling
    # every concurrent tool call in the same turn. Scanning the first
    # _MAX_SCAN_CHARS is deterministic and covers any realistic article/filing;
    # spans still index the ORIGINAL note, so citations stay exact.
    scan_n = min(n, _MAX_SCAN_CHARS)
    step = max(600, width // 3)
    low = note[:scan_n].lower()  # lower() preserves length (casefold can change it)
    # sort tuples are built pre-negated so a plain .sort() gives "highest
    # density first, earliest position breaking ties" with NO key= callable.
...
def _attempt_budget(deadline, nominal: float, floor: float) -> float:
    """Time this attempt may take. Returns 0.0 when the attempt must be skipped.

    v34.1: the retry ladders below were deadline-blind — three search attempts
    could burn 54s and two fetch attempts 32s with no view of the wall, and the
    result of a retry that lands after WRAPUP is unusable anyway. A caller that
    passes no deadline keeps the old fixed-timeout behaviour exactly."""
    if deadline is None:
        return nominal
    left = deadline - monotonic()
    if left <= MIN_TAIL_S + floor:
        return 0.0
    return max(floor, min(nominal, left - MIN_TAIL_S))
...
async def _do_search(query_text: str, ledger: EvidenceLedger, deadline=None):
    ...
    for attempt, allow_repeat in ((query_text, False), (query_text, True),
        ...
        if not attempt.strip() or (attempt in fired and not allow_repeat):
            continue
        budget = _attempt_budget(deadline, SEARCH_TIMEOUT_S, 6.0)
        if budget <= 0.0:
            break          # no time left for this attempt to be worth anything
        fired.add(attempt)
        try:
...
async def _do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger,
    ...
    for _attempt in (0, 1):  # one retry: crawls intermittently return empty
        budget = _attempt_budget(deadline, FETCH_TIMEOUT_S, 6.0)
        if budget <= 0.0:
            break
        try:
            payload = await asyncio.wait_for(
...
_SEC_CACHE_MAX = 8              # v34.1: bound process-lifetime memory
...
async def _fetch_json(url: str, deadline: float):
    ...
    for _attempt in (0, 1):   # large-JSON crawls intermittently return empty
        ...
        if isinstance(obj, dict):
            # v34.1: the cache is process-lifetime state and the ticker index
            # alone is ~10MB. Cap it so a long-lived worker cannot grow without
            # bound across hundreds of queries; the hot entries are re-fetched
...
async def _knowledge_brief(question: str, deadline=None) -> tuple[str, str]:
    ...
    # v34.1: both lanes at a flat 50s could spend 100s of a 260s wall before the
    # first search ever fires. Cap each lane at a fraction of the time actually
    # left; at the normal entry point (~250s remaining) this evaluates to the
    # unchanged 50s, so the measured brief behaviour is preserved exactly.
    def _brief_budget() -> float:
        if deadline is None:
            return BRIEF_TIMEOUT_S
        return max(10.0, min(BRIEF_TIMEOUT_S, (deadline - monotonic()) * 0.22))

    raw = ""
    try:
        raw = await _chat_simple(LLM_LANE_A, LOOP_MODEL_A, system, user,
                                 max_tokens=2400, timeout=_brief_budget(),
...
async def _preseed(question: str, set_question: bool, ledger: EvidenceLedger,
    ...
    for seed in seeds:
        if (deadline - monotonic()) < 30.0:
            break
        # v34.1: on a set question the roster pre-pass has ALREADY fired
        # "list of <subject>"-shaped queries. Re-running an identical query
        # costs a full search timeout and appends duplicate ledger rows that
        # split the citation weight of one source across two [n].
        if _already_fired(seed):
            continue
        _mark_fired(seed)
        try:
...
async def _roster_prepass(question: str, ledger: EvidenceLedger,
    ...
    for qy in queries:
        _mark_fired(qy)   # the pre-seed skips anything already fired here
...
async def _loop(question: str, brief: str, ledger: EvidenceLedger,
    ...
    for turn in range(1, turn_cap + 1):
        ...
        # v34.1 FAULT ISOLATION. One unexpected payload shape used to raise out
        # of `_loop` entirely, so `_solve` caught it, lost `messages`, and every
        # later phase (audit, claim-repair, both value guards) was skipped even
        # though the ledger was full. A turn fault now ends the loop but RETURNS
        # what has been gathered. `turn_start` lets a half-written turn be rolled
        # back: an assistant tool_call message whose tool replies never got
        # appended would invalidate the transcript for the patch loops.
        ...
        try:
            ...
            # roll the faulted turn out of the transcript so what remains is a
            # valid conversation the patch loops can still be carried onto
            messages[turn_start:] = []
            break
...
async def _verify_and_repair(question: str, answer: str, messages: list[dict],
    ...
    if repair_queries and (deadline - monotonic()) > 72.0:
        ...
        tasks = [asyncio.ensure_future(_do_search(qy, ledger, deadline))
                 for qy in repair_queries]
        try:
            await asyncio.wait(tasks, timeout=budget)
        except Exception:
            pass
...
def _merge_spans(spans: list[tuple[int, int]]) -> list[tuple[int, int]]:
    ...
    # plain tuple ordering IS (start, end) ordering — no key= callable needed.
...
def _citations_for(answer: str, ledger: EvidenceLedger) -> list[CitationRef]:
    ...
    #    The rank is precomputed into a sort tuple so a plain .sort() suffices —
    #    identical ordering, no key= callable.
    ranked: list[tuple[int, int, int]] = []   # (has_value_rank, order, index)
    for idx, grp in enumerate(built):
        ranked.append((0 if grp["has_value"] else 1, grp["order"], idx))
    ranked.sort()
    refs: list[CitationRef] = []
    spent = 0
    for rank_row in ranked:
        grp = built[rank_row[2]]
...
def _is_degenerate_repetition(text: str) -> bool:
    ...
    body = text or ""
    # v34.1: one tally pass instead of count()-per-distinct-line. Identical
    # verdicts; a 60k-char roster answer no longer costs a quadratic rescan on
    # every floor check (and the floor is consulted on every turn).
    lines = [ln.strip().lower() for ln in body.split("\n") if len(ln.strip()) > 25]
    if len(lines) >= 3:
        line_tally: dict = {}
        for ln in lines:
            line_tally[ln] = line_tally.get(ln, 0) + 1
        for count in line_tally.values():
            if count >= 3:
                return True                      # same line repeated = a stall
        if len(line_tally) * 2 > len(lines):
    ...
    sent_tally: dict = {}
    for s in sents:
        sent_tally[s] = sent_tally.get(s, 0) + 1
    if len(sent_tally) * 2 <= len(sents):
        return True
    # or one sentence repeated 3+ times anywhere
    for count in sent_tally.values():
        if count >= 3:
            return True
    return False
...
# ── primary/official-source preference guard (agent_c, additive) ─────────────
# When the question NAMES or implies an authoritative source (official /
# government / primary filing / statistics agency / named report) and the
# answer's key claim is NOT yet cited to such a source, this runs ONE search
# biased toward the authoritative source and PREFERS its value only when an
# authoritative row (a .gov / primary-source domain) clearly states a different
# value present verbatim in that row. Non-destructive: it swaps at most one
# value grounded in a real authoritative ledger row, else keeps the answer.
...
async def _official_source_guard(question: str, answer: str,
                                 ledger: EvidenceLedger, deadline: float) -> str:
    # already citing an authoritative source? then nothing to prefer.
    for n in _cited_numbers(answer, len(ledger.rows)):
    ...
    try:
        out = await asyncio.wait_for(_do_search(query, ledger, deadline),
    ...
    if new_val not in (row.get("preview") or ""):
        return answer            # preference must be a real value in a real row
    return answer.replace(old_val, new_val, 1)
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # v34.1: a worker serves many queries in one process. Clearing the run state
    # HERE is what stops the previous task's spend reading (and its fired-query
    # set) from silently disabling this task's brief / audit / repair phases.
    _reset_run_state()
    try:
    ...
    # ── PHASE 3: synthesis (completeness audit + patch) ──────────────────────
    # v34.1: `messages` must be non-empty. A faulted loop used to hand back [],
    # and a patch loop carried onto an EMPTY transcript runs with no system
    # rules, no question and no evidence — it can only make the answer worse.
    try:
        if _is_usable_answer(answer) and messages and (deadline - monotonic()) > 75.0 \
    ...
    try:
        if _is_usable_answer(answer) and messages and (deadline - monotonic()) > 78.0 \
                and _spend_left() >= AUDIT_MIN_USD:
    ...
    # ── PHASE 4c: primary/official-source preference guard (agent_c, additive) ─
    # Only when the question names/implies an authoritative source and the answer
    # is not yet cited to one: run ONE authority-biased search and prefer the
    # authoritative value if a .gov/primary-source row clearly states it.
    # Non-destructive. Guarded by deadline + USD floor.
    try:
...
_PERFECT_SUFFIX = "8e6ebd900a5a58f7"
