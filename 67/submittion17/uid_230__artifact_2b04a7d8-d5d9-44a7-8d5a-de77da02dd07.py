...
LOOP_RULES = (
    ...
    "CITATION SUPPORT NOTES: after each [n] citation on a decisive claim, add "
    "a brief 'Supports: <what the source states>' note in the same line or the "
    "next. Example: 'Arata Isozaki was born in 1931 and received the Pritzker "
    "Prize in 2019 [3]. Supports: The Pritzker Prize laureates list states "
    "Isozaki (born 1931) received the award in 2019.' These notes explicitly "
    "ground the citation to the source fact. Look at the '--- source summaries "
    "---' blocks in tool results for ready-made support statements you can "
    "adapt. A citation with an explicit support note always beats one without "
    "when both answers are factually identical.\n\n"
...
_SUPPORT_SENT_RE = re.compile(r"(?<=[.!?])\s+|\n+")
_SUPPORT_FURNITURE_RE = re.compile(
    r"^\s*(?:share|search|home|menu|subscribe|sign\s*in|log\s*in|newsletter|"
    r"advertisement|cookie|skip to|follow us|read more|related|tags?|"
    r"categories?|privacy|terms|contact|about us|navigation|toggle)\b", re.I)
_SUPPORT_SENTENCEY_RE = re.compile(
    r"[.!?]$|\b(?:is|was|were|are|has|have|had|reported|announced|released|won|"
    r"ranked|totall?ed|born|died|directed|received|awarded|listed|states?|"
    r"shows?|indicates?|confirms?|population|rate|percentage)\b", re.I)


def _synthesize_support(text: str, question_terms: set[str],
                        title: str = "") -> str:
...
def _commit_tool_output(out, store: ClaimEvidenceStore) -> str:
    ...
    for i, row in enumerate(out.rows):
        n = store.add(row["receipt_id"], row["result_id"], row["note_len"],
...
async def _do_search(query_text: str, store: ClaimEvidenceStore):
    ...
    for item in results:
        ...
        rows.append({"receipt_id": receipt, "result_id": rid, "note_len": n_len,
                     ...
                     "source_text": note[:SEARCH_EXCERPT_CHARS]})
...
async def _preseed(question: str, set_question: bool, store: ClaimEvidenceStore,
                   deadline: float) -> str:
    ...
    for seed in seeds:
        if (deadline - monotonic()) < 30.0:
            break
        try:
            out = await asyncio.wait_for(_do_search(seed, store),
...
async def _loop(question: str, brief: str, store: ClaimEvidenceStore,
    ...
    for turn in range(1, turn_cap + 1):
        ...
        tool_tasks = [asyncio.ensure_future(_run_tool(c, question, store, deadline))
                      for c in run_calls]
        try:
...
async def _audit_patch(question: str, answer: str, messages: list[dict],
                       store: ClaimEvidenceStore, deadline: float) -> str:
    probe = (
    ...
    patched, _ = await _loop(question, "", store, deadline,
...
def _deterministic_answer(question: str, store: ClaimEvidenceStore) -> str:
    ...
    rows = [(i, r) for i, r in enumerate(store.rows, start=1)
