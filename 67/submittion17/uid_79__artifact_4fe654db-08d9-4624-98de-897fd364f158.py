# 20260805 hk405 — champion origin_159 (uid159) + improved HardPath citation construction
# dedupe + widen only (the exact change measured to beat baseline)
...
class HardPath:
    ...
    def _compile(self):
        ...
        # ── improved citation construction (2026-08-05) ───────────────────────
        # POLICY: dedupe + widen only (the exact change measured to beat baseline)
        ...
        _AUTH_URL_RE = re.compile(
            ...
            r"\.go\.[a-z]{2}\b|\.gc\.ca\b|\.gov\.uk\b|\.edu\b", re.I)
        ...
        def _citations_for(answer: str, ledger: EvidenceLedger) -> list[CitationRef]:
            try:
                ...
                for n in _cited_numbers(answer, len(ledger.rows)):
                    ...
                    spans = row.get('spans')
                    if not rid or not res or not spans:
                        continue
                    note_len = int(row.get('note_len') or 0)
                    kind = row.get('kind') or ''
                    widened = []
                    for s, e in spans:
                        s2 = max(0, min(int(s), note_len))
                        e2 = max(s2, min(int(e), note_len))
                        if kind == 'search':
                            e2 = min(note_len, max(e2, s2 + SEARCH_SLICE_WIDEN))
                        widened.append((s2, e2))
                    key = (rid, res)
                    grp = groups.get(key)
                    if grp is None:
                        grp = {'order': order, 'receipt_id': rid, 'result_id': res,
                               'note_len': note_len, 'spans': [], 'value': False, 'auth': False}
                        groups[key] = grp
                        order += 1
                    grp['spans'].extend(widened)
                    prev = row.get('preview') or ''
                    if not grp['value'] and _VALUE_SIGNAL_RE.search(prev):
                        grp['value'] = True
                    if not grp['auth'] and _AUTH_URL_RE.search(row.get('url') or ''):
                        grp['auth'] = True
                built = []
                for grp in groups.values():
                    ...
                    if not merged:
                        continue
                    grp['merged'] = merged
                    grp['cost'] = sum((e - s for s, e in merged))
                    built.append(grp)
                built.sort(key=lambda g: g['order'])
                refs = []
                spent = 0
                for grp in built:
                    if len(refs) >= CITATION_CAP:
                    ...
                    if grp['cost'] > room:
                        ...
                        for s, e in spans:
                            ...
                            w = e - s
                            if w <= budget:
                                trimmed.append((s, e)); budget -= w
                            else:
                                trimmed.append((s, min(e, s + budget))); budget = 0
                        spans = trimmed
                    slices = []
                    nl = grp['note_len']
                    for s, e in spans:
                        st = max(0, min(int(s), nl))
                        en = max(st + 1, min(int(e), nl))
                        slices.append(CitationSlice(start=st, end=en))
                    if not slices:
                        continue
                ...
                # never let an improvement cost the run its evidence
                return _citations_for_base(answer, ledger)
