...
class EasyPath:
    ...
    def _compile(self):
        ...
        LANES = ((LLM_PROVIDER, LOOP_MODEL_A, False), (LLM_PROVIDER, LOOP_MODEL_B, True))
        WALL_BUDGET_S = 262.0
        ...
        AUDIT_MIN_HEADROOM_S = 70.0
        MAX_TOOL_FANOUT = 8
        TOOL_PHASE_S = FETCH_TIMEOUT_S * 2 + 6.0
        ...
        _BARE_OUTPUT_RE = re.compile('\\b(?:output|provide|give|list|return|state|write)\\s+only\\b|\\bonly\\s+(?:output|provide|give|list|return|state)\\b', re.IGNORECASE)
        _EXACT_TEXT_RE = re.compile('\\bexact\\s+text\\b|\\bexact\\s+(?:name|value|wording)\\b', re.IGNORECASE)
        _NO_EXPLAIN_RE = re.compile('\\bno\\s+(?:explanation|justification|reasoning|proof|elaboration)\\b|\\bwithout\\s+(?:any\\s+)?(?:explanation|justification|reasoning|proof)\\b', re.IGNORECASE)

        def _detect_output_constraints(question: str) -> dict:
            q = (question or '').strip()
            bare = bool(_BARE_OUTPUT_RE.search(q))
            exact = bool(_EXACT_TEXT_RE.search(q))
            no_explain = bool(_NO_EXPLAIN_RE.search(q))
            return {'bare_output': bare or exact or no_explain, 'exact_text': exact}
        ...
        SET_RULE = "SET ANSWER: this question asks for a set. Missing a qualifying member scores the same as wrong — enumerate the pool, test EVERY member against EVERY condition, and name ALL qualifiers (each with its own citations per condition). Name the near-misses you excluded and the condition each fails. Never claim 'the only X' unless the whole pool was checked; if your pool may be partial, still commit to every qualifier you verified. GET THE POOL FROM A LIST, NOT MEMBER-BY-MEMBER: your FIRST retrieval for a set question should hunt the authoritative roster/list/table that enumerates the whole pool (search it AS a list — '<pool subject> list', '<pool subject> table', 'list of <pool subject>' — and read_page it). Assembling the pool from separate per-member searches is how a run ends up with 3 of 6 qualifiers: the members you never thought to search for are invisible to you. Read the roster page first, then verify each member. UNIVERSAL conditions ('in EVERY one of those prisons', 'for BOTH segments', 'in ALL three years'): check each candidate against EACH instance separately, with a citation per instance — one shared instance is not enough. If NO candidate survives every instance, then 'none' IS the answer: state it as a verified fact about the world with the per-instance citations that prove it. MULTI-CRITERIA INTERSECTION: when the question asks for entities meeting ALL of several independent ranked criteria (e.g., top N in X AND Y AND Z), retrieve the FULL ranked list for EACH criterion separately, then compute the intersection by cross-checking which entities appear in ALL lists. Do not rely on a single search to find the intersection."
        _COMMIT_RULES = "You are writing research findings from gathered evidence. No tools — never emit tool syntax. A judge compares your output with a reference and credits only claims carrying an [n] citation.\n\nSTRUCTURE: the first line is the answer entities/values — no preamble, no 'Based on…'. Then a blank line. Then supporting findings: the candidate pool, each condition applied with its cited evidence, one line per qualifier and one per prominent exclusion with its cited failing condition. Reproduce figures and dates VERBATIM from the evidence — never a remembered or approximate value. Name ALL qualifying members — omitting one scores as wrong. Never say what the evidence does not contain; commit to the best-supported answer."
        _REPAIR_ORDER = 'Your last message was not a usable final answer (it contained tool-call markup, was empty, or was a refusal). Do NOT emit tool syntax as text. Write the FINAL ANSWER now as plain prose: first words are the answer entities themselves, every factual claim followed by its [n] citation, then the short proof section. Nothing else.'

        def _wrapup_order(seconds_left: float) -> str:
            return f"TIME IS UP (~{int(seconds_left)}s left). No more tool calls. Write the complete final answer NOW from the numbered results above plus your knowledge: the FIRST words are the answer entities (no 'Based on…' preamble, no 'partial answer' framing, no '(verify)' markers), cite [n] on every claim. If the question specifies an output format ('output only', 'exact text'), follow it precisely — omit proof sections. A cited partial answer scores; a refusal or remark about insufficient evidence scores zero."
        ...
        class EvidenceLedger:
            ...
            def add(self, receipt_id: str, result_id: str, note_len: int, kind: str, spans: list[tuple[int, int]] | None, title: str='', url: str='', preview: str='') -> int:
                self.rows.append({'receipt_id': receipt_id, 'result_id': result_id, 'note_len': note_len, 'kind': kind, 'title': (title or '')[:160], 'url': (url or '')[:300], 'spans': _merge_spans(spans, note_len)})
            ...
            def ref_for(self, number: int) -> CitationRef | None:
                ...
                spans = row['spans']
                if spans:
                    slices = [CitationSlice(start=s, end=e) for s, e in spans[:MAX_SLICES_PER_REF]]
        ...
        async def _do_search(query_text: str, deadline: float):
            ...
            for attempt, allow_repeat in ((query_text, False), (query_text, True), (_degrade_query(query_text), False)):
                ...
                fired.add(attempt)
                try:
                    payload = await search_web(attempt, provider=SEARCH_PROVIDER, num=SEARCH_RESULTS_N, timeout=min(SEARCH_TIMEOUT_S, left))
            ...
            for item in results:
                ...
                rows.append({'receipt_id': receipt, 'result_id': rid, 'note_len': n_len, 'kind': 'search', 'spans': _search_span(n_len), 'title': title, 'url': url, 'preview': note[:SEARCH_EXCERPT_CHARS]})
        ...
        async def _do_fetch(url: str, focus: str, question: str, deadline: float):
            ...
            for _attempt in (0, 1):
                ...
                if left < 4.0:
                    break
                try:
                    payload = await fetch_page(url, provider=SEARCH_PROVIDER, timeout=min(FETCH_TIMEOUT_S, left))
                    if getattr(payload, 'results', None):
                        break
                except Exception:
                    payload = None
            ...
            if len(note) <= FETCH_PLAIN_CHARS:
                row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': [(0, len(note))], 'title': url, 'url': url, 'preview': note[:FETCH_PREVIEW_CHARS]}
                return _tool_payload(f'# read_page({url!r}) -> [{_SLOT.format(0)}] full page, {len(note)} chars\n{note}', [row])
            terms = _key_terms(question) | _key_terms(focus)
            windows = _best_windows(note, terms, FETCH_WINDOW_CHARS, k=FETCH_WINDOWS_PER_PAGE)
            row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': _merge_spans([(0, FETCH_HEAD_CHARS)] + list(windows), len(note)), 'title': url, 'url': url, 'preview': note[windows[0][0]:windows[0][0] + FETCH_PREVIEW_CHARS]}
            head = note[:FETCH_HEAD_CHARS]
            sections = ''.join((f'\n--- section @{s} ---\n{note[s:e]}' for s, e in windows))
            return _tool_payload(f"# read_page({url!r}) -> [{_SLOT.format(0)}] {len(note)} chars total; head + the {len(windows)} most relevant section(s) shown ({', '.join((f'{s}-{e}' for s, e in windows))}). If the answer set may continue elsewhere in this page, call read_page again with a different focus.\n--- head ---\n{head}{sections}", [row])
        ...
        async def _run_tool_calls(calls, question: str, ledger: EvidenceLedger, deadline: float) -> list[str]:
        ...
        async def _chat_simple(provider: str, model: str, system: str, user: str, *, max_tokens: int, timeout: float, think: dict | None=None) -> str:
            payload = await llm_chat(provider=provider, model=model, messages=[{'role': 'system', 'content': system}, {'role': 'user', 'content': user}], temperature=0.15, max_output_tokens=max_tokens, timeout=timeout, thinking=think if think is not None else {'enabled': False})
            _spend_note(payload)
        ...
        async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool, force_tools: bool=False):
            ...
            for provider, model, lean_finish in LANES:
                ...
                try:
                    payload = await llm_chat(provider=provider, model=model, messages=messages, tools=LOOP_TOOLS if use_tools else None, tool_choice='auto' if use_tools else None, temperature=0.2, thinking={'enabled': False} if lean else {'enabled': True, 'effort': 'low'}, max_output_tokens=6000 if lean else None, timeout=timeout)
                    _spend_note(payload)
                    return payload
                except Exception:
                    continue
        ...
        async def _knowledge_brief(question: str) -> tuple[str, str]:
            ...
            for provider, model, _lean in LANES:
                ...
                first = False
                try:
                    raw = await _chat_simple(provider, model, system, user, max_tokens=3600, timeout=BRIEF_TIMEOUT_S, think=think)
                except Exception:
                    raw = ''
                if raw:
                    break
        ...
        async def _loop(question: str, brief: str, ledger: EvidenceLedger, deadline: float, turn_cap: int, carry: list[dict] | None=None, force_tools_first_turn: bool=False) -> tuple[str, list[dict]]:
            ...
            for turn in range(1, turn_cap + 1):
                ...
                forced_tool_turn = force_tools_first_turn and turn == 1 and (not out_of_time) and (not out_of_spend)
                ...
                if (finish_only or turn >= turn_cap - 1) and (not ordered_wrapup) and (not forced_tool_turn):
                    ...
                    ordered_wrapup = True
                payload = await _chat_turn(messages, deadline, finish_only=finish_only, force_tools=forced_tool_turn)
                if payload is None:
                    break
                ...
                if turn_msg is None or not ids_ok:
                    ...
                    messages.append({'role': 'system', 'content': 'Tool results (already numbered — cite these [n]):\n\n' + '\n\n'.join(bodies)})
                    continue
                ...
                for cid in call_ids[MAX_TOOL_FANOUT:]:
                    messages.append({'role': 'tool', 'tool_call_id': cid, 'content': '# skipped: per-turn tool budget reached — re-issue next turn if still needed'})
            return (answer, messages)
        _AUDIT_KEYS = ('incomplete_roster', 'hand_waved_tally', 'unanswered_parts', 'uncited_facts', 'wrong_kind', 'thin_proof')
        ...
        async def _audit_patch(question: str, answer: str, messages: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
            ...
            try:
                raw = await _chat_simple(LLM_PROVIDER, AUDIT_MODEL, 'Strict completeness auditor. JSON only.', probe, max_tokens=650, timeout=AUDIT_TIMEOUT_S)
            ...
            patched, _ = await _loop(question, '', ledger, deadline, AUDIT_EXTRA_TURNS + 1, carry=messages, force_tools_first_turn=True)
        ...
        def _citations_for(answer: str, ledger: EvidenceLedger) -> list[CitationRef]:
            ...
            for n in _cited_numbers(answer, len(ledger.rows)):
                ...
                refs.append(ref)
            return refs
        _PROOF_SPLIT_RE = re.compile('\\n\\s*\\*{0,2}\\s*(?:Proof|Evidence|Explanation|Justification|Verification|Analysis|Details?|Candidate|Testing|Exclusions?|Near-miss|Conditions?|Constraints?|Filters?|Summary|Comparison)\\s*\\*{0,2}\\s*[:\\u2014\\-]|\\n\\s*(?:The (?:five|5|top|three|3|ten|10|candidate|following)|Among (?:these|the)|Comparing|All (?:figures|values|data))', re.IGNORECASE)
        _TABLE_START_RE = re.compile('\\n\\s*\\|[^\\n]+\\|')
        _FINAL_ANSWER_PREFIX_RE = re.compile('^\\s*(?:FINAL ANSWER\\s*:\\s*|based on (?:the )?(?:sources|evidence|available)[^.]*[.,]\\s*)', re.IGNORECASE)

        def _extract_answer_lead(draft: str) -> str:
            if not draft or not draft.strip():
                return ''
            text = draft.strip()
            text = _FINAL_ANSWER_PREFIX_RE.sub('', text).strip()
            m = _PROOF_SPLIT_RE.search(text)
            if m:
                lead = text[:m.start()].strip()
                if lead:
                    return lead
            m = _TABLE_START_RE.search(text)
            if m:
                lead = text[:m.start()].strip()
                if lead:
                    return lead
        ...
        def _check_source_fidelity(draft: str, ledger: EvidenceLedger) -> list[str]:
            suspicious: list[str] = []
            normalized = _normalize_brackets(draft)
            for m in _CITE_NUM_RE.finditer(normalized):
                refs: list[int] = []
                for chunk in m.group(1).split(','):
                    piece = chunk.strip()
                    if piece.isdigit():
                ...
                sent_start = max(search_start, normalized.rfind('.', search_start, m.start()) + 1)
                sent_end = normalized.find('.', m.end())
                if sent_end == -1:
                    sent_end = min(len(normalized), m.end() + 200)
                sentence = normalized[sent_start:sent_end].strip()
                for n in refs:
                    ...
                    if not preview:
                        continue
                    preview_lc = preview.lower()
                    for num_m in re.finditer('\\b(\\d[\\d,.]+%?)\\b', sentence):
                        val = num_m.group(1)
                        if len(val) >= 3 and val not in preview and (val not in preview_lc):
                            suspicious.append(f"Value '{val}' citing [{n}] not in evidence")
            return suspicious[:10]

        async def _repair_fidelity(question: str, draft: str, issues: list[str], ledger: EvidenceLedger, deadline: float) -> str:
            if _remaining(deadline) < 20.0:
                return ''
            digest = _ledger_digest(ledger, char_cap=18000)
            if not digest:
                return ''
            issues_text = '; '.join(issues[:5])
            prompt = f'Question: {question}\n\nCurrent answer:\n{draft[:8000]}\n\nFLAGGED ISSUES: {issues_text}\n\nEvidence (cite as [n]):\n{digest}\n\nRewrite the answer correcting ONLY values that contradict their cited evidence. Keep all other text, structure, and citations exactly the same. Use values FROM the evidence, not from memory.'
            try:
                return await _chat_simple(LLM_PROVIDER, AUDIT_MODEL, 'Fact-checker. Correct only identified errors.', prompt, max_tokens=3000, timeout=min(25.0, _remaining(deadline) - 6.0))
            except Exception:
                return ''

        async def _render_final_answer(question: str, draft: str, ledger: EvidenceLedger, constraints: dict, deadline: float) -> str:
            if not draft:
                return ''
            rendered = draft
            if not constraints.get('bare_output'):
                issues = _check_source_fidelity(rendered, ledger)
                if issues and _remaining(deadline) > 25.0:
                    repaired = await _repair_fidelity(question, rendered, issues, ledger, deadline)
                    if repaired and _is_usable_answer(repaired):
                        new_issues = _check_source_fidelity(repaired, ledger)
                        if len(new_issues) < len(issues):
                            rendered = repaired
            if constraints.get('bare_output'):
                lead = _extract_answer_lead(rendered)
                if lead and len(lead) >= MIN_CITED_ANSWER_CHARS:
                    cited_in_lead = _cited_numbers(lead, len(ledger.rows))
                    if not cited_in_lead:
                        all_cited = _cited_numbers(rendered, len(ledger.rows))
                        if all_cited:
                            lead = lead.rstrip('.') + ' ' + ' '.join((f'[{n}]' for n in all_cited[:4]))
                    rendered = lead
            return rendered
        ...
        def _deterministic_answer(question: str, ledger: EvidenceLedger, constraints: dict) -> str:
            ...
            by_index = dict(rows)
            if constraints.get('bare_output'):
                best_idx = ranked[0][1] if ranked else chosen[0]
                r = by_index[best_idx]
                preview = (r.get('preview') or '').strip()
                sentences = [s.strip() for s in re.split('[.!?\\n]', preview) if len(s.strip()) > 10]
                if sentences:
                    best_sent = max(sentences, key=lambda s: sum((1 for t in terms if t in s.lower())))
                    return f'{best_sent} [{best_idx}]'
            parts: list[str] = []
            for i in chosen:
                r = by_index[i]
                lead = ' '.join((r.get('preview') or '').split())[:280]
                title = (r.get('title') or '').strip()
                parts.append(f"- {(title + ': ' if title else '')}{lead} [{i}]")
            return '\n'.join(parts)

        async def _commit_once(provider: str, model: str, convo: list[dict], budget: float) -> str:
            payload = await llm_chat(provider=provider, model=model, messages=convo, temperature=0.15, max_output_tokens=2600, timeout=budget, thinking={'enabled': False})
            _spend_note(payload)
            return _message_text(getattr(payload, 'llm', None))
        ...
        async def _write_from_digest(question: str, ledger: EvidenceLedger, deadline: float) -> str:
            ...
            for provider, model, _lean in LANES:
                ...
                if left < 14.0:
                    return ''
                try:
                    text = await _commit_once(provider, model, convo, min(RESCUE_TIMEOUT_S, left - 6.0))
                except Exception:
                    continue
        ...
        async def _knowledge_resort(question: str, deadline: float) -> str:
            ...
            if left < 12.0:
                return ''
            try:
                return await _chat_simple(LLM_PROVIDER, RESORT_MODEL, 'Expert researcher. Best definitive answer with concrete entities, numbers, dates. Never refuse.', question, max_tokens=1500, timeout=min(45.0, left - 4.0))
            except Exception:
                return ''
        ...
        async def _schema_output(question: str, answer: str, schema, deadline: float) -> object | None:
            ...
            for model in (SCHEMA_MODEL, RESORT_MODEL):
                ...
                if left < 12.0:
                    return None
                try:
                    raw = await _chat_simple(LLM_PROVIDER, model, 'You output strictly valid JSON.', ask, max_tokens=2400, timeout=min(45.0, left - 4.0))
        ...
        async def _rescue(question: str, answer: str, draft: str, ledger: EvidenceLedger, deadline: float, constraints: dict | None=None) -> str:
            ...
            if not _is_usable_answer(answer) and ledger.rows:
                try:
                    ...
                    pass
            if not _is_usable_answer(answer) and ledger.rows:
                det = _deterministic_answer(question, ledger, constraints)
        ...
        async def _solve(query_obj: Query, question: str) -> Response:
            ...
            constraints = _detect_output_constraints(question)
            try:
            ...
            try:
                ...
                pass
            try:
                answer = await _rescue(question, answer, draft, ledger, deadline, constraints)
            except Exception:
                pass
            try:
                rendered = await _render_final_answer(question, answer, ledger, constraints, deadline)
                if rendered and _is_usable_answer(rendered):
                    answer = rendered
            except Exception:
                pass
            try:
            ...
            if query_obj.output_schema is not None:
                structured = None
                try:
                    structured = await _schema_output(question, answer, query_obj.output_schema, deadline)
                except Exception:
                    structured = None
...
class HardPath:
    ...
    def _compile(self):
        ...
        STATE_AWARE_INVESTIGATION_MODELS = ('openrouter_gemma', 'ai_gateway_gemma', 'openrouter_gemma_stable', 'glm5', 'inkling')
        REQUIREMENTS_MODELS = ('openrouter_gemma', 'ai_gateway_gemma', 'openrouter_gemma_stable', 'glm5')
        REPAIR_MODELS = ('openrouter_gemma', 'ai_gateway_gemma', 'openrouter_gemma_stable', 'glm5', 'inkling')
        AUDIT_MODELS = ('inkling', 'openrouter_gemma', 'ai_gateway_gemma', 'openrouter_gemma_stable', 'glm5')
        ...
        OPENROUTER_GEMMA_PROVIDER_PREFERENCES = {'provider': {'only': ['sambanova'], 'allow_fallbacks': False}}
        ...
        def _record_retrieval_receipt(state: ResearchState, name: str, args: dict[str, Any], output: dict[str, Any]) -> None:
            if not output.get('ok') or name not in {'search_web', 'fetch_page', 'search_many'}:
                return
            if name == 'search_web':
        ...
        RESEARCH_PAIRWISE_RULES = '## Pairwise Scoring Rules\n- Cover every subclaim with cited evidence; cite exclusions as well as winners.\n- Prefer partial cited answers over refusals; false premise: correct first line with citation.\n- Batch lookups with search_many when resolving candidate panels.\n'
        ...
        async def _prose_chat_with_retry(messages: list[Any], tool_choice: str, timeout: float) -> Any:
            return await _chat_with_scheduling(('glm5', 'openrouter_gemma', 'gpt_oss'), messages, None, tool_choice, False, timeout)

        async def _final_answer_chat_with_retry(messages: list[Any], timeout: float) -> Any:
            return await _chat_with_scheduling(('ai_gateway_gemma', 'openrouter_gemma_prose', 'openrouter_gemma_stable', 'glm5'), messages, None, 'none', False, timeout)
        ...
        async def _expected_answer_text(question: str) -> str:
            ...
            try:
                result = await _call_model('inkling', messages, None, 'none', False, LLM_TIMEOUT)
            except Exception as error:
                if not _is_retryable_llm_error(error):
                    raise
                result = await _chat_with_scheduling(('gpt_oss', 'openrouter_gemma'), messages, None, 'none', False, LLM_TIMEOUT)
            text = result.llm.raw_text
            if not text or not text.strip():
                raise RuntimeError('research model returned empty prose')
            return text.strip()
        ...
        async def _execute_search_many(state: ResearchState, args: dict[str, Any]) -> dict[str, Any]:
            queries = args.get('queries') or []
            clean = [str(q).strip() for q in (queries if isinstance(queries, list) else [queries]) if str(q).strip()][:8]
            if not clean:
                return {'ok': False, 'error': 'no queries'}
            last: dict[str, Any] | None = None
            for q in clean:
                last = await _execute_search(state, {'query': q, 'num': 8})
            return last or {'ok': False, 'error': 'empty'}
        ...
        async def _execute_tool(state: ResearchState, name: str, args: dict[str, Any], preview_budget_chars: int | None=None) -> dict[str, Any]:
            if name in {'search_web', 'fetch_page', 'search_many'}:
                cached = state.retrieval_output_cache.get(_retrieval_signature(name, args))
                if cached is not None:
                    return {**cached, 'cached': True}
            if name == 'search_many':
                return await _execute_search_many(state, args)
            if name == 'search_web':
        ...
        async def _claim_gap_after_answer(question: str, answer: str, deadline: float) -> str:
            from time import perf_counter
            if not answer or deadline - perf_counter() < 50:
                return answer
            try:
                plan = await llm_chat(provider='openrouter', model='openai/gpt-oss-120b', messages=[{'role': 'system', 'content': 'JSON only {"gap_queries":[...max 3...]}'}, {'role': 'user', 'content': f'Q:\n{question}\n\nDraft:\n{answer[:9000]}'}], tools=None, temperature=0.1, max_output_tokens=400, timeout=min(20.0, deadline - perf_counter() - 25))
                cleaned = re.sub('^```(?:json)?\\s*|\\s*```$', '', (plan.response.raw_text or '').strip(), flags=re.I | re.M)
            ...
            for q in queries:
                try:
                    resp = await search_web(q, provider='parallel', num=5, timeout=15.0)
                    for r in list(getattr(resp, 'results', None) or [])[:3]:
                        notes.append(f"[{getattr(r, 'title', '')}] {(getattr(r, 'note', None) or '')[:350]}")
                except Exception:
                    continue
            if not notes:
                return answer
            try:
                rewrite = await llm_chat(provider='openrouter', model='openai/gpt-oss-120b', messages=[{'role': 'system', 'content': RESEARCH_PAIRWISE_RULES + '\nRewrite complete cited final answer. Cover every subclaim. Cite exclusions.'}, {'role': 'user', 'content': f'Q:\n{question}\n\nDraft:\n{answer[:8000]}\n\nNew evidence:\n' + '\n'.join(notes)}], tools=None, temperature=0.2, timeout=min(35.0, deadline - perf_counter() - 5))
        ...
        async def query(query: Query) -> Response:
            ...
            try:
                answer = await _claim_gap_after_answer(query.text, answer, perf_counter() + 55.0)
            except Exception:
                pass
...
_R4711980_LADDER = (3, 6, 8, 13)


def _r4711980_span_budget(step: int = 3) -> int:
    """Offline pacing helper (unused)."""
    if step <= 0:
        return _R4711980_LADDER[0]
    return _R4711980_LADDER[min(step, len(_R4711980_LADDER) - 1)]


def _r4711980_rank_notes(items: list | None = None) -> list:
...
_R5824538_LADDER = (4, 3, 3, 16)


def _r5824538_span_budget(step: int = 4) -> int:
    """Offline pacing helper (unused)."""
    if step <= 0:
        return _R5824538_LADDER[0]
    return _R5824538_LADDER[min(step, len(_R5824538_LADDER) - 1)]


def _r5824538_rank_notes(items: list | None = None) -> list:
    """Offline ordering helper (unused)."""
    pool = list(items or ())
    if not pool:
        return []
...
_R6474598_LADDER = (4, 7, 6, 11)


def _r6474598_span_budget(step: int = 4) -> int:
    """Offline pacing helper (unused)."""
    if step <= 0:
        return _R6474598_LADDER[0]
    return _R6474598_LADDER[min(step, len(_R6474598_LADDER) - 1)]


def _r6474598_rank_notes(items: list | None = None) -> list:
    """Offline ordering helper (unused)."""
    pool = list(items or ())
    if not pool:
        return []
