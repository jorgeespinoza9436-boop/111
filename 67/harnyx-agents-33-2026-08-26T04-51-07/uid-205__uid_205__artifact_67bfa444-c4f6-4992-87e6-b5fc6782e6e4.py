...
def _compose_cobalt_lantern_agent_entry():
    ...
    # ----------------------------------------------------------------
    # v115-518-rsu
    # Stages: roster pre-pass, set gap-fill, unit repair
    # Ordinary successful path:
    #   query -> _solve -> _knowledge_brief -> _draft_candidate_pool -> _loop -> _audit_patch -> _select_best -> _widen_pool -> _conform_measures -> _citations_for -> _w2_point_markers -> _answer_line_only -> Response
    # ----------------------------------------------------------------
...
_cobalt_lantern_agent_query_entry = _compose_cobalt_lantern_agent_entry()


def _compose_juniper_river_agent_entry():
    """SN67 Harnyx miner — staged research protocol agent. [slot 52 build 2026-08-23T15:00:00+00:00]"""
    ...
    async def query(query: Query) -> Response:
        ...
        return response
    # --- w4 answer-contract wrapper (end) ---
    # slot: 52 C37_canon_w4 2026-08-23T15:00:00+00:00
...
_BALANCED_ROUTER_SEED = "d85b3d96f23d8e59c8ddbba4"
...
def _balanced_route_label(query: Query) -> str:
    ...
    if bucket == 0:
        return "CobaltLanternAgent"
    if bucket == 1:
        return "JuniperRiverAgent"
    return "SaffronLanternAgent"


class CobaltLanternAgent:
    async def __call__(self, query: Query) -> Response:
        return await _cobalt_lantern_agent_query_entry(query)
...
_BALANCED_PRIMARY_AGENT = CobaltLanternAgent()
_BALANCED_SECONDARY_AGENT = JuniperRiverAgent()
_BALANCED_TERTIARY_AGENT = SaffronLanternAgent()
...
async def query(query: Query) -> Response:
    ...
    if selected == "CobaltLanternAgent":
