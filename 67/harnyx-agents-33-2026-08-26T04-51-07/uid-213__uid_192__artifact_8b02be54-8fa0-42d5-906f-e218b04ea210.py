...
# variant 01 — stages combined from validator_reference.txt:
#   premise      <- uid 53  (post-audit slot 0)
#   authority    <- uid 163  (post-audit slot 1)
#   corroborate  <- uid 24  (post-audit slot 2)


# --- _s01 verification stages (begin) ---
_s01_MIN_REWRITE_S = 66.0
_s01_MIN_SEARCH_S = 82.0
_s01_ADOPT_RATIO = 0.60
_s01_PROBE_TIMEOUT_S = 16.0

_s01_YEAR_RE = re.compile(r"\b(1[89]\d\d|20[0-4]\d)\b")
_s01_NUM_RE = re.compile(r"\d[\d,]*(?:\.\d+)?")
_s01_NAME_RE = re.compile(r"\b[A-Z][A-Za-z0-9'\u2019\-]{2,}(?:\s+[A-Z][A-Za-z0-9'\u2019\-]{2,}){0,3}")
_s01_STOP = {"The", "This", "That", "What", "Which", "Who", "When", "Where", "How",
...
_s01_OFFICIAL = ("gov", "gov.uk", "europa.eu", "int", "edu", "who.int", "un.org",
                "oecd.org", "imf.org", "worldbank.org", "sec.gov", "nasa.gov",
                "ec.europa.eu", "parliament.uk", "canada.ca", "gov.au")


def _s01_time_ok(deadline: float, need: float) -> bool:
    return (deadline - monotonic()) >= need and _spend_left() > WRAPUP_MIN_USD


def _s01_evidence_blob(ledger: EvidenceLedger, cap: int = 48000) -> str:
    parts = []
    spent = 0
...
def _s01_cited_rows(answer: str, ledger: EvidenceLedger) -> list:
    picked = []
    for n in _cited_numbers(answer, len(ledger.rows)):
...
def _s01_names(text: str) -> list:
    out = []
    seen = set()
    for m in _s01_NAME_RE.finditer(text or ""):
        token = m.group(0).strip()
        head = token.split(" ")[0]
        if head in _s01_STOP or len(token) < 4:
            continue
        low = token.lower()
        if low in seen:
            continue
...
async def _s01_rewrite(question: str, answer: str, messages: list,
                      ledger: EvidenceLedger, deadline: float,
                      order: str) -> str:
    if not _s01_time_ok(deadline, _s01_MIN_REWRITE_S):
        return answer
    ...
    if not _is_usable_answer(redone):
        return answer
    if len(redone) < int(len(answer) * _s01_ADOPT_RATIO):
        return answer
    return redone


async def _s01_premise(question: str, answer: str, messages: list,
                      ledger: EvidenceLedger, deadline: float) -> str:
    """Named subjects of the question absent from the gathered evidence."""
    if not _is_usable_answer(answer) or not ledger.rows:
        return answer
    if not _s01_time_ok(deadline, _s01_MIN_SEARCH_S):
        return answer
    subjects = _s01_names(question)
    if not subjects:
        return answer
    blob = _s01_evidence_blob(ledger).lower()
    if not blob:
        return answer
    missing = []
    ...
    return await _s01_rewrite(question, answer, messages, ledger, deadline, order)


def _s01_is_official(url: str) -> bool:
    low = (url or "").lower()
    for suffix in _s01_OFFICIAL:
        if ("." + suffix + "/") in low or low.endswith("." + suffix):
            return True
    return False


async def _s01_authority(question: str, answer: str, messages: list,
                        ledger: EvidenceLedger, deadline: float) -> str:
    """No cited source is an official / primary domain."""
    if not _is_usable_answer(answer) or not ledger.rows:
        return answer
    if not _s01_time_ok(deadline, _s01_MIN_SEARCH_S):
        return answer
    cited = _s01_cited_rows(answer, ledger)
    if not cited:
        return answer
    official = 0
    for row in cited:
        if _s01_is_official(row.get("url") or ""):
            official = official + 1
    if official > 0:
        return answer
    subjects = _s01_names(question)[:2]
    order = (
        "SOURCE AUTHORITY: every citation in this answer is secondary — no "
        "official, primary or statutory source is behind any load-bearing "
        "claim. Run one search aimed at the body that PUBLISHES this fact "
        "(the agency, registry, regulator, statistical office or the primary "
        "document itself; try: " +
        (" ".join(subjects) + " official report").strip()[:160] +
        "), read the best hit, then rewrite the COMPLETE answer anchoring the "
        "decisive claims to that source. If no primary source is reachable, "
        "keep the current figures and state plainly which body would publish "
        "the authoritative version. Keep the required output shape."
    )
    return await _s01_rewrite(question, answer, messages, ledger, deadline, order)


async def _s01_corroborate(question: str, answer: str, messages: list,
                          ledger: EvidenceLedger, deadline: float) -> str:
    ...
    if not _s01_time_ok(deadline, _s01_MIN_SEARCH_S):
        return answer
    lead = ""
    for m in _s01_NUM_RE.finditer(answer[:2400]):
        token = m.group(0)
...
async def _solve(query: Query, question: str) -> Response:
    ...
    try:
        if _is_usable_answer(answer):
            staged = await _s01_premise(question, answer, messages, ledger, deadline)
    ...
    try:
        if _is_usable_answer(answer):
            staged = await _s01_authority(question, answer, messages, ledger, deadline)
            if _is_usable_answer(staged):
                answer = staged
    except Exception:
        pass
    ...
    try:
        if _is_usable_answer(answer):
            staged = await _s01_corroborate(question, answer, messages, ledger, deadline)
            if _is_usable_answer(staged):
                answer = staged
    except Exception:
        pass
