...
# variant 03 — stages combined from validator_reference.txt:
#   temporal     <- uid 81  (post-audit slot 0)
#   setgap       <- uid 193  (post-audit slot 1)
#   unit         <- uid 236  (post-audit slot 2)


# --- _s03 verification stages (begin) ---
_s03_MIN_REWRITE_S = 74.0
_s03_MIN_SEARCH_S = 78.0
_s03_ADOPT_RATIO = 0.55
_s03_PROBE_TIMEOUT_S = 20.0

_s03_YEAR_RE = re.compile(r"\b(1[89]\d\d|20[0-4]\d)\b")
_s03_NUM_RE = re.compile(r"\d[\d,]*(?:\.\d+)?")
_s03_NAME_RE = re.compile(r"\b[A-Z][A-Za-z0-9'\u2019\-]{2,}(?:\s+[A-Z][A-Za-z0-9'\u2019\-]{2,}){0,3}")
_s03_STOP = {"The", "This", "That", "What", "Which", "Who", "When", "Where", "How",
...
_s03_OFFICIAL = ("gov", "gov.uk", "europa.eu", "int", "edu", "who.int", "un.org",
                "oecd.org", "imf.org", "worldbank.org", "sec.gov", "nasa.gov",
                "ec.europa.eu", "parliament.uk", "canada.ca", "gov.au")


def _s03_time_ok(deadline: float, need: float) -> bool:
    return (deadline - monotonic()) >= need and _spend_left() > WRAPUP_MIN_USD


def _s03_evidence_blob(ledger: EvidenceLedger, cap: int = 48000) -> str:
    parts = []
    spent = 0
...
def _s03_cited_rows(answer: str, ledger: EvidenceLedger) -> list:
    picked = []
    for n in _cited_numbers(answer, len(ledger.rows)):
...
def _s03_names(text: str) -> list:
    out = []
    seen = set()
    for m in _s03_NAME_RE.finditer(text or ""):
        token = m.group(0).strip()
        head = token.split(" ")[0]
        if head in _s03_STOP or len(token) < 4:
            continue
        low = token.lower()
        if low in seen:
            continue
...
async def _s03_rewrite(question: str, answer: str, messages: list,
                      ledger: EvidenceLedger, deadline: float,
                      order: str) -> str:
    if not _s03_time_ok(deadline, _s03_MIN_REWRITE_S):
        return answer
    ...
    if not _is_usable_answer(redone):
        return answer
    if len(redone) < int(len(answer) * _s03_ADOPT_RATIO):
        return answer
    return redone


async def _s03_temporal(question: str, answer: str, messages: list,
                       ledger: EvidenceLedger, deadline: float) -> str:
    """Anchor years demanded by the question absent from the cited evidence."""
    if not _is_usable_answer(answer) or not ledger.rows:
        return answer
    if not _s03_time_ok(deadline, _s03_MIN_SEARCH_S):
        return answer
    anchors = []
    for m in _s03_YEAR_RE.finditer(question or ""):
        year = m.group(0)
        if year not in anchors:
            anchors.append(year)
    if not anchors:
        return answer
    cited = _s03_cited_rows(answer, ledger)
    pool = cited if cited else ledger.rows
    blob = []
    for row in pool:
        blob.append(_row_evidence_text(row, 1600))
        blob.append(row.get("title") or "")
    joined = "\n".join(blob)
    absent = [y for y in anchors[:3] if y not in joined]
    if not absent:
        return answer
    subjects = _s03_names(question)[:2]
    order = (
        "TEMPORAL ALIGNMENT: the question is anchored to " +
        ", ".join(absent) + ", but the evidence behind this answer never "
        "shows that period. A figure from the wrong year reads as a wrong "
        "figure. Run one search pinned to the period (try: " +
        (" ".join(subjects) + " " + absent[0]).strip()[:160] +
        "), read the best hit, then rewrite the COMPLETE answer so every "
        "period-bound claim cites evidence that actually carries the anchor "
        "year. If only a neighbouring period is available, say which period "
        "the figure belongs to instead of implying the asked-for one. Keep "
        "the required output shape."
    )
    return await _s03_rewrite(question, answer, messages, ledger, deadline, order)


async def _s03_setgap(question: str, answer: str, messages: list,
                     ledger: EvidenceLedger, deadline: float) -> str:
    ...
    if len(named) >= 5 and lines >= 2 and not hedged:
        return answer
    subjects = _s03_names(question)[:2]
    order = (
...
def _s03_unit_gap(question: str, answer: str) -> list:
    q = (question or "").lower()
    a = (answer or "").lower()
    gaps = []
    for label, tokens in _s03_UNIT_CUES:
        wanted = False
        for tok in tokens:
            if tok in q:
                wanted = True
...
async def _s03_unit(question: str, answer: str, messages: list,
                   ledger: EvidenceLedger, deadline: float) -> str:
    ...
    if not _s03_time_ok(deadline, _s03_MIN_REWRITE_S):
        return answer
    gaps = _s03_unit_gap(question, answer)
    if not gaps:
        return answer
    order = (
...
async def _solve(query: Query, question: str) -> Response:
    ...
    try:
        if _is_usable_answer(answer):
            staged = await _s03_temporal(question, answer, messages, ledger, deadline)
    ...
    try:
        if _is_usable_answer(answer):
            staged = await _s03_setgap(question, answer, messages, ledger, deadline)
            if _is_usable_answer(staged):
                answer = staged
    except Exception:
        pass
    ...
    try:
        if _is_usable_answer(answer):
            staged = await _s03_unit(question, answer, messages, ledger, deadline)
            if _is_usable_answer(staged):
                answer = staged
    except Exception:
        pass
