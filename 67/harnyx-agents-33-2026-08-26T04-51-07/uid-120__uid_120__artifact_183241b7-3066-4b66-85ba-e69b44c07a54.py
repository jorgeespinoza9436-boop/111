...
class Sable3a42b7:
    ...
    def _pallet_62996f(self):
        _SEARCH_BUDGET_S_b6e2ce = 70.0
        _KEEP_b6e2ce = 8
        _MODEL_b6e2ce = 'openai/gpt-oss-120b'

        def _plan_b6e2ce(question):
            """Turn one question into the retrieval queries this reserve will run."""
            stop = {'what', 'which', 'when', 'where', 'the', 'and', 'for', 'with'}
            core = [w for w in question.lower().split() if w not in stop and len(w) >= 5]
            return [question, ' '.join(core[:8]), ' '.join(core[8:16])][:4 + 1]

        def _order_b6e2ce(results, terms):
        ...
        def _digest_b6e2ce(results):
            parts = []
            for item in results[:_KEEP_b6e2ce]:
                body = (getattr(item, 'content', None) or getattr(item, 'snippet', None) or '').strip()
                if body:
                    parts.append(body[:1200])
            return '\n\n'.join(parts)

        async def _gather_b6e2ce(question):
            queries = _plan_b6e2ce(question)
            collected = []
            for q in queries[:4 + 1]:
                try:
                    found = await asyncio.wait_for(search_web(q, provider='parallel', num=4), timeout=min(_SEARCH_BUDGET_S_b6e2ce, 30.0))
                except Exception:
                    continue
        ...
        async def query(query):
            question = getattr(query, 'text', '') or ''
            terms = [w.lower() for w in question.split() if len(w) >= 5]
            results = await _gather_b6e2ce(question)
            if not results:
                return Response(text='No answer produced.')
            ordered = _order_b6e2ce(results, terms)
            evidence = _digest_b6e2ce(ordered)
            if not evidence:
                return Response(text='No answer produced.')
            try:
                reply = await llm_chat(provider='openrouter', model=_MODEL_b6e2ce, messages=[{'role': 'system', 'content': 'Answer strictly from the supplied evidence.'}, {'role': 'user', 'content': question + '\n\nEvidence:\n' + evidence}])
            except Exception:
                return Response(text='No answer produced.')
...
def _basalt_a6861f(factory):
    """Build a source closure; a source that dies on import must not kill the agent."""
    try:
        return factory()._pallet_62996f()
    except Exception:
        return None

def _nimbus_2660c7(response):
    if response is None:
        return ''
    return (getattr(response, 'text', None) or '').strip()

def _umber_eb8550(response):
    if response is None:
        return 0
    return len(getattr(response, 'citations', None) or ())

def _kestrel_ef9c3e(response):
    return response is not None and getattr(response, 'output', None) is not None

def _basalt_986f20(query, response):
    """Deterministic answer quality. No model call, so auditing is free."""
    if response is None:
        return 0.0
    if query.output_schema is not None and (not _kestrel_ef9c3e(response)):
        return 0.0
    text = _nimbus_2660c7(response)
    if not _kestrel_ef9c3e(response) and len(text) < 40:
        return 0.0
    score = 1.0
    if _kestrel_ef9c3e(response):
        score += 1.0
    score += min(_umber_eb8550(response), 12) * 0.05
    score += min(len(text), 4000) / 4000.0
    return score

class Fathomaa8b90:
    """Answer with the primary; consult the reserve when no evidence was cited."""
    _ZEPHYR_3CF68A = 290.0
    _INGOT_D14EC1 = 270.0
    _PALLET_BB463F = 45.0
    _JUNIPER_DC69E9 = 1
    ...
    def __init__(self, primary, reserve):
        ...
        self._trellis_4cadcb = []

    def _vellum_4a8213(self, query, response):
        if response is None:
            return True
        cited = _umber_eb8550(response)
        self._trellis_4cadcb.append(cited)
        if cited >= self._JUNIPER_DC69E9:
            return False
        return _basalt_986f20(query, response) <= 0.0 or not _kestrel_ef9c3e(response)

    async def _girder_73dce3(self, run, request, budget):
    ...
    async def rivet_d26f04(self, query: Query) -> Response:
        started = monotonic()
        first = await self._girder_73dce3(self._primary, query, self._INGOT_D14EC1)
        if not self._vellum_4a8213(query, first):
            return first if first is not None else Response(text='No answer produced.')
        remaining = self._ZEPHYR_3CF68A - (monotonic() - started)
        if remaining <= self._PALLET_BB463F:
            return first if first is not None else Response(text='No answer produced.')
        second = await self._girder_73dce3(self._reserve, query, remaining)
        candidates = [r for r in (first, second) if r is not None]
        if not candidates:
            return Response(text='No answer produced.')
        return max(candidates, key=lambda r: _basalt_986f20(query, r))
_LANTERN_04FB3F = query
_MARLIN_B2C78A = _basalt_a6861f(Sable3a42b7)
_QUARRY_E5AD0B = Fathomaa8b90(_LANTERN_04FB3F, _MARLIN_B2C78A)

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _QUARRY_E5AD0B.rivet_d26f04(query)
