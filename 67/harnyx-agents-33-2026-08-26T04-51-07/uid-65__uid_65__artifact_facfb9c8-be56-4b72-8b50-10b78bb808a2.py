...
class Sable5b0149:
    ...
    def _trellis_27a9eb(self):
        ...
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import CitationRef, CitationSlice, Query, Response
        VERSION = 'v143-citation-pointers'
        LLM_LANE_A = 'openrouter'
        ...
        FETCH_WINDOW_CHARS = 3600
        CITATION_MIN_SPAN_CHARS = 6000
        CITATION_MIN_SPAN_PROSE = 12000
        CITATION_MAX_REF_CHARS = 30000
        FETCH_WINDOWS_PER_PAGE = 3
        FETCH_PLAIN_CHARS = 6500
        ANSWER_CHAR_CAP = 60000
        CITATION_CAP = 24
        ...
        WRAPUP_MIN_USD = 0.02
        _SPEND = {'left': None}
        _WIDTH = {'min': CITATION_MIN_SPAN_CHARS}
            ...
            def ref_for(self, number: int) -> CitationRef | None:
                ...
                if spans:
                    ...
                    if merged and note_len and room:
                        extra = room // len(merged)
                        for w in merged:
                            pad = min(extra, max(0, _WIDTH['min'] - (w[1] - w[0])))
                            if pad:
                                left = min(pad // 2, w[0])
                                w[0] -= left
                                rest = pad - left
        ...
        def _least_think(lane: str, model: str='') -> dict:
            for prefix in _REASONING_MANDATORY:
                if model.startswith(prefix):
                    return {'enabled': True, 'effort': 'low'}
            return {'enabled': False}
        _FAST_UPSTREAMS = ('Decart', 'CoreWeave', 'Alibaba', 'StreamLake')
        _FAST_UPSTREAMS_OSS = ('Cerebras', 'Groq', 'BaseTen')
        ...
        async def _solve(query: Query, question: str) -> Response:
            deadline = monotonic() + WALL_BUDGET_S
            _WIDTH['min'] = CITATION_MIN_SPAN_CHARS if query.output_schema is not None else CITATION_MIN_SPAN_PROSE
            try:
                info = await tooling_info(timeout=10.0)
                _spend_note(info)
            except Exception:
            ...
            if query.output_schema is not None:
                ...
                if structured is not None:
                    try:
                        ...
                        pass
                    try:
                        if _schema_row_filled(structured):
                            structured = None
                    except Exception:
                        pass
                    try:
...
def _vellum_416010(factory):
    """Build a source closure; a source that dies on import must not kill the agent."""
    try:
        return factory()._trellis_27a9eb()
    except Exception:
        return None

def _ember_9e7da6(response):
    if response is None:
        return ''
    return (getattr(response, 'text', None) or '').strip()

def _kestrel_7b037b(response):
    if response is None:
        return 0
    return len(getattr(response, 'citations', None) or ())

def _zephyr_7b545f(response):
    return response is not None and getattr(response, 'output', None) is not None

def _vellum_b24491(query, response):
    """Deterministic answer quality. No model call, so auditing is free."""
    if response is None:
        return 0.0
    if query.output_schema is not None and (not _zephyr_7b545f(response)):
        return 0.0
    text = _ember_9e7da6(response)
    if not _zephyr_7b545f(response) and len(text) < 40:
        return 0.0
    score = 1.0
    if _zephyr_7b545f(response):
        score += 1.0
    score += min(_kestrel_7b037b(response), 12) * 0.05
    score += min(len(text), 4000) / 4000.0
    return score

class Ingot8223d8:
    """Answer with the primary; consult the reserve when no evidence was cited."""
    _JUNIPER_9E67F8 = 290.0
    _UMBER_7EDF5A = 270.0
    _TRELLIS_339D95 = 45.0
    _DOVETAIL_B17540 = 1
    ...
    def _alder_edd8e0(self, query, response):
        if response is None:
            return True
        cited = _kestrel_7b037b(response)
        self._cinder_ef4df6.append(cited)
        if cited >= self._DOVETAIL_B17540:
            return False
        return _vellum_b24491(query, response) <= 0.0 or not _zephyr_7b545f(response)

    async def _yarrow_124b8b(self, run, request, budget):
    ...
    async def nimbus_ce7ca0(self, query: Query) -> Response:
        started = monotonic()
        first = await self._yarrow_124b8b(self._primary, query, self._UMBER_7EDF5A)
        if not self._alder_edd8e0(query, first):
            return first if first is not None else Response(text='No answer produced.')
        remaining = self._JUNIPER_9E67F8 - (monotonic() - started)
        if remaining <= self._TRELLIS_339D95:
            return first if first is not None else Response(text='No answer produced.')
        second = await self._yarrow_124b8b(self._reserve, query, remaining)
        candidates = [r for r in (first, second) if r is not None]
        if not candidates:
            return Response(text='No answer produced.')
        return max(candidates, key=lambda r: _vellum_b24491(query, r))
_GIRDER_32E562 = query
_QUARRY_63C08F = _vellum_416010(Sable5b0149)
_MARLIN_D839CF = Ingot8223d8(_GIRDER_32E562, _QUARRY_63C08F)

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _MARLIN_D839CF.nimbus_ce7ca0(query)
