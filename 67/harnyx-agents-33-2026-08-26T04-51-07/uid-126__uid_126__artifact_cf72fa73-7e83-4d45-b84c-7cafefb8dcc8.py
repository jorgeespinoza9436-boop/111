...
# variant 37 — stages combined from validator_reference.txt:
#   corroborate  <- uid 24  (post-audit slot 0)
#   setgap       <- uid 193  (post-audit slot 1)
#   unit         <- uid 236  (post-audit slot 2)


# --- _s37 verification stages (begin) ---
_s37_MIN_REWRITE_S = 74.0
_s37_MIN_SEARCH_S = 88.0
_s37_ADOPT_RATIO = 0.60
_s37_PROBE_TIMEOUT_S = 22.0

_s37_YEAR_RE = re.compile(r"\b(1[89]\d\d|20[0-4]\d)\b")
_s37_NUM_RE = re.compile(r"\d[\d,]*(?:\.\d+)?")
_s37_NAME_RE = re.compile(r"\b[A-Z][A-Za-z0-9'\u2019\-]{2,}(?:\s+[A-Z][A-Za-z0-9'\u2019\-]{2,}){0,3}")
_s37_STOP = {"The", "This", "That", "What", "Which", "Who", "When", "Where", "How",
...
_s37_OFFICIAL = ("gov", "gov.uk", "europa.eu", "int", "edu", "who.int", "un.org",
                "oecd.org", "imf.org", "worldbank.org", "sec.gov", "nasa.gov",
                "ec.europa.eu", "parliament.uk", "canada.ca", "gov.au")


def _s37_time_ok(deadline: float, need: float) -> bool:
    return (deadline - monotonic()) >= need and _spend_left() > WRAPUP_MIN_USD


def _s37_evidence_blob(ledger: EvidenceLedger, cap: int = 48000) -> str:
    parts = []
    spent = 0
...
def _s37_cited_rows(answer: str, ledger: EvidenceLedger) -> list:
    picked = []
    for n in _cited_numbers(answer, len(ledger.rows)):
...
def _s37_names(text: str) -> list:
                                                                               
                                                                            
                                                                          
    out = []
    seen = set()
    for m in _s37_NAME_RE.finditer(text or ""):
        parts = m.group(0).strip().split()
        while parts and parts[0] in _s37_STOP:
            parts = parts[1:]
        while parts and parts[-1] in _s37_STOP:
            parts = parts[:-1]
        if not parts:
            continue
        token = " ".join(parts)
...
async def _s37_rewrite(question: str, answer: str, messages: list,
                      ledger: EvidenceLedger, deadline: float,
                      order: str) -> str:
    if not _s37_time_ok(deadline, _s37_MIN_REWRITE_S):
        return answer
    ...
    if not _is_usable_answer(redone):
        return answer
    if len(redone) < int(len(answer) * _s37_ADOPT_RATIO):
        return answer
    return redone


async def _s37_corroborate(question: str, answer: str, messages: list,
                          ledger: EvidenceLedger, deadline: float) -> str:
    """Lead decisive figure carried by only one source -> targeted second source."""
    if not _is_usable_answer(answer) or not ledger.rows:
        return answer
    if not _s37_time_ok(deadline, _s37_MIN_SEARCH_S):
        return answer
    lead = ""
    for m in _s37_NUM_RE.finditer(answer[:2400]):
        token = m.group(0)
    ...
    if carriers != 1:
        return answer
    subjects = _s37_names(question)[:2]
    ...
    return await _s37_rewrite(question, answer, messages, ledger, deadline, order)


async def _s37_setgap(question: str, answer: str, messages: list,
                     ledger: EvidenceLedger, deadline: float) -> str:
    ...
    if len(named) >= 3 and lines >= 3 and not hedged:
        return answer
    subjects = _s37_names(question)[:2]
    order = (
...
_s37_UNIT_CUES = (
    ...
    ("billion", ("billion", "bn", "billions")),
    ("million", ("million", "mn", "millions")),
    ("thousand", ("thousand", "000s")),
    ("usd", ("$", "usd", "dollar")),
    ("eur", ("\u20ac", "eur", "euro")),
    ("gbp", ("\u00a3", "gbp", "pound sterling")),
    ("jpy", ("\u00a5", "jpy", "yen")),
    ("km", ("kilometre", "kilometer", " km")),
    ("mile", ("mile", " mi ")),
    ("kg", ("kilogram", " kg")),
...
def _s37_unit_gap(question: str, answer: str) -> list:
    q = (question or "").lower()
    a = (answer or "").lower()
    gaps = []
    for label, tokens in _s37_UNIT_CUES:
        wanted = False
        for tok in tokens:
            if tok in q:
                wanted = True
...
async def _s37_unit(question: str, answer: str, messages: list,
                   ledger: EvidenceLedger, deadline: float) -> str:
    """Scale / currency the question demands but the answer never states."""
    if not _is_usable_answer(answer):
        return answer
    if not _s37_time_ok(deadline, _s37_MIN_REWRITE_S):
        return answer
    gaps = _s37_unit_gap(question, answer)
    if not gaps:
        return answer
    order = (
        "UNIT CONSISTENCY: the question asks in " + ", ".join(gaps[:3]) +
        ", and the answer never states that scale or currency. A bare number "
        "in an unstated unit is scored as a wrong number. Rewrite the "
        "COMPLETE answer with every figure carrying its explicit unit and "
        "scale, converted where the question demands a scale the source does "
        "not print, and state the conversion you applied next to the figure. "
        "Do NOT invent a converted value the evidence cannot support — if the "
        "source prints only a different scale, give the source's own figure "
        "with its unit and say so. Keep the required output shape and every "
        "existing [n] citation."
    )
    return await _s37_rewrite(question, answer, messages, ledger, deadline, order)

# --- _s37 verification stages (end) ---
...
async def _solve(query: Query, question: str) -> Response:
    ...
    try:
        if _is_usable_answer(answer):
            staged = await _s37_corroborate(question, answer, messages, ledger, deadline)
    ...
    try:
        if _is_usable_answer(answer):
            staged = await _s37_setgap(question, answer, messages, ledger, deadline)
            if _is_usable_answer(staged):
                answer = staged
    except Exception:
        pass
    ...
    try:
        if _is_usable_answer(answer):
            staged = await _s37_unit(question, answer, messages, ledger, deadline)
            if _is_usable_answer(staged):
                answer = staged
    except Exception:
        pass
