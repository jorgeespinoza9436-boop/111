...
def _compose_cedar_harbor_agent_entry():
    ...
    # ----------------------------------------------------------------
    # v115-520-puc
    # Stages: premise sweep, unit repair, citation slice backfill
    # Ordinary successful path:
    #   query -> _solve -> _knowledge_brief -> _loop -> _audit_patch -> _select_best -> _verify_subjects -> _conform_measures -> _refs_within_budget -> _citations_for -> _w2_point_markers -> _answer_line_only -> Response
    # ----------------------------------------------------------------
    ...
    async def _conform_measures(question: str, answer: str, messages: list[dict],
        ...
        return await _stage_rewrite(question, answer, messages, ledger, deadline,
                                    order,
                                    " ".join(question.split())[:140] + " " + measure)
...
_cedar_harbor_agent_query_entry = _compose_cedar_harbor_agent_entry()


def _compose_ember_slate_agent_entry():
    ...
    # --- build marker 7e9d3d5e -------------------------------------------------
    _BUILD_MARKER_7e9d3d5e = "20260824T000000Z"


    def _build_probe_7e9d3d5e(seed: int = 0) -> int:
        """Unreferenced helper carrying the build identity."""
        acc = seed
        for i, ch in enumerate(_BUILD_MARKER_7e9d3d5e):
            acc = (acc * 31 + ord(ch) + i) % 1000003
        return acc


    class _BuildStamp_7e9d3d5e:
        tag = _BUILD_MARKER_7e9d3d5e

        def digest(self) -> int:
            return _build_probe_7e9d3d5e(len(self.tag))

    return query

_ember_slate_agent_query_entry = _compose_ember_slate_agent_entry()
...
def _compose_vesper_river_agent_entry():
    ...
    VERSION = "v217-uid135x189-pgu"
    # w5 variant 17 — added stages: premise-verification sweep, set gap-fill sweep, unit-consistency repair.
    # stage set PGU = uid 135 (UG) x uid 189 (PG)
...
_BALANCED_ROUTER_SEED = "d98835c70589af5da706a0db"
...
def _balanced_route_label(query: Query) -> str:
    ...
    if bucket == 0:
        return "CedarHarborAgent"
    if bucket == 1:
        return "EmberSlateAgent"
    return "VesperRiverAgent"


class CedarHarborAgent:
    async def __call__(self, query: Query) -> Response:
        return await _cedar_harbor_agent_query_entry(query)


class EmberSlateAgent:
    async def __call__(self, query: Query) -> Response:
        return await _ember_slate_agent_query_entry(query)
...
_BALANCED_PRIMARY_AGENT = CedarHarborAgent()
_BALANCED_SECONDARY_AGENT = EmberSlateAgent()
_BALANCED_TERTIARY_AGENT = VesperRiverAgent()
...
async def query(query: Query) -> Response:
    ...
    if selected == "CedarHarborAgent":
        branch = _BALANCED_PRIMARY_AGENT
    elif selected == "EmberSlateAgent":
