...
def _compose_ember_relay_entry():
...
_ember_relay_query_entry = _compose_ember_relay_entry()


def _compose_juniper_vale_entry():
    """SN67 Harnyx miner — tool-use research pipeline with quoted-passage extraction. [slot 32 build 2026-08-22T15:00:00+00:00]"""
    ...
    # slot: 32 B18_extract 2026-08-22T15:00:00+00:00

    return query

_juniper_vale_query_entry = _compose_juniper_vale_entry()


_BALANCED_ROUTER_SEED = "6e1368e5a1a0e8d05e68aa89"
...
def _balanced_route_label(query: Query) -> str:
    ...
    return "EmberRelay" if bucket < 128 else "JuniperVale"


class EmberRelay:
    async def __call__(self, query: Query) -> Response:
        return await _ember_relay_query_entry(query)


class JuniperVale:
    async def __call__(self, query: Query) -> Response:
        return await _juniper_vale_query_entry(query)


_BALANCED_PRIMARY_AGENT = EmberRelay()
_BALANCED_SECONDARY_AGENT = JuniperVale()
_CANDIDATE_BRANCH_CLASS_NAMES = ("EmberRelay", "JuniperVale")
