...
BRIEF_MIN_USD = 0.03
AUDIT_MIN_USD = 0.05
WRAPUP_MIN_USD = 0.08    # 2026-08-21: 0.02 exhausted the $0.50 session budget mid-turn

# ── v56c answer sheet: the final answer is a structured commit, rendered by code
SHEET_MAX_MEMBERS = 40
SHEET_RESOLVE_ROUNDS = 1      # bounded re-entry when the committed sheet has open items
SHEET_RESOLVE_MIN_USD = 0.20  # a resolve round replays the transcript twice
SHEET_RESOLVE_MIN_S = 70.0    # a resolve round needs a search plus a re-commit
SHEET_TEXT_TIMEOUT_S = 24.0   # restructuring a prose answer into a sheet
...
COMMIT_TOOL = {
    ...
    "function": {
        "name": "commit_answer",
        "description": ("Submit the FINAL ANSWER as a structured sheet. This is the only "
                        "way to finish: the sheet is rendered into the answer text for "
                        "you. lead = the answer line itself (the exact entities/values/"
                        "list asked for, in the requested format, each claim followed by "
                        "its [n]); members = one entry per candidate-pool member with its "
                        "verdict and cited proof (every qualifier AND every member you "
                        "rule out); notes = computed values, conditions applied, or "
                        "caveats, each with its [n]."),
        "parameters": {
            "type": "object",
            "properties": {
                "lead": {"type": "string",
                         "description": ("the answer line: first words are the answer "
                                         "entities, in the requested shape, cited [n]")},
                "members": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "properties": {
                            "name": {"type": "string"},
                            "verdict": {"type": "string",
                                        "enum": ["qualifies", "excluded", "unresolved"]},
                            "proof": {"type": "string",
                                      "description": ("the deciding fact(s) for this member, "
                                                      "figures verbatim, each followed by its [n]")},
                            "sort_key": {"type": "string",
                                         "description": ("the value the question sorts or "
                                                         "ranks on, if any")},
                        },
                        "required": ["name", "verdict", "proof"],
                    },
                },
...
LOOP_RULES = (
    ...
    "FINISH: never mix research tool calls and the final answer in one turn. "
    "When the constraints are verified (or best-effort covered), finish by "
    "calling commit_answer with the complete cited sheet: the lead (answer "
    "line), one member entry per pool member with its verdict and cited "
    "proof, and notes for computed values. Plain text is NOT an answer; only "
    "the committed sheet is rendered and returned."
)


def _wrapup_order(seconds_left: float) -> str:
    return (
        f"TIME IS UP (~{int(seconds_left)}s left). No more research tool calls. "
        "Call commit_answer NOW with the complete final sheet from the numbered "
        "results above plus your knowledge: the lead's FIRST words are the answer "
        "entities (no 'Based on…' preamble, no 'partial answer' framing, no "
        "'(verify)' markers), cite [n] on every member proof and note, keep the "
        "required format. A cited partial sheet scores; a refusal or a remark "
        "about insufficient evidence scores zero."
    ...
    for lane_model in ((LLM_LANE_A, LOOP_MODEL_A, True),
        ...
        try:
            ...
            payload = await asyncio.wait_for(llm_chat(
                ...
                model=model,
                messages=messages,
                tools=LOOP_TOOLS if (force_tools or not finish_only) else [COMMIT_TOOL],
                tool_choice="auto" if (force_tools or not finish_only) else _COMMIT_CHOICE,
...
# ── v56c answer sheet ────────────────────────────────────────────────────────
# The loop no longer returns whatever prose the model last wrote. It finishes by
# COMMITTING a sheet (lead / per-member verdicts / notes) through a tool; code
# checks the sheet against the ledger, sends the model back to research when it
# has open members or uncited proof, and RENDERS the accepted sheet into the
# answer text. The renderer, not the model's message, is the answer authority.
class AnswerSheet:
...
def _sheet_from_obj(obj) -> AnswerSheet | None:
    ...
    for item in (raw_members if isinstance(raw_members, list) else []):
        ...
        members.append({"name": name[:200], "verdict": verdict,
                        "proof": proof[:1500], "sort_key": key[:80]})
...
async def _sheet_from_text(question: str, text: str, deadline: float) -> AnswerSheet | None:
    ...
    if left < 16.0 or _spend_left() < WRAPUP_MIN_USD:
        return None
    ask = (
        "Restructure this answer into a sheet WITHOUT changing, adding or dropping "
        "any claim or any [n] marker. JSON only: "
        '{"lead": "<the answer line: the exact entities/values/list asked for, with its [n]>", '
        '"members": [{"name": "<pool member>", "verdict": "qualifies|excluded|unresolved", '
        '"proof": "<its deciding facts, verbatim from the answer, with their [n]>", '
        '"sort_key": "<the value sorted or ranked on, or empty>"}], '
        '"notes": ["<any remaining cited sentence: computed values, conditions, caveats>"]}. '
        "Every sentence of the answer lands in exactly one field; keep figures and "
        "[n] verbatim.\n\n"
        f"Question:\n{question}\n\nAnswer:\n{text[:12000]}"
    )
    try:
        raw = await _chat_simple(LLM_LANE_A, SCHEMA_MODEL,
                                 "You restructure text into JSON. JSON only.", ask,
                                 max_tokens=3000,
                                 timeout=max(8.0, min(SHEET_TEXT_TIMEOUT_S, left - 10.0)))
    ...
    if sheet is None:
        return None
    before = len(_cited_numbers(text, 10_000))
    after = len(_cited_numbers(_sheet_cites(sheet), 10_000))
    if after * 10 < before * 8:
        return None          # the restructure lost citations: a regression
    return sheet
...
def _sheet_issues(sheet: AnswerSheet, ledger: EvidenceLedger, set_question: bool) -> list[str]:
    ...
    for m in sheet.members:
        if m["verdict"] == "unresolved":
            issues.append(f"member '{m['name']}' is unresolved: settle its condition from a "
                          "source, or keep it as a qualifier with its best verified fact")
    ...
    if set_question and not sheet.members:
        issues.append("this is a set question but the sheet lists no pool members: add one "
                      "entry per candidate with its verdict")
    return issues[:6]
...
def _resolve_order(issues: list[str]) -> str:
    return ("# commit_answer: NOT accepted — open items on the sheet:\n- "
            + "\n- ".join(issues)
            + "\nUse at most 3 research tool calls to settle them (search the deciding "
              "fact, open the page, retain the quote), then call commit_answer again with "
              "the COMPLETE sheet — every member, every [n]. If an item cannot be settled, "
              "keep the member as a qualifier with its strongest verified fact and commit.")
...
async def _loop(question: str, brief: str, ledger: EvidenceLedger,
                ...
                ) -> tuple[str, AnswerSheet | None, list[dict]]:
    ...
    for turn in range(1, turn_cap + 1):
        ...
        if not calls:
            ...
            if not _is_usable_answer(candidate):
                ...
                answer = ""
                break
            # prose instead of a commit: structure it, then render like any sheet
        ...
        # the commit is judged AFTER this turn's research landed, so its [n] resolve
        for call in commits:
            parsed = _sheet_from_call(call)
            if parsed is None:
                messages.append({"role": "tool", "tool_call_id": call.id,
                                 "content": ("# commit_answer: unreadable sheet — call it "
                                             "again with lead (string) and members (array)")})
                continue
            issues = _sheet_issues(parsed, ledger, set_q)
            if (issues and resolves_left > 0 and not finish_only
                    and (deadline - monotonic()) > SHEET_RESOLVE_MIN_S
                    and _spend_left() >= SHEET_RESOLVE_MIN_USD):
                resolves_left -= 1
                sheet = parsed
                messages.append({"role": "tool", "tool_call_id": call.id,
                                 "content": _resolve_order(issues)})
                continue
            sheet = parsed
            answer = _render_sheet(parsed, ledger, question)
            messages.append({"role": "tool", "tool_call_id": call.id,
                             "content": "# commit_answer: accepted — the sheet is the final answer"})
        if answer:
            break
...
async def _audit_patch(question: str, answer: str, messages: list[dict],
    ...
    patched, _, _ = await _loop(question, "", ledger, deadline,
...
# ── claim-binding table: the representation that carries evidence from the
# ── answer stage to the delivery stage (edit 8; owns every name it reads) ─────
#
# The base carries evidence between stages as ledger ROWS: a cited marker [n]
# resolves to row n-1 and the row's shown/retained spans become the citation.
# The claim is nowhere in that path -- the row knows which characters the model
# saw, never which sentence rests on them. Everything downstream (the citation
# array, the [[k]] map, the public note) is then rebuilt by re-parsing the answer
# text a second and a third time, each pass repeating the same marker scan.
#
# This table is built once, between the answer and the delivery path, and binds
# each cited CLAIM to the row that supports it and to the values it asserts. The
# citation builder and the note builder both read the table instead of re-parsing
# the answer, so the binding -- not the row -- is what crosses the stage boundary.
#
# The reference answers we are scored against bind evidence the same way:
# domain_tweak_generation/source_evidence.py emits an excerpt capped at
# _MAX_CITATION_SOURCE_EXCERPT_CHARS = 2000 that ends in an explicit
# "Supports: <claim>" binding. Narrowing our own slices onto the bound claim is
# what the table exists to make possible.
#
# ADOPTION IS GATED. This build binds and routes; it does not yet narrow. Every
# slice is still the one `EvidenceLedger.ref_for` computes, byte for byte, so
# the shipped citations are identical to the base's and the table cannot cost a
# point. `_CB_ADOPT_NARROW` turns narrowing on once it has been measured.
_CB_ADOPT_NARROW = False          # gate: slice selection defers to the ledger
_CB_TARGET_SPAN_CHARS = 2000      # the reference's own per-excerpt cap
_CB_MIN_SPAN_CHARS = 700          # below this a slice is a sliver, not evidence
# No claim cap. An earlier build stopped binding after N sentences and lost the
# note whenever the decisive claim sat past N -- the exact failure the table is
# meant to prevent. The only bound is a sanity ceiling far above any answer the
# response cap can carry; reaching it would already mean a malformed answer.
_CB_BINDING_CEILING = 20_000
_CB_CLAIM_CHARS = 300
_CB_SENT_SPLIT_RE = re.compile(r"(?<=[.!?])\s+|\n+")
_CB_MARKER_RE = re.compile(r"\[([0-9][0-9,\s\-]*)\]")
_CB_VALUE_RE = re.compile(r"\d[\d,.]*%?|[A-Z][A-Za-z0-9&'’.\-]{2,}")
_CB_VALUE_STOP = frozenset(
    "The And For With From That This Have Has Was Were Are Is Been Its Their "
    "Both First Second Answer Note Source Sources Evidence Based".split()
)


class _CbBinding:
    """One cited claim, the ledger row that supports it, and its asserted values."""

    __slots__ = ("marker", "markers", "claim", "values")

    def __init__(self, marker: int, markers: tuple, claim: str, values: tuple) -> None:
        self.marker = marker
    ...
    def supports(self, text: str) -> bool:
        """Does the row's text actually state what this claim asserts?"""
        if not self.values or not text:
            return False
        low = text.casefold()
        return all(v.casefold() in low for v in self.values)


class _CbTable:
    """The evidence state that crosses from the answer stage to delivery.

    Built once from the answer and the ledger. `order()` is the marker sequence
    the citation array is assembled from; `claims_for()` is what the public note
    is written from. Neither consumer re-parses the answer.
    """

    __slots__ = ("bindings", "by_marker", "spine", "_ledger")

    def __init__(self, bindings: list, spine: list, ledger) -> None:
    ...
    def _index(self) -> None:
        """Marker -> bindings. Never raises: an unusable binding is dropped, and
        the citation array does not depend on this index at all."""
        index: dict = {}
        try:
            for b in self.bindings:
                index.setdefault(b.marker, []).append(b)
        except Exception:
            index = {}
        self.by_marker = index

    def order(self) -> list:
        """Cited markers in first-appearance order — the citation array's spine.

        Carried verbatim from the marker scan so the array the table assembles is
        the array the base assembled: same markers, same order, same duplicates
        already collapsed.
        """
        try:
    ...
    def claims_for(self, positions: dict) -> list:
        """Bound claim sentences whose row shipped a citation, in answer order.

        One entry per SENTENCE (not per marker): a sentence citing three rows is
        one claim, exactly as the note has always rendered it.
        """
        out = []
        try:
            for b in self.bindings:
                if any(positions.get(m) for m in b.markers):
                    out.append(b)
        except Exception:
            return []
        return out

    def rebind(self, text: str) -> None:
        """Re-bind the claims onto the text actually being delivered.

        The citation array is fixed the moment the spine is read, but the answer
        is still reshaped afterwards (bracket normalisation, lead stripping, the
        output-only line). Re-binding keeps the claim side of the table pointing
        at the sentences that will really ship, without disturbing the array.
        """
        try:
            self.bindings = _cb_bindings(text)
        except Exception:
            return          # keep the previous binding set; never lose the answer
        self._index()

    def narrow_slice(self, marker: int, shown: list) -> tuple:
        """The tightest region of the row that carries this claim's values.

        Returns (start, end) or (). Only ever a region the model was SHOWN, so a
        narrowed citation can never make a claim dangle outside the materialized
        slice. Consulted only when `_CB_ADOPT_NARROW` is on.
        """
        if not _CB_ADOPT_NARROW:
            return ()
        rows = getattr(self._ledger, "rows", None) or []
        if not (1 <= marker <= len(rows)):
            return ()
        text = rows[marker - 1].get("text") or ""
        if not text:
            return ()
        best = ()
        best_width = 0
        for b in self.by_marker.get(marker, ()):
            if not b.supports(text):
                continue
            low = text.casefold()
            hits = []
            for v in b.values:
                i = low.find(v.casefold())
                if i < 0:
                    hits = []
                    break
                hits.append((i, i + len(v)))
            if not hits:
                continue
            lo = min(h[0] for h in hits)
            hi = max(h[1] for h in hits)
            if hi - lo > _CB_TARGET_SPAN_CHARS:
                continue
            pad = max(0, (_CB_TARGET_SPAN_CHARS - (hi - lo)) // 2)
            start = max(0, lo - pad)
            ...
            for s, e in shown:
                if start >= s and end <= e:
                    inside = True
                    break
            if not inside or (end - start) < _CB_MIN_SPAN_CHARS:
                continue
            if best and (end - start) >= best_width:
                continue
            best = (start, end)
...
def _cb_values(sentence: str) -> tuple:
    """The values a claim asserts, in the forms a source writes them."""
    out = []
    for m in _CB_VALUE_RE.finditer(sentence or ""):
        v = m.group(0).strip(".,;:")
        if len(v) < 2 or v in _CB_VALUE_STOP:
            continue
        if v not in out:
            out.append(v)
        if len(out) >= 6:
...
def _cb_build(answer: str, ledger) -> _CbTable:
    """Bind every cited claim in the answer to the ledger rows that support it.

    The spine is taken from the whole-answer marker scan, never from the bound
    sentences: a marker sitting in a fragment too short to be a claim still owns
    its citation position. Binding enriches the path, it does not gate it.
    """
    rows = getattr(ledger, "rows", None) or []
    try:
        spine = _cited_numbers(answer or "", len(rows))
    except Exception:
        spine = []          # exactly what the base ships when its own scan fails
    try:
        bindings = _cb_bindings(answer)
    except Exception:
        bindings = []       # no claims bound: the array still ships, note goes quiet
    return _CbTable(bindings, spine, ledger)


def _cb_bindings(text: str) -> list:
    """One binding per cited sentence, in the order the reader meets them."""
    bindings: list = []
    try:
        body = _normalize_brackets(text or "")
    except Exception:
        body = text or ""
    for raw in _CB_SENT_SPLIT_RE.split(body):
        sent = " ".join(raw.split()).strip("-*• ").strip()
        if len(sent) < 12 or not _CB_MARKER_RE.search(sent):
            continue
        markers = tuple(_cited_numbers(sent, 10_000))
        if not markers:
            continue
        bindings.append(_CbBinding(markers[0], markers, sent, _cb_values(sent)))
        if len(bindings) >= _CB_BINDING_CEILING:
            break
    return bindings


def _cb_refine(ref, table: _CbTable, marker: int, row: dict):
    """Narrow a citation onto the claim the table bound it to.

    Identity by default: with `_CB_ADOPT_NARROW` off — and whenever the table has
    no strictly tighter region that the model was actually shown — the ledger's
    own ref is returned untouched, so the shipped array is the base's array.
    """
    if not _CB_ADOPT_NARROW:
        return ref
    try:
        slices = getattr(ref, "slices", None) or []
        shown = [(int(s.start), int(s.end)) for s in slices]
        if not shown:
            return ref
        window = table.narrow_slice(marker, shown)
        if not window:
            return ref
        start, end = window
        if (end - start) >= sum(max(0, e - s) for s, e in shown):
            return ref
        return CitationRef(receipt_id=row["receipt_id"],
...
def _pointer_refs(table: _CbTable, ledger: EvidenceLedger) -> tuple[list[CitationRef], dict[int, int]]:
    ...
    for n in table.order():
        ...
        if ref is None:
            continue
        row = ledger.rows[n - 1]
        ref = _cb_refine(ref, table, n, row)
...
def _pointerize(text: str, positions: dict[int, int]) -> str:
    ...
    def fix(m):
        seen: list[int] = []
        for n in _cited_numbers(m.group(0), 10_000):
            k = positions.get(n)
...
def _evidence_note(table: _CbTable, proof: str, shipped, positions: dict[int, int]) -> str | None:
    ...
    lines: list[str] = []
    for _bound in table.claims_for(positions):
        sent = _bound.claim
...
def _respond(*, text=None, output=None, citations=None, note=None) -> Response:
    ...
    # v-422: explicit keywords only -- call-site ** expansion is rejected by the
    # upload gate (expanded_keywords). Response has exactly three fields
    # (text/output/citations, all defaulting to None) and no `note` field in this
    # SDK, so the note branch can never fire; it is kept as a literal-keyword
    # call so behaviour is unchanged if one is ever added.
...
_REPAIR_ORDER = (
    ...
    "Call commit_answer now: lead = the answer entities themselves, one member "
    "entry per pool member with its cited proof, notes for the rest. Nothing else."
)
...
# ── research controller: a declared plan an executor drives ───────────────────
#
# The base runs research as straight-line code: seven stages written one after
# another inside `_solve`, each with its precondition inlined as an `if` and its
# failure policy inlined as its own `try`. Nothing in the artifact can name a
# stage, ask what ran, or state the order except by reading that function.
#
# Here the sequence, the preconditions and the failure policies are DATA. The
# stages no longer decide when they run or what a failure means -- `_plan_drive`
# does, reading `_RESEARCH_PLAN`. That is the whole of the controller: `_solve`
# holds no research control flow at all any more, only the delivery path.
#
# The plan below is the base's order, the base's preconditions, and the base's
# failure policies, stage for stage -- including the two rungs the base leaves
# UNGUARDED. Making those two catch would be an improvement, and an improvement
# is a behaviour change; they are declared `_PL_RAISE` so they keep escaping to
# the entrypoint handler exactly as before.
#
# `_PL_ADAPTIVE` is the gate. Off, the executor walks the plan once, in order:
# the controller is new, the control flow is the base's. On, the executor may
# re-plan from what the run has already produced.
_PL_ADAPTIVE = False

_PL_PASS = "pass"            # swallow the failure, change nothing
_PL_BLANK_BRIEF = "blank_brief"     # swallow, then brief = ""
_PL_BLANK_ANSWER = "blank_answer"   # swallow, then answer = ""
_PL_RAISE = "raise"          # the base guards this rung with nothing


class _RunState:
    """Everything the research stages read and write, in one place.

    The base kept this as seven locals in `_solve`; a driven plan needs them
    addressable, because a stage is now a value rather than a line of code.
    """

    __slots__ = ("question", "deadline", "draft", "brief", "ledger",
                 "answer", "sheet", "messages", "ran", "skipped")

    def __init__(self, question: str, deadline: float, ledger) -> None:
...
class _PlanStage:
    """A named unit of research work: when it may run, what it does, how it fails."""

    __slots__ = ("name", "ready", "work", "on_error")

    def __init__(self, name: str, ready, work, on_error: str) -> None:
        self.name = name
        self.ready = ready
        self.work = work
...
def _pl_always(st: _RunState) -> bool:
    """No precondition of its own — the base inlines the test inside the guard."""
    return True


def _pl_needs_rescue(st: _RunState) -> bool:
    """Nothing usable yet, but there is evidence to build an answer from.

    Evaluated OUTSIDE the stage guard, exactly where the base evaluates it, so a
    failure here escapes rather than being read as `not ready`.
    """
    return bool(not _is_usable_answer(st.answer) and st.ledger.rows)


def _pl_needs_answer(st: _RunState) -> bool:
    """Nothing usable yet, with or without evidence — the last rung."""
    return bool(not _is_usable_answer(st.answer))


async def _pl_tooling(st: _RunState) -> None:
    info = await tooling_info(timeout=10.0)
    _spend_note(info)


async def _pl_brief(st: _RunState) -> None:
    if _spend_left() >= BRIEF_MIN_USD and (st.deadline - monotonic()) > 120.0:
        st.draft, st.brief = await _knowledge_brief(st.question)


async def _pl_research(st: _RunState) -> None:
    st.answer, st.sheet, st.messages = await _loop(
        st.question, st.brief, st.ledger, st.deadline, MAX_TURNS)


async def _pl_audit(st: _RunState) -> None:
    if _is_usable_answer(st.answer) and (st.deadline - monotonic()) > 75.0 \
            and _spend_left() >= AUDIT_MIN_USD:
        patched = await _audit_patch(st.question, st.answer, st.messages,
                                     st.ledger, st.deadline)
        # the patch loop can itself return junk — only take it if it passes
        if _is_usable_answer(patched):
            st.answer = patched


async def _pl_digest(st: _RunState) -> None:
    # v32.4 RESCUE LADDER rung 1 — rewrite from the clean evidence digest
    # (min reasoning, no tools). Every rung is cited; none advertises failure.
    rescued = await _write_from_digest(st.question, st.ledger, st.deadline)
    if _is_usable_answer(rescued):
        st.answer = rescued


async def _pl_deterministic(st: _RunState) -> None:
    # rung 2 — deterministic, CITED, zero-LLM. F4: this must come BEFORE the
    # knowledge draft — the draft is written pre-research and carries no [n] at
    # all, so it passed the floor and permanently shadowed the only cited rung.
    det = _deterministic_answer(st.question, st.ledger)
    if _is_usable_answer(det):
        st.answer = det


async def _pl_resort(st: _RunState) -> None:
    # rung 3 — last resort: model knowledge (uncited, but better than nothing)
    fallback = _sanitize_draft(st.draft) or await _knowledge_resort(st.question,
                                                                    st.deadline)
    if _is_usable_answer(fallback):
        st.answer = fallback          # F4: never destroy a usable answer with ""


_RESEARCH_PLAN = (
    _PlanStage("tooling", _pl_always, _pl_tooling, _PL_PASS),
    _PlanStage("brief", _pl_always, _pl_brief, _PL_BLANK_BRIEF),
    _PlanStage("research_loop", _pl_always, _pl_research, _PL_BLANK_ANSWER),
    _PlanStage("audit_patch", _pl_always, _pl_audit, _PL_PASS),
    _PlanStage("digest_rescue", _pl_needs_rescue, _pl_digest, _PL_PASS),
    _PlanStage("deterministic", _pl_needs_rescue, _pl_deterministic, _PL_RAISE),
    _PlanStage("knowledge_resort", _pl_needs_answer, _pl_resort, _PL_RAISE),
)


async def _plan_drive(plan, st: _RunState) -> _RunState:
    """Walk the plan. The stages do the work; this decides what running means.

    A stage whose precondition is false is skipped without cost. A stage that
    fails is handled by the policy IT declared, never by a policy this executor
    assumes — which is why the two unguarded rungs stay unguarded.
    """
    for stage in plan:
        if not stage.ready(st):
            st.skipped.append(stage.name)
            continue
        if stage.on_error == _PL_RAISE:
            await stage.work(st)
            st.ran.append(stage.name)
            continue
        try:
            await stage.work(st)
        except Exception:
            if stage.on_error == _PL_BLANK_BRIEF:
                st.brief = ""
            elif stage.on_error == _PL_BLANK_ANSWER:
                st.answer = ""
        st.ran.append(stage.name)
        if _PL_ADAPTIVE:
            plan = _pl_replan(plan, stage, st)
    return st


def _pl_replan(plan, done: _PlanStage, st: _RunState):
    """Gated: with `_PL_ADAPTIVE` off this is never reached and the plan is fixed."""
    return plan
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # the research controller: seven declared stages, one executor. `_solve`
    # keeps no research control flow of its own — see `_RESEARCH_PLAN`.
    _st = await _plan_drive(_RESEARCH_PLAN,
                            _RunState(question, deadline, EvidenceLedger()))
    ledger = _st.ledger
    answer = _st.answer


    positions: dict[int, int] = {}
    _cb_tab = _cb_build(answer, ledger)
    try:
        citations, positions = _pointer_refs(_cb_tab, ledger)
    except Exception:
        citations, positions = [], {}
    ...
    proof = answer
    # the array is fixed; re-bind the claim side onto the text that will ship
    _cb_tab.rebind(proof)
    # after _pointer_refs: the citation array keeps the proof section's [n]
    answer = _answer_line_only(answer, question)
    # the judge's pointer grammar: [[k]] = submitted citation k; a bare [n] is content.
    # An output-only line ships without markers, so its proof travels in the note.
    note = _evidence_note(_cb_tab, proof, None, positions) if answer is not proof else None
    ...
    if query.output_schema is not None:
        ...
        if structured is not None:
            ...
            try:
                ...
                pass
            try:
                return _respond(output=structured, citations=citations or None,
                                note=_evidence_note(_cb_tab, proof, structured, positions))
            except Exception:
        ...
        if basis is not answer:
            ...
            if salvaged is not None:
                try:
                    ...
                    pass
                try:
                    return _respond(output=salvaged, citations=citations or None,
                                    note=_evidence_note(_cb_tab, proof, salvaged, positions))
                except Exception:
                    pass
        ...
        try:
            ...
            try:
                return Response(output=_fit_string(_cap(basis), query.output_schema, 2000),
