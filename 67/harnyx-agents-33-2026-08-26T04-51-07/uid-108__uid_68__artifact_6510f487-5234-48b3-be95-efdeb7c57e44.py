...
VERSION = "v225-uid81x236-rtu"
# w5 variant 25 — added stages: roster pre-pass, temporal-alignment repair, unit-consistency repair.
# stage set RTU = uid 81 (UT) x uid 236 (RU)
...
# --- stage T: temporal-alignment repair (uid 53 / 81 / 137 lineage) ---
_W5T_MIN_TAIL_S = 66.0
_W5T_MAX_YEARS = 4


class _TemporalAnchors:
    """Years the question anchors on, against years the evidence covers."""

    def __init__(self, anchors: list, uncovered: list) -> None:
...
def _temporal_anchors(question: str, ledger: EvidenceLedger) -> _TemporalAnchors:
    """Deterministic: an anchor year absent from all evidence is a mismatch."""
    anchors = []
    for match in _W5_YEAR_RE.finditer(question or ""):
        year = match.group(0)
        if year not in anchors:
            anchors.append(year)
        if len(anchors) >= _W5T_MAX_YEARS:
            break
    if not anchors or not ledger.rows:
        return _TemporalAnchors(anchors, [])
    blob = _w5_ledger_blob(ledger)
    return _TemporalAnchors(anchors, [y for y in anchors if y not in blob])


def _w5t_accept(draft: str, revision: str, anchors: list) -> bool:
    """The revision must end up carrying the anchor years it was sent to fix."""
    if not _is_usable_answer(revision):
        return False
    if not _w5_keeps_entities(draft, revision):
        return False
    if not any(year in revision for year in anchors):
        return False
    return _w5_long_enough(draft, revision)


async def _temporal_repair(question: str, answer: str, messages: list,
                           ledger: EvidenceLedger, deadline: float) -> str:
    """Stage — the answer is cited to the wrong period.

    A figure that is correct for 2023 and cited for a question about 2019 reads
    as a sourced answer and scores as a wrong one, so the anchor years are
    checked against the evidence rather than against the prose.
    """
    if not _is_usable_answer(answer) or not ledger.rows:
        return answer
    anchors = _temporal_anchors(question, ledger)
    if anchors.is_aligned():
        return answer
    if (deadline - monotonic()) < _W5T_MIN_TAIL_S or _spend_left() <= WRAPUP_MIN_USD:
        return answer

    missing = anchors.uncovered[:2]
    body = await _w5_targeted_search(
        _w5_search_subject(question) + " " + " ".join(missing), ledger)
    order = (
        "TEMPORAL ALIGNMENT: the question anchors on " + ", ".join(missing) +
        " and no retrieved source covers that period. Every figure you report "
        "must be cited to a source that states it FOR THE ANCHOR PERIOD — a "
        "value from an adjacent year presented as the answer is wrong, not "
        "approximate. Use the evidence just retrieved. Where only an adjacent "
        "period is available, say which period each figure is actually from and "
        "cite it as such. Keep every entity and citation you already support, "
    ...
    revised = await _w5_bounded_rewrite(question, messages, ledger, deadline,
                                        order, body)
    return revised if _w5t_accept(answer, revised, anchors.anchors) else answer
...
async def _solve(query: Query, question: str) -> Response:
    ...
    try:
        _staged = await _temporal_repair(question, answer, messages, ledger, deadline)
...
_PERFECT_SUFFIX = "b84fb21ad3e98a1e"
