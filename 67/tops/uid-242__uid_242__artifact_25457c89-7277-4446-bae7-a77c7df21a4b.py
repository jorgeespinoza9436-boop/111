...
MAX_TURNS = 15  # the field runs 14-16; 13 was the most turn-starved in the class
AUDIT_EXTRA_TURNS = 2
...
# supporting text, and it reads only the spans we cite. Slices are materialized
# platform-side from the stored tool result, so widening costs us no tokens or
# latency -- but the validator rejects the whole response past 120k materialized
# characters, so the total stays budgeted.
...
def _search_extras(provider: str, plan: QuestionPlan | None) -> list[dict | None]:
    ...
    """
    if provider != "parallel" or plan is None or not plan.domains:
        return [None]
    pinned = {"mode": "advanced", "source_policy": {"include_domains": list(plan.domains)}}
    return [pinned, None]
...
def _wants_second_opinion(plan: QuestionPlan) -> bool:
    ...
    nothing citable, and Parallel always returns something, so desearch never
    ran once in production (batch 7af93041: every search cost row is parallel).
    A question naming an official source is where a second index actually pays,
    and _take_extra_call keeps it to one call for the whole task.
    """
    return bool(plan.domains) and "desearch" in _search_providers() and _take_extra_call("second_opinion")


async def _second_opinion_rows(query_text: str, num: int) -> list[dict]:
    """Citable rows from desearch for the same query, or none."""
    try:
        payload = await search_web(query_text, provider="desearch", num=num, timeout=SEARCH_TIMEOUT_S)
    except Exception:
...
def _schema_skeleton(schema: object, depth: int = 0) -> object:
    ...
    if kind == "object":
        ...
        return {
            key: _schema_skeleton(properties.get(key) or {}, depth + 1)
            for key in required
...
def _shape_candidates(value: object, ledger: EvidenceLedger, guess: str) -> list[object]:
...
async def _solve(query: Query, question: str) -> Response:
    ...
    if query.output_schema is not None:
        ...
        def _ship(value: object) -> Response | None:
            ...
            if value is None:
                return None
            for shaped in _shape_candidates(value, ledger, guess):
        ...
        if shipped is not None:
            return shipped
        # Last rung. The old fallback here shipped a bare string, which no object
        # schema accepts, so the response was discarded and the task scored zero
        # instead of scoring badly. A skeleton built from the schema is at least
        # gradeable.
