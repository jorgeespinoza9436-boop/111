...
def _compose_lumen_anvil_agent_entry():
    ...
    # ── CICLO CONTRATO→BUSCA→REVERIFICACAO (23/08) ───────────────────────────
    # No wrapper w4 a auditoria flui UMA VEZ para frente: planeja contrato,
    # pesquisa, verifica, entrega. A rubrica do juiz de duplicata diz, literal:
    # "Passing an intermediate result forward once is not a cycle."
    #
    # Aqui a auditoria ganha uma ARESTA DE VOLTA: quando, DEPOIS de verificada,
    # a resposta ainda nao menciona um elemento que o contrato marcou como
    # obrigatorio, o estagio faz UMA busca dirigida a esse elemento e
    # re-verifica com a evidencia nova. Ciclo entre estagios — o resultado da
    # revisao reabre a recuperacao de evidencia, coisa que o wrapper nao fazia.
    #
    # 🚨 CAMINHO COMUM INALTERADO: exige TRES condicoes em serie — contrato
    # construido, verificacao executada, e um requisito ainda SEM NENHUM token
    # de conteudo presente no texto auditado. O verificador acabou de ser
    # instruido a preencher lacunas, entao sobreviver a ele e raro.
    # Teto de UM ciclo; guardas de tempo proprias; qualquer falha devolve a
    # resposta anterior intacta.
    _CY_MIN_LEFT_S = 55.0
    _CY_STOP = {"the","a","an","of","for","and","or","in","on","to","its","that",
                "must","state","each","every","answer","value","values","name",
                "names","with","from","by","as","is","are","be","this","those"}

    def _cy_tokens(texto: str) -> list:
        return [w for w in re.findall(r"[A-Za-z][A-Za-z0-9\-]{2,}", texto or "")
                if w.lower() not in _CY_STOP][:6]

    def _cy_unmet(contract, answer: str) -> list:
        """Requisitos do contrato sem NENHUM token de conteudo na resposta."""
        try:
            req = getattr(contract, "required", None) or []
        except Exception:
            return []
        baixo = (answer or "").lower()
        faltando = []
        for item in req:
            toks = _cy_tokens(item)
            if not toks:
                continue
            if not any(t.lower() in baixo for t in toks):
                faltando.append(item)
        return faltando[:1]

    async def _cy_close_gap(contract, question: str, answer: str, deadline: float) -> str:
        if _w4_remaining(deadline) < _CY_MIN_LEFT_S:
            return answer
        falta = _cy_unmet(contract, answer)
        if not falta:
            return answer
        alvo = falta[0]
        try:
            achado = await search_web(
                search_queries=[" ".join(_cy_tokens(question)[:5] + _cy_tokens(alvo)[:4])],
                provider=SEARCH_PROVIDER, num=4, timeout=SEARCH_TIMEOUT_S)
        except Exception:
            return answer
        trechos = []
        try:
            for it in (getattr(achado, "results", None) or [])[:4]:
                nota = getattr(it, "note", None) or getattr(it, "snippet", None) or ""
                if isinstance(nota, str) and nota.strip():
                    trechos.append(nota[:900])
        except Exception:
            pass
        if not trechos:
            return answer
        revisado = await _w4_chat(
            [{"role": "system", "content": _W2_VERIFY_SYSTEM},
             {"role": "user", "content": (
                 f"Question:\n{question}\n\nAnswer contract:\n{_w4_contract_block(contract)}"
                 f"\n\nThe draft never states this required element: {alvo}\n"
                 f"Fresh evidence gathered for it:\n" + "\n---\n".join(trechos) +
                 f"\n\nDraft answer:\n{answer[:_W2_DRAFT_PROMPT_CHARS]}")}],
            timeout=min(_W2_VERIFY_TIMEOUT_SECONDS,
                        _w4_remaining(deadline) - _W2_TAIL_RESERVE_SECONDS),
            temperature=_W2_VERIFY_TEMPERATURE)
        revisado = (revisado or "").strip()
        return revisado if _w4_accept_revision(answer, revisado) else answer
    ...
    async def query(query: Query) -> Response:
        ...
        if contract is not None:
            ...
            if draft:
                ...
                # aresta de volta: requisito ainda ausente reabre a busca
                try:
                    fechado = await _cy_close_gap(
                        contract, question, _w4_response_text(response) or audited, deadline)
                    if fechado and fechado != (_w4_response_text(response) or audited):
                        response = _w4_with_text(response, fechado)
                except Exception:
                    pass
