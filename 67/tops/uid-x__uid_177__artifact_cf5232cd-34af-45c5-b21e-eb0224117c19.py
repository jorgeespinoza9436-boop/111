...
_W6_VERSION = "v410-uid79xuid135xuid236-rgvu"
# w6-tri variant 10 on the uid_193 three-branch router.
# added stages (4): roster pre-pass, set gap-fill sweep, value-support repair, unit-consistency repair
# stage set RGVU = uid 79 (VU) x uid 135 (UG) x uid 236 (RU)
...
def _compose_lumen_anvil_agent_entry():
    ...
    # --- stage U: unit-consistency repair (uid 79 / 81 / 135 lineage) ---
    _W6U_MIN_TAIL_S = 58.0
    _W6U_DEMANDS = (
        ("USD", r"\b(?:in|to)\s+(?:usd|us dollars|dollars)\b|\bus\$", (r"\$", r"\busd\b", r"\bdollars?\b")),
        ("EUR", r"\b(?:in|to)\s+(?:eur|euros?)\b", (r"€", r"\beur\b", r"\beuros?\b")),
        ("millions", r"\bin millions\b|\bmillions of\b", (r"\bmillions?\b", r"\bm\b")),
        ("billions", r"\bin billions\b|\bbillions of\b", (r"\bbillions?\b", r"\bbn\b", r"\bb\b")),
        ("percent", r"\bpercent(?:age)?\b|\bwhat share\b|\bwhat proportion\b", (r"%", r"\bpercent\b")),
        ("per capita", r"\bper capita\b|\bper person\b|\bper head\b", (r"\bper capita\b", r"\bper person\b")),
        ("metric", r"\bin (?:kilometres|kilometers|km|kilograms|kg|metres|meters|tonnes)\b",
         (r"\bkm\b", r"\bkg\b", r"\bkilomet", r"\bkilogram", r"\bmet(?:re|er)s?\b", r"\btonnes?\b")),
        ("imperial", r"\bin (?:miles|pounds|lbs|feet|ft|inches)\b",
         (r"\bmiles?\b", r"\bpounds?\b", r"\blbs?\b", r"\bfeet\b", r"\bft\b", r"\binch")),
    )


    class _UnitDemand:
        """Units the question demanded against units the answer actually carries."""

        def __init__(self, required: list, missing: list) -> None:
    ...
    def _unit_demand(question: str, answer: str) -> _UnitDemand:
        """Deterministic scan; no model call, so a clean answer costs nothing."""
        q = " ".join((question or "").split())
        required = []
        missing = []
        for label, ask, marks in _W6U_DEMANDS:
            if not re.search(ask, q, re.I):
                continue
            required.append(label)
            if not any(re.search(mark, answer or "", re.I) for mark in marks):
                missing.append(label)
        return _UnitDemand(required, missing)


    async def _unit_repair(question: str, answer: str, ctx) -> str:
        """Stage — the question named a unit or scale the answer never used.

        No search: the evidence is already retrieved and the defect is one of
        presentation, so this stage buys a rewrite and nothing else. Rescaling is
        the point, so its guard permits figures to change while entities may not.
        """
        if not ctx.usable(answer):
            return answer
        demand = _unit_demand(question, answer)
        if demand.is_satisfied():
            return answer
        if ctx.tail() < _W6U_MIN_TAIL_S or not ctx.spend_ok():
            return answer

        order = (
            "UNIT CONSISTENCY: the question demands the answer be expressed in "
            + ", ".join(demand.missing) + " and no figure in the answer carries it. "
            "Restate every headline figure in the demanded unit or scale. Show the "
            "conversion basis where you converted — rate, year, source — and cite it. "
            "Never silently relabel a figure into a unit its source did not use. Keep "
            "every named entity and every existing citation, then return the COMPLETE "
            "final answer."
        )
        revised = await ctx.rewrite(question, order)
        if not ctx.usable(revised):
            return answer
        if not _w6_keeps_entities(answer, revised) or not _w6_long_enough(answer, revised):
            return answer
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        if _w6_gov.admits("value_repair"):
            try:
                ...
                pass
        if _w6_gov.admits("unit_repair"):
            try:
                _w6_was = answer
                _w6_out = await _unit_repair(question, answer, _w6_ctx)
                if _is_usable_answer(_w6_out):
                    answer = _w6_out
                _w6_gov.note("unit_repair", _w6_was, answer)
            except Exception:
                pass
            ...
            if display:
                ...
                if _w6_gov.admits("unit_repair"):
                    try:
                        _w6_was = decided
                        _w6_ctx.stage_draft(decided)
                        _w6_out = await _unit_repair(query.text, decided, _w6_ctx)
                        if _w6_ctx.usable(_w6_out):
                            decided = _w6_out
                        _w6_gov.note("unit_repair", _w6_was, decided)
                    except Exception:
                        pass
