...
# --- stage: authority sweep -----------------------------------------------
# A figure sourced only to an aggregator or a news rewrite is one hop away from
# the body that published it, and the hop is where the number drifts. This
# checks whether anything the answer cites is an official publisher and spends
# one search on the primary source when nothing is.
...
async def _authority_sweep(
    ...
    if _has_authority(answer, index):
        return answer
    await _stage_search(
        _stage_terms_query(terms, "official statistics report publication"), index, deadline,
    )
    directive = (
        "Nothing the draft cites is an official or primary publisher. If the "
        "evidence now contains an official source for the reported figures, cite "
        "it and prefer its value where the two disagree. If it does not, keep the "
        "answer as it stands and do not weaken it."
    )
    return await _stage_rewrite(question, answer, index, directive, deadline)
...
async def _plain_query(query: Query, budget: float) -> Response:
    ...
    try:
        ...
        # --- ADDED STAGES: temporal-alignment repair then authority sweep ---
