...
def _compose_garnet_talon_slot01_agent_entry():
...
_garnet_talon_slot01_agent_query_entry = _compose_garnet_talon_slot01_agent_entry()


def _compose_umber_orchid_slot01_agent_entry():
    ...
    # ---- v240-5-flzu ----
    # Added: research-loop fallback model, list-first roster directive, corroboration audit, unit conformance audit
    # Ordinary successful path:
    #   query -> _plain_query -> briefing -> ROSTER_DIRECTIVE -> research loop (+_chat_turn_fallback) -> _relocate -> checkpoint -> _commit_call -> forced retry -> _audit_corroboration -> _audit_measure -> _amended_answer -> _deliverable -> Response
    ...
    async def _chat_turn_fallback(
        ...
        try:
            return await llm_chat(
                provider=LLM_PROVIDER, model=RESEARCH_FALLBACK_MODEL,
    ...
    def _audit_corroboration(display: str, index: "_ResultIndex") -> str:
        ...
        if len(backers) != 1:
            return ""
        return ("\n\nCORROBORATION: the decisive figure " + lead + " rests on a "
                "single source. Cite an independent one as well. If they agree, "
                "cite both; if they disagree, give both figures with their markers "
        ...
        try:
            ...
            while turn < RESEARCH_TURN_CAP and perf_counter() < research_stop:
                ...
                if chat_result is None:
                    # Primary model exhausted its retries. The commit path already
                    # has a fallback model; the research path had none, so this
                    # otherwise ends research early with a thinner index.
                    chat_result = await _chat_turn_fallback(
                        messages, deadline=research_stop, thinking_on=thinking_on)
                if chat_result is None:
                    break
...
_umber_orchid_slot01_agent_query_entry = _compose_umber_orchid_slot01_agent_entry()


def _compose_indigo_vector_slot01_agent_entry():
...
_indigo_vector_slot01_agent_query_entry = _compose_indigo_vector_slot01_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "GarnetTalonSlot01Agent"
    ...
    if (
        ...
        return "UmberOrchidSlot01Agent"
    return "IndigoVectorSlot01Agent"


class GarnetTalonSlot01Agent:
    async def __call__(self, query: Query) -> Response:
        return await _garnet_talon_slot01_agent_query_entry(query)


class UmberOrchidSlot01Agent:
    async def __call__(self, query: Query) -> Response:
        return await _umber_orchid_slot01_agent_query_entry(query)


class IndigoVectorSlot01Agent:
    async def __call__(self, query: Query) -> Response:
        return await _indigo_vector_slot01_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = GarnetTalonSlot01Agent()
_ANALYTICAL_FIELD_AGENT = UmberOrchidSlot01Agent()
_BROAD_FIELD_AGENT = IndigoVectorSlot01Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "GarnetTalonSlot01Agent",
    "UmberOrchidSlot01Agent",
    "IndigoVectorSlot01Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "GarnetTalonSlot01Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "UmberOrchidSlot01Agent":
