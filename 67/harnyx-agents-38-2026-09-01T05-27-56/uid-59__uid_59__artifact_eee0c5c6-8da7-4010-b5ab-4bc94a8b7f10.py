...
def _compose_tidal_prism_agent_entry():
...
_tidal_prism_agent_query_entry = _compose_tidal_prism_agent_entry()
...
def _compose_basalt_anvil_agent_entry():
...
_basalt_anvil_agent_query_entry = _compose_basalt_anvil_agent_entry()


_SHAPE_ROUTER_SEED = "bf322f5f44bb8d05a9aa9552"
...
def _balanced_route_label(query: Query) -> str:
    ...
    order = ("TidalPrismAgent", "BasaltLanternAgent", "BasaltAnvilAgent")
    if shape == 3:
        return order[bucket]
...
class TidalPrismAgent:
    async def __call__(self, query: Query) -> Response:
        return await _tidal_prism_agent_query_entry(query)
...
class BasaltAnvilAgent:
    async def __call__(self, query: Query) -> Response:
        return await _basalt_anvil_agent_query_entry(query)


_SHAPE_PRIMARY_AGENT = TidalPrismAgent()
_SHAPE_SECONDARY_AGENT = BasaltLanternAgent()
_SHAPE_TERTIARY_AGENT = BasaltAnvilAgent()
