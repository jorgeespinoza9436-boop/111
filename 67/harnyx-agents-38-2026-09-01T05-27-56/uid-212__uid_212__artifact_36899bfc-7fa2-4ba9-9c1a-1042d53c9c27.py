...
_gtiiueccni = {'started': None, 'text': None}
_rzziidcpue = 24000
_mecfehjbxx = 290.0
_hetqiycgkn = 250.0

def _asrsywlnvd() -> float:
    started = _gtiiueccni['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _pqleuecfiy() -> float:
    return _mecfehjbxx - _asrsywlnvd()
_skdypcatgt = _hsapi.llm_chat
_xogkntotrs = _hsapi.search_web
_swfdcbjxxf = _hsapi.fetch_page
_igoruepvut = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _yczvoustcf(*args, **kwargs):
    if _asrsywlnvd() >= _hetqiycgkn:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _igoruepvut})
            kwargs['messages'] = steered
    _result = await _skdypcatgt(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _ndrxwdyysu(_result)
    return _result

async def _thxenlxmwh(*args, **kwargs):
    if _asrsywlnvd() >= _hetqiycgkn:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _xogkntotrs(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _kuthgkwhdf(*args, **kwargs):
    if _asrsywlnvd() >= _hetqiycgkn:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _swfdcbjxxf(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _yczvoustcf
_hsapi.search_web = _thxenlxmwh
_hsapi.fetch_page = _kuthgkwhdf
_uyyjjpueln = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_joemboweft = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_bshdrkjjfr = 900
_aisulftbts = 2

def _ydqjhyocfw(query: Query) -> int:
...
def _fnrbrxectk(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _kdjripgqln(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _ydqjhyocfw(query)
    if fields >= 3:
        return 2
    if _fnrbrxectk(lowered, _uyyjjpueln):
        return 1
    if fields <= _aisulftbts and len(text) <= _bshdrkjjfr:
        return 0
    if _fnrbrxectk(lowered, _joemboweft):
        return 0
    return 1

def _ndrxwdyysu(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _gtiiueccni['text'] = text.strip()[:_rzziidcpue]
    except Exception:
        pass

def _nlxnazmdfb(text: str):
...
def _ywcvevyrcg(query: Query) -> Response:
    text = _gtiiueccni['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_rzziidcpue]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _nlxnazmdfb(text)
...
def _wwyafnnywk():
    _S444S666_QUERY_TAG = 's444s666b2-hk6717'
    ...
    **kwargs is satisfied.
    ...
    def _compose_rill_examiner_entry():
        ...
        AUDIT_MIN_USD = 0.05
        WRAPUP_MIN_USD = 0.02
        DOSSIER_MODEL = 'openai/gpt-oss-120b'
        DOSSIER_MAP_TIMEOUT_S = 14.0
        DOSSIER_MIN_LEFT_S = 195.0
        DOSSIER_RESERVE_S = 175.0
        DOSSIER_LATE_RESERVE_S = 28.0
        DOSSIER_LATE_MIN_LEFT_S = 90.0
        DOSSIER_STEP_S = 40.0
        DOSSIER_MAX_ROUNDS = 2
        DOSSIER_MAX_CONDITIONS = 5
        DOSSIER_TARGETS_PER_ROUND = 2
        DOSSIER_FACTS_PER_PAGE = 4
        ...
        async def _preseed(question: str, set_question: bool, ledger: EvidenceLedger, deadline: float) -> str:
            ...
            if not good:
                return ''
            return 'Automatic first-pass searches (already numbered — cite these [n] directly, and search further as needed):\n\n' + '\n'.join(good)
        _AUTHORITY_TIERS = ((re.compile('\\.gov$|\\.gov\\.[a-z]{2}$|\\.gouv\\.fr$|\\.mil$|(?:^|\\.)(?:europa\\.eu|un\\.org|who\\.int|worldbank\\.org|imf\\.org|oecd\\.org|ecb\\.europa\\.eu)$', re.I), 0), (re.compile('\\.edu$|\\.ac\\.[a-z]{2}$|\\.int$|\\.org$', re.I), 1), (re.compile('wikipedia\\.org$', re.I), 2))

        def _row_domain(url: str) -> str:
            dom = re.sub('^https?://', '', (url or '').strip(), flags=re.I).split('/')[0].lower()
        ...
        async def _source_map(question: str, deadline: float) -> dict:
            probe = f'Map this research question for source acquisition. JSON only: {{"conditions": ["<each atomic condition or fact the answer must establish, self-contained, with the entity it applies to>"], "entities": ["<every entity, work, place, body or date the question names>"], "sources": [{{"name": "<the body that publishes the asked data, or a source the question names>", "site": "<its domain, e.g. sec.gov, or empty>", "query": "<one precise web search that reaches its page>"}}]}}. At most {DOSSIER_MAX_CONDITIONS} conditions, ordered by how decisive they are; at most 4 sources. No commentary.\n\nQuestion:\n{question}'
            raw = await _chat_simple(LLM_LANE_A, DOSSIER_MODEL, 'Research source mapper. JSON only.', probe, max_tokens=900, timeout=max(6.0, min(DOSSIER_MAP_TIMEOUT_S, deadline - monotonic() - 160.0)))
            raw = re.sub('^```(?:json)?\\s*|\\s*```$', '', raw.strip(), flags=re.I | re.M)
            data = json.loads(raw)
            if not isinstance(data, dict):
                data = {}
            conditions = [' '.join(str(c).split())[:300] for c in data.get('conditions') or [] if isinstance(data.get('conditions'), list) and str(c).strip()]
            entities = [' '.join(str(e).split())[:120] for e in data.get('entities') or [] if isinstance(data.get('entities'), list) and str(e).strip()]
            sources: list[dict] = []
            for src in data.get('sources') if isinstance(data.get('sources'), list) else []:
                if not isinstance(src, dict):
                    continue
                site = str(src.get('site') or '').strip().lower()
                site = re.sub('^https?://', '', site).split('/')[0]
                site = site[4:] if site.startswith('www.') else site
                sources.append({'name': ' '.join(str(src.get('name') or '').split())[:120], 'site': site[:80], 'query': ' '.join(str(src.get('query') or '').split())[:200]})
            return {'conditions': conditions[:DOSSIER_MAX_CONDITIONS], 'entities': entities[:12], 'sources': sources[:4]}
        ...
        class Dossier:
            ...
            def __init__(self, question: str, spec: dict) -> None:
                ...
                for i, text in enumerate(conditions, start=1):
                    self.conditions.append({'idx': i, 'text': text, 'terms': _key_terms(text), 'status': 'open', 'rows': [], 'attempts': 0})
            ...
            and the fact lines the coverage review reads."""
                ...
                for m in re.finditer('[^\\n]+', text):
                    ...
                    numeric = bool(re.search('\\d', line))
                    if hits < 2 or not (numeric or hits >= 3):
                        continue
                    scored.append((hits + (1 if numeric else 0), m.start(), min(m.end(), m.start() + DOSSIER_FACT_CHARS)))
            ...
            does, OPEN otherwise."""
                for cond in self.conditions:
                    ...
                    full = sorted({f['row'] for f in self.facts if len(terms & _key_terms(f['text'])) >= need})
                    part: list[int] = []
                    if not full:
                        for n, row in enumerate(ledger.rows, start=1):
            ...
            def targets(self) -> list[dict]:
                ...
                pending = [c for c in self.conditions if c['status'] != 'covered' and c['attempts'] < DOSSIER_MAX_ROUNDS + 1]
            ...
            def digest(self, late: bool=False) -> str:
                ...
                for cond in self.conditions:
                    ...
                    if cond['status'] == 'covered':
                        ...
                        tag = f'PARTIAL: only search excerpts {cites} mention it; open the page that states it (read_page its URL) before relying on it'
                    else:
                ...
                if late:
                    out = "PRIMARY-SOURCE DOSSIER — coverage review of the draft's conditions against every page opened so far (numbered results; cite them directly).\nCONDITIONS:\n" + '\n'.join(lines)
                else:
                    out = 'PRIMARY-SOURCE DOSSIER — assembled before this conversation from the bodies that publish the asked data; its results are already numbered, cite them directly and page_grep/page_read work on every page it opened.\nCONDITIONS:\n' + '\n'.join(lines)
                if seen:
                    out += '\nPAGES OPENED: ' + ', '.join(seen)
                if facts:
                    out += '\nFACT LINES (verbatim from those pages; pinned, so citing their [n] materializes this text):\n[' + '\n['.join(facts)
                return out

        def _pick_source(ledger: EvidenceLedger, rows: range, terms: set[str], named_sites: list[str], seen: set[str]) -> str:
            best = None
            for n in rows:
                row = ledger.rows[n - 1]
        ...
        async def _acquire_round(question: str, dossier: Dossier, ledger: EvidenceLedger, deadline: float, reserve: float=DOSSIER_RESERVE_S) -> None:
        ...
        numbering is a function of the question alone."""
            ...
            for cond in dossier.targets()[:DOSSIER_TARGETS_PER_ROUND]:
                ...
                if cond['status'] == 'partial' and cond['rows']:
                    url = _pick_source(ledger, range(cond['rows'][0], cond['rows'][-1] + 1), cond['terms'] | dossier.entity_terms, dossier.named_sites, dossier.pages)
                if not url:
                    before = len(ledger.rows)
                    try:
                        out = await asyncio.wait_for(_do_search(dossier.query_for(cond), ledger), timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
                        _commit_tool_output(out, ledger)
                    except Exception:
                        continue
                    url = _pick_source(ledger, range(before + 1, len(ledger.rows) + 1), cond['terms'] | dossier.entity_terms, dossier.named_sites, dossier.pages)
                if not url:
                ...
                before = len(ledger.rows)
                try:
                    out = await asyncio.wait_for(_do_fetch(url, cond['text'], question, ledger), timeout=FETCH_TIMEOUT_S * 2 + 6.0)
                    _commit_tool_output(out, ledger)
                except Exception:
                    continue
        ...
        async def _assemble_dossier(question: str, ledger: EvidenceLedger, deadline: float) -> tuple[str, 'Dossier']:
        ...
        _DOSSIER_REWRITE_RULES = "Coverage editor. You restate a research answer so that every condition the dossier now marks COVERED is supported by the page that states it: cite that page's [n] beside the claim and use its fact line verbatim where the answer states the same fact. Keep every figure, name, date, verdict and the required shape exactly as written; change only citations and add the support the covered conditions now have. Never narrate the dossier, never add a claim the pages do not state, never drop one. Output only the restated answer."

        async def _dossier_regenerate(question: str, answer: str, dossier: 'Dossier', ledger: EvidenceLedger, deadline: float) -> str:
        ...
        round for conditions still uncovered -> restatement against the dossier."""
            ...
            try:
                restated = await _chat_simple(LLM_LANE_A, LOOP_MODEL_A, _DOSSIER_REWRITE_RULES, ask, max_tokens=3200, timeout=max(8.0, min(DOSSIER_REWRITE_TIMEOUT_S, deadline - monotonic() - 12.0)))
            except Exception:
                return answer
            restated = (restated or '').strip()
            if not _is_usable_answer(restated) or len(restated) < int(len(answer) * 0.6) or _unmakes_draft(_CITE_NUM_RE.sub(' ', answer), _CITE_NUM_RE.sub(' ', restated)):
                return answer
            return restated

        async def _loop(question: str, brief: str, ledger: EvidenceLedger, deadline: float, turn_cap: int, carry: list[dict] | None=None, allow_tools_in_wrapup: bool=False, dossier_note: str='') -> tuple[str, list[dict]]:
            if carry is not None:
                messages = carry
            else:
                set_q = _needs_set_completeness(question)
        ...
        async def _solve(query: Query, question: str) -> Response:
            ...
            messages: list[dict] = []
            try:
                answer, messages = await _loop(question, brief, ledger, deadline, MAX_TURNS, dossier_note=dossier_note)
            except Exception:
                answer = ''
            try:
                if _is_usable_answer(answer) and deadline - monotonic() > 75.0 and (_spend_left() >= AUDIT_MIN_USD):
                ...
                pass
            try:
                if dossier is not None and _is_usable_answer(answer) and (deadline - monotonic() > DOSSIER_LATE_MIN_LEFT_S) and (_spend_left() >= AUDIT_MIN_USD):
    ...
    async def _w5_base_query(query: Query) -> Response:
        ...
        branch = _BALANCED_PRIMARY_AGENT if selected == 'KestrelCurator' else _BALANCED_SECONDARY_AGENT
    ...
    _W5_GAP = '[\\s_*~`]+'
    _W5_ADMISSION_RE = _w5_re.compile('does not contain|provides no|no data on|cannot be (?:verified|determined|corrected)|not specified in|is not available|could not be (?:found|verified)|insufficient evidence|no evidence (?:was )?(?:found|available)', _w5_re.I)
    ...
    async def _w5_note_board(question, schema, response, deadline):
        ...
        composed = _w5_compose_note(question, output, anchored, order) if field_refs else ''
...
def _hjjdyqfaal():
    ...
    async def _chat_turn_fallback(messages: list[dict[str, object]], *, deadline: float, thinking_on: bool):
        """Second model for the RESEARCH loop.

    `_chat_turn` retries the same model twice and then returns None, which ends
    research early and hands the commit stage a thinner index. The commit path
    already has COMMIT_FALLBACK_MODEL; the research path had nothing.
    """
        timeout = min(LLM_TURN_TIMEOUT_SECONDS, deadline - perf_counter())
        if timeout <= 0:
            return None
        try:
            return await llm_chat(provider=LLM_PROVIDER, model=RESEARCH_FALLBACK_MODEL, messages=messages, tools=TOOLS, tool_choice='auto', temperature=0.2, thinking=LlmThinkingConfig(enabled=thinking_on, effort='low'), timeout=timeout)
        except Exception:
            return None
    _AUD_SET_RE = re.compile('\\b(which|what|list|name|how many)\\b[^.?!]{0,90}\\b(all|every|each|both|countries|companies|films|members|winners|distributors|those)\\b', re.I)

    def _needs_roster(text: str) -> bool:
        return bool(_AUD_SET_RE.search(text or ''))
    ROSTER_DIRECTIVE = 'SET TASK. Your FIRST retrieval must hunt the authoritative roster that enumerates the WHOLE pool -- search it AS a list ("<pool subject> list", "<pool subject> table") and fetch that page before checking any member. Building the pool from per-member lookups is how a run ships 3 of 6 qualifiers: the members you never thought to search for are invisible to you. Then give EVERY member its own line with its own [n] -- including each member you rule OUT and the condition it fails.'
    _AUD_LEAD_CHARS = 500
        ...
        try:
            ...
            while turn < RESEARCH_TURN_CAP and perf_counter() < research_stop:
                ...
                if chat_result is None:
                    chat_result = await _chat_turn_fallback(messages, deadline=research_stop, thinking_on=thinking_on)
                if chat_result is None:
                    break
...
"""
    ...
    """
        ...
        return response
    return query
_niphovkrbg = _wwyafnnywk()
_wcqqatldkt = _hjjdyqfaal()
_araztpvzxw = _dckpdiqslo()
_wjkyxtkqrd = _imssokkdet()
_aocjqjpvlj = _zqrxeqvkls()
_mecfehjbxx = 290.0
_zyphxzyecc = 250.0
_nwdbebnsce = 90.0

async def _bcfylmmqps(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _mecfehjbxx - (time.monotonic() - started)
        if first:
            budget = _zyphxzyecc if _zyphxzyecc < remaining else remaining
            first = False
        else:
            if remaining < _nwdbebnsce:
                break
        ...
        try:
            ...
            last_exc = exc
    return _ywcvevyrcg(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _gtiiueccni['started'] = time.monotonic()
    try:
        index = _kdjripgqln(query)
        if index == 0:
            agents = (_niphovkrbg, _wcqqatldkt, _araztpvzxw, _wjkyxtkqrd, _aocjqjpvlj)
        elif index == 1:
            agents = (_wcqqatldkt, _araztpvzxw, _wjkyxtkqrd, _aocjqjpvlj, _niphovkrbg)
        elif index == 2:
            agents = (_araztpvzxw, _wjkyxtkqrd, _aocjqjpvlj, _niphovkrbg, _wcqqatldkt)
        elif index == 3:
            agents = (_wjkyxtkqrd, _aocjqjpvlj, _niphovkrbg, _wcqqatldkt, _araztpvzxw)
        elif index == 4:
            agents = (_aocjqjpvlj, _niphovkrbg, _wcqqatldkt, _araztpvzxw, _wjkyxtkqrd)
        else:
            agents = (_niphovkrbg, _wcqqatldkt, _araztpvzxw, _wjkyxtkqrd, _aocjqjpvlj)
        return await _bcfylmmqps(query, agents)
    except Exception:
        return _ywcvevyrcg(query)