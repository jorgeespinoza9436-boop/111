...
# ── V07  CONSTRAINT ELIMINATION OVER THE TAP ──────────────────────────────
# Controller: extract the decision structure, then test every candidate
#             against every constraint using tap page text.
...
#             surviving set and the elimination record.
...
_V07_EXTRACT_SYSTEM = (
    "You read a research question and its draft answer and extract the "
    'decision structure. Return JSON only: {"constraints": ["<one testable '
    'condition>", ...], "candidates": [{"name": "<candidate>", "evidence": '
    '"<what the draft says about it>"}, ...]}. Constraints are literal: '
    "'more than 25' means strictly greater than 25. Include candidates the "
    "draft RULES OUT as well as those it keeps. At most five constraints and "
    "twelve candidates. Never invent either."
)
...
async def _v07_run(query):
    ...
    raw = await _cx_chat(
        _V07_EXTRACT_SYSTEM,
        "QUESTION:\n" + question[:2500] + "\n\nDRAFT:\n" + draft[:18000],
        _CX_FAST, min(16.0, max(5.0, _cx_left(t0, _CX_WALL_S) - 16.0)),
        1400, 0.0)
    parsed = _cx_json(raw)
    if parsed is None:
        return base
