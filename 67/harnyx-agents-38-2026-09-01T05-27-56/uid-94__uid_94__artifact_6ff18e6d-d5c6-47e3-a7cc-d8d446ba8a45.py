...
def _compose_amber_meadow_agent_entry():
    ...
    # --- stage: premise-verification sweep ------------------------------------
    # An answer can be fluent, cited and about the wrong subject: the loop retrieved
    # around the question's entity rather than about it. This checks that every
    # named subject in the question is present in what was read, and spends one
    # search on the one that is not.

    PREMISE_MIN_ENTITY_CHARS = 4
    PREMISE_MAX_ENTITIES = 4
    PREMISE_SKIP = frozenset((
        "what", "which", "who", "when", "where", "why", "how", "the", "and", "for",
        "was", "were", "did", "does", "list", "name", "give", "state", "compare",
        "according", "based", "using", "between", "during", "since", "from", "into",
    ))
    ...
    async def _premise_sweep(
        ...
        listed = ", ".join(absent)
        directive = (
            f"The question names {listed}, and no retrieved passage mentions it. "
            "Answer about the subject the question actually names, citing a passage "
            "that names it. If the evidence does not cover that subject, say so "
            "plainly rather than answering about a neighbouring one."
        )
        return await _stage_rewrite(question, answer, index, directive, deadline)
    ...
    async def _plain_query(query: Query, budget: float) -> Response:
        ...
        try:
            ...
            # --- ADDED STAGES: premise-verification sweep then value-support repair ---
...
def _compose_beryl_lantern_agent_entry():
...
_beryl_lantern_agent_query_entry = _compose_beryl_lantern_agent_entry()


def _compose_lumen_talon_agent_entry():
...
_lumen_talon_agent_query_entry = _compose_lumen_talon_agent_entry()
...
class BerylLanternAgent:
    async def __call__(self, query: Query) -> Response:
        return await _beryl_lantern_agent_query_entry(query)


class LumenTalonAgent:
    async def __call__(self, query: Query) -> Response:
        return await _lumen_talon_agent_query_entry(query)


_TASK_SHAPE_PRIMARY_AGENT = AmberMeadowAgent()
_TASK_SHAPE_SECONDARY_AGENT = BerylLanternAgent()
_TASK_SHAPE_TERTIARY_AGENT = LumenTalonAgent()
