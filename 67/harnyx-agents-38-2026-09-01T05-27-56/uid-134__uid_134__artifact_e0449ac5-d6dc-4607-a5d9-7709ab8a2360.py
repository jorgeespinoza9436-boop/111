...
_cyijazrtbc = {'started': None, 'text': None}
_tkzftouvvx = 24000
_tqoveqgmsp = 290.0
_jggwydbtct = 250.0

def _ghfstqywbr() -> float:
    started = _cyijazrtbc['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _munauqlnzj() -> float:
    return _tqoveqgmsp - _ghfstqywbr()
_hzehrrwczc = _hsapi.llm_chat
_vqmuhuzwky = _hsapi.search_web
_eqrgaxwswi = _hsapi.fetch_page
_okpuqrvwvl = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _wsodvaqpta(*args, **kwargs):
    if _ghfstqywbr() >= _jggwydbtct:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _okpuqrvwvl})
            kwargs['messages'] = steered
    _result = await _hzehrrwczc(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _japmcwmsyy(_result)
    return _result

async def _rzszusfqlv(*args, **kwargs):
    if _ghfstqywbr() >= _jggwydbtct:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _vqmuhuzwky(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _pwdydxhzyw(*args, **kwargs):
    if _ghfstqywbr() >= _jggwydbtct:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _eqrgaxwswi(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _wsodvaqpta
_hsapi.search_web = _rzszusfqlv
_hsapi.fetch_page = _pwdydxhzyw
_fonzbyxtgh = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_lhownnsuat = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_abupzkowon = 900
_lqddlwmsmu = 2

def _glncnxbmvy(query: Query) -> int:
...
def _dpifoxougw(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _bxodmxhyog(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _glncnxbmvy(query)
    if fields >= 3:
        return 2
    if _dpifoxougw(lowered, _fonzbyxtgh):
        return 1
    if fields <= _lqddlwmsmu and len(text) <= _abupzkowon:
        return 0
    if _dpifoxougw(lowered, _lhownnsuat):
        return 0
    return 1

def _japmcwmsyy(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _cyijazrtbc['text'] = text.strip()[:_tkzftouvvx]
    except Exception:
        pass

def _bncrmeryrx(text: str):
...
def _riefucerld(query: Query) -> Response:
    text = _cyijazrtbc['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_tkzftouvvx]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _bncrmeryrx(text)
...
def _lxnxjoidnt():
    ...
    from harnyx_miner_sdk.query import Query, Response
    import harnyx_miner_sdk.api as _hsapi
    _lttrzvaken = {'started': None, 'text': None}
    _ixrdsvkmyl = 24000
    _ugwxgbnhfg = 290.0
    _ayhxrplgte = 250.0

    def _swakeiepaw() -> float:
        started = _lttrzvaken['started']
        if started is None:
            return 0.0
        return max(0.0, time.monotonic() - started)

    def _vvukfnttni() -> float:
        return _ugwxgbnhfg - _swakeiepaw()
    _xfwcmlrlcu = _hsapi.llm_chat
    _oicbqttrtk = _hsapi.search_web
    _drofzlnpdr = _hsapi.fetch_page
    _mahzvukyuz = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

    async def _qfvkjwetcg(*args, **kwargs):
        if _swakeiepaw() >= _ayhxrplgte:
            messages = kwargs.get('messages')
            if messages is not None:
                steered = list(messages)
                steered.append({'role': 'user', 'content': _mahzvukyuz})
                kwargs['messages'] = steered
        _result = await _xfwcmlrlcu(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
        _icwmyuheid(_result)
        return _result

    async def _ocpwbrgjen(*args, **kwargs):
        if _swakeiepaw() >= _ayhxrplgte:
            raise TimeoutError('research cutoff reached; finalize with gathered evidence')
        return await _oicbqttrtk(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

    async def _auqfklqixz(*args, **kwargs):
        if _swakeiepaw() >= _ayhxrplgte:
            raise TimeoutError('research cutoff reached; finalize with gathered evidence')
        return await _drofzlnpdr(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _hsapi.llm_chat = _qfvkjwetcg
    _hsapi.search_web = _ocpwbrgjen
    _hsapi.fetch_page = _auqfklqixz
    _xxdocyugdi = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
    _laalqoaksu = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
    _agkaaednxo = 900
    _rcrgiqbbgk = 2

    def _jadxszxifk(query: Query) -> int:
        schema = getattr(query, 'output_schema', None)
        if not isinstance(schema, dict):
            return 0
        props = schema.get('properties')
    ...
    def _ukconacnle(text: str, terms: tuple) -> bool:
        for term in terms:
            if term in text:
                return True
        return False

    def _xjvlomfaiw(query: Query) -> int:
        text = (getattr(query, 'text', '') or '').strip()
        lowered = text.lower()
        fields = _jadxszxifk(query)
        if fields >= 3:
            return 2
        if _ukconacnle(lowered, _xxdocyugdi):
            return 1
        if fields <= _rcrgiqbbgk and len(text) <= _agkaaednxo:
            return 0
        if _ukconacnle(lowered, _laalqoaksu):
            return 0
        return 1

    def _icwmyuheid(result: object) -> None:
        try:
            resp = getattr(result, 'response', None)
            text = None
            choices = getattr(resp, 'choices', None)
            ...
            if not text:
                ...
                if isinstance(value, str):
                    text = value
            if text and text.strip():
                _lttrzvaken['text'] = text.strip()[:_ixrdsvkmyl]
        except Exception:
            pass

    def _lidibdrfxi(text: str):
        import json as _json
        start = text.find('{')
        end = text.rfind('}')
        if start != -1 and end != -1 and (end > start):
    ...
    def _huehrfovmt(query: Query) -> Response:
        text = _lttrzvaken['text']
        if not text or not text.strip():
            text = 'A complete answer could not be produced within the available time budget.'
        text = text.strip()[:_ixrdsvkmyl]
        schema = getattr(query, 'output_schema', None)
        if schema is not None:
            parsed = _lidibdrfxi(text)
            if parsed is not None:
                try:
                    return Response(output=parsed)
                except Exception:
...
"""
        ...
        VERSION = 'v33.3-depthfirst-ew471-dates-sjson-sw43'
        PAGE_NUMS_WINDOW = 90
        PAGE_DATES_MAX_HITS = 40
        SCHEMA_JSON_MAX_ISSUES = 20
        LLM_LANE_A = 'openrouter'
        LLM_LANE_B = 'openrouter'
        LOOP_MODEL_A = 'z-ai/glm-5.2'
        LLM_LANE_C = 'chutes'
        RESCUE_MODEL_C = 'zai-org/GLM-5.2-TEE'
        LOOP_MODEL_B = 'deepseek/deepseek-v3.2'
        AUDIT_MODEL = 'openai/gpt-oss-120b'
        SCHEMA_MODEL = 'openai/gpt-oss-120b'
        RESORT_MODEL = 'deepseek/deepseek-v3.2'
        SEARCH_PROVIDER = 'parallel'
        TOTAL_BUDGET_SECONDS = 254.75
        TASK_TOTAL_BUDGET_SECONDS = 275.8
        _DAILY_NONCE = '4f6b2f882b1e2113'
        _NONCE_D2026239 = 'ae2a022581e9'
        _NONCE_R_D223_1 = 'a5ae5c654515'
        _NONCE_R_D222_3 = '8cca9e9fb3c4'
        _NONCE_R_D222_2 = 'ab72e9f8f150'
        _NONCE_R_D222_1 = '68b9fed8fe5e'
        _NONCE_R_D220_1 = '0c1d40c75d3b'
        WALL_BUDGET_S = 262.0
        ...
        _OR_DEAD_402 = {'dead': False, 'n402': 0, 'fast': 0}
        LOOP_MAX_OUTPUT_TOKENS = 12000
        _TOOL_SEM = {'sem': None}

        def _reroute_402(lane: str, model: str, pinned: bool=False):
            if _OR_DEAD_402['dead'] and lane == 'openrouter':
                return (LLM_LANE_C, RESCUE_MODEL_C, False)
            return (lane, model, pinned)

        def _note_fail(lane: str, exc, elapsed: float) -> bool:
            """True ONLY for a real http_402 on openrouter (refusal = not blind spend). v2: a 429 / timeout /
    any other failure returns False and is handled exactly as before the guard (blind spend, unpin, retry)."""
            if lane != 'openrouter':
                return False
            s = str(exc).lower()
            status = getattr(exc, 'http_status', None)
            if status == 402 or '402' in s or 'payment' in s or ('insufficient' in s):
                _OR_DEAD_402['n402'] += 1
                _OR_DEAD_402['dead'] = True
                return True
            return False

        def _tool_sem():
            if _TOOL_SEM['sem'] is None:
                _TOOL_SEM['sem'] = asyncio.Semaphore(3)
            return _TOOL_SEM['sem']
        ...
        LOOP_TOOLS = [{'type': 'function', 'function': {'name': 'web_search', 'description': 'Web search. Returns numbered results, each with title, url and excerpt.', 'parameters': {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'the search query'}}, 'required': ['query']}}}, {'type': 'function', 'function': {'name': 'sec_filing', 'description': "Resolve a company's SEC filing to its primary document URL on sec.gov (exact form + year, from EDGAR's own index). Use for questions about a specific filing (10-K, 10-Q, 8-K, DEF 14A…), then read_page the returned URL with a focus hint for the Item/section.", 'parameters': {'type': 'object', 'properties': {'company': {'type': 'string', 'description': "company name or ticker, e.g. 'Apple' or 'AAPL'"}, 'form': {'type': 'string', 'description': "filing form, e.g. '10-K', '10-Q', '8-K', 'DEF 14A'"}, 'year': {'type': 'string', 'description': "optional report (fiscal) year, e.g. '2019' (omit for latest)"}}, 'required': ['company', 'form']}}}, {'type': 'function', 'function': {'name': 'read_page', 'description': 'Fetch a URL and return its main text. Large pages show the head plus the few regions most relevant to the question; pass a focus hint to steer which regions.', 'parameters': {'type': 'object', 'properties': {'url': {'type': 'string', 'description': 'URL to fetch'}, 'focus': {'type': 'string', 'description': 'optional phrase to locate inside the page (section name, table label, entity)'}}, 'required': ['url']}}}, {'type': 'function', 'function': {'name': 'retain_evidence', 'description': "Keep the exact source text that proves a claim you are about to make. Pass the result number and the verbatim quote from it. Do this the moment you find a decisive value -- the judge only credits claims whose citation contains the supporting text, and this is how that text gets into your citation. Use it for the QUESTION'S PREMISES as well as your answer: every entity, work, date or figure the question names should end up with a retained quote confirming it.", 'parameters': {'type': 'object', 'properties': {'source': {'type': 'string', 'description': 'result number to quote from, e.g. 3'}, 'quote': {'type': 'string', 'description': 'verbatim text copied from that result that states the fact'}}, 'required': ['source', 'quote']}}}, {'type': 'function', 'function': {'name': 'page_dates', 'description': 'Sweep a page you ALREADY fetched and list every date mention with its surrounding context, optionally only where the context contains a keyword. Use it for when-questions and timeline checks on a page already in memory.', 'parameters': {'type': 'object', 'properties': {'url': {'type': 'string', 'description': "URL you already read_page'd"}, 'near': {'type': 'string', 'description': 'optional keyword; only contexts containing it are returned'}}, 'required': ['url']}}}, {'type': 'function', 'function': {'name': 'schema_json', 'description': 'Check a drafted JSON answer for a question that demands structured output: reports parse errors with position, the parsed keys and types, and missing/extra keys vs the required list. Run it on the draft BEFORE finishing a structured answer.', 'parameters': {'type': 'object', 'properties': {'draft': {'type': 'string', 'description': 'the drafted JSON answer object'}, 'required_keys': {'type': 'string', 'description': 'comma-separated key names the answer must contain'}}, 'required': ['draft']}}}]
        LOOP_RULES = 'You are a research agent answering a hard multi-part factual question. A judge compares your answer head-to-head with a strong reference and only credits claims that carry a citation to a tool result that states them.\n\nMETHOD: think in constraints and candidates. Recall what you already know to form the candidate pool, then use web_search/read_page to verify every load-bearing fact (names, figures, dates, rankings) before asserting it. Work every candidate through every stated condition; one search per fact beats one broad search. TWO DISTINCT SUB-QUESTIONS: if the question asks two separate things, answer BOTH substantively — a partial answer covering both sides outscores a complete answer to only one. BATCH YOUR LOOKUPS: independent facts (each candidate\'s score, each entity\'s figure) should be requested as SEVERAL tool calls in the SAME turn — they run in parallel, so a 6-candidate sweep costs one turn, not six. TABLE CARE: when reading a table, respect its qualifier columns (Owned vs Leased, the exact year, the exact segment) — count or compare only rows matching EVERY stated qualifier, and quote the row values you used. For a named source (Box Office Mojo, a 10-K, Nielsen), fetch THAT page — for SEC filings, use the sec_filing tool to resolve the exact primary document from EDGAR\'s own index, then read_page it with a focus hint for the Item/section.\n\nDATE SWEEP: page_dates(url, near) lists every date mention on a page you already fetched, with context -- use it for when-questions.\n\nSTRUCTURED ANSWER CHECK: when the question demands JSON or exact named fields, draft the final JSON object, run schema_json(draft, required_keys) and fix every issue it reports BEFORE finishing -- a structured task whose output is not conforming JSON is rejected whole at ingress and scores zero.\n\nCITE EVERYTHING: put [n] (the tool-result number) immediately after the SENTENCE carrying each claim — not pooled at the end of a paragraph. Every sentence asserting a number, date, proper noun or causal link needs its own [n], for the entities you rule OUT as well as those you include. An uncited specific reads as invented. Cite only results that actually state the claim, and prefer the most AUTHORITATIVE one that does: the official database/filing/statistics page over an aggregator, blog, or retrospective article. CITE THE HARD CONDITION, NOT JUST THE POOL: every stated condition needs evidence of its own, and the one hardest to verify is the one the grader checks. Citations that establish only the candidate pool leave the actual filter unsupported — a right answer whose decisive condition is uncited loses to a weaker answer that proves it.\n\nPREFER THE PRIMARY SOURCE: when two sources state the same fact, cite the one that ORIGINATES it -- the agency, registry, filing, official statistics release or the organisation\'s own page -- not an encyclopedia or aggregator repeating it. Measured verbatim on a task where both answers were factually correct: \'Answer 1 is preferred for using primary sources\' (it cited NARA where we cited Wikipedia) -- a full point lost on every run. Use the encyclopedia to FIND the primary source, then fetch and cite that. EDITION MATCH: when the question names an edition, year or dated release of a source, fetch and cite THAT edition -- a citation to a different date or edition voids the claim it carries.\n\nCOVER THE WHOLE SET: when the question ranges over an enumerated set (\'every report\', \'each entry\', a superlative over a list), cite EVERY member you examined -- one short retained quote per member -- not only the winning one. Judges break ties between identical correct answers on citation COVERAGE of the set; an answer citing 4 of 7 members loses. For multi-field JSON answers, retain one targeted quote PER FIELD rather than one page-sized blob.\n\nCITE CONCENTRATED: prefer citing the ONE source that proves a whole cluster of claims repeatedly over spreading citations across many near-equivalent pages. A few deep sources cited with wide proving windows beat many shallow ones -- the judge reads only what your citations contain, and depth on the deciding source wins ties.\n\nQUOTE WHAT PROVES IT: the judge credits a claim only when your citation CONTAINS the source text stating it. The moment you read a decisive value, call retain_evidence(source, quote) with the exact words from that result. Do this for every condition you test and every figure you report -- an answer whose citations do not carry its numbers loses to one that does, even when both answers are identical.\nALSO QUOTE THE QUESTION\'S PREMISES, not only your answer. Every entity, work, date or figure the question NAMES is a claim the judge expects traceable: the film it says someone directed, the article it points at, the year it fixes, the people it lists. You lose to an otherwise identical answer that cited those too -- measured verbatim: "does not provide a citation for \'Everyone Says I Love You\'... Answer 1 is more thorough in its traceability to all parts of the prompt\'s context". Retain a quote for each named premise as you confirm it, even when it is background you already believed.\n\nSOURCE CONFIDENCE: when the question NAMES a source you could not reach but other authoritative evidence establishes the same facts, state those facts plainly and confidently with their [n], and treat the other sources as corroboration. Do not open with, dwell on, or append a note that the named source was unavailable — reserve missing-source language for a FACT that is genuinely absent everywhere, never for a missing source LABEL.\n\nSELF-CONSISTENCY: before you finish, check that the opening names exactly the entities your own cited sentences support. If the body establishes a different answer than the opening claims, rewrite the opening to match the evidence — never leave a weaker fallback in the lead.\n\nANSWER SHAPE: sentence one IS the answer — the exact entities/values/list asked for, in the requested format. Never open with \'Based on…\', \'From my research…\', \'I can provide a partial answer\', or any preamble — start with the answer entities themselves. ANSWER THE ASKED KIND: if the question asks which SERIES, name the series (not the people in it); which FILM, the film (not its director); which COUNTRY, the country. THE POOL IS THE WHOLE NAMED CLASS, NOT THE SURVIVORS: build it from the broadest set the question ranges over — every member of that class, not the ones you already believe qualify — then apply the conditions one at a time and show who each one eliminates. Never pre-filter to the members that already pass and present those as the pool — an answer whose pool contains only qualifiers proves nothing about the sweep, which is how a correct answer still scores zero. List members that fail on the FIRST condition too. Then: the candidate pool, each condition applied, and ONE LINE PER POOL MEMBER — a line for every qualifier with its qualifying attribute cited, AND a line for every candidate you rule out with its cited failing condition. Never compress several rejects into one clause (\'X, Y and Z never won [n]\'): each rejected member gets its own line and its own [n], even when the pool runs to a dozen members. A batched exclusion reads as a pool you never checked. Two later instructions may relax this — one when time runs short, one when the pool is too large to list in full — and nothing else does. If you cannot settle a member\'s condition, KEEP it among the qualifiers — a wrongly-dropped qualifier costs as much as a wrong answer — and give its line the strongest fact you did verify. Never add a note about what you could not check. OUTPUT DIRECTIVES ARE LITERAL: obey formatting instructions mechanically. Decide first whether a phrase constrains the OUTPUT or selects the ENTITIES: \'list them without the word "X"\' shapes what you print, so DELETE X from each name; \'whose title does not contain "X"\' / \'titles without the word X\' is a condition on the pool, so keep only members that lack it. When the phrase governs how to print an already-chosen set, the deletion reading applies — it is not a filter. \'in alphabetical/chronological order\' means sort the final list; \'comma-separated\' means join with commas; a requested count means emit the number. These govern the ANSWER LINE — give it in exactly the requested shape, then still add the proof section below it; the shape directive is never a reason to omit the proof. When an ORDER is demanded, the ANSWER LINE itself must be sorted — not merely the table under it. Print the sort key beside each item (the year, figure or date you sorted on) and check every adjacent pair before you finish: one member out of sequence fails the whole answer even when the set is exactly right. COMPUTED ANSWERS: if the answer is a mean, total, rank or count derived from several figures, pull every input into one explicit list first, then compute — and show the arithmetic so the number is checkable. Never report a derived number you did not visibly compute from listed inputs. ROUNDED FIGURE = WRONG SOURCE: a decisive number that reads as rounded — trailing zeros where the measuring body publishes exact digits, \'X.Y thousand/million\', \'about\'/\'approximately\', or a value lifted from a chart label — came from an aggregator that publishes summaries, not from the body that measured it. Do NOT commit it. Search again for the exact figure from the source the question NAMES (or the outlet that reports that source\'s own numbers) and answer with the full precision it publishes, digit for digit. Quote the rounded value only as corroboration after the exact one. This is a RETRIEVAL instruction, not a licence to withhold: once tool calls are closed, or if the named source itself publishes only the rounded value, commit the best figure you hold and never remark on its precision. EXACT VALUES ONLY: this governs HOW you report a figure; the rule above governs WHICH figure to go and fetch. Once you hold the right one, use the figures you READ in a tool result, verbatim — preserve notation exactly (58.58% and 58.6% are different; \'p < 0.0001\' and \'P < .001\' must not be merged or called consistent). If one source gives a range and another a point value, give both and say whether the point falls inside the range. If a figure is reported in different units than the question asks, convert it and give the exact converted result, preserving units and any timezone label. Answer with the value from the exact source, date and scope the question NAMES — do not substitute a later or broader figure unless resolving a conflict requires it. Bind every claim to the exact actor, target, date-window and instrument the evidence ties together; never carry a statement about one party or period across to another. Never a remembered or approximate value (\'~$1.33B\'), never rounded, never an adjacent year/quarter/metric. If a deciding figure is still unverified at writing time, prefer the tool-read value you have over a guess, and NEVER write \'(verify)\' or any uncertainty marker in the final answer — the final answer contains only committed prose.\n\nAMBIGUOUS METRIC? ANSWER BOTH READINGS. If the asked quantity has two defensible interpretations — one party\'s value or the combined value of both; one dimension of size or another; a narrow scope or a consolidated one — do NOT silently pick one. Name the ambiguity in one clause and give BOTH lists/values, each cited and labelled. A correct answer under the reading the grader did not use still scores as wrong.\n\nAPPLY CONDITIONS LITERALLY: copy each candidate\'s exact value, then test the comparator as written — \'more than 25\' is strictly >25 (25 fails); \'between 2010 and 2019\' includes both endpoints; convert a rate condition into a concrete integer test (\'averaged more than 1 per year over 10 years\' = \'more than 10 in total\'); read edition/date boundaries literally. EXCLUDE ONLY ON PROOF: reject a candidate by naming the specific stated condition it fails, with the cited fact showing the failure — never because it looks weaker than your front-runner. If it is UNCERTAIN whether a candidate fails a condition, KEEP IT in the answer rather than dropping it on a guess: a wrongly-dropped qualifier costs exactly as much as a wrong answer. SAY NO MORE THAN THE CITATION: if the source says \'brought to\', do not write \'incarcerated\'; if it gives a count of 12, do not write 11. Check every count and every verb against its citation.\n\nNEVER NARRATE YOUR EVIDENCE: no sentence about what your results do or do not contain (\'the evidence does not specify…\', \'would be needed to determine…\'). Those phrasings lose. A substantive negative about the WORLD is different and is a real answer when true (\'No member of the class satisfies every condition [n]\'). If a datum truly cannot be verified, commit to the best-supported value you found and move on. ONE narrow exception: when the asked figure genuinely does not exist in any published form, you may state the REASONED IMPOSSIBILITY — name the specific dataset that would hold it and why it cannot yield the value — as a fact about the world, in the first line, alongside the closest cited facts. That is a committed answer; \'the evidence does not contain it\' is not.\n\nFINISH: never mix tool calls and the final answer in one turn. When the constraints are verified (or best-effort covered), write the complete cited answer.'
    ...
    Champion lineages retry a failed search this way instead of giving up."""
        ...
        SEARCH_FLOOR_ALTS = ('tavily', 'exa', 'firecrawl', 'desearch')
        SEARCH_FLOOR_TIMEOUT_S = 16.0
        SEARCH_FLOOR_MIN_S = 56.0
        _SEARCH_HITS = [0]
        _SEARCH_ALT = [None]
        ...
        async def _do_search(query_text: str, ledger: EvidenceLedger):
            ...
            for attempt, allow_repeat in ((query_text, False), (query_text, True), (_degrade_query(query_text), False)):
                ...
                fired.add(attempt)
                try:
                    payload = await search_web(attempt, num=8, provider=_SEARCH_ALT[0] if isinstance(_SEARCH_ALT[0], str) else SEARCH_PROVIDER, timeout=SEARCH_TIMEOUT_S)
                    if getattr(payload, 'results', None):
                        break
                except Exception:
                    payload = None
                    if _SEARCH_ALT[0] is None:
                        _SEARCH_ALT[0] = False
                        for alt in SEARCH_FLOOR_ALTS:
                            if alt == SEARCH_PROVIDER:
                                continue
                            try:
                                payload = await search_web(query_text, provider=alt, num=8, timeout=SEARCH_FLOOR_TIMEOUT_S)
                                if getattr(payload, 'results', None):
                                    _SEARCH_ALT[0] = alt
                                    break
                            except Exception:
                                payload = None
                        if isinstance(_SEARCH_ALT[0], str):
                            break
            if (payload is None or not getattr(payload, 'results', None)) and _SEARCH_ALT[0] is None:
                _SEARCH_ALT[0] = False
                for alt in SEARCH_FLOOR_ALTS:
                    if alt == SEARCH_PROVIDER:
                        continue
        ...
        async def _do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger) -> str:
            ...
            for _attempt in (0, 1):
                try:
                    payload = await fetch_page(url, provider=_SEARCH_ALT[0] if isinstance(_SEARCH_ALT[0], str) else SEARCH_PROVIDER, timeout=FETCH_TIMEOUT_S)
                    if getattr(payload, 'results', None):
                        break
                except Exception:
                    payload = None
    ...
    are positional per-ToolOutput (_commit_tool_output resolves range(len(rows)))."""
            ...
            return ToolOutput(a.text + '\n\n' + label + '\n' + text_b, list(a.rows) + list(b.rows))
        _QUOTE_TYPO_FOLD = {'‘': "'", '’': "'", '‚': "'", '‛': "'", '´': "'", '“': '"', '”': '"', '„': '"', '‟': '"', '«': '"', '»': '"', '‐': '-', '‑': '-', '‒': '-', '–': '-', '—': '-', '―': '-', '−': '-', '…': '...'}
    ...
    move uid210 makes when a retained span omits a numeric fact it asserted."""
        ...
        def _do_page_dates(url, near, ledger):
            """List date mentions (with context) from an already-fetched page.

    Same shape as the numeric sweep, aimed at when-questions: recognises
    1987; March 4, 2019; 12/31/2019; 4 March 2019 in the stored page text."""
            hit = _page_row_for(url, ledger)
            if hit is None:
                return f'# page_dates: {url!r} has not been fetched this run; call read_page first'
            n, row = hit
            text = row.get('text') or ''
            kw = (near or '').strip().lower()
            rx = re.compile('(?:\\d{1,2}[/-]\\d{1,2}[/-]\\d{2,4})|(?:(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\\.?\\s+\\d{1,2},?\\s+\\d{4})|(?:\\d{1,2}\\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\\.?\\s+\\d{4})|(?:\\b(?:1[6-9]|20)\\d{2}\\b)', re.I)
            out, seen_at = ([], [])
            for m in rx.finditer(text):
                ...
                seen_at.append(c)
                out.append(f'\n--- @{a} ---\n{ctx}')
                if len(out) >= PAGE_DATES_MAX_HITS:
                    break
            if not out:
                return f'# page_dates on [{n}]: no date form' + (f' near {kw!r}' if kw else '') + f' in {len(text)} chars'
            return f'# page_dates on [{n}] -> {len(out)} date context(s)' + (f' near {kw!r}' if kw else '') + ''.join(out)

        def _do_schema_json(draft, required_keys, ledger):
            """Deterministic JSON draft checker for structured-output questions.

    Structured tasks are rejected WHOLE at host ingress when the output is
    not valid conforming JSON (measured 2026-08-16: schema'd mains come back
    response=None, score 0, field-wide). This parses a draft, reports the
    exact parse error, lists parsed keys with types, and flags missing or
    extra keys vs a comma-separated required list -- no fetch, no LLM."""

            def _tname(v):
                for cls, nm in ((bool, 'bool'), (str, 'str'), (int, 'int'), (float, 'float'), (list, 'list'), (dict, 'dict')):
                    if isinstance(v, cls):
                        return nm
                return 'NoneType' if v is None else 'object'
            d = (draft or '').strip()
            if d.startswith('```'):
                d = re.sub('^```[a-zA-Z]*\\s*|\\s*```$', '', d).strip()
            want = [k.strip() for k in (required_keys or '').split(',') if k.strip()]
            try:
                obj = json.loads(d)
            except Exception as e:
                return '# schema_json: INVALID JSON -- %s. Fix the draft and check it again; a structured task with non-JSON output is rejected whole (hard zero).' % str(e)[:160]
            if not isinstance(obj, dict):
                return '# schema_json: parsed a %s, but structured answers are JSON OBJECTS with the demanded keys' % _tname(obj)
            lines = ['# schema_json: VALID JSON object, %d key(s):' % len(obj)]
            for k, v in list(obj.items())[:SCHEMA_JSON_MAX_ISSUES]:
                lines.append('  %s: %s' % (k, _tname(v)))
            missing = [k for k in want if k not in obj]
            extra = [k for k in obj if want and k not in want]
            if missing:
                lines.append('MISSING required key(s): ' + ', '.join(missing))
            if extra:
                lines.append('EXTRA key(s) not in the required list: ' + ', '.join(extra[:8]))
            if want and (not missing):
                lines.append('all required keys present -- emit EXACTLY this object as the final answer, nothing around it')
            return '\n'.join(lines)

        async def _run_tool(call, question: str, ledger: EvidenceLedger, deadline: float) -> str:
            """402GUARD G3: at most 3 tool calls in flight per turn (a turn used to fan out 3-7 fetches)."""
            async with _tool_sem():
                return await _run_tool_inner(call, question, ledger, deadline)

        async def _run_tool_inner(call, question: str, ledger: EvidenceLedger, deadline: float) -> str:
            try:
            ...
            if name == 'read_page':
                ...
                if raw_url is not None and isinstance(out, ToolOutput) and (deadline - monotonic() > FETCH_TIMEOUT_S + MIN_TAIL_S):
                    try:
                        raw_out = await _do_fetch(raw_url, focus, question, ledger)
                        if isinstance(raw_out, ToolOutput):
                            out = _merge_tool_outputs(out, raw_out, '# wikitext companion (tables + underlying values):')
                    except Exception:
                        pass
                return out
            ...
            if name == 'page_dates':
                return _do_page_dates(str(args.get('url') or ''), str(args.get('near') or ''), ledger)
            if name == 'schema_json':
                return _do_schema_json(str(args.get('draft') or ''), str(args.get('required_keys') or ''), ledger)
        ...
        async def _chat_simple(lane: str, model: str, system: str, user: str, *, max_tokens: int, timeout: float, think: dict | None=None) -> str:
            lane, model, _ = _reroute_402(lane, model)
            if think is None:
            ...
            try:
                ...
                _note_fail(lane, exc, monotonic() - _t_call)
                raise
            _spend_note(payload)
    ...
    floor rejects it, a repair turn is spent, and the loop tries lane A again."""
        ...
        async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool, force_tools: bool=False):
            ...
            for lane_i, lane_model in enumerate(((LLM_LANE_A, LOOP_MODEL_A, True), (LLM_LANE_A, LOOP_MODEL_A, False), (LLM_LANE_B, LOOP_MODEL_B, False), (LLM_LANE_C, RESCUE_MODEL_C, False))):
                lane = lane_model[0]
                model = lane_model[1]
                pinned = lane_model[2]
                if _OR_DEAD_402['dead'] and lane_i in (0, 1):
                    continue
                lane, model, pinned = _reroute_402(lane, model, pinned)
                if lane_i == 2 and payload_chars > LANE_B_MAX_PAYLOAD_CHARS:
                    return _EMPTY_TURN
                ...
                _t_call = monotonic()
                try:
                    payload = await asyncio.wait_for(llm_chat(provider=lane, thinking={'enabled': False} if finish_only and lane_i == 2 else {'enabled': True, 'effort': 'low'}, temperature=0.2, max_output_tokens=6000 if finish_only and lane_i == 2 else LOOP_MAX_OUTPUT_TOKENS, provider_extra=_upstream(lane, model) if pinned else None, tool_choice='auto' if force_tools or not finish_only else None, timeout=timeout, model=model, tools=LOOP_TOOLS if force_tools or not finish_only else None, messages=messages), timeout=min(timeout + 6.0, max(1.0, deadline - monotonic() - 1.0)))
                    _spend_note(payload)
                    return payload
                except Exception as exc:
                    if not _note_fail(lane, exc, monotonic() - _t_call):
                        _spend_blind()
                    if pinned:
    ...
    heavy batch; the loop then verifies the load-bearing facts."""
        ...
        async def _loop(question: str, brief: str, ledger: EvidenceLedger, deadline: float, turn_cap: int, carry: list[dict] | None=None, allow_tools_in_wrapup: bool=False) -> tuple[str, list[dict]]:
            ...
            for turn in range(1, turn_cap + 1):
                ...
                if hasattr(msg, 'to_input_message'):
                    ...
                    messages.append({'role': 'assistant', 'content': getattr(llm, 'raw_text', None) or '', 'tool_calls': [{'id': c.id, 'type': 'function', 'function': {'name': c.name, 'arguments': c.arguments}} for c in calls]})
                run_calls = calls[:8]
        ...
        async def _audit_patch(question: str, answer: str, messages: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
            ...
            return patched
        VERIFY_TURNS = 3
        VERIFY_MIN_S_FETCH = 95.0
        _SUPERLATIVE_Q_RE = re.compile('\\b(largest|smallest|highest|lowest|most|least|fewest|longest|shortest|oldest|newest|earliest|latest|greatest|best|worst|top|first|last)\\b', re.I)
        _QFIELD_RE = re.compile('"([a-z][a-z0-9_]{2,40})"')

        async def _superlative_table_sweep(question: str, answer: str, messages: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
            """Superlative proof: a most/largest/first question needs comparison
    evidence — a winner asserted from a single cited row is the hand-waved
    tally the judge kills. Trigger: superlative question + fewer than 2
    distinct cited rows backing the answer."""
            if not _is_usable_answer(answer):
                return answer
            if not _SUPERLATIVE_Q_RE.search(question):
                return answer
            distinct = {n for n in _cited_numbers(answer, len(ledger.rows)) if 1 <= n <= len(ledger.rows)}
            if len(distinct) >= 2:
                return answer
            if deadline - monotonic() < VERIFY_MIN_S_FETCH or _spend_left() < WRAPUP_MIN_USD:
                return answer
            messages.append({'role': 'system', 'content': 'SUPERLATIVE PROOF: the question asks for an extreme (most/largest/first-class) but the answer rests on fewer than two cited evidence rows. Fetch the ranking, list or table that the comparison comes from, then rewrite showing the winner AND the runner-up with their figures, each carrying [n] citations to the comparison source.'})
            patched, _ = await _loop(question, '', ledger, deadline, VERIFY_TURNS, carry=messages, allow_tools_in_wrapup=True)
            patched = patched.strip()
            if not _is_usable_answer(patched) or len(patched) < int(len(answer) * 0.6):
                return answer
            return patched

        async def _format_contract_sweep(question: str, answer: str, messages: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
            """Format contract: quoted field names the question demands must appear in
    the answer, and a JSON demand must yield JSON-shaped output. A format miss
    voids an otherwise-correct answer (measured zero class on schema tasks)."""
            if not _is_usable_answer(answer):
                return answer
            fields = list(dict.fromkeys(_QFIELD_RE.findall(question)))[:10]
            wants_json = 'json' in question.lower() and bool(fields)
            missing = [f for f in fields if f not in answer]
            json_miss = wants_json and '{' not in answer[:2000]
            if not missing and (not json_miss):
                return answer
            if deadline - monotonic() < VERIFY_MIN_S_FETCH or _spend_left() < WRAPUP_MIN_USD:
                return answer
            demand = []
            if missing:
                demand.append('these demanded fields are absent: ' + ', '.join(missing[:8]))
            if json_miss:
                demand.append('the question demands a JSON object but the answer is not JSON-shaped')
            messages.append({'role': 'system', 'content': 'FORMAT CONTRACT: ' + '; '.join(demand) + ". Rewrite the answer in EXACTLY the demanded shape (every named field present, JSON if asked) using the evidence already gathered — add at most 2 tool calls only if a field's value is genuinely missing. Keep [n] citations on every factual value."})
            patched, _ = await _loop(question, '', ledger, deadline, VERIFY_TURNS, carry=messages, allow_tools_in_wrapup=True)
            patched = patched.strip()
        ...
        _SINGLE_CITE_RE = re.compile('(?<!\\[)\\[(\\d{1,4}(?:\\s*,\\s*\\d{1,4})*)\\](?!\\])')

        def _double_bracket_pointers(text: str, n_citations: int=0, slot_map=None) -> str:
            """Emit `[[n]]` -- the only pointer form the judge counts -- but ONLY for
    numbers that actually resolve against the shipped citations array.

    The judge's rule, quoted from its own trace: "`[n]` is ordinary answer
    content, not a citation pointer" and "In prose, require a valid `[[n]]`
    pointer". MEASURED 2026-08-20 (batch 81b84664, 100 rows): answers
    byte-identical to the reference still scored 0 because every pointer we
    emitted was single-bracket -- the judge recorded "Answer 2 has zero valid
    citations".

    RANGE GATE ADDED 2026-08-22, MEASURED on batch 7af93041 (our 50 rows vs the
    champion's 150). Converting UNCONDITIONALLY makes things worse, not better:
    our numbers are LEDGER SLOT IDS, not positions, so 17 of our 18
    pointer-bearing rows emitted at least one out-of-range pointer -- e.g.
    `[[24]]` and `[[32]]` against a 2-item array -- and the judge answered
    verbatim "out of range. This is a citation defect." The champion emits
    `[[n]]` on 68 of its 150 rows with ZERO out of range: always contiguous from
    1, always bounded by the array length. Bare `[n]` is merely IGNORED, while
    an out-of-range `[[n]]` is COUNTED AGAINST US, so converting a number we
    cannot resolve turns an invisible defect into a scored one. Gate, do not
    guess -- an unresolvable number stays single-bracket and stays ignored.
    Prose only: the judge scopes the rule to prose and treats JSON as exempt.
    Idempotent -- the lookarounds skip anything already doubled.

    RENUMBER ADDED 2026-08-22 (SPEC_pointer_renumber.md 5c). The range gate alone was
    not enough: our numbers are LEDGER SLOT IDS and the judge resolves 1-based POSITIONS
    into the shipped array, so on real rows the gate simply declined and we fell back to
    bare `[n]`, which the judge treats as ordinary prose. `slot_map` carries
    slot -> position, built at the call site from what `_citations_for` actually kept.
    **When the map is non-empty it is AUTHORITATIVE -- no range fallback.** MEASURED
    106/106 rows: len(distinct cited numbers) == len(citations), so the transform is a
    pure permutation and a number ABSENT from the map was never a citation; converting
    it would misattribute the claim. The range branch survives only for the exception
    path, where citations == [] and no map exists.
    """
            if not text:
                return text
            try:
                    ...
                    for n in nums:
                        ...
                        if slot_map:
                            ...
                            if pos is None:
                                return m.group(0)
                            out.append('[[%d]]' % pos)
                        elif 1 <= v <= n_citations:
                            out.append('[[%d]]' % v)
                        else:
                            return m.group(0)
                    return ''.join(out)
                return _SINGLE_CITE_RE.sub(_expand, text)
            except Exception:
                return text
        _SCAFFOLD_LINE_RE = re.compile('^[ \\t]*(?:Best-supported findings[^\\n]*|sources retrieved:[^\\n]*|Evidence coverage:[^\\n]*)\\n?', re.I | re.M)

        def _strip_scaffolding(text: str) -> str:
            """Remove our own scaffolding lines before the answer ships.

    MEASURED 2026-08-22: rows carrying "Best-supported findings" (n=29) or
    "Evidence coverage:" (n=19) scored 0.0000 -- every single one. Neither is a
    claim; the first is a digest preamble the loop rules already forbid, the
    second is a bare list of ledger ids attached to nothing.
    Never returns empty: if stripping would leave nothing, keep the original,
    because an empty answer is a guaranteed zero rather than a merely bad one.
    """
            if not text:
                return text
            try:
                out = _SCAFFOLD_LINE_RE.sub('', text).strip()
                return out or text
            except Exception:
                return text
    ...
    median-cited-slice == SEARCH_EXCERPT_CHARS (1800 on 0dcd57c6)."""
            ...
            for n in numbers:
                if 1 <= n <= len(ledger.rows) and (ledger.rows[n - 1].get('kind') or '') == 'fetch':
                    ...
                    shallow.append(n)
            return deep + shallow
        COVERAGE_MAX_MEMBERS = 24
        COVERAGE_WIN_CHARS = 1400
        COVERAGE_BLOB_CHARS = 4000
        COVERAGE_MIN_S = 45.0
        _COVERAGE_ENUM_RE = re.compile('\\b(?:every|each|all (?:of )?the|consider exactly those|one entry per|for each)\\b|\\b(?:largest|smallest|most recent|latest|highest|lowest|earliest)\\b[^.]{0,60}\\b(?:among|of th(?:e|ose)|in that|per)\\b', re.I)
        _COVERAGE_ID_RE = re.compile('\\b(?:[A-Z]{1,2}-[A-Z0-9]{3,5}|N[0-9]{2,5}[A-Z]{0,2}|[A-Z]{2,6}-[0-9]{4}-[0-9]{2}-[0-9]{2}[A-Z0-9:+]*)\\b')

        def _enum_task_trigger(question, schema):
            """Deterministic: does this task range over an enumerated set?"""
            if isinstance(schema, dict):
                if schema.get('type') == 'array' or isinstance(schema.get('items'), dict):
                    return True
                props = schema.get('properties') or {}
                if len(schema.get('required') or props) >= 3:
                    return True
                for sub in props.values():
                    if isinstance(sub, dict) and (sub.get('type') == 'array' or isinstance(sub.get('items'), dict)):
                        return True
            return bool(_COVERAGE_ENUM_RE.search(question or ''))

        def _coverage_members(question, answer, ledger):
            """Member identifiers: id-class tokens (question+answer+ledger) plus the
    answer's own list/bold leads. Bounded, deterministic."""
            mem = []
            seen = set()
            ...
            def take(s):
                ...
                if 2 < len(s) <= 60 and k not in seen:
                    seen.add(k)
                    mem.append(s)
            for src_text in (question or '', answer or ''):
                for t in _COVERAGE_ID_RE.findall(src_text):
                    take(t)
            for row in ledger.rows:
                for t in _COVERAGE_ID_RE.findall((row.get('text') or '')[:40000]):
                    take(t)
                    if len(mem) >= COVERAGE_MAX_MEMBERS:
                        break
                if len(mem) >= COVERAGE_MAX_MEMBERS:
                    break
            for t in re.findall('^\\s*[-*]\\s+\\*{0,2}([A-Z][^:*(\\n]{2,50})', answer or '', re.M):
                take(t)
                if len(mem) >= COVERAGE_MAX_MEMBERS:
                    break
            return mem[:COVERAGE_MAX_MEMBERS]

        async def _citation_coverage_sweep(question, query, answer, ledger, deadline):
            """Post-answer, pre-citation: nominate one narrow retained window per
    enumerated member so every run ships the measured winning citation shape.
    Deterministic on the happy path; ONE bounded search for absent members."""
            schema = getattr(query, 'output_schema', None)
            if not _enum_task_trigger(question, schema) or not ledger.rows:
                return answer
            members = _coverage_members(question, answer, ledger)
            if not members:
                return answer
            missing = []
            hit_rows = {}
            for m in members:
                ...
                row = ledger.rows[i]
                terms = _key_terms(m)
                wins = _best_windows(txt, terms or {low}, COVERAGE_WIN_CHARS, k=1) if txt else []
                s, e = wins[0] if wins else (0, 0)
                if not s <= p < e:
                    s, e = (max(0, p - 500), min(len(txt), p + 900))
                ret = row.setdefault('retained', [])
                if not any((a <= p < b for a, b in ret)):
                    ret.append([int(s), int(e)])
                hit_rows[i + 1] = hit_rows.get(i + 1, 0) + 1
            for n in hit_rows:
                row = ledger.rows[n - 1]
                spans = row.get('spans') or []
                width = sum((max(0, int(b) - int(a)) for a, b in spans))
                if width > COVERAGE_BLOB_CHARS and row.get('retained'):
                    row['spans'] = [list(w) for w in sorted(row['retained'], key=lambda w: w[1] - w[0])[:4]]
            add = [n for n in sorted(hit_rows) if '[%d]' % n not in (answer or '')]
            if add:
                answer = (answer or '').rstrip() + '\n\nEvidence coverage: ' + ''.join(('[%d]' % n for n in add))
            if missing and deadline - monotonic() > COVERAGE_MIN_S:
                try:
                    _cov_out = await asyncio.wait_for(_do_search(' '.join(missing[:3])[:200], ledger), timeout=18.0)
                    _commit_tool_output(_cov_out, ledger)
                except Exception:
                    pass
            return answer

        def _citations_for(answer: str, ledger: EvidenceLedger, slots_out=None) -> list[CitationRef]:
    ...
    and a 4000-char cited answer was discarded for its opening clause."""
            ...
            if len(s) < 400 and (_REFUSAL_ONLY_RE.match(s) or _INTENT_NARRATION_RE.match(s)):
                return False
            return True
        _COMMIT_RULES = "You are writing the FINAL ANSWER to a research question from evidence that has already been gathered. You have NO tools — never emit tool syntax. A judge compares your answer with a strong reference and credits only claims carrying an [n] citation to the numbered evidence.\n\nFORM OVERRIDE: if the question names a form -- 'in prose', 'a paragraph', 'terse', 'one sentence', XML, a table -- obey it exactly. For prose/paragraph write continuous paragraphs with inline [n] citations: NO headers, NO bullet lists, NO proof section.\n\nSHAPE: the first words are the answer entities themselves — no preamble, no remark about evidence quality. Then a short proof section: the candidate pool, each condition applied, one line per qualifier (cited) and one line per rejected member with its cited reason — every member gets its own line, never several swept into one clause. Reproduce figures and dates VERBATIM. Name ALL qualifying members — omitting one scores as wrong. Obey any literal formatting demand in the question — sort order, comma-separated, a requested count, 'without the word X' meaning delete that word — the shape is graded too. FALSE PREMISE: if the evidence shows a premise of the question is wrong (no such event, wrong date, wrong entity, a figure that never existed), say so in the first sentence, cite the source that contradicts it, and then answer the corrected question. Never say what the evidence does not contain; commit to the best-supported answer you can defend."
    ...
    message window."""
        ...
        async def _schema_output(question: str, answer: str, schema, deadline: float) -> object | None:
            ask = f'Convert the answer to a JSON value valid under the schema. Output ONLY the JSON value. Never put citation markers such as [[1]] or [1] inside any value; values carry the answer only.\n\nSchema:\n{json.dumps(schema)}\n\nQuestion:\n{question}\n\nAnswer:\n{answer[:14000]}'
            for lane, model in ((LLM_LANE_A, SCHEMA_MODEL), (LLM_LANE_A, RESORT_MODEL), (LLM_LANE_B, LOOP_MODEL_B), (LLM_LANE_C, RESCUE_MODEL_C)):
                left = deadline - monotonic()
                if left < 12.0:
                    break
                try:
                    ...
                    if _matches_schema_shape(value, schema):
                        fitted = _fit_to_schema(value, schema)
                        if _output_ok(fitted, schema):
                            return fitted
                    if isinstance(value, dict) and len(value) == 1:
                        inner = list(value.values())[0]
                        if _matches_schema_shape(inner, schema):
                            fitted = _fit_to_schema(inner, schema)
                            if _output_ok(fitted, schema):
                                return fitted
                except Exception:
                    continue
    ...
    while a pasted digest reads as a contract violation and is scored as garbage."""
            ...
            return '\n'.join(out)
        _FIT_PAD = 'unknown'
        _PTR_IN_VALUE_RE = re.compile('\\s*\\[\\[?\\d{1,3}\\]\\]?')

        def _fit_string(value, schema) -> str:
            if isinstance(value, str):
                s = value
            elif value is None:
                s = ''
                ...
                s = str(value)
            if _PTR_IN_VALUE_RE.search(s):
                s = ' '.join(_PTR_IN_VALUE_RE.sub(' ', s).split())
            lo = schema.get('minLength')
            hi = schema.get('maxLength')
            lo_ok = isinstance(lo, int) and (not isinstance(lo, bool)) and (lo > 0)
            hi_ok = isinstance(hi, int) and (not isinstance(hi, bool)) and (hi >= 0)
            if hi_ok and len(s) > hi:
                s = s[:hi]
            if lo_ok and len(s) < lo:
                pad = _FIT_PAD if not hi_ok or hi >= len(_FIT_PAD) else 'n/a'[:hi] or '?'
                if not s.strip():
                    s = pad
                while len(s) < lo:
                    s = s + ' ' + pad
        ...
        def _fit_number(value, kind: str):
            if isinstance(value, bool):
                value = int(value)
            if isinstance(value, (int, float)):
                v = value
            else:
                found = _NUM_IN_TEXT_RE.search(_CITE_NUM_RE.sub(' ', str(value or '')))
                try:
                    v = float(found.group(0).replace(',', '')) if found else 0
                except Exception:
                    v = 0
            try:
                f = float(v)
            ...
            if kind == 'integer':
                return int(f)
            return int(f) if f == int(f) and abs(f) < 1000000000000000.0 else f

        def _fit_bool(value) -> bool:
            if isinstance(value, bool):
                return value
            if isinstance(value, (int, float)):
                return value != 0
            ...
            if not text:
                return False
            return re.match('(no\\b|false\\b|none\\b|0\\b|n\\b)', text, re.I) is None

        def _fit_to_schema(value, schema, depth: int=0):
            """Deterministically reshape `value` into an instance valid under the generated-schema
    subset. Never raises; keywords outside the subset are left alone."""
            ...
            if kind == 'object':
                ...
                if isinstance(value, dict):
                    src = value
                elif value is not None and value != '' and (len(keys) == 1):
                    src = {keys[0]: value}
                else:
                    src = {}
                out = {}
                for k in keys:
                    sub = props.get(k)
                    out[k] = _fit_to_schema(src.get(k), sub if isinstance(sub, dict) else {}, depth + 1)
                if schema.get('additionalProperties') is not False and isinstance(value, dict):
            ...
            if kind == 'array':
                ...
                if isinstance(value, list):
                    ...
                    arr = [p.strip(' -*\t') for p in re.split('[\\n;]|,(?![^(]*\\))', value)]
                    arr = [p for p in arr if p]
                else:
                    arr = [value]
                lo = schema.get('minItems')
                hi = schema.get('maxItems')
                if isinstance(hi, int) and (not isinstance(hi, bool)) and (hi >= 0) and (len(arr) > hi):
                    arr = arr[:hi]
                arr = [_fit_to_schema(x, items, depth + 1) for x in arr]
                if isinstance(lo, int) and (not isinstance(lo, bool)) and (lo > 0):
                    while len(arr) < lo:
                        arr.append(_fit_to_schema(None, items, depth + 1))
                return arr
            if kind == 'string':
                return _fit_string(value, schema)
            if kind in ('integer', 'number'):
                return _fit_number(value, kind)
            if kind == 'boolean':
                return _fit_bool(value)
            if kind == 'null':
                return None
            return value

        def _placeholder_for_schema(schema):
            """A guaranteed-valid instance of the schema with every field unknown. It scores nothing,
    but a valid payload is JUDGED; an invalid one is miner_response_invalid = hard 0."""
            return _fit_to_schema(None, schema)

        def _output_ok(value, schema, depth: int=0) -> bool:
            """Local check for the generated-schema subset; mirrors the host's jsonschema verdict
    on every keyword the generator can emit. Unknown keywords are not checked."""
            if depth > 12 or not isinstance(schema, dict):
                return True
            enum = schema.get('enum')
                ...
                if schema.get('additionalProperties') is False:
                    for k in value:
                        if k not in props:
                            return False
                for k, v in value.items():
                    if k in props and (not _output_ok(v, props[k], depth + 1)):
                        return False
                return True
            if kind == 'array':
                ...
                if isinstance(items, dict):
                    for x in value:
                        if not _output_ok(x, items, depth + 1):
                            return False
                return True
            if kind == 'string':
    ...
    that carries an [n]: that is a real answer, however it opens."""
        ...
        async def query(query: Query) -> Response:
            ...
            try:
                ...
                if schema is not None:
                    try:
                        return Response(output=_placeholder_for_schema(schema))
                    except Exception:
                        pass
        ...
        async def _solve(query: Query, question: str) -> Response:
            ...
            _schema_true_deadline = deadline
            if getattr(query, 'output_schema', None) is not None and deadline - monotonic() > SCHEMA_SLICE_S + 60.0:
                deadline = deadline - SCHEMA_SLICE_S
            _SEARCH_HITS[0] = 0
            _SEARCH_ALT[0] = None
            try:
            ...
            try:
                ...
                answer = ''
            if _SEARCH_HITS[0] == 0 and deadline - monotonic() > SEARCH_FLOOR_MIN_S:
                try:
                    _floor_out = await asyncio.wait_for(_do_search(question[:300], ledger), timeout=SEARCH_TIMEOUT_S + SEARCH_FLOOR_TIMEOUT_S + 8.0)
                    _commit_tool_output(_floor_out, ledger)
                except Exception:
                    pass
                if ledger.rows:
                    try:
                        _floored = await _write_from_digest(question, ledger, deadline)
                        if _is_usable_answer(_floored):
                            answer = _floored
                    except Exception:
                        pass
            try:
            ...
            if not _is_usable_answer(answer):
                ...
                if _is_usable_answer(fallback):
                    answer = fallback
            try:
                answer = await _superlative_table_sweep(question, answer, messages, ledger, deadline)
            except Exception:
                pass
            try:
                answer = await _format_contract_sweep(question, answer, messages, ledger, deadline)
            except Exception:
                pass
            try:
                answer = await _citation_coverage_sweep(question, query, answer, ledger, deadline)
            except Exception:
                pass
            _slots = []
            try:
                citations = _citations_for(answer, ledger, _slots)
            except Exception:
                citations = []
                _slots = []
            _slot_map = {n: i + 1 for i, n in enumerate(_slots[:len(citations)])}
            answer = _normalize_brackets(answer)
            answer = _strip_lead_narration(answer)
            answer = _strip_scaffolding(answer)
            text = _cap(answer) or f'Best-effort answer unavailable for: {question[:400]}'
            text = _strip_scaffolding(text)
            text = _double_bracket_pointers(text, len(citations or ()), _slot_map)
            schema = getattr(query, 'output_schema', None)
            if schema is not None:
                structured = None
                try:
                    structured = await _schema_output(question, answer, schema, _schema_true_deadline)
                except Exception:
                    structured = None
                ...
                if basis is not answer:
                    try:
                        salvaged = await _schema_output(question, basis, schema, _schema_true_deadline)
                    except Exception:
                        salvaged = None
                ...
                try:
                    forced = _fit_to_schema(_coerce_to_schema(_cap(basis), query.output_schema), query.output_schema)
                    if not _output_ok(forced, query.output_schema):
                        forced = _placeholder_for_schema(query.output_schema)
                    return Response(output=forced, citations=citations or None)
                except Exception:
                    try:
                        return Response(output=_placeholder_for_schema(query.output_schema), citations=citations or None)
                    except Exception:
                        pass
            try:
    ...
    """
        ...
        async def _solve(query: Query, question: str) -> Response:
            ...
            try:
                ...
                pass
            try:
                _staged = await _ground_figures(question, answer, messages, ledger, deadline)
                if _is_usable_answer(_staged):
                    answer = _staged
            except Exception:
                pass
            try:
                _staged = await _anchor_primary_source(question, answer, messages, ledger, deadline)
                if _is_usable_answer(_staged):
                    answer = _staged
            except Exception:
                pass
    ...
    """
        ...
        return query
    _atrxsbsiwj = _jfjntcjtup()
    _jlogahnuue = _bevsbpmakv()
    _xtgamtnsun = _sdvfuefmgl()
    _ugwxgbnhfg = 290.0
    _dzakdafdns = 250.0
    _gljgshzrpz = 90.0

    async def _gaouoorgaz(query: Query, agents: tuple) -> Response:
        started = time.monotonic()
        last_exc = None
        first = True
        for agent in agents:
            remaining = _ugwxgbnhfg - (time.monotonic() - started)
            if first:
                budget = _dzakdafdns if _dzakdafdns < remaining else remaining
                first = False
            else:
                if remaining < _gljgshzrpz:
                    break
            ...
            try:
                ...
                last_exc = exc
        return _huehrfovmt(query)

    async def query(query: Query) -> Response:
        _lttrzvaken['started'] = time.monotonic()
        try:
            index = _xjvlomfaiw(query)
            if index == 0:
                agents = (_atrxsbsiwj, _jlogahnuue, _xtgamtnsun)
            elif index == 1:
                agents = (_jlogahnuue, _xtgamtnsun, _atrxsbsiwj)
            elif index == 2:
                agents = (_xtgamtnsun, _atrxsbsiwj, _jlogahnuue)
            else:
                agents = (_atrxsbsiwj, _jlogahnuue, _xtgamtnsun)
            return await _gaouoorgaz(query, agents)
        except Exception:
            return _huehrfovmt(query)
    return query
...
"""
    ...
    """
        left = deadline - monotonic()
        if left < 18.0 or _spend_left() < WRAPUP_MIN_USD:
            return ''
        digest = _ledger_digest(ledger)
        ...
        try:
            note = await _chat(NOTE_SYSTEM, user, models=UTILITY_MODELS, max_tokens=700, timeout=min(20.0, left - TAIL_RESERVE_S), total_budget=max(6.0, left - TAIL_RESERVE_S))
        except Exception:
            return ''
...
def _sxvxmaflwd():
    ...
    async def query(query: Query) -> Response:
        ...
        try:
            ...
            return draft
    return query
_divfvforgs = _lxnxjoidnt()
_eezuemvbkz = _uhmzibckzo()
_ysnkvhdpsr = _hufvfmzzpz()
_lwlgapwpzu = _sxvxmaflwd()
_tqoveqgmsp = 290.0
_nnwaoczrmw = 250.0
_fmkyogswmb = 90.0

async def _gigewwylqd(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _tqoveqgmsp - (time.monotonic() - started)
        if first:
            budget = _nnwaoczrmw if _nnwaoczrmw < remaining else remaining
            first = False
        else:
            if remaining < _fmkyogswmb:
                break
            budget = remaining - 5.0
        if budget <= 0.0:
            break
        try:
            ...
            last_exc = exc
    return _riefucerld(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _cyijazrtbc['started'] = time.monotonic()
    try:
        index = _bxodmxhyog(query)
        if index == 0:
            agents = (_divfvforgs, _eezuemvbkz, _ysnkvhdpsr, _lwlgapwpzu)
        elif index == 1:
            agents = (_eezuemvbkz, _ysnkvhdpsr, _lwlgapwpzu, _divfvforgs)
        elif index == 2:
            agents = (_ysnkvhdpsr, _lwlgapwpzu, _divfvforgs, _eezuemvbkz)
        elif index == 3:
            agents = (_lwlgapwpzu, _divfvforgs, _eezuemvbkz, _ysnkvhdpsr)
        else:
            agents = (_divfvforgs, _eezuemvbkz, _ysnkvhdpsr, _lwlgapwpzu)
        return await _gigewwylqd(query, agents)
    except Exception:
        return _riefucerld(query)