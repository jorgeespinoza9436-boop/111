...
_vcfdhngkzy = {'started': None, 'text': None}
_aawksxvozu = 24000
_pmaoonvepz = 290.0
_dbooephpap = 250.0

def _qlhkxbsiww() -> float:
    started = _vcfdhngkzy['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _khgbedqqpf() -> float:
    return _pmaoonvepz - _qlhkxbsiww()
_txdkobpgys = _hsapi.llm_chat
_mwkiyrggvd = _hsapi.search_web
_ggrmvxxhzx = _hsapi.fetch_page
_jbfuashsvj = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _bpwipuxfxo(*args, **kwargs):
    if _qlhkxbsiww() >= _dbooephpap:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _jbfuashsvj})
            kwargs['messages'] = steered
    _result = await _txdkobpgys(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _gwitluxynq(_result)
    return _result

async def _ezcwhuorej(*args, **kwargs):
    if _qlhkxbsiww() >= _dbooephpap:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _mwkiyrggvd(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _byvukihzxv(*args, **kwargs):
    if _qlhkxbsiww() >= _dbooephpap:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _ggrmvxxhzx(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _bpwipuxfxo
_hsapi.search_web = _ezcwhuorej
_hsapi.fetch_page = _byvukihzxv
_zlrflrlebz = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_xnrcnrkoid = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_xeesucvlqk = 900
_kzjqbxvaik = 2

def _mcaupsvjyp(query: Query) -> int:
...
def _xyrpbvfzta(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _eequqfqpqg(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _mcaupsvjyp(query)
    if fields >= 3:
        return 2
    if _xyrpbvfzta(lowered, _zlrflrlebz):
        return 1
    if fields <= _kzjqbxvaik and len(text) <= _xeesucvlqk:
        return 0
    if _xyrpbvfzta(lowered, _xnrcnrkoid):
        return 0
    return 1

def _gwitluxynq(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _vcfdhngkzy['text'] = text.strip()[:_aawksxvozu]
    except Exception:
        pass

def _zuvllziofp(text: str):
...
def _xxcnrciegr(query: Query) -> Response:
    text = _vcfdhngkzy['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_aawksxvozu]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _zuvllziofp(text)
...
def _fmevocyrvg():
    """SN67 Harnyx miner — staged research protocol agent. [slot 11 build 2026-08-26T15:01:12+00:00]"""
    _S444S666_QUERY_TAG = 's444s666b2-hk6720'
    import asyncio
    import json
    import re
    from time import perf_counter
    ...
    async def query(query: Query) -> Response:
        ...
        try:
            ...
            return draft
    return query
_nxjbuqsmdu = _ikspouvxls()
_pntdiaulmm = _evvpznuzbh()
_lxpfmtcvkh = _bysaqswshe()
_zmhfolfcfe = _bskrtctqrj()
_vjriiaexbd = _fmevocyrvg()
_pmaoonvepz = 290.0
_jkstjwbxwu = 250.0
_sfnnmojbpw = 90.0

async def _zrfdxmarwq(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _pmaoonvepz - (time.monotonic() - started)
        if first:
            budget = _jkstjwbxwu if _jkstjwbxwu < remaining else remaining
            first = False
        else:
            if remaining < _sfnnmojbpw:
                break
            budget = remaining - 5.0
        if budget <= 0.0:
            break
        try:
            ...
            last_exc = exc
    return _xxcnrciegr(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _vcfdhngkzy['started'] = time.monotonic()
    try:
        index = _eequqfqpqg(query)
        if index == 0:
            agents = (_nxjbuqsmdu, _pntdiaulmm, _lxpfmtcvkh, _zmhfolfcfe, _vjriiaexbd)
        elif index == 1:
            agents = (_pntdiaulmm, _lxpfmtcvkh, _zmhfolfcfe, _vjriiaexbd, _nxjbuqsmdu)
        elif index == 2:
            agents = (_lxpfmtcvkh, _zmhfolfcfe, _vjriiaexbd, _nxjbuqsmdu, _pntdiaulmm)
        elif index == 3:
            agents = (_zmhfolfcfe, _vjriiaexbd, _nxjbuqsmdu, _pntdiaulmm, _lxpfmtcvkh)
        elif index == 4:
            agents = (_vjriiaexbd, _nxjbuqsmdu, _pntdiaulmm, _lxpfmtcvkh, _zmhfolfcfe)
        else:
            agents = (_nxjbuqsmdu, _pntdiaulmm, _lxpfmtcvkh, _zmhfolfcfe, _vjriiaexbd)
        return await _zrfdxmarwq(query, agents)
    except Exception:
        return _xxcnrciegr(query)