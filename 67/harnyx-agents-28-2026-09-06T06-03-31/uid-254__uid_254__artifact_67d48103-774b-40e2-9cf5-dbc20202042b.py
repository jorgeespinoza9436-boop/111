...
def _compose_talus_archive_slot06_agent_entry():
...
_talus_archive_slot06_agent_query_entry = _compose_talus_archive_slot06_agent_entry()


def _compose_quartz_lantern_slot06_agent_entry():
...
_quartz_lantern_slot06_agent_query_entry = _compose_quartz_lantern_slot06_agent_entry()


def _compose_lumen_harbor_slot06_agent_entry():
...
_lumen_harbor_slot06_agent_query_entry = _compose_lumen_harbor_slot06_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "TalusArchiveSlot06Agent"
    ...
    if (
        ...
        return "QuartzLanternSlot06Agent"
    return "LumenHarborSlot06Agent"


class TalusArchiveSlot06Agent:
    async def __call__(self, query: Query) -> Response:
        return await _talus_archive_slot06_agent_query_entry(query)


class QuartzLanternSlot06Agent:
    async def __call__(self, query: Query) -> Response:
        return await _quartz_lantern_slot06_agent_query_entry(query)


class LumenHarborSlot06Agent:
    async def __call__(self, query: Query) -> Response:
        return await _lumen_harbor_slot06_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = TalusArchiveSlot06Agent()
_ANALYTICAL_FIELD_AGENT = QuartzLanternSlot06Agent()
_BROAD_FIELD_AGENT = LumenHarborSlot06Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "TalusArchiveSlot06Agent",
    "QuartzLanternSlot06Agent",
    "LumenHarborSlot06Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "TalusArchiveSlot06Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "QuartzLanternSlot06Agent":
