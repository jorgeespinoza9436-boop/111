...
_embkuogruq = {'started': None, 'text': None}
_ojxquwojcv = 24000
_updsxzccoj = 290.0
_ficzdlwnyq = 250.0

def _lekznhfgak() -> float:
    started = _embkuogruq['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _luagzofggb() -> float:
    return _updsxzccoj - _lekznhfgak()
_jvuaxiefbz = _hsapi.llm_chat
_zrsqziekeo = _hsapi.search_web
_ozmwaodcdj = _hsapi.fetch_page
_kfkospfdme = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _vyhuoiekwm(*args, **kwargs):
    if _lekznhfgak() >= _ficzdlwnyq:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _kfkospfdme})
            kwargs['messages'] = steered
    _result = await _jvuaxiefbz(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _cayxctlfqt(_result)
    return _result

async def _cvcjluvqpi(*args, **kwargs):
    if _lekznhfgak() >= _ficzdlwnyq:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _zrsqziekeo(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _wkynjzyfga(*args, **kwargs):
    if _lekznhfgak() >= _ficzdlwnyq:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _ozmwaodcdj(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _vyhuoiekwm
_hsapi.search_web = _cvcjluvqpi
_hsapi.fetch_page = _wkynjzyfga
_ehmxzbaitk = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_srzeiwwcyb = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_jwznoxaouh = 900
_zpszmcqhyx = 2

def _lbmpadljqq(query: Query) -> int:
...
def _rcqbylcugz(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _pnkghqauag(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _lbmpadljqq(query)
    if fields >= 3:
        return 2
    if _rcqbylcugz(lowered, _ehmxzbaitk):
        return 1
    if fields <= _zpszmcqhyx and len(text) <= _jwznoxaouh:
        return 0
    if _rcqbylcugz(lowered, _srzeiwwcyb):
        return 0
    return 1

def _cayxctlfqt(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _embkuogruq['text'] = text.strip()[:_ojxquwojcv]
    except Exception:
        pass

def _aihossrugh(text: str):
...
def _sjjnzhotzq(query: Query) -> Response:
    text = _embkuogruq['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_ojxquwojcv]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _aihossrugh(text)
...
def _compose_graft1_s0_entry():
    ...
    STAGE_EVIDENCE_CHARS = 9500
    STAGE_UNSPANNED_CHARS = 700
    STAGE_REWRITE_TIMEOUT = 30.0
    STAGE_REWRITE_MIN_SECONDS = 16.0
    STAGE_SEARCH_MIN_SECONDS = 50.0
    STAGE_TAIL_RESERVE_SECONDS = 26.0
    ROSTER_BUDGET_SECONDS = 25.0


    # --- stage: value-support repair ------------------------------------------
    # Every number in the delivered answer has to be a number some retrieved page
    # actually states. This checks each one against the note text and, when a value
    # has no source behind it, makes the rewrite drop or replace it rather than
    # leaving an unsupported figure carrying a marker.
    ...
    async def _value_repair(
        ...
        listed = ", ".join(bad)
        directive = (
            f"These values appear in the draft but no retrieved passage states them: "
            f"{listed}. Replace each with the figure the evidence does state, carrying "
            "that source's [n] marker, or remove the claim. Do not restate any of them "
            "unless a passage below contains it."
        )
    ...
    async def _plain_query(query: Query, budget: float) -> Response:
        ...
        try:
            ...
            # --- ADDED STAGES: temporal-alignment repair then value-support repair ---
...
def _diltxnbxlz():
    ...
    """
        ...
        return response
    return query
_zfecnoqksw = _compose_graft1_s0_entry()
_wmfdxonrxg = _quyxvmeuoh()
_ngkxedvlke = _zjttxxdgog()
_ludwztjdri = _diltxnbxlz()
_updsxzccoj = 290.0
_rgrabkednm = 250.0
_nnhzuizgqv = 90.0

async def _jfjstxlwwp(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _updsxzccoj - (time.monotonic() - started)
        if first:
            budget = _rgrabkednm if _rgrabkednm < remaining else remaining
            first = False
        else:
            if remaining < _nnhzuizgqv:
                break
        ...
        try:
            ...
            last_exc = exc
    return _sjjnzhotzq(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _embkuogruq['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _jfjstxlwwp(query, (_ngkxedvlke, _zfecnoqksw, _wmfdxonrxg, _ludwztjdri))
        index = _pnkghqauag(query)
        if index == 0:
            agents = (_zfecnoqksw, _wmfdxonrxg, _ngkxedvlke, _ludwztjdri)
        elif index == 1:
            agents = (_wmfdxonrxg, _ngkxedvlke, _ludwztjdri, _zfecnoqksw)
        elif index == 2:
            agents = (_ngkxedvlke, _ludwztjdri, _zfecnoqksw, _wmfdxonrxg)
        elif index == 3:
            agents = (_ludwztjdri, _zfecnoqksw, _wmfdxonrxg, _ngkxedvlke)
        else:
            agents = (_zfecnoqksw, _wmfdxonrxg, _ngkxedvlke, _ludwztjdri)
        return await _jfjstxlwwp(query, agents)
    except Exception:
        return _sjjnzhotzq(query)
_BUILDW_TAG_m2 = "sn45-a9476a184dc5"
