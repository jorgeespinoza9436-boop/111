...
def _compose_yarrow_archive_slot02_agent_entry():
...
_yarrow_archive_slot02_agent_query_entry = _compose_yarrow_archive_slot02_agent_entry()


def _compose_garnet_archive_slot02_agent_entry():
...
_garnet_archive_slot02_agent_query_entry = _compose_garnet_archive_slot02_agent_entry()


def _compose_fjord_marten_slot02_agent_entry():
...
_fjord_marten_slot02_agent_query_entry = _compose_fjord_marten_slot02_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "YarrowArchiveSlot02Agent"
    ...
    if (
        ...
        return "GarnetArchiveSlot02Agent"
    return "FjordMartenSlot02Agent"


class YarrowArchiveSlot02Agent:
    async def __call__(self, query: Query) -> Response:
        return await _yarrow_archive_slot02_agent_query_entry(query)


class GarnetArchiveSlot02Agent:
    async def __call__(self, query: Query) -> Response:
        return await _garnet_archive_slot02_agent_query_entry(query)


class FjordMartenSlot02Agent:
    async def __call__(self, query: Query) -> Response:
        return await _fjord_marten_slot02_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = YarrowArchiveSlot02Agent()
_ANALYTICAL_FIELD_AGENT = GarnetArchiveSlot02Agent()
_BROAD_FIELD_AGENT = FjordMartenSlot02Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "YarrowArchiveSlot02Agent",
    "GarnetArchiveSlot02Agent",
    "FjordMartenSlot02Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "YarrowArchiveSlot02Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "GarnetArchiveSlot02Agent":
