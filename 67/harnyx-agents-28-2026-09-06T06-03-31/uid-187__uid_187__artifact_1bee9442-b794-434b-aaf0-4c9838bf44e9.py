...
def _compose_granite_ledger_agent_entry():
...
def _compose_quartz_current_agent_entry():
...
_GRANITE_LEDGER = _compose_granite_ledger_agent_entry()
_QUARTZ_CURRENT = _compose_quartz_current_agent_entry()
_HARBOR_BEACON = _compose_harbor_beacon_agent_entry()

import re as _route_re

# Long source-table questions need complete row traversal with compact in-page
# grep; the ledger branch owns that path. Output contracts go to the branch
# whose structured lane is checked against the official validator. Everything
# else takes the grounding branch.
...
def _route_label(query: Query) -> str:
    ...
    if not text.strip():
        return "harbor"
    if _route_re.search(r"\b(?:verify|true or false|fact-check)\b", text, _route_re.IGNORECASE):
        return "harbor"
    table_hits = len(_ROUTE_TABLE_RE.findall(text)) + len(_ROUTE_SWEEP_RE.findall(text))
    if _ROUTE_TABLE_RE.search(text) and _ROUTE_SWEEP_RE.search(text) and table_hits >= 3:
        return "granite"
    return "harbor"


_ROUTES = {
    "granite": (_GRANITE_LEDGER, _HARBOR_BEACON),
    "quartz": (_QUARTZ_CURRENT, _GRANITE_LEDGER),
    "harbor": (_HARBOR_BEACON, _GRANITE_LEDGER),
}


@entrypoint("query")
async def query(query: Query) -> Response:
    primary, fallback = _ROUTES[_route_label(query)]
    try:
