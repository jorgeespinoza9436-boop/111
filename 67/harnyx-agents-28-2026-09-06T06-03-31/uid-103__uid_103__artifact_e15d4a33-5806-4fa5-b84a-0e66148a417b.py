...
def _compose_fjord_orchid_slot13_agent_entry():
    ...
    _GIVEUP_RE = re.compile(
        r"cannot be (?:determined|produced|established|computed|conclusively|reliably)|"
        r"could not be (?:determined|produced|established|computed)|"
        r"can(?:no|')t be (?:determined|produced|established)|"
        r"unable to (?:determine|produce|establish|compute|provide a)|"
        r"do(?:es)? not (?:contain|include|provide) (?:the )?(?:complete|full|enough)|"
        r"insufficient (?:evidence|information|data)|not (?:be )?possible to determine|"
        r"the requested .{0,40}?cannot be", re.I)
    ...
    async def _write_from_digest(question: str, ledger: EvidenceLedger, deadline: float,
        ...
        if commit:
            sys_content = _COMMIT_RULES + (
                "\n\nCRITICAL — DO NOT HEDGE. Never write that the answer 'cannot be "
                "determined / produced / established', that evidence is 'insufficient', "
                "or that a section 'does not contain' enough: the judge makes a FORCED "
                "choice and any such hedge scores zero. From the evidence, COMMIT to the "
                "single most-supported specific answer — name the exact entity, value, "
                "and figures, choosing the best-supported candidate even under partial "
                "uncertainty. If the evidence positively PROVES that no item satisfies "
                "the criteria, state THAT as the definitive answer with its proof; only "
                "absence-of-evidence is forbidden, not a proven negative. Put any "
                "residual uncertainty in at most one short trailing clause.")
            user_content = (
                f"Question: {question}\n\nNumbered evidence you gathered (cite facts by "
                f"these [n]):\n\n{digest}\n\nYour previous draft hedged instead of "
                "answering. Rewrite now as a COMMITTED specific answer: first words are "
                "the answer entities with their figures; every claim carries its [n]; "
                "then a short proof section.")
        else:
            sys_content = _COMMIT_RULES
            user_content = (
        ...
        convo = [{"role": "system", "content": sys_content},
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        if _is_usable_answer(answer) and ledger.rows and _GIVEUP_RE.search(answer):
            try:
...
_fjord_orchid_slot13_agent_query_entry = _compose_fjord_orchid_slot13_agent_entry()


def _compose_aster_orchid_slot13_agent_entry():
    ...
    _dblzilceef = {'started': None, 'text': None}
    _oezjhejbom = 24000
    _vwerolfzil = 290.0
    _hpsyyhmuns = 250.0

    def _bsrxbxsucz() -> float:
        started = _dblzilceef['started']
        if started is None:
            return 0.0
        return max(0.0, time.monotonic() - started)

    def _hnrntgdohv() -> float:
        return _vwerolfzil - _bsrxbxsucz()
    _qcowhuvjnd = _hsapi.llm_chat
    _fhxtsvuipg = _hsapi.search_web
    _lnwwrbpjke = _hsapi.fetch_page
    _jjsoaeexrl = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

    async def _ldgmfbvfxx(*args, **kwargs):
        if _bsrxbxsucz() >= _hpsyyhmuns:
            messages = kwargs.get('messages')
            if messages is not None:
                steered = list(messages)
                steered.append({'role': 'user', 'content': _jjsoaeexrl})
                kwargs['messages'] = steered
        _result = await _qcowhuvjnd(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
        _ccjqcxxopi(_result)
        return _result

    async def _qlhhqtasew(*args, **kwargs):
        if _bsrxbxsucz() >= _hpsyyhmuns:
            raise TimeoutError('research cutoff reached; finalize with gathered evidence')
        return await _fhxtsvuipg(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

    async def _ehrrmltqbd(*args, **kwargs):
        if _bsrxbxsucz() >= _hpsyyhmuns:
            raise TimeoutError('research cutoff reached; finalize with gathered evidence')
        return await _lnwwrbpjke(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _hsapi.llm_chat = _ldgmfbvfxx
    _hsapi.search_web = _qlhhqtasew
    _hsapi.fetch_page = _ehrrmltqbd
    _nrtvfzubez = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
    _hctpjlngpg = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
    _txiifehzck = 900
    _zzakiykyyx = 2

    def _wzlmwcjjpw(query: Query) -> int:
        schema = getattr(query, 'output_schema', None)
        if not isinstance(schema, dict):
            return 0
    ...
    def _phztzlqfvk(text: str, terms: tuple) -> bool:
        for term in terms:
            if term in text:
                return True
        return False

    def _bflyordldu(query: Query) -> int:
        text = (getattr(query, 'text', '') or '').strip()
        lowered = text.lower()
        fields = _wzlmwcjjpw(query)
        if fields >= 3:
            return 2
        if _phztzlqfvk(lowered, _nrtvfzubez):
            return 1
        if fields <= _zzakiykyyx and len(text) <= _txiifehzck:
            return 0
        if _phztzlqfvk(lowered, _hctpjlngpg):
            return 0
        return 1

    def _ccjqcxxopi(result: object) -> None:
        try:
            ...
            if text and text.strip():
                _dblzilceef['text'] = text.strip()[:_oezjhejbom]
        except Exception:
            pass

    def _lrlmpdgkue(text: str):
    ...
    def _kthdbpvoxv(query: Query) -> Response:
        text = _dblzilceef['text']
        if not text or not text.strip():
            text = 'A complete answer could not be produced within the available time budget.'
        text = text.strip()[:_oezjhejbom]
        schema = getattr(query, 'output_schema', None)
        if schema is not None:
            parsed = _lrlmpdgkue(text)
    ...
    def _tiwdnesmfp():
        ...
        async def query(query: Query) -> Response:
            ...
            try:
                ...
                return response
        return query
    _bbosgzqmyp = _lfffjjaxho()
    _eckmbpzbse = _akrrcccega()
    _kiweqoyehv = _cuvdvrtzky()
    _tuqajkwbkk = _tiwdnesmfp()
    _vwerolfzil = 290.0
    _januggvpnp = 250.0
    _xolcogfely = 90.0

    async def _odwincvxga(query: Query, agents: tuple) -> Response:
        started = time.monotonic()
        last_exc = None
        first = True
        for agent in agents:
            remaining = _vwerolfzil - (time.monotonic() - started)
            if first:
                budget = _januggvpnp if _januggvpnp < remaining else remaining
                first = False
            else:
                if remaining < _xolcogfely:
                    break
            ...
            try:
                ...
                last_exc = exc
        return _kthdbpvoxv(query)

    async def query(query: Query) -> Response:
        _dblzilceef['started'] = time.monotonic()
        try:
            if getattr(query, 'fast', False):
                return await _odwincvxga(query, (_kiweqoyehv, _bbosgzqmyp, _eckmbpzbse, _tuqajkwbkk))
            index = _bflyordldu(query)
            if index == 0:
                agents = (_bbosgzqmyp, _eckmbpzbse, _kiweqoyehv, _tuqajkwbkk)
            elif index == 1:
                agents = (_eckmbpzbse, _kiweqoyehv, _tuqajkwbkk, _bbosgzqmyp)
            elif index == 2:
                agents = (_kiweqoyehv, _tuqajkwbkk, _bbosgzqmyp, _eckmbpzbse)
            elif index == 3:
                agents = (_tuqajkwbkk, _bbosgzqmyp, _eckmbpzbse, _kiweqoyehv)
            else:
                agents = (_bbosgzqmyp, _eckmbpzbse, _kiweqoyehv, _tuqajkwbkk)
            return await _odwincvxga(query, agents)
        except Exception:
            return _kthdbpvoxv(query)
    _BUILDW_TAG_m8 = "sn45-1d1d903887e0"

    return query

_aster_orchid_slot13_agent_query_entry = _compose_aster_orchid_slot13_agent_entry()


def _compose_umber_ledger_slot13_agent_entry():
...
_umber_ledger_slot13_agent_query_entry = _compose_umber_ledger_slot13_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "FjordOrchidSlot13Agent"
    ...
    if (
        ...
        return "AsterOrchidSlot13Agent"
    return "UmberLedgerSlot13Agent"


class FjordOrchidSlot13Agent:
    async def __call__(self, query: Query) -> Response:
        return await _fjord_orchid_slot13_agent_query_entry(query)


class AsterOrchidSlot13Agent:
    async def __call__(self, query: Query) -> Response:
        return await _aster_orchid_slot13_agent_query_entry(query)


class UmberLedgerSlot13Agent:
    async def __call__(self, query: Query) -> Response:
        return await _umber_ledger_slot13_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = FjordOrchidSlot13Agent()
_ANALYTICAL_FIELD_AGENT = AsterOrchidSlot13Agent()
_BROAD_FIELD_AGENT = UmberLedgerSlot13Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "FjordOrchidSlot13Agent",
    "AsterOrchidSlot13Agent",
    "UmberLedgerSlot13Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "FjordOrchidSlot13Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "AsterOrchidSlot13Agent":
