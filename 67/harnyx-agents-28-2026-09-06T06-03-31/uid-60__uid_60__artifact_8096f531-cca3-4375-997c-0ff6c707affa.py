...
def _compose_flint_ledger_agent_entry():
...
def _compose_tide_current_agent_entry():
...
def _compose_larch_torrent_agent_entry():
...
_FLINT_LEDGER = _compose_flint_ledger_agent_entry()
_TIDE_CURRENT = _compose_tide_current_agent_entry()
_LARCH_TORRENT = _compose_larch_torrent_agent_entry()
...
def _route(query: Query):
    if getattr(query, "output_schema", None) is not None:
        return _TIDE_CURRENT, _FLINT_LEDGER
    text = (getattr(query, "text", "") or "")
    if _ROUTE_TABLE_RE.search(text) and _ROUTE_SWEEP_RE.search(text):
        return _LARCH_TORRENT, _FLINT_LEDGER
    if len(text) > 7000:
        return _LARCH_TORRENT, _FLINT_LEDGER
    return _FLINT_LEDGER, _TIDE_CURRENT
