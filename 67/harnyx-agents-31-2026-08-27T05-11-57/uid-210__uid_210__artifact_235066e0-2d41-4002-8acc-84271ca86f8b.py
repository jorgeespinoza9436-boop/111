...
def _compose_juniper_compass_agent_entry():
    ...
    RESEARCH_TIME_CAP_SECONDS = 118.0
...
_AGENT_0 = JuniperCompassAgent()
_AGENT_1 = CedarQuillAgent()

def _route_bucket(query: Query) -> int:
    import hashlib as _h
    text = (getattr(query, 'text', '') or '').strip()
    schema = getattr(query, 'output_schema', None)
    kind = 'dict' if isinstance(schema, dict) else 'schema' if schema is not None else 'none'
    payload = (_ROUTE_SEED + '|' + kind + '|' + text[:512] + '|' + text[-256:]).encode('utf-8', 'ignore')
    return int.from_bytes(_h.sha256(payload).digest()[:8], 'big') % 2

async def query(query: Query) -> Response:
    bucket = _route_bucket(query)
    if bucket == 0:
        branch = _AGENT_0
    else:
        branch = _AGENT_1
...
class Ember88c58b:
    ...
    def _cinder_62f0fd(self):
        _SEARCH_BUDGET_S_7fd02c = 70.0
        _KEEP_7fd02c = 6
        _MODEL_7fd02c = 'openai/gpt-oss-120b'

        def _plan_7fd02c(question):
            """Turn one question into the retrieval queries this reserve will run."""
            words = sorted({w for w in question.split() if len(w) >= 3}, key=len, reverse=True)
            return [question] + [' '.join(words[:k]) for k in (3, 3 + 3) if words]

        def _order_7fd02c(results, terms):
            """Impose this reserve's own preference order on retrieved evidence."""
            return sorted(results, key=lambda r: -sum((t in (getattr(r, 'content', '') or '').lower() for t in terms)))

        def _digest_7fd02c(results):
            parts = []
            for item in results[:_KEEP_7fd02c]:
                body = (getattr(item, 'content', None) or getattr(item, 'snippet', None) or '').strip()
                if body:
                    parts.append(body[:1200])
            return '\n\n'.join(parts)

        async def _gather_7fd02c(question):
            queries = _plan_7fd02c(question)
            collected = []
            for q in queries[:2 + 1]:
                try:
                    found = await asyncio.wait_for(search_web(q, provider='parallel', num=3), timeout=min(_SEARCH_BUDGET_S_7fd02c, 30.0))
                except Exception:
                    continue
        ...
        async def query(query):
            ...
            results = await _gather_7fd02c(question)
            if not results:
                return Response(text='No answer produced.')
            ordered = _order_7fd02c(results, terms)
            evidence = _digest_7fd02c(ordered)
            if not evidence:
                return Response(text='No answer produced.')
            try:
                reply = await llm_chat(provider='openrouter', model=_MODEL_7fd02c, messages=[{'role': 'system', 'content': 'Answer strictly from the supplied evidence.'}, {'role': 'user', 'content': question + '\n\nEvidence:\n' + evidence}])
            except Exception:
                return Response(text='No answer produced.')
...
def _kestrel_fc571a(factory):
    """Build a source closure; a source that dies on import must not kill the agent."""
    try:
        return factory()._cinder_62f0fd()
    except Exception:
        return None

def _umber_ea98fe(response):
    if response is None:
        return ''
    return (getattr(response, 'text', None) or '').strip()

def _lantern_78fa9b(response):
    if response is None:
        return 0
    return len(getattr(response, 'citations', None) or ())

def _marlin_b0634a(response):
    return response is not None and getattr(response, 'output', None) is not None

def _kestrel_c3f5f1(query, response):
    """Deterministic answer quality. No model call, so auditing is free."""
    if response is None:
        return 0.0
    if query.output_schema is not None and (not _marlin_b0634a(response)):
        return 0.0
    text = _umber_ea98fe(response)
    if not _marlin_b0634a(response) and len(text) < 40:
        return 0.0
    score = 1.0
    if _marlin_b0634a(response):
        score += 1.0
    score += min(_lantern_78fa9b(response), 12) * 0.05
    score += min(len(text), 4000) / 4000.0
    return score

class Willowe1c075:
    """Answer with the primary; fall through only when nothing usable came back."""
    _NIMBUS_3D3FF0 = 290.0
    _RIVET_F6ABB0 = 270.0
    _CINDER_22F5D1 = 45.0
    ...
    def _sable_158c32(self, query, response):
        return _kestrel_c3f5f1(query, response) <= 0.0

    async def _fathom_9483e0(self, run, request, budget):
    ...
    async def willow_6617db(self, query: Query) -> Response:
        started = monotonic()
        first = await self._fathom_9483e0(self._primary, query, self._RIVET_F6ABB0)
        if not self._sable_158c32(query, first):
            return first if first is not None else Response(text='No answer produced.')
        remaining = self._NIMBUS_3D3FF0 - (monotonic() - started)
        if remaining <= self._CINDER_22F5D1:
            return first if first is not None else Response(text='No answer produced.')
        second = await self._fathom_9483e0(self._reserve, query, remaining)
        candidates = [r for r in (first, second) if r is not None]
        if not candidates:
            return Response(text='No answer produced.')
        return max(candidates, key=lambda r: _kestrel_c3f5f1(query, r))
_GIRDER_E36E59 = query
_JUNIPER_C7E1B7 = _kestrel_fc571a(Ember88c58b)
_VELLUM_CD7C59 = Willowe1c075(_GIRDER_E36E59, _JUNIPER_C7E1B7)

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _VELLUM_CD7C59.willow_6617db(query)
_TAG_4B53E9E0="4b53e9e0f75f48eaac4c79f6bfb8fa05"
import logging as _tag_logging_4b53e9e0
_tag_logging_4b53e9e0.getLogger("miner.tag").debug("tag=%s", _TAG_4B53E9E0)
