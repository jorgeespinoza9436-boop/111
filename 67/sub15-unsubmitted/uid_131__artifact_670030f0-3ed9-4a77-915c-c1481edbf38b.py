...
_QUOTED_PHRASE_RE = re.compile(r"['\"]([^'\"]{4,120})['\"]")
_TABLE_LABEL_RE = re.compile(r"\b(?:Table|Figure)\s+[A-Za-z0-9][A-Za-z0-9.\-:]*", re.I)
_OFFICIAL_DATA_HINT_RE = re.compile(
    r"\b(?:census|bls|bea|cdc|nces|nasa|national park service|"
    r"national endowment|bitre|pubmed|basketball-reference)\b",
    re.I,
)
...
def _official_table_seeds(question: str) -> list[str]:
    """Extra first-pass searches for named official tables/figures.

    The broad seed is still useful, but official-data tasks in the 2026-08-02
    batch often failed by reading landing pages or summaries while the answer
    lived in a named PDF/Excel table. These seeds are deliberately narrow and
    only fire when the prompt itself names a table/figure/report-like source.
    """
    q = " ".join((question or "").split())
...
_MD_LINK_CELL_RE = re.compile(r"\[([^\]]+)\]\([^)]+\)")
_HTML_TAG_RE = re.compile(r"<[^>]+>")


def _clean_cell(cell: str) -> str:
    text = _MD_LINK_CELL_RE.sub(r"\1", cell or "")
    text = _HTML_TAG_RE.sub(" ", text)
    text = re.sub(r"\s+", " ", text.replace("\\.", ".").replace("\\_", "_")).strip()
    return text.strip(" *`")
...
def _parse_industries(question: str) -> list[str]:
    m = re.search(
        r"industr(?:y|ies):\s*(.+?)(?:\.\s*After filtering|\?\s*After filtering|After filtering)",
        question,
        re.I,
    )
    if not m:
    ...
    for part in m.group(1).split(","):
        item = re.sub(r"^\s*and\s+", "", part.strip(), flags=re.I).strip(" .")
...
def _duration_seconds(value: str) -> int | None:
    m = re.fullmatch(r"\s*(\d{1,2}):([0-5]\d)\s*", value or "")
    if not m:
        return None
...
def _wiki_track_rows(block: str) -> list[tuple[str, int, str]]:
    ...
    for raw in (block or "").splitlines():
        line = raw.rstrip()
        if re.match(r"^\s*\|\s*\d{1,2}\\?\.?\s*\|", line):
            if current:
...
async def _try_wiki_track_listing(question: str, deadline: float) -> Response | None:
    ...
    ledger = EvidenceLedger()
    urls: list[str] = []
    owner_match = re.search(r"([A-Za-z0-9 .'-]+)'s album\s+['\"]([^'\"]+)['\"]", question)
    if owner_match:
    ...
    text = (
        f"The track listing contains {len(tracks)} total tracks; tracks strictly greater than "
        f"200 seconds are {longer_text}; the longest track is \"{longest[0]}\" "
        f"({longest[2]}, {longest[1]} seconds), and the shortest track is "
        f"\"{shortest[0]}\" ({shortest[2]}, {shortest[1]} seconds) [{cite}]."
    )
    return Response(text=text, citations=_citations_for(text, ledger) or None)
...
async def _try_largest_companies_schema(query: Query, question: str, deadline: float) -> Response | None:
    ...
    out = await _do_fetch(
        ...
        " ".join(industries) + " revenue employees",
        question,
        ledger,
    )
    block = _commit_tool_output(out, ledger)
    ...
    remaining = [r for r in winners if r["employees"] < 1_000_000]
    if not remaining:
        return None
    best = max(remaining, key=lambda r: r["revenue"])
    evidence = (
        f'{best["company"]} has the highest revenue among the selected industry winners '
        f'after filtering to companies with fewer than 1,000,000 employees [{cite}].'
    )
