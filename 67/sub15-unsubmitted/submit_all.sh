#!/usr/bin/env bash
set -euo pipefail
# Submit matched sub15-unsubmitted agents to registered SN67 hotkeys.
# Usage: bash /root/turtle/111/67/sub15-unsubmitted/submit_all.sh [PARALLEL_JOBS]
ROOT="/root/turtle/harnyx"
JOBS_FILE="/root/turtle/111/67/sub15-unsubmitted/submit_jobs.txt"
PARALLEL_JOBS="${1:-21}"
WALLET="money"
cd "$ROOT"
