...
_gkndtvxsab = {'started': None, 'text': None}
_wxnvasqxwx = 24000
_kiubpjgozh = 290.0
_zdrqcywnct = 250.0

def _ueoxqgfyqp() -> float:
    started = _gkndtvxsab['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _tximnuxaif() -> float:
    return _kiubpjgozh - _ueoxqgfyqp()
_icnvqqpwac = _hsapi.llm_chat
_prerkfoqnh = _hsapi.search_web
_uucggteznh = _hsapi.fetch_page
_kpuwqrxdmc = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _xxrdvjbuhm(*args, **kwargs):
    if _ueoxqgfyqp() >= _zdrqcywnct:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _kpuwqrxdmc})
            kwargs['messages'] = steered
    _result = await _icnvqqpwac(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _iincnluxpz(_result)
    return _result

async def _bcvflupmqa(*args, **kwargs):
    if _ueoxqgfyqp() >= _zdrqcywnct:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _prerkfoqnh(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _gvicdtxhkl(*args, **kwargs):
    if _ueoxqgfyqp() >= _zdrqcywnct:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _uucggteznh(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _xxrdvjbuhm
_hsapi.search_web = _bcvflupmqa
_hsapi.fetch_page = _gvicdtxhkl
_lmmiajiadm = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_pgonnjepbm = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_utkeavfslq = 900
_wsnfxbgzji = 2

def _nfrsoixqtv(query: Query) -> int:
...
def _hffektlanw(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _zpktfipqxp(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _nfrsoixqtv(query)
    if fields >= 3:
        return 2
    if _hffektlanw(lowered, _lmmiajiadm):
        return 1
    if fields <= _wsnfxbgzji and len(text) <= _utkeavfslq:
        return 0
    if _hffektlanw(lowered, _pgonnjepbm):
        return 0
    return 1

def _iincnluxpz(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _gkndtvxsab['text'] = text.strip()[:_wxnvasqxwx]
    except Exception:
        pass

def _xqwacwpwhd(text: str):
...
def _qynmmehjhx(query: Query) -> Response:
    text = _gkndtvxsab['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_wxnvasqxwx]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _xqwacwpwhd(text)
...
def _sbziuljypv():
    ...
    async def _close_fast(question: str, response, fast_run: bool, closing: float):
        if not fast_run or not question:
            return response
        try:
            ...
            if not evidence:
                evidence = await _close_probe(question, open_parts, closing - _close_now() - 8.0)
            ...
            closed = await _close_ask(_CLOSE_BRIEF, body, _CLOSE_ANSWER_TOKENS, min(45.0, closing - _close_now() - 4.0))
    ...
    async def _w4_baseline_query(query: Query) -> Response:
        ...
        try:
            settled = await _solve(query, question)
            return await _close_fast(question, settled, fast_run, close_wall)
        except Exception:
            schema = getattr(query, 'output_schema', None)
            if schema is not None:
                try:
...
def _tvyyrdbtcr():
    ...
    def _sc_json_salvage(raw: str):
        ...
        try:
            ...
            pass
        try:
            return json.loads(re.sub(',\\s*([}\\]])', '\\1', text_value))
        except Exception:
            pass
...
"""
    ...
    *3,100 (footnoted); (12.4) negative; 2,567 lb with a unit; =WR and CR marks;
    ...
    """
        ...
        members = [Member(key=key, source=sources[key], cells=[(w, '') for w in won], leads=won) for key, won in leads.items() if key != leader]
...
def _slbpowdnqq():
    ...
    return query
_inburdymbn = _sbziuljypv()
_fuagkzekec = _tvyyrdbtcr()
_jplpsctliq = _mqwjhnvumj()
_jpzmnhowge = _slbpowdnqq()
_kiubpjgozh = 290.0
_dqktpodncg = 250.0
_lbbdmkreuh = 90.0

async def _hirgpazoiy(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _kiubpjgozh - (time.monotonic() - started)
        if first:
            budget = _dqktpodncg if _dqktpodncg < remaining else remaining
            first = False
        else:
            if remaining < _lbbdmkreuh:
                break
        ...
        try:
            ...
            last_exc = exc
    return _qynmmehjhx(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _gkndtvxsab['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _hirgpazoiy(query, (_jplpsctliq, _inburdymbn, _fuagkzekec, _jpzmnhowge))
        index = _zpktfipqxp(query)
        if index == 0:
            agents = (_inburdymbn, _fuagkzekec, _jplpsctliq, _jpzmnhowge)
        elif index == 1:
            agents = (_fuagkzekec, _jplpsctliq, _jpzmnhowge, _inburdymbn)
        elif index == 2:
            agents = (_jplpsctliq, _jpzmnhowge, _inburdymbn, _fuagkzekec)
        elif index == 3:
            agents = (_jpzmnhowge, _inburdymbn, _fuagkzekec, _jplpsctliq)
        else:
            agents = (_inburdymbn, _fuagkzekec, _jplpsctliq, _jpzmnhowge)
        return await _hirgpazoiy(query, agents)
    except Exception:
        return _qynmmehjhx(query)
_BUILDM_TAG_m22 = "sn45-9fccb41dce32"
