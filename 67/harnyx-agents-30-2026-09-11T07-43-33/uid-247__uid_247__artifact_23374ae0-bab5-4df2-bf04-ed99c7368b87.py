...
def _compose_fjord_forge_slot08_agent_entry():
...
_fjord_forge_slot08_agent_query_entry = _compose_fjord_forge_slot08_agent_entry()


def _compose_fjord_orchid_slot08_agent_entry():
...
_fjord_orchid_slot08_agent_query_entry = _compose_fjord_orchid_slot08_agent_entry()


def _compose_amber_voyager_slot08_agent_entry():
...
_amber_voyager_slot08_agent_query_entry = _compose_amber_voyager_slot08_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "FjordForgeSlot08Agent"
    ...
    if (
        ...
        return "FjordOrchidSlot08Agent"
    return "AmberVoyagerSlot08Agent"


class FjordForgeSlot08Agent:
    async def __call__(self, query: Query) -> Response:
        return await _fjord_forge_slot08_agent_query_entry(query)


class FjordOrchidSlot08Agent:
    async def __call__(self, query: Query) -> Response:
        return await _fjord_orchid_slot08_agent_query_entry(query)


class AmberVoyagerSlot08Agent:
    async def __call__(self, query: Query) -> Response:
        return await _amber_voyager_slot08_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = FjordForgeSlot08Agent()
_ANALYTICAL_FIELD_AGENT = FjordOrchidSlot08Agent()
_BROAD_FIELD_AGENT = AmberVoyagerSlot08Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "FjordForgeSlot08Agent",
    "FjordOrchidSlot08Agent",
    "AmberVoyagerSlot08Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "FjordForgeSlot08Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "FjordOrchidSlot08Agent":
