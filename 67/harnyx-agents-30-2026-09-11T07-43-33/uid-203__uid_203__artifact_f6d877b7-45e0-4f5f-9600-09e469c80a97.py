...
_yytxjhhrcu = {'started': None, 'text': None}
_omjvmjgwqj = 24000
_qhktervufw = 290.0
_qafdjmdqam = 250.0

def _fteqwnozog() -> float:
    started = _yytxjhhrcu['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _yessembsjz() -> float:
    return _qhktervufw - _fteqwnozog()
_lgtcsekltn = _hsapi.llm_chat
_gcbsqgpfsb = _hsapi.search_web
_grnztyqljg = _hsapi.fetch_page
_jfjxkfuhhp = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _cnihtljdgn(*args, **kwargs):
    if _fteqwnozog() >= _qafdjmdqam:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _jfjxkfuhhp})
            kwargs['messages'] = steered
    _result = await _lgtcsekltn(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _vwrsczubss(_result)
    return _result

async def _toqjfeukuz(*args, **kwargs):
    if _fteqwnozog() >= _qafdjmdqam:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _gcbsqgpfsb(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _gvuykpasdq(*args, **kwargs):
    if _fteqwnozog() >= _qafdjmdqam:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _grnztyqljg(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _cnihtljdgn
_hsapi.search_web = _toqjfeukuz
_hsapi.fetch_page = _gvuykpasdq
_rvdhbrazed = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_hopkvflcmv = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_ikwacmqbdw = 900
_ixbhnuxaxv = 2

def _xejlgkvlcg(query: Query) -> int:
...
def _bxkqimibtr(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _gfmnpxuhhv(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _xejlgkvlcg(query)
    if fields >= 3:
        return 2
    if _bxkqimibtr(lowered, _rvdhbrazed):
        return 1
    if fields <= _ixbhnuxaxv and len(text) <= _ikwacmqbdw:
        return 0
    if _bxkqimibtr(lowered, _hopkvflcmv):
        return 0
    return 1

def _vwrsczubss(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _yytxjhhrcu['text'] = text.strip()[:_omjvmjgwqj]
    except Exception:
        pass

def _eesmmyalgu(text: str):
...
def _ecysculfbf(query: Query) -> Response:
    text = _yytxjhhrcu['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_omjvmjgwqj]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _eesmmyalgu(text)
...
def _adhohrotgi():
    ...
    def _cap(text: str) -> str:
        ...
        if len(t) > ANSWER_CHAR_CAP:
            return t[:ANSWER_CHAR_CAP - 16] + ' …'
        return t
    _ARCH_COMPILE_TIMEOUT_S = 36.0
    _ARCH_EXTRACT_TIMEOUT_S = 28.0
    _ARCH_SYNTH_TIMEOUT_S = 48.0
    _ARCH_NOTE_TIMEOUT_S = 22.0
    _ARCH_MAX_CLAIMS = 10
    _ARCH_MAX_ROUNDS = 3
    _ARCH_FETCHES_PER_CLAIM = 2
    _ARCH_SEARCHES_PER_CLAIM = 2
    _ARCH_CONFLICT_FETCHES = 2

    def _arch_json(raw: str):
        if not raw:
            return None
        text = re.sub('^```(?:json)?\\s*|\\s*```$', '', raw.strip(), flags=re.I | re.M).strip()
        start = text.find('{')
        end = text.rfind('}')
        if start < 0 or end <= start:
            start = text.find('[')
    ...
    class ClaimBoard:
        """Coordinating evidence state: claim-keyed records, not conversational history."""
        ...
        def __init__(self) -> None:
            ...
            self.reentered_retrieval = False
            self.kind = ARCH
            self.fast = False
            self.schema = None
            self.false_premise = ''
        ...
        def open_claims(self) -> list[dict]:
            filled = {r.get('claim_id') for r in self.records if (r.get('value') or '').strip()}
            return [c for c in self.claims if c.get('id') not in filled]

        def records_for(self, claim_id: str) -> list[dict]:
            return [r for r in self.records if r.get('claim_id') == claim_id]

        def render(self, cap: int=24000) -> str:
            parts: list[str] = []
            if self.false_premise:
                parts.append('PREMISE CORRECTION: ' + self.false_premise)
            if self.roster:
                parts.append('ROSTER: ' + '; '.join(self.roster[:40]))
            if self.answer_shape:
                parts.append('ANSWER SHAPE: ' + self.answer_shape)
            for claim in self.claims:
                cid = claim.get('id') or '?'
                need = claim.get('need') or ''
                kind = claim.get('kind') or 'fact'
                recs = self.records_for(cid)
                if recs:
                    body = []
                    for rec in recs[:4]:
                        ...
                        extra = f' scope={scope}' if scope else ''
                        body.append(f'  [{cite}] {val}{extra} ({src})')
                    parts.append(f'- {cid} ({kind}): {need}\n' + '\n'.join(body))
                else:
                    parts.append(f'- {cid} ({kind}) OPEN: {need}')
            if self.conflicts:
                parts.append('RECONCILED CONFLICTS:')
                for conf in self.conflicts[:8]:
                    parts.append(f"  {conf.get('claim_id')}: {conf.get('left')} vs {conf.get('right')} -> {conf.get('ruling')} ({conf.get('basis')})")
            text = '\n'.join(parts)
            return text[:cap]

    def _norm_val(text: str) -> str:
        t = re.sub('\\s+', ' ', (text or '').strip().casefold())
        t = t.replace(',', '')
        return t[:240]
    ...
    def _values_conflict(a: str, b: str) -> bool:
        ...
        if na in nb or nb in na:
            return False
        fa = set(_NUM_IN_TEXT_RE.findall(a or ''))
        fb = set(_NUM_IN_TEXT_RE.findall(b or ''))
    ...
    def _detect_board_conflicts(board: ClaimBoard) -> list[dict]:
        found: list[dict] = []
        by: dict[str, list[dict]] = {}
        for rec in board.records:
            by.setdefault(rec.get('claim_id') or '', []).append(rec)
        for cid, recs in by.items():
            if len(recs) < 2:
                continue
            for i, left in enumerate(recs):
                for right in recs[i + 1:]:
                    if _values_conflict(left.get('value') or '', right.get('value') or ''):
                        found.append({'claim_id': cid, 'left': left.get('value') or '', 'right': right.get('value') or '', 'left_cite': left.get('cite'), 'right_cite': right.get('cite'), 'left_scope': left.get('scope') or '', 'right_scope': right.get('scope') or ''})
                        break
                if found and found[-1]['claim_id'] == cid:
                    break
        return found[:6]
    _COMPILE_SYSTEM = 'You compile a research obligation graph for a deep-research question. JSON only. Do not answer the question. Identify every load-bearing claim the answer must prove from public sources: named premises, each comparison side, complete-pool membership, each filter condition, period/basis/jurisdiction/version, derived calculations, and every structured field. A false, stale, or misclassified named premise is itself an obligation. Keys: claims (array of {id, kind, need, search, fetch_hint}), roster_query (string or empty), answer_shape (how the public answer must look), false_premise_risk (string or empty). kind is one of: premise, pool_member, condition, comparison_side, metric, date, status, category, version, calculation, field, conflict. search is a precise web query. fetch_hint is a page-region phrase. At most 10 claims. Prefer official/primary-source queries.'
    _EXTRACT_SYSTEM = "Extract grounded claim records from numbered tool evidence. JSON only. Return {records:[{claim_id,value,scope,source,cite,quote}]} where cite is the integer [n] of the evidence row that states the value, scope names period/basis/jurisdiction/version if stated, quote is a short verbatim span. Omit a record rather than guess. If two sources disagree, emit both records with different cite numbers. If the named premise is false, set value to the correction and scope to 'premise_correction'."
    _SYNTH_SYSTEM = "You write the public research answer from a claim board of already-retrieved records. You have NO tools. Cite only the integer [n] markers attached to those records. A pairwise judge scores correctness, requested coverage, instruction following, evidence support, and calibrated uncertainty ahead of presentation.\nRULES:\n- First words are the asked entities/values in the requested form. No preamble.\n- Every material researched claim in prose carries its [n] immediately after the sentence. Ordinary connective phrasing needs none.\n- Cover every requested fact, comparison side, and pool member. Missing a required member is a coverage failure.\n- For set/superlative questions: name the complete pool you established, give one cited line per qualifier and one cited line per exclusion. Do not compress several rejects into one clause.\n- For comparisons: state each side's value, period, and basis, then the reconciled conclusion. Do not silently pick one source.\n- If a named premise is false, correct it from the board first, then answer the follow-up. Cite the correction.\n- Derived totals/ranks must show the listed inputs and the arithmetic.\n- Copy source strings verbatim. Do not add parenthetical aliases.\n- Do not invent time-sensitive facts absent from the board. Omit them.\n- No Evidence/Sources/References dump. No narration about missing evidence.\n- If the question demands output-only/terse form, put the bare answer first and keep proof below it.\n- Do not add extra claims the question did not ask. Extra claims lose fast component F1.\n- Structured field meanings live in the question; do not put citation markers into atomic values."
    _NOTE_SYSTEM = 'Write an optional public note that explains why the required answer follows from the cited claim-board records. JSON {note: string|null}. Include a note when the required answer is atomic/structured or when a premise was corrected or a source conflict was reconciled. Cite with [n]. Do not repeat the answer verbatim. Do not add new factual claims. Null if the answer already explains itself.'
    _RECONCILE_SYSTEM = 'Two retrieved records disagree. JSON only: {ruling, basis, keep_cite, drop_cite, search, fetch_hint, reopen_claim_ids}. ruling is the value that survives after comparing period, basis, jurisdiction, version, or official-versus-secondary status. search is a targeted query that will retrieve the governing primary source. reopen_claim_ids lists claims that must be gathered again after the new retrieval. Never invent a value.'

    async def _compile_board(question: str, query: Query, deadline: float) -> ClaimBoard:
        board = ClaimBoard()
        board.fast = bool(getattr(query, 'fast', False))
        board.schema = getattr(query, 'output_schema', None)
        schema_bit = ''
        if board.schema is not None:
            try:
                schema_bit = '\nPublic output schema:\n' + json.dumps(board.schema)[:2500]
            except Exception:
                schema_bit = ''
        user = f'Question:\n{question}\n{schema_bit}\nfast_mode={board.fast}\ncontroller={ARCH}\nCompile the obligation graph.'
        if ARCH == 'roster_join':
            user += '\nThis controller enumerates a complete roster first, then joins each member to status/metric records, then applies filters.'
        elif ARCH == 'contract_solver':
            user += '\nThis controller emits testable hypotheses and executes them against fetched sources rather than chatting with tools.'
        elif ARCH == 'dual_track':
            user += '\nEmit two alternate search framings per claim when sources may disagree (official vs contemporaneous independent).'
        raw = ''
        try:
            left = deadline - monotonic()
            raw = await _chat_simple(LLM_LANE_A, SCHEMA_MODEL, _COMPILE_SYSTEM, user, max_tokens=1800, timeout=max(8.0, min(_ARCH_COMPILE_TIMEOUT_S, left - 80.0)), think=_least_think(LLM_LANE_A, SCHEMA_MODEL))
        except Exception:
            raw = ''
        parsed = _arch_json(raw) if raw else None
        claims: list[dict] = []
        if isinstance(parsed, dict):
            board.answer_shape = str(parsed.get('answer_shape') or '')[:400]
            board.false_premise = str(parsed.get('false_premise_risk') or '')[:400]
            rq = str(parsed.get('roster_query') or '').strip()
            raw_claims = parsed.get('claims')
            if isinstance(raw_claims, list):
                for i, item in enumerate(raw_claims[:_ARCH_MAX_CLAIMS], start=1):
                    if not isinstance(item, dict):
                        continue
                    cid = str(item.get('id') or f'C{i}')[:24]
                    need = str(item.get('need') or '').strip()
                    if not need:
                        continue
                    claims.append({'id': cid, 'kind': str(item.get('kind') or 'fact')[:32], 'need': need[:400], 'search': str(item.get('search') or need)[:240], 'fetch_hint': str(item.get('fetch_hint') or '')[:160], 'roster_query': rq, 'alt_search': str(item.get('alt_search') or '')[:240]})
            if rq:
                claims.insert(0, {'id': 'ROSTER', 'kind': 'pool_member', 'need': 'Enumerate the complete in-scope roster/list/table', 'search': rq[:240], 'fetch_hint': 'list table roster standings', 'roster_query': rq, 'alt_search': 'list of ' + rq[:200]})
        if not claims:
            seeds = _seed_queries(question, _needs_set_completeness(question)) or [question[:240]]
            for i, seed in enumerate(seeds[:4], start=1):
                claims.append({'id': f'C{i}', 'kind': 'metric' if i == 1 else 'condition', 'need': seed, 'search': seed[:240], 'fetch_hint': '', 'roster_query': '', 'alt_search': ''})
            if _needs_set_completeness(question):
                claims.insert(0, {'id': 'ROSTER', 'kind': 'pool_member', 'need': 'Complete candidate pool named by the question', 'search': 'list of ' + ' '.join(_seed_queries(question, True)[:1] or [question[:80]]), 'fetch_hint': 'list table', 'roster_query': '', 'alt_search': question[:240]})
        seen: set[str] = set()
        uniq: list[dict] = []
        for claim in claims:
            cid = claim['id']
            ...
            seen.add(cid)
            uniq.append(claim)
        board.claims = uniq[:_ARCH_MAX_CLAIMS]
        return board

    def _urls_from_ledger(ledger: EvidenceLedger, hint: str, limit: int) -> list[tuple[int, str, str]]:
        terms = _key_terms(hint)
        ...
        for i, row in enumerate(ledger.rows, start=1):
            ...
            if not url.startswith('http') or url in seen:
                continue
            seen.add(url)
            blob = ((row.get('title') or '') + ' ' + (row.get('preview') or '')).lower()
            hits = sum((1 for t in terms if t in blob)) if terms else 0
            scored.append((hits, i, url, row.get('title') or ''))
        scored.sort(key=lambda item: (-item[0], item[1]))
        return [(n, url, title) for _, n, url, title in scored[:limit]]

    async def _run_searches(queries: list[str], ledger: EvidenceLedger, deadline: float) -> str:
        queries = [q.strip() for q in queries if q and q.strip()]
        if not queries or deadline - monotonic() < 18.0:
            return ''
        budget = max(5.0, min(SEARCH_TIMEOUT_S * 2 + 4.0, deadline - monotonic() - MIN_TAIL_S))
        tasks = [asyncio.ensure_future(_do_search(q, ledger)) for q in queries[:4]]
        try:
    ...
    async def _run_fetches(targets: list[tuple[str, str]], question: str, ledger: EvidenceLedger, deadline: float) -> str:
        if not targets or deadline - monotonic() < 16.0:
            return ''
        budget = max(5.0, min(FETCH_TIMEOUT_S * 2 + 4.0, deadline - monotonic() - MIN_TAIL_S))
        tasks = [asyncio.ensure_future(_do_fetch(url, focus, question, ledger)) for url, focus in targets[:4]]
        try:
            await asyncio.wait(tasks, timeout=budget)
        except Exception:
            pass
        ...
        for task in tasks:
            ...
            try:
                ...
                continue
            if isinstance(out, str):
                blocks.append(out if out.startswith('#') else _commit_tool_output(out, ledger))
            else:
                blocks.append(_commit_tool_output(out, ledger))
        return '\n'.join(blocks)

    async def _extract_records(question: str, board: ClaimBoard, evidence: str, deadline: float) -> None:
        if not evidence or deadline - monotonic() < 16.0:
            return
        claim_blob = json.dumps([{'id': c['id'], 'kind': c['kind'], 'need': c['need']} for c in board.claims], ensure_ascii=False)[:3000]
        user = f'Question:\n{question[:2500]}\n\nOpen/all claims:\n{claim_blob}\n\nNumbered evidence:\n{evidence[:14000]}\n\nExtract records.'
        try:
            raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, _EXTRACT_SYSTEM, user, max_tokens=1600, timeout=max(8.0, min(_ARCH_EXTRACT_TIMEOUT_S, deadline - monotonic() - 50.0)))
        except Exception:
            return
        parsed = _arch_json(raw)
        rows = parsed.get('records') if isinstance(parsed, dict) else parsed
        if not isinstance(rows, list):
            return
        valid_ids = {c['id'] for c in board.claims}
        for item in rows[:24]:
            if not isinstance(item, dict):
                continue
            cid = str(item.get('claim_id') or '')
            value = str(item.get('value') or '').strip()
            if not value or (cid and cid not in valid_ids):
                continue
            cite = item.get('cite')
            try:
                cite_n = int(cite)
            except Exception:
                found = _CITE_NUM_RE.search(str(cite or ''))
                cite_n = int(found.group(1).split(',')[0]) if found else 0
            quote = str(item.get('quote') or '')[:240]
            board.records.append({'claim_id': cid or 'C1', 'value': value[:400], 'scope': str(item.get('scope') or '')[:160], 'source': str(item.get('source') or '')[:160], 'cite': cite_n, 'quote': quote})
            if cid == 'ROSTER' or item.get('scope') == 'roster':
                for name in re.split('[;\\n,/]| and ', value):
                    name = name.strip(' -*[]')
                    if 2 <= len(name) <= 80 and name not in board.roster:
                        board.roster.append(name)

    def _bind_extract_cites(board: ClaimBoard, ledger: EvidenceLedger) -> None:
        top = len(ledger.rows)
        for rec in board.records:
            n = int(rec.get('cite') or 0)
            if not 1 <= n <= top:
                rec['cite'] = 0
                quote = rec.get('quote') or rec.get('value') or ''
                qlow = quote[:80].lower()
                if qlow:
                    for i, row in enumerate(ledger.rows, start=1):
                        blob = ((row.get('text') or '') + ' ' + (row.get('preview') or '')).lower()
                        if qlow in blob:
                            rec['cite'] = i
                            break
            n = int(rec.get('cite') or 0)
            quote = rec.get('quote') or rec.get('value') or ''
            if n and quote and (len(quote) >= RETAIN_MIN_QUOTE):
                try:
                    _do_retain_evidence(str(n), quote, ledger)
                except Exception:
                    pass

    def _deepen_fetched(targets: list[tuple[str, str]], claim: dict, ledger: EvidenceLedger) -> str:
        """Scheduler-issued in-page grep so deep table values enter the claim board."""
        parts: list[str] = []
        hint = claim.get('fetch_hint') or claim.get('need') or ''
        tokens = [t for t in re.findall('[A-Za-z0-9][A-Za-z0-9.\\-]{2,}', hint) if t.lower() not in _STOP][:3]
        if not tokens:
            return ''
        seen: set[str] = set()
        for url, _focus in targets[:4]:
            if url in seen:
                continue
            seen.add(url)
            for tok in tokens:
                try:
                    got = _do_page_grep(url, tok, ledger)
                except Exception:
                    continue
                if isinstance(got, str) and 'match' in got.lower():
                    parts.append(got[:4000])
                    break
        return '\n'.join(parts)

    async def _fill_claim(claim: dict, question: str, board: ClaimBoard, ledger: EvidenceLedger, deadline: float, alt: bool=False) -> None:
        if deadline - monotonic() < 22.0 or _spend_left() < 0.04:
            return
        query_text = claim.get('alt_search') if alt and claim.get('alt_search') else claim.get('search') or claim.get('need') or question
        queries = [query_text]
        if ARCH == 'dual_track':
            alt_q = claim.get('alt_search') or 'official primary source ' + query_text[:180]
            if alt_q not in queries:
                queries.append(alt_q)
        elif ARCH == 'roster_join' and claim.get('kind') == 'pool_member':
            queries = [query_text, 'full list ' + query_text[:160], 'table ' + query_text[:160]]
        elif ARCH == 'contract_solver':
            queries = [query_text, (claim.get('need') or '')[:240]]
        evidence = await _run_searches(queries[:_ARCH_SEARCHES_PER_CLAIM], ledger, deadline)
        targets = [(url, claim.get('fetch_hint') or claim.get('need') or question[:80]) for _, url, _ in _urls_from_ledger(ledger, claim.get('fetch_hint') or claim.get('need') or query_text, _ARCH_FETCHES_PER_CLAIM)]
        fetched = await _run_fetches(targets, question, ledger, deadline)
        grepped = _deepen_fetched(targets, claim, ledger)
        blob = '\n'.join((x for x in (evidence, fetched, grepped) if x))
        if blob:
            await _extract_records(question, board, blob, deadline)
            _bind_extract_cites(board, ledger)

    async def _conflict_cycle(question: str, board: ClaimBoard, ledger: EvidenceLedger, deadline: float) -> None:
        conflicts = _detect_board_conflicts(board)
        if not conflicts or deadline - monotonic() < 40.0:
            return
        for conf in conflicts[:3]:
            if deadline - monotonic() < 32.0 or _spend_left() < 0.05:
                break
            user = f"Question:\n{question[:2000]}\n\nConflict on {conf['claim_id']}:\nA [{conf.get('left_cite')}] {conf['left']} scope={conf.get('left_scope')}\nB [{conf.get('right_cite')}] {conf['right']} scope={conf.get('right_scope')}\nBoard:\n{board.render(6000)}\nReconcile via period/basis/jurisdiction/version/official status, then name the follow-up retrieval."
            try:
                raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, _RECONCILE_SYSTEM, user, max_tokens=900, timeout=max(8.0, min(22.0, deadline - monotonic() - 40.0)))
            except Exception:
                continue
            parsed = _arch_json(raw)
            ...
            hint = str(parsed.get('fetch_hint') or conf['claim_id'])
            ruling = str(parsed.get('ruling') or '').strip()
            basis = str(parsed.get('basis') or '').strip()
            if search:
                board.reentered_retrieval = True
                evidence = await _run_searches([search], ledger, deadline)
                targets = [(url, hint) for _, url, _ in _urls_from_ledger(ledger, hint + ' ' + search, _ARCH_CONFLICT_FETCHES)]
                fetched = await _run_fetches(targets, question, ledger, deadline)
                blob = '\n'.join((x for x in (evidence, fetched) if x))
                if blob:
                    await _extract_records(question, board, blob, deadline)
                    _bind_extract_cites(board, ledger)
            reopen = parsed.get('reopen_claim_ids')
            if isinstance(reopen, list):
                keep = {str(x) for x in reopen if str(x)}
                board.records = [r for r in board.records if r.get('claim_id') not in keep]
                board.reentered_retrieval = True
            if ruling:
                board.conflicts.append({'claim_id': conf['claim_id'], 'left': conf['left'][:160], 'right': conf['right'][:160], 'ruling': ruling[:200], 'basis': basis[:200]})
                board.resolved_conflicts += 1
                keep_cite = parsed.get('keep_cite')
                try:
                    keep_n = int(keep_cite)
                except Exception:
                    keep_n = 0
                if keep_n:
                    board.records.append({'claim_id': conf['claim_id'], 'value': ruling[:400], 'scope': basis[:160], 'source': 'reconciled primary', 'cite': keep_n, 'quote': ruling[:200]})

    async def _synthesize_from_board(question: str, board: ClaimBoard, ledger: EvidenceLedger, deadline: float) -> str:
        if deadline - monotonic() < 16.0:
            return ''
        extra = []
        if board.fast:
            extra.append('FAST MODE: return only the required answer components. Do not add background, extra entities, or unasked figures. Extra claims lower F1.')
        if board.schema is not None:
            extra.append('A structured object will be derived later. Write a complete prose answer that names every field value, with [n] on researched claims.')
        if _needs_set_completeness(question):
            extra.append(SET_RULE)
        if _needs_superlative_proof(question):
            extra.append(SUPERLATIVE_RULE)
        digest = _ledger_digest(ledger, char_cap=18000)
        user = f"Question:\n{question}\n\nController: {ARCH}. Answer shape: {board.answer_shape or 'question-specified'}\nClaim board (coordinating evidence):\n{board.render()}\n\nNumbered source rows you may cite:\n{digest}\n\n" + ('\n'.join(extra) + '\n' if extra else '') + 'Write the final public answer.'
        text = ''
        try:
            text = await _chat_simple(LLM_LANE_A, LOOP_MODEL_A, _SYNTH_SYSTEM, user, max_tokens=2600, timeout=max(10.0, min(_ARCH_SYNTH_TIMEOUT_S, deadline - monotonic() - MIN_TAIL_S)), think=_least_think(LLM_LANE_A, LOOP_MODEL_A))
        except Exception:
            text = ''
        if not _is_usable_answer(text):
            try:
                text = await _chat_simple(LLM_LANE_B, LOOP_MODEL_B, _SYNTH_SYSTEM, user, max_tokens=2600, timeout=max(10.0, min(40.0, deadline - monotonic() - MIN_TAIL_S)), think=_least_think(LLM_LANE_B, LOOP_MODEL_B))
            except Exception:
                text = ''
        return (text or '').strip()

    async def _public_note(question: str, answer: str, board: ClaimBoard, deadline: float) -> str | None:
        if board.fast:
            return None
        if deadline - monotonic() < 18.0:
            return None
        useful = bool(board.schema is not None or board.false_premise or board.conflicts)
        if not useful:
            return None
        user = f'Question:\n{question[:2000]}\n\nAnswer:\n{answer[:6000]}\n\nClaim board:\n{board.render(8000)}\nstructured={board.schema is not None} premise={board.false_premise!r}'
        try:
            raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, _NOTE_SYSTEM, user, max_tokens=700, timeout=max(8.0, min(_ARCH_NOTE_TIMEOUT_S, deadline - monotonic() - 8.0)))
        except Exception:
            return None
        parsed = _arch_json(raw)
        note = parsed.get('note') if isinstance(parsed, dict) else None
        if isinstance(note, str) and note.strip() and (note.strip() != answer.strip()):
            return note.strip()[:8000]
        return None

    async def _claim_board_controller(question: str, query: Query, ledger: EvidenceLedger, deadline: float) -> str:
        board = await _compile_board(question, query, deadline)
        for _round in range(_ARCH_MAX_ROUNDS):
            open_claims = board.open_claims()
            if not open_claims:
                break
            if deadline - monotonic() < 55.0 or _spend_left() < 0.06:
                break
            batch = open_claims[:3]
            await asyncio.gather(*[_fill_claim(c, question, board, ledger, deadline) for c in batch], return_exceptions=True)
            await _conflict_cycle(question, board, ledger, deadline)
            if board.reentered_retrieval:
                continue
        draft = await _synthesize_from_board(question, board, ledger, deadline)
        if board.reentered_retrieval and _is_usable_answer(draft) and (deadline - monotonic() > 50.0):
            again = await _synthesize_from_board(question, board, ledger, deadline)
            if _is_usable_answer(again):
                draft = again
        _BOARD_STORE['board'] = board
        return draft
    _BOARD_STORE: dict = {'board': None}

    async def _arch_run(query: Query, question: str, ledger: EvidenceLedger, deadline: float) -> str:
        return await _claim_board_controller(question, query, ledger, deadline)
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        answer = ''
        try:
            answer = await _arch_run(query, question, ledger, deadline)
        except Exception:
            answer = ''
        if not _is_usable_answer(answer) and ledger.rows:
            try:
        ...
        text = _cap(_repoint(answer, _slot_pos)) or f'Best-effort answer unavailable for: {question[:400]}'
        public_note = None
        board = _BOARD_STORE.get('board')
        try:
            if isinstance(board, ClaimBoard):
                public_note = await _public_note(question, text, board, deadline)
        except Exception:
            public_note = None
        if public_note:
            public_note = _cap(_repoint(_normalize_brackets(public_note), _slot_pos))
        ...
        if query.output_schema is not None:
            ...
            if structured is not None:
                ...
                try:
                    ...
                    pass
                note = None if fast else public_note or synth_note
                try:
                    return Response(output=structured, note=note, citations=citations or None)
                except Exception:
                    structured = None
            basis = answer if _is_usable_answer(answer) else ''
            if not basis:
            ...
            if basis is not answer:
                ...
                if salvaged is not None:
                    try:
                        return Response(output=salvaged, note=None if fast else public_note, citations=citations or None)
                    except Exception:
                        pass
            if basis is not answer:
                cleaned = _undigest_for_schema(basis)
            ...
            try:
                forced = _coerce_to_schema(_cap(basis), query.output_schema)
                return Response(output=forced, note=None if fast else public_note, citations=citations or None)
            except Exception:
                try:
                    return Response(output=_cap(basis)[:2000], citations=citations or None)
                except Exception:
                    pass
        try:
            return Response(text=text, note=None if fast else public_note, citations=citations or None)
        except Exception:
            return Response(text=text)
    ...
    def _gx_defects(question: str, answer: str) -> list:
        ...
        window = _gx_out_of_window(question, answer)
        if window:
            notes.append('The question fixes a year window but the answer asserts years outside it: ' + ', '.join(window))
        missing = _gx_missing_entities(question, answer)
        if missing:
            notes.append('Named question entities never appear in the answer: ' + ', '.join(missing[:3]))
    ...
    VERSION = 'v53-claim-board'
    _GX_ACTIVE = ('cite', 'super', 'unit', 'window', 'entity')
    return query
...
def _sgqagitcyt():
    ...
    return query
_vwfujjzrya = _qnbrtuoyda()
_wdqfkpzsen = _adhohrotgi()
_arnefjyucn = _egudwhlyjz()
_ffwljttnjq = _sgqagitcyt()
_qhktervufw = 290.0
_kgnquwygin = 250.0
_svawwlfdnv = 90.0

async def _rnypllftvp(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _qhktervufw - (time.monotonic() - started)
        if first:
            budget = _kgnquwygin if _kgnquwygin < remaining else remaining
            first = False
        else:
            if remaining < _svawwlfdnv:
                break
        ...
        try:
            ...
            last_exc = exc
    return _ecysculfbf(query)
_ylycwsstiu = 235.0

async def _kpdvxgwctb(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _lsqnpkxrrb(query, agents):
    if len(agents) < 2:
        return await _rnypllftvp(query, agents)
    r0, r1 = await asyncio.gather(_kpdvxgwctb(agents[0], query, _ylycwsstiu), _kpdvxgwctb(agents[1], query, _ylycwsstiu))
    ok0 = isinstance(r0, Response) and _kkohiygwhz(r0) != ''
    ok1 = isinstance(r1, Response) and _kkohiygwhz(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_lgtcsekltn(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _yytxjhhrcu['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _rnypllftvp(query, (_arnefjyucn, _wdqfkpzsen, _vwfujjzrya, _ffwljttnjq))
        index = _gfmnpxuhhv(query)
        if index == 0:
            agents = (_vwfujjzrya, _wdqfkpzsen, _arnefjyucn, _ffwljttnjq)
        elif index == 1:
            agents = (_wdqfkpzsen, _arnefjyucn, _ffwljttnjq, _vwfujjzrya)
        elif index == 2:
            agents = (_arnefjyucn, _ffwljttnjq, _vwfujjzrya, _wdqfkpzsen)
        elif index == 3:
            agents = (_ffwljttnjq, _vwfujjzrya, _wdqfkpzsen, _arnefjyucn)
        else:
            agents = (_vwfujjzrya, _wdqfkpzsen, _arnefjyucn, _ffwljttnjq)
        return await _lsqnpkxrrb(query, agents)
    except Exception:
        return _ecysculfbf(query)
_BUILDM_TAG_m12 = "sn45-af692ba82cb2"
