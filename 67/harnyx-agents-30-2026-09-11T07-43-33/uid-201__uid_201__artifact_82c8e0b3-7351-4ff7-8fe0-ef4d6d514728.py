...
def _compose_saffron_prism_agent_entry():
...
_saffron_prism_agent_query_entry = _compose_saffron_prism_agent_entry()


def _compose_willow_relay_agent_entry():
...
_willow_relay_agent_query_entry = _compose_willow_relay_agent_entry()


def _compose_amber_prism_agent_entry():
...
_amber_prism_agent_query_entry = _compose_amber_prism_agent_entry()


_SHAPE_ROUTER_SEED = "baf2c46ac27926d14734ff9d"
...
def _balanced_route_label(query: Query) -> str:
    if getattr(query, "fast", False):
        return "SaffronPrismAgent"
    text = (getattr(query, "text", "") or "").strip()
    shape = _shape_class(query)
    if shape == 0:
        return "WillowRelayAgent"
    if shape == 1:
        return "AmberPrismAgent"
...
class SaffronPrismAgent:
    async def __call__(self, query: Query) -> Response:
        return await _saffron_prism_agent_query_entry(query)


class WillowRelayAgent:
    async def __call__(self, query: Query) -> Response:
        return await _willow_relay_agent_query_entry(query)


class AmberPrismAgent:
    async def __call__(self, query: Query) -> Response:
        return await _amber_prism_agent_query_entry(query)


_SHAPE_PRIMARY_AGENT = SaffronPrismAgent()
_SHAPE_SECONDARY_AGENT = WillowRelayAgent()
_SHAPE_TERTIARY_AGENT = AmberPrismAgent()
