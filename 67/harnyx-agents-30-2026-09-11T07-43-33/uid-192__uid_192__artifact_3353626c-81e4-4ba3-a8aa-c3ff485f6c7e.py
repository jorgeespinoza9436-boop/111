...
def _compose_branch_6bcb88f58192a49c645cfbf25b021b5ff5fb96502977a0b416f6a24beb305518_entry():
    ...
    def _sn67_successfully_parsed_null_reply(text):
        """Recognize null only through the native parser's whole-value/fence lane."""
        ...
        if fenced:
            body = fenced.group(1).strip()
        try:
            return json.loads(body) is None
    ...
    async def _structured_response(query: Query, schema: object, drafted: Response, deadline: float) -> Response:
        ...
        for attempt in range(STRUCTURED_ATTEMPTS):
            ...
            if (parsed is None and _sn67_successfully_parsed_null_reply(raw)
                    and _sn67_null_schema_status(schema) is True):
                proof = await _so_proof(question, None, answer, evidence, deadline)
                return _so_response(None, citations,
                                    _so_best_note(proof, answer, None, citations))
            if parsed is None:
    ...
    def _sn67_empty_schema_status(value, schema):
        'True/False/None for only empty string/list/object; bounded, no imports or IO.\n\n    Unknown schema features never bypass the original native repair. This is\n    not a general validator and does not replace trusted host schema ingress.\n    '
        kind = "string" if isinstance(value, str) and value == "" else (
            "array" if isinstance(value, list) and len(value) == 0 else (
                "object" if isinstance(value, dict) and len(value) == 0 else None))
        if kind is None:
            return None
        active = set()
        work = [0]
        ...
        allowed = {
            ...
            "minLength", "maxLength", "minItems", "maxItems", "items", "prefixItems", "uniqueItems",
            "contains", "minContains", "maxContains", "unevaluatedItems",
            "required", "minProperties", "maxProperties", "properties", "patternProperties",
            "additionalProperties", "unevaluatedProperties", "propertyNames", "dependentRequired", "dependentSchemas",
        ...
        def resolve(reference):
            if not isinstance(reference, str) or len(reference) > 4096 or "%" in reference:
                return False, None
            if reference != "#" and not reference.startswith("#/"):
                return False, None
            target = schema
            for token in reference[2:].split("/") if reference != "#" else ():
                work[0] += 1
                if work[0] > 1500 or "~" in token.replace("~0", "").replace("~1", ""):
                    return False, None
                token = token.replace("~1", "/").replace("~0", "~")
                if isinstance(target, dict):
                    if "$id" in target or token not in target:
                    ...
                    if (not token.isascii() or not token.isdecimal()
                            ...
                            or len(token) > 8 or int(token) >= len(target)):
                        return False, None
                    target = target[int(token)]
                else:
                    return False, None
            return isinstance(target, (bool, dict)), target

        # Trusted host ingress resolves refs even in unevaluated schema positions.
        # Conservatively inspect that context without treating item schemas as
        # constraints on the empty container itself. No annotations are traversed.
        pending = [(schema, 0)]
        seen = set()
        while pending:
            node, depth = pending.pop()
            work[0] += 1
            if work[0] > 1500 or depth > 48:
                return None
            if isinstance(node, bool):
                continue
            if (not isinstance(node, dict) or len(node) > 1500
                    or any(not isinstance(key, str) or len(key) > 256 or key not in allowed for key in node)
                    or node.get("$schema", dialect) != dialect):
                return None
            if id(node) in seen:
                continue
            seen.add(id(node))
            children = []
            if "$ref" in node:
                resolved, target = resolve(node["$ref"])
                if not resolved:
                    return None
                children.append(target)
            for keyword in ("$defs", "definitions", "properties", "patternProperties", "dependentSchemas"):
                if keyword in node:
                    if not isinstance(node[keyword], dict) or len(node[keyword]) > 1500:
                        return None
                    children.extend(node[keyword].values())
            for keyword in ("allOf", "anyOf", "oneOf", "prefixItems"):
                if keyword in node:
                    if not isinstance(node[keyword], list) or len(node[keyword]) > 1500:
                        return None
                    children.extend(node[keyword])
            for keyword in ("not", "if", "then", "else", "items", "contains", "unevaluatedItems",
                            "additionalProperties", "unevaluatedProperties", "propertyNames"):
                if keyword in node:
                    children.append(node[keyword])
            if len(children) + len(pending) > 1500:
                return None
        ...
        def check(node, depth):
            ...
            if isinstance(node, bool):
                return node
            if (not isinstance(node, dict) or id(node) in active or len(node) > 1500
                    or any(not isinstance(key, str) or len(key) > 256 or key not in allowed for key in node)):
                return None
            if node.get("$schema", dialect) != dialect:
                return None
            ...
            try:
                ...
                if "type" in node:
                    kinds = node["type"] if isinstance(node["type"], list) else [node["type"]]
                    if not kinds or len(kinds) > 1500 or any(item not in (
                        "null", "object", "array", "number", "integer", "boolean", "string",
                    ) for item in kinds):
                        return None
                    outcomes.append(kind in kinds)
                if "const" in node:
                    outcomes.append(node["const"] == value)
                if "enum" in node:
                    if not isinstance(node["enum"], list) or not node["enum"] or len(node["enum"]) > 1500:
                        return None
                    outcomes.append(any(item == value for item in node["enum"]))
                for keyword in ("minLength", "maxLength", "minItems", "maxItems", "minContains", "maxContains",
                                "minProperties", "maxProperties"):
                    if keyword in node and (not isinstance(node[keyword], int)
                            or isinstance(node[keyword], bool) or node[keyword] < 0):
                        return None
                if "uniqueItems" in node and not isinstance(node["uniqueItems"], bool):
                    return None
                if "prefixItems" in node and not isinstance(node["prefixItems"], list):
                    return None
                for keyword in ("items", "contains", "unevaluatedItems"):
                    if keyword in node and not isinstance(node[keyword], (bool, dict)):
                        return None
                if "required" in node and (not isinstance(node["required"], list) or len(node["required"]) > 1500
                        or any(not isinstance(item, str) or len(item) > 4096 for item in node["required"])):
                    return None
                if "dependentRequired" in node:
                    dependencies = node["dependentRequired"]
                    if not isinstance(dependencies, dict) or len(dependencies) > 1500:
                        return None
                    count = 0
                    for names in dependencies.values():
                        if not isinstance(names, list) or len(names) > 1500:
                            return None
                        count += len(names)
                        if count > 1500 or any(not isinstance(item, str) or len(item) > 4096 for item in names):
                            return None
                if kind == "string":
                    outcomes.append(node.get("minLength", 0) == 0)
                elif kind == "array":
                    outcomes.append(node.get("minItems", 0) == 0)
                    if "contains" in node:
                        outcomes.append(node.get("minContains", 1) == 0)
                else:
                    outcomes.append(node.get("minProperties", 0) == 0 and not node.get("required", []))
                for keyword in ("allOf", "anyOf", "oneOf"):
                    if keyword not in node:
                        continue
                    if not isinstance(node[keyword], list) or not node[keyword] or len(node[keyword]) > 1500:
                        return None
                    values = []
                    for child in node[keyword]:
                        values.append(check(child, depth + 1))
                        if work[0] > 1500:
                            return None
                    if keyword == "allOf":
                        outcome = conjunction(values)
                    elif keyword == "anyOf":
                        outcome = True if True in values else (None if None in values else False)
                    else:
                        positives = sum(item is True for item in values)
                        outcome = False if positives > 1 else (None if None in values else positives == 1)
                    outcomes.append(outcome)
                if "not" in node:
                    outcome = check(node["not"], depth + 1)
                    outcomes.append(None if outcome is None else not outcome)
                if "if" in node:
                    condition = check(node["if"], depth + 1)
                    outcomes.append(None if condition is None else check(
                        node.get("then" if condition else "else", True), depth + 1))
                return conjunction(outcomes)
            finally:
                active.remove(id(node))
    ...
    async def _w4_repair_structured_output(
        ...
        if ("output" in getattr(response, "model_fields_set", ())
                and getattr(response, "text", None) is None
                and _sn67_empty_schema_status(output, schema) is True):
            return response
        if (output is None and "output" in getattr(response, "model_fields_set", ())
                and getattr(response, "text", None) is None
                and _sn67_null_schema_status(schema) is True):
            return response
...
_branch_6bcb88f58192a49c645cfbf25b021b5ff5fb96502977a0b416f6a24beb305518_query_entry = _compose_branch_6bcb88f58192a49c645cfbf25b021b5ff5fb96502977a0b416f6a24beb305518_entry()


def _compose_branch_6a6e7719f79bd1fe10696d6cd80071fd31b70f7be1fade279f6628a483f3b33e_entry():
...
_branch_6a6e7719f79bd1fe10696d6cd80071fd31b70f7be1fade279f6628a483f3b33e_query_entry = _compose_branch_6a6e7719f79bd1fe10696d6cd80071fd31b70f7be1fade279f6628a483f3b33e_entry()


def _compose_branch_83f6cc1b0b6965f48c533b90898933c61c230d5ef4152d22d966c1a1698eaf64_entry():
...
_branch_83f6cc1b0b6965f48c533b90898933c61c230d5ef4152d22d966c1a1698eaf64_query_entry = _compose_branch_83f6cc1b0b6965f48c533b90898933c61c230d5ef4152d22d966c1a1698eaf64_entry()
...
class Branch_6bcb88f58192a49c645cfbf25b021b5ff5fb96502977a0b416f6a24beb305518:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch_6bcb88f58192a49c645cfbf25b021b5ff5fb96502977a0b416f6a24beb305518_query_entry(query=query)


class Branch_6a6e7719f79bd1fe10696d6cd80071fd31b70f7be1fade279f6628a483f3b33e:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch_6a6e7719f79bd1fe10696d6cd80071fd31b70f7be1fade279f6628a483f3b33e_query_entry(query=query)


class Branch_83f6cc1b0b6965f48c533b90898933c61c230d5ef4152d22d966c1a1698eaf64:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch_83f6cc1b0b6965f48c533b90898933c61c230d5ef4152d22d966c1a1698eaf64_query_entry(query=query)


_TASK_SHAPE_PRIMARY_AGENT = Branch_6bcb88f58192a49c645cfbf25b021b5ff5fb96502977a0b416f6a24beb305518()
_TASK_SHAPE_SECONDARY_AGENT = Branch_6a6e7719f79bd1fe10696d6cd80071fd31b70f7be1fade279f6628a483f3b33e()
_TASK_SHAPE_TERTIARY_AGENT = Branch_83f6cc1b0b6965f48c533b90898933c61c230d5ef4152d22d966c1a1698eaf64()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "Branch_6bcb88f58192a49c645cfbf25b021b5ff5fb96502977a0b416f6a24beb305518",
    "Branch_6a6e7719f79bd1fe10696d6cd80071fd31b70f7be1fade279f6628a483f3b33e",
    "Branch_83f6cc1b0b6965f48c533b90898933c61c230d5ef4152d22d966c1a1698eaf64",
)
