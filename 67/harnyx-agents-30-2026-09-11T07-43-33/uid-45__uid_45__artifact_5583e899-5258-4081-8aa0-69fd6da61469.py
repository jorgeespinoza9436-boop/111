...
def _compose_willow_cairn_slot04_agent_entry():
...
_willow_cairn_slot04_agent_query_entry = _compose_willow_cairn_slot04_agent_entry()


def _compose_nimbus_mosaic_slot04_agent_entry():
...
_nimbus_mosaic_slot04_agent_query_entry = _compose_nimbus_mosaic_slot04_agent_entry()


def _compose_vesper_cairn_slot04_agent_entry():
    ...
    # ---- v270-12-khsa ----
    # Stages: coverage nudge, temporal repair, set gap-fill, authority sweep
    # Ordinary successful path:
    #   query -> _solve -> _knowledge_brief -> _loop (+_open_criteria_hint) -> _audit_patch -> _align_timeframe -> _widen_pool -> _anchor_primary_source -> _citations_for -> _answer_line_only -> Response
...
_vesper_cairn_slot04_agent_query_entry = _compose_vesper_cairn_slot04_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "WillowCairnSlot04Agent"
    ...
    if (
        ...
        return "NimbusMosaicSlot04Agent"
    return "VesperCairnSlot04Agent"


class WillowCairnSlot04Agent:
    async def __call__(self, query: Query) -> Response:
        return await _willow_cairn_slot04_agent_query_entry(query)


class NimbusMosaicSlot04Agent:
    async def __call__(self, query: Query) -> Response:
        return await _nimbus_mosaic_slot04_agent_query_entry(query)


class VesperCairnSlot04Agent:
    async def __call__(self, query: Query) -> Response:
        return await _vesper_cairn_slot04_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = WillowCairnSlot04Agent()
_ANALYTICAL_FIELD_AGENT = NimbusMosaicSlot04Agent()
_BROAD_FIELD_AGENT = VesperCairnSlot04Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "WillowCairnSlot04Agent",
    "NimbusMosaicSlot04Agent",
    "VesperCairnSlot04Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "WillowCairnSlot04Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "NimbusMosaicSlot04Agent":
