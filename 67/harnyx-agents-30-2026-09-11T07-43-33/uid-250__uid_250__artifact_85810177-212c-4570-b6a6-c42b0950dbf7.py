...
def _compose_ember_quill_agent_entry():
...
_ember_quill_agent_query_entry = _compose_ember_quill_agent_entry()


def _compose_slate_quill_agent_entry():

    _COOKING_QUERY_TAG = "cooking-hk6738"  # per-hotkey canonical uniqueness
    ...
    async def _ck67_base_query(query: Query) -> Response:
    ...
    # --- ck67 confrontation ledger (start) ---------------------------------------
    # Ordinary-path cycle added on top of the baseline agent:
    #   draft -> independently-sourced member ledger -> if any required member is
    #   missing, conflicted across sources, or uncited -> targeted retrieval ->
    #   regenerated answer. That is a cross-stage feedback edge: an intermediate
    #   ledger decides whether research re-enters and the answer is produced again.
    # The baseline research corpus and final answer type remain the root; this
    # cycle only fires when the ledger shows a query-required synthesis defect.

    import json as _ck67_json
    import re as _ck67_re
    from time import monotonic as _ck67_monotonic

    _CK67_LLM_PROVIDER = "openrouter"
    _CK67_LLM_MODELS = ("z-ai/glm-5.2", "deepseek/deepseek-v3.2")
    _CK67_SEARCH_PROVIDERS = ("parallel", "desearch")
    _CK67_OPEN_COVERAGE = frozenset({"missing", "conflicted", "uncited"})
    _CK67_MAX_OPEN = 3
    _CK67_MAX_NEW_CITES = 6
    _CK67_MAX_SNIPPETS = 4
    _CK67_AUDIT_TIMEOUT_S = 18.0
    _CK67_SEARCH_TIMEOUT_S = 12.0
    _CK67_REGEN_TIMEOUT_S = 22.0
    _CK67_SKIP_AFTER_S = 248.0
    _CK67_ANSWER_CHARS = 12000
    _CK67_DIGEST_CHARS = 9000
    _CK67_POINTER_RE = _ck67_re.compile(r"\[\[(\d{1,3})\]\]")

    _CK67_AUDIT_SYSTEM = (
        "You audit a drafted research answer against the user query.\n"
        "Build a confrontation ledger of independently sourced members the query "
        "requires. Members are comparison sides, the reconciled conclusion, pool "
        "survivors, decisive exclusions, period/basis pairs, status/identity/date/"
        "category/version joins, and load-bearing uncited facts.\n"
        "coverage values:\n"
        "- established: the draft already states the member and it is internally consistent\n"
        "- missing: the query requires this independently sourced member and the draft omits it\n"
        "- conflicted: the draft asserts incompatible values, periods, or bases for this member\n"
        "- uncited: the member is a material researched claim in prose and has no [[n]] pointer\n"
        "needs_fresh_research_and_rewrite is true only when at least one member is "
        "missing, conflicted, or uncited. Those outcomes mean the required synthesis "
        "is incomplete, so retrieval must re-enter and the answer must be produced again.\n"
        "Return JSON only with keys shape, members, needs_fresh_research_and_rewrite.\n"
        "shape is comparison, pool, reconciliation, or lookup.\n"
        "members is an array of {id, role, claim, coverage, search_query}. "
        "role is side_a, side_b, conclusion, pool_member, exclusion, period_basis, "
        "status_join, or uncited_claim. At most 6 members. search_query is a short "
        "targeted retrieval query for that member."
    )

    _CK67_REGEN_SYSTEM = (
        "Rewrite the drafted answer using the confrontation ledger and the fresh "
        "evidence packet. The query requires independent-source synthesis: cover "
        "every required member, both comparison sides plus the reconciled "
        "conclusion, and decisive pool exclusions when asked.\n"
        "Ground every material researched claim in the numbered fresh evidence or "
        "keep a baseline claim only when the ledger marked it established. Do not "
        "invent names, dates, figures, statuses, or exclusions. If a required "
        "member is still unsupported after the packet, state that limitation "
        "briefly instead of guessing. Prefer a shorter fully supported answer over "
        "a longer unsupported one. Do not add Proof, Candidate pool, Evidence, or "
        "Sources sections.\n"
        "For prose answers: use [[n]] pointers. Existing citations occupy 1..E. "
        "Fresh evidence occupies E+1..E+F as numbered in the packet. [n] is not a "
        "citation. Do not dump provenance.\n"
        "For structured output: keep the exact schema. Do not put citation syntax "
        "in atomic fields. Put the why-this-value explanation in note with [[n]] "
        "when the structured value is atomic.\n"
        "Return JSON only with keys text, output, note. Use null for unused fields. "
        "text is the full prose answer or null. output is the structured object or "
        "null. note is optional public supplementary text or null."
    )


    def _ck67_llm_text(result: object) -> str:
        llm = getattr(result, "llm", None) or getattr(result, "response", None)
        text = getattr(llm, "raw_text", None) if llm is not None else None
        if isinstance(text, str) and text.strip():
            return text
    ...
    def _ck67_parse_json(raw: str) -> dict:
        text = (raw or "").strip()
        if not text:
            return {}
        fence = _ck67_re.search(r"```(?:json)?\s*(\{.*\})\s*```", text, _ck67_re.S)
        if fence:
            text = fence.group(1)
        start = text.find("{")
        end = text.rfind("}")
        if start < 0 or end <= start:
            return {}
        try:
            payload = _ck67_json.loads(text[start : end + 1])
        except Exception:
            return {}
        return payload if isinstance(payload, dict) else {}


    async def _ck67_chat(system: str, user: str, *, max_output_tokens: int, timeout: float) -> dict:
        from harnyx_miner_sdk.api import llm_chat

        last_error: Exception | None = None
        for model in _CK67_LLM_MODELS:
            try:
                result = await llm_chat(
                    provider=_CK67_LLM_PROVIDER,
                    model=model,
                    messages=(
                        {"role": "system", "content": system},
                        {"role": "user", "content": user},
                ...
                payload = _ck67_parse_json(_ck67_llm_text(result))
                if payload:
                    return payload
            except Exception as exc:
                last_error = exc
    ...
    def _ck67_draft_blob(response: object) -> str:
        parts: list[str] = []
        text = getattr(response, "text", None)
        output = getattr(response, "output", None)
        note = getattr(response, "note", None)
        if isinstance(text, str) and text.strip():
            parts.append("ANSWER_TEXT:\n" + text.strip()[:_CK67_ANSWER_CHARS])
        if output is not None:
            try:
                parts.append("STRUCTURED_OUTPUT:\n" + _ck67_json.dumps(output, ensure_ascii=False)[:8000])
            except Exception:
                parts.append("STRUCTURED_OUTPUT:\n" + str(output)[:8000])
    ...
    def _ck67_deterministic_members(question: str, draft: str) -> list[dict]:
        q = (question or "").strip()
        d = (draft or "").strip()
        ql = q.lower()
        dl = d.lower()
        ...
        if any(marker in ql for marker in compare_markers):
            if not any(token in dl for token in ("higher", "lower", "same", "equal", "greater", "less", "agree", "differ")):
                members.append(
                    {
                        "id": "D_COMPARE",
                        "role": "conclusion",
                        "claim": "reconciled comparison conclusion with both independently sourced sides and a shared period or basis",
                        "coverage": "missing",
                        "search_query": q[:280],
                    }
                )
        ...
        if any(marker in ql for marker in pool_markers):
            if not any(token in dl for token in ("exclud", "not included", "removed", "do not qualify", "outside")):
                members.append(
                    {
                        "id": "D_POOL",
                        "role": "exclusion",
                        "claim": "complete survivor set and the decisive exclusions that change the pool",
                        "coverage": "missing",
                        "search_query": (q + " official list exclusions")[:280],
                    }
                )
        if _ck67_re.search(r"\b(20\d{2}|percent|percentage|%|rank|vote|effective|ceo|director)\b", ql):
            if not _ck67_re.search(r"\d", d):
                members.append(
                    {
                        "id": "D_VALUE",
                        "role": "status_join",
                        "claim": "the concrete figure, date, rank, or named official the query asks for",
                        "coverage": "missing",
                        "search_query": q[:280],
                    }
                )
            ...
            members.append(
                {
                    "id": "D_CITE",
                    "role": "uncited_claim",
                    "claim": "traceable [[n]] support for each material researched claim",
                    "coverage": "uncited",
                    "search_query": q[:280],
                }
            )
        return members[:_CK67_MAX_OPEN]


    def _ck67_normalize_member(item: object, index: int, question: str) -> dict | None:
        if not isinstance(item, dict):
            return None
        ...
        if coverage not in {"established", "missing", "conflicted", "uncited"}:
            return None
        search_query = str(item.get("search_query") or "").strip() or (question[:200] + " " + claim[:80])
        role = str(item.get("role") or "uncited_claim").strip().lower()
        return {
    ...
    async def _ck67_build_ledger(question: str, draft: str) -> dict:
        payload = await _ck67_chat(
            _CK67_AUDIT_SYSTEM,
            "Query:\n"
            + question[:4000]
            + "\n\nDraft:\n"
            + draft[:_CK67_ANSWER_CHARS]
            + "\n\nList every independently sourced member the query requires and mark coverage.",
            max_output_tokens=1400,
            timeout=_CK67_AUDIT_TIMEOUT_S,
        )
        members: list[dict] = []
        ...
        if isinstance(rows, list):
            for item in rows:
                row = _ck67_normalize_member(item, len(members) + 1, question)
                if row is None:
                    continue
                key = (row["coverage"], row["claim"][:80].lower())
                if key in seen:
                    continue
                seen.add(key)
                members.append(row)
        for row in _ck67_deterministic_members(question, draft):
            key = (row["coverage"], row["claim"][:80].lower())
            if key in seen:
                continue
            seen.add(key)
            members.append(row)
        return {"members": members, "shape": str(payload.get("shape") or "lookup")}


    def _ck67_open_members(ledger: dict) -> list[dict]:
        open_rows: list[dict] = []
        for row in ledger.get("members") or []:
            coverage = row.get("coverage")
            if coverage in _CK67_OPEN_COVERAGE:
                open_rows.append(row)
        return open_rows[:_CK67_MAX_OPEN]


    async def _ck67_search(query_text: str) -> tuple[object | None, list[object]]:
        from harnyx_miner_sdk.api import search_web

        q = (query_text or "").strip()[:300]
        if not q:
            return None, []
        for provider in _CK67_SEARCH_PROVIDERS:
            try:
                packet = await search_web(
                    q,
                    provider=provider,
                    num=5,
                    timeout=_CK67_SEARCH_TIMEOUT_S,
                )
            except Exception:
                continue
    ...
    def _ck67_row_fields(row: object) -> tuple[str, str, str, str]:
        result_id = str(getattr(row, "result_id", "") or "")
        title = str(getattr(row, "title", "") or "")
        url = str(getattr(row, "url", "") or getattr(row, "link", "") or "")
        note = str(getattr(row, "note", "") or getattr(row, "snippet", "") or "")
        return result_id, title, url, note


    def _ck67_cite(receipt_id: str, row: object):
        from harnyx_miner_sdk.query import CitationRef, CitationSlice

        result_id, _title, _url, note = _ck67_row_fields(row)
    ...
    async def _ck67_retrieve_open_members(question: str, open_members: list[dict]) -> dict:
        digest_parts: list[str] = []
        citations: list = []
        seen: set[tuple[str, str]] = set()
        marker = 0
        for row in open_members:
            packet, results = await _ck67_search(str(row.get("search_query") or question))
            ...
            for result in results[:_CK67_MAX_SNIPPETS]:
                result_id, title, url, note = _ck67_row_fields(result)
                marker += 1
                digest_parts.append(
                    f"[{marker}] {title}\nurl: {url}\nexcerpt: {note[:700]}"
                )
                ...
                if key in seen:
                    continue
                cite = _ck67_cite(receipt_id, result)
                if cite is None:
                    continue
                citations.append(cite)
                seen.add(key)
                if len(citations) >= _CK67_MAX_NEW_CITES:
                    break
            if len(citations) >= _CK67_MAX_NEW_CITES:
                break
        return {
            "digest": "\n".join(digest_parts)[:_CK67_DIGEST_CHARS],
            "citations": citations[:_CK67_MAX_NEW_CITES],
            "fresh_count": min(marker, _CK67_MAX_NEW_CITES),
        }


    def _ck67_merge_citations(existing: object, added: list) -> list:
        merged: list = []
        seen: set[tuple[str, str, tuple]] = set()
        for cite in list(existing or []) + list(added or []):
            receipt_id = str(getattr(cite, "receipt_id", "") or "")
            result_id = str(getattr(cite, "result_id", "") or "")
            slices = tuple(
                (getattr(sl, "start", 0), getattr(sl, "end", 0))
                for sl in (getattr(cite, "slices", None) or ())
            )
            key = (receipt_id, result_id, slices)
    ...
    def _ck67_rebuild(response: object, text, output, note, citations):
        from harnyx_miner_sdk.query import Response as _CkResp

        final_note = note if isinstance(note, str) and note.strip() else None
        final_cites = citations if citations else None
        try:
            if output is not None:
                if final_note is not None and final_cites is not None:
                    return _CkResp(output=output, note=final_note, citations=final_cites)
                if final_note is not None:
                    return _CkResp(output=output, note=final_note)
                if final_cites is not None:
                    return _CkResp(output=output, citations=final_cites)
                return _CkResp(output=output)
            final_text = text.strip() if isinstance(text, str) and text.strip() else getattr(response, "text", None)
            if not isinstance(final_text, str) or not final_text.strip():
                return response
            if final_note is not None and final_cites is not None:
                return _CkResp(text=final_text, note=final_note, citations=final_cites)
            if final_note is not None:
                return _CkResp(text=final_text, note=final_note)
            if final_cites is not None:
                return _CkResp(text=final_text, citations=final_cites)
            return _CkResp(text=final_text)
        except Exception:
            return response


    def _ck67_schema_json(query_input: object) -> str:
        schema = getattr(query_input, "output_schema", None)
        if schema is None:
            return "null"
        try:
            return _ck67_json.dumps(schema, ensure_ascii=False)[:4000]
        except Exception:
            return "null"


    async def _ck67_regenerate(
        question: str,
        query_input: object,
        response: object,
        ...
        fresh = list(packet.get("citations") or [])
        payload = await _ck67_chat(
            _CK67_REGEN_SYSTEM,
            "Query:\n"
            + question[:4000]
            + "\n\noutput_schema:\n"
            + _ck67_schema_json(query_input)
            + "\n\nDraft:\n"
            + _ck67_draft_blob(response)[:_CK67_ANSWER_CHARS]
            + "\n\nOpen independently-sourced members that forced re-entry:\n"
            + _ck67_json.dumps(open_members, ensure_ascii=False)[:3000]
            + f"\n\nExisting citations occupy [[1]]..[[{existing_n or 0}]]. "
            + f"Fresh evidence below occupies [[{existing_n + 1}]]..[[{existing_n + len(fresh)}]].\n"
            + "Fresh evidence packet:\n"
            + str(packet.get("digest") or "")
            + "\n\nRewrite the complete answer from the ledger and packet.",
            max_output_tokens=1800,
            timeout=_CK67_REGEN_TIMEOUT_S,
        )
        if not payload:
            return None
        ...
        if schema is not None:
            if output is None:
                return None
            merged = _ck67_merge_citations(existing, fresh)
            return _ck67_rebuild(response, None, output, note, merged)
        if text is None:
            return None
        merged = _ck67_merge_citations(existing, fresh)
        return _ck67_rebuild(response, text, None, note, merged)


    async def _ck67_confrontation_cycle(query_input: object, response: object, started: float):
        if _ck67_monotonic() - started >= _CK67_SKIP_AFTER_S:
            return response
        question = str(getattr(query_input, "text", "") or "").strip()
        if not question:
            return response
        draft = _ck67_draft_blob(response)
        if not draft:
            return response
        ledger = await _ck67_build_ledger(question, draft)
        open_members = _ck67_open_members(ledger)
        if not open_members:
            return response
        if _ck67_monotonic() - started >= _CK67_SKIP_AFTER_S:
            return response
        packet = await _ck67_retrieve_open_members(question, open_members)
        if not packet.get("digest"):
            return response
        rewritten = await _ck67_regenerate(question, query_input, response, open_members, packet)
        if rewritten is None:
            return response
        return rewritten


    # --- ck67 confrontation ledger (end) -----------------------------------------

    async def query(query: Query) -> Response:
        _ck67_started = _ck67_monotonic()
        try:
            _ck67_draft = await _ck67_base_query(query)
        except Exception:
            from harnyx_miner_sdk.query import Response as _Ck67Floor
            try:
                return _Ck67Floor(text='No verifiable source-backed answer was reached for this question.')
            except Exception:
                raise
        try:
            return await _ck67_confrontation_cycle(query, _ck67_draft, _ck67_started)
        except Exception:
            return _ck67_draft
...
_SHAPE_ROUTER_SEED = "40d1d404c2ad33acc124192c"
...
def _balanced_route_label(query: Query) -> str:
    if getattr(query, "fast", False):
        return "CobaltRelayAgent"
    text = (getattr(query, "text", "") or "").strip()
    shape = _shape_class(query)
    if shape == 0:
        return "EmberQuillAgent"
    if shape == 1:
        return "SlateQuillAgent"
...
class EmberQuillAgent:
    async def __call__(self, query: Query) -> Response:
        return await _ember_quill_agent_query_entry(query)
...
_SHAPE_PRIMARY_AGENT = CobaltRelayAgent()
_SHAPE_SECONDARY_AGENT = EmberQuillAgent()
