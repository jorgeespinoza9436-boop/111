...
import re  # ⚠️ esta base NAO importa `re` no topo (so nos escopos aninhados):
           # sem isto o upload falha com "name 're' is not defined".

# ── REEXTRACAO: saida com cara de default e falha de LEITURA ─────────────────
# MEDIDO 15/08 em tres arquiteturas distintas (uid212, uid113 e o w2 do uid224):
# a task 475f0b1b saia {"value_2025_edition":0,...} — juiz: "This is completely
# incorrect" — e passou a sair titanium 86->87 / palladium 36->34, IDENTICO a
# referencia. Zero em todo campo / lista vazia e o que uma projecao devolve
# quando NAO CONSEGUIU LER, nao fato apurado; uma 2a projecao sobre a MESMA
# evidencia costuma ler o valor.
#
# 🚨 NAO ATUA com UM zero isolado (pode ser verdade): exige >= 2 zeros, ou lista
# vazia com a pergunta pressupondo membros.
# ⚠️ sem `nonlocal` — a plataforma REJEITA no upload (422 invalid_script_payload).

EX_MIN_ZEROS = 2
EX_PRESSUPOE = re.compile(
    r"\b(identify|list|name|which|what) [^.?]{0,60}\b(where|that|whose|which)\b"
    r"|\ba (?:couple|few|number) of\b|\bevery\b", re.I)


def _ex_suspeito(estruturado, question):
    conta = [0, 0]
    ...
    def anda(o):
        if isinstance(o, dict):
            ...
            conta[0] += 1
    anda(estruturado)
    if conta[0] >= EX_MIN_ZEROS:
        return True
    return bool(conta[1] and EX_PRESSUPOE.search(question or ""))


def _ex_conta_conteudo(estruturado):
    conta = [0]
        ...
        async def execute(self) -> Response:
            ...
            if query.output_schema is not None:
                ...
                try:
                    ...
                    # REEXTRAI se a projecao veio com cara de default (zeros/lista vazia):
                    # e leitura que falhou, nao fato. Medido em 3 arquiteturas.
                    if structured is not None and _ex_suspeito(structured, question):
                        try:
                            outra = await _schema_output(question, answer,
                                                         query.output_schema, deadline)
                            if outra is not None and \
                                    _ex_conta_conteudo(outra) > _ex_conta_conteudo(structured):
                                structured = outra
                        except Exception:
                            pass
                except Exception:
