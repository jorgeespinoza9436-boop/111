...
def _compose_cinder_tern_entry():
    from harnyx_miner_sdk.decorators import entrypoint
    from harnyx_miner_sdk.query import Query, Response
    ...
    class CottonSpool_2a7da6:
        ...
        def _compile(self):
            ...
            _AGENT_VARIANT = 'cleanfetch_variant'
    ...
    # --- build marker 20260815T0140Z_03_orok2530 -------------------------------------------------
    _BUILD_MARKER_20260815T0140Z_03_orok2530 = "20260815T0140Z"


    def _build_probe_20260815T0140Z_03_orok2530(seed: int = 0) -> int:
        """Unreferenced helper carrying the build identity."""
        acc = seed
        for i, ch in enumerate(_BUILD_MARKER_20260815T0140Z_03_orok2530):
            acc = (acc * 31 + ord(ch) + i) % 1000003
        return acc


    class _BuildStamp_20260815T0140Z_03_orok2530:
        tag = _BUILD_MARKER_20260815T0140Z_03_orok2530

        def digest(self) -> int:
            return _build_probe_20260815T0140Z_03_orok2530(len(self.tag))

    return query

_cinder_tern_query_entry = _compose_cinder_tern_entry()


def _compose_harbor_vellum_entry():
    ...
    VERSION = "v53-pool-timeframe-measure"
    ...
    # answer's claims (retained-evidence instructions says so). A question naming
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        # subject or the WRONG period is unsalvageable by formatting, so both scope
        # repairs run first. Measure conformance is LAST because both stages above
        # rewrite the whole answer and would discard a unit annotation applied
        # earlier — prior artifact shipped the opposite order and silently threw that work
        # away. Each stage re-checks its own time/spend floor and returns `answer`
        # untouched on any failure.
        for _sweep in (_verify_subjects, _align_timeframe, _conform_measures):
            try:
                if not _is_usable_answer(answer):
                    break
    ...
    _PERFECT_SUFFIX = "5690d80878c6f708"

    return query

_harbor_vellum_query_entry = _compose_harbor_vellum_entry()
...
class CinderTern:
    async def __call__(self, query: Query) -> Response:
        return await _cinder_tern_query_entry(query)


class HarborVellum:
    async def __call__(self, query: Query) -> Response:
        return await _harbor_vellum_query_entry(query)


_CLEAR_PRIMARY_AGENT = CinderTern()
_CLEAR_SECONDARY_AGENT = HarborVellum()
_CANDIDATE_BRANCH_CLASS_NAMES = ("CinderTern", "HarborVellum")
