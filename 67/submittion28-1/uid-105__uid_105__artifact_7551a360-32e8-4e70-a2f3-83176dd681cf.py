...
VERSION = "v53-openrouter-seven-b"
...
# wholesale) and before both numeric stages, which inspect the figures the
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # SIX sweeps share one tail. A firing sweep costs a search plus up to three
    # loop turns, so in practice only the first one or two that trigger will
    # execute and the rest close on their own floors. Treat the order as a
    ...
    # The in-loop coverage hint is the only stage guaranteed to run: it fires
    # before the answer exists and costs no time. Each sweep re-checks its own
    # floor and returns `answer` untouched on any failure, so a starved tail
    # degrades to the audited answer, never worse.
    for _sweep in (_verify_subjects, _align_timeframe, _widen_pool,
                   _ground_figures, _second_source_check, _conform_measures):
        try:
...
_PERFECT_SUFFIX = "996c5794870d9ca2"
