...
def _compose_copper_harbor_entry():
...
_copper_harbor_query_entry = _compose_copper_harbor_entry()


def _compose_velvet_cairn_entry():
    """agent_d — v32 "toolloop": model-driven research agent.
...
_velvet_cairn_query_entry = _compose_velvet_cairn_entry()
...
def _route_label(query: Query) -> str:
    ...
    if schema_depth >= 5 or comparison >= 4:
        return "CopperHarbor" if ripple not in {1, 6, 11} else "VelvetCairn"
    if length <= 360 and directish >= 2:
        return "VelvetCairn" if ripple not in {3, 8, 14} else "CopperHarbor"
    if document >= 3 and length >= 500:
        return "CopperHarbor"
    return "VelvetCairn" if ripple in {0, 2, 5, 9, 13, 16} else "CopperHarbor"


class CopperHarbor:
    async def __call__(self, query: Query) -> Response:
        return await _copper_harbor_query_entry(query)


class VelvetCairn:
    async def __call__(self, query: Query) -> Response:
        return await _velvet_cairn_query_entry(query)


_LEFT_FIELD_AGENT = CopperHarbor()
_RIGHT_FIELD_AGENT = VelvetCairn()
_CANDIDATE_BRANCH_CLASS_NAMES = ("CopperHarbor", "VelvetCairn")
_CANDIDATE_ROUTE_FUNCTION = "_route_label"


async def _s28_base_query(query: Query) -> Response:
    selected = _route_label(query)
    first = _LEFT_FIELD_AGENT if selected == "CopperHarbor" else _RIGHT_FIELD_AGENT
    second = _RIGHT_FIELD_AGENT if selected == "CopperHarbor" else _LEFT_FIELD_AGENT
    try:
