...
def _compose_granite_wisp_entry():
    from harnyx_miner_sdk.decorators import entrypoint
    from harnyx_miner_sdk.query import Query, Response
...
_granite_wisp_query_entry = _compose_granite_wisp_entry()


def _compose_meadow_quill_entry():
...
_meadow_quill_query_entry = _compose_meadow_quill_entry()


def _extract_route_grains(query: Query) -> tuple[int, int, int, int, int]:
    ...
    if isinstance(schema, dict):
        ...
        schema_depth += 3 if props else 1
        schema_depth += min(5, len(props)) if isinstance(props, dict) else 0
        schema_depth += min(3, len(required)) if isinstance(required, list) else 0
    elif schema is not None:
        schema_depth = 2
    comparison_grains = sum(
        1
        for marker in (
            "compare", "difference", "highest", "lowest", "largest", "smallest",
            "rank", "ordered", "sort", "first three", "top", "bottom",
            "greater than", "less than", "at least", "at most", "between",
            "exclude", "excluding", "include only", "only those", "calculate",
            "percentage", "ratio", "median", "average", "sum",
        )
    ...
    document_grains = sum(
        1
        for marker in (
            "pdf", "report", "appendix", "section", "table", "bulletin",
            "catalogue", "catalog", "annual", "statistics", "census",
            "register", "inventory", "filing", "official", "press kit",
        )
        if marker in lowered
    )
    direct_grains = sum(
        1
        for marker in (
            "who is", "what is", "where is", "when did", "name the",
            "identify the", "which", "give the complete list", "list the",
        )
        if marker in lowered
    )
    punctuation_grains = lowered.count(",") + lowered.count(";") * 2 + lowered.count("(")
    return (len(text), schema_depth, comparison_grains, document_grains, direct_grains + punctuation_grains)


def _ripple_index(query: Query) -> int:
    length, schema_depth, comparison, document, directish = _extract_route_grains(query)
    base = schema_depth * 19 + comparison * 13 + document * 7 + directish * 3
    if length > 1200:
        base += 23
    elif length > 720:
        base += 17
    ...
    import hashlib as _ripple_hashlib

    checksum = int(
        _ripple_hashlib.sha1(
            ("ripple-index-v2|" + text[:256] + "|" + text[-256:]).encode("utf-8", "ignore")
        ).hexdigest()[:6],
        16,
    )
    return (base + checksum) % 17


def _route_label(query: Query) -> str:
    length, schema_depth, comparison, document, directish = _extract_route_grains(query)
    ripple = _ripple_index(query)
    if schema_depth >= 5 or comparison >= 4:
        return "GraniteWisp" if ripple not in {1, 6, 11} else "MeadowQuill"
    if length <= 360 and directish >= 2:
        return "MeadowQuill" if ripple not in {3, 8, 14} else "GraniteWisp"
    if document >= 3 and length >= 500:
        return "GraniteWisp"
    return "MeadowQuill" if ripple in {0, 2, 5, 9, 13, 16} else "GraniteWisp"


class GraniteWisp:
    async def __call__(self, query: Query) -> Response:
        return await _granite_wisp_query_entry(query)


class MeadowQuill:
    async def __call__(self, query: Query) -> Response:
        return await _meadow_quill_query_entry(query)


_LEFT_FIELD_AGENT = GraniteWisp()
_RIGHT_FIELD_AGENT = MeadowQuill()
_CANDIDATE_BRANCH_CLASS_NAMES = ("GraniteWisp", "MeadowQuill")
_CANDIDATE_ROUTE_FUNCTION = "_route_label"


async def _s28_base_query(query: Query) -> Response:
    selected = _route_label(query)
    first = _LEFT_FIELD_AGENT if selected == "GraniteWisp" else _RIGHT_FIELD_AGENT
    second = _RIGHT_FIELD_AGENT if selected == "GraniteWisp" else _LEFT_FIELD_AGENT
    try:
