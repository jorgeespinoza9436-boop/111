...
VERSION = "v53-openrouter-pool-scope"
...
# block. Stage 3w below then verifies the result after the answer exists, so the
# two form a before/after pair on the same failure mode and share the same
# set/superlative gate. Any failure here means the block is simply absent.
#
# It is the only stage that acts BEFORE the answer exists, which is why it
# survives the tail contention the four post-audit sweeps compete in.
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # pipeline: whichever detector fires first will consume much of the window.
    #   subject — wrong entity makes every later check moot
    #   period  — wrong period invalidates pool membership itself
...
_PERFECT_SUFFIX = "873f387f542e63f1"
