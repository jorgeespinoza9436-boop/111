...
def _compose_coral_index_entry():
    ...
    _PERFECT_SUFFIX = "2e2be62884bcfc01"

    return query

_coral_index_query_entry = _compose_coral_index_entry()


def _compose_mistral_relay_entry():
    """agent_d — v32 "toolloop": model-driven research agent.
    ...
    # prior artifact shipped this pattern TWICE under two different names, with
    ...
    # 2+ = corroborated"). prior artifact shipped them in the opposite order, so a
    ...
    # annotation applied before them would be discarded by the next rewrite. prior artifact
...
_mistral_relay_query_entry = _compose_mistral_relay_entry()
...
def _route_label(query: Query) -> str:
    ...
    if schema_depth >= 5 or comparison >= 4:
        return "CoralIndex" if ripple not in {1, 6, 11} else "MistralRelay"
    if length <= 360 and directish >= 2:
        return "MistralRelay" if ripple not in {3, 8, 14} else "CoralIndex"
    if document >= 3 and length >= 500:
        return "CoralIndex"
    return "MistralRelay" if ripple in {0, 2, 5, 9, 13, 16} else "CoralIndex"


class CoralIndex:
    async def __call__(self, query: Query) -> Response:
        return await _coral_index_query_entry(query)


class MistralRelay:
    async def __call__(self, query: Query) -> Response:
        return await _mistral_relay_query_entry(query)


_LEFT_FIELD_AGENT = CoralIndex()
_RIGHT_FIELD_AGENT = MistralRelay()
_CANDIDATE_BRANCH_CLASS_NAMES = ("CoralIndex", "MistralRelay")
_CANDIDATE_ROUTE_FUNCTION = "_route_label"


async def _s28_base_query(query: Query) -> Response:
    selected = _route_label(query)
    first = _LEFT_FIELD_AGENT if selected == "CoralIndex" else _RIGHT_FIELD_AGENT
    second = _RIGHT_FIELD_AGENT if selected == "CoralIndex" else _LEFT_FIELD_AGENT
    try:
