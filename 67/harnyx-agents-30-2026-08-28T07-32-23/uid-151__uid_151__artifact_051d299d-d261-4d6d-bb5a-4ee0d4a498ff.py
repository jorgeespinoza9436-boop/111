...
def _build_agent_2():
    _S222S35_QUERY_TAG = 's222s35-hk6721'
    SEARCH_TIMEOUT_S = 18.0
    LANE_B_MAX_PAYLOAD_CHARS = 144000
    TURN_TIMEOUT_S = 75.0
    AUDIT_TIMEOUT_S = 28.0
    ...
    _W5G_MIN_MEMBERS = 3
    _W5G_HEDGE_RE = re.compile('\\bamong others\\b|\\band (?:several|many|a few) (?:more|others)\\b|\\band others\\b|\\bet al\\.?|\\betc\\.?|\\bmultiple (?:other|more)\\b|\\band more\\b|\\bincluding but not limited to\\b', re.I)
    _W5G_MEMBER_LINE_RE = re.compile('(?m)^[ \\t]*(?:[-*•]|\\(?\\d{1,2}[.)\\]])[ \\t]+\\S')
    ...
    async def _set_gapfill(question: str, answer: str, messages: list, ledger: EvidenceLedger, deadline: float) -> str:
    ...
    """
        ...
        if deadline - monotonic() < _W5G_MIN_TAIL_S or _spend_left() <= WRAPUP_MIN_USD:
            return answer
        body = await _w5_targeted_search(_w5_search_subject(question) + ' full list', ledger)
        detail = ' and hand-waves with: ' + ', '.join(before.hedges[:3]) if before.hedges else ''
        order = 'SET COMPLETENESS: the answer enumerates ' + str(before.members) + ' member lines' + detail + ". A pool reported short scores as WRONG, not as partial. Read the list evidence just retrieved, enumerate the WHOLE pool, and give EVERY member its own line with a verdict — qualifies (with its citation per condition) or excluded because X (with its citation). Replace every 'among others' style phrase with the actual members. If the pool is genuinely too large, rank it, show every member down to a stated cutoff, and state the cutoff. Rewrite the COMPLETE final answer in the required shape."
        revised = await _w5_bounded_rewrite(question, messages, ledger, deadline, order, body)
