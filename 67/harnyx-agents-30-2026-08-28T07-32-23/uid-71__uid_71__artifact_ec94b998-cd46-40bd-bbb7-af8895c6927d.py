...
def _compose_amber_vector_agent_entry():
    ...
    SPIKE_VERSION = "v1-spike-uid87-deepseek-retain"

    LLM_LANE_A = "openrouter"
    LLM_LANE_B = "openrouter"
    LOOP_MODEL_A = "deepseek/deepseek-v4-flash-0731"
    ...
    def _upstream(lane: str, model: str) -> dict | None:
        ...
        if lane != LLM_LANE_A:
            return None
        if model.startswith("deepseek/deepseek-v4-flash-0731"):
...
def _compose_ember_talon_agent_entry():
    # ==========================================================================
    # uid_101 variant v03 - anchor-year alignment + official-source anchoring
    # Base: uid_101.py (unchanged root: _solve / _loop / EvidenceLedger)
    # Stages combined, with the corpus uid each is distilled from:
    #   _temporal_repair     donor uid 53   (also shipped by uid 53, 81, 137)
    ...
    VERSION = "v52-v03-from-uid" + "-".join(['53', '111'])
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            if _is_usable_answer(answer):
                answer = await _temporal_repair(question, answer, messages, ledger, deadline)
        except Exception:
            pass
...
_ember_talon_agent_query_entry = _compose_ember_talon_agent_entry()


def _compose_vesper_lantern_agent_entry():
...
_vesper_lantern_agent_query_entry = _compose_vesper_lantern_agent_entry()


_BALANCED_ROUTER_SEED = "e527da0c58436e56a6d9d682"
...
def _balanced_route_label(query: Query) -> str:
    ...
    if bucket == 0:
        return "AmberVectorAgent"
    if bucket == 1:
        return "EmberTalonAgent"
    return "VesperLanternAgent"
...
class EmberTalonAgent:
    async def __call__(self, query: Query) -> Response:
        return await _ember_talon_agent_query_entry(query)


class VesperLanternAgent:
    async def __call__(self, query: Query) -> Response:
        return await _vesper_lantern_agent_query_entry(query)


_BALANCED_PRIMARY_AGENT = AmberVectorAgent()
_BALANCED_SECONDARY_AGENT = EmberTalonAgent()
_BALANCED_TERTIARY_AGENT = VesperLanternAgent()
