...
def _compose_quartz_orchard_entry():
    ...
    WALL_BUDGET_S = 238.0
    # v-422: boxed so the entrypoint can rebind the budget without `global`.
    _W5_WALL = [WALL_BUDGET_S]
    ...
    VERSION = "v215-uid150x189-rpg"
    # w5 variant 15 — added stages: roster pre-pass, premise-verification sweep, set gap-fill sweep.
    # stage set RPG = uid 150 (RP) x uid 189 (PG)
    ...
    #
    # Batch 4b4eff44 (2026-08-24), artifact 2f1dc330-0ebf-48e2-9b99-1833653888a7, uid 211,
    # 50 replays over 10 tasks. Artifact mean 0.160:
    # structured lane 0.157 over 7 tasks, free-text lane
    # 0.167 over 3 tasks.
    #
    ...
    #     scored replays, and in those it scores 0.167; the 7 replays where the
    #     note is never discussed score 0.214.
    #
    ...
    #     This artifact's median submitted slice is 5398 chars; the answers it
    #     is compared against use a median of 2576 (this batch's 13 well-sampled artifacts,
    #     n=5 readable here). The band below is sized from that. This matters
    ...
    #
    # L3  LATE RUNS RETURNED NOTHING: 243s, 248s. The research
    #     wall is trimmed to 248.0s so the run finishes inside the cap.
    #
...
_quartz_orchard_query_entry = _compose_quartz_orchard_entry()


def _compose_river_notary_entry():
    import asyncio
    from time import monotonic
    ...
    _TAG_86F27AC1="86f27ac1af3a41d4b70d3d297b058999"
    import logging as _tag_logging_86f27ac1
    _tag_logging_86f27ac1.getLogger("miner.tag").debug("tag=%s", _TAG_86F27AC1)

    return query

_river_notary_query_entry = _compose_river_notary_entry()


_BALANCED_ROUTER_SEED = "3a7470098941193134ffdd27"
...
def _balanced_route_label(query: Query) -> str:
    ...
    return "QuartzOrchard" if bucket < 128 else "RiverNotary"


class QuartzOrchard:
    async def __call__(self, query: Query) -> Response:
        return await _quartz_orchard_query_entry(query)


class RiverNotary:
    async def __call__(self, query: Query) -> Response:
        return await _river_notary_query_entry(query)


_BALANCED_PRIMARY_AGENT = QuartzOrchard()
_BALANCED_SECONDARY_AGENT = RiverNotary()
_CANDIDATE_BRANCH_CLASS_NAMES = ("QuartzOrchard", "RiverNotary")
