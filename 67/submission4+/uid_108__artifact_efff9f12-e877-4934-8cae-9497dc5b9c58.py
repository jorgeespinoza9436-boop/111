...
async def _do_search_many(queries, ledger: EvidenceLedger):
    """Concrete tool-use change: parallel multi-query retrieval (sequential ledger writes)."""
...
SPLIT_TIMEOUT_S = 22.0
SPLIT_MAX_PARTS = 2      # 3 concurrent loops saturated the tool host
PART_TURNS = 4
PART_MIN_BUDGET_S = 80.0
COMPOSE_TIMEOUT_S = 45.0
COMPOSE_RESERVE_S = 95.0   # compose + tail + margin under contention

_SPLIT_SYSTEM = (
    "Split a research question into the fewest INDEPENDENT parts that can be researched "
    "separately and then combined. Return JSON only: {\"parts\":[\"...\"]}\n"
    "Each part must be self-contained: name its own entities and period, since the part is "
    "researched with no knowledge of the others. Split only where the parts genuinely do not "
    "depend on each other -- a per-entity comparison splits by entity, a two-period question "
    "splits by period. If the question is one indivisible lookup, or a later part needs an "
    "earlier part's answer, return a single part: the whole question, unchanged. "
    "At most 3 parts. Never invent constraints the question does not state."
)

_COMPOSE_SYSTEM_TAIL = (
    "\n\nYou are composing ONE answer from independent research parts. Each part below was "
    "researched separately and its [n] markers have already been renumbered into a single "
    "shared evidence list, so cite them exactly as written. Parts may overlap or disagree: "
    "prefer the part whose citation states the fact most directly, and if two parts conflict, "
    "say which source you followed and why. Cover every part -- an answer that silently drops "
    "one is incomplete. Do not mention the parts, the split, or this process; write the single "
...
async def _split_question(question: str, deadline: float) -> list[str]:
    """One structured call. Falls back to the whole question, which degrades this
    controller to a single ordinary research loop."""
    whole = [question]
    if (deadline - monotonic()) < PART_MIN_BUDGET_S + 30.0:
        return whole
    try:
        raw = await _chat_simple(LLM_LANE_A, LOOP_MODEL_A, _SPLIT_SYSTEM, question,
                                 max_tokens=600, timeout=SPLIT_TIMEOUT_S)
    except Exception:
        return whole
    ...
    try:
        ...
        return whole
    parts = []
    for item in (data.get("parts") or [])[:SPLIT_MAX_PARTS]:
...
def _merge_part(part_answer: str, part_ledger: "EvidenceLedger",
                global_ledger: "EvidenceLedger") -> str:
    """Append one part's rows to the global ledger and rewrite its [n] markers.

    Called sequentially in part order after all parts have finished, so the
    global numbering does not depend on which part returned first."""
    offset = len(global_ledger.rows)
    for row in part_ledger.rows:
        global_ledger.rows.append(dict(row))

    def shift(match) -> str:
        number = int(match.group(1))
        if 1 <= number <= len(part_ledger.rows):
            return "[%d]" % (number + offset)
        return ""          # a marker pointing outside its own ledger is not citable

    return _MARKER_RE.sub(shift, part_answer or "")


async def _solve_part(part: str, brief: str, deadline: float) -> tuple[str, "EvidenceLedger"]:
    ledger = EvidenceLedger()
    try:
        answer, _msgs = await _loop(part, brief, ledger, deadline, PART_TURNS)
    except Exception:
        answer = ""
    return answer, ledger


async def _compose(question: str, parts: list[str], answers: list[str],
                   ledger: EvidenceLedger, deadline: float) -> str:
    body = []
    for part, answer in zip(parts, answers):
        if answer.strip():
            body.append("PART: %s\nFINDING: %s" % (part[:300], answer.strip()[:6000]))
    if not body:
        return ""
    try:
        raw = await _chat_simple(
            LLM_LANE_A, LOOP_MODEL_A,
            LOOP_RULES + _COMPOSE_SYSTEM_TAIL,
            "QUESTION: %s\n\n%s" % (question, "\n\n".join(body)),
            max_tokens=2600,
            timeout=max(12.0, min(COMPOSE_TIMEOUT_S, (deadline - monotonic()) - DIGEST_TAIL_S)),
        )
    except Exception:
        return ""
    return (raw or "").strip()
...
async def _parallel_research(question: str, brief: str, ledger: EvidenceLedger,
                             deadline: float) -> tuple[str, list[str]]:
    """Returns (answer, part_answers). Degrades to a single loop on one part."""
    parts = await _split_question(question, deadline)
    if len(parts) < 2:
        answer, _msgs = await _loop(question, brief, ledger, deadline, MAX_TURNS)
        return answer, [answer]

    # Parts run CONCURRENTLY and share one tool host, so their effective latency is
    # far worse than a solo loop's: the first cut of this design let parts run to
    # 110s each and three tasks finished at 257s of the 262s wall, dropping onto
    # the rescue rung and scoring 0. Cap hard and leave compose untouchable.
    part_deadline = min(deadline - COMPOSE_RESERVE_S, monotonic() + PART_MIN_BUDGET_S)
    results = await asyncio.gather(
        *[_solve_part(p, brief, part_deadline) for p in parts],
        return_exceptions=True,
    )
    merged: list[str] = []
    for index, outcome in enumerate(results):          # part order, not arrival order
        if isinstance(outcome, BaseException) or not isinstance(outcome, tuple):
            merged.append("")
            continue
        part_answer, part_ledger = outcome
        merged.append(_merge_part(part_answer, part_ledger, ledger))

    if not any(m.strip() for m in merged):
        answer, _msgs = await _loop(question, brief, ledger, deadline, MAX_TURNS)
        return answer, [answer]

    composed = await _compose(question, parts, merged, ledger, deadline)
    if not _is_usable_answer(composed):
        best = max(merged, key=lambda m: len(m or ""))
        return best, merged
...
RESEARCH_RESERVE_S = 72.0     # commit (<=46s) + digest/schema tail (14s) + 12s margin.
...
async def _solve(query: Query, question: str) -> Response:
    ...
    try:
        ...
        if _is_usable_answer(answer or "") and (deadline - monotonic()) > 35:
            try:
                _v3_qs: list[str] = []
                ...
                _v3_qs = _deduped[:6]
                if _v3_qs:
                    _v3_blob = await _do_search_many(_v3_qs, ledger)
                    messages.append({
                        "role": "system",
                        "content": (
