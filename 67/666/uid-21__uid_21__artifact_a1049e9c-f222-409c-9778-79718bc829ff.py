...
# ── V07  CONSTRAINT ELIMINATION GRID ──────────────────────────────────────
# Controller: two branches concurrently supply the candidate universe, then
#             the constraints are applied deterministically.
# Evidence:   candidate x constraint verdict grid (_CxConstraintGrid).
# Authority:  the GRID decides who survives; the model only formats.
# Not the engine's roster/gap-fill stages: those grow a pool toward
# completeness. This SHRINKS a pool by literal constraint arithmetic and
# ships an elimination record.
...
_V07_EXTRACT_SYSTEM = (
    "You read a research question and draft answers and extract the decision "
    'structure. Return JSON only: {"constraints": ["<one testable '
    'condition>", ...], "candidates": [{"name": "<candidate>", "evidence": '
    '"<what the drafts say about it>"}, ...]}. Constraints are literal: '
    "'more than 25' means strictly greater than 25. Include candidates the "
    "drafts RULE OUT as well as those they keep. At most five constraints "
    "and twelve candidates. Never invent either."
)

_V07_VERDICT_SYSTEM = (
    "You test one candidate against one constraint using only the evidence "
    'given. Return JSON only: {"verdict": "pass"|"fail"|"unknown"}. Answer '
    "unknown when the evidence does not settle it. Never guess."
)

_V07_FORMAT_SYSTEM = (
    "You format an already-decided result. You are given SURVIVORS and an "
    "ELIMINATION RECORD. Do not re-decide anything - the survivors are the "
    "answer. Sentence one names them exactly. Then give ONE LINE PER "
    "ELIMINATED CANDIDATE with the constraint it failed. Keep every [[n]] "
    "pointer that appears in the evidence. Add no new facts."
)


class _CxConstraintGrid:
    """(candidate, constraint) -> pass | fail | unknown."""
    ...
    def note(self, candidate, text):
        self.evidence[candidate] = text[:1100]
...
async def _v07_cell(grid, candidate, constraint, budget):
    raw = await _cx_chat(
        _V07_VERDICT_SYSTEM,
        "CANDIDATE: " + candidate + "\nCONSTRAINT: " + constraint
        + "\nEVIDENCE:\n" + grid.evidence.get(candidate, "")[:1600],
        _CX_FAST, min(9.0, budget), 200, 0.0)
...
async def _v07_run(query):
    ...
    order = _cx_order_for(query, "v07-constraint-grid")
    panel = await _cx_panel(query, order, 2, _V07_MECH_S, t0)
    if not panel:
        return _cx_response(None, None, [])

    best = _cx_best(query, panel)
    if getattr(query, "output_schema", None) is not None:
        return best[1]
    if _cx_left(t0, _CX_WALL_S) < 30.0:
        return best[1]

    blocks = []
    for pair in panel:
        blocks.append(_cx_text_of(pair[1])[:14000])
    raw = await _cx_chat(
        _V07_EXTRACT_SYSTEM,
        "QUESTION:\n" + question[:2500] + "\n\nDRAFTS:\n"
        + "\n\n---\n\n".join(blocks),
        _CX_FAST, min(16.0, _cx_left(t0, _CX_WALL_S) - 18.0), 1400, 0.0)
    parsed = _cx_json_of(raw)
    if parsed is None:
        return best[1]
    ...
    for name in names:
        for constraint in constraints:
            jobs.append(_v07_cell(grid, name, constraint, budget))
    await _cx_asyncio.gather(*jobs, return_exceptions=True)
    ...
    for pair in panel:
        for ref in _cx_cites_of(pair[1]):
            _cx_merge(citations, ref)
    formatted = await _cx_chat(
        _V07_FORMAT_SYSTEM,
        "QUESTION:\n" + question[:2500]
        + "\n\nSURVIVORS:\n" + grid.render_survivors()
        + "\n\nELIMINATION RECORD:\n- " + "\n- ".join(grid.eliminations())
        + "\n\nSOURCE PROSE (for pointer numbering):\n"
        + _cx_text_of(best[1])[:14000],
        _CX_MODEL, min(20.0, max(6.0, _cx_left(t0, _CX_WALL_S) - 3.0)),
        2800, 0.15)
    if len(formatted) < 60:
        return best[1]
