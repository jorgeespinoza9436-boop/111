...
def _compose_ember_lantern_agent_entry():
    ...
    def _compose_cobalt_kestrel_agent_entry():
        ...
        # ----------------------------------------------------------------
        # v115-503-rsv
        # Stages: roster pre-pass, set gap-fill, value repair
        # Ordinary successful path:
        #   query -> _solve -> _knowledge_brief -> _draft_candidate_pool -> _loop -> _audit_patch -> _select_best -> _widen_pool -> _ground_figures -> _citations_for -> _w2_point_markers -> _answer_line_only -> Response
        # ----------------------------------------------------------------
    ...
    def _compose_cedar_harbor_agent_entry():
        ...
        # ----------------------------------------------------------------
        # v115-508-kps
        # Stages: coverage nudge, premise sweep, set gap-fill
        # Ordinary successful path:
        #   query -> _solve -> _knowledge_brief -> _loop (+_open_criteria_hint at midpoint) -> _audit_patch -> _select_best -> _verify_subjects -> _widen_pool -> _citations_for -> _w2_point_markers -> _answer_line_only -> Response
        # ----------------------------------------------------------------
        ...
        _CRITERION_ROW_RE = re.compile(
            r"(?:^|[;\n]|\bthat\b|\bwhich\b|\bwith\b|\band\b)\s*"
            r"([a-z][a-z0-9 ,'\-]{8,90})", re.I)
        MAX_CRITERIA = 5
    ...
    def _compose_ivory_beacon_agent_entry():
        ...
        _AC_MAX_REQUIRED = 6
        _AC_DRAFT_PROMPT_CHARS = 6_000

        _AC_FIGURE_RE = re.compile(r"\d+(?:[.,]\d+)*")
        _AC_WORD_RE = re.compile(r"[A-Z][A-Za-z0-9&'’.\-]*")
        ...
        def _ac_json_object(text: str) -> dict | None:
            if not text or not text.strip():
                return None
            raw = re.sub(r"^```(?:json)?\s*|\s*```$", "", text.strip(), flags=re.I | re.M)
            try:
                obj = json.loads(raw)
        ...
        async def _ac_build_contract(question: str, *, deadline: float) -> _AnswerContract | None:
            ...
            try:
                raw = await _chat_simple(
                    LLM_LANE_A, LOOP_MODEL_A, _AC_PLAN_SYSTEM,
                    f"Question:\n{question}",
            ...
            if payload is None:
                return None
            deliverable = payload.get("deliverable")
            contract = _AnswerContract(
                deliverable=deliverable.strip() if isinstance(deliverable, str) else "",
                required=_ac_string_list(payload.get("required"), _AC_MAX_REQUIRED),
                pitfalls=_ac_string_list(payload.get("pitfalls"), 3),
            )
            return contract if contract.is_actionable() else None


        async def _ac_verify(
            contract: _AnswerContract, question: str, draft: str, *, deadline: float,
        ) -> str:
            ...
            try:
                revision = await _chat_simple(
                    LLM_LANE_A, LOOP_MODEL_A, _AC_VERIFY_SYSTEM, user,
                    max_tokens=3400, timeout=timeout,
                    think=_least_think(LLM_LANE_A, LOOP_MODEL_A),
                )
            except Exception:
                return draft
    ...
    _ivory_beacon_agent_query_entry = _compose_ivory_beacon_agent_entry()


    _BALANCED_ROUTER_SEED = "2f88058eb27c67a48576e248"
    ...
    def _balanced_route_label(query: Query) -> str:
        ...
        if bucket == 0:
            return "CobaltKestrelAgent"
        if bucket == 1:
            return "CedarHarborAgent"
        return "IvoryBeaconAgent"
    ...
    class IvoryBeaconAgent:
        async def __call__(self, query: Query) -> Response:
            return await _ivory_beacon_agent_query_entry(query)


    _BALANCED_PRIMARY_AGENT = CobaltKestrelAgent()
    _BALANCED_SECONDARY_AGENT = CedarHarborAgent()
    _BALANCED_TERTIARY_AGENT = IvoryBeaconAgent()
    ...
    async def query(query: Query) -> Response:
        ...
        elif selected == "CedarHarborAgent":
...
_SHAPE_ROUTER_SEED = "e9e1fcc4e5bf146295b07380"
...
def _balanced_route_label(query: Query) -> str:
    ...
    order = ("CedarLatticeAgent", "EmberLanternAgent", "SaffronLatticeAgent")
    if shape == 3:
        return order[bucket]
...
_SHAPE_PRIMARY_AGENT = CedarLatticeAgent()
_SHAPE_SECONDARY_AGENT = EmberLanternAgent()
