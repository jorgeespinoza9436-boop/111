...
_opal_cairn_query_entry = _compose_opal_cairn_entry()


def _compose_parchment_tide_entry():
...
_parchment_tide_query_entry = _compose_parchment_tide_entry()


_BALANCED_ROUTER_SEED = "bcbf01b7caf434837dd9dca2"
...
def _balanced_route_label(query: Query) -> str:
    ...
    return "OpalCairn" if bucket < 128 else "ParchmentTide"


class OpalCairn:
    async def __call__(self, query: Query) -> Response:
        return await _opal_cairn_query_entry(query)


class ParchmentTide:
    async def __call__(self, query: Query) -> Response:
        return await _parchment_tide_query_entry(query)


_BALANCED_PRIMARY_AGENT = OpalCairn()
_BALANCED_SECONDARY_AGENT = ParchmentTide()
_CANDIDATE_BRANCH_CLASS_NAMES = ("OpalCairn", "ParchmentTide")
