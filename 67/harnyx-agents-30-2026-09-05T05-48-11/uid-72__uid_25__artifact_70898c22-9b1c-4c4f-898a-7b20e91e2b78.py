...
# fork of 11_uid_49_score_0.750.py: added cov:document:steer  (full mechanism set: COV+DRV)

from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import Query, Response


def _compose_willow_quill_slot12_agent_entry():
    ...
    # --- element-coverage controller [u49-cov-document-steer: named-document coverage, mid-loop steer + finish-gate re-entry] --------
...
_willow_quill_slot12_agent_query_entry = _compose_willow_quill_slot12_agent_entry()


def _compose_fjord_marten_slot12_agent_entry():
...
_fjord_marten_slot12_agent_query_entry = _compose_fjord_marten_slot12_agent_entry()


def _compose_zephyr_willow_slot12_agent_entry():
...
_zephyr_willow_slot12_agent_query_entry = _compose_zephyr_willow_slot12_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "WillowQuillSlot12Agent"
    ...
    if (
        ...
        return "FjordMartenSlot12Agent"
    return "ZephyrWillowSlot12Agent"


class WillowQuillSlot12Agent:
    async def __call__(self, query: Query) -> Response:
        return await _willow_quill_slot12_agent_query_entry(query)


class FjordMartenSlot12Agent:
    async def __call__(self, query: Query) -> Response:
        return await _fjord_marten_slot12_agent_query_entry(query)


class ZephyrWillowSlot12Agent:
    async def __call__(self, query: Query) -> Response:
        return await _zephyr_willow_slot12_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = WillowQuillSlot12Agent()
_ANALYTICAL_FIELD_AGENT = FjordMartenSlot12Agent()
_BROAD_FIELD_AGENT = ZephyrWillowSlot12Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "WillowQuillSlot12Agent",
    "FjordMartenSlot12Agent",
    "ZephyrWillowSlot12Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "WillowQuillSlot12Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "FjordMartenSlot12Agent":
