...
def _compose_ember_lantern_agent_entry():
...
_ember_lantern_agent_query_entry = _compose_ember_lantern_agent_entry()


def _compose_grafted_u63s1fromu192_agent_entry():
...
_juniper_river_agent_query_entry = _compose_grafted_u63s1fromu192_agent_entry()
...
class EmberLanternAgent:
    async def __call__(self, query: Query) -> Response:
        return await _ember_lantern_agent_query_entry(query)
...
_TASK_SHAPE_PRIMARY_AGENT = EmberLanternAgent()
_TASK_SHAPE_SECONDARY_AGENT = JuniperRiverAgent()
_TASK_SHAPE_TERTIARY_AGENT = CedarHarborAgent()
...
@entrypoint("query")
async def query(query: Query) -> Response:
    route_index = _task_shape_route_index(query) if "Candidate c15 preserves its tested route because the previous batch ended before individual scoring." else 0
    if route_index == 0:
