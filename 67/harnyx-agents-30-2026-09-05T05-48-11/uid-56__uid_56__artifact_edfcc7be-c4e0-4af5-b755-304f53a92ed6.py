...
_qrbrnijxjr = {'started': None, 'text': None}
_cfvuozbnic = 24000
_llkybvhozh = 290.0
_fmrbzjcvzi = 250.0

def _zbgnqfjthj() -> float:
    started = _qrbrnijxjr['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _leeiwdiocr() -> float:
    return _llkybvhozh - _zbgnqfjthj()
_uemfeyjgrs = _hsapi.llm_chat
_tyqpdypxpo = _hsapi.search_web
_lmnzwjcymw = _hsapi.fetch_page
_aqcizgaftl = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _xomyahwreg(*args, **kwargs):
    if _zbgnqfjthj() >= _fmrbzjcvzi:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _aqcizgaftl})
            kwargs['messages'] = steered
    _result = await _uemfeyjgrs(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _ithemjbtks(_result)
    return _result

async def _cotxwagdsr(*args, **kwargs):
    if _zbgnqfjthj() >= _fmrbzjcvzi:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _tyqpdypxpo(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _lowpskdvlw(*args, **kwargs):
    if _zbgnqfjthj() >= _fmrbzjcvzi:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _lmnzwjcymw(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _xomyahwreg
_hsapi.search_web = _cotxwagdsr
_hsapi.fetch_page = _lowpskdvlw
_arpdznkyby = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_blawskqowh = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_mmbgopbwoq = 900
_rjfggqafwv = 2

def _hgpbcmtmzc(query: Query) -> int:
...
def _fecsoffafr(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _mlttrrybpj(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _hgpbcmtmzc(query)
    if fields >= 3:
        return 2
    if _fecsoffafr(lowered, _arpdznkyby):
        return 1
    if fields <= _rjfggqafwv and len(text) <= _mmbgopbwoq:
        return 0
    if _fecsoffafr(lowered, _blawskqowh):
        return 0
    return 1

def _ithemjbtks(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _qrbrnijxjr['text'] = text.strip()[:_cfvuozbnic]
    except Exception:
        pass

def _zqicwwmerb(text: str):
...
def _szawdfkmeq(query: Query) -> Response:
    text = _qrbrnijxjr['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_cfvuozbnic]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _zqicwwmerb(text)
...
"""
    ...
    return query
_qmxpdsavko = _xnixiqywyo()
_veaupmtrat = _ejgrytrxap()
_pvknvrszyv = _ucgfcyhnns()
_xiszulzisw = _rfetaambou()
_llkybvhozh = 290.0
_sqorbgtkeh = 250.0
_zeoudokzxc = 90.0

async def _jcnnpqynza(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _llkybvhozh - (time.monotonic() - started)
        if first:
            budget = _sqorbgtkeh if _sqorbgtkeh < remaining else remaining
            first = False
        else:
            if remaining < _zeoudokzxc:
                break
        ...
        try:
            ...
            last_exc = exc
    return _szawdfkmeq(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _qrbrnijxjr['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _jcnnpqynza(query, (_pvknvrszyv, _qmxpdsavko, _veaupmtrat, _xiszulzisw))
        index = _mlttrrybpj(query)
        if index == 0:
            agents = (_qmxpdsavko, _veaupmtrat, _pvknvrszyv, _xiszulzisw)
        elif index == 1:
            agents = (_veaupmtrat, _pvknvrszyv, _xiszulzisw, _qmxpdsavko)
        elif index == 2:
            agents = (_pvknvrszyv, _xiszulzisw, _qmxpdsavko, _veaupmtrat)
        elif index == 3:
            agents = (_xiszulzisw, _qmxpdsavko, _veaupmtrat, _pvknvrszyv)
        else:
            agents = (_qmxpdsavko, _veaupmtrat, _pvknvrszyv, _xiszulzisw)
        return await _jcnnpqynza(query, agents)
    except Exception:
        return _szawdfkmeq(query)
_BUILDW_TAG_m29 = "sn45-0b2fd8ec6006"
