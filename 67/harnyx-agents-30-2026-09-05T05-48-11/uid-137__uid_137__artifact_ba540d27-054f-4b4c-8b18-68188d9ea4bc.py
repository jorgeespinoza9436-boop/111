...
# fork of 10_uid_183_score_0.669.py: added cov:quantitative:steer (variant u183)

from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import Query, Response


def _compose_marble_mosaic_slot13_agent_entry():
    ...
    # --- element-coverage controller [u183-cov-quantitative-steer: quantitative-figure coverage, mid-loop steer + finish-gate re-entry] --------
...
_marble_mosaic_slot13_agent_query_entry = _compose_marble_mosaic_slot13_agent_entry()


def _compose_xenon_atlas_slot13_agent_entry():
...
_xenon_atlas_slot13_agent_query_entry = _compose_xenon_atlas_slot13_agent_entry()


def _compose_marble_relay_slot13_agent_entry():
    ...
    # --- drv wrap: claim-conflict ledger (start) ---
    # batch_tag='drv000' salt='62463ecba0e3'
    # Ordinary-path architecture added relative to the baseline agent:
    #   baseline research -> draft answer
    #   -> claim-conflict ledger audit (required elements, unsupported claims,
    #      comparison/period-basis gaps, official-vs-independent conflict,
...
_marble_relay_slot13_agent_query_entry = _compose_marble_relay_slot13_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "MarbleMosaicSlot13Agent"
    ...
    if (
        ...
        return "XenonAtlasSlot13Agent"
    return "MarbleRelaySlot13Agent"


class MarbleMosaicSlot13Agent:
    async def __call__(self, query: Query) -> Response:
        return await _marble_mosaic_slot13_agent_query_entry(query)


class XenonAtlasSlot13Agent:
    async def __call__(self, query: Query) -> Response:
        return await _xenon_atlas_slot13_agent_query_entry(query)


class MarbleRelaySlot13Agent:
    async def __call__(self, query: Query) -> Response:
        return await _marble_relay_slot13_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = MarbleMosaicSlot13Agent()
_ANALYTICAL_FIELD_AGENT = XenonAtlasSlot13Agent()
_BROAD_FIELD_AGENT = MarbleRelaySlot13Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "MarbleMosaicSlot13Agent",
    "XenonAtlasSlot13Agent",
    "MarbleRelaySlot13Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "MarbleMosaicSlot13Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "XenonAtlasSlot13Agent":
