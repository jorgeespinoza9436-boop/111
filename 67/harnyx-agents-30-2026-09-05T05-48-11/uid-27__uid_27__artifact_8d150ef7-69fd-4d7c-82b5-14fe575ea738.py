...
_rpigjjsegd = {'started': None, 'text': None}
_wqfsjrpnzs = 24000
_cwsexzqpgs = 290.0
_wjhorbfjpa = 250.0

def _dmxsgjwceh() -> float:
    started = _rpigjjsegd['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _gmgyasflco() -> float:
    return _cwsexzqpgs - _dmxsgjwceh()
_qlmgyrojwp = _hsapi.llm_chat
_bmvmypksxd = _hsapi.search_web
_qdibjdtggt = _hsapi.fetch_page
_edmusmtirs = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _udjglcwliq(*args, **kwargs):
    if _dmxsgjwceh() >= _wjhorbfjpa:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _edmusmtirs})
            kwargs['messages'] = steered
    _result = await _qlmgyrojwp(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _nrqvushsed(_result)
    return _result

async def _mcsxchavur(*args, **kwargs):
    if _dmxsgjwceh() >= _wjhorbfjpa:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _bmvmypksxd(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _leklrqlren(*args, **kwargs):
    if _dmxsgjwceh() >= _wjhorbfjpa:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _qdibjdtggt(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _udjglcwliq
_hsapi.search_web = _mcsxchavur
_hsapi.fetch_page = _leklrqlren
_bqpcsjeomf = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_ddurumwrvw = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_vjgolvkdto = 900
_kihtyfykxl = 2

def _otfcqwsbye(query: Query) -> int:
...
def _crawtitorq(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _wlarpmfovj(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _otfcqwsbye(query)
    if fields >= 3:
        return 2
    if _crawtitorq(lowered, _bqpcsjeomf):
        return 1
    if fields <= _kihtyfykxl and len(text) <= _vjgolvkdto:
        return 0
    if _crawtitorq(lowered, _ddurumwrvw):
        return 0
    return 1

def _nrqvushsed(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _rpigjjsegd['text'] = text.strip()[:_wqfsjrpnzs]
    except Exception:
        pass

def _diodfhdsch(text: str):
...
def _trmccdqhwc(query: Query) -> Response:
    text = _rpigjjsegd['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_wqfsjrpnzs]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _diodfhdsch(text)
...
def _wdxrjfeikp():
    ...
    """
        ...
        return response
    return query
_rfspeknamz = _rizghdsbwu()
_ufnrsectco = _bxrseafene()
_izgzmkkddp = _idezdeetjh()
_ndsdzvheew = _wdxrjfeikp()
_cwsexzqpgs = 290.0
_zwsevjqctq = 250.0
_dhvkezugda = 90.0

async def _tcmvftxqrm(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _cwsexzqpgs - (time.monotonic() - started)
        if first:
            budget = _zwsevjqctq if _zwsevjqctq < remaining else remaining
            first = False
        else:
            if remaining < _dhvkezugda:
                break
        ...
        try:
            ...
            last_exc = exc
    return _trmccdqhwc(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _rpigjjsegd['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _tcmvftxqrm(query, (_izgzmkkddp, _rfspeknamz, _ufnrsectco, _ndsdzvheew))
        index = _wlarpmfovj(query)
        if index == 0:
            agents = (_rfspeknamz, _ufnrsectco, _izgzmkkddp, _ndsdzvheew)
        elif index == 1:
            agents = (_ufnrsectco, _izgzmkkddp, _ndsdzvheew, _rfspeknamz)
        elif index == 2:
            agents = (_izgzmkkddp, _ndsdzvheew, _rfspeknamz, _ufnrsectco)
        elif index == 3:
            agents = (_ndsdzvheew, _rfspeknamz, _ufnrsectco, _izgzmkkddp)
        else:
            agents = (_rfspeknamz, _ufnrsectco, _izgzmkkddp, _ndsdzvheew)
        return await _tcmvftxqrm(query, agents)
    except Exception:
        return _trmccdqhwc(query)
_BUILDW_TAG_m1 = "sn45-079d991172b5"
