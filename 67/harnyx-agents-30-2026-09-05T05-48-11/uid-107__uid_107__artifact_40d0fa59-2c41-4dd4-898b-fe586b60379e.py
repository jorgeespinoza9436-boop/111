...
# fork of 9_uid_211_score_0.675.py: added drv:comparison_member:board_or_shape (variant u211)

from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import Query, Response


def _compose_ivory_slate_slot14_agent_entry():
...
_ivory_slate_slot14_agent_query_entry = _compose_ivory_slate_slot14_agent_entry()


def _compose_zephyr_quill_slot14_agent_entry():
...
_zephyr_quill_slot14_agent_query_entry = _compose_zephyr_quill_slot14_agent_entry()


def _compose_yarrow_compass_slot14_agent_entry():
...
_yarrow_compass_slot14_agent_query_entry = _compose_yarrow_compass_slot14_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "IvorySlateSlot14Agent"
    ...
    if (
        ...
        return "ZephyrQuillSlot14Agent"
    return "YarrowCompassSlot14Agent"


class IvorySlateSlot14Agent:
    async def __call__(self, query: Query) -> Response:
        return await _ivory_slate_slot14_agent_query_entry(query)


class ZephyrQuillSlot14Agent:
    async def __call__(self, query: Query) -> Response:
        return await _zephyr_quill_slot14_agent_query_entry(query)


class YarrowCompassSlot14Agent:
    async def __call__(self, query: Query) -> Response:
        return await _yarrow_compass_slot14_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = IvorySlateSlot14Agent()
_ANALYTICAL_FIELD_AGENT = ZephyrQuillSlot14Agent()
_BROAD_FIELD_AGENT = YarrowCompassSlot14Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "IvorySlateSlot14Agent",
    "ZephyrQuillSlot14Agent",
    "YarrowCompassSlot14Agent",
)
...
async def _drv_base_query(query: Query) -> Response:
    ...
    if selected == "IvorySlateSlot14Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "ZephyrQuillSlot14Agent":
        branch = _ANALYTICAL_FIELD_AGENT
    else:
        branch = _BROAD_FIELD_AGENT
    return await branch(query)

# --- drv wrap: comparison_member dual-corpus reconciler (start) ---
# variant u211-drv-comparison_member-board_or_shape: per-member comparison corpus; trigger = board deficiency OR synthesis/set question shape
...
_DRV_TAG = 'u211-drv-comparison_member-board_or_shape'
_DRV_SALT = '6a44ae680058'
...
async def _drv_audit_board(question: str, draft: str, schema, citations: list) -> _DrvBoard:
    ...
    system = (
        "You audit a research draft against a user question. Focus first on comparison members: whether EVERY member of the comparison or set has its own sourced value. "
...
async def _drv_retrieve_dual_corpus(question: str, claims: list[str]) -> list[dict]:
    """Per-member retrieval: one focused search per open claim/member, fetch the first."""
    members = [c for c in (claims or []) if c][:3]
    if not members:
        members = [question[:180]]
    rows: list[dict] = []
    fetch_url = ""
    for k, member in enumerate(members):
        member_q = " ".join((member[:140], question[:100], "value status record")).strip()[:280]
        payload = await _drv_search(member_q)
        row = _drv_row_from_payload(payload, True) if payload else None
        if row:
            row["corpus"] = "member_" + str(k + 1)
            rows.append(row)
            if not fetch_url:
    ...
    if fetch_url:
        ...
        if fetched_row:
            fetched_row["corpus"] = "member_document"
...
async def _drv_regenerate(
    ...
    if is_text:
        system = (
            "Rewrite the research answer after a second retrieval pass over one source per comparison member, with one cited line per member. Return JSON only with keys "
        ...
        system = (
            "Rewrite the structured research answer after a second retrieval pass over one source per comparison member, with one cited line per member. Return JSON only "
...
async def query(query: _DrvQuery) -> _DrvResponse:
    ...
    try:
        ...
        return draft
# --- drv wrap: comparison_member dual-corpus reconciler (end) ---
