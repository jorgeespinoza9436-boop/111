...
# fork of 5_uid_120_score_0.750.py: added corr:independent  (full mechanism set: CORR+COV+K2)
...
def _compose_ivory_talon_agent_entry():
    ...
    VERIFY_SUBJECTS_MIN_LEFT_S = 99.7
...
def _compose_ember_river_agent_entry():
...
_ember_river_agent_query_entry = _compose_ember_river_agent_entry()
...
async def _corr_base_query(query: Query) -> Response:
...
# --- corr wrap: lead-figure independence corroboration (start) [variant u120-corr-independent] ---
...
_CORR_TAG = "u120-corr-independent"
...
async def _corr_cycle(query, response):
    ...
    for figure in figures:
        ...
        if len(domains) < _CORR_MIN_DOMAINS:
            ...
            payload2 = await _corr_search(f"{figure} {terms} independent report")
