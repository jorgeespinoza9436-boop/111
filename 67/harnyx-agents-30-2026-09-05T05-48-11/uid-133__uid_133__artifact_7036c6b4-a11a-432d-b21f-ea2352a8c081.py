...
_vlgaznjjqz = {'started': None, 'text': None}
_qmvdkitmfs = 24000
_xmtcshdxkp = 290.0
_zpqerrkqva = 250.0

def _dbjdpavzrv() -> float:
    started = _vlgaznjjqz['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _dfjthbravq() -> float:
    return _xmtcshdxkp - _dbjdpavzrv()
_lwnpkfpvwo = _hsapi.llm_chat
_ciwipxmwte = _hsapi.search_web
_ombvysdbrl = _hsapi.fetch_page
...
async def _kzlwynmyxn(*args, **kwargs):
    if _dbjdpavzrv() >= _zpqerrkqva:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _wosizevrhf})
            kwargs['messages'] = steered
    _result = await _lwnpkfpvwo(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _piwwdxxhqq(_result)
    return _result

async def _kamiootebk(*args, **kwargs):
    if _dbjdpavzrv() >= _zpqerrkqva:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _ciwipxmwte(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _xqcnoshlbk(*args, **kwargs):
    if _dbjdpavzrv() >= _zpqerrkqva:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _ombvysdbrl(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _kzlwynmyxn
_hsapi.search_web = _kamiootebk
_hsapi.fetch_page = _xqcnoshlbk
_kstpzbkqzn = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_mofjjmyfdb = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_mzgzqvabbt = 900
_zeajguuxqv = 2

def _ohtahqafpi(query: Query) -> int:
...
def _xaidlxposc(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _mnuifxvunv(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _ohtahqafpi(query)
    if fields >= 3:
        return 2
    if _xaidlxposc(lowered, _kstpzbkqzn):
        return 1
    if fields <= _zeajguuxqv and len(text) <= _mzgzqvabbt:
        return 0
    if _xaidlxposc(lowered, _mofjjmyfdb):
        return 0
    return 1

def _piwwdxxhqq(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _vlgaznjjqz['text'] = text.strip()[:_qmvdkitmfs]
    except Exception:
        pass

def _pdshzpcuig(text: str):
...
def _tveskdlspf(query: Query) -> Response:
    text = _vlgaznjjqz['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_qmvdkitmfs]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _pdshzpcuig(text)
...
def _iqixffmhdc():
    ...
    async def _drv_base_query(query: Query) -> Response:
        ...
        return response
    VERSION = 'c4-422'
    _GX_ACTIVE = ('super', 'entity', 'window')
...
def _mxcwffgvrf():
    """uid_195 research agent - variant 05.
...
"""
    ...
    return query
_pozdhiqkyh = _iqixffmhdc()
_fwkznowusp = _bahnxxoizl()
_klpjqmeyyp = _htzafaecqb()
_gtyddvmblm = _mxcwffgvrf()
_xmtcshdxkp = 290.0
_juaiwseyob = 250.0
_sklrmpezjf = 90.0

async def _uxotbbdsqc(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _xmtcshdxkp - (time.monotonic() - started)
        if first:
            budget = _juaiwseyob if _juaiwseyob < remaining else remaining
            first = False
        else:
            if remaining < _sklrmpezjf:
                break
        ...
        try:
            ...
            last_exc = exc
    return _tveskdlspf(query)
_meopznuznd = 235.0

async def _pxazwxmytw(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _ndnourlqrf(query, agents):
    if len(agents) < 2:
        return await _uxotbbdsqc(query, agents)
    r0, r1 = await asyncio.gather(_pxazwxmytw(agents[0], query, _meopznuznd), _pxazwxmytw(agents[1], query, _meopznuznd))
    ok0 = isinstance(r0, Response) and _lqkpvyzgrw(r0) != ''
    ok1 = isinstance(r1, Response) and _lqkpvyzgrw(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_lwnpkfpvwo(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _vlgaznjjqz['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _uxotbbdsqc(query, (_klpjqmeyyp, _pozdhiqkyh, _fwkznowusp, _gtyddvmblm))
        index = _mnuifxvunv(query)
        if index == 0:
            agents = (_pozdhiqkyh, _fwkznowusp, _klpjqmeyyp, _gtyddvmblm)
        elif index == 1:
            agents = (_fwkznowusp, _klpjqmeyyp, _gtyddvmblm, _pozdhiqkyh)
        elif index == 2:
            agents = (_klpjqmeyyp, _gtyddvmblm, _pozdhiqkyh, _fwkznowusp)
        elif index == 3:
            agents = (_gtyddvmblm, _pozdhiqkyh, _fwkznowusp, _klpjqmeyyp)
        else:
            agents = (_pozdhiqkyh, _fwkznowusp, _klpjqmeyyp, _gtyddvmblm)
        return await _ndnourlqrf(query, agents)
    except Exception:
        return _tveskdlspf(query)
_BUILDW_TAG_m15 = "sn45-8b908ef8e259"
