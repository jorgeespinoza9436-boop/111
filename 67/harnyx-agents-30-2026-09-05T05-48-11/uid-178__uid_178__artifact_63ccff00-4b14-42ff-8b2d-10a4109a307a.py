...
# fork of 2_uid_73_score_0.700.py: added cov:comparison:finish (variant u73)
...
def _compose_lumen_anvil_agent_entry():
    ...
    # --- element-coverage controller [u73-cov-comparison-finish: comparison-set coverage, finish-gate re-entry] --------
    ...
    # evidence was gathered. Here the question's required elements (comparison) are
    ...
    # --- required elements (comparison): the controller's completion criterion -------
    _ELEMTOK_STOP = (
    ...
    _CMP_SPLIT_RE = re.compile(r"(?i)\s+(?:vs\.?|versus|compared (?:to|with)|against|or|and)\s+")
    _CMP_CAP_RE = re.compile(r"\b[A-Z][A-Za-z0-9&.\-]{1,}(?:\s+[A-Z][A-Za-z0-9&.\-]{1,}){0,3}\b")
    def _seed_elements(question: str) -> list[str]:
        out: list[str] = []
        for chunk in _CMP_SPLIT_RE.split(question or ""):
            caps = [c for c in _CMP_CAP_RE.findall(chunk) if len(c) >= 4]
            if not caps:
                continue
            phrase = "value for member " + caps[-1]
    ...
    def _element_hard_keys(element: str) -> list[str]:
        """Tokens that MUST appear in a row for it to cover the element (comparison)."""
        return []
    ...
    async def _required_elements(question: str, deadline: float) -> list[str]:
        """Resolve the question's required comparison elements BEFORE research.
        ...
        probe = ("List every MEMBER of the comparison or set this question ranges over, one per entry, and what must be found for each member (its value, status, or qualifying condition). If the set is open-ended, name the pool that must be enumerated. JSON only: a list of strings, at most 6." + "\n\nQuestion:\n" + question[:4000])
        try:
    ...
    def _coverage_order(missing: list[str]) -> str:
        ...
        return (
            "COVERAGE (comparison): research is NOT complete. Nothing you have gathered carries "
    ...
    def _coverage_steer(missing: list[str]) -> str:
        return (
            "COVERAGE CHECK (comparison): the evidence gathered so far does not yet carry:\n- " +
            "\n- ".join(missing[:ELEMENT_MAX]) +
    ...
    #   query -> _balanced_route_label -> LumenAnvil / CedarQuill branch -> _solve -> _knowledge_brief -> _draft_candidate_pool -> _loop -> _audit_patch -> _corroborate -> _value_repair -> _refs_within_budget -> _citations_for -> _finalize_branch_response -> _sanitize_outer_citations -> Response
    ...
    VALUE_MAX_UNSUPPORTED = 3
    VALUE_MIN_TOKEN_LEN = 2


    def _unsupported_values(answer: str, ledger: EvidenceLedger) -> list:
        """Figures asserted in the answer that no CITED row contains."""
        ...
        out = []
        seen = set()
        for match in _NUMERIC_TOKEN_RE.finditer(_strip_markers(answer or "")):
            token = match.group(0)
            if len(token) < VALUE_MIN_TOKEN_LEN:
                continue
            key = _norm_num(token)
            if key in seen:
                continue
            seen.add(key)
            if token in blob or key in flat:
                continue
            out.append(token)
            if len(out) >= VALUE_MAX_UNSUPPORTED:
                break
        return out
    ...
    async def _value_repair(question: str, answer: str, messages: list[dict],
                            ledger: EvidenceLedger, deadline: float) -> str:
        """Value support: a stated figure is in no source the answer cites.

        This is the grounding stage. An unsupported number is either a model
        invention or a citation pointing at the wrong row; both are fatal to
        the judge and both are fixed the same way.
        """
        if not _stage_ready(deadline, STAGE_MIN_LEFT_S):
            return answer
        loose = _unsupported_values(answer, ledger)
        if not loose:
            return answer
        listed = ", ".join(loose)
        order = ("VALUE SUPPORT. These figures appear in the answer but in no "
                 "source it cites: " + listed + ". For each one, either cite "
                 "the source that actually states it, or remove it and say what "
                 "the evidence does support. Do not carry a number that no [n] "
                 "backs. Rewrite the COMPLETE answer with [n] citations.")
        return await _stage_rewrite(question, answer, messages, ledger, deadline,
                                    order, _probe_from(question, loose[0], 140))

    # ---------------------------------------------------------------------
    # POST-AUDIT REPAIR CHAIN -- variant 82 (corroboration + value repair)
    #
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        if _is_usable_answer(answer):
            try:
                ...
                pass
            try:
                answer = await _value_repair(question, answer, messages,
                                               ledger, deadline)
            except Exception:
                pass
