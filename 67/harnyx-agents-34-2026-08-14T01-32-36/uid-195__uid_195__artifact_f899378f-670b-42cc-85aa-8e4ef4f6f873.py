...
#   1) Easyquestion._stage() builds the EASY-path agent (uid_182-style PurseWatch /
#      QuerySniffs / PipelineCore / AnswerMill family) and returns its async
#      `query` handler.
#   2) Hardquestion._stage() builds the HARD-path agent (silver5-style BurnMeter /
#      ThinkLoop / QueryEngine family) and returns its async `query` handler.
#   3) MainRouter._sort() classifies a question as "easy" or "hard" via a short
#      LLM call.
#   4) The @entrypoint `query` routes to _EASY_ROUTER or _HARD_ROUTER.
...
# ---------------------------------------------------------------------------
# Easyquestion - EASY path factory
# Compiles the uid_182-style research loop (PurseWatch, QuerySniffs, ActionBay,
# PipelineCore, AnswerMill, RescuePad, SchemaMill, VerifyLane, etc.) inside
# `_stage()`, then returns that agent's async `query` handler.
# ---------------------------------------------------------------------------
class Easyquestion:
    ...
    def _stage(self):
        ...
        # ---------------------------------------------------------------------
        # PurseWatch - spend / budget remaining helpers for the easy path
        # ---------------------------------------------------------------------
        class PurseWatch:
        ...
        _spend_note = PurseWatch.spend_note
        _spend_left = PurseWatch.spend_left
        ...
        # ---------------------------------------------------------------------
        # QuerySniffs - question-type detectors, wrap-up orders, seed queries
        # ---------------------------------------------------------------------
        class QuerySniffs:
        ...
        _wrapup_order = QuerySniffs.wrapup_order
        _has_superlative = QuerySniffs.has_superlative
        _needs_superlative_proof = QuerySniffs.needs_superlative_proof
        _needs_set_completeness = QuerySniffs.needs_set_completeness
        _seed_queries = QuerySniffs.seed_queries
        ...
        # ---------------------------------------------------------------------
        # EvidenceLedger - claim/evidence registry for the easy path
        # ---------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------
        # RegionScorer - pick salient text windows from fetched pages
        # ---------------------------------------------------------------------
        class RegionScorer:
        ...
        _key_terms = RegionScorer.key_terms
        _best_windows = RegionScorer.best_windows
        ...
        # ---------------------------------------------------------------------
        # ToolOutput - small carrier for a single tool call result
        # ---------------------------------------------------------------------
        class ToolOutput:
        ...
        # ---------------------------------------------------------------------
        # ActionBay - tool execution (search / fetch / page / claims)
        # ---------------------------------------------------------------------
        class ActionBay:
            """Tool-call execution: search, fetch, grep, read, retain evidence."""
        ...
        _commit_tool_output = ActionBay.commit_tool_output
        _degrade_query = ActionBay.degrade_query
        _do_search = ActionBay.do_search
        _do_fetch = ActionBay.do_fetch
        _ledger_page = ActionBay.ledger_page
        _do_page_grep = ActionBay.do_page_grep
        _do_page_read = ActionBay.do_page_read
        _do_retain_evidence = ActionBay.do_retain_evidence
        _run_tool = ActionBay.run_tool
        _preseed = ActionBay.preseed
        ...
        # ---------------------------------------------------------------------
        # EdgarLane - SEC / EDGAR filing resolve helpers
        # ---------------------------------------------------------------------
        class EdgarLane:
        ...
        _sec_tokens = EdgarLane.sec_tokens
        _sec_norm_form = EdgarLane.sec_norm_form
        _fetch_json = EdgarLane.fetch_json
        _sec_pick_filing = EdgarLane.sec_pick_filing
        _do_sec_filing = EdgarLane.do_sec_filing
        ...
        # ---------------------------------------------------------------------
        # ModelPipe - LLM chat wrappers (brief / turn / simple)
        # ---------------------------------------------------------------------
        class ModelPipe:
            ...
            @staticmethod
            async def chat_simple(lane: str, model: str, system: str, user: str, *,
        ...
        _least_think = ModelPipe.least_think
        _chat_simple = ModelPipe.chat_simple
        _chat_turn = ModelPipe.chat_turn
        _knowledge_brief = ModelPipe.knowledge_brief
        ...
        # Stub LLM choice/message types used when a chat call returns empty.
        ...
        # Stub choice wrapper for empty LLM responses.
        class _EmptyChoice:
            message = _EmptyChoiceMessage()



        # Stub LLM response object for empty / failed chats.
        class _EmptyLlm:
            raw_text = ""
        ...
        # Stub turn object pairing empty LLM payload with metadata.
        class _EmptyTurn:
        ...
        # ---------------------------------------------------------------------
        # PipelineCore - main research loop + audit + solve entry
        # ---------------------------------------------------------------------
        class PipelineCore:
        ...
        _loop = PipelineCore.loop
        _audit_patch = PipelineCore.audit_patch
        _solve = PipelineCore.solve
        ...
        # ---------------------------------------------------------------------
        # AnswerMill - draft shaping, citation hygiene, usability checks
        # ---------------------------------------------------------------------
        class AnswerMill:
        ...
        _normalize_brackets = AnswerMill.normalize_brackets
        _cited_numbers = AnswerMill.cited_numbers
        _answer_line_only = AnswerMill.answer_line_only
        _verbatim_from_source = AnswerMill.verbatim_from_source
        _verbatim_structured = AnswerMill.verbatim_structured
        _citations_for = AnswerMill.citations_for
        _looks_like_tool_json = AnswerMill.looks_like_tool_json
        _is_degenerate_repetition = AnswerMill.is_degenerate_repetition
        _is_usable_answer = AnswerMill.is_usable_answer
        _sanitize_draft = AnswerMill.sanitize_draft
        _strip_lead_narration = AnswerMill.strip_lead_narration
        _cap = AnswerMill.cap
        ...
        # ---------------------------------------------------------------------
        # RescuePad - ledger digest / deterministic / resort answer builders
        # ---------------------------------------------------------------------
        class RescuePad:
        ...
        _ledger_digest = RescuePad.ledger_digest
        _informative_lead = RescuePad.informative_lead
        _deterministic_answer = RescuePad.deterministic_answer
        _quote_table = RescuePad.quote_table
        _retained_count = RescuePad.retained_count
        _write_from_digest = RescuePad.write_from_digest
        _knowledge_resort = RescuePad.knowledge_resort
        ...
        # ---------------------------------------------------------------------
        # SchemaMill - schema-shaped output detection and coercion
        # ---------------------------------------------------------------------
        class SchemaMill:
        ...
        _schema_output = SchemaMill.schema_output
        _schema_kind = SchemaMill.schema_kind
        _matches_schema_shape = SchemaMill.matches_schema_shape
        _undigest_for_schema = SchemaMill.undigest_for_schema
        _coerce_to_schema = SchemaMill.coerce_to_schema
        ...
        # ---------------------------------------------------------------------
        # VerifyLane - gap-fill / claim verification patch lane
        # ---------------------------------------------------------------------
        class VerifyLane:
            """hv16 claim verification, gap fill, and rewrite helpers."""
        ...
        _hv16_base_query = VerifyLane.hv16_base_query
        _hv16_extract_json_object = VerifyLane.hv16_extract_json_object
        _hv16_identify_gaps = VerifyLane.hv16_identify_gaps
        _hv16_fresh_search_digest = VerifyLane.hv16_fresh_search_digest
        _hv16_verify_claim = VerifyLane.hv16_verify_claim
        _hv16_rewrite_without_claim = VerifyLane.hv16_rewrite_without_claim
        _hv16_fill_missing_element = VerifyLane.hv16_fill_missing_element
        _hv16_verification_patch = VerifyLane.hv16_verification_patch
        ...
        # Async query handler returned by Easyquestion._stage() (easy path).
...
# ---------------------------------------------------------------------------
# MainRouter - difficulty classifier
# Short LLM call that labels the user question "easy" or "hard". Used by the
# module entrypoint to choose _EASY_ROUTER vs _HARD_ROUTER. On parse/timeout
# error the entrypoint treats the query as hard.
# ---------------------------------------------------------------------------
class MainRouter:
    ...
    # Classify question text as 'easy' or 'hard' (defaults toward hard).
...
# ---------------------------------------------------------------------------
# Hardquestion - HARD path factory
# Compiles the silver5-style research loop (BurnMeter, WebSeeker, EdgarLookup,
# ThinkLoop, QueryEngine, SchemaPress, etc.) inside `_stage()`, then returns
# that agent's async `query` handler.
# ---------------------------------------------------------------------------
class Hardquestion:
    ...
    def _stage(self):
        ...
        # ---------------------------------------------------------------------
        # BurnMeter - spend / budget remaining helpers for the hard path
        # ---------------------------------------------------------------------
        class BurnMeter:
            @staticmethod
            def _burn_left() -> float:
                left = _SPEND["left"]
                if isinstance(left, (int, float)):
                    return float(left)
                return 1.0
            ...
            @staticmethod
            def _burn_note(payload) -> None:
                budget = getattr(payload, "budget", None)
                left = getattr(budget, "session_remaining_budget_usd", None)
                if isinstance(left, (int, float)):
                    _SPEND["left"] = float(left)

        _burn_note = BurnMeter._burn_note
        _burn_left = BurnMeter._burn_left
        ...
        # ---------------------------------------------------------------------
        # QueryTraits - question-type heuristics (set / max / wrap-up)
        # ---------------------------------------------------------------------
        class QueryTraits:
            @staticmethod
            def _needs_entire_set(question: str) -> bool:
                q = " ".join((question or "").split())
                if _SET_HINT_RE.search(q):
                    return True
                ...
                m = _PLURAL_HEAD_RE.search(q)
                if m and m.group(1).lower() not in _PLURAL_FALSE:
                    if not _has_apex_word(q) or re.search(r"\b(?:all|every|each)\b", q, re.IGNORECASE):
                        return True

                return bool(re.search(r"\bwhich\b", q, re.IGNORECASE)) and bool(_SET_CONNECTIVE_RE.search(q))

            @staticmethod
            def _needs_apex_proof(question: str) -> bool:


                q = " ".join((question or "").split())
                if not q:
                    return False
                return _has_apex_word(q) or bool(
                    re.search(r"\b(?:most|least) (?:common|frequent|number|amount)\b|\bhow many\b", q, re.I))

            @staticmethod
            def _has_apex_word(text: str) -> bool:
                if _ONE_WINNER_RE.search(text or ""):
                    return True
                for m in _EST_RE.finditer(text or ""):
                    if m.group(0).lower() not in _EST_STOP:
            ...
            @staticmethod
            def _finale_order(seconds_left: float) -> str:
                return (
                    f"TIME IS UP (~{int(seconds_left)}s left). No more tool calls. Write the "
                    "complete final answer NOW from the numbered results above plus your "
                    "knowledge: the FIRST words are the answer entities (no 'Based on…' "
        ...
        _finale_order = QueryTraits._finale_order
        _has_apex_word = QueryTraits._has_apex_word
        _needs_apex_proof = QueryTraits._needs_apex_proof
        _needs_entire_set = QueryTraits._needs_entire_set
        ...
        # ---------------------------------------------------------------------
        # TraceBook - evidence rows and citation refs for the hard path
        # ---------------------------------------------------------------------
        class TraceBook:
            def __init__(self) -> None:
                self.rows: list[dict] = []

            def insert_row(self, receipt_id: str, result_id: str, note_len: int,
                    kind: str, spans: list[tuple[int, int]] | None,
                    title: str = "", url: str = "", preview: str = "",
                    text: str = "") -> int:
                self.rows.append({
            ...
            def fetch_ref(self, number: int) -> CitationRef | None:
                if not (1 <= number <= len(self.rows)):
                    return None
                row = self.rows[number - 1]
                if row.get("kind") == "reserved":
        ...
        # ---------------------------------------------------------------------
        # SpanScorer - salient window / term picking from page text
        # ---------------------------------------------------------------------
        class SpanScorer:
            @staticmethod
            def _focus_windows(note: str, terms: set[str], width: int,
                              k: int = 1) -> list[tuple[int, int]]:
        ...
        _focus_terms = SpanScorer._focus_terms
        _focus_windows = SpanScorer._focus_windows
        ...
        # ---------------------------------------------------------------------
        # ToolCargo - carrier for a sealed tool-call result
        # ---------------------------------------------------------------------
        class ToolCargo:
        ...
        # ---------------------------------------------------------------------
        # ToolFixer - normalize / seal tool output into ToolCargo form
        # ---------------------------------------------------------------------
        class ToolFixer:
            @staticmethod
            def _fix_tool_output(out, ledger: TraceBook) -> str:

                if isinstance(out, str):
                    return out
                if not isinstance(out, ToolCargo):
                    return f"# tool crashed: {out}"
                text = out.text
                for i, row in enumerate(out.rows):
                    n = ledger.insert_row(row["receipt_id"], row["result_id"], row["note_len"],
                                   row["kind"], row["spans"], title=row.get("title", ""),
                                   url=row.get("url", ""), preview=row.get("preview", ""),
                                   text=row.get("text", ""))
                    text = text.replace(_SLOT.format(i), str(n))
        ...
        _fix_tool_output = ToolFixer._fix_tool_output
        ...
        # ---------------------------------------------------------------------
        # WebSeeker - web search + page fetch (+ query loosening)
        # ---------------------------------------------------------------------
        class WebSeeker:
            @staticmethod
            async def _launch_fetch(url: str, focus: str, question: str, ledger: TraceBook) -> str:
                if not url.strip():
                    return "# read_page: empty url"
                payload = None
                for _attempt in (0, 1):
                ...
                if len(note) <= FETCH_PLAIN_CHARS:
                    row = {"receipt_id": receipt, "result_id": rid, "note_len": len(note),
                           "kind": "fetch", "spans": [(0, len(note))], "title": url,
                           "url": url, "preview": note[:1200], "text": note}
                    return ToolCargo(f"# read_page({url!r}) -> [{_SLOT.format(0)}] full page, "
                                      f"{len(note)} chars\n{note}", [row])

                terms = _focus_terms(question) | _focus_terms(focus)
                windows = _focus_windows(note, terms, FETCH_WINDOW_CHARS, k=FETCH_WINDOWS_PER_PAGE)
                row = {"receipt_id": receipt, "result_id": rid, "note_len": len(note),
                       "kind": "fetch", "spans": [(0, FETCH_HEAD_CHARS)] + list(windows),
                       "title": url, "url": url,
                       "preview": note[windows[0][0]:windows[0][0] + 1200], "text": note}
                ...
                sections = "".join(
                    f"\n--- section @{s} ---\n{note[s:e]}" for s, e in windows)
                return ToolCargo(f"# read_page({url!r}) -> [{_SLOT.format(0)}] {len(note)} chars total; head + "
                        f"the {len(windows)} most relevant section(s) shown "
                        f"({', '.join(f'{s}-{e}' for s, e in windows)}). If the answer set may "
                        f"continue elsewhere in this page, call read_page again with a "
                        f"different focus.\n--- head ---\n{head}{sections}", [row])
            ...
            @staticmethod
            async def _launch_search(query_text: str, ledger: TraceBook):
                if not query_text.strip():
                    return "# web_search: empty query"
            ...
            @staticmethod
            def _thin_query(q: str) -> str:


                out = _SITE_OP_RE.sub("", q or "").replace('"', " ")
                return " ".join(out.split())

        _thin_query = WebSeeker._thin_query
        _launch_search = WebSeeker._launch_search
        _launch_fetch = WebSeeker._launch_fetch
        ...
        # ---------------------------------------------------------------------
        # EdgarLookup - SEC filing resolve / select / JSON pull helpers
        # ---------------------------------------------------------------------
        class EdgarLookup:
            @staticmethod
            async def _launch_sec_filing(company: str, form: str, year: str, deadline: float) -> str:
                company = (company or "").strip()
                form = (form or "").strip() or "10-K"
                year = (year or "").strip()[:4]
                hint = _SEC_SEARCH_HINT.format(company=company, year=year, form=form)
                ...
                if (deadline - monotonic()) < _SEC_MIN_HEADROOM_S:
                    return f"# sec_filing: skipped (low time) — {hint}"
                tickers = await _grab_json(_SEC_TICKERS_URL, deadline)
                if not isinstance(tickers, dict):
                    return f"# sec_filing: EDGAR ticker index unavailable — {hint}"
                want = _sec_chunks(company)
                best = None
                for row in tickers.values():
                    if not isinstance(row, dict):
                        continue
                ...
                if best is None:
                    return f"# sec_filing({company!r}): no confident EDGAR match — {hint}"
                cik10, title = best[2], best[3]
                subs = await _grab_json(_SEC_SUBMISSIONS_URL.format(cik10=cik10), deadline)
                filings = subs.get("filings") if isinstance(subs, dict) else None
                recent = filings.get("recent") if isinstance(filings, dict) else None
                if not isinstance(recent, dict):
                    return f"# sec_filing({company!r}): EDGAR submissions unavailable for {title} — {hint}"
                pick = _sec_grab_filing(recent, form, year)
                if pick is None:
                    return (f"# sec_filing({company!r}, {form!r}, year={year or 'latest'}): no matching "
                            f"filing in EDGAR's recent index for {title} — check the form/year, or {hint}")
                accession, doc = pick
            ...
            @staticmethod
            def _sec_grab_filing(recent: dict, form: str, year: str):
                ...
                if not (isinstance(forms, list) and isinstance(accs, list) and isinstance(docs, list)):
                    return None
                n = min(len(forms), len(accs), len(docs))
                form_norm = _sec_fix_form(form)
                best_year = None
                best_any = None
                for i in range(n):
                    if _sec_fix_form(str(forms[i])) != form_norm:
                        continue
                    if accs[i] is None or docs[i] is None:
                        continue
                    acc = str(accs[i]); doc = str(docs[i])
            ...
            @staticmethod
            async def _grab_json(url: str, deadline: float):
                cached = _SEC_CACHE.get(url)
                if cached is not None:
                    return cached
                for _attempt in (0, 1):
            ...
            @staticmethod
            def _sec_fix_form(form: str) -> str:
            ...
            @staticmethod
            def _sec_chunks(text: str) -> list[str]:


                return [w for w in _SEC_ALNUM_RE.findall((text or "").lower())
                        if w not in _SEC_STOPWORDS]

        _sec_chunks = EdgarLookup._sec_chunks
        _sec_fix_form = EdgarLookup._sec_fix_form
        _grab_json = EdgarLookup._grab_json
        _sec_grab_filing = EdgarLookup._sec_grab_filing
        _launch_sec_filing = EdgarLookup._launch_sec_filing
        ...
        # ---------------------------------------------------------------------
        # PageBin - page grep / read / retain-evidence tooling
        # ---------------------------------------------------------------------
        class PageBin:
            @staticmethod
            def _launch_retain_evidence(source: str, quote: str, ledger: TraceBook) -> str:
            ...
            @staticmethod
            def _launch_page_read(url: str, offset: int, length: int, ledger: TraceBook) -> str:

                hit = _trace_page(url, ledger)
                if hit is None:
                    return f"# page_read: {url!r} has not been fetched this run; call read_page first"
                n, row = hit
                text = row.get("text") or ""
            ...
            @staticmethod
            def _launch_page_grep(url: str, pattern: str, ledger: TraceBook) -> str:
            ...
            @staticmethod
            def _trace_page(url: str, ledger: TraceBook) -> tuple[int, dict] | None:
        ...
        _trace_page = PageBin._trace_page
        _launch_page_grep = PageBin._launch_page_grep
        _launch_page_read = PageBin._launch_page_read
        _launch_retain_evidence = PageBin._launch_retain_evidence



        # ---------------------------------------------------------------------
        # ToolHub - dispatch named tools to WebSeeker / Edgar / PageBin
        # ---------------------------------------------------------------------
        class ToolHub:
            @staticmethod
            async def _spin_tool(call, question: str, ledger: TraceBook, deadline: float) -> str:
                try:
                    args = json.loads(getattr(call, "arguments", None) or "{}")
                except Exception:
                    args = {}
                ...
                if name == "web_search":
                    return await _launch_search(str(args.get("query") or ""), ledger)
                if name == "read_page":
                    return await _launch_fetch(str(args.get("url") or ""), str(args.get("focus") or ""),
                                           question, ledger)
                if name == "retain_evidence":
                    return _launch_retain_evidence(str(args.get("source") or ""),
                                               str(args.get("quote") or ""), ledger)
                if name == "page_grep":
                    return _launch_page_grep(str(args.get("url") or ""),
                                         str(args.get("pattern") or ""), ledger)
                if name == "page_read":
                    return _launch_page_read(str(args.get("url") or ""),
                                         args.get("offset") or 0,
                                         args.get("length") or PAGE_READ_MAX_CHARS, ledger)
                if name == "sec_filing":
                    return await _launch_sec_filing(str(args.get("company") or ""),
                                                str(args.get("form") or ""),
                                                str(args.get("year") or ""), deadline)
                return f"# unknown tool {name!r}"

        _spin_tool = ToolHub._spin_tool
        ...
        # ---------------------------------------------------------------------
        # TalkLink - LLM generation bridge (turn / simple / upstream route)
        # ---------------------------------------------------------------------
        class TalkLink:
            @staticmethod
            async def _talk_round(messages: list[dict], deadline: float, *, finish_only: bool,
                                 force_tools: bool = False):
                ...
                for lane_model in ((LLM_LANE_A, LOOP_MODEL_A, True),
                    ...
                    try:
                        ...
                        payload = await asyncio.wait_for(llm_chat(
                            ...
                            max_output_tokens=6000 if (finish_only and lane == LLM_LANE_B) else None,
                            provider_extra=_prefer_upstream(lane, model) if pinned else None,
                            timeout=timeout,
                        ), timeout=min(timeout + 6.0,
                                       max(1.0, deadline - monotonic() - 1.0)))
                        _burn_note(payload)
            ...
            @staticmethod
            async def _talk_plain(lane: str, model: str, system: str, user: str, *,
                                   max_tokens: int, timeout: float,
                                   think: dict | None = None) -> str:
                if think is None:
                    think = _mild_think(lane, model)


                _pin0 = _prefer_upstream(lane, model)
                payload = None
            ...
            @staticmethod
            def _prefer_upstream(lane: str, model: str) -> dict | None:
            ...
            @staticmethod
            def _mild_think(lane: str, model: str = "") -> dict:
        ...
        _mild_think = TalkLink._mild_think
        _prefer_upstream = TalkLink._prefer_upstream
        _talk_plain = TalkLink._talk_plain
        _talk_round = TalkLink._talk_round
        ...
        # ---------------------------------------------------------------------
        # WarmBrief - seed searches and forge the opening knowledge brief
        # ---------------------------------------------------------------------
        class WarmBrief:
            @staticmethod
            async def _kick_search(question: str, set_question: bool, ledger: TraceBook,
                               deadline: float) -> str:

                seeds = _kick_queries(question, set_question)
                if not seeds or (deadline - monotonic()) < 40.0:
                    return ""
                ...
                for seed in seeds:
                    if (deadline - monotonic()) < 30.0:
                        break
                    try:
                        out = await asyncio.wait_for(_launch_search(seed, ledger),
                                                      timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
                        blocks.append(_fix_tool_output(out, ledger))
                    except Exception:
                        continue
                good = [b for b in blocks if isinstance(b, str) and _CITE_MARK_RE.search(b)]
                if not good:
            ...
            @staticmethod
            def _kick_queries(question: str, set_question: bool) -> list[str]:
                q = " ".join((question or "").split())
                if not q:
                    return []
                seeds = [q[:300]]
            ...
            @staticmethod
            async def _warm_brief(question: str) -> tuple[str, str]:
                ...
                raw = ""
                try:
                    raw = await _talk_plain(LLM_LANE_A, LOOP_MODEL_A, system, user,
                                             max_tokens=2400, timeout=BRIEF_TIMEOUT_S,
                                             think=_mild_think(LLM_LANE_A, LOOP_MODEL_A))
                except Exception:
                    try:
                        raw = await _talk_plain(LLM_LANE_B, LOOP_MODEL_B, system, user,
                                                 max_tokens=2400, timeout=BRIEF_TIMEOUT_S,
                                                 think=_mild_think(LLM_LANE_B, LOOP_MODEL_B))
                    except Exception:
                        raw = ""
                if not raw:
                    return "", ""
        ...
        _warm_brief = WarmBrief._warm_brief
        _kick_queries = WarmBrief._kick_queries
        _kick_search = WarmBrief._kick_search

        # ---------------------------------------------------------------------
        # ThinkLoop - main tool/LLM cycle plus audit amend for the hard path
        # ---------------------------------------------------------------------
        class ThinkLoop:
            @staticmethod
            async def _audit_touch(question: str, answer: str, messages: list[dict],
                                   ledger: TraceBook, deadline: float) -> str:
                probe = (
                    "Audit the answer against the question. JSON only, keys: "
                    '"unanswered_parts" (list; question elements not addressed), '
                    '"uncited_facts" (list; load-bearing claims without [n]), '
                ...
                try:
                    raw = await _talk_plain(LLM_LANE_A, AUDIT_MODEL,
                                             "Strict completeness auditor. JSON only.",
                                             probe, max_tokens=2200,
                                             timeout=max(8.0, min(AUDIT_TIMEOUT_S,
                                                                  (deadline - monotonic()) - 72.0)))
                ...
                order += ("\nUse at most 3 tool calls to close the most important gaps, then "
                          ...
                          "required shape.")
                messages.append({"role": "system", "content": order})
                patched, _ = await _think_loop(question, "", ledger, deadline,
                                         AUDIT_EXTRA_TURNS + 1, carry=messages,
                                         allow_tools_in_wrapup=True)
                patched = patched.strip()

                if not _is_fine_answer(patched) or len(patched) < int(len(answer) * 0.6):
                    return answer
                return patched

            @staticmethod
            async def _think_loop(question: str, brief: str, ledger: TraceBook,
                            deadline: float, turn_cap: int,
                            carry: list[dict] | None = None,
                            allow_tools_in_wrapup: bool = False) -> tuple[str, list[dict]]:
                if carry is not None:
                    ...
                    set_q = _needs_entire_set(question)
                    messages = [{"role": "system", "content": LOOP_RULES}]
                    if set_q:
                        messages.append({"role": "system", "content": SET_RULE})
                    if _needs_apex_proof(question):
                        messages.append({"role": "system", "content": SUPERLATIVE_RULE})
                    if brief:
                        messages.append({"role": "system", "content": brief})

                    seeded = await _kick_search(question, set_q, ledger, deadline)
                    if seeded:
                        messages.append({"role": "system", "content": seeded})
                    messages.append({"role": "user", "content": question})
                ...
                for turn in range(1, turn_cap + 1):
                    ...
                    if left <= MIN_TAIL_S:
                        break
                    out_of_time = left <= WRAPUP_AT_S
                    out_of_spend = _burn_left() <= WRAPUP_MIN_USD
                    finish_only = out_of_time or out_of_spend or turn >= turn_cap
                    if (finish_only or turn >= turn_cap - 1) and not ordered_wrapup:
                        messages.append({"role": "system", "content": _finale_order(left)})
                        ordered_wrapup = True

                    payload = await _talk_round(messages, deadline, finish_only=finish_only,
                                               force_tools=allow_tools_in_wrapup and turn == 1)
                    if payload is None:
                        break
                    llm = getattr(payload, "llm", None)
                    ...
                    if not calls:
                        ...
                        if not _is_fine_answer(candidate):
                            if repairs_left > 0 and (deadline - monotonic()) > MIN_TAIL_S + 10.0:
                                repairs_left -= 1
                    ...
                    tool_tasks = [asyncio.ensure_future(_spin_tool(c, question, ledger, deadline))
                                  for c in run_calls]
                    try:
                        await asyncio.wait(tool_tasks, timeout=tool_budget)
                    except Exception:
                    ...
                    for call_result in zip(run_calls, results):
                        ...
                        body = _fix_tool_output(call_result[1], ledger)
                        messages.append({"role": "tool", "tool_call_id": call.id, "content": body})
                    for call in calls[8:]:
                        messages.append({"role": "tool", "tool_call_id": call.id,
                                         "content": "# skipped: per-turn tool budget reached — re-issue next turn if still needed"})
        ...
        _think_loop = ThinkLoop._think_loop
        _audit_touch = ThinkLoop._audit_touch
        ...
        # ---------------------------------------------------------------------
        # TextForm - citation gathering and source-mirroring for the answer
        # ---------------------------------------------------------------------
        class TextForm:
            @staticmethod
            def _make_citations(answer: str, ledger: TraceBook) -> list[CitationRef]:
                ...
                for n in _cite_vals(answer, len(ledger.rows)):
                    if len(refs) >= CITATION_CAP:
                        break
                    ref = ledger.fetch_ref(n)
                    if ref is None:
                        continue
                    row = ledger.rows[n - 1]
                    slices = getattr(ref, "slices", None)
            ...
            @staticmethod
            def _clone_structured(obj, ledger: TraceBook, depth: int = 0):

                if depth > 6:
                    return obj
                if isinstance(obj, str):
                    return _clone_from_source(obj, ledger)
                if isinstance(obj, list):
                    return [_clone_structured(x, ledger, depth + 1) for x in obj]
                if isinstance(obj, dict):
                    return {k: _clone_structured(v, ledger, depth + 1) for k, v in obj.items()}
                return obj

            @staticmethod
            def _clone_from_source(value: str, ledger: TraceBook) -> str:
                ...
                if not texts:
                    return value
                def found_in(t: str) -> bool:
                    return bool(t) and any(t in src for src in texts)
                if found_in(v):
                    return value
                a, b = m.group("a").strip(), m.group("b").strip()
                hits = [x for x in (b, a) if found_in(x)]
                if len(hits) == 1:
                    return hits[0]
                if len(hits) == 2:
                    lo, hi = sorted(hits, key=len)
            ...
            @staticmethod
            def _text_line_only(answer: str, question: str) -> str:
            ...
            @staticmethod
            def _cite_vals(answer: str, top: int) -> list[int]:
                answer = _smooth_brackets(answer)
                seen: set[int] = set()
                out: list[int] = []
                for m in _CITE_NUM_RE.finditer(answer):
                    for chunk in m.group(1).split(","):
            ...
            @staticmethod
            def _smooth_brackets(text: str) -> str:
                return (text or "").translate(_BRACKET_FIX)

        _smooth_brackets = TextForm._smooth_brackets
        _cite_vals = TextForm._cite_vals
        _text_line_only = TextForm._text_line_only
        _clone_from_source = TextForm._clone_from_source
        _clone_structured = TextForm._clone_structured
        _make_citations = TextForm._make_citations
        ...
        # ---------------------------------------------------------------------
        # TextGuard - draft wash / readiness / junk-repeat / tool-json checks
        # ---------------------------------------------------------------------
        class TextGuard:
            @staticmethod
            def _cleanse_draft(text: str) -> str:


                return _VERIFY_MARK_RE.sub("", text or "").strip()

            @staticmethod
            def _is_fine_answer(text: str) -> bool:


                s = _smooth_brackets(text).strip()
                if not s:
                    return False

                if _TOOL_MARKUP_RE.search(s) or _smells_tool_json(s):
                    return False
                if _STUB_ANSWER_RE.match(s) or _is_loop_spam(s):
                    return False
                cited = bool(_CITE_MARK_RE.search(s))
                if cited and len(s) >= MIN_CITED_ANSWER_CHARS:
                    return True
            ...
            @staticmethod
            def _is_loop_spam(text: str) -> bool:
            ...
            @staticmethod
            def _smells_tool_json(s: str) -> bool:


                return bool(re.match(r'\s*\{\s*"(?:name|tool|function)"\s*:', s))

        _smells_tool_json = TextGuard._smells_tool_json
        _is_loop_spam = TextGuard._is_loop_spam
        _is_fine_answer = TextGuard._is_fine_answer
        _cleanse_draft = TextGuard._cleanse_draft
        ...
        # ---------------------------------------------------------------------
        # SchemaPress - detect schema flavor and mold the final answer to it
        # ---------------------------------------------------------------------
        class SchemaPress:
            @staticmethod
            def _press_to_schema(answer: str, schema, depth: int = 0):
                ...
                if not kind:
                    ...
                    for key in ("anyOf", "oneOf", "allOf"):
                        ...
                        if isinstance(branch, list) and branch:
                            for sub in branch:
                                if isinstance(sub, dict) and sub.get("type") != "null":
                                    return _press_to_schema(answer, sub, depth + 1)
                    kind = "string"
                if kind == "array":
                    items = schema.get("items") or {}
                    parts = [p.strip(" -*\t") for p in re.split(r"[\n;]|,(?![^(]*\))", answer or "")]
                    ...
                    if not parts:
                        parts = [answer[:400]]
                    return [_press_to_schema(p, items, depth + 1) for p in parts]
                if kind == "object":
                    props = schema.get("properties") or {}
                    required = schema.get("required") or list(props.keys())
                    out = {}
                    for key in required:
                        ...
                        out[key] = _press_to_schema(answer, props.get(key) or {}, depth + 1)
                    return out
                if kind in ("number", "integer"):
            ...
            @staticmethod
            def _strip_for_schema(basis: str) -> str:
            ...
            @staticmethod
            def _equals_schema_shape(value, schema) -> bool:
                kind = _schema_bucket(schema)
                if not kind:
                    return True
                if kind == "array":
            ...
            @staticmethod
            async def _schema_forge(question: str, answer: str, schema, deadline: float) -> object | None:
                ask = ("Convert the answer to a JSON value valid under the schema. Output "
                       "ONLY the JSON value.\n\n"
                       f"Schema:\n{json.dumps(schema)}\n\nQuestion:\n{question}\n\n"
                       f"Answer:\n{answer[:14000]}")
                ...
                for lane, model in ((LLM_LANE_A, SCHEMA_MODEL),
                    ...
                    if left < 12.0:
                        break
                    try:
                        raw = await _talk_plain(lane, model,
                                                 "You output strictly valid JSON.", ask,
                                                 max_tokens=3400, timeout=min(45.0, left - 4.0))
                        raw = re.sub(r"^```(?:json)?\s*|\s*```$", "", raw.strip(),
                                     flags=re.I | re.M).strip()
                        ...
                        if _equals_schema_shape(value, schema):
                            return value
                        if isinstance(value, dict) and len(value) == 1:
                            inner = list(value.values())[0]
                            if _equals_schema_shape(inner, schema):
                                return inner
                    except Exception:
                        continue
                return None

        _schema_forge = SchemaPress._schema_forge
        _schema_bucket = SchemaPress._schema_bucket
        _equals_schema_shape = SchemaPress._equals_schema_shape
        _strip_for_schema = SchemaPress._strip_for_schema
        _press_to_schema = SchemaPress._press_to_schema
        ...
        # ---------------------------------------------------------------------
        # TextPolish - truncate and strip lead narration before emit
        # ---------------------------------------------------------------------
        class TextPolish:
            @staticmethod
            ...
            @staticmethod
            def _drop_open_narration(text: str) -> str:
        ...
        _drop_open_narration = TextPolish._drop_open_narration
        _cutoff = TextPolish._cutoff


        # Async query handler returned by Hardquestion._stage() (hard path).
        async def query(query: Query) -> Response:
            question = (query.text or "").strip()
            if not question:
                return Response(text="No question provided.")
        ...
        # ---------------------------------------------------------------------
        # QueryEngine - hard-path solve orchestration
        # Wires brief -> cycle -> guards -> schema/polish into one solve path.
        # ---------------------------------------------------------------------
        class QueryEngine:
            @staticmethod
            async def _solve(query: Query, question: str) -> Response:
                ...
                brief = ""
                try:
                    if _burn_left() >= BRIEF_MIN_USD and (deadline - monotonic()) > 120.0:
                        draft, brief = await _warm_brief(question)
                except Exception:
                    brief = ""
                ...
                messages: list[dict] = []
                try:
                    answer, messages = await _think_loop(question, brief, ledger, deadline, MAX_TURNS)
                except Exception:
                    answer = ""

                try:
                    if _is_fine_answer(answer) and (deadline - monotonic()) > 75.0 \
                            and _burn_left() >= AUDIT_MIN_USD:
                        patched = await _audit_touch(question, answer, messages, ledger, deadline)

                        if _is_fine_answer(patched):
                            answer = patched
                except Exception:
                    pass


                if not _is_fine_answer(answer) and ledger.rows:
                    try:
                        rescued = await _emit_from_digest(question, ledger, deadline)
                        if _is_fine_answer(rescued):
                            answer = rescued
                    except Exception:
                        pass
                ...
                if not _is_fine_answer(answer) and ledger.rows:
                    det = _formula_answer(question, ledger)
                    if _is_fine_answer(det):
                        answer = det

                if not _is_fine_answer(answer):
                    fallback = _cleanse_draft(draft) or await _warm_resort(question, deadline)
                    if _is_fine_answer(fallback):
                        answer = fallback

                try:
                    citations = _make_citations(answer, ledger)
                except Exception:
                    citations = []

                answer = _smooth_brackets(answer)
                answer = _drop_open_narration(answer)

                answer = _text_line_only(answer, question)
                text = _cutoff(answer) or f"Best-effort answer unavailable for: {question[:400]}"

                if query.output_schema is not None:
                    structured = None
                    try:
                        structured = await _schema_forge(question, answer, query.output_schema, deadline)
                    except Exception:
                        structured = None
                    if structured is not None:
                        try:
                            structured = _clone_structured(structured, ledger)
                        except Exception:
                            pass
                        try:
                            return Response(output=structured, citations=citations or None)
                    ...
                    basis = answer if _is_fine_answer(answer) else ""
                    if not basis:
                        basis = _formula_answer(question, ledger)
                    if not basis or _STUB_ANSWER_RE.match(basis.strip()):
                        basis = question[:400]
                    ...
                    if basis is not answer:
                        try:
                            salvaged = await _schema_forge(question, basis, query.output_schema,
                                                            deadline)
                        except Exception:
                            salvaged = None
                        if salvaged is not None:
                    ...
                    if basis is not answer:
                        cleaned = _strip_for_schema(basis)
                        basis = cleaned if cleaned else ""
                    try:
                        forced = _press_to_schema(_cutoff(basis), query.output_schema)
                        return Response(output=forced, citations=citations or None)
                    except Exception:
                        try:
                            return Response(output=_cutoff(basis)[:2000],
                                            citations=citations or None)
                        except Exception:
                            pass
        ...
        _solve = QueryEngine._solve


        # ---------------------------------------------------------------------
        # SpareAnswer - digest / resort / last-resort answer builders
        # ---------------------------------------------------------------------
        class SpareAnswer:
            @staticmethod
            async def _warm_resort(question: str, deadline: float) -> str:
                left = deadline - monotonic()
                if left < 12.0:
                    return ""
                try:
                    return await _talk_plain(
                        LLM_LANE_A, RESORT_MODEL,
                        ("Expert researcher. Best definitive answer with concrete entities, "
                         "numbers, dates. Never refuse."),
                        question, max_tokens=2600, timeout=min(45.0, left - 4.0))
            ...
            @staticmethod
            async def _emit_from_digest(question: str, ledger: TraceBook, deadline: float) -> str:


                left = deadline - monotonic()
                if left < 14.0:
                    return ""
                digest = _trace_digest(ledger)
                if not digest:
                    return ""
                convo = [{"role": "system", "content": _COMMIT_RULES},
                         {"role": "user", "content": (
                             ...
                             "claim carries its [n]; then the short proof section (pool, "
                             "conditions, qualifiers, exclusions).")}]
                async def _trial_lane(lane: str, model: str, budget: float) -> str:


                    _p0 = _prefer_upstream(lane, model)
                    payload = None
                    for _p in ((_p0, None) if _p0 is not None else (None,)):
                        try:
                            payload = await llm_chat(
                                provider=lane, model=model, messages=convo,
                                temperature=0.15, max_output_tokens=2600,
                                timeout=budget, thinking=_mild_think(lane, model),
                                provider_extra=_p,
                            )
                            break
                        except Exception:
                ...
                for i, lane_model in enumerate(lanes):
                    ...
                    if budget < 8.0:
                        return ""
                    try:
                        text = await _trial_lane(lane_model[0], lane_model[1], budget)
                    except Exception:
                        continue
                    if _is_fine_answer(text):
                        return text
                return ""

            @staticmethod
            def _retain_n(ledger: TraceBook) -> int:
                return sum(len(r.get("retained") or []) for r in ledger.rows)

            @staticmethod
            def _quote_grid(ledger: TraceBook) -> str:
            ...
            @staticmethod
            def _formula_answer(question: str, ledger: TraceBook) -> str:
                ...
                for i, r in rows:
                    if picked >= 6:
                        break
                    lead = _meaty_lead(r.get("preview") or "")
                    if not lead:
                        continue
                    title = (r.get("title") or "").strip()
                    out.append(f"- {title + ': ' if title else ''}{lead} [{i}]")
            ...
            @staticmethod
            def _meaty_lead(preview: str, limit: int = 280) -> str:
            ...
            @staticmethod
            def _trace_digest(ledger: TraceBook, char_cap: int = 60000) -> str:
        ...
        _trace_digest = SpareAnswer._trace_digest
        _meaty_lead = SpareAnswer._meaty_lead
        _formula_answer = SpareAnswer._formula_answer
        _quote_grid = SpareAnswer._quote_grid
        _retain_n = SpareAnswer._retain_n
        _emit_from_digest = SpareAnswer._emit_from_digest
        _warm_resort = SpareAnswer._warm_resort
...
# =============================================================================
# Module wiring - compile both paths once, keep classifier handle
# =============================================================================
# _EASY_ROUTER / _HARD_ROUTER are the async query callables from Easyquestion /
# Hardquestion. _ROOT_ROOT_ROUTER is the MainRouter classifier used by the
# entrypoint.
# =============================================================================
_EASY_ROUTER = Easyquestion()._stage()
# Hard-path query callable from Hardquestion._stage().
_HARD_ROUTER = Hardquestion()._stage()

# Difficulty classifier instance (MainRouter._sort).
_ROOT_ROOT_ROUTER = MainRouter()



# =============================================================================
# Entrypoint - classify then dispatch
# =============================================================================
# On classifier failure, default to hard. Easy -> _EASY_ROUTER; else -> _HARD_ROUTER.
# =============================================================================
@entrypoint('query')
async def query(query: Query) -> Response:

    try:
        level = await _ROOT_ROOT_ROUTER._sort(query.text)
    except Exception:
        level = 'hard'

    if level == 'easy':
        return await _EASY_ROUTER(query)

    return await _HARD_ROUTER(query)
