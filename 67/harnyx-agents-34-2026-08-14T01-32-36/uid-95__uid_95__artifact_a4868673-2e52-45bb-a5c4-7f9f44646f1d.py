...
class CedarLattice_99cd82:
    ...
    def _compile(self):
        ...
        LOOP_MIN_REMAINING_SECONDS = 5
        RETRY_MIN_REMAINING_SECONDS = 12
        SEARCH_EXCERPT_CHARS = 700
        FETCH_CONTENT_CHARS = 6000
        MAX_CITATIONS = 16
        MIN_ANSWER_CHARS = 40
        DETERMINISTIC_MAX_SOURCES = 6
        LEAD_CHARS = 300
        REFUSAL_CHECK_BEFORE_FINAL_ANSWER = True
        CAP_AFTER_FILTER = True
        CHECKPOINT_KEEPS_CITATIONS = True
        ...
        LEAK_REPROMPT = "That response contained literal tool-call markup instead of a real tool call. Either issue a proper tool call, or write the final answer as plain prose starting with 'FINAL ANSWER: '."
        ...
        REFUSAL_MARKERS = ('i could not complete', 'insufficient evidence', 'unable to determine', 'cannot be determined from')

        class _IndexEntry:

            def __init__(self, receipt_id: str, result_id: str, width: int, note_len: int, title: str, url: str, lead: str) -> None:
        ...
        class _ResultIndex:
            ...
            def record(self, receipt_id: str, results: object, *, shown_chars: int) -> list[tuple[int, object]]:
                ...
                for r in results or ():
                    ...
                    self._by_number[n] = _IndexEntry(receipt_id=receipt_id, result_id=result_id, width=shown_chars, note_len=len(note), title=getattr(r, 'title', None) or '', url=getattr(r, 'url', None) or '', lead=note[:LEAD_CHARS])
                    recorded.append((n, r))
                return recorded
        ...
        def _checkpoint_answer(index: _ResultIndex) -> str:
            parts = []
            for n in index.numbers()[:DETERMINISTIC_MAX_SOURCES]:
                meta = index.get(n)
                if meta is None:
                    continue
                ...
                parts.append(f"- {(title + ': ' if title else '')}{lead[:LEAD_CHARS]} [{n}]")
            if not parts:
                return ''
        ...
        def _citations_from_inline_markers(answer_text: str, index: _ResultIndex) -> tuple[CitationRef, ...]:
            ...
            for n in candidates:
                ...
                if meta.note_len <= 0:
                    continue
                end = min(meta.width, meta.note_len)
        ...
        async def _dispatch_tool_calls(tool_calls, messages: list[dict[str, object]], index: _ResultIndex, *, deadline: float) -> None:
        ...
        async def _research_loop(messages: list[dict[str, object]], index: _ResultIndex, *, deadline: float) -> str | None:
        ...
        async def query(query: Query) -> Response:
            ...
            try:
                ...
                if not _is_usable_answer(final_answer or '') and deadline - perf_counter() > RETRY_MIN_REMAINING_SECONDS:
                ...
                try:
                    ...
                    checkpoint = _checkpoint_answer(index)
                    if checkpoint:
                        if CHECKPOINT_KEEPS_CITATIONS:
                            return _respond(checkpoint, index)
                        return Response(text=checkpoint)
                    return Response(text=INSUFFICIENT_ANSWER)
        return query

class PrismValve_99cd82:

    @staticmethod
    def _prefer_alloy_99cd82() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_ALLOY_RUN_99cd82 = AlloyRivets_99cd82()._compile()
_CEDAR_RUN_99cd82 = CedarLattice_99cd82()._compile()
_PRISM_VALVE_99cd82 = PrismValve_99cd82()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _PRISM_VALVE_99cd82._prefer_alloy_99cd82():
        return await _ALLOY_RUN_99cd82(query)
    return await _CEDAR_RUN_99cd82(query)
