...
# It also runs after timeframe alignment, which can replace figures wholesale.
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # NOTE: three stages, not four — the two donor builds both carried the
    # second-source check, so their union is {timeframe, grounding, corroboration}.
    # Ordered: PERIOD first (a wrong period invalidates the figures the next two
    # stages inspect, and its repair can replace them wholesale), then the numeric
    # pair in backer-count order — grounding owns the zero-backer case and
    # corroboration the one-backer case, so grounding must precede it. Each stage
    # re-checks its own time/spend floor and returns `answer` untouched on failure.
