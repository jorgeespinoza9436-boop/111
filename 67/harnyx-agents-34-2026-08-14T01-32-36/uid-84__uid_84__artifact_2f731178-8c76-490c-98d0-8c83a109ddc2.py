    ...
    def _compile(self):
        ...
        def _dump_floor_answer(index: _ResultIndex) -> str | None:
            ...
            if len(parts) == 1:
                return None
            return '\n'.join(parts)
        _SCRATCH_LINE_RE = re.compile("^\\s*(?:let me (?:search|check|look|find|verify|see)|i need to\\b|i'?ll (?:search|start|first|now|check|look)|first,? i'?ll|next,? i'?ll|i should (?:search|check|look)|let'?s (?:search|start)|i (?:will|am going to) (?:search|now|start|check))\\b", re.IGNORECASE)

        def _unfinished_answer_guard(text: str) -> str:
            if not text:
                return text
            kept = [ln for ln in text.splitlines() if not _SCRATCH_LINE_RE.match(ln)]
            result = '\n'.join(kept).strip()
        ...
        def _deliverable(text: str | None, index: _ResultIndex, *, cite_text: str | None=None) -> Response:
            ...
            answer = _unfinished_answer_guard(answer)
...
class LatchHub_6a3ff5:

    @staticmethod
    def _pick_marble_6a3ff5() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_MARBLE_RUN_6a3ff5 = MarbleQuay_6a3ff5()._compile()
_GRANITE_RUN_6a3ff5 = GraniteQuay_6a3ff5()._compile()
_LATCH_HUB_6a3ff5 = LatchHub_6a3ff5()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _LATCH_HUB_6a3ff5._pick_marble_6a3ff5():
        return await _MARBLE_RUN_6a3ff5(query)
    return await _GRANITE_RUN_6a3ff5(query)
