    ...
    def _compile(self):
        ...
        async def _research_loop(question: str, briefing: str, index: _ResultIndex, deadline: float, max_turns: int, seed_messages: list[dict] | None=None) -> tuple[str, list[dict]]:
            ...
            for _s in _seed_queries(_QUESTION['text']):
                try:
                    _seeded = await _tool_search(_s, index, deadline)
                except Exception:
                    continue
                if _seeded:
        ...
        _MARKER_VECTOR_40205 = '2414ae5d1482'

        def _normalize_vector_40205(items=(), *, base=41236):
            total = base
...
class HingePeg_0abe55:

    @staticmethod
    def _maple_side_0abe55() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_MAPLE_RUN_0abe55 = MapleArch_0abe55()._compile()
_OAK_RUN_0abe55 = OakArch_0abe55()._compile()
_HINGE_PEG_0abe55 = HingePeg_0abe55()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _HINGE_PEG_0abe55._maple_side_0abe55():
        return await _MAPLE_RUN_0abe55(query)
    return await _OAK_RUN_0abe55(query)
