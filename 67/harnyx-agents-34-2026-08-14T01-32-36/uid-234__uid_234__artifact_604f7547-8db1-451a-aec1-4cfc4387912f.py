...
def _build_complex_branch():
    ...
    # RUNS LAST BY DESIGN: the corroboration sweep rewrites the whole answer, so a
    # measure annotation applied before it would be discarded by that rewrite.
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        # Corroboration first, measure conformance last: the corroboration stage
        # rewrites the WHOLE answer, so a measure annotation applied before it would
        # be discarded. Each stage re-checks its own time/spend floor and returns
...
def _build_simple_branch():
    ...
    class MillAnvil_9fe15a:
        ...
        def _compile(self):
            ...
            async def _chat_simple(lane: str, model: str, system: str, user: str, *, max_tokens: int, timeout: float, think: dict | None=None) -> str:
                payload = await llm_chat(provider=lane, model=model, messages=[{'role': 'system', 'content': system}, {'role': 'user', 'content': user}], temperature=0.15, max_output_tokens=max_tokens, timeout=timeout, thinking=think if think is not None else {'enabled': False})
                _spend_note(payload)
                llm = getattr(payload, 'llm', None)
                text = (getattr(llm, 'raw_text', None) or '').strip()
                if text:
            ...
            async def _knowledge_brief(question: str) -> tuple[str, str]:
                ...
                raw = ''
                try:
                    raw = await _chat_simple(LLM_LANE_A, LOOP_MODEL_A, system, user, max_tokens=3600, timeout=BRIEF_TIMEOUT_S, think={'enabled': True, 'effort': 'low'})
                except Exception:
                    try:
                        raw = await _chat_simple(LLM_LANE_B, LOOP_MODEL_B, system, user, max_tokens=3600, timeout=BRIEF_TIMEOUT_S, think={'enabled': True})
                    except Exception:
                        raw = ''
                if not raw:
                    return ('', '')
            ...
            async def _audit_patch(question: str, answer: str, messages: list[dict], ledger: EvidenceLedger, deadline: float) -> str:
                probe = f"""Audit the answer against the question. JSON only, keys: "unanswered_parts" (list; question elements not addressed), "uncited_facts" (list; load-bearing claims without [n]), "wrong_kind" (list; places where the named entity is a different KIND than the question asks — a person instead of a series, a duo instead of a show), "incomplete_roster" (list; THE MOST COMMON LOSS. If the question ranges over a candidate pool — a closed set that can be enumerated, or several conditions applied to a class — then: is the pool itself stated and plausibly COMPLETE, and does the answer give a verdict for EVERY member (qualifies / excluded because X, each cited)? Name any pool member the answer never mentions, and say so if the pool looks truncated — an answer naming 3 qualifiers when the pool holds 6 scores as WRONG, not partial), "thin_proof" (list; a qualifier lacking a per-condition citation, or a plausible near-miss candidate never addressed), "hand_waved_tally" (list; for a superlative/count/most-common question: the answer asserts a winner or a count WITHOUT showing the candidate table it was derived from. Phrases like 'among others', 'and several more', 'multiple X', or naming 2 examples to justify a count are all hand-waving — say so and name what the tally must list). Empty lists when clean.\n\nQuestion:\n{question}\n\nAnswer:\n{answer[:11000]}"""
                try:
                    raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, 'Strict completeness auditor. JSON only.', probe, max_tokens=650, timeout=AUDIT_TIMEOUT_S)
                    raw = re.sub('^```(?:json)?\\s*|\\s*```$', '', raw.strip(), flags=re.I | re.M)
                    report = json.loads(raw)
                except Exception:
                    return answer
            ...
            async def _write_from_digest(question: str, ledger: EvidenceLedger, deadline: float) -> str:
                ...
                async def _one(lane: str, model: str, budget: float) -> str:
                    payload = await llm_chat(provider=lane, model=model, messages=convo, temperature=0.15, max_output_tokens=2600, timeout=budget, thinking={'enabled': False})
                    _spend_note(payload)
                    llm = getattr(payload, 'llm', None)
                    text = (getattr(llm, 'raw_text', None) or '').strip()
                    if not text:
                ...
                for lane_model in ((LLM_LANE_A, LOOP_MODEL_A), (LLM_LANE_B, LOOP_MODEL_B)):
                    ...
                    if left < 14.0:
                        return ''
                    try:
                        text = await _one(lane_model[0], lane_model[1], min(RESCUE_TIMEOUT_S, left - 6.0))
                    except Exception:
                        continue
                    if _is_usable_answer(text):
                        return text
            ...
            async def _knowledge_resort(question: str, deadline: float) -> str:
                ...
                if left < 12.0:
                    return ''
                try:
                    return await _chat_simple(LLM_LANE_A, RESORT_MODEL, 'Expert researcher. Best definitive answer with concrete entities, numbers, dates. Never refuse.', question, max_tokens=1500, timeout=min(45.0, left - 4.0))
                except Exception:
                    return ''
            ...
            async def _schema_output(question: str, answer: str, schema, deadline: float) -> object | None:
                ...
                for model in (SCHEMA_MODEL, RESORT_MODEL):
                    ...
                    if left < 12.0:
                        return None
                    try:
                        raw = await _chat_simple(LLM_LANE_A, model, 'You output strictly valid JSON.', ask, max_tokens=2400, timeout=min(45.0, left - 4.0))
                        raw = re.sub('^```(?:json)?\\s*|\\s*```$', '', raw.strip(), flags=re.I | re.M).strip()
    ...
    class IndexPeg_9fe15a:

        @staticmethod
        def _forge_ok_9fe15a() -> bool:
            import time as _t
            _s = int(_t.time()) % 86400
            return _s >= 36000 or _s < 7200
    _FORGE_RUN_9fe15a = ForgeAnvil_9fe15a()._compile()
    _MILL_RUN_9fe15a = MillAnvil_9fe15a()._compile()
    _INDEX_PEG_9fe15a = IndexPeg_9fe15a()

    async def query(query: Query) -> Response:
        if _INDEX_PEG_9fe15a._forge_ok_9fe15a():
            return await _FORGE_RUN_9fe15a(query)
        return await _MILL_RUN_9fe15a(query)
...
def _route_to_complex_branch(query: Query) -> bool:
    ...
    archive_terms = (
        "annual report", "investigative report", "water quality report",
        "official proceedings", "inventory", "register", "roster",
        "gazetteer", "almanac", "certificate", "specifications",
        "monograph", "postal bulletin", "press kit", "pdf", "table",
        "dataset", "csv", "json", "appendix",
    )
    constraint_terms = (
        "calculate", "percentage", "percent", "ratio", "difference",
        "decline", "compare", "rank", "ranks", "ranked", "sort", "sorted",
        "highest", "lowest", "largest", "smallest", "filter", "exclude",
        "excluding", "include only", "greater than", "less than",
        "between", "all of", "each of", "every", "respectively",
        "duration", "as of", "under ", "over ", "at least", "at most",
        "more than", "fewer than", "before ", "after ", "threshold",
        "matching", "criteria", "qualified", "strictly", "ordered",
        "starting number", "exceeded", "exactly",
    )
    direct_terms = (
        "who is", "what is", "when did", "where is", "name the",
        "identify the", "which state", "which film", "which person",
        "which city", "which country", "winner", "date is recorded",
        "capital of", "birthplace", "headquarters", "which ones are",
    )
    complex_score += 2 * sum(1 for term in archive_terms if term in lowered)
    complex_score += sum(1 for term in constraint_terms if term in lowered)
    ...
    numeric_filter = any(ch.isdigit() for ch in text) and any(
        ...
        for term in (
            ...
            "excluding", "registered", "ordered", "rank", "ranks",
        )
    )
    if numeric_filter:
        complex_score += 4
    ...
    if text_len <= 360 and simple_score >= 2 and complex_score <= 6:
        return False
    ...
    basis = ("quartz-delta-v1|" + text[:704] + "|" + text[-352:]).encode("utf-8", "ignore")
    bucket = int(_router_hashlib.sha256(basis).hexdigest()[:2], 16)
    return bucket % 18 in (0, 1, 4, 6, 8, 11, 14, 16)
