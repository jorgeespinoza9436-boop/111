"""hk411 "authority+corrob" — champion-v52 toolloop + authority sweep + lead-figure corroboration.
...
# authoritative domain; then ONE targeted search for the official page plus a
# bounded rewrite round. The domain regex is unit-tested against real URLs —
# an earlier generation shipped a raw-string \\.gov that never matched.
_AU_NEED_RE = re.compile(
...
async def _authority_sweep(question: str, answer: str, messages: list[dict],
                           ledger: EvidenceLedger, deadline: float) -> str:
    if (deadline - monotonic()) < 75.0 or _spend_left() <= AUDIT_MIN_USD:
        return answer
    if not _AU_NEED_RE.search(question or ""):
        return answer
    urls = _cited_row_urls(answer, ledger)
    if not urls or any(_AU_DOMAIN_RE.search(u) for u in urls):
    ...
    query = " ".join(salient[:7]) + " official source"
    try:
...
# ── entrypoint ────────────────────────────────────────────────────────────────
# ── CADEIA DE VERIFICACAO POS-AUDITORIA ─────────────────────────────────────
# Quatro estagios entre _audit_patch e a escada de resgate. Cada um: detector
# DETERMINISTICO (custo zero de LLM) -> so se acusar, gasta busca/reescrita.
# Contrato uniforme: (question, answer, messages, ledger, deadline) -> str,
# devolvendo `answer` intacto em qualquer falha.
#
# ⚠️ Detectores proprios de proposito: copiar implementacao alheia nos expoe a
# cair como duplicata DELA se virar referencia.

SWEEP_MIN_LEFT_S = 80.0          # piso de tempo comum a todos os estagios
SWEEP_REWRITE_TURNS = 3          # teto de turnos da reescrita
_SW_MARKER_RE = re.compile(r"\[[0-9][0-9,\s\-]*\]")
_SW_NUM_RE = re.compile(r"\d[\d,]*(?:\.\d+)?")
_SW_MAG = {"thousand": 1e3, "million": 1e6, "billion": 1e9, "trillion": 1e12,
           "mil": 1e3, "milhao": 1e6, "bilhao": 1e9}
_SW_STOP = {"The", "A", "An", "In", "On", "At", "Of", "For", "As", "By", "And",
            "Or", "But", "If", "When", "What", "Which", "Who", "How", "Why",
            "According", "Provide", "List", "Give", "Between", "During", "From"}
MAX_SWEEP_FLAGS = 3


def _sw_floors_ok(answer, deadline, min_left=SWEEP_MIN_LEFT_S):
    """Piso de tempo + dinheiro + resposta utilizavel. Vale para todo estagio."""
    if not _is_usable_answer(answer):
        return False
    if (deadline - monotonic()) < min_left:
        return False
    return _spend_left() > AUDIT_MIN_USD


def _sw_cited_text(answer, ledger):
    """Texto bruto SO das linhas que a resposta realmente cita."""
    out = []
    try:
        for n in _cited_numbers(answer, len(ledger.rows)):
            row = ledger.rows[n - 1]
            out.append((row.get("text") or "") + " " + (row.get("preview") or ""))
    except Exception:
        return []
    return out


def _sw_numbers(text):
    """Valores numericos de um texto, como float, sem os marcadores [n]."""
    body = _SW_MARKER_RE.sub(" ", text or "")
    vals = []
    for m in _SW_NUM_RE.finditer(body):
        ...
        if len(re.sub(r"\D", "", m.group(0))) < 2:
            continue                      # digito unico: indice, ordinal
        vals.append((m.group(0), v))
    return vals


def _sw_numeros_preservados(answer, novo):
    """Todo numero do original sobreviveu na reescrita?

    Guarda dos estagios SEM fonte propria (`_check_derivations`,
    `_conform_measures`). A ordem deles pede acrescimo; esta guarda garante que
    a reescrita nao substituiu mesmo assim. Sem ela, ordem e guarda ficariam
    dizendo coisas diferentes — o defeito nº5 da auditoria de 13/08.
    """
    antes = {b for b, _ in _sw_numbers(answer or "")}
    depois = {b for b, _ in _sw_numbers(novo or "")}
    return antes.issubset(depois)


async def _sw_rewrite(question, answer, messages, ledger, deadline, ordem,
                      so_acrescimo=False):
    """Reescrita limitada, reusando o loop que ja existe.

    ⚠️ NAO muta `messages`: cada estagio leva uma COPIA com a SUA ordem. Se
    mutasse, as ordens se acumulariam (ate 7 no uid212, somando os estagios da
    base) e a instrucao de um estagio ficaria no contexto do seguinte, mandando
    reescrever a resposta inteira por criterios ja aplicados.
    """
    carry = list(messages) + [{"role": "system", "content": ordem}]
    try:
        novo, _ = await _loop(question, "", ledger, deadline, SWEEP_REWRITE_TURNS,
                              carry=carry, allow_tools_in_wrapup=True)
    except Exception:
        return answer                     # o contrato e devolver intacto
    if not (_is_usable_answer(novo) and len(novo) >= 0.6 * len(answer or "")):
        return answer                     # guarda de regressao de tamanho
    if so_acrescimo and not _sw_numeros_preservados(answer, novo):
        return answer                     # substituiu apesar da ordem: descarta
    return novo


# ── estagio 1: sujeito da pergunta ausente da evidencia ─────────────────────
# Se a pergunta nomeia um sujeito que NAO aparece em nenhuma linha coletada, a
# resposta pode ser sobre outra entidade (homonimo, empresa irma, edicao errada).
# Erro de sujeito torna todo teste posterior irrelevante -> este estagio vem 1o.
MIN_ROWS_PARA_JULGAR_AUSENCIA = 3
MIN_CHARS_PARA_JULGAR_AUSENCIA = 1500


def _sw_named_subjects(question):
    """Nomes proprios (sequencias capitalizadas) e termos entre aspas."""
    q = question or ""
    achados = []
    for m in re.finditer(r"[\"'“‘]([^\"'”’]{3,60})[\"'”’]", q):
        achados.append(m.group(1).strip())
    for m in re.finditer(r"\b([A-Z][\w&.-]+(?:\s+[A-Z][\w&.-]+){0,3})", q):
        t = m.group(1).strip()
        if t.split()[0] in _SW_STOP and len(t.split()) == 1:
            continue
        if len(t) >= 4:
            achados.append(t)
    vistos, saida = set(), []
...
def _sw_absent_subjects(question, ledger):
    """So acusa quando NENHUM token distintivo do sujeito aparece na evidencia.

    ⚠️ Exigir a FRASE exata dava falso positivo: evidencia correta sobre 'WWE
    Elimination Chamber' que nao repete o termo era acusada. Falso positivo aqui
    custa uma reescrita inteira, entao o detector erra para o lado de nao
    disparar.
    """
    if len(ledger.rows) < MIN_ROWS_PARA_JULGAR_AUSENCIA:
        return []                          # pouca evidencia: ausencia nao informa
    corpo = " ".join(((r.get("text") or "") + " " + (r.get("preview") or ""))
                     for r in ledger.rows).lower()
    if len(corpo.strip()) < MIN_CHARS_PARA_JULGAR_AUSENCIA:
        return []
    fora = []
    for s in _sw_named_subjects(question):
        if s.lower() in corpo:
            continue
        tokens = [t for t in re.findall(r"[\w]{4,}", s.lower())]
        if not tokens:
            continue
        if any(t in corpo for t in tokens):
            continue                       # algum token distintivo presente
        fora.append(s)
    return fora[:MAX_SWEEP_FLAGS]


async def _verify_subjects(question, answer, messages, ledger, deadline):
    if not _sw_floors_ok(answer, deadline, SWEEP_MIN_LEFT_S + 20.0):
        return answer
    fora = _sw_absent_subjects(question, ledger)
    if not fora:
        return answer
    alvo = fora[0]
    try:
        achado = await asyncio.wait_for(_do_search(alvo, ledger),
                                        timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
        corpo = _commit_tool_output(achado, ledger)
    except Exception:
        return answer
    if not (corpo and _CITE_MARK_RE.search(corpo)):
        return answer
    ordem = (
        f"SUBJECT CHECK: the question names '{alvo}', which appears in NO row of "
        "the evidence the answer was written from. The answer may describe a "
        "different entity with a similar name. One search for that exact subject "
        "is numbered below. Confirm every claim concerns this exact subject; "
        "correct anything that was about a namesake, sibling entity, or wrong "
        "edition, then rewrite the COMPLETE final answer with [n] citations.\n\n"
        + corpo)
    return await _sw_rewrite(question, answer, messages, ledger, deadline, ordem)


# ── estagio 2: numero afirmado sem suporte na linha citada ──────────────────
# O juiz so credita alegacao que a linha CITADA sustenta. Detectamos numero
# visivel na resposta que nao aparece em nenhuma linha citada.
# ⚠️ DIFERENCA NOSSA: um numero legitimamente CALCULADO (razao, percentual,
# diferenca) nunca aparece literal na fonte. Sinaliza-lo seria falso positivo,
# e dispararia reparo em cima de conta certa. Por isso excluimos aqui tudo que
# _sw_derivable() reconhece como derivavel dos numeros citados.
# 🚨 TETO OBRIGATORIO — medido em 13/08 com ledger realista:
#   900 numeros citados  -> 1,07 s por verificacao ->  21 s numa resposta de 20
#   2.700 numeros        -> 10,4 s por verificacao -> 207 s  (orcamento: 266 s)
# O casamento e O(n²) e roda CPU SINCRONA dentro de corrotina async: trava o
# event loop e come o deadline ate a plataforma matar o sandbox (response=None
# = 0,0). Com 120 valores: ~0,02 s por verificacao. Deduplicar importa porque
# pagina real repete o mesmo numero dezenas de vezes.
MAX_VALORES_CITADOS = 120
MAX_NUMEROS_DA_RESPOSTA = 40


def _sw_base_valores(trechos):
    """Valores citados, deduplicados e limitados — protege o event loop."""
    vistos, saida = set(), []
    for t in trechos:
        for _, v in _sw_numbers(t):
            if v in vistos:
                continue
            vistos.add(v)
            saida.append(v)
            if len(saida) >= MAX_VALORES_CITADOS:
                return saida
    return saida


JANELA_OPERANDOS = 200


def _sw_frase_em(texto, pos):
    """Janela de +-JANELA_OPERANDOS caracteres em volta de `pos`.

    🚨 NAO usar ponto como fronteira de frase: `97.2` tem ponto DENTRO do
    numero, e `i.e.` / `U.S.` / `No.` tambem — o corte cairia no meio do
    proprio valor e o pool perderia os operandos. Janela fixa e imune a isso.
    """
    ini = max(0, pos - JANELA_OPERANDOS)
    fim = min(len(texto), pos + JANELA_OPERANDOS)
    return texto[ini:fim]


def _sw_pool_local(corpo, bruto, v):
    """Operandos candidatos: so os numeros da MESMA FRASE do valor.

    🚨 MEDIDO em 13/08 — parear com TODOS os numeros das paginas citadas nao
    discrimina nada: com 120 valores, 100% dos numeros aleatorios sao
    'derivaveis' (as somas/razoes cobrem a reta densa demais). Com o pool da
    frase, o falso positivo cai para 1-6% e a deteccao de conta errada vai a
    100%. E tambem o desenho certo pela linguagem: quem enuncia uma conta poe
    os operandos ao lado do resultado.
    """
    pos = corpo.find(bruto)
    if pos < 0:
        return []
    return [x for _, x in _sw_numbers(_sw_frase_em(corpo, pos))
            if abs(x - v) > 1e-9]


def _sw_derivable(v, base, tol=0.02):
    """v sai de alguma operacao simples entre dois valores citados?"""
    for a in base:
        for b in base:
            if a is b:
                continue
            ...
            if b:
                cands += [a / b, a / b * 100.0]
                if b != 0:
                    cands.append((a - b) / b * 100.0)
            for c in cands:
                if c and abs(v - c) <= tol * max(abs(c), 1e-9):
                    return True
    return False


def _sw_ungrounded(answer, ledger):
    trechos = _sw_cited_text(answer, ledger)
    if not trechos:
        return []                          # nada citado: problema de outro estagio
    citados = _sw_base_valores(trechos)
    junto = " ".join(trechos)
    fora = []
    for bruto, v in _sw_numbers(answer)[:MAX_NUMEROS_DA_RESPOSTA]:
        plano = bruto.replace(",", "")
        if bruto in junto or plano in junto:
            continue                       # aparece literal na fonte citada
        if _sw_derivable(v, _sw_pool_local(answer, bruto, v), tol=0.005):
            continue                       # e conta correta, nao alucinacao
        fora.append(bruto)
    return fora[:MAX_SWEEP_FLAGS]


async def _ground_figures(question, answer, messages, ledger, deadline):
    if not _sw_floors_ok(answer, deadline):
        return answer
    fora = _sw_ungrounded(answer, ledger)
    if not fora:
        return answer
    alvo = fora[0]
    try:
        achado = await asyncio.wait_for(
            _do_search(f"{question[:120]} {alvo}", ledger),
            timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
        corpo = _commit_tool_output(achado, ledger)
    except Exception:
        corpo = ""
    ordem = (
        "FIGURE GROUNDING: these values appear in the answer but are carried by "
        f"NO row it cites: {', '.join(fora)}. For each one, exactly one of three "
        "things is true, and the fix differs:\n"
        "  (a) it is a CORRECT CALCULATION from the cited numbers — then KEEP the "
        "value and state the derivation inline so a reader can reproduce it;\n"
        "  (b) the [n] points at the wrong row — then fix the citation;\n"
        "  (c) the value is wrong — then correct it from the cited evidence.\n"
        "Never delete a claim the evidence supports, and never introduce a number "
        "the evidence cannot produce. Rewrite the COMPLETE final answer with [n] "
        "citations."
        + (("\n\n" + corpo) if corpo and _CITE_MARK_RE.search(corpo) else ""))
    return await _sw_rewrite(question, answer, messages, ledger, deadline, ordem)


# ── estagio 3: conta declarada que NAO fecha ────────────────────────────────
# Cobre `evidence_grounded_calculation_or_proof`: conclusao que nenhuma fonte
# declara sozinha e precisa de calculo reproduzivel. Sinalizamos so o caso FORTE
# -- valor que erra por POUCO uma derivacao dos numeros citados (0,5% a 25%).
# Erro pequeno = conta feita errada; erro grande = outro numero, nao e o nosso
# caso e cairia em falso positivo.
DERIV_MIN_ERRO = 0.005
DERIV_MAX_ERRO = 0.08
# ⚠️ So checamos numero que a RESPOSTA apresenta como calculado. Sem esta
# exigencia o detector achava "derivacao errada" em coincidencia aritmetica
# (com N numeros citados e 5 operacoes, bater por acaso a <25% e comum) e
# disparava reescrita em resposta certa. Teto de erro tambem caiu de 25% para
# 8%: erro de conta real e de arredondamento/digito, nao de ordem de grandeza.
_SW_CALC_CUE_RE = re.compile(
    r"\b(total|sum|combined|together|aggregate|average|mean|median|ratio|share|"
    r"percent|percentage|proportion|difference|gap|increase|decrease|decline|"
    r"growth|change|per|rate|margin|yield|soma|total|media|razao|diferenca)\b",
    re.I)
CALC_CUE_JANELA = 70


def _sw_bad_derivations(answer, ledger):
    trechos = _sw_cited_text(answer, ledger)
    if not trechos:
        return []
    citados = _sw_base_valores(trechos)
    junto = " ".join(trechos)
    corpo_ans = _SW_MARKER_RE.sub(" ", answer or "")
    ruins = []
    for bruto, v in _sw_numbers(answer)[:MAX_NUMEROS_DA_RESPOSTA]:
        plano = bruto.replace(",", "")
        if bruto in junto or plano in junto:
            continue                       # veio direto da fonte, nao e derivado
        pos = corpo_ans.find(bruto)
        if pos < 0:
            continue
        redor = corpo_ans[max(0, pos - CALC_CUE_JANELA):pos + len(bruto) + 20]
        if not _SW_CALC_CUE_RE.search(redor):
            continue                       # a resposta nao apresenta como conta
        # 🚨 se o valor E derivacao EXATA de algum par, a conta esta certa.
        # Sem isto, o pareamento verdadeiro (erro ~0) cai ABAIXO da banda e e
        # descartado, e o laco acha outro par qualquer DENTRO da banda —
        # acusando de errada uma conta correta.
        local = _sw_pool_local(corpo_ans, bruto, v)
        if len(local) < 2:
            continue                       # sem operandos ao lado: nao e conta
        if _sw_derivable(v, local, tol=DERIV_MIN_ERRO):
            continue                       # derivacao EXATA = conta certa
        melhor = None
        for a in local:
            for b in local:
                if a is b or not b:
                    continue
                for rotulo, c in (("a+b", a + b), ("a-b", a - b),
                                  ("a/b", a / b), ("a/b*100", a / b * 100.0),
                                  ("(a-b)/b*100", (a - b) / b * 100.0)):
                    if not c:
                        continue
                    erro = abs(v - c) / max(abs(c), 1e-9)
                    if DERIV_MIN_ERRO < erro <= DERIV_MAX_ERRO:
                        if melhor is None or erro < melhor[1]:
                            melhor = ((a, b, rotulo, c), erro)
        if melhor:
            (a, b, rotulo, c), erro = melhor
            ruins.append((bruto, f"{rotulo} sobre {a:g} e {b:g}", c, erro))
    return ruins[:MAX_SWEEP_FLAGS]


async def _check_derivations(question, answer, messages, ledger, deadline):
    if not _sw_floors_ok(answer, deadline):
        return answer
    ruins = _sw_bad_derivations(answer, ledger)
    if not ruins:
        return answer
    linhas = "\n".join(
        f"- the answer states {b}, but {desc} over the CITED values gives "
        f"{c:.4g} ({100*e:.1f}% apart)"
        for b, desc, c, e in ruins)
    # ⚠️ SO ACRESCIMO. Este estagio NAO busca evidencia nova — mandar substituir
    # o valor aqui e reescrever com MENOS informacao do que a base tinha, que e
    # o modo de dano medido em 13/08 (uid224: -2,0 livre vs -1,0 estrito; das
    # 10 tasks, as 2 que MELHORARAM foram acrescimo e as 5 que pioraram foram
    # substituicao). Estagio sem fonte propria so pode ACRESCENTAR.
    ordem = (
        "DERIVATION CHECK: the answer reports computed quantities that do not "
        "reconcile with the numbers in the rows it cites:\n" + linhas +
        "\n\nKEEP every figure already in the answer — do not replace or delete "
        "any of them. Beside each one, ADD the explicit derivation from the "
        "cited operands, so a reader can reproduce the arithmetic and see which "
        "reading the evidence supports. If the cited rows are the wrong inputs, "
        "ADD the correct citation alongside. Rewrite the COMPLETE final answer "
        "with [n] citations.")
    return await _sw_rewrite(question, answer, messages, ledger, deadline, ordem,
                             so_acrescimo=True)


# ── estagio 4: unidade/escala divergente da fonte ───────────────────────────
# Cobre `structured_field_semantics` (unidade, escala, regra de data/versao).
# ⚠️ POR ULTIMO: cada estagio reescreve a resposta INTEIRA, entao arrumar a
# apresentacao antes de arrumar o VALOR faz a reescrita seguinte descartar.
def _sw_scale_conflicts(answer, ledger):
    trechos = _sw_cited_text(answer, ledger)
    if not trechos:
        return []
    junto = " ".join(trechos).lower()
    corpo = _SW_MARKER_RE.sub(" ", answer or "")
    conflitos = []
    for m in re.finditer(r"(\d[\d,]*(?:\.\d+)?)\s*(thousand|million|billion|trillion)\b",
                         corpo, re.I):
        num, mag = m.group(1), m.group(2).lower()
        outras = [k for k in ("thousand", "million", "billion", "trillion") if k != mag]
        achou_mesma = f"{num.lower()} {mag}" in junto
        achou_outra = any(f"{num.lower()} {o}" in junto for o in outras)
        if achou_outra and not achou_mesma:
            conflitos.append(f"{num} {mag}")
    return conflitos[:MAX_SWEEP_FLAGS]


async def _conform_measures(question, answer, messages, ledger, deadline):
    if not _sw_floors_ok(answer, deadline, SWEEP_MIN_LEFT_S - 20.0):
        return answer
    conflitos = _sw_scale_conflicts(answer, ledger)
    if not conflitos:
        return answer
    # ⚠️ SO ACRESCIMO — mesmo motivo do estagio de derivacao: sem fonte propria,
    # substituir e trocar numero certo por errado.
    ordem = (
        "MEASURE CONFORMANCE: the answer prints these with a magnitude the cited "
        f"rows do not use for that value: {', '.join(conflitos)}. KEEP the "
        "figures already present — do not replace them. Beside each, ADD the "
        "equivalent in the unit and scale the cited source uses, and obey any "
        "unit or format the question demands. Rewrite the COMPLETE final answer "
        "with [n] citations.")
    return await _sw_rewrite(question, answer, messages, ledger, deadline, ordem,
                             so_acrescimo=True)


_SWEEP_CHAIN = (_verify_subjects, _ground_figures,
                _check_derivations, _conform_measures)
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # stage 3e: official-source questions must cite an official domain
    try:
        if _is_usable_answer(answer) and (deadline - monotonic()) > 75.0 \
                and _spend_left() >= AUDIT_MIN_USD:
            anchored = await _authority_sweep(question, answer, messages,
                                              ledger, deadline)
            if _is_usable_answer(anchored):
                answer = anchored
    except Exception:
        pass

    # ── cadeia de verificacao pos-auditoria (13/08) ─────────────────────────
    # Ordem fixa: sujeito -> numero -> conta -> escala. Sujeito primeiro porque
    # resposta sobre a entidade ERRADA torna todo teste posterior irrelevante;
    # escala por ultimo porque cada estagio reescreve a resposta INTEIRA e a
    # reescrita seguinte descartaria a formatacao. Cada estagio re-checa os
    # proprios pisos de tempo/dinheiro e devolve `answer` intacto se algo falhar.
    for _sw in _SWEEP_CHAIN:
        try:
            if not _is_usable_answer(answer):
                break
            varrido = await _sw(question, answer, messages, ledger, deadline)
            if _is_usable_answer(varrido):
                answer = varrido
        except Exception:
            continue
