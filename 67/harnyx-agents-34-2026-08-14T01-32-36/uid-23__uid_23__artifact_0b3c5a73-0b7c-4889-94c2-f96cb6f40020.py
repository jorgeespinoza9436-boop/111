...
def _build_complex_branch():
    ...
    # ── quote-first evidence (FRONT / Grounding-led-Generation pattern) ──────────
    ...
    # answer's claims (retained-evidence advice says so). A question naming
...
def _route_to_complex_branch(query: Query) -> bool:
    ...
    complex_terms = (
        ...
        "exceeded", "excluding", "address", "bulletin", "register",
        "appendix", "gazetteer", "certified", "monograph", "inventory",
        "quality report", "annual report", "schedule", "inspection",
        "ratification", "programme", "fellowship", "official page",
        "candidate pool",
    )
    force_complex_terms = (
        ...
        "bulletin", "register", "appendix", "gazetteer", "inventory",
        "quality report", "annual report", "schedule", "inspection",
    )
    direct_terms = (
        ...
        "among the", "which ones", "named", "credited", "announced",
        "described as", "which of",
    )
    complex_score += sum(1 for term in complex_terms if term in lowered)
    direct_hits = sum(1 for term in direct_terms if term in lowered)
    simple_score += direct_hits
    if (
        schema_weight == 0
        and text_len <= 345
        and direct_hits >= 1
        and not any(
            term in lowered
            for term in (
                "table", "ordered", "ranked", "sorted", "strictly", "exceeded",
                "exact", "duration", "starting number", "percentage", "ratio",
            )
        )
    ):
        return False
    ...
    numeric_filter = any(ch.isdigit() for ch in text) and any(
        ...
        for term in (
            ...
            "excluding", "registered", "ordered", "register", "appendix",
    ...
    if "first" in lowered and "second" in lowered:
        complex_score += 1
    if text_len <= 505 and simple_score >= 2 and complex_score <= 5:
        return False
    ...
    basis = ("cobalt-router|" + text[:416] + "|" + text[-352:]).encode("utf-8", "ignore")
    bucket = int(_router_hashlib.sha256(basis).hexdigest()[:2], 16)
    return bucket % 12 in (0, 2, 3, 6, 8, 11)
