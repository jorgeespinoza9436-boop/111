...
TD_XSFKUX = "openrouter"          # primary lane (loop + briefing)

TD_JGBPFE = re.compile(r"^\s*Best-supported findings|^\s*sources retrieved:", re.I)
...
class EvidenceLedger:
    ...
    def add(self, receipt_id: str, result_id: str, note_len: int,
        ...
        self.rows.append({
            ...
            "text": (text or "")[:TD_RFZRMT],   # in-process only, never shipped
...
TD_YQARJY = 90.0           # remaining <= this -> stop researching, write. v32.6 tried 105 to dodge the

async def _td_bbbifp(question: str, answer: str, messages: list[dict],
...
TD_PYKQIH = 50.0       # v32.10: MEASURED on glm-5, reasoning OFF. Unchanged for v33.1: the
...
def _td_bywmjt(source: str, quote: str, ledger: EvidenceLedger) -> str:
    ...
    if len(q) < TD_IIBJGC:
        return (f"# retain_evidence: quote too short ({len(q)} chars); quote at least "
                f"{TD_IIBJGC} characters of the source text")
    if not text:
...
TD_THTBDT = re.compile(r"\[[0-9]{1,3}\]")   # ASCII, matching _CITE_NUM_RE

TD_VUSHJX = 14_000   # one ledger row must not eat the whole budget

TD_ZGGUYT = {0x3010: "[", 0x3011: "]", 0xFF3B: "[", 0xFF3D: "]",
                0xFF08: "(", 0xFF09: ")", 0x2011: "-", 0x2212: "-"}

for _d in range(10):                      # U+FF10..U+FF19 -> ASCII 0-9
    TD_ZGGUYT[0xFF10 + _d] = chr(48 + _d)

TD_JYTWSJ = frozenset(
    "inc incorporated corp corporation company companies co ltd limited llc plc "
    "lp llp group holdings the".split())

TD_KXENJQ = re.compile(r"\b(?:both|also|and (?:also|had|has|was|were)|as well as)\b",
                                re.IGNORECASE)

TD_PMRDAP = 26.0     # large JSON needs more than the page default (lineage lesson)
...
def _td_vcetyc(note: str, terms: set[str], width: int,
...
TD_PSJRTU = re.compile(
    r"\b(?:I|my|we|let me|let's)\b|\b(?:fail|failed|above|output|outputs|gathered|"
    r"retrieved|fetched|queried|verbatim|formatting)\b", re.IGNORECASE)

TD_VJMPUT = re.compile(r"\[[0-9][0-9,\s\-]*\]")

TD_ISHSSA = 20.0   # ceiling for the whole finalize stage

TD_KKYCTP = 14.0     # reserved for _knowledge_resort / _schema_output (both need 12s)

TD_YWQBMI = 3600     # champion and the rank-2/268 v33.1, ran 3000/3600.
...
async def _td_iwbitd(question: str, answer: str, schema, deadline: float) -> object | None:
    ...
    for lane, model in ((TD_XSFKUX, TD_JSEGDF),
                        (TD_XSFKUX, TD_PAGENW),
                        (TD_UBCBYC, TD_WXFBRD)):
        ...
        if left < 12.0:
            break
        try:
            raw = await _td_mncsau(lane, model,
...
TD_XTNVTT = re.compile(
    r"\baudit(?:'s|s')?\s+(?:premise|premises|claim|claims|claimed|note|notes|noted|"
    r"flag|flags|flagged|report|finding|findings|assertion|says|said|states|stated)\b",
    re.IGNORECASE)

TD_SKJXFP = re.compile(
...
TD_DVSAQB = 1400          # per quote, shown to the synthesiser

TD_DZCISE = 3

TD_YJXVQZ = 2      # v32.4: bounded retries when the model emits junk instead of an answer

TD_RWSDMF = "parallel"             # only search/fetch key we store
...
TD_KCVVIY = re.compile(r"\b(?:sorted?|order(?:ed)?) alphabetical(?:ly)?\b", re.IGNORECASE)

async def _td_btfbys(messages: list[dict], deadline: float, *, finish_only: bool,
    ...
    for lane_model in ((TD_XSFKUX, TD_BKIJDD, True),
                       (TD_XSFKUX, TD_BKIJDD, False),
                       (TD_UBCBYC, TD_WXFBRD, False)):
        lane = lane_model[0]
        model = lane_model[1]
        pinned = lane_model[2]
        if model == TD_WXFBRD and payload_chars > TD_FEUDJP:
            ...
            return TD_BEJDCS
        timeout = min(TD_GDXXSD, deadline - monotonic() - 5.0,
                      turn_wall - monotonic())
        if timeout <= 5.0:
            return None
        try:
            ...
            payload = await asyncio.wait_for(llm_chat(
                ...
                model=model,
                messages=messages,
                tools=TD_THRQJY if (force_tools or not finish_only) else None,
                ...
                thinking=({"enabled": False} if (finish_only and model == TD_WXFBRD)
                          else {"enabled": True, "effort": "low"}),
                max_output_tokens=6000 if (finish_only and model == TD_WXFBRD) else None,
                provider_extra=_td_eemqqj(lane, model) if pinned else None,
                timeout=timeout,
...
TD_GKXIQZ = re.compile(
    r"<\s*/?\s*tool_call|<\s*/?\s*(?:arg_key|arg_value|function_call|invoke)\b"
    r"|\bweb_search\s*[（(]\s*query|\bread_page\s*[（(]\s*url|\bsec_filing\s*[（(]\s*company",
    re.I)

TD_DYPAWU = re.compile(
    r"\b(?:highest|lowest|largest|smallest|most|least|greatest|fewest|longest|"
    r"shortest|first|last|best|worst|only|oldest|youngest|newest|biggest)\b",
    re.IGNORECASE)

TD_FEUDJP = 144000   # ~36k tokens: above the largest lane-B

async def _td_rgckkq(question: str, set_question: bool, ledger: EvidenceLedger,
                   deadline: float) -> str:
    ...
    for seed in seeds:
        if (deadline - monotonic()) < 30.0:
            break
        try:
            out = await asyncio.wait_for(_td_ttvpxb(seed, ledger),
                                          timeout=TD_WSSXKB * 2 + 6.0)   # R3: _do_search now retries
...
TD_KYAKCK = ("Decart", "CoreWeave", "Alibaba")        # z-ai/glm-5.2
...
async def _td_hbehir(response, started: float):
    ...
    window = min(TD_ISHSSA,
                 max(TD_GPDGVW, TD_UYNZBT - elapsed))
    try:
...
async def _loop(question: str, brief: str, ledger: EvidenceLedger,
    ...
    for turn in range(1, turn_cap + 1):
        ...
        payload = await _td_btfbys(messages, deadline, finish_only=finish_only,
        ...
        tool_budget = max(5.0, min(TD_CCJMIF * 2 + 6.0,
                                   deadline - monotonic() - TD_RGYAQP))
        ...
        tool_tasks = [asyncio.ensure_future(_td_sfhtbq(c, question, ledger, deadline))
                      for c in run_calls]
        try:
...
TD_YZDNDC = re.compile(r"\[([0-9][0-9,\s\-]*)\]")

TD_KVIRKS = "search \"site:sec.gov {company} {year} {form}\" and read_page the Archives result"

TD_GGRGWR = re.compile(
    r"\b(?:retain_evidence|retain call|read_page|page_read|page_grep|search_web|"
    r"fetch_page|read_json|the tool|tool call)s?\b", re.IGNORECASE)

TD_BZTEJR = re.compile(r"\]\(")
...
TD_YIKMYK = re.compile(r"(?:\b[A-Z]|\b(?:Inc|Ltd|Co|No|vs|St|Dr|Mr|Ms|Mt|Jr|Sr|etc|e\.g|i\.e))\.$")
...
TD_KIJDNZ = ("Cerebras", "Groq", "BaseTen")       # openai/gpt-oss-120b

TD_UNZUWC = re.compile(r"\s*\((?:verify|unverified|uncertain)[^)]*\)", re.I)
...
TD_TPMYTC = re.compile(r"[A-Za-z0-9][A-Za-z0-9.\-']+")
...
async def _td_vrpubm(question: str, ledger: EvidenceLedger, deadline: float) -> str:
    ...
    if not digest:
        return ""
    convo = [{"role": "system", "content": TD_KQMEEX},
             {"role": "user", "content": (
...
TD_ETKWRQ = re.compile(r"\$?\b\d{3,}[\d,]*(?:\.\d+)?\b|\b\d+(?:\.\d+)?%")
...
TD_TENVMT = re.compile(r"\bsite:\S+\s*", re.I)
...
def _td_raibkk(question: str, set_question: bool) -> list[str]:
    ...
    salient = [t for t in TD_TPMYTC.findall(q)
               if len(t) >= 3 and t.lower() not in TD_MRKHZD and t.lower() not in TD_TTXKIJ]
    if len(salient) >= 2:
...
TD_CAKVSE = 15          # v32.4: field runs 14-16; 13 was the most turn-starved in the class
    ...
    for _attempt in (0, 1):   # large-JSON crawls intermittently return empty
        ...
        try:
            payload = await asyncio.wait_for(
                fetch_page(url, provider=TD_RWSDMF,
                           timeout=min(TD_PMRDAP, left - 6.0)),
                timeout=min(TD_PMRDAP, left - 6.0) + 4.0)
        except Exception:
            continue
        _td_tajytv(payload)
...
def _td_nfinhq(text: str) -> list[str]:
    ...
    return [w for w in TD_FTHHEC.findall((text or "").lower())
            if w not in TD_JYTWSJ]

TD_QQVUDJ = 30.0

TD_QNQHWD = "v52-pin-reviewed"

TD_YHHDBH = 12   # F8: '42 [3]' is a legitimate answer
...
TD_VGGJGT = 260     # context kept either side of a retained quote

async def _td_sehryw(question: str, answer: str, messages: list[dict],
                          ledger: EvidenceLedger, deadline: float) -> str:
    if deadline - monotonic() < 65.0 or _td_tukgzs() <= TD_EXPJUF:
        return answer
    if not ledger.rows:
...
TD_KUGQWC = re.compile(r"\b(?:as|in) a table\b|\btable format\b", re.IGNORECASE)

TD_PAGENW = "deepseek/deepseek-v3.2"  # lane A

TD_GDXXSD = 75.0

TD_RFZRMT = 400_000   # in-process only; never shipped, so it costs nothing
...
TD_UYNZBT = 280.0        # what we budget the finalize stage against
...
def _td_vuenev(preview: str, limit: int = 280) -> str:
    ...
    for chunk in re.split(r"(?<=[.!?])\s+|\n+", TD_TCAJTB.sub("", preview or "")):
        ...
        if TD_IZGFDQ.match(seg) and not re.search(r"\d", seg):
            if kept:
                broke = True
                break
            continue
...
async def _td_pyzqbv(query: Query, question: str) -> Response:
    TD_WEDRCY.clear()   # per-query reset
    ...
    try:
        if _td_tsfnqx(answer) and (deadline - monotonic()) > 75.0 \
                and _td_tukgzs() >= TD_MHZPXQ:
    ...
    answer = _td_ipfakn(answer)   # the judge reads THIS, not the ref list
    ...
    if query.output_schema is not None:
        ...
        if basis is not answer:
            try:
                salvaged = await _td_iwbitd(question, basis, query.output_schema,
                                                deadline)
            except Exception:
                salvaged = None
        ...
        try:
            ...
            try:
                return Response(output=_td_tkdguz(basis)[:2000],
...
TD_WATXNQ = re.compile(r"\[slice \d+:\d+\]|https?://\S+")
...
TD_HMGHMM = re.compile(r"\bone per line\b|\beach on (?:a|its own) (?:new )?line\b",
                            re.IGNORECASE)

TD_FMATMA = 3   # v32.4: show the top-K disjoint regions, not just one

TD_EIAIQJ = 6500     # small pages render whole
...
TD_MXEBHT = 6   # +2: premises are retained alongside answer evidence
...
def _td_ehwunq(question: str) -> bool:
    ...
    if m and m.group(1).lower() not in TD_MWSIEZ:
        if not _td_ysbjns(q) or re.search(r"\b(?:all|every|each)\b", q, re.IGNORECASE):
            return True
    # multi-criteria phrasing ("that X and also Y") usually means a filtered SET
    return bool(re.search(r"\bwhich\b", q, re.IGNORECASE)) and bool(TD_KXENJQ.search(q))

TD_QJXRSW = re.compile(r"^(?P<a>[^()]{2,60}?)\s*\((?P<b>[^()]{2,60})\)$")
...
async def _td_ttvpxb(query_text: str, ledger: EvidenceLedger):
    ...
    for attempt, allow_repeat in ((query_text, False), (query_text, True),
                                  (_td_kdbxmj(query_text), False)):
        if not attempt.strip() or (attempt in fired and not allow_repeat):
            continue
        fired.add(attempt)
        try:
            payload = await search_web(attempt, provider=TD_RWSDMF, num=8,
                                       timeout=TD_WSSXKB)
    ...
    for item in results:
        ...
        n_len = len(note)
        span = ([(0, min(max(TD_NKIRBY, 100), n_len))] if n_len >= 100
                else ([(0, n_len)] if n_len else None))
        title = (getattr(item, "title", None) or "").strip()
        url = (getattr(item, "url", None) or "").strip()
        rows.append({"receipt_id": receipt, "result_id": rid, "note_len": n_len,
                     "kind": "search", "spans": span, "title": title, "url": url,
                     "preview": note[:TD_NKIRBY], "text": note})
...
async def _td_sfhtbq(call, question: str, ledger: EvidenceLedger, deadline: float) -> str:
    ...
    if name == "read_page":
        return await _td_zgscnh(str(args.get("url") or ""), str(args.get("focus") or ""),
                               question, ledger)
    if name == "retain_evidence":
        return _td_bywmjt(str(args.get("source") or ""),
                                   str(args.get("quote") or ""), ledger)
    if name == "page_grep":
        return _td_cagfce(str(args.get("url") or ""),
                             str(args.get("pattern") or ""), ledger)
    if name == "page_read":
        return _td_sztyqs(str(args.get("url") or ""),
                             args.get("offset") or 0,
                             args.get("length") or TD_TJGMPS, ledger)
    if name == "sec_filing":
        return await _td_mredvs(str(args.get("company") or ""),
                                    str(args.get("form") or ""),
                                    str(args.get("year") or ""), deadline)
    return f"# unknown tool {name!r}"

def _td_sivkgc(head: str) -> bool:
    return TD_GGRGWR.search(head or "") is not None and \
        TD_PSJRTU.search(head or "") is not None

TD_MVEVGN = 2

async def _td_vffmxz(question: str, answer: str, messages: list[dict],
                       ledger: EvidenceLedger, deadline: float) -> str:
    probe = (
    ...
    try:
        raw = await _td_mncsau(TD_XSFKUX, TD_ZTQMBI,
                                 "Strict completeness auditor. JSON only.",
                                 probe, max_tokens=2200,
                                 timeout=max(8.0, min(TD_FVTESB,
...
async def _td_mredvs(company: str, form: str, year: str, deadline: float) -> str:
    ...
    accession, doc = pick
    url = TD_WHSSMD.format(cik=cik10.lstrip("0") or cik10,
                              accession=accession.replace("-", ""), doc=doc)
    return (f"# sec_filing -> {title} {form} {year or '(latest)'} primary document:\n"
            f"{url}\nNow call read_page on this URL with a focus hint for the "
            f"section you need, and cite figures from that read_page result.")

TD_WUQKMX = re.compile(r"^\s*(?:[-*\u2022]|\d+[.)])\s+(\S.*)$", re.MULTILINE)
...
TD_BNYZFF = re.compile(r"[.!?]\s|[.!?]$|\b(?:is|was|were|are|has|have|had|"
...
TD_KYZGBA = re.compile(r"^\s*(?:best-effort answer unavailable|no question provided)", re.I)

TD_UBCBYC = "openrouter"          # fallback lane -- openrouter only, different MODEL
...
TD_EWNRGW = 266.0        # 2026-07-31: 262 -> 266. The platform hard kill is 270

TD_ZTQMBI = "openai/gpt-oss-120b"      # lane A

TD_KWVCTJ = re.compile(r"\b([a-z]{3,})est\b")   # NO IGNORECASE: proper

TD_SKQEWM = re.compile(r"\bcomma[- ]separated\b", re.IGNORECASE)
...
TD_YKHGTE = 3000       # restored: every build v32.0->v33.8, including the

async def _td_mncsau(lane: str, model: str, system: str, user: str, *,
...
TD_TCAJTB = re.compile(r"\[\s*\d{1,3}\s*\]")

TD_RGXHPQ = re.compile(r"(?<!\]\()https?://")
...
TD_JSEGDF = "openai/gpt-oss-120b"     # lane A
...
TD_BCDJJY = 250.0   # past this, do the free pass only -- never start work

TD_EAHWHI = re.compile(r"\b(?:which|what)\b(?:\s+\w+){0,2}?\s+([a-z]{3,}s)\b", re.IGNORECASE)

TD_UAJYDS = re.compile(r"[a-z0-9][a-z0-9'.\-]{2,}")
...
TD_HIQJMS: dict = {}           # url -> parsed JSON (tickers is ~10MB; fetch once)
...
async def _td_hnnzvg(question: str) -> tuple[str, str]:
    ...
    raw = ""
    try:
        raw = await _td_mncsau(TD_XSFKUX, TD_BKIJDD, system, user,
                                 max_tokens=2400, timeout=TD_PYKQIH,
                                 think=_least_think(TD_XSFKUX, TD_BKIJDD))
    except Exception:
        try:
            raw = await _td_mncsau(TD_UBCBYC, TD_WXFBRD, system, user,
                                     max_tokens=2400, timeout=TD_PYKQIH,
                                     think=_least_think(TD_UBCBYC, TD_WXFBRD))
        except Exception:
            raw = ""
    if not raw:
        return "", ""
...
async def _td_zgscnh(url: str, focus: str, question: str, ledger: EvidenceLedger) -> str:
    ...
    for _attempt in (0, 1):
        ...
        try:
            ...
            if "Timeout" not in _why:
                break
    if payload is None:
        return _td_rmxhus(url, f"# read_page({url!r}) failed ({_why}). This URL returns no "
    ...
    if len(note) <= TD_EIAIQJ:
        ...
        return ToolOutput(f"# read_page({url!r}) -> [{TD_WEETQJ.format(0)}] full page, "
                          f"{len(note)} chars\n{note}", [row])
    ...
    row = {"receipt_id": receipt, "result_id": rid, "note_len": len(note),
           "kind": "fetch", "spans": [(0, TD_YKHGTE)] + list(windows),
           "title": url, "url": url,
           "preview": note[windows[0][0]:windows[0][0] + 1200], "text": note}
    head = note[:TD_YKHGTE]
    sections = "".join(
        f"\n--- section @{s} ---\n{note[s:e]}" for s, e in windows)
    return ToolOutput(f"# read_page({url!r}) -> [{TD_WEETQJ.format(0)}] {len(note)} chars total; head + "
            f"the {len(windows)} most relevant section(s) shown "
            f"({', '.join(f'{s}-{e}' for s, e in windows)}). If the answer set may "
            f"continue elsewhere in this page, call read_page again with a "
            f"different focus.\n--- head ---\n{head}{sections}", [row])

TD_TTXKIJ = frozenset("name list give tell show find identify please could would "
