...
# ── v0805-c3 Parallel page prefetch (approvement_idea #3) ─────────────────────
# Overlay patches _solve to prefetch brief URLs in parallel BEFORE the loop.
# Refine rung: after baseline, parallel-fetch a fresh candidate URL set and
# produce an alternate cited answer — a second answer-production path.
...
# Mutable cell: holds the last brief WITHOUT needing a `global` rebind
# (the upload subset rejects `global`). Mutation is visible module-wide.
...
def _v0805_c3_extract_urls(*blobs: str) -> list[str]:
    ...
    for blob in blobs:
        for m in _V0805_URL_RE.finditer(blob or ""):
            u = m.group(0).rstrip(".,;:)")
            low = u.lower()
            if any(b in low for b in ("google.com/search", "bing.com/search",
                                      "duckduckgo.com", "youtube.com/watch")):
                continue
            if u not in found:
                found.append(u)
...
async def _v0805_c3_parallel_fetch(urls: list[str], question: str,
...
async def _solve(query: Query, question: str) -> Response:
    ...
    try:
        ...
        answer = ""
    try:
        if _is_usable_answer(answer) and (deadline - _v0805_clock()) > 75.0 \
                and _spend_left() >= AUDIT_MIN_USD:
            patched = await _audit_patch(question, answer, messages, ledger, deadline)
            if _is_usable_answer(patched):
                answer = patched
    ...
    if query.output_schema is not None:
        ...
        if structured is not None:
            try:
                ...
                pass
            # Reject polluted LLM schema packs (same a99f failure mode).
        ...
        try:
            ...
            pass
        # Last resort: entity-only single-field coerce, never the proof essay.
        try:
...
def _v0805_install_overlay() -> None:
    ...
    `_solve` IS the c3 prefetch solver (the base is _solve_orig), so there is
    nothing to swap at runtime.
    """
    return


async def _v0805_c3_candidate_urls(question: str, baseline: Response,
                                   deadline: float) -> list[str]:
    """Discover URLs for the refine parallel fan-out (search + harvest)."""
    ...
    for seed in _seed_queries(question, _needs_set_completeness(question))[:2]:
        if (deadline - _v0805_clock()) < 40.0:
            break
        try:
            out = await asyncio.wait_for(_do_search(seed, tmp),
...
async def _v0805_refine(query: Query, baseline: Response, deadline: float) -> Response | None:
    ...
    if not _v0805_is_thin(baseline):
        # Parallel refine wins ties on citation density.
        base_txt = _v0805_response_text(baseline)
        base_cites = len(_v0805_re.findall(r"\[\d+\]", base_txt))
        new_cites = len(_v0805_re.findall(r"\[\d+\]", text))
