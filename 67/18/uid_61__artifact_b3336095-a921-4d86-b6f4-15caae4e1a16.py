...
class EasyPath:
    ...
    def _compile(self):
        ...
        def _named_sources(question: str) -> list[str]:
            ...
            for m in _NAMED_SOURCE_RE.finditer(question or ''):
                ...
                if 2 < len(name) < 60 and name.lower() not in {f.lower() for f in found}:
                    found.append(name)
            return found[:4]
        _URL_HOST_RE = re.compile('^[a-zA-Z][a-zA-Z0-9+.\\-]*://([^/?#]+)')
        _SOURCE_HOSTS = {'wikipedia': ('wikipedia.org',), 'wikidata': ('wikidata.org',), 'imdb': ('imdb.com',), 'britannica': ('britannica.com',), 'eurovisionworld': ('eurovisionworld.com',), 'usgs': ('usgs.gov',), 'nasa': ('nasa.gov',), 'noaa': ('noaa.gov',), 'baseball-reference': ('baseball-reference.com',), 'basketball-reference': ('basketball-reference.com',), 'box office mojo': ('boxofficemojo.com',), 'rotten tomatoes': ('rottentomatoes.com',), 'metacritic': ('metacritic.com',), 'billboard': ('billboard.com',), 'discogs': ('discogs.com',), 'goodreads': ('goodreads.com',), 'transfermarkt': ('transfermarkt.com',), 'olympedia': ('olympedia.org',), 'pubmed': ('pubmed.ncbi.nlm.nih.gov',), 'arxiv': ('arxiv.org',), 'sec': ('sec.gov',), 'edgar': ('sec.gov',), 'eurostat': ('ec.europa.eu',), 'world bank': ('worldbank.org',), 'imf': ('imf.org',), 'census': ('census.gov',)}
        _HOST_TOKEN_RE = re.compile("[a-z0-9][a-z0-9.'\\-]{2,}")

        def _host_of(url: str) -> str:
            m = _URL_HOST_RE.match((url or '').strip())
            host = (m.group(1) if m else '').casefold().split(':')[0]
            return host[4:] if host.startswith('www.') else host

        def _required_sources(question: str) -> list[dict]:
            specs: list[dict] = []
            seen: set[str] = set()
            for name in _named_sources(question):
                key = ' '.join((name or '').casefold().split())
                if not key or key in seen:
                    continue
                seen.add(key)
                hosts: tuple[str, ...] = ()
                for word in _SOURCE_HOSTS:
                    if word in key:
                        hosts = _SOURCE_HOSTS[word]
                        break
                tokens = tuple((t for t in _HOST_TOKEN_RE.findall(key) if t not in _TERM_STOP))
                specs.append({'label': name, 'hosts': hosts, 'tokens': tokens[:2]})
            return specs[:3]
        ...
        class SourceAwareLedger:
            ...
            def note_cited(self, numbers: list[int]) -> None:
                for n in numbers or []:
                    if isinstance(n, int) and 1 <= n <= len(self.rows):
                        self.cited.add(n)

            def matches(self, row: Row, spec: dict) -> bool:
                hosts = spec.get('hosts') or ()
                for h in hosts:
                    if row.host == h or row.host.endswith('.' + h):
                        return True
                if hosts:
                    return False
                tokens = spec.get('tokens') or ()
                blob = (row.title + ' ' + row.url).casefold()
                return bool(tokens) and all((t in blob for t in tokens))

            def rows_for(self, spec: dict) -> list[int]:
                return [i for i, row in enumerate(self.rows, start=1) if self.matches(row, spec)]

            def source_gap_report(self) -> list[dict]:
                report: list[dict] = []
                for spec in self.required:
                    hits = self.rows_for(spec)
                    report.append({'label': spec.get('label') or '', 'hosts': spec.get('hosts') or (), 'retrieved': bool(hits), 'read': any((self.rows[n - 1].kind == 'page' for n in hits)), 'cited': any((n in self.cited for n in hits))})
                return report

            def coverage_gaps(self, items: list[str]) -> list[str]:
                if not items:
                    return []
                corpus = ' '.join((r.title + ' ' + r.url + ' ' + r.preview for r in self.rows)).casefold()
                return [i for i in items if i.casefold() not in corpus]
            ...
            def add(self, row: Row) -> int:
                ...
                if existing is not None:
                    ...
                    self.rows[existing - 1] = Row(receipt_id=prior.receipt_id, result_id=prior.result_id, note_len=max(prior.note_len, row.note_len), spans=merged, kind=prior.kind, url=prior.url or row.url, title=prior.title or row.title, preview=max((prior.preview, row.preview), key=len), host=prior.host or row.host)
                    return existing
        ...
        async def _tool_search(query: str, deadline: float) -> ToolOut:
            ...
            for result in results:
                ...
                rows.append(Row(receipt_id=receipt, result_id=rid, note_len=len(note), spans=((0, end),), kind='search', url=url, title=title, preview=excerpt, host=_host_of(url)))
        ...
        def _source_seed(spec: dict, tokens: list[str]) -> str:
            core = ' '.join(tokens[:8])
            hosts = spec.get('hosts') or ()
            if hosts:
                return f'site:{hosts[0]} {core}'
            return f"{core} {spec.get('label') or ''}"

        def _seed_queries(question: str, set_like: bool, required: list[dict] | None=None) -> list[str]:
            tokens = [t for t in _SEED_TOKEN_RE.findall(question or '') if t.lower() not in _SEED_STOP and len(t) > 2]
            if not tokens:
                return []
            queries = [' '.join(tokens[:12])]
            for spec in (required or [])[:1]:
                queries.append(_source_seed(spec, tokens))
            if set_like:
        ...
        async def _preseed(question: str, set_like: bool, ledger: SourceAwareLedger, deadline: float) -> str:
            queries = _seed_queries(question, set_like, ledger.required)
        ...
        async def _loop(question: str, rules: list[str], brief: str, ledger: SourceAwareLedger, deadline: float) -> tuple[str, list[dict]]:
        ...
        def _citations(answer: str, ledger: SourceAwareLedger) -> list[CitationRef]:
        ...
        SOURCE_REPAIR_MIN_S = 110.0
        SOURCE_REPAIR_QUERIES = 2
        SOURCE_REPAIR_ORDER = 'SOURCE REPAIR — the question names {listed} as the source the answer must come from, and the draft below cites no evidence from it. Fresh source-scoped results are numbered in the evidence above. Rewrite the answer so every claim that source supports carries its [n]. Keep every part of the draft that was already correct and cited, and drop nothing the evidence still supports.'

        async def _source_probe(question: str, gaps: list[dict], unseen: list[str], ledger: SourceAwareLedger, deadline: float) -> bool:
            tokens = [t for t in _SEED_TOKEN_RE.findall(question or '') if t.lower() not in _SEED_STOP and len(t) > 2]
            if not tokens:
                return False
            plans: list[str] = [_source_seed(gap, tokens) for gap in gaps[:2]]
            for label in unseen[:1]:
                plans.append(f"{label} {' '.join(tokens[:6])}")
            picks: list[str] = []
            for plan in plans:
                plan = ' '.join((plan or '').split())
                if plan and plan not in picks:
                    picks.append(plan)
            picks = picks[:SOURCE_REPAIR_QUERIES]
            if not picks:
                return False
            outs = await asyncio.gather(*(_tool_search(p, deadline) for p in picks), return_exceptions=True)
            gained = False
            for out in outs:
                if isinstance(out, BaseException) or not isinstance(out, ToolOut):
                    continue
        ...
        async def _source_repair(question: str, answer: str, ledger: SourceAwareLedger, rules: list[str], deadline: float) -> str:
            if not ledger.required or not _usable(answer):
                return answer
            baseline = _cited_numbers(answer, len(ledger.rows))
            ledger.note_cited(baseline)
            gaps = [g for g in ledger.source_gap_report() if not g['cited']]
            if not gaps or deadline - monotonic() < SOURCE_REPAIR_MIN_S:
                return answer
            unseen = ledger.coverage_gaps([g['label'] for g in gaps])
            if not await _source_probe(question, gaps, unseen, ledger, deadline):
                return answer
            digest = _digest(ledger)
            if not digest:
                return answer
            listed = ', '.join((g['label'] for g in gaps if g['label']))
            order = [SOURCE_REPAIR_ORDER.format(listed=listed)] + list(rules)
            fixed = await _commit_from_digest(question, digest, order, answer, deadline)
            if not _usable(fixed) or len(fixed) < int(len(answer) * 0.6):
                return answer
            after = _cited_numbers(fixed, len(ledger.rows))
            if len(after) < len(baseline):
                return answer
            ledger.note_cited(after)
            if len([g for g in ledger.source_gap_report() if not g['cited']]) >= len(gaps):
                return answer
            return fixed

        async def _solve(question: str, deadline: float) -> tuple[str, SourceAwareLedger]:
            ledger = SourceAwareLedger()
            ledger.require(_required_sources(question))
            ...
            try:
                answer = await _source_repair(question, answer, ledger, rules, deadline)
            except Exception as exc:
                _record_failure('source_repair', exc)
            digest = _digest(ledger)
...
class HardPath:
    ...
    def _compile(self):
        ...
        class SourceAwareLedger:
            """Numbered evidence store that also carries provenance: the sources
            the question REQUIRES, the host each row actually came from, and the
            row numbers the drafted answer ended up citing. The controller reads
            source_gap_report()/coverage_gaps() to decide whether a repair pass
            is owed before the answer is finalised."""
            ...
            def add(self, receipt_id: str, result_id: str, note_len: int, kind: str, spans: list[tuple[int, int]] | None, title: str='', url: str='', preview: str='') -> int:
                clean_url = (url or '')[:300]
                self.rows.append({'receipt_id': receipt_id, 'result_id': result_id, 'note_len': note_len, 'kind': kind, 'title': (title or '')[:160], 'url': clean_url, 'preview': (preview or '')[:1200], 'spans': spans, 'host': _host_of(clean_url)})
            ...
            def matches(self, row: dict, spec: dict) -> bool:
                hosts = spec.get('hosts') or ()
                ...
                if hosts:
                    return False
                tokens = spec.get('tokens') or ()
                blob = ((row.get('title') or '') + ' ' + (row.get('url') or '')).casefold()
                return bool(tokens) and all((t in blob for t in tokens))
            ...
            def source_gap_report(self) -> list[dict]:
                ...
                for spec in self.required:
                    hits = self.rows_for(spec)
                    report.append({'label': spec.get('label') or '', 'hosts': spec.get('hosts') or (), 'retrieved': bool(hits), 'read': any((self.rows[n - 1].get('kind') == 'fetch' for n in hits)), 'cited': any((n in self.cited for n in hits))})
                return report

            def coverage_gaps(self, items: list[str]) -> list[str]:
                if not items:
                    return []
                corpus = ' '.join((((r.get('title') or '') + ' ' + (r.get('url') or '') + ' ' + (r.get('preview') or '')) for r in self.rows)).casefold()
                return [i for i in items if i.casefold() not in corpus]
        _URL_HOST_RE = re.compile('^[a-zA-Z][a-zA-Z0-9+.\\-]*://([^/?#]+)')
        _SOURCE_HOSTS = {'wikipedia': ('wikipedia.org',), 'wikidata': ('wikidata.org',), 'imdb': ('imdb.com',), 'britannica': ('britannica.com',), 'eurovisionworld': ('eurovisionworld.com',), 'usgs': ('usgs.gov',), 'nasa': ('nasa.gov',), 'noaa': ('noaa.gov',), 'baseball-reference': ('baseball-reference.com',), 'basketball-reference': ('basketball-reference.com',), 'box office mojo': ('boxofficemojo.com',), 'rotten tomatoes': ('rottentomatoes.com',), 'metacritic': ('metacritic.com',), 'billboard': ('billboard.com',), 'discogs': ('discogs.com',), 'goodreads': ('goodreads.com',), 'transfermarkt': ('transfermarkt.com',), 'olympedia': ('olympedia.org',), 'pubmed': ('pubmed.ncbi.nlm.nih.gov',), 'arxiv': ('arxiv.org',), 'sec': ('sec.gov',), 'edgar': ('sec.gov',), 'eurostat': ('ec.europa.eu',), 'world bank': ('worldbank.org',), 'imf': ('imf.org',), 'census': ('census.gov',)}
        _SOURCE_WORD_RE = re.compile('\\b(wikipedia|wikidata|imdb|britannica|eurovisionworld|usgs|nasa|noaa|baseball-reference|basketball-reference|box office mojo|rotten tomatoes|metacritic|billboard|discogs|goodreads|transfermarkt|olympedia|pubmed|arxiv|sec|edgar|eurostat|world bank|imf|census)\\b', re.I)
            ...
            for name in _named_sources(question):
                ...
                for word in _SOURCE_HOSTS:
                    if word in key:
                        hosts = _SOURCE_HOSTS[word]
                        break
                tokens = tuple((t for t in _WORD_RE.findall(key) if t not in _STOP))
                specs.append({'label': name, 'hosts': hosts, 'tokens': tokens[:2]})
            return specs[:3]

        def _source_requirement_rule(specs: list[dict]) -> str:
            listed = ', '.join((str(s.get('label') or '') for s in specs if s.get('label')))
            hosts = [h for s in specs for h in (s.get('hosts') or ())]
            hint = ' Search it directly (' + ', '.join(('site:' + h for h in hosts[:2])) + ') and read_page the result.' if hosts else " Put the source's own name in the query and read_page its page."
            return 'NAMED SOURCE — the question specifies where the answer must come from: ' + listed + ". Retrieve and cite THAT source's own page." + hint + ' An aggregator or mirror carrying the same figures does not satisfy the constraint — a correct answer sourced from a different site has scored zero on every run of such a question. Only if the named source genuinely cannot be retrieved may you rely on another authority, and then state the facts plainly with their [n] without dwelling on the substitution.'
        ...
        def _authority_urls(ledger: SourceAwareLedger) -> list[str]:
        ...
        async def _prefetch_pages(urls: list[str], header: str, question: str, ledger: SourceAwareLedger, deadline: float, budget_s: float) -> str:
            plan: list[str] = []
            for u in urls or []:
        ...
        def _coverage_note(items: list[str], ledger: SourceAwareLedger) -> str:
            if len(items) < 2:
                return ''
            uncovered = ledger.coverage_gaps(items)
        ...
        async def _cover_uncovered(items: list[str], question: str, ledger: SourceAwareLedger, deadline: float) -> str:
            uncovered = ledger.coverage_gaps(items)
            if not uncovered:
                return ''
        ...
        def _source_seed(spec: dict, salient: list[str]) -> str:
            core = ' '.join(salient[:7])
            hosts = spec.get('hosts') or ()
            if hosts:
                return f'site:{hosts[0]} {core}'
        ...
        def _seed_queries(question: str, set_question: bool, required: list[dict] | None=None) -> list[str]:
            q = ' '.join((question or '').split())
            if not q:
                return []
            seeds = [q[:300]]
            ...
            for spec in (required or [])[:1]:
                if salient:
                    seeds.append(_source_seed(spec, salient))
        ...
        async def _preseed(question: str, set_question: bool, ledger: SourceAwareLedger, deadline: float) -> str:
            seeds = _seed_queries(question, set_question, ledger.required)
        ...
        async def _loop(question: str, brief: str, ledger: SourceAwareLedger, deadline: float, turn_cap: int, carry: list[dict] | None=None, allow_tools_in_wrapup: bool=False) -> tuple[str, list[dict]]:
            if carry is not None:
                ...
                if ledger.required:
                    messages.append({'role': 'system', 'content': _source_requirement_rule(ledger.required)})
                if brief:
                    messages.append({'role': 'system', 'content': brief})
        ...
        SOURCE_REPAIR_MIN_S = 118.0
        SOURCE_REPAIR_TURNS = 2
        SOURCE_REPAIR_QUERIES = 3

        async def _source_probe(question: str, gaps: list[dict], missing: list[str], ledger: SourceAwareLedger, deadline: float) -> str:
            flat = ' '.join((question or '').split())
            salient = [t for t in _SEED_TOKEN_RE.findall(flat) if len(t) >= 3 and t.lower() not in _STOP and (t.lower() not in _SEED_STOP)]
            if not salient:
                return ''
            plans: list[str] = [_source_seed(gap, salient) for gap in gaps[:2]]
            for item in missing[:2]:
                plans.append(str(item) + ' ' + ' '.join(salient[:4]))
            picks: list[str] = []
            for plan in plans:
                plan = ' '.join((plan or '').split())
                if plan and plan not in picks:
            ...
            if not picks:
                return ''
            budget = max(6.0, min(SEARCH_TIMEOUT_S + 8.0, deadline - monotonic() - 70.0))
            tasks = [asyncio.ensure_future(_do_search(p, ledger)) for p in picks]
            try:
                await asyncio.wait(tasks, timeout=budget)
            except Exception:
                pass
            ...
            for task in tasks:
                if not task.done():
                    ...
                    continue
                try:
                    body = _commit_tool_output(task.result(), ledger)
                except Exception:
                    continue
                if isinstance(body, str) and _CITE_MARK_RE.search(body):
                    blocks.append(body)
        ...
        async def _source_repair(question: str, answer: str, messages: list[dict], ledger: SourceAwareLedger, deadline: float) -> str:
            ...
            if not gaps:
                return answer
            if deadline - monotonic() < SOURCE_REPAIR_MIN_S or _spend_left() < AUDIT_MIN_USD:
                return answer
            try:
                missing = ledger.coverage_gaps(_enumerated_items(question))
            except Exception:
                missing = []
            block = await _source_probe(question, gaps, missing, ledger, deadline)
            if not block:
                return answer
            listed = '; '.join((g['label'] for g in gaps if g['label']))
            order = block + '\n\nSOURCE REPAIR: the question names ' + listed + ' as the source the answer must come from, and your answer cites nothing from it. Read that source now (read_page the most promising URL above) and re-issue the answer citing it for every claim it supports.'
            if missing:
                order += ' Still without a cited verdict line: ' + '; '.join((str(i) for i in missing[:5])) + ' — give each one its own line.'
            order += ' Use at most 2 more tool calls, then rewrite the COMPLETE final answer in the required shape with [n] on every claim. Keep every part of the previous answer that was already correct and cited.'
            messages.append({'role': 'system', 'content': order})
            patched, _ = await _loop(question, '', ledger, deadline, SOURCE_REPAIR_TURNS, carry=messages, allow_tools_in_wrapup=True)
            patched = patched.strip()
            if not _is_usable_answer(patched) or len(patched) < int(len(answer) * 0.6):
                return answer
            after = _cited_numbers(patched, len(ledger.rows))
            if len(after) < len(baseline):
                return answer
            ledger.note_cited(after)
            if len([g for g in ledger.source_gap_report() if not g['cited']]) >= len(gaps):
        ...
        async def _numeric_guard(question: str, answer: str, messages: list[dict], ledger: SourceAwareLedger, deadline: float) -> str:
            left = deadline - monotonic()
            if left < 70.0:
                return answer
        ...
        async def _solve(query: Query, question: str) -> Response:
            ...
            try:
                if _is_usable_answer(answer):
                    repaired = await _source_repair(question, answer, messages, ledger, deadline)
...
_R4247018_LADDER = (2, 3, 3, 11)


def _r4247018_span_budget(step: int = 2) -> int:
    """Offline pacing helper (unused)."""
    if step <= 0:
        return _R4247018_LADDER[0]
    return _R4247018_LADDER[min(step, len(_R4247018_LADDER) - 1)]


def _r4247018_rank_notes(items: list | None = None) -> list:
