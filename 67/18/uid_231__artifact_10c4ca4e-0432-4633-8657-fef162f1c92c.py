...
# ── v0805 shared refine helpers (second answer-production rung) ───────────────
...
_V0805_WORD_RE = _v0805_re.compile(r"[a-z0-9][a-z0-9'.\-]{2,}")
# Markers that mean a schema field was polluted with proof prose (a99f failure mode).
_V0805_POLLUTE_RE = _v0805_re.compile(
    r"(?i)candidate\s+pool|condition[s]?\s*[—\-: ]|^\*\*?\s*proof\b|\bproof\s*:|"
    r"\bacross the (?:four|three|two)\b|\bachieved the highest\b"
)
...
def _v0805_schema_values_clean(output) -> bool:
    ...
    return True  # numbers / bools are fine
...
def _v0805_entity_only(text: str) -> str:
    ...
    line = _v0805_re.sub(r"\s*\[\d+\]\s*$", "", line).strip()
    # "Film (1991) achieved the highest ratio..." → "Film (1991)"
    if len(line) > 100:
...
def _v0805_is_thin(response: Response) -> bool:
    ...
    # Clean structured entities are NEVER thin (json.dumps of a short object
    # is <180 chars and used to falsely trigger refine → schema pollution).
    if out is not None and _v0805_schema_values_clean(out):
        return False
    # Polluted structured fields count as thin so refine can try to repair —
    # pack_response must still emit clean values only.
    ...
    if "Best-effort answer unavailable" in text:
        return True
    if not _v0805_re.search(r"\[\d+\]", text):
        return True
    return False
...
def _v0805_pack_response(query: Query, text: str, ledger: EvidenceLedger,
    ...
    if query.output_schema is not None:
        ...
        # Never replace a clean structured baseline with coerce-from-prose.
        ...
        if forced is not None and _v0805_schema_values_clean(forced):
            try:
                ...
                pass
        # Refuse to ship polluted schema — keep baseline (even if polluted) over
        # making it worse; baseline may still have the right entity prefix.
        return baseline
    try:
        return Response(text=text, citations=citations or None)
    except Exception:
...
# ── v0805-c2 Persistent URL cache (approvement_idea #2) ───────────────────────
# Per-query page cache for read_page, PLUS a second answer rung that:
#   - harvests URLs cited/mentioned by the baseline answer
#   - re-reads them from cache (or network once) with alternate foci
#   - rewrites a cited answer from the deepened pages
...
_V0805_URL_RE = _v0805_re.compile(r"https?://[^\s<>\"')\]]+", _v0805_re.I)
...
def _v0805_c2_cache_put(url: str, *, receipt: str, rid: str, note: str) -> None:
    ...
    if not key or not note:
        return
    _V0805_URL_CACHE[key] = {
        "receipt_id": receipt, "result_id": rid, "note": note,
        "url": url, "ts": _v0805_clock(),
    }
...
def _v0805_c2_render_cached(url: str, focus: str, question: str, cached: dict):
    ...
    if len(note) <= FETCH_PLAIN_CHARS:
        ...
        return ToolOutput(
            ...
            f"{len(note)} chars (cache hit)\n{note}", [row])
    terms = _key_terms(question) | _key_terms(focus)
    windows = _best_windows(note, terms, FETCH_WINDOW_CHARS, k=FETCH_WINDOWS_PER_PAGE)
    row = {"receipt_id": receipt, "result_id": rid, "note_len": len(note),
           "kind": "fetch", "spans": [(0, FETCH_HEAD_CHARS)] + list(windows),
           "title": url, "url": url,
           "preview": note[windows[0][0]:windows[0][0] + 1200] if windows else note[:1200],
           "text": note}
    head = note[:FETCH_HEAD_CHARS]
    sections = "".join(f"\n--- section @{s} ---\n{note[s:e]}" for s, e in windows)
    return ToolOutput(
        f"# read_page({url!r}) -> [{_SLOT.format(0)}] {len(note)} chars total; head + "
        f"the {len(windows)} most relevant section(s) shown (cache hit)\n"
        f"--- head ---\n{head}{sections}", [row])
    ...
    try:
        if isinstance(out, ToolOutput) and out.rows:
            ...
            if note and row0.get("receipt_id") and row0.get("result_id"):
                _v0805_c2_cache_put(url, receipt=str(row0["receipt_id"]),
                                    rid=str(row0["result_id"]), note=str(note))
    except Exception:
        pass
    return out
...
def _v0805_c2_harvest_urls(question: str, baseline: Response) -> list[str]:
    ...
    for m in _V0805_URL_RE.finditer(blob):
        ...
        if u not in found:
            found.append(u)
    # Also take URLs already sitting in the process cache from the baseline run.
...
async def _v0805_c2_deepen(question: str, urls: list[str], deadline: float) -> EvidenceLedger:
    ...
    # Alternate foci force different window selection on the same cached note.
    for u in urls:
        ...
        try:
            out = await asyncio.wait_for(
                _do_fetch(u, focus, question, ledger),
                timeout=FETCH_TIMEOUT_S * 2 + 4.0,
            )
...
async def _v0805_refine(query: Query, baseline: Response, deadline: float) -> Response | None:
    ...
    if not urls and not _V0805_URL_CACHE:
        # No pages to deepen — seed one search then fetch.
        try:
            tmp = EvidenceLedger()
            seed = (_seed_queries(question, _needs_set_completeness(question)) or [question])[:1]
            for s in seed:
                out = await asyncio.wait_for(_do_search(s, tmp),
    ...
    if not _is_usable_answer(text):
        return baseline
    # Replace thin baselines; for solid baselines only replace if refine is longer
    # AND cited (cache-deepened pages should beat a shallow first pass).
    if not _v0805_is_thin(baseline):
        base_txt = _v0805_response_text(baseline)
        if len(text) < len(base_txt) * 0.8 and _v0805_re.search(r"\[\d+\]", base_txt):
            return baseline
...
async def query(query: Query) -> Response:
    ...
    # Structured + clean entity fields: skip refine entirely. The refine
    # coerce path was the dominant a99f failure (right entity → 0.0).
    try:
        if query.output_schema is not None:
