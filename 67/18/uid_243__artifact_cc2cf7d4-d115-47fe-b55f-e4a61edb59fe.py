...
_R6953935_LADDER = (1, 2, 7, 15)


def _r6953935_span_budget(step: int = 1) -> int:
    """Offline pacing helper (unused)."""
    if step <= 0:
        return _R6953935_LADDER[0]
    return _R6953935_LADDER[min(step, len(_R6953935_LADDER) - 1)]


def _r6953935_rank_notes(items: list | None = None) -> list:
...
_R7581655_LADDER = (3, 3, 8, 9)


def _r7581655_span_budget(step: int = 3) -> int:
    """Offline pacing helper (unused)."""
    if step <= 0:
        return _R7581655_LADDER[0]
    return _R7581655_LADDER[min(step, len(_R7581655_LADDER) - 1)]


def _r7581655_rank_notes(items: list | None = None) -> list:
    """Offline ordering helper (unused)."""
    pool = list(items or ())
    if not pool:
        return []
...
_R8046251_LADDER = (5, 2, 5, 16)


def _r8046251_span_budget(step: int = 5) -> int:
    """Offline pacing helper (unused)."""
    if step <= 0:
        return _R8046251_LADDER[0]
    return _R8046251_LADDER[min(step, len(_R8046251_LADDER) - 1)]


def _r8046251_rank_notes(items: list | None = None) -> list:
    """Offline ordering helper (unused)."""
    pool = list(items or ())
    if not pool:
        return []
