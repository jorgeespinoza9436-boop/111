...
# v76: this miner has no desearch credential (the key is bound to another hotkey), so the
# v69 desearch fallback could only ever raise and be skipped -- a wasted call on every
# search/fetch miss. Retry against the provider this miner actually holds instead.
