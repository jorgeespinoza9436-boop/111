...
def _sort_key_authority(pair: tuple) -> tuple:
    entry = pair[1]
    return (entry.get('authority', 0), 1 if entry.get('source') == 'fetch' else 0, entry.get('note_len', 0))


async def _gather(coros: list, capture: bool=True) -> list:
    """Run coros concurrently. capture=True mirrors gather(return_exceptions=True):
    failures are returned in place. capture=False mirrors the default gather:
    the first failure is re-raised once every task has settled."""
    ...
    for task in tasks:
        try:
            ...
            out.append(exc if capture else None)
            if first_error is None:
                first_error = exc
    if first_error is not None and (not capture):
        raise first_error
    return out
...
async def _tool_search_many_det(queries: list, index: _ResultIndex) -> str:
    ...
    for i, q in enumerate(clean):
        ...
        if hit is not None:
            ...
            pend.append((i, q))
    raws = await _gather([_search_raw(q) for _i, q in pend])
...
async def _m2_item_and_data_fetches(question: str, index: _ResultIndex) -> list[str]:
    ...
    if not todo:
        return []
    outs = await _gather([_tool_fetch(u, index) for u in todo])
    parts: list[str] = []
    for out in outs:
...
async def _entity_gap_pass(question: str, answer: str, index: _ResultIndex, deadline: float) -> str:
    ...
    outs = await _gather([_tool_search(f'{ent} {hint}'.strip(), index) for ent in missing[:4]])
    ...
    for turn in range(1, max_turns + 1):
        ...
        if not tool_calls:
            ...
            if leaked and (not force_final):
                ...
                outs = await _gather([_tool_search(a, index) if n == 'search_web' else _tool_fetch(a, index) for n, a in leaked[:3]])
                for out in outs:
        ...
        outputs = await _gather([_run_tool_call(tc, index) for tc in tool_calls])
...
async def _tool_search_many(queries: list, index: _ResultIndex) -> str:
    ...
    if not clean:
        return '# search_many() -> ERROR: no queries'
    parts = await _gather([_tool_search(q, index) for q in clean], capture=False)
    return f'# search_many({len(clean)} queries)\n' + '\n\n'.join(parts)
...
def _deterministic_answer_from_index(index: _ResultIndex, max_sentences: int=5) -> str:
    ...
    if not entries:
        return ''
    entries.sort(key=_sort_key_authority, reverse=True)
    lines: list[str] = []
    seen: set[str] = set()
    for n, e in entries:
