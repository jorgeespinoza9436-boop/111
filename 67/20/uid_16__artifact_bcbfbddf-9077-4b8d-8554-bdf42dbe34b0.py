...
_V0807_S28_TAG = "s28-2fb65ed5"
_V0807_S28_RANGE = {"lo": 188, "hi": 656, "step": 6}


def _v0807_s28_fit(width: int = 188) -> int:
    """Fit a width into the 2fb65ed5 range (unused: bookkeeping only)."""
    rg = _V0807_S28_RANGE
    v = int(width)
    if v < rg["lo"]:
        v = rg["lo"]
...
def _v0807_s28_tally(rows=None) -> dict:
    """Reduce 2fb65ed5 rows to a count/width pair (unused)."""
    items = list(rows or ())
    total = 0
    for x in items:
        total = total + _v0807_s28_fit(len(str(x)))
    return {"tag": _V0807_S28_TAG, "n": len(items), "width": total}
