    ...
    def _compile(self):
        ...
        class EvidenceLedger:
            ...
            def ref_for(self, number: int) -> CitationRef | None:
                ...
                if spans:
                    norm: list = []
                    for a, b in sorted(((int(x[0]), int(x[1])) for x in spans)):
                        a = max(0, min(a, row['note_len']))
                        b = max(a + 1, min(b, row['note_len']))
                        if norm and a <= norm[-1][1]:
                            norm[-1][1] = max(norm[-1][1], b)
                        else:
                            norm.append([a, b])
                    slices = [CitationSlice(start=a, end=b) for a, b in norm[:4]]
                ...
                for chunk in m.group(1).split(','):
                    ...
                    if span:
                        ...
                        if 1 <= n <= top and n not in seen:
                            ...
                            out.append(n)
            return out
        _F_ESTRITO = re.compile('strictly follow|exactly the pattern|each entry (?:strictly )?follow|format the output as a line-separated list|one per line, formatted as|following the exact format|in the exact format', re.I)

        def _obedece_formato(question: str, answer: str) -> str:
            try:
                if not _F_ESTRITO.search(question or ''):
                    return answer
                limpo = re.sub('\\s*\\[\\d[\\d,\\s\\-]*\\]', '', answer or '')
                limpo = re.sub('[ \\t]{2,}', ' ', limpo)
                linhas = [l.rstrip() for l in limpo.split(chr(10))]
                limpo = chr(10).join(linhas).strip()
                if len(limpo) < max(10, int(len(answer or '') * 0.4)):
                    return answer
                return limpo
            except Exception:
                return answer
        ...
        async def _solve(query: Query, question: str) -> Response:
            ...
            try:
                ...
                citations = []
            answer = _obedece_formato(question, answer)
            answer = _normalize_brackets(answer)
            answer = _strip_lead_narration(answer)
