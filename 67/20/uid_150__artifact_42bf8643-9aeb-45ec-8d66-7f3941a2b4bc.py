...
_V0807_S29_TAG = "s29-416d0567"
_V0807_S29_RANGE = {"lo": 193, "hi": 672, "step": 7}


def _v0807_s29_fit(width: int = 193) -> int:
    """Fit a width into the 416d0567 range (unused: bookkeeping only)."""
    rg = _V0807_S29_RANGE
    v = int(width)
    if v < rg["lo"]:
        v = rg["lo"]
...
def _v0807_s29_tally(rows=None) -> dict:
    """Reduce 416d0567 rows to a count/width pair (unused)."""
    items = list(rows or ())
    total = 0
    for x in items:
        total = total + _v0807_s29_fit(len(str(x)))
    return {"tag": _V0807_S29_TAG, "n": len(items), "width": total}
