...
stages and an explicitly phased orchestrator are layered on top:
  ...
  - single-provider LLM lanes: BOTH lanes are openrouter (this miner stores only
    openrouter + parallel credentials, and no gateway-style provider). Lane A is
...
NEW in v35 (STRUCTURAL HARDENING ONLY — no behavioural/logic change to any
stage; every phase, prompt, threshold and ladder rung is byte-identical in
intent, and the model/provider lanes stay openrouter-only):
  - PER-TASK STATE RESET (_reset_run_state): the worker process is reused across
    tasks, so the module-level spend reading from a PREVIOUS task could gate this
    task's brief/roster/audit phases before `tooling_info` re-primed it. Reset at
    entry; bound the EDGAR cache so it cannot grow without limit.
  - HARD WALL + SALVAGE (_RunState / _salvage_response): every phase used to write
    to LOCAL variables, so if any single await overshot the soft deadline the task
    returned NOTHING and scored a hard zero. Partial results now live in a
    _RunState the entrypoint owns; _solve runs under an absolute wait_for and a
    timeout still ships the best cited answer built so far (text or schema).
  - DEADLINE-AWARE TOOL I/O: _do_search (up to 3 attempts x 18s) and _do_fetch
    (2 x 16s) were deadline-BLIND — the one hazard this file's own header called
    out. Both now take the deadline, clamp each attempt to the real headroom, and
    stop retrying instead of running past the wall.
  - PLACEHOLDER-COLLISION GUARD (_clean): [n] numbering is carried through tool
    text as NUL-delimited sentinels. Provider-supplied page text is attacker-shaped
    and a stray NUL byte could corrupt the substitution and hence citation
    alignment. Every provider string is now NUL-scrubbed with a LENGTH-PRESERVING
    replacement, so ledger spans still index the server-side note exactly.
  - CANCELLATION HYGIENE: timed-out tool tasks were cancelled but never awaited,
    leaving orphan coroutines able to fire after the transcript was built.
Kill-safety: everything bounded by one deadline; force-commit well before it.
Validator-safety: no dunder attribute access, no dynamic getattr names, no
dynamically-selected callables, and no imports beyond the four the platform
allows — the tool router stays a static if/elif chain.
"""
...
# `parallel`. There is NO gateway-style provider key, so EVERY llm_chat must use
...
MIN_TAIL_S = 8.0
# v35: WALL_BUDGET_S is the SOFT deadline every phase negotiates against. This is
# the ABSOLUTE one the entrypoint enforces with wait_for: if any single await
# ignores its timeout, _solve is torn down here and the salvage path still ships
# a cited best-effort instead of the hard zero an overshoot used to cost.
AUDIT_EXTRA_TURNS = 2
...
# v35 PER-TASK RESET. The miner process is reused across tasks. `_SPEND` is
# module-level, so a task that ended with a drained reading left that reading in
# place for the NEXT task and silently suppressed its brief / roster / audit
# phases (each gated on `_spend_left()`), until `tooling_info` re-primed it in
# Phase 0 — after the first gate had already been evaluated in some paths.
# Clearing it restores the optimistic 1.0 default, and Phase 0's `tooling_info`
# immediately replaces that with the real figure.
def _reset_run_state() -> None:
    _SPEND["left"] = None
    # The EDGAR cache is deliberately KEPT across tasks (the tickers file is
    # ~10MB and re-fetching it is pure latency), but it must not grow forever in
    # a long-lived worker.
...
def _headroom(deadline, floor: float = 0.0) -> float:
    """Seconds of usable time left before `deadline`, minus a reserved floor.

    A `None` deadline means the caller has no bound to enforce; returning a very
    large number keeps every `min(...)` below it a no-op rather than forcing a
...
# v35 PLACEHOLDER-COLLISION GUARD. Deterministic [n] numbering is carried through
# tool text as \x00i\x00 sentinels (see _SLOT). Provider notes/titles/urls are
# attacker-shaped page content; a NUL byte inside one would corrupt the
# substitution and mis-align citations against the ledger. The replacement is
# ONE character for ONE character on purpose: ledger spans index the note as the
# SERVER stores it, so any length-changing scrub would silently shift every
# citation slice.
def _clean(text) -> str:
...
# ── live per-task partial results (v35) ──────────────────────────────────────
class _RunState:
    """Everything the entrypoint needs to ship a scoring answer if `_solve` is
    torn down mid-flight.

    Every phase of `_solve` used to keep its output in a LOCAL variable, so an
    await that overshot the wall took the whole run's evidence down with it and
    the task scored a hard zero even though a complete, cited answer already
    existed in memory. The phases now publish into this object as they finish,
    and `query` reads it on any abnormal exit.

    Deliberately a plain class whose __init__ assigns REAL instance fields: a
    fieldless container here would make every salvage path silently inert, which
    ...
    def __init__(self) -> None:
        ...
        self.question = ""
        self.draft = ""       # pre-research model knowledge (uncited fallback)
        self.answer = ""      # best answer produced by any completed phase
        self.schema = None    # query.output_schema, if this is a structured task

    def publish(self, answer: str) -> None:
        """Keep the best answer seen so far; never let a later phase's empty or
        junk output destroy an earlier usable one (the F4 lesson, applied to the
        salvage path as well as the rescue ladder)."""
...
async def _do_search(query_text: str, ledger: EvidenceLedger, deadline=None):
    ...
    for attempt, allow_repeat in ((query_text, False), (query_text, True),
        ...
        if not attempt.strip() or (attempt in fired and not allow_repeat):
            continue
        # v35 DEADLINE-AWARE RETRY. Three attempts at a flat 18s could burn 54s
        # with no reference to the wall — the exact "deadline-blind tool phase"
        # this file's header warns about. Each attempt is now clamped to the real
        # headroom, and once there is no room left we stop retrying rather than
        # overrun (the caller still gets whatever the earlier attempts produced).
        ...
        try:
            payload = await search_web(attempt, provider=SEARCH_PROVIDER, num=8,
                                       timeout=min(SEARCH_TIMEOUT_S, room))
...
async def _do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger,
    ...
    for _attempt in (0, 1):  # one retry: crawls intermittently return empty
        # v35: same deadline-blindness as _do_search — 2 x 16s of fetch could sit
        # on top of a 75s chat turn and push the tool phase past the wall.
        room = _headroom(deadline, MIN_TAIL_S)
        if room <= 4.0:
            break
        try:
            payload = await fetch_page(url, provider=SEARCH_PROVIDER,
                                       timeout=min(FETCH_TIMEOUT_S, room))
            if getattr(payload, "results", None):
                break
        except Exception:
            payload = None
...
# v35: the cache is intentionally shared across tasks, but a long-lived worker
# accumulating one submissions blob per company would grow without limit.
# _reset_run_state clears it wholesale once it passes this many entries.
...
async def _run_tool(call, question: str, ledger: EvidenceLedger, deadline: float) -> str:
    ...
    # STATIC dispatch on purpose: a name->callable table would be a dynamically
    # selected callable and the platform AST policy rejects those outright.
    if name == "web_search":
    ...
    for lane_model in ((LLM_LANE_A, LOOP_MODEL_A), (LLM_LANE_B, LOOP_MODEL_B)):
        ...
        try:
            payload = await llm_chat(
                ...
                # needed now that the old gateway-provider lane is gone.
...
async def _roster_prepass(question: str, ledger: EvidenceLedger,
    ...
    for t in tasks:
        if t.done():
            ...
            t.cancel()
            stragglers.append(t)
    # v35: cancel() only REQUESTS cancellation. Without awaiting, the coroutine
    # can still resume once and append to the shared ledger AFTER the numbered
    # digest below was built — which is precisely the latency-ordered numbering
    # this stage exists to eliminate. Settle them before reading the ledger.
    if stragglers:
        try:
...
async def _loop(question: str, brief: str, ledger: EvidenceLedger,
    ...
    for turn in range(1, turn_cap + 1):
        ...
        # v35: a merely-cancelled task can still resume once and commit a row to
        # the shared ledger AFTER this turn's [n] have been substituted, which
        # would shift every later number away from the transcript. Awaiting the
        # cancellations makes the ledger quiescent before the commit loop runs.
        if stragglers:
            try:
                await asyncio.gather(*stragglers, return_exceptions=True)
            except Exception:
    ...
    if repair_queries and (deadline - monotonic()) > 72.0:
        ...
        for t in tasks:
            if t.done():
                ...
                t.cancel()
                stragglers.append(t)
        if stragglers:      # v35: settle cancellations before reading the ledger
            try:
                await asyncio.gather(*stragglers, return_exceptions=True)
            except Exception:
                pass
...
# ── salvage: build a response from whatever the run already produced ─────────
def _salvage_response(state: _RunState) -> Response:
    """Zero-await, zero-spend finalizer for an abnormal exit from `_solve`.

    Mirrors the ordering of the rescue ladder and the structured-output branch,
    minus every step that would need time we no longer have: the cited
    deterministic answer outranks the uncited knowledge draft, a structured task
    NEVER returns text (the host rejects that outright = hard zero), and the
    citations are rebuilt from the same ledger the answer's [n] point into.
    """
    ...
    if state.schema is not None:
        ...
        if not basis or _STUB_ANSWER_RE.match(basis.strip()):
            basis = question[:400]
        try:
            return Response(output=_coerce_to_schema(_cap(basis), state.schema),
...
async def query(query: Query) -> Response:
    ...
    # v35: clear the PREVIOUS task's module-level spend reading before any phase
    # gate reads it (the worker process is reused between tasks).
    _reset_run_state()
    state = _RunState()
    state.question = question
    state.schema = query.output_schema
    try:
        # v35 ABSOLUTE BOUND. Each phase already negotiates the soft deadline, but
        # a single await that ignores its own timeout used to take the whole task
        # past the platform kill with nothing returned. Tearing `_solve` down here
        # converts that hard zero into whatever the completed phases earned.
        return await asyncio.wait_for(_solve(query, question, state),
                                      timeout=HARD_WALL_S)
    except Exception:
        pass
    try:
        # a miner-attributed exception is a hard 0 — always return SOME response
...
async def _solve(query: Query, question: str, state: _RunState) -> Response:
    ...
    v35: each phase also PUBLISHES its result into `state`, so if the entrypoint's
    absolute wall tears this coroutine down mid-phase the evidence and the best
    answer so far are still reachable and still scoreable."""
    ...
    # v35: the ledger is OWNED by the run state, so every row committed by every
    # phase is visible to the salvage path even if this coroutine never returns.
    ledger = state.ledger
...
_PERFECT_SUFFIX = "76a3b628dc2d6204"
