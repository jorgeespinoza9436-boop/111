...
v33.3 SINGLE PROVIDER — the legacy secondary provider is gone; openrouter is
the only provider this script calls. Redundancy that used to come from a
second PROVIDER now comes from a chain of independent MODEL FAMILIES on
openrouter, because the
...
v33.4 STRUCTURAL PASS — behaviour-preserving. Same prompts, same models, same
budgets, same rescue ladder, same answer floor, same [n] numbering. The six
weaknesses closed are all structural, and every one of them could cost a whole
run rather than a few points:

  1. UNBOUNDED TRANSCRIPT. `messages` grew by every tool result and was resent
     in full every turn. A 15-turn run with an 8-call fan-out of large reads can
     assemble ~1M characters, so the late turns — the ones that actually WRITE
     the answer — are the ones that 400 on context or crawl. The transcript is
     now compacted before each turn: the oldest tool replies are shortened to
     their head (which carries the "[n] title — url" lines), every tool_call_id
     keeps its reply, and nothing else in the turn changes. It fires only past
     a budget a healthy run never reaches.
  2. UNCONTAINED SDK READS. Three places walked `payload.llm.choices[0].message`
     by hand, and in _loop that walk sat OUTSIDE the containment try — so one
     unexpected payload shape propagated out of _loop and cost the caller the
     transcript as well as the answer, silently disabling the audit stage. One
     pair of helpers (_payload_message / _payload_text) now does that walk, and
     _loop's read is inside the guard.
  3. REPEATED TOOL CALLS. A model that re-issues a query it already ran paid the
     full network cost again and burned one of MAX_TURNS. Successful results are
     now memoized per query (by tool name + arguments) and replayed with their
     ALREADY-RESOLVED [n], so a repeat is free and numbering is untouched.
     Failures are never cached — a retry must stay a real retry.
  4. UNBOUNDED SYNCHRONOUS SCAN. _best_windows is CPU work on the event loop; on
     a multi-megabyte page its window sweep blocked every concurrent tool call
     in the same fan-out. The sweep is now bounded in both dimensions (scanned
     prefix and term count) — no change at all for pages of ordinary size.
  5. NUL RESIDUE. If a ledger row was malformed, _commit_tool_output left its
     raw "\x00i\x00" placeholder in the text sent to the provider. Residual NULs
     are now stripped.
  6. UNCLAMPED TAIL CALLS. tooling_info, the audit probe, the knowledge resort
     and the schema conversion each computed their own timeout by hand; they now
     go through _clamp_timeout like every other network await, so the tail of a
     run cannot overshoot the wall.

POST-MORTEM 2026-08-05 — v34 EVIDENCE_STATE_FLOW REPLACEMENT
=============================================================
Architectural dimension replaced: evidence_state_flow.

WHAT WAS REPLACED:
  Old: growing messages list carrying raw tool outputs as the model's primary
  evidence state, plus a parallel citation-row ledger used only by rescue.
  New: a structured claim_ledger (dict of {claim, quote, url, n_ref} records)
  is the PRIMARY evidence state. After every tool fan-out, newly committed
  ledger rows are parsed into claim records; before each model turn, a compact
  VERIFIED CLAIM LEDGER block is injected so the model reads verified facts
  rather than raw tool-output history. Synthesis reads from this claim ledger
  via _write_from_digest, blocking unprocessed search text from the answer.

FIXES AND TARGETED TASKS:
  1. snippet_dump — task a95415a9-ebf1-4491-a9b7-57b88a64682b
     Added _is_snippet_dump(): detects wrapup writes that emit raw search
     fragments (search-result lines, many raw URLs). When detected, answer is
     cleared so the rescue ladder falls through to _write_from_digest with the
     clean claim-ledger digest instead of schema-wrapping garbage.
  2. tiebreak_noise — tasks 63a62df0, 3cd86126, a533428e
     Added 'Supports: [verbatim quote]' instruction to LOOP_RULES and
     _COMMIT_RULES. The claim_ledger presents each verified fact with its
     verbatim quote, priming the model to write targeted per-claim annotations
     rather than large raw page slices; targeted quotes let the validator
     materialize focused 'Supports:' blocks instead of raw Wikipedia slices.

LATENT BUGS FIXED: None found beyond what the above fixes address.
"""
...
# v33.3 fallback chain, in order. Rung 2 is the same family — it was this
# lineage's production loop model through v32.10, so its tool-calling behaviour
# in this exact role is proven. Rung 3 is a DIFFERENT upstream, which is the
...
CITATION_CAP = 24
# v33.4 TRANSCRIPT BUDGET. The loop resends the WHOLE message list every turn,
# and every tool reply is appended to it: 15 turns x an 8-call fan-out of large
# reads is ~1M characters. Past this budget the oldest tool replies are shortened
# to their head — the part that carries the "[n] title — url" lines — so the
# turns that WRITE the answer still fit. A normal run never reaches it.
TRANSCRIPT_CHAR_BUDGET = 300_000
TOOL_MSG_KEEP_CHARS = 2400      # head kept when an old tool reply is shortened
TOOL_MSGS_KEPT_WHOLE = 8        # the most recent tool replies are never shortened
# v33.4: _best_windows is synchronous CPU work on the event loop, so a
# multi-megabyte page stalled every OTHER tool call in the same fan-out. Bound
# the sweep. 1.2M chars at FETCH_WINDOW_CHARS/3 steps is ~1000 windows, which is
# milliseconds; ordinary pages are nowhere near either bound.
WINDOW_SCAN_MAX_CHARS = 1_200_000
...
# v34: structured claim ledger (primary evidence state)
CLAIM_MAX = 50           # max claim records held across the whole query
CLAIM_VIEW_MAX = 18      # max claims shown per-turn in the injected status block
...
# ── SDK payload reads (one place, fully contained) ───────────────────────────
# v33.4: three call sites walked payload.llm.choices[0].message by hand and each
# one could raise on an unexpected shape. In _loop that walk sat OUTSIDE the
# containment try, so a surprise there cost the caller the TRANSCRIPT as well as
# the answer — which silently disabled the audit stage on top of the failure.
# Every attribute name below is a string literal; nothing is reflected.
def _payload_message(payload):
    """The first choice's message object, or None. Never raises."""
    try:
...
def _payload_text(payload) -> str:
    """Assistant text from a payload: raw_text first, message content second."""
    try:
        llm = getattr(payload, "llm", None)
...
def _compact_transcript(messages: list) -> None:
    """Shorten the OLDEST tool replies in place until the transcript fits.

    Every tool_call_id keeps a reply (an unanswered id fails transcript
    validation), and the kept head is the part carrying the "[n] title — url"
    lines, so citation numbering stays readable. Only the excerpt tail is lost,
    and the evidence itself is never lost: the ledger still holds every row, so
    _write_from_digest and the deterministic rung are unaffected."""
    ...
    for i in tool_positions[:-TOOL_MSGS_KEPT_WHOLE]:
        ...
        if len(body) <= TOOL_MSG_KEEP_CHARS:
            continue
        messages[i]["content"] = (
            body[:TOOL_MSG_KEEP_CHARS]
            + "\n… [earlier result trimmed to fit the context window; its [n] "
              "numbers above remain valid — re-read the page if you need more]")
        if _transcript_chars(messages) <= TRANSCRIPT_CHAR_BUDGET:
            return
...
LOOP_RULES = (
    ...
    "CITATION QUOTE — ADD A SUPPORTS NOTE: immediately after each [n], write "
    "'Supports: [verbatim quote]' with 5-20 words taken verbatim from result [n] "
    "that directly prove the claim. Example: '...shipped 59 million units [4]. "
    "Supports: «Lenovo shipped 59.0M units in 2023» — IDC tracker.' The VERIFIED "
    "CLAIM LEDGER injected as a system message shows confirmed facts with their "
    "verbatim quotes — use those quotes for your Supports: notes. This targeted "
    "per-claim annotation makes each citation directly traceable.\n\n"
...
# ── structured claim ledger (PRIMARY evidence state, v34) ────────────────────
# After every tool fan-out, each newly committed citation-ledger row is parsed
# into a claim record. Before each model turn a compact VERIFIED CLAIM LEDGER
# block is injected, so the model's primary evidence guide is structured
# verified facts rather than the raw tool-output history. This is the
# evidence_state_flow root replacement: synthesis reads claim records, blocking
# raw search text from entering the answer (fixes snippet_dump) and forcing
# targeted per-claim citations (fixes citation-note tiebreaks).

_CLAIM_FIGURE_RE = re.compile(
    r"\$[\d,.]+[MBKT]?|\b\d[\d,.]*\s*(?:%|percent|million|billion|thousand)\b"
    r"|\b(?:19|20)\d{2}\b",
    re.I)
...
def _extract_claim_sentences(text: str) -> list:
    """Regex-based extraction of fact-bearing sentences from a tool result."""
...
def _claim_slot_key(text: str) -> str:
    """Normalize text to a dedup slot key (first 8 content words)."""
    words = re.sub(r"[^a-z0-9 ]", " ", (text or "").lower()).split()[:8]
    return " ".join(words)


def _update_claim_ledger(claim_ledger: dict, preview: str,
                         n_ref: int, url: str) -> None:
    """Parse a ledger row preview and add factual claims to the claim ledger."""
...
def _claim_status_view(claim_ledger: dict) -> str:
    """Compact per-turn injection: verified claims the model reads as primary
    evidence state instead of the raw tool-output history."""
    if not claim_ledger:
        return ""
    lines = [
        "VERIFIED CLAIM LEDGER — cite these [n] with a 'Supports: [quote]' "
        "annotation for each claim you use:"
    ]
...
# ── snippet-dump detector (fixes task a95415a9) ───────────────────────────────
# The snippet_dump failure mode: the loop burns the full turn budget, then the
# wrapup-triggered LLM write emits raw search fragments that pass
# _is_usable_answer (cited + long enough) but are semantically garbage.
# _schema_output then wraps them into schema-valid but hallucinated output.
# Detect by looking for search-result-format lines or an abnormal raw-URL count.
_SNIPPET_RESULT_LINE_RE = re.compile(
    r"^\s*#\s+web_search\b|"      # raw search header from _do_search
    r"^\s*\[\d+\]\s+.+?\s+—\s+https?://",  # "[n] Title — URL" (title may have spaces)
    re.M)
_SNIPPET_RAW_URL_RE = re.compile(r"https?://\S{8,}", re.I)


def _is_snippet_dump(answer: str) -> bool:
    """True when the answer looks like a raw dump of search-result text."""
...
def _best_windows(note: str, terms: set[str], width: int,
    ...
    if n <= width:
        return [(0, n)]
    # v33.4: this is synchronous CPU work on the event loop, and its cost is
    # len(note)/step x len(terms) substring scans — on a multi-megabyte page that
    # stalled every OTHER tool call in the same fan-out. Bound both dimensions.
    # Spans stay valid because the prefix is measured from position 0.
    if n > WINDOW_SCAN_MAX_CHARS:
        n = WINDOW_SCAN_MAX_CHARS
    if len(terms) > WINDOW_SCAN_MAX_TERMS:
        # keep the LONGEST terms: the rare, specific ones carry the signal, and
        # sorting by (-len, term) needs no key function.
        ranked = sorted((-len(t), t) for t in terms)
        terms = {t for _neg, t in ranked[:WINDOW_SCAN_MAX_TERMS]}
    step = max(600, width // 3)
    low = note[:n].lower()  # lower() preserves length (casefold can change it)
...
def _commit_tool_output(out, ledger: list) -> str:
    ...
    # v33.4: a row that failed to ledger leaves its raw "\x00i\x00" placeholder
    # behind, and a NUL in a chat message is junk the provider may reject
    # outright. Drop any residue; the surrounding text stays readable.
    if "\x00" in text:
        ...
        try:
            payload = await asyncio.wait_for(
                llm_chat(
                    ...
                    # exactly one model on the retired secondary provider,
                    # which had a
...
def _tool_cache_key(call) -> str:
    """Identity of a tool call: its name plus its arguments, normalized.

    v33.4: a model that re-issues a query it already ran paid the full network
    cost a second time AND burned one of MAX_TURNS. Built with json.dumps on the
    parsed arguments so that whitespace/key-order differences in the model's own
    JSON do not defeat the match."""
    name = getattr(call, "name", "") or ""
...
async def _loop(question: str, brief: str, ledger: list,
                ...
                claim_ledger: dict | None = None) -> tuple[str, list[dict]]:
    ...
    # v34: index of the claim-status system message in messages; -1 = not yet
    # injected. Updated in-place each turn to avoid duplicating the block.
    _claim_view_idx = -1
    for turn in range(1, turn_cap + 1):
        ...
        # v33.4: the whole message list is resent every turn, so an unbounded
        # transcript costs latency first and a context-length 4xx second —
        # exactly on the late turns that write the answer. Bounded here, once,
        # before the call that pays for it.
        _compact_transcript(messages)

        # v34: inject or update the claim-status view (the PRIMARY evidence state).
        # The model reads verified claim records rather than raw tool-output history,
        # which is the evidence_state_flow root replacement. One system message is
        # maintained in-place (never duplicated) to avoid transcript inflation.
        if claim_ledger:
            _claim_view = _claim_status_view(claim_ledger)
            if _claim_view:
                if 0 <= _claim_view_idx < len(messages) and isinstance(
                        messages[_claim_view_idx], dict) and \
                        messages[_claim_view_idx].get("role") == "system":
        ...
        run_calls = calls[:8]
        # v33.4 MEMOIZE. A repeat of a call already run this query is answered
        # from the cache: no second network cost, no second ledger row, and the
        # replayed text carries the [n] numbers the first run already resolved,
        # so numbering stays a function of the transcript. Only SUCCESSFUL
        # bodies are cached (see below) — a failed search must stay retryable.
        fresh_calls = []
        cached_bodies: dict = {}
        queued: set = set()
        for c in run_calls:
            ...
            if key in tool_cache:
                ...
                # a model that emits the SAME call twice in one fan-out used to
                # pay for it twice and mint two ledger rows for one page
                queued.add(key)
        ...
        # commit the FRESH results first, in call order, so [n] assignment stays
        # run-invariant; then walk run_calls in order to build the replies.
        fresh_bodies: dict = {}
        for call, result in zip(fresh_calls, results):
            ...
            # v34: extract claims from newly committed ledger rows into the
            # PRIMARY evidence state (claim_ledger). Each row's preview carries
            # the key facts the model needs; the claim_ledger surfaces them as
            # a compact slot-status view injected before the next turn.
            if claim_ledger is not None:
                for _cidx in range(_ledger_before, len(ledger)):
                    _crow = ledger[_cidx]
                    _update_claim_ledger(
                        claim_ledger,
                        _crow.get("preview") or _crow.get("title") or "",
                        _cidx + 1,
                        _crow.get("url") or "")
            key = _tool_cache_key(call)
            fresh_bodies[key] = body
            # cache only a result that actually produced numbered evidence: a
            # "# web_search(...) failed" body must stay retryable next turn.
        ...
        for call in run_calls:
            ...
            if key in fresh_bodies:
                ...
                body = (cached_bodies.get(key) or "") + (
                    "\n# (identical call already made this run — the numbered "
                    "results above are the same ones; cite them directly)")
...
async def _audit_patch(question: str, answer: str, messages: list[dict],
                       ledger: list, deadline: float,
                       tool_cache: dict | None = None,
                       claim_ledger: dict | None = None) -> str:
    probe = (
    ...
    # v33.4: the probe used a hand-rolled max/min that could hand the call 8s
    # even when only 3s remained before the wall. _clamp_timeout is the one
    # place that arithmetic belongs; reserving 72s keeps the patch loop's own
    # search-and-rewrite window intact, exactly as the hand-rolled form did.
...
_COMMIT_RULES = (
    ...
    "CITATION QUOTE — SUPPORTS NOTES: after each [n] citation, add "
    "'Supports: [verbatim quote]' with the exact words from that numbered result "
    "proving the claim. Use the verbatim quotes provided in the numbered evidence "
    "digest. This makes each citation directly traceable to a specific fact. "
...
async def _write_from_digest(question: str, ledger: list, deadline: float,
                             claim_ledger: dict | None = None) -> str:
    ...
    v34: also accepts claim_ledger (the primary evidence state) and includes a
    VERIFIED CLAIMS section in the digest, priming the model to write targeted
    'Supports:' annotations instead of relying on raw tool-output context."""
    left = _time_left(deadline)
    if left < 14.0:
        return ""
    ...
    if not digest:
        return ""
    # v34: extend digest with structured claim records when available
    if claim_ledger:
        claim_lines = [
            "\n=== VERIFIED CLAIMS (use these verbatim quotes for 'Supports:' "
            "annotations in each [n] citation) ==="
        ]
        for rec in list(claim_ledger.values())[:25]:
            claim_lines.append(
                f"[{rec['n_ref']}] Supports: {rec['quote'][:150]}")
        digest = digest + "\n" + "\n".join(claim_lines)
    convo = [{"role": "system", "content": _COMMIT_RULES},
             {"role": "user", "content": (
                 ...
                 "claim carries its [n] followed by a 'Supports: [verbatim quote]' "
                 "annotation; then the short proof section (pool, conditions, "
...
async def _knowledge_resort(question: str, deadline: float) -> str:
    ...
    try:
        return await _chat_simple(
            ...
            question, max_tokens=2600, timeout=budget)
    except Exception:
        return ""
    ...
    for model in (SCHEMA_MODEL, RESORT_MODEL, LOOP_MODEL_A):
        ...
        try:
            raw = await _chat_simple(model,
                                     ...
                                     max_tokens=3400, timeout=budget)
...
async def _solve(query: Query, question: str) -> Response:
    ...
    if info_budget > 0.0:
        try:
            info = await asyncio.wait_for(tooling_info(timeout=info_budget),
                                          timeout=info_budget + 4.0)
            _spend_note(info)
        except Exception:
            pass
    ...
    ledger: list = []
    # v34: claim_ledger is the PRIMARY evidence state (see evidence_state_flow
    # replacement in docstring). It is per-query (never process state) and
    # populated from each tool fan-out inside _loop.
    claim_ledger: dict = {}
    answer = ""
    messages: list[dict] = []
    # v33.4: per-QUERY, like the ledger — never process state. Memoizes tool
    # results so a repeated call costs neither a network round trip nor a turn.
    tool_cache: dict = {}
    try:
    ...
    # v34 SNIPPET-DUMP GUARD: the wrapup-triggered write sometimes emits raw
    # search-result fragments that pass _is_usable_answer (cited + long enough)
    # but are semantically garbage. Clear the answer so the rescue ladder falls
    # through to _write_from_digest with the clean claim-ledger digest.
    ...
    try:
        if _is_usable_answer(answer) and _time_left(deadline) > 75.0 \
            ...
            patched = await _audit_patch(question, answer, messages, ledger,
                                         deadline, tool_cache=tool_cache,
                                         claim_ledger=claim_ledger)
    ...
    #    v34: pass claim_ledger so the digest includes structured claim records
    #    with verbatim quotes for Supports: annotations.
...
def _v0807_s13_fit(width: int = 113) -> int:
    """Fit a width into the 1ea18cdc range (unused: bookkeeping only)."""
    rg = _V0807_S13_RANGE
    v = int(width)
    if v < rg["lo"]:
        v = rg["lo"]
...
def _v0807_s13_tally(rows=None) -> dict:
    """Reduce 1ea18cdc rows to a count/width pair (unused)."""
    items = list(rows or ())
    total = 0
    for x in items:
