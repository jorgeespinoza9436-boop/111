...
stages and an explicitly phased orchestrator are layered on top:
  ...
  - single-provider LLM lanes: BOTH lanes are openrouter (this miner stores only
    openrouter + parallel credentials). Lane A is the primary
...
# `parallel`. EVERY llm_chat call must use provider="openrouter" or it scores
# 0 in production. Both LLM lanes are
...
# Plain-dict state plus module-level helpers. No class body, no dunder method
# names, nothing here for the server-side AST policy to reject. Shape is
# {"rows": [...]} and a row's 1-based position in that list IS its [n] number,
# exactly as before.
def _ledger_new() -> dict:
    return {"rows": []}


def _ledger_add(ledger: dict, receipt_id: str, result_id: str, note_len: int,
...
def _ledger_ref_for(ledger: dict, number: int):
    rows = ledger["rows"]
...
# ── sort keys: named module-level functions, never anonymous callables ──────
def _key_density_then_pos(hs):
    return (-hs[0], hs[1])


def _key_span_bounds(p):
    return (p[0], p[1])


def _key_value_then_order(g):
    return (0 if g["has_value"] else 1, g["order"])
...
def _best_windows(note: str, terms: set[str], width: int,
    ...
    scored.sort(key=_key_density_then_pos)
...
# A tool's return value is a tagged plain dict rather than an instance: same
# two fields, same call-order commit, but no class body and no dunder method.
_TOOL_OUTPUT_TAG = "harnyx.tool_output"


def _tool_output(text: str, rows: "list | None" = None) -> dict:
    return {"tag": _TOOL_OUTPUT_TAG, "text": text, "rows": rows or []}


def _is_tool_output(out) -> bool:
    return isinstance(out, dict) and out.get("tag") == _TOOL_OUTPUT_TAG


def _commit_tool_output(out, ledger: dict) -> str:
...
async def _do_search(query_text: str, ledger: dict):
...
async def _do_fetch(url: str, focus: str, question: str, ledger: dict) -> str:
    if not url.strip():
    ...
    return _tool_output(f"# read_page({url!r}) -> [{_SLOT.format(0)}] {len(note)} chars total; head + "
...
async def _run_tool(call, question: str, ledger: dict, deadline: float) -> str:
    try:
    ...
    for lane_model in ((LLM_LANE_A, LOOP_MODEL_A), (LLM_LANE_B, LOOP_MODEL_B)):
        ...
        try:
            payload = await llm_chat(
                ...
                # needed now that the second-provider fast lane is gone.
...
async def _preseed(question: str, set_question: bool, ledger: dict,
                   deadline: float) -> str:
...
async def _roster_prepass(question: str, ledger: dict,
                          deadline: float) -> str:
...
async def _loop(question: str, brief: str, ledger: dict,
...
async def _audit_patch(question: str, answer: str, messages: list[dict],
                       ledger: dict, deadline: float) -> str:
    probe = (
...
async def _verify_and_repair(question: str, answer: str, messages: list[dict],
    ...
    for c in claims:
        ...
        resolves = any(_ledger_ref_for(ledger, n) is not None for n in cited_ns)
...
def _citations_for(answer: str, ledger: dict) -> list[CitationRef]:
    ...
    built.sort(key=_key_value_then_order)
...
def _ledger_digest(ledger: dict, char_cap: int = 60000) -> str:
...
def _deterministic_answer(question: str, ledger: dict) -> str:
...
async def _write_from_digest(question: str, ledger: dict, deadline: float) -> str:
