...
LOOP_RULES = (
    ...
    "cited answer."
    "\n\nDO NOT CITE THE QUESTION'S PREAMBLE. Questions often identify the subject obliquely ('the studio that distributed X and Y', 'the artist known for song Z'). Those naming films, songs or works are a POINTER to the subject, not something your answer asserts — resolve them from knowledge or a search you do NOT cite. Cite ONLY sources that establish a value the answer actually returns. A citation supporting a fact absent from your answer is a rule-12 irrelevant citation and costs you a tie you would otherwise win."
)
...
async def _do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger) -> str:
    ...
    _why = ""
    for _attempt in (0, 1):
        # The provider raises ToolProviderError for a 200 with an EMPTY body
        # (pydantic string_too_short on FetchPageResult.content). That is
        # deterministic per-URL, so only a genuine timeout is worth a retry.
        try:
...
async def _solve(query: Query, question: str) -> Response:
    _DEAD_URLS.clear()   # per-query reset: never leak a dead URL between queries
