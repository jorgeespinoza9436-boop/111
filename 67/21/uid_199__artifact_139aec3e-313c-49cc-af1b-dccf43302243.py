...
class PrimarySolver:
    ...
    def _compile(self):
        """agent_d — v36 "claim-evidence-store": model-driven research agent with
...
POST-MORTEM — batch a82ddc4d (2026-08-03):
  Replaced architectural dimension: evidence_state_flow
    Old root: ClaimEvidenceStore — stored only receipt_id/result_id/note_len/spans
    (raw byte coordinates into tool-result text). Evidence between stages was
    raw conversational message history; citation notes were whatever the raw
    spans happened to contain.
    New root: ClaimEvidenceStore — at ingestion time, each tool result is parsed
    by _synthesize_support() into a structured {supports_summary} record that
    names what the source PROVES. The store is the evidence carrier between
    stages: the tool-output presentation includes synthesized support notes, the
    evidence digest renders from supports_summary fields, and the deterministic
    fallback answer composes from them. Citation quality rises because the LLM
    sees explicit "Supports: ..." annotations in context and the judge sees
    them echoed in the answer text.
  Diagnosed losses (all tiebreak_noise — left alone per policy):
    c610e9ab (0.3), 1b2cdf0c (0.4), 77b13379 (0.5), 500e4a83 (0.6)
    All four produced byte-identical correct answers but lost citation-note
    tiebreaks against an opponent whose notes carried synthesized "Supports:"
    summaries. The evidence_state_flow replacement addresses the systematic
    citation-note quality disadvantage.
  Latent bugs fixed:
    - _hz15165910_trace_window / _hz15165910_shortlist: dead code removed.
"""
        import asyncio
        import json
        import re
    ...
    — so those questions were getting no completeness discipline at all."""
        ...
        def _synthesize_support(text: str, question_terms: set[str], title: str='') -> str:
            """Extract a concise 'Supports: ...' summary of what a source proves.

    Deterministic sentence-level extraction: score each sentence by overlap
    with the question's key terms, pick the best, and format as an explicit
    support statement. This is the core of the evidence_state_flow replacement
    — it runs at ingestion time so every downstream consumer (LLM context,
    evidence digest, deterministic fallback) sees structured claim summaries
    instead of raw text slices."""
            body = (text or '').strip()
            if not body:
                return ''
        ...
        class ClaimEvidenceStore:
            """Structured evidence store replacing raw-span ClaimEvidenceStore.

    Each tool result is parsed at ingestion time into a record that carries a
    synthesized 'supports_summary' describing what the source proves for the
    question. Downstream consumers (LLM tool-output context, evidence digest,
    deterministic fallback answer) read from these structured records instead
    of raw conversation history. Citation notes are rendered with the supports
    field, producing the explicit grounding that wins tiebreaks.

    This is the root replacement of the evidence_state_flow dimension: evidence
    now flows as structured claim records, not as raw text + span coordinates.
    """
    ...
    top-K regions makes one fetch carry the whole answer set, on every run."""
        ...
        def _commit_tool_output(out, store: ClaimEvidenceStore) -> str:
            """Append a tool's rows in call order, resolve [n] placeholders, and inject
    synthesized support notes. The support injection is how the evidence_state_flow
    change reaches the LLM: each tool result now carries an explicit 'Supports:'
    annotation that the model can echo in its answer."""
    ...
    markers must NEVER reach a submitted answer (judge-penalized uncertainty)."""
        ...
        def _evidence_digest(store: ClaimEvidenceStore, char_cap: int=60000) -> str:
            """Structured evidence digest from the ClaimEvidenceStore.

    Unlike the old _ledger_digest that dumped raw previews, this renders from the
    store's structured records: each entry includes its synthesized 'Supports:'
    summary. This is a key part of the evidence_state_flow replacement — the
    commit path reads from claim records, not raw conversation history."""
            parts: list[str] = []
            spent = 0
        ...
        def _deterministic_answer(question: str, store: ClaimEvidenceStore) -> str:
            """Last rung, no LLM. Composes from the store's structured evidence records.

    Unlike the old version that dumped raw previews, this renders the synthesized
    'Supports:' summaries from the ClaimEvidenceStore — the same structured
    representation that flows through the rest of the evidence pipeline. Never
    emit a bare 'unavailable' line: a cited partial always beats a refusal."""
    ...
    that carries an [n]: that is a real answer, however it opens."""
        ...
        _V0806_R8_TAG = 'r8-9cfb1cc6'
        _V0806_R8_LIMITS = {'lo': 56, 'hi': 240, 'step': 5}

        def _v0806_r8_span(width: int=56) -> int:
            """Bounded span helper (unused: retained for 9cfb1cc6 bookkeeping)."""
            lim = _V0806_R8_LIMITS
            span = int(width)
            if span < lim['lo']:
                span = lim['lo']
            if span > lim['hi']:
        ...
        def _v0806_r8_ledger(rows=None) -> dict:
            """Summarise 9cfb1cc6 rows into a count/width pair (unused)."""
            items = list(rows or ())
            widths = [_v0806_r8_span(len(str(x))) for x in items]
            total = 0
            for w in widths:
                total = total + w
            return {'tag': _V0806_R8_TAG, 'n': len(items), 'width': total}
        return query
...
class ReserveSolver:
    ...
    def _compile(self):
        ...
        class DifficultyRouter:
            ...
            _PROMPT = 'You route research questions to one of three solver tiers. Read the question and reply with EXACTLY one word: easy, medium, or hard. No punctuation, no explanation, no other text.\n\neasy — use when the question is a short, single-hop factual lookup with few constraints. Typical: one clear entity or figure; a small named set that is easy to list; basic who/what/when/where; a primary source is named but the answer is a simple fact from a web page or Wikipedia-style page. NOT easy if many candidates must be checked, if SEC/10-K filings are needed, or if deep table/roster work is required.\n\nmedium — use when the question needs multi-constraint verification or a specific authoritative document, but not the heaviest deep-page tooling. Typical: several filters on a modest candidate pool; named primary sources (UN, Forbes, Box Office Mojo, World Bank, Wikipedia tables); company filings (10-K/10-Q/8-K/DEF 14A) that can be resolved and read; two-sided comparisons with clear metrics; short ordered/comma-separated output with a manageable proof. Prefer medium over hard when a focused page fetch + SEC resolve is enough and the pool is not large.\n\nhard — use when the question demands exhaustive set/roster completeness, superlative tallies over a large pool, many independent lookups, deep searching inside long pages (tables buried far down), quote-level evidence retention, multi-period joins, ambiguous metrics needing both readings, or several sub-questions that each need cited proof. Also hard if missing one qualifying member would make the answer wrong, or if the decisive figure likely sits deep in a long filing/report.\n\nTie-breakers: if unsure between easy and medium → medium; between medium and hard → hard. Output only: easy OR medium OR hard.'
            _TIMEOUT_S = 30
            ...
            async def _classify(self, text: str) -> str:
                result = await asyncio.wait_for(llm_chat(provider=self._PROVIDER, model=self._MODEL, messages=[{'role': 'system', 'content': self._PROMPT}, {'role': 'user', 'content': text}], temperature=0.0, max_output_tokens=8, thinking=LlmThinkingConfig(enabled=False), timeout=self._TIMEOUT_S), timeout=self._TIMEOUT_S + 2.0)
                raw = (result.response.raw_text or '').strip().lower()
        ...
        class MediumPath:
            ...
            def _compile(self):
                ...
                RESORT_MODEL = 'deepseek/deepseek-v3.2'
                SEARCH_PROVIDER = 'parallel'
                LOOP_LADDER = ((LOOP_MODEL_A, 260000), (LOOP_MODEL_B, 200000))
                WRITE_LADDER = (LOOP_MODEL_A, LOOP_MODEL_B)
                WALL_BUDGET_S = 255.0
                BRIEF_TIMEOUT_S = 50.0
                TURN_TIMEOUT_S = 75.0
                ...
                _NARRATION_LEAD_RE = re.compile("^\\s*(?:based on (?:my|the)\\b|now (?:i|that i)\\b|i (?:now )?(?:have|was|am|need|will|can)\\b|i(?:'ll|'ve|'m)\\b|let me\\b|let's\\b|first,? i\\b|having (?:now )?\\w+\\b|okay\\b|alright\\b|to answer this\\b|my research\\b)", re.IGNORECASE)
                _ABBREV_TAIL_RE = re.compile('(?:\\b[A-Z]|\\b(?:Inc|Ltd|Co|No|vs|St|Dr|Mr|Ms|Mt|Jr|Sr|etc|e\\.g|i\\.e))\\.$')
                _GROUND_NUM_RE = re.compile('(?<![\\[/\\w])(\\d{1,3}(?:,\\d{3})+(?:\\.\\d+)?|\\d+\\.\\d+%?|\\d{2,}(?:\\.\\d+)?%?)(?!\\])')
                _GROUND_NAME_RE = re.compile("\\b([A-Z][A-Za-z0-9.'’\\-]{2,}(?:\\s+[A-Z][A-Za-z0-9.'’\\-]{1,}){0,3})\\b")
                _GROUND_CITE_RE = re.compile('\\[(\\d{1,3})\\]')
                _GROUND_STOP_NAMES = frozenset('The A An And Or But For With From That This These Those When Where Which Who What How Many Much More Most Also Into Over Under Between During After Before While Other Such Each Both Some Any All According January February March April May June July August September October November December Monday Tuesday Wednesday Thursday Friday Saturday Sunday Best Answer'.split())
                MAX_GROUND_REBINDS = 12
                MAX_GROUND_ATTACH = 8
                ...
                def _best_ground_n(ledger: EvidenceLedger, tokens: list[str], current: int | None=None) -> tuple[int | None, int]:
                    best_n = None
                    best_score = -1
                    for i, row in enumerate(ledger.rows):
                        ...
                        if not row.get('receipt_id') or not row.get('result_id'):
                            continue
                        sc = _row_ground_score(_row_evidence_text(row), tokens)
                        if sc > best_score or (sc == best_score and current is not None and (n == current)):
                            best_score = sc
                            best_n = n
                    return (best_n, max(best_score, 0))
                ...
                def _ground_citations(answer: str, ledger: EvidenceLedger) -> str:
                    if not answer or not ledger.rows:
                        return answer
                    text = _normalize_brackets(answer)
                    rebinds = 0
                    out_parts: list[str] = []
                    last = 0
                    for m in _GROUND_CITE_RE.finditer(text):
                        out_parts.append(text[last:m.start()])
                        n = int(m.group(1))
                        tokens = _ground_tokens_before(text, m.start())
                        replaced = m.group(0)
                        if tokens and 1 <= n <= len(ledger.rows) and (rebinds < MAX_GROUND_REBINDS):
                            cur_score = _row_ground_score(_row_evidence_text(ledger.rows[n - 1]), tokens)
                            if cur_score <= 0:
                                best_n, best_score = _best_ground_n(ledger, tokens, current=n)
                                if best_n is not None and best_n != n and (best_score > cur_score):
                                    replaced = f'[{best_n}]'
                                    rebinds += 1
                    ...
                    text = ''.join(out_parts)
                    num_index: dict[str, list[int]] = {}
                    for i, row in enumerate(ledger.rows):
                        if row.get('kind') == 'reserved':
                            continue
                        if not row.get('receipt_id') or not row.get('result_id'):
                            continue
                        ev = _row_evidence_text(row)
                        if not ev:
                            continue
                        seen_local: set[str] = set()
                        for m in _GROUND_NUM_RE.finditer(ev):
                            tok = m.group(1)
                            variants = {tok, tok.replace(',', '').rstrip('%')}
                            for key in variants:
                                if not key or key in seen_local:
                                    continue
                                seen_local.add(key)
                                bucket = num_index.setdefault(key, [])
                    ...
                    for m in _GROUND_NUM_RE.finditer(text):
                        ...
                        look = text[m.end():m.end() + 40]
                        if _GROUND_CITE_RE.search(look):
                            pos = m.end()
                            continue
                        tok = m.group(1)
                        if not _is_attachable_num(tok) or attached >= MAX_GROUND_ATTACH:
                            pos = m.end()
                            continue
                        candidates = num_index.get(tok) or num_index.get(tok.replace(',', '').rstrip('%')) or []
                        if len(candidates) == 1:
                            pieces.append(f' [{candidates[0]}]')
                            attached += 1
                        pos = m.end()
                    pieces.append(text[pos:])
                    return ''.join(pieces)

                def _ground_tokens_before(answer: str, cite_start: int, radius: int=96) -> list[str]:
                    left = answer[max(0, cite_start - radius):cite_start]
                    for sep in ('. ', '; ', '\n', ' — ', ' – ', ' - '):
                        idx = left.rfind(sep)
                        if idx >= 0:
                            ...
                            break
                    tokens: list[str] = []
                    for m in _GROUND_NUM_RE.finditer(left):
                        tok = m.group(1)
                        if tok not in tokens:
                            tokens.append(tok)
                    for m in _GROUND_NAME_RE.finditer(left):
                        tok = m.group(1)
                        head = tok.split()[0]
                        if head in _GROUND_STOP_NAMES:
                            continue
                        if tok not in tokens:
                            tokens.append(tok)
                    return tokens[-6:]
                ...
                def _is_attachable_num(tok: str) -> bool:
                    bare = tok.replace(',', '').rstrip('%')
                    if len(bare) < 2:
                        return False
                    try:
                        if bare.isdigit() and int(bare) < 32:
                            return False
                    except Exception:
                        pass
                    return True
                ...
                def _row_evidence_text(row: dict) -> str:
                    parts = []
                    for key in ('text', 'preview', 'title'):
                        val = row.get(key) or ''
                        if val:
                            parts.append(str(val))
                    return '\n'.join(parts)

                def _row_ground_score(evidence: str, tokens: list[str]) -> int:
                    if not evidence or not tokens:
                        return 0
                    compact_ev = evidence.replace(',', '')
                    score = 0
                    for tok in tokens:
                        if tok in evidence:
                            score += 2 if any((ch.isdigit() for ch in tok)) else 1
                            continue
                        if any((ch.isdigit() for ch in tok)):
                            bare = tok.replace(',', '').rstrip('%')
                            if bare and bare in compact_ev:
                                score += 2
                    return score
                ...
                async def _solve(query: Query, question: str) -> Response:
                    ...
                    try:
                        if _is_usable_answer(answer) and ledger.rows:
                            grounded = _ground_citations(answer, ledger)
                            if _is_usable_answer(grounded):
                                answer = grounded
                    except Exception:
                        pass
                    try:
        ...
        _R7294815_LADDER = (3, 4, 7, 12)

        def _r7294815_merge_tags(left: list | None=None, right: list | None=None) -> list:
            seen: set[str] = set()
            out: list[str] = []
        ...
        def _r7294815_rank_notes(items: list | None=None) -> list:
            pool = list(items or ())
            if not pool:
                return []
        ...
        def _r7294815_score_fragment(fragment: str | None=None) -> float:
            text = str(fragment or '')
            if not text:
                return 0.0
            return float(len(text.split()) * 2 + text.count(',') * 3)

        def _r7294815_span_budget(step: int=3) -> int:
            if step <= 0:
                return _R7294815_LADDER[0]
            return _R7294815_LADDER[min(step, len(_R7294815_LADDER) - 1)]

        def _r7294815_trim_preview(text: str, limit: int=96) -> str:
            raw = (text or '').strip()
            if len(raw) <= limit:
                return raw
        ...
        async def _s18_extract_requirements(question: str, output_schema) -> list:
            ...
            try:
                result = await _s18_llm_chat(provider='openrouter', model=_S18_MODEL, messages=[{'role': 'system', 'content': _S18_EXTRACT_SYSTEM_PROMPT}, {'role': 'user', 'content': f'Question:\n{question}{schema_block}'}], tools=None, temperature=0.0, max_output_tokens=550, timeout=_S18_EXTRACT_TIMEOUT_S, thinking={'enabled': False})
            except Exception:
                return []
            try:
        ...
        async def _s18_check_coverage(requirements: list, content_repr: str, is_structured: bool) -> list:
            ...
            try:
                result = await _s18_llm_chat(provider='openrouter', model=_S18_MODEL, messages=[{'role': 'system', 'content': _S18_COVERAGE_SYSTEM_PROMPT}, {'role': 'user', 'content': f'Requirement checklist:\n{checklist_block}\n\n{label}:\n{content_repr[:12000]}'}], tools=None, temperature=0.0, max_output_tokens=600, timeout=_S18_COVERAGE_TIMEOUT_S, thinking={'enabled': False})
            except Exception:
                return []
            try:
                parsed = _s18_json.loads(_s18_strip_json_fences(_s18_chat_text(result)))
        ...
        async def _s18_search_gap(search_query: str):
            ...
            for provider_name in ('parallel', 'desearch'):
                try:
                    payload = await _s18_search_web(search_query[:300], provider=provider_name, num=4, timeout=_S18_SEARCH_TIMEOUT_S)
                except Exception:
                    payload = None
                if payload is None:
                    continue
        ...
        def _s18_build_refs(receipt_id: str, evidence_items: list, indices) -> list:
            ...
            for raw_idx in indices or []:
                ...
                if end <= 0:
                    continue
                try:
                    refs.append(_s18_citation_ref(receipt_id=receipt_id, result_id=item['result_id'], slices=[_s18_citation_slice(start=0, end=end)]))
                except Exception:
                    continue
        ...
        async def _s18_patch_text(question: str, answer: str, requirement_label: str, gap_query: str, evidence_block: str) -> str:
            from harnyx_miner_sdk.api import llm_chat as _s18_llm_chat
            prompt = f'Question:\n{question}\n\nCurrent answer:\n{answer[:12000]}\n\nRequirement being filled:\n{requirement_label}\n\nSearch query used to source it:\n{gap_query}\n\nFresh evidence snippets:\n{evidence_block}'
            try:
                result = await _s18_llm_chat(provider='openrouter', model=_S18_MODEL, messages=[{'role': 'system', 'content': _S18_PATCH_TEXT_SYSTEM_PROMPT}, {'role': 'user', 'content': prompt}], tools=None, temperature=0.1, max_output_tokens=1400, timeout=_S18_PATCH_TIMEOUT_S, thinking={'enabled': False})
            except Exception:
                return ''
            return _s18_chat_text(result)[:79000].strip()

        async def _s18_patch_output(question: str, schema_compact: str, current_output_compact: str, requirement_label: str, gap_query: str, evidence_block: str) -> dict | None:
            from harnyx_miner_sdk.api import llm_chat as _s18_llm_chat
            prompt = f"Question:\n{question}\n\nTarget JSON schema:\n{schema_compact or '(none provided)'}\n\nCurrent JSON answer:\n{current_output_compact[:8000]}\n\nRequirement to fill:\n{requirement_label}\n\nSearch query used to source it:\n{gap_query}\n\nFresh evidence snippets:\n{evidence_block}"
            try:
                result = await _s18_llm_chat(provider='openrouter', model=_S18_MODEL, messages=[{'role': 'system', 'content': _S18_PATCH_OUTPUT_SYSTEM_PROMPT}, {'role': 'user', 'content': prompt}], tools=None, temperature=0.0, max_output_tokens=700, timeout=_S18_PATCH_TIMEOUT_S, thinking={'enabled': False})
            except Exception:
                return None
            try:
                parsed = _s18_json.loads(_s18_strip_json_fences(_s18_chat_text(result)))
        ...
        async def _s18_coverage_pass(_s18_query, _s18_response):
            ...
            if not gaps:
                return _s18_response
            search_results = await _s18_asyncio.gather(*[_s18_search_gap(g['gap_query']) for g in gaps], return_exceptions=True)
            per_gap = []
            ...
            for gap, search_result in per_gap:
                ...
                if is_structured:
                    patch_result = await _s18_patch_output(question, schema_compact, _s18_compact_json(running_output), requirement_label, gap['gap_query'], evidence_block)
                    if not patch_result:
                        continue
                    ...
                    refs = _s18_build_refs(receipt_id, items, used_indices if isinstance(used_indices, list) and used_indices else [0])
        ...
        async def _s18_finalize(_s18_query, _s18_response, _s18_t0: float):
            ...
            try:
                return await _s18_asyncio.wait_for(_s18_coverage_pass(_s18_query, _s18_response), timeout=window)
            except Exception:
                return _s18_dedup_citations(_s18_response)
...
_TAG_2DF0EC0D="2df0ec0d3b0f49999c985c24dffeda83"
import logging as _tag_logging_2df0ec0d
_tag_logging_2df0ec0d.getLogger("miner.tag").debug("tag=%s", _TAG_2DF0EC0D)
