...
class SecondPath:
    ...
    def _compile(self):
        ...
        SOURCE_REPAIR_DEADLINE_SECONDS = 150.0
        SOURCE_REPAIR_MIN_COVERAGE_RATIO = 0.34
        SOURCE_REPAIR_MIN_REQUIREMENT_TERMS = 3
        SOURCE_REPAIR_MAX_REPORTED_GAPS = 3
        REQUIRED_SOURCE_TLDS = ('com', 'org', 'net', 'gov', 'edu', 'int', 'mil', 'io', 'ai', 'co', 'uk', 'us', 'ca', 'au', 'de', 'fr', 'jp', 'cn', 'br', 'ru', 'eu', 'info')
        REQUIRED_SOURCE_KEYWORDS = ('wikipedia', 'wikidata', 'britannica', 'imdb', 'rotten tomatoes', 'metacritic', 'billboard', 'goodreads', 'github', 'arxiv', 'pubmed', 'clinicaltrials', 'semantic scholar', 'google scholar', 'crossref', 'openalex', 'edgar', '10-k', '10-q', '8-k', 'world bank', 'eurostat', 'united nations', 'unesco', 'unicef', 'census bureau', 'bureau of labor statistics', 'federal reserve', 'national weather service', 'associated press', 'financial times', 'wall street journal', 'new york times', 'the guardian', 'reuters', 'bloomberg', 'steam', 'spotify')
        REQUIRED_SOURCE_ACRONYMS = ('WHO', 'SEC', 'FDA', 'CDC', 'NIH', 'NASA', 'NOAA', 'USGS', 'EPA', 'IMF', 'OECD', 'BLS', 'EIA', 'FRED', 'IRS', 'FBI', 'ESPN', 'NCBI', 'IUCN', 'FIFA')
        REQUIRED_SOURCE_NOUNS = ('report', 'reports', 'database', 'dataset', 'filing', 'filings', 'website', 'site', 'page', 'registry', 'register', 'index', 'list', 'almanac', 'encyclopedia', 'journal', 'bulletin', 'survey', 'census', 'yearbook', 'handbook', 'archive', 'archives', 'records', 'statistics', 'publication', 'edition', 'release', 'table')
        REQUIRED_SOURCE_ALIASES = {'worldbank': ('worldbank', 'world bank', 'wdi', 'databank'), 'unitednations': ('un.org', 'united nations', 'unstats'), 'censusbureau': ('census.gov', 'census bureau'), 'bureauoflaborstatistics': ('bls.gov', 'bureau of labor statistics'), 'federalreserve': ('federalreserve', 'fred', 'stlouisfed'), '10k': ('sec.gov', '10-k', 'annual report', 'edgar'), '10q': ('sec.gov', '10-q', 'quarterly report', 'edgar'), '8k': ('sec.gov', '8-k', 'edgar'), 'sec': ('sec.gov', 'edgar', 'securities and exchange'), 'who': ('who.int', 'world health organization'), 'cdc': ('cdc.gov', 'centers for disease control'), 'fda': ('fda.gov', 'food and drug administration'), 'nih': ('nih.gov', 'national institutes of health'), 'nasa': ('nasa.gov',), 'noaa': ('noaa.gov', 'weather.gov'), 'usgs': ('usgs.gov',), 'epa': ('epa.gov',), 'imf': ('imf.org', 'international monetary fund'), 'oecd': ('oecd.org',), 'bls': ('bls.gov', 'bureau of labor statistics'), 'eia': ('eia.gov',), 'fred': ('stlouisfed', 'fred'), 'irs': ('irs.gov',), 'fbi': ('fbi.gov',), 'ncbi': ('ncbi.nlm.nih.gov', 'pubmed'), 'iucn': ('iucnredlist', 'iucn'), 'fifa': ('fifa.com',), 'espn': ('espn.com',), 'pubmed': ('pubmed', 'ncbi.nlm.nih.gov'), 'arxiv': ('arxiv.org',), 'clinicaltrials': ('clinicaltrials.gov',), 'googlescholar': ('scholar.google',), 'semanticscholar': ('semanticscholar',), 'newyorktimes': ('nytimes', 'new york times'), 'wallstreetjournal': ('wsj.com', 'wall street journal'), 'financialtimes': ('ft.com', 'financial times'), 'theguardian': ('theguardian', 'guardian'), 'associatedpress': ('apnews', 'associated press'), 'rottentomatoes': ('rottentomatoes',)}
        LEDGER_STOPWORDS = frozenset({'about', 'after', 'against', 'another', 'answer', 'based', 'because', 'been', 'before', 'being', 'between', 'both', 'candidate', 'could', 'current', 'does', 'during', 'each', 'either', 'evidence', 'external', 'from', 'give', 'given', 'have', 'however', 'into', 'list', 'made', 'make', 'many', 'more', 'most', 'must', 'name', 'named', 'only', 'other', 'over', 'question', 'same', 'should', 'since', 'some', 'source', 'such', 'than', 'that', 'their', 'them', 'then', 'there', 'these', 'they', 'this', 'those', 'through', 'under', 'until', 'used', 'using', 'value', 'were', 'what', 'when', 'where', 'whether', 'which', 'while', 'with', 'within', 'would'})
        ...
        def _ledger_token(value: str) -> str:
            return ''.join((character for character in value.lower() if character.isalnum()))

        def _ledger_terms(text: str) -> set[str]:
            terms: set[str] = set()
            for word in re.findall("[A-Za-z][A-Za-z'-]{2,}|\\d[\\d.,/%-]*", text.lower()):
                cleaned = word.strip("'-.,/%")
                if not cleaned:
                    continue
                if cleaned[0].isdigit():
                    terms.add(cleaned)
                elif len(cleaned) >= 4 and cleaned not in LEDGER_STOPWORDS:
                    terms.add(cleaned)
            return terms
        ...
        class SourceAwareLedger:
            ...
            def __init__(self, question: str='') -> None:
                ...
                self.required_sources: list[dict[str, Any]] = []
                self.observed: dict[str, dict[str, Any]] = {}
                self.requirement_lines: list[str] = []
                self.retained_refs: list[str] = []
                self.note_required_sources(question)

            def note_required_sources(self, question: str) -> None:
                text = str(question or '')
                if not text.strip():
                    return
                lowered = text.lower()
                found: dict[str, dict[str, Any]] = {str(entry['token']): entry for entry in self.required_sources}
                for match in re.finditer('\\b([a-z0-9][a-z0-9-]*(?:\\.[a-z0-9-]+)+)\\b', lowered):
                    host = match.group(1)
                    labels = host.split('.')
                    if len(labels) < 2 or labels[-1] not in REQUIRED_SOURCE_TLDS:
                        continue
                    if any((label.isdigit() for label in labels)):
                        continue
                    found.setdefault(_ledger_token(host), {'token': _ledger_token(host), 'label': host, 'origin': 'domain'})
                for keyword in REQUIRED_SOURCE_KEYWORDS:
                    if keyword in lowered:
                        found.setdefault(_ledger_token(keyword), {'token': _ledger_token(keyword), 'label': keyword, 'origin': 'publisher'})
                for acronym in REQUIRED_SOURCE_ACRONYMS:
                    if re.search('(?<![A-Za-z])' + acronym + '(?![A-Za-z])', text):
                        found.setdefault(_ledger_token(acronym), {'token': _ledger_token(acronym), 'label': acronym, 'origin': 'publisher'})
                for match in re.finditer("(?:according to|as reported by|as published by|published by|as listed in|as listed on|listed in|listed on|drawn from|taken from|from the|in the|on the|using the)\\s+((?:[A-Z][A-Za-z0-9&'-]*|of|the|and|for|de|del)(?:\\s+(?:[A-Z][A-Za-z0-9&'-]*|of|the|and|for|de|del)){0,5}\\s+[a-z]+)", text):
                    phrase = ' '.join(match.group(1).split())
                    tail = phrase.rsplit(' ', 1)[-1].lower()
                    if tail not in REQUIRED_SOURCE_NOUNS:
                        continue
                    if not re.search('[A-Z]', phrase):
                        continue
                    found.setdefault(_ledger_token(phrase), {'token': _ledger_token(phrase), 'label': phrase, 'origin': 'named_artifact'})
                for match in re.finditer('\\b(?:' + '|'.join(REQUIRED_SOURCE_NOUNS) + ")\\s+(?:of|for|from|by)\\s+(?:the\\s+)?([A-Z][A-Za-z0-9&'-]*(?:\\s+(?:[A-Z][A-Za-z0-9&'-]*|of|and)){0,4})", text):
                    publisher = ' '.join(match.group(1).split()).rstrip(',.;:')
                    if len(publisher) < 3:
                        continue
                    found.setdefault(_ledger_token(publisher), {'token': _ledger_token(publisher), 'label': publisher, 'origin': 'named_artifact'})
                self.required_sources = list(found.values())

            def note_requirements(self, requirements: str) -> None:
                lines: list[str] = []
                for raw_line in str(requirements or '').splitlines():
                    line = raw_line.strip().lstrip('-*0123456789.) ').strip()
                    if len(line) >= 20:
                        lines.append(line)
                self.requirement_lines = lines

            def observe(self, source: Source) -> None:
                host = ''
                host_match = re.match('(?:[a-z]+://)?([^/?#]+)', str(source.url or ''), flags=re.IGNORECASE)
                if host_match:
                    host = host_match.group(1).lower()
                entry = self.observed.get(source.ref)
                if entry is None:
                    entry = {'source_ref': source.ref, 'host': host, 'title': str(source.title or ''), 'url': str(source.url or ''), 'retained': False, 'notes': [], 'quote_terms': set()}
                    self.observed[source.ref] = entry
                else:
                    entry['host'] = host or entry['host']
                    entry['title'] = str(source.title or entry['title'])
                    entry['url'] = str(source.url or entry['url'])

            def mark_retained(self, source: Source, note: str, quote: str) -> None:
                self.observe(source)
                entry = self.observed[source.ref]
                entry['retained'] = True
                cleaned_note = str(note or '').strip()
                if cleaned_note and cleaned_note not in entry['notes']:
                    entry['notes'].append(cleaned_note)
                entry['quote_terms'] = set(entry['quote_terms']) | _ledger_terms(str(quote or ''))
                if source.ref not in self.retained_refs:
                    self.retained_refs.append(source.ref)

            def _match_tokens(self, requirement: dict[str, Any]) -> tuple[str, ...]:
                token = str(requirement['token'])
                aliases = REQUIRED_SOURCE_ALIASES.get(token)
                if aliases:
                    return tuple((_ledger_token(alias) for alias in aliases)) + (token,)
                return (token,)

            def _entry_matches(self, entry: dict[str, Any], match_tokens: tuple[str, ...]) -> bool:
                haystack = _ledger_token(' '.join([str(entry['host']), str(entry['title']), str(entry['url'])]))
                return any((candidate and candidate in haystack for candidate in match_tokens))

            def source_gap_report(self) -> list[dict[str, Any]]:
                gaps: list[dict[str, Any]] = []
                for requirement in self.required_sources:
                    match_tokens = self._match_tokens(requirement)
                    observed_refs = [str(entry['source_ref']) for entry in self.observed.values() if self._entry_matches(entry, match_tokens)]
                    retained_refs = [str(entry['source_ref']) for entry in self.observed.values() if entry['retained'] and self._entry_matches(entry, match_tokens)]
                    if retained_refs:
                        continue
                    gaps.append({'required_source': str(requirement['label']), 'origin': str(requirement['origin']), 'status': 'observed_not_retained' if observed_refs else 'never_observed', 'observed_source_refs': observed_refs[:4]})
                return gaps

            def coverage_gaps(self, answer: str='') -> list[dict[str, Any]]:
                if not self.requirement_lines:
                    return []
                haystack: set[str] = _ledger_terms(str(answer or ''))
                for entry in self.observed.values():
                    if not entry['retained']:
                        continue
                    haystack |= set(entry['quote_terms'])
                    for note in entry['notes']:
                        haystack |= _ledger_terms(note)
                gaps: list[dict[str, Any]] = []
                for line in self.requirement_lines:
                    terms = _ledger_terms(line)
                    if len(terms) < SOURCE_REPAIR_MIN_REQUIREMENT_TERMS:
                        continue
                    covered = len(terms & haystack)
                    ratio = covered / len(terms)
                    if ratio >= SOURCE_REPAIR_MIN_COVERAGE_RATIO:
                        continue
                    gaps.append({'evidence_question': line, 'covered_term_ratio': round(ratio, 2), 'missing_terms': sorted(terms - haystack)[:8]})
                return gaps

            def provenance_digest(self) -> dict[str, Any]:
                return {'required_sources': [str(item['label']) for item in self.required_sources], 'retained_provenance': [{'source_ref': str(entry['source_ref']), 'host': str(entry['host'])} for entry in self.observed.values() if entry['retained']]}

            def repair_brief(self, answer: str='') -> str:
                if not self.retained_refs:
                    return ''
                source_gaps = self.source_gap_report()[:SOURCE_REPAIR_MAX_REPORTED_GAPS]
                uncovered = self.coverage_gaps(answer)
                if self.requirement_lines and len(uncovered) * 2 > len(self.requirement_lines):
                    uncovered = []
                uncovered = uncovered[:SOURCE_REPAIR_MAX_REPORTED_GAPS]
                if not source_gaps and (not uncovered):
                    return ''
                sections: list[str] = []
                for gap in source_gaps:
                    label = str(gap['required_source'])
                    if gap['status'] == 'observed_not_retained':
                        visible = ', '.join((f'[{ref}]' for ref in gap['observed_source_refs']))
                        sections.append(f'The question requires {label} as a source. It was retrieved ({visible}) but no excerpt from it was retained, so the answer does not currently cite it. Retain the exact lines from that source that carry the premise.')
                    else:
                        sections.append(f'The question requires {label} as a source and nothing from it has been retrieved. Search or fetch that source directly and retain the lines carrying the premise.')
                for gap in uncovered:
                    missing = ', '.join((str(term) for term in gap['missing_terms']))
                    sections.append(f"This evidence question is not touched by any retained excerpt or by the current answer: {gap['evidence_question']} Terms absent from all retained evidence: {missing}. Either retain the source text that settles it, or update the research state to record why supported filtering made it immaterial.")
                return '\n\n'.join(sections)
        ...
        class ResearchState:
            ...
            def __init__(self, question: str='') -> None:
                ...
                self.research_state = ''
                self.audit_gap = ''
                self.source_ledger = SourceAwareLedger(question)
                self.source_repair_gap = ''
        ...
        def _refresh_retrieval_receipt_message(messages: list[Any], state: ResearchState) -> None:
            ...
            if not state.research_state and (not state.audit_gap) and (not state.source_repair_gap) and (not state.budget_snapshot) and (not state.retrieval_receipts) and (not state.vfs_operation_receipts) and (not state.retained_evidence) and (not state.focused_lines) and (not state.reasoning_observations):
                return
            sections: list[str] = []
            ...
            if state.source_repair_gap:
                sections.append('Harness source-provenance check. This is a mechanical comparison of the source constraints in the original question against the provenance of the evidence actually retained, not a model judgement. Close it with the smallest useful operation before finalizing again:\n' + state.source_repair_gap)
            if state.audit_gap:
            ...
            if state.source_ledger.required_sources and state.retained_evidence:
                sections.append('Source provenance ledger. The original question names these sources explicitly, so they remain required premises. The second list is the host behind each retained excerpt. A required source with no matching host is not yet supported:\n' + json.dumps(state.source_ledger.provenance_digest(), ensure_ascii=False, separators=(',', ':')))
        ...
        async def _execute_search(state: ResearchState, args: dict[str, Any], preview_budget_chars: int | None=None) -> dict[str, Any]:
            ...
            for index, item in enumerate(result.response.data):
                ...
                state.source_ledger.observe(state.sources[ref])
        ...
        def _execute_retain_evidence(state: ResearchState, args: dict[str, Any]) -> dict[str, Any]:
            ...
            state.source_ledger.mark_retained(source, note, str(retained.get('quote', '')))
        ...
        async def _investigate(question: str, expected_answer: str) -> tuple[str, list[CitationRef]]:
            ...
            for _turn in range(160):
                ...
                for call_index, call in enumerate(calls):
                    ...
                    try:
                        ...
                        if call.name == 'set_evidence_requirements':
                            ...
                            state.source_ledger.note_requirements(requirements)
                            output = {'ok': True}
                ...
                if ready_requested:
                    ...
                    repair_brief = ''
                    state.source_repair_gap = ''
                    if source_repair_rounds < MAX_SOURCE_REPAIR_ROUNDS and audit_elapsed < SOURCE_REPAIR_DEADLINE_SECONDS:
                        try:
                            repair_brief = state.source_ledger.repair_brief(current_answer)
                        except Exception:
                            repair_brief = ''
                    if repair_brief:
                        verdict, audit_payload = ('REPAIR', repair_brief)
                        final_audit = ''
                    elif audit_elapsed >= TIME_GOVERNOR_ABSOLUTE_SECONDS:
                        final_audit = ''
                        state.audit_gap = ''
                        audit_ready = True
                    ...
                    if verdict == 'CONTINUE' and audit_continue_rounds >= MAX_AUDIT_CONTINUE_ROUNDS:
                        ...
                        final_audit = ''
                    if verdict == 'REPAIR':
                        source_repair_rounds += 1
                        state.source_repair_gap = audit_payload
                        state.clear_focused_lines()
                        audit_ready = False
                        messages = [{'role': 'system', 'content': INVESTIGATION_SYSTEM}, {'role': 'user', 'content': f'Original question:\n{question}\n\nThe harness compared the source constraints stated in the original question against the provenance of the evidence you retained and found this mismatch:\n{audit_payload}\n\nThe harness will preserve the existing VFS, source references, retained evidence, retrieval receipts, and research state. This is a provenance check, not a verdict on your answer: if the answer is already correct, it stays. Close the mismatch with the smallest useful operation, then finalize. If supported evidence shows the flagged premise is immaterial, record that with update_research_state and finalize instead of researching further.'}]
        ...
        async def query(query: Query) -> Response:
            ...
            return Response(text=answer, citations=citations)
        _PERFECT_SUFFIX = '5c70ed6465e6b2dd'
        return query
