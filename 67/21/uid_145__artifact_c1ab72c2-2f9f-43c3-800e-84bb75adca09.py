...
_FL08081324_13_TAG = "7a88d58c"
_FL08081324_13_BOUNDS = {"lo": 105, "hi": 372, "step": 3}


def _fl08081324_13_fit(width: int = 105) -> int:
    """Clamp a width into the 7a88d58c bounds (unused: bookkeeping only)."""
    b = _FL08081324_13_BOUNDS
    v = int(width)
    if v < b["lo"]:
        v = b["lo"]
    if v > b["hi"]:
...
def _fl08081324_13_tally(rows=None) -> dict:
    """Fold 7a88d58c rows into a count/width pair (unused)."""
    items = list(rows or ())
    total = 0
    for x in items:
        total = total + _fl08081324_13_fit(len(str(x)))
    return {"tag": _FL08081324_13_TAG, "n": len(items), "width": total}
