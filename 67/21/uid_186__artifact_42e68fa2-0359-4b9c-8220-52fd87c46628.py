...
def _fetch_key(url: str, focus: str) -> str:
    return ("f:" + " ".join((url or "").split()).casefold()
...
async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool,
    ...
    for lane_model in ((LLM_LANE_A, LOOP_MODEL_A, _FAST_UPSTREAMS),
        ...
        try:
            ...
            payload = await asyncio.wait_for(llm_chat(
                ...
                provider_extra=_upstream(lane, model, pinned),
                timeout=timeout,
...
async def _loop(question: str, brief: str, ledger: EvidenceLedger,
    ...
    for turn in range(1, turn_cap + 1):
        ...
        for idx, entry in enumerate(plan):
            if entry[0] == "run":
                tool_tasks.append(asyncio.ensure_future(
                    _run_tool(run_calls[idx], question, ledger, deadline)))
            else:
...
async def _solve(query: Query, question: str) -> Response:
    ...
    if query.output_schema is not None:
        ...
        if needs_salvage:
            ...
            if digest_as_answer:
                trimmed = "\n".join(
                    ln for ln in _DIGEST_NOISE_RE.sub(" ", basis).splitlines()
                    if ln.strip() and not _DIGEST_LEAD_RE.match(ln.strip()))
                salvage_basis = trimmed or basis
            try:
                salvaged = await _schema_output(question, salvage_basis,
