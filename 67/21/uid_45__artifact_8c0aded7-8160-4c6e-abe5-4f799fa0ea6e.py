"""hk402 "temporal+recall" — champion-v52 toolloop + temporal-alignment repair + free recall_evidence tool.
    ...
    },
{
    ...
    },
    {
        ...
        "function": {
            ...
            "description": ("Search every tool result you ALREADY gathered this "
                            "run — all search excerpts and fetched pages at once "
                            "— free and instant. Returns the best-matching "
                            "regions with their [n] numbers so you can cite or "
                            "retain them. Use this BEFORE paying for a repeat "
                            "web_search on something you may have already seen, "
                            "and when you remember reading a value but not where."),
            "parameters": {
                "type": "object",
                "properties": {"query": {"type": "string",
                                         "description": ("the fact or phrase to "
                                                         "look for in gathered "
                                                         "evidence")}},
                "required": ["query"],
            },
        },
...
# ── recall_evidence: free re-query of the evidence already gathered ──────────
# A repeat web_search is a flat $0.005 and a re-fetch costs latency, yet by
# mid-run the answer to "wait, which page had that figure?" is usually already
# in the ledger — spread across rows the model saw hundreds of turns-tokens ago.
# This tool greps ALL stored row text at once (search excerpts and full fetched
# pages), returns the densest window per matching row labelled with its [n], and
# APPENDS that window to the row's shown spans so anything surfaced here stays
# inside the judge-materialized slice (the coverage invariant ref_for enforces).
# Purely additive: if the model never calls it, behavior is byte-identical.
RECALL_WINDOW = 700
RECALL_MAX_ROWS = 3
RECALL_MAX_SPANS = 4       # ref_for reads spans[:4]; past that a span is invisible
...
def _do_recall_evidence(query: str, ledger: EvidenceLedger) -> str:
    try:
        ...
        scored: list[tuple[int, int, tuple[int, int]]] = []   # (hits, n, window)
        for idx, row in enumerate(ledger.rows):
            ...
            if not isinstance(spans, list) or len(spans) >= RECALL_MAX_SPANS:
                continue      # cannot keep the coverage invariant — skip the row
        ...
        if not scored:
            return (f"# recall_evidence({q!r}): nothing gathered so far matches — "
                    "this needs a real web_search or read_page.")
        ...
        for hits, n, win in scored[:RECALL_MAX_ROWS]:
            ...
            row["spans"].append([a, b])     # shown => citable, like any fetch window
            label = row.get("title") or row.get("url") or row.get("kind") or "result"
            out.append(f"\n--- [{n}] {label} @{a}:{b} ---\n{text[a:b]}")
        return (f"# recall_evidence({q!r}) -> {len(out)} stored result(s); cite "
                "their [n] directly, or retain_evidence the exact quote:"
                + "".join(out))
