...
"""
    ...
    Champion lineages retry a failed search this way instead of giving up."""
        ...
        async def _do_fetch_orig(url: str, focus: str, question: str, ledger: EvidenceLedger) -> str:
            if not url.strip():
                return '# read_page: empty url'
            payload = None
            for _attempt in (0, 1):
    ...
    that carries an [n]: that is a real answer, however it opens."""
        ...
        def _v0805_schema_values_clean(output) -> bool:
            """True when structured field values look like entities, not proof essays."""
            if output is None:
                return False
        ...
        def _v0805_entity_only(text: str) -> str:
            """Strip proof body before schema coerce — keep the lead entity line only."""
            if not text:
                return ''
            cut = text.strip()
            for marker in ('\n\nCandidate pool', '\n\n**Candidate', '\nCandidate pool', '\n\nProof:', '\n\n**Proof', '\n\nCondition', '\n\n**Condition', '\n\nConditions', '\n\n**Conditions'):
        ...
        def _v0805_is_thin(response: Response) -> bool:
            """Heuristic: baseline answer is short, stubby, or uncited — worth a refine."""
        ...
        async def _v0805_write_cited(question: str, ledger: EvidenceLedger, deadline: float) -> str:
            """Second-path write: digest commit with the same floor as baseline rescues."""
        ...
        def _v0805_pack_response(query: Query, text: str, ledger: EvidenceLedger, baseline: Response) -> Response:
            """Build a Response from refined text, falling back to baseline on failure."""
            ...
            try:
                ...
                return baseline
        _V0805_C2_TAG = 'v0805-c2-url-cache'
        _V0805_URL_CACHE: dict[str, dict] = {}
        _V0805_URL_CACHE_MAX = 64
        _V0805_ORIG_DO_FETCH = None
        _V0805_C2_INSTALLED = False
        _V0805_URL_RE = _v0805_re.compile('https?://[^\\s<>\\"\')\\]]+', _v0805_re.I)

        def _v0805_c2_norm_url(url: str) -> str:
            u = (url or '').strip()
            if not u:
                return ''
            if '#' in u:
        ...
        def _v0805_c2_cache_put(url: str, *, receipt: str, rid: str, note: str) -> None:
            key = _v0805_c2_norm_url(url)
            if not key or not note:
                return
            _V0805_URL_CACHE[key] = {'receipt_id': receipt, 'result_id': rid, 'note': note, 'url': url, 'ts': _v0805_clock()}
            if len(_V0805_URL_CACHE) > _V0805_URL_CACHE_MAX:
                oldest = min(_V0805_URL_CACHE.items(), key=lambda kv: kv[1].get('ts', 0.0))[0]
                _V0805_URL_CACHE.pop(oldest, None)

        def _v0805_c2_render_cached(url: str, focus: str, question: str, cached: dict):
            note = cached.get('note') or ''
            receipt = cached.get('receipt_id') or ''
            rid = cached.get('result_id') or ''
            if not note or not receipt or (not rid):
                return None
            if len(note) <= FETCH_PLAIN_CHARS:
                row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': [(0, len(note))], 'title': url, 'url': url, 'preview': note[:1200], 'text': note}
                return ToolOutput(f'# read_page({url!r}) -> [{_SLOT.format(0)}] full page, {len(note)} chars (cache hit)\n{note}', [row])
            terms = _key_terms(question) | _key_terms(focus)
            windows = _best_windows(note, terms, FETCH_WINDOW_CHARS, k=FETCH_WINDOWS_PER_PAGE)
            row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': [(0, FETCH_HEAD_CHARS)] + list(windows), 'title': url, 'url': url, 'preview': note[windows[0][0]:windows[0][0] + 1200] if windows else note[:1200], 'text': note}
            head = note[:FETCH_HEAD_CHARS]
            sections = ''.join((f'\n--- section @{s} ---\n{note[s:e]}' for s, e in windows))
            return ToolOutput(f'# read_page({url!r}) -> [{_SLOT.format(0)}] {len(note)} chars total; head + the {len(windows)} most relevant section(s) shown (cache hit)\n--- head ---\n{head}{sections}', [row])

        async def _do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger):
            key = _v0805_c2_norm_url(url)
            if key and key in _V0805_URL_CACHE:
                hit = _v0805_c2_render_cached(url, focus, question, _V0805_URL_CACHE[key])
                if hit is not None:
                    return hit
            out = await _do_fetch_orig(url, focus, question, ledger)
            try:
                if isinstance(out, ToolOutput) and out.rows:
                    row0 = out.rows[0]
                    note = row0.get('text') or ''
                    if note and row0.get('receipt_id') and row0.get('result_id'):
                        _v0805_c2_cache_put(url, receipt=str(row0['receipt_id']), rid=str(row0['result_id']), note=str(note))
            except Exception:
                pass
            return out

        def _v0805_on_query_start() -> None:
            _V0805_URL_CACHE.clear()

        def _v0805_install_overlay() -> None:
            """No-op: the overlay is now installed statically.

    _do_fetch IS the cache wrapper (it calls _do_fetch_orig), so there is
    nothing to swap at runtime. Kept as a no-op so the call in query() and
    the idempotence contract stay untouched.
    """
            return

        def _v0805_c2_harvest_urls(question: str, baseline: Response) -> list[str]:
            blob = question + '\n' + _v0805_response_text(baseline)
            found: list[str] = []
            for m in _V0805_URL_RE.finditer(blob):
                u = m.group(0).rstrip('.,;:)')
                low = u.lower()
                if any((b in low for b in ('google.com/search', 'bing.com/search', 'duckduckgo.com'))):
                    continue
                if u not in found:
                    found.append(u)
            for key, row in list(_V0805_URL_CACHE.items()):
                u = str(row.get('url') or key)
                if u.startswith('http') and u not in found:
                    found.append(u)
            return found[:5]

        async def _v0805_c2_deepen(question: str, urls: list[str], deadline: float) -> EvidenceLedger:
            ledger = EvidenceLedger()
            foci = ['', 'table', 'statistics', 'results', 'list']
            for u in urls:
                if deadline - _v0805_clock() < 32.0:
                    break
                focus = foci[len(ledger.rows) % len(foci)]
                try:
                    out = await asyncio.wait_for(_do_fetch(u, focus, question, ledger), timeout=FETCH_TIMEOUT_S * 2 + 4.0)
        ...
        async def _v0805_refine(query: Query, baseline: Response, deadline: float) -> Response | None:
            ...
            if not question or deadline - _v0805_clock() < _V0805_REFINE_MIN_S:
                return baseline
            urls = _v0805_c2_harvest_urls(question, baseline)
            if not urls and (not _V0805_URL_CACHE):
                try:
                    tmp = EvidenceLedger()
                    seed = (_seed_queries(question, _needs_set_completeness(question)) or [question])[:1]
                    for s in seed:
                        out = await asyncio.wait_for(_do_search(s, tmp), timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
            ...
            if not urls:
                return baseline
            ledger = await _v0805_c2_deepen(question, urls, deadline)
            if not ledger.rows:
                return baseline
            ...
            if not _v0805_is_thin(baseline):
                ...
                if len(text) < len(base_txt) * 0.8 and _v0805_re.search('\\[\\d+\\]', base_txt):
                    return baseline
            return _v0805_pack_response(query, text, ledger, baseline)

        async def _s18_base_query(query: Query) -> Response:
            """v0805 idea candidate: baseline first, then one idea-specific refine rung."""
            try:
                _v0805_install_overlay()
            except Exception:
                pass
            try:
                _v0805_on_query_start()
            except Exception:
                pass
...
_TAG_51FA9535="51fa953545ff4aae9bb49431fd86d414"
import logging as _tag_logging_51fa9535
_tag_logging_51fa9535.getLogger("miner.tag").debug("tag=%s", _TAG_51FA9535)
