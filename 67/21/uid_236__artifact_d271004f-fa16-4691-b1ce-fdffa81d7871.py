...
LLM_LANE_B = "openrouter"          # fallback lane -- was "ai_gateway" (Vercel);
# repointed 2026-08-08 after that lane threw repeated 402 insufficient_funds
# errors mid-run even after a manual account top-up, making it an unreliable
# rescue rung. Kept as openrouter with a DIFFERENT model (below) rather than
# collapsing to a single rung: the ladder's purpose is a second, independently-
# failing attempt when lane A's model/upstream is the problem, and a distinct
# model still gives that even on the same provider string.
...
LOOP_MODEL_B = "qwen/qwen3.6-27b"  # was "zai/glm-5.2-fast" (ai_gateway-only id,
# not resolvable on any other provider); qwen3.6-27b is on OpenRouter's own
# allowlist (tool_models.py) and is the field-observed most-available/reliable
# model of the set -- a real second attempt, not a same-model retry.
    ...
    },
{
    ...
    },
    {
        ...
        "function": {
            "name": "compute",
            "description": ("Do arithmetic on numbers you already have -- sums, "
                            "differences, percent changes, ratios, comparisons. "
                            "LOOP_RULES already requires showing your arithmetic for "
                            "any derived answer (mean, total, rank, count); use this "
                            "tool to actually run it instead of doing it in your head, "
                            "so a derived number can never be a silent mental-math "
                            "error."),
            "parameters": {
                "type": "object",
                "properties": {
                    "operation": {"type": "string",
                                  "enum": ["sum", "difference", "percent_change",
                                          "ratio", "compare"],
                                  "description": ("sum/difference/ratio: pairwise on "
                                                  "`values`. percent_change: (b-a)/a*100 "
                                                  "on the first two `values`. compare: "
                                                  "returns which of `values` is largest "
                                                  "and smallest.")},
...
LOOP_RULES = (
    ...
    "CITE THE STATEMENT, NOT JUST THE TABLE: for any fact you had to compute or "
    "derive -- a ranking, a sum, a comparison, an intersection of two "
    "conditions -- if a source's text ALSO states that derived conclusion in "
    "prose (not just the raw inputs you computed it from), cite that "
    "stated-conclusion sentence, in addition to or instead of the raw table. A "
    "citation the judge can read as already proving the claim beats one that "
    "requires the judge to redo your arithmetic from a table slice. When the "
    "primary data required real computation, search specifically for a source "
    "that states the conclusion directly. DO NOT PAD CITATIONS: cite the "
    "fewest sources that each distinctly support a claim -- several "
    "overlapping or redundant citations for the same single fact read as "
    "noise, not thoroughness; drop a citation if another one already covers "
...
def _do_compute(operation: str, values: list) -> str:
    ...
    if operation == "compare":
        return (f"# compute: compare({nums}) = largest {max(nums)!r}, "
                f"smallest {min(nums)!r}")
...
async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool,
    ...
    # v33.2 COST (HISTORICAL -- measured against the old ai_gateway/glm-5.2-fast
    # lane B, since removed; not re-measured for the current qwen3.6-27b lane B):
    # lane B was the priciest model on the allowlist -- 2.10/6.60 per 1M vs lane
    # A's 0.8008/2.5168 -- and it returned EMPTY above a payload it couldn't
    # handle, while still billing for the prompt. Last batch: 7 lane-B calls,
    # $0.518 (17% of spend); the two that returned zero completion tokens had
    # 50,444 and 37,227 prompt tokens and cost $0.202, while every call that
    # produced output was <= 34,196. So above the threshold the fallback was
    # pure waste -- skip it and let the turn fail over to the existing
    # retry/rescue paths instead of paying for a guaranteed empty reply. The
    # LANE_B_MAX_PAYLOAD_CHARS gate below is kept as inherited caution but has
    # not been re-validated against qwen3.6-27b's actual failure profile.
...
async def _loop(question: str, brief: str, ledger: EvidenceLedger,
    ...
    for turn in range(1, turn_cap + 1):
        ...
        tool_budget = max(5.0, min(FETCH_TIMEOUT_SECONDS * 2 + 6.0,
...
# ── dedicated structured-output research pipeline ─────────────────────────────
# _loop above researches generally, in prose, and only gets converted to the
# schema shape at the very end by _schema_output. That conversion step has no
# visibility into which schema fields were actually researched, so a field the
# loop never targeted just comes back missing or guessed. This dedicated path
# instead tells the model, from the FIRST turn, exactly which fields the
# schema requires -- research is aimed at the shape from the start, not
# retrofitted onto it afterward.
STRUCTURED_LOOP_RULES = LOOP_RULES + (
    "\n\nSTRUCTURED OUTPUT COMPLETENESS: this question requires a specific JSON "
    "shape. If a schema field is an array/list of qualifying items, before you "
    "write the final answer, RE-READ the evidence you have ALREADY GATHERED "
    "(no new tool calls) for other items meeting the question's stated "
    "criteria -- an array missing a qualifying item scores exactly as wrong as "
    "an incorrect answer. This is a one-time re-read of existing evidence, not "
    "a license for more research: only issue ONE additional search, and only "
    "for a named field that currently has ZERO evidence -- never re-search a "
    "field you already have a cited value for just to double-check it."
)
...
def _schema_field_brief(schema) -> str:
    ...
    return ("STRUCTURED OUTPUT SHAPE (for orientation, not a per-field search "
            "checklist) — the final answer needs a well-cited value for each "
            "of these; research normally and keep this shape in mind, it is "
            "not a mandate to search for each field separately:\n" + "\n".join(lines))


async def _structured_loop(question: str, brief: str, ledger: EvidenceLedger,
                           deadline: float, turn_cap: int,
                           carry: list[dict] | None = None,
                           allow_tools_in_wrapup: bool = False) -> tuple[str, list[dict]]:
...
def _verbatim_from_source(value: str, ledger: EvidenceLedger) -> str:
    ...
    # Compound-label check FIRST, independent of the generic seen() gate below.
    # Batch 3f1a7810 task 5db263a3: emitted "Solar" would have been caught by
    # this check (source's own line reads "Renewables - Solar (23.0%)"), but
    # "Solar" is a common enough word that it also appears, unrelated, in
    # OTHER pages fetched during the same research session -- so gating this
    # check behind "does v appear anywhere at all" (seen(v)) made it bail out
    # before ever reaching the compound-line match. The compound-line match
    # itself (v as the exact tail of a dash/colon-joined phrase) is already a
    # specific, scoped signal and does not need that gate.
    if len(v) >= 3:
        pat = re.compile(r"(\w[\w\s]{0,40}?)\s*[-:–—]\s*" + re.escape(v) + r"\b")
        candidates = set()
        for src in texts:
    ...
    if m:
        ...
        if len(hits) == 2:
            ...
            # "Dammam (Ad-Dammam)": the short form only "appears" because it is
            # a substring of the long one, so the long one is the source's own
            # label. Unrelated words ("Riyadh (capital)") stay ambiguous and
            # are left alone.
...
# Judges repeatedly penalize redundant citations for a single claim ("Rule 12:
# do not reward citation count by itself; too many irrelevant, repetitive, or
# weakly related validated citations should count against answer quality"),
# and separately prefer whichever citation's note reads as a STATED
# conclusion over one that hands the judge a raw table to recompute from
# ("first answer's note explicitly says... second answer just provides the
# raw table slice"). Both were previously only prompted for; this scores and
# trims deterministically so it fires every run, not only when the model
# happens to follow the instruction.
def _prose_score(text: str) -> float:
    ...
    if digits / max(len(t), 1) > 0.15:
        score *= 0.6
    if re.search(r"\b(?:were|was|totaled|totalled|ranked|reported|accounted "
                 r"for|the (?:top|highest|lowest|largest|smallest))\b", t, re.I):
        score *= 1.4
    return score
...
def _citations_for(answer: str, ledger: EvidenceLedger) -> list[CitationRef]:
    ...
    for cluster in _citation_clusters(answer, len(ledger.rows)):
        if len(cluster) <= 2:
            continue          # not redundant -- leave it alone
        scored = sorted(
            cluster,
            key=lambda n: _prose_score(ledger.rows[n - 1].get("text") or ""),
            reverse=True,
        )
        drop.update(scored[2:])   # keep the 2 best-stated, trim the rest
...
async def _schema_output(question: str, answer: str, schema, deadline: float) -> object | None:
    ask = ("Convert the answer to a JSON value valid under the schema. Output "
           "ONLY the JSON value. If a field is an array of qualifying items, "
           "include EVERY qualifying item the answer supports -- do not "
           "truncate to one example when the answer names several; an array "
           "missing a qualifying item scores exactly as wrong as an incorrect "
           "value.\n\n"
           f"Schema:\n{json.dumps(schema)}\n\nQuestion:\n{question}\n\n"
           f"Answer:\n{answer[:14000]}")
...
# A lead sentence naming one of OUR OWN tools while also talking about the
# research process is narration even when it doesn't open with one of the
# fixed phrases above (e.g. "Using web_search, I found..."). Two conditions,
# not one: a tool name alone (a real answer can legitimately mention a filing
# by name) or process language alone (ordinary first-person prose) is too
# broad; both together is narration.
_TOOL_NAME_RE = re.compile(
    r"\b(?:web_search|sec_filing|read_page|page_grep|page_read|retain_evidence|compute)\b",
    re.IGNORECASE)
...
# The audit-patch stage (below) sometimes narrates ABOUT its own finding
# instead of just applying it -- "The audit's premise notes that X..." leaking
# into the shipped answer as commentary rather than content. Strip it the same
# way as lead narration: only uncited sentences, only when substantial cited
# text survives.
_AUDIT_META_RE = re.compile(
    ...
    r"|states|stated)\b", re.IGNORECASE)
