    ...
    def _compile(self):
        ...
        class EvidenceLedger:
            ...
            def add(self, receipt_id: str, result_id: str, note_len: int, kind: str, spans: list[tuple[int, int]] | None, title: str='', url: str='', preview: str='', note_text: str='') -> int:
                self.rows.append({'receipt_id': receipt_id, 'result_id': result_id, 'note_len': note_len, 'kind': kind, 'title': (title or '')[:160], 'url': (url or '')[:300], 'preview': (preview or '')[:1200], 'spans': spans, 'text': note_text or ''})
        ...
        def _commit_tool_output(out, ledger: EvidenceLedger) -> str:
            ...
            for i, row in enumerate(out.rows):
                n = ledger.add(row['receipt_id'], row['result_id'], row['note_len'], row['kind'], row['spans'], title=row.get('title', ''), url=row.get('url', ''), preview=row.get('preview', ''), note_text=row.get('text', ''))
        ...
        def _obedece_formato(question: str, answer: str) -> str:
            try:
                ...
                return answer
        D_MAX_POR_AFIRMACAO = 1

        def _d_apoio(row: dict, termos: set) -> int:
            txt = (row.get('text') or row.get('preview') or '').lower()
            if not txt or not termos:
                return 0
            return sum((1 for t in termos if t and t.lower() in txt))

        def _dedupe_citacoes(answer: str, ledger: EvidenceLedger) -> str:
            try:
                linhas = (answer or '').split('\n')
                saida = []
                for linha in linhas:
                    nums = [int(x) for x in re.findall('\\[(\\d+)\\]', linha)]
                    unicos = list(dict.fromkeys(nums))
                    if len(unicos) <= D_MAX_POR_AFIRMACAO:
                        saida.append(linha)
                        continue
                    base = re.sub('\\[\\d+\\]', ' ', linha)
                    termos = _key_terms(base) | set(re.findall('\\d+(?:[.,]\\d+)*', base))
                    melhor, best = (None, -1)
                    for n in unicos:
                        if not 1 <= n <= len(ledger.rows):
                            continue
                        s = _d_apoio(ledger.rows[n - 1], termos)
                        if s > best:
                            best, melhor = (s, n)
                    if melhor is None:
                        saida.append(linha)
                        continue
                    nova = re.sub('\\[(\\d+)\\]', lambda m: m.group(0) if int(m.group(1)) == melhor else '', linha)
                    nova = re.sub('[ \\t]{2,}', ' ', nova)
                    saida.append(nova)
                return '\n'.join(saida)
            except Exception:
        ...
        async def _solve(query: Query, question: str) -> Response:
            ...
            if not _is_usable_answer(answer):
                ...
                if _is_usable_answer(fallback):
                    answer = fallback
            answer = _dedupe_citacoes(answer, ledger)
            try:
