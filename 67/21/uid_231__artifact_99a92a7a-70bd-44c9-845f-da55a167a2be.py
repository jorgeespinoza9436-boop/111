...
class SecondPath:
    ...
    def _compile(self):
        ...
        _TAVILY_SEARCH_EXTRA = {'search_depth': 'advanced', 'chunks_per_source': 3}
        _PARALLEL_SEARCH_EXTRA = {'mode': SEARCH_MODE}
        WALL_BUDGET_S = 266.0
        BRIEF_TIMEOUT_S = 50.0
        TURN_TIMEOUT_S = 75.0
        AUDIT_TIMEOUT_S = 28.0
        ...
        async def _do_search(query_text: str, ledger: EvidenceLedger):
            ...
            payload = None
            fired: set[str] = set()
            for attempt, allow_repeat, prov, pextra in ((query_text, False, SEARCH_PROVIDER, _TAVILY_SEARCH_EXTRA), (query_text, True, SEARCH_FALLBACK_PROVIDER, _PARALLEL_SEARCH_EXTRA), (_degrade_query(query_text), False, SEARCH_FALLBACK_PROVIDER, _PARALLEL_SEARCH_EXTRA)):
                if not attempt.strip() or (attempt in fired and (not allow_repeat)):
                    continue
                fired.add(attempt)
                try:
                    payload = await search_web(attempt, provider=prov, num=8, timeout=SEARCH_TIMEOUT_S, provider_extra=pextra)
                    if getattr(payload, 'results', None):
                        break
                except Exception:
                    payload = None
        ...
        async def _do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger) -> str:
            ...
            for _attempt in (0, 1):
                try:
                    payload = await fetch_page(url, provider=FETCH_PROVIDER, timeout=FETCH_TIMEOUT_S)
                    if getattr(payload, 'results', None):
                        break
                except Exception:
                    payload = None
        ...
        async def _fetch_json(url: str, deadline: float):
            ...
            for _attempt in (0, 1):
                ...
                if left < 12.0:
                    return None
                try:
                    payload = await asyncio.wait_for(fetch_page(url, provider=FETCH_PROVIDER, timeout=min(_SEC_FETCH_TIMEOUT_S, left - 6.0)), timeout=min(_SEC_FETCH_TIMEOUT_S, left - 6.0) + 4.0)
                except Exception:
                    continue
                _spend_note(payload)
                results = list(getattr(payload, 'results', None) or [])
        ...
        def _least_think(lane: str, model: str='') -> dict:
            for prefix in _REASONING_MANDATORY:
                if model.startswith(prefix):
                    return {'enabled': True, 'effort': 'low'}
            return {'enabled': False}
        _UPSTREAM_PRIMARY = ('Decart',)
        _UPSTREAM_BACKUP = ('CoreWeave',)
        _FAST_UPSTREAMS = _UPSTREAM_PRIMARY
        ...
        async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool, force_tools: bool=False):
            ...
            for lane_model in ((LLM_LANE_A, LOOP_MODEL_A, _UPSTREAM_PRIMARY), (LLM_LANE_A, LOOP_MODEL_A, _UPSTREAM_BACKUP), (LLM_LANE_B, LOOP_MODEL_B, False)):
                lane = lane_model[0]
                model = lane_model[1]
                pinned = lane_model[2]
                ...
                if timeout <= 5.0:
                    return None
                try:
                    payload = await llm_chat(provider=lane, model=model, messages=messages, tools=LOOP_TOOLS if force_tools or not finish_only else None, tool_choice='auto' if force_tools or not finish_only else None, temperature=0.2, provider_extra=_upstream(lane, model, pinned), thinking={'enabled': False} if finish_only and lane == LLM_LANE_B else {'enabled': True, 'effort': 'low'}, max_output_tokens=6000 if finish_only and lane == LLM_LANE_B else None, timeout=timeout)
                    _spend_note(payload)
                    return payload
                except Exception:
                    continue
