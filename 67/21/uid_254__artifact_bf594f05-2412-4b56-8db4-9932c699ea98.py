...
STRUCTURAL HARDENING (this revision), logic and prompts otherwise unchanged:
  - openrouter only; every ai_gateway call path removed and replaced by the
    next model on the same lane;
...
# slot: harnyx 2026-08-07T14:18:25+00:00

# perfect_suffix: openrouter/parallel
_PERFECT_SUFFIX = "24a13617e97faecf"



# =====================================================================
# submittion20 MECHANISM — temporal-anchor completeness gate (staleness
# defense for un-anchored "current state" assertions) followed by
# cross-value basis/period reconciliation (comparability audit for
# quantitative or ranked figures stated across entities/periods)
# =====================================================================
#
# Runs after the base pipeline above has produced a draft Response.
# Earlier lineage stages in this family decompose by comparison SIDE
# (s19) or by arbitrary load-bearing CLAIM for correctness re-checking
# (s17), and s19 separately targets NEGATION/ABSENCE polarity. Neither
# stage below shares that decomposition target or verification
# semantics:
#   - Stage A targets POSITIVE "current state" assertions (incumbency,
#     standing records, active status, latest edition) that lack an
#     explicit temporal anchor, and verifies STALENESS/SUPERSESSION —
#     not factual contradiction and not existence/absence.
#   - Stage B targets stated VALUES already present in the draft for a
#     multi-entity/period/metric comparison, auditing whether their
#     BASIS (currency, fiscal vs calendar period, adjusted vs
#     unadjusted, methodology) is mutually consistent — not whether a
#     value is present at all (s19 Stage A) and not whether a claim is
#     internally correct (s17).
#
# STAGE A - temporal-anchor completeness gate (text AND structured
# JSON):
#   1. Code-level keyword pre-filter shortlists sentences bearing
#      current-state language (currently, remains, still holds,
#      incumbent, reigning, latest, to date, as of now, ongoing, ...).
#   2. An LLM confirms which shortlisted sentences are load-bearing
#      current-state assertions that LACK a specific temporal anchor
#      (no explicit date, version, or as-of qualifier), and produces a
#      staleness-testing search query for each ("has X changed since
#      / is Y still the case as of the latest available information")
#      rather than a fact-check query.
#   3. Issues ONE NEW, independently targeted search_web call per
#      confirmed gap (concurrently, capped at 3).
#   4. Verifies each against only its own freshly retrieved evidence
#      with an asymmetric staleness verdict distinct from a symmetric
#      contradiction check or an existence check: still_current
#      (evidence shows the state is unchanged as of a determinable
#      recent point -- triggers an anchor-insertion patch that adds
#      the as-of marker without altering the substance of the claim),
#      superseded (evidence shows the state has since changed --
#      triggers a corrective rewrite grounded in the newer fact), or
#      unverifiable (strict no-op for that sentence).
#
# STAGE B - cross-value basis/period reconciliation (text AND
# structured JSON):
#   1. Extracts, from the CURRENT DRAFT CONTENT (not the question),
#      every stated numeric/ranked value that participates in a
#      cross-entity, cross-period, or cross-item comparison sharing
#      one metric, together with whatever basis descriptor (currency,
#      fiscal/calendar period, adjusted/unadjusted, methodology,
#      as-of date) is explicitly attached to that value, or null.
#   2. Audits the extracted set for basis consistency, classifying the
#      comparison as aligned (bases match or normalization is not
#      required for this metric), mismatched (two or more values use
#      explicitly different, incompatible bases), or unstated (a
#      basis-sensitive metric is compared without any stated basis for
#      one or more values), producing one targeted search query per
#      mismatched/unstated finding aimed at the missing basis
#      descriptor or a comparable-basis figure.
#   3. Issues ONE NEW targeted search_web call per finding
#      (concurrently, capped at 3).
...
#      naming the flagged value(s) to state the basis explicitly and,
#      when the fresh evidence supplies a comparable-basis figure,
#      normalize the comparison -- otherwise add an explicit
#      non-comparability caveat rather than silently normalizing.
#
# This changes decomposition target (un-anchored current-state
# sentences; stated-value basis descriptors), verification semantics
# (staleness/supersession distinct from contradiction or existence;
# basis-comparability distinct from value-presence), and remediation
# control flow (anchor-insertion as a non-substantive patch path;
# explicit non-comparability caveats when normalization evidence is
# unavailable) relative to the base pipeline and relative to any
# earlier stage in this lineage; it is not a prompt or parameter
# tweak. Any failure, missing evidence, non-dict structured output, or
# time shortage is a strict no-op that returns the base pipeline's own
# response (after cheap exact duplicate-citation cleanup only).
...
_S20_CURRENT_STATE_KEYWORD_RE = _s20_re.compile(
    r"\b(currently|is\s+the\s+current|remains?\s+the|still\s+(?:holds?|serves?|leads?|"
    r"stands?)|to\s+date|as\s+of\s+(?:now|today|writing)?|the\s+incumbent|reigning|"
    r"ongoing|latest|most\s+recent|record\s+holder|so\s+far|present(?:ly)?\s+(?:the|"
    r"serves)|active\s+(?:champion|holder|record))\b",
    _s20_re.IGNORECASE,
)

_S20_ANCHOR_CONFIRM_SYSTEM_PROMPT = (
    "You audit candidate sentences from a research answer for load-bearing "
    "POSITIVE current-state assertions that LACK an explicit temporal anchor -- "
    "statements that something currently holds, remains true, is the latest, or "
    "is still the case, without a specific date, version, or as-of qualifier "
    "tying that state to a point in time.\n"
    "Discard candidates that already carry an explicit date, version number, or "
    "as-of qualifier elsewhere in the same sentence; discard stylistic, trivial, "
    "or non-factual current-state phrasing (e.g. 'currently' used as filler); "
    "discard claims about stable facts that do not meaningfully change over "
    "time.\n"
    "For each CONFIRMED un-anchored current-state claim, write a short, "
    "targeted web search query (5-15 words) phrased to test whether that exact "
    "state has changed or is still accurate as of the latest available "
    "information -- not a restatement of the claim itself.\n"
    "Return JSON only: {\"gaps\": [{\"sentence\": str, \"search_query\": str}, "
    "...]}. Return an empty list if none of the candidates qualify."
)

_S20_STALENESS_VERIFY_SYSTEM_PROMPT = (
    "You are a strict staleness-verification auditor for ONE un-anchored "
    "current-state claim from a research answer.\n"
    "You receive the claim and up to 4 freshly retrieved, independent evidence "
    "snippets gathered specifically to test whether that state has changed.\n"
    "Classify strictly from this evidence:\n"
    "- still_current: a snippet directly shows the state is unchanged as of a "
    "determinable recent point; report that point as as_of_date (a short "
    "human-readable date or period string, not necessarily an ISO date).\n"
    "- superseded: a snippet directly shows the state has since changed -- "
    "report the new state as correction.\n"
    "- unverifiable: the evidence neither confirms currency nor shows a "
    "change.\n"
    "Return JSON only with keys: verdict "
    "('still_current'|'superseded'|'unverifiable'), "
    "as_of_date (string or null, only for still_current), "
    "correction (string or null, only for superseded), "
    "supporting_snippet_indices (array of 0-based ints, may be empty)."
)

_S20_PATCH_ANCHOR_SYSTEM_PROMPT = (
    "You add an explicit temporal anchor to ONE current-state sentence inside "
    "a research answer, using freshly retrieved evidence that confirms the "
    "state is still accurate as of a specific point.\n"
    "Rewrite the COMPLETE answer: keep every part unrelated to this sentence "
    "byte-for-byte, and edit only the flagged sentence to add the as-of date "
    "or period the evidence supports, without changing what the sentence "
    "otherwise claims. Do not soften or strengthen the underlying assertion; "
...
_S20_PATCH_SUPERSESSION_SYSTEM_PROMPT = (
    "You correct ONE stale current-state claim inside a research answer using "
    "freshly retrieved evidence that shows the state has since changed.\n"
    "Rewrite the COMPLETE answer: keep every part unrelated to this claim "
    "byte-for-byte where feasible, and replace only the outdated assertion "
    "with the newer state the fresh evidence supports, including when the "
    "change occurred if the evidence gives that detail.\n"
    "Preserve all existing citation markers whose underlying claims are "
    "unchanged. Output plain answer text only: no preamble, no markdown "
    "fences, no meta-commentary about the correction process."
)

_S20_BASIS_EXTRACT_SYSTEM_PROMPT = (
    "You extract quantitative comparison points from a research answer (text "
    "or JSON).\n"
    "Find every place the answer states or compares a numeric or ranked value "
    "(money amount, percentage, count, rate, score, or rank) for 2 or more "
    "distinct named entities, periods, or items that share one metric.\n"
    "If no such multi-value comparison exists in the content, set "
    "is_comparison to false and leave other fields empty.\n"
    "Otherwise, name the shared metric, and for up to 5 of the compared "
    "values, give: the entity/item/period it belongs to, and the basis "
    "descriptor explicitly stated for that value in the content (currency, "
    "fiscal-vs-calendar period, adjusted-vs-unadjusted, methodology or "
    "reporting standard, or an explicit as-of date) -- or null if the content "
    "states no basis for that value at all.\n"
    "Only report values that are actually present in the given content; do "
    "not infer values the content does not state.\n"
    "Return JSON only: {\"is_comparison\": bool, \"metric\": str, \"values\": "
    "[{\"index\": int, \"entity\": str, \"basis\": str or null}, ...]}."
)

_S20_BASIS_AUDIT_SYSTEM_PROMPT = (
    "You are a strict basis-comparability auditor for a quantitative "
    "comparison inside a research answer.\n"
    "You receive the shared metric and a list of compared values, each with "
    "its 0-based index, the entity/item/period it belongs to, and its stated "
    "basis descriptor (or null if none was stated).\n"
    "Decide one overall verdict:\n"
    "- aligned: every value shares the same basis, or this metric does not "
    "require a basis descriptor to be meaningfully compared (e.g. simple "
    "counts of discrete named items).\n"
    "- mismatched: two or more values explicitly state different, "
    "incompatible bases (different currencies, different period types, "
    "adjusted vs unadjusted, or different methodologies).\n"
    "- unstated: this metric is basis-sensitive (money, fiscal or "
    "period-bound figures, methodology-dependent rates) and one or more "
    "compared values have no stated basis at all.\n"
    "When the verdict is mismatched or unstated, list up to 3 finding items, "
    "each naming one value by its index and giving a short, targeted web "
    "search query (5-15 words) that would source that value's basis "
    "descriptor or a version of that value expressed on a comparable basis to "
    "the others -- never a restatement of the whole original question.\n"
    "Return JSON only: {\"verdict\": \"aligned\"|\"mismatched\"|\"unstated\", "
    "\"findings\": [{\"index\": int, \"gap_query\": str}, ...]}. Return an "
    "empty findings list when the verdict is aligned."
)

_S20_PATCH_BASIS_TEXT_SYSTEM_PROMPT = (
    "You correct ONE value's basis inside a comparison research answer, using "
    "freshly retrieved evidence.\n"
    "Rewrite the COMPLETE answer: keep every part unrelated to this one value "
    "byte-for-byte where feasible, and edit only the span naming this value to "
    "state its basis explicitly. If the evidence gives a comparable-basis "
    "figure for this value, use it to make the comparison apples-to-apples. If "
    "the evidence does not resolve a comparable figure, keep the original "
    "value but add a brief explicit caveat that it is not on the same basis as "
    "the other compared value(s).\n"
    "Preserve all existing citation markers whose underlying content is "
    "unchanged. Output plain answer text only: no preamble, no markdown "
    "fences, no meta-commentary about this process."
)

_S20_PATCH_BASIS_OUTPUT_SYSTEM_PROMPT = (
    "You correct ONE value's basis inside a structured JSON comparison "
    "answer, using freshly retrieved evidence.\n"
    "You receive the target JSON schema (if any), the CURRENT JSON answer, "
    "which one compared value (by entity/item) needs its basis addressed, and "
    "fresh evidence snippets gathered to resolve it.\n"
    "Return ONLY the JSON keys (top-level, or one level nested) whose values "
    "must be added or corrected to state this value's basis, or to replace it "
    "with a comparable-basis figure, using ONLY key names that already exist "
    "in the schema or current answer -- never invent new keys. If the "
    "evidence does not give a confident comparable-basis figure, return an "
    "empty patch rather than guessing.\n"
...
def _s20_strip_json_fences(raw: str) -> str:
    return _s20_re.sub(r"^```(?:json)?\s*|\s*```$", "", raw or "", flags=_s20_re.I | _s20_re.M).strip()
...
async def _s20_search_gap(search_query: str):
    ...
    for provider_name in ("parallel", "desearch"):
        try:
            payload = await _s20_search_web(
                search_query[:300],
                provider=provider_name,
                num=4,
                timeout=_S20_SEARCH_TIMEOUT_S,
            )
        except Exception:
            payload = None
        if payload is None:
...
def _s20_build_refs(receipt_id: str, evidence_items: list, indices) -> list:
    ...
    for raw_idx in (indices or []):
        ...
        if end <= 0:
            continue
        try:
            refs.append(_s20_citation_ref(
                receipt_id=receipt_id,
                result_id=item["result_id"],
                slices=[_s20_citation_slice(start=0, end=end)],
            ))
        except Exception:
            continue
...
# --------------------------------------------------------------------
# Stage A -- temporal-anchor completeness gate
# --------------------------------------------------------------------

def _s20_shortlist_anchor_sentences(answer: str) -> list:
    sentences = _s20_re.split(r"(?<=[.!?])\s+", answer)
    candidates = []
...
async def _s20_confirm_anchor_gaps(question: str, candidates: list) -> list:
    ...
    try:
        result = await _s20_llm_chat(
            provider="openrouter",
            model=_S20_MODEL,
            messages=[
                {"role": "system", "content": _S20_ANCHOR_CONFIRM_SYSTEM_PROMPT},
                {
                    "role": "user",
                    "content": f"Question:\n{question}\n\nCandidate sentences:\n{candidates_block}",
                },
            ...
            timeout=_S20_ANCHOR_CONFIRM_TIMEOUT_S,
...
async def _s20_verify_staleness(claim: str, evidence_items: list) -> dict:
    ...
    try:
        result = await _s20_llm_chat(
            ...
            model=_S20_MODEL,
            messages=[
                {"role": "system", "content": _S20_STALENESS_VERIFY_SYSTEM_PROMPT},
                {
                    "role": "user",
                    "content": f"Current-state claim:\n{claim}\n\nFresh evidence snippets:\n{evidence_block}",
                },
            ],
            tools=None,
            temperature=0.0,
            max_output_tokens=350,
            timeout=_S20_STALENESS_VERIFY_TIMEOUT_S,
            thinking={"enabled": False},
        )
    except Exception:
...
async def _s20_patch_anchor(question: str, answer: str, claim: str, as_of_date: str, evidence_block: str) -> str:
    ...
    prompt = (
        f"Question:\n{question}\n\n"
        f"Current answer:\n{answer[:12000]}\n\n"
        f"Current-state sentence needing an as-of anchor:\n{claim}\n\n"
        f"Confirmed as-of point to anchor it to:\n{as_of_date or 'see evidence below'}\n\n"
    ...
    try:
        result = await _s20_llm_chat(
            ...
            model=_S20_MODEL,
            messages=[
                {"role": "system", "content": _S20_PATCH_ANCHOR_SYSTEM_PROMPT},
                {"role": "user", "content": prompt},
            ],
            tools=None,
            temperature=0.1,
            max_output_tokens=1400,
            timeout=_S20_PATCH_TIMEOUT_S,
            thinking={"enabled": False},
        )
    except Exception:
        return ""
...
async def _s20_patch_supersession(question: str, answer: str, claim: str, correction: str, evidence_block: str) -> str:
    ...
    prompt = (
        f"Question:\n{question}\n\n"
        f"Current answer:\n{answer[:12000]}\n\n"
        f"Stale current-state claim being corrected:\n{claim}\n\n"
    ...
    try:
        result = await _s20_llm_chat(
            ...
            model=_S20_MODEL,
            messages=[
                {"role": "system", "content": _S20_PATCH_SUPERSESSION_SYSTEM_PROMPT},
                {"role": "user", "content": prompt},
            ],
            tools=None,
            temperature=0.1,
...
async def _s20_temporal_anchor_gate(_s20_query, _s20_response):
    if getattr(_s20_response, "output", None) is not None:
        return _s20_response  # structured outputs are out of scope for this stage
    ...
    search_results = await _s20_asyncio.gather(
        *[_s20_search_gap(g["search_query"]) for g in gaps],
    ...
    verify_results = await _s20_asyncio.gather(
        *[_s20_verify_staleness(g["sentence"], sr["items"]) for g, sr in per_gap],
        return_exceptions=True,
    )
    ...
    for (gap, search_result), verdict_report in zip(per_gap, verify_results):
        ...
        if verdict == "still_current":
            new_text = await _s20_patch_anchor(
                question, running_answer, gap["sentence"],
                str(verdict_report.get("as_of_date") or ""), evidence_block,
            )
            if new_text:
                running_answer = new_text
                refs = _s20_build_refs(
                    receipt_id, items,
        ...
        if verdict == "superseded":
            new_text = await _s20_patch_supersession(
                question, running_answer, gap["sentence"],
...
# --------------------------------------------------------------------
# Stage B -- cross-value basis/period reconciliation
# --------------------------------------------------------------------
...
async def _s20_extract_basis_values(content_repr: str, is_structured: bool) -> dict | None:
    ...
    try:
        result = await _s20_llm_chat(
            ...
            model=_S20_MODEL,
            messages=[
                {"role": "system", "content": _S20_BASIS_EXTRACT_SYSTEM_PROMPT},
                {"role": "user", "content": f"{label}:\n{content_repr[:12000]}"},
            ],
            tools=None,
            temperature=0.0,
            max_output_tokens=500,
            timeout=_S20_BASIS_EXTRACT_TIMEOUT_S,
            thinking={"enabled": False},
        )
    except Exception:
        return None
...
async def _s20_audit_basis(comparison: dict) -> dict | None:
    ...
    values_block = "\n".join(
        f"{v['index']}. entity={v['entity']!r} basis={v['basis']!r}" for v in comparison["values"]
    )
    user_content = (
        f"Shared metric: {comparison['metric'] or '(unspecified)'}\n\n"
        f"Compared values:\n{values_block}"
    )
    try:
        result = await _s20_llm_chat(
            ...
            model=_S20_MODEL,
            messages=[
                {"role": "system", "content": _S20_BASIS_AUDIT_SYSTEM_PROMPT},
                {"role": "user", "content": user_content},
            ],
            tools=None,
            temperature=0.0,
            max_output_tokens=500,
            timeout=_S20_BASIS_AUDIT_TIMEOUT_S,
            thinking={"enabled": False},
        )
    except Exception:
        return None
...
async def _s20_patch_basis_text(answer: str, entity: str, gap_query: str, evidence_block: str) -> str:
    ...
    prompt = (
        f"Current answer:\n{answer[:12000]}\n\n"
        f"Value needing an explicit or normalized basis:\n{entity}\n\n"
    ...
    try:
        result = await _s20_llm_chat(
            ...
            model=_S20_MODEL,
            messages=[
                {"role": "system", "content": _S20_PATCH_BASIS_TEXT_SYSTEM_PROMPT},
                {"role": "user", "content": prompt},
            ],
            tools=None,
            temperature=0.1,
...
async def _s20_patch_basis_output(
    schema_compact: str,
    ...
    try:
        result = await _s20_llm_chat(
            ...
            model=_S20_MODEL,
            messages=[
                {"role": "system", "content": _S20_PATCH_BASIS_OUTPUT_SYSTEM_PROMPT},
                {"role": "user", "content": prompt},
            ],
            tools=None,
            temperature=0.0,
...
async def _s20_basis_reconciliation_gate(_s20_query, _s20_response):
    ...
    search_results = await _s20_asyncio.gather(
        *[_s20_search_gap(f["gap_query"]) for f in findings],
        return_exceptions=True,
    )
    ...
    for finding, search_result in per_finding:
        ...
        if is_structured:
            patch_result = await _s20_patch_basis_output(
                schema_compact, _s20_compact_json(running_output),
                finding["entity"], finding["gap_query"], evidence_block,
            )
            if not patch_result:
                continue
            ...
            patched = await _s20_patch_basis_text(
                running_text, finding["entity"], finding["gap_query"], evidence_block,
            )
            if not patched:
                continue
...
async def _s20_finalize(_s20_query, _s20_response, _s20_t0: float):
    """Bounded temporal-anchor completeness gate + basis reconciliation pass."""
    ...
    try:
        return await _s20_asyncio.wait_for(
            _s20_run_stages(_s20_query, _s20_response),
            timeout=window,
        )
    except Exception:
        return _s20_dedup_citations(_s20_response)
