...
async def query(query: Query) -> Response:
    ...
    # Bootstrap: seed grounded evidence via search_many (parallel multi-query tool path).
    try:
        seeds = _seed_queries(query.text)
        seeded_blob = await asyncio.wait_for(
            _do_search_many(seeds, ledger, time_left=SEARCH_TIMEOUT_S + 6.0),
