...
async def _answer(query: Query, question: str) -> Response:
    ...
    try:
        if answer and _remaining(deadline) > 25.0 and _looks_hedged(answer):
            _rw = await _plain_chat(
                LOOP_MODEL,
                system="You rewrite research answers to commit. Keep every [n] bracket.",
                user=(COMMIT_REWRITE_INSTRUCTION + "\n\nQuestion:\n" + question
                      + "\n\nAnswer:\n" + answer[:12000]),
                max_tokens=2000,
                timeout=min(40.0, _remaining(deadline) - 8.0),
            )
            if _rw and not _looks_hedged(_rw):
                answer = _rw
    except Exception:
        pass
...
_MARKER_VECTOR_20305 = "fa76bdb87844"


def _normalize_vector_20305(items=(), *, base=92118):
    total = base
