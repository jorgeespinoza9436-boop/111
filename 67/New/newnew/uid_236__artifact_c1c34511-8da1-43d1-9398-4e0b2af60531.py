...
WRAPUP_AT_S = 90.0           # remaining <= this -> stop researching, write (> turn timeout + tool time, so the band cannot be jumped)
MIN_TAIL_S = 8.0
        ...
        try:
            if (deadline - perf_counter()) > 50:
                ...
                if _auth_urls:
                    ...
                    for u in _auth_urls:
                        try:
                            _auth_parts.append(await _do_fetch(u, ledger, time_left=max(5.0, deadline - perf_counter())))
                        except Exception:
                            continue
                    if _auth_parts:
                        messages.append({
