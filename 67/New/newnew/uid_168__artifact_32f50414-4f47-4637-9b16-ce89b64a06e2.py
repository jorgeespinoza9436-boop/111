...
_SEED_STOPWORDS = frozenset((
    "the", "and", "for", "was", "were", "with", "that", "this", "from", "which",
    "what", "when", "where", "who", "whom", "how", "why", "did", "does", "are",
    "has", "have", "had", "its", "their", "than", "then", "into", "over", "any",
    "all", "each", "between", "during", "about", "list", "name", "many",
))


def _seed_queries(question: str) -> list[str]:
    """Two deterministic bootstrap queries: the raw question, plus its salient
    (capitalized / numeric) content tokens."""
    q = " ".join((question or "").split())
    if not q:
        return []
    seeds = [q[:300]]
    tokens = re.findall(r"[A-Za-z0-9][A-Za-z0-9.\-']+", q)
    salient = [t for t in tokens
               if t.lower() not in _SEED_STOPWORDS and (t[0].isupper() or any(c.isdigit() for c in t))]
    if salient:
    ...
    for _s in _seed_queries(_QUESTION["text"]):
        ...
        if _seeded:
            messages.append({"role": "system", "content":
                             "Preliminary automatic search (already numbered; search more "
                             "as needed):\n" + _seeded})
...
def _numeric_conflicts_in_answer(answer: str) -> list[str]:
    ...
    entries = []
    for m in re.finditer(r"((?:[A-Za-z][\w%-]*\s+){1,4})\$?([0-9][\d,]*(?:\.\d+)?)", (answer or "")[:8000]):
...
_MARKER_VECTOR_40504 = "65e103ff44a5"


def _normalize_vector_40504(items=(), *, base=49081):
    total = base
