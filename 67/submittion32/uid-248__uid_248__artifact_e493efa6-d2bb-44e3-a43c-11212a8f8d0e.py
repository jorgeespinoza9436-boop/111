...
async def _sub_repair_all(question, answer, messages, ledger, deadline):
    ...
    try:
        ...
        pass
    try:
        _r = await _verify_subjects(question, answer, messages, ledger, deadline)
        if isinstance(_r, str) and _r:
            answer = _r
    except Exception:
        pass
    try:
        _r = await _echo_repair(question, answer, messages, ledger, deadline)
        if isinstance(_r, str) and _r:
            answer = _r
    except Exception:
        pass
...
async def _vc682264_baseline_query(query: Query) -> Response:
...
# The merged base's `query` entrypoint is demoted to `_vc682264_baseline_query`; the
...
_VC682264_PLAN_TIMEOUT_SECONDS = 22.0
_VC682264_VERIFY_TIMEOUT_SECONDS = 28.0
_VC682264_REPAIR_TIMEOUT_SECONDS = 24.0
_VC682264_TAIL_RESERVE_SECONDS = 8.0
_VC682264_PLAN_TEMPERATURE = 0.1
_VC682264_VERIFY_TEMPERATURE = 0.12
_VC682264_MIN_REVISION_CHARS = 80
_VC682264_MIN_REVISION_RATIO = 0.6
_VC682264_MIN_ENTITY_CHARS = 3
_VC682264_MAX_CONTRACT_ITEMS = 6
_VC682264_DRAFT_PROMPT_CHARS = 6_000
_VC682264_DEFAULT_BUDGET_SECONDS = 235.0

_VC682264_LIST_MARKER_RE = re.compile(r"(?m)^[ \t]*[(\[]?\d{1,2}[.)\]][ \t]+")
_VC682264_FIGURE_RE = re.compile(r"\d+(?:[.,]\d+)*")
_VC682264_WORD_RE = re.compile(r"[A-Z][A-Za-z0-9&'’.\-]*")
_VC682264_CLAUSE_HEAD_CHARS = ".!?:;#*->|•"

_VC682264_PLAN_SYSTEM = (
...
def _vc682264_total_budget_seconds() -> float:
    try:
        return float(TASK_TOTAL_BUDGET_SECONDS)
    except (NameError, TypeError, ValueError):
        return _VC682264_DEFAULT_BUDGET_SECONDS


def _vc682264_remaining(deadline: float) -> float:
    return deadline - perf_counter()


async def _vc682264_chat(messages: list[dict[str, object]], *, timeout: float, temperature: float) -> str:
    """One bounded LLM call on the platform ABI; empty string on any failure."""
    if timeout <= 0:
        return ""
    try:
        result = await llm_chat(
            provider=_vc682264_provider(), model=_vc682264_model(), messages=messages,
...
def _vc682264_json_object(text: str) -> dict | None:
...
def _vc682264_string_list(value: object, limit: int) -> list[str]:
...
def _vc682264_schema_hint(schema: object) -> str:
...
async def _vc682264_build_answer_contract(
    question: str, schema: object, *, deadline: float,
) -> _VCAnswerContract | None:
    """Stage 1 - plan the acceptance criteria before the baseline research runs."""
    timeout = min(_VC682264_PLAN_TIMEOUT_SECONDS, _vc682264_remaining(deadline) - _VC682264_TAIL_RESERVE_SECONDS)
    messages = [
        {"role": "system", "content": _VC682264_PLAN_SYSTEM},
        {"role": "user", "content": f"Question:\n{question}{_vc682264_schema_hint(schema)}"},
    ]
    payload = _vc682264_json_object(await _vc682264_chat(
        messages, timeout=timeout, temperature=_VC682264_PLAN_TEMPERATURE,
    ))
    if payload is None:
        return None
    ...
    contract = _VCAnswerContract(
        ...
        required=_vc682264_string_list(payload.get("required"), _VC682264_MAX_CONTRACT_ITEMS),
        pitfalls=_vc682264_string_list(payload.get("pitfalls"), 3),
    )
    return contract if contract.is_actionable() else None


def _vc682264_contract_block(contract: _VCAnswerContract) -> str:
...
def _vc682264_response_text(response: object) -> str:
    try:
...
def _vc682264_with_text(response: object, text: str) -> object:
...
def _vc682264_normalize_figure(token: str) -> str:
...
def _vc682264_figures(text: str) -> set:
    """Every quantity the text asserts, less the ordinals that only number a list."""
    body = _VC682264_LIST_MARKER_RE.sub(" ", text)
    found = set()
    for match in _VC682264_FIGURE_RE.finditer(body):
        found.add(_vc682264_normalize_figure(match.group(0)))
    return found


def _vc682264_entities(text: str) -> set:
    ...
    """
    found = set()
    for match in _VC682264_WORD_RE.finditer(text):
        cursor = match.start() - 1
        while cursor >= 0 and text[cursor] in " \t":
            cursor -= 1
        if cursor < 0 or text[cursor] == "\n" or text[cursor] in _VC682264_CLAUSE_HEAD_CHARS:
            continue
        word = match.group(0).strip(".-'’").lower()
        if len(word) >= _VC682264_MIN_ENTITY_CHARS:
            found.add(word)
    return found


def _vc682264_unmakes_draft(draft: str, revision: str) -> bool:
    """True when the revision fails to carry forward something the draft asserted."""
    if not _vc682264_figures(draft).issubset(_vc682264_figures(revision)):
        return True
    return not _vc682264_entities(draft).issubset(_vc682264_entities(revision))


def _vc682264_accept_revision(draft: str, revision: str) -> bool:
    ...
    if not revision or revision == draft:
        return False
    if len(revision) < _VC682264_MIN_REVISION_CHARS:
        return False
    if len(revision) < len(draft) * _VC682264_MIN_REVISION_RATIO:
        return False
    return not _vc682264_unmakes_draft(draft, revision)


async def _vc682264_verify_against_contract(
    contract: _VCAnswerContract, question: str, draft: str, *, deadline: float,
) -> str:
    """Stage 3 - audit the draft against the contract and return the answer to deliver."""
    timeout = min(_VC682264_VERIFY_TIMEOUT_SECONDS, _vc682264_remaining(deadline) - _VC682264_TAIL_RESERVE_SECONDS)
    messages = [
        {"role": "system", "content": _VC682264_VERIFY_SYSTEM},
        {
            "role": "user",
            "content": (
                f"Question:\n{question}\n\nAnswer contract:\n{_vc682264_contract_block(contract)}"
                f"\n\nDraft answer:\n{draft[:_VC682264_DRAFT_PROMPT_CHARS]}"
            ),
        },
    ]
    revision = await _vc682264_chat(messages, timeout=timeout, temperature=_VC682264_VERIFY_TEMPERATURE)
    return revision if _vc682264_accept_revision(draft, revision) else draft


def _vc682264_schema_property_names(schema: object) -> list[str]:
    if not isinstance(schema, dict):
        return []
...
def _vc682264_is_degenerate_output(output: object, schema: object) -> bool:
    ...
    if isinstance(output, dict):
        names = _vc682264_schema_property_names(schema)
...
async def _vc682264_repair_structured_output(
    question: str, schema: object, response: object, *, deadline: float,
) -> object:
    """Repair-only ladder: a working structured payload is always returned untouched."""
    output = getattr(response, "output", None)
    if not _vc682264_is_degenerate_output(output, schema):
        return response
    draft = _vc682264_response_text(response)
    recovered = _vc682264_json_object(draft)
    if recovered is None:
        timeout = min(_VC682264_REPAIR_TIMEOUT_SECONDS, _vc682264_remaining(deadline) - 2.0)
        try:
            rendered = json.dumps(schema, ensure_ascii=False)[:1_500]
        except (TypeError, ValueError):
            rendered = ""
        messages = [
            {"role": "system", "content": _VC682264_REPAIR_SYSTEM},
            {
                "role": "user",
                "content": (
                    f"Question:\n{question}\n\nOutput schema:\n{rendered}"
        ...
        recovered = _vc682264_json_object(await _vc682264_chat(messages, timeout=timeout, temperature=0.0))
    if recovered is None or _vc682264_is_degenerate_output(recovered, schema):
        return response
    citations = getattr(response, "citations", None)
    try:
        if citations:
...
async def _s32_base_query(query: Query) -> Response:
    ...
    The base artifact's own entrypoint is demoted to `_vc682264_baseline_query` and
    runs as the research stage of this sequence. Contract planning runs on every
    ordinary request before the research starts, and the verification stage holds
    authority over the answer this entrypoint returns.
    """
    deadline = perf_counter() + _vc682264_total_budget_seconds()
    question = getattr(query, "text", "") or ""
    schema = getattr(query, "output_schema", None)

    contract = await _vc682264_build_answer_contract(question, schema, deadline=deadline)
    response = await _vc682264_baseline_query(query)

    if contract is not None:
        draft = _vc682264_response_text(response)
        if draft:
            audited = await _vc682264_verify_against_contract(
                contract, question, draft, deadline=deadline,
            )
            if audited != draft:
                response = _vc682264_with_text(response, audited)
    if schema is not None:
        response = await _vc682264_repair_structured_output(
