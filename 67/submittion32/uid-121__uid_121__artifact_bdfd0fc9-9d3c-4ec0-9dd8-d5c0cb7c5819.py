...
#
# It is the ONLY stage in this build that acts before the answer exists, so it
# is the only one guaranteed to run: the SIX post-audit sweeps below all share
# one tail and in practice only the first one or two that fire will execute.
...
# pool. Runs FIRST: it can add members, and every numeric stage below inspects
# the figures those members carry.
...
#
# RUNS BEFORE both the authority sweep and the second-source check ON PURPOSE:
# fix WHAT the number is before fixing WHICH SOURCE carries it or whether a
# second source agrees. Re-anchoring a hallucinated value is wasted work.
#
# It also runs before the second-source check for the backer-count reason: The two stages partition the
...
FIGURE_GROUND_MIN_LEFT_S = 100.0
...
# BOTH donor builds that carry this stage shipped the opposite order -- uid 111
# put it at 70s ahead of the authority sweep at 75s and uid 79 put it at 70s
# ahead of value repair at 80s -- so in each case every annotation it produced
# was silently thrown away whenever that later sweep fired.
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # SIX sweeps share one tail — the largest chain in this family. A firing
    # sweep costs a search plus up to three loop turns, so in practice only the
    # first one or two that trigger will execute and the rest close on their own
    # floors. Treat the order as a PRIORITY RANKING, not a pipeline:
    #   subject  — wrong entity makes every later check moot
    #   pool     — missing members change what the checks below describe
    #   grounded — figures with ZERO cited backers: fix WHAT the number is before
    #              fixing which source carries it
    #   source   — re-anchor to the official source; MUST precede corroboration
    #              (it skips whenever any cited row is already authoritative, so
    #              a .gov pulled in by corroboration would mask the problem)
    ...
    for _sweep in (_verify_subjects, _widen_pool, _ground_figures,
...
class TrailingBoundaryIndex_757e37:
    ...
    REGION_THRESHOLD_757E37 = 12000          # hard ceiling for any single accounted window
    EXTENT_CEILING_757E37 = 96             # symmetric padding applied when widening
    ROW_HEADROOM_757E37 = 3              # smallest sample worth reporting on

    def gather_slices_757e37(self, spans, limit=0):
        ...
        """
        ceiling = int(limit or self.REGION_THRESHOLD_757E37)
        cleaned = []
    ...
    def widen_coverage_757e37(self, spans, note_len, pad=0):
        """Widen merged windows by a fixed margin without crossing the note."""
        room = max(0, int(pad or self.EXTENT_CEILING_757E37))
        total = max(0, int(note_len or 0))
        widened = []
        for window in self.gather_slices_757e37(spans, total or self.REGION_THRESHOLD_757E37):
    ...
    def tabulate_slices_757e37(self, rows, note_len=0):
        ...
        for row in rows or []:
            ...
            if not isinstance(length, (int, float)) or isinstance(length, bool):
                length = note_len
            for window in self.widen_coverage_757e37(spans, int(length or 0)):
    ...
    def summarize_slices_757e37(self, rows, note_len=0):
        ...
        for row in rows or []:
            ...
            if not isinstance(row, dict):
                continue
            stats = self.tabulate_slices_757e37([row], note_len)
            if stats["windows"] < 1:
                continue
            label = row.get("title", "")
        ...
        if len(report) < self.ROW_HEADROOM_757E37:
