...
class BindRing_f3788f:

    @staticmethod
    def _reed_ok_f3788f() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_REED_RUN_f3788f = ReedMat_f3788f()._compile()
_RUSH_RUN_f3788f = RushMat_f3788f()._compile()
_BIND_RING_f3788f = BindRing_f3788f()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _BIND_RING_f3788f._reed_ok_f3788f():
        return await _REED_RUN_f3788f(query)
    return await _RUSH_RUN_f3788f(query)
