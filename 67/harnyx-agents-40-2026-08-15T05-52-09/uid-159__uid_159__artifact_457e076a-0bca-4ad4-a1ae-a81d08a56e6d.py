...
class _Nhsd8:
    ...
    def _compile(self):
        ...
        LLM_LANE_A = AmberLanePod.LLM_LANE_A

        LLM_LANE_B = AmberLanePod.LLM_LANE_B

        LOOP_MODEL_A = AmberModelPod.LOOP_MODEL_A

        LOOP_MODEL_B = AmberModelPod.LOOP_MODEL_B

        AUDIT_MODEL = AmberModelPod.AUDIT_MODEL

        SCHEMA_MODEL = AmberSchemaPod.SCHEMA_MODEL

        RESORT_MODEL = AmberSchemaPod.RESORT_MODEL

        SEARCH_PROVIDER = AmberSchemaPod.SEARCH_PROVIDER

        _REASONING_MANDATORY = AmberUpstreamPod._REASONING_MANDATORY

        _FAST_UPSTREAMS = AmberUpstreamPod._FAST_UPSTREAMS

        _FAST_UPSTREAMS_OSS = AmberUpstreamPod._FAST_UPSTREAMS_OSS

        WALL_BUDGET_S = AmberWallPod.WALL_BUDGET_S

        WRAPUP_AT_S = AmberWallPod.WRAPUP_AT_S

        MIN_TAIL_S = AmberWallPod.MIN_TAIL_S

        DIGEST_TAIL_S = AmberTailPod.DIGEST_TAIL_S

        RESCUE_TIMEOUT_S = AmberTailPod.RESCUE_TIMEOUT_S

        BRIEF_TIMEOUT_S = AmberCallPod.BRIEF_TIMEOUT_S

        TURN_TIMEOUT_S = AmberCallPod.TURN_TIMEOUT_S

        AUDIT_TIMEOUT_S = AmberCallPod.AUDIT_TIMEOUT_S

        SEARCH_TIMEOUT_S = AmberNetPod.SEARCH_TIMEOUT_S

        FETCH_TIMEOUT_S = AmberNetPod.FETCH_TIMEOUT_S

        LANE_B_MAX_PAYLOAD_CHARS = AmberNetPod.LANE_B_MAX_PAYLOAD_CHARS

        MAX_TURNS = AmberQuotaPod.MAX_TURNS

        AUDIT_EXTRA_TURNS = AmberQuotaPod.AUDIT_EXTRA_TURNS

        ANSWER_REPAIR_TURNS = AmberQuotaPod.ANSWER_REPAIR_TURNS

        BRIEF_MIN_USD = AmberSpendPod.BRIEF_MIN_USD

        AUDIT_MIN_USD = AmberSpendPod.AUDIT_MIN_USD

        WRAPUP_MIN_USD = AmberSpendPod.WRAPUP_MIN_USD

        _SPEND = AmberSpendPod._SPEND

        RETAIN_MARGIN_CHARS = AmberRetainPod.RETAIN_MARGIN_CHARS

        RETAIN_MAX_PER_ROW = AmberRetainPod.RETAIN_MAX_PER_ROW

        RETAIN_MIN_QUOTE = AmberRetainPod.RETAIN_MIN_QUOTE

        PAGE_READ_MAX_CHARS = AmberPagePod.PAGE_READ_MAX_CHARS

        PAGE_GREP_WINDOW = AmberPagePod.PAGE_GREP_WINDOW

        PAGE_GREP_MAX_HITS = AmberPagePod.PAGE_GREP_MAX_HITS

        SEARCH_EXCERPT_CHARS = AmberSearchPod.SEARCH_EXCERPT_CHARS

        _LEDGER_TEXT_CAP = AmberSearchPod._LEDGER_TEXT_CAP

        FETCH_HEAD_CHARS = AmberFetchPod.FETCH_HEAD_CHARS

        FETCH_WINDOW_CHARS = AmberFetchPod.FETCH_WINDOW_CHARS

        FETCH_WINDOWS_PER_PAGE = AmberFetchPod.FETCH_WINDOWS_PER_PAGE

        FETCH_PLAIN_CHARS = AmberFetchPod.FETCH_PLAIN_CHARS

        CITATION_MIN_SPAN_CHARS = AmberCitePod.CITATION_MIN_SPAN_CHARS

        CITATION_MAX_REF_CHARS = AmberCitePod.CITATION_MAX_REF_CHARS

        CITATION_CAP = AmberCitePod.CITATION_CAP

        ANSWER_CHAR_CAP = AmberCapPod.ANSWER_CHAR_CAP

        EVIDENCE_CHAR_BUDGET = AmberCapPod.EVIDENCE_CHAR_BUDGET

        LOOP_RULES = AmberPromptPod.LOOP_RULES

        SET_RULE = AmberPromptPod.SET_RULE

        SUPERLATIVE_RULE = AmberPromptPod.SUPERLATIVE_RULE

        _COMMIT_RULES = AmberCommitPod._COMMIT_RULES

        _REPAIR_ORDER = AmberCommitPod._REPAIR_ORDER

        _SLOT = AmberCommitPod._SLOT

        LOOP_TOOLS = AmberToolPod.LOOP_TOOLS

        _SEC_SEARCH_HINT = AmberToolPod._SEC_SEARCH_HINT

        _SET_HINT_RE = AmberPluralPod._SET_HINT_RE

        _SET_CONNECTIVE_RE = AmberPluralPod._SET_CONNECTIVE_RE

        _PLURAL_HEAD_RE = AmberPluralPod._PLURAL_HEAD_RE

        _PLURAL_FALSE = AmberPluralPod._PLURAL_FALSE

        _ONE_WINNER_RE = AmberEstPod._ONE_WINNER_RE

        _EST_STOP = AmberEstPod._EST_STOP

        _EST_RE = AmberEstPod._EST_RE

        _WORD_RE = AmberLexPod._WORD_RE

        _STOP = AmberLexPod._STOP

        _SITE_OP_RE = AmberLexPod._SITE_OP_RE

        _SEED_TOKEN_RE = AmberSeedPod._SEED_TOKEN_RE

        _SEED_STOP = AmberSeedPod._SEED_STOP

        MAX_SEED_QUERIES = AmberSeedPod.MAX_SEED_QUERIES

        _BRACKET_FIX = AmberSeedPod._BRACKET_FIX

        _CITE_NUM_RE = AmberSeedPod._CITE_NUM_RE

        _OUTPUT_ONLY_RE = AmberRefusePod._OUTPUT_ONLY_RE

        _OUTPUT_ONLY_MIN_CHARS = AmberRefusePod._OUTPUT_ONLY_MIN_CHARS

        _GLOSS_RE = AmberRefusePod._GLOSS_RE

        _VERIFY_MARK_RE = AmberRefusePod._VERIFY_MARK_RE

        _TOOL_MARKUP_RE = AmberStubPod._TOOL_MARKUP_RE

        _STUB_ANSWER_RE = AmberStubPod._STUB_ANSWER_RE

        _REFUSAL_ONLY_RE = AmberStubPod._REFUSAL_ONLY_RE

        _INTENT_NARRATION_RE = AmberStubPod._INTENT_NARRATION_RE

        MIN_ANSWER_CHARS = AmberFloorPod.MIN_ANSWER_CHARS

        MIN_CITED_ANSWER_CHARS = AmberFloorPod.MIN_CITED_ANSWER_CHARS

        _CITE_MARK_RE = AmberFloorPod._CITE_MARK_RE

        _FURNITURE_RE = AmberFurnishPod._FURNITURE_RE

        _SRC_FOOTNOTE_RE = AmberFurnishPod._SRC_FOOTNOTE_RE

        _MD_LINK_RE = AmberFurnishPod._MD_LINK_RE

        _BARE_URL_RE = AmberFurnishPod._BARE_URL_RE

        _SENTENCEY_RE = AmberFurnishPod._SENTENCEY_RE

        _NUM_IN_TEXT_RE = AmberDigestPod._NUM_IN_TEXT_RE

        _DIGEST_LEAD_RE = AmberDigestPod._DIGEST_LEAD_RE

        _DIGEST_NOISE_RE = AmberDigestPod._DIGEST_NOISE_RE

        _VALUE_MAX_CHARS = AmberDigestPod._VALUE_MAX_CHARS

        _NARRATION_LEAD_RE = AmberNarratePod._NARRATION_LEAD_RE

        _ABBREV_TAIL_RE = AmberNarratePod._ABBREV_TAIL_RE

        QUOTE_SYNTH_TIMEOUT_S = AmberQuotePod.QUOTE_SYNTH_TIMEOUT_S

        QUOTE_SYNTH_MIN_BUDGET_S = AmberQuotePod.QUOTE_SYNTH_MIN_BUDGET_S

        QUOTE_SYNTH_MIN_QUOTES = AmberQuotePod.QUOTE_SYNTH_MIN_QUOTES

        QUOTE_TABLE_CHARS = AmberQuotePod.QUOTE_TABLE_CHARS

        _SEC_TICKERS_URL = AmberEdgarUrlPod._SEC_TICKERS_URL

        _SEC_SUBMISSIONS_URL = AmberEdgarUrlPod._SEC_SUBMISSIONS_URL

        _SEC_DOC_URL = AmberEdgarUrlPod._SEC_DOC_URL

        _SEC_FETCH_TIMEOUT_S = AmberEdgarRunPod._SEC_FETCH_TIMEOUT_S

        _SEC_MIN_HEADROOM_S = AmberEdgarRunPod._SEC_MIN_HEADROOM_S

        _SEC_CACHE = AmberEdgarRunPod._SEC_CACHE

        _SEC_STOPWORDS = AmberEdgarRunPod._SEC_STOPWORDS

        _SEC_ALNUM_RE = AmberEdgarRunPod._SEC_ALNUM_RE
        ...
        class FuelMeter:
            ...
            @staticmethod
            def vx_wrapup_order(seconds_left: float) -> str:
                return (
            ...
            @staticmethod
            def vx_spend_note(payload) -> None:
                budget = getattr(payload, "budget", None)
                left = getattr(budget, "session_remaining_budget_usd", None)
                if isinstance(left, (int, float)):
                    _SPEND["left"] = float(left)
        ...
        class AskSignals:
            ...
            @staticmethod
            def vx_seed_queries(question: str, set_question: bool) -> list[str]:
                q = " ".join((question or "").split())
                if not q:
                    return []
                seeds = [q[:300]]
            ...
            @staticmethod
            def vx_degrade_query(q: str) -> str:

                out = _SITE_OP_RE.sub("", q or "").replace('"', " ")
                return " ".join(out.split())

            @staticmethod
            def vx_key_terms(text: str) -> set[str]:
                return {w for w in _WORD_RE.findall((text or "").casefold()) if w not in _STOP}

            @staticmethod
            def vx_needs_set_completeness(question: str) -> bool:
                q = " ".join((question or "").split())
                if _SET_HINT_RE.search(q):
                    return True
            ...
            @staticmethod
            def vx_needs_superlative_proof(question: str) -> bool:
            ...
            @staticmethod
            def vx_has_superlative(text: str) -> bool:
                if _ONE_WINNER_RE.search(text or ""):
                    return True
                for m in _EST_RE.finditer(text or ""):
                    if m.group(0).lower() not in _EST_STOP:
        ...
        class ClipEngine:
            ...
            @staticmethod
            def vx_do_retain_evidence(source: str, quote: str, ledger: EvidenceLedger) -> str:
            ...
            @staticmethod
            def vx_do_page_read(url: str, offset: int, length: int, ledger: EvidenceLedger) -> str:
            ...
            @staticmethod
            def vx_do_page_grep(url: str, pattern: str, ledger: EvidenceLedger) -> str:
            ...
            @staticmethod
            def vx_ledger_page(url: str, ledger: EvidenceLedger) -> tuple[int, dict] | None:
            ...
            @staticmethod
            def vx_commit_tool_output(out, ledger: EvidenceLedger) -> str:
            ...
            @staticmethod
            def vx_best_windows(note: str, terms: set[str], width: int,
        ...
        class ProbeArms:
            ...
            @staticmethod
            async def vx_run_tool(call, question: str, ledger: EvidenceLedger, deadline: float) -> str:
                try:
                    args = json.loads(getattr(call, "arguments", None) or "{}")
                except Exception:
                    args = {}
            ...
            @staticmethod
            async def vx_do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger) -> str:
                if not url.strip():
                    return "# read_page: empty url"
                payload = None
                for _attempt in (0, 1):
            ...
            @staticmethod
            async def vx_do_search(query_text: str, ledger: EvidenceLedger):
                if not query_text.strip():
                    return "# web_search: empty query"
        ...
        class SecPipeline:
            ...
            @staticmethod
            async def vx_do_sec_filing(company: str, form: str, year: str, deadline: float) -> str:
                company = (company or "").strip()
                form = (form or "").strip() or "10-K"
                year = (year or "").strip()[:4]
                hint = _SEC_SEARCH_HINT.format(company=company, year=year, form=form)
            ...
            @staticmethod
            def vx_sec_pick_filing(recent: dict, form: str, year: str):
            ...
            @staticmethod
            async def vx_fetch_json(url: str, deadline: float):
                cached = _SEC_CACHE.get(url)
                if cached is not None:
                    return cached
                for _attempt in (0, 1):
            ...
            @staticmethod
            def vx_sec_norm_form(form: str) -> str:
            ...
            @staticmethod
            def vx_sec_tokens(text: str) -> list[str]:
        ...
        class ModelRails:
            ...
            @staticmethod
            async def vx_chat_turn(messages: list[dict], deadline: float, *, finish_only: bool,
            ...
            @staticmethod
            async def vx_chat_simple(lane: str, model: str, system: str, user: str, *,
            ...
            @staticmethod
            def vx_upstream(lane: str, model: str) -> dict | None:
            ...
            @staticmethod
            def vx_least_think(lane: str, model: str = "") -> dict:
        ...
        class ResearchLoop:
            ...
            @staticmethod
            async def vx_audit_patch(question: str, answer: str, messages: list[dict],
            ...
            @staticmethod
            async def vx_loop(question: str, brief: str, ledger: EvidenceLedger,
            ...
            @staticmethod
            async def vx_preseed(question: str, set_question: bool, ledger: EvidenceLedger,
                               deadline: float) -> str:
            ...
            @staticmethod
            async def vx_knowledge_brief(question: str) -> tuple[str, str]:
        ...
        class CiteExact:
            ...
            @staticmethod
            def vx_retained_count(ledger: EvidenceLedger) -> int:
                return sum(len(r.get("retained") or []) for r in ledger.rows)

            @staticmethod
            def vx_quote_table(ledger: EvidenceLedger) -> str:
            ...
            @staticmethod
            def vx_citations_for(answer: str, ledger: EvidenceLedger) -> list[CitationRef]:
            ...
            @staticmethod
            def vx_verbatim_structured(obj, ledger: EvidenceLedger, depth: int = 0):
            ...
            @staticmethod
            def vx_verbatim_from_source(value: str, ledger: EvidenceLedger) -> str:
            ...
            @staticmethod
            def vx_answer_line_only(answer: str, question: str) -> str:
            ...
            @staticmethod
            def vx_cited_numbers(answer: str, top: int) -> list[int]:
                answer = _normalize_brackets(answer)
                seen: set[int] = set()
                out: list[int] = []
                for m in _CITE_NUM_RE.finditer(answer):
            ...
            @staticmethod
            def vx_normalize_brackets(text: str) -> str:
                return (text or "").translate(_BRACKET_FIX)
        ...
        class DraftScreen:
            ...
            @staticmethod
            def vx_strip_lead_narration(text: str) -> str:
            ...
            @staticmethod
            def vx_sanitize_draft(text: str) -> str:

                return _VERIFY_MARK_RE.sub("", text or "").strip()

            @staticmethod
            def vx_is_usable_answer(text: str) -> bool:
            ...
            @staticmethod
            def vx_is_degenerate_repetition(text: str) -> bool:
            ...
            @staticmethod
            def vx_looks_like_tool_json(s: str) -> bool:
        ...
        class OutputForge:
            ...
            @staticmethod
            def vx_coerce_to_schema(answer: str, schema, depth: int = 0):
            ...
            @staticmethod
            def vx_undigest_for_schema(basis: str) -> str:
            ...
            @staticmethod
            def vx_matches_schema_shape(value, schema) -> bool:
                kind = _schema_kind(schema)
                if not kind:
                    return True
                if kind == "array":
            ...
            @staticmethod
            async def vx_schema_output(question: str, answer: str, schema, deadline: float) -> object | None:
            ...
            @staticmethod
            async def vx_knowledge_resort(question: str, deadline: float) -> str:
                left = deadline - monotonic()
                if left < 12.0:
                    return ""
                try:
            ...
            @staticmethod
            async def vx_write_from_digest(question: str, ledger: EvidenceLedger, deadline: float) -> str:
            ...
            @staticmethod
            def vx_deterministic_answer(question: str, ledger: EvidenceLedger) -> str:
            ...
            @staticmethod
            def vx_informative_lead(preview: str, limit: int = 280) -> str:
            ...
            @staticmethod
            def vx_ledger_digest(ledger: EvidenceLedger, char_cap: int = 60000) -> str:
        ...
        # Helper classes (10) — aliases preserve call sites


        _seed_queries = AskSignals.vx_seed_queries
        _best_windows = ClipEngine.vx_best_windows
        _commit_tool_output = ClipEngine.vx_commit_tool_output
        _audit_patch = ResearchLoop.vx_audit_patch
        _normalize_brackets = CiteExact.vx_normalize_brackets
        _cited_numbers = CiteExact.vx_cited_numbers
        _answer_line_only = CiteExact.vx_answer_line_only
        _verbatim_from_source = CiteExact.vx_verbatim_from_source
        _verbatim_structured = CiteExact.vx_verbatim_structured
        _chat_simple = ModelRails.vx_chat_simple
        _chat_turn = ModelRails.vx_chat_turn
        _knowledge_brief = ResearchLoop.vx_knowledge_brief
        _preseed = ResearchLoop.vx_preseed
        _strip_lead_narration = DraftScreen.vx_strip_lead_narration
        _cap = DraftScreen.vx_cap
        _ledger_digest = OutputForge.vx_ledger_digest
        _informative_lead = OutputForge.vx_informative_lead
        _deterministic_answer = OutputForge.vx_deterministic_answer
        _write_from_digest = OutputForge.vx_write_from_digest
        _knowledge_resort = OutputForge.vx_knowledge_resort
        _schema_output = OutputForge.vx_schema_output
        _schema_kind = OutputForge.vx_schema_kind
        _matches_schema_shape = OutputForge.vx_matches_schema_shape
        _undigest_for_schema = OutputForge.vx_undigest_for_schema
        _coerce_to_schema = OutputForge.vx_coerce_to_schema
        _ledger_page = ClipEngine.vx_ledger_page
        _do_page_grep = ClipEngine.vx_do_page_grep
        _do_page_read = ClipEngine.vx_do_page_read
        _citations_for = CiteExact.vx_citations_for
        _quote_table = CiteExact.vx_quote_table
        _retained_count = CiteExact.vx_retained_count
        _looks_like_tool_json = DraftScreen.vx_looks_like_tool_json
        _is_degenerate_repetition = DraftScreen.vx_is_degenerate_repetition
        _is_usable_answer = DraftScreen.vx_is_usable_answer
        _sanitize_draft = DraftScreen.vx_sanitize_draft
        _spend_note = FuelMeter.vx_spend_note
        _spend_left = FuelMeter.vx_spend_left
        _wrapup_order = FuelMeter.vx_wrapup_order
        _has_superlative = AskSignals.vx_has_superlative
        _needs_superlative_proof = AskSignals.vx_needs_superlative_proof
        _needs_set_completeness = AskSignals.vx_needs_set_completeness
        _key_terms = AskSignals.vx_key_terms
        _degrade_query = AskSignals.vx_degrade_query
        _do_search = ProbeArms.vx_do_search
        _do_fetch = ProbeArms.vx_do_fetch
        _run_tool = ProbeArms.vx_run_tool
        _sec_tokens = SecPipeline.vx_sec_tokens
        _sec_norm_form = SecPipeline.vx_sec_norm_form
        _fetch_json = SecPipeline.vx_fetch_json
        _sec_pick_filing = SecPipeline.vx_sec_pick_filing
        _do_sec_filing = SecPipeline.vx_do_sec_filing
        _least_think = ModelRails.vx_least_think
        _upstream = ModelRails.vx_upstream
        _do_retain_evidence = ClipEngine.vx_do_retain_evidence
        _loop = ResearchLoop.vx_loop
        async def query(query: Query) -> Response:
            question = (query.text or "").strip()
            if not question:
                return Response(text="No question provided.")
...
_EASY_RUN = _Qsdfnl8()._compile()
_HARD_RUN = _Nhsd8()._compile()
