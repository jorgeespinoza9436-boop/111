    ...
    def _compile(self):
        ...
        async def _solve(query: Query, question: str) -> Response:
            ...
            try:
                ...
                pass
            for _sweep in (_verify_subjects, _widen_pool, _anchor_primary_source, _second_source_check):
                try:
                    if not _is_usable_answer(answer):
                        break
...
class StubKey_1791cb:

    @staticmethod
    def _opal_frame_1791cb() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_OPAL_RUN_1791cb = OpalFrame_1791cb()._compile()
_ONYX_RUN_1791cb = OnyxFrame_1791cb()._compile()
_STUB_KEY_1791cb = StubKey_1791cb()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _STUB_KEY_1791cb._opal_frame_1791cb():
        return await _OPAL_RUN_1791cb(query)
    return await _ONYX_RUN_1791cb(query)
