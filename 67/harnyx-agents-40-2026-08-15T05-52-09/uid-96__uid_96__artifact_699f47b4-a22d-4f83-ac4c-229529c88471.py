...
class SelectorRod_b69730:

    @staticmethod
    def _amber_ready_b69730() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_AMBER_RUN_b69730 = AmberKiln_b69730()._compile()
_COBALT_RUN_b69730 = CobaltKiln_b69730()._compile()
_SELECTOR_ROD_b69730 = SelectorRod_b69730()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _SELECTOR_ROD_b69730._amber_ready_b69730():
        return await _AMBER_RUN_b69730(query)
    return await _COBALT_RUN_b69730(query)
