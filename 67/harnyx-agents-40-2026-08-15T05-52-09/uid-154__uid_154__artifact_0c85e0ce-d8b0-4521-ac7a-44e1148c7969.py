...
class _Pnks92342:
    ...
    def _compile(self):
        ...
        LLM_LANE_A = OrbitalLaneBag.LLM_LANE_A

        LLM_LANE_B = OrbitalLaneBag.LLM_LANE_B

        LOOP_MODEL_A = OrbitalLaneBag.LOOP_MODEL_A

        LOOP_MODEL_B = OrbitalLaneBag.LOOP_MODEL_B

        SEARCH_PROVIDER = OrbitalLaneBag.SEARCH_PROVIDER

        AUDIT_MODEL = NovaModelBag.AUDIT_MODEL

        SCHEMA_MODEL = NovaModelBag.SCHEMA_MODEL

        RESORT_MODEL = NovaModelBag.RESORT_MODEL

        _REASONING_MANDATORY = NovaModelBag._REASONING_MANDATORY

        _FAST_UPSTREAMS = NovaModelBag._FAST_UPSTREAMS

        _FAST_UPSTREAMS_OSS = NovaModelBag._FAST_UPSTREAMS_OSS

        WALL_BUDGET_S = ChronoWallBag.WALL_BUDGET_S

        WRAPUP_AT_S = ChronoWallBag.WRAPUP_AT_S

        MIN_TAIL_S = ChronoWallBag.MIN_TAIL_S

        DIGEST_TAIL_S = ChronoWallBag.DIGEST_TAIL_S

        RESCUE_TIMEOUT_S = ChronoWallBag.RESCUE_TIMEOUT_S

        BRIEF_TIMEOUT_S = ChronoCallBag.BRIEF_TIMEOUT_S

        TURN_TIMEOUT_S = ChronoCallBag.TURN_TIMEOUT_S

        AUDIT_TIMEOUT_S = ChronoCallBag.AUDIT_TIMEOUT_S

        SEARCH_TIMEOUT_S = ChronoCallBag.SEARCH_TIMEOUT_S

        FETCH_TIMEOUT_S = ChronoCallBag.FETCH_TIMEOUT_S

        LANE_B_MAX_PAYLOAD_CHARS = ChronoCallBag.LANE_B_MAX_PAYLOAD_CHARS

        MAX_TURNS = TurnQuotaBag.MAX_TURNS

        AUDIT_EXTRA_TURNS = TurnQuotaBag.AUDIT_EXTRA_TURNS

        ANSWER_REPAIR_TURNS = TurnQuotaBag.ANSWER_REPAIR_TURNS

        BRIEF_MIN_USD = TurnQuotaBag.BRIEF_MIN_USD

        AUDIT_MIN_USD = TurnQuotaBag.AUDIT_MIN_USD

        WRAPUP_MIN_USD = TurnQuotaBag.WRAPUP_MIN_USD

        RETAIN_MARGIN_CHARS = RetainSpanBag.RETAIN_MARGIN_CHARS

        RETAIN_MAX_PER_ROW = RetainSpanBag.RETAIN_MAX_PER_ROW

        RETAIN_MIN_QUOTE = RetainSpanBag.RETAIN_MIN_QUOTE

        PAGE_READ_MAX_CHARS = RetainSpanBag.PAGE_READ_MAX_CHARS

        PAGE_GREP_WINDOW = RetainSpanBag.PAGE_GREP_WINDOW

        PAGE_GREP_MAX_HITS = RetainSpanBag.PAGE_GREP_MAX_HITS

        FETCH_HEAD_CHARS = FetchSliceBag.FETCH_HEAD_CHARS

        FETCH_WINDOW_CHARS = FetchSliceBag.FETCH_WINDOW_CHARS

        FETCH_WINDOWS_PER_PAGE = FetchSliceBag.FETCH_WINDOWS_PER_PAGE

        FETCH_PLAIN_CHARS = FetchSliceBag.FETCH_PLAIN_CHARS

        SEARCH_EXCERPT_CHARS = FetchSliceBag.SEARCH_EXCERPT_CHARS

        _LEDGER_TEXT_CAP = FetchSliceBag._LEDGER_TEXT_CAP

        CITATION_MIN_SPAN_CHARS = CiteBudgetBag.CITATION_MIN_SPAN_CHARS

        CITATION_MAX_REF_CHARS = CiteBudgetBag.CITATION_MAX_REF_CHARS

        CITATION_CAP = CiteBudgetBag.CITATION_CAP

        ANSWER_CHAR_CAP = CiteBudgetBag.ANSWER_CHAR_CAP

        EVIDENCE_CHAR_BUDGET = CiteBudgetBag.EVIDENCE_CHAR_BUDGET

        LOOP_RULES = PromptCanonBag.LOOP_RULES

        SET_RULE = PromptCanonBag.SET_RULE

        SUPERLATIVE_RULE = PromptCanonBag.SUPERLATIVE_RULE

        _COMMIT_RULES = PromptCanonBag._COMMIT_RULES

        _REPAIR_ORDER = PromptCanonBag._REPAIR_ORDER

        _SLOT = PromptCanonBag._SLOT

        LOOP_TOOLS = ToolSpecBag.LOOP_TOOLS

        _SEC_SEARCH_HINT = ToolSpecBag._SEC_SEARCH_HINT

        _SET_HINT_RE = PluralCueBag._SET_HINT_RE

        _SET_CONNECTIVE_RE = PluralCueBag._SET_CONNECTIVE_RE

        _PLURAL_HEAD_RE = PluralCueBag._PLURAL_HEAD_RE

        _PLURAL_FALSE = PluralCueBag._PLURAL_FALSE

        _ONE_WINNER_RE = PluralCueBag._ONE_WINNER_RE

        _EST_STOP = PluralCueBag._EST_STOP

        _EST_RE = PluralCueBag._EST_RE

        _WORD_RE = LexStopBag._WORD_RE

        _STOP = LexStopBag._STOP

        _SITE_OP_RE = LexStopBag._SITE_OP_RE

        _SEED_TOKEN_RE = LexStopBag._SEED_TOKEN_RE

        _SEED_STOP = LexStopBag._SEED_STOP

        MAX_SEED_QUERIES = LexStopBag.MAX_SEED_QUERIES

        _BRACKET_FIX = LexStopBag._BRACKET_FIX

        _CITE_NUM_RE = LexStopBag._CITE_NUM_RE

        _OUTPUT_ONLY_RE = OutputGateBag._OUTPUT_ONLY_RE

        _OUTPUT_ONLY_MIN_CHARS = OutputGateBag._OUTPUT_ONLY_MIN_CHARS

        _GLOSS_RE = OutputGateBag._GLOSS_RE

        _VERIFY_MARK_RE = OutputGateBag._VERIFY_MARK_RE

        _TOOL_MARKUP_RE = OutputGateBag._TOOL_MARKUP_RE

        _STUB_ANSWER_RE = OutputGateBag._STUB_ANSWER_RE

        _REFUSAL_ONLY_RE = OutputGateBag._REFUSAL_ONLY_RE

        _INTENT_NARRATION_RE = OutputGateBag._INTENT_NARRATION_RE

        MIN_ANSWER_CHARS = AnswerFloorBag.MIN_ANSWER_CHARS

        MIN_CITED_ANSWER_CHARS = AnswerFloorBag.MIN_CITED_ANSWER_CHARS

        _CITE_MARK_RE = AnswerFloorBag._CITE_MARK_RE

        _FURNITURE_RE = AnswerFloorBag._FURNITURE_RE

        _SRC_FOOTNOTE_RE = AnswerFloorBag._SRC_FOOTNOTE_RE

        _MD_LINK_RE = AnswerFloorBag._MD_LINK_RE

        _BARE_URL_RE = AnswerFloorBag._BARE_URL_RE

        _SENTENCEY_RE = AnswerFloorBag._SENTENCEY_RE

        _NUM_IN_TEXT_RE = DigestCueBag._NUM_IN_TEXT_RE

        _DIGEST_LEAD_RE = DigestCueBag._DIGEST_LEAD_RE

        _DIGEST_NOISE_RE = DigestCueBag._DIGEST_NOISE_RE

        _VALUE_MAX_CHARS = DigestCueBag._VALUE_MAX_CHARS

        _NARRATION_LEAD_RE = DigestCueBag._NARRATION_LEAD_RE

        _ABBREV_TAIL_RE = DigestCueBag._ABBREV_TAIL_RE

        QUOTE_SYNTH_TIMEOUT_S = QuoteSynthBag.QUOTE_SYNTH_TIMEOUT_S

        QUOTE_SYNTH_MIN_BUDGET_S = QuoteSynthBag.QUOTE_SYNTH_MIN_BUDGET_S

        QUOTE_SYNTH_MIN_QUOTES = QuoteSynthBag.QUOTE_SYNTH_MIN_QUOTES

        QUOTE_TABLE_CHARS = QuoteSynthBag.QUOTE_TABLE_CHARS

        _SEC_TICKERS_URL = EdgarWireBag._SEC_TICKERS_URL

        _SEC_SUBMISSIONS_URL = EdgarWireBag._SEC_SUBMISSIONS_URL

        _SEC_DOC_URL = EdgarWireBag._SEC_DOC_URL

        _SEC_FETCH_TIMEOUT_S = EdgarWireBag._SEC_FETCH_TIMEOUT_S

        _SEC_MIN_HEADROOM_S = EdgarWireBag._SEC_MIN_HEADROOM_S

        _SEC_CACHE = EdgarWireBag._SEC_CACHE

        _SEC_STOPWORDS = EdgarWireBag._SEC_STOPWORDS

        _SEC_ALNUM_RE = EdgarWireBag._SEC_ALNUM_RE

        _SPEND = SpendStateBag._SPEND
        ...
        class SignalProbe:
            ...
            @staticmethod
            def fn_needs_set_completeness(question: str) -> bool:
                q = " ".join((question or "").split())
                if _SET_HINT_RE.search(q):
                    return True
            ...
            @staticmethod
            def fn_needs_superlative_proof(question: str) -> bool:
            ...
            @staticmethod
            def fn_has_superlative(text: str) -> bool:
                if _ONE_WINNER_RE.search(text or ""):
                    return True
                for m in _EST_RE.finditer(text or ""):
                    if m.group(0).lower() not in _EST_STOP:
        ...
        class TermHarvest:
            ...
            @staticmethod
            def fn_seed_queries(question: str, set_question: bool) -> list[str]:
                q = " ".join((question or "").split())
                if not q:
                    return []
                seeds = [q[:300]]
            ...
            @staticmethod
            def fn_degrade_query(q: str) -> str:

                out = _SITE_OP_RE.sub("", q or "").replace('"', " ")
                return " ".join(out.split())

            @staticmethod
            def fn_key_terms(text: str) -> set[str]:
                return {w for w in _WORD_RE.findall((text or "").casefold()) if w not in _STOP}
        ...
        class WindowCutter:
            ...
            @staticmethod
            def fn_commit_tool_output(out, ledger: EvidenceLedger) -> str:
            ...
            @staticmethod
            def fn_best_windows(note: str, terms: set[str], width: int,
        ...
        class PageSurgeon:
            ...
            @staticmethod
            def fn_do_retain_evidence(source: str, quote: str, ledger: EvidenceLedger) -> str:
            ...
            @staticmethod
            def fn_do_page_read(url: str, offset: int, length: int, ledger: EvidenceLedger) -> str:
            ...
            @staticmethod
            def fn_do_page_grep(url: str, pattern: str, ledger: EvidenceLedger) -> str:
            ...
            @staticmethod
            def fn_ledger_page(url: str, ledger: EvidenceLedger) -> tuple[int, dict] | None:
        ...
        # Helper classes (15) — aliases preserve call sites

        class MeterKeeper:
            """Behavior-preserving helper group."""

            @staticmethod
            def fn_wrapup_order(seconds_left: float) -> str:
                return (
            ...
            @staticmethod
            def fn_spend_note(payload) -> None:
                budget = getattr(payload, "budget", None)
                left = getattr(budget, "session_remaining_budget_usd", None)
                if isinstance(left, (int, float)):
                    _SPEND["left"] = float(left)
        ...
        class HuntFetch:
            ...
            @staticmethod
            async def fn_run_tool(call, question: str, ledger: EvidenceLedger, deadline: float) -> str:
                try:
                    args = json.loads(getattr(call, "arguments", None) or "{}")
                except Exception:
                    args = {}
            ...
            @staticmethod
            async def fn_do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger) -> str:
                if not url.strip():
                    return "# read_page: empty url"
                payload = None
                for _attempt in (0, 1):
            ...
            @staticmethod
            async def fn_do_search(query_text: str, ledger: EvidenceLedger):
                if not query_text.strip():
                    return "# web_search: empty query"
        ...
        class ThinkDial:
            ...
            @staticmethod
            def fn_upstream(lane: str, model: str) -> dict | None:
            ...
            @staticmethod
            def fn_least_think(lane: str, model: str = "") -> dict:
        ...
        class FilingRouter:
            ...
            @staticmethod
            async def fn_do_sec_filing(company: str, form: str, year: str, deadline: float) -> str:
                company = (company or "").strip()
                form = (form or "").strip() or "10-K"
                year = (year or "").strip()[:4]
                hint = _SEC_SEARCH_HINT.format(company=company, year=year, form=form)
            ...
            @staticmethod
            def fn_sec_pick_filing(recent: dict, form: str, year: str):
            ...
            @staticmethod
            async def fn_fetch_json(url: str, deadline: float):
                cached = _SEC_CACHE.get(url)
                if cached is not None:
                    return cached
                for _attempt in (0, 1):
            ...
            @staticmethod
            def fn_sec_norm_form(form: str) -> str:
            ...
            @staticmethod
            def fn_sec_tokens(text: str) -> list[str]:
        ...
        class LaneCourier:
            ...
            @staticmethod
            async def fn_chat_turn(messages: list[dict], deadline: float, *, finish_only: bool,
            ...
            @staticmethod
            async def fn_chat_simple(lane: str, model: str, system: str, user: str, *,
        ...
        class VerbatimMill:
            ...
            @staticmethod
            def fn_verbatim_structured(obj, ledger: EvidenceLedger, depth: int = 0):
            ...
            @staticmethod
            def fn_verbatim_from_source(value: str, ledger: EvidenceLedger) -> str:
            ...
            @staticmethod
            def fn_answer_line_only(answer: str, question: str) -> str:
        ...
        class DraftFilter:
            ...
            @staticmethod
            def fn_strip_lead_narration(text: str) -> str:
            ...
            @staticmethod
            def fn_sanitize_draft(text: str) -> str:

                return _VERIFY_MARK_RE.sub("", text or "").strip()

            @staticmethod
            def fn_is_usable_answer(text: str) -> bool:
            ...
            @staticmethod
            def fn_is_degenerate_repetition(text: str) -> bool:
            ...
            @staticmethod
            def fn_looks_like_tool_json(s: str) -> bool:
        ...
        class DigestWriter:
            ...
            @staticmethod
            async def fn_knowledge_resort(question: str, deadline: float) -> str:
                left = deadline - monotonic()
                if left < 12.0:
                    return ""
                try:
            ...
            @staticmethod
            async def fn_write_from_digest(question: str, ledger: EvidenceLedger, deadline: float) -> str:
            ...
            @staticmethod
            def fn_deterministic_answer(question: str, ledger: EvidenceLedger) -> str:
            ...
            @staticmethod
            def fn_informative_lead(preview: str, limit: int = 280) -> str:
            ...
            @staticmethod
            def fn_ledger_digest(ledger: EvidenceLedger, char_cap: int = 60000) -> str:
        ...
        class ShapeCoercer:
            ...
            @staticmethod
            def fn_coerce_to_schema(answer: str, schema, depth: int = 0):
            ...
            @staticmethod
            def fn_undigest_for_schema(basis: str) -> str:
            ...
            @staticmethod
            def fn_matches_schema_shape(value, schema) -> bool:
                kind = _schema_kind(schema)
                if not kind:
                    return True
                if kind == "array":
            ...
            @staticmethod
            async def fn_schema_output(question: str, answer: str, schema, deadline: float) -> object | None:
        ...
        class BriefEngine:
            ...
            @staticmethod
            async def fn_audit_patch(question: str, answer: str, messages: list[dict],
            ...
            @staticmethod
            async def fn_loop(question: str, brief: str, ledger: EvidenceLedger,
            ...
            @staticmethod
            async def fn_preseed(question: str, set_question: bool, ledger: EvidenceLedger,
                               deadline: float) -> str:
            ...
            @staticmethod
            async def fn_knowledge_brief(question: str) -> tuple[str, str]:
        ...
        class CiteForge:
            ...
            @staticmethod
            def fn_retained_count(ledger: EvidenceLedger) -> int:
                return sum(len(r.get("retained") or []) for r in ledger.rows)

            @staticmethod
            def fn_quote_table(ledger: EvidenceLedger) -> str:
            ...
            @staticmethod
            def fn_citations_for(answer: str, ledger: EvidenceLedger) -> list[CitationRef]:
            ...
            @staticmethod
            def fn_cited_numbers(answer: str, top: int) -> list[int]:
                answer = _normalize_brackets(answer)
                seen: set[int] = set()
                out: list[int] = []
                for m in _CITE_NUM_RE.finditer(answer):
            ...
            @staticmethod
            def fn_normalize_brackets(text: str) -> str:
                return (text or "").translate(_BRACKET_FIX)


        _sec_tokens = FilingRouter.fn_sec_tokens
        _sec_norm_form = FilingRouter.fn_sec_norm_form
        _fetch_json = FilingRouter.fn_fetch_json
        _sec_pick_filing = FilingRouter.fn_sec_pick_filing
        _do_sec_filing = FilingRouter.fn_do_sec_filing
        _least_think = ThinkDial.fn_least_think
        _upstream = ThinkDial.fn_upstream
        _chat_simple = LaneCourier.fn_chat_simple
        _chat_turn = LaneCourier.fn_chat_turn
        _knowledge_brief = BriefEngine.fn_knowledge_brief
        _preseed = BriefEngine.fn_preseed
        _loop = BriefEngine.fn_loop
        _audit_patch = BriefEngine.fn_audit_patch
        _normalize_brackets = CiteForge.fn_normalize_brackets
        _cited_numbers = CiteForge.fn_cited_numbers
        _spend_note = MeterKeeper.fn_spend_note
        _spend_left = MeterKeeper.fn_spend_left
        _wrapup_order = MeterKeeper.fn_wrapup_order
        _has_superlative = SignalProbe.fn_has_superlative
        _needs_superlative_proof = SignalProbe.fn_needs_superlative_proof
        _needs_set_completeness = SignalProbe.fn_needs_set_completeness
        _key_terms = TermHarvest.fn_key_terms
        _degrade_query = TermHarvest.fn_degrade_query
        _seed_queries = TermHarvest.fn_seed_queries
        _best_windows = WindowCutter.fn_best_windows
        _commit_tool_output = WindowCutter.fn_commit_tool_output
        _ledger_page = PageSurgeon.fn_ledger_page
        _do_page_grep = PageSurgeon.fn_do_page_grep
        _do_page_read = PageSurgeon.fn_do_page_read
        _do_retain_evidence = PageSurgeon.fn_do_retain_evidence
        _do_search = HuntFetch.fn_do_search
        _do_fetch = HuntFetch.fn_do_fetch
        _run_tool = HuntFetch.fn_run_tool
        _citations_for = CiteForge.fn_citations_for
        _quote_table = CiteForge.fn_quote_table
        _retained_count = CiteForge.fn_retained_count
        _answer_line_only = VerbatimMill.fn_answer_line_only
        _verbatim_from_source = VerbatimMill.fn_verbatim_from_source
        _verbatim_structured = VerbatimMill.fn_verbatim_structured
        _looks_like_tool_json = DraftFilter.fn_looks_like_tool_json
        _is_degenerate_repetition = DraftFilter.fn_is_degenerate_repetition
        _is_usable_answer = DraftFilter.fn_is_usable_answer
        _sanitize_draft = DraftFilter.fn_sanitize_draft
        _strip_lead_narration = DraftFilter.fn_strip_lead_narration
        _cap = DraftFilter.fn_cap
        async def query(query: Query) -> Response:
            question = (query.text or "").strip()
            if not question:
                return Response(text="No question provided.")
            try:
                ...
                return Response(text=f"Best-effort answer unavailable for: {question[:500]}")
        _ledger_digest = DigestWriter.fn_ledger_digest
        _informative_lead = DigestWriter.fn_informative_lead
        _deterministic_answer = DigestWriter.fn_deterministic_answer
        _write_from_digest = DigestWriter.fn_write_from_digest
        _knowledge_resort = DigestWriter.fn_knowledge_resort
        _schema_output = ShapeCoercer.fn_schema_output
        _schema_kind = ShapeCoercer.fn_schema_kind
        async def _solve(query: Query, question: str) -> Response:
            deadline = monotonic() + WALL_BUDGET_S
            try:
                info = await tooling_info(timeout=10.0)
            ...
            try:
                ...
                return Response(text=text)
        _matches_schema_shape = ShapeCoercer.fn_matches_schema_shape
        _undigest_for_schema = ShapeCoercer.fn_undigest_for_schema
        _coerce_to_schema = ShapeCoercer.fn_coerce_to_schema



        return query

_EASY_RUN = _Anks324234()._compile()
_HARD_RUN = _Pnks92342()._compile()
