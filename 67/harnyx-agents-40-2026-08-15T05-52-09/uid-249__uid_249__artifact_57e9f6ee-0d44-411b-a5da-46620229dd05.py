...
class BailPin_ddbc35:

    @staticmethod
    def _cedar_dock_ddbc35() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_CEDAR_DOCK_RUN_ddbc35 = CedarDock_ddbc35()._compile()
_PINE_DOCK_RUN_ddbc35 = PineDock_ddbc35()._compile()
_BAIL_PIN_ddbc35 = BailPin_ddbc35()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _BAIL_PIN_ddbc35._cedar_dock_ddbc35():
        return await _CEDAR_DOCK_RUN_ddbc35(query)
    return await _PINE_DOCK_RUN_ddbc35(query)
