...
class MicaPit_e3a3f9:
    ...
    def _compile(self):
        ...
        def _unseen_subjects(subjects: list[str], board: EvidenceBoard) -> list[str]:
            stored = [((r.get('text') or '') + ' ' + (r.get('preview') or '')).casefold() for r in board.rows]
            absent = []
            for s in subjects:
                needle = s.casefold()
        ...
        async def _verify_subjects(question: str, answer: str, messages: list[dict], board: EvidenceBoard, deadline: float) -> str:
            if deadline - monotonic() < SUBJECT_CHECK_MIN_LEFT_S or _spend_left() <= AUDIT_MIN_USD:
                return answer
            absent = _unseen_subjects(_named_subjects(question), board)
            if not absent:
                return answer
            target = absent[0]
            try:
                found = await asyncio.wait_for(_do_search(target, board), timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
        ...
        def _unevidenced_years(question: str, answer: str, board: EvidenceBoard) -> list[str]:
        ...
        async def _align_timeframe(question: str, answer: str, messages: list[dict], board: EvidenceBoard, deadline: float) -> str:
            if deadline - monotonic() < TIMEFRAME_MIN_LEFT_S or _spend_left() <= AUDIT_MIN_USD:
                return answer
            uncovered = _unevidenced_years(question, answer, board)
            if not uncovered:
                return answer
            year = uncovered[0]
            try:
                found = await asyncio.wait_for(_do_search(_year_probe_query(question, year), board), timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
                body = _commit_tool_output(found, board)
            except Exception:
                body = ''
...
class BraceRod_e3a3f9:

    @staticmethod
    def _feldspar_ok_e3a3f9() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_FELDSPAR_RUN_e3a3f9 = FeldsparPit_e3a3f9()._compile()
_MICA_RUN_e3a3f9 = MicaPit_e3a3f9()._compile()
_BRACE_ROD_e3a3f9 = BraceRod_e3a3f9()

@entrypoint('query')
async def query(query: Query) -> Response:
    if _BRACE_ROD_e3a3f9._feldspar_ok_e3a3f9():
        return await _FELDSPAR_RUN_e3a3f9(query)
    return await _MICA_RUN_e3a3f9(query)
