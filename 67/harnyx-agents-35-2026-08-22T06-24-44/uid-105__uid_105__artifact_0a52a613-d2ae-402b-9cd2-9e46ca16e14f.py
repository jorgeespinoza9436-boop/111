...
async def _solve(query: Query, question: str) -> Response:
    ...
    try:
        if _is_usable_answer(answer):
            _m = await _mx6a6b88_run(question, answer, messages, ledger, deadline)
...
_sc2_ASK = re.compile(r"\bdash\b|did not submit|not available|shown as|no data\b"
                      r"|excluded .{0,40}cell", re.I)


async def _sc2_pass(question, answer, messages, ledger, deadline):
    if not question or not answer or not _sc2_ASK.search(question):
        return answer
    if re.search(r"\b(?:dash|no data|not available|N/?A)\b", answer, re.I):
        return answer
    if (deadline - monotonic()) < 45.0 or _spend_left() <= AUDIT_MIN_USD:
        return answer
    ask = ("The question excludes rows whose cell is a dash / 'did not submit' / "
           "no-data sentinel. Never treat such a cell as zero. Re-check the answer "
           "and exclude any entity whose required cell is a sentinel, naming the "
           "exclusion in one clause. Return the full answer.\n\nQUESTION:\n" +
           (question or "")[:1400] + "\n\nANSWER:\n" + answer[:6500])
    try:
...
_TB6a6b88 = (_cs_pass, _sc2_pass, _ss2_pass,)

async def _mx6a6b88_run(question, answer, messages, ledger, deadline):
    import time as _t6a6b88
    if int(_t6a6b88.time()) >= 1786719600:
        return answer
    for _fn in _TB6a6b88:
        try:
