...
def _compose_ember_violet_agent_entry():
...
_ember_violet_agent_query_entry = _compose_ember_violet_agent_entry()


def _compose_sable_mistral_agent_entry():
    ...
    Stages added on the ordinary successful path in this build (rkcau):
      candidate-pool pre-pass, open-criteria hint, primary-source anchoring, measure conformance, citation dedupe + anchor backfill.
    Kill-safety: everything bounded by one deadline; force-commit well before it.
    """

    _S31_QUERY_TAG = "s31-hk6730"
    ...
    # ── post-audit stage: primary-source anchoring ────────────────────────────────
    _PRIMARY_CUE_RE = re.compile(
        r"\b(?:official|statistics?|census|population|gdp|budget|revenue|deficit|"
        r"filing|filed|regulation|statute|treaty|ruling|registry|register|"
        r"per capita|unemployment|inflation|mortality|enrolment|enrollment|"
        r"casualties|emissions|reserves)\w*\b", re.I)
    _PRIMARY_SUFFIXES = (".gov", ".mil", ".int", ".edu")
    _PRIMARY_INFIXES = (".gov.", ".edu.", ".mil.", ".ac.", ".gob.", ".gouv.")
    _PRIMARY_HOSTS = ("europa.eu", "un.org", "who.int", "imf.org", "worldbank.org",
                      "oecd.org", "eurostat", "sec.gov", "nasa.gov", "noaa.gov",
                      "bls.gov", "statcan", "ons.gov.uk", "destatis.de")
    ...
    async def _anchor_primary_source(question: str, answer: str, messages: list[dict],
        ...
        try:
            ...
            queries = [(tail + " official statistics").strip(),
                       (tail + " site:.gov").strip()]
            order = (
                "SOURCE CHECK — every source this answer cites is a secondary one "
                "(" + ", ".join(sorted(set(hosts))[:4]) + "), on a question whose "
                "values have an official publisher. Retrieve the publishing body's "
                "own page — the statistical agency, the regulator's filing, the "
                "official register — and re-anchor the load-bearing figures on it, "
                "citing the primary [n] beside each. Keep a secondary citation only "
                "where it adds something the primary source does not carry."
                + REWRITE_TAIL)
...
_sable_mistral_agent_query_entry = _compose_sable_mistral_agent_entry()


_BALANCED_ROUTER_SEED = "d31be0f91a76c4428e59f0ad"
...
def _balanced_route_label(query: Query) -> str:
    ...
    return "EmberVioletAgent" if bucket < 128 else "SableMistralAgent"


class EmberVioletAgent:
    async def __call__(self, query: Query) -> Response:
        return await _ember_violet_agent_query_entry(query)


class SableMistralAgent:
    async def __call__(self, query: Query) -> Response:
        return await _sable_mistral_agent_query_entry(query)


_BALANCED_PRIMARY_AGENT = EmberVioletAgent()
_BALANCED_SECONDARY_AGENT = SableMistralAgent()
_CANDIDATE_BRANCH_CLASS_NAMES = ("EmberVioletAgent", "SableMistralAgent")
