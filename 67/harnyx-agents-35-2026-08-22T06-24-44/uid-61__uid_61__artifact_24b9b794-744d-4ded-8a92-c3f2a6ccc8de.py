...
class Harborcb9830:
    ...
    def _ember_0bad75(self):
        """SN67 Harnyx miner — staged research protocol agent. [slot 10 build 2026-08-19T11:19:42+00:00]"""
        import asyncio
        import json
        import re
        from time import perf_counter
...
def _dovetail_c32b98(factory):
    """Build the reserve closure; a source that dies on import must not kill the agent."""
    try:
        return factory()._ember_0bad75()
    except Exception:
        return None

def _rivet_4a1686(response):
    if response is None:
        return ''
    return (getattr(response, 'text', None) or '').strip()

def _umber_0dd07f(response):
    if response is None:
        return 0
    return len(getattr(response, 'citations', None) or ())

def _pallet_6d8194(response):
    return response is not None and getattr(response, 'output', None) is not None

def _dovetail_9ee321(query, response):
    """Deterministic answer quality. No model call, so auditing is free."""
    if response is None:
        return 0.0
    if query.output_schema is not None and (not _pallet_6d8194(response)):
        return 0.0
    text = _rivet_4a1686(response)
    if not _pallet_6d8194(response) and len(text) < 40:
        return 0.0
    score = 1.0
    if _pallet_6d8194(response):
        score += 1.0
    score += min(_umber_0dd07f(response), 12) * 0.05
...
class Willow28e402:
    ...
    def _kestrel_4b3150(self, query, response):
    ...
    def _juniper_7054ad(self, query, response):
        return _dovetail_9ee321(query, response) <= 0.0

    async def _girder_7ca612(self, run, request, budget):
    ...
    async def quarry_ccebc2(self, query: Query) -> Response:
        started = monotonic()
        first = await self._girder_7ca612(self._primary, query, self._FATHOM_E92FA3)
        if not self._juniper_7054ad(query, first):
            return first if first is not None else Response(text='No answer produced.')
        remaining = self._INGOT_95D648 - (monotonic() - started)
        if remaining <= self._EMBER_452442:
            return first if first is not None else Response(text='No answer produced.')
        second = await self._girder_7ca612(self._reserve, query, remaining)
        candidates = [r for r in (first, second) if r is not None]
        if not candidates:
            return Response(text='No answer produced.')
        return max(candidates, key=lambda r: _dovetail_9ee321(query, r))
_LANTERN_01D958 = query
_BASALT_09628E = _dovetail_c32b98(Harborcb9830)
_TRELLIS_171871 = Willow28e402(_LANTERN_01D958, _BASALT_09628E)

@entrypoint('query')
async def query(query: Query) -> Response:
    return await _TRELLIS_171871.quarry_ccebc2(query)
