...
def _compose_violet_brook_entry():
...
_violet_brook_query_entry = _compose_violet_brook_entry()


def _compose_copper_ridge_entry():
    """agent_d — v32 "toolloop": model-driven research agent.
...
_copper_ridge_query_entry = _compose_copper_ridge_entry()


_BALANCED_ROUTER_SEED = "9ec420a6f15bd037"
...
def _balanced_route_label(query: Query) -> str:
    ...
    return "VioletBrook" if bucket < 128 else "CopperRidge"


class VioletBrook:
    async def __call__(self, query: Query) -> Response:
        return await _violet_brook_query_entry(query)


class CopperRidge:
    async def __call__(self, query: Query) -> Response:
        return await _copper_ridge_query_entry(query)


_BALANCED_PRIMARY_AGENT = VioletBrook()
_BALANCED_SECONDARY_AGENT = CopperRidge()
_CANDIDATE_BRANCH_CLASS_NAMES = ("VioletBrook", "CopperRidge")
