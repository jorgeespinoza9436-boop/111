...
def _compose_marigold_sparrow_entry():
...
_marigold_sparrow_query_entry = _compose_marigold_sparrow_entry()


def _compose_quartz_ibis_entry():
    ...
    VERSION = "v53-subject-period-slice"
    ...
    # ── numeric scanning, used by the citation post-pass ─────────────────────────────
    # One constant, one consumer: _answer_figures in the citation post-pass. This
    # build wires no numeric detector in the sweep chain -- every stage here is a
    # SCOPE repair, so on a figure-free answer the chain still does useful work.
    # The post-pass is what keeps figures honest: it anchors each one to a
    # materialized slice instead of trusting the whole stored row.
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        # SUBJECT, THEN PERIOD. Both sweeps are scope repairs and they compose in
        # one direction only. A wrong subject invalidates the period check outright:
        # searching the right year against the wrong entity spends the timeframe
        # stage's single search on evidence that cannot help. The reverse is not
        # true -- a wrong period leaves the subject correct -- so subject runs first.
        # Each sweep rewrites the WHOLE answer, so the order cannot be repaired
        # afterwards by whichever stage ran second.
        #
        ...
        #
        # With the pool pre-pass before the loop and the citation post-pass inside
        # _citations_for, this build covers all three scope axes -- POOL membership,
        # SUBJECT identity, PERIOD alignment -- and wires no numeric detector at all.
        ...
        for _sweep in (_verify_subjects, _align_timeframe):
            try:
                if not _is_usable_answer(answer):
                    break
                # the LOWEST floor of the wired sweeps (subject 105s, period 95s):
...
_quartz_ibis_query_entry = _compose_quartz_ibis_entry()


_BALANCED_ROUTER_SEED = "a49c2d83f71e650b"
...
def _balanced_route_label(query: Query) -> str:
    ...
    return "MarigoldSparrow" if bucket < 128 else "QuartzIbis"


class MarigoldSparrow:
    async def __call__(self, query: Query) -> Response:
        return await _marigold_sparrow_query_entry(query)


class QuartzIbis:
    async def __call__(self, query: Query) -> Response:
        return await _quartz_ibis_query_entry(query)


_BALANCED_PRIMARY_AGENT = MarigoldSparrow()
_BALANCED_SECONDARY_AGENT = QuartzIbis()
_CANDIDATE_BRANCH_CLASS_NAMES = ("MarigoldSparrow", "QuartzIbis")
