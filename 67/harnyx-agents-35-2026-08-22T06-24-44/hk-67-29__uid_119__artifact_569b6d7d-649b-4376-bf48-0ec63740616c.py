...
def _compose_onyx_lattice_agent_entry():
    ...
    _S282_QUERY_TAG = "s282-hk6729"  # per-hotkey canonical uniqueness
    ...
    # ── stage 3i: format-compliance repair ────────────────────────────────────────
    # Batch ce955ea6 taught the "output only X" lesson; this is its structural
    # sibling: a question demanding a TABLE, a JSON value, one-per-line, or a
    # comma-separated/sorted list loses the tie when the answer arrives in prose.
    # Deterministic: detect the demanded shape, check the answer's surface for it,
    # and only on a mismatch spend one bounded NO-TOOL rewrite round. Detection is
    # conservative — a false positive costs a rewrite, a false negative costs
    # nothing — and the champion's output-schema machinery is untouched.
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        # stage 3i: explicit output-shape demands must be honored
        try:
            if _is_usable_answer(answer) and (deadline - monotonic()) > 65.0:
                shaped = await _format_repair(question, answer, messages,
                                              ledger, deadline)
