...
# G3: USGS/NASA data-query (port uid71 via linhagem uid17)
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # G3: authoritative data-query prefetch (fail-open, time-gated)
    try:
    ...
    # G1 (graft F1, medido na linhagem uid17: 'output only X' + linhas de prova = 0 na task
    # 4b74e8b1 apesar de resposta certa): quando a pergunta exige saída exclusiva, manter só o
    # primeiro bloco (o estilo desta linhagem já abre com as entidades) e derrubar as linhas de
    # prova. Fora desses casos, NADA muda (a prova ganha desempates).
    try:
        if re.search(r"\boutput only\b|\bonly the exact text\b|\brespond only\b|\breturn only\b|"
            ...
            if len(_first) >= 3:
                answer = re.sub(r"^\s*\*{0,2}answer:?\*{0,2}\s*", "", _first, flags=re.I).strip()
    except Exception:
        pass
