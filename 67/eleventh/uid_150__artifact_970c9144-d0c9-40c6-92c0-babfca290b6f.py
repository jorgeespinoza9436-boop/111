...
LOOP_RULES = (
    ...
    "When the question NAMES a specific source ('Based on the Wikipedia "
    "table…', 'According to the BLS report…'), cite THAT source — the "
    "named-source match is more important than general authoritativeness. "
    "Otherwise prefer the most AUTHORITATIVE source that actually states the "
    "claim: the official database/filing/statistics page over an aggregator, "
    ...
    "CITATION NOTES: immediately after each [n] bracket, append a brief "
    "'Supports: [source title] states [the specific datum]' annotation — the "
    "exact figure, name, or date the citation proves for the claim in that "
    "sentence. A citation whose purpose the judge must INFER from a raw page "
    "excerpt scores lower than one whose 'Supports:' line states the proved "
    "fact explicitly. Never omit these annotations.\n\n"
...
class EvidenceLedger:
    ...
    def __init__(self) -> None:
        self.rows: list[dict] = []
        self.summaries: dict[int, str] = {}  # 1-indexed row -> "Supports:" summary
...
_CITE_REF_LINE_RE = re.compile(
    r"^\s*(?:\[?\d{1,3}\]?\s*[-–—:]\s*(?:http|www\.|Source|Title|URL|Result|Wikipedia|"
    r"Census|Bureau|BLS|NASA|SIPRI))", re.I)
...
def _is_citation_metadata_dump(text: str) -> bool:
    ...
    prose_count = sum(1 for l in lines[:12]
                      if len(l) > 60 and not _CITE_REF_LINE_RE.match(l)
...
def _is_usable_answer(text: str) -> bool:
    ...
    if _STUB_ANSWER_RE.match(s) or _is_degenerate_repetition(s):
        return False
    # v35: detect citation-metadata dumps (snippet_dump label) — answer is
    # mostly citation references / source listings with no substantive prose
...
_COMMIT_RULES = (
    ...
    "ANNOTATION: after each [n], write a brief 'Supports: [source] states "
    "[fact]' — the specific datum the citation proves. Where the evidence "
    "includes a pre-generated 'Supports:' summary, reproduce it after the [n] "
    "citation verbatim.\n\n"
...
async def _write_from_digest(question: str, ledger: EvidenceLedger, deadline: float) -> str:
    ...
    convo = [{"role": "system", "content": _COMMIT_RULES},
             {"role": "user", "content": (
                 ...
                 "claim carries its [n]; after each [n], include a brief "
                 "'Supports: [source] states [fact]' annotation (reproduce "
                 "any pre-generated Supports: lines from the evidence above). "
                 "Then the short proof section (pool, "
...
_SUMMARY_LINE_RE = re.compile(r"\s*\[(\d+)\]\s*Supports:\s*(.+)")


async def _generate_evidence_summaries(question: str, ledger: EvidenceLedger,
                                        deadline: float) -> None:
    ...
    prompt = (
        f"Question:\n{question}\n\n"
        f"Evidence items:\n\n{evidence_text}\n\n"
        "For each evidence item that is RELEVANT to answering the question, "
        "write exactly one line:\n"
        "[N] Supports: According to [source title/name], [the specific fact — "
        "exact figure, name, date, or data point — that this evidence "
        "establishes for answering the question].\n\n"
        "Skip irrelevant items. Be specific: cite exact numbers, names, or "
        "dates from the evidence text. Do not paraphrase or generalize."
    )

    try:
        raw = await _chat_simple(
            LLM_LANE_A, LOOP_MODEL_A,
            "Evidence summarizer. One-line 'Supports:' summaries only.",
            prompt,
            max_tokens=2000,
            timeout=min(20.0, left - 50.0),
            think=_least_think(LLM_LANE_A, LOOP_MODEL_A),
        )
    except Exception:
        return
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # Post-loop: generate per-evidence "Supports:" summaries (root change)
    try:
