"""SN67 Harnyx miner — staged research protocol agent. [slot 10 build 2026-07-25T00:51:14+00:00]"""
...
async def query(query: Query) -> Response:
    ...
    try:
        ...
        last_content = ""
        for _extra in range(CHECKPOINT_TOOL_TURNS):
            if deadline - perf_counter() <= FINAL_RESERVE_SECONDS:
                break
        ...
        if not final_answer and last_content:
            # a partially-written verification turn beats the raw-notes floor
        ...
        if final_answer:
            stripped_answer = _strip_tool_markup(final_answer)
            return _deliverable(_final_section(stripped_answer), index, cite_text=stripped_answer)
        return _deliverable(None, index)
    except Exception:
        return _deliverable(None, index)
# slot: 10 C1 2026-07-25T00:51:14+00:00
