...
def _exact_phrase_windows(text: str, phrases: list[str]) -> list[tuple[int, int, str]]:
    ...
    for phrase in phrases:
        ...
        # str.find("") returns search_from for any position, so an empty phrase
        # would never advance the cursor and would spin here forever.
...
# Guard budget and model, stated directly.
#
# These replace two chains of `try: <NAME> except NameError:` probes that
# searched for constants (TASK_TOTAL_BUDGET_SECONDS, TOTAL_BUDGET_SECONDS,
# BUDGET_SECONDS, TASK_BUDGET_SECONDS / AUDIT_MODEL, SCHEMA_MODEL, CLAIM_MODEL,
# RESORT_MODEL, LOOP_MODEL_B, LOOP_MODEL_A, MODEL). None of those eleven names
# is defined anywhere in this module, so every probe fell through to the
# hardcoded tail. The values below are exactly the values those chains produced,
# so behaviour is unchanged; only the runtime name probing is gone.
#
# Tail arithmetic against the 300s hard wall, for whoever tunes this next:
# the base query may return as late as ~284s on its own worst-case path
# (see TIME_GOVERNOR_RESERVE_SECONDS), while this budget lets the guard start
# work whenever the clock reads under 245s. The internal 35s/25s headroom
# checks below are what actually keep the guard off the wall.
V401_GUARD_TOTAL_BUDGET_SECONDS = 280.0
V401_GUARD_PROVIDER = "openrouter"
V401_GUARD_MODEL = "openai/gpt-oss-120b"


def _v401_total_budget(default: float = V401_GUARD_TOTAL_BUDGET_SECONDS) -> float:
    """Total task budget the post-answer guard plans against."""
    return float(default)


def _v401_provider_model() -> tuple[str, str]:
    """Provider and model used by the post-answer guard."""
    return V401_GUARD_PROVIDER, V401_GUARD_MODEL
...
_V401_PUBLIC_MARKER_RE = re.compile(r"\[\[(\d+)]]")


def _v401_public_marker_indices(text: str) -> set[int]:
    return {int(value) for value in _V401_PUBLIC_MARKER_RE.findall(text or "")}


def _v401_rewrite_preserves_citations(
    original: str,
    revised: str,
    citation_count: int,
) -> bool:
    """Reject a rewrite that would desynchronise markers from the citation list.

    By the time a response reaches the guard, _render_public_citations has
    already compacted the answer so that marker [[N]] resolves to
    citations[N - 1]. The rewrite is free-form prose generation, so it can drop,
    renumber, or invent markers; reusing the original citation list against such
    text silently misattributes evidence. The base pipeline raises on exactly
    these conditions, so the guard must not wave them through.
    """
    revised_markers = _v401_public_marker_indices(revised)
    # An invented or out-of-range index has no citation behind it.
    if any(index < 1 or index > citation_count for index in revised_markers):
        return False
    # A marker the original never used means the model renumbered rather than
    # preserved, so surviving markers can no longer be trusted to point at the
    # source they did before.
    if not revised_markers.issubset(_v401_public_marker_indices(original)):
        return False
    # Dropping every marker from a cited answer strips the support the judge
    # scores on.
    if citation_count > 0 and not revised_markers:
        return False
    return True


async def _v401_scoring_guard(query: "Query", response: "Response", deadline: float) -> "Response":
    # json, re, time and llm_chat are already module-level imports; the previous
    # function-local aliases re-imported them on every call and hid the fact that
    # this guard bypasses the module's own model-ladder helpers.
    _v401_clock = time.monotonic
    ...
    try:
        ...
        if revised and len(revised) >= max(60, int(len(answer_text) * 0.35)):
            existing_citations = getattr(response, "citations", None)
            citation_count = len(existing_citations) if existing_citations else 0
            # Only adopt the rewrite if its markers still line up with the
            # citation list being carried over from the original response.
            if not _v401_rewrite_preserves_citations(
                answer_text, revised, citation_count
            ):
                return response
            try:
                return Response(text=revised, citations=existing_citations)
            except Exception:
                return response
        return response
    except Exception:
...
_PERFECT_SUFFIX = "62585e4b77088ff9"
