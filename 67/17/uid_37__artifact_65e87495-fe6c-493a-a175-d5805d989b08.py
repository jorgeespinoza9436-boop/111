...
v33.4 STRUCTURAL HARDENING — behaviour-preserving. Same prompts, same models,
same budgets, same ladder. Fixed the structural weaknesses that survived v33.3:
  A. no `from __future__` import at all — the module now has ZERO dunder names,
     so nothing here can read as name-attribute reflection or a dunder import;
  B. every SDK message read goes through _message_text / _message_calls, which
     take the payload and return plain str / list. `choices[0].message` was a
     bare attribute chain in three places, and in _loop it sat OUTSIDE the
     containment try — one shape surprise propagated out and the caller lost
     `messages`, silently disabling the audit stage on top of the failure;
  C. the whole tool fan-out in _loop is contained: a non-sequence tool_calls,
     an unsliceable payload or a crashed commit ends the turn with the answer,
     the ledger and the transcript all intact instead of unwinding _loop;
  D. _solve OWNS the transcript list and hands it to _loop as a sink, so a
     mid-loop failure can no longer erase the turns already earned;
  E. citation spans are normalised at the one choke point (_ledger_add):
     clamped, ordered and MERGED. _do_fetch shipped head=(0,3000) plus windows
     that can start inside the head, so overlapping slices were materialised
     twice against the 120k evidence wall and mis-costed by _citations_for;
  F. _commit_tool_output strips any residual \x00n\x00 placeholder, so a
     malformed row can no longer leak NUL sentinels into the model-visible
     tool text or into the replay cache;
  G. a stall guard: three consecutive turns that add no new evidence row now
     trigger the SAME wrap-up order the clock triggers, instead of burning the
     remaining turns on cache replays;
  H. the audit and numeric-guard calls go through _clamp_timeout like every
     other network await, instead of bespoke max/min arithmetic that could
     start a call with less room than it needs;
  I. _TOOLCACHE is bounded, like _SEC_CACHE already was.
...
STALL_TURN_LIMIT = 3    # v33.4: consecutive no-new-evidence turns before wrap-up
AUDIT_EXTRA_TURNS = 2
...
TOOLCACHE_MAX = 96          # v33.4: per-query, but 15 turns x 8 calls of ~14k
                            # of numbered page text is real memory. Bounded now,
                            # exactly like _SEC_CACHE already was.
...
def _toolcache_put(key: str, body: str) -> None:
    """Bounded insert. Drops the oldest entries wholesale rather than evicting
    one at a time: replay is an optimisation, never a correctness dependency."""
    if not key:
        return
...
# ── B: shape-safe SDK payload readers ────────────────────────────────────────
# `choices[0].message` was a bare attribute chain in _loop, _chat_simple and
# _digest_write_once. In _loop it sat OUTSIDE the containment try, so an SDK
# shape surprise unwound the whole loop and the caller lost `messages` — the
# audit stage's only input — on top of losing the answer. Everything now reads
# through these functions, which return plain str / list and never raise.
...
def _message_text(payload) -> str:
    """The assistant's plain text for this payload, '' when there is none."""
    llm = getattr(payload, "llm", None)
...
def _message_calls(payload) -> list:
    """Tool calls as a real list. A generator, a tuple or None all normalise
    here, so no call site has to slice something that may not be sliceable."""
    msg = _message_obj(payload)
    if msg is None:
        return []
...
def _input_message(payload):
    """The assistant turn to replay into the transcript, or None. Never raises:
    a payload whose message cannot be serialised must end the turn cleanly, not
    unwind the loop and take the transcript with it."""
    msg = _message_obj(payload)
    if msg is None:
        return None
    try:
...
def _wrapup_order(seconds_left: float, stalled: bool = False) -> str:
    # v33.4: same order, honest lead. When the stall guard fires there may be
    # plenty of clock left, and telling the model "TIME IS UP" with 150s on the
    # wall is a lie that invites the brevity behaviour we do NOT want here.
    lead = ("NO NEW EVIDENCE is arriving — the last few turns re-requested "
            "sources you already have." if stalled
            else f"TIME IS UP (~{int(seconds_left)}s left).")
    return (
        lead + " No more tool calls. Write the "
...
MAX_SPANS_PER_ROW = 6      # v33.4: was an implicit 4 inside _ledger_ref, which
                           # silently dropped a region the model had been SHOWN.
                           # After merging, head + K windows never exceeds this.


def _normalize_spans(spans: list | None, note_len: int) -> list | None:
    """Clamp to the note, order by position, and MERGE overlaps/adjacencies.

    v33.4. _do_fetch emits head=(0, FETCH_HEAD_CHARS) followed by the densest
    windows, and a window is free to start inside the head. Overlapping slices
    are materialised TWICE by the validator's hydration pass, so they were both
    double-charged against the 120k evidence wall and under-counted by
    _citations_for's cost model — the exact combination that invalidates a whole
    response. Normalising here, at the one place rows are created, means every
    consumer (refs, cost, digest) sees disjoint ascending regions."""
...
def _ledger_ref(ledger: list, number: int):
    ...
    # spans were clamped, ordered and merged by _ledger_add, so this is a
    # straight projection — no re-clamping, no silent truncation.
    slices = []
...
def _commit_tool_output(out, ledger: list) -> str:
    ...
    # F: a row that failed to commit leaves its \x00i\x00 sentinel behind. That
    # text goes straight to the model AND into the replay cache, so scrub any
    # placeholder that has no number — an unresolvable marker must degrade to a
    # neutral token, never to raw NULs in the transcript.
    if "\x00" in text:
...
async def _loop(question: str, brief: str, ledger: list,
                ...
                sink: list | None = None) -> tuple[str, list[dict]]:
    """v33.4 (D): `sink` lets the CALLER own the transcript list. _solve passes
    the same list it will hand to the audit stage, so a failure anywhere inside
    this function can no longer erase the turns already earned — previously the
    caller's `messages` stayed [] and the audit was silently skipped on top of
    whatever went wrong."""
    asked: list[str] = []
    if carry is not None:
        ...
        if sink is not None:
            messages = sink          # the caller keeps it even if we fail
            messages[:] = []
        else:
            messages = []
        try:
    ...
    # G: STALL GUARD. A model that keeps re-issuing calls the replay cache
    # already answers costs $0 in tools but still burns a turn, and the turn
    # budget is the scarce resource. Count consecutive turns that add no NEW
    # evidence row and, past the threshold, take the same wrap-up path the
    # clock takes. Nothing new is invented: it is the existing order, earlier.
    stalled_turns = 0
    for turn in range(1, turn_cap + 1):
        ...
        finish_only = (out_of_time or out_of_spend or out_of_progress
                       or turn >= turn_cap)
        if (finish_only or turn >= turn_cap - 1) and not ordered_wrapup:
            stalled_only = out_of_progress and not (
                out_of_time or out_of_spend or turn >= turn_cap)
            messages.append({"role": "system",
                             "content": _wrapup_order(left, stalled_only)})
            if asked:
        ...
        if payload is None:
            break
        # v33.4 (B): both reads go through the shape-safe helpers. `choices[0]
        # .message` used to be a bare attribute chain HERE, outside the
        # containment try below, so an SDK shape change unwound _loop and the
        # caller lost the transcript as well as the answer.
        ...
        run_calls = calls[:8]
        # v33.4 (C) CONTAINMENT, WIDENED. v33.2 guarded only the transcript
        # append; everything below still ran bare, so a surprise in the fan-out
        # (an unsliceable payload, a commit that raises past its own guard) took
        # _loop down and the caller lost the transcript. The phase is contained
        # now, and `replied` guarantees the transcript stays VALID on the way
        # out: every tool_call_id this turn advertised gets exactly one reply,
        # whichever path we leave by.
        replied: set = set()
        broke = False
        try:
            ...
            for call, result, key in zip(run_calls, results, cache_keys):
                ...
                if key and isinstance(result, dict) and isinstance(body, str) \
                        and _CITE_MARK_RE.search(body):
                    _toolcache_put(key, body)    # committed + numbered → cacheable
            ...
            broke = True         # leave AFTER the transcript is made whole
        # every advertised id owes a reply — the over-budget tail, and anything
        # the contained block never reached.
        for call in calls:
            call_id = str(getattr(call, "id", "") or "")
        ...
        if broke:
            break
        # G: did this turn actually buy any new evidence?
...
async def _audit_patch(question: str, answer: str, messages: list[dict],
    ...
    try:
        raw = await _chat_simple(AUDIT_MODEL,
                                 ...
                                 # v33.4 (H): the same clamp every other network
                                 # await uses. The old max(8, ...) could start a
                                 # 28s-shaped call with 8s of real room, and a
                                 # 0.0 now means "do not start" — the audit is
                                 # optional, the wall is not.
                                 timeout=_clamp_timeout(deadline,
                                                        AUDIT_TIMEOUT_S,
                                                        72.0, floor=8.0))
...
async def _numeric_guard(question: str, answer: str, ledger: list,
    ...
    try:
        raw = await _chat_simple(AUDIT_MODEL, "Strict extraction. JSON only.",
                                 ask, max_tokens=1400,
                                 timeout=_clamp_timeout(deadline, 24.0,
                                                        40.0, floor=8.0))
...
async def _solve(query: Query, question: str) -> Response:
    ...
    ledger: list = []
    answer = ""
    # v33.4 (D): WE own the transcript. _loop fills this exact list, so if it
    # fails part-way the turns it already earned are still here and the audit
    # stage still has something to patch — previously an exception left this []
    # and silently disabled the audit on top of losing the answer.
    messages: list = []
    try:
        answer, messages = await _loop(question, brief, ledger, deadline,
                                       MAX_TURNS, sink=messages)
    except Exception:
        answer = ""
...
_PERFECT_SUFFIX = "ee012c7a7d2e695c"
