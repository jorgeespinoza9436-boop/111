...
# ── Easy / Hard profiles (uid_66 pattern: one pipeline, dial overrides) ───────
# EasyPath / HardPath both call _solve; only these dials differ. Hard mirrors
# the prior single-path defaults. Easy shortens the wall, turns, and wrap-up
# window, and skips the audit-patch stage (Hard-only completeness repair).
EASY_PROFILE: dict = {
    "budget": 230.0,                 # total wall seconds for this path
    "max_turns": 10,                 # fewer research turns than Hard
    "wrapup_at": 105.0,              # stop researching earlier (more remaining)
    "do_audit": False,               # skip audit_patch (Hard-only)
    "brief_min_left": 90.0,          # attempt knowledge brief above this remaining
    "audit_min_left": 75.0,          # unused when do_audit is False
}
HARD_PROFILE: dict = {
    "budget": WALL_BUDGET_S,         # 266s full protocol
    "max_turns": MAX_TURNS,          # 15 research turns
    "wrapup_at": WRAPUP_AT_S,        # 90s remaining -> wrap up
    "do_audit": True,                # enable completeness audit + patch
    "brief_min_left": 120.0,         # prior brief gate
    "audit_min_left": 75.0,          # prior audit entry gate
}
...
async def _loop(question: str, brief: str, ledger: EvidenceLedger,
                ...
                wrapup_at: float | None = None) -> tuple[str, list[dict]]:
    wrap_at = WRAPUP_AT_S if wrapup_at is None else float(wrapup_at)
...
# ── EasyPath / HardPath / DifficultyRouter (uid_66 profile pattern) ───────────
class EasyPath:
    """Slim path: shorter wall, fewer turns, earlier wrap-up, no audit patch."""

    async def run(self, query: Query) -> Response:
        question = (query.text or "").strip()
        return await _solve(query, question, EASY_PROFILE)


class HardPath:
    """Full path: prior single-path budgets including audit_patch."""

    async def run(self, query: Query) -> Response:
        question = (query.text or "").strip()
        return await _solve(query, question, HARD_PROFILE)
...
class DifficultyRouter:
    ...
    _PROMPT = (
        ...
        "SEC/EDGAR, two distinct sub-questions, or high risk of missing "
        "coverage.\n\n"
    ...
    async def _is_easy(self, text: str) -> bool:
        raw = await _chat_simple(
            self._MODEL, self._PROMPT, text,
            max_tokens=8, timeout=self._TIMEOUT_S,
            think={"enabled": False},
        )
        return (raw or "").strip().lower().startswith("easy")


@entrypoint("query")
async def query(query: Query) -> Response:
    """Difficulty-route like uid_66, then run EasyPath or HardPath."""
    question = (query.text or "").strip()
    if not question:
        return Response(text="No question provided.")
    try:
        easy = await DifficultyRouter()._is_easy(question)
    except Exception:
        easy = False
    try:
        if easy:
            return await EasyPath().run(query)
        return await HardPath().run(query)
    except Exception:
        return Response(text=f"Best-effort answer unavailable for: {question[:500]}")


async def _solve(query: Query, question: str, profile: dict | None = None) -> Response:
    """Shared research pipeline under an Easy or Hard profile.

    profile dials: budget, max_turns, wrapup_at, do_audit, brief_min_left,
    audit_min_left. Defaults to HARD_PROFILE when profile is None.
    """
    cfg = profile if profile is not None else HARD_PROFILE
    budget = float(cfg.get("budget", WALL_BUDGET_S))
    turn_cap = int(cfg.get("max_turns", MAX_TURNS))
    wrapup_at = float(cfg.get("wrapup_at", WRAPUP_AT_S))
    do_audit = bool(cfg.get("do_audit", True))
    brief_min_left = float(cfg.get("brief_min_left", 120.0))
    audit_min_left = float(cfg.get("audit_min_left", 75.0))
    ...
    brief = ""
    try:
        if _spend_left() >= BRIEF_MIN_USD and (deadline - monotonic()) > brief_min_left:
    ...
    try:
        answer, messages = await _loop(
            question, brief, ledger, deadline, turn_cap, wrapup_at=wrapup_at,
        )
    except Exception:
        answer = ""

    try:
        if (do_audit and _is_usable_answer(answer)
                and (deadline - monotonic()) > audit_min_left
