...
#
# This is the only stage here that acts BEFORE the answer exists, which is why
# it survives the tail contention the five post-audit sweeps compete in.
...
# numeric answers with a single supporting source. Runs LAST of the four: it is
# the narrowest fix, and every sweep above rewrites the answer it inspects.
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # three loop turns, so in practice the first one or two that trigger will
    # consume the window and the rest close on their own floors. The order is
    # therefore a PRIORITY ranking, not a pipeline: widest-effect first.
    #   subject  — wrong entity makes every later check moot
    #   pool     — missing members change what the figures below describe
    #   source   — must precede corroboration (see its docstring: running it
    #              after would let a pulled-in .gov result mask an answer whose
    #              load-bearing claims still sit on an aggregator)
    ...
    for _sweep in (_verify_subjects, _widen_pool, _anchor_primary_source,
...
_PERFECT_SUFFIX = "4baec9630fec00bf"
