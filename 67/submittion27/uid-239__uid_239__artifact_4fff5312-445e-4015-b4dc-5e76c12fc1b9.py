...
VERSION = "v53-criteria-authority-measure"
...
# it never competes with the post-audit sweeps for the tail.
...
PRIMARY_ANCHOR_MIN_LEFT_S = 88.0
...
# annotation applied before them would be discarded by the next rewrite. uid 111
# shipped it at 70s ahead of the authority sweep at 75s, which silently threw
# away every annotation this stage produced whenever that sweep fired.
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # Ordered by how fundamental the failure is: an answer about the WRONG
    # subject cannot be rescued by re-anchoring or reformatting, so subject
    # verification runs first; then which source carries the claims; then how the
    # figures are printed. Measure conformance is LAST because both stages above
    # rewrite the whole answer and would discard a unit annotation applied
    # earlier — uid 111 shipped the opposite order and threw that work away.
    # Each stage re-checks its own floor and returns `answer` untouched on any
    # failure.
    for _sweep in (_verify_subjects, _anchor_primary_source, _conform_measures):
        try:
...
_PERFECT_SUFFIX = "31140de8fa00c7f9"
