...
class DrawCord_3c86b6:

    @staticmethod
    def _canvas_ok_3c86b6() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_CANVAS_RUN_3c86b6 = CanvasBag_3c86b6()._compile()
_BURLAP_RUN_3c86b6 = BurlapBag_3c86b6()._compile()
_DRAW_CORD_3c86b6 = DrawCord_3c86b6()

async def _s27_base_query(query: Query) -> Response:
    if _DRAW_CORD_3c86b6._canvas_ok_3c86b6():
        return await _CANVAS_RUN_3c86b6(query)
    return await _BURLAP_RUN_3c86b6(query)


# --- build marker 20260815T0140Z_00_orok2447 -------------------------------------------------
_BUILD_MARKER_20260815T0140Z_00_orok2447 = "20260815T0140Z"


def _build_probe_20260815T0140Z_00_orok2447(seed: int = 0) -> int:
    """Unreferenced helper carrying the build identity."""
    acc = seed
    for i, ch in enumerate(_BUILD_MARKER_20260815T0140Z_00_orok2447):
        acc = (acc * 31 + ord(ch) + i) % 1000003
    return acc


class _BuildStamp_20260815T0140Z_00_orok2447:
    tag = _BUILD_MARKER_20260815T0140Z_00_orok2447

    def digest(self) -> int:
        return _build_probe_20260815T0140Z_00_orok2447(len(self.tag))
