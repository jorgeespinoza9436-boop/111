...
class PrimarySolver:
    ...
    def _compile(self):
        ...
        def _build_complex_branch():
            """hk421 "format+density" — champion-v52 toolloop + format-compliance repair + citation-density repair.
    ...
    """
        ...
        def _build_simple_branch():
            ...
            class DifficultyRouter:
                ...
                _PROMPT = 'Is this question easy, medium, or hard? Always reply with only one word: easy'
                _TIMEOUT_S = 30
            ...
            class Rank2:
                ...
                def _compile(self):
                    ...
                    _chat_turn = LlmClient._chat_turn
                    _knowledge_brief = ResearchLoop._knowledge_brief
                    _build_roster = ResearchLoop._build_roster
                    _seed_queries = ResearchLoop._seed_queries
                    _preseed = ResearchLoop._preseed
                    _loop = ResearchLoop._loop
                    _audit_patch = ResearchLoop._audit_patch
                    _lead_figure = Corroborator._lead_figure
                    _figure_sources = Corroborator._figure_sources
                    _corroborate = Corroborator._corroborate
                    _normalize_brackets = CitationBuilder._normalize_brackets
                    _cited_numbers = CitationBuilder._cited_numbers
                    _answer_line_only = CitationBuilder._answer_line_only
                    _verbatim_from_source = CitationBuilder._verbatim_from_source
            ...
            class Rank3:
                ...
                def _compile(self):
                    ...
                    _loop = ResearchLoop._loop
                    _audit_patch = ResearchLoop._audit_patch
                    _answer_values = ValueGuard._answer_values
                    _value_supported = ValueGuard._value_supported
                    _unsupported_values = ValueGuard._unsupported_values
                    _value_repair = ValueGuard._value_repair
                    _lead_figure = Corroborator._lead_figure
                    _figure_sources = Corroborator._figure_sources
                    _corroborate = Corroborator._corroborate
                    _normalize_brackets = CitationBuilder._normalize_brackets
            ...
            class ShadowCenterPad:
                """Center-pad a string; unused by live paths."""

                @staticmethod
                def pad(text: str, width: int=20, fill: str=' ') -> str:
                    s = text or ''
                    w = max(0, int(width))
                    f = fill[:1] or ' '
                    if len(s) >= w:
                    ...
                    left = (w - len(s)) // 2
                    right = w - len(s) - left
                    return f * left + s + f * right

            class ShadowCooldown:
                """Cooldown counter; unused by live paths."""

                def __init__(self, cool: int=3) -> None:
                    self.cool = max(0, int(cool))
                    self._left = 0

                def trigger(self) -> None:
                    self._left = self.cool
            ...
            class ShadowQueryKey:
                """Normalize a query into a cache key; unused by live paths."""

                @staticmethod
                def key(text: str) -> str:
                    return ' '.join((text or '').lower().split())

            class ShadowBracketBalance:
                """Check square-bracket balance; unused by live paths."""
            ...
            class ShadowCharClass:
                """Classify a character class mix; unused by live paths."""

                @staticmethod
                def kind(text: str) -> str:
                    s = text or ''
                    has_a = any((ch.isalpha() for ch in s))
                    has_d = any((ch.isdigit() for ch in s))
                    if has_a and has_d:
                        return 'alnum'
                    if has_a:
                        return 'alpha'
            ...
            class ShadowRunningSum:
                """Running sum tracker; unused by live paths."""
            ...
            class ShadowTogglePair:
                """A/B toggle; unused by live paths."""
            ...
            class ShadowRecentMap:
                """Bounded recent map; unused by live paths."""
            ...
            class ShadowProtoHint:
                """Hint protocol from text; unused by live paths."""
            ...
            class ShadowKebab:
                """Make kebab-case; unused by live paths."""

                @staticmethod
                def kebab(text: str) -> str:
                    parts = [p for p in (text or '').lower().replace('_', ' ').split() if p]
                    return '-'.join(parts)

            def shadow_compact(text: str) -> str:
                """Remove all whitespace; unused by live paths."""
                return ''.join((text or '').split())

            def shadow_as_bool(value: object) -> bool:
                """Loose bool coerce; unused by live paths."""
                if isinstance(value, bool):
                    return value
                s = str(value).strip().lower()
                return s in {'1', 'true', 'yes', 'y', 'on'}

            def shadow_first_digit_span(text: str) -> str:
                """First contiguous digit span; unused by live paths."""
                out: list[str] = []
                started = False
                for ch in text or '':
                    if ch.isdigit():
            ...
            def shadow_zip_shortest(a: list[str], b: list[str]) -> list[tuple[str, str]]:
                """Zip to shortest length; unused by live paths."""
                n = min(len(a), len(b))
                return [(a[i], b[i]) for i in range(n)]

            def shadow_repeat_to(text: str, n: int) -> str:
                """Repeat text to at least n chars then clip; unused by live paths."""
                s = text or ''
                if not s:
                    return ''
                need = max(0, int(n))
            ...
            def shadow_tail_words(text: str, n: int=5) -> str:
                """Keep last N words; unused by live paths."""
                words = (text or '').split()
                k = max(0, int(n))
                return ' '.join(words[-k:]) if k else ''

            def shadow_avg_len(items: list[str]) -> float:
                """Average string length; unused by live paths."""
                if not items:
                    return 0.0
                return sum((len(x or '') for x in items)) / float(len(items))

            def shadow_only_ascii(text: str) -> bool:
                """True if all chars are ASCII; unused by live paths."""
                return all((ord(ch) < 128 for ch in text or ''))

            def shadow_bullet(text: str) -> str:
                """Prefix a bullet; unused by live paths."""
                return f'- {shadow_tail_words(text, 40)}'

            def shadow_mix_hash(text: str) -> int:
                """Tiny mix hash; unused by live paths."""
                h = 5381
                for ch in shadow_compact(text):
                    h = (h << 5) + h + ord(ch) & 4294967295
                return h
            _EASY_RUN = Rank1()._compile()
            _MEDIUM_RUN = Rank2()._compile()
            _HARD_RUN = Rank3()._compile()
        ...
        def _route_to_complex_branch(query: Query) -> bool:
            ...
            if text_len >= 850:
                ...
                simple_score += 2
            ledger_terms = ('table', 'dataset', 'csv', 'json', 'spreadsheet', 'appendix', 'annual report', 'certificate', 'inventory', 'gazetteer', 'register', 'roster', 'specifications', 'monograph', 'postal bulletin', 'proceedings', 'press kit')
            reasoning_terms = ('calculate', 'percentage', 'percent', 'ratio', 'difference', 'decline', 'compare', 'rank', 'ranked', 'sort', 'sorted', 'highest', 'lowest', 'largest', 'smallest', 'filter', 'exclude', 'including', 'only', 'greater than', 'less than', 'between', 'under ', 'over ', 'at least', 'at most', 'more than', 'fewer than', 'strictly', 'exceeded', 'matching', 'criteria', 'qualified', 'ordered')
            direct_terms = ('who is', 'what is', 'when did', 'where is', 'name the', 'identify the', 'which state', 'which country', 'which city', 'which person', 'winner', 'capital', 'birthplace', 'release year', 'date is recorded')
            complex_score += 2 * sum((1 for term in ledger_terms if term in lowered))
            complex_score += sum((1 for term in reasoning_terms if term in lowered))
            simple_score += 2 * sum((1 for term in direct_terms if term in lowered))
            numeric_filter = any((ch.isdigit() for ch in text)) and any((term in lowered for term in ('under', 'over', 'less than', 'greater than', 'at least', 'at most', 'more than', 'fewer than', 'before', 'after', 'between', 'as of', 'exact', 'strictly', 'exceeded', 'excluding', 'registered', 'ordered', 'rank', 'ranks')))
            if numeric_filter:
                complex_score += 4
            if 'which of those' in lowered or 'among those' in lowered:
                complex_score += 2
            if 'list each' in lowered or 'list them all' in lowered or 'name all' in lowered:
                complex_score += 2
            if lowered.count(',') >= 5:
                complex_score += 2
            ...
            if 'first' in lowered and 'second' in lowered:
                complex_score += 1
            if text_len <= 330 and simple_score >= 2 and (complex_score <= 5):
                return False
            if complex_score >= simple_score + 5:
                return True
            if simple_score >= complex_score + 3:
                return False
            import hashlib as _router_hashlib
            basis = ('basalt-lantern-v1|' + text[:640] + '|' + text[-320:]).encode('utf-8', 'ignore')
            bucket = int(_router_hashlib.sha256(basis).hexdigest()[:2], 16)
            return bucket % 20 in (0, 2, 3, 5, 8, 11, 13, 17, 19)
...
"""
    ...
    evidence-grounded calculation, or structured field semantics) and return a
    small targeted reminder to append to this run's own brief. Purely
    additive: when no signal fires this returns None and behavior is
    unchanged from the baseline loop."""
    ...
    Wave 1 (producers) runs concurrently and is committed to the board, so its
    [n] numbers exist. Wave 2 (consumers) then runs against a board that
    ...
    Shaped like a real payload with one empty choice, so `_execute_research_loop` takes the same
    ...
    output is what the next turn reasons over; the board still holds the rest,
    and every [n] stays resolvable because numbering lives in the board, not
    in the transcript.
    """
    ...
    The searches are ISSUED in parallel but COMMITTED to the board strictly in
    ...
    A tool_call left unanswered makes the transcript malformed: `_verify_and_refine`
...
_TAG_2DA81F54="2da81f54a4004715a0c3e42325937c05"
import logging as _tag_logging_2da81f54
_tag_logging_2da81f54.getLogger("miner.tag").debug("tag=%s", _TAG_2DA81F54)
