...
# RUNS LAST BY DESIGN: every other sweep rewrites the whole answer, so a measure
# annotation applied before them would be discarded by the next rewrite.
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # Ordered widest-effect first: pool membership, then the decisive figure,
    # then which source carries it, then how the figures are printed. Each stage
    # rewrites the WHOLE answer, so measure conformance must come last or the
    # next rewrite discards it. Every stage re-checks its own time/spend floor
    # and returns `answer` untouched on any failure.
    for _sweep in (_widen_pool, _second_source_check,
                   _anchor_primary_source, _conform_measures):
        try:
