...
# block. The loop then researches against an explicit checklist, and stage 3w
# verifies the result afterwards. Fires only on questions the set/superlative
# detectors already flag, with time and spend floors, and any failure means the
# block is simply absent (champion behavior).
...
#
# NOTE the pairing with stage 1c: the pre-pass drafts the pool BEFORE research
# and this sweep verifies it AFTER, both gated on the same set/superlative
# predicate. They are complementary, not redundant — but on a question that
# trips neither detector, both are inert.
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # Ordered widest-effect first: is it about the right SUBJECT, then are all
    # pool MEMBERS present, then are the figures printed in the demanded unit.
    # Measure conformance is LAST because both stages above rewrite the whole
    # answer and would discard a unit annotation applied earlier — uid 135
    # shipped the opposite order and silently threw that work away. Each stage
...
_PERFECT_SUFFIX = "506ca99c92f86a4d"
