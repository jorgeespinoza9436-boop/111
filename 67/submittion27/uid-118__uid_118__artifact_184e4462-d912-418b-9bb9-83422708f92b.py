...
# ONE corroborating search. Runs after the two scope repairs, either of which
# can replace the decisive figure wholesale.
#
...
# RUNS LAST BY DESIGN: all THREE sweeps above rewrite the whole answer, so a
# measure annotation applied before them would be discarded by the next rewrite.
# uid 163 shipped it at 70s ahead of the authority sweep at 75s, which silently
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # FOUR sweeps share one tail, so the order is a PRIORITY ranking as much as a
    # pipeline: whichever detector fires first consumes much of the window.
    #   subject — wrong entity makes every later check moot
    #   period  — wrong period invalidates the figure the next stage inspects,
    #             and its repair can replace that figure wholesale
    #   figure  — single-source decisive value
...
_PERFECT_SUFFIX = "6dea015af32fd11b"
