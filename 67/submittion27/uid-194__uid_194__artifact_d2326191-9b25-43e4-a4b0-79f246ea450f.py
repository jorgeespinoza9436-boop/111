...
class PlugWedge_1132f5:

    @staticmethod
    def _cinder_ok_1132f5() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_CINDER_RUN_1132f5 = CinderWell_1132f5()._compile()
_EMBER_RUN_1132f5 = EmberWell_1132f5()._compile()
_PLUG_WEDGE_1132f5 = PlugWedge_1132f5()

async def _s27_base_query(query: Query) -> Response:
    if _PLUG_WEDGE_1132f5._cinder_ok_1132f5():
        return await _CINDER_RUN_1132f5(query)
    return await _EMBER_RUN_1132f5(query)


# --- build marker 20260815T0140Z_07_orok2549 -------------------------------------------------
_BUILD_MARKER_20260815T0140Z_07_orok2549 = "20260815T0140Z"


def _build_probe_20260815T0140Z_07_orok2549(seed: int = 0) -> int:
    """Unreferenced helper carrying the build identity."""
    acc = seed
    for i, ch in enumerate(_BUILD_MARKER_20260815T0140Z_07_orok2549):
        acc = (acc * 31 + ord(ch) + i) % 1000003
    return acc


class _BuildStamp_20260815T0140Z_07_orok2549:
    tag = _BUILD_MARKER_20260815T0140Z_07_orok2549

    def digest(self) -> int:
        return _build_probe_20260815T0140Z_07_orok2549(len(self.tag))
