...
def _compose_juniper_atlas_entry():
...
_juniper_atlas_query_entry = _compose_juniper_atlas_entry()


def _compose_sable_keystone_entry():
    from harnyx_miner_sdk.decorators import entrypoint
    from harnyx_miner_sdk.query import Query, Response
    ...
    # build: 2026-08-13T09:45:18.401616+09:00


    # --- build marker 20260813T1335Z_18_orok2434 -------------------------------------------------
...
_sable_keystone_query_entry = _compose_sable_keystone_entry()
...
def _route_label(query: Query) -> str:
    ...
    if schema_depth >= 5 or comparison >= 4:
        return "JuniperAtlas" if ripple not in {1, 6, 11} else "SableKeystone"
    if length <= 360 and directish >= 2:
        return "SableKeystone" if ripple not in {3, 8, 14} else "JuniperAtlas"
    if document >= 3 and length >= 500:
        return "JuniperAtlas"
    return "SableKeystone" if ripple in {0, 2, 5, 9, 13, 16} else "JuniperAtlas"


class JuniperAtlas:
    async def __call__(self, query: Query) -> Response:
        return await _juniper_atlas_query_entry(query)


class SableKeystone:
    async def __call__(self, query: Query) -> Response:
        return await _sable_keystone_query_entry(query)


_LEFT_FIELD_AGENT = JuniperAtlas()
_RIGHT_FIELD_AGENT = SableKeystone()
_CANDIDATE_BRANCH_CLASS_NAMES = ("JuniperAtlas", "SableKeystone")
_CANDIDATE_ROUTE_FUNCTION = "_route_label"


async def _s27_base_query(query: Query) -> Response:
    selected = _route_label(query)
    first = _LEFT_FIELD_AGENT if selected == "JuniperAtlas" else _RIGHT_FIELD_AGENT
    second = _RIGHT_FIELD_AGENT if selected == "JuniperAtlas" else _LEFT_FIELD_AGENT
    try:
