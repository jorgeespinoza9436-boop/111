...
# ── numeric scanning, shared by stages 3g/3m and the post-pass ──────────
# The donor builds defined this pattern twice under two different names with
# byte-identical bodies. One constant here, read by the grounding stage, the
# measure stage, and _answer_figures in the citation post-pass.
...
#
# NOTE ON SCOPE. The donor carried this stage next to a second-source check and
# its comment there explained the two split the space by backer count (zero vs
# exactly one). This build wires NO corroboration stage, so that split does not
# apply: grounding here handles every figure with no cited support, and a figure
# that has exactly one backer is simply left alone. Nothing downstream is
# waiting on it.
#
# RUNS BEFORE the measure stage ON PURPOSE. Every sweep rewrites the WHOLE
# answer, so unit and scale annotations applied first are discarded by any
# later rewrite. The measure stage must be LAST in any chain that contains it.
# Six donor builds -- uids 79, 111, 135, 81, 163 and 236 -- ship it ahead of a
# rewriting sweep by floor value; this build does not.
#
# The detector's exemption clause for COMPUTED values is load-bearing: LOOP_RULES
# demands derived means and totals, which by construction appear in no source, so
# without it every computed answer is flagged.
...
#
# It is the ONLY post-audit sweep in this build, so there is no ordering hazard
# with a later rewrite. It does interact with the citation post-pass below: that
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # GROUND, THEN CONFORM. The order is forced, not preferred: every sweep
    # re-invokes _loop and gets back a rewrite of the WHOLE answer, so any unit
    # or scale annotation the measure stage applies is discarded by a later
    # rewrite. The measure stage is therefore LAST here, and must stay last in
    # any chain it appears in.
    #
    # Both sweeps are numeric detectors: neither fires on a figure-free answer,
    # and that is by design. The two stages outside this chain carry the build on
    # those questions -- the pool pre-pass ran before the loop, and the citation
    # post-pass runs inside _citations_for for free.
    #
    # Two sweeps is also the measured ceiling on how many can fire: a firing
    # sweep costs ~42s of search envelope plus up to three loop turns, and
    # entering this chain with 230s still only funds two. Nothing is ranked below
    # the cut line. Each stage re-checks its own floor and returns `answer`
    # untouched on any failure.
    for _sweep in (_ground_figures, _conform_measures):
        try:
            if not _is_usable_answer(answer):
                break
            # the LOWEST floor of the wired sweeps (ground 90s, measure 70s):
            # breaking above it would skip a stage that could still have run.
