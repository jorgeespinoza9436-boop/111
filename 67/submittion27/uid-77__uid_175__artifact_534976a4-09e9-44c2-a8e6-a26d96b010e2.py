...
class LeadSolver:
    ...
    behavior is unchanged from the baseline loop."""
        ...
        class TinLatch_f217b3:
            ...
            def _compile(self):
                ...
                async def _run_search_web(query: str, index: _ResultBoard, *, deadline: float) -> str:
                    ...
                    results = tuple(_rank_search_results(results))
                ...
                async def _run_search_many(queries: list, index: _ResultBoard, deadline: float | None=None) -> str:
                ...
                async def query(query: Query) -> Response:
                    ...
                    try:
                        ...
                        if _is_usable_answer(final_answer or '') and deadline - perf_counter() > 45:
                            try:
                                audit = await llm_chat(provider=LLM_PROVIDER, model=MODEL, messages=[{'role': 'system', 'content': '# Strict Answer Auditor\n\nOutput JSON only with keys missing_elements, uncited_claims, suspect_attributions (arrays).'}, {'role': 'user', 'content': f"Audit vs question. JSON only.\n\nQuestion:\n{query.text}\n\nAnswer:\n{(final_answer or '')[:12000]}"}], tools=None, temperature=0.1, max_output_tokens=700, thinking=LlmThinkingConfig(enabled=False), timeout=min(30.0, max(5.0, deadline - perf_counter() - 10)))
                        ...
                        final_answer = _apply_output_directives(_QUESTION['text'], final_answer)
                        try:
                ...
                _MARKER_VECTOR_20509 = 'ade4deac36f2'

                def _normalize_vector_20509(items=(), *, base=99516):
                    total = base
        ...
        class SnapCollar_f217b3:

            @staticmethod
            def _zinc_ok_f217b3() -> bool:
                import time as _t
                _s = int(_t.time()) % 86400
                return _s >= 36000 or _s < 7200
        _ZINC_RUN_f217b3 = ZincLatch_f217b3()._compile()
        _TIN_RUN_f217b3 = TinLatch_f217b3()._compile()
        _SNAP_COLLAR_f217b3 = SnapCollar_f217b3()

        async def query(query: Query) -> Response:
            if _SNAP_COLLAR_f217b3._zinc_ok_f217b3():
                return await _ZINC_RUN_f217b3(query)
            return await _TIN_RUN_f217b3(query)
        return query
...
"""
    ...
    heavy batch; the loop then verifies the load-bearing facts."""
        ...
        async def _density_repair(question: str, answer: str, messages: list[dict], board: EvidenceBoard, deadline: float) -> str:
            if deadline - monotonic() < 65.0 or _spend_left() <= WRAPUP_MIN_USD:
                return answer
            if not board.rows:
                return answer
            ...
            async def execute(self) -> Response:
                ...
                try:
                    if _is_usable_answer(answer) and deadline - monotonic() > 65.0:
                        densified = await _density_repair(question, answer, messages, board, deadline)
                ...
                try:
                    if _is_usable_answer(answer) and deadline - monotonic() > 65.0:
                        shaped = await _format_repair(question, answer, messages, board, deadline)
