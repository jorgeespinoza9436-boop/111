...
def _compose_zephyr_notary_entry():
    """hk405 "units+format" — champion-v52 toolloop + unit-consistency repair + format-compliance repair.
    ...
    _S25_QUERY_TAG = "hk6717"  # per-hotkey canonical uniqueness
    ...
    CITATION_MAX_REF_CHARS = 14_000   # one board row must not eat the whole budget
    ...
    # ── evidence board (tool-result numbering for [n] citations) ─────────────────
    class EvidenceBoard:
    ...
    # append to the board as its OWN network call returned, so [n] assignment was
        ...
        for item in results:
            ...
            if not note.strip():
                continue   # F1: no source text -> the platform rejects any citation
                           ...
                           # response is invalidated. Never board it.
    ...
    async def _preseed(question: str, set_question: bool, board: EvidenceBoard,
        ...
        # shared board as its own network call returns, so [n] assignment depended on
    ...
    async def _execute_research_loop(question: str, brief: str, board: EvidenceBoard,
        ...
        for turn in range(1, turn_cap + 1):
            ...
            # wrote the board itself, and the deferred-commit refactor must not lose it.
            ...
            for call_result in zip(run_calls, results):
                call = call_result[0]
                # v32.5: board rows are appended HERE, in call order — never inside
    ...
    def _deterministic_answer(question: str, board: EvidenceBoard) -> str:
        ...
        for i, r in rows:                    # filter FIRST, then take 6: rows 1-6 are
            if picked >= 6:                  # page heads (nav chrome); the prose is
                break                        # usually further down the board
    ...
    # ── outermost stage: fetch cascade (hx55) ─────────────────────────────
    ...
    class _Hx55CascadeEvidence:
        """Tracks a search-to-fetch cascade; only fetch receipts can become citations."""
