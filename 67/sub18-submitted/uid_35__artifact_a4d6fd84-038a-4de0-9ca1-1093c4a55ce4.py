"""agent_x — brain-planned ledger research agent.

This candidate starts from the UID186 quote-first toolloop backbone, then adds a
reachable planning controller:

  query -> deterministic contract parse -> narrow LLM BrainPlan ->
  plan-shaped seed searches -> quote/evidence ledger -> typed audit hint ->
  final citation gate.

The change is architectural, not answer-memory: the BrainPlan never contains old
round answers and is forbidden to guess facts. After local validator-parity
testing showed the narrow planner LLM timing out on every sampled task, the
reachable BrainPlan is intentionally deterministic: it decides task type, source
route, needed claims, output contract, and audit risks without spending a dead
early chat call.
...
architecture, keeping the assets our line already validated:
  ...
  - OpenRouter primary plus a different-family OpenRouter fallback model.
...
VERSION = "v53.3-source-priority-citation-saturation"

# ── providers / models ────────────────────────────────────────────────────────
LLM_LANE_A = "openrouter"          # primary lane (loop + briefing)
LLM_LANE_B = "openrouter"          # fallback model on same configured key
# Keep UID186's primary loop model. Lane B is not an AI Gateway escape hatch in
# this candidate; it is a cheaper different-family OpenRouter fallback for empty
# replies, pin outages, and schema/rescue calls.
...
# needless failover to a weaker fallback. Runs that reached that lane scored
# 0.09 mean against 0.69.
#
...
TURN_TIMEOUT_S = 75.0
LANE_B_MAX_PAYLOAD_CHARS = 144000   # protect fallback from oversized transcripts
...
# ── brain-planned controller layer ────────────────────────────────────────────
# The UID186 backbone already knows how to search, fetch, retain quote spans and
# build CitationRef objects. This layer decides WHAT evidence graph to build
# before the first search. It is intentionally typed/deterministic at the edges:
# the LLM planner may classify and propose missing claims, but it may not provide
# final factual values.
BRAIN_PLANNER_TIMEOUT_S = 16.0
BRAIN_PLANNER_LLM_ENABLED = False  # v53.2: random-10 showed 10/10 planner timeouts.
BRAIN_PLANNER_MIN_USD = 0.015
BRAIN_MAX_SEED_QUERIES = 4
BRAIN_MODEL_A = "deepseek/deepseek-v3.2"   # cheap semantic planner; no facts
BRAIN_MODEL_B = "z-ai/glm-5.2"             # fallback when planner JSON fails
PRESEED_TOTAL_TIMEOUT_S = 54.0
PRESEED_PER_QUERY_TIMEOUT_S = 26.0

_YEAR_RE = re.compile(r"\b(?:18|19|20)\d{2}\b")
_TOPK_RE = re.compile(
    r"\b(?:top\s*\d+|highest|lowest|largest|smallest|most|least|first|last|"
    r"oldest|youngest|newest|rank(?:ed|ing)?|leader|winner)\b",
    re.I,
)
_DERIVED_RE = re.compile(
    r"\b(?:difference|increase|decrease|change|ratio|percentage|percent|"
    r"average|median|sum|total|minus|divided|per capita|growth rate)\b",
    re.I,
)
_INTERSECTION_RE = re.compile(
    r"\b(?:both|all of|each of|also|and also|as well as|intersection|"
    r"overlap|shared|common to)\b",
    re.I,
)
_OFFICIAL_HINT_RE = re.compile(
    r"\b(?i:according to|per|based on)\s+(?i:the\s+)?([A-Z][A-Za-z0-9&.'’\- ]{2,80})"
)
_SOURCE_FAMILY_HINTS = (
    ("wikipedia", re.compile(r"\bwikipedia\b|en\.wikipedia\.org", re.I)),
    ("sec_filing", re.compile(r"\b(?:10-K|10-Q|8-K|S-1|DEF 14A|annual report|sec filing)\b", re.I)),
    ("imdb", re.compile(r"\bimdb\b", re.I)),
    ("boxofficemojo", re.compile(r"\bbox office mojo\b|boxofficemojo", re.I)),
    ("world_bank", re.compile(r"\bworld bank\b|data\.worldbank", re.I)),
    ("official", re.compile(r"\bofficial\b|government|census|statistics office|bureau\b", re.I)),
)
_MEDIA_SOURCE_RE = re.compile(
    r"\b(?:song|track|single|album|soundtrack|movie|film|directed|director|"
    r"helmed|principal photography|filmed|starring|episode|series)\b",
    re.I,
)
...
def _schema_kind_light(schema) -> str:
    """Small schema-kind detector for the early BrainPlan layer."""
...
def _schema_contract(schema) -> dict:
    if not isinstance(schema, dict):
        return {"answer_mode": "text", "required_fields": [], "schema_kind": ""}
    required: list[str] = []
    props = schema.get("properties")
    if isinstance(props, dict):
        required.extend(str(k) for k in props.keys())
    explicit_required = schema.get("required")
    if isinstance(explicit_required, list):
        for k in explicit_required:
            if isinstance(k, str) and k not in required:
                required.append(k)
    return {
        "answer_mode": "schema",
        "required_fields": required[:20],
        "schema_kind": _schema_kind_light(schema),
    }


def _deterministic_brain_contract(question: str, schema) -> dict:
    q = " ".join((question or "").split())
    qlow = q.lower()
    years = _YEAR_RE.findall(q)
    schema_bits = _schema_contract(schema)
    source_targets: list[dict] = []
    for family, rx in _SOURCE_FAMILY_HINTS:
        if rx.search(q):
            source_targets.append({"kind": family, "name": family, "priority": 1})
    # Media clue chains are a common SN67 shape: identify a film/song/show, then
    # use that entity to compute something elsewhere. Wikipedia is not always the
    # final authority, but it is usually the cleanest bridge source for cast,
    # soundtrack, director, release and filming-location facts. Making it a
    # deterministic first-pass source reduces run-to-run drift to random blog/news
    # pages without hardcoding any particular prior task.
    if _MEDIA_SOURCE_RE.search(q) and not any(
        str(s.get("kind") or "") == "wikipedia" for s in source_targets
    ):
        source_targets.append({"kind": "wikipedia", "name": "wikipedia", "priority": 2})
    for m in _OFFICIAL_HINT_RE.finditer(q):
        name = m.group(1).strip(" .,:;")
        if name and not any(name.lower() == str(s.get("name", "")).lower() for s in source_targets):
            source_targets.append({"kind": "named_source", "name": name[:80], "priority": 1})

    if _TOPK_RE.search(q):
        task_type = "ranking_topk"
    elif _INTERSECTION_RE.search(q) or _needs_set_completeness(q):
        task_type = "set_intersection"
    elif _DERIVED_RE.search(q):
        task_type = "derived_numeric"
    elif len(years) >= 2 or re.search(r"\b(?:between|from)\s+(?:18|19|20)\d{2}\b", qlow):
        task_type = "time_series_numeric"
    elif re.search(r"\bwhich\b|\bwhat\b.*\b(?:that|with|where|whose)\b", qlow):
        task_type = "closed_list_filter"
    else:
        task_type = "direct_lookup"

    constraints: list[dict] = []
    for op, value in re.findall(r"(>=|<=|>|<|at least|at most|more than|less than|over|under)\s+\$?([0-9][0-9,]*(?:\.\d+)?)", q, re.I):
        constraints.append({
            "id": f"c{len(constraints) + 1}",
            "description": f"{op} {value}",
            "operator": op,
            "threshold": value,
        })
    if years:
        constraints.append({
            "id": f"c{len(constraints) + 1}",
            "description": "date/year scope: " + ", ".join(_dedupe_strings(years, 8)),
            "metric": "year",
        })

    needed_claims = [
        {"claim_id": "answer_value", "description": "the final answer value is directly supported"},
        {"claim_id": "source_basis", "description": "the cited source is the correct source family for the query"},
    ]
    if task_type in ("ranking_topk", "set_intersection", "closed_list_filter"):
        needed_claims.append({"claim_id": "candidate_pool", "description": "the candidate pool/list/table is covered"})
    if task_type in ("derived_numeric", "time_series_numeric", "ranking_topk"):
        needed_claims.append({"claim_id": "decisive_metric", "description": "decisive numbers/dates/ranks are retained with labels"})

    agenda = [q[:280]]
    if source_targets:
        for src in source_targets[:3]:
            name = str(src.get("name") or src.get("kind") or "").strip()
            if name and src.get("kind") == "wikipedia":
                agenda.append(f"site:en.wikipedia.org {q[:220]}")
            elif name and src.get("kind") == "sec_filing":
                agenda.append(q[:240] + " sec filing")
            elif name:
                agenda.append(f"{name} {q[:220]}")
    if task_type in ("ranking_topk", "set_intersection", "closed_list_filter"):
        # First find the roster/table. Per-member lookup is the classic zero-ish
        # loss mode on SN67 set questions.
        salient = [t for t in _SEED_TOKEN_RE.findall(q)
                   if len(t) >= 3 and t.lower() not in _STOP and t.lower() not in _SEED_STOP]
        if salient:
            agenda.append("list table " + " ".join(salient[:8]))

    return {
        "task_type": task_type,
        "answer_mode": schema_bits["answer_mode"],
        "source_targets": source_targets[:5] or [{"kind": "generic_web", "name": "web", "priority": 3}],
        "candidate_seed": [],
        "constraints": constraints[:8],
        "needed_claims": needed_claims[:8],
        "search_agenda": [{"purpose": "evidence", "query": s, "priority": i + 1}
                          for i, s in enumerate(_dedupe_strings(agenda, BRAIN_MAX_SEED_QUERIES))],
        "output_contract": {
            "mode": schema_bits["answer_mode"],
            "required_fields": schema_bits["required_fields"],
            "format": "schema" if schema_bits["answer_mode"] == "schema" else "answer-first prose",
        },
        "risk_flags": [],
...
def _merge_brain_plan(base: dict, llm_obj: dict) -> dict:
    if not llm_obj:
        return base
    out = dict(base)
    allowed_types = {
        "closed_table_filter", "closed_list_filter", "ranking_topk",
        "time_series_numeric", "set_intersection", "derived_numeric",
        "multi_hop_join", "direct_lookup",
    }
    task_type = str(llm_obj.get("task_type") or "")
    if task_type in allowed_types and base.get("task_type") == "direct_lookup":
        out["task_type"] = task_type
    for key in ("source_targets", "candidate_seed", "constraints", "needed_claims", "risk_flags"):
        vals = llm_obj.get(key)
        if isinstance(vals, list):
            merged = list(base.get(key) or [])
            for v in vals:
                if isinstance(v, dict):
                    sig = json.dumps(v, sort_keys=True)[:500]
                    if all(json.dumps(x, sort_keys=True)[:500] != sig for x in merged if isinstance(x, dict)):
                        merged.append(v)
                elif isinstance(v, str) and key == "risk_flags" and v not in merged:
                    merged.append(v)
            out[key] = merged[:12]
    agenda = list(base.get("search_agenda") or [])
    llm_agenda = llm_obj.get("search_agenda")
    if isinstance(llm_agenda, list):
        for item in llm_agenda:
            if isinstance(item, dict):
                query_text = str(item.get("query") or "").strip()
                purpose = str(item.get("purpose") or "evidence")
            else:
                query_text = str(item or "").strip()
                purpose = "evidence"
            if not query_text:
                continue
            if not any(str(a.get("query", "")).lower() == query_text.lower()
                       for a in agenda if isinstance(a, dict)):
                agenda.append({"purpose": purpose, "query": query_text[:300],
                               "priority": len(agenda) + 1})
    out["search_agenda"] = agenda[:BRAIN_MAX_SEED_QUERIES]
    # Deterministic schema/format always wins.
    out["answer_mode"] = base.get("answer_mode")
    out["output_contract"] = base.get("output_contract")
    out["planner_source"] = "deterministic+llm"
    return out


async def _build_brain_plan(question: str, schema, deadline: float) -> dict:
    base = _deterministic_brain_contract(question, schema)
    if not BRAIN_PLANNER_LLM_ENABLED:
        return base
    if (deadline - monotonic()) < 150.0 or _spend_left() < BRAIN_PLANNER_MIN_USD:
        return base
    schema_text = json.dumps(schema)[:3000] if schema is not None else "null"
    prompt = (
        "Question:\n" + question[:4000] + "\n\n"
        "Output schema, if any:\n" + schema_text + "\n\n"
        "Return JSON only. You are PLANNING web research; do not answer the "
        "question and do not invent factual values. Fields:\n"
        "{task_type, source_targets:[{kind,name,priority}], candidate_seed, "
        "constraints:[{id,description,metric,operator,threshold}], "
        "needed_claims:[{claim_id,description,required_for}], "
        "search_agenda:[{purpose,query,expected_source,priority}], "
        "risk_flags:[...]}. Prefer named source/list/table search first."
    )
    system = (
        "Narrow SN67 research planner. Classify the task and evidence needs. "
        "Forbidden: final answers, guessed facts, old task memories. JSON only."
    )
    for lane, model in ((LLM_LANE_A, BRAIN_MODEL_A), (LLM_LANE_A, BRAIN_MODEL_B)):
        if (deadline - monotonic()) < 125.0:
            break
        try:
            raw = await _chat_simple(lane, model, system, prompt, max_tokens=1300,
                                     timeout=min(BRAIN_PLANNER_TIMEOUT_S,
                                                 max(8.0, deadline - monotonic() - 110.0)),
                                     think=_least_think(lane, model))
            obj = _safe_json_obj(raw)
            if obj:
                return _merge_brain_plan(base, obj)
        except Exception:
            continue
    return base


def _brain_plan_summary(plan: dict) -> str:
    if not isinstance(plan, dict):
        return ""
    rows = []
    rows.append(f"task_type={plan.get('task_type', 'direct_lookup')}")
    rows.append(f"answer_mode={plan.get('answer_mode', 'text')}")
    srcs = []
    for src in plan.get("source_targets") or []:
        if isinstance(src, dict):
            srcs.append(f"{src.get('kind', '')}:{src.get('name', '')}".strip(":"))
    if srcs:
        rows.append("source_targets=" + "; ".join(srcs[:5]))
    claims = []
    for c in plan.get("needed_claims") or []:
        if isinstance(c, dict):
            claims.append(str(c.get("description") or c.get("claim_id") or "")[:160])
    if claims:
        rows.append("needed_claims=" + " | ".join(claims[:6]))
    risks = [str(r)[:140] for r in (plan.get("risk_flags") or []) if str(r).strip()]
    if risks:
        rows.append("risk_flags=" + " | ".join(risks[:5]))
    agenda = []
    for item in plan.get("search_agenda") or []:
        if isinstance(item, dict):
            agenda.append(str(item.get("query") or "")[:180])
    if agenda:
        rows.append("first_searches=" + " | ".join(agenda[:5]))
    return "\n".join(rows)


def _brain_system_brief(plan: dict) -> str:
    summary = _brain_plan_summary(plan)
    if not summary:
        return ""
    return (
        "BRAIN PLAN — internal controller state. Use this to choose searches and "
        "evidence; never copy this heading or this plan into the final answer.\n"
        + summary +
        "\n\nExecution policy:\n"
        "- Search the named/list/table source first when present.\n"
        "- Think candidate x constraint, not answer-first guessing.\n"
        "- For every decisive claim, call retain_evidence with the exact quote "
        "from the fetched/source-local window before finalizing.\n"
        "- Arithmetic, ordering, and schema shape must be mechanically checked in "
        "the answer; do not hallucinate numbers or cite snippets that lack the "
        "decisive value."
    )


def _brain_seed_queries(question: str, set_question: bool, plan: dict | None) -> list[str]:
    seeds: list[str] = []
    if isinstance(plan, dict):
        for item in plan.get("search_agenda") or []:
            if isinstance(item, dict):
                seeds.append(str(item.get("query") or ""))
            elif isinstance(item, str):
                seeds.append(item)
    seeds.extend(_seed_queries(question, set_question))
    return _dedupe_strings(seeds, BRAIN_MAX_SEED_QUERIES)


def _brain_ledger_digest(plan: dict | None, ledger) -> str:
    if not isinstance(plan, dict):
        return ""
    retained = 0
    source_rows = 0
    try:
        source_rows = len(getattr(ledger, "rows", []) or [])
        retained = sum(len(r.get("retained") or []) for r in ledger.rows)
    except Exception:
        pass
    return (
        "Typed coverage digest:\n"
        f"- task_type: {plan.get('task_type', 'direct_lookup')}\n"
        f"- answer_mode: {plan.get('answer_mode', 'text')}\n"
        f"- evidence_rows: {source_rows}\n"
        f"- retained_quote_spans: {retained}\n"
        f"- needed_claims: "
        + "; ".join(str(c.get("description") or c.get("claim_id") or "")[:120]
                    for c in (plan.get("needed_claims") or []) if isinstance(c, dict))[:600]
    )


def _brain_citation_gate_issue(answer: str, ledger: EvidenceLedger, citations: list[CitationRef],
                               plan: dict | None) -> str:
    if not isinstance(plan, dict):
        return ""
    rows = getattr(ledger, "rows", []) or []
    task_type = str(plan.get("task_type") or "direct_lookup")
    cited_nums = _cited_numbers(answer, len(rows)) if rows else []
    if rows and cited_nums and not citations:
        return "answer cites unavailable source numbers"
    if rows and task_type != "direct_lookup" and not cited_nums:
        return "search-dependent structured task has no inline citation markers"
    if rows and task_type in ("ranking_topk", "set_intersection", "closed_list_filter"):
        retained = 0
        try:
            retained = sum(len(r.get("retained") or []) for r in rows)
        except Exception:
            retained = 0
        if retained == 0 and len(citations) <= 1:
            return "candidate/constraint task has weak retained evidence surface"
    return ""


def _should_run_knowledge_brief(plan: dict | None, deadline: float) -> bool:
    """The UID186 knowledge brief is expensive; run it only when it adds value."""
    if _spend_left() < BRIEF_MIN_USD:
        return False
    if (deadline - monotonic()) < 180.0:
        return False
    if not isinstance(plan, dict):
        return True
    task_type = str(plan.get("task_type") or "direct_lookup")
    planner_source = str(plan.get("planner_source") or "")
    # If the cheap BrainPlan already classified a structured evidence task, the
    # old answer-shaped knowledge brief is mostly duplicate planning. Keep it for
    # direct/multi-hop questions where world knowledge can suggest the right path.
    if planner_source == "deterministic+llm" and task_type in {
        "ranking_topk", "set_intersection", "closed_list_filter",
        "closed_table_filter", "derived_numeric", "time_series_numeric",
    }:
        return False
    return True
...
# ── LLM plumbing (OpenRouter primary + OpenRouter fallback model) ─────────────
...
async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool,
    ...
    """One loop turn; pinned primary, unpinned primary, then fallback model."""
    # Keep a transcript-size guard for the fallback model. Very large transcripts
    # are exactly when the primary loop already consumed most of the budget; if
    # the fallback is likely to return empty/late, let existing rescue paths handle
    # the partial evidence instead.
    # The ladder is THREE rungs (pinned A, unpinned A, lane B), each bounded by
    ...
    # call alive at 60s is 60% salvageable and forcing failover scored 0.09
    # against 0.69). The wall only truncates the LATER rungs, and only
    ...
    # An UNPINNED lane-A rung sits between pinned lane A and the fallback model.
    # The pin is a hard filter (404 when every listed provider is down), so falling
    # straight from a pin outage to lane B would skip the cheaper unpinned primary.
    # Ordering is deliberate: fast, then slow-but-working, then different-family.
    for lane_model in ((LLM_LANE_A, LOOP_MODEL_A, True),
        ...
        try:
            ...
            payload = await asyncio.wait_for(llm_chat(
                ...
                temperature=0.2,
                # Keep UID186's loop reasoning behavior for the primary model.
                # On the fallback model's final-only turn, minimize extra reasoning
                # and ask for a larger direct completion instead.
...
async def _preseed(question: str, set_question: bool, ledger: EvidenceLedger,
                   deadline: float, brain_plan: dict | None = None) -> str:
    """Run the seed queries concurrently; return a numbered digest to inject."""
    seeds = _brain_seed_queries(question, set_question, brain_plan)
    if not seeds or (deadline - monotonic()) < 135.0:
        return ""
    preseed_stop = min(monotonic() + PRESEED_TOTAL_TIMEOUT_S,
                       deadline - WRAPUP_AT_S)
    if preseed_stop - monotonic() < 10.0:
        return ""
    ...
    blocks: list = []
    for seed in seeds:
        left_for_preseed = preseed_stop - monotonic()
        if left_for_preseed < 8.0:
            break
        try:
            per_query_timeout = min(PRESEED_PER_QUERY_TIMEOUT_S, left_for_preseed)
...
async def _loop(question: str, brief: str, ledger: EvidenceLedger,
                ...
                brain_plan: dict | None = None) -> tuple[str, list[dict]]:
    if carry is not None:
        ...
        seeded = await _preseed(question, set_q, ledger, deadline, brain_plan)
        if seeded:
...
async def _audit_patch(question: str, answer: str, messages: list[dict],
                       ...
                       brain_plan: dict | None = None) -> str:
    coverage_digest = _brain_ledger_digest(brain_plan, ledger)
    probe = (
        ...
        f"Question:\n{question}\n\n"
        f"{coverage_digest}\n\n"
        f"Brain plan:\n{_brain_plan_summary(brain_plan)[:3000] if isinstance(brain_plan, dict) else 'none'}\n\n"
        f"Answer:\n{answer[:11000]}"
    )
    try:
...
def _citation_ref_cost(ref: CitationRef, row: dict | None = None) -> int:
    slices = getattr(ref, "slices", None)
    if slices:
        return sum(max(0, int(s.end) - int(s.start)) for s in slices)
    if isinstance(row, dict):
        return int(row.get("note_len") or 0)
    return 0


def _citation_ref_key(ref: CitationRef) -> tuple[str, str]:
    return (str(getattr(ref, "receipt_id", "") or ""),
            str(getattr(ref, "result_id", "") or ""))


def _row_source_priority(question: str, plan: dict | None, row: dict) -> int:
    """Score rows worth appending to schema-only responses.

    For structured outputs the visible answer can be just {"country":"Cuba"} or
    {"city":"..."}. The judge then leans almost entirely on the citation array.
    This does not invent support text; it simply keeps high-value official/source
    rows that the agent already fetched from being dropped because the final
    schema conversion erased the proof-line citation markers.
    """
    title = str(row.get("title") or "")
    url = str(row.get("url") or "")
    preview = str(row.get("preview") or "")
    hay = " ".join((title, url, preview)).lower()
    score = 0

    if "api.worldbank.org" in hay or "data.worldbank" in hay or "worldbank.org" in hay:
        score += 22
    if "sec.gov" in hay:
        score += 18
    if "boxofficemojo.com" in hay or "imdb.com" in hay:
        score += 12
    if "wikipedia.org" in hay:
        score += 9 if _MEDIA_SOURCE_RE.search(question or "") else 5
    if str(row.get("kind") or "") == "fetch":
        score += 3
    if row.get("retained"):
        score += 7
    ...
    if isinstance(plan, dict):
        ...
        if task_type in {
            "ranking_topk", "set_intersection", "closed_list_filter",
            "closed_table_filter", "derived_numeric", "time_series_numeric",
        } and ("api." in hay or "data." in hay or ".gov" in hay):
            score += 4

    overlap = len(_key_terms(question or "") & _key_terms(title + " " + preview[:2000]))
    score += min(8, overlap)
    return score


def _augment_schema_citations(question: str, plan: dict | None, ledger: EvidenceLedger,
                              refs: list[CitationRef]) -> list[CitationRef]:
    """Add already-fetched high-value evidence refs for schema responses only."""
    out = list(refs or [])
    seen = {_citation_ref_key(r) for r in out}
    spent = 0
    ...
    for row in ledger.rows:
        ...
        if key[0] and key[1]:
            by_key[key] = row
    for ref in out:
        spent += _citation_ref_cost(ref, by_key.get(_citation_ref_key(ref)))

    candidates: list[tuple[int, int, int, CitationRef, tuple[str, str]]] = []
    for idx, row in enumerate(ledger.rows, start=1):
        priority = _row_source_priority(question, plan, row)
        if priority < 9:
            continue
        ref = ledger.ref_for(idx)
        if ref is None:
            continue
        key = _citation_ref_key(ref)
        if key in seen:
            continue
        cost = _citation_ref_cost(ref, row)
        if cost <= 0:
            continue
        candidates.append((-priority, idx, cost, ref, key))

    candidates.sort()
    for _neg_priority, _idx, cost, ref, key in candidates:
...
async def _write_from_digest(question: str, ledger: EvidenceLedger, deadline: float) -> str:
    ...
    # failure — the exact case lane B exists for — meant lane B was
...
async def _solve(query: Query, question: str) -> Response:
    ...
    brain_plan: dict = {}
    try:
        brain_plan = await _build_brain_plan(question, query.output_schema, deadline)
    except Exception:
        brain_plan = _deterministic_brain_contract(question, query.output_schema)
    brain_brief = _brain_system_brief(brain_plan)

    draft = ""
    brief = ""
    try:
        if _should_run_knowledge_brief(brain_plan, deadline):
    ...
    try:
        if _is_usable_answer(answer) and (deadline - monotonic()) > 98.0 \
    ...
    try:
        gate_issue = _brain_citation_gate_issue(answer, ledger, citations, brain_plan)
    except Exception:
        gate_issue = ""
    if gate_issue and ledger.rows:
        # Final citation gate: if the model wrote unsupported prose on a task that
        # clearly needed cited evidence, fall back to the deterministic cited
        # digest rather than ship answer-looking text with no valid evidence.
        det = _deterministic_answer(question, ledger)
        if _is_usable_answer(det):
            answer = det
            try:
    ...
    if query.output_schema is not None:
        try:
            citations = _augment_schema_citations(question, brain_plan, ledger, citations)
        except Exception:
            pass
        structured = None
        try:
