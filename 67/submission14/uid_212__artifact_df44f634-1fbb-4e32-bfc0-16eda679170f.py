...
# ── cross-source agreement guard (agent_b, additive, non-destructive) ────────
# For single-key-answer questions, a value resting on ONE source domain is a
# risk. This checks how many DISTINCT source domains back the answer's
# citations; if fewer than two, it runs ONE targeted confirmation search for an
# independent second source and APPENDS a corroboration line only when a row
# from a distinct domain literally states the same key value. It never rewrites
# the existing answer — it can only add a cited corroboration sentence.
_URL_HOST_RE = re.compile(r"https?://(?:www\.)?([^/]+)", re.IGNORECASE)
...
async def _cross_source_guard(question: str, answer: str,
                              ledger: EvidenceLedger, deadline: float) -> str:
    ...
    for n in cited:
        ...
        if h:
            domains.add(h)
    # already backed by >=2 distinct sources (or no headroom): leave untouched.
    ...
    try:
        out = await asyncio.wait_for(_do_search(subject, ledger),
                                     timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
    except Exception:
        return answer
    _commit_tool_output(out, ledger)
    # keep only NEW rows whose domain is not already cited (an independent source)
    fresh = [n for n in range(before + 1, len(ledger.rows) + 1)
             if _host(ledger.rows[n - 1].get("url") or "")
             and _host(ledger.rows[n - 1].get("url") or "") not in domains]
    ...
    for n in fresh[:8]:
        ...
        lines.append(f"[{n}] {row.get('title') or ''} ({row.get('url') or ''})\n"
                     f"{(row.get('preview') or '')[:500]}")
    digest = "\n\n".join(lines)
    system = (
        "You check whether an independent second source confirms the SINGLE key "
        "value of a finished answer. The numbered rows below are from sources NOT "
        "already cited. Reply on ONE line only: 'CONFIRM: <value> [n]' when a row "
        "states the SAME key value as the answer (copy the value verbatim from "
        "that row and give its [n]); otherwise reply 'NONE'. Never invent a row "
        "number, and never confirm from a value the row does not contain.")
    user = (f"QUESTION:\n{question}\n\nANSWER:\n{answer[:6000]}\n\n"
            f"INDEPENDENT SOURCES (numbered):\n{digest}")
    try:
        raw = await _chat_simple(
            LLM_LANE_A, LOOP_MODEL_A, system, user, max_tokens=120,
            timeout=max(8.0, min(AUDIT_TIMEOUT_S, (deadline - monotonic()) - 56.0)),
            think=_least_think(LLM_LANE_A, LOOP_MODEL_A))
    except Exception:
        return answer
    m = re.search(r"CONFIRM\s*:\s*(?P<val>.+?)\s*\[(?P<n>\d{1,3})\]", raw or "",
                  re.IGNORECASE | re.DOTALL)
    if m is None:
        return answer
    ...
    if val not in (row.get("preview") or ""):
        return answer            # corroboration must be a real value in a real row
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # ── PHASE 4c: cross-source agreement guard (agent_b, additive) ────────────
    # For single-key-answer (non-set) questions: if the answer's citations rest
    # on a single source domain, run ONE confirmation search and APPEND a cited
    # corroboration line only when an independent second source confirms the
    # value. Non-destructive. Guarded by deadline + USD floor.
    try:
        if _is_usable_answer(answer) and not _needs_set_completeness(question) \
                and (deadline - monotonic()) > 72.0 and _spend_left() >= AUDIT_MIN_USD:
