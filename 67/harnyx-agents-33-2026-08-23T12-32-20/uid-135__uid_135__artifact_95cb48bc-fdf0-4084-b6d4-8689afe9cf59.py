...
def _compose_zephyr_notary_entry():
    ...
    _LITERAL_PREFIX_RE = re.compile(
        r"(?:^|[\s(\[*_|])((?:No|Nos|Doc|Docket|Report|Rev|Ser|Vol|Pub|Ref|Case"
        r"|File|Order|Reg|Form|Part)\.\s?)$")
    ...
    def _schema_degenerate(value) -> bool:
        ...
        for probe in (_schema_digest_paste, _schema_all_empty,
                      _schema_uniform_scalars, _schema_row_filled):
            try:
                if probe(value):
                    return True
            except Exception:
    ...
    _POINTER_GROUP_RE = re.compile(r"\[\[?([0-9][0-9,\s\-]*)\]\]?")
    ...
    def _widen_citations(answer: str, holder, base: list, spent: int) -> tuple:
        ...
        out_kept: list = []
        for entry in entries:
            out_refs.append(CitationRef(receipt_id=entry[0].receipt_id,
                                        result_id=entry[0].result_id,
                                        slices=entry[2] or None))
    ...
    _REFUSAL_HEAD_RE = re.compile(
        r"^\s*(?:the |unfortunately[, ]*the )?(?:provided |available |gathered |retrieved )?"
        r"(?:evidence|excerpts?|sources?|search results?|citations?|documents?)\b"
        r"[^.]{0,100}\b(?:do(?:es)? not|don't|cannot|could not|lacks?|is insufficient"
        r"|are insufficient|is not sufficient)\b", re.I)


    _AUDIT_REPLY_RE = re.compile(
        r"tool\s+result|tool\s+call|retain_evidence|web_search|read_page"
        r"|value\s+audit|flagged\s+(?:value|figure|number|citation)"
        r"|i(?:'ll|\s+will)\s+(?:rewrite|re-?check|re-?verify|move|correct|revise)", re.I)
        ...
        async def _adopt_usable(self, should_run, producer) -> None:
            try:
                if should_run():
                    ...
                    if _is_usable_answer(patched) and not _AUDIT_REPLY_RE.search(
                            (patched or "")[:AUDIT_REPLY_HEAD_CHARS]):
                        self.answer = patched
            except Exception:
                pass
...
_ELEMENT_BRIEF_HEAD = (
    "REQUIRED ELEMENTS extracted from the query. This is an internal checklist: never "
    "echo, quote, or restate this block in your answer.\n\n")
_ELEMENT_BRIEF_TAIL = (
    "\n\nEvery [report] element must be answered and separately cited. Every [scope] "
    "element is a hard constraint on which evidence is admissible.")
...
_REPORT_ITEM_RE = re.compile(r"\((\d{1,2})\)\s*([^()]{3,240}?)(?=\s*\(\d{1,2}\)|\s*$)", re.S)
_ORDINAL_ITEM_RE = re.compile(r"(?m)^\s*(\d{1,2})[.)]\s+(.{3,240}?)\s*$")
_SCOPE_RE = re.compile(
    r"(?i)\b(using only|use only|based only|only the|exclude|excluding|ignore|do not|"
    r"treat\s+\w+|exactly as|verbatim|as given|must be|counted only|restrict)\b[^.;]{0,150}")
_ASK_RE = re.compile(
    r"(?i)\b(report|identify|list|name|give|state|determine|compute|calculate|find|"
    r"which|what|how many|how much|whether)\b[^.;]{0,190}")
