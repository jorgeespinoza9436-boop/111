...
def _compose_orchid_analyst_entry():
...
_orchid_analyst_query_entry = _compose_orchid_analyst_entry()


def _compose_quartz_archivist_entry():
    """SN67 Harnyx miner — staged research protocol agent. [slot 10 build 2026-08-19T15:00:54+00:00]"""
    ...
    # slot: 10 C35_qcase 2026-08-19T15:00:54+00:00

    return query

_quartz_archivist_query_entry = _compose_quartz_archivist_entry()


_BALANCED_ROUTER_SEED = "74bf2de3aea606384193ebef"
...
def _balanced_route_label(query: Query) -> str:
    ...
    return "OrchidAnalyst" if bucket < 128 else "QuartzArchivist"


class OrchidAnalyst:
    async def __call__(self, query: Query) -> Response:
        return await _orchid_analyst_query_entry(query)


class QuartzArchivist:
    async def __call__(self, query: Query) -> Response:
        return await _quartz_archivist_query_entry(query)


_BALANCED_PRIMARY_AGENT = OrchidAnalyst()
_BALANCED_SECONDARY_AGENT = QuartzArchivist()
_CANDIDATE_BRANCH_CLASS_NAMES = ("OrchidAnalyst", "QuartzArchivist")
