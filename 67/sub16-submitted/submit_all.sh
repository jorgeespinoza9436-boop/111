#!/usr/bin/env bash
set -euo pipefail
# Submit matched sub16-submitted agents to registered-unsubmitted SN67 hotkeys.
# Usage: bash /root/turtle/111/67/sub16-submitted/submit_all.sh [PARALLEL_JOBS]
ROOT="/root/turtle/harnyx"
JOBS_FILE="/root/turtle/111/67/sub16-submitted/submit_jobs.txt"
PARALLEL_JOBS="${1:-10}"
WALLET="money"
cd "$ROOT"
