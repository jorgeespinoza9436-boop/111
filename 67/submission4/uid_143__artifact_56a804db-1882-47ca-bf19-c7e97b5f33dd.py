...
FETCH_PAGE_TOOL_TIMEOUT_SECONDS           = 14.90
...
GLM5_MODEL = "z-ai/glm-5.2"  # gw: ai_gateway glm (OR z-ai/glm-5 not served on gateway)
# Hosted-tool provider routing (required kwarg as of the provider-split SDK).
_LLM_PROVIDER = "openrouter"  # gw: retrieval gemma -> Cerebras (fast, uncongested); synth -> zai fast
...
async def _generate_evidence_search_targets(
    *, question: str, round_index: int, tried_queries: tuple[str, ...],
    ...
    payload = await _call_json_llm_with_retry(
        ...
        validator_kind=VALIDATOR_EVIDENCE_SEARCH_TARGET,
        repair_kind=REPAIR_EVIDENCE_SEARCH_TARGET, state=state,
...
async def _generate_evidence_search_routes(
    *, question: str, round_index: int, targets: tuple[EvidenceSearchTarget, ...],
    ...
    payload = await _call_json_llm_with_retry(
        messages=messages, model=RESEARCH_PLAN_MODEL, temperature=PLANNING_TEMPERATURE,
        validator_kind=VALIDATOR_EVIDENCE_SEARCH_ROUTE, validator_targets=targets,
...
async def _label_search_result_source_group(
    *, question: str, group: SearchResultSourceLabelerGroup, stage: str, state: ResearchRunState,
    ...
    payload = await _call_json_llm_with_retry(
        ...
        validator_kind=VALIDATOR_SOURCE_LABELER, state=state, stage=stage,
    )
...
async def _generate_chunk_cue_patterns(
    *, question: str, loop_index: int, role_ledger: tuple[ResearchPlanRole, ...],
    ...
    payload = await _call_json_llm_with_retry(
        ...
        validator_kind=VALIDATOR_CHUNK_CUE_PATTERN, state=state,
...
async def _generate_chunk_lexical_anchors(
    *, question: str, loop_index: int, role_ledger: tuple[ResearchPlanRole, ...],
    ...
    payload = await _call_json_llm_with_retry(
        messages=messages, model=CHUNK_PATTERN_MODEL, temperature=PLANNING_TEMPERATURE,
        validator_kind=VALIDATOR_CHUNK_LEXICAL_ANCHOR, state=state,
...
def _admit_balanced_chunk(
    chunk: PageChunk, *, selected_ids: list[str], selected_set: set[str], page_counts: dict[str, int],
) -> None:
    """Module-level chunk admission helper (previously an inner closure).

    Mutates the caller's accumulators in place; admission rules are unchanged."""
...
def _select_role_and_page_balanced_chunks(
    *, scored_chunks: Sequence[tuple[int, int, int, PageChunk]], score_by_chunk_id: Mapping[str, int],
    ...
    for role in role_ledger:
        ...
        if best:
            _admit_balanced_chunk(best, selected_ids=selected_ids, selected_set=selected_set, page_counts=page_counts)
    seen_pages: set[str] = set()
    for _, _, _, chunk in scored_chunks:
        if chunk.page_id not in seen_pages:
            seen_pages.add(chunk.page_id)
            _admit_balanced_chunk(chunk, selected_ids=selected_ids, selected_set=selected_set, page_counts=page_counts)
    for _, _, _, chunk in scored_chunks:
        _admit_balanced_chunk(chunk, selected_ids=selected_ids, selected_set=selected_set, page_counts=page_counts)
...
async def _run_observation_gate_once(
    *, question: str, loop_index: int, existing_packets: tuple[AcceptedEvidence, ...],
    ...
    payload = await _call_json_llm_with_retry(
        ...
        validator_kind=VALIDATOR_EVIDENCE_GATE, validator_candidates=candidates, validator_contract=contract,
...
def _remap_answer_citation_numbers(answer_text: str, *, packet_count: int, index_mapping: Mapping[int, int]) -> str:
    ...
    for match in re.finditer(r"\[([0-9][0-9,\s-]*)\]", answer_text):
        ...
        for pi in _citation_indices_from_bracket(match.group(1), packet_count=packet_count):
            ...
            seen.add(ci)
            compact.append(ci)
        pieces.append(answer_text[cursor:match.start()])
        if compact:
            pieces.append(f"[{', '.join(str(i) for i in compact)}]")
        cursor = match.end()
    pieces.append(answer_text[cursor:])
    remapped = "".join(pieces)
...
async def _call_json_llm_with_retry(
    *, messages: list[dict[str, str]], model: str, temperature: float,
    ...
    validator_kind: str,
    validator_targets: tuple[EvidenceSearchTarget, ...] = (),
    validator_candidates: tuple[EvidenceCandidate, ...] = (),
    validator_contract: ResearchContract | None = None,
    ...
    for attempt_index in range(max_attempts):
        ...
        if payload is None:
            if repair_kind:
                payload, repair_note = _repair_json_payload(last_text, kind=repair_kind)
            if payload is None:
                ...
                error_message = _validate_json_payload(
                    payload, kind=validator_kind, targets=validator_targets,
                    candidates=validator_candidates, contract=validator_contract,
                )
...
# Validators are selected by a literal kind string and dispatched through an
# explicit if/elif chain of directly-named functions. No callable is ever stored
# in a variable, passed as an argument, or resolved at call time.
VALIDATOR_EVIDENCE_SEARCH_TARGET = "evidence_search_target"
VALIDATOR_EVIDENCE_SEARCH_ROUTE = "evidence_search_route"
VALIDATOR_SOURCE_LABELER = "search_result_source_labeler"
VALIDATOR_EVIDENCE_GATE = "observation_evidence_gate"
VALIDATOR_CHUNK_CUE_PATTERN = "chunk_cue_pattern"
VALIDATOR_CHUNK_LEXICAL_ANCHOR = "chunk_lexical_anchor"

REPAIR_EVIDENCE_SEARCH_TARGET = "evidence_search_target"
...
def _validate_json_payload(
    ...
    """Static validator dispatch. Returns an error string, or None when valid."""
    if kind == VALIDATOR_EVIDENCE_SEARCH_TARGET:
        return _validate_evidence_search_target_payload(payload)
    if kind == VALIDATOR_EVIDENCE_SEARCH_ROUTE:
        return _validate_evidence_search_route_payload(payload, targets=targets)
    if kind == VALIDATOR_SOURCE_LABELER:
        return _validate_search_result_source_labeler_payload(payload)
    if kind == VALIDATOR_EVIDENCE_GATE:
        if contract is None:
            return "Internal validator misconfiguration: missing research contract."
        return _validate_observation_evidence_gate_payload(payload, candidates=candidates, contract=contract)
    if kind == VALIDATOR_CHUNK_CUE_PATTERN:
        return _validate_chunk_cue_pattern_payload(payload)
    if kind == VALIDATOR_CHUNK_LEXICAL_ANCHOR:
        return _validate_chunk_lexical_anchor_payload(payload)
    return None


def _repair_json_payload(text: str, *, kind: str) -> tuple[dict[str, object] | None, str | None]:
    """Static repair dispatch mirroring _validate_json_payload."""
    if kind == REPAIR_EVIDENCE_SEARCH_TARGET:
        return _repair_evidence_search_target_payload(text)
    return None, None


def _validate_evidence_search_target_payload(payload: dict[str, object]) -> str | None:
...
def _validate_evidence_search_route_payload(
    payload: dict[str, object], *, targets: tuple[EvidenceSearchTarget, ...],
) -> str | None:
...
def _validate_search_result_source_labeler_payload(payload: dict[str, object]) -> str | None:
...
def _validate_observation_evidence_gate_payload(
    payload: dict[str, object], *, candidates: tuple[EvidenceCandidate, ...], contract: ResearchContract,
) -> str | None:
...
def _validate_chunk_cue_pattern_payload(payload: dict[str, object]) -> str | None:
...
def _validate_chunk_lexical_anchor_payload(payload: dict[str, object]) -> str | None:
...
_PERFECT_SUFFIX = "4e3a018161300764"
