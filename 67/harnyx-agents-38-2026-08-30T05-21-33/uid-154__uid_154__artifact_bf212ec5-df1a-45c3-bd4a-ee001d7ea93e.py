...
def _compose_saffron_harbor_agent_entry():
    ...
    # --- stage: set-completeness gap-fill -------------------------------------
    # A question that asks for N things is scored on all N. A draft that lists four
    # of five is not partially right, it is wrong, and the model rarely notices the
    # shortfall on its own. This counts what the draft enumerated against what was
    # asked and spends one search on the remainder.

    SETGAP_COUNT_RE = re.compile(
        r"\b(?:top|first|last|leading|largest|smallest|highest|lowest|best|worst|list|name|give)\s+"
        r"(\d{1,2}|two|three|four|five|six|seven|eight|nine|ten)\b",
        re.IGNORECASE,
    )
    ...
    async def _set_gapfill(
        ...
        directive = (
            f"The question asks for {wanted} entries and the draft enumerates {have}. "
            "Complete the list from the evidence, one entry per line, each with the "
            "[n] marker of the passage that places it in the set. If the evidence "
            "supports fewer than the number asked for, list what it supports and name "
            "the shortfall explicitly."
        )
        return await _stage_rewrite(question, answer, index, directive, deadline)
    ...
    async def _plain_query(query: Query, budget: float) -> Response:
        ...
        try:
            ...
            # --- ADDED STAGES: temporal-alignment repair then set-completeness gap-fill ---
            # These run on the answer the commit stage produced and before the
            # amend stage, so what amend re-projects is already repaired.
            stage_input = display
            stage_deadline = deadline - STAGE_TAIL_RESERVE_SECONDS
...
_TASK_SHAPE_TERTIARY_AGENT = SaffronHarborAgent()
