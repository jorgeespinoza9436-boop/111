...
async def _commit_call(messages: list[dict[str, object]], *, deadline: float) -> str | None:
    ...
    for _attempt in range(3):
        ...
        if _attempt == 0 and budget >= 70:
            ...
            # attempts 0-1 run the primary model, which requires reasoning; the
            # fallback model on attempt 2 keeps the cheaper no-reasoning call.
            thinking = (LlmThinkingConfig(enabled=False) if _attempt >= 2
...
# ── delivery: declared rungs, one executor decides what ships ─────────────────
...
# A rung that produces nothing says so with this sentinel, because `None` is a
# legitimate thing for the structured path to hand back.
_NO_ANSWER = object()
...
async def _rung_draft(st: _DeliveryState):
    try:
        st.drafted = await _plain_query(
            st.query, TASK_TOTAL_BUDGET_SECONDS - STRUCTURED_RESERVE_SECONDS)
    except Exception:
        st.drafted = Response(
...
async def _rung_structured(st: _DeliveryState):
    try:
        return await _structured_response(
            st.query, st.schema, st.drafted,
            perf_counter() + STRUCTURED_RESERVE_SECONDS)
    except Exception:
        return _so_response(_so_skeleton(st.schema, st.schema), None)


_DELIVERY_LADDER = (
    _Rung("plain", _rung_plain_ready, _rung_plain),
    _Rung("draft", _rung_schema_ready, _rung_draft),
    _Rung("structured", _rung_schema_ready, _rung_structured),
)
