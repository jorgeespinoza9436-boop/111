...
_BALANCED_ROUTER_SEED = "7d5eb0491d6f82ac"
...
def _balanced_route_label(query: Query) -> str:
    ...
    low = text.lower()
    # comparative / superlative questions need the comparison-set machinery
    if _rt_re.search(r"\b(?:most|least|highest|lowest|largest|smallest|greatest|"
                     r"fewest|longest|shortest|shorter|longer|more than|fewer than|"
                     r"compared with|versus)\b", low):
        return "JuniperCompassAgent"
    # enumeration / completeness questions need the pool-coverage branch
    if _rt_re.search(r"\b(?:all|each|every|list|how many|total|count|complete)\b", low):
