...
def _compose_quartz_relay_agent_entry():
    ...
    # ── V04  DUAL-FRAMING RECONCILIATION ──────────────────────────────────────
    # Controller: two concurrent runs of the same engine, one neutral and one
    #             framed to hunt disconfirming evidence, then a reconciliation.
    # Evidence:   claim -> which framing asserted it (_CxFramingTally).
    # Authority:  agreed claims ship bare, split claims ship hedged; the answer
    #             is rebuilt, never forwarded.
    ...
    _V04_ADVERSE_PREFIX = (
        "Research the question below with a disconfirming bias: before accepting "
        "any value, look for a source that contradicts it, and prefer the source "
        "closest to the origin of the figure. Report what you can defend.\n\n"
    )

    _V04_SPLIT_SYSTEM = (
        "You split a research answer into atomic factual claims. Return JSON "
        'only: {"claims": ["<one self-contained assertion>", ...]}. One fact each. '
        "Copy figures and names exactly. At most twelve."
    )

    _V04_WRITE_SYSTEM = (
        "You write a final answer from two independent research passes. AGREED "
        "claims were reached by both passes - state them plainly. SPLIT claims "
        "were reached by only one - include them only with an explicit hedge, "
        "and never in the answer line. Sentence one is the direct answer. Keep "
        "every [[n]] pointer exactly as given. No preamble, no meta-commentary."
    )


    class _CxFramingTally:
        """claim signature -> which framings asserted it."""

        def __init__(self):
            self.order = []
            self.rows = {}

        def add(self, framing, claim):
            signature = _cx_content_toks(claim)
            if len(signature) < 3:
                return
            for key in self.order:
                row = self.rows[key]
                if _cx_overlap(row["signature"], signature) >= _V04_AGREE:
                    row["framings"].add(framing)
                    if len(claim) > len(row["text"]):
                        row["text"] = claim
                    return
            key = len(self.order)
            self.order.append(key)
            self.rows[key] = {"signature": signature, "text": claim,
                              "framings": set([framing])}

        def agreed(self):
            out = []
            for key in self.order:
                if len(self.rows[key]["framings"]) >= 2:
                    out.append(self.rows[key]["text"])
            return out
    ...
    async def _v04_claims(draft, budget):
        if not draft or budget <= 6.0:
            return []
        raw = await _cx_chat(_V04_SPLIT_SYSTEM, "ANSWER:\n" + draft[:20000],
                             _CX_FAST, min(13.0, budget), 1000, 0.0)
        parsed = _cx_json(raw)
        found = []
        if parsed is not None:
            found = _cx_strs(parsed.get("claims"), _V04_MAX_CLAIMS)
        if found:
            return found
        return _cx_sentences(draft)[:_V04_MAX_CLAIMS]


    async def _v04_run(query):
        t0 = _w5_clock()
        question = (getattr(query, "text", "") or "").strip()
        schema = getattr(query, "output_schema", None)

        if schema is not None:
            base = await _cx_engine(query, _cx_engine_budget(t0, 16.0))
            if not _cx_usable(query, base):
                return _cx_response(None, None, [])
            return await _cx_schema_finish(question, schema, base, t0)

        span = _cx_engine_budget(t0, _V04_MECH_S)
        outcomes = await asyncio.gather(
            _cx_engine(query, span),
            _cx_engine(_CxSteer(_V04_ADVERSE_PREFIX + question, None), span),
            return_exceptions=True,
        )
        framings = ("neutral", "adverse")
        panel = []
        slot = -1
        for outcome in outcomes:
            ...
            if _cx_usable(query, outcome):
                panel.append((framings[slot], outcome))
        if not panel:
            return _cx_response(None, None, [])

        best_pair = panel[0]
        for pair in panel[1:]:
            if _cx_quality(query, pair[1]) > _cx_quality(query, best_pair[1]):
                best_pair = pair
        if len(panel) < 2 or _cx_left(t0, _CX_WALL_S) < 22.0:
            return best_pair[1]
        ...
        budget = max(6.0, _cx_left(t0, _CX_WALL_S) - 18.0)
        jobs = []
        for pair in panel:
            jobs.append(_v04_claims(texts[pair[0]], budget))
        harvested = await asyncio.gather(*jobs, return_exceptions=True)

        tally = _CxFramingTally()
        slot = -1
        for claims in harvested:
            slot = slot + 1
            if isinstance(claims, BaseException):
                continue
            for claim in claims:
                tally.add(panel[slot][0], claim)

        agreed = tally.agreed()
        if not agreed:
            return _cx_response(texts[best_pair[0]], None, citations)

        split_claims = tally.split()[:8]
        split_block = "(none)"
        if split_claims:
            split_block = "\n- ".join(split_claims)
        rebuilt = await _cx_chat(
            _V04_WRITE_SYSTEM,
            "QUESTION:\n" + question[:3000]
            + "\n\nAGREED:\n- " + "\n- ".join(agreed[:_V04_MAX_CLAIMS])
            + "\n\nSPLIT:\n- " + split_block,
            _CX_MODEL, min(22.0, max(6.0, _cx_left(t0, _CX_WALL_S) - 3.0)),
            3000, 0.2)
        if len(rebuilt) < 60:
            return _cx_response(texts[best_pair[0]], None, citations)
        return _cx_response(rebuilt, None, citations)


    async def query(query: Query) -> Response:
        try:
            return await _v04_run(query)
        except Exception:
            return _cx_response(None, None, [])
...
def _compose_juniper_harbor_agent_entry():
...
def _compose_ember_vector_agent_entry():
    ...
    # research loop discovers the pool from whatever the first search happens to
    # return. This runs before the loop and puts an explicit checklist in front of
    # it, so the loop verifies a pool instead of inventing one.
    ...
    ROSTER_GATE_RE = re.compile(
        ...
        r"leading|best|worst|first|last|rank(?:ed|ing)?)\b",
        re.IGNORECASE,
    )
    ROSTER_LINE_RE = re.compile(r"^\s*[-*\u2022]\s*(.+?)\s*$", re.MULTILINE)
    ...
    def _roster_message(roster: list[str]) -> str:
        ...
        return (
            "CANDIDATE POOL (unverified, from prior knowledge - treat as a checklist, "
            "not as evidence):\n" + listed + "\n"
            "Confirm or eliminate each entry against retrieved sources before you "
            "answer, and add anything the sources show that this list missed. Do not "
            "cite this list."
        )
    ...
    async def _build_roster(question: str, deadline: float) -> list[str]:
        ...
        try:
            ...
            result = await llm_chat(
                provider=LLM_PROVIDER, model=MODEL, messages=messages, temperature=0.0,
                thinking=LlmThinkingConfig(enabled=False),
                timeout=min(ROSTER_TIMEOUT_SECONDS, budget - 2),
            )
            text = (result.response.raw_text or "").strip()
    ...
    # The decisive figure is the one the answer is graded on, and a single source
    # carrying it is a single point of failure. When only one source states it,
    # this spends one search looking for a second, and rewrites against whatever
    # that search adds.
    ...
    async def _corroborate(
        ...
        directive = (
            f"The decisive figure {figure} rests on a single source. Confirm it "
            "against an independent source in the evidence and cite both. If no "
            "second source supports it, state the figure with the qualification that "
            "one source reports it, and keep its marker."
        )
    ...
    async def _plain_query(query: Query, budget: float) -> Response:
        ...
        try:
            ...
            # --- ADDED STAGES: lead-figure corroboration ---
...
_ember_vector_agent_query_entry = _compose_ember_vector_agent_entry()
...
class JuniperHarborAgent:
    async def __call__(self, query: Query) -> Response:
        return await _juniper_harbor_agent_query_entry(query)


class EmberVectorAgent:
    async def __call__(self, query: Query) -> Response:
        return await _ember_vector_agent_query_entry(query)


_TASK_SHAPE_PRIMARY_AGENT = QuartzRelayAgent()
_TASK_SHAPE_SECONDARY_AGENT = JuniperHarborAgent()
_TASK_SHAPE_TERTIARY_AGENT = EmberVectorAgent()
