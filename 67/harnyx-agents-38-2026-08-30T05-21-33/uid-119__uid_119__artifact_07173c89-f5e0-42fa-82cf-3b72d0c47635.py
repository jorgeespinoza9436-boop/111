"""uid_90 research agent - variant 10.

Two stages added to the h666 ledger cycle, one taken from each of two
validator_reference candidates:
  uid  89 - pointer-integrity backfill (backfill, repair)
  uid  24 - candidate-roster pre-pass (roster, prepass)

The three-branch router, the branch pipelines and the ledger's own audit
are unchanged. A gate stage folds a deterministic finding into the ledger
so its existing retrieve-and-regenerate cycle fires; a repair stage runs
on the final response. All model calls go through _h666_chat, pinned to
...
def _compose_lumen_anvil_agent_entry():
    ...
    LLM_LANE_B = "openrouter_secondary"
...
def _compose_juniper_compass_agent_entry():
    ...
    def _so_evidence(limit: int = STRUCTURED_EVIDENCE_PROMPT_CHARS) -> str:
        ...
        # callable out of a container and calling it is an unsupported_callable
        # in the miner-script subset, and this reads the same evidence either way.
        if not _SO_EVIDENCE_HOOK:
            return ""
        try:
            return (_serializer_evidence(_SO_EVIDENCE_HOOK[0], limit) or "")[:limit]
        except Exception:
            return ""
...
async def _h666_base_query(query: Query) -> Response:
    ...
    # Each branch is awaited by name. Binding one of three agents to a variable
    # and calling that variable selects a callable at run time, which the miner
    # -script subset rejects; the routing decision and the behaviour are
    # unchanged.
    if selected == "LumenAnvilAgent":
        response = await _lumen_anvil_agent_query_entry(query)
    elif selected == "CedarQuillAgent":
        response = await _cedar_quill_agent_query_entry(query)
    else:
        response = await _juniper_compass_agent_query_entry(query)
...
# --- added stage kit ------------------------------------------------------
# Shared plumbing for the added stages. The ledger's own audit is a model
# judgement about the draft; these detectors are deterministic, so they fire on
# defects the audit turn misses and they cost nothing when they do not fire.

_STAGE_FIGURE_RE = _h666_re.compile(r"\d[\d,]*(?:\.\d+)?")
_STAGE_YEAR_RE = _h666_re.compile(r"\b(?:19|20)\d{2}\b")
_STAGE_ITEM_RE = _h666_re.compile(r"^\s*(?:[-*\u2022]|\d{1,2}[.)])\s+\S", _h666_re.M)
_STAGE_ENTITY_RE = _h666_re.compile(r"\b[A-Z][\w&.-]*(?:\s+[A-Z][\w&.-]*)*")
_STAGE_SENTENCE_RE = _h666_re.compile(r"(?<=[.!?])\s+|\n+")
_STAGE_POINTER_RE = _h666_re.compile(r"\[\[(\d{1,3})\]\]")
_STAGE_BARE_RE = _h666_re.compile(r"(?<!\[)\[(\d{1,3})\](?!\])")
_STAGE_WORD_NUMBERS = {
    "two": 2, "three": 3, "four": 4, "five": 5, "six": 6,
    "seven": 7, "eight": 8, "nine": 9, "ten": 10,
}
_STAGE_MAX_CLAIMS = 4
_STAGE_MAX_QUERIES = 4
_STAGE_TOTAL_S = 268.0
_STAGE_MIN_LEFT_S = 16.0
_STAGE_CHAT_TIMEOUT_S = 13.0


def _stage_left(started: float) -> float:
    """Seconds of task budget still unspent."""
    return _STAGE_TOTAL_S - (_h666_monotonic() - started)
...
def _stage_head(question: str) -> str:
    return " ".join((question or "").split())[:150]


def _stage_fold(ledger: _H666Ledger, claims: list, queries: list) -> None:
    """Put a deterministic finding where the ledger's own cycle will act on it.

    missing_elements is the list requires_fresh_retrieval_and_rewrite() reads
    and open_claims() hands to the regenerate prompt, so a stage that appends
    here gets retrieval and a rewrite without a parallel pipeline of its own.
    """
    if claims:
        merged = list(ledger.missing_elements)
        for claim in claims:
            if claim and claim not in merged:
                merged.append(claim)
        ledger.missing_elements = merged[:6]
    if queries:
        merged_q: list = []
        for text in queries:
            if text and text not in merged_q:
                merged_q.append(text)
        for text in ledger.targeted_queries:
            if text and text not in merged_q:
                merged_q.append(text)
        ledger.targeted_queries = merged_q[:_STAGE_MAX_QUERIES]


def _stage_note_of(response) -> str | None:
    note = getattr(response, "note", None)
...
async def _stage_repair_rewrite(
    question: str, response, citations: list, defect: str, started: float,
):
    """One bounded rewrite aimed at a single named defect.

    Returns None whenever the repair cannot be trusted - no budget, no parse,
    or a reply that is shorter than half the draft - so the caller keeps the
    answer it already had.
    """
    left = _stage_left(started)
    if left < _STAGE_MIN_LEFT_S:
        return None
    draft_text = getattr(response, "text", None)
    is_text = isinstance(draft_text, str) and bool(draft_text.strip())
    if is_text:
        system = (
            "Repair one named defect in a research answer. Return JSON only with "
            "keys text (string), note (string or null). Change only what the "
            "defect names and keep every other sentence, figure and [[n]] pointer "
            "exactly as it stands. Sentence one is the answer. Every material "
            "researched claim keeps its [[n]] pointer to the numbered citation "
            "array; ordinary [n] is not a citation. Do not invent facts and do "
            "not introduce a pointer number the draft does not already use. If "
            "the defect does not actually apply, return the draft unchanged."
        )
    else:
        system = (
            "Repair one named defect in a structured research answer. Return JSON "
            "only with keys output (JSON value matching the public schema), note "
            "(string or null). Follow the schema exactly and change only what the "
            "defect names. Do not put citation syntax in atomic fields. Keep the "
            "explanation in note with the same [[n]] pointers. Do not invent "
            "facts. If the defect does not actually apply, return the draft "
            "output unchanged."
        )
    user = (
        f"Question:\n{question[:2500]}\n\n"
    ...
    parsed = _h666_parse_json(
        await _h666_chat(system, user, max_tokens=1600,
                         timeout=min(_STAGE_CHAT_TIMEOUT_S, left - 3.0))
    )
    if not parsed:
        return None
    note = parsed.get("note")
    if isinstance(note, str) and note.strip():
        note_text = " ".join(note.split()).strip()
    else:
        note_text = _stage_note_of(response)
    if is_text:
        text = parsed.get("text")
        if not isinstance(text, str):
            return None
        candidate = text.strip()
        if len(candidate) < max(80, int(len(draft_text.strip()) * 0.5)):
            return None
        return _h666_rebuild(response, candidate, None, note_text, citations)
    output = parsed.get("output")
    if output is None:
        return None
    return _h666_rebuild(response, None, output, note_text, citations)
...
# --- stage: pointer-integrity backfill ------------------------------------
# _h666_normalize_pointers promotes [n] to [[n]] when n is in range and leaves
# everything else alone, so a pointer that overruns the citation array survives
# into the delivered answer and reads as a citation to a source that is not
# there. This is the deterministic pass that finishes the job: out-of-range
# pointers are dropped and repeated adjacent pointers collapsed. No model call
# and no retrieval - it re-projects what is already there.

_BACKFILL_ADJACENT_RE = _h666_re.compile(r"(\[\[(\d{1,3})\]\])(?:\s*\[\[\2\]\])+")


def _stage_pointer_fix(text: str, n_cites: int) -> str:
    if not isinstance(text, str) or not text:
        return text
    kept: list = []
    cursor = 0
    for match in _STAGE_POINTER_RE.finditer(text):
    ...
    joined = "".join(kept)
    joined = _BACKFILL_ADJACENT_RE.sub(r"\1", joined)
    return " ".join(joined.split(" ")).strip()


async def _stage_backfill_repair(question: str, response, citations: list, started: float):
    n_cites = len(citations or ())
    text = getattr(response, "text", None)
    note = getattr(response, "note", None)
    output = getattr(response, "output", None)
    new_text = _stage_pointer_fix(text, n_cites) if isinstance(text, str) else None
    new_note = _stage_pointer_fix(note, n_cites) if isinstance(note, str) else None
    if new_text == text and new_note == note:
        return None
    if isinstance(text, str) and new_text:
        return _h666_rebuild(response, new_text, None, new_note, list(citations or ()))
    if output is not None:
        return _h666_rebuild(response, None, output, new_note, list(citations or ()))
    return None


# --- stage: candidate-roster pre-pass -------------------------------------
# A set or superlative question is lost the moment the pool is wrong, and the
# ledger audit only ever sees what the draft already says - it cannot miss a
# member it was never shown. This enumerates the pool independently and folds
# every member the draft does not mention into the open claims, so the existing
# retrieval cycle goes looking for them.

_ROSTER_MIN_LEFT_S = 26.0
_ROSTER_MAX_ITEMS = 12
_ROSTER_MAX_MISSING = 4
_ROSTER_SYSTEM = (
    "You enumerate the candidate pool for a research question. Do not answer the "
    "question and do not rank anything. List, one per line as '- NAME', every "
    "entity that plausibly belongs in the pool the question draws from. If the "
    "question is not about a set or a superlative, output the single line NONE."
)
_ROSTER_GATE_RE = _h666_re.compile(
    r"\b(list|name|enumerate|all\s+of|every|which\s+of|how\s+many|top\s+\d+|"
    r"most|least|largest|smallest|highest|lowest|biggest|greatest|fewest|"
    r"leading|best|worst|rank(?:ed|ing)?)\b",
    _h666_re.IGNORECASE,
)
_ROSTER_LINE_RE = _h666_re.compile(r"^\s*[-*\u2022]\s*(.+?)\s*$", _h666_re.M)


async def _stage_roster(question: str, blob: str, started: float) -> tuple:
    try:
        if _ROSTER_GATE_RE.search(question or "") is None:
            return [], []
        left = _stage_left(started)
        if left < _ROSTER_MIN_LEFT_S:
            return [], []
        text = await _h666_chat(
            _ROSTER_SYSTEM, (question or "")[:1500], max_tokens=400,
            timeout=min(10.0, left - 4.0),
        )
        if not text or text.strip().upper().startswith("NONE"):
            return [], []
        names: list = []
        seen = set()
        for match in _ROSTER_LINE_RE.finditer(text):
            name = match.group(1).strip(" .;:")
            if len(name) < 2 or len(name) > 90 or name.lower() in seen:
                continue
            seen.add(name.lower())
            names.append(name)
            if len(names) >= _ROSTER_MAX_ITEMS:
                break
        lowered = (blob or "").lower()
        missing = [n for n in names if n.lower() not in lowered][:_ROSTER_MAX_MISSING]
        if not missing:
            return [], []
        claims = [f"pool member the draft never addresses: {n}" for n in missing]
        head = _stage_head(question)
        queries = [f"{head} {n}" for n in missing[:2]]
        return claims, queries
    except Exception:
        return [], []

# --- added stage budgets --------------------------------------------------
# Every added stage is opportunistic: it runs only while the task budget
# can still pay for it, and returns the untouched answer when it cannot.
_STAGE_MIN_LEFT_S = 17.0
_STAGE_CHAT_TIMEOUT_S = 12.0
_STAGE_MAX_QUERIES = 4
_ROSTER_MIN_LEFT_S = 26.0
...
async def query(query: Query) -> Response:
    ...
    try:
        ...
        # --- ADDED STAGE (prepass): candidate-roster pre-pass ---
        stage_claims, stage_queries = await _stage_roster(
            question, blob, started)
        _stage_fold(ledger, stage_claims, stage_queries)
        ...
        if ledger.requires_fresh_retrieval_and_rewrite():
            ...
            if rows:
                ...
                if rewritten is not None:
                    final = rewritten
        if final is None:
            final = _h666_pointer_only(draft)
        # --- ADDED STAGE (repair): pointer-integrity backfill ---
        backfill_fixed = await _stage_backfill_repair(question, final, citations, started)
        if backfill_fixed is not None:
            final = backfill_fixed
