...
def _compose_lumen_anvil_agent_entry():
    ...
    # ---- v250-6-rsu :: _compose_lumen_anvil_agent_entry ----
    # Stages: roster pre-pass, set gap-fill, unit repair
    # Ordinary successful path:
    #   query -> _balanced_route_label -> LumenAnvil / CedarQuill branch -> _solve -> _knowledge_brief -> _draft_candidate_pool -> _loop -> _audit_patch -> _widen_pool -> _conform_measures -> _citations_for -> _finalize_branch_response -> _sanitize_outer_citations -> Response
...
def _compose_cedar_quill_agent_entry():
    ...
    # ---- v250-6-rsu :: _compose_cedar_quill_agent_entry ----
    # Stages: roster pre-pass, set gap-fill, unit repair
    # Ordinary successful path:
    #   query -> _balanced_route_label -> LumenAnvil / CedarQuill branch -> _solve -> _knowledge_brief -> _draft_candidate_pool -> _loop -> _audit_patch -> _widen_pool -> _conform_measures -> _citations_for -> _finalize_branch_response -> _sanitize_outer_citations -> Response
