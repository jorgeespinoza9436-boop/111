...
_rpnhhavkep = {'started': None, 'text': None}
_ahqfaypkst = 24000
_kelvyhrznq = 290.0
_dvgjhlcwhv = 250.0

def _reryatcseq() -> float:
    started = _rpnhhavkep['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _zigaijwoow() -> float:
    return _kelvyhrznq - _reryatcseq()
_tywbxvfnxq = _hsapi.llm_chat
_zlwdvqocmr = _hsapi.search_web
_pxftnrwbcg = _hsapi.fetch_page
_bhfcvszvfv = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _tncadexlga(*args, **kwargs):
    if _reryatcseq() >= _dvgjhlcwhv:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _bhfcvszvfv})
            kwargs['messages'] = steered
    _result = await _tywbxvfnxq(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _uhilveuftt(_result)
    return _result

async def _lzzjcufonu(*args, **kwargs):
    if _reryatcseq() >= _dvgjhlcwhv:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _zlwdvqocmr(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _xwvrckryaa(*args, **kwargs):
    if _reryatcseq() >= _dvgjhlcwhv:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _pxftnrwbcg(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _tncadexlga
_hsapi.search_web = _lzzjcufonu
_hsapi.fetch_page = _xwvrckryaa
_izexzstycy = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_dsziieqtqs = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_csrwlhdamy = 900
_rjxiemejry = 2

def _ulktjtviry(query: Query) -> int:
...
def _hjxepltruq(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _lgsnoccvws(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _ulktjtviry(query)
    if fields >= 3:
        return 2
    if _hjxepltruq(lowered, _izexzstycy):
        return 1
    if fields <= _rjxiemejry and len(text) <= _csrwlhdamy:
        return 0
    if _hjxepltruq(lowered, _dsziieqtqs):
        return 0
    return 1

def _uhilveuftt(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _rpnhhavkep['text'] = text.strip()[:_ahqfaypkst]
    except Exception:
        pass

def _sqhkrskyie(text: str):
...
def _dresencimh(query: Query) -> Response:
    text = _rpnhhavkep['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_ahqfaypkst]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _sqhkrskyie(text)
...
def _xgzxdxtpdi():
    _S555S37_QUERY_TAG = 's555s37-hk6719'
...
"""
            ...
            _rpnhhavkep: dict[str, Any] = {'models': (), 'budget_left': None, 'models_by_provider': {}}

            def _remember_budget(payload: Any) -> None:
                budget = getattr(payload, 'budget', None)
                value = getattr(budget, 'session_remaining_budget_usd', None)
                if isinstance(value, (int, float)):
                    _rpnhhavkep['budget_left'] = float(value)
            ...
            async def _load_models() -> None:
                try:
                    ...
                    if by_provider:
                        _rpnhhavkep['models_by_provider'] = by_provider
                        _rpnhhavkep['models'] = by_provider.get(LLM_PROVIDER) or next(iter(by_provider.values()))
                except Exception:
                    return

            def _model_order(preferred: tuple[str, ...], provider: str=LLM_PROVIDER) -> list[str]:
                by_provider = _rpnhhavkep.get('models_by_provider')
                live = by_provider.get(provider) if isinstance(by_provider, dict) else None
                if not live and provider == LLM_PROVIDER:
                    live = _rpnhhavkep.get('models')
...
def _bfrafvhjjc():
    ...
    async def query(query: _s37_Query) -> _s37_Response:
        ...
        try:
            ...
            return draft
    return query
_fdsbadgxzg = _fspoxntmky()
_ifllnwwnai = _xgzxdxtpdi()
_hfyizvdvta = _bfrafvhjjc()
_kelvyhrznq = 290.0
_cdumeftupg = 250.0
_oajmhbfflc = 90.0

async def _nevbxowqtv(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _kelvyhrznq - (time.monotonic() - started)
        if first:
            budget = _cdumeftupg if _cdumeftupg < remaining else remaining
            first = False
        else:
            if remaining < _oajmhbfflc:
                break
        ...
        try:
            ...
            last_exc = exc
    return _dresencimh(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _rpnhhavkep['started'] = time.monotonic()
    try:
        index = _lgsnoccvws(query)
        if index == 0:
            agents = (_fdsbadgxzg, _ifllnwwnai, _hfyizvdvta)
        elif index == 1:
            agents = (_ifllnwwnai, _hfyizvdvta, _fdsbadgxzg)
        elif index == 2:
            agents = (_hfyizvdvta, _fdsbadgxzg, _ifllnwwnai)
        else:
            agents = (_fdsbadgxzg, _ifllnwwnai, _hfyizvdvta)
        return await _nevbxowqtv(query, agents)
    except Exception:
        return _dresencimh(query)