...
def _compose_vesper_meadow_agent_entry():
    ...
    async def _plain_query(query: Query, budget: float) -> Response:
        ...
        try:
            ...
            # --- ADDED STAGES: unit-consistency repair then premise-verification sweep ---
...
_vesper_meadow_agent_query_entry = _compose_vesper_meadow_agent_entry()


def _compose_saffron_talon_agent_entry():
    'uid_195 research agent - variant 20.\n\nTwo stages added to the uid_195 pipeline, one taken from each of two\nvalidator_reference candidates:\n  uid  89 - citation slice backfill (backfill)\n  uid 212 - lead-figure corroboration (corroborate)\n\nThe primary controller, the evidence state (_ResultIndex) and the amend\nstage that decides the delivered answer are unchanged. All model calls go\nthrough LLM_PROVIDER = "openrouter".\n'
    ...
    async def _plain_query(query: Query, budget: float) -> Response:
        ...
        try:
            ...
            # --- ADDED STAGES: lead-figure corroboration then citation slice backfill ---
            # These run on the answer the commit stage produced and before the
            # amend stage, so what amend re-projects is already repaired.
            stage_input = display
            stage_deadline = deadline - STAGE_TAIL_RESERVE_SECONDS
...
_saffron_talon_agent_query_entry = _compose_saffron_talon_agent_entry()


def _compose_quartz_lantern_agent_entry():
...
_quartz_lantern_agent_query_entry = _compose_quartz_lantern_agent_entry()
...
class VesperMeadowAgent:
    async def __call__(self, query: Query) -> Response:
        return await _vesper_meadow_agent_query_entry(query)


class SaffronTalonAgent:
    async def __call__(self, query: Query) -> Response:
        return await _saffron_talon_agent_query_entry(query)


class QuartzLanternAgent:
    async def __call__(self, query: Query) -> Response:
        return await _quartz_lantern_agent_query_entry(query)


_TASK_SHAPE_PRIMARY_AGENT = VesperMeadowAgent()
_TASK_SHAPE_SECONDARY_AGENT = SaffronTalonAgent()
_TASK_SHAPE_TERTIARY_AGENT = QuartzLanternAgent()
