...
def _compose_lumen_beacon_agent_entry():
    _S111U_QUERY_TAG = "s111u-hk6734"  # per-hotkey canonical uniqueness
...
def _compose_raven_beacon_agent_entry():
...
_raven_beacon_agent_query_entry = _compose_raven_beacon_agent_entry()


def _compose_cedar_lattice_agent_entry():
    ...
    _S444S666_QUERY_TAG = "s444s666-hk6724"  # per-hotkey canonical uniqueness
...
_cedar_lattice_agent_query_entry = _compose_cedar_lattice_agent_entry()


_SHAPE_ROUTER_SEED = "e0dd82d8dec6a4a890a73ace"
...
def _balanced_route_label(query: Query) -> str:
    ...
    order = ("LumenBeaconAgent", "RavenBeaconAgent", "CedarLatticeAgent")
    if shape == 3:
        return order[bucket]
...
class RavenBeaconAgent:
    async def __call__(self, query: Query) -> Response:
        return await _raven_beacon_agent_query_entry(query)


class CedarLatticeAgent:
    async def __call__(self, query: Query) -> Response:
        return await _cedar_lattice_agent_query_entry(query)


_SHAPE_PRIMARY_AGENT = LumenBeaconAgent()
_SHAPE_SECONDARY_AGENT = RavenBeaconAgent()
_SHAPE_TERTIARY_AGENT = CedarLatticeAgent()
