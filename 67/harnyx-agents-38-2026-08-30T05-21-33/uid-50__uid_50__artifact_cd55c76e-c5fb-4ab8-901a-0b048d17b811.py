...
# ---- v240-4-phzu ----
# Added: premise traceability audit, temporal alignment audit, corroboration audit, unit conformance audit
# Ordinary successful path:
#   query -> _plain_query -> briefing -> research loop -> _relocate -> checkpoint -> _commit_call -> forced retry -> _audit_premise -> _audit_timeframe -> _audit_corroboration -> _audit_measure -> _amended_answer -> _deliverable -> Response
...
def _audit_premise(question: str, display: str, index: "_ResultIndex") -> str:
    ...
    if not shown:
        return ""
    gaps = [name for name in _aud_entities(question or "")
            if name.casefold() not in shown]
    if not gaps:
        return ""
    return ("\n\nTRACEABILITY: your cited passages state nothing about "
            + "; ".join(gaps[:4])
            + ". Each is named by the question and the judge expects it "
            "traceable. Cite a numbered passage that states it, or say plainly "
