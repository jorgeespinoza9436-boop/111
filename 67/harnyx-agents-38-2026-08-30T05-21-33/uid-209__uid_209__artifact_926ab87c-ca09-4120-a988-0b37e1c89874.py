...
def _compose_quartz_kestrel_slot01_agent_entry():
...
_quartz_kestrel_slot01_agent_query_entry = _compose_quartz_kestrel_slot01_agent_entry()


def _compose_ember_talon_slot01_agent_entry():
...
_ember_talon_slot01_agent_query_entry = _compose_ember_talon_slot01_agent_entry()


def _compose_aster_relay_slot01_agent_entry():
...
_aster_relay_slot01_agent_query_entry = _compose_aster_relay_slot01_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "QuartzKestrelSlot01Agent"
    ...
    if (
        ...
        return "EmberTalonSlot01Agent"
    return "AsterRelaySlot01Agent"


class QuartzKestrelSlot01Agent:
    async def __call__(self, query: Query) -> Response:
        return await _quartz_kestrel_slot01_agent_query_entry(query)


class EmberTalonSlot01Agent:
    async def __call__(self, query: Query) -> Response:
        return await _ember_talon_slot01_agent_query_entry(query)


class AsterRelaySlot01Agent:
    async def __call__(self, query: Query) -> Response:
        return await _aster_relay_slot01_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = QuartzKestrelSlot01Agent()
_ANALYTICAL_FIELD_AGENT = EmberTalonSlot01Agent()
_BROAD_FIELD_AGENT = AsterRelaySlot01Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "QuartzKestrelSlot01Agent",
    "EmberTalonSlot01Agent",
    "AsterRelaySlot01Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "QuartzKestrelSlot01Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "EmberTalonSlot01Agent":
