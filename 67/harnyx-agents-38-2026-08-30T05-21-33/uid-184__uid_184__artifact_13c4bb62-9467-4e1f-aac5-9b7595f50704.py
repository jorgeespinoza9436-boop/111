...
def _compose_garnet_wren_slot13_agent_entry():
    ...
    # slot: 11 C38_note 2026-08-26T15:01:12+00:00

    return query

_garnet_wren_slot13_agent_query_entry = _compose_garnet_wren_slot13_agent_entry()


def _compose_aster_marten_slot13_agent_entry():
...
_aster_marten_slot13_agent_query_entry = _compose_aster_marten_slot13_agent_entry()


def _compose_aster_compass_slot13_agent_entry():
...
_aster_compass_slot13_agent_query_entry = _compose_aster_compass_slot13_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "GarnetWrenSlot13Agent"
    ...
    if (
        ...
        return "AsterMartenSlot13Agent"
    return "AsterCompassSlot13Agent"


class GarnetWrenSlot13Agent:
    async def __call__(self, query: Query) -> Response:
        return await _garnet_wren_slot13_agent_query_entry(query)


class AsterMartenSlot13Agent:
    async def __call__(self, query: Query) -> Response:
        return await _aster_marten_slot13_agent_query_entry(query)


class AsterCompassSlot13Agent:
    async def __call__(self, query: Query) -> Response:
        return await _aster_compass_slot13_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = GarnetWrenSlot13Agent()
_ANALYTICAL_FIELD_AGENT = AsterMartenSlot13Agent()
_BROAD_FIELD_AGENT = AsterCompassSlot13Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "GarnetWrenSlot13Agent",
    "AsterMartenSlot13Agent",
    "AsterCompassSlot13Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "GarnetWrenSlot13Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "AsterMartenSlot13Agent":
