...
def _compose_quartz_slate_agent_entry():
    ...
    # ── V05  TAP COVERAGE SWEEP ───────────────────────────────────────────────
    # Controller: one engine run, then a completeness audit against the tap. The
    #             engine often FETCHES material it never uses; this recovers it
    #             with zero extra retrieval.
    # Evidence:   required element x tap page coverage grid (_CxCoverageGrid).
    # Authority:  elements the tap already supports but the answer omitted are
    #             written in with citations minted from the tap.

    _V05_MECH_S = 34.0
    _V05_MAX_ELEMENTS = 7

    _V05_PLAN_SYSTEM = (
        "You list what a complete answer to a research question must contain. "
        'Return JSON only: {"elements": ["<one checkable element>", ...]}. Each '
        "names one thing - a quantity, an entity, a date range, an enumeration, "
        "a comparison side. At most seven. Never guess the answer itself."
    )

    _V05_FILL_SYSTEM = (
        "You extend a research draft using evidence it already retrieved but did "
        "not use. The DRAFT is authoritative for everything it states: never "
        "drop, reword or renumber a figure, name, date or [[n]] pointer. Add one "
        "or two sentences per RECOVERED ELEMENT, each carrying the pointer given "
        "in its evidence block. Return the full extended answer only."
    )
    ...
    async def _v05_run(query):
        ...
        plan = asyncio.ensure_future(_cx_chat(
            _V05_PLAN_SYSTEM, "QUESTION:\n" + question[:4000],
            _CX_FAST, 16.0, 600, 0.0))
...
_quartz_slate_agent_query_entry = _compose_quartz_slate_agent_entry()


def _compose_orchid_mistral_agent_entry():
...
_orchid_mistral_agent_query_entry = _compose_orchid_mistral_agent_entry()


def _compose_beryl_harbor_agent_entry():
    ...
    async def _plain_query(query: Query, budget: float) -> Response:
        ...
        try:
            ...
            # --- ADDED STAGES: claim-level verify-and-repair then lead-figure corroboration ---
            # These run on the answer the commit stage produced and before the
            # amend stage, so what amend re-projects is already repaired.
            stage_input = display
            stage_deadline = deadline - STAGE_TAIL_RESERVE_SECONDS
...
_beryl_harbor_agent_query_entry = _compose_beryl_harbor_agent_entry()
...
class QuartzSlateAgent:
    async def __call__(self, query: Query) -> Response:
        return await _quartz_slate_agent_query_entry(query)


class OrchidMistralAgent:
    async def __call__(self, query: Query) -> Response:
        return await _orchid_mistral_agent_query_entry(query)


class BerylHarborAgent:
    async def __call__(self, query: Query) -> Response:
        return await _beryl_harbor_agent_query_entry(query)


_TASK_SHAPE_PRIMARY_AGENT = QuartzSlateAgent()
_TASK_SHAPE_SECONDARY_AGENT = OrchidMistralAgent()
_TASK_SHAPE_TERTIARY_AGENT = BerylHarborAgent()
