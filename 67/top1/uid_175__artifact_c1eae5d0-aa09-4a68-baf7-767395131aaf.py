"""SN67 Harnyx miner -- v55_dethrone: v54 + hardened named-source extraction (anti-garbage-dump guard, exact full-name precision, mandatory-fetch/full-table/compute-enforced directive) to dominate the 58% extraction class and dethrone the champion. Single-model glm-5.2, structured output-only+coerce, citation gate+floor, budget force-commit."""
...
async def _do_search_many(queries, index):
    ...
    parts = await asyncio.gather(*(_do_search(q, index) for q in clean))
...
_SET_DIRECTIVE = (
    ...
    "per key EXCLUDED candidate with the reason it fails. Enumerate them ALL. Keep each line short."
)
_NUMERIC_DIRECTIVE = (
...
def _named_source(q):
    return bool(_NAMED_SOURCE_RE.search(q or ""))


_EXTRACTION_DIRECTIVE = (
    "\n\nNAMED-SOURCE EXTRACTION -- this question demands EXACT data from a specific named source/table/dataset. "
    "You MUST fetch_page that source's ACTUAL page or JSON API and read the WHOLE table/fact-sheet -- do NOT answer "
    "from search_web snippets or memory. Steps: (1) find the exact source URL (search its name, or a known API like "
    "Wikipedia REST/action); (2) fetch it and read every relevant row/figure VERBATIM; (3) if the needed rows are "
    "deep in a big table, fetch again targeting them; (4) apply each filter/ranking to the EXTRACTED rows and use the "
    "compute tool for any top-N / comparison / arithmetic; (5) cite the fetched source [n] for every value. NEVER "
    "output raw 'search findings', a list of result titles, or a partial sentence as the answer -- only the "
    "extracted, computed result.\n"
...
async def _run_tool(c, index, question=""):
    ...
    if c.name == "search_many":
        qs = args.get("queries") or []
        return await _do_search_many(qs if isinstance(qs, list) else [qs], index)
    if c.name == "search_ai":
...
async def query(query: Query) -> Response:
    ...
    extract = _named_source(q)                                     # NAMED-SOURCE EXTRACTION (field's #1 weakness)
    _EXTRACT_MODE["on"] = extract                                  # -> _do_fetch shows a 9000-char window (full table)
    hard = classify_hard(q, cls) or extract                       # extraction always gets the full machinery
    ...
    try:
        for turn in range(1, max_turns + 1):
            ...
            if calls:
                ...
                for c, tr in zip(calls, outs):
                    ...
                    if c.name in ("search_web", "search_many", "search_ai", "fetch_page") and "ERROR" not in tr:
                        search_fetch_used += 1
        ...
        # --- HARD-only: best-of-N self-consistency over shared evidence ---
        if hard and index.top() > 0 and final and not _invalid_final(final) \
        ...
        # HARD-only uncorrelated audit (reasoning-on for gpt-oss); prose only
        if hard and deadline - perf_counter() > AUDIT_MIN_REMAINING and _spend_left() >= MIN_AUDIT_USD:
