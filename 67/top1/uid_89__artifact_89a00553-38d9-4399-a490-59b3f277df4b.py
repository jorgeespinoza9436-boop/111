...
"""
# MECHANISM_UPGRADE (vs 111/67/12): search_many batch retrieval tool + post-draft coverage verify-patch.
...
TOOLS = [
    {
        ...
        "function": {
            "name": "search_many",
            "description": (
                "Run several web searches (up to 8) and return all numbered results. "
                "Use to enumerate or verify a whole candidate set in one step."
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
async def _run_search_many(queries, index: _ResultIndex, ledger: _ClaimLedger) -> str:
    """MECHANISM: batch retrieval tool — one call enumerates up to 8 queries (ledger-ordered)."""
    clean = [str(q).strip() for q in (queries or []) if str(q).strip()][:8]
    if not clean:
        return "# search_many() -> ERROR: no queries"
    parts = []
    for q in clean:
        parts.append(await _run_search_web(q, index, ledger))
...
async def _dispatch_tool_call(
    ...
    if name == "search_many":
        ...
        return await _run_search_many(qs if isinstance(qs, list) else [qs], index, ledger)
    if name == "fetch_page":
...
async def _pairwise_verify_patch(question: str, answer: str, messages: list, deadline: float, *, model: str, provider: str = "openrouter") -> str:
    """MECHANISM: post-draft coverage/citation audit → tools-off rewrite."""
    ...
    try:
        audit = await llm_chat(
            ...
            tools=None, temperature=0.1, max_output_tokens=700,
            thinking=LlmThinkingConfig(enabled=False),
            timeout=min(28.0, max(5.0, deadline - perf_counter() - 10)),
        )
    ...
    if not issues or (deadline - perf_counter()) < 25:
        return answer
    gap = "## Audit Gaps\n\n" + "\n".join(f"- {x}" for x in issues[:6]) + "\n\nRewrite COMPLETE final answer with inline [n] including exclusions."
    try:
        rewrite = await llm_chat(
            provider=provider, model=model,
            messages=list(messages) + [{"role": "assistant", "content": answer}, {"role": "user", "content": gap}],
            tools=None, temperature=0.2,
            thinking=LlmThinkingConfig(enabled=False),
            timeout=min(35.0, max(5.0, deadline - perf_counter() - 5)),
        )
    ...
    try:
        ...
        if final_answer and (deadline - perf_counter()) > 45:
            final_answer = await _pairwise_verify_patch(
                query.text or "", final_answer, messages, deadline, model=MODEL, provider=LLM_PROVIDER,
            )
