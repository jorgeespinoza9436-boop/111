...
v33.3 SINGLE PROVIDER — the legacy second provider is gone; openrouter is the only one
...
v34.0 STRUCTURAL HARDENING — behaviour-preserving. Every prompt, model, budget,
rescue rung and answer-floor rule is byte-identical to v33.3; what changed is
the load-bearing structure around them.

  A. THE WALL IS NOW INVIOLABLE. Each network await is
     `wait_for(call(timeout=budget), timeout=budget + GRACE)`, and that GRACE
     used to sit OUTSIDE the deadline arithmetic: _clamp_timeout measured its
     `reserve` against the INNER timeout, so the OUTER wait could still land
     past the wall — measured +1.0s on search/fetch/turn, +4.0s on the brief,
     and +2.0s each on _knowledge_resort and _schema_output, which never
     clamped at all. Those two run LAST, with nothing behind them to absorb the
     overshoot, and the errors compound down the rescue tail. _clamp_timeout
     now takes `grace` and subtracts it, so `now + budget + grace <= deadline -
     reserve` holds at EVERY call site, and the two hand-rolled rungs go
     through the same helper as everything else.

  B. THE AUDIT STAGE NO LONGER BURNS TIME IT CANNOT USE. The probe and the
     patch had two independent gates: _solve entered at >75s left, _audit_patch
     refused to patch under 70s. Entering anywhere in 75-103s therefore ALWAYS
     bought the probe (8-23s plus grace) and then never ran the patch — pure
     cost in precisely the squeezed regime where the rescue ladder needs those
     seconds. One gate now (AUDIT_ENTRY_S), and the probe is clamped with the
     patch window held in reserve, so the 70s check is a post-condition rather
     than a coin toss. Whenever the audit could do work before, it still does.

  C. TRANSCRIPT INTEGRITY. Every tool_call in an assistant message must receive
     a matching tool reply or the next request is rejected outright — and
     _chat_turn would then burn the entire model chain on a transcript we
     malformed ourselves. Replies were written only `if call_id`, so one call
     the SDK reported without an id silently poisoned the rest of the run. Ids
     are resolved BEFORE the assistant message is appended; if any is missing
     the message is never appended and a bounded repair turn runs instead.

  D. TASK HYGIENE. The tool fan-out's `except Exception` could not catch
     CancelledError (a BaseException), so a cancelled turn left the fan-out
     running against a wall we had already conceded. Cancellation moved to a
     `finally`, and a cancelled task is no longer asked for its result.

  E. ONE LLM PAYLOAD READER. The raw_text -> choices[0].message.content walk
     existed in three copies with three slightly different guards, giving any
     payload-shape change three chances to bite. _llm_text / _llm_message are
     now the only readers.

STRUCTURAL CONTRACT (the server-side AST policy). This file is written to make
each rejection class impossible by construction, not merely absent today:
  * forbidden_import      — imports are asyncio, json, re, time.monotonic and
                            the SDK. No sys, os, subprocess, importlib, socket.
  * dunder_attribute      — no `__name__`/`__class__`/`__dict__` anywhere; the
                            only dunder token in the file is the
                            `from __future__ import annotations` statement,
                            which is an import, not an attribute read.
  * unsupported_callable  — no lambdas, no nested defs, no callable stored in a
                            variable, dict or list, no `key=` function argument,
                            no `sorted(..., key=...)`, no map/filter/partial.
                            Every call site names its target statically; tool
                            dispatch in _run_tool is an if-chain over string
                            literals, and every rescue rung calls a module-level
                            function by name.
  * dynamic_getattr_name  — every getattr passes a STRING LITERAL as the field
                            name. No setattr/hasattr/delattr/eval/exec/compile/
                            globals/locals/vars/__import__ anywhere.
  * provider              — openrouter only. No second provider is imported, named
                            or reachable; redundancy comes from the model-family
                            chain, not from a second provider.
"""
...
# ── SDK grace (v34) ───────────────────────────────────────────────────────────
# Every network await is `wait_for(call(timeout=budget), timeout=budget+GRACE)`.
# Until v34 that GRACE sat OUTSIDE the deadline arithmetic, so _clamp_timeout's
# `reserve` was measured against the INNER timeout and the outer wait could still
# land past the wall: measured +1.0s on search/fetch/turn and +4.0s on the brief,
# plus +2.0s each on the two hand-rolled rungs that never clamped at all. They
# compound down the rescue tail. _clamp_timeout now subtracts the grace too, so
# `now + budget + grace <= deadline - reserve` holds at EVERY call site.
SDK_GRACE_S = 6.0        # llm_chat wrappers
NET_GRACE_S = 4.0        # search_web / fetch_page wrappers
...
AUDIT_EXTRA_TURNS = 2
# v34 AUDIT GATE ARITHMETIC. The probe and the patch had two INDEPENDENT time
# gates: _solve entered at >75s left, then _audit_patch refused to patch under
# 70s. Between them sat a guaranteed-waste band — entering at 75-103s always
# bought the probe (8-23s + grace) and then never ran the patch, so the stage
# was pure cost in exactly the squeezed regime where the rescue ladder needs
# that time. One gate now: enter only when the probe AND the patch both fit.
AUDIT_PATCH_MIN_S = 70.0     # room a patch loop needs to search + rewrite
AUDIT_ENTRY_S = AUDIT_PATCH_MIN_S + AUDIT_TIMEOUT_S + 8.0   # = 106.0
...
def _clamp_timeout(deadline: float, want: float, reserve: float = 4.0,
                   floor: float = 4.0, grace: float = 0.0) -> float:
    """Largest INNER timeout whose OUTER wait still lands `reserve` seconds
    before `deadline`.
    ...
    v34: `grace` is what the caller adds to its asyncio.wait_for bound. Charging
    it HERE is what makes the wall inviolable — the caller returns by
    `deadline - reserve`, not by `deadline - reserve + grace`."""
    room = deadline - monotonic() - reserve - grace
    if room < floor:
        return 0.0
    if want < room:
        return want
...
async def _do_search_many(queries, deadline: float):
    ...
    tasks = [asyncio.ensure_future(_do_search(q, deadline)) for q in clean]
    try:
...
async def _run_tool(call, question: str, deadline: float):
    ...
    if name == "web_search":
        ...
        if name == "search_many":
            ...
            return await _do_search_many(qs if isinstance(qs, list) else [qs], deadline)
    if name == "read_page":
...
def _llm_message(payload):
    """The first choice's message object, or None. v34: this walk lived inline
    in three places (_chat_simple, _loop, _digest_write_once) with three slightly
    different guards, so a payload-shape change had three chances to bite. One
    reader, string-literal field names only, no reflection."""
...
async def _chat_simple(model: str, system: str, user: str, *,
    ...
    payload = await asyncio.wait_for(
        ...
        timeout=timeout + SDK_GRACE_S)
    _spend_note(payload)
    return _llm_text(payload)
        ...
        try:
            payload = await asyncio.wait_for(
                llm_chat(
                    ...
                    # exactly one model, zai/glm-5.2-fast on the legacy provider, which had a
...
async def _knowledge_brief(question: str, deadline: float) -> tuple[str, str]:
    ...
    for model in LOOP_MODEL_CHAIN:
        budget = _clamp_timeout(min(deadline, phase_end), BRIEF_TIMEOUT_S,
                                2.0, floor=12.0, grace=SDK_GRACE_S)
        if budget <= 0.0:
            break
        try:
...
async def _loop(question: str, brief: str, ledger: list,
    ...
    for turn in range(1, turn_cap + 1):
        ...
        # v34 TRANSCRIPT INTEGRITY. Every tool_call in an assistant message must
        # receive a matching tool reply or the NEXT request is rejected outright,
        # and _chat_turn then burns the whole model chain on a transcript we
        # ourselves malformed. The reply was previously written only `if call_id`,
        # so a call the SDK reported without an id silently poisoned the turn.
        # Resolve every id FIRST; if any is missing, never append the assistant
        # message at all — spend a repair turn instead and keep the transcript
        # valid. (Answers already earned are untouched either way.)
        call_ids = []
        for c in calls:
            call_ids.append(str(getattr(c, "id", "") or ""))
        if not call_ids or not all(call_ids):
            if repairs_left > 0 and _time_left(deadline) > MIN_TAIL_S + 10.0:
                repairs_left -= 1
                messages.append({"role": "system", "content": _REPAIR_ORDER})
                answer = ""
        ...
        tool_tasks = [asyncio.ensure_future(_run_tool(c, question, deadline))
                      for c in run_calls]
        # v34: try/finally. asyncio.wait can raise CancelledError, which is a
        # BaseException and so walked straight past the old `except Exception` —
        # leaving the fan-out running against a wall we had already conceded.
        # Cancellation is now unconditional on every exit path.
        try:
        ...
        for idx, result in enumerate(results):
            ...
            messages.append({"role": "tool", "tool_call_id": call_ids[idx],
                             "content": body})
        # ids were all validated above, so the stubbed tail cannot be skipped
        for idx in range(len(run_calls), len(call_ids)):
            messages.append({"role": "tool", "tool_call_id": call_ids[idx],
...
async def _audit_patch(question: str, answer: str, messages: list[dict],
    ...
    # v34: clamped like every other call instead of a hand-rolled max/min. The
    # old expression had an 8.0 FLOOR, so with little time left it requested more
    # than remained and the outer wait_for could land past the wall. The clamp
    # reserves the patch window and the SDK grace, and returns 0.0 -> skip.
    budget = _clamp_timeout(deadline, AUDIT_TIMEOUT_S,
                            reserve=AUDIT_PATCH_MIN_S, floor=8.0,
                            grace=SDK_GRACE_S)
    if budget <= 0.0:
        return answer
    try:
    ...
    # audit is a pure cost with no possible effect. v34: the clamp above already
    # reserved AUDIT_PATCH_MIN_S, so this is now a post-condition, not a coin toss.
    if not gaps or _time_left(deadline) < AUDIT_PATCH_MIN_S:
        return answer
...
async def _knowledge_resort(question: str, deadline: float) -> str:
    # v34: was `min(45.0, left - 4.0)` against a wait_for of `timeout + 6.0`,
    # i.e. a structural 2s overrun of the wall on the last rung before we ship.
    budget = _clamp_timeout(deadline, 45.0, reserve=2.0, floor=8.0,
                            grace=SDK_GRACE_S)
    if budget <= 0.0:
        return ""
    try:
...
async def _schema_output(question: str, answer: str, schema, deadline: float) -> object | None:
    ...
    for model in (SCHEMA_MODEL, RESORT_MODEL, LOOP_MODEL_A):
        # v34: clamped. The old bound overran the wall by 2s per rung, and this
        # runs LAST — after it there is nothing left to absorb the overshoot.
        budget = _clamp_timeout(deadline, 45.0, reserve=2.0, floor=8.0,
                                grace=SDK_GRACE_S)
        if budget <= 0.0:
            break
...
async def _solve(query: Query, question: str) -> Response:
    ...
    try:
        if _is_usable_answer(answer) and _time_left(deadline) > AUDIT_ENTRY_S \
...
_PERFECT_SUFFIX = "786245f071b8a8fb"
