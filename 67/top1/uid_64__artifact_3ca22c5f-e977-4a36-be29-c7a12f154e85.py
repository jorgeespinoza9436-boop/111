...
# Hard ceiling on cumulative prompt tokens. The soft deadline notice is advisory
# -- the model may ignore it -- and `for _turn in range(160)` is effectively
# unbounded, which is how single runs reached 669,200 prompt tokens (5.5x the
# median) at $0.44 and 250s+. Past ~150k tokens the measured score is LOWER, so
# forcing finalisation there costs nothing and removes the cost/runtime tail
# that is the only thing keeping this agent off the champion slot.
PROMPT_TOKEN_CEILING = 150_000
# MEASURED on batch c4c8bef0 (150 runs): prompt tokens correlate +0.952 with
# cost and +0.831 with runtime, but -0.098 with SCORE. Bucketed, the 50k-100k
# prompt band scores HIGHEST (0.779) while the 250k+ band scores 0.706 at four
# times the cost. The 63 runs above 150k tokens averaged 0.619 at $0.173/169s
# against 0.730 at $0.056/81s for the rest -- heavier context bought a worse
# answer, more slowly, for triple the money.
#
# uid 210 already beats the champion on score (+0.067) and is blocked purely by
# cost (+26.2%) and runtime (+3.1%). These four budgets are the resident context
# that rides in every one of ~12 turns per run, so each char is paid ~12 times.
# Halving them targets the waste and leaves the research behaviour intact.
BATCHED_RETRIEVAL_PREVIEW_CHARS = 96_000
VFS_READ_PAGE_CHARS = 40_000
FOCUSED_OBSERVATION_MEMORY_CHARS = 32_000
VFS_SEARCH_PAGE_CHARS = 30_000
VFS_SIMILARITY_MIN_CHUNKS = 3
VFS_SIMILARITY_MAX_CHUNKS = 5
VFS_SIMILARITY_RESULT_CHARS = 24_000
...
# SINGLE PROVIDER: openrouter only. ai_gateway is gone, and with it
# thinkingmachines/inkling, which has no openrouter equivalent.
#
# Measured on batch c4c8bef0 (150 runs), this is also a cost win -- ai_gateway was
# 47% of LLM spend ($6.28 of $13.29) and the SAME model is cheaper on openrouter:
#     openrouter google/gemma-4-31b-it   $0.35/Mtok
#     ai_gateway google/gemma-4-31b-it   $0.74/Mtok   (2.1x)
#     ai_gateway thinkingmachines/inkling $1.49/Mtok  (4.3x, no openrouter route)
# ai_gateway_gemma maps 1:1 onto openrouter_gemma (identical model). inkling has
# no equivalent, so its slots fall to the next rung already in each ladder.
#
# Ladder depth is preserved by keeping all three openrouter gemma routes, which
# pin DIFFERENT upstreams (sambanova / modelrun) with allow_fallbacks=False, plus
# glm5 on a third (amazon-bedrock). A single upstream outage therefore still has
# somewhere to fall through to.
MODEL_SCHEDULING = "state_aware"
INVESTIGATION_MODELS = ("glm5", "openrouter_gemma", "openrouter_gemma_stable")
STATE_AWARE_INVESTIGATION_MODELS = (
    "openrouter_gemma",
    "openrouter_gemma_stable",
    "glm5",
    "openrouter_gemma_prose",
)
...
def _record_retrieval_receipt(
    ...
    if not output.get("ok") or name not in {"search_web", "search_many", "fetch_page"}:
        return
    if name == "search_web":
...
def _capture_budget(state: ResearchState, result: Any) -> None:
    state.prompt_tokens_used += _prompt_tokens_of(result)
...
def _prompt_tokens_of(result: object) -> int:
    """Prompt tokens billed for one chat result, 0 when unavailable.

    Read defensively: the accessor differs across SDK payload shapes and a
    missing field must never break the research loop.
    """
    llm = getattr(result, "llm", None)
    for holder in (getattr(llm, "usage", None),
                   getattr(result, "usage", None),
                   getattr(getattr(result, "response", None), "usage", None),
                   llm):
        if holder is None:
            continue
        value = getattr(holder, "prompt_tokens", None)
        if value is None and isinstance(holder, dict):
            value = holder.get("prompt_tokens")
...
async def _call_model(
    ...
    # Legacy aliases: any caller still naming an ai_gateway model is routed to the
    # openrouter equivalent rather than raising. Silently switching provider would
    # be wrong; here the model is identical (gemma-4-31b-it) or the closest
    # available substitute (inkling -> glm5, both reasoning-capable).
    if model_name in ("ai_gateway_gemma", "inkling"):
        return await _call_model(
            "openrouter_gemma" if model_name == "ai_gateway_gemma" else "glm5",
            messages,
            tools,
            tool_choice,
            parallel_tool_calls,
...
async def _final_answer_chat_with_retry(
    ...
    return await _chat_with_scheduling(
        ("openrouter_gemma_prose", "openrouter_gemma_stable", "openrouter_gemma", "glm5"),
        messages,
        None,
        "none",
        False,
...
async def _execute_search_many(state, args):
    """MECHANISM: parallel batch VFS search."""
    qs = args.get("queries") or []
    clean = [str(q).strip() for q in (qs if isinstance(qs, list) else [qs]) if str(q).strip()][:8]
    if not clean:
        return {"ok": False, "error": "no queries"}
    parts = await asyncio.gather(*(_execute_search(state, {"query": q, "num": 8}) for q in clean))
    ok_parts = [p for p in parts if isinstance(p, dict) and p.get("ok")]
    return ok_parts[-1] if ok_parts else {"ok": False, "error": "all failed"}
...
async def _execute_tool(
    ...
    if name in {"search_web", "search_many", "fetch_page"}:
...
async def _investigate(question: str, expected_answer: str) -> tuple[str, list[CitationRef]]:
    ...
    for _turn in range(160):
        if not deadline_notice_sent and state.prompt_tokens_used >= PROMPT_TOKEN_CEILING:
            messages.append(
                {
                    "role": "user",
                    "content": (
                        "The context budget for this task is spent. Do not search, fetch or read "
                        "any further. Retain the excerpts you already need and call "
                        "ready_to_finalize now, answering from the evidence already gathered."
                    ),
                }
            )
        ...
        retrieval_call_count = sum(call.name in {"search_web", "search_many", "fetch_page"} for call in calls)
