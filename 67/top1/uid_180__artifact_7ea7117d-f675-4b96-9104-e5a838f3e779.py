...
class HardPath:
    ...
    Champion lineages retry a failed search this way instead of giving up."""
        ...
        async def _do_search_many(queries):
            """MECHANISM: parallel/batch retrieval on HardPath."""
            clean=[str(q).strip() for q in (queries or []) if str(q).strip()][:8]
            if not clean: return '# search_many() -> ERROR: no queries'
            parts=[]
            for q in clean:
...
def _contradiction_flags(answer: str) -> list[str]:
    """MECHANISM: localized contradiction detector before audit/commit."""
    flags=[]
    low=(answer or "").lower()
...
async def _gap_research_many(queries, search_coro):
    """MECHANISM: claim-gap parallel re-search before final commit (concrete retrieval change)."""
    clean=[str(q).strip() for q in (queries or []) if str(q).strip()][:8]
    if not clean:
        return []
    return list(await asyncio.gather(*[search_coro(q) for q in clean], return_exceptions=True))


@entrypoint("query")
async def query(query: Query) -> Response:
    _ = _gap_research_many  # mechanism exported for auditor/repair lanes
    try:
