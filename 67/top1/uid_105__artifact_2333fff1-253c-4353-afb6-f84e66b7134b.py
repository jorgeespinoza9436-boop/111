...
LOOP_TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "search_many",
            "description": "Run up to 8 web searches in parallel for candidate/metric panels.",
            "parameters": {
                "type": "object",
                "properties": {
                    "queries": {"type": "array", "items": {"type": "string"}, "description": "up to 8 queries"}
                },
                "required": ["queries"],
            },
        },
...
async def _claim_gap_reresearch(question: str, answer: str, messages: list[dict], ledger, deadline: float) -> str:
    ...
    try:
        chat = await llm_chat(
            ...
            messages=[
                {"role": "system", "content": "Claim-gap planner. JSON only {\"gap_queries\":[...max 4...]}"},
                {"role": "user", "content": f"Q:\n{question}\n\nDraft:\n{answer[:10000]}"},
            ],
            tools=None, temperature=0.1, max_output_tokens=500,
            timeout=min(25.0, rem - 20),
        )
        cleaned = re.sub(r"^```(?:json)?\s*|\s*```$", "", (chat.response.raw_text or "").strip(), flags=re.I | re.M)
    ...
    try:
        rewrite = await llm_chat(
            ...
            tools=None, temperature=0.2, timeout=min(40.0, max(10.0, deadline - _pc() - 5)),
        )
