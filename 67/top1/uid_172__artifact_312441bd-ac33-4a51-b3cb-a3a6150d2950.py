...
# - search_many parallel VFS retrieval
#
...
TOOLS = [
    ...
    _schema(
        "search_many",
        "Run up to 8 web searches in parallel; each result is retained in VFS with a source reference.",
        {
...
async def _execute_search_many(state: ResearchState, args: dict[str, Any]) -> dict[str, Any]:
    """MECHANISM: parallelize search_many (was sequential) — retrieval control-flow change."""
    ...
    # Prefer last ok result structure; concatenate notes in state via each _execute_search side effect
    ok_parts = [p for p in parts if isinstance(p, dict) and p.get("ok")]
    return ok_parts[-1] if ok_parts else (parts[-1] if parts else {"ok": False, "error": "all failed"})
...
async def _claim_gap_after_answer(question: str, answer: str, deadline: float) -> str:
    """Forced claim-gap searches then tools-off rewrite for VFS agent."""
    ...
    try:
        plan = await llm_chat(
            ...
            model="openai/gpt-oss-120b",
            messages=[
                {"role": "system", "content": "JSON only {\"gap_queries\":[...max 3...]}"},
                {"role": "user", "content": f"Q:\n{question}\n\nDraft:\n{answer[:9000]}"},
            ],
            tools=None,
            temperature=0.1,
            max_output_tokens=400,
            timeout=min(20.0, deadline - perf_counter() - 25),
        )
        cleaned = re.sub(r"^```(?:json)?\s*|\s*```$", "", (plan.response.raw_text or "").strip(), flags=re.I | re.M)
    ...
    try:
        rewrite = await llm_chat(
            ...
            model="openai/gpt-oss-120b",
            messages=[
                {"role": "system", "content": RESEARCH_PAIRWISE_RULES + "\nRewrite complete cited final answer. Cover every subclaim. Cite exclusions."},
                {"role": "user", "content": f"Q:\n{question}\n\nDraft:\n{answer[:8000]}\n\nNew evidence:\n" + "\n".join(notes)},
            ],
            tools=None,
            temperature=0.2,
            timeout=min(35.0, deadline - perf_counter() - 5),
        )
