...
v36 FUSION PATCHES:
  ...
  6. Proof-of-completeness format rule — final answers must use the locked
     ...
     PASS/FAIL lines.
"""
# MECHANISM_UPGRADE (vs 111/67/12): contradiction pre-check mechanism before outbound answer.
...
def _contradiction_flags(answer: str) -> list[str]:
    """MECHANISM: localized contradiction detector (yes/no conflicts in FINAL ANSWER)."""
    flags: list[str] = []
    low = (answer or "").lower()
...
async def query(query: Query) -> Response:
    ...
    _cf = _contradiction_flags(str(answer or ""))
    if _cf:
        # keep going — flags inform residual doubt only
        pass
