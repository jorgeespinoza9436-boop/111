...
def _compose_lumen_relay_agent_entry():
...
_lumen_relay_agent_query_entry = _compose_lumen_relay_agent_entry()
...
def _compose_juniper_river_agent_entry():
    ...
    # ---- v260-3-rksz ----
    # Stages: roster pre-pass, coverage nudge, set gap-fill, corroborate
    # Ordinary successful path:
    #   query -> _solve -> _knowledge_brief -> _draft_candidate_pool -> _loop (+_open_criteria_hint) -> _audit_patch -> _widen_pool -> _second_source_check -> _citations_for -> _answer_line_only -> Response
...
class LumenRelayAgent:
    async def __call__(self, query: Query) -> Response:
        return await _lumen_relay_agent_query_entry(query)
...
_TASK_SHAPE_PRIMARY_AGENT = VesperKestrelAgent()
_TASK_SHAPE_SECONDARY_AGENT = LumenRelayAgent()
_TASK_SHAPE_TERTIARY_AGENT = JuniperRiverAgent()
...
@entrypoint("query")
async def query(query: Query) -> Response:
    route_index = _task_shape_route_index(query) if "Candidate c9 preserves its tested route because the previous batch ended before individual scoring." else 0
    if route_index == 0:
