...
def _compose_cobalt_cairn_agent_entry():
    ...
    # ── cx: independent-source claim corroboration ────────────────────────────────
    # New evidence-state mechanism absent from the base agent: decisive load-bearing
    # claims (a cited sentence carrying a number, year, or superlative) are checked
    # against the evidence ledger for a second, independently-domained source. A
    # claim resting on a single source triggers exactly one targeted search aimed
    # at that claim; unresolved gaps conditionally re-enter the research loop with
    # a reconciliation order before the answer is finalized. Bounded to at most
    # _CX_MAX_CLAIMS claims and one search each, fails open, and only replaces the
    # draft when the revision passes the same non-destructive accept-gate used by
    # the other repair paths, so it can never make the answer worse.
    _CX_MAX_CLAIMS = 2
    _CX_VALUE_RE = re.compile(r"\d[\d,]*(?:\.\d+)?%?")
    _CX_DOMAIN_RE = re.compile(r"^(?:https?://)?(?:www\.)?([^/]+)", re.IGNORECASE)
    ...
    async def _cx_corroborate(question: str, answer: str, messages: list[dict],
                              ledger: EvidenceLedger, deadline: float) -> str:
        try:
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            if _is_usable_answer(answer) and (deadline - monotonic()) > 90.0 \
                    and _spend_left() >= AUDIT_MIN_USD:
    ...
    def _gx_defects(question: str, answer: str) -> list:
        ...
        if missing_ent:
            notes.append("The question names entities the answer never mentions: "
                         + ", ".join(missing_ent))
        missing_units = _gx_missing_units(question, answer)
        if missing_units:
            notes.append("The question demands units the answer never renders: "
                         + ", ".join(missing_units))
        if _gx_has_superlative(question) and not _gx_comparison_shown(answer):
            notes.append("The question asks for a superlative/most-common answer but the "
                         "answer shows no comparison against runner-up candidates.")
        uncited = _gx_uncited_claims(answer)
        if uncited:
            notes.append("These factual sentences carry no [n] citation; attach the "
                         "marker for the evidence they came from: " + " | ".join(uncited[:2]))
...
_cobalt_cairn_agent_query_entry = _compose_cobalt_cairn_agent_entry()


def _compose_cedar_vector_agent_entry():
...
_cedar_vector_agent_query_entry = _compose_cedar_vector_agent_entry()


def _compose_vesper_slate_agent_entry():
...
_vesper_slate_agent_query_entry = _compose_vesper_slate_agent_entry()
...
class CobaltCairnAgent:
    async def __call__(self, query: Query) -> Response:
        return await _cobalt_cairn_agent_query_entry(query)


class CedarVectorAgent:
    async def __call__(self, query: Query) -> Response:
        return await _cedar_vector_agent_query_entry(query)


class VesperSlateAgent:
    async def __call__(self, query: Query) -> Response:
        return await _vesper_slate_agent_query_entry(query)


_TASK_SHAPE_PRIMARY_AGENT = CobaltCairnAgent()
_TASK_SHAPE_SECONDARY_AGENT = CedarVectorAgent()
_TASK_SHAPE_TERTIARY_AGENT = VesperSlateAgent()
...
@entrypoint("query")
async def query(query: Query) -> Response:
    route_index = _task_shape_route_index(query) if "Candidate c7 preserves its tested route because the previous batch ended before individual scoring." else 0
    if route_index == 0:
