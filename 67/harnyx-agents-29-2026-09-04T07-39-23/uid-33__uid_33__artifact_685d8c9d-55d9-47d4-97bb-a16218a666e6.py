...
def _compose_lumen_ledger_slot06_agent_entry():
...
_lumen_ledger_slot06_agent_query_entry = _compose_lumen_ledger_slot06_agent_entry()


def _compose_vesper_willow_slot06_agent_entry():
...
_vesper_willow_slot06_agent_query_entry = _compose_vesper_willow_slot06_agent_entry()


def _compose_marble_notary_slot06_agent_entry():
...
_marble_notary_slot06_agent_query_entry = _compose_marble_notary_slot06_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "LumenLedgerSlot06Agent"
    ...
    if (
        ...
        return "VesperWillowSlot06Agent"
    return "MarbleNotarySlot06Agent"


class LumenLedgerSlot06Agent:
    async def __call__(self, query: Query) -> Response:
        return await _lumen_ledger_slot06_agent_query_entry(query)


class VesperWillowSlot06Agent:
    async def __call__(self, query: Query) -> Response:
        return await _vesper_willow_slot06_agent_query_entry(query)


class MarbleNotarySlot06Agent:
    async def __call__(self, query: Query) -> Response:
        return await _marble_notary_slot06_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = LumenLedgerSlot06Agent()
_ANALYTICAL_FIELD_AGENT = VesperWillowSlot06Agent()
_BROAD_FIELD_AGENT = MarbleNotarySlot06Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "LumenLedgerSlot06Agent",
    "VesperWillowSlot06Agent",
    "MarbleNotarySlot06Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "LumenLedgerSlot06Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "VesperWillowSlot06Agent":
