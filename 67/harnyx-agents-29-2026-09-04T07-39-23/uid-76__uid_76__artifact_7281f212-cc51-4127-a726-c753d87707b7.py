...
def _compose_lumen_kestrel_slot15_agent_entry():
...
_lumen_kestrel_slot15_agent_query_entry = _compose_lumen_kestrel_slot15_agent_entry()


def _compose_umber_vector_slot15_agent_entry():
...
_umber_vector_slot15_agent_query_entry = _compose_umber_vector_slot15_agent_entry()


def _compose_quartz_quill_slot15_agent_entry():
...
_quartz_quill_slot15_agent_query_entry = _compose_quartz_quill_slot15_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "LumenKestrelSlot15Agent"
    ...
    if (
        ...
        return "UmberVectorSlot15Agent"
    return "QuartzQuillSlot15Agent"


class LumenKestrelSlot15Agent:
    async def __call__(self, query: Query) -> Response:
        return await _lumen_kestrel_slot15_agent_query_entry(query)


class UmberVectorSlot15Agent:
    async def __call__(self, query: Query) -> Response:
        return await _umber_vector_slot15_agent_query_entry(query)


class QuartzQuillSlot15Agent:
    async def __call__(self, query: Query) -> Response:
        return await _quartz_quill_slot15_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = LumenKestrelSlot15Agent()
_ANALYTICAL_FIELD_AGENT = UmberVectorSlot15Agent()
_BROAD_FIELD_AGENT = QuartzQuillSlot15Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "LumenKestrelSlot15Agent",
    "UmberVectorSlot15Agent",
    "QuartzQuillSlot15Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "LumenKestrelSlot15Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "UmberVectorSlot15Agent":
