...
def _schema_skeleton(schema, depth: int = 0):
    """A minimal value that satisfies a schema's STRUCTURAL constraints.

    Last-resort only. MEASURED: the previous terminal fallback returned
    `Response(output=<bare string>)`, and the host requires, for a structured query,
    that `output` be present AND validate against the schema
    (`miner_response_hydration._validate_answer_for_query`). A bare string can never
    satisfy an object schema, so that path was a guaranteed `miner_response_invalid`
    -- a hard zero rather than a wrong-but-scored answer. Seen once in a local run
    that exhausted its wall (0 of 190 production rows, so this is insurance, not a
    live loss). Honours enum, required, type, and string minLength/maxLength.
    """
        ...
        if isinstance(branch, list):
            for sub in branch:
                if isinstance(sub, dict) and sub.get("type") != "null":
                    return _schema_skeleton(sub, depth + 1)
    kind = _schema_kind(schema)
    if kind == "object":
        out: dict = {}
        ...
        for name in required:
            if not isinstance(name, str):
                continue
            out[name] = _schema_skeleton(props.get(name) or {}, depth + 1)
        return out
    if kind == "array":
        least = schema.get("minItems")
        least = least if isinstance(least, int) and least > 0 else 0
        item = schema.get("items") or {}
        return [_schema_skeleton(item, depth + 1) for _ in range(least)]
    if kind == "integer":
        return 0
    if kind == "number":
        return 0
    ...
    if isinstance(low, int) and low > len(text):
        text = text + ("0" * (low - len(text)))
...
async def _k2_base_query(query: Query) -> Response:
    ...
    try:
        ...
        if schema is not None:
            ...
            try:
                return Response(output=_schema_skeleton(schema))
            except Exception:
                pass
...
async def _k2_cycle(query: object, response: object) -> object:
    """Audit the draft; on a research deficiency re-enter retrieval, then regenerate.

    The baseline stage researches and drafts. This stage holds authority over what
    the entrypoint finally returns: every ordinary successful request reaches the
    loop below, and the answer is not settled until the loop exits.

    The loop is the cross-stage edge. `_k2_audit_ledger` classifies each researched
    fact the question requires, and `_k2_deficient` selects the ones the sources do
    not settle -- `missing`, `contradicted`, or `unreconciled`. That verdict reads
    the evidence itself, not the run's clock or budget, and it is what sends control
    back to retrieval (`_k2_targeted_research`) and then to answer regeneration
    (`_k2_regenerate`) before the answer is returned. A ledger with no unsettled
    fact exits the loop and keeps the draft, because further retrieval cannot change
    facts the sources already settle.
    """
    question = str(getattr(query, "text", "") or "").strip()
    if not question:
        return response
    current = response
    for _ in range(_K2_MAX_CYCLES):
        draft = _k2_draft_view(current)
        if not draft:
            break
        ...
        new_text, new_note = await _k2_regenerate(
            question, current, deficient, digest, new_citations
        )
        merged = _k2_merge_citations(getattr(current, "citations", None), new_citations)
        if new_text is None and new_note is None:
            if merged is None:
                break
            current = _k2_rebuild(current, None, None, merged)
            continue
        current = _k2_rebuild(current, new_text, new_note, merged)
    return current
