...
criteria: list | None = None) -> tuple[str, list[dict]]:
    ...
    # ---- v250-1-rkp :: _compose_lumen_anvil_agent_entry ----
    # Stages: roster pre-pass, coverage nudge, premise sweep
    # Ordinary successful path:
    #   query -> _balanced_route_label -> LumenAnvil / CedarQuill branch -> _solve -> _knowledge_brief -> _draft_candidate_pool -> _loop (+_open_criteria_hint) -> _audit_patch -> _verify_subjects -> _citations_for -> _finalize_branch_response -> _sanitize_outer_citations -> Response
...
criteria: list | None = None) -> tuple[str, list[dict]]:
    ...
    # ---- v250-1-rkp :: _compose_cedar_quill_agent_entry ----
    # Stages: roster pre-pass, coverage nudge, premise sweep
    # Ordinary successful path:
    #   query -> _balanced_route_label -> LumenAnvil / CedarQuill branch -> _solve -> _knowledge_brief -> _draft_candidate_pool -> _loop (+_open_criteria_hint) -> _audit_patch -> _verify_subjects -> _citations_for -> _finalize_branch_response -> _sanitize_outer_citations -> Response
