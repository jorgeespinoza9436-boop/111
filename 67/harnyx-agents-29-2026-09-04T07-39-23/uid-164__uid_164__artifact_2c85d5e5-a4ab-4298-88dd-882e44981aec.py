...
def _compose_sable_lantern_agent_entry():
...
_SABLE_LANTERN = _compose_sable_lantern_agent_entry()

import re as _route_re

# Contracts with many required fields go to the grounding branch, whose w4
# stage repairs structured output against an answer contract; small contracts
# go to the branch whose output is checked against the official validator.
# Table sweeps take the turbo-lane branch; general research takes grounding.
...
def _route_label(query: Query) -> str:
    ...
    if schema is not None:
        ...
        if isinstance(required, list) and len(required) >= 4:
            return "harbor"
        return "quartz"
...
_ROUTES = {
    "quartz": (_QUARTZ_CURRENT, _HARBOR_BEACON),
    "harbor": (_HARBOR_BEACON, _QUARTZ_CURRENT),
    "sable": (_SABLE_LANTERN, _HARBOR_BEACON),
}
