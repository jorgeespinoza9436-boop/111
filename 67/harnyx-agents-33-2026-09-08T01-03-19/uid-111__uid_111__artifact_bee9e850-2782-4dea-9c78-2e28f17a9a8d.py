...
_cekslymtga = {'started': None, 'text': None}
_dgwnfifuvv = 24000
_nwoxeycejr = 290.0
_kenlqfiwvk = 250.0

def _onihehyndy() -> float:
    started = _cekslymtga['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _jrerhbpnea() -> float:
    return _nwoxeycejr - _onihehyndy()
_hkhhzdjceq = _hsapi.llm_chat
_cgoymwlgly = _hsapi.search_web
_lhhzqzsnld = _hsapi.fetch_page
_voiaylsipm = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _omiimyknpa(*args, **kwargs):
    if _onihehyndy() >= _kenlqfiwvk:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _voiaylsipm})
            kwargs['messages'] = steered
    _result = await _hkhhzdjceq(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _dfremfzhgz(_result)
    return _result

async def _rzowtnsajq(*args, **kwargs):
    if _onihehyndy() >= _kenlqfiwvk:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _cgoymwlgly(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _jktfraewsy(*args, **kwargs):
    if _onihehyndy() >= _kenlqfiwvk:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _lhhzqzsnld(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _omiimyknpa
_hsapi.search_web = _rzowtnsajq
_hsapi.fetch_page = _jktfraewsy
_duvxrzsgiz = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_ccohokxlrz = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_skryrrzgcl = 900
_nicynjlgyo = 2

def _icgelkrdlp(query: Query) -> int:
...
def _nxslhowstx(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _veybnyygjy(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _icgelkrdlp(query)
    if fields >= 3:
        return 2
    if _nxslhowstx(lowered, _duvxrzsgiz):
        return 1
    if fields <= _nicynjlgyo and len(text) <= _skryrrzgcl:
        return 0
    if _nxslhowstx(lowered, _ccohokxlrz):
        return 0
    return 1

def _dfremfzhgz(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _cekslymtga['text'] = text.strip()[:_dgwnfifuvv]
    except Exception:
        pass

def _ksfvjstoni(text: str):
...
def _jkcgjaxwdx(query: Query) -> Response:
    text = _cekslymtga['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_dgwnfifuvv]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _ksfvjstoni(text)
...
"""
    ...
    return query
_jphgmtzcjr = _kirrdxofjk()
_jzwfubcqgp = _fduwrwfodk()
_adrrrsixhj = _cxptarirdb()
_iutoacfcgu = _manhrabnkr()
_nwoxeycejr = 290.0
_lcyfucurrr = 250.0
_glbxnubvsp = 90.0

async def _zkmzwevnsd(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _nwoxeycejr - (time.monotonic() - started)
        if first:
            budget = _lcyfucurrr if _lcyfucurrr < remaining else remaining
            first = False
        else:
            if remaining < _glbxnubvsp:
                break
        ...
        try:
            ...
            last_exc = exc
    return _jkcgjaxwdx(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _cekslymtga['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _zkmzwevnsd(query, (_adrrrsixhj, _jphgmtzcjr, _jzwfubcqgp, _iutoacfcgu))
        index = _veybnyygjy(query)
        if index == 0:
            agents = (_jphgmtzcjr, _jzwfubcqgp, _adrrrsixhj, _iutoacfcgu)
        elif index == 1:
            agents = (_jzwfubcqgp, _adrrrsixhj, _iutoacfcgu, _jphgmtzcjr)
        elif index == 2:
            agents = (_adrrrsixhj, _iutoacfcgu, _jphgmtzcjr, _jzwfubcqgp)
        elif index == 3:
            agents = (_iutoacfcgu, _jphgmtzcjr, _jzwfubcqgp, _adrrrsixhj)
        else:
            agents = (_jphgmtzcjr, _jzwfubcqgp, _adrrrsixhj, _iutoacfcgu)
        return await _zkmzwevnsd(query, agents)
    except Exception:
        return _jkcgjaxwdx(query)
_BUILDT_TAG_m34 = "sn45-801bc4d11843"
