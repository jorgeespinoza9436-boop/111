...
def _compose_ivory_lantern_agent_entry():
    ...
    async def _schema_output(question: str, answer: str, schema, deadline: float) -> object | None:
        ...
        for lane, model, cap in ((LLM_LANE_A, SCHEMA_MODEL, 20.0),
            ...
            try:
                ...
                continue
        return _SN67_NULL_CONVERSION_FAILURE
            ...
            if structured is not _SN67_NULL_CONVERSION_FAILURE:
                ...
                for candidate in (structured, original_structured):
                    ...
                    try:
                        ...
                        continue
                structured = _SN67_NULL_CONVERSION_FAILURE  # fall through to the deterministic shape
    ...
    def _w4_is_degenerate_output(output: object, schema: object) -> bool:
        ...
        if output is None:
            return not _sn67_null_is_valid(schema)
...
_ivory_lantern_agent_query_entry = _compose_ivory_lantern_agent_entry()
...
def _compose_quartz_meadow_agent_entry():
    ...
    # fork of 8_uid_64_score_0.582.py: added k2 (variant u64r)
    ...
    # ---------------------------------------------------------------------------
    # K2 claim-ledger cycle  [variant u64r-k2]
    #
    # Ordinary successful path after the baseline draft:
    #   draft -> claim-ledger audit -> (if required researched facts are missing,
    #   contradicted, or unreconciled) targeted fresh search -> regenerate draft
...
class IvoryLanternAgent:
    async def __call__(self, query: Query) -> Response:
        return await _ivory_lantern_agent_query_entry(query)
...
_TASK_SHAPE_PRIMARY_AGENT = VesperSlateAgent()
_TASK_SHAPE_SECONDARY_AGENT = IvoryLanternAgent()
_TASK_SHAPE_TERTIARY_AGENT = QuartzMeadowAgent()
