...
def _compose_ivory_talon_slot03_agent_entry():
    ...
    def _gx_missing_currency(question: str, answer: str) -> list:
        ...
        for word, symbol in (("dollars", "$"), ("euros", "\u20ac"), ("pounds sterling", "\u00a3")):
...
_ivory_talon_slot03_agent_query_entry = _compose_ivory_talon_slot03_agent_entry()


def _compose_saffron_mosaic_slot03_agent_entry():
...
_saffron_mosaic_slot03_agent_query_entry = _compose_saffron_mosaic_slot03_agent_entry()


def _compose_cedar_wren_slot03_agent_entry():
...
_cedar_wren_slot03_agent_query_entry = _compose_cedar_wren_slot03_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "IvoryTalonSlot03Agent"
    ...
    if (
        ...
        return "SaffronMosaicSlot03Agent"
    return "CedarWrenSlot03Agent"


class IvoryTalonSlot03Agent:
    async def __call__(self, query: Query) -> Response:
        return await _ivory_talon_slot03_agent_query_entry(query)


class SaffronMosaicSlot03Agent:
    async def __call__(self, query: Query) -> Response:
        return await _saffron_mosaic_slot03_agent_query_entry(query)


class CedarWrenSlot03Agent:
    async def __call__(self, query: Query) -> Response:
        return await _cedar_wren_slot03_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = IvoryTalonSlot03Agent()
_ANALYTICAL_FIELD_AGENT = SaffronMosaicSlot03Agent()
_BROAD_FIELD_AGENT = CedarWrenSlot03Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "IvoryTalonSlot03Agent",
    "SaffronMosaicSlot03Agent",
    "CedarWrenSlot03Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "IvoryTalonSlot03Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "SaffronMosaicSlot03Agent":
