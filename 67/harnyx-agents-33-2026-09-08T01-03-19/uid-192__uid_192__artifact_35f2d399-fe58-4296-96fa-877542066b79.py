...
def _compose_ember_beacon_agent_entry():
    ...
    async def _corr_regenerate(question: str, response, weak: list[dict], evidence_rows: list[dict], offset: int):
        schema_mode = _sn67_response_has_output(response)
        lines = []
...
def _compose_quartz_kestrel_agent_entry():
...
_quartz_kestrel_agent_query_entry = _compose_quartz_kestrel_agent_entry()
...
class QuartzKestrelAgent:
    async def __call__(self, query: Query) -> Response:
        return await _quartz_kestrel_agent_query_entry(query)


_TASK_SHAPE_PRIMARY_AGENT = IvoryBeaconAgent()
_TASK_SHAPE_SECONDARY_AGENT = EmberBeaconAgent()
_TASK_SHAPE_TERTIARY_AGENT = QuartzKestrelAgent()
