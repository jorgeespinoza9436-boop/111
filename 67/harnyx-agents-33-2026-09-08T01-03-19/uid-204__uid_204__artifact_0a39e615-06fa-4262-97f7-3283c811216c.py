...
_wtjrzgjqit = {'started': None, 'text': None}
_uiuuvcazln = 24000
_xitrkftnqp = 290.0
_ribgflywlf = 250.0

def _ynsozpprey() -> float:
    started = _wtjrzgjqit['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _hgejvnofwu() -> float:
    return _xitrkftnqp - _ynsozpprey()
_jmkagiaudv = _hsapi.llm_chat
_ldgfjvoxog = _hsapi.search_web
_qvefkeqhyj = _hsapi.fetch_page
_pzkebhvjjo = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _ihxlcwinob(*args, **kwargs):
    if _ynsozpprey() >= _ribgflywlf:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _pzkebhvjjo})
            kwargs['messages'] = steered
    _result = await _jmkagiaudv(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _wtfzehvway(_result)
    return _result

async def _lvbxjpcars(*args, **kwargs):
    if _ynsozpprey() >= _ribgflywlf:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _ldgfjvoxog(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _tvebuvogqb(*args, **kwargs):
    if _ynsozpprey() >= _ribgflywlf:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _qvefkeqhyj(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _ihxlcwinob
_hsapi.search_web = _lvbxjpcars
_hsapi.fetch_page = _tvebuvogqb
_gwwcnhbfbi = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_drvhmiqxhc = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_vpecxgnvmf = 900
_bzisltvsfm = 2

def _oruuaazofn(query: Query) -> int:
...
def _mudmvfginp(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _yifnbsrceo(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _oruuaazofn(query)
    if fields >= 3:
        return 2
    if _mudmvfginp(lowered, _gwwcnhbfbi):
        return 1
    if fields <= _bzisltvsfm and len(text) <= _vpecxgnvmf:
        return 0
    if _mudmvfginp(lowered, _drvhmiqxhc):
        return 0
    return 1

def _wtfzehvway(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _wtjrzgjqit['text'] = text.strip()[:_uiuuvcazln]
    except Exception:
        pass

def _jsylxzewzj(text: str):
...
def _jtkecvzmic(query: Query) -> Response:
    text = _wtjrzgjqit['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_uiuuvcazln]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _jsylxzewzj(text)
...
def _lajsuwsymt():
    ...
    """
        ...
        return response
    return query
_nhtqunqzei = _fobfctytah()
_iaynazmqjd = _vrvjiryksu()
_rneblupirm = _mogjobihpv()
_qphrbfdlzy = _lajsuwsymt()
_xitrkftnqp = 290.0
_msicwgglhi = 250.0
_xwwpklxlwt = 90.0

async def _ichuckqrrb(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _xitrkftnqp - (time.monotonic() - started)
        if first:
            budget = _msicwgglhi if _msicwgglhi < remaining else remaining
            first = False
        else:
            if remaining < _xwwpklxlwt:
                break
...
def _a4s_factory():
    ...
    _PASSAGE_TOKEN_RE = re.compile("[a-z][a-z']{4,}|\\d[\\d/.,]*")
    ...
    def _citations_from_inline_markers(answer_text: str, index: _ResultIndex) -> tuple[tuple[CitationRef, ...], dict[int, int], dict[int, list[tuple[int, str, int, int]]]]:
    ...
    """
        ...
        for key in kept:
            ...
            meta = entry['meta']
            s_, e_ = entry['span']
            citations.append(CitationRef(receipt_id=meta['receipt_id'], result_id=meta['result_id'], slices=[CitationSlice(start=s_, end=e_)]))
        ...
        for n in ordered:
            ...
            for key in hits_for.get(n, {}):
                ...
                entry = regions[key]
                span = entry['span']
                regions_by_number.setdefault(n, []).append((position_of_key[key], entry['meta']['note'], span[0], span[1]))
    ...
    def _repoint_markers(text: str, position_of: dict[int, int], *, max_number: int, regions_by_number: dict[int, list[tuple[int, str, int, int]]] | None=None) -> str:
    ...
    def _deliverable(text: str | None, index: _ResultIndex, *, cite_text: str | None=None) -> Response:
        ...
        citations, position_of, regions_by_number = _citations_from_inline_markers(cite_text or answer, index)
        answer = _repoint_markers(answer, position_of, max_number=index.max_number(), regions_by_number=regions_by_number)
...
_a4s_slot = _a4s_factory()

@entrypoint('query')
async def query(query: Query) -> Response:
    _wtjrzgjqit['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _ichuckqrrb(query, (_rneblupirm, _nhtqunqzei, _iaynazmqjd, _qphrbfdlzy))
        index = _yifnbsrceo(query)
        if index == 0:
            agents = (_nhtqunqzei, _iaynazmqjd, _rneblupirm, _qphrbfdlzy)
        elif index == 1:
            agents = (_iaynazmqjd, _rneblupirm, _qphrbfdlzy, _nhtqunqzei)
        elif index == 2:
            agents = (_a4s_slot, _rneblupirm, _qphrbfdlzy, _nhtqunqzei, _iaynazmqjd)
        elif index == 3:
            agents = (_qphrbfdlzy, _nhtqunqzei, _iaynazmqjd, _rneblupirm)
        else:
            agents = (_nhtqunqzei, _iaynazmqjd, _rneblupirm, _qphrbfdlzy)
        return await _ichuckqrrb(query, agents)
    except Exception:
        return _jtkecvzmic(query)
_BUILDU_TAG_m22 = "sn45-5b6a6151e505"
