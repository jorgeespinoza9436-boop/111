...
# fork of 6_uid_19_score_0.675.py: added drv:current_historical:board_only (variant u19)
...
def _compose_lumen_anvil_agent_entry():
    ...
    #   query -> _balanced_route_label -> LumenAnvil / CedarQuill branch -> _solve -> _knowledge_brief -> _draft_candidate_pool -> _loop -> _audit_patch -> _temporal_repair -> _corroborate -> _refs_within_budget -> _citations_for -> _finalize_branch_response -> _sanitize_outer_citations -> Response
    ...
    TEMPORAL_MAX_ANCHORS = 2


    def _temporal_anchors(question: str) -> list:
        """Years the question anchors on, in order of appearance."""
        out = []
        for year in _YEAR_TOKEN_RE.findall(question or ""):
            if year not in out:
                out.append(year)
            if len(out) >= TEMPORAL_MAX_ANCHORS:
                break
        return out


    def _temporal_gap(question: str, answer: str, ledger: EvidenceLedger) -> str:
        """An anchor year the CITED evidence never mentions."""
        anchors = _temporal_anchors(question)
        if not anchors:
            return ""
        blob = _cited_blob(answer, ledger)
        if not blob:
            return ""
        for year in anchors:
            if year not in blob:
    ...
    async def _temporal_repair(question: str, answer: str, messages: list[dict],
                               ledger: EvidenceLedger, deadline: float) -> str:
        """Temporal alignment: the cited evidence is off the question's year.

        A correct-looking figure drawn from the wrong year is the failure this
        catches. Detection is on the CITED rows only -- an anchor sitting in an
        uncited row is not what the judge will read.
        """
        if not _stage_ready(deadline, STAGE_MIN_LEFT_S):
            return answer
        year = _temporal_gap(question, answer, ledger)
        if not year:
            return answer
        order = ("TEMPORAL ALIGNMENT. The question is anchored on " + year +
                 ", but no cited source carries that year. Find evidence for "
                 + year + " specifically. If only another period is available, "
                 "state the period the figure actually covers in the same "
                 "sentence as the figure. Never present a figure from one year "
                 "as if it were another. Rewrite the COMPLETE answer with [n] "
                 "citations.")
        return await _stage_rewrite(question, answer, messages, ledger, deadline,
                                    order, _probe_from(question, year, 140))

    CORROBORATE_MIN_LEFT_S = 80.0


    async def _corroborate(question: str, answer: str, messages: list[dict],
                           ledger: EvidenceLedger, deadline: float) -> str:
        """Lead-figure corroboration: the decisive number rests on one source.

        Fires on exactly one backer. Zero backers is a grounding failure and
        belongs to the value stage, not here; two or more is already
        corroborated and costs nothing.
        """
        if not _stage_ready(deadline, CORROBORATE_MIN_LEFT_S):
            return answer
        ...
        if _value_backers(lead, ledger) != 1:
            return answer
        order = ("CORROBORATION. The decisive figure " + lead + " rests on a "
                 "single source. Find an independent one. If the second source "
                 "agrees, cite both. If it disagrees, say so and give both "
                 "figures with their [n] citations rather than picking "
                 "silently. Rewrite the COMPLETE answer with [n] citations.")
        return await _stage_rewrite(question, answer, messages, ledger, deadline,
                                    order, _probe_from(question, lead, 130))

    # ---------------------------------------------------------------------
    # POST-AUDIT REPAIR CHAIN -- variant 137 (temporal repair + corroboration)
    #
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        if _is_usable_answer(answer):
            try:
                answer = await _temporal_repair(question, answer, messages,
                                                  ledger, deadline)
            except Exception:
                pass
            try:
                answer = await _corroborate(question, answer, messages,
                                              ledger, deadline)
            except Exception:
                pass
...
# --- drv wrap: current_historical dual-corpus reconciler (start) ---
# variant u19-drv-current_historical-board_only: current-period + historical-baseline corpus; trigger = board deficiency only (strict)
...
_DRV_TAG = 'u19-drv-current_historical-board_only'
_DRV_SALT = '2bfbdc583dba'
...
async def _drv_audit_board(question: str, draft: str, schema, citations: list) -> _DrvBoard:
    ...
    system = (
        "You audit a research draft against a user question. Focus first on period alignment: whether the draft mixes a current value with a baseline period, or lacks the period the question fixes. "
...
def _drv_current_year() -> str:
    import time as _drv_time
    return str(_drv_time.gmtime().tm_year)


def _drv_earliest_year(question: str) -> str:
    years = sorted(set(_drv_re.findall(r"\b(?:19|20)\d{2}\b", question or "")))
    return years[0] if years else ""


async def _drv_retrieve_dual_corpus(question: str, claims: list[str]) -> list[dict]:
    """current-period + historical-baseline corpus: two corpus-specific searches plus one fetch of the first corpus."""
    focus = "; ".join(claims[:3]) if claims else question[:180]
    first_q = " ".join((question[:120], focus[:140], ("latest " + _drv_current_year()))).strip()[:280]
    second_q = " ".join((question[:120], focus[:140], ("historical " + (_drv_earliest_year(question) or "baseline")))).strip()[:280]
    rows: list[dict] = []
    ...
    fetch_url = ""
    if first_row:
        first_row["corpus"] = "current_period"
        fetch_url = first_row.get("url") or ""
        rows.append(first_row)
    if second_row:
        second_row["corpus"] = "historical_baseline"
    ...
    if fetch_url:
        ...
        if fetched_row:
            fetched_row["corpus"] = "current_period_document"
...
async def _drv_regenerate(
    ...
    if is_text:
        system = (
            "Rewrite the research answer after a second retrieval pass over current-period and historical-baseline sources with the period stated beside every value. Return JSON only with keys "
        ...
        system = (
            "Rewrite the structured research answer after a second retrieval pass over current-period and historical-baseline sources with the period stated beside every value. Return JSON only "
...
async def query(query: _DrvQuery) -> _DrvResponse:
    ...
    try:
        ...
        return draft
# --- drv wrap: current_historical dual-corpus reconciler (end) ---
