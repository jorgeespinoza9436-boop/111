...
_nrmgygimhn = {'started': None, 'text': None}
_zoslpoteud = 24000
_txoohqwgpd = 290.0
_znmbihgrax = 250.0

def _figkovhxao() -> float:
    started = _nrmgygimhn['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _vcusmjegjp() -> float:
    return _txoohqwgpd - _figkovhxao()
_zyutfviafh = _hsapi.llm_chat
_cmznzktztr = _hsapi.search_web
_umdqkiqfpd = _hsapi.fetch_page
_dnexfobgru = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _ribjbtbtoo(*args, **kwargs):
    if _figkovhxao() >= _znmbihgrax:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _dnexfobgru})
            kwargs['messages'] = steered
    _result = await _zyutfviafh(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _sobdhodklp(_result)
    return _result

async def _cgjsosymrq(*args, **kwargs):
    if _figkovhxao() >= _znmbihgrax:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _cmznzktztr(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _bjtocqdzek(*args, **kwargs):
    if _figkovhxao() >= _znmbihgrax:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _umdqkiqfpd(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _ribjbtbtoo
_hsapi.search_web = _cgjsosymrq
_hsapi.fetch_page = _bjtocqdzek
_aijgoejtri = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_dtjrwklhup = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_bsrimnhwvp = 900
_zmvelhgnyq = 2

def _wzcxpwbrai(query: Query) -> int:
...
def _pwuqhtqqjb(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _noadussjtd(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _wzcxpwbrai(query)
    if fields >= 3:
        return 2
    if _pwuqhtqqjb(lowered, _aijgoejtri):
        return 1
    if fields <= _zmvelhgnyq and len(text) <= _bsrimnhwvp:
        return 0
    if _pwuqhtqqjb(lowered, _dtjrwklhup):
        return 0
    return 1

def _sobdhodklp(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _nrmgygimhn['text'] = text.strip()[:_zoslpoteud]
    except Exception:
        pass

def _pzgezhgkor(text: str):
...
def _jvozgyelbo(query: Query) -> Response:
    text = _nrmgygimhn['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_zoslpoteud]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _pzgezhgkor(text)
...
def _addfzibphm():
    ...
    """
        ...
        return response
    return query
_hwpbfkltrw = _qsfmbzvxcg()
_tvyrbxshjy = _dgttztscwp()
_vfobazouku = _kbgkoklirt()
_umbpqfmndw = _addfzibphm()
_txoohqwgpd = 290.0
_xjkmtvymrq = 250.0
_pznaqvrkei = 90.0

async def _haqzanwlre(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _txoohqwgpd - (time.monotonic() - started)
        if first:
            budget = _xjkmtvymrq if _xjkmtvymrq < remaining else remaining
            first = False
        else:
            if remaining < _pznaqvrkei:
                break
        ...
        try:
            ...
            last_exc = exc
    return _jvozgyelbo(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _nrmgygimhn['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _haqzanwlre(query, (_vfobazouku, _hwpbfkltrw, _tvyrbxshjy, _umbpqfmndw))
        index = _noadussjtd(query)
        if index == 0:
            agents = (_hwpbfkltrw, _tvyrbxshjy, _vfobazouku, _umbpqfmndw)
        elif index == 1:
            agents = (_tvyrbxshjy, _vfobazouku, _umbpqfmndw, _hwpbfkltrw)
        elif index == 2:
            agents = (_vfobazouku, _umbpqfmndw, _hwpbfkltrw, _tvyrbxshjy)
        elif index == 3:
            agents = (_umbpqfmndw, _hwpbfkltrw, _tvyrbxshjy, _vfobazouku)
        else:
            agents = (_hwpbfkltrw, _tvyrbxshjy, _vfobazouku, _umbpqfmndw)
        return await _haqzanwlre(query, agents)
    except Exception:
        return _jvozgyelbo(query)
_BUILDT_TAG_m22 = "sn45-a358e6f3688e"
