...
_izfewqmtjo = {'started': None, 'text': None}
_fldpeehuzj = 24000
_ldgmgiwxgw = 290.0
_zurpvimwzm = 250.0

def _hbscthzvim() -> float:
    started = _izfewqmtjo['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _ynwrniygrd() -> float:
    return _ldgmgiwxgw - _hbscthzvim()
_minwnoaobv = _hsapi.llm_chat
_ojvrkkxqjh = _hsapi.search_web
_ruajcuecpm = _hsapi.fetch_page
_tmjhfwgahz = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _xhuczbvipd(*args, **kwargs):
    if _hbscthzvim() >= _zurpvimwzm:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _tmjhfwgahz})
            kwargs['messages'] = steered
    _result = await _minwnoaobv(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _yyjkbmsopq(_result)
    return _result

async def _mpcevkxkif(*args, **kwargs):
    if _hbscthzvim() >= _zurpvimwzm:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _ojvrkkxqjh(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _mzdkpuotvg(*args, **kwargs):
    if _hbscthzvim() >= _zurpvimwzm:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _ruajcuecpm(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _xhuczbvipd
_hsapi.search_web = _mpcevkxkif
_hsapi.fetch_page = _mzdkpuotvg
_xmnpaftszw = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_qcmtjaeioa = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_athjjjhiys = 900
_doyfqgxukf = 2

def _hobssaufxp(query: Query) -> int:
...
def _wphdvtpcjh(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _kfmhwwjjfi(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _hobssaufxp(query)
    if fields >= 3:
        return 2
    if _wphdvtpcjh(lowered, _xmnpaftszw):
        return 1
    if fields <= _doyfqgxukf and len(text) <= _athjjjhiys:
        return 0
    if _wphdvtpcjh(lowered, _qcmtjaeioa):
        return 0
    return 1

def _yyjkbmsopq(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _izfewqmtjo['text'] = text.strip()[:_fldpeehuzj]
    except Exception:
        pass

def _xdwpflnoup(text: str):
...
def _gfquwymaxl(query: Query) -> Response:
    text = _izfewqmtjo['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_fldpeehuzj]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _xdwpflnoup(text)
...
def _jlazukcser():
    ...
    def _gx_missing_date_format(question: str, answer: str) -> list:
        if re.search('\\bYYYY-MM-DD\\b|\\bISO\\s*8601\\b', question, re.I):
            if re.search('\\b\\d{4}-\\d{2}-\\d{2}\\b', answer):
                return []
            return ['YYYY-MM-DD']
        if re.search('\\bDD Month YYYY\\b', question, re.I):
            if re.search('\\b\\d{1,2} [A-Z][a-z]+ \\d{4}\\b', answer):
                return []
            return ['DD Month YYYY']
        if re.search('\\bMonth DD, YYYY\\b', question, re.I):
            if re.search('\\b[A-Z][a-z]+ \\d{1,2}, \\d{4}\\b', answer):
                return []
    ...
    def _gx_defects(question: str, answer: str) -> list:
        ...
        df = _gx_missing_date_format(question, answer)
        if df:
            notes.append('The question demands dates in this exact format and the answer never uses it: ' + ', '.join(df))
    ...
    async def query(query: Query) -> Response:
        ...
        return response
    VERSION = 'c10-420'
    _GX_ACTIVE = ('super', 'entity', 'window', 'dateformat')
    return query
...
def _jikvzxxydl():
    ...
    PAGE_REREAD_TOTAL_CHARS = 132000
    ...
    def _page_spans(note: str, terms: list[str], windows: int=PAGE_WINDOWS_PER_PAGE) -> list[tuple[int, int]]:
    ...
    """
        ...
        if len(note) > head_end and windows > 0:
            spans.extend(_best_windows(note, terms, PAGE_WINDOW_CHARS, windows, skip_before=head_end))
        return spans
    ...
    _REREAD_EXHAUSTED = '# the allowance for re-reading already-retrieved results is spent for this run. Write the answer from what you have been shown.'
    ...
    """
        ...
        if resolved is None:
            return '# page_grep: no such result. Pass the number in brackets next to a result returned earlier in this run; nothing else can be read.'
        n, meta = resolved
        pat = (pattern or '').strip()
        if not pat:
        ...
        if scans >= PAGE_GREP_CALLS_PER_PAGE:
            return f'# page_grep([{n}]): this result has been scanned as often as it can be. Read a region of it, or work with what you already have.'
        ...
        if not offsets:
            return f'# page_grep({pat!r}) on [{n}]: no match in {len(note)} chars. Try a shorter string, a different spelling, or a different result.'
    ...
    """
        resolved = _held_result(number, index)
        if resolved is None:
            return '# page_read: no such result. Pass the number in brackets next to a result returned earlier in this run; nothing else can be read.'
        n, meta = resolved
        reads = int(meta.get('reads') or 0)
        if reads >= PAGE_READ_CALLS_PER_PAGE:
            return f'# page_read([{n}]): this result has been opened as often as it can be. Work with what you already have, or try a different result.'
        if index.reread_budget <= 0:
            return _REREAD_EXHAUSTED
        note = meta['note']
        try:
        ...
        if end <= start:
            return f"# page_read([{n}] @{start}): that offset is at or past the end of this result's {len(note)} chars."
        meta['reads'] = reads + 1
    ...
    async def _run_fetch_page(url: str, index: _ResultIndex, terms: list[str], question: str='', budget: float=0.0) -> str:
        ...
        return f'# fetch_page({url!r}) -> [{n}] {len(note)} chars total, {len(body)} shown' + (f'; the remaining {len(note) - len(body)} chars are held under [{n}] and NOT shown -- reach them with page_grep({n}, ...) and page_read({n}, offset, length), up to {PAGE_READ_MAX_CHARS} chars per read. Re-fetching this URL returns the same opening again.' if len(note) > len(body) else '') + f'\n{body}'
    ...
    def _digest_spans(note: str, spans: list[tuple[int, int]], terms: list[str], window: int, verified: list[tuple[int, int]] | None=None) -> list[tuple[int, int]]:
    ...
    section is absent or too bare to stand alone."""
        ...
        return section
    SCAFFOLD_HEAD_RE = re.compile('^\\s*(?:#{1,4}\\s*)?(?:\\*{1,2})?\\s*(?:VERIFY|BRIEFING)\\b', re.IGNORECASE)
    ...
    evidence digest, blind to the research conversation's framing."""
        ...
        if budget <= 14:
            return ''
        try:
            result = await llm_chat(provider=LLM_PROVIDER, model=MODEL, messages=[{'role': 'system', 'content': 'Write the COMPLETE final answer to the question from the numbered evidence alone. Every load-bearing figure or name carries the [n] marker of the source that states it. No preamble, no process narration -- the answer only.'}, {'role': 'user', 'content': 'QUESTION:\n' + (question or '')[:4000] + '\n\n' + digest}], temperature=0.2, thinking=LlmThinkingConfig(enabled=False), timeout=min(AB_CANDIDATE_TIMEOUT_SECONDS, budget - 2))
            return (result.response.raw_text or '').strip()
        except Exception:
            return ''
    ...
    async def _ab_judge(question: str, answer_a: str, answer_b: str, deadline: float) -> str:
        ...
        if budget <= 8:
            return 'A'
        try:
            result = await llm_chat(provider=AB_JUDGE_PROVIDER, model=AB_JUDGE_MODEL, messages=[{'role': 'system', 'content': 'You are a strict grader. Two candidate answers to the same research question are given. Reply with exactly one letter, A or B, naming the answer that is more complete, specific, and correct.'}, {'role': 'user', 'content': 'QUESTION:\n' + (question or '')[:4000] + '\n\nANSWER A:\n' + answer_a[:6000] + '\n\nANSWER B:\n' + answer_b[:6000] + '\n\nWhich is better? Reply A or B.'}], temperature=0.0, thinking=LlmThinkingConfig(enabled=False), timeout=min(AB_JUDGE_TIMEOUT_SECONDS, budget - 2))
        ...
        try:
            ...
            for _extra in range(CHECKPOINT_TOOL_TURNS + 1):
                if deadline - perf_counter() <= FINAL_RESERVE_SECONDS + 25:
                    break
                chat_result = await _chat_turn(messages, deadline=min(deadline - FINAL_RESERVE_SECONDS, perf_counter() + 45.0), thinking_on=True)
                if chat_result is None:
                    break
                choice_message = chat_result.response.choices[0].message
                content = (chat_result.response.raw_text or '').strip()
            ...
            if display:
                ...
                try:
                    decided = await _ab_select(query.text, decided, index, deadline - 4)
                except Exception:
                    pass
    ...
    """
        ...
        if nothing_happened and elapsed < COLD_START_WINDOW_SECONDS and (budget - elapsed - COLD_START_BACKOFF_SECONDS > COLD_START_MIN_BUDGET_SECONDS):
    ...
    """
        ...
        try:
            drafted = await _plain_query_with_cold_retry(query, TASK_TOTAL_BUDGET_SECONDS - STRUCTURED_RESERVE_SECONDS)
        except Exception:
    ...
    return query
_carvbgkwvh = _znyrcgxyvu()
_birtvqhehw = _jlazukcser()
_cmxyqdtuio = _mtbilqlheg()
_siopohezyh = _jikvzxxydl()
_ldgmgiwxgw = 290.0
_ggmwdugpwm = 250.0
_kewjhqbybi = 90.0

async def _akvubrruxu(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _ldgmgiwxgw - (time.monotonic() - started)
        if first:
            budget = _ggmwdugpwm if _ggmwdugpwm < remaining else remaining
            first = False
        else:
            if remaining < _kewjhqbybi:
                break
        ...
        try:
            ...
            last_exc = exc
    return _gfquwymaxl(query)
_uyeotohcaf = 235.0

async def _znzcyeistd(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _brtaymcxnv(query, agents):
    if len(agents) < 2:
        return await _akvubrruxu(query, agents)
    r0, r1 = await asyncio.gather(_znzcyeistd(agents[0], query, _uyeotohcaf), _znzcyeistd(agents[1], query, _uyeotohcaf))
    ok0 = isinstance(r0, Response) and _sgcjueyqzs(r0) != ''
    ok1 = isinstance(r1, Response) and _sgcjueyqzs(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_minwnoaobv(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _izfewqmtjo['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _akvubrruxu(query, (_cmxyqdtuio, _carvbgkwvh, _birtvqhehw, _siopohezyh))
        index = _kfmhwwjjfi(query)
        if index == 0:
            agents = (_carvbgkwvh, _birtvqhehw, _cmxyqdtuio, _siopohezyh)
        elif index == 1:
            agents = (_birtvqhehw, _cmxyqdtuio, _siopohezyh, _carvbgkwvh)
        elif index == 2:
            agents = (_cmxyqdtuio, _siopohezyh, _carvbgkwvh, _birtvqhehw)
        elif index == 3:
            agents = (_siopohezyh, _carvbgkwvh, _birtvqhehw, _cmxyqdtuio)
        else:
            agents = (_carvbgkwvh, _birtvqhehw, _cmxyqdtuio, _siopohezyh)
        return await _brtaymcxnv(query, agents)
    except Exception:
        return _gfquwymaxl(query)
_BUILDT_TAG_m12 = "sn45-a366b4560bee"
