"""Champion line under the clock guard, with schema gate, fast-mode precision, answer shape and condensing pass.
...
_DUO_TOTAL_S = 288.0        # never return later than this after start
_DUO_AGENT_S = 238.0        # hard wait on any single lineage
_DUO_CUTOFF_S = 232.0       # after this, tools refuse research and the model is told to finish
_DUO_BACKUP_MIN_S = 55.0    # a backup lineage only starts with at least this much left
_DUO_TEXT_CAP = 24000
_DUO_GRADER_MODEL = "openai/gpt-oss-120b"
_DUO_GRADER_TIMEOUT_S = 25.0

_DUO_STATE = {"started": None, "text": None, "cutoff": None}
_DUO_SCHEMA_FIRST_S = 150.0   # first lineage on a structured task; leaves a backup its own clock
_DUO_FINISH_ORDER = (
...
async def _duo_run(agent, query: Query, budget: float):
    try:
        return await asyncio.wait_for(agent(query), timeout=max(1.0, budget))
    except Exception:
        return None
...
def _compose_champion_line():
    ...
    LOOP_RULES = (
        ...
        "check who each one eliminates. Never pre-filter to the members that already "
        "pass: test the whole class, including members that fail on the FIRST "
        "condition. But the CHECKING is your work, not the reader's. The final "
        "answer names every qualifier with its cited qualifying fact, and treats "
        "exclusions the way the question does: one short clause naming the excluded "
        "members and the criterion they fail when the question asks for a complete "
        "sweep, nothing at all when it asks only for the qualifiers. The grader's "
        "own rubric says more detail is not inherently better and that a candidate "
        "dump, a line per rejected member, or a provenance dump does not help; it "
        "loses to a concise answer that is equally correct. "
    ...
    SET_RULE = (
        ...
        "condition). Mention excluded members only as briefly as the question needs: "
        "one clause with the criterion they fail, never a line per member, and "
        "nothing when only the qualifiers were asked for. Never claim 'the only X' unless "
...
def _sg_default(schema):
    ...
    if t == "object":
        return {k: _sg_default(v) for k, v in ((schema.get("properties") or {}).items()) if k in (schema.get("required") or [])}
    return {"string": "", "number": 0, "integer": 0, "boolean": False, "array": []}.get(t, None)
...
# --- tighten stage (start) -----------------------------------------------------
#
# Batch 6a0f7806 (2026-09-05): our cited-task rows still lost to answers the
# judge called more concise, with reasons like "adds a huge list of sectors
# that lead zero columns" and "unnecessary analysis, a candidate dump, or a
# provenance dump does not help" (its own rubric, quoted back). The lineage
# prompts were fixed, but a model does not always obey them, so a final
# condensing pass rewrites the cited answer to exactly what was asked and is
# accepted only when it is a strict subset of the draft: no new figures, no
# new names, no new pointers, not longer. Anything else keeps the draft.
import re as _tt_re

_TT_MODEL = "deepseek/deepseek-v3.2"
_TT_TIMEOUT_S = 30.0
_TT_MIN_CHARS = 600
_TT_MIN_KEEP_RATIO = 0.35
_TT_POINTER_RE = _tt_re.compile(r"\[\[(\d{1,3})\]\]")
_TT_FIGURE_RE = _tt_re.compile(r"\d[\d,]*(?:\.\d+)?")
_TT_ENTITY_RE = _tt_re.compile(r"\b[A-Z][A-Za-z0-9&'\-]{2,}\b")
_TT_FENCE_RE = _tt_re.compile(r"^```[a-z]*\s*|\s*```$", _tt_re.I | _tt_re.M)
_TT_SYSTEM = (
    "You edit a research answer for a strict pairwise grader. Rewrite the draft so "
    "it answers exactly what the question asks and nothing else: lead with the "
    "answer entities; keep every qualifying member and every figure, date and name "
    "that supports it; keep each [[n]] citation pointer exactly as written, right "
    "after the claim it supports. Cut what the question did not ask for: lists of "
    "excluded or zero-result candidates (compress to one clause naming them and the "
    "criterion they fail, or drop them when only the qualifiers were asked for), "
    "step or condition headings, working tables, notes on method, restatements of "
    "the question, hedges, and any 'draft' or 'note' remarks. Never add a fact, a "
    "number or a pointer. Return plain prose only, no code fences."
)


def _tt_figures(text: str) -> set:
    return {f.replace(",", "") for f in _TT_FIGURE_RE.findall(text) if len(f.replace(",", "").replace(".", "")) >= 2}


def _tt_pointers(text: str) -> list:
    return [int(n) for n in _TT_POINTER_RE.findall(text)]


def _tt_renumber(text: str, citations):
    if not citations:
        return text, citations
    order = []
    for n in _tt_pointers(text):
        if n < 1 or n > len(citations):
            return text, citations
        if n not in order:
            order.append(n)
    if not order:
        return text, citations
    mapping = {n: i + 1 for i, n in enumerate(order)}
    return _TT_POINTER_RE.sub(lambda m: f"[[{mapping[int(m.group(1))]}]]", text), [citations[n - 1] for n in order]


def _tt_accept(draft: str, rewrite: str) -> bool:
    rewrite = rewrite.strip()
    if not rewrite or len(rewrite) > len(draft.strip()):
        return False
    if len(rewrite) < max(200, int(_TT_MIN_KEEP_RATIO * len(draft.strip()))):
        return False
    if not set(_tt_pointers(rewrite)) <= set(_tt_pointers(draft)):
        return False
    if _tt_pointers(draft) and not _tt_pointers(rewrite):
        return False
    if not _tt_figures(rewrite) <= _tt_figures(draft):
        return False
    if not set(_TT_ENTITY_RE.findall(rewrite)) <= set(_TT_ENTITY_RE.findall(draft)):
        return False
    return True


async def _tt_tighten(query_input, response):
    """Condense a cited prose answer; keep the draft on any doubt."""
    try:
        if getattr(query_input, "fast", False) is True or getattr(query_input, "output_schema", None) is not None:
            return response
        draft = getattr(response, "text", None)
        if not isinstance(draft, str) or len(draft.strip()) < _TT_MIN_CHARS or getattr(response, "output", None) is not None:
            return response
        if _duo_remaining() < _TT_TIMEOUT_S + 12.0:
            return response
        question = str(getattr(query_input, "text", "") or "")[:4000]
        result = await asyncio.wait_for(_hsapi.llm_chat(
            provider="openrouter", model=_TT_MODEL,
            messages=[{"role": "system", "content": _TT_SYSTEM},
                      {"role": "user", "content": f"QUESTION:\n{question}\n\nDRAFT ANSWER:\n{draft[:14000]}"}],
            temperature=0.0, max_output_tokens=2400, timeout=_TT_TIMEOUT_S,
        ), timeout=_TT_TIMEOUT_S + 3.0)
        rewrite = getattr(getattr(result, "response", None), "raw_text", None) or ""
        if not rewrite:
            choices = getattr(getattr(result, "response", None), "choices", None) or []
            content = getattr(getattr(choices[0], "message", None), "content", None) if choices else None
            rewrite = content if isinstance(content, str) else ""
        rewrite = _TT_FENCE_RE.sub("", rewrite).strip()
        if not _tt_accept(draft, rewrite):
            return response
        citations = list(getattr(response, "citations", None) or [])
        new_text, new_cits = _tt_renumber(rewrite, citations)
        try:
...
_DUO_CHAMPION_LINE = _compose_champion_line()


async def _duo_post(query: Query, response: Response) -> Response:
    """Final hygiene, each stage fail-open: schema gate, fast-mode precision,
    answer shape, condensing pass."""
    response = _sg_gate(query, response)
    response = _fp_clean(query, response)
    response = _sh_shape(query, response)
    response = await _tt_tighten(query, response)
    return response
...
async def query(query: Query) -> Response:
    ...
    try:
        response = await _duo_run(_DUO_CHAMPION_LINE, query, _DUO_AGENT_S)
        if not (isinstance(response, Response) and (response.text or response.output is not None)):
            response = _duo_emergency(query)
