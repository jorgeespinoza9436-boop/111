...
_pkigcdqjax = {'started': None, 'text': None}
_urcsakxvmf = 24000
_blgjsjyvzt = 290.0
_bhgimwfkor = 250.0

def _xhxsbwxuey() -> float:
    started = _pkigcdqjax['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _hplrzwrzfa() -> float:
    return _blgjsjyvzt - _xhxsbwxuey()
_bzxymadpar = _hsapi.llm_chat
_xdhaxxdroh = _hsapi.search_web
_vmaknhzles = _hsapi.fetch_page
...
async def _xmeyeeazdi(*args, **kwargs):
    if _xhxsbwxuey() >= _bhgimwfkor:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _gdghpuvfco})
            kwargs['messages'] = steered
    _result = await _bzxymadpar(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _xjyjwmcxgp(_result)
    return _result

async def _dfvlhgrzrs(*args, **kwargs):
    if _xhxsbwxuey() >= _bhgimwfkor:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _xdhaxxdroh(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _epynmrzqyq(*args, **kwargs):
    if _xhxsbwxuey() >= _bhgimwfkor:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _vmaknhzles(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _xmeyeeazdi
_hsapi.search_web = _dfvlhgrzrs
_hsapi.fetch_page = _epynmrzqyq
_avzgzzhaic = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_elbfyzyijj = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_vflpbsyqke = 900
_rjpmsplnme = 2

def _xptsstbbup(query: Query) -> int:
...
def _zilwohckje(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _lgtsfmxqze(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _xptsstbbup(query)
    if fields >= 3:
        return 2
    if _zilwohckje(lowered, _avzgzzhaic):
        return 1
    if fields <= _rjpmsplnme and len(text) <= _vflpbsyqke:
        return 0
    if _zilwohckje(lowered, _elbfyzyijj):
        return 0
    return 1

def _xjyjwmcxgp(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _pkigcdqjax['text'] = text.strip()[:_urcsakxvmf]
    except Exception:
        pass

def _wjlagcruce(text: str):
...
def _xspcxunbri(query: Query) -> Response:
    text = _pkigcdqjax['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_urcsakxvmf]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _wjlagcruce(text)
...
def _cmtjnulekh():
    ...
    return query
_fliilyfqkp = _ogxsnprnaq()
_bbfscyrtyw = _ltkfkjqrox()
_vsjofosxds = _zlewkcsqqk()
_soqoclpamx = _cmtjnulekh()
_blgjsjyvzt = 290.0
_bugbqbwwqa = 250.0
_vpedobskjd = 90.0

async def _ionxkbbmau(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _blgjsjyvzt - (time.monotonic() - started)
        if first:
            budget = _bugbqbwwqa if _bugbqbwwqa < remaining else remaining
            first = False
        else:
            if remaining < _vpedobskjd:
                break
        ...
        try:
            ...
            last_exc = exc
    return _xspcxunbri(query)
_ncwmkmmzvg = 235.0

async def _yzsedykees(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _lckpuqyoah(query, agents):
    if len(agents) < 2:
        return await _ionxkbbmau(query, agents)
    r0, r1 = await asyncio.gather(_yzsedykees(agents[0], query, _ncwmkmmzvg), _yzsedykees(agents[1], query, _ncwmkmmzvg))
    ok0 = isinstance(r0, Response) and _kbtcrexoaq(r0) != ''
    ok1 = isinstance(r1, Response) and _kbtcrexoaq(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_bzxymadpar(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _pkigcdqjax['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _ionxkbbmau(query, (_vsjofosxds, _fliilyfqkp, _bbfscyrtyw, _soqoclpamx))
        index = _lgtsfmxqze(query)
        if index == 0:
            agents = (_fliilyfqkp, _bbfscyrtyw, _vsjofosxds, _soqoclpamx)
        elif index == 1:
            agents = (_bbfscyrtyw, _vsjofosxds, _soqoclpamx, _fliilyfqkp)
        elif index == 2:
            agents = (_vsjofosxds, _soqoclpamx, _fliilyfqkp, _bbfscyrtyw)
        elif index == 3:
            agents = (_soqoclpamx, _fliilyfqkp, _bbfscyrtyw, _vsjofosxds)
        else:
            agents = (_fliilyfqkp, _bbfscyrtyw, _vsjofosxds, _soqoclpamx)
        return await _lckpuqyoah(query, agents)
    except Exception:
        return _xspcxunbri(query)
_BUILDT_TAG_m6 = "sn45-4444fe614979"
