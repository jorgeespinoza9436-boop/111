...
def _compose_aster_slate_slot02_agent_entry():
...
_aster_slate_slot02_agent_query_entry = _compose_aster_slate_slot02_agent_entry()


def _compose_saffron_vector_slot02_agent_entry():
...
_saffron_vector_slot02_agent_query_entry = _compose_saffron_vector_slot02_agent_entry()


def _compose_ember_archive_slot02_agent_entry():
...
_ember_archive_slot02_agent_query_entry = _compose_ember_archive_slot02_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "AsterSlateSlot02Agent"
    ...
    if (
        ...
        return "SaffronVectorSlot02Agent"
    return "EmberArchiveSlot02Agent"


class AsterSlateSlot02Agent:
    async def __call__(self, query: Query) -> Response:
        return await _aster_slate_slot02_agent_query_entry(query)


class SaffronVectorSlot02Agent:
    async def __call__(self, query: Query) -> Response:
        return await _saffron_vector_slot02_agent_query_entry(query)


class EmberArchiveSlot02Agent:
    async def __call__(self, query: Query) -> Response:
        return await _ember_archive_slot02_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = AsterSlateSlot02Agent()
_ANALYTICAL_FIELD_AGENT = SaffronVectorSlot02Agent()
_BROAD_FIELD_AGENT = EmberArchiveSlot02Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "AsterSlateSlot02Agent",
    "SaffronVectorSlot02Agent",
    "EmberArchiveSlot02Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "AsterSlateSlot02Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "SaffronVectorSlot02Agent":
