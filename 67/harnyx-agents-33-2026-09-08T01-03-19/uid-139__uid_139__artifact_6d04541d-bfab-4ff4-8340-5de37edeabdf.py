...
_lgkgdrhjyw = {'started': None, 'text': None}
_vkfkoblfbu = 24000
_bosuptloli = 290.0
_emmpjrsbzz = 250.0

def _trybkebrhb() -> float:
    started = _lgkgdrhjyw['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _trgnbdffya() -> float:
    return _bosuptloli - _trybkebrhb()
_waoljrhkxt = _hsapi.llm_chat
_tfxpovpktc = _hsapi.search_web
_lxzfpzvfdv = _hsapi.fetch_page
_lixnvrsetw = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _rqxyiaffcw(*args, **kwargs):
    if _trybkebrhb() >= _emmpjrsbzz:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _lixnvrsetw})
            kwargs['messages'] = steered
    _result = await _waoljrhkxt(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _svbjpnzjai(_result)
    return _result

async def _oomxuylhxj(*args, **kwargs):
    if _trybkebrhb() >= _emmpjrsbzz:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _tfxpovpktc(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _crwxrkyhst(*args, **kwargs):
    if _trybkebrhb() >= _emmpjrsbzz:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _lxzfpzvfdv(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _rqxyiaffcw
_hsapi.search_web = _oomxuylhxj
_hsapi.fetch_page = _crwxrkyhst
_vtdbbssxug = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_sxtzyxqtgk = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_rpiqazqkhb = 900
_volxarsjde = 2

def _pldrgvpeqc(query: Query) -> int:
...
def _ekrgpusjqd(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _wapubtpngz(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _pldrgvpeqc(query)
    if fields >= 3:
        return 2
    if _ekrgpusjqd(lowered, _vtdbbssxug):
        return 1
    if fields <= _volxarsjde and len(text) <= _rpiqazqkhb:
        return 0
    if _ekrgpusjqd(lowered, _sxtzyxqtgk):
        return 0
    return 1

def _svbjpnzjai(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _lgkgdrhjyw['text'] = text.strip()[:_vkfkoblfbu]
    except Exception:
        pass

def _rtrxmktpfv(text: str):
...
def _toweagxwvh(query: Query) -> Response:
    text = _lgkgdrhjyw['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_vkfkoblfbu]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _rtrxmktpfv(text)
...
def _vimpdqlmno():
    ...
    def _gx_sparse_citations(question: str, answer: str) -> list:
        if len(answer) < 800:
            return []
        sentences = len(re.findall('[.!?](?:\\s|$)', answer))
        if sentences < 3:
            return []
        if answer.count('\n') >= 8:
            return []
        marks = len(set(re.findall('\\[(\\d+)\\]', answer)))
        if marks >= 2:
            return []
        return [f'{marks} distinct [n] markers across {sentences} sentences']
    ...
    def _gx_defects(question: str, answer: str) -> list:
        ...
        if sparse:
            notes.append('The answer is long but cites almost nothing (' + '; '.join(sparse) + ') — attach [n] markers to the load-bearing claims.')
...
"""
    ...
    async def query(query: Query) -> Response:
        ...
        try:
            ...
            return draft
    return query
_pmnnkzwcsa = _vimpdqlmno()
_bzwuuhnhpf = _kkeggjntrk()
_erasoovwms = _rzllzlznfa()
_uvxavrnorx = _bwehvdhksl()
_bosuptloli = 290.0
_ktfagglxup = 250.0
_hgoelmvskd = 90.0

async def _xawfqgqwdx(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _bosuptloli - (time.monotonic() - started)
        if first:
            budget = _ktfagglxup if _ktfagglxup < remaining else remaining
            first = False
        else:
            if remaining < _hgoelmvskd:
                break
        ...
        try:
            ...
            last_exc = exc
    return _toweagxwvh(query)
_jhdttajduq = 235.0

async def _ptfknkdhku(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _loxrafpyln(query, agents):
    if len(agents) < 2:
        return await _xawfqgqwdx(query, agents)
    r0, r1 = await asyncio.gather(_ptfknkdhku(agents[0], query, _jhdttajduq), _ptfknkdhku(agents[1], query, _jhdttajduq))
    ok0 = isinstance(r0, Response) and _ujypnpxilg(r0) != ''
    ok1 = isinstance(r1, Response) and _ujypnpxilg(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_waoljrhkxt(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _lgkgdrhjyw['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _xawfqgqwdx(query, (_erasoovwms, _pmnnkzwcsa, _bzwuuhnhpf, _uvxavrnorx))
        index = _wapubtpngz(query)
        if index == 0:
            agents = (_pmnnkzwcsa, _bzwuuhnhpf, _erasoovwms, _uvxavrnorx)
        elif index == 1:
            agents = (_bzwuuhnhpf, _erasoovwms, _uvxavrnorx, _pmnnkzwcsa)
        elif index == 2:
            agents = (_erasoovwms, _uvxavrnorx, _pmnnkzwcsa, _bzwuuhnhpf)
        elif index == 3:
            agents = (_uvxavrnorx, _pmnnkzwcsa, _bzwuuhnhpf, _erasoovwms)
        else:
            agents = (_pmnnkzwcsa, _bzwuuhnhpf, _erasoovwms, _uvxavrnorx)
        return await _loxrafpyln(query, agents)
    except Exception:
        return _toweagxwvh(query)
_BUILDT_TAG_m18 = "sn45-cf88bd2a7301"
