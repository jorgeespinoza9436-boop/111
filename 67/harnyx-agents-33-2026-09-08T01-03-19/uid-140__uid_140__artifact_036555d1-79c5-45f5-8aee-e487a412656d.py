...
# fork of 4_uid_113_score_0.692.py: added k2  (full mechanism set: CORR+DRV+K2)
import asyncio
import time
from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import Query, Response
import harnyx_miner_sdk.api as _hsapi
_cxjjpgclfj = {'started': None, 'text': None}
_mgfcmkbcfc = 24000
_kydtetqkfb = 290.0
_xgtvtiwoxg = 250.0

def _jzbtdfbraz() -> float:
    started = _cxjjpgclfj['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _vafsbkrsew() -> float:
    return _kydtetqkfb - _jzbtdfbraz()
_kotuautbjt = _hsapi.llm_chat
_obljfjtfku = _hsapi.search_web
_lnvkjfozwq = _hsapi.fetch_page
_syfqchlmhs = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _dkyycyqyjs(*args, **kwargs):
    if _jzbtdfbraz() >= _xgtvtiwoxg:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _syfqchlmhs})
            kwargs['messages'] = steered
    _result = await _kotuautbjt(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _kdoxivticw(_result)
    return _result

async def _mrjpalhixf(*args, **kwargs):
    if _jzbtdfbraz() >= _xgtvtiwoxg:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _obljfjtfku(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _xldcdegpnb(*args, **kwargs):
    if _jzbtdfbraz() >= _xgtvtiwoxg:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _lnvkjfozwq(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _dkyycyqyjs
_hsapi.search_web = _mrjpalhixf
_hsapi.fetch_page = _xldcdegpnb
_arheacgbze = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_yaofmjdscy = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_ntiaykxxnb = 900
_oyzegxjluz = 2

def _qrbluaiuub(query: Query) -> int:
...
def _cgqzkyaoti(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _tpfbqiglwe(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _qrbluaiuub(query)
    if fields >= 3:
        return 2
    if _cgqzkyaoti(lowered, _arheacgbze):
        return 1
    if fields <= _oyzegxjluz and len(text) <= _ntiaykxxnb:
        return 0
    if _cgqzkyaoti(lowered, _yaofmjdscy):
        return 0
    return 1

def _kdoxivticw(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _cxjjpgclfj['text'] = text.strip()[:_mgfcmkbcfc]
    except Exception:
        pass

def _nfxymglggg(text: str):
...
def _ufcdyvbehv(query: Query) -> Response:
    text = _cxjjpgclfj['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_mgfcmkbcfc]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _nfxymglggg(text)
...
def _yeoxllopwh():
    ...
    return query
_fgclaoqmtn = _zjgbhnlzoj()
_wpttmbsnju = _laldqanvoa()
_iensurgjfr = _sftlxdsrfy()
_irzcantvis = _yeoxllopwh()
_kydtetqkfb = 290.0
_lwkxnkcexl = 250.0
_qcxufpgndk = 90.0

async def _pfqggnypvb(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _kydtetqkfb - (time.monotonic() - started)
        if first:
            budget = _lwkxnkcexl if _lwkxnkcexl < remaining else remaining
            first = False
        else:
            if remaining < _qcxufpgndk:
                break
        ...
        try:
            ...
            last_exc = exc
    return _ufcdyvbehv(query)

async def _k2_base_query(query: Query) -> Response:
    _cxjjpgclfj['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _pfqggnypvb(query, (_iensurgjfr, _fgclaoqmtn, _wpttmbsnju, _irzcantvis))
        index = _tpfbqiglwe(query)
        if index == 0:
            agents = (_fgclaoqmtn, _wpttmbsnju, _iensurgjfr, _irzcantvis)
        elif index == 1:
            agents = (_wpttmbsnju, _iensurgjfr, _irzcantvis, _fgclaoqmtn)
        elif index == 2:
            agents = (_iensurgjfr, _irzcantvis, _fgclaoqmtn, _wpttmbsnju)
        elif index == 3:
            agents = (_irzcantvis, _fgclaoqmtn, _wpttmbsnju, _iensurgjfr)
        else:
            agents = (_fgclaoqmtn, _wpttmbsnju, _iensurgjfr, _irzcantvis)
        return await _pfqggnypvb(query, agents)
    except Exception:
        return _ufcdyvbehv(query)
_BUILDU_TAG_m28 = "sn45-a2cf1e49f030"

# ---------------------------------------------------------------------------
# K2 claim-ledger cycle  [variant u113-k2]
#
