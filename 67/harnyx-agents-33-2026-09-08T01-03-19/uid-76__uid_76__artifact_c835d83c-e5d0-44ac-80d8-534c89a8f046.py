...
def _compose_pine_ledger_agent_entry():
...
def _compose_brook_current_agent_entry():
...
def _compose_heath_ground_agent_entry():
...
def _compose_dune_torrent_agent_entry():
...
_PINE_LEDGER = _compose_pine_ledger_agent_entry()
_BROOK_CURRENT = _compose_brook_current_agent_entry()
_HEATH_GROUND = _compose_heath_ground_agent_entry()
_DUNE_TORRENT = _compose_dune_torrent_agent_entry()
...
def _route(query: Query):
    if getattr(query, "output_schema", None) is not None:
        return _BROOK_CURRENT, _PINE_LEDGER
    text = (getattr(query, "text", "") or "")
    if _ROUTE_TABLE_RE.search(text) and _ROUTE_SWEEP_RE.search(text):
        return _DUNE_TORRENT, _HEATH_GROUND
    if len(text) > 7000:
        return _DUNE_TORRENT, _HEATH_GROUND
    return _HEATH_GROUND, _PINE_LEDGER
