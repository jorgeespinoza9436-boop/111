...
# fork of 8_uid_216_score_0.669.py: added k2  (full mechanism set: CORR+K2)
import asyncio
import time
from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import Query, Response
import harnyx_miner_sdk.api as _hsapi
_fdianqtfjb = {'started': None, 'text': None}
_xicuauimze = 24000
_wjgddccbvx = 290.0
_kkygxabltl = 250.0

def _zpssekogsb() -> float:
    started = _fdianqtfjb['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _kcnzhgbise() -> float:
    return _wjgddccbvx - _zpssekogsb()
_xztgqkieyj = _hsapi.llm_chat
_sxzyuoqohs = _hsapi.search_web
_oxdwcrgmvs = _hsapi.fetch_page
_zfozvqndxl = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _bkmojivubf(*args, **kwargs):
    if _zpssekogsb() >= _kkygxabltl:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _zfozvqndxl})
            kwargs['messages'] = steered
    _result = await _xztgqkieyj(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _tfplndpadh(_result)
    return _result

async def _unxgkbhzsn(*args, **kwargs):
    if _zpssekogsb() >= _kkygxabltl:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _sxzyuoqohs(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _mflfcxhklx(*args, **kwargs):
    if _zpssekogsb() >= _kkygxabltl:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _oxdwcrgmvs(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _bkmojivubf
_hsapi.search_web = _unxgkbhzsn
_hsapi.fetch_page = _mflfcxhklx
_zdhkpeyeia = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_jbquwmyuma = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_dcbvnarqjp = 900
_siivhqjowd = 2

def _upjjbejrgz(query: Query) -> int:
...
def _wgnadictiv(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _dlglwqgqoc(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _upjjbejrgz(query)
    if fields >= 3:
        return 2
    if _wgnadictiv(lowered, _zdhkpeyeia):
        return 1
    if fields <= _siivhqjowd and len(text) <= _dcbvnarqjp:
        return 0
    if _wgnadictiv(lowered, _jbquwmyuma):
        return 0
    return 1

def _tfplndpadh(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _fdianqtfjb['text'] = text.strip()[:_xicuauimze]
    except Exception:
        pass

def _wuncltyqrm(text: str):
...
def _mnzlpvdrrj(query: Query) -> Response:
    text = _fdianqtfjb['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_xicuauimze]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _wuncltyqrm(text)
...
def _bdhsstvrmt():
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        question = question.translate({8212: ' - '})
    ...
    async def _gx_repair(question: str, answer: str, deadline: float) -> str:
        try:
            ...
            return answer
    _GX_COUNT_WORDS = {'two': 2, 'three': 3, 'four': 4, 'five': 5, 'six': 6, 'seven': 7, 'eight': 8, 'nine': 9, 'ten': 10}

    def _gx_short_enumeration(question: str, answer: str) -> list:
        m = re.search('\\b(two|three|four|five|six|seven|eight|nine|ten|[2-9]|10)\\s+(?:largest|smallest|top|main|biggest|most|key|distinct)\\s+[a-z]+s\\b', question, re.I)
        if not m:
            return []
        token = m.group(1).lower()
        want = _GX_COUNT_WORDS.get(token) or int(token)
        items = len(re.findall('(?:^|\\n)\\s*(?:[-*\\u2022]|\\d+[.)])\\s+', answer))
        if items and items < want:
            return [f'only {items} of the {want} requested items are enumerated']
        return []
    ...
    def _gx_defects(question: str, answer: str) -> list:
        ...
        short = _gx_short_enumeration(question, answer)
        if short:
            notes.append('The question fixes how many items the answer must cover: ' + '; '.join(short))
    ...
    async def query(query: Query) -> Response:
        ...
        return response
    VERSION = 'c8-412'
    _GX_ACTIVE = ('super', 'entity', 'unit', 'listcount')
    return query
...
architecture, keeping the assets our line already validated:
  ...
  - dual-provider LLM lanes (openrouter primary, our paid openrouter fallback).
...
"""
    ...
    SEARCH_TIMEOUT_S = 18.0
    WALL_BUDGET_S = 210.0
    TASK_TOTAL_BUDGET_SECONDS = 210.0
    TURN_TIMEOUT_S = 75.0
    FETCH_TIMEOUT_S = 16.0
    LANE_B_MAX_PAYLOAD_CHARS = 144000
    ...
    async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool, force_tools: bool=False):
        """One loop turn; lane A first, lane B (our paid openrouter) on failure."""
        turn_wall = monotonic() + TURN_TIMEOUT_S + 35.0
        payload_chars = sum((len(str(msg.get('content') or '')) for msg in messages if isinstance(msg, dict)))
        for lane_model in ((LLM_LANE_A, LOOP_MODEL_A, True), (LLM_LANE_A, LOOP_MODEL_A, False), (LLM_LANE_B, LOOP_MODEL_B, False)):
            lane = lane_model[0]
    ...
    _W2_DEFAULT_BUDGET_SECONDS = 210.0
...
def _mryxuszvlj():
        ...
        for match in _AUD_ENTITY_RE.finditer(text or ''):
            for piece in _AUD_SPLIT_RE.split(match.group(0)):
                ...
                if len(found) >= limit:
                    return found
        return found
    RESEARCH_FALLBACK_MODEL = 'z-ai/glm-5.2'
    ...
    return query
_lmsuaeqbfs = _bdhsstvrmt()
_zqtyqctzmz = _dibdpatupl()
_omffihuyzj = _roodgavqtq()
_rqprzjkmpl = _mryxuszvlj()
_wjgddccbvx = 290.0
_hlezigvijl = 250.0
_gxosiercgn = 90.0

async def _lcvywxglyh(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _wjgddccbvx - (time.monotonic() - started)
        if first:
            budget = _hlezigvijl if _hlezigvijl < remaining else remaining
            first = False
        else:
            if remaining < _gxosiercgn:
                break
        ...
        try:
            ...
            last_exc = exc
    return _mnzlpvdrrj(query)
_jxsjifphcq = 235.0

async def _kwjkaufafg(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _ilpwzjnjpq(query, agents):
    if len(agents) < 2:
        return await _lcvywxglyh(query, agents)
    r0, r1 = await asyncio.gather(_kwjkaufafg(agents[0], query, _jxsjifphcq), _kwjkaufafg(agents[1], query, _jxsjifphcq))
    ok0 = isinstance(r0, Response) and _rsmgrgxsxh(r0) != ''
    ok1 = isinstance(r1, Response) and _rsmgrgxsxh(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_xztgqkieyj(provider='openrouter', messages=msgs, model='z-ai/glm-5.2', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
async def _k2_base_query(query: Query) -> Response:
    _fdianqtfjb['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _lcvywxglyh(query, (_omffihuyzj, _lmsuaeqbfs, _zqtyqctzmz, _rqprzjkmpl))
        index = _dlglwqgqoc(query)
        if index == 0:
            agents = (_lmsuaeqbfs, _zqtyqctzmz, _omffihuyzj, _rqprzjkmpl)
        elif index == 1:
            agents = (_zqtyqctzmz, _omffihuyzj, _rqprzjkmpl, _lmsuaeqbfs)
        elif index == 2:
            agents = (_omffihuyzj, _rqprzjkmpl, _lmsuaeqbfs, _zqtyqctzmz)
        elif index == 3:
            agents = (_rqprzjkmpl, _lmsuaeqbfs, _zqtyqctzmz, _omffihuyzj)
        else:
            agents = (_lmsuaeqbfs, _zqtyqctzmz, _omffihuyzj, _rqprzjkmpl)
        return await _ilpwzjnjpq(query, agents)
    except Exception:
        return _mnzlpvdrrj(query)
_BUILDU_TAG_m24 = "sn45-381660b84f80"

# ---------------------------------------------------------------------------
# K2 claim-ledger cycle  [variant u216-k2]
#
