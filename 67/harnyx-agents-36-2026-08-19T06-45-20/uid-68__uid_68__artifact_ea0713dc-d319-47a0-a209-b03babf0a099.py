...
VERSION = "v53-subject-ground-measure-roster"
...
#
# Stage 3w below then verifies the result after the answer exists, so the two
...
#
# This build wires NO corroboration stage, so this is the only figure-support
# check in the file and it owns the ZERO-backer case alone. A figure with
# exactly one cited backer passes untouched -- that is a deliberate gap, not an
# oversight: single-source figures are left for a build that wires 3s.
#
# RUNS AFTER the subject sweep and BEFORE the measure stage. Wrong subject
# invalidates every figure this stage would inspect, and the measure stage must
# see the figures this stage may have replaced.
#
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # SCOPE, THEN SUPPORT, THEN PRESENTATION. Three sweeps share one tail against
    # a measured ceiling of TWO firings -- a firing sweep costs ~42s of search
    # envelope plus up to three loop turns, and entering this chain with 230s
    # still only funds two. Treat the order as a PRIORITY RANKING, not a pipeline:
    #   subject  -- wrong entity makes every later check moot, and since each
    #               sweep rewrites the whole answer it cannot be repaired later
    #   grounded -- figures with ZERO cited backers; the only figure-support
    #               check in this build, since 3s is not wired here
    #   measure  -- pure formatting, and LAST because both sweeps above rewrite
    #               the whole answer and would discard its annotations
    # The measure stage is therefore the least likely of the three to run, which
    # is correct: it is also the only one that changes nothing about what the
    # answer claims. The roster pre-pass is the one stage outside the contention
    # -- it ran before the loop and is guaranteed on any set or superlative
    # question, and the subject sweep verifies the draft it produced.
    ...
    for _sweep in (_verify_subjects, _ground_figures, _conform_measures):
        try:
            if not _is_usable_answer(answer):
                break
            # the LOWEST floor of the wired sweeps (subject 110s, grounding 90s,
            # measure 70s): breaking above it would skip a stage that could
            # still run. If a sweep is ever swapped in or out here, RE-DERIVE
            # this guard -- leaving it at a removed stage's floor silently
