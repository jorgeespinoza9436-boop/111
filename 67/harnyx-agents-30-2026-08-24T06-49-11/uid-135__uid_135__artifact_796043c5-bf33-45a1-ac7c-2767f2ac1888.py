...
def _compose_lumen_anvil_agent_entry():
    ...
    _FAST_UPSTREAMS = ("Decart", "Ambient", "DeepInfra", "Baidu")
    ...
    def _upstream(lane: str, model: str) -> dict | None:
        """Ordered single-pin: `provider.only` discards order, so choose in Python.

    Handing the whole pool to the router with fallbacks on lets it load-balance by
    price, which lands every call on the cheapest member whatever its position in
    the tuple. Emitting a ONE-element `only` is the only way the order is honoured.
    A member that fails is retired to `dead` and the next one takes over, so a
    single pin never strands the run.
    """
        if lane != LLM_LANE_A:
            return None
    ...
    def _upstream_failed(model: str) -> None:
        """Retire the current pin so the next call takes the next pool member."""
        key = _upstream_key(model)
        if key is None:
            return
        chosen = _RUN_UPSTREAM.get(key)
    ...
    _LAT_MAX_COMPLETION = 200
    _LAT_SLOW_SECONDS = 6.0
    _LAT_WINDOW = 4
    _LAT_SLOW_IN_WINDOW = 3


    def _lat_completion(payload) -> int:
        """completion_tokens off either wrapper shape; -1 when unavailable.

    Unrolled rather than looped over ("llm", "response"): the upload policy
    rejects getattr with a non-literal field name (dynamic_getattr_name).
    """
        _usage = getattr(getattr(payload, "llm", None), "usage", None)
        _n = getattr(_usage, "completion_tokens", None)
        if isinstance(_n, int):
            return _n
        _usage = getattr(getattr(payload, "response", None), "usage", None)
        _n = getattr(_usage, "completion_tokens", None)
        if isinstance(_n, int):
            return _n
        return -1
    ...
    def _lat_note(seconds: float, payload, key: str) -> None:
        """Retire a pin that stays slow without ever raising.

    Retirement otherwise happens only in an `except`, so an upstream that
    degrades to a steady multiple of its normal latency but keeps answering is
    never retired -- the exact regime that cost this miner a batch. Raw call
    latency cannot be thresholded: it is dominated by output length (production
    p50 3.6s, p90 14.4s, p99 39.7s over 1,716 calls). Restricting the sample to
    calls that generated at most _LAT_MAX_COMPLETION tokens removes that confound
    and collapses the healthy spread to p50 2.3s / p90 4.1s / p99 6.4s.

    _LAT_SLOW_IN_WINDOW of _LAT_WINDOW is required, so no single outlier can
    retire anything, and at most ONE latency retirement is allowed per task, so a
    wrong call can never cascade past the second pool member. Replayed over 399
    production task-executions from four different builds, this fires on 1 of
    them (0.25%) and costs +0.11% LLM wall on the worst of the four; against the
    same traces scaled 2x it fires on 22-45% of tasks for -3.9 to -9.2% LLM wall,
    and scaled 3x on 54-65% for -17 to -24%.
    """
        if key != "glm":
            return
        chosen = _RUN_UPSTREAM.get(key)
        if not chosen or _RUN_UPSTREAM.get("latn"):
            return
        count = _lat_completion(payload)
        if count < 0 or count > _LAT_MAX_COMPLETION:
            return
        window = _RUN_UPSTREAM.get("lat")
        if not isinstance(window, list):
            window = []
            _RUN_UPSTREAM["lat"] = window
        window.append(1 if seconds > _LAT_SLOW_SECONDS else 0)
        while len(window) > _LAT_WINDOW:
            window.pop(0)
        if len(window) >= _LAT_WINDOW and sum(window) >= _LAT_SLOW_IN_WINDOW:
            _RUN_UPSTREAM["dead"].add(chosen)
            _RUN_UPSTREAM[key] = None
            _RUN_UPSTREAM["latn"] = 1
            _RUN_UPSTREAM["lat"] = []


    def _upstream_slow(started: float, payload, model: str) -> None:
        """Latency sample for a call that was pinned and succeeded."""
        _lat_note(perf_counter() - started, payload, _upstream_key(model) or "")
    ...
    async def _chat_simple(lane: str, model: str, system: str, user: str, *,
        ...
        for _pin in ((_pin0, None) if _pin0 is not None else (None,)):
            try:
                ...
                if _pin is not None:
                    _upstream_slow(_lat_t0, payload, model)
                break
            except Exception:
                if _pin is None:
                    raise
    ...
    async def _w4_chat(messages: list[dict[str, object]], *, timeout: float, temperature: float) -> str:
        ...
        try:
            ...
            result = await llm_chat(
                ...
                provider_extra=_upstream(_w4_provider(), _w4_model()),
            )
            _upstream_slow(_lat_t0, result, _w4_model())
        except Exception:
            _upstream_failed(_w4_model())
            return ""
        try:
...
def _compose_cedar_quill_agent_entry():
    ...
    def _upstream_slow(started: float, payload, model: str) -> None:
        """Latency sample for a call that was pinned and succeeded."""
        _lat_note(monotonic() - started, payload, _upstream_key(model) or "")
...
def _compose_juniper_compass_agent_entry():
    ...
    _FAST_UPSTREAMS = ("Decart", "Ambient", "DeepInfra", "Baidu")
    _RUN_UPSTREAM: dict = {"glm": None, "dead": set()}


    def _upstream(provider: str, model: str) -> dict | None:
        """Ordered single-pin for glm-5.2 on openrouter; None for anything else.

    This engine shipped unpinned, which the router serves at ~5.3 s/call against
    Decart's ~3.0. `provider.only` discards order, so the choice is made here and
    emitted as a ONE-element list. A failed member is retired and the next takes
    over; an exhausted pool returns None, i.e. exactly the unpinned behaviour this
    engine had before, so the degraded case is no worse than the original.
    """
        if provider != LLM_PROVIDER or not model.startswith("z-ai/glm-5.2"):
            return None
        chosen = _RUN_UPSTREAM.get("glm")
        if chosen is None or chosen in _RUN_UPSTREAM["dead"]:
            live = [u for u in _FAST_UPSTREAMS if u not in _RUN_UPSTREAM["dead"]]
            if not live:
                return None
            chosen = live[0]
            _RUN_UPSTREAM["glm"] = chosen
        return {"provider": {"only": [chosen], "allow_fallbacks": False}}


    def _upstream_failed(provider: str, model: str) -> None:
        """Retire the current pin so the next call takes the next pool member."""
        if provider != LLM_PROVIDER or not model.startswith("z-ai/glm-5.2"):
            return
        chosen = _RUN_UPSTREAM.get("glm")
    ...
    def _upstream_slow(started: float, payload, provider: str, model: str) -> None:
        """Latency sample for a call that was pinned and succeeded."""
        if provider != LLM_PROVIDER or not model.startswith("z-ai/glm-5.2"):
            return
        _lat_note(perf_counter() - started, payload, "glm")
    ...
    async def _amend(
        ...
        try:
            ...
            result = await llm_chat(
                ...
                provider_extra=_upstream(LLM_PROVIDER, MODEL),
            )
            _upstream_slow(_lat_t0, result, LLM_PROVIDER, MODEL)
    ...
    async def _chat_turn(
        ...
        for _attempt in range(MAX_RETRY_ATTEMPTS_PER_TURN):
            ...
            try:
                ...
                _upstream_slow(_lat_t0, _lat_out, LLM_PROVIDER, MODEL)
                return _lat_out
            except Exception:
                _upstream_failed(LLM_PROVIDER, MODEL)
                continue
        return None
    ...
    async def _so_call(messages: list[dict[str, str]], timeout: float) -> str:
        try:
            ...
            result = await llm_chat(
                ...
                temperature=0.0,
                timeout=timeout,
                provider_extra=_upstream(_STRUCTURED_PROVIDER, _STRUCTURED_MODEL),
            )
            _upstream_slow(_lat_t0, result, _STRUCTURED_PROVIDER, _STRUCTURED_MODEL)
        except Exception:
            _upstream_failed(_STRUCTURED_PROVIDER, _STRUCTURED_MODEL)
            return ""
        try:
            return (result.response.raw_text or "").strip()
        except Exception:
    ...
    async def _w4_chat(messages: list[dict[str, object]], *, timeout: float, temperature: float) -> str:
        ...
        try:
            ...
            _upstream_slow(_lat_t0, result, _w4_provider(), _w4_model())
        except Exception:
            _upstream_failed(_w4_provider(), _w4_model())
            return ""
        try:
            return (result.response.raw_text or "").strip()
        except Exception:
