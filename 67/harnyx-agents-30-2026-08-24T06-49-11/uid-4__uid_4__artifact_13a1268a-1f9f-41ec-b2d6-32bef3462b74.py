...
_STRUCTURED_AGENT = JuniperCompassAgent()
_PROSE_AGENT = CedarQuillAgent()
...
async def query(query: Query) -> Response:
    schema = getattr(query, "output_schema", None)
    if schema is not None:
        return await _STRUCTURED_AGENT(query)
    return await _PROSE_AGENT(query)
