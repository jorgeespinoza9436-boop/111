...
def _build_agent_1():
    ...
    async def _mfold(prompt, deadline):
        ...
        if left < 22.0:
            return ''
        try:
            out = await llm_chat(provider=LLM_LANE_A, model=AUDIT_MODEL, messages=[{'role': 'user', 'content': prompt}], temperature=0.15, max_output_tokens=2600, timeout=min(AUDIT_TIMEOUT_S, max(8.0, left - 10.0)))
        except Exception:
            return ''
        try:
            _spend_note(out)
        ...
        try:
            ...
            if isinstance(out, dict):
                ...
                return ((ch.get('message') or {}).get('content') or ch.get('text') or out.get('content') or '').strip()
            return str(getattr(out, 'content', '') or '').strip()
        except Exception:
            return ''
    _lf_ASK = re.compile('\\banswer (?:as|with)\\b|\\bplain list\\b|\\bseparated by\\b|\\breport:\\s*\\(1\\)|\\blist them\\b', re.I)
    _lf_HEAD = re.compile('^\\s*(?:#{1,6}\\s|\\|)|^\\s*\\*\\*(?:proof|source|pool|method)', re.I)
    ...
    async def _lf_pass(question, answer, messages, ledger, deadline):
        ...
        return answer
    _vl_ASK = re.compile('exactly as (?:printed|written|spelled|it appears|given)|as printed in\\b|verbatim\\b|exact wording', re.I)
    _vl_Q = re.compile('"([^"\\n]{3,90})"')
    ...
    async def _vl_pass(question, answer, messages, ledger, deadline):
        ...
        if len(blob) < 200:
            return answer
        fold = re.sub('\\s+', '', blob).lower()
        out, ch = (answer, 0)
        for m in list(_vl_Q.finditer(answer))[:20]:
            val = m.group(1)
            key = re.sub('\\s+', '', val).lower()
            if not key:
                continue
            i = fold.find(key)
            if i < 0:
            ...
            if exact and exact != val and (len(exact) <= len(val) + 8):
                out = out.replace('"%s"' % val, '"%s"' % exact)
                ch += 1
            if ch >= 8:
                break
        return out if ch and _is_usable_answer(out) else answer
    _ns_RE = re.compile('\\bUNKNOWN\\b|\\bcannot (?:be )?(?:determined|identified|confirmed)|not (?:captured|present) in the (?:captured )?excerpts|\\bunable to\\b|no jurisdiction is verified', re.I)
    ...
    def _ns_default(v, d=0):
        ...
        if isinstance(v, (list, tuple, dict)):
            return len(v) == 0 or (d < 5 and all((_ns_default(x, d + 1) for x in (v.values() if isinstance(v, dict) else v))))
        return False
    ...
    async def _ns_pass(question, answer, messages, ledger, deadline):
        ...
        if deadline - monotonic() < 55.0 or _spend_left() <= AUDIT_MIN_USD:
            return answer
        ask = "Your draft surrendered, but the evidence below was already retrieved. Answer DIRECTLY from it. Do not output UNKNOWN, zeros, or 'cannot determine' unless the evidence is truly silent. Cite with [[n]].\n\nQUESTION:\n" + (question or '')[:1500] + '\n\nEVIDENCE:\n' + _quote_table(ledger)[:11000]
        try:
        ...
        if got and _is_usable_answer(got) and (not _ns_RE.search(got)):
            return got
        return answer
    _ss2_ASK = re.compile('\\bplain list\\b|\\bplain text\\b|\\bseparated by (?:commas|semicolons)\\b|\\banswer with (?:just|only)\\b', re.I)
    ...
    async def _ss2_pass(question, answer, messages, ledger, deadline):
        if not question or not answer or (not _ss2_ASK.search(question)):
            return answer
        t = answer
        t = re.sub('^\\s{0,3}#{1,6}\\s*', '', t, flags=re.M)
        t = re.sub('^\\s*\\|.*\\|\\s*$', '', t, flags=re.M)
        t = re.sub('^\\s*-{3,}\\s*$', '', t, flags=re.M)
        t = re.sub('\\n{3,}', '\n\n', t).strip()
