...
def _compose_cobalt_kestrel_agent_entry():
    ...
    VERSION = "v209-uid24x236-rcu"
    # w5 variant 09 — added stages: roster pre-pass, lead-figure corroboration, unit-consistency repair.
    # stage set RCU = uid 24 (RC) x uid 236 (RU)
...
def _compose_saffron_cairn_agent_entry():

    _S32U_QUERY_TAG = "s32u-hk6716"
    ...
    async def _s32_base_query(query: Query) -> Response:
        ...
        return response
    # --- w4 answer-contract wrapper (end) ---
    # slot: 01 FB_0f3a1c28_w4 2026-08-19T09:57:45+00:00
...
_saffron_cairn_agent_query_entry = _compose_saffron_cairn_agent_entry()


_BALANCED_ROUTER_SEED = "cb934ab98a58cfaa25fc7e63"
...
def _balanced_route_label(query: Query) -> str:
    ...
    return "CobaltKestrelAgent" if bucket < 128 else "SaffronCairnAgent"
...
class SaffronCairnAgent:
    async def __call__(self, query: Query) -> Response:
        return await _saffron_cairn_agent_query_entry(query)


_BALANCED_PRIMARY_AGENT = CobaltKestrelAgent()
_BALANCED_SECONDARY_AGENT = SaffronCairnAgent()
_CANDIDATE_BRANCH_CLASS_NAMES = ("CobaltKestrelAgent", "SaffronCairnAgent")
...
async def query(query: Query) -> Response:
    ...
    branch = (
        ...
        if selected == "CobaltKestrelAgent"
