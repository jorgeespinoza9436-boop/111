...
TURN_TIMEOUT_S = 75.0
# The pinned rung gets a shorter fuse than the rungs after it. The pin exists
# only to buy latency by steering to a fast upstream, so a pinned call still
# running at 45s has already failed at its one job -- the allowlist is congested,
# and waiting the remaining 30s is the worst of both outcomes. The rung it falls
# through to is the SAME model on the SAME lane with the pin removed, so cutting
# early costs a re-request rather than a lane change. Only applied while enough
# of the deadline survives to actually use that next rung.
...
VERSION = "v117-datum-source-exact-sym"
...
# Six was set when evidence arrived from many pages, so a cap per row was a cap
# per source. The task distribution moved: "using ONLY <one named source>" was 25
# of 30 questions in the newest batch, and there a whole roster -- every qualifier
# AND every member ruled out, each of which the rubric wants carrying its own [n]
# -- lands in a SINGLE row. Six excerpts cannot cite a nine-member pool, and the
# loop's own auditor names incomplete_roster as its most common loss. This was
# never the real budget guard either: at ~620 chars per excerpt, sixteen rows'
# worth is under a tenth of EVIDENCE_CHAR_BUDGET, and _citations_for already
# enforces the actual wall.
...
# A separate, far lower floor for spans the model CHOSE by quoting them. The
# 6000 floor exists for raw search excerpts, where nothing has identified which
# sentence matters and surrounding context is what makes the citation checkable.
# A retained span is the opposite case: it is already centred on the quote the
# model offered as proof, with RETAIN_MARGIN_CHARS of context either side, so
# padding it to 6000 adds ~5.4k characters that argue for nothing.
#
# That padding is not free. In batch 81b84664 the champion answered task
# 8788381c with output BYTE-IDENTICAL to the reference answer and scored 0.0,
# losing BOTH swapped judge runs on citation quality alone -- the judge called
# its slice "very long and messy", "a huge slice including the table", "a
# massive dump", and preferred the reference for citations that "directly target
# the relevant slices". Identical answer, zero score, purely for burying the
# proof. The floor below keeps a table row plus its header and neighbours, which
# is what makes a quote verifiable, and stops well short of the whole table.
...
class EvidenceLedger:
    ...
    def ref_for(self, number: int) -> CitationRef | None:
        ...
        if spans:
            ...
            floor = (CITATION_RETAINED_MIN_SPAN_CHARS if was_retained
...
# The tools that actually leave the machine. Everything else reads the ledger.
_PRODUCER_TOOLS = frozenset(("web_search", "read_page", "sec_filing"))
MAX_NETWORK_CALLS_PER_TURN = 8
# A ceiling on the local calls too, purely so a model that loops cannot stall a
# turn. It sits well above the largest roster a question realistically ranges
# over, so it should not bind in practice.
...
async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool,
    ...
    for lane_model in ((LLM_LANE_A, LOOP_MODEL_A, True),
        ...
        timeout = min(TURN_TIMEOUT_S, deadline - monotonic() - 5.0,
                      turn_wall - monotonic())
        if pinned and timeout > PINNED_TURN_TIMEOUT_S \
                and (deadline - monotonic()) > PINNED_RETRY_MIN_LEFT_S:
...
async def _loop(question: str, brief: str, ledger: EvidenceLedger,
    ...
    for turn in range(1, turn_cap + 1):
        ...
        # The per-turn cap exists to bound NETWORK concurrency, and only three of
        # the six tools reach the network. retain_evidence, page_grep and
        # page_read all read text the ledger already holds, so running one costs
        # a dict lookup. Counting those against the same eight is what caps a
        # roster answer: the model does exactly what SET_RULE asks and issues one
        # retain_evidence per pool member, and every call past the eighth comes
        # back "per-turn tool budget reached" despite there being no budget being
        # spent. Cap the fetches; let the local reads run. Every tool_call id
        # still gets a reply -- an unanswered id fails transcript validation.
        run_calls = []
        skipped_calls = []
        network_calls = 0
        for call in calls:
...
# ── source-exact projection ──────────────────────────────────────────────────
# Getting every fact right is not enough to win a task. Scoring is a pairwise
# judgement against a reference answer, run twice with the positions swapped,
# and on task 3c305fcc of batch 81b84664 the champion produced every value
# correctly, cited every one, and scored 0.0 in BOTH runs. The judge's stated
# reasons were that it wrote "Spiral Galaxy" where the bulletin prints "Spiral
# Galaxy Stamp", and "Big Sky, Montana" where the bulletin prints "Big Sky, MT".
# The questions ask for values "exactly as printed", and a model that has read
# the page still paraphrases it: it drops a trailing noun it reads as a label,
# and it expands abbreviations because expanding them normally reads better.
#
# _verbatim_from_source above was meant to cover this but only fires on the
# gloss shape "X (Y)", so a bare field value is never compared with the page at
# all. These two rules cover the observed losses, and both obey one invariant:
# a replacement is only ever made to a string that literally occurs in a cited
# source. The output can therefore only become MORE grounded, never less, and
# when no source form is found the value is returned untouched.
_STATE_ABBR = {
...
_SOURCE_EXACT_MAX_CHARS = 90
_LABELLED_FIELD_RE = re.compile(
    r"^[\s>*_`|-]*(?P<key>[A-Z][A-Za-z0-9 &/'()-]{1,40}?)\s*[:|]\s*(?P<val>\S.*?)\s*$"
)
...
def _source_texts(ledger: EvidenceLedger) -> list[str]:
    return [r.get("text") or "" for r in ledger.rows if r.get("text")]


def _source_exact(value: str, ledger: EvidenceLedger) -> str:
    """Project a field value onto the exact wording its source prints."""
    v = (value or "").strip()
    if not v or len(v) > _SOURCE_EXACT_MAX_CHARS:
        return value
    texts = _source_texts(ledger)
    if not texts:
        return value
    # No early "it already appears" check: a substring hit does not mean the
    # value is the complete printed form. "Spiral Galaxy" IS inside "_Spiral
    # Galaxy_ Stamp", and returning on that is exactly how the truncation
    # survives. The scan below decides instead -- if the value is ever printed
    # standalone, bare_seen records it and the value is kept unchanged.

    # Rule A: the value sits inside a LABELLED field and the model truncated it.
    # "Issue: _Spiral Galaxy_ Stamp" asked for the issue name, and the answer
    # dropped the trailing noun. Scoping to labelled lines is what makes this
    # safe: an unscoped "extend by the next word" rule would read the table row
    # "Piedras Very Serious Marine Casualty" and rewrite the vessel name
    # "Piedras" to "Piedras Very", corrupting a value that was already correct.
    # A label is also precisely what the questions point at when they ask for a
    # value "exactly as printed in the ... line".
    extended: str | None = None
    for text in texts:
        low_v, low_t = v.lower(), text.lower()
        at = low_t.find(low_v)
        while at >= 0:
            line_start = text.rfind("\n", 0, at) + 1
            line_end = text.find("\n", at)
            line = text[line_start:line_end if line_end > 0 else len(text)]
            match = _LABELLED_FIELD_RE.match(line)
            if match:
                printed = _strip_emphasis(match.group("val"))
                if (printed != v and len(printed) <= _SOURCE_EXACT_MAX_CHARS
                        and printed.lower().startswith(v.lower())):
                    if extended is None:
                        extended = printed
                    elif extended != printed:
                        return value  # the page is inconsistent; do not guess
            at = low_t.find(low_v, at + len(low_v))
    if extended:
        return extended

    # Rule B: the value and the page disagree about how a US state is written.
    # Both directions occur. The model expands "MT" to "Montana" because that
    # reads better, and it also abbreviates a spelled-out state to keep a field
    # short. Either way the page's own form is the one being asked for, so try
    # each substitution and keep whichever the page actually prints. A state
    # code is matched on word boundaries: a bare "in" or "or" inside a word
    # must never be mistaken for Indiana or Oregon.
    lowered = v.lower()
    for name, code in _STATE_ABBR.items():
        if name in lowered:
            swapped = re.sub(re.escape(name), code, v, count=1, flags=re.I)
            if swapped != v and any(swapped in text for text in texts):
                return swapped
        code_re = re.compile(rf"\b{code}\b")
        if code_re.search(v):
            spelled = name.title()
            swapped = code_re.sub(spelled, v, count=1)
            if swapped != v and any(swapped in text for text in texts):
                return swapped
    return value
...
def _verbatim_structured(obj, ledger: EvidenceLedger, depth: int = 0):
    ...
    if isinstance(obj, str):
        return _source_exact(_verbatim_from_source(obj, ledger), ledger)
