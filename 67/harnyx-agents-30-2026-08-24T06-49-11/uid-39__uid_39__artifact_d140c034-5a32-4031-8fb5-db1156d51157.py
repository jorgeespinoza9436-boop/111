...
def _compose_basalt_lantern_agent_entry():
...
_basalt_lantern_agent_query_entry = _compose_basalt_lantern_agent_entry()


def _compose_amber_anvil_agent_entry():
    ...
    """
        ...
        from harnyx_miner_sdk.decorators import entrypoint
        from harnyx_miner_sdk.query import CitationRef, CitationSlice, Query, Response
        VERSION = 'v53-openrouter-authority'
        LLM_LANE_A = 'openrouter'
        LLM_LANE_B = 'openrouter'
        LOOP_MODEL_A = 'z-ai/glm-5.2'
        LOOP_MODEL_B = 'z-ai/glm-5'
        ...
        class AggregateMarkerAuditor_9e292c:
        ...
        """
            BLOCK_RESERVE_9E292C = 12000
            SPAN_MARGIN_9E292C = 96
            WINDOW_PADDING_9E292C = 3

            def bracket_coverage_9e292c(self, spans, limit=0):
            ...
            """
                ceiling = int(limit or self.BLOCK_RESERVE_9E292C)
                cleaned = []
            ...
            def gather_regions_9e292c(self, spans, note_len, pad=0):
                """Widen merged windows by a fixed margin without crossing the note."""
                room = max(0, int(pad or self.SPAN_MARGIN_9E292C))
                total = max(0, int(note_len or 0))
                widened = []
                for window in self.bracket_coverage_9e292c(spans, total or self.BLOCK_RESERVE_9E292C):
            ...
            def collate_intervals_9e292c(self, rows, note_len=0):
                ...
                for row in rows or []:
                    ...
                    if not isinstance(length, (int, float)) or isinstance(length, bool):
                        length = note_len
                    for window in self.gather_regions_9e292c(spans, int(length or 0)):
            ...
            def normalize_regions_9e292c(self, rows, note_len=0):
                ...
                for row in rows or []:
                    ...
                    if not isinstance(row, dict):
                        continue
                    stats = self.collate_intervals_9e292c([row], note_len)
                    if stats['windows'] < 1:
                        continue
                    label = row.get('title', '')
                ...
                if len(report) < self.WINDOW_PADDING_9E292C:
...
_amber_anvil_agent_query_entry = _compose_amber_anvil_agent_entry()


_SHAPE_ROUTER_SEED = "ea05989eae4f408df77c0678"
...
def _balanced_route_label(query: Query) -> str:
    ...
    order = ("TidalQuillAgent", "BasaltLanternAgent", "AmberAnvilAgent")
    if shape == 3:
        return order[bucket]
...
class BasaltLanternAgent:
    async def __call__(self, query: Query) -> Response:
        return await _basalt_lantern_agent_query_entry(query)


class AmberAnvilAgent:
    async def __call__(self, query: Query) -> Response:
        return await _amber_anvil_agent_query_entry(query)


_SHAPE_PRIMARY_AGENT = TidalQuillAgent()
_SHAPE_SECONDARY_AGENT = BasaltLanternAgent()
_SHAPE_TERTIARY_AGENT = AmberAnvilAgent()
...
async def query(query: Query) -> Response:
    ...
    if selected == "BasaltLanternAgent":
        try:
            return await _SHAPE_SECONDARY_AGENT(query)
        except Exception:
