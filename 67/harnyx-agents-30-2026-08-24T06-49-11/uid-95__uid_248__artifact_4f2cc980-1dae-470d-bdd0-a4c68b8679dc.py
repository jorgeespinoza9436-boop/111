...
_HX67RB_SINGLE_RE = re.compile(r"(?<!\[)\[(\d{1,3})\](?!\])")
_HX67RB_DOUBLE_RE = re.compile(r"\[\[(\d{1,3})\]\]")
_HX67RB_YEAR_RE = re.compile(r"\b(?:19|20)\d{2}\b")
_HX67RB_COMPARE_RE = re.compile(
    r"\b(?:compar(?:e|ison)|versus|\bvs\.?\b|differ(?:ence|s)?|reconcile|"
    r"which (?:is|company|entity|value) (?:higher|lower|larger|greater|smaller)|"
    r"both .+ and|independent[- ]source|period bases?|official .+ independent|"
    r"agree on|what (?:detail|obligation|figure) differs)\b",
    re.I,
)
_HX67RB_TIME_RE = re.compile(
    r"\b(?:as of|currently|latest|most recent|effective date|fiscal|"
    r"calendar year|period ending|as at|jurisdiction|version|effective)\b",
    re.I,
)
_HX67RB_PREMISE_RE = re.compile(
    r"\b(?:used to be|former|formerly|originally|was known as|renamed|"
    r"no longer|previously|is it true that|isn't|wasn't|dropped|never)\b",
    re.I,
)
_HX67RB_CALC_RE = re.compile(
    r"\b(?:how many|how much|total|difference|percentage|percent|sum of|"
    r"average|ratio|combined|product of|divided by|normalize)\b",
    re.I,
)
_HX67RB_POOL_RE = re.compile(
    r"\b(?:list (?:every|all)|every member|complete list|ranking|highest|"
    r"lowest|which entries|which of the|every entry|roster|standings)\b",
    re.I,
)
_HX67RB_REWRITE_SYSTEM = (
    "You close a live dual-source reconciliation board around an already-produced "
    "research draft. Return JSON only with keys text (string) and cite_indexes "
    "(integer array). The numbered board rows are independently retrieved "
    "official/primary evidence and independent/contemporaneous evidence. Do not "
    "invent facts. Grounding beats completeness. Keep every verified name, date, "
    "figure, and entity from the draft unless the board proves a correction. "
    "Cover every query-required element the board actually supports. Comparison "
    "and synthesis queries must state each side and an explicit reconciled "
    "conclusion on matching period, basis, and jurisdiction. If official and "
    "independent sources disagree, name each scope and the residual difference; "
    ...
    "pool queries must name the in-scope set and the decisive exclusions the "
    "board supports. First sentence is the direct answer; no preamble. Use "
    "Markdown only when it lowers reader effort. Every material researched claim "
    "must carry a [[n]] pointer: n is 1-based into the combined citation list "
    "(existing citations first, then board rows in the user payload). Do not use "
    "bare [n]. Do not write Supports:, Claim:, evidence IDs, or fake source lists. "
    "cite_indexes are 0-based indexes of numbered board rows that directly support "
    "answer-visible claims; at most 8. If the query asks to output only the "
    "answer, keep that exact form on the first line and put [[n]] pointers in a "
    "short proof section below it. Structured-looking JSON is not allowed unless "
    "the query itself is already a prose answer."
)
...
def _hx67rb_core_terms(question: str) -> str:
    tokens = re.findall(r"[A-Za-z][A-Za-z0-9\-']{2,}|[0-9]{4}", question or "")
    stop = {
        ...
        "between", "against", "among", "under", "over", "official", "report",
        "source", "according", "please", "could", "would", "return", "names",
    }
...
def _hx67rb_open_queries(question: str) -> list[str]:
    ...
    queries = [
        f"{core} official filing OR announcement OR primary source",
        f"{core} independent contemporaneous report OR coverage",
    ]
...
def _hx67rb_official_rank(url: str, title: str) -> int:
    ...
    for token in (
        ...
        "sec.gov", "federalregister", "legislation",
    ):
        if token in blob:
            score += 3
...
def _hx67rb_empty_board(question: str) -> dict:
    return {
        "question": question,
        "regimes": _hx67rb_regimes(question),
        "packets": [],
        "rows": [],
        "digest": "",
        "lanes": set(),
        ...
        rows.append(
            {
                ...
                "title": title,
                "note": note,
                "rank": _hx67rb_official_rank(url, title),
            }
        )
...
async def _hx67rb_search(query_text: str, timeout: float):
    try:
        return await search_web(
            query_text,
            provider=_HX67RB_SEARCH_PROVIDER,
            num=6,
...
def _hx67rb_needs_cycle(question: str, draft: str, board: dict) -> bool:
    if not draft or not board.get("rows"):
        ...
        return bool(draft) and (
            _hx67rb_has_pointer_defect(draft) or not _HX67RB_DOUBLE_RE.search(draft)
        )
...
def _hx67rb_merge_citations(existing, rows: list[dict], cite_indexes: list[int]):
    ...
    seen = {
        (getattr(cite, "receipt_id", None), getattr(cite, "result_id", None))
        for cite in merged
    }
...
async def _hx67rb_fetch_pair(board: dict, deadline: float) -> None:
    ...
    async def _one(row):
        ...
        if not isinstance(url, str) or not url:
            return None, row
        try:
            packet = await fetch_page(
                url,
                provider=_HX67RB_FETCH_PROVIDER,
                timeout=timeout,
            )
        except Exception:
            packet = None
    ...
    for task in tasks:
        ...
        board["rows"] = list(board.get("rows") or []) + [
            {
                "packet": packet,
                "item": item,
                "lane": lane,
...
async def _hx67rb_rewrite(
    ...
    user = json.dumps(
        {
            "query": _hx67rb_clip(question, 4000),
            "prior_draft": _hx67rb_clip(draft, 8000),
            "regimes": board.get("regimes") or [],
            "gap_queries_used": board.get("gap_queries") or [],
            "citation_map": {
                "existing_citations": f"[[1]]..[[{existing_n}]]" if existing_n else "none",
                "board_rows_start": existing_n + 1,
            },
            "dual_source_board": _hx67rb_clip(board.get("digest"), 14000),
            "work_order": (
                "Consume the live dual-source reconciliation board. The board was "
                "updated after the draft by a gap ledger that triggered targeted "
                "retrieval. Cite official/primary and independent/contemporaneous "
                "rows for every load-bearing claim. Reconcile period/basis/"
                "jurisdiction disagreement. Cover every required comparison side "
                "and the conclusion. Name pool members and exclusions the board "
                "supports. Convert every material claim to [[n]] pointers."
            ),
        },
        ensure_ascii=False,
    )
    try:
        turn = await llm_chat(
            provider=_HX67RB_LLM_PROVIDER,
            model=_HX67RB_LLM_MODEL,
            messages=(
                {"role": "system", "content": _HX67RB_REWRITE_SYSTEM},
                {"role": "user", "content": user},
            ),
            temperature=0.0,
            max_output_tokens=1400,
            timeout=min(_HX67RB_REWRITE_TIMEOUT_S, max(8.0, left - 2.0)),
        )
    except Exception:
        return None
...
# slot: 04 FB_820d790c_w4 2026-08-20T13:10:05+00:00


async def _crb_base_query(query: Query) -> Response:
    started = _hx67rb_now()
    ...
    try:
        ...
        return Response(
            text=(
                ...
                if not question
                else f"No verifiable source-backed answer was reached for: {question[:500]}"
            )
        )
...
# ======================================================================== #
# refactor mechanism: r70-corroboration-board  (self-contained cross-stage cycle wrapper)  #
# ======================================================================== #
import asyncio as _crb_asyncio
import json as _crb_json
import re as _crb_re
from time import monotonic as _crb_monotonic
from urllib.parse import urlparse as _crb_urlparse

from harnyx_miner_sdk.api import fetch_page as _crb_fetch_page
from harnyx_miner_sdk.api import llm_chat as _crb_llm_chat
from harnyx_miner_sdk.api import search_web as _crb_search_web
from harnyx_miner_sdk.query import CitationRef as _crbCitationRef
from harnyx_miner_sdk.query import CitationSlice as _crbCitationSlice
from harnyx_miner_sdk.query import Response as _crbResponse

_crb_PROVIDER = "openrouter"
_crb_AUDIT_MODEL = "openai/gpt-oss-120b"
_crb_WRITE_MODEL = "z-ai/glm-5.2"
_crb_SEARCH_PROVIDERS = ("parallel", "desearch")
_crb_HARD_S = 292.0          # sandbox kill is 300s; never cross this
_crb_MIN_CYCLE_S = 16.0      # skip the whole cycle unless this much wall remains
_crb_CYCLE_CAP_S = 30.0
_crb_AUDIT_TIMEOUT_S = 12.0
_crb_SEARCH_TIMEOUT_S = 10.0
_crb_FETCH_TIMEOUT_S = 9.0
_crb_WRITE_TIMEOUT_S = 16.0
_crb_CITATION_CAP = 190
_crb_TEXT_CAP = 78000
_crb_PTR_RE = _crb_re.compile(r"\[\[(\d+)\]\]")
_crb_SINGLE_RE = _crb_re.compile(r"(?<!\[)\[(\d{1,3})\](?!\])")
_crb_STOP = frozenset((
    "the", "and", "for", "that", "with", "from", "this", "what", "which",
    ...
    "please", "could", "would", "return", "using", "only", "name", "names",
))


def _crb_now() -> float:
    return _crb_monotonic()


def _crb_clip(value: object, limit: int) -> str:
    if not isinstance(value, str):
        return ""
    text = value.strip()
...
def _crb_core_terms(question: str) -> str:
    tokens = _crb_re.findall(r"[A-Za-z][A-Za-z0-9\-']{2,}|[0-9]{4}", question or "")
    salient = [t for t in tokens if t.casefold() not in _crb_STOP][:9]
    core = " ".join(salient).strip()
    return core or _crb_clip(question, 160)


async def _crb_llm(system: str, user: str, model: str, max_tokens: int, timeout: float) -> str:
    try:
        payload = await _crb_llm_chat(
            provider=_crb_PROVIDER,
            model=model,
            messages=[
...
def _crb_parse_json(raw: object):
    if not isinstance(raw, str) or not raw.strip():
        return None
    body = raw.strip()
    if body.startswith("```"):
    ...
    for opener, closer in (("{", "}"), ("[", "]")):
        ...
        if 0 <= start < end:
            try:
                return _crb_json.loads(body[start:end + 1])
            except (ValueError, TypeError):
                continue
    return None


def _crb_item_note(item) -> str:
    value = getattr(item, "note", None)
    if isinstance(value, str) and value.strip():
        return value.strip()
    value = getattr(item, "snippet", None)
...
def _crb_item_title(item) -> str:
    value = getattr(item, "title", None)
    return value.strip() if isinstance(value, str) else ""


def _crb_domain(url: str) -> str:
    try:
        host = _crb_urlparse(url).netloc.lower()
    except Exception:
        return ""
    return host[4:] if host.startswith("www.") else host


def _crb_cite_from_pack(pack: object, limit: int) -> tuple[list, list]:
    refs: list = []
    lines: list = []
    ...
    for row in rows[:limit]:
        ...
        if not receipt or not result_id:
            continue
        note = _crb_item_note(row)
        title = _crb_item_title(row)
        url = _crb_item_url(row)
        slices = []
        if note:
            end = min(len(note), 1200)
            if end > 0:
                slices.append(_crbCitationSlice(start=0, end=end))
        refs.append(_crbCitationRef(receipt_id=str(receipt), result_id=str(result_id), slices=slices))
        excerpt = " ".join(note.split())[:900]
        lines.append({"title": title, "url": url, "domain": _crb_domain(url), "excerpt": excerpt})
    return refs, lines


async def _crb_search(query_text: str, num: int) -> tuple[list, list, str]:
    for provider in _crb_SEARCH_PROVIDERS:
        try:
            pack = await _crb_search_web(query_text, provider=provider, num=num,
                                          timeout=_crb_SEARCH_TIMEOUT_S)
        except Exception:
            continue
        refs, lines = _crb_cite_from_pack(pack, num)
        url = ""
        rows = list(getattr(pack, "results", None) or [])
        if rows:
            url = _crb_item_url(rows[0])
        return refs, lines, url
    return [], [], ""


async def _crb_fetch(url: str) -> tuple[list, list]:
    if not url:
        return [], []
    for provider in _crb_SEARCH_PROVIDERS:
        try:
            pack = await _crb_fetch_page(url, provider=provider, timeout=_crb_FETCH_TIMEOUT_S)
        except Exception:
            continue
        refs, lines = _crb_cite_from_pack(pack, 2)
        if not refs:
            resp = getattr(pack, "response", None)
            note = _crb_item_note(resp) if resp is not None else ""
            receipt = getattr(pack, "receipt_id", None)
            rid = getattr(resp, "result_id", None)
            if note and receipt and rid:
                end = min(len(note), 1200)
                refs = [_crbCitationRef(receipt_id=str(receipt), result_id=str(rid),
                                         slices=[_crbCitationSlice(start=0, end=end)] if end else [])]
                lines = [{"title": _crb_item_title(resp), "url": url,
                          "domain": _crb_domain(url), "excerpt": " ".join(note.split())[:900]}]
        return refs, lines
    return [], []


def _crb_merge_refs(old, extra):
    merged = []
    seen = set()
    for item in list(old or []) + list(extra or []):
        ...
        seen.add(key)
        merged.append(item)
        if len(merged) >= _crb_CITATION_CAP:
            break
    return merged or None


def _crb_draft_blob(resp) -> tuple[str, str]:
    out = getattr(resp, "output", None)
    if out is not None:
        try:
            blob = _crb_json.dumps(out, ensure_ascii=False, indent=2)
        except Exception:
            blob = str(out)
...
def _crb_usable(text: str) -> bool:
    t = (text or "").strip()
    if len(t) < 24:
        return False
    low = t.casefold()
    return not low.startswith(("best-effort answer unavailable", "no verifiable",
                               "i cannot", "i can't", "unable to", "sorry"))


def _crb_cap(text: str) -> str:
    t = (text or "").strip()
    return t if len(t) <= _crb_TEXT_CAP else t[:_crb_TEXT_CAP - 4] + " ..."


def _crb_renumber(text: str, old_n: int, extra_n: int) -> str:
    if extra_n <= 0:
        return text
    ...
    return _crb_PTR_RE.sub(_swap, text)


def _crb_accept_prose(resp, rewritten: str, merged, draft: str):
    rewritten = (rewritten or "").strip()
    if not _crb_usable(rewritten):
        if merged is None:
            return resp
        try:
            return _crbResponse(text=getattr(resp, "text", None) or draft, citations=merged)
        except Exception:
            return resp
    if len(rewritten) < int(len(draft) * 0.45) and draft:
        try:
            return _crbResponse(text=getattr(resp, "text", None) or draft, citations=merged)
        except Exception:
            return resp
    try:
        return _crbResponse(text=_crb_cap(rewritten), citations=merged)
    except Exception:
        return resp


def _crb_accept_structured(resp, rewritten: str, merged, base_output):
    parsed = _crb_parse_json(rewritten)
    keep = base_output
    if isinstance(base_output, dict) and isinstance(parsed, dict):
        ...
        keep = parsed
    try:
        return _crbResponse(output=keep, citations=merged)
    except Exception:
        return resp


async def _crb_claims(question: str, draft: str, timeout: float) -> list:
    user = (
        "List the material, checkable factual claims the draft makes (numbers, dates, "
        "names, yes/no determinations). JSON only: {\"claims\": [{\"claim\": short text, "
        "\"terms\": [key terms to search]}]}. At most 5, most load-bearing first.\n"
        f"QUESTION:\n{question}\n\nDRAFT:\n{(draft or '')[:9000]}"
    )
    parsed = _crb_parse_json(await _crb_llm(
        "You extract checkable claims for corroboration. JSON only.", user,
        _crb_AUDIT_MODEL, 900, timeout))
    claims = []
    if isinstance(parsed, dict):
        for item in (parsed.get("claims") or []):
            ...
            claims.append({"claim": text, "terms": terms[:5] or text.split()[:5]})
    return claims[:5]


async def _crb_standing(question: str, claims: list, deadline: float) -> tuple[list, dict]:
    core = _crb_core_terms(question)
    board = {}
    refs = []
    for claim in claims[:3]:
        if (deadline - _crb_now()) < 9.0:
            break
        query_text = f"{core} " + " ".join(claim["terms"][:4])
        fr, lines, _ = await _crb_search(query_text.strip(), 5)
        refs.extend(fr)
        low_terms = [t.casefold() for t in claim["terms"]]
        domains = set()
        for ln in lines:
            excerpt = (ln.get("excerpt") or "").casefold()
            if any(t in excerpt for t in low_terms):
                dom = ln.get("domain")
                if dom:
                    domains.add(dom)
        board[claim["claim"]] = sorted(domains)
    return refs, board


async def _crb_rewrite(question, draft, mode, schema, board, old_n, timeout):
    strong = [c for c, d in board.items() if len(d) >= 2]
    weak = [c for c, d in board.items() if len(d) < 2]
    note = ("Corroborated by >=2 independent domains: " + "; ".join(strong[:6]) + "\n"
            "Single-source / uncorroborated (hedge or drop): " + "; ".join(weak[:6]))
    if mode == "structured":
        system = ("Rewrite the structured answer as JSON only matching the public output schema. "
                  "Keep well-corroborated values; hedge or remove single-source claims. No markdown.")
    else:
        system = ("Rewrite the answer using the corroboration board. State well-corroborated "
                  "claims plainly with [[n]] pointers; explicitly hedge or drop claims backed by "
                  "a single source. Open with the resolved answer. Reconcile any conflict between "
                  f"independent sources. [[1]]..[[{old_n}]] stay valid; fresh rows map to "
                  f"[[{old_n + 1}]] onward.")
    schema_note = ""
    if schema is not None:
        try:
            schema_note = _crb_json.dumps(schema, ensure_ascii=False)[:3500]
        except Exception:
            schema_note = str(schema)[:3500]
    user = (f"QUESTION:\n{question}\n\nCORROBORATION BOARD:\n{note}\n\n"
            f"CURRENT DRAFT:\n{(draft or '')[:9000]}\n")
    if schema_note:
        user += f"\nOUTPUT_SCHEMA:\n{schema_note}\n"
    user += "\nReturn only the rewritten answer."
    return await _crb_llm(system, user, _crb_WRITE_MODEL, 3200, timeout)


async def _crb_cycle(query, resp, t0):
    draft, mode = _crb_draft_blob(resp)
    if mode == "empty" or not _crb_usable(draft):
        return resp
    remaining = _crb_HARD_S - (_crb_now() - t0)
    if remaining < _crb_MIN_CYCLE_S:
        return resp
    deadline = _crb_now() + min(_crb_CYCLE_CAP_S, remaining - 4.0)
    question = getattr(query, "text", "") or ""
    schema = getattr(query, "output_schema", None)
    claims = await _crb_claims(question, draft,
                                min(_crb_AUDIT_TIMEOUT_S, max(7.0, deadline - _crb_now() - 17.0)))
    if not claims or (deadline - _crb_now()) < 12.0:
        return resp
    extra_refs, board = await _crb_standing(question, claims, deadline)
    if not board:
        return resp
    weak = [c for c, d in board.items() if len(d) < 2]
    if not weak:
        # everything already corroborated by >=2 independent domains: keep base,
        # only enrich the citation set with the corroborating rows
        merged = _crb_merge_refs(list(getattr(resp, "citations", None) or []), extra_refs)
        if mode == "structured":
            try:
                return _crbResponse(output=getattr(resp, "output", None), citations=merged)
            except Exception:
                return resp
        try:
            return _crbResponse(text=getattr(resp, "text", None) or draft, citations=merged)
    ...
    old_n = len(old_refs)
    rewritten = await _crb_rewrite(question, draft, mode, schema, board, old_n,
                                    min(_crb_WRITE_TIMEOUT_S, max(9.0, deadline - _crb_now() - 0.5)))
    merged = _crb_merge_refs(old_refs, extra_refs)
    if mode == "structured":
        return _crb_accept_structured(resp, rewritten, merged, getattr(resp, "output", None))
    extra_n = max(0, len(merged or []) - old_n)
    rewritten = _crb_renumber((rewritten or "").strip(), old_n, extra_n)
    return _crb_accept_prose(resp, rewritten, merged, draft)
...
async def query(query: Query) -> Response:
    _t0 = _crb_now()
    try:
        resp = await _crb_base_query(query)
    except Exception:
        _q = (getattr(query, "text", "") or "").strip()
        try:
            return _crbResponse(text="No verifiable source-backed answer was reached"
                                 + (f" for: {_q[:400]}" if _q else "."))
        except Exception:
            return _crbResponse(text="No verifiable source-backed answer was reached.")
    try:
        return await _crb_asyncio.wait_for(_crb_cycle(query, resp, _t0),
                                            timeout=_crb_CYCLE_CAP_S)
    except Exception:
        return resp

# --- build marker 20464efa -------------------------------------------------
_BUILD_MARKER_20464efa = "20260823T000000Z"


def _build_probe_20464efa(seed: int = 0) -> int:
    """Unreferenced helper carrying the build identity."""
    acc = seed
    for i, ch in enumerate(_BUILD_MARKER_20464efa):
        acc = (acc * 31 + ord(ch) + i) % 1000003
    return acc


class _BuildStamp_20464efa:
    tag = _BUILD_MARKER_20464efa

    def digest(self) -> int:
        return _build_probe_20464efa(len(self.tag))
