...
def _build_agent_1():
    ...
    def _value_support_table(question: str, answer: str, ledger: EvidenceLedger) -> _ValueSupportTable:
    ...
    async def _value_repair(question: str, answer: str, messages: list, ledger: EvidenceLedger, deadline: float) -> str:
        ...
        body = await _w5_targeted_search(_w5_search_subject(question) + ' ' + named[0], ledger)
        order = 'VALUE SUPPORT: these figures appear in your answer but appear in NO retrieved source:\n- ' + '\n- '.join(named) + '\nFor each one: either cite the source that states it with its [n], or remove the figure and rewrite the sentence around what the evidence does support. Do NOT restate an unsupported figure with a hedge — an uncited number scores as a fabrication, a stated absence does not. Keep every entity, every other figure and every existing citation, then rewrite the COMPLETE final answer in the required shape.'
    ...
    _W5U_DEMANDS = (('USD', '\\b(?:in|to)\\s+(?:usd|us dollars|dollars)\\b|\\bus\\$', ('\\$', '\\busd\\b', '\\bdollars?\\b')), ('EUR', '\\b(?:in|to)\\s+(?:eur|euros?)\\b', ('€', '\\beur\\b', '\\beuros?\\b')), ('millions', '\\bin millions\\b|\\bmillions of\\b', ('\\bmillions?\\b', '\\bm\\b')), ('billions', '\\bin billions\\b|\\bbillions of\\b', ('\\bbillions?\\b', '\\bbn\\b', '\\bb\\b')), ('percent', '\\bpercent(?:age)?\\b|\\bwhat share\\b|\\bwhat proportion\\b', ('%', '\\bpercent\\b')), ('per capita', '\\bper capita\\b|\\bper person\\b|\\bper head\\b', ('\\bper capita\\b', '\\bper person\\b')), ('metric', '\\bin (?:kilometres|kilometers|km|kilograms|kg|metres|meters|tonnes)\\b', ('\\bkm\\b', '\\bkg\\b', '\\bkilomet', '\\bkilogram', '\\bmet(?:re|er)s?\\b', '\\btonnes?\\b')), ('imperial', '\\bin (?:miles|pounds|lbs|feet|ft|inches)\\b', ('\\bmiles?\\b', '\\bpounds?\\b', '\\blbs?\\b', '\\bfeet\\b', '\\bft\\b', '\\binch')))
    ...
    async def _unit_repair(question: str, answer: str, messages: list, ledger: EvidenceLedger, deadline: float) -> str:
    ...
    """
        ...
        if deadline - monotonic() < _W5U_MIN_TAIL_S or _spend_left() <= WRAPUP_MIN_USD:
            return answer
        order = 'UNIT CONSISTENCY: the question demands the answer be expressed in ' + ', '.join(demand.missing) + ' and no figure in the answer carries it. Restate every headline figure in the demanded unit or scale. Show the conversion basis where you converted (rate, year, source) and cite it. Never silently relabel a figure into a unit its source did not use. Keep every named entity and every existing citation, then rewrite the COMPLETE final answer in the required shape.'
