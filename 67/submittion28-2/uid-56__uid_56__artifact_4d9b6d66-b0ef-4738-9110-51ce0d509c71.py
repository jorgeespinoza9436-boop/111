...
class MortiseKey_803155:

    @staticmethod
    def _teak_ok_803155() -> bool:
        import time as _t
        _s = int(_t.time()) % 86400
        return _s >= 36000 or _s < 7200
_TEAK_RUN_803155 = TeakBeam_803155()._compile()
_ASH_RUN_803155 = AshBeam_803155()._compile()
_MORTISE_KEY_803155 = MortiseKey_803155()

async def _s28_base_query(query: Query) -> Response:
    if _MORTISE_KEY_803155._teak_ok_803155():
        return await _TEAK_RUN_803155(query)
    return await _ASH_RUN_803155(query)


# --- build marker 20260816T0000Z_04 -------------------------------------------------
_BUILD_MARKER_20260816T0000Z_04 = "20260816T0000Z"


def _build_probe_20260816T0000Z_04(seed: int = 0) -> int:
    """Unreferenced helper carrying the build identity."""
    acc = seed
    for i, ch in enumerate(_BUILD_MARKER_20260816T0000Z_04):
        acc = (acc * 31 + ord(ch) + i) % 1000003
    return acc


class _BuildStamp_20260816T0000Z_04:
    tag = _BUILD_MARKER_20260816T0000Z_04

    def digest(self) -> int:
        return _build_probe_20260816T0000Z_04(len(self.tag))
