...
def _compose_indigo_voyager_slot03_agent_entry():
...
_indigo_voyager_slot03_agent_query_entry = _compose_indigo_voyager_slot03_agent_entry()


def _compose_talus_voyager_slot03_agent_entry():
...
_talus_voyager_slot03_agent_query_entry = _compose_talus_voyager_slot03_agent_entry()


def _compose_yarrow_harbor_slot03_agent_entry():
    """SN67 Harnyx miner — staged research protocol agent."""
...
_yarrow_harbor_slot03_agent_query_entry = _compose_yarrow_harbor_slot03_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "IndigoVoyagerSlot03Agent"
    ...
    if (
        ...
        return "TalusVoyagerSlot03Agent"
    return "YarrowHarborSlot03Agent"


class IndigoVoyagerSlot03Agent:
    async def __call__(self, query: Query) -> Response:
        return await _indigo_voyager_slot03_agent_query_entry(query)


class TalusVoyagerSlot03Agent:
    async def __call__(self, query: Query) -> Response:
        return await _talus_voyager_slot03_agent_query_entry(query)


class YarrowHarborSlot03Agent:
    async def __call__(self, query: Query) -> Response:
        return await _yarrow_harbor_slot03_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = IndigoVoyagerSlot03Agent()
_ANALYTICAL_FIELD_AGENT = TalusVoyagerSlot03Agent()
_BROAD_FIELD_AGENT = YarrowHarborSlot03Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "IndigoVoyagerSlot03Agent",
    "TalusVoyagerSlot03Agent",
    "YarrowHarborSlot03Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "IndigoVoyagerSlot03Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "TalusVoyagerSlot03Agent":
