...
_tyefjxndrq = {'started': None, 'text': None}
_sofnvvxwka = 24000
_lrxzzoajdn = 290.0
_hudwausgjm = 250.0

def _xiyquhwtfl() -> float:
    started = _tyefjxndrq['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _znmshswsex() -> float:
    return _lrxzzoajdn - _xiyquhwtfl()
_jnqmehjjas = _hsapi.llm_chat
_sqzyecvilg = _hsapi.search_web
_fqbyquxbpo = _hsapi.fetch_page
_dhszwaowpw = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _mdxkpygyfu(*args, **kwargs):
    if _xiyquhwtfl() >= _hudwausgjm:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _dhszwaowpw})
            kwargs['messages'] = steered
    _result = await _jnqmehjjas(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _sjtuxqftqf(_result)
    return _result

async def _pbmlyjgouk(*args, **kwargs):
    if _xiyquhwtfl() >= _hudwausgjm:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _sqzyecvilg(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _jbeipdysal(*args, **kwargs):
    if _xiyquhwtfl() >= _hudwausgjm:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _fqbyquxbpo(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _mdxkpygyfu
_hsapi.search_web = _pbmlyjgouk
_hsapi.fetch_page = _jbeipdysal
_qyisefxadg = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_qwdbwrnlir = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_fnutqmelfr = 900
_otorxwcgun = 2

def _mozhrimxyf(query: Query) -> int:
...
def _kftvqznfmj(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _jxswlkbnxi(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _mozhrimxyf(query)
    if fields >= 3:
        return 2
    if _kftvqznfmj(lowered, _qyisefxadg):
        return 1
    if fields <= _otorxwcgun and len(text) <= _fnutqmelfr:
        return 0
    if _kftvqznfmj(lowered, _qwdbwrnlir):
        return 0
    return 1

def _sjtuxqftqf(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _tyefjxndrq['text'] = text.strip()[:_sofnvvxwka]
    except Exception:
        pass

def _tnlgpwqrwi(text: str):
...
def _cnpwontdti(query: Query) -> Response:
    text = _tyefjxndrq['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_sofnvvxwka]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _tnlgpwqrwi(text)
...
def _hcqvyzsihq():
    ...
    async def _preseed(question: str, set_question: bool, ledger: EvidenceLedger, deadline: float) -> str:
        ...
        if not good:
            return ''
        return 'Automatic first-pass searches (already numbered — cite these [n] directly, and search further as needed):\n\n' + '\n'.join(good)
    _COVELEM_REVISION_TAG = 'f64-r3-20260905'
    COVELEM_MAX = 8
    COVELEM_MODEL = AUDIT_MODEL
    COVELEM_PLAN_TIMEOUT_S = 20.0
    COVELEM_PLAN_MAX_TOKENS = 400
    COVELEM_PLAN_MIN_SECONDS = 150.0
    COVELEM_COVER_RATIO = 0.6
    COVELEM_MAX_REENTRIES = 2
    COVELEM_MIN_SECONDS = 75.0
    COVELEM_MIN_USD = 0.04
    _COVELEM = {'elements': [], 'reentries': 0}
    _COVELEM_SPLIT_RE = re.compile('[^A-Za-z0-9]+')
    _COVELEM_TOK_STOP = frozenset(('the', 'and', 'for', 'its', 'their', 'both', 'this', 'that', 'with', 'from', 'full', 'official', 'quarterly', 'each', 'all', 'new', 'one', 'page', 'dated', 'list', 'every', 'must', 'then', 'into', 'over', 'under', 'only', 'same', 'record', 'value', 'member', 'figure', 'evidence'))
    _COVELEM_DOCNOUN_RE = re.compile('(?i)\\b((?:[a-z0-9][\\w/\\-]*\\s+){0,4}(?:tables?|sheets?|summar(?:y|ies)|reports?|bulletins?|appendix|appendices|indexe?s?|rosters?|listings?|schedules?|registers?|catalogues?|notices?|filings?|10-k|10-q|annual report|press release|datasets?|statistics))\\b')
    _COVELEM_DOCNOUN_STOP = ('the following', 'these', 'those', 'such ', 'any ', 'each ', 'both ', 'and ', 'or ', 'in ', 'on ', 'at ', 'to ', 'of ', 'as ', 'by ', 'for ', 'with ', 'from ', 'between ', 'using ', 'consider ', 'together ', 'that ', 'their ', 'its ', 'his ', 'her ', 'this ', 'contains ', 'credited ', 'appear ', 'shown ', 'written ', 'state ', 'working ', 'all ')

    def _covelem_seed(question: str) -> list:
        out = []
        for m in _COVELEM_DOCNOUN_RE.finditer(question or ''):
            phrase = ' '.join(m.group(1).split())
            low = phrase.lower()
            if len(phrase) < 12 or len(phrase.split()) < 2:
                continue
            if any((low.startswith(s) for s in _COVELEM_DOCNOUN_STOP)) or re.match('^\\W*\\d', phrase):
                continue
    ...
    def _covelem_keys(element: str) -> list:
        keys = []
        for token in _COVELEM_SPLIT_RE.split((element or '').lower()):
            if len(token) >= 4 and token not in _COVELEM_TOK_STOP and (token not in keys):
                keys.append(token)
        return keys[:6]

    async def _covelem_plan(question: str, deadline: float) -> None:
        """Resolve, ANTES da coleta, quais documentos/registros a pergunta exige.

    A lista vira o estado de cobertura que decide o término do loop: enquanto
    algum elemento não tiver linha de evidência que o carregue, o controlador
    devolve o modelo à coleta em vez de aceitar a resposta.
    """
        elements = list(_covelem_seed(question))
        probe = 'Name the distinct DOCUMENTS or authoritative records that must be RETRIEVED before this question is answerable: every named report, filing, table, register, dataset or official page the question names or implies. One short noun phrase per item. JSON only: a list of strings, at most 6.\n\nQuestion:\n' + question[:4000]
        try:
            raw = await _chat_simple(LLM_LANE_A, COVELEM_MODEL, 'Research planner. Name the evidence to gather. JSON list only.', probe, max_tokens=COVELEM_PLAN_MAX_TOKENS, timeout=max(6.0, min(COVELEM_PLAN_TIMEOUT_S, deadline - monotonic() - 90.0)))
        ...
        if isinstance(parsed, list):
            for item in parsed:
                ...
                if len(text) < 8 or len(_covelem_keys(text)) < 2:
                    continue
                if not any((text.lower() == e.lower() for e in elements)):
                    elements.append(text)
        _COVELEM['elements'] = elements[:COVELEM_MAX]

    def _covelem_uncovered(ledger) -> list:
        """Elementos exigidos que nenhuma linha coletada carrega. Determinístico."""
        elements = _COVELEM['elements']
        if not elements:
            return []
        hay = []
        for row in getattr(ledger, 'rows', []) or []:
            hay.append(' '.join((row.get('title') or '', row.get('url') or '', row.get('preview') or '', row.get('text') or '')).lower())
        missing = []
        for element in elements:
            keys = _covelem_keys(element)
            if not keys:
                continue
            covered = False
            for blob in hay:
                found = sum((1 for key in keys if key in blob))
                if float(found) / float(len(keys)) >= COVELEM_COVER_RATIO:
                    covered = True
                    break
            if not covered:
    ...
    def _covelem_order(missing: list) -> str:
        return 'COVERAGE VERIFICATION (required documents): research is NOT complete. No gathered row carries these required elements:\n- ' + '\n- '.join(missing[:COVELEM_MAX]) + '\nSearch or fetch each one specifically now -- query it by its own name, not the question as a whole. If one genuinely does not exist, state that explicitly in the answer and cite what you checked. Then produce the COMPLETE final answer with [n] citations in the required shape.'
    ...
    async def _loop(question: str, brief: str, ledger: EvidenceLedger, deadline: float, turn_cap: int, carry: list[dict] | None=None, allow_tools_in_wrapup: bool=False, criteria: list | None=None) -> tuple[str, list[dict]]:
        _covelem_primary = carry is None
        if carry is not None:
            messages = carry
        else:
            set_q = _needs_set_completeness(question)
        ...
        for turn in range(1, turn_cap + 1):
            ...
            if not calls:
                ...
                if _covelem_primary:
                    _cov_missing = []
                    try:
                        _cov_missing = _covelem_uncovered(ledger)
                    except Exception:
                        _cov_missing = []
                    if _cov_missing and (not finish_only) and (_COVELEM['reentries'] < COVELEM_MAX_REENTRIES) and (deadline - monotonic() > COVELEM_MIN_SECONDS) and (_spend_left() >= COVELEM_MIN_USD):
                        _COVELEM['reentries'] = _COVELEM['reentries'] + 1
                        messages.append({'role': 'assistant', 'content': answer})
                        messages.append({'role': 'system', 'content': _covelem_order(_cov_missing)})
                        answer = ''
                        continue
                messages.append({'role': 'assistant', 'content': answer})
                break
    ...
    _TEMPORAL_YEAR_RE = re.compile('\\b(?:19|20)\\d{2}\\b')
    _TEMPORAL_MAX_YEARS = 3
    _TEMPORAL_MIN_LEFT_S = 100.0
    _TEMPORAL_TAIL_RESERVE_S = 42.0
    _TEMPORAL_SCAN_CHARS = 400000
    _TEMPORAL_KEEP_PCT = 60
    _TEMPORAL_MARK_RE = re.compile('\\[[0-9][0-9,\\s\\-]*\\]')
    _TEMPORAL_NUM_RE = re.compile('\\d[\\d,.]*')
    _TEMPORAL_NAME_RE = re.compile("[A-Z][A-Za-z0-9&'\\u2019.\\-]{2,}")
    ...
    async def _temporal_recheck(question: str, answer: str, messages: list, ledger, deadline: float) -> str:
    ...
    period than the one asked, which reads as wrong rather than incomplete."""
        try:
            ...
            if not missing:
                return answer
            tail = ' '.join((w for w in (question or '').split() if w.lower() not in ('the', 'a', 'an', 'of', 'for', 'and', 'or', 'to', 'in', 'on')))[:100]
            for year in missing[:2]:
                try:
                    ...
                    pass
            listed = ', '.join(missing)
            messages.append({'role': 'system', 'content': 'TEMPORAL CHECK. The question is anchored to ' + listed + ', and nothing retrieved covers that period. Report the figure for the year the question names, citing the row that dates it with [n]. If the evidence still only covers another year, state plainly which year your figure belongs to instead of presenting it as the answer for the year asked. Then rewrite the COMPLETE final answer with [n] citations.'})
            patched, _ = await _loop(question, '', ledger, deadline - _TEMPORAL_TAIL_RESERVE_S, AUDIT_EXTRA_TURNS + 1, carry=messages, allow_tools_in_wrapup=True)
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            ...
            criteria = []
        _COVELEM['elements'] = []
        _COVELEM['reentries'] = 0
        try:
            if deadline - monotonic() > COVELEM_PLAN_MIN_SECONDS:
                await _covelem_plan(question, deadline)
        except Exception:
            _COVELEM['elements'] = []
        answer = ''
        messages: list[dict] = []
        ...
        if _is_usable_answer(answer):
            ...
            try:
                ...
                pass
            try:
                answer = await _temporal_recheck(question, answer, messages, ledger, deadline)
            except Exception:
                pass
        if not _is_usable_answer(answer) and ledger.rows:
            try:
...
def _qyqpicauma():
    """uid_195 research agent - variant 14.

Two stages added to the uid_195 pipeline, one taken from each of two
validator_reference candidates:
  uid  82 - value-support repair (value)
  uid 150 - premise-verification sweep (premise)
...
"""
    ...
    STAGE_EVIDENCE_CHARS = 8500
    STAGE_UNSPANNED_CHARS = 580
    STAGE_REWRITE_TIMEOUT = 33.0
    STAGE_REWRITE_MIN_SECONDS = 14.0
    STAGE_SEARCH_MIN_SECONDS = 50.0
    STAGE_TAIL_RESERVE_SECONDS = 27.0
    ROSTER_BUDGET_SECONDS = 23.0
    VALUE_MIN_DIGITS = 3
    VALUE_MAX_REPORTED = 8

    def _unsupported_values(answer: str, index: _ResultIndex) -> list[str]:
        corpus = index.all_note_text()
        corpus_digits = _stage_digits(corpus)
        seen: set[str] = set()
        bad: list[str] = []
        for match in STAGE_FIGURE_RE.finditer(answer or ''):
            raw = match.group(0).strip(' .,')
            plain = _stage_digits(raw)
            if len(plain) < VALUE_MIN_DIGITS:
                continue
            if plain in seen:
                continue
            seen.add(plain)
            if raw in corpus or plain in corpus_digits:
                continue
            bad.append(raw)
            if len(bad) >= VALUE_MAX_REPORTED:
                break
        return bad

    async def _value_repair(question: str, answer: str, index: _ResultIndex, terms: list[str], deadline: float) -> str:
        bad = _unsupported_values(answer, index)
        if not bad:
            return answer
        listed = ', '.join(bad)
        directive = f"These values appear in the draft but no retrieved passage states them: {listed}. Replace each with the figure the evidence does state, carrying that source's [n] marker, or remove the claim. Do not restate any of them unless a passage below contains it."
        return await _stage_rewrite(question, answer, index, directive, deadline)
    PREMISE_MIN_ENTITY_CHARS = 4
    PREMISE_MAX_ENTITIES = 4
    PREMISE_SKIP = frozenset(('what', 'which', 'who', 'when', 'where', 'why', 'how', 'the', 'and', 'for', 'was', 'were', 'did', 'does', 'list', 'name', 'give', 'state', 'compare', 'according', 'based', 'using', 'between', 'during', 'since', 'from', 'into'))

    def _question_entities(question: str) -> list[str]:
        found: list[str] = []
        seen: set[str] = set()
        for match in STAGE_ENTITY_RE.finditer(question or ''):
            phrase = match.group(0).strip(' .,')
            if len(phrase) < PREMISE_MIN_ENTITY_CHARS:
                continue
            if phrase.lower() in PREMISE_SKIP:
                continue
            ...
            seen.add(key)
            found.append(phrase)
        return found[:PREMISE_MAX_ENTITIES]

    def _absent_entities(question: str, index: _ResultIndex) -> list[str]:
        corpus = _stage_note_text(index)
        return [e for e in _question_entities(question) if e.lower() not in corpus]

    async def _premise_sweep(question: str, answer: str, index: _ResultIndex, terms: list[str], deadline: float) -> str:
        absent = _absent_entities(question, index)
        if not absent:
            return answer
        await _stage_search(_stage_terms_query(terms, absent[0]), index, deadline)
        listed = ', '.join(absent)
        directive = f'The question names {listed}, and no retrieved passage mentions it. Answer about the subject the question actually names, citing a passage that names it. If the evidence does not cover that subject, say so plainly rather than answering about a neighbouring one.'
        return await _stage_rewrite(question, answer, index, directive, deadline)
    ...
    async def _plain_query(query: Query, budget: float) -> Response:
        ...
        try:
            ...
            if display:
                display = await _premise_sweep(query.text, display, index, terms, stage_deadline)
            if display:
                display = await _value_repair(query.text, display, index, terms, stage_deadline)
    ...
    return query
_pmtstymghg = _knmglxkxhx()
_rmhhxoeqnf = _hcqvyzsihq()
_nehjgxuibn = _krlrbxljsa()
_hfmwjgjrib = _qyqpicauma()
_lrxzzoajdn = 290.0
_trucisfsok = 250.0
_ezmzisdrct = 90.0

async def _fjhqcjzzpu(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _lrxzzoajdn - (time.monotonic() - started)
        if first:
            budget = _trucisfsok if _trucisfsok < remaining else remaining
            first = False
        else:
            if remaining < _ezmzisdrct:
                break
        ...
        try:
            ...
            last_exc = exc
    return _cnpwontdti(query)

async def _drv_base_query(query: Query) -> Response:
    _tyefjxndrq['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _fjhqcjzzpu(query, (_nehjgxuibn, _pmtstymghg, _rmhhxoeqnf, _hfmwjgjrib))
        index = _jxswlkbnxi(query)
        if index == 0:
            agents = (_pmtstymghg, _rmhhxoeqnf, _nehjgxuibn, _hfmwjgjrib)
        elif index == 1:
            agents = (_rmhhxoeqnf, _nehjgxuibn, _hfmwjgjrib, _pmtstymghg)
        elif index == 2:
            agents = (_nehjgxuibn, _hfmwjgjrib, _pmtstymghg, _rmhhxoeqnf)
        elif index == 3:
            agents = (_hfmwjgjrib, _pmtstymghg, _rmhhxoeqnf, _nehjgxuibn)
        else:
            agents = (_pmtstymghg, _rmhhxoeqnf, _nehjgxuibn, _hfmwjgjrib)
        return await _fjhqcjzzpu(query, agents)
    except Exception:
        return _cnpwontdti(query)
_BUILDT_TAG_m23 = "sn45-b54179eb92bf"

# --- drv wrap: claim-conflict ledger (start) ---
# batch_tag='drv9' salt='0a724338db32'
