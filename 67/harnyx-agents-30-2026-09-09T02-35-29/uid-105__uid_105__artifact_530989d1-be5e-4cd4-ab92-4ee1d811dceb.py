...
_ecfpaehmln = {'started': None, 'text': None}
_egzmfvshmr = 24000
_exiosdtlvd = 290.0
_xwkbrmzvsc = 250.0

def _ikqqgsevjg() -> float:
    started = _ecfpaehmln['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _nvyvmvrsyt() -> float:
    return _exiosdtlvd - _ikqqgsevjg()
_mbvmhivofr = _hsapi.llm_chat
_cwxpliainj = _hsapi.search_web
_utgkwafned = _hsapi.fetch_page
...
async def _vpnwxntjfi(*args, **kwargs):
    if _ikqqgsevjg() >= _xwkbrmzvsc:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _nqxiqtoktt})
            kwargs['messages'] = steered
    _result = await _mbvmhivofr(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _vrnngqnmpx(_result)
    return _result

async def _vjhxbehvpx(*args, **kwargs):
    if _ikqqgsevjg() >= _xwkbrmzvsc:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _cwxpliainj(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _vegynzhmwc(*args, **kwargs):
    if _ikqqgsevjg() >= _xwkbrmzvsc:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _utgkwafned(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _vpnwxntjfi
_hsapi.search_web = _vjhxbehvpx
_hsapi.fetch_page = _vegynzhmwc
_iiqgmusrcc = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_eukmrrnygc = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_qmuxyqdezn = 900
_gppvevghrm = 2

def _oayflvpuwx(query: Query) -> int:
...
def _rhntqskakk(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _kumhbbmfrn(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _oayflvpuwx(query)
    if fields >= 3:
        return 2
    if _rhntqskakk(lowered, _iiqgmusrcc):
        return 1
    if fields <= _gppvevghrm and len(text) <= _qmuxyqdezn:
        return 0
    if _rhntqskakk(lowered, _eukmrrnygc):
        return 0
    return 1

def _vrnngqnmpx(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _ecfpaehmln['text'] = text.strip()[:_egzmfvshmr]
    except Exception:
        pass

def _mbiyopmuyx(text: str):
...
def _okirjsshde(query: Query) -> Response:
    text = _ecfpaehmln['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_egzmfvshmr]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _mbiyopmuyx(text)
...
def _kkfykvxypc():
    ...
    def _upstream(lane: str, model: str) -> dict | None:
        ...
        if chosen is None or chosen in _RUN_UPSTREAM['dead']:
            ...
            chosen = live[0]
            _RUN_UPSTREAM[key] = chosen
        return {'provider': {'only': [chosen], 'allow_fallbacks': True}}
    ...
    async def query(query: Query) -> Response:
        ...
        try:
            ...
            return draft
    return query
_kxkynmttuo = _svpfhexcny()
_nozjivkunc = _cvnosgbudn()
_ehfhhdkvfv = _dyxxxwsqmn()
_zjtzdrrrrt = _zcvgwzxseh()
_udkubeinqo = _kkfykvxypc()
_exiosdtlvd = 290.0
_kyjflsdopf = 250.0
_kylqgbytoi = 90.0

async def _rdjwunpuxb(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _exiosdtlvd - (time.monotonic() - started)
        if first:
            budget = _kyjflsdopf if _kyjflsdopf < remaining else remaining
            first = False
        else:
            if remaining < _kylqgbytoi:
                break
        ...
        try:
            ...
            last_exc = exc
    return _okirjsshde(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _ecfpaehmln['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _rdjwunpuxb(query, (_ehfhhdkvfv, _kxkynmttuo, _nozjivkunc, _zjtzdrrrrt, _udkubeinqo))
        index = _kumhbbmfrn(query)
        if index == 0:
            agents = (_kxkynmttuo, _nozjivkunc, _ehfhhdkvfv, _zjtzdrrrrt, _udkubeinqo)
        elif index == 1:
            agents = (_nozjivkunc, _ehfhhdkvfv, _zjtzdrrrrt, _udkubeinqo, _kxkynmttuo)
        elif index == 2:
            agents = (_ehfhhdkvfv, _zjtzdrrrrt, _udkubeinqo, _kxkynmttuo, _nozjivkunc)
        elif index == 3:
            agents = (_zjtzdrrrrt, _udkubeinqo, _kxkynmttuo, _nozjivkunc, _ehfhhdkvfv)
        elif index == 4:
            agents = (_udkubeinqo, _kxkynmttuo, _nozjivkunc, _ehfhhdkvfv, _zjtzdrrrrt)
        else:
            agents = (_kxkynmttuo, _nozjivkunc, _ehfhhdkvfv, _zjtzdrrrrt, _udkubeinqo)
        return await _rdjwunpuxb(query, agents)
    except Exception:
        return _okirjsshde(query)
_BUILDS_TAG_m20 = "sn45-91c77362b308"
