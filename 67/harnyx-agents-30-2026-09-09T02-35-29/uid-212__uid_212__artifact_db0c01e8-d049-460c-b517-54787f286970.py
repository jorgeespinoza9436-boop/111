...
def _compose_marble_lantern_slot09_agent_entry():
...
_marble_lantern_slot09_agent_query_entry = _compose_marble_lantern_slot09_agent_entry()


def _compose_ivory_survey_slot09_agent_entry():
...
_ivory_survey_slot09_agent_query_entry = _compose_ivory_survey_slot09_agent_entry()


def _compose_nimbus_kestrel_slot09_agent_entry():
...
_nimbus_kestrel_slot09_agent_query_entry = _compose_nimbus_kestrel_slot09_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "MarbleLanternSlot09Agent"
    ...
    if (
        ...
        return "IvorySurveySlot09Agent"
    return "NimbusKestrelSlot09Agent"


class MarbleLanternSlot09Agent:
    async def __call__(self, query: Query) -> Response:
        return await _marble_lantern_slot09_agent_query_entry(query)


class IvorySurveySlot09Agent:
    async def __call__(self, query: Query) -> Response:
        return await _ivory_survey_slot09_agent_query_entry(query)


class NimbusKestrelSlot09Agent:
    async def __call__(self, query: Query) -> Response:
        return await _nimbus_kestrel_slot09_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = MarbleLanternSlot09Agent()
_ANALYTICAL_FIELD_AGENT = IvorySurveySlot09Agent()
_BROAD_FIELD_AGENT = NimbusKestrelSlot09Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "MarbleLanternSlot09Agent",
    "IvorySurveySlot09Agent",
    "NimbusKestrelSlot09Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "MarbleLanternSlot09Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "IvorySurveySlot09Agent":
