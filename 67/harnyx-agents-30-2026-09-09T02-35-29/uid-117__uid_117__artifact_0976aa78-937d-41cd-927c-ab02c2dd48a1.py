...
_wpuynbbyuu = {'started': None, 'text': None}
_kbtfoqwbgc = 24000
_mzpuzmjqrp = 290.0
_afqdvdhubn = 250.0

def _mqdwkhwjog() -> float:
    started = _wpuynbbyuu['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _dnzvtzknle() -> float:
    return _mzpuzmjqrp - _mqdwkhwjog()
_ffoepyomrs = _hsapi.llm_chat
_odxnzhcoln = _hsapi.search_web
_ronqfylpie = _hsapi.fetch_page
_hxzrbxjobu = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _wbandnltpw(*args, **kwargs):
    if _mqdwkhwjog() >= _afqdvdhubn:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _hxzrbxjobu})
            kwargs['messages'] = steered
    _result = await _ffoepyomrs(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _ecywquilzo(_result)
    return _result

async def _dzprvaouwd(*args, **kwargs):
    if _mqdwkhwjog() >= _afqdvdhubn:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _odxnzhcoln(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _oielsutepk(*args, **kwargs):
    if _mqdwkhwjog() >= _afqdvdhubn:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _ronqfylpie(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _wbandnltpw
_hsapi.search_web = _dzprvaouwd
_hsapi.fetch_page = _oielsutepk
_ixotwhrrgu = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_huofxkeqwr = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_xbflmbfntj = 900
_ukzpbxbxhz = 2

def _rernamkzfr(query: Query) -> int:
...
def _ivlfxnuxwa(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _tnbyjwakae(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _rernamkzfr(query)
    if fields >= 3:
        return 2
    if _ivlfxnuxwa(lowered, _ixotwhrrgu):
        return 1
    if fields <= _ukzpbxbxhz and len(text) <= _xbflmbfntj:
        return 0
    if _ivlfxnuxwa(lowered, _huofxkeqwr):
        return 0
    return 1

def _ecywquilzo(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _wpuynbbyuu['text'] = text.strip()[:_kbtfoqwbgc]
    except Exception:
        pass

def _fyyluatale(text: str):
...
def _wqlskvbtnm(query: Query) -> Response:
    text = _wpuynbbyuu['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_kbtfoqwbgc]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _fyyluatale(text)
...
"""
    ...
    return query
_jedfzaaqrz = _ebynyxvwyv()
_cjvnezozyw = _zxpzwzmpiu()
_itcvvvkndo = _qrozexqiad()
_uwzaidmfre = _qzkoslrduy()
_mzpuzmjqrp = 290.0
_sycenibpcs = 250.0
_ugdwkqftqo = 90.0

async def _zlbgplcxjb(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _mzpuzmjqrp - (time.monotonic() - started)
        if first:
            budget = _sycenibpcs if _sycenibpcs < remaining else remaining
            first = False
        else:
            if remaining < _ugdwkqftqo:
                break
        ...
        try:
            ...
            last_exc = exc
    return _wqlskvbtnm(query)
_ywlcnqudgd = 235.0

async def _hywspdnuss(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _ssytzllcat(query, agents):
    if len(agents) < 2:
        return await _zlbgplcxjb(query, agents)
    r0, r1 = await asyncio.gather(_hywspdnuss(agents[0], query, _ywlcnqudgd), _hywspdnuss(agents[1], query, _ywlcnqudgd))
    ok0 = isinstance(r0, Response) and _ourpefytjb(r0) != ''
    ok1 = isinstance(r1, Response) and _ourpefytjb(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_ffoepyomrs(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
async def _drv_base_query(query: Query) -> Response:
    _wpuynbbyuu['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _zlbgplcxjb(query, (_itcvvvkndo, _jedfzaaqrz, _cjvnezozyw, _uwzaidmfre))
        index = _tnbyjwakae(query)
        if index == 0:
            agents = (_jedfzaaqrz, _cjvnezozyw, _itcvvvkndo, _uwzaidmfre)
        elif index == 1:
            agents = (_cjvnezozyw, _itcvvvkndo, _uwzaidmfre, _jedfzaaqrz)
        elif index == 2:
            agents = (_itcvvvkndo, _uwzaidmfre, _jedfzaaqrz, _cjvnezozyw)
        elif index == 3:
            agents = (_uwzaidmfre, _jedfzaaqrz, _cjvnezozyw, _itcvvvkndo)
        else:
            agents = (_jedfzaaqrz, _cjvnezozyw, _itcvvvkndo, _uwzaidmfre)
        return await _ssytzllcat(query, agents)
    except Exception:
        return _wqlskvbtnm(query)
_BUILDT_TAG_m15 = "sn45-d36d54d64c3d"

# --- drv wrap: claim-conflict ledger (start) ---
# batch_tag='drv9' salt='2486d25d4413'
