...
_czaemuvgzg = {'started': None, 'text': None}
_gysoiqqyfh = 24000
_uuyhfeddmo = 290.0
_bxqkxcptxu = 250.0

def _cotiduiyud() -> float:
    started = _czaemuvgzg['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _faixlwjuct() -> float:
    return _uuyhfeddmo - _cotiduiyud()
_adykstyajj = _hsapi.llm_chat
_zdwrwteggy = _hsapi.search_web
_iijzarwtkb = _hsapi.fetch_page
_fvvguuevgt = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _tqxdtkflof(*args, **kwargs):
    if _cotiduiyud() >= _bxqkxcptxu:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _fvvguuevgt})
            kwargs['messages'] = steered
    _result = await _adykstyajj(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _fqrsgbgxme(_result)
    return _result

async def _iphteahoom(*args, **kwargs):
    if _cotiduiyud() >= _bxqkxcptxu:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _zdwrwteggy(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _kafwxwgmmv(*args, **kwargs):
    if _cotiduiyud() >= _bxqkxcptxu:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _iijzarwtkb(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _tqxdtkflof
_hsapi.search_web = _iphteahoom
_hsapi.fetch_page = _kafwxwgmmv
_fivprqnmky = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_epkmdwwsxf = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_bkrsmlpmmm = 900
_lwfayuyagl = 2

def _asizignjet(query: Query) -> int:
...
def _ukkkwuzlnd(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _awlnqeesqt(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _asizignjet(query)
    if fields >= 3:
        return 2
    if _ukkkwuzlnd(lowered, _fivprqnmky):
        return 1
    if fields <= _lwfayuyagl and len(text) <= _bkrsmlpmmm:
        return 0
    if _ukkkwuzlnd(lowered, _epkmdwwsxf):
        return 0
    return 1

def _fqrsgbgxme(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _czaemuvgzg['text'] = text.strip()[:_gysoiqqyfh]
    except Exception:
        pass

def _fvtpdgxbnf(text: str):
...
def _iscnzqdgie(query: Query) -> Response:
    text = _czaemuvgzg['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_gysoiqqyfh]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _fvtpdgxbnf(text)
...
def _qhbkfwidrj():
    ...
    return query
_kzrlfafocv = _hrsthzvrit()
_pufxnbnkqf = _fyywhwoxxs()
_nbyqklacnq = _aqbvznphnc()
_axlhlsfjwe = _wfnyblxeut()
_mafcflprcj = _qhbkfwidrj()
_uuyhfeddmo = 290.0
_aurlovujix = 250.0
_lccsrnouwq = 90.0

async def _liiemxwqcl(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _uuyhfeddmo - (time.monotonic() - started)
        if first:
            budget = _aurlovujix if _aurlovujix < remaining else remaining
            first = False
        else:
            if remaining < _lccsrnouwq:
                break
        ...
        try:
            ...
            last_exc = exc
    return _iscnzqdgie(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _czaemuvgzg['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _liiemxwqcl(query, (_nbyqklacnq, _kzrlfafocv, _pufxnbnkqf, _axlhlsfjwe, _mafcflprcj))
        index = _awlnqeesqt(query)
        if index == 0:
            agents = (_kzrlfafocv, _pufxnbnkqf, _nbyqklacnq, _axlhlsfjwe, _mafcflprcj)
        elif index == 1:
            agents = (_pufxnbnkqf, _nbyqklacnq, _axlhlsfjwe, _mafcflprcj, _kzrlfafocv)
        elif index == 2:
            agents = (_nbyqklacnq, _axlhlsfjwe, _mafcflprcj, _kzrlfafocv, _pufxnbnkqf)
        elif index == 3:
            agents = (_axlhlsfjwe, _mafcflprcj, _kzrlfafocv, _pufxnbnkqf, _nbyqklacnq)
        elif index == 4:
            agents = (_mafcflprcj, _kzrlfafocv, _pufxnbnkqf, _nbyqklacnq, _axlhlsfjwe)
        else:
            agents = (_kzrlfafocv, _pufxnbnkqf, _nbyqklacnq, _axlhlsfjwe, _mafcflprcj)
        return await _liiemxwqcl(query, agents)
    except Exception:
        return _iscnzqdgie(query)
_BUILDS_TAG_m12 = "sn45-ee91932dedc1"
