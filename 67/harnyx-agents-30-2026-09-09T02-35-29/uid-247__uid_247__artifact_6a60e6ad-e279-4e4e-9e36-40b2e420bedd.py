...
def _compose_garnet_cairn_slot01_agent_entry():
...
_garnet_cairn_slot01_agent_query_entry = _compose_garnet_cairn_slot01_agent_entry()


def _compose_talus_atlas_slot01_agent_entry():
...
_talus_atlas_slot01_agent_query_entry = _compose_talus_atlas_slot01_agent_entry()


def _compose_zephyr_wren_slot01_agent_entry():
    """SN67 Harnyx miner — staged research protocol agent."""
...
_zephyr_wren_slot01_agent_query_entry = _compose_zephyr_wren_slot01_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "GarnetCairnSlot01Agent"
    ...
    if (
        ...
        return "TalusAtlasSlot01Agent"
    return "ZephyrWrenSlot01Agent"


class GarnetCairnSlot01Agent:
    async def __call__(self, query: Query) -> Response:
        return await _garnet_cairn_slot01_agent_query_entry(query)


class TalusAtlasSlot01Agent:
    async def __call__(self, query: Query) -> Response:
        return await _talus_atlas_slot01_agent_query_entry(query)


class ZephyrWrenSlot01Agent:
    async def __call__(self, query: Query) -> Response:
        return await _zephyr_wren_slot01_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = GarnetCairnSlot01Agent()
_ANALYTICAL_FIELD_AGENT = TalusAtlasSlot01Agent()
_BROAD_FIELD_AGENT = ZephyrWrenSlot01Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "GarnetCairnSlot01Agent",
    "TalusAtlasSlot01Agent",
    "ZephyrWrenSlot01Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "GarnetCairnSlot01Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "TalusAtlasSlot01Agent":
