...
def _compose_quartz_lattice_agent_entry():
...
_quartz_lattice_agent_query_entry = _compose_quartz_lattice_agent_entry()


def _compose_amber_lantern_agent_entry():
    _S444S666_QUERY_TAG = "s444s666b2-hk674"  # per-hotkey canonical uniqueness
...
_SHAPE_ROUTER_SEED = "738cdd5666fb4a07490463e6"
...
def _balanced_route_label(query: Query) -> str:
    if getattr(query, "fast", False):
        return "QuartzLanternAgent"
    text = (getattr(query, "text", "") or "").strip()
    shape = _shape_class(query)
    if shape == 0:
        return "QuartzLatticeAgent"
    if shape == 1:
...
class QuartzLatticeAgent:
    async def __call__(self, query: Query) -> Response:
        return await _quartz_lattice_agent_query_entry(query)
...
_SHAPE_PRIMARY_AGENT = QuartzLanternAgent()
_SHAPE_SECONDARY_AGENT = QuartzLatticeAgent()
_SHAPE_TERTIARY_AGENT = AmberLanternAgent()
