...
_ymsdahsjll = {'started': None, 'text': None}
_bqckskqrtp = 24000
_ktnjvvkjec = 290.0
_zubypwmmfg = 250.0

def _wpkbrptidf() -> float:
    started = _ymsdahsjll['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _ugliljffkg() -> float:
    return _ktnjvvkjec - _wpkbrptidf()
_bhtiqbjers = _hsapi.llm_chat
_gaitvgatfi = _hsapi.search_web
_bzystfctfn = _hsapi.fetch_page
_ambzjcnczi = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _siclyqgwxq(*args, **kwargs):
    if _wpkbrptidf() >= _zubypwmmfg:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _ambzjcnczi})
            kwargs['messages'] = steered
    _result = await _bhtiqbjers(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _xrmnwejitu(_result)
    return _result

async def _ptkcvojmlm(*args, **kwargs):
    if _wpkbrptidf() >= _zubypwmmfg:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _gaitvgatfi(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _akjelczsmb(*args, **kwargs):
    if _wpkbrptidf() >= _zubypwmmfg:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _bzystfctfn(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _siclyqgwxq
_hsapi.search_web = _ptkcvojmlm
_hsapi.fetch_page = _akjelczsmb
_akihjtpmef = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_whldcnuwzw = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_kkedtrwkso = 900
_tmlodfqsue = 2

def _gomxafnjyz(query: Query) -> int:
...
def _crfnrodand(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _pujvmfhsdw(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _gomxafnjyz(query)
    if fields >= 3:
        return 2
    if _crfnrodand(lowered, _akihjtpmef):
        return 1
    if fields <= _tmlodfqsue and len(text) <= _kkedtrwkso:
        return 0
    if _crfnrodand(lowered, _whldcnuwzw):
        return 0
    return 1

def _xrmnwejitu(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _ymsdahsjll['text'] = text.strip()[:_bqckskqrtp]
    except Exception:
        pass

def _zibzabfrrj(text: str):
...
def _wfpxjtxobt(query: Query) -> Response:
    text = _ymsdahsjll['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_bqckskqrtp]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _zibzabfrrj(text)
...
def _diqjjqzthj():
    ...
    return query
_oihmegftxc = _vfztskjngs()
_emgaysysgy = _rypntphehj()
_otsdiqykuk = _jxpyqvoscg()
_yydmlargmd = _diqjjqzthj()
_ktnjvvkjec = 290.0
_bldhwydosg = 250.0
_qpkbhdoujg = 90.0

async def _iniledohog(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _ktnjvvkjec - (time.monotonic() - started)
        if first:
            budget = _bldhwydosg if _bldhwydosg < remaining else remaining
            first = False
        else:
            if remaining < _qpkbhdoujg:
                break
        ...
        try:
            ...
            last_exc = exc
    return _wfpxjtxobt(query)

async def _drv_base_query(query: Query) -> Response:
    _ymsdahsjll['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _iniledohog(query, (_otsdiqykuk, _oihmegftxc, _emgaysysgy, _yydmlargmd))
        index = _pujvmfhsdw(query)
        if index == 0:
            agents = (_oihmegftxc, _emgaysysgy, _otsdiqykuk, _yydmlargmd)
        elif index == 1:
            agents = (_emgaysysgy, _otsdiqykuk, _yydmlargmd, _oihmegftxc)
        elif index == 2:
            agents = (_otsdiqykuk, _yydmlargmd, _oihmegftxc, _emgaysysgy)
        elif index == 3:
            agents = (_yydmlargmd, _oihmegftxc, _emgaysysgy, _otsdiqykuk)
        else:
            agents = (_oihmegftxc, _emgaysysgy, _otsdiqykuk, _yydmlargmd)
        return await _iniledohog(query, agents)
    except Exception:
        return _wfpxjtxobt(query)
_BUILDT_TAG_m20 = "sn45-16dad7c839e0"

# --- drv wrap: claim-conflict ledger (start) ---
# batch_tag='drv9' salt='fab8b1f4f3df'
