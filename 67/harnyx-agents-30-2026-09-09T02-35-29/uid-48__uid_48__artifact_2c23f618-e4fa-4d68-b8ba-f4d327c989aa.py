...
_mgfkjkredz = {'started': None, 'text': None}
_ijcbdvfhtj = 24000
_ccyywnmeio = 290.0
_fdhszhmqaq = 250.0

def _jyywzzmrqb() -> float:
    started = _mgfkjkredz['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _xmptjpglyg() -> float:
    return _ccyywnmeio - _jyywzzmrqb()
_ntnktuchor = _hsapi.llm_chat
_skkzvxlrzd = _hsapi.search_web
_mubvlmhwtv = _hsapi.fetch_page
_jolapmxneq = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _jezyborhsc(*args, **kwargs):
    if _jyywzzmrqb() >= _fdhszhmqaq:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _jolapmxneq})
            kwargs['messages'] = steered
    _result = await _ntnktuchor(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _rykhrawleq(_result)
    return _result

async def _lzchpfnoth(*args, **kwargs):
    if _jyywzzmrqb() >= _fdhszhmqaq:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _skkzvxlrzd(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _mpflmqxeti(*args, **kwargs):
    if _jyywzzmrqb() >= _fdhszhmqaq:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _mubvlmhwtv(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _jezyborhsc
_hsapi.search_web = _lzchpfnoth
_hsapi.fetch_page = _mpflmqxeti
_hinhqvivmt = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_oztlbgjuxz = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_cwatgxkfnn = 900
_msxeqwvibe = 2

def _vtnjfdxazx(query: Query) -> int:
...
def _uwahvmhbbp(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _fgafemakod(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _vtnjfdxazx(query)
    if fields >= 3:
        return 2
    if _uwahvmhbbp(lowered, _hinhqvivmt):
        return 1
    if fields <= _msxeqwvibe and len(text) <= _cwatgxkfnn:
        return 0
    if _uwahvmhbbp(lowered, _oztlbgjuxz):
        return 0
    return 1

def _rykhrawleq(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _mgfkjkredz['text'] = text.strip()[:_ijcbdvfhtj]
    except Exception:
        pass

def _cynmuecmwl(text: str):
...
def _yzzyhpudhg(query: Query) -> Response:
    text = _mgfkjkredz['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_ijcbdvfhtj]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _cynmuecmwl(text)
...
def _cvwnravmka():
    """uid_195 research agent - variant 30.
...
"""
    ...
    return query
_cgeplflrbq = _druidhjcmx()
_tosrmjonfx = _qafulkxkyn()
_nmwrjhnyfm = _zhemuaqhmv()
_fvuiktseal = _cvwnravmka()
_ccyywnmeio = 290.0
_ijfeitmeua = 250.0
_zynrthxyfo = 90.0

async def _vqawucfvij(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _ccyywnmeio - (time.monotonic() - started)
        if first:
            budget = _ijfeitmeua if _ijfeitmeua < remaining else remaining
            first = False
        else:
            if remaining < _zynrthxyfo:
                break
        ...
        try:
            ...
            last_exc = exc
    return _yzzyhpudhg(query)

async def _drv_base_query(query: Query) -> Response:
    _mgfkjkredz['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _vqawucfvij(query, (_nmwrjhnyfm, _cgeplflrbq, _tosrmjonfx, _fvuiktseal))
        index = _fgafemakod(query)
        if index == 0:
            agents = (_cgeplflrbq, _tosrmjonfx, _nmwrjhnyfm, _fvuiktseal)
        elif index == 1:
            agents = (_tosrmjonfx, _nmwrjhnyfm, _fvuiktseal, _cgeplflrbq)
        elif index == 2:
            agents = (_nmwrjhnyfm, _fvuiktseal, _cgeplflrbq, _tosrmjonfx)
        elif index == 3:
            agents = (_fvuiktseal, _cgeplflrbq, _tosrmjonfx, _nmwrjhnyfm)
        else:
            agents = (_cgeplflrbq, _tosrmjonfx, _nmwrjhnyfm, _fvuiktseal)
        return await _vqawucfvij(query, agents)
    except Exception:
        return _yzzyhpudhg(query)
_BUILDT_TAG_m2 = "sn45-56a0a4f1f5f1"

# --- drv wrap: claim-conflict ledger (start) ---
# batch_tag='drv9' salt='a72b32d631b8'
