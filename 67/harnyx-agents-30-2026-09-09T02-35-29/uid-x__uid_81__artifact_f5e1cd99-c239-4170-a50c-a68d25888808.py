...
_tloquzjywi = {'started': None, 'text': None}
_fixhkuqxsn = 24000
_usssnptszp = 290.0
_eoyjrzwokb = 250.0

def _wsmudqfqrp() -> float:
    started = _tloquzjywi['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _yonwofrqup() -> float:
    return _usssnptszp - _wsmudqfqrp()
_ebznvsdxkf = _hsapi.llm_chat
_qjugrakvud = _hsapi.search_web
_sofpwjcxcj = _hsapi.fetch_page
_klxfxekmvj = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _yhbkhgppcv(*args, **kwargs):
    if _wsmudqfqrp() >= _eoyjrzwokb:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _klxfxekmvj})
            kwargs['messages'] = steered
    _result = await _ebznvsdxkf(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _vvzqvvkcvb(_result)
    return _result

async def _xoetnupkmk(*args, **kwargs):
    if _wsmudqfqrp() >= _eoyjrzwokb:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _qjugrakvud(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _comhkyobvm(*args, **kwargs):
    if _wsmudqfqrp() >= _eoyjrzwokb:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _sofpwjcxcj(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _yhbkhgppcv
_hsapi.search_web = _xoetnupkmk
_hsapi.fetch_page = _comhkyobvm
_vqegmdndip = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_nxllpcgdkk = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_mottffekda = 900
_xadnybxnpp = 2

def _irkcytsqqp(query: Query) -> int:
...
def _kaxcrgathr(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _xnyjchlvvt(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _irkcytsqqp(query)
    if fields >= 3:
        return 2
    if _kaxcrgathr(lowered, _vqegmdndip):
        return 1
    if fields <= _xadnybxnpp and len(text) <= _mottffekda:
        return 0
    if _kaxcrgathr(lowered, _nxllpcgdkk):
        return 0
    return 1

def _vvzqvvkcvb(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _tloquzjywi['text'] = text.strip()[:_fixhkuqxsn]
    except Exception:
        pass

def _kkkpaxyyud(text: str):
...
def _igtdvygwar(query: Query) -> Response:
    text = _tloquzjywi['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_fixhkuqxsn]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _kkkpaxyyud(text)
...
def _zbgvqvvvra():
    ...
    def _cb_shape_ok(value, schema) -> bool:
        """Tolerant shape check: mirrors what the platform validator accepts."""
        if not isinstance(schema, dict):
            return True
        for key in ('anyOf', 'oneOf'):
            branch = schema.get(key)
            if isinstance(branch, list) and branch:
                return any(_cb_shape_ok(value, sub) for sub in branch)
        kind = _schema_kind(schema)
        if not kind:
            return True
        if kind == 'string':
            ...
            return not (isinstance(limit, int) and 0 < limit < len(value))
        if kind == 'integer':
        ...
        if kind == 'array':
            ...
            return not (isinstance(cap, int) and 0 <= cap < len(value))
        if kind == 'object':
            return isinstance(value, dict)
        return True

    def _cb_schema_fit(value, schema, depth: int = 0):
        """Last-mile type repair of a structured value against its schema.

        A value that violates the declared schema is rejected wholesale by the
        platform: the task scores zero even when the answer is right. A value
        that already satisfies the schema is returned untouched.
        """
        if not isinstance(schema, dict) or depth > 6:
            return value
        kind_now = _schema_kind(schema)
        if kind_now not in ('object', 'array') and _cb_shape_ok(value, schema):
            return value
        for key in ('anyOf', 'oneOf'):
            branch = schema.get(key)
            if isinstance(branch, list) and branch:
                for sub in branch:
                    fitted = _cb_schema_fit(value, sub, depth + 1)
                    if _cb_shape_ok(fitted, sub):
                        return fitted
                return value
        kind = kind_now
        if kind == 'object' and isinstance(value, dict):
            ...
            if not isinstance(props, dict):
                return value
            return {k: (_cb_schema_fit(v, props[k], depth + 1) if isinstance(props.get(k), dict) else v)
                    for k, v in value.items()}
        if kind == 'array' and isinstance(value, list):
            items = schema.get('items')
            fitted = ([_cb_schema_fit(v, items, depth + 1) for v in value]
                      if isinstance(items, dict) else list(value))
            cap = schema.get('maxItems')
            if isinstance(cap, int) and 0 <= cap < len(fitted):
                return fitted[:cap]
            return fitted
        if kind == 'string':
    ...
    def _cb_schema_ready(value, schema):
        """The value fitted to the schema, or None when it cannot be fitted."""
        try:
            fitted = _cb_schema_fit(value, schema)
        except Exception:
            fitted = value
        return fitted if _cb_shape_ok(fitted, schema) else None
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            ...
            pass
        try:
            checked = await _temporal_recheck(question, answer, messages,
                                              ledger, deadline)
        ...
        if query.output_schema is not None:
            ...
            if structured is not None:
                ...
                try:
                    ...
                    pass
                structured = _cb_schema_ready(structured, query.output_schema)
                if structured is not None:
                    try:
            ...
            if basis is not answer:
                ...
                if salvaged is not None:
                    salvaged = _cb_schema_ready(salvaged, query.output_schema)
                    if salvaged is not None:
                        try:
            ...
            try:
                forced = _cb_schema_fit(_coerce_to_schema(_cap(basis), query.output_schema),
                                        query.output_schema)
...
def _bkkrlqsjrn():
    ...
    return query
_eiqrqbxmfd = _zbgvqvvvra()
_kzflbmfuhu = _fwshnhpqzl()
_kzrkbnsgmj = _ybgbsnxbem()
_utkkdbdshn = _bkkrlqsjrn()
_usssnptszp = 290.0
_hreirqpnvm = 250.0
_stogpqqofc = 90.0

async def _eeofichzbc(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _usssnptszp - (time.monotonic() - started)
        if first:
            budget = _hreirqpnvm if _hreirqpnvm < remaining else remaining
            first = False
        else:
            if remaining < _stogpqqofc:
                break
        ...
        try:
            ...
            last_exc = exc
    return _igtdvygwar(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _tloquzjywi['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _eeofichzbc(query, (_kzrkbnsgmj, _eiqrqbxmfd, _kzflbmfuhu, _utkkdbdshn))
        index = _xnyjchlvvt(query)
        if index == 0:
            agents = (_eiqrqbxmfd, _kzflbmfuhu, _kzrkbnsgmj, _utkkdbdshn)
        elif index == 1:
            agents = (_kzflbmfuhu, _kzrkbnsgmj, _utkkdbdshn, _eiqrqbxmfd)
        elif index == 2:
            agents = (_kzrkbnsgmj, _utkkdbdshn, _eiqrqbxmfd, _kzflbmfuhu)
        elif index == 3:
            agents = (_utkkdbdshn, _eiqrqbxmfd, _kzflbmfuhu, _kzrkbnsgmj)
        else:
            agents = (_eiqrqbxmfd, _kzflbmfuhu, _kzrkbnsgmj, _utkkdbdshn)
        return await _eeofichzbc(query, agents)
    except Exception:
        return _igtdvygwar(query)
_BUILDT_TAG_m1 = "sn45-6e34692786fb"
