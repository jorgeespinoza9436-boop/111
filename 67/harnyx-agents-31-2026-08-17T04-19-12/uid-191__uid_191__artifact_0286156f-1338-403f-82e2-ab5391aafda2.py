...
# uid 81 shipped it at 70s ahead of temporal repair at 75s, which silently threw
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # SIX sweeps share one tail — every stage in this build is post-answer, so
    # nothing runs before the loop to absorb the load. A firing sweep costs a
    # search plus up to three loop turns, so in practice only the first one or
    # two that trigger will run and the rest close on their own floors. Treat the
    # order as a PRIORITY RANKING, not a pipeline:
    #   subject — wrong entity makes every later check moot
    #   pool    — missing members change what the later checks describe
    #   period  — wrong period invalidates the figures below it
    #   source  — must precede corroboration (see its docstring: running it after
    #             would let a pulled-in .gov result mask an aggregator-anchored
    #             answer)
    ...
    for _sweep in (_verify_subjects, _widen_pool, _align_timeframe,
