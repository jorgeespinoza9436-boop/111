...
_GLEAN_AUDIT_PROMPT = (
    ...
    "Verdict shortfall requires concrete missing or wrong entries taken from "
    "the pools; answers that are not set-shaped always pass."
)
...
def _glean_set_shaped(question_text: str) -> bool:
    ...
    pattern = _glean_re.compile(
        ...
        r"\b(?:all|both|each|every)\s+(?:of\s+)?the\b", _glean_re.I)
...
async def _glean_fetch_sources(question_text: str, deadline: float):
    ...
    hints = [head]
    if len(years) >= 2:
        hints = [f"{head} {year}" for year in years]
    pages = []
    seen = set()
    for hint in hints[:4]:
...
async def _glean_pools(question_text: str, pages, deadline: float):
    ...
    for url, text in pages:
        for offset in range(0, len(text), _GLEAN_CHUNK):
            jobs.append((url, text[offset:offset + _GLEAN_CHUNK]))
...
async def query(query: Query) -> Response:
    ...
    if len(pools) < 2 or total_entries < 8:
        return response
    ...
    if len(corroborated_missing) < 2 and not refusal_or_empty:
        return response
    shortfall = {"missing": corroborated_missing[:40], "wrong": []}

    # Regeneration authority: only where it cannot cost the base its
    # citation alignment - fast-scored queries ignore citations entirely,
    # and a text answer that names a wrong or incomplete set is already
    # losing its set component.
