"""uid_90 research agent - variant 01.
...
validator_reference candidates:
  ...
  uid 212 - lead-figure corroboration (corroborate, gate)
...
# --- stage: lead-figure corroboration -------------------------------------
# The decisive figure is the one the answer is graded on. When the draft leads
# with a number and the whole answer rests on at most one pointer, that figure
# has no independent confirmation behind it, whatever the audit turn thought.

_CORROBORATE_HEAD_CHARS = 500
_CORROBORATE_MIN_DIGITS = 2


def _stage_lead_figure(blob: str) -> str:
    for match in _STAGE_FIGURE_RE.finditer((blob or "")[:_CORROBORATE_HEAD_CHARS]):
        raw = match.group(0).strip(" .,")
        if len(_stage_digits(raw)) >= _CORROBORATE_MIN_DIGITS:
            return raw
    return ""


def _stage_distinct_pointers(blob: str) -> int:
    return len({m.group(1) for m in _STAGE_POINTER_RE.finditer(blob or "")})


def _stage_corroborate(question: str, blob: str, citations: list) -> tuple:
    figure = _stage_lead_figure(blob)
    if not figure:
        return [], []
    if _stage_distinct_pointers(blob) >= 2:
        return [], []
    claims = [
        f"independent confirmation of the decisive figure {figure}, or an "
        "explicit statement that a single source reports it"
    ]
    head = _stage_head(question)
    queries = [
        f"{head} {figure} official source",
        f"{head} {figure} independent report",
    ]
    return claims, queries
...
async def query(query: Query) -> Response:
    ...
    try:
        ...
        # --- ADDED STAGE (gate): lead-figure corroboration ---
        corroborate_claims, corroborate_queries = _stage_corroborate(question, blob, citations)
        _stage_fold(ledger, corroborate_claims, corroborate_queries)
