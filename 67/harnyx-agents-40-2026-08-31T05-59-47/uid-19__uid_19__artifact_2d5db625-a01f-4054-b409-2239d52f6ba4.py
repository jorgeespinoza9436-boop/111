...
# The ledger judges the answer as one object, so a single unsupported sentence
# inside an otherwise sound answer survives it. This decomposes the delivered
# answer into atomic claims, keeps the ones asserting a figure, and hands the
# rewrite the specific sentences that state a number with nothing behind them.

_VERIFY_MIN_CLAIM_CHARS = 24
_VERIFY_MAX_CLAIMS = 6


def _stage_uncited_claims(blob: str) -> list:
    out: list = []
    for sentence in _STAGE_SENTENCE_RE.split(blob or ""):
        text = sentence.strip()
        if len(text) < _VERIFY_MIN_CLAIM_CHARS:
            continue
        if _STAGE_FIGURE_RE.search(text) is None:
            continue
        if _STAGE_POINTER_RE.search(text) is not None:
            continue
        out.append(text[:180])
        if len(out) >= _VERIFY_MAX_CLAIMS:
            break
    return out


async def _stage_verify_repair(question: str, response, citations: list, started: float):
    blob = _h666_draft_blob(response)
    if _STAGE_POINTER_RE.search(blob) is None:
        # nothing is pointered, so this is not a per-claim gap
        return None
    loose = _stage_uncited_claims(blob)
    if not loose:
        return None
    listed = "\n".join(f"- {claim}" for claim in loose)
    defect = (
        "These claims state a figure but carry no [[n]] pointer:\n" + listed +
        "\nFor each, attach the pointer of the source that supports it, correct "
        "the figure to what that source states, or remove the claim. Leave every "
        "already-pointered claim exactly as it stands."
    )
    return await _stage_repair_rewrite(question, response, citations, defect, started)
...
_STAGE_MIN_LEFT_S = 16.0
_STAGE_CHAT_TIMEOUT_S = 14.0
_STAGE_MAX_QUERIES = 3
_ROSTER_MIN_LEFT_S = 28.0
...
async def query(query: Query) -> Response:
    ...
    try:
        ...
        # --- ADDED STAGE (repair): claim-level verify-and-repair ---
        verify_fixed = await _stage_verify_repair(question, final, citations, started)
        if verify_fixed is not None:
            final = verify_fixed
