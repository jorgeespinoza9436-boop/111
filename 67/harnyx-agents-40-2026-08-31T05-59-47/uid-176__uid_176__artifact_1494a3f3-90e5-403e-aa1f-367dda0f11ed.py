...
_depzzexedq = {'started': None, 'text': None}
_yedhyacdgy = 24000
_avzbumbhuf = 290.0
_bqjazbhdlw = 250.0

def _fpgycfitea() -> float:
    started = _depzzexedq['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _cwfbydtate() -> float:
    return _avzbumbhuf - _fpgycfitea()
_phpflvgged = _hsapi.llm_chat
_pbmchpdbem = _hsapi.search_web
_yykkdoofje = _hsapi.fetch_page
_sxlvvcyhgs = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _bbiclslvgk(*args, **kwargs):
    if _fpgycfitea() >= _bqjazbhdlw:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _sxlvvcyhgs})
            kwargs['messages'] = steered
    _result = await _phpflvgged(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _fwqldqxied(_result)
    return _result

async def _bqjwzbzvmv(*args, **kwargs):
    if _fpgycfitea() >= _bqjazbhdlw:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _pbmchpdbem(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _scxvkkwrks(*args, **kwargs):
    if _fpgycfitea() >= _bqjazbhdlw:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _yykkdoofje(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _bbiclslvgk
_hsapi.search_web = _bqjwzbzvmv
_hsapi.fetch_page = _scxvkkwrks
_snuusjypbu = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_zeffttxcud = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_gcydjpyacq = 900
_xcbznfpigo = 2

def _ykawlpelku(query: Query) -> int:
...
def _rffndzzvpr(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _ktgccrmfpx(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _ykawlpelku(query)
    if fields >= 3:
        return 2
    if _rffndzzvpr(lowered, _snuusjypbu):
        return 1
    if fields <= _xcbznfpigo and len(text) <= _gcydjpyacq:
        return 0
    if _rffndzzvpr(lowered, _zeffttxcud):
        return 0
    return 1

def _fwqldqxied(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _depzzexedq['text'] = text.strip()[:_yedhyacdgy]
    except Exception:
        pass

def _dpungdvbko(text: str):
...
def _fmivwcprvn(query: Query) -> Response:
    text = _depzzexedq['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_yedhyacdgy]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _dpungdvbko(text)
...
def _gayctbgffj():
    ...
    def _compose_lumen_anvil_agent_entry():
        ...
        AUDIT_MIN_USD = 0.05
        WRAPUP_MIN_USD = 0.06
        _SPEND = _TaskLocalDict('harnyx_lumen_spend', lambda: {'left': None})
        ...
        async def _do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger) -> str:
            ...
            row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': citation_spans, 'title': url, 'url': url, 'preview': note[windows[0][0]:windows[0][0] + 1200], 'text': note}
            head = note[:FETCH_HEAD_CHARS]
            sections = ''.join((f'\n--- section @{s} ---\n{note[s:e]}' for s, e in windows))
            return ToolOutput(f"# read_page({url!r}) -> [{_SLOT.format(0)}] {len(note)} chars total; head + the {len(windows)} most relevant section(s) shown ({', '.join((f'{s}-{e}' for s, e in windows))}). If the answer set may continue elsewhere in this page, call page_grep on this URL with a row label/code or page_read with an offset; do not re-fetch it.\n--- head ---\n{head}{sections}", [row])
        ...
        async def _fetch_json(url: str, deadline: float):
            ...
            for _attempt in (0, 1):
                ...
                if left < 12.0:
                    return None
                try:
                    payload = await asyncio.wait_for(_inherit_task_locals(fetch_page(url, provider=SEARCH_PROVIDER, timeout=min(_SEC_FETCH_TIMEOUT_S, left - 6.0)), _task_key()), timeout=min(_SEC_FETCH_TIMEOUT_S, left - 6.0) + 4.0)
                except Exception:
                    continue
                _spend_note(payload)
                results = list(getattr(payload, 'results', None) or [])
        ...
        def _do_page_grep(url: str, pattern: str, ledger: EvidenceLedger) -> str:
            ...
            return f'# page_grep({pat!r}) on [{n}] -> {len(out)} match(es) of {len(text)} chars ({mode})' + ''.join(out)
        ...
        async def _chat_turn(messages: list[dict], deadline: float, *, finish_only: bool, force_tools: bool=False):
            ...
            for lane_model in ((LLM_LANE_A, LOOP_MODEL_A, True), (LLM_LANE_A, LOOP_MODEL_A, False), (LLM_LANE_A, AUDIT_MODEL, False)):
                lane = lane_model[0]
                model = lane_model[1]
                pinned = lane_model[2]
                ...
                if timeout <= 5.0:
                    return None
                try:
                    payload = await asyncio.wait_for(_inherit_task_locals(llm_chat(provider=lane, model=model, messages=messages, tools=LOOP_TOOLS if force_tools or not finish_only else None, tool_choice='auto' if force_tools or not finish_only else None, temperature=0.2, thinking={'enabled': False} if finish_only and lane == LLM_LANE_B else {'enabled': True, 'effort': 'low'}, max_output_tokens=6000 if finish_only and lane == LLM_LANE_B else None, provider_extra=_upstream(lane, model) if pinned else None, timeout=timeout), _task_key()), timeout=min(timeout + 6.0, max(1.0, deadline - monotonic() - 1.0)))
                    _spend_note(payload)
                    return payload
                except Exception:
                    continue
        ...
        async def _knowledge_brief(question: str) -> tuple[str, str]:
            ...
            try:
                ...
                try:
                    raw = await _chat_simple(LLM_LANE_A, AUDIT_MODEL, system, user, max_tokens=2400, timeout=BRIEF_TIMEOUT_S, think=_least_think(LLM_LANE_A, AUDIT_MODEL))
                except Exception:
                    raw = ''
            if not raw:
                return ('', '')
        ...
        async def _preseed(question: str, set_question: bool, ledger: EvidenceLedger, deadline: float) -> str:
            ...
            for seed in seeds:
                if deadline - monotonic() < 30.0:
                    break
                try:
                    out = await asyncio.wait_for(_inherit_task_locals(_do_search(seed, ledger), _task_key()), timeout=SEARCH_TIMEOUT_S * 2 + 6.0)
        ...
        async def _loop(question: str, brief: str, ledger: EvidenceLedger, deadline: float, turn_cap: int, carry: list[dict] | None=None, allow_tools_in_wrapup: bool=False) -> tuple[str, list[dict]]:
            ...
            for turn in range(1, turn_cap + 1):
                ...
                tool_tasks = [asyncio.ensure_future(_inherit_task_locals(_run_tool(c, question, ledger, deadline), parent_key)) for c in run_calls]
                try:
        ...
        _CITE_NUM_RE = re.compile('(?<![\\w\\[])\\[([0-9][0-9,\\s\\-]*)\\](?!\\])')
        ...
        async def _schema_output(question: str, answer: str, schema, deadline: float) -> object | None:
            ...
            for lane, model in ((LLM_LANE_A, SCHEMA_MODEL), (LLM_LANE_A, RESORT_MODEL), (LLM_LANE_A, LOOP_MODEL_A)):
                left = deadline - monotonic()
                if left < 12.0:
                    break
                try:
        ...
        async def _solve(query: Query, question: str) -> Response:
            ...
            deadline = task_deadline - SCHEMA_RESERVE_S if query.output_schema is not None else task_deadline
            try:
            ...
            if query.output_schema is not None:
                structured = None
                try:
                    structured = await _schema_output(question, answer, query.output_schema, task_deadline)
                except Exception:
                    structured = None
                if structured is not None:
                    try:
                        ...
                        pass
                    try:
                        return Response(output=structured, note=proof_note, citations=citations or None)
                    except Exception:
                        structured = None
                ...
                if basis is not answer:
                    try:
                        salvaged = await _schema_output(question, basis, query.output_schema, task_deadline)
                    except Exception:
                        salvaged = None
                    if salvaged is not None:
                        try:
                            return Response(output=salvaged, note=proof_note, citations=citations or None)
                        except Exception:
                            pass
                if basis is not answer:
                ...
                try:
                    ...
                    return Response(output=forced, note=proof_note, citations=citations or None)
                except Exception:
                    try:
                        return Response(output=_cap(basis)[:2000], note=proof_note, citations=citations or None)
                    except Exception:
                        pass
            try:
                ...
                return Response(text=text)
        _W2_CITE_POS = _TaskLocalDict('harnyx_lumen_citation_positions', dict)
        _W2_CITE_NUM_RE = re.compile('(?<![\\w\\[])\\[([0-9][0-9,\\s\\-]*)\\](?!\\])')
            ...
            def _point(match):
                ...
                for chunk in match.group(1).split(','):
                    ...
                    if piece.isdigit() and int(piece) in _W2_CITE_POS:
                        ...
                        continue
                    range_match = re.fullmatch('(\\d+)\\s*-\\s*(\\d+)', piece)
                    if range_match:
                        first, last = map(int, range_match.groups())
                        if first <= last and last - first <= 40:
                            out.extend(('[[%d]]' % _W2_CITE_POS[number] for number in range(first, last + 1) if number in _W2_CITE_POS))
    ...
    """
        ...
        async def _w4_repair_structured_output(question: str, schema: object, response: object, *, deadline: float) -> object:
            ...
            if recovered is None:
                ...
                recovered = _w4_json_value(await _w4_chat(messages, timeout=timeout, temperature=0.0))
    ...
    def _compose_cedar_quill_agent_entry():
        ...
        TASK_BUDGET_USD = 0.5
        BLIND_LIMIT = 3
        _SPEND = _TaskLocalDict('harnyx_cedar_spend', lambda: {'left': None, 'blind': 0})
        ...
        class EvidenceLedger:
            ...
            def refs_for(self, number: int, anchor_text: str='') -> list[CitationRef]:
                ...
                if spans:
                    ...
                    if retained:
                        shown = retained
                    elif anchor_text and row.get('text') and all((end <= len(row['text']) for _start, end in shown)):
                        source_lower = row['text'].casefold()
                        anchor_terms = {term for term in _key_terms(anchor_text) if term in source_lower}
                        if anchor_terms:
                            anchored = _best_windows(row['text'], anchor_terms, CITATION_MIN_SPAN_CHARS, k=2)

                            def coverage(windows: list[list[int]] | list[tuple[int, int]]) -> set[str]:
                                return {term for term in anchor_terms if any((term in row['text'][a:b].casefold() for a, b in windows))}
                            if anchored:
                    ...
                    if merged and note_len and room:
                        ...
                        for s, e in merged:
                            if grown and s <= grown[-1][1]:
                                ...
                                grown.append([s, e])
                        merged = grown
                    slices = [CitationSlice(start=s, end=e) for s, e in merged[:CITATION_REFS_PER_ROW] if e > s]
                    if not slices:
                        return []
                    return [CitationRef(receipt_id=row['receipt_id'], result_id=row['result_id'], slices=slices)]
                return []
        ...
        _FETCH_STATE = _TaskLocalDict('harnyx_cedar_fetch_state', lambda: {'spent_s': 0.0, 'dead': [], 'sdss_pages': {}, 'ready_answer': ''})
        ...
        def _degrade_query(q: str) -> str:
            out = _SITE_OP_RE.sub('', q or '').replace('"', ' ')
            return ' '.join(out.split())
        _SDSS_DATAMODEL_RE = re.compile('^https?://data\\.sdss\\.org/datamodel/files/.*/([^/?#]+)\\.html(?:[?#].*)?$', re.IGNORECASE)
        _SDSS_STATIC_RE = re.compile('^https?://raw\\.githubusercontent\\.com/sdss/datamodel/[^/]+/datamodel/products/md/([^/?#]+)\\.md(?:[?#].*)?$', re.IGNORECASE)

        def _sdss_product_name(url: str) -> str:
            for pattern in (_SDSS_DATAMODEL_RE, _SDSS_STATIC_RE):
                match = pattern.match((url or '').strip())
                if match:
                    return re.sub('_DR\\d+$', '', match.group(1), flags=re.IGNORECASE)
            return ''
        ...
        def _official_release_page(url: str, question: str) -> str:
            ...
            release = re.search('\\bDR\\d+\\b', question or '', re.IGNORECASE)
            if not match or not release:
                return ''
            raw_product = match.group(1)
            release_name = release.group(0).upper()
            product = re.sub('_DR\\d+$', '', raw_product, flags=re.IGNORECASE)
        ...
        def _official_static_fallback(url: str) -> str:
            ...
            if not product or _SDSS_DATAMODEL_RE.match((url or '').strip()) is None:
                return ''
            return f'https://raw.githubusercontent.com/sdss/datamodel/main/datamodel/products/md/{product}.md'
        ...
        def _sdss_binary_unit_rows(note: str) -> list[tuple[str, str]]:
            ...
            for line in body.splitlines():
                ...
                if stripped.startswith('ROW\t'):
                    ...
                    if len(cells) >= 5 and re.fullmatch('[A-Z][A-Z0-9_]*', cells[1] or ''):
                ...
                if len(cells) == 3 and re.fullmatch('[A-Z][A-Z0-9_]*', cells[0] or ''):
                    rows.append((cells[0], ''))
                    continue
                if len(cells) < 4:
                    continue
                name, _type_name, unit = cells[:3]
                if not re.fullmatch('[A-Z][A-Z0-9_]*', name or ''):
                    continue
        ...
        def _sdss_row_span(note: str, names: list[str]) -> tuple[int, int]:
            positions = []
            for name in names:
                match = re.search('(?m)^\\s*\\|\\s*' + re.escape(name) + '\\s*\\|', note or '')
                if match:
        ...
        def _sdss_table_spans(note: str, names: list[str]) -> list[tuple[int, int]]:
            ...
            starts = [position for position in ((note or '').find('HEADER\tName\tType\tUnit\tDescription'), (note or '').find('Binary Table Caption')) if position >= 0]
        ...
        def _sdss_unit_comparison(question: str) -> ToolOutput | None:
            ...
            q = (question or '').lower()
            if 'unit' not in q or not re.search('\\b(compare|comparison|difference|differs?)\\b', q):
                return None
            ...
            lines = ['# Exhaustive official SDSS binary-table Unit comparison', f'Compared every documented row of {left_name} with {right_name}, in {left_name} order.', 'Rows where exactly one Unit cell is blank:']
            for name, left_unit, right_unit in differences:
                lines.append(f"- {name}: {left_name} Unit = {left_unit or 'BLANK'}; {right_name} Unit = {right_unit or 'BLANK'}")
            lines.append(f'Exact source rows: [{_SLOT.format(0)}] [{_SLOT.format(1)}].')
            citation_rows = []
            for page in (left, right):
                citation_rows.append({'receipt_id': page['receipt_id'], 'result_id': page['result_id'], 'note_len': len(page['note']), 'kind': 'fetch', 'spans': _sdss_table_spans(page['note'], names), 'title': page['url'], 'url': page['url'], 'preview': page['note'][:1200], 'text': page['note']})
            result = ToolOutput('\n'.join(lines), citation_rows)
            prose_rows = []
            pointers = f'[{_SLOT.format(0)}][{_SLOT.format(1)}]'
            for name, left_unit, right_unit in differences:
                prose_rows.append(f'{name}: {left_name} supplies `{left_unit}`' if left_unit else f'{name}: {left_name} leaves Unit blank')
                prose_rows[-1] += f', while {right_name} supplies `{right_unit}`' if right_unit else f', while {right_name} leaves Unit blank'
            result.ready_answer = 'In table order, the columns whose Unit is physical on exactly one page are ' + '; '.join(prose_rows) + f'. Every remaining Unit entry agrees between the two complete ' + f'release-specific tables {pointers}.'
            return result
        ...
        async def _do_fetch(url: str, focus: str, question: str, ledger: EvidenceLedger) -> str:
            ...
            payload = None
            resolved_url = url
            preferred_static_url = _official_release_page(url, question) or _official_static_fallback(url)
            if preferred_static_url:
                started = monotonic()
                try:
                    static_payload = await fetch_page(preferred_static_url, provider=SEARCH_PROVIDER, timeout=FETCH_TIMEOUT_S)
                except Exception:
                    _spend_blind()
                    static_payload = None
                _FETCH_STATE['spent_s'] = _FETCH_STATE['spent_s'] + monotonic() - started
            ...
            if payload is None or not getattr(payload, 'results', None):
                ...
                if fallback_url:
                    started = monotonic()
                    try:
                        fallback_payload = await fetch_page(fallback_url, provider=SEARCH_PROVIDER, timeout=FETCH_TIMEOUT_S)
                    except Exception:
                        _spend_blind()
            ...
            if product:
                _FETCH_STATE['sdss_pages'][product] = {'receipt_id': receipt, 'result_id': rid, 'note': note, 'url': resolved_url}
                comparison = _sdss_unit_comparison(question)
                if comparison is not None:
                    comparison.memo_key = plain_key
                    return comparison
            show_full = len(note) <= FETCH_PLAIN_CHARS or (resolved_url != url and len(note) <= 20000)
            if show_full:
                row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': [(0, len(note))], 'title': resolved_url, 'url': resolved_url, 'preview': note[:1200], 'text': note}
                fallback_note = ' via official static source' if resolved_url != url else ''
                return ToolOutput(f'# read_page({url!r}){fallback_note} -> [{_SLOT.format(0)}] full page, {len(note)} chars\n{_lossless_view(note)}', [row], memo_key=plain_key)
            terms = _key_terms(question) | _key_terms(focus)
            windows = _best_windows(note, terms, FETCH_WINDOW_CHARS, k=FETCH_WINDOWS_PER_PAGE)
            row = {'receipt_id': receipt, 'result_id': rid, 'note_len': len(note), 'kind': 'fetch', 'spans': [(0, FETCH_HEAD_CHARS)] + list(windows), 'title': resolved_url, 'url': resolved_url, 'preview': note[windows[0][0]:windows[0][0] + 1200], 'text': note}
        ...
        _FAST_UPSTREAMS = ('Decart', 'CoreWeave', 'Alibaba')
        _FAST_UPSTREAMS_OSS = ('Cerebras', 'Groq', 'BaseTen')
        _RUN_UPSTREAM = _TaskLocalDict('harnyx_cedar_upstream_state', lambda: {'glm': None, 'oss': None, 'dead': set()})
        ...
        _BRIEF_STORE = _TaskLocalDict('harnyx_cedar_brief_store', lambda: {'raw': '', 'plan': ''})
        ...
        async def _preseed(question: str, set_question: bool, ledger: EvidenceLedger, deadline: float) -> str:
            ...
            parent_key = _task_key()
            seed_tasks = [asyncio.ensure_future(_inherit_task_locals(_do_search(seed, ledger), parent_key)) for seed in seeds]
            try:
        ...
        def _citation_anchor_text(answer: str, number: int, top: int) -> str:
            ...
            for match in matches:
                gap = normalized[groups[-1][-1].end():match.start()] if groups else ''
                if groups and re.fullmatch('[\\s,;\\[\\]]*', gap):
                    groups[-1].append(match)
                else:
                    groups.append([match])
            first_prefix_line = normalized[:groups[0][0].start()].rsplit('\n', 1)[-1]
            leading_style = not re.search('[A-Za-z0-9]', first_prefix_line)
            contexts: list[str] = []
            previous_end = 0
            for index, group in enumerate(groups):
                ...
                if number in numbers:
                    ...
                    right = groups[index + 1][0].start() if index + 1 < len(groups) else min(len(normalized), last.end() + 600)
                    after = normalized[last.end():right]
                    after = after.lstrip(' \t\r\n-*#>')
                    right_claim = after.split('\n', 1)[0][:600]
                    marker_leads_clause = leading_style or not re.search('[A-Za-z0-9]', before.rsplit('\n', 1)[-1])
                    if marker_leads_clause and re.search('[A-Za-z0-9]', right_claim):
                        claim = right_claim
                    elif not re.search('[A-Za-z0-9]', claim):
                        claim = right_claim
                    if claim.strip():
        ...
        def _repoint(answer: str, slot_pos: dict[int, int]) -> str:
            ...
            def sub(m: 're.Match[str]') -> str:
                whole = m.group(0)
                if m.start() > 0 and (answer[m.start() - 1].isalnum() or answer[m.start() - 1] == '_'):
                    return whole
                e = m.end()
        ...
        async def _solve(query: Query, question: str) -> Response:
            ...
            if _is_usable_answer(ready_answer):
                answer = ready_answer
            try:
                if not ready_answer and _is_usable_answer(answer) and (deadline - monotonic() > 75.0) and (_spend_left() >= AUDIT_MIN_USD):
                    patched = await _audit_patch(question, answer, messages, ledger, deadline)
            ...
            answer = _normalize_brackets(answer)
            answer = _strip_lead_narration(answer)
            proof_note = _cap(_repoint(answer, _slot_pos)) if citations and '[[' in _repoint(answer, _slot_pos) else None
            if query.output_schema is None:
                answer = _answer_line_only(answer, question)
    ...
    def _balanced_route_label(query: Query) -> str:
        ...
        text = (getattr(query, 'text', '') or '').strip()
        digest = _rt_h.blake2b((_BALANCED_ROUTER_SEED + '|' + text).encode('utf-8', 'ignore'), digest_size=8).digest()
    ...
    _ROUTE_TARGETS = {'LumenAnvilAgent': _BRANCH_0, 'CedarQuillAgent': _BRANCH_1, 'JuniperCompassAgent': _BRANCH_2}
...
def _syeesontio():
    ...
    def _compose_orchid_harbor_agent_entry():
...
"""
    ...
    """
        ...
        async def _citation_backfill(question: str, answer: str, index: _ResultIndex, terms: list[str], deadline: float) -> str:
    ...
    """
        ...
        return query
    _orchid_harbor_agent_query_entry = _compose_orchid_harbor_agent_entry()

    def _compose_cobalt_kestrel_agent_entry():
...
"""
    ...
    """
            ...
            try:
                ...
                return _so_response(_so_skeleton(schema, schema), None)
        return query
    _cobalt_kestrel_agent_query_entry = _compose_cobalt_kestrel_agent_entry()

    def _compose_lumen_mistral_agent_entry():
    ...
    """
        ...
        return query
    _lumen_mistral_agent_query_entry = _compose_lumen_mistral_agent_entry()
    ...
    class OrchidHarborAgent:

        async def __call__(self, query: Query) -> Response:
            return await _orchid_harbor_agent_query_entry(query)

    class CobaltKestrelAgent:

        async def __call__(self, query: Query) -> Response:
            return await _cobalt_kestrel_agent_query_entry(query)

    class LumenMistralAgent:

        async def __call__(self, query: Query) -> Response:
            return await _lumen_mistral_agent_query_entry(query)
    _TASK_SHAPE_PRIMARY_AGENT = OrchidHarborAgent()
    _TASK_SHAPE_SECONDARY_AGENT = CobaltKestrelAgent()
    _TASK_SHAPE_TERTIARY_AGENT = LumenMistralAgent()
    _CANDIDATE_BRANCH_CLASS_NAMES = ('OrchidHarborAgent', 'CobaltKestrelAgent', 'LumenMistralAgent')
    ...
    return query
_gpsjfkxzon = _gayctbgffj()
_ugtjgngxje = _syeesontio()
_avzbumbhuf = 290.0
_renlraosad = 250.0
_rgnevwfkgj = 90.0

async def _lqgixflivp(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _avzbumbhuf - (time.monotonic() - started)
        if first:
            budget = _renlraosad if _renlraosad < remaining else remaining
            first = False
        else:
            if remaining < _rgnevwfkgj:
                break
        ...
        try:
            ...
            last_exc = exc
    return _fmivwcprvn(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _depzzexedq['started'] = time.monotonic()
    try:
        index = _ktgccrmfpx(query)
        if index == 0:
            agents = (_gpsjfkxzon, _ugtjgngxje)
        elif index == 1:
            agents = (_ugtjgngxje, _gpsjfkxzon)
        else:
            agents = (_gpsjfkxzon, _ugtjgngxje)
        return await _lqgixflivp(query, agents)
    except Exception:
        return _fmivwcprvn(query)