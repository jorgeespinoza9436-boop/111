...
# ============================================================================
# embedded agent: n10 (agents/n1.0.py)
# ============================================================================
def _r_build_n10():
    ...
    def _n1_blob_len(response) -> int:
        """Delivered size of a response, prose or structured, on one scale."""
        try:
            return len(_h666_draft_blob(response))
        except Exception:
            return 0


    def _n1_keeps_substance(new_response, draft) -> bool:
        """Accept the rewrite only when it does not shrink the delivered answer.

        Paired within task, the longest of five runs beat the shortest 20-10 (mean
        +0.1471, p=0.049). A rewrite that comes back materially shorter than the draft
        it replaces is that losing draw, chosen deliberately.
        """
        new_len = _n1_blob_len(new_response)
        if new_len <= 0:
            return False
        draft_len = _n1_blob_len(draft)
        if draft_len <= 0:
            return True
        return new_len >= int(draft_len * 0.85)


    def _n1_draft_is_settled(blob: str, citations: list, schema) -> bool:
        """True when the audit has nothing left to buy.

        The ledger post-pass is what separates this lineage from uid223 (+0.0667 score)
        and also what makes it 36% more llm_chat calls per run and lose both efficiency
        doors. Spending it on a draft that is already long and source-backed is the part
        with no hypothesis behind it. The 1900-char line is the top length quartile
        boundary (1941) rounded down; structured payloads are exempt because a schema
        answer is short by contract.
        """
        if schema is not None:
            return False
        try:
            return len(blob) >= 1900 and len(citations) >= 2
        except Exception:
            return False
    ...
    async def query(query: Query) -> Response:
        ...
        try:
            ...
            if _n1_draft_is_settled(blob, citations, schema):
                return _h666_pointer_only(draft)
            ...
            if ledger.requires_fresh_retrieval_and_rewrite():
                ...
                if rows:
                    ...
                    if rewritten is not None and _n1_keeps_substance(rewritten, draft):
                        return rewritten
            return _h666_pointer_only(draft)
        except Exception:
            return draft
...
# ============================================================================
# embedded agent: n20 (agents/n2.0.py)
# ============================================================================
def _r_build_n20():
    ...
    EVIDENCE_CHAR_BUDGET = 84_000


    BRIEF_MIN_USD = 0.03
    AUDIT_MIN_USD = 0.05
    AUDIT_EVIDENCE_CHARS = 7200                                                    
    WRAPUP_MIN_USD = 0.02
...
_R_MIN_TEXT_CHARS = 40


def _r_wants_structured(query: Query) -> bool:
    try:
...
def _r_delivered(response, structured: bool) -> bool:
    """Did this lane actually return an answer the judge can score?

    Not a quality test -- a floor test. uid144's 50 empty runs in 1c1bdd63 are the
    exact shape being caught: the lane returns, but with nothing in it.
    """
    if response is None:
        return False
    try:
        if structured:
            if getattr(response, "output", None) is not None:
                return True
            text = getattr(response, "text", None)
            return isinstance(text, str) and len(text.strip()) >= _R_MIN_TEXT_CHARS
        text = getattr(response, "text", None)
        if isinstance(text, str) and len(text.strip()) >= _R_MIN_TEXT_CHARS:
            return True
...
@entrypoint("query")
async def query(query: Query) -> Response:
    """Lane A every time; lane B only when lane A delivers nothing.

    A lane raising is charged as MINER_UNHANDLED_EXCEPTION -- score 0, no retry -- so
    neither lane is allowed to escape.
    """
    structured = _r_wants_structured(query)
    primary = None
    try:
        primary = await _R_N10(query)
    except Exception:
        primary = None
    if _r_delivered(primary, structured):
        return primary
    try:
        secondary = await _R_N20(query)
    except Exception:
        secondary = None
    if _r_delivered(secondary, structured):
        return secondary
