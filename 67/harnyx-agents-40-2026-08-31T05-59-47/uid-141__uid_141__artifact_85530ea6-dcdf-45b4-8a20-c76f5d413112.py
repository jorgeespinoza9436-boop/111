...
def _compose_willow_prism_agent_entry():
...
_willow_prism_agent_query_entry = _compose_willow_prism_agent_entry()


def _compose_quartz_vector_agent_entry():
...
_quartz_vector_agent_query_entry = _compose_quartz_vector_agent_entry()


_SHAPE_ROUTER_SEED = "6b315b8fd4f140d8c6ce2ad3"
...
def _balanced_route_label(query: Query) -> str:
    ...
    order = ("AmberBeaconAgent", "WillowPrismAgent", "QuartzVectorAgent")
    if shape == 3:
        return order[bucket]
...
class WillowPrismAgent:
    async def __call__(self, query: Query) -> Response:
        return await _willow_prism_agent_query_entry(query)


class QuartzVectorAgent:
    async def __call__(self, query: Query) -> Response:
        return await _quartz_vector_agent_query_entry(query)


_SHAPE_PRIMARY_AGENT = AmberBeaconAgent()
_SHAPE_SECONDARY_AGENT = WillowPrismAgent()
_SHAPE_TERTIARY_AGENT = QuartzVectorAgent()
