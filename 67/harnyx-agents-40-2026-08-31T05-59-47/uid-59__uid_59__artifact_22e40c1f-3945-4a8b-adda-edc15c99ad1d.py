...
def _compose_raven_vector_agent_entry():
...
_raven_vector_agent_query_entry = _compose_raven_vector_agent_entry()


_SHAPE_ROUTER_SEED = "c0be51a3c76571dadd579dc1"
...
def _balanced_route_label(query: Query) -> str:
    ...
    order = ("EmberQuillAgent", "EmberRelayAgent", "RavenVectorAgent")
    if shape == 3:
        return order[bucket]
...
class RavenVectorAgent:
    async def __call__(self, query: Query) -> Response:
        return await _raven_vector_agent_query_entry(query)


_SHAPE_PRIMARY_AGENT = EmberQuillAgent()
_SHAPE_SECONDARY_AGENT = EmberRelayAgent()
_SHAPE_TERTIARY_AGENT = RavenVectorAgent()
