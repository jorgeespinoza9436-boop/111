...
OPENROUTER_GEMMA_MAX_OUTPUT_TOKENS = 131_072
...
STATE_AWARE_INVESTIGATION_MODELS = ("ai_gateway_gemma", "glm5", "inkling")
REQUIREMENTS_MODELS = ("ai_gateway_gemma", "glm5")
REPAIR_MODELS = ("inkling", "ai_gateway_gemma", "glm5")
AUDIT_MODELS = ("inkling", "ai_gateway_gemma", "glm5")
...
needed to construct the complete candidate sets. Do not create a separate question asking whether a source or set is
complete; the final audit judges whether the observed source scope is sufficient.
...
Search snippets are evidence when their visible text directly supports the premise. Do not fetch a page when its
search snippet already supplies the needed evidence. Use fetch_page only when the snippet lacks necessary context or
when inspecting a discovered page is the most direct remaining evidence route. fetch_page accepts a full URL,
including one discovered inside a search result or another page. Do not construct a URL from a guessed site pattern.
...
visibly describes a different period.
Before finalizing, check each evidence question against words or table cells actually visible in the retained source
text. Your memory of a source is not visible evidence. If a required row or relationship is absent from the excerpt,
locate it with VFS search or fetch the discovered page; if it remains unavailable, state the limitation instead of
silently supplying it.
...
the cited source records. Search-result evidence does not require retention.

Tool failures are observations: correct the call or change approach. Tool calls in one response execute sequentially,
so a later call must not depend on a result not yet seen. Emit each distinct operation at most once per response."""
...
duplicates or merely corroborating background. If the question asks what a source explicitly reports, state that
...
question, including descriptors and relationships.
...
A candidate excluded by one supported condition does not need evidence for the other conditions.
...
If an evidence requirement asks for every candidate's raw values even though the original question does not ask the
answer to output that inventory, treat it as a requirement to establish that the final qualifying set is complete.
A complete comparison on one condition may reduce the candidate set, after which only the survivors need support for
the remaining conditions.

Before deciding, write one short line for every supplied evidence requirement and then for any additional factual
statement in the current answer. Use exactly one of:
...
Do not combine requirements on one line. A source ref without the establishing words is not support. Use only the
...
unknown could change the result. READY and REVISE are invalid if an evidence requirement is MISSING. A source
...
class ResearchState:
    ...
    def remember_focused_lines(self, key: str, indices: set[int] | range) -> None:
        line_count = len(self.vfs[key].splitlines() or [""])
        valid_indices = {index for index in indices if 0 <= index < line_count}
        if valid_indices:
            self.focused_lines.setdefault(key, set()).update(valid_indices)
...
def _render_public_citations(
    ...
    if plan.citations and not marker_indices:
        raise ValueError("answer has response citations but no inline citation markers")

    used_indices = sorted(set(marker_indices))
    compact_indices = {
        old_index: new_index
...
def _refresh_retrieval_receipt_message(
    ...
    if state.retrieval_receipts:
        sections.append(
            "Completed external retrieval receipts. These record actions and VFS "
            "destinations, not evidence:\n"
            + json.dumps(
                list(state.retrieval_receipts.values()),
                ensure_ascii=False,
                indent=2,
            )
        )
    ...
    if state.focused_lines:
        sections.append(
            "Focused VFS observations returned by prior regex or similarity searches. "
            "These remain visible so separate retrieval steps can be combined. Their "
            "presence does not establish relevance; retain the lines that support or "
            "contradict a material premise:\n"
            + json.dumps(
...
async def _final_answer_chat_with_retry(
    ...
    return await _chat_with_scheduling(
        ("ai_gateway_gemma", "openrouter_gemma_prose", "glm5"),
        messages,
        None,
        "none",
        False,
...
async def _answer_text(
    *,
    ...
    for attempt in range(3):
        ...
        text = text.strip()
        try:
            _validate_private_answer_refs(text, allowed_refs)
    ...
    for attempt in range(3):
        ...
        try:
            ...
            if verdict in {"READY", "REVISE"} and re.search(r"(?m)^MISSING:", text):
                raise ValueError(
                    f"VERDICT {verdict} is invalid while an evidence requirement is MISSING; "
                    "use CONTINUE and name the most important missing observation"
                )
            if verdict == "REVISE":
                _validate_private_answer_refs(payload, allowed_refs)
        except ValueError as error:
            if attempt == 2:
                raise
            messages.extend(
...
def _execute_read(state: ResearchState, args: dict[str, Any]) -> dict[str, Any]:
    key = str(args["key"])
...
def _execute_retain_evidence(
    ...
    retained = packet[0]
    retained["research_note"] = str(args["note"]).strip()
    ...
    if existing is not None:
        ...
        retained["research_note"] = "\n".join(note for note in (previous_note, str(args["note"]).strip()) if note)
    ...
    remaining_focused = state.focused_lines.get(source.key)
    if remaining_focused is not None:
        remaining_focused.difference_update(retained_indices)
        if not remaining_focused:
            state.focused_lines.pop(source.key, None)
...
async def _finalize_answer(
    *,
    ...
    finalization_context = "\n\n".join(
        value
        for value in (state.research_state.strip(), reason.strip())
        if value
    )
    packet = state.source_packet(
        finalization_context,
    ...
    for _turn in range(160):
        ...
        if not calls:
            ...
            if prose:
                ...
                return _render_public_citations(current_answer, plan)
            messages.extend(
                [
                    assistant.to_input_message(),
                    {
        ...
        if ready_requested:
            final_audit = await _audit(
                question,
                state.evidence_requirements or "",
                current_answer,
                last_packet,
            )
            ...
            if verdict == "CONTINUE":
                ...
                _validate_private_answer_refs(audit_payload, allowed_refs)
        ...
        if MODEL_SCHEDULING == "state_aware" and not ready_requested:
            ...
            next_switch_reason = ""
            if current_failures and current_failures == previous_failure_signatures:
                next_switch_reason = (
                    "The previous model repeated the same tool failure after receiving detailed feedback. "
                    "Choose a different valid operation that advances the investigation."
                )
            elif (
                current_calls
            ...
            previous_failure_signatures = current_failures
...
async def query(query: Query) -> Response:
    # S9 claim-sheet seed (retrieval planning change)
    try:
        try:
        ...
        _S9_CLAIM_STATE["queries"] = tuple(await _s9_decompose_claims(query.text, deadline=_s9_dl))
    except Exception:
        _S9_CLAIM_STATE["queries"] = ()
    try:
    ...
    # S9 contradiction/coverage gate
    try:
        try:
            _store = index
        except NameError:
        ...
        try:
            ...
            _dl = perf_counter() + 60.0
        if answer and _store is not None and (_dl - perf_counter()) > 40:
            answer = await _s9_contradiction_coverage_gate(
                query.text, answer, [], _store, deadline=_dl,
            )
    except Exception:
        pass
