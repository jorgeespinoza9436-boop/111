"""agent_d — v39 kingplus fleet-00 (t15 w90 e550) (king uid186 base + audit-loop + lane-B re-home + forensic mechanisms).
...
async def _solve(query: Query, question: str) -> Response:
    ...
    # v39c: M1's runtime span-moving is DISABLED on A/B evidence — two full-batch
    # runs showed its target tasks unmoved (8d3d9f9f, a894019e stayed 0) while the
    # citation-rich table tasks it touched regressed reproducibly (77898d52,
    # bbaac121, 3d715137): the extra anchored windows inflate per-ref materialized
    # cost, later refs get dropped under the 105k budget, and dangling [n]s are
    # judge-penalized. The Supports:-line prompt rider (the other half of M1) stays.
    try:
    ...
    if query.output_schema is not None:
        ...
        if basis and (deadline - perf_counter()) > S9_GATE_MIN_SECONDS:
            try:
                _s9_store = index
            except NameError:
                try:
            ...
            if _s9_store is not None:
                try:
                    basis = await _s9_contradiction_coverage_gate(
                        query.text,
                        basis,
                        messages,
                        _s9_store,
