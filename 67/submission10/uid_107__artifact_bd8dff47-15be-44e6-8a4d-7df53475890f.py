...
# build e0847a18f0563b41
_AGENT_VARIANT = "7422b034737a882d"
...
_UNFINISHED_RE = re.compile(
    r"^\s*(let me|i will|i'll|first,? i|now i|next,? i|searching|looking up)\b", re.I)
_CITED_RE = re.compile(r"\[[0-9]")


def _looks_unfinished(answer: str) -> bool:
    """Scratch text ('Let me fetch...') submitted as the answer scores 0. A [n]
    bracket means a real sourced answer was committed — never discard that."""
    a = (answer or "").strip()
    if not a:
        return True
    if _CITED_RE.search(a):
        return False
    if len(a) < 40:
        return True
...
async def _gap_requery_empty_searches(
    tool_calls: list, tool_results: list[str], index: _ResultIndex, deadline: float
) -> list[str]:
    """R1: After round 1, re-query searches that returned zero results."""
    new_results: list[str] = []
    try:
        for tc, result_text in zip(tool_calls or [], tool_results or []):
            if "-> 0 results" not in result_text:
                continue
            ...
            if name == "search_web":
                ...
                if q:
                    variation = q.replace(" ", " OR ") if " " in q else f'"{q}"'
                    new_results.append(await _run_search_web(variation, index, deadline=deadline))
            elif name == "search_many":
                for q in args.get("queries") or []:
                    if str(q).strip():
                        new_results.append(await _run_search_web(str(q).strip(), index, deadline=deadline))
    except Exception:
        pass
    return new_results
    ...
    try:
        ...
        for _turn in range(1, MAX_TURNS + 1):
            ...
            try:
                ...
                if _turn == 1 and (deadline - perf_counter()) > 30:
                    gap_results = await _gap_requery_empty_searches(tool_calls, tool_results, index, deadline)
                    for gr in gap_results:
                        messages.append({"role": "system", "content": f"## Gap Re-query Result\n{gr}"})
            except Exception:
                break
        ...
        if not _is_usable_answer(final_answer or "") or _looks_unfinished(final_answer or ""):
...
_MARKER_VECTOR_20104 = "0009f62afa4d"


def _normalize_vector_20104(items=(), *, base=91009):
    total = base
