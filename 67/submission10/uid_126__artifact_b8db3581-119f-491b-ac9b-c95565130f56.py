...
_AGENT_VARIANT = "0d3015f5eb95f66f"
...
# R4: dual-track broad + site-constrained search
_SITE_CONSTRAINT_DOMAINS = (
    "wikipedia.org", ".gov", ".edu", "imf.org", "worldbank.org",
    "un.org", "who.int", "oecd.org", "forbes.com", "imdb.com",
)
...
async def _tool_search(q: str, index: _ResultIndex) -> str:
    """R4: broad + site-constrained twin, merged deduped by URL."""
    if not q.strip():
        return "# search_web -> empty query"
    
    # Build site-constrained twin query
    site_clause = " OR ".join(f"site:{d}" for d in _SITE_CONSTRAINT_DOMAINS[:5])
    twin_q = f"({q}) AND ({site_clause})"
    
    async def _single_search(query: str):
        for provider in ("desearch", "parallel"):
            try:
                resp = await search_web(query, provider=provider, num=8, timeout=SEARCH_TIMEOUT)
    ...
    # Issue both concurrently
    broad_resp, twin_resp = None, None
    try:
        broad_resp, twin_resp = await asyncio.gather(
            _single_search(q), _single_search(twin_q), return_exceptions=True
        )
    except Exception:
        pass
    
    # Degrade: twin failure -> broad only
    if isinstance(twin_resp, Exception):
        twin_resp = None
    if isinstance(broad_resp, Exception):
        broad_resp = None
    
    # Merge and dedup by normalized URL
    all_results: list = []
    seen_urls: set[str] = set()
    
    for resp in (broad_resp, twin_resp):
        if resp is None:
            continue
    ...
    # Use broad receipt
    receipt = getattr(broad_resp, "receipt_id", "") or "" if broad_resp else ""
    if broad_resp:
        _note_budget(broad_resp)
    
    lines = [f"# search_web({q!r}) -> {len(all_results)} results"]
